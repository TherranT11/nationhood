...
/**
 * Refresh the topbar cash pill from the database and sync the session cache.
 * Call this after any action that changes funds to keep the UI accurate.
 */
export async function refreshAP(factionId) {
    ...
    try {
        const { data, error } = await _supabase
            .from('factions')
            .select('party_funds, corp_cash_reserves, faction_type')
            .eq('id', factionId)
            .single();
