...
export async function bootstrapPolitician(activeTab) {
  ...
  const [facRes, shardRes] = await Promise.all([
    _supabase.from('factions')
      .select('id, faction_type, faction_name, nation_id, branch, leader_first_name, leader_last_name, nickname, founded_tick, party_funds, abandoned_at, is_banned, politician_influence, politician_reputation, politician_skill, political_capital, politician_suspicion, volunteers, politician_party_id, politician_office, politician_office_won_at_tick, politician_ministry, civil_service_exam_cooldown_until_tick, speech_cooldown_until_tick, door_knock_cooldown_until_tick, next_member_action_tick, next_party_motion_tick, next_mp_action_tick, next_local_action_tick, next_chair_bid_tick, bar_admitted_nation_id, bar_admitted_at_tick, bar_last_attempt_tick, politician_experienced_advocate_at_tick, politician_magistrate_at_tick, politician_state_prosecutor_at_tick, try_case_cooldown_until_tick, party_cooldown_until_tick')
