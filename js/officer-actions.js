...
const OA = {
    forward_stockpiling:     { name: 'Forward Stockpiling',     up: 'Supply',          down: 'Logistics',       cost: 8,  blurb: 'Cram the forward depots to bursting — the overloaded transport net slows even as stocks rise.' },
    logistics_overhaul:      { name: 'Logistics Overhaul',      up: 'Logistics',       down: 'Supply',          cost: 12, blurb: 'Re-rationalise convoys and motor pools for throughput — existing stockpiles are consumed in the reshuffle.' },
    intensive_drills:        { name: 'Intensive Drills',        up: 'Training',        down: 'Cohesion',        cost: 10, blurb: 'Grueling drill cycles sharpen combat skill — exhausted, resentful troops fray.' },
    internal_security_sweep: { name: 'Internal Security Sweep', up: 'Cohesion',        down: 'Professionalism', cost: 12, blurb: 'Root out defeatists and informants — ranks close up, but the climate of surveillance corrodes professional trust.' },
    doctrine_reform:         { name: 'Doctrine Reform',         up: 'Professionalism', down: 'Training',        cost: 15, blurb: 'Rewrite the field manual and enforce standards — conduct focus pulls troops off the live-fire range.' },
};
...
export function openOfficerAction(faction, key, onDone) {
    ...
    function render() {
        ...
        overlay.innerHTML = `<div class="oa-modal">
            ...
            <div class="oa-body">
                ...
                <div class="oa-cost">Cost <b>${money(costRaw)}</b> &middot; Army Funds <b class="${enough ? 'ok' : 'no'}">${money(funds)}</b></div>
            ...
            <div class="oa-foot">
                ...
                ${done ? '' : `<div class="oa-btn pri ${(!enough || busy) ? 'off' : ''}" data-oa="go">${busy ? 'Working…' : 'Execute — ' + money(costRaw)}</div>`}
            </div>
        </div>`;
    }
    ...
    overlay.onclick = async (e) => {
        ...
        if (v === 'go') {
            if (busy || done || funds < costRaw) return;
