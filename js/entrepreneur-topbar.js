...
export async function bootstrapEntrepreneur(activeTab) {
  ...
  const [facRes, shardRes, allFacRes] = await Promise.all([
    _supabase.from('factions')
      .select('faction_name, leader_first_name, leader_last_name, leader_age, nation, entrepreneur_archetype, ent_ambition, ent_cunning, ent_reputation, ent_vision, party_funds')
