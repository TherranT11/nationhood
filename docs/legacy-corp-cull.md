...
- [~] Phase 4 — Tick / edge decoupling (IN PROGRESS, done in verified sub-steps; `advance-corp-tick` is hand-written, verify with `node --check`; **deploy the edge before applying the function-drop migrations**).
  ...
  - [~] 4d legacy corp economy (IN PROGRESS). **Done:** removed `processPropertyEffects`, `processRegionalHqIncome`, `processCorpMonthlyIncome` (its `corp_loans` ref was a comment only — no entrepreneur write), the reputation-decay block, and the `assess_corporate_taxes` per-tick call + dropped that fn (`20270244`). **Remaining (all verified legacy):** the construction cluster (`processCorpContracts` + permit lifecycle + `apply_construction_wages_for_nation` + GDP boost — `apply_construction_wages_for_nation` filters faction_type='corporation'), `processVesselOrderDeliveries`/`processEquipmentDeliveries` (vessel_orders/corp_equipment), ship-market generation, executive-contract-expiry. KEEP `processCentralBankLoanPayments`.
