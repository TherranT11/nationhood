...
- **Pre-existing orphans — corp ones DELETED (5-closeout, 2026-05-24):** `js/game/materials.js` (266 lines, construction-materials defs) + `js/game/vessels.js` (21 lines, ship specs) — both legacy construction/shipping subsystems removed in 5B, verified zero referrers (filename, module path, *and* exported symbols `MATERIAL_QUALITY_COLORS`/`VESSEL_SPECS`/`VESSEL_CLASSES`). The edge runtime keeps its own inline vessel copy (unaffected). NOT deleted: `js/guide.js`, `js/politics.js` (non-corp, out of scope; `politics.js` is live via `electorate.js`).
...
- [~] Phase 5 — Schema teardown (staged, confirm each destructive step). **KEEP (reclassified):** `commercial_lawsuits` (modernized 4f), `corp_contracts` (live government stadium/infrastructure/military contracts + main-tick processor).
  ...
  - [ ] 5B — DROP migration for the verified-dead tables (confirm list first).
    - **Closure computed:** the legacy faction-corp economy is a **50-table connected FK component**, verified **closed** (no KEEP table FK-references into it) and **reader-free** (zero client/edge reads). Outgoing FKs only to `factions`/`nations`/`trade_agreements`/`airline_routes` (all KEEP). Subsystems: loans, shipping, aviation-incidents, construction-materials, subsidiaries, executives, corp-tax, aircraft-RFP/design, **equity** (`equity_positions/offers/raises`), **strategic-alliances** (`strategic_alliances` + `alliance_*`). All faction-keyed (`*_faction_id`/`corp_id`→factions) → legacy, no live UI.
    - [x] **5B-prep (non-destructive): removed 3 mis-kept legacy tick calls** (the last live entry points into the component): `process_equity_dividends` (advance-corp-tick), `dissolve_failed_alliance_negotiations` + `sweep_alliance_interest_votes` (advance-tick **and** its source template `scripts/advance-tick-handler-template.ts`). `emit_corp_cash_event` and all ~44 legacy fns are now callerless.
    - [x] **5B-drop — migration `20270251_corp_cull_p5b_teardown_legacy_economy.sql`:** DROP 52 dead tables (CASCADE) + 41 dead functions (CASCADE, also unbinds 3 vestigial `factions` triggers). Idempotent, in a transaction. Audited clean (no surviving view, client/edge `.from()`, or function references a dropped object → CASCADE drops nothing live). **Carve-outs kept:** `corp_cash_events`+`factions.corp_cash_reserves` (kept for live client/edge **readers** — createparty/admin/common/bills; their writer `emit_corp_cash_event` is itself dead but kept here, drop is a 5C concern), `corp_contract_events`/`shipping_contract_events` (live audit logs), entrepreneur economy.
      - **CORRECTION (supersedes earlier "live ledger" note):** the bare `overhaul_aircraft` / `place_construction_bid` are **legacy/dead** — earlier flagged "live" via a substring false-positive (`entrepreneur-corp.html` actually calls `entrepreneur_overhaul_aircraft` / `ent_place_construction_bid`, which use `entrepreneur_corps.treasury`, NOT the faction `corp_cash_events` ledger). So the faction cash ledger has no live *writer*; it's kept only because the columns are read live.
  - [~] **5C-1 — drop remaining callerless legacy-corp functions: PAUSED (deliberate, 2026-05-24).** Verified each is callerless (zero client/edge), but reliably enumerating the *complete* set by static analysis kept tripping this codebase: (a) drop-and-recreate (a fn appears in `DROP FUNCTION` then is re-`CREATE`d in the same file → naive scans mis-mark it dropped), (b) helpers shared between legacy + live entrepreneur processors (e.g. `airline_condition_decay` is called by the LIVE `process_entrepreneur_airline_routes` → must NOT drop). A sanity gate caught a different tooling artifact on every run, so no list could be vouched for. **Decision:** these fns are callerless (zero runtime cost, invisible to players); the only harm is tidiness, while a misclassification = a core-loop production bug. Per SIMPLIFY/"do less", left as documented inert residue rather than risk the sweeping drop. 5B already removed the dead tables + the bulk of functions.
  - [ ] 5C-2 (deferred) — `factions.corp_*` column drop. Mostly held by the same dead fn family (so blocked on 5C-1). Live readers to KEEP: `corp_id`/`corp_sector`/`corp_subsector`/`corp_ticker`/`corp_company_type`/`corp_reputation`/`corp_route_risk`/`corp_last_processed_tick`/`corp_cash_reserves`. Needs per-column liveness pass.
  - [ ] 5D (deferred) — leftover trigger fns tied to surviving tables/columns: `refresh_corp_routes_count` (fires on live `airline_routes`, writes legacy `factions.corp_routes`), `set_finance_active_loans_original_principal`, `corp_ownership_*` — drop alongside their columns/tables in 5C-2.
  - [x] **5-closeout (2026-05-24): deleted the 2 confirmed-orphan legacy JS modules** (`js/game/materials.js`, `js/game/vessels.js`) — see P2 follow-up note above.

### Cull outcome (2026-05-24)
**The legacy faction-corp cull is functionally complete.** Phases 1–4 removed the entire player-facing/runtime surface (frontend pages + modules, client RPCs, all tick/edge processors). 5B dropped the 52-table dead economy + 41 dead functions. 5-closeout deleted the last 2 orphan JS modules. **Deliberately left as documented inert residue** (DB-level, callerless/unread, zero runtime cost — not worth the core-table risk for tidiness): the remaining callerless legacy functions (5C-1, paused), the unread `factions.corp_*` columns (5C-2), and their vestigial trigger fns (5D). Resume points are documented above if that polish is ever wanted.
