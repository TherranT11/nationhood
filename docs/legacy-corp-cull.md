...
- [~] Phase 5 — Schema teardown (staged, confirm each destructive step). **KEEP (reclassified):** `commercial_lawsuits` (modernized 4f), `corp_contracts` (live government stadium/infrastructure/military contracts + main-tick processor).
  ...
  - [ ] 5B — DROP migration for the verified-dead tables (confirm list first).
    ...
    - [x] **5B-drop — migration `20270251_corp_cull_p5b_teardown_legacy_economy.sql`:** DROP 52 dead tables (CASCADE) + 41 dead functions (CASCADE, also unbinds 3 vestigial `factions` triggers). Idempotent, in a transaction. **Verified LIVE carve-outs (kept):** `corp_cash_events`+`emit_corp_cash_event`+`factions.corp_cash_reserves` (live ledger, written by `overhaul_aircraft`/`place_construction_bid`), `corp_contract_events`/`shipping_contract_events` (live audit logs), entrepreneur economy.
  - [ ] 5C — `factions.corp_*` column drop. **Partly live:** `corp_id`/`corp_sector`/`corp_subsector`/`corp_ticker`/`corp_company_type`/`corp_reputation`/`corp_route_risk`/`corp_last_processed_tick`/`corp_cash_reserves` have client/edge readers; `corp_regulatory_standing`/`corp_supply_chain`/`corp_work_crews`/`corp_debt`/`corp_freighters`/`corp_op_safety`/`corp_overleverage`/`corp_lending_capital*`/… are read by **live** `place_construction_bid` + a broad still-uncategorised fn family (construction-bid, shipping-action, finance-stats, loan-offer). Needs per-column + per-fn liveness pass before any drop. Only ~14 columns currently look reference-free.
  - [ ] 5D — leftover trigger fn `refresh_corp_routes_count` (fires on live `airline_routes`, writes legacy `factions.corp_routes`) — drop alongside the `corp_routes` column in 5C.
