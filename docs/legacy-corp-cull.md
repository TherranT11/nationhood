...
- [~] Phase 4 — Tick / edge decoupling (IN PROGRESS, done in verified sub-steps; `advance-corp-tick` is hand-written, verify with `node --check`; **deploy the edge before applying the function-drop migrations**).
  ...
  - [~] 4d legacy corp economy (IN PROGRESS). **Done:** removed `processPropertyEffects`, `processRegionalHqIncome`, `processCorpMonthlyIncome` (its `corp_loans` ref was a comment only — no entrepreneur write), the reputation-decay block, the `assess_corporate_taxes` per-tick call + dropped that fn (`20270244`), and the three self-contained delivery/market processors `generateShipMarketListings`/`processVesselOrderDeliveries`/`processEquipmentDeliveries` (defs + their once-guarded call try-blocks; tables `ship_market_listings`/`vessel_orders`/`corp_equipment_deliveries` are legacy — only economy.html's legacy corp-economy display reads them, removed in Phase 5; no surviving client writes them, generated `advance-tick` does not process them). **Remaining (all verified legacy, all INLINE inside the per-nation legacy loop):** the construction cluster (`processCorpContracts` call + permit lifecycle + `apply_construction_wages_for_nation` + GDP boost), executive-contract-expiry (`expireExecutiveContracts`), transit-cycles block. KEEP `processCentralBankLoanPayments`.
    - **Structural finding (next pass):** after the safe-three removal, everything left in 4d/4e/4f lives INLINE inside one per-nation loop `for (const nation of nationList)` (corps fetched via `.eq('faction_type','corporation')`) that also contains the 4e finance-loan call and the vessel-decay/energy/defense/specialty-building legacy blocks, with the 4f `process_lawsuit_deadlines` sweep after it. KEEP processors (`processBankLoanPayments`, `processCentralBankLoanPayments`, `processTradeAgreementShipping`, `process_entrepreneur_airline_routes`) are already hoisted to top-level AFTER this loop. Recommend removing the whole legacy per-nation loop as ONE unit (collapsing 4d+4e-loop+4f-loop) rather than phase-by-phase inline surgery — safer and leaves no frankensteined loop. Deploy checkpoint advised before that cut.
