...
- [~] Phase 4 — Tick / edge decoupling (IN PROGRESS, done in verified sub-steps; `advance-corp-tick` is hand-written, verify with `node --check`; **deploy the edge before applying the function-drop migrations**).
  ...
  - [ ] 4g-rest — **legacy loan-negotiation cull** (remove the mis-kept `auto_abandon_stale_negotiations` tick sweep + drop the `loan_negotiations` RPCs; `loan_negotiations` is faction-keyed legacy; `js/loan-negotiation-modal.js` appears unimported) and the **`recompute_finance_stats` finance ecosystem** (~22-caller cluster: syndicated lending / joint equity / alliance LC / mutual aid / `fire_finance_action`).
