import{_supabase as f}from"./supabase-client-qEAQbBjE.js";/* empty css                    *//* empty css                         */import{escapeHtml as v,tickToDate as kt}from"./utils-A98FEun4.js";import{initMessaging as ne}from"./messaging-1y3PTVCT.js";import{c as oe}from"./equipment-DsuDdEne.js";import{a as ae}from"./corp-valuation-C0hsb2EQ.js";import{s as ie,c as se}from"./corp-refurbish-eZ1qOCh2.js";import{m as zt}from"./loan-math-Q4nHfU_i.js";let ht=[],d=null,Q=null;function _(t){return Math.abs(t)>=1e6?"$"+(t/1e6).toFixed(2)+"M":Math.abs(t)>=1e3?"$"+(t/1e3).toFixed(1)+"k":"$"+Math.round(t).toLocaleString()}function mt(t){const e=Number(t)||0,n=Math.abs(e);return n>=1e9?{main:"$"+(e/1e9).toFixed(2),unit:"B"}:n>=1e6?{main:"$"+(e/1e6).toFixed(2),unit:"M"}:n>=1e3?{main:"$"+(e/1e3).toFixed(1),unit:"k"}:{main:"$"+Math.round(e).toLocaleString(),unit:""}}function re(t){const e=Math.max(0,Math.min(100,Number(t)||0));return e>=90?{tag:"Aaa",tone:"good"}:e>=80?{tag:"Aa1",tone:"good"}:e>=70?{tag:"A1",tone:"good"}:e>=60?{tag:"Baa1",tone:"gold"}:e>=50?{tag:"Baa3",tone:"gold"}:e>=40?{tag:"Ba1",tone:"gold"}:e>=30?{tag:"Ba3",tone:"red"}:e>=20?{tag:"B2",tone:"red"}:e>=10?{tag:"Caa1",tone:"red"}:{tag:"Ca",tone:"red"}}function vt(t,e){return Number(t?.[e]??50)}async function le(){const{data:{user:t}}=await f.auth.getUser();if(!t){window.location.href="login.html";return}const{data:e}=await f.from("factions").select("*").or(`id.eq.${t.id},linked_user_id.eq.${t.id}`);ht=(e||[]).filter(b=>b.nation_id&&!b.abandoned_at);const n=sessionStorage.getItem("active_faction_id");if(d=ht.find(b=>b.id===n)||ht.find(b=>b.faction_type==="corporation")||ht[0],!d){sessionStorage.removeItem("active_faction_id"),window.location.href="faction-select.html";return}if(sessionStorage.setItem("active_faction_id",d.id),d.faction_type!=="corporation"){window.location.href="dashboard.html";return}let o=d.nation||"",i=null;const[r,p]=await Promise.all([d.nation_id?f.from("nations").select("*").eq("id",d.nation_id).single():Promise.resolve({data:null}),f.from("shard").select("current_tick, current_date, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single()]);r.error&&console.warn("Nation load failed:",r.error.message),r.data&&(o=r.data.name,i=r.data),p.error&&console.warn("Shard load failed:",p.error.message),Q=p.data;let l=0;if(d?.id){const{data:b}=await f.from("construction_contracts").select("id, contract_bids!inner(id)").eq("issuer_faction_id",d.id).in("status",["open","bidding"]);if(b)for(const g of b)l+=(g.contract_bids||[]).length}(function(){const g=(d.corp_ticker||d.abbreviation||"").toUpperCase(),c=Q?.current_date||"",K=c?c.replace(/.*,\s*/,""):"",B=Q?.current_tick,Y=document.getElementById("h2-plate-logo");Y&&(d.custom_logo_url?Y.innerHTML=`<img src="${v(d.custom_logo_url)}" alt="logo">`:Y.textContent=g.slice(0,3)||"—");const $=document.getElementById("h2-eyebrow-loc");$&&($.textContent=o?`The Boardroom · ${o}`:"The Boardroom");const X=document.getElementById("h2-tick-date");if(X){const k=[];B!=null&&k.push("Tick "+B),c&&k.push(c),X.textContent=k.length?k.join(" · "):"Tick — · —"}const J=document.getElementById("h2-next-close");J&&(J.textContent="Next close —");const G=document.getElementById("h2-ceo");if(G){const k=[d.leader_first_name,d.leader_last_name].filter(Boolean);if(k.length){const R=d.leader_age?" ("+d.leader_age+")":"",V=d.leader_role||"Chairman & Chief Executive";G.textContent=`${k.join(" ")} · ${V}${R}`}else G.textContent="—"}const st=document.getElementById("h2-brand");if(st){const k=d.faction_name||"Unnamed Corporation",R=k.split(" ");if(R.length>1){const V=R.slice(0,-1).join(" "),Z=R[R.length-1];st.innerHTML=`${v(V)} <em>${v(Z)}</em>`}else st.textContent=k}const dt=document.getElementById("h2-brand-sub");if(dt){const k=[];d.corp_company_type&&k.push(d.corp_company_type),K&&k.push("Est. "+K);const R=d.corp_subsector||d.corp_sector;R&&k.push(R),dt.textContent=k.length?k.join(" · "):"—"}const pt=document.getElementById("h2-tail-code");if(pt){const k=(o||"").split(" ").map(Z=>Z[0]||"").join("").toUpperCase().slice(0,4),R=d.party_description?'"'+d.party_description+'"':"",V=[];g&&V.push(g),k&&V.push(k+" EXCH"),R&&V.push(R),pt.textContent=V.length?V.join(" · "):"—"}const T=document.getElementById("h2-wire-corp");if(T){const k=(d.faction_name||"your corp").split(" ").slice(0,2).join(" ");T.textContent=k}const h=document.getElementById("h2-edit-toggle");h&&h.addEventListener("click",()=>{const k=document.body.classList.toggle("h2-edit-open");h.classList.toggle("on",k),h.textContent=k?"Close ✕":"Logo"})})(),(function(){const g=document.getElementById("h2-logout-btn");if(g&&g.addEventListener("click",async()=>{try{sessionStorage.clear(),await f.auth.signOut()}catch{}window.location.href="login.html"}),l>0){const c=document.getElementById("h2-nav-actions-badge");c&&(c.textContent=l,c.style.display="",c.classList.add("ok"))}})(),document.getElementById("id-type-badge").textContent=d.corp_company_type||"—";const a=document.getElementById("id-logo"),s=(d.corp_ticker||d.abbreviation||"").toUpperCase();d.custom_logo_url?a.innerHTML=`<img src="${v(d.custom_logo_url)}" alt="logo" style="width:100%;height:100%;object-fit:contain;border-radius:4px;">`:a.textContent=s.slice(0,3)||"—",document.getElementById("id-corp-name").textContent=d.faction_name||"Unnamed Corp";const m=d.party_description||"";document.getElementById("id-slogan").textContent=m?'"'+m+'"':'"--"';const u=Q?.current_date?Q.current_date.replace(/.*,\s*/,""):"—",x=d.leader_first_name&&d.leader_last_name?d.leader_first_name+" "+d.leader_last_name+(d.leader_age?" ("+d.leader_age+")":""):"—";document.getElementById("id-rows").innerHTML=`
        <div class="id-row">
        ...
        <div class="id-row">
            ...
            <span class="id-row__value">${v(o||"—")}</span>
        </div>
        <div class="id-row">
        ...
        <div class="id-row">
            ...
            <span class="id-row__value" style="color:var(--teal);letter-spacing:0.1em;">${v(s)}</span>
        </div>
    `;const E=d.last_rename_tick||0,S=Q?.current_tick||0,q=Math.max(0,E+120-S),O=!m||m==="-"||m==='"-"'||q<=0,U=document.getElementById("slogan-editor");U.innerHTML=`
        <div class="slogan-bar">
            <input type="text" id="slogan-input" placeholder="Enter a slogan..." maxlength="60" value="${v(m)}">
    ...
    `,document.getElementById("corp-logo-file")?.addEventListener("change",xe),window._corpFactionId=d.id,window._currentTick=S,window._nationStats=i,window._factionData=d;const w=ce(i,o,d);_e(o,d);const L=await St(i,o,d,Q);let H=0;if(d?.id){const{data:b,error:g}=await f.from("corp_equipment").select("equipment_key, owned").eq("faction_id",d.id);g||(H=oe(b||[]))}let M=0;if(d?.id){const{data:b}=await f.from("corp_executives").select("salary_per_year").eq("faction_id",d.id).eq("status","active");M=(b||[]).reduce((g,c)=>g+(Number(c.salary_per_year)||0),0)}let N=0,F=0;if(d?.id&&d.corp_sector==="Shipping"){const{data:b}=await f.from("corp_vessels").select("base_maintenance, purchase_price, condition, built_at_tick, status").eq("faction_id",d.id).neq("status","for_sale");N=(b||[]).reduce((g,c)=>g+(Number(c.base_maintenance)||0),0),F=ae(b,S)}await de(i,Q,w,d,L.propertyMaintenance||0,H,M,L,N),await ve(i,o,d,w,L,F),ne(d,i,Q),ct={nationId:d.nation_id},Kt(),document.getElementById("loading").style.display="none",document.getElementById("page-content").style.display="block"}function ce(t,e,n){const o=g=>vt(t,g),i=(e||"UNKNOWN").toUpperCase(),r=Number(n?.corp_general_workforce??2250),p=Number(n?.corp_skilled_workforce??600),l=Number(n?.corp_innovative_workforce??150),a=r+p+l,s=2,m=3,u=6,x=o("minimum_wage"),E=x/100*48e3,S=o("inflation"),A=o("standard_of_living"),q=1+(S-50)/100*.5,W=1+(A-50)/100*.5,O=g=>Math.round(E*g*q*W),U=O(s),D=O(m),w=O(u),L=r*U,H=p*D,M=l*w,N=L+H+M;function F(g){return"$"+Math.round(g).toLocaleString()+"/yr"}const b=`${q.toFixed(2)} &times; ${W.toFixed(2)}`;return document.getElementById("wf-total-header").textContent=a.toLocaleString(),document.getElementById("wf-body").innerHTML=`
        <div class="wf-tier">
            ...
            <div class="wf-tier__detail">
                <span class="wf-tier__label">Wage (min &times; ${s}.0 &times; ${b})</span>
    ...
    `,{totalWages:N,generalTotal:L,skilledTotal:H,innovativeTotal:M,monthlyWages:Math.round(N/12)}}async function de(t,e,n,o,i,r,p,l,a){const s=e?.current_tick||0;document.getElementById("fin-tick").textContent="TICK "+s;const m=87e6,u=y=>vt(t,y),x=1+(u("gdp_growth")-50)/100*.4,E=1+(u("urbanization")-50)/100*.3,S=1+(u("population_growth")-50)/100*.2,A=1+(u("standard_of_living")-50)/100*.15,q=1+(50-u("physical_infrastructure"))/100*.1,W=1-Math.max(0,u("inflation")-50)/100*.1,O=1-Math.max(0,u("interest_rates")-50)/100*.1,U=x*E*S*A*q*W*O,D=Math.round(m*U),w=(o.corp_general_workforce||0)+(o.corp_skilled_workforce||0)+(o.corp_innovative_workforce||0),L=Math.max(500,l?.totalCapacity||500),H=Math.min(1,w/L),M=l?.propertyRevBonus||0,N=Math.round(Math.round(D/12)*H)+M;let F=0,b=0,g=0;if(o?.id){const y=o.corp_sector||"";if(y==="Finance"){const{data:I}=await f.from("finance_active_loans").select("monthly_payment, interest_rate, remaining_principal, finance_loan_requests(request_type)").eq("lender_faction_id",o.id).in("status",["current","late","delinquent"]);for(const C of I||[]){const P=C.finance_loan_requests?.request_type||"loan";if(P==="insurance")b+=Number(C.monthly_payment||0);else if(P==="loan"){const z=Math.max(0,Number(C.remaining_principal||0));b+=zt(z,C.interest_rate)}else P==="bond"&&(b+=Number(C.monthly_payment||0))}}else if(y==="Construction"){const{data:I}=await f.from("construction_contracts").select("id, timeline_ticks").eq("awarded_to_faction",o.id).eq("status","in_progress"),C=(I||[]).map(z=>z.id).filter(Boolean);if(C.length>0){const{data:z}=await f.from("contract_bids").select("contract_id, estimated_cost").in("contract_id",C).eq("status","won"),it={};for(const at of z||[])it[at.contract_id]=Number(at.estimated_cost||0);for(const at of I||[]){const j=it[at.id]||0;g+=Math.round(j/Math.max(1,at.timeline_ticks||1))}}const{data:P}=await f.from("construction_deliveries").select("payment_received").eq("faction_id",o.id).eq("delivered_at_tick",s);for(const z of P||[])b+=Number(z.payment_received||0)}else if(y==="Shipping"){const{data:I}=await f.from("shipping_claims").select("revenue_per_transit").eq("faction_id",o.id).eq("status","active");for(const C of I||[])b+=Number(C.revenue_per_transit||0)}}let c=[],K=0;try{const{data:y}=await f.from("corp_properties").select("id, nation_id, nations!nation_id(name)").eq("faction_id",o.id).eq("type","fuel_depot").eq("is_active",!0);if(y&&y.length>0){const I=y.map(C=>C.nation_id).filter(Boolean);if(I.length>0){const{data:C}=await f.from("shipping_claims").select("faction_id, shipping_routes!inner(destination_nation_id, status)").eq("status","active").in("shipping_routes.destination_nation_id",I),P=[...new Set((C||[]).map(j=>j.faction_id).filter(j=>j&&j!==o.id))],z=new Set;if(P.length>0){const{data:j}=await f.from("corp_properties").select("faction_id, nation_id").in("faction_id",P).in("nation_id",I).eq("type","fuel_depot").eq("is_active",!0);for(const nt of j||[])z.add(nt.faction_id+"|"+nt.nation_id)}const it={};for(const j of C||[]){const nt=j.shipping_routes?.destination_nation_id;nt&&j.faction_id!==o.id&&(z.has(j.faction_id+"|"+nt)||(it[nt]=(it[nt]||0)+1))}const at=7500;for(const j of y){const nt=it[j.nation_id]||0,Rt=nt*at;c.push({nation:j.nations?.name||"Unknown",revenue:Rt,visitors:nt}),K+=Rt}c.sort((j,nt)=>nt.revenue-j.revenue)}}}catch(y){console.warn("Fuel depot revenue estimate failed (non-fatal):",y?.message||y)}const B=F+b+N+K,Y=n?.totalWages||0,$=Math.round(Y/12),X=0,J=i||0,G=r||0,st=Number(o?.corp_loans)||0,dt=.05,pt=st>0?Math.round(st*(dt/12)/(1-Math.pow(1+dt/12,-120))):0;let T=0,h=0;if(o?.id)try{const{data:y}=await f.from("finance_active_loans").select("monthly_payment, finance_loan_requests(request_type)").eq("borrower_faction_id",o.id).in("status",["current","late","delinquent"]);for(const I of y||[]){const C=I.finance_loan_requests?.request_type||"loan",P=Number(I.monthly_payment||0);if(!(P<=0))if(C==="insurance")h+=P;else{if(C==="bond")continue;T+=P}}}catch(y){console.warn("[Finances] borrower finance_active_loans lookup failed:",y)}const k=Math.round((p||0)/12),R=a||0,V=75e3,Z=Math.max(0,Math.min(1,Number(t?.corporate_tax??0)/100||0)),rt=$+k+X+J+G+R+pt+T+h+g+V,_t=Math.max(0,B-rt),ft=Math.round(_t*Z);let ut="";try{const y=new Set([o.nation_id]),{data:I}=await f.from("corp_properties").select("nation_id").eq("faction_id",o.id).eq("is_active",!0);if((I||[]).forEach(C=>{C.nation_id&&y.add(C.nation_id)}),y.size>0){const{data:C}=await f.from("nations").select("id, name, corporate_tax").in("id",[...y]);C&&C.length>0&&(ut=C.sort((P,z)=>(P.name||"").localeCompare(z.name||"")).map(P=>{const z=Math.round(Number(P.corporate_tax??0)),it=Math.round(_t*(z/100)/C.length),at=z>25?"#c55":z>15?"#ca5":"#5c5";return`<div style="display:flex;justify-content:space-between;padding:1px 0;font-family:var(--font-mono);font-size:8px;">
                        <span style="color:var(--text-dim);">${P.name} (<span style="color:${at}">${z}%</span>)</span>
                        <span style="color:#a44;">${_(it)}</span>
                    </div>`}).join(""))}}catch{}const Et=rt+ft,ot=Number(o?.monthly_profit||0),tt=Number(o?.corp_cash_reserves??0),Qt=st;let gt=null,et=null,Mt=null,Tt="Does not include all capital/financing cash transfers.";if(o?.id)try{const{data:y}=await f.from("corp_cash_history").select("tick, cash_start, cash_end, cash_delta, non_pnl_cash_movements").eq("faction_id",o.id).lte("tick",s).order("tick",{ascending:!1}).limit(2),I=(y||[]).find(P=>Number(P.tick)===Number(s))||(y||[])[0]||null,C=(y||[]).find(P=>Number(P.tick)<Number(I?.tick??s))||null;I?(gt=I.cash_start!=null?Number(I.cash_start):C?.cash_end!=null?Number(C.cash_end):null,et=I.cash_delta!=null?Number(I.cash_delta):gt!=null?tt-gt:null,Mt=I.non_pnl_cash_movements!=null?Number(I.non_pnl_cash_movements):et!=null?et-ot:null):Tt="Does not include all capital/financing cash transfers. Cash history snapshot not yet available."}catch(y){console.warn("[Finances] corp_cash_history lookup failed:",y),Tt="Does not include all capital/financing cash transfers. Cash history snapshot unavailable."}me(B,ot);const Xt=[{stat:"gdp_growth",value:u("gdp_growth"),weight:"0.4"},{stat:"urbanization",value:u("urbanization"),weight:"0.3"},{stat:"population_growth",value:u("population_growth"),weight:"0.2"},{stat:"standard_of_living",value:u("standard_of_living"),weight:"0.15"},{stat:"physical_infra",value:u("physical_infrastructure"),weight:"0.1",note:"INV"},{stat:"inflation",value:u("inflation"),weight:"-0.1",neg:!0},{stat:"interest_rates",value:u("interest_rates"),weight:"-0.1",neg:!0}];function Jt(y){return y.neg?y.value>50?"var(--red)":"var(--green)":y.note?y.value<50?"var(--green)":"var(--red)":y.value>=50?"var(--green)":y.value>=35?"var(--amber)":"var(--red)"}const It=B||1,Zt=(F/It*100).toFixed(1),te=((b+K)/It*100).toFixed(1),ee=(N/It*100).toFixed(1);document.getElementById("fin-body").innerHTML=`
        <!-- Revenue -->
        <div class="fin-section">
            <div class="fin-section__title" style="color:var(--green);">Revenue</div>
            <div class="seg-bar">
                <div class="seg-bar__seg" style="width:${Zt}%;background:var(--teal);"></div>
                <div class="seg-bar__seg" style="width:${te}%;background:var(--amber);"></div>
                <div class="seg-bar__seg" style="width:${ee}%;background:var(--text-dim);"></div>
            </div>
        ...
        <div class="fin-cash-debt">
            ...
            <div class="fin-cash-debt__cell">
                ...
                <div class="fin-cash-debt__value" style="color:var(--amber)">${_(Qt)}</div>
            </div>
        </div>
        <!-- Purchasing Power -->
        ...
        <div style="padding:8px 14px;">
            ...
            ${Xt.map(y=>`
                <div class="drv-row">
                    ...
                    <div class="drv-row__bar"><div class="drv-row__bar-fill" style="width:${y.value}%;background:${Jt(y)}"></div></div>
    ...
    `,Nt()}let Lt=!1;async function pe(t,e){if(!(!d||Lt)){Lt=!0;try{const{data:n,error:o}=await f.from("finance_loan_offers").select("*").eq("id",t).single();if(o||!n)return;const{data:i,error:r}=await f.from("finance_loan_requests").select("*").eq("id",e).single();if(r||!i||i.status!=="open")return;const p=i.term_months,l=zt(i.amount,n.interest_rate),a=Math.round(i.amount/p),s=l+a,m=Q?.current_tick||0,{error:u}=await f.from("finance_loan_requests").update({status:"funded",accepted_offer_id:t,funded_tick:m}).eq("id",e);if(u)return;await f.from("finance_loan_offers").update({status:"accepted"}).eq("id",t),await f.from("finance_loan_offers").update({status:"declined"}).eq("request_id",e).neq("id",t).eq("status","pending"),await f.from("finance_active_loans").insert({request_id:e,offer_id:t,borrower_faction_id:i.requesting_faction_id,lender_faction_id:n.offering_faction_id,nation_id:i.nation_id,principal:i.amount,interest_rate:n.interest_rate,term_months:i.term_months,collateral_type:n.collateral_type,purpose:i.purpose,monthly_payment:s,started_tick:m});const{data:x}=await f.from("factions").select("corp_cash_reserves").eq("id",n.offering_faction_id).single();x&&await f.from("factions").update({corp_cash_reserves:Math.max(0,(Number(x.corp_cash_reserves)||0)-i.amount)}).eq("id",n.offering_faction_id);const{data:E}=await f.from("factions").select("corp_cash_reserves, corp_debt").eq("id",i.requesting_faction_id).single();if(E){const{error:S}=await f.from("factions").update({corp_cash_reserves:(Number(E.corp_cash_reserves)||0)+i.amount,corp_debt:(Number(E.corp_debt)||0)+i.amount}).eq("id",i.requesting_faction_id);S&&console.error("[Loans] Failed to credit borrower + track debt:",S.message)}}finally{Lt=!1}Nt()}}async function fe(t){await f.from("finance_loan_requests").update({status:"cancelled"}).eq("id",t),Nt()}async function Nt(){if(!d)return;const t=document.getElementById("fin-loans-list");if(t)try{const{data:e,error:n}=await f.from("finance_loan_requests").select("*, finance_loan_offers!request_id(*)").eq("requesting_faction_id",d.id).in("status",["open","funded"]).neq("request_type","equity").order("created_tick",{ascending:!1});n&&console.error("[Loans] Request query error:",n.message);const{data:o,error:i}=await f.from("finance_active_loans").select("*, finance_loan_requests!inner(request_type, insured_contract_id), lender:factions!lender_faction_id(faction_name, abbreviation)").eq("borrower_faction_id",d.id).in("status",["current","late","delinquent"]).is("equity_pct",null).order("started_tick",{ascending:!1});i&&console.error("[Loans] Active loans query error:",i.message);const r=[];for(const s of o||[]){const m=s.finance_loan_requests?.insured_contract_id;m&&r.push(m)}const p=[...new Set(r)];let l={};if(p.length>0){const{data:s,error:m}=await f.from("construction_contracts").select("id, status").in("id",p);m?console.error("[Loans] Contract status query error:",m.message):l=Object.fromEntries((s||[]).map(u=>[u.id,u.status]))}let a="";if(e&&e.length>0){for(const s of e)if(s.status==="open"){const m=(s.finance_loan_offers||[]).filter(u=>u.status==="pending");if(a+=`<div style="padding:6px 0;border-bottom:1px solid var(--border-hair);">
                    ...
                    <div style="font-size:7px;color:var(--text-dim);margin-top:2px;">${v(s.purpose||"")}</div>`,m.length>0){a+=`<div style="margin-top:4px;font-size:7px;color:#5a8aaa;font-weight:700;">${m.length} OFFER${m.length>1?"S":""}</div>`;for(const u of m.sort((x,E)=>x.interest_rate-E.interest_rate))a+=`<div style="display:flex;align-items:center;gap:6px;padding:3px 0;border-top:1px solid var(--border-hair);">
                            ...
                            <span style="padding:2px 8px;cursor:pointer;color:#000;background:#5a8aaa;font-weight:700;font-size:7px;" onclick="acceptOffer('${u.id}','${s.id}')">ACCEPT</span>
                        </div>`}else a+='<div style="margin-top:2px;font-size:7px;color:var(--text-dim);">Awaiting offers from finance corporations...</div>';a+="</div>"}}if(o&&o.length>0)for(const s of o){const m=s.finance_loan_requests?.request_type||"loan",u=s.finance_loan_requests?.insured_contract_id,x=u?l[u]:null;if(m==="insurance"){const O=(s.status==="late"||s.status==="delinquent")&&Number(s.payments_missed||0)===0,U=O?"#d9a441":s.status==="current"?"var(--green)":s.status==="late"?"#c84":"#c55",D=x==="completed"?"Project Completed":x==="in_progress"?"Project In Progress":null;a+=`<div style="padding:6px 0;border-bottom:1px solid var(--border-hair);">
                    <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;">
                        <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
                            <span style="color:#5a8aaa;font-weight:700;">INSURANCE</span>
                            <span style="color:${U};font-weight:700;">${s.status.toUpperCase()}</span>
                    ...
                    <div style="display:flex;justify-content:space-between;margin-top:3px;font-size:7px;color:var(--text-dim);">
                        <span>Monthly Premium: ${_(s.monthly_payment)}/mo</span>
                        <span>Coverage: ${_(s.principal)}</span>
                    </div>
                ...
                </div>`;continue}const E=(s.status==="late"||s.status==="delinquent")&&Number(s.payments_missed||0)===0,S=E?"#d9a441":s.status==="current"?"var(--green)":s.status==="late"?"#c84":"#c55",A=s.term_months>0?Math.round(s.payments_made/s.term_months*100):0,q=s.lender?.faction_name||"Unknown bank",W=kt(s.started_tick);a+=`<div style="padding:6px 0;border-bottom:1px solid var(--border-hair);">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <div>
                        <span style="color:${S};font-weight:700;">${s.status.toUpperCase()}</span>
                ...
                <div style="display:flex;justify-content:space-between;margin-top:2px;font-size:7px;color:var(--text-dim);">
                    <span>Bank: <span style="color:var(--text-primary);">${v(q)}</span></span>
            ...
            <span class="stat-item__delta" style="color:${m}">${Math.abs(s).toFixed(1)}</span>
                ...
                </div>`;continue}a.section&&(l+=`<div class="stats-section"><span class="stats-section__label">${a.section}</span></div>`);const s=a.max&&a.max<=100;l+=`
            <div class="stat-item">
                ...
                <div class="stat-item__right">
                    <span class="stat-item__val" style="color:${s?r(a.value,a.max):"var(--text-primary)"}">${typeof a.value=="number"?s?Math.round(a.value):a.value.toLocaleString():a.value}</span>
                    ${s?'<span class="stat-item__max">/100</span>':""}
                    ${p(a.change)}
                </div>
            </div>`}document.getElementById("stats-body").innerHTML=l}async function St(t,e,n,o){const i=(e||"UNKNOWN").toUpperCase();let r=[];if(n?.id){const{data:g}=await f.from("corp_properties").select("*").eq("faction_id",n.id).eq("is_active",!0).order("type",{ascending:!1}).order("purchased_at_tick",{ascending:!1});r=g||[]}const p={Basic:{color:"#6a6660"},Modern:{color:"#8b9a6b"},Sustainable:{color:"#5c5"},Innovative:{color:"#c8a832"},Heritage:{color:"#c84"},Premium:{color:"#ca5"}};let l=0,a=0;const s=Number(n?.corp_general_workforce??0)+Number(n?.corp_skilled_workforce??0)+Number(n?.corp_innovative_workforce??0),m=500,u=r.map(g=>{const c=Number(g.capacity||0),K=Number(g.condition||0)/100;return Math.floor(c*K)}),x=m+u.reduce((g,c)=>g+c,0),E=x>0?Math.min(s,Math.round(s*(m/x))):s,S=5e7,A=1+(vt(t,"inflation")-50)/100*.3,q=.8+vt(t,"stability")/100*.4,W=Math.round(S*A*q),O=Math.round(W*.005);l+=W,a+=O;let U=`
    ...
    </div>`,D=E;for(let g=0;g<r.length;g++){const c=r[g],K=p[c.style]||p.Basic;l+=Number(c.purchase_price||0),a+=Number(c.monthly_maintenance||0);const B=c.condition>=75?"var(--green)":c.condition>=50?"var(--amber)":"var(--orange)",Y=Number(c.capacity||0),$=u[g]||0,X=x>0?Math.min(s-D,Math.round(s*($/x))):0;D+=X,U+=`
        <div class="prop-asset" style="margin-bottom:6px;">
            <div class="prop-asset__top">
                <div>
            ...
            ${c.refurbish_until_tick&&c.refurbish_until_tick>(o?.current_tick||0)?`<div style="margin-top:4px;padding:3px 6px;background:var(--amber-faint, rgba(176,154,91,0.08));border:1px solid var(--amber-border, rgba(176,154,91,0.2));font-size:8px;font-family:var(--font-mono);color:var(--amber, #b09a5b);text-align:center;">REFURBISHING — ${c.refurbish_until_tick-(o?.current_tick||0)} tick${c.refurbish_until_tick-(o?.current_tick||0)!==1?"s":""} remaining</div>`:`<div style="display:flex;gap:4px;margin-top:4px;">
                ...
                <div class="cp-section__header">
                ...
                </div>`;for(const $ of L){const X=B[$.status]||B.open,J=($.contract_bids||[]).filter(T=>T.status==="pending"),G=($.contract_bids||[]).find(T=>T.status==="won"),st=o?.current_tick||0,dt=c[$.id]||[],pt=$.nation_id===n.nation_id?i:"";if(w+=`<div class="cp-item">
                    <div class="cp-item__top">
                        <div>
                            <div class="cp-item__name">${v($.name)}</div>
                            <div class="cp-item__sub">${v($.project_code||"")} · ${v($.sector||"")}${pt?" · "+v(pt):""}</div>
                        </div>
                    ...
                    <div style="display:flex;gap:0;background:var(--bg-3);border:1px solid var(--border-hair);margin:2px 0;">
                        ...
                        </div>`,$.status==="in_progress"&&$.awarded_at_tick!=null){const R=st-$.awarded_at_tick,V=$.timeline_ticks||1,Z=$.stalled_ticks||0,rt=Math.min(100,Math.round(R/(V+Z)*100)),_t=rt>=75?"#5c5":rt>=40?"#ca5":"#5aaa8b",ft=Math.max(0,V+Z-R);w+=`<div style="display:flex;justify-content:space-between;font-size:8px;font-family:var(--font-mono);margin-top:6px;">
                            <span style="color:var(--text-dim);">PROGRESS</span>
                            <span style="color:${_t};">${rt}%${Z>0?" · "+Z+" stalled":""} · ${ft} tick${ft!==1?"s":""} left</span>
                        </div>
                        <div class="cp-progress"><div class="cp-progress__bar" style="width:${rt}%;background:${_t};"></div></div>`}else w+='<div style="font-size:8px;font-family:var(--font-mono);color:var(--amber);margin-top:6px;text-align:center;">Awarded — construction begins next tick</div>';w+="</div>"}if(dt.length>0)for(const T of dt){const h=Y[T.severity]||"#ca5";w+=`<div style="display:flex;align-items:center;gap:5px;margin-top:4px;padding:3px 6px;background:${h}08;border:1px solid ${h}20;">
                            <span class="cp-badge" style="color:${h};border-color:${h}40;background:${h}12;">${T.severity}</span>
                            <span style="font-size:8px;font-family:var(--font-mono);color:${h};">${v(T.title)}</span>
                        </div>`}if(($.status==="open"||$.status==="bidding")&&J.length>0)for(let T=0;T<J.length;T++){const h=J[T],k=$.id.slice(0,8)+"-"+T,R=Number(h.factions?.corp_reputation??50),V=R>=70?"#5c5":R>=40?"#ca5":"#c55",Z=h.estimated_quality>=75?"#5c5":h.estimated_quality>=50?"#ca5":"#c55",rt=h.markup_pct<=10?"#5c5":h.markup_pct<=20?"#ca5":"#c55",_t=h.material_grades||{},ft=Object.entries(_t),ut=ot=>ot.replace(/_/g," ").replace(/\b\w/g,tt=>tt.toUpperCase()),Et=ot=>ot==="HIGH"?"#5c5":ot==="LOW"?"#c55":"var(--text-muted)";w+=`<div class="cp-bid" style="flex-direction:column;gap:0;padding:0;cursor:pointer;" onclick="cpToggleBid('${k}')">
                            <div style="display:flex;align-items:center;gap:6px;padding:4px 8px;">
                                ...
                                <span class="cp-bid__btn" style="color:#5c5;border-color:#5c540;background:#5c508;" onclick="event.stopPropagation();cpAcceptBid('${$.id}','${h.id}','${v((h.factions?.faction_name||"").replace(/'/g,""))}',${h.bid_price},${h.estimated_quality},${h.labor_count},'${h.faction_id}')">ACCEPT</span>
                            </div>
                            <div id="cp-bid-${k}" style="display:none;padding:4px 8px 6px;border-top:1px solid var(--border-hair);background: var(--border-hair);">
                                <div style="display:flex;gap:0;margin-bottom:4px;">
                                    ...
                                    <div style="flex:1;padding:2px 4px;border-right:1px solid var(--border-hair);">
                                        ...
                                        <div style="font-size:9px;font-weight:700;color:${rt};font-family:var(--font-mono);">${h.markup_pct}%</div>
                                    </div>
                                    <div style="flex:1;padding:2px 4px;">
                                        <div style="font-size:6px;color:var(--text-dim);font-family:var(--font-mono);letter-spacing:0.5px;">QUALITY</div>
                    ...
                    </div>`}w+="</div>"}w+="</div>"}}const H=document.getElementById("prop-count"),M=r.length+1,N=L.length,F=M+" ASSET"+(M!==1?"S":"")+(N>0?" · "+N+" PROJECT"+(N!==1?"S":""):"");H&&(H.textContent=F),document.getElementById("prop-body").innerHTML=`
        ${U}
        ${w}
    ...
    `;let b=0;b+=Math.round(m*50);for(const g of r){if(g.refurbish_until_tick&&(o?.current_tick||0)<g.refurbish_until_tick)continue;const c=Number(g.condition||0)/100;c>=.6&&(b+=Math.round(Number(g.capacity||0)*c*50))}return{propertyValue:l,propertyMaintenance:a,totalCapacity:x,propertyRevBonus:b}}async function ve(t,e,n,o,i,r=0){(e||"UNKNOWN").toUpperCase();const p=n.corp_company_type||"Private",l=Number(n.corp_cash_reserves)||0,a=i?.propertyValue||0;let s=0;if(n?.id&&n.corp_sector==="Finance")try{const{data:L}=await f.from("finance_active_loans").select("remaining_principal, finance_loan_requests(request_type)").eq("lender_faction_id",n.id).in("status",["current","late","delinquent"]);for(const H of L||[]){const M=H.finance_loan_requests?.request_type||"loan";if(M==="loan"||M==="bond"){const N=Math.max(0,Number(H.remaining_principal||0));s+=N}}}catch(L){console.warn("[Valuation] finance_active_loans lookup failed:",L)}const m=l+a+r+s,u=Number(n.corp_loans)||0,E=o?.monthlyWages||0,S=0,A=u+E+S,q=m-A,O=Math.round(q*(1+.3)),U=O-q,D=U>0;document.getElementById("val-type-badge").textContent=p.toUpperCase();function w(L,H,M={}){const N=M.indent?"val-line val-line--indent":"val-line",F=M.bold?"val-line__label val-line__label--bold":"val-line__label",b=M.bold?"val-line__value val-line__value--bold":"val-line__value",g=M.color||(M.bold?"var(--text-bright)":"var(--text-muted)");return`<div class="${N}"><span class="${F}">${L}</span><span class="${b}" style="color:${g}">${_(H)}</span></div>`}document.getElementById("val-body").innerHTML=`
        ...
        ${w("Equipment",r,{indent:!0})}
        ${w("Active Contracts",s,{indent:!0})}
        ${w("Total Assets",m,{bold:!0,color:"var(--green)"})}
    ...
    `,ue({faction:n,nationName:e,cashReserves:l,propertyValue:a,equipmentValue:r,outstandingLoans:u,accountsPayable:E,totalLiabilities:A,netWorth:q,currentTickDate:t?._h2_close_label})}function me(t,e){const n=document.getElementById("h2-kpi-rev"),o=document.getElementById("h2-kpi-rev-d");if(!n)return;const i=mt(t);if(n.innerHTML=`${i.main}<small>${i.unit}</small>`,o){const r=mt(Math.abs(e)),p=e>=0?"+":"−",l=e>=0?"up":"down";o.className="delta "+l,o.textContent=`${p}${r.main}${r.unit} net`}}function ue({faction:t,nationName:e,cashReserves:n,propertyValue:o,equipmentValue:i,outstandingLoans:r,accountsPayable:p,totalLiabilities:l,netWorth:a}){if(!document.getElementById("h2-kpi-cash"))return;const s=mt(n);document.getElementById("h2-kpi-cash").innerHTML=`${s.main}<small>${s.unit}</small>`;const m=document.getElementById("h2-kpi-cash-d");m&&(m.textContent=l>0?`Liab. ${mt(l).main}${mt(l).unit}`:"No liabilities");const u=mt(a);document.getElementById("h2-kpi-nw").innerHTML=`${u.main}<small>${u.unit}</small>`;const x=document.getElementById("h2-kpi-nw-d");if(x){const g=a>=0?"up":"down";x.className="delta "+g,x.textContent=a>=0?"positive equity":"negative equity"}const E=Number(t.corp_reputation??0),S=document.getElementById("h2-kpi-rep");S&&(S.innerHTML=`${Math.round(E)}<small> / 100</small>`);const A=document.getElementById("h2-kpi-rep-d");A&&(A.className="delta "+(E>=70?"up":E>=40?"flat":"down"),A.textContent=E>=70?"strong":E>=40?"steady":"weak");const q=Number(t.corp_market_share??0),W=document.getElementById("h2-kpi-mkt");W&&(W.innerHTML=`${Math.round(q)}<small>%</small>`);const O=document.getElementById("h2-kpi-mkt-d");O&&(O.className="delta "+(q>=15?"up":q>=5?"flat":"down"),O.textContent=q>=15?"major player":q>=5?"mid-tier":"niche");const U=(t.corp_ticker||t.abbreviation||"CORP").toUpperCase(),D=document.getElementById("h2-nw-sym");D&&(D.textContent=U);const w=document.getElementById("h2-nw-ex");if(w){const g=(e||"").split(" ").map(c=>c[0]||"").join("").toUpperCase().slice(0,4);w.textContent=`${t.corp_company_type||"Private"}${g?" · "+g+" EXCH":""}`}const L=document.getElementById("h2-nw-price");L&&(L.innerHTML=`${u.main}<small>${u.unit}</small>`);const H=document.getElementById("h2-nw-d");H&&(H.className="d flat",H.textContent="no history yet");const M=(g,c,K)=>{const B=document.getElementById(g);B&&(B.textContent=_(c))};M("h2-tr-cash",n),M("h2-tr-property",o),M("h2-tr-equipment",i);const N=document.getElementById("h2-tr-loans");N&&(N.textContent=_(r),N.classList.toggle("good",r===0),N.classList.toggle("red",r>0)),M("h2-tr-payable",p);const F=re(t.corp_credit_rating??50),b=document.getElementById("h2-tr-credit");b&&(b.textContent=`${F.tag} · ${F.tone==="good"?"stable":F.tone==="gold"?"watch":"caution"}`,b.classList.remove("good","gold","red"),b.classList.add(F.tone))}async function ge(){const t=document.getElementById("slogan-input"),e=document.getElementById("slogan-hint"),n=document.getElementById("slogan-save-btn"),o=(t.value||"").trim().slice(0,60);if(o.length===0){e.textContent="Slogan cannot be empty.",e.className="slogan-hint slogan-hint--error";return}n.disabled=!0,n.textContent="...",e.textContent="";try{const{error:i}=await f.from("factions").update({party_description:o,last_rename_tick:window._currentTick}).eq("id",window._corpFactionId);if(i)throw i;document.getElementById("id-slogan").textContent='"'+o+'"',e.textContent="Slogan saved! Next change in 120 ticks.",e.className="slogan-hint slogan-hint--ok",n.textContent="Save"}catch(i){console.error("Slogan save failed:",i),e.textContent="Failed to save slogan.",e.className="slogan-hint slogan-hint--error",n.disabled=!1,n.textContent="Save"}}async function ye(){await f.auth.signOut(),window.location.href="login.html"}function be(){const t=document.getElementById("corp-faction-dropdown");t&&t.classList.toggle("open")}function he(t,e){const n=document.getElementById("corp-faction-dropdown");n&&n.classList.remove("open"),sessionStorage.setItem("active_faction_id",t),e==="corporation"?window.location.href="corp-dashboard.html":window.location.href="dashboard.html"}document.addEventListener("click",t=>{const e=document.getElementById("faction-switcher"),n=document.getElementById("corp-faction-dropdown");n&&e&&!e.contains(t.target)&&n.classList.remove("open")});window.doLogout=ye;async function xe(t){const e=t.target.files?.[0];if(!e)return;if(e.size>128*1024){alert("Logo must be under 128KB.");return}const n=window._corpFactionId;if(!n)return;const o=document.getElementById("corp-logo-label");o&&(o.textContent="Uploading...");try{const i=e.name.split(".").pop()||"png",r=`party-logos/${n}/${Date.now()}.${i}`,{error:p}=await f.storage.from("public-assets").upload(r,e,{contentType:e.type,upsert:!0});if(p)throw p;const{data:l}=f.storage.from("public-assets").getPublicUrl(r),a=l?.publicUrl||null;await f.from("factions").update({custom_logo_url:a}).eq("id",n);const s=document.getElementById("id-logo");s&&(s.innerHTML=`<img src="${a}" alt="logo" style="width:100%;height:100%;object-fit:contain;border-radius:4px;">`);const m=document.getElementById("corp-logo");m&&(m.innerHTML=`<img src="${a}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:5px;">`),o&&(o.textContent="Change Logo")}catch(i){console.error("Logo upload failed:",i),alert("Upload failed: "+(i.message||"Unknown error")),o&&(o.textContent="Upload Logo")}}window.saveSlogan=ge;window.toggleCorpDropdown=be;window.switchToFaction=he;let xt=!1;function we(t,e,n,o){if(xt)return;const i=window._nationStats,p=1+(vt(i,"inflation")-50)/100*.3,l=Math.max(.1,o/100),a=Math.round(n*p*l),s=document.getElementById("prop-modal-overlay"),m=document.getElementById("prop-modal-content");m.innerHTML=`
    ...
    `,s.style.display="flex"}async function $e(t,e){if(xt)return;xt=!0;const n=document.getElementById("prop-sell-confirm");n&&(n.disabled=!0,n.textContent="Selling...");try{const o=window._corpFactionId;if(!o)throw new Error("No faction");const{error:i}=await f.from("corp_properties").update({is_active:!1}).eq("id",t).eq("faction_id",o);if(i)throw new Error("Failed to sell property: "+i.message);const{data:r}=await f.from("factions").select("corp_cash_reserves").eq("id",o).single(),p=Number(r?.corp_cash_reserves??0),{error:l}=await f.from("factions").update({corp_cash_reserves:p+e}).eq("id",o);l&&console.error("[Property] Failed to credit cash:",l.message),Ct(),alert("Property sold for "+_(e)+". Cash credited."),location.reload()}catch(o){alert("Sale failed: "+o.message)}finally{xt=!1,n&&(n.disabled=!1,n.textContent="Confirm Sale")}}let wt=!1;function ke(t,e,n,o,i){if(wt)return;const r=window._nationStats,p=window._factionData,a=1+(vt(r,"inflation")-50)/100*.3,s=se({purchase_price:o,refurbish_count:i},a),u=Number(p?.corp_cash_reserves??0)>=s,x=document.getElementById("prop-modal-overlay"),E=document.getElementById("prop-modal-content"),S=u&&n<95;let A="";n>=95?A='<div style="color:var(--green);font-size:9px;margin-bottom:8px;">Property already at excellent condition ('+n+"%).</div>":u||(A='<div style="color:var(--red);font-size:9px;margin-bottom:8px;">Insufficient cash reserves.</div>'),E.innerHTML=`
        ...
        <div style="background:var(--bg-3);border:1px solid var(--border-hair);padding:8px;margin-bottom:10px;">
            <div style="display:flex;justify-content:space-between;font-size:9px;font-family:var(--font-mono);margin-bottom:4px;">
                ...
                <span style="color:${u?"var(--gold, #c8a832)":"var(--red)"};">${_(s)}</span>
            </div>
            <div style="display:flex;justify-content:space-between;font-size:9px;font-family:var(--font-mono);margin-bottom:4px;">
    ...
    `,x.style.display="flex"}async function Ce(t,e,n,o){if(wt)return;wt=!0;const i=document.getElementById("prop-refurb-confirm");i&&(i.disabled=!0,i.textContent="Starting...");try{const r=window._corpFactionId,p=window._currentTick,l=window._factionData,a=window._nationStats;if(!r)throw new Error("No faction");const m=1+(vt(a,"inflation")-50)/100*.3,x=await ie(f,r,{id:t,purchase_price:e,refurbish_count:n,condition:o},p,m);if(!x.ok)throw new Error(x.error||"Refurbishment failed.");l&&(l.corp_cash_reserves=x.newCash),Ct(),alert(`Refurbishment started! Duration: ${x.duration} ticks. Target condition: ${x.targetCondition}%. Cost: ${_(x.cost)}.`),location.reload()}catch(r){alert("Refurbishment failed: "+r.message)}finally{wt=!1,i&&(i.disabled=!1,i.textContent="Begin Refurbishment")}}function Ct(){const t=document.getElementById("prop-modal-overlay");t&&(t.style.display="none")}window.showSellModal=we;window.confirmSellProperty=$e;window.showRefurbishModal=ke;window.confirmRefurbish=Ce;window.closePropModal=Ct;window.showConvertModal=Te;window.confirmConvertProperty=Ie;let qt=!1;async function Ee(t,e,n,o,i,r,p){if(!qt&&confirm("Accept bid from "+n+`?

Bid Price: `+_(o)+`
Quality: `+i+`/100
Workers: `+r+`

This will award the contract. The project begins immediately.`)){qt=!0;try{const{data:l}=await f.from("shard").select("current_tick").eq("name","Alpha Shard").single(),a=l?.current_tick||0,{error:s}=await f.from("contract_bids").update({status:"won"}).eq("id",e);if(s)throw s;const{error:m}=await f.from("contract_bids").update({status:"lost"}).eq("contract_id",t).neq("id",e);if(m)throw m;const{error:u}=await f.from("construction_contracts").update({status:"awarded",awarded_to_faction:p,awarded_at_tick:a}).eq("id",t);if(u)throw u;alert("Contract awarded to "+n+`!

Bid: `+_(o)+`
Project begins immediately.`),window._nationStats&&window._factionData&&Q&&await St(window._nationStats,window._nationStats?.name||"",window._factionData,Q)}catch(l){alert("Failed to accept bid: "+(l.message||l))}finally{qt=!1}}}window.cpAcceptBid=Ee;function Me(t){const e=document.getElementById("cp-bid-"+t);e&&(e.style.display=e.style.display==="none"?"":"none")}window.cpToggleBid=Me;let $t="branch_office";function Te(t,e,n){const o=(d?.corp_subsector||"").toLowerCase(),i=o==="banking"?[["branch_office","Branch Office"]]:o==="investment"?[["trading_floor","Trading Floor"]]:o==="insurance"?[["claims_office","Claims Office"],["insurance_office","Insurance Office"]]:[];if(i.length===0)return;$t=i[0][0];const r=Math.round(n*.15),p=Math.floor(Math.random()*6)+4,l=document.getElementById("prop-modal-overlay"),a=document.getElementById("prop-modal-content");a.innerHTML=`
        ...
        <div style="display:flex;gap:4px;margin-bottom:10px;">
            ${i.map(([s,m])=>`<span onclick="_convertTargetType='${s}';document.querySelectorAll('.conv-opt').forEach(e=>e.style.background='transparent');this.style.background='rgba(138,106,170,0.15)'" class="conv-opt" style="flex:1;text-align:center;padding:6px;font-family:var(--font-mono);font-size:9px;font-weight:700;cursor:pointer;border:1px solid rgba(138,106,170,0.3);color:#8a6aaa;${s===$t?"background:rgba(138,106,170,0.15)":""}">${m}</span>`).join("")}
        </div>
        <div style="display:flex;justify-content:space-between;font-size:9px;font-family:var(--font-mono);margin-bottom:4px;">
            <span style="color:var(--text-dim);">Conversion Cost</span>
            <span style="color:var(--gold);">${_(r)}</span>
        </div>
        <div style="display:flex;justify-content:space-between;font-size:9px;font-family:var(--font-mono);margin-bottom:4px;">
        ...
        <div style="display:flex;gap:6px;justify-content:flex-end;">
            ...
            <button class="prop-action-btn" style="background:rgba(138,106,170,0.12);border-color:rgba(138,106,170,0.3);color:#8a6aaa;" onclick="confirmConvertProperty('${t}',${r},${p})">Convert</button>
        </div>
    `,l.style.display="flex"}async function Ie(t,e,n){const o=Number(d?.corp_cash_reserves??0);if(o<e){alert("Insufficient cash. Need "+_(e)+".");return}const{data:i,error:r}=await f.from("corp_properties").select("role").eq("id",t).single();if(r){alert("Conversion failed: "+r.message);return}if(i?.role==="subsidiary"){alert("Subsidiary HQs cannot be converted.");return}const p=Q?.current_tick||0;try{await f.from("factions").update({corp_cash_reserves:Math.max(0,o-e)}).eq("id",d.id),d.corp_cash_reserves=Math.max(0,o-e),await f.from("corp_properties").update({type:$t,role:$t,refurbish_until_tick:p+n,condition:100}).eq("id",t),Ct();const l=window._nationStats;await St(l,l?.name||d?.nation,d,Q)}catch(l){alert("Conversion failed: "+l.message)}}const At={crisis:{icon:"⚠",color:"#e74c3c",bg:"rgba(231,76,60,0.12)",label:"CRISIS"},government:{icon:"⚖",color:"#f1c40f",bg:"rgba(241,196,15,0.10)",label:"GOV"},executive_order:{icon:"✍",color:"#e67e22",bg:"rgba(230,126,34,0.12)",label:"EXEC ORDER"},political:{icon:"★",color:"#3498db",bg:"rgba(52,152,219,0.12)",label:"POLITICAL"},trade:{icon:"⚓",color:"#2ecc71",bg:"rgba(46,204,113,0.12)",label:"TRADE"},diplomatic:{icon:"🌐",color:"#9b59b6",bg:"rgba(155,89,182,0.12)",label:"DIPLOMATIC"},bill:{icon:"📜",color:"#3498db",bg:"rgba(52,152,219,0.10)",label:"BILL"},new_party:{icon:"🏳",color:"#9b59b6",bg:"rgba(155,89,182,0.10)",label:"NEW PARTY"},protest:{icon:"✊",color:"#e67e22",bg:"rgba(230,126,34,0.10)",label:"PROTEST"},military:{icon:"⚔",color:"#b07a4a",bg:"rgba(176,122,74,0.12)",label:"MILITARY"},economy:{icon:"💰",color:"#7a9a5b",bg:"rgba(122,154,91,0.12)",label:"ECONOMY"},corporate:{icon:"🏢",color:"#c8a64e",bg:"rgba(200,166,78,0.10)",label:"CORP"}},Ot={icon:"•",color:"#888",bg:"rgba(136,136,136,0.08)",label:""},Le={crisis:"alert",protest:"alert",trade:"fin",economy:"fin",corporate:"fin",executive_order:"ops",military:"ops",bill:"pr",government:"pr",political:"pr",diplomatic:"pr",new_party:"pr"},qe={alert:"Alert",fin:"Fin",ops:"Ops",pr:"PR",crew:"Crew"};function Ne(t){return Le[(t||"").toLowerCase()]||"crew"}function lt(t){const e=document.getElementById("h2-wire-dispatches"),n=document.getElementById("h2-wire-live");if(!e)return;const o=Array.isArray(t)?t:[];if(o.length===0){e.innerHTML='<div class="h2-wire-empty">No dispatches</div>',n&&(n.textContent="0 events");return}n&&(n.textContent=`${o.length} event${o.length!==1?"s":""}`);const i=o.slice(0,12);e.innerHTML=i.map(r=>{const p=Ne(r.category),l=r.fired_at_tick!=null?`T${r.fired_at_tick}`:"—",a=r.description_chosen||r.description_used||"",s=Dt(r.event_name),m=s&&a?`<b>${v(s)}</b> — ${v(a)}`:v(s||a||"Event");return`<div class="h2-disp">
            <span class="h2-when">${v(l)}</span>
            <span class="h2-src ${p}">${qe[p]}</span>
            <span class="h2-ln">${m}</span>
        </div>`}).join("")}const Se={1:"Embarrassing Backfire",2:"Protests Don't Materialise",3:"Modest Turnout",4:"Respectable Protest",5:"Strong Demonstration",6:"Nationwide Protests",7:"The Big One"};let yt="nation",bt="local",ct=null;function Dt(t){return t?t.replace(/_/g," ").replace(/\b\w/g,e=>e.toUpperCase()):""}function Bt(t,e){if(!t)return"<em>Unknown</em>";const n=v(t);return e?`<span style="color:${e.replace(/[^a-zA-Z0-9#(),.\s%]/g,"")};font-weight:600">${n}</span>`:`<strong>${n}</strong>`}function Ht(t,e,n){const o=t.factions?.nation_id===(t.nation_id||e),i=t.proposer_name||(o?t.factions?.faction_name:null)||"A former party",r=t.proposer_color||(o?t.factions?.party_color:null);return{fired_at_tick:t.proposed_tick,event_name:t.bill_name,_desc_html:`${Bt(i,r)} proposed "${v(t.bill_name)}"`,category:"bill",_synthetic:!0,...n}}function Ft(t,e){const n=t.leader_first_name&&t.leader_last_name?`${t.leader_first_name} ${t.leader_last_name}`:null,o=n?` led by <strong>${v(n)}</strong>`:"";return{fired_at_tick:0,event_name:t.faction_name,_desc_html:`${Bt(t.faction_name,t.party_color)} founded${o}`,category:"new_party",_synthetic:!0,_created_at:t.created_at,...e}}function Ut(t,e){const n=Se[t.tier]||`Tier ${t.tier}`,o=t.demand_label?` demanding "${v(t.demand_label)}"`:"",i=t.status==="crisis_active",r=t.tier>=6?"#e74c3c":t.tier>=4?"#f39c12":"",p=r?`<span style="color:${r};font-weight:600">${v(n)}</span>`:`<strong>${v(n)}</strong>`;return{fired_at_tick:t.tick_resolved||t.tick_called,event_name:n,_desc_html:`${Bt(t.factions?.faction_name,t.factions?.party_color)} organised a protest${o} — ${p}${i?' <span style="color:#e74c3c;font-weight:700;">(CRISIS)</span>':""}`,category:"protest",_synthetic:!0,...e}}function jt(t,e,n,o,i){return[...t.map(r=>({...r,_synthetic:!1})),...e,...n,...o].sort((r,p)=>{const l=(p.fired_at_tick||0)-(r.fired_at_tick||0);if(l!==0)return l;const a=r._created_at||r.created_at||"",s=p._created_at||p.created_at||"";return s>a?1:s<a?-1:0}).slice(0,i)}function Wt(t){if(t._synthetic&&t._desc_html)return t._desc_html;const e=t.description_chosen||t.description_used||"",n=Dt(t.event_name),o=n?`<strong>${v(n)}</strong>`:"",i=e?v(e):"";return o&&i?`${o} — ${i}`:i||o||"Event"}function Vt(t){return t.map(e=>{const n=kt(e.fired_at_tick),o=At[(e.category||"").toLowerCase()]||Ot;return`<div class="corp-ev-row">
        ...
        </div>`}).join("")}const Pt=[{color:"#6b8cae",bg:"rgba(107,140,174,0.10)",border:"rgba(107,140,174,0.3)"},{color:"#7a9a5b",bg:"rgba(122,154,91,0.10)",border:"rgba(122,154,91,0.3)"},{color:"#b07a4a",bg:"rgba(176,122,74,0.10)",border:"rgba(176,122,74,0.3)"},{color:"#8b7ec8",bg:"rgba(139,126,200,0.10)",border:"rgba(139,126,200,0.3)"},{color:"#5b9a8b",bg:"rgba(91,154,139,0.10)",border:"rgba(91,154,139,0.3)"},{color:"#a65d5d",bg:"rgba(166,93,93,0.10)",border:"rgba(166,93,93,0.3)"}];function Be(t){let e=0;for(let n=0;n<t.length;n++)e=(e<<5)-e+t.charCodeAt(n)|0;return Pt[Math.abs(e)%Pt.length]}function Gt(t){return t.map(e=>{const n=kt(e.fired_at_tick),o=At[(e.category||"").toLowerCase()]||Ot,i=e.nations?.name||"Unknown",r=e.nations?.nation_profiles,p=Array.isArray(r)?r[0]?.flag_url:r?.flag_url,l=Be(i),a=p?`<img src="${v(p)}" alt="">`:"";return`<div class="corp-ev-row">
            <span class="corp-ev-date-col">
                <span class="corp-ev-date">${v(n)}</span>
                <span class="corp-ev-nation-badge" style="color:${l.color};background:${l.bg};border-color:${l.border};">${a}${v(i)}</span>
            </span>
            <span class="corp-ev-text">${Wt(e)}</span>
            ${o.label?`<span class="corp-ev-cat" style="color:${o.color};background:${o.bg}">${o.label}</span>`:""}
        </div>`}).join("")}async function Re(){const t=document.getElementById("corp-events-list");if(!t||!ct)return;const{nationId:e}=ct;if(!e){t.innerHTML='<div class="subs-card__empty">No nation linked.</div>';return}t.innerHTML='<div style="color:var(--text-secondary);padding:16px;font-size:0.7rem;text-align:center;">Loading…</div>';try{const[n,o]=await Promise.all([f.from("factions").select("faction_name, abbreviation, corp_sector, corp_subsector, corp_ticker, leader_first_name, leader_last_name, nation_id, nation, created_at, founded_tick").eq("faction_type","corporation").eq("nation_id",e).is("abandoned_at",null).order("created_at",{ascending:!1}).limit(30),f.from("event_log").select("*").eq("nation_id",e).eq("category","corporate").order("fired_at_tick",{ascending:!1}).limit(30)]),i=n.data||[],r=o.data||[],p=i.map(a=>({event_name:"Corporation Founded",category:"corporate",description_chosen:`${a.faction_name} [${a.corp_ticker||a.abbreviation||"??"}] was founded with a specialty in ${a.corp_subsector||a.corp_sector||"General"}. Led by CEO ${[a.leader_first_name,a.leader_last_name].filter(Boolean).join(" ")||"Unknown"}.`,fired_at_tick:a.founded_tick||0})),l=[...r,...p].sort((a,s)=>(s.fired_at_tick||0)-(a.fired_at_tick||0)).slice(0,40);if(l.length===0){t.innerHTML='<div class="subs-card__empty" style="padding:40px 0;">No corporate events in this nation yet.</div>',lt([]);return}t.innerHTML=Vt(l),lt(l)}catch(n){console.error("Corp local events error:",n),t.innerHTML='<div class="subs-card__empty">Failed to load events.</div>'}}async function Pe(){const t=document.getElementById("corp-events-list");if(!t||!ct)return;const{nationId:e}=ct;if(!e){t.innerHTML='<div class="subs-card__empty">No nation linked.</div>';return}t.innerHTML='<div style="color:var(--text-secondary);padding:16px;font-size:0.7rem;text-align:center;">Loading…</div>';try{const[n,o]=await Promise.all([f.from("factions").select("faction_name, abbreviation, corp_sector, corp_subsector, corp_ticker, leader_first_name, leader_last_name, nation_id, nation, created_at, founded_tick").eq("faction_type","corporation").neq("nation_id",e).is("abandoned_at",null).order("created_at",{ascending:!1}).limit(30),f.from("event_log").select("*, nations(name, nation_profiles(flag_url))").neq("nation_id",e).eq("category","corporate").order("fired_at_tick",{ascending:!1}).limit(30)]),i=n.data||[],r=o.data||[],p=i.map(a=>({event_name:"Corporation Founded",category:"corporate",description_chosen:`${a.faction_name} [${a.corp_ticker||a.abbreviation||"??"}] was founded in ${a.nation||"Unknown"} with a specialty in ${a.corp_subsector||a.corp_sector||"General"}. Led by CEO ${[a.leader_first_name,a.leader_last_name].filter(Boolean).join(" ")||"Unknown"}.`,fired_at_tick:a.founded_tick||0,nations:{name:a.nation||"Unknown"}})),l=[...r,...p].sort((a,s)=>(s.fired_at_tick||0)-(a.fired_at_tick||0)).slice(0,40);if(l.length===0){t.innerHTML='<div class="subs-card__empty" style="padding:40px 0;">No corporate events from other nations yet.</div>',lt([]);return}t.innerHTML=Gt(l),lt(l);return}catch(n){console.error("Corp world events error:",n),t.innerHTML='<div class="subs-card__empty">Failed to load events.</div>';return}try{const{data:n,error:o}=await f.from("factions").select("faction_name, abbreviation, corp_sector, corp_subsector, corp_ticker, leader_first_name, leader_last_name, nation_id, nation, created_at, founded_tick").eq("faction_type","corporation").neq("nation_id",e).is("abandoned_at",null).order("created_at",{ascending:!1}).limit(30);if(o)throw o;if(!n||n.length===0){t.innerHTML='<div class="subs-card__empty" style="padding:40px 0;">No corporate events from other nations yet.</div>',lt([]);return}t.innerHTML=ze(n,!0)}catch(n){console.error("Corp world events error:",n),t.innerHTML='<div class="subs-card__empty">Failed to load events.</div>'}}function ze(t,e){return t.map(n=>{const o=[n.leader_first_name,n.leader_last_name].filter(Boolean).join(" ")||"Unknown",i=n.nation||"Unknown",r=n.corp_subsector||n.corp_sector||"General",p=n.corp_ticker||n.abbreviation||"",l=n.founded_tick?kt(n.founded_tick):"";let a='<div class="corp-event-row">';return a+='<div style="font-family:var(--font-mono);font-size:7px;font-weight:700;color:var(--teal);background:rgba(90,175,165,0.08);border:1px solid rgba(90,175,165,0.15);padding:1px 5px;display:inline-block;margin-bottom:2px;">'+v(i.toUpperCase())+"</div>",a+='<div style="font-size:11px;color:var(--text-primary);line-height:1.5;">',a+='<span style="font-weight:600;">'+v(n.faction_name)+"</span>",p&&(a+=' <span style="font-family:var(--font-mono);font-size:0.65rem;color:var(--text-dim);">['+v(p)+"]</span>"),a+=' was founded in <span style="font-weight:500;">'+v(i)+"</span>",a+=' with a specialty in <span style="color:var(--teal);font-weight:500;">'+v(r)+"</span>.",a+=' Led by CEO <span style="font-weight:500;">'+v(o)+"</span>.",a+="</div>",l&&(a+='<div style="font-family:var(--font-mono);font-size:8px;color:var(--text-dim);margin-top:1px;">'+v(l)+"</div>"),a+="</div>",a}).join("")}async function Kt(){const t=document.getElementById("corp-events-list");if(!t||!ct)return;const{nationId:e}=ct;if(!e){t.innerHTML='<div class="subs-card__empty">No nation linked.</div>';return}t.innerHTML='<div style="color:var(--text-secondary);padding:16px;font-size:0.7rem;text-align:center;">Loading…</div>';try{const[n,o,i,r]=await Promise.all([f.from("event_log").select("*").eq("nation_id",e).order("fired_at_tick",{ascending:!1}).limit(50),f.from("bills").select("bill_name, proposed_tick, proposed_by, bill_type, proposer_name, proposer_color, factions!proposed_by(faction_name, party_color, nation_id)").eq("nation_id",e).not("bill_type","in",'("no_confidence","confirmation","minister_confirmation","veto_override","impeachment_conviction")').order("proposed_tick",{ascending:!1}).limit(30),f.from("factions").select("faction_name, party_color, leader_first_name, leader_last_name, created_at").eq("nation_id",e).order("created_at",{ascending:!1}).limit(20),f.from("protest_log").select("id, tick_called, tick_resolved, tier, status, demand_label, faction_id, factions(faction_name, party_color)").eq("nation_id",e).in("status",["resolved","crisis_active"]).order("tick_called",{ascending:!1}).limit(20)]);if(n.error)throw n.error;const p=n.data||[],l=jt(p,(o.data||[]).map(a=>Ht(a,e)),(i.data||[]).map(a=>Ft(a)),(r.data||[]).map(a=>Ut(a)),60);if(l.length===0){t.innerHTML='<div class="subs-card__empty" style="padding:40px 0;">No nation events recorded yet.</div>',lt([]);return}t.innerHTML=Vt(l),lt(l)}catch(n){console.error("Nation events error:",n),t.innerHTML='<div class="subs-card__empty">Failed to load events.</div>'}}async function Ae(){const t=document.getElementById("corp-events-list");if(!t||!ct)return;const{nationId:e}=ct;if(!e){t.innerHTML='<div class="subs-card__empty">No nation linked.</div>';return}t.innerHTML='<div style="color:var(--text-secondary);padding:16px;font-size:0.7rem;text-align:center;">Loading world events…</div>';try{const[n,o,i,r]=await Promise.all([f.from("event_log").select("*, nations!inner(name, nation_profiles(flag_url))").neq("nation_id",e).order("fired_at_tick",{ascending:!1}).limit(60),f.from("bills").select("bill_name, proposed_tick, nation_id, proposed_by, bill_type, proposer_name, proposer_color, factions!proposed_by(faction_name, party_color, nation_id), nations!inner(name, nation_profiles(flag_url))").neq("nation_id",e).not("bill_type","in",'("no_confidence","confirmation","minister_confirmation","veto_override","impeachment_conviction")').order("proposed_tick",{ascending:!1}).limit(20),f.from("factions").select("faction_name, party_color, leader_first_name, leader_last_name, nation_id, created_at, nations!inner(name, nation_profiles(flag_url))").neq("nation_id",e).order("created_at",{ascending:!1}).limit(15),f.from("protest_log").select("id, tick_called, tick_resolved, tier, status, demand_label, faction_id, nation_id, factions(faction_name, party_color), nations!inner(name, nation_profiles(flag_url))").neq("nation_id",e).in("status",["resolved","crisis_active"]).order("tick_called",{ascending:!1}).limit(15)]);if(n.error)throw n.error;const p=n.data||[],l=jt(p,(o.data||[]).map(a=>Ht(a,null,{nations:a.nations})),(i.data||[]).map(a=>Ft(a,{nations:a.nations})),(r.data||[]).map(a=>Ut(a,{nations:a.nations})),60);if(l.length===0){t.innerHTML='<div class="subs-card__empty" style="padding:40px 0;">No world events recorded yet.</div>',lt([]);return}t.innerHTML=Gt(l),lt(l)}catch(n){console.error("World events error:",n),t.innerHTML='<div class="subs-card__empty">Failed to load events.</div>'}}window.switchCorpEventsCat=function(t){t!==yt&&(yt=t,document.querySelectorAll("#corp-events-cat-bar .corp-events-tab").forEach(e=>e.classList.toggle("active",e.dataset.cat===t)),Yt())};window.switchCorpEventsScope=function(t){t!==bt&&(bt=t,document.querySelectorAll("#corp-events-scope-bar .corp-events-tab").forEach(e=>e.classList.toggle("active",e.dataset.scope===t)),Yt())};function Yt(){yt==="nation"&&bt==="local"?Kt():yt==="nation"&&bt==="world"?Ae():yt==="corporate"&&bt==="local"?Re():Pe()}le();
