import{_ as re}from"./supabase-client-BXEzLDpS.js";/* empty css                  */import{_ as oe,i as ce}from"./common-DeIzzava.js";import{t as x}from"./utils-C2W-HleY.js";let b=null,w=null,H=[],B="all",T=null,P=null,q=null,A=null,D=!1,y="cruceran",V=!1;const Y={cruceran:{key:"cruceran",name:"The Cruceran",tagline:"Truth in the service of the people",nations:["Avelia","Palvera","San Estrella","Montequilla","Melizea","Sangreza"],style:"cruceran"},continental:{key:"continental",name:"The Continental",tagline:"Where Ideas Converge",nations:["Calveth","Flandis","Vostia"],style:"continental"}};function de(e){for(const[s,n]of Object.entries(Y))if(n.nations.some(a=>a.toLowerCase()===(e||"").toLowerCase()))return s;return"cruceran"}function Q(e,s){const n=Y[e];return n?n.nations.some(a=>a.toLowerCase()===(s||"").toLowerCase()):!1}const pe=["Winter","Spring","Spring","Spring","Summer","Summer","Summer","Fall","Fall","Fall","Winter","Winter"];function S(e){const s=e%12,n=2e3+Math.floor(e/12),a=pe[s],l=s===0?n-1:n;return`${a} ${l}`}async function F(e,s){b=e,w=s,T=null,B="all",q=null;const n=document.getElementById("newspaper-root");if(!n)return;V||(y=de(s.nation?.name));const a=s.shard?.current_date||"[Month], [Year]",t=Q(y,s.nation?.name)?"Write Article":"Write Article (1 AP)",v=Object.entries(Y).map(([p,h])=>`<option value="${p}" ${p===y?"selected":""}>${h.name}</option>`).join("");n.innerHTML=`<div class="newspaper-container nws-pub-${y}">
    ...
    </div>`;const d=q,u=d?()=>le(d.root,d.articles,d.dateLabel):()=>F(b,w);document.getElementById("nws-back-btn")?.addEventListener("click",u),document.getElementById("nws-back-btn-bottom")?.addEventListener("click",u),e.scrollTop=0}function r(e){const s=document.createElement("div");return s.textContent=e,s.innerHTML}function se(e,s=1200){if(e.length<=s)return ne(e);let n=e.lastIndexOf(" ",s);n<s*.5&&(n=s);const l=e.substring(0,n).split(/\n\n+/).filter(t=>t.trim());return l.map((t,v)=>`<p class="${v===0?"nws-drop-cap":""}">${W(r(t.trim()))}${v===l.length-1?"...":""}</p>`).join("")+'<p class="nws-read-more">Read More &rarr;</p>'}function W(e){return e.replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>").replace(/\*(.+?)\*/g,"<em>$1</em>").replace(/__(.+?)__/g,"<u>$1</u>")}function ne(e){const s=e.split(/\n\n+/).filter(n=>n.trim());return s.length<=1?`<p class="nws-drop-cap">${W(r(e))}</p>`:s.map((n,a)=>`<p class="${a===0?"nws-drop-cap":""}">${W(r(n.trim()))}</p>`).join("")}function $(e){return e==="elections"&&(e="politics"),{politics:"Politics",economy:"Business",international:"International",social:"Social",entertainment:"Entertainment",sports:"Sports",opinion:"Opinion"}[e]||e}function te(e){return e==="elections"&&(e="politics"),{politics:"linear-gradient(135deg,#2a1a2a,#1a0d1a)",economy:"linear-gradient(135deg,#2a1a0a,#1a0d00)",international:"linear-gradient(135deg,#0a1a2a,#001a2a)",social:"linear-gradient(135deg,#1a2a1a,#0d1a0d)",entertainment:"linear-gradient(135deg,#2a2a0a,#1a1a00)",sports:"linear-gradient(135deg,#2a0a0a,#1a0000)",opinion:"linear-gradient(135deg,#2a2a2a,#1a1a1a)"}[e]||"linear-gradient(135deg,#1a1a1a,#0d0d0d)"}async function ae(){if(T)return T;const e=w?.shard,s=w?.nation;if(!e||!s)return[s?.id].filter(Boolean);const{data:n,error:a}=await b.from("nations").select("id").eq("shard_id",e.id);return a&&console.error("[News] Failed to fetch shard nations:",a),T=n&&n.length>0?n.map(l=>l.id):[s.id],T}async function R(){if(!(!b||!w))try{const e=await ae();let s=b.from("player_articles").select("*").in("nation_id",e).eq("status","published").order("created_at",{ascending:!1});y!=="cruceran"?s=s.or(`publication.eq.${y},publication.eq.international`):s=s.or("publication.eq.cruceran,publication.is.null,publication.eq.international");const{data:n,error:a}=await s;if(a){console.error("[News] Failed to load articles:",a);return}if(!n||n.length===0)return;const l=w.shard?.current_tick??0,i=S(l),t=n.filter(m=>S(m.published_tick??0)===i),v=n.filter(m=>S(m.published_tick??0)!==i),d=new Set(t.map(m=>m.category)),u=[],p=new Set;for(const m of v)m.category!=="opinion"&&(d.has(m.category)||(p.has(m.category)||p.add(m.category),u.push(m)));const h=n.filter(m=>m.category==="opinion").slice(0,4),c=[...t.filter(m=>m.category!=="opinion"),...u,...h];if(c.length===0)return;H=c;const o=B&&B!=="all"?c.filter(m=>(m.category==="elections"?"politics":m.category)===B):c;if(o.length===0&&B!=="all"){const m=`<p class="nws-placeholder" style="text-align:center;padding:40px;grid-column:1/-1;">No ${$(B)} articles in this edition.</p>`,k=document.getElementById("nws-lead-section");k&&(k.innerHTML=m);const O=document.getElementById("nws-secondary-grid");O&&(O.innerHTML="");const z=document.querySelector(".nws-opinion-grid");z&&(z.innerHTML="");const U=document.querySelector(".nws-bottom-left");U&&(U.innerHTML="");return}const f=[...o.filter(m=>m.category!=="opinion")].sort((m,k)=>(k.published_tick??0)-(m.published_tick??0)),_=f[0],E=f.slice(1,4),N=f.slice(4,7),C=[...c].sort((m,k)=>new Date(k.created_at)-new Date(m.created_at)).slice(0,5);y==="continental"?_e(_,E,N,h.slice(0,4),C):(_&&ge(_,E),be(N),ye(h.slice(0,4)),fe(C))}catch(e){console.error("[News] Error loading articles:",e)}}function ge(e,s){const n=document.getElementById("nws-lead-section");if(!n||!e)return;const a=s.length>0?s.map(d=>`
    ...
    `}function be(e){const s=document.getElementById("nws-secondary-grid");if(!s||e.length===0)return;const n=[...e],a=["Crisis","Election","Business"],l=[0,1,2].map(i=>{const t=n[i];if(t){const v=t.image_url?`<img src="${r(t.image_url)}" alt="${r(t.headline)}" style="width:100%;height:100%;object-fit:cover;">`:`<div class="nws-img-ph" style="background:${te(t.category)};">${r($(t.category))}</div>`;return`<div class="nws-sec-story" data-article-id="${t.id}">
        ...
        <div class="nws-brief-row" data-article-id="${t.id}">
            ...
            <div class="nws-brief-text">
                <strong>${r(t.headline)}${M(t)}${L(t)}</strong>
    ...
    `).join(""),l=5-e.length;let i="";for(let t=0;t<l;t++)i+=`
        ...
        </div>`,c+='<div class="ct-story-grid">';for(const o of s){const g=h[o.category]||"",f=o.image_url?`<img src="${t(o.image_url)}" alt="" style="width:100%;height:100%;object-fit:cover;">`:"";c+=`<div class="ct-card" data-article-id="${o.id}">
    ...
    </div>`,i.innerHTML=c,i.querySelectorAll("[data-article-id]").forEach(o=>{o.style.cursor="pointer",o.addEventListener("click",g=>{if(g.target.closest(".nws-edit-btn, .nws-delete-btn"))return;const f=o.dataset.articleId,_=H.find(E=>String(E.id)===String(f));_&&ee(document.getElementById("newspaper-root"),_)})})}function $e(e){const s=e.querySelectorAll(".nws-nav-item[data-category]");s.forEach(n=>{n.addEventListener("click",()=>{s.forEach(a=>a.classList.remove("active")),n.classList.add("active"),B=n.dataset.category,R()})})}function ke(e){const s=document.getElementById("nws-nav-archives");s&&s.addEventListener("click",()=>ie(e))}async function ie(e){if(!(!b||!w))try{const s=w.shard?.current_tick??0,n=w.shard?.current_date||"[Month], [Year]",a=await ae(),{data:l,error:i}=await b.from("player_articles").select("*").in("nation_id",a).eq("status","published").order("published_tick",{ascending:!1});if(i){console.error("[News] Failed to load archive articles:",i);return}const t={};for(const p of l||[]){const h=p.published_tick??0,c=S(h);t[c]||(t[c]={label:c,maxTick:h,articles:[]}),t[c].articles.push(p),h>t[c].maxTick&&(t[c].maxTick=h)}const v=Object.values(t).sort((p,h)=>h.maxTick-p.maxTick),d=S(s),u=v.length>0?v.map(p=>{const h=p.label===d?' <span class="nws-archive-current">Current</span>':"";return`<div class="nws-archive-month" data-archive-season="${r(p.label)}">
        ...
        </div>`,document.getElementById("nws-back-btn")?.addEventListener("click",()=>F(b,w)),e.querySelectorAll("[data-archive-season]").forEach(p=>{p.addEventListener("click",()=>{const h=p.dataset.archiveSeason,c=t[h];c&&le(e,c.articles,c.label)})}),e.scrollTop=0}catch(s){console.error("[News] Error loading archives:",s)}}function le(e,s,n){H=s;const a=s.filter(f=>f.category==="opinion"),i=[...s.filter(f=>f.category!=="opinion")].sort((f,_)=>(_.body||"").length-(f.body||"").length),t=i[0],v=i.slice(1,4),d=i.slice(4,7),u=i.slice(7,12),p=a.slice(0,4),h=t?Ee(t,v,n):'<p class="nws-placeholder" style="padding:40px;text-align:center;">[No articles in this edition]</p>',c=Ie(d),o=Ce(p),g=Be(u);e.innerHTML=`<div class="newspaper-container">
        <!-- TOP RIBBON -->
        <div class="nws-top-ribbon">
            <div class="nws-top-ribbon-inner">
                <span>${r(n)}</span>
    ...
    `).join("")}function G(e){return e.w*3+e.d}function Te(e){return Object.values(e).sort((s,n)=>{const a=G(n)-G(s);return a!==0?a:n.w-s.w})}function Ae(e){return`<span class="vln-form-dot" style="background:${{W:"#1a4a1a",D:"#8a6a20",L:"#8b1a1a"}[e]||"#999"}"></span>`}function Se(e,s){return e<=3?"vln-pos-top":e>=s-2?"vln-pos-bottom":""}function Z(e,s={}){const n=Te(e),a=s.muted||!1,l=n.length;let i=`<div class="vln-table${a?" vln-table-muted":""}">
        <div class="vln-thead">
        ...
        </div>`;return n.forEach((t,v)=>{const d=v+1,u=Se(d,l),p=d===3||d===7?" vln-zone-break":"",h=G(t),c=a?"":t.form.map(o=>Ae(o)).join("");i+=`<div class="vln-row${p}">
