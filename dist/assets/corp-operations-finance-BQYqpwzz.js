import{_ as U}from"./supabase-client-BXEzLDpS.js";import{t as Ft,e as b}from"./utils-C2W-HleY.js";let J=[],g=null,z=null;function n(a){return Math.abs(a)>=1e6?"$"+(a/1e6).toFixed(1)+"M":Math.abs(a)>=1e3?"$"+(a/1e3).toFixed(0)+"k":"$"+Math.round(a).toLocaleString()}const gt={LOAN:{class:"loan",label:"LOAN"},INSURE:{class:"insure",label:"INSURE"},BOND:{class:"bond",label:"BOND"}},Vt={LOW:{class:"df-risk-low",label:"LOW RISK"},MODERATE:{class:"df-risk-moderate",label:"MODERATE"},ELEVATED:{class:"df-risk-elevated",label:"ELEVATED"},HIGH:{class:"df-risk-high",label:"HIGH RISK"}};let D=[],lt="ALL",W=-1;async function It(){if(!g||!z)return;const{data:a}=await U.from("finance_loan_requests").select("*, requesting_faction:factions!requesting_faction_id(id, faction_name, abbreviation, corp_ticker, corp_subsector, nation_id)").eq("status","open").order("created_tick",{ascending:!1}),{data:t}=await U.from("finance_loan_offers").select("request_id").eq("offering_faction_id",g.id),p=new Set((t||[]).map(d=>d.request_id));D=(a||[]).map(d=>({id:d.id,type:"LOAN",applicant:d.requesting_faction?.faction_name||"Unknown",abbr:d.requesting_faction?.abbreviation||d.requesting_faction?.corp_ticker||"??",entity:"CORP",nation:"N/A",amount:d.amount,term:d.term_months,purpose:d.purpose||"",reputation:50,creditRating:50,risk:"MODERATE",isNew:!p.has(d.id),ticksLeft:(d.expires_tick||0)-(z?.current_tick||0),requestId:d.id,alreadyOffered:p.has(d.id)})),bt()}function Lt(a){if(!g)return!1;const t=(g.corp_subsector||"").toLowerCase(),p=Wt[t];return a.type===p}function qt(a){lt=a,W=-1,bt()}function Ht(a){W=W===a?-1:a,bt()}function bt(){const a=document.getElementById("df-container");if(!a)return;const t=lt==="ALL"?D:D.filter(o=>o.type===lt),p=D.filter(o=>o.isNew).length,d=D.length;let r=`<div class="df-header">
        ...
        <div style="display:flex;gap:4px;align-items:center;">
    ...
    </div>`;const l=[{id:"ALL",label:"All",activeClass:"active-all"},{id:"LOAN",label:"Loans",activeClass:"active-loan"},{id:"INSURE",label:"Insurance",activeClass:"active-insure"},{id:"BOND",label:"Bonds",activeClass:"active-bond"}];r+='<div class="df-filters">';for(const o of l)r+=`<span class="df-pill${lt===o.id?" "+o.activeClass:""}" onclick="dfSetFilter('${o.id}')">${o.label}</span>`;r+="</div>",r+='<div class="df-list">',t.length===0&&(r+='<div class="ap-empty">No deals available.<br>Deals appear when corporations request financing or governments issue bonds.</div>');for(let o=0;o<t.length;o++){const s=t[o],y=D.indexOf(s),E=W===y,A=gt[s.type],w=Vt[s.risk],R=Lt(s);r+=`<div class="df-deal${E?" sel-"+A.class:""}" onclick="dfSelectDeal(${y})" style="${R?"":"opacity:0.5;"}">`,s.isNew&&R&&(r+='<div class="df-new-dot"></div>'),r+=`<div style="display:flex;align-items:center;gap:6px;margin-bottom:3px;">
            ...
            <span class="df-badge ${w.class}" style="font-weight:700;line-height:12px;background:currentColor;-webkit-background-clip:unset;padding:0 4px;">`,r+="</span>",r=r.slice(0,r.lastIndexOf('<span class="df-badge '+w.class));const O=w.class==="df-risk-low"?"#5c5":w.class==="df-risk-moderate"?"#ca5":w.class==="df-risk-elevated"?"#c84":"#c55";r+=`<span style="font-family:var(--font-mono);font-size:7px;font-weight:700;padding:0 4px;line-height:12px;color:${O};background:${O}12;border:1px solid ${O}25;">${w.label}</span>
        </div>`;const P=s.type==="BOND"?"FACE VALUE":s.type==="INSURE"?"COVERAGE":"AMOUNT",T=s.type==="BOND"?"COUPON":"REP",B=s.type==="BOND"?s.couponRate+"%":s.reputation||s.stability,m=s.type==="BOND"?s.couponRate*10:s.reputation||s.stability,c=s.type==="BOND"?"#c8a832":m>=60?"#5c5":m>=35?"#ca5":"#c84";if(r+=`<div class="df-metrics">
            <div style="flex:1;">
            ...
            <div style="flex:0.8;text-align:center;">
                ...
                <div class="df-metrics__value" style="font-size:11px;color:${c};">${B}</div>
            </div>
        </div>`,E){if(r+=`<div style="margin-top:6px;font-size:10px;color:#9e9a92;line-height:1.5;margin-bottom:6px;">${b(s.purpose)}</div>`,R)r+='<div class="df-detail">';else{const f=s.type==="LOAN"?"Banking":s.type==="INSURE"?"Insurance":"Investment";r+=`<div style="padding:6px 8px;background:rgba(106,102,96,0.06);border:1px solid #2a2a24;font-family:var(--font-mono);font-size:8px;color:#6a6660;line-height:1.5;">
                ...
                </div>`}if(R){if(s.type==="LOAN"){const f=Math.round(s.debt/s.revenue*100),x=f>50?"#c84":"#5c5",N=s.debt>s.revenue*.5?"#c84":"#9e9a92";r+=`<div class="df-detail-row"><span class="df-detail-label">ANNUAL REVENUE</span><span class="df-detail-value" style="color:#9e9a92;">${n(s.revenue)}</span></div>`,r+=`<div class="df-detail-row"><span class="df-detail-label">EXISTING DEBT</span><span class="df-detail-value" style="color:${N};">${n(s.debt)}</span></div>`,r+=`<div class="df-detail-row"><span class="df-detail-label">DEBT/REVENUE</span><span class="df-detail-value" style="color:${x};font-weight:700;">${f}%</span></div>`}else if(s.type==="BOND"){const f=s.stability>=50?"#5c5":s.stability>=30?"#ca5":"#c84",x=s.debtToGdp>60?"#c55":s.debtToGdp>40?"#c84":"#5c5",N=s.creditRating>=60?"#5c5":s.creditRating>=35?"#ca5":"#c55";r+=`<div class="df-detail-row"><span class="df-detail-label">STABILITY</span><span class="df-detail-value" style="color:${f};">${s.stability}/100</span></div>`,r+=`<div class="df-detail-row"><span class="df-detail-label">DEBT/GDP</span><span class="df-detail-value" style="color:${x};">${s.debtToGdp}%</span></div>`,r+=`<div class="df-detail-row"><span class="df-detail-label">CREDIT RATING</span><span class="df-detail-value" style="color:${N};font-weight:700;">${s.creditRating}/100</span></div>`}else if(s.type==="INSURE"){const f=s.reputation>=60?"#5c5":s.reputation>=35?"#ca5":"#c84",x=s.projectValue?"PROJECT VALUE":"FLEET VALUE",N=s.projectValue||s.fleetValue;r+=`<div class="df-detail-row"><span class="df-detail-label">REPUTATION</span><span class="df-detail-value" style="color:${f};">${s.reputation}/100</span></div>`,r+=`<div class="df-detail-row"><span class="df-detail-label">${x}</span><span class="df-detail-value" style="color:#9e9a92;">${n(N)}</span></div>`}r+="</div>"}}r+="</div>"}r+="</div>";const v=D.filter(o=>o.type==="LOAN").length,e=D.filter(o=>o.type==="INSURE").length,u=D.filter(o=>o.type==="BOND").length;r+=`<div class="df-footer">
        ...
        ${(()=>{const o=W>=0?D[W]:null,s=o&&Lt(o);return s?`<div class="df-review-btn active" onclick="rdOpen(${W})">REVIEW DEAL</div>`:o&&!s?'<div class="df-review-btn" style="opacity:0.4;cursor:not-allowed;" title="Requires matching subsector">&#128274; LOCKED</div>':'<div class="df-review-btn">REVIEW DEAL</div>'})()}
    </div>`,a.innerHTML=r}window.dfSetFilter=qt;window.dfSelectDeal=Ht;const Rt={CURRENT:{color:"#5c5",label:"CURRENT"},LATE:{color:"#c84",label:"LATE"},DELINQUENT:{color:"#c55",label:"DELINQUENT"},CLAIM:{color:"#c55",label:"CLAIM FILED"},ACTIVE:{color:"#5c5",label:"ACTIVE"},WATCHLIST:{color:"#ca5",label:"WATCHLIST"},DEFAULT:{color:"#c55",label:"DEFAULT"}};let L=[],it="ALL",rt=-1;function jt(a){it=a,rt=-1,ct()}function Yt(a){rt=rt===a?-1:a,ct()}function ct(){const a=document.getElementById("ap-container");if(!a)return;const t=it==="ALL"?L:L.filter(v=>v.type===it),p=L.reduce((v,e)=>v+(e.remaining||e.coverage||e.faceValue||0),0),d=L.reduce((v,e)=>v+(e.earned||e.premiumsCollected||e.couponsReceived||0),0),r=L.filter(v=>v.alert).length;let l=`<div class="df-header">
        <div class="df-header__left">
            <span class="df-header__dot">&#9679;</span>
        ...
        <div class="ap-summary__cell" style="flex:0.6;">
            ...
            <span style="font-size:11px;font-weight:600;color:#e8e4dc;flex:1;">${b(e.counterparty)}</span>
        ...
        </div>`;const P=E?y.color:s.class==="loan"?"#5a8aaa":s.class==="insure"?"#aa7a5a":"#8a6aaa";l+=`<div class="rd-bar"><div class="rd-bar__fill" style="width:${Math.min(R,100)}%;background:${P};"></div></div>`;const T=e.type==="LOAN"?"REMAINING":e.type==="INSURE"?"COVERAGE":"FACE VALUE",B=e.remaining||e.coverage||e.faceValue||0,m=e.type==="LOAN"?"RATE":e.type==="INSURE"?"PREMIUM":"COUPON",c=e.rate||e.premiumRate||e.coupon||0,f=e.earned||e.premiumsCollected||e.couponsReceived||0,x=s.class==="loan"?"#5a8aaa":s.class==="insure"?"#aa7a5a":"#8a6aaa";if(l+=`<div class="df-metrics" style="margin-top:4px;">
            ...
            <div style="flex:0.8;padding:3px 6px;text-align:center;border-right:1px solid #2a2a24;">
                ...
                <div style="font-family:var(--font-mono);font-size:10px;font-weight:700;color:${x};margin-top:1px;">${c}%</div>
            </div>
        ...
        </div>`,E&&(l+=`<div class="ap-deal__alert" style="background:${y.color}08;border:1px solid ${y.color}20;color:${y.color};">${b(e.alert)}</div>`),o){if(l+='<div class="ap-deal__expanded">',e.type==="LOAN"){const N=[{label:"PRINCIPAL",value:n(e.principal||0)},{label:"REMAINING",value:n(e.remaining||0),color:"#e8e4dc"},{label:"MONTHLY PAYMENT",value:n(e.monthlyPayment||0)},{label:"MISSED PAYMENTS",value:String(e.missedPayments||0),color:(e.missedPayments||0)>0?"#c55":"#5c5"},{label:"NEXT DUE",value:e.nextPayment||"—",color:e.status==="LATE"?"#c55":"#9e9a92"}];for(const h of N)l+=`<div class="ap-detail-row"><span class="ap-detail-label">${h.label}</span><span class="ap-detail-value" style="color:${h.color||"#9e9a92"};">${h.value}</span></div>`;e.status!=="CURRENT"&&(l+='<div class="ap-actions"><div class="ap-action-btn green">RESTRUCTURE</div><div class="ap-action-btn orange">CALL LOAN</div><div class="ap-action-btn red">FORECLOSE</div></div>')}else if(e.type==="INSURE"){const N=[{label:"COVERAGE",value:n(e.coverage||0)},{label:"PREMIUMS COLLECTED",value:n(e.premiumsCollected||0),color:"#5c5"},{label:"CLAIMS FILED",value:String(e.claims||0),color:(e.claims||0)>0?"#c84":"#5c5"},{label:"PAID OUT",value:n(e.paidOut||0),color:(e.paidOut||0)>0?"#c55":"#6a6660"}];for(const h of N)l+=`<div class="ap-detail-row"><span class="ap-detail-label">${h.label}</span><span class="ap-detail-value" style="color:${h.color||"#9e9a92"};">${h.value}</span></div>`;e.status==="CLAIM"&&e.claimAmount&&(l+=`<div class="ap-claim-box"><div style="font-family:var(--font-mono);font-size:7px;color:#c55;letter-spacing:0.8px;margin-bottom:2px;">PENDING CLAIM</div><div style="font-family:var(--font-mono);font-size:11px;font-weight:700;color:#c55;">${n(e.claimAmount)}</div></div>`,l+='<div class="ap-actions"><div class="ap-action-btn green">PAY IN FULL</div><div class="ap-action-btn orange">NEGOTIATE</div><div class="ap-action-btn red">DISPUTE</div></div>')}else if(e.type==="BOND"){const N=[{label:"FACE VALUE",value:n(e.faceValue||0)},{label:"COUPONS RECEIVED",value:n(e.couponsReceived||0),color:"#5c5"},{label:"NEXT COUPON",value:e.nextCoupon||"—"},{label:"ANNUAL YIELD",value:n(Math.round((e.faceValue||0)*(e.coupon||0)/100)),color:"#8a6aaa"}];for(const h of N)l+=`<div class="ap-detail-row"><span class="ap-detail-label">${h.label}</span><span class="ap-detail-value" style="color:${h.color||"#9e9a92"};">${h.value}</span></div>`;l+='<div class="ap-actions"><div class="ap-action-btn purple">SELL POSITION</div><div class="ap-action-btn olive">HOLD</div></div>'}l+="</div>"}l+="</div></div>"}l+="</div>",l+=`<div class="df-footer">
        <div style="display:flex;gap:10px;">
            ...
            <div><div style="font-family:var(--font-mono);font-size:7px;color:#6a6660;letter-spacing:0.8px;">EARNED</div><div style="font-family:var(--font-mono);font-size:11px;font-weight:700;color:#5c5;">${n(d)}</div></div>
        </div>
        <div style="display:flex;gap:6px;">
            ${["LOAN","INSURE","BOND"].map(v=>{const e=v==="LOAN"?"#5a8aaa":v==="INSURE"?"#aa7a5a":"#8a6aaa",u=L.filter(o=>o.type===v).length;return`<div style="text-align:center;padding:1px 6px;border:1px solid ${u>0?e+"33":"#2a2a24"};background:${u>0?e+"0a":"transparent"};"><div style="font-family:var(--font-mono);font-size:6px;color:${e};">${v}</div><div style="font-family:var(--font-mono);font-size:9px;font-weight:700;color:${u>0?"#e8e4dc":"#6a6660"};">${u}</div></div>`}).join("")}
        </div>
    </div>`,a.innerHTML=l}window.apSetFilter=jt;window.apToggle=Yt;function Tt(a,t){const p=a.reduce((d,r)=>d+r.value,0);return p===0?`<div class="rr-seg-bar" style="height:${t}px;background:#2a2a24;"></div>`:`<div class="rr-seg-bar" style="height:${t}px;">${a.map(d=>`<div style="width:${(d.value/p*100).toFixed(1)}%;height:100%;background:${d.color};"></div>`).join("")}</div>`}function Gt(){const a=document.getElementById("rr-container");if(!a)return;const t=Number(g?.corp_cash_reserves)||0,p=L.filter(c=>c.type==="LOAN").reduce((c,f)=>c+(f.remaining||0),0),d=L.filter(c=>c.type==="INSURE").reduce((c,f)=>c+(f.coverage||0),0),r=L.filter(c=>c.type==="BOND").reduce((c,f)=>c+(f.faceValue||0),0),l=p+d+r,v=l,e=t+v,u=Math.round(l*.15),o=l>0?Math.round(t/l*100):100,s=o>=30?"HEALTHY":o>=20?"ADEQUATE":o>=15?"THIN":"CRITICAL",y=o>=30?"#5c5":o>=20?"#ca5":o>=15?"#c84":"#c55",E=Math.max(0,t-u),A={};for(const c of L){const f=c.nation||"Unknown",x=c.remaining||c.coverage||c.faceValue||0;A[f]=(A[f]||0)+x}const w=Object.entries(A).map(([c,f])=>({name:c,exposure:f,pct:l>0?Math.round(f/l*100):0})).sort((c,f)=>f.exposure-c.exposure),R={};for(const c of L){const f=c.type==="BOND"?"Government":c.sector||"Other",x=c.remaining||c.coverage||c.faceValue||0;R[f]=(R[f]||0)+x}const O=Object.entries(R).map(([c,f])=>({name:c,exposure:f,pct:l>0?Math.round(f/l*100):0})).sort((c,f)=>f.exposure-c.exposure),P=w.length>0?w[0].pct:0,T=P>60?"HIGH":P>40?"MODERATE":"LOW",B=T==="HIGH"?"#c55":T==="MODERATE"?"#ca5":"#5c5";let m=`<div class="df-header">
        <div class="df-header__left">
            <span class="df-header__dot">&#9679;</span>
        ...
        <span style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:#e8e4dc;">${n(e)}</span>
    ...
    </div>`,m+="</div>",m+='<div class="rr-section-bar">Exposure by Type</div>',m+='<div class="rr-section">',l>0){m+=Tt([{value:p,color:"#5a8aaa"},{value:d,color:"#aa7a5a"},{value:r,color:"#8a6aaa"}],6),m+='<div style="margin-top:6px;">';const c=[{label:"Loans",value:p,color:"#5a8aaa",pct:l>0?Math.round(p/l*100):0},{label:"Insurance",value:d,color:"#aa7a5a",pct:l>0?Math.round(d/l*100):0},{label:"Bonds",value:r,color:"#8a6aaa",pct:l>0?Math.round(r/l*100):0}];for(let f=0;f<c.length;f++){const x=c[f];m+=`<div class="rr-type-row">
    ...
    </div>`,m+='<div class="rr-section">',w.length>0){m+='<div style="font-family:var(--font-mono);font-size:8px;color:#6a6660;letter-spacing:0.8px;margin-bottom:4px;">BY NATION</div>';for(const c of w){const f=c.pct>50?"#c84":c.pct>30?"#ca5":"#5c5";m+=`<div class="rr-conc-row">
                <span style="font-family:var(--font-mono);font-size:7px;padding:0 4px;color:#8b9a6b;background:rgba(139,154,107,0.08);border:1px solid rgba(139,154,107,0.15);line-height:12px;width:52px;text-align:center;">${b(c.name.toUpperCase().slice(0,6))}</span>
                <div style="flex:1;margin:0 6px;"><div class="rd-bar"><div class="rd-bar__fill" style="width:${c.pct}%;background:${f};"></div></div></div>
                <span style="font-family:var(--font-mono);font-size:9px;color:#e8e4dc;width:50px;text-align:right;">${n(c.exposure)}</span>
                <span style="font-family:var(--font-mono);font-size:8px;font-weight:700;width:28px;text-align:right;color:${c.pct>50?"#c84":"#9e9a92"};">${c.pct}%</span>
            </div>`}}if(O.length>0){m+='<div style="font-family:var(--font-mono);font-size:8px;color:#6a6660;letter-spacing:0.8px;margin-top:8px;margin-bottom:4px;">BY SECTOR</div>';for(const c of O){const f=c.pct>50?"#c84":c.pct>30?"#ca5":"#5c5";m+=`<div class="rr-conc-row">
                <span style="font-family:var(--font-mono);font-size:8px;color:#9e9a92;width:52px;">${b(c.name)}</span>
                <div style="flex:1;margin:0 6px;"><div class="rd-bar"><div class="rd-bar__fill" style="width:${c.pct}%;background:${f};"></div></div></div>
                <span style="font-family:var(--font-mono);font-size:9px;color:#e8e4dc;width:50px;text-align:right;">${n(c.exposure)}</span>
                <span style="font-family:var(--font-mono);font-size:8px;font-weight:700;width:28px;text-align:right;color:${c.pct>50?"#c84":"#9e9a92"};">${c.pct}%</span>
    ...
    </div>`,a.innerHTML=m}const Nt={REPAID:{color:"#5c5",label:"REPAID"},DEFAULTED:{color:"#c55",label:"DEFAULTED"},EXPIRED:{color:"#5c5",label:"EXPIRED"},CLAIMED:{color:"#c84",label:"CLAIMED"},MATURED:{color:"#5c5",label:"MATURED"},SOLD:{color:"#ca5",label:"SOLD EARLY"},FORECLOSED:{color:"#c55",label:"FORECLOSED"}};let I=[],mt=-1;function Xt(a){mt=mt===a?-1:a,yt()}function yt(){const a=document.getElementById("cc-container");if(!a)return;const t=I.reduce((u,o)=>u+(o.earned||0),0),p=I.reduce((u,o)=>u+(o.lost||0),0),d=I.reduce((u,o)=>u+(o.net||0),0),r=I.filter(u=>u.net>0).length,l=I.filter(u=>u.net<0).length,v=d>=0;let e=`<div class="df-header">
        <div class="df-header__left">
            <span class="df-header__dot">&#9679;</span>
    ...
    </div>`;if(e+=`<div class="cc-scorecard">
        ...
        <div class="cc-scorecard__cell" style="background:${v?"rgba(92,204,92,0.03)":"rgba(204,85,85,0.03)"};">
            ...
            <div class="cc-scorecard__value" style="color:${v?"#5c5":"#c55"};">${v?"+":""}${n(d)}</div>
        </div>
    </div>`,I.length>0){const u=r/I.length*100;e+=`<div class="cc-winloss">
        ...
        </div>`}e+='<div class="cc-list">',I.length===0&&(e+='<div class="cc-empty">No resolved deals yet.<br>Completed loans, expired policies, and matured bonds appear here.</div>');for(let u=0;u<I.length;u++){const o=I[u],s=gt[o.type]||{class:"loan",label:o.type},y=Nt[o.outcome]||{color:"#9e9a92",label:o.outcome},E=mt===u,A=o.net>=0;e+=`<div class="cc-deal" onclick="ccToggle(${u})" style="border-left:2px solid ${A?"#5c5":"#c55"};">
        <div class="cc-deal__inner" style="${E?"background:"+(o.type==="LOAN"?"rgba(90,138,170,0.08)":o.type==="INSURE"?"rgba(170,122,90,0.08)":"rgba(138,106,170,0.08)"):""}">`,e+=`<div style="display:flex;align-items:center;gap:5px;margin-bottom:3px;">
            <span class="df-badge df-badge-${s.class}">${s.label}</span>
            <span style="font-size:11px;font-weight:600;color:#e8e4dc;flex:1;">${b(o.counterparty)}</span>
            <span style="font-family:var(--font-mono);font-size:7px;font-weight:700;padding:0 5px;line-height:12px;color:${y.color};background:${y.color}12;border:1px solid ${y.color}25;">${y.label}</span>
        </div>`,e+=`<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;">
        ...
        </div>`,e+='<div class="df-metrics">',e+=`<div style="flex:1;padding:3px 8px;">
        ...
        </div>`,e+=`<div style="flex:0.8;padding:3px 8px;text-align:center;border-left:1px solid #2a2a24;">
        ...
        </div>`,o.lost>0&&(e+=`<div style="flex:0.8;padding:3px 8px;text-align:center;border-left:1px solid #2a2a24;">
            ...
            </div>`),e+=`<div style="flex:1;padding:3px 8px;text-align:right;border-left:1px solid #2a2a24;background:${A?"rgba(92,204,92,0.03)":"rgba(204,85,85,0.03)"};">
            ...
            <div class="df-metrics__value" style="font-size:11px;color:${A?"#5c5":"#c55"};margin-top:1px;">${A?"+":""}${n(o.net||0)}</div>
        </div>`,e+="</div>",E&&(e+='<div class="cc-deal__expanded">',o.term&&(e+=`<div class="cc-detail-row">
                ...
                </div>`),o.rate&&(e+=`<div class="cc-detail-row">
                ...
                </div>`),o.note&&(e+=`<div style="padding:4px 0;">
                    <div style="font-size:9px;color:${A?"#9e9a92":"#c84"};line-height:1.5;">${b(o.note)}</div>
                </div>`),e+="</div>"),e+="</div></div>"}e+="</div>",e+=`<div class="df-footer" style="justify-content:space-between;">
        <div>
            <div style="font-family:var(--font-mono);font-size:7px;color:#6a6660;letter-spacing:0.8px;">LIFETIME P&amp;L</div>
            <div style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:${v?"#5c5":"#c55"};">${v?"+":""}${n(d)}</div>
        </div>
        <div style="display:flex;gap:6px;">
    ...
    </div>`,a.innerHTML=e}window.ccToggle=Xt;const Wt={banking:"LOAN",insurance:"INSURE",investment:"BOND"};let K=null,_="LOAN",j=8,M=18e6,et=24,F="equipment",at=3.5,q=12e6,Q=10,H=25e6;const Ct=[{id:"none",label:"None",recovery:0,desc:"Unsecured loan. No recovery on default."},{id:"equipment",label:"Equipment",recovery:60,desc:"Seize financed equipment on default — recover ~60%."},{id:"property",label:"Property",recovery:75,desc:"Corporate property lien — recover ~75%."},{id:"full",label:"Full Assets",recovery:90,desc:"Full asset claim — recover ~90% but harder sell."}];function Kt(a){const t=D[a];t&&(K=t,_=t.type,t.type==="LOAN"?(j=8,M=t.amount,et=t.term||24,F="equipment"):t.type==="INSURE"?(at=3.5,q=t.amount,Q=10):t.type==="BOND"&&(H=Math.round(t.amount*.25)),document.getElementById("rd-overlay").classList.add("open"),document.body.style.overflow="hidden",Y())}function Ot(){document.getElementById("rd-overlay").classList.remove("open"),document.body.style.overflow="",K=null}function Qt(a){j=Number(a),Y()}function Jt(a){M=Number(a),Y()}function Zt(a){et=Number(a),Y()}function te(a){F=a,Y()}function ee(a){at=Number(a),Y()}function ae(a){q=Number(a),Y()}function oe(a){Q=Number(a),Y()}function se(a){H=Number(a),Y()}function ot(a,t,p){return`<div class="rd-bar"><div class="rd-bar__fill" style="width:${Math.min(a/t*100,100)}%;background:${p};"></div></div>`}function Y(){const a=document.getElementById("rd-modal");if(!a||!K)return;const t=K,p=_==="LOAN"?"#5a8aaa":_==="INSURE"?"#aa7a5a":"#8a6aaa",d=Math.round(M*(j/100)*(et/12)),r=Math.round((M+d)/et),l=t.revenue||474e5,v=Math.round(r/l*1200),e=12,u=Math.max(0,(j-6)*1.5),o=M>15e6?3:0,s=F==="none"?3:F==="full"?-2:0,y=Math.min(60,Math.max(2,Math.round(e+u+o+s))),E=y<=15?"#5c5":y<=30?"#ca5":y<=45?"#c84":"#c55",A=y<=15?"LOW":y<=30?"MODERATE":y<=45?"ELEVATED":"HIGH",w=95,R=(j-4)*8,O=M<(t.amount||18e6)?10:0,P=F==="full"?15:F==="property"?8:F==="none"?-5:0,T=Math.max(10,Math.min(95,Math.round(w-R-O-P))),B=T>=70?"#5c5":T>=45?"#ca5":T>=25?"#c84":"#c55",m=Ct.find(k=>k.id===F),c=Math.round(d*(1-y/100)),f=(t.term||18)/12,x=Math.round(q*(at/100)*f),N=100-(t.reputation||50),h=Math.max(5,Math.min(50,Math.round(N*.4))),dt=Math.round(q*(1-Q/100)),_t=Math.round(dt*(h/100)),st=x-_t,pt=h<=12?"#5c5":h<=22?"#ca5":h<=35?"#c84":"#c55",vt=t.couponRate||6.2,xt=t.term||60,ht=xt/12,kt=Math.round(H*(vt/100)),$t=Math.round(H*(vt/100)*ht),Z=t.stability||50,Et=t.creditRating||50,wt=t.debtToGdp||30,St=Math.max(2,Math.round((100-Z)*.15+(100-Et)*.15+Math.max(0,wt-30)*.3)),G=Math.min(60,St),ft=G<=10?"#5c5":G<=20?"#ca5":G<=35?"#c84":"#c55",At=Math.round($t*(1-G/100));let i=`<div class="rd-header">
        ...
        <span class="rd-tab ${_==="LOAN"?"active-loan":_==="INSURE"?"active-insure":"active-bond"}" style="cursor:default;">${_==="LOAN"?"Loan":_==="INSURE"?"Insure":"Bond"} — ${b(t.applicant)}</span>
        ...
        </div>`;const V=15e7,X=(M-5e6)/Math.max(1,V-5e6)*100;i+=`<div class="rd-control">
            <div class="rd-control__header">
            ...
            <div class="rd-control__header">
                ...
                <span class="rd-control__value" style="font-size:14px;color:#e8e4dc;">${et}mo</span>
            </div>
            <div class="rd-presets">`;for(const C of[12,24,36,48,60,72,84,96,108,120])i+=`<span class="rd-preset" onclick="rdSetLoanTerm(${C})" style="${et===C?"color:#000;background:#5a8aaa;border-color:#5a8aaa;":""}">${C}</span>`;i+="</div></div>",i+=`<div class="rd-control">
        ...
        </div>`,i+='<div class="rd-section-bar" style="color:#aa7a5a;">Set Policy Terms</div>';const X=(at-1)/7*100;i+=`<div class="rd-control">
            <div class="rd-control__header">
                <span class="rd-control__label">PREMIUM RATE (ANNUAL)</span>
                <span class="rd-control__value" style="font-size:16px;color:#aa7a5a;">${at}%</span>
            </div>
            <input type="range" class="rd-control__range" min="1" max="8" step="0.5" value="${at}"
                oninput="rdSetInsurePremium(this.value)"
                style="background:linear-gradient(90deg,#aa7a5a ${X}%,#2a2a24 ${X}%);">
        ...
        </div>`,i+='<div class="rd-section-bar" style="color:#8a6aaa;">Purchase Amount</div>';const S=t.amount,V=Math.max(5e6,Math.ceil(S*.05/5e6)*5e6),X=(H-V)/(S-V)*100;i+=`<div class="rd-control">
            <div class="rd-control__header">
            ...
            <input type="range" class="rd-control__range" min="${V}" max="${S}" step="5000000" value="${H}"
                ...
                style="background:linear-gradient(90deg,#8a6aaa ${X}%,#2a2a24 ${X}%);">
        ...
        </div>`,i+='<div class="rd-section-bar" style="color:#5c5;">Projected Returns</div>',i+=`<div class="rd-risk-row"><span class="rd-risk-label">CAPITAL AT RISK</span><span class="rd-risk-value" style="color:#c55;">${n(M)}</span></div>`,i+=`<div class="rd-risk-row"><span class="rd-risk-label">TOTAL INTEREST</span><span class="rd-risk-value" style="color:#5c5;">${n(d)}</span></div>`,i+=`<div class="rd-risk-row"><span class="rd-risk-label">MONTHLY PAYMENT</span><span class="rd-risk-value" style="color:#9e9a92;">${n(r)}</span></div>`;const k=v>30?"#c55":v>15?"#ca5":"#5c5";i+=`<div class="rd-risk-row"><span class="rd-risk-label">BORROWER DEBT SERVICE</span><span class="rd-risk-value" style="color:${k};">${v}% of revenue</span></div>`,i+=`<div class="rd-expected"><span style="font-family:var(--font-mono);font-size:9px;font-weight:700;color:#e8e4dc;">EXPECTED RETURN</span><span style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:#5c5;">${n(c)}</span></div>`,i+=`<div class="rd-formula">Risk-adjusted: total interest × (1 - ${y}% default)</div>`}if(_==="INSURE"){i+='<div class="rd-section-bar" style="color:#a3b07e;">Risk Assessment</div>',i+=`<div class="rd-control">
            <div class="rd-control__header">
            ...
            <div class="rd-control__header">
                ...
                <span class="rd-control__value" style="font-size:14px;color:${ft};">${G}%</span>
            </div>
            ${ot(G,100,ft)}
        ...
        </div>`,i+='<div class="rd-section-bar" style="color:#5c5;">Projected Returns</div>',i+=`<div class="rd-risk-row"><span class="rd-risk-label">CAPITAL AT RISK</span><span class="rd-risk-value" style="color:#c55;">${n(H)}</span></div>`,i+=`<div class="rd-risk-row"><span class="rd-risk-label">ANNUAL COUPON</span><span class="rd-risk-value" style="color:#5c5;">${n(kt)}</span></div>`,i+=`<div class="rd-risk-row"><span class="rd-risk-label">TOTAL RETURN (${Math.round(ht)}yr)</span><span class="rd-risk-value" style="color:#5c5;">${n($t)}</span></div>`,i+=`<div class="rd-expected"><span style="font-family:var(--font-mono);font-size:9px;font-weight:700;color:#e8e4dc;">EXPECTED RETURN</span><span style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:#5c5;">${n(At)}</span></div>`,i+=`<div class="rd-formula">Risk-adjusted: total coupon × (1 - ${G}% default)</div>`),i+="</div>",i+="</div>";const Dt=_==="LOAN"?M:_==="INSURE"?q:H,Mt=_==="LOAN"?c:_==="INSURE"?st:At,zt=_==="LOAN"?y:_==="INSURE"?h:G,Ut=_==="LOAN"?E:_==="INSURE"?pt:ft,Pt=_==="LOAN"?"ISSUE LOAN":_==="INSURE"?"WRITE POLICY":"BUY BONDS",Bt=_.toLowerCase();i+=`<div class="rd-footer">
    ...
    </div>`,a.innerHTML=i}window.rdOpen=Kt;window.rdClose=Ot;window.rdSetLoanRate=Qt;window.rdSetLoanAmount=Jt;window.rdSetLoanTerm=Zt;window.rdSetCollateral=te;window.rdSetInsurePremium=ee;window.rdSetInsureCoverage=ae;window.rdSetInsureDeductible=oe;window.rdSetBondAmount=se;let tt=!1;async function ne(){if(!K||!g||!z||tt||K.type!=="LOAN")return;tt=!0;const a=j;if(a<1||a>20){alert("Interest rate must be 1-20%."),tt=!1;return}if((Number(g.corp_cash_reserves)||0)<K.amount){alert("Insufficient cash reserves to fund this loan."),tt=!1;return}const d={none:"unsecured",equipment:"equipment",property:"property",full:"property"}[F]||"unsecured",r=z.current_tick||0,{error:l}=await U.from("finance_loan_offers").insert({request_id:K.requestId,offering_faction_id:g.id,interest_rate:a,collateral_type:d,created_tick:r});if(l){tt=!1,l.message.includes("unique")||l.message.includes("duplicate")?alert("You have already submitted an offer for this loan request."):alert("Failed to submit offer: "+l.message);return}Ot(),W=-1,await It(),tt=!1}window.rdSubmitOffer=ne;async function le(){if(!g){ct();return}const{data:a}=await U.from("finance_active_loans").select("*, borrower:factions!borrower_faction_id(id, faction_name, abbreviation, corp_ticker)").eq("lender_faction_id",g.id).in("status",["current","late","delinquent"]).order("started_tick",{ascending:!1});L=(a||[]).map(t=>({id:t.id,type:"LOAN",counterparty:t.borrower?.faction_name||"Unknown",abbr:t.borrower?.abbreviation||t.borrower?.corp_ticker||"??",remaining:t.principal-t.total_paid,principal:t.principal,earned:t.total_interest_paid||0,rate:t.interest_rate,term:t.term_months,paymentsMade:t.payments_made,paymentsMissed:t.payments_missed,monthlyPayment:t.monthly_payment,status:t.status.toUpperCase(),collateral:t.collateral_type,purpose:t.purpose||"",alert:t.status==="late"||t.status==="delinquent",alertLevel:t.status==="delinquent"?"red":t.status==="late"?"orange":null,alertMsg:t.status==="delinquent"?`${t.payments_missed} missed payments. Default imminent.`:t.status==="late"?`${t.payments_missed} missed payment${t.payments_missed>1?"s":""}. Monitor closely.`:null})),ct()}async function ie(){if(!g){yt();return}const{data:a}=await U.from("finance_active_loans").select("*, borrower:factions!borrower_faction_id(id, faction_name, abbreviation, corp_ticker)").eq("lender_faction_id",g.id).in("status",["repaid","defaulted"]).order("completed_tick",{ascending:!1});I=(a||[]).map(t=>{const p=t.total_interest_paid||0,d=t.status==="defaulted"?Math.max(0,t.principal-t.total_paid):0;return{type:"LOAN",counterparty:t.borrower?.faction_name||"Unknown",abbr:t.borrower?.abbreviation||t.borrower?.corp_ticker||"??",nation:"",outcome:t.status==="repaid"?"REPAID":"DEFAULTED",principal:t.principal,earned:p,lost:d,net:p-d,resolved:t.completed_tick?"Tick "+t.completed_tick:"",term:t.term_months+"mo",rate:t.interest_rate+"%",note:t.status==="repaid"?`Fully repaid over ${t.payments_made} payments.`:`Defaulted after ${t.payments_missed} missed payments. ${t.collateral_type!=="unsecured"?"Collateral ("+t.collateral_type+") partially recovered losses.":"Unsecured — no collateral recovery."}`}}),yt()}async function re(){const{data:{user:a}}=await U.auth.getUser();if(!a){window.location.href="login.html";return}const{data:t}=await U.from("factions").select("*").or(`id.eq.${a.id},linked_user_id.eq.${a.id}`);J=(t||[]).filter(l=>l.nation_id);const p=sessionStorage.getItem("active_faction_id");if(g=J.find(l=>l.id===p)||J.find(l=>l.faction_type==="corporation")||J[0],!g){await U.auth.signOut(),window.location.href="login.html";return}if(g.faction_type!=="corporation"){window.location.href="dashboard.html";return}if(g.corp_sector!=="Finance"){window.location.href="corp-operations.html"+window.location.search;return}sessionStorage.setItem("active_faction_id",g.id);const[d,r]=await Promise.all([g.nation_id?U.from("nations").select("*").eq("id",g.nation_id).single():Promise.resolve({data:null}),U.from("shard").select("current_tick, current_date, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single()]);d.data&&d.data,z=r.data,document.getElementById("corp-name-bar").textContent=g.faction_name||"Corp",document.getElementById("corp-name-badge").textContent=(g.abbreviation||g.corp_ticker||g.faction_name||"--").toUpperCase(),document.getElementById("topbar-cash").textContent="CASH: "+n(Number(g.corp_cash_reserves)||0),z&&(document.getElementById("game-date").textContent=z.current_date||Ft(z.current_tick),document.getElementById("tick-number").textContent=z.current_tick||"--"),ce(),await It(),await le(),Gt(),await ie(),z?.next_tick_at&&ue(z.next_tick_at)}function ce(){const a=document.getElementById("corp-faction-dropdown");if(!a||J.length<=1)return;let t="";for(const p of J){const d=p.id===g.id,r=p.faction_type==="corporation"?"CORP":"PARTY";t+=`<div class="corp-faction-dropdown__item${d?" active":""}" onclick="switchFaction('${p.id}')">
            <span style="font-size:7px;padding:1px 4px;background:${p.faction_type==="corporation"?"rgba(90,175,165,0.1)":"rgba(200,168,50,0.1)"};border:1px solid ${p.faction_type==="corporation"?"var(--teal-border)":"rgba(200,168,50,0.2)"};color:${p.faction_type==="corporation"?"var(--teal)":"var(--amber)"}">${r}</span>
            <span>${b(p.faction_name||"--")}</span>
        </div>`}a.innerHTML=t}function de(){document.getElementById("corp-faction-dropdown").classList.toggle("open")}function pe(a){sessionStorage.setItem("active_faction_id",a);const t=J.find(p=>p.id===a);t&&t.faction_type==="party"?window.location.href="dashboard.html":window.location.reload()}function ve(){document.body.classList.toggle("light-mode");const a=document.body.classList.contains("light-mode");localStorage.setItem("theme",a?"light":"dark"),document.getElementById("theme-toggle").textContent=a?"Dark":"Light"}async function fe(){await U.auth.signOut(),window.location.href="login.html"}function ue(a){const t=document.getElementById("tick-countdown");function p(){const d=new Date(a)-new Date;if(d<=0){t.textContent="Processing...";return}const r=Math.floor(d/36e5),l=Math.floor(d%36e5/6e4),v=Math.floor(d%6e4/1e3);t.textContent=`${r}h ${l}m ${v}s`}p(),setInterval(p,1e3)}localStorage.getItem("theme")==="light"&&(document.body.classList.add("light-mode"),document.getElementById("theme-toggle").textContent="Dark");window.toggleCorpDropdown=de;window.switchFaction=pe;window.toggleTheme=ve;window.doLogout=fe;re();
