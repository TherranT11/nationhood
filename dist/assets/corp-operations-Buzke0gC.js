...
import{_supabase as g}from"./supabase-client-CiYoFhIh.js";/* empty css                    *//* empty css                         */import{_ as fe}from"./preload-helper-BXl3LOEh.js";import{t as te,e as o,a as Q}from"./utils-DGqmZD5X.js";const ge={Finance:"corp-operations-finance.html",Shipping:"corp-operations-shipping.html"};function ve(e){const t=Number(e)||0;return t>=7?"good":t>=4?"mid":"warn"}const be=[{key:"crews",column:"corp_work_crews",eyebrow:"CAPACITY",name:"Work",emName:"Crews",tooltip:"Your pool of skilled labor — engineers, foremen, equipment operators. Caps how many construction projects you can bid on at once and drives bid-prep speed. Strong crews mean faster turnarounds and more bids in flight; weak crews stall the pipeline.",blurb:{good:"<strong>Strong skilled labor pool.</strong> Crews fully staffed; apprenticeship pipeline producing qualified hires.",mid:"<strong>Adequate crew strength.</strong> Operating at workable capacity, room to scale before next bid window.",warn:"<strong>Crew shortages.</strong> Active bids limited; recruitment overdue and turnover rising."},impacts:{good:[{label:"Max Active Projects",value:"5+"},{label:"Avg. Bid Speed",value:"+18%",tone:"good"}],mid:[{label:"Max Active Projects",value:"3"},{label:"Avg. Bid Speed",value:"Average"}],warn:[{label:"Max Active Projects",value:"1–2"},{label:"Avg. Bid Speed",value:"−12%",tone:"warn"}]}},{key:"regulatory",column:"corp_regulatory_standing",eyebrow:"RISK",name:"Regulatory",emName:"Standing",tooltip:"Your relationship with the host nation's ministries and inspectors. High standing = permits clear quickly, audits stay light, government contracts flow your way. Low standing = approvals stall, audit risk climbs, sanctioned projects slip away to rivals.",blurb:{good:"<strong>Strong ministry rapport.</strong> Permits clear quickly; audit risk minimal; the regime takes your calls.",mid:"<strong>Average standing.</strong> No major scandals on the books; permits move at the standard pace.",warn:"<strong>Recent incidents have soured ministry relations.</strong> Permits stall; audit risk elevated; favors are no longer free."},impacts:{good:[{label:"Permit Speed",value:"+22%",tone:"good"},{label:"Audit Risk",value:"Low",tone:"good"}],mid:[{label:"Permit Speed",value:"Average"},{label:"Audit Risk",value:"Standard"}],warn:[{label:"Permit Speed",value:"−15%",tone:"warn"},{label:"Audit Risk",value:"Elevated",tone:"warn"}]}},{key:"supply",column:"corp_supply_chain",eyebrow:"REACH",name:"Material",emName:"Supply Chain",tooltip:"How diversified your sourcing is — concrete, steel, fixtures, fuel. Strong chains keep reserves deep and shrug off single-supplier disruptions or sanctions. Weak chains run hand-to-mouth: one shock and active projects slip into delay penalties.",blurb:{good:"<strong>Diversified sourcing across domestic and overseas suppliers.</strong> Reserves healthy; sanctions exposure manageable.",mid:"<strong>Mixed sourcing.</strong> Reserves stable but vulnerable to single-source disruption; suppliers cordial, not loyal.",warn:"<strong>Single-source dependence.</strong> Stock running low; one disruption away from project delays."},impacts:{good:[{label:"Material Reserves",value:"60+ days",tone:"good"},{label:"Sanctions Exposure",value:"Low",tone:"good"}],mid:[{label:"Material Reserves",value:"30–45 days"},{label:"Sanctions Exposure",value:"Moderate",tone:"rust"}],warn:[{label:"Material Reserves",value:"<14 days",tone:"warn"},{label:"Sanctions Exposure",value:"High",tone:"warn"}]}}];function se(){const e=document.getElementById("co-hero-stats");e&&(e.innerHTML=be.map(ye).join(""))}function ye(e){const t=Number(c[e.column])||0,s=ve(t),a=Number.isInteger(t)?String(t):t.toFixed(1),n='<div class="co-hero-stat-trend">— Latest</div>',i=_e(t,s),r=e.blurb[s]||"",l=(e.impacts[s]||[]).map(m=>`<div>
        ...
        <div class="co-hero-stat-name">${o(e.name)} <em>${o(e.emName)}</em>${f}</div>
        ...
        <div class="co-hero-stat-impact">${l}</div>
    </div>`}function _e(e,t){const s=Math.max(0,Math.min(10,Math.round(Number(e)||0))),a=t==="good"?"filled good":t==="warn"?"filled warn":"filled",n=[];for(let i=0;i<10;i++)n.push(`<div class="co-hero-meter-cell${i<s?" "+a:""}"></div>`);return`<div class="co-hero-meter">${n.join("")}</div>`}let I=[],L=[];async function M(){if(!c?.id)return;const[e,t]=await Promise.all([g.from("corp_contract_events").select("id, contract_id, type, severity, title, description, impact, responses, expires_at_tick, corp_contracts:contract_id(name)").eq("faction_id",c.id).eq("status","ACTIVE").order("expires_at_tick",{ascending:!0}),g.from("bank_loan_offers").select(`
            ...
            <div class="co-contract-name">${o(m)} — ${o(d)}</div>
            ${h?`<div class="co-contract-client">— ${o(h)}</div>`:""}
            <div class="co-issue-terms">
                <span><span class="label">PRINCIPAL</span><span class="value">${o(y(w))}</span></span>
        ...
        </div>`});e.innerHTML=n.concat(i).join("")}let P=new Set,E=new Set;async function he(e){if(!(!e||E.has(e))){E.add(e),N();try{const{data:t,error:s}=await g.rpc("accept_loan_offer",{p_offer_id:e});s?p("Failed: "+s.message,"error"):t?.success?p("Offer accepted. Awaiting bank disbursement.","success"):p(t?.error||"Failed to accept","error"),await M()}catch(t){console.error("[corp-operations] accept_loan_offer failed:",t),p("Accept failed: "+(t?.message||"unknown"),"error")}finally{E.delete(e),N()}}}async function ke(e){if(!(!e||E.has(e))){E.add(e),N();try{const{data:t,error:s}=await g.rpc("reject_loan_offer",{p_offer_id:e});s?p("Failed: "+s.message,"error"):t?.success?p("Offer rejected.","success"):p(t?.error||"Failed to reject","error"),await M()}catch(t){console.error("[corp-operations] reject_loan_offer failed:",t),p("Reject failed: "+(t?.message||"unknown"),"error")}finally{E.delete(e),N()}}}async function $e(e){if(!P.has(e)){P.add(e);try{const{data:t,error:s}=await g.rpc("acknowledge_corp_contract_event",{p_event_id:e,p_response_key:null});if(s)return p("Failed: "+s.message,"error");if(t&&t.success===!1)return p(t.error||"Failed to acknowledge","error");const a=t?.cost_applied?` − ${y(t.cost_applied)}`:"",n=t?.delay_applied?` ◊ +${t.delay_applied}t delay`:"";p(`Acknowledged${a}${n}`,"success"),await V(),await M()}catch(t){console.error("[corp-operations] acknowledge failed:",t),p("Acknowledge failed: "+(t?.message||"unknown"),"error")}finally{P.delete(e)}}}let x=[],$=[],D=new Map,j=!1,F=!1;function y(e){const t=Number(e)||0,s=t<0?"-":"",a=Math.abs(t);return a>=1e9?s+"$"+(a/1e9).toFixed(2)+"B":a>=1e6?s+"$"+(a/1e6).toFixed(1)+"M":a>=1e3?s+"$"+(a/1e3).toFixed(1)+"k":s+"$"+Math.round(a).toLocaleString()}const Z=["id","contract_number","name","description","contract_type","issuer_name","issuer_nation_id","budget","timeline_months","status","requirements","expires_at_tick","deadline_tick","expected_finish_tick","started_at_tick","progress_pct","amount_spent","crews_assigned","winner_faction_id"].join(", ")+", nations:issuer_nation_id(name)";async function W(){const{data:e}=await g.from("corp_properties").select("nation_id").eq("faction_id",c.id).eq("role","regional_hq").eq("is_active",!0),t=Array.from(new Set([c.nation_id,...(e||[]).map(i=>i.nation_id).filter(Boolean)])),[s,a]=await Promise.all([g.from("corp_contracts").select(Z).eq("required_sector","Construction").eq("status","open").in("issuer_nation_id",t).order("budget",{ascending:!1}).limit(40),g.from("corp_contracts").select(Z).eq("winner_faction_id",c.id).in("status",["awarded","active"]).order("expected_finish_tick",{ascending:!0})]);s.error?(console.error("[corp-operations] available contracts error:",s.error.message),x=[]):x=s.data||[],a.error?(console.error("[corp-operations] active contracts error:",a.error.message),$=[]):$=a.data||[],D=new Map;const n=x.map(i=>i.id);if(n.length>0){const{data:i,error:r}=await g.from("corp_contract_bids").select("contract_id, bid_amount, crews_committed, markup_pct, quoted_timeline_months").eq("faction_id",c.id).in("contract_id",n);if(r)console.warn("[corp-operations] my bids fetch failed:",r.message);else for(const u of i||[])D.set(u.contract_id,u)}ae(),Ce()}function ae(){const e=document.getElementById("co-available-list"),t=document.getElementById("co-available-meta");if(!e||!t)return;const s=re(),a=x.filter(n=>!s.has(n.id));if(a.length===0){t.textContent="None open",e.innerHTML='<div class="co-contract-empty">No open contracts at the moment. Check back next tick.</div>';return}t.textContent=`${a.length} Open Bid${a.length===1?"":"s"}`,e.innerHTML=a.map(xe).join("")}function Ce(){const e=document.getElementById("co-active-list"),t=document.getElementById("co-active-meta");if(!e||!t)return;if($.length===0){t.textContent="None active",e.innerHTML='<div class="co-contract-empty">No active contracts. Win a bid to get going.</div>';return}const s=$.reduce((a,n)=>a+(Number(n.budget)||0),0);t.textContent=`${$.length} In Progress ◊ Total Value ${y(s)}`,e.innerHTML=$.map(Be).join("")}function ne(e){const t=String(e||"").toLowerCase();return t==="private"?"private":t==="foreign"?"foreign":"gov"}function oe(e){switch(e){case"work_crews":return"Work Crews";case"regulatory_standing":return"Reg. Standing";case"supply_chain":return"Supply Chain";default:return e}}function Ee(e){if(!c)return null;switch(e){case"work_crews":return Number(c.corp_work_crews??0);case"regulatory_standing":return Number(c.corp_regulatory_standing??0);case"supply_chain":return Number(c.corp_supply_chain??0);default:return null}}function xe(e){const t=ne(e.contract_type),s=String(e.contract_type||"GOVERNMENT"),a=Number(C?.current_tick)||0,n=e.expires_at_tick!=null?Math.max(0,e.expires_at_tick-a):null,i=n==null?"No deadline":n<=0?"Closing now":`Closes in ${n} tick${n===1?"":"s"}`,r=e.requirements&&typeof e.requirements=="object"?e.requirements:{},u=Number(r.work_crews)||0;let l=!0;const f=Object.entries(r).map(([h,k])=>{const q=Ee(h),T=Number(k),B=q!=null&&Number.isFinite(T)&&q>=T;return B||(l=!1),`<span class="co-req-chip ${B?"met":"unmet"}">${B?"✓":"✗"} ${o(oe(h))} ${o(String(k))}+</span>`}).join(""),m=e.issuer_name?`— ${o(e.issuer_name)}${e.nations?.name?`, ${o(e.nations.name)}`:""}`:"— Unknown issuer",d=D.get(e.id),b=l?"Place Bid ▸":"Requirements Unmet",_=l?`data-action="bid" data-id="${o(e.id)}"`:"disabled",w=d?`<div class="co-contract-bid-placed">
        ...
        <div class="co-contract-actions">
            ...
            <button class="co-contract-btn" data-action="view-details" data-id="${o(e.id)}">Details</button>
    ...
    </div>`}function ie(){return`co-declined-${c?.id||"anon"}`}function re(){try{const e=localStorage.getItem(ie());return new Set(e?JSON.parse(e):[])}catch{return new Set}}function Ae(e){const t=re();t.add(e);try{localStorage.setItem(ie(),JSON.stringify(Array.from(t)))}catch{}}function Ne(){document.getElementById("co-available-list")?.addEventListener("click",ee),document.getElementById("co-active-list")?.addEventListener("click",ee),document.getElementById("co-issues-list")?.addEventListener("click",e=>{const t=e.target.closest("[data-action]");if(!t||t.disabled)return;const s=t.getAttribute("data-action"),a=t.getAttribute("data-id");if(a){if(s==="ack-issue")return $e(a);if(s==="accept-offer")return he(a);if(s==="reject-offer")return ke(a)}}),document.getElementById("co-modal-overlay")?.addEventListener("click",e=>{(e.target===e.currentTarget||e.target.matches("[data-modal-close]"))&&S()}),document.addEventListener("keydown",e=>{e.key==="Escape"&&S()})}async function ee(e){const t=e.target.closest(".co-contract-btn");if(!t||t.disabled)return;const s=t.getAttribute("data-action"),a=t.getAttribute("data-id");if(!(!s||!a)){if(s==="decline"){Ae(a),ae();return}if(s==="bid"){const n=x.find(i=>i.id===a);n&&Ie(n);return}if(s==="view-details"){const n=$.find(i=>i.id===a)||x.find(i=>i.id===a);n&&Me(n);return}if(s==="manage-crews"){const n=$.find(i=>i.id===a);n&&Re(n);return}if(s==="renegotiate"){p("Renegotiation isn't available yet.","error");return}}}const Se={1:1,2:1.05,3:1.1},qe={1:1,2:.8,3:.7};function Te(e,t,s){const a=1.3-(Number(c.corp_supply_chain)||0)*.06,n=Se[t]||1,i=qe[t]||1,r=(Number(e.budget)||0)*.7,u=Math.round(r*a*n*(1+s/100)),l=Math.round((Number(e.timeline_months)||0)*i);return{bidAmount:u,months:l,supplyMult:a,crewCost:n,crewTime:i}}function Ie(e){const t=document.getElementById("co-modal");if(!t)return;t.classList.add("co-bid-modal");const s=Math.floor(Number(c.corp_work_crews)||0),a=Number(c.corp_regulatory_standing)||0,n=Number(c.corp_supply_chain)||0,i=e.requirements&&typeof e.requirements=="object"?e.requirements:{},r=Number(i.regulatory_standing)||0,u={1:s>=1,2:s>=2,3:s>=3};let l=u[3]?3:u[2]?2:(u[1],1),f=30;const m=()=>{const{bidAmount:d,months:b,supplyMult:_}=Te(e,l,f),w=Number(e.budget)||0,h=w>0?d/w*100:0,k=d>w,q=[1,2,3].map(v=>{const J=u[v];return`<button type="button" class="co-bid-pill${v===l&&J?" active":""}"
                ...
                <span class="sub">${v===1?"baseline":v===2?"−20% time · ×1.05 cost":"−30% time · ×1.10 cost"}</span>
                    ...
                    data-bid-markup="${v}">${v}%</button>`).join(""),B=a>=r?"good":"warn",G=`${a.toFixed(a%1===0?0:1)}<span class="muted"> / 10${r?` · req ${r}+`:""}</span>`,de=k?"warn":h<85?"good":"",ue=`${y(d)}<span class="muted"> · ${h.toFixed(0)}% of budget</span>`,K=5e4,Y=!u[l],R=(Number(c.corp_cash_reserves)||0)<K,z=a<r,pe=Y||R||z,me=R?"Need $50k":z?`Need Reg. ${r}+`:Y?"No crews available":"Submit Bid ▸";t.innerHTML=`
            ...
            <div class="co-bid-section co-bid-summary" style="margin-top:14px;">
                ...
                <div class="co-bid-summary-row">
                    ...
                    <span class="value">${_.toFixed(2)}×<span class="muted"> · supply ${n.toFixed(n%1===0?0:1)}/10</span></span>
                </div>
                <div class="co-bid-summary-row">
            ...
            <div class="co-modal-actions" style="margin-top:18px;">
                ...
                <button class="co-contract-btn primary" id="co-bid-submit" ${pe?"disabled":""}>${o(me)}</button>
            </div>
        `,t.querySelectorAll("[data-bid-crews]").forEach(v=>{v.addEventListener("click",()=>{v.disabled||(l=Number(v.getAttribute("data-bid-crews")),m())})}),t.querySelectorAll("[data-bid-markup]").forEach(v=>{v.addEventListener("click",()=>{f=Number(v.getAttribute("data-bid-markup")),m()})}),t.querySelector("#co-bid-submit")?.addEventListener("click",()=>{Le(e.id,l,f)})};m(),document.getElementById("co-modal-overlay").classList.add("open")}async function Le(e,t,s){if(j)return;j=!0;const a=document.getElementById("co-bid-submit");a&&(a.disabled=!0);try{const{data:n,error:i}=await g.rpc("place_construction_bid",{p_contract_id:e,p_bidder_faction_id:c.id,p_crews_committed:t,p_markup_pct:s,p_bid_message:null});if(i){p(i.message,"error");return}if(!n?.success){p(n?.error||"Failed to place bid","error");return}p(`Bid placed: ${y(n.bid_amount)} · ${n.quoted_timeline_months} ticks`,"success"),S(),await V(),await W()}catch(n){console.error("[corp-operations] place bid failed:",n),p("Failed to place bid: "+(n?.message||"unknown"),"error")}finally{j=!1,a&&(a.disabled=!1)}}function Me(e){const t=document.getElementById("co-modal");if(!t)return;const s=e.requirements&&typeof e.requirements=="object"?e.requirements:{},a=Object.entries(s).map(([n,i])=>`<div class="co-modal-detail-row">
            <span class="label">${o(oe(n))}</span>
    ...
    `,document.getElementById("co-modal-overlay").classList.add("open")}function Re(e){const t=document.getElementById("co-modal");if(!t)return;const s=Number(c.corp_work_crews)||0,a=Number(e.crews_assigned)||0;t.innerHTML=`
    ...
    </div>`}function le(e){const t=[`<span class="co-effect cost">−${y(e.cost).replace("-","")}</span>`];for(const[s,a]of Object.entries(e.effects||{})){const n=a>0?"+":"",i=a>0?"positive":"negative";t.push(`<span class="co-effect ${i}">${n}${a} ${o(Pe[s]||s)}</span>`)}return t.join("")}function Oe(){document.getElementById("co-actions-grid")?.addEventListener("click",e=>{const t=e.target.closest(".co-action-option");if(!t||t.hasAttribute("data-locked"))return;const s=Number(t.getAttribute("data-card-idx")),a=Number(t.getAttribute("data-opt-idx")),n=U[s],i=n?.options[a];!n||!i||De(n,i)})}function De(e,t){const s=document.getElementById("co-modal");if(!s)return;const a=le(t);s.innerHTML=`
