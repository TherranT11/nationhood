import{_ as U}from"./supabase-client-BXEzLDpS.js";import{t as Ft,e as b}from"./utils-C2W-HleY.js";let J=[],g=null,z=null;function n(a){return Math.abs(a)>=1e6?"$"+(a/1e6).toFixed(1)+"M":Math.abs(a)>=1e3?"$"+(a/1e3).toFixed(0)+"k":"$"+Math.round(a).toLocaleString()}const gt={LOAN:{class:"loan",label:"LOAN"},INSURE:{class:"insure",label:"INSURE"},BOND:{class:"bond",label:"BOND"}},Vt={LOW:{class:"df-risk-low",label:"LOW RISK"},MODERATE:{class:"df-risk-moderate",label:"MODERATE"},ELEVATED:{class:"df-risk-elevated",label:"ELEVATED"},HIGH:{class:"df-risk-high",label:"HIGH RISK"}};let D=[],lt="ALL",X=-1;async function It(){if(!g||!z)return;const{data:a}=await U.from("finance_loan_requests").select("*, requesting_faction:factions!requesting_faction_id(id, faction_name, abbreviation, corp_ticker, corp_subsector, corp_reputation, corp_credit_rating, nation_id)").eq("status","open").order("created_tick",{ascending:!1}),{data:t}=await U.from("finance_loan_offers").select("request_id").eq("offering_faction_id",g.id),p=new Set((t||[]).map(r=>r.request_id));D=(a||[]).map(r=>({id:r.id,type:"LOAN",applicant:r.requesting_faction?.faction_name||"Unknown",abbr:r.requesting_faction?.abbreviation||r.requesting_faction?.corp_ticker||"??",entity:"CORP",nation:"N/A",amount:r.amount,term:r.term_months,purpose:r.purpose||"",reputation:r.requesting_faction?.corp_reputation??50,creditRating:r.requesting_faction?.corp_credit_rating??50,risk:(r.requesting_faction?.corp_credit_rating??50)>=60?"LOW":(r.requesting_faction?.corp_credit_rating??50)>=40?"MODERATE":(r.requesting_faction?.corp_credit_rating??50)>=25?"ELEVATED":"HIGH",isNew:!p.has(r.id),ticksLeft:(r.expires_tick||0)-(z?.current_tick||0),requestId:r.id,alreadyOffered:p.has(r.id)})),bt()}function Lt(a){if(!g)return!1;const t=(g.corp_subsector||"").toLowerCase(),p=Xt[t];return a.type===p}function Ht(a){lt=a,X=-1,bt()}function qt(a){X=X===a?-1:a,bt()}function bt(){const a=document.getElementById("df-container");if(!a)return;const t=lt==="ALL"?D:D.filter(o=>o.type===lt),p=D.filter(o=>o.isNew).length,r=D.length;let c=`<div class="df-header">
        ...
        <div style="display:flex;gap:4px;align-items:center;">
            ...
            <div style="flex:0.8;text-align:center;">
                ...
                </div>`}if(R){if(s.type==="LOAN"){const f=Math.round(s.debt/s.revenue*100),x=f>50?"#c84":"#5c5",N=s.debt>s.revenue*.5?"#c84":"#9e9a92";c+=`<div class="df-detail-row"><span class="df-detail-label">ANNUAL REVENUE</span><span class="df-detail-value" style="color:#9e9a92;">${n(s.revenue)}</span></div>`,c+=`<div class="df-detail-row"><span class="df-detail-label">EXISTING DEBT</span><span class="df-detail-value" style="color:${N};">${n(s.debt)}</span></div>`,c+=`<div class="df-detail-row"><span class="df-detail-label">DEBT/REVENUE</span><span class="df-detail-value" style="color:${x};font-weight:700;">${f}%</span></div>`}else if(s.type==="BOND"){const f=s.stability>=50?"#5c5":s.stability>=30?"#ca5":"#c84",x=s.debtToGdp>60?"#c55":s.debtToGdp>40?"#c84":"#5c5",N=s.creditRating>=60?"#5c5":s.creditRating>=35?"#ca5":"#c55";c+=`<div class="df-detail-row"><span class="df-detail-label">STABILITY</span><span class="df-detail-value" style="color:${f};">${s.stability}/100</span></div>`,c+=`<div class="df-detail-row"><span class="df-detail-label">DEBT/GDP</span><span class="df-detail-value" style="color:${x};">${s.debtToGdp}%</span></div>`,c+=`<div class="df-detail-row"><span class="df-detail-label">CREDIT RATING</span><span class="df-detail-value" style="color:${N};font-weight:700;">${s.creditRating}/100</span></div>`}else if(s.type==="INSURE"){const f=s.reputation>=60?"#5c5":s.reputation>=35?"#ca5":"#c84",x=s.projectValue?"PROJECT VALUE":"FLEET VALUE",N=s.projectValue||s.fleetValue;c+=`<div class="df-detail-row"><span class="df-detail-label">REPUTATION</span><span class="df-detail-value" style="color:${f};">${s.reputation}/100</span></div>`,c+=`<div class="df-detail-row"><span class="df-detail-label">${x}</span><span class="df-detail-value" style="color:#9e9a92;">${n(N)}</span></div>`}c+="</div>"}}c+="</div>"}c+="</div>";const v=D.filter(o=>o.type==="LOAN").length,e=D.filter(o=>o.type==="INSURE").length,u=D.filter(o=>o.type==="BOND").length;c+=`<div class="df-footer">
    ...
    </div>`,a.innerHTML=c}window.dfSetFilter=Ht;window.dfSelectDeal=qt;const Rt={CURRENT:{color:"#5c5",label:"CURRENT"},LATE:{color:"#c84",label:"LATE"},DELINQUENT:{color:"#c55",label:"DELINQUENT"},CLAIM:{color:"#c55",label:"CLAIM FILED"},ACTIVE:{color:"#5c5",label:"ACTIVE"},WATCHLIST:{color:"#ca5",label:"WATCHLIST"},DEFAULT:{color:"#c55",label:"DEFAULT"}};let L=[],it="ALL",rt=-1;function jt(a){it=a,rt=-1,ct()}function Gt(a){rt=rt===a?-1:a,ct()}function ct(){const a=document.getElementById("ap-container");if(!a)return;const t=it==="ALL"?L:L.filter(v=>v.type===it),p=L.reduce((v,e)=>v+(e.remaining||e.coverage||e.faceValue||0),0),r=L.reduce((v,e)=>v+(e.earned||e.premiumsCollected||e.couponsReceived||0),0),c=L.filter(v=>v.alert).length;let l=`<div class="df-header">
        <div class="df-header__left">
            <span class="df-header__dot">&#9679;</span>
        ...
        </div>`;const P=E?y.color:s.class==="loan"?"#5a8aaa":s.class==="insure"?"#aa7a5a":"#8a6aaa";l+=`<div class="rd-bar"><div class="rd-bar__fill" style="width:${Math.min(R,100)}%;background:${P};"></div></div>`;const T=e.type==="LOAN"?"REMAINING":e.type==="INSURE"?"COVERAGE":"FACE VALUE",B=e.remaining||e.coverage||e.faceValue||0,m=e.type==="LOAN"?"RATE":e.type==="INSURE"?"PREMIUM":"COUPON",d=e.rate||e.premiumRate||e.coupon||0,f=e.earned||e.premiumsCollected||e.couponsReceived||0,x=s.class==="loan"?"#5a8aaa":s.class==="insure"?"#aa7a5a":"#8a6aaa";if(l+=`<div class="df-metrics" style="margin-top:4px;">
    ...
    </div>`,a.innerHTML=m}const Nt={REPAID:{color:"#5c5",label:"REPAID"},DEFAULTED:{color:"#c55",label:"DEFAULTED"},EXPIRED:{color:"#5c5",label:"EXPIRED"},CLAIMED:{color:"#c84",label:"CLAIMED"},MATURED:{color:"#5c5",label:"MATURED"},SOLD:{color:"#ca5",label:"SOLD EARLY"},FORECLOSED:{color:"#c55",label:"FORECLOSED"}};let I=[],mt=-1;function Wt(a){mt=mt===a?-1:a,yt()}function yt(){const a=document.getElementById("cc-container");if(!a)return;const t=I.reduce((u,o)=>u+(o.earned||0),0),p=I.reduce((u,o)=>u+(o.lost||0),0),r=I.reduce((u,o)=>u+(o.net||0),0),c=I.filter(u=>u.net>0).length,l=I.filter(u=>u.net<0).length,v=r>=0;let e=`<div class="df-header">
        <div class="df-header__left">
            <span class="df-header__dot">&#9679;</span>
    ...
    </div>`,I.length>0){const u=c/I.length*100;e+=`<div class="cc-winloss">
    ...
    </div>`,a.innerHTML=e}window.ccToggle=Wt;const Xt={banking:"LOAN",insurance:"INSURE",investment:"BOND"};let K=null,_="LOAN",j=8,M=18e6,et=24,F="equipment",at=3.5,H=12e6,Q=10,q=25e6;const Ct=[{id:"none",label:"None",recovery:0,desc:"Unsecured loan. No recovery on default."},{id:"equipment",label:"Equipment",recovery:60,desc:"Seize financed equipment on default — recover ~60%."},{id:"property",label:"Property",recovery:75,desc:"Corporate property lien — recover ~75%."},{id:"full",label:"Full Assets",recovery:90,desc:"Full asset claim — recover ~90% but harder sell."}];function Kt(a){const t=D[a];t&&(K=t,_=t.type,t.type==="LOAN"?(j=8,M=t.amount,et=t.term||24,F="equipment"):t.type==="INSURE"?(at=3.5,H=t.amount,Q=10):t.type==="BOND"&&(q=Math.round(t.amount*.25)),document.getElementById("rd-overlay").classList.add("open"),document.body.style.overflow="hidden",G())}function Ot(){document.getElementById("rd-overlay").classList.remove("open"),document.body.style.overflow="",K=null}function Qt(a){j=Number(a),G()}function Jt(a){M=Number(a),G()}function Zt(a){et=Number(a),G()}function te(a){F=a,G()}function ee(a){at=Number(a),G()}function ae(a){H=Number(a),G()}function oe(a){Q=Number(a),G()}function se(a){q=Number(a),G()}function ot(a,t,p){return`<div class="rd-bar"><div class="rd-bar__fill" style="width:${Math.min(a/t*100,100)}%;background:${p};"></div></div>`}function G(){const a=document.getElementById("rd-modal");if(!a||!K)return;const t=K,p=_==="LOAN"?"#5a8aaa":_==="INSURE"?"#aa7a5a":"#8a6aaa",r=Math.round(M*(j/100)*(et/12)),c=Math.round((M+r)/et),l=t.revenue||474e5,v=Math.round(c/l*1200),e=12,u=Math.max(0,(j-6)*1.5),o=M>15e6?3:0,s=F==="none"?3:F==="full"?-2:0,y=Math.min(60,Math.max(2,Math.round(e+u+o+s))),E=y<=15?"#5c5":y<=30?"#ca5":y<=45?"#c84":"#c55",A=y<=15?"LOW":y<=30?"MODERATE":y<=45?"ELEVATED":"HIGH",w=95,R=(j-4)*8,O=M<(t.amount||18e6)?10:0,P=F==="full"?15:F==="property"?8:F==="none"?-5:0,T=Math.max(10,Math.min(95,Math.round(w-R-O-P))),B=T>=70?"#5c5":T>=45?"#ca5":T>=25?"#c84":"#c55",m=Ct.find(k=>k.id===F),d=Math.round(r*(1-y/100)),f=(t.term||18)/12,x=Math.round(H*(at/100)*f),N=100-(t.reputation||50),h=Math.max(5,Math.min(50,Math.round(N*.4))),dt=Math.round(H*(1-Q/100)),_t=Math.round(dt*(h/100)),st=x-_t,pt=h<=12?"#5c5":h<=22?"#ca5":h<=35?"#c84":"#c55",vt=t.couponRate||6.2,xt=t.term||60,ht=xt/12,kt=Math.round(q*(vt/100)),$t=Math.round(q*(vt/100)*ht),Z=t.stability||50,Et=t.creditRating||50,wt=t.debtToGdp||30,St=Math.max(2,Math.round((100-Z)*.15+(100-Et)*.15+Math.max(0,wt-30)*.3)),Y=Math.min(60,St),ft=Y<=10?"#5c5":Y<=20?"#ca5":Y<=35?"#c84":"#c55",At=Math.round($t*(1-Y/100));let i=`<div class="rd-header">
        ...
        <span class="rd-tab ${_==="LOAN"?"active-loan":_==="INSURE"?"active-insure":"active-bond"}" style="cursor:default;">${_==="LOAN"?"Loan":_==="INSURE"?"Insure":"Bond"} — ${b(t.applicant)}</span>
        ...
        </div>`,i+='<div class="rd-section-bar" style="color:#aa7a5a;">Set Policy Terms</div>';const W=(at-1)/7*100;i+=`<div class="rd-control">
            <div class="rd-control__header">
        ...
        </div>`;const C=Math.round((t.projectValue||t.fleetValue||t.amount)*.7),$=Math.round(t.amount*.33),nt=(H-$)/(C-$)*100;i+=`<div class="rd-control">
            <div class="rd-control__header">
                <span class="rd-control__label">COVERAGE AMOUNT</span>
                <span class="rd-control__value" style="font-size:14px;color:#e8e4dc;">${n(H)}</span>
            </div>
            <input type="range" class="rd-control__range" min="${$}" max="${C}" step="1000000" value="${H}"
            ...
            <div style="font-family:var(--font-mono);font-size:7px;color:#6a6660;margin-top:4px;">Policyholder pays first ${Q}% of any claim (${n(Math.round(H*Q/100))})</div>
        ...
        </div>`,i+='<div class="rd-section-bar" style="color:#8a6aaa;">Purchase Amount</div>';const S=t.amount,V=Math.max(5e6,Math.ceil(S*.05/5e6)*5e6),W=(q-V)/(S-V)*100;i+=`<div class="rd-control">
            <div class="rd-control__header">
                <span class="rd-control__label">BUY AMOUNT</span>
                <span class="rd-control__value" style="font-size:16px;color:#8a6aaa;">${n(q)}</span>
            </div>
            <input type="range" class="rd-control__range" min="${V}" max="${S}" step="5000000" value="${q}"
        ...
        </div>`,i+='<div class="rd-section-bar" style="color:#5c5;">Projected Returns</div>',i+=`<div class="rd-risk-row"><span class="rd-risk-label">CAPITAL AT RISK</span><span class="rd-risk-value" style="color:#c55;">${n(q)}</span></div>`,i+=`<div class="rd-risk-row"><span class="rd-risk-label">ANNUAL COUPON</span><span class="rd-risk-value" style="color:#5c5;">${n(kt)}</span></div>`,i+=`<div class="rd-risk-row"><span class="rd-risk-label">TOTAL RETURN (${Math.round(ht)}yr)</span><span class="rd-risk-value" style="color:#5c5;">${n($t)}</span></div>`,i+=`<div class="rd-expected"><span style="font-family:var(--font-mono);font-size:9px;font-weight:700;color:#e8e4dc;">EXPECTED RETURN</span><span style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:#5c5;">${n(At)}</span></div>`,i+=`<div class="rd-formula">Risk-adjusted: total coupon × (1 - ${Y}% default)</div>`),i+="</div>",i+="</div>";const Dt=_==="LOAN"?M:_==="INSURE"?H:q,Mt=_==="LOAN"?d:_==="INSURE"?st:At,zt=_==="LOAN"?y:_==="INSURE"?h:Y,Ut=_==="LOAN"?E:_==="INSURE"?pt:ft,Pt=_==="LOAN"?"ISSUE LOAN":_==="INSURE"?"WRITE POLICY":"BUY BONDS",Bt=_.toLowerCase();i+=`<div class="rd-footer">
