import{_supabase as g}from"./supabase-client-qEAQbBjE.js";import{countEligibleParties as S,escapeHtml as u}from"./utils-A98FEun4.js";const z={Melizea:"assets/flags/Melizea.png",Avelia:"assets/flags/Avelia.png",Sangreza:"assets/flags/sangreza.png",Montequilla:"assets/flags/Montequilla.png","San Estrella":"assets/flags/sanestrella.png",Palvera:"assets/flags/Palvera.png",Calveth:"assets/flags/Calveth.png",Flandis:"assets/flags/Flandis.png",Vostia:"assets/flags/Vostia.png",Sierramar:"assets/flags/Sierramar.png",Hajjara:"assets/flags/Hajjara.png",Dravka:"assets/flags/Dravka.png",Danwei:"assets/flags/Danwei.png"},N={Melizea:"A parliamentary republic with a diverse economy. Analogous to Colombia.",Sangreza:"A parliamentary democracy with strong traditions. Analogous to Spain.","San Estrella":"A presidential republic with a growing economy. Analogous to Mexico.",Palvera:"A presidential nation rich in natural resources. Analogous to Venezuela.",Montequilla:"A parliamentary republic with a service-driven economy. Analogous to Argentina.",Avelia:"A semi-presidential republic blending Spanish and Italian culture. Analogous to Italy.",Calveth:"A parliamentary democracy with strong social systems. Analogous to Denmark.",Flandis:"A parliamentary democracy with a trade-focused economy. Analogous to the Netherlands.",Vostia:"A parliamentary nation with a complex political landscape. Analogous to Serbia.",Sierramar:"A small Caribbean island democracy with a young, religious population. Analogous to Puerto Rico.",Hajjara:"A vast desert monarchy ruled by an absolute king, rich in oil and gas with deep Islamic traditions. Analogous to Iran.",Dravka:"Dravka is analogous culturally to Albania.",Danwei:"This island nation in Faresia is analogous to real world Taiwan."},_=["#5b9bd5","#d48a3c","#5aafa5","#8b7ec8","#5cb85c","#d9534f","#c8a64e"];let w=null,I=[],y=null,L="Crucera",v={},h={};const $=new Set;async function H(){try{const{data:{user:e}}=await g.auth.getUser();if(!e){window.location.href="login.html";return}if(w=e,sessionStorage.getItem("pending_faction_type")==="corp"){window.location.href="corp-setup.html";return}const n=sessionStorage.getItem("pending_faction_type")==="party",{data:i}=await g.from("factions").select("id, nation_id, faction_type").eq("id",e.id).maybeSingle();if(i&&i.nation_id&&!(n&&i.faction_type==="corporation")){window.location.href="dashboard.html";return}await G(),document.getElementById("loading").style.display="none";const o=document.getElementById("page-content");o.style.display="flex"}catch(e){document.getElementById("loading").textContent="FAILED TO LOAD — PLEASE REFRESH";const n=document.getElementById("error-message");n&&(n.textContent=e.message||"Unknown error",n.style.display="block")}}function V(e){L=e,document.querySelectorAll(".continent-tab").forEach(n=>{n.classList.toggle("active",n.dataset.continent===e)}),B(I,v,h)}async function G(){const[e,n,i]=await Promise.all([g.from("nations").select("*, nation_profiles(flag_url)"),g.from("factions").select("id, faction_type, faction_name, abbreviation, seats, nation_id, party_color").eq("faction_type","party"),g.from("active_crises").select("nation_id, crisis_id, crisis_templates(name)")]),o=e.data||[],l=n.data||[],t=i.data||[];v={},h={};for(const a of l)v[a.nation_id]||(v[a.nation_id]=[]),v[a.nation_id].push(a);for(const a of t)h[a.nation_id]||(h[a.nation_id]=[]),h[a.nation_id].push(a);I=o,B(o,v,h)}function P(e){return e==="Presidential"||e==="Semi-Presidential"?"gov-presidential":"gov-democracy"}function T(e){const n=typeof e=="string"?e:e?.government_type,i=typeof e=="object"?e?.hos_election_method:null;return n==="Presidential"?"PRESIDENTIAL":n==="Semi-Presidential"?"SEMI-PRESIDENTIAL":i==="hereditary"?"CONSTITUTIONAL MONARCHY":"PARLIAMENTARY"}function B(e,n,i){const o=document.getElementById("nations-grid"),l=e.filter(t=>(t.continent||"Crucera")===L);if(l.sort((t,a)=>{const r=S(n[t.id]||[],t.id),m=S(n[a.id]||[],a.id),c=r>=(t.max_parties||8)?1:0,d=m>=(a.max_parties||8)?1:0;return c!==d?c-d:(t.name||"").localeCompare(a.name||"")}),l.length===0){o.innerHTML='<div style="font-family:var(--font-mono);font-size:10px;color:var(--text-dim);text-align:center;padding:40px 0;grid-column:1/-1;">No nations in this continent yet.</div>';return}o.innerHTML=l.map(t=>{const a=n[t.id]||[],r=i[t.id]||[],m=t.total_seats||100,c=t.max_parties||8,d=S(a,t.id),b=d>=c,A=t.nation_profiles,x=(Array.isArray(A)?A[0]?.flag_url:A?.flag_url)||t.flag_url||z[t.name]||`assets/flags/${t.name}.png`,k=P(t.government_type),D=T(t),F=$.has(t.id),C=[...a].sort((s,p)=>(p.seats||0)-(s.seats||0)),R=a.reduce((s,p)=>s+(p.seats||0),0),O=C.filter(s=>s.seats>0).map((s,p)=>{const E=(s.seats/m*100).toFixed(1),q=s.party_color||_[p%_.length];return`<div class="seat-bar__segment" style="width:${E}%;background:${q};" title="${u(s.faction_name)}: ${s.seats} seats"></div>`}).join(""),U=C.filter(s=>s.seats>0).map((s,p)=>`<span class="seat-legend-item"><span class="seat-legend-dot" style="background:${s.party_color||_[p%_.length]};"></span>${u(s.abbreviation||"?")} ${s.seats}</span>`).join(""),M=C.map((s,p)=>`
        <div class="party-row">
          <span class="party-row__dot" style="background:${s.party_color||_[p%_.length]};"></span>
          <span class="party-row__name">${u(s.faction_name)}</span>
          <span class="party-row__abbr">${u(s.abbreviation||"?")}</span>
          <span class="party-row__seats">${s.seats||0}</span>
        </div>
      `).join(""),j=r.length>0?r.map(s=>`
        <div class="crisis-row">
          <span class="crisis-pip"></span>
          <span class="crisis-name">${u(s.crisis_templates?.name||"Unknown Crisis")}</span>
        </div>
