import{_}from"./supabase-client-BXEzLDpS.js";import{e as g}from"./utils-C2W-HleY.js";import{initMessaging as Ni}from"./messaging-B5Fng3EZ.js";import{c as Ri,a as kt,E as Re,b as Qe,d as ii,e as Pi,f as Oi,h as Jt}from"./equipment-DsuDdEne.js";const ni={LOW:"#c84",STD:"#ca5",HIGH:"#5c5"},de=[{key:"concrete",name:"Concrete",category:"RAW",tiers:{LOW:{requirements:[]},STD:{requirements:[{stat:"manufacturing_output",min:20}]},HIGH:{requirements:[{stat:"manufacturing_output",min:50},{stat:"higher_education",min:40}]}},priceDrivers:["manufacturing_output","inflation","fuel_prices","urbanization"]},{key:"steel",name:"Steel",category:"RAW",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:10}]},STD:{requirements:[{stat:"manufacturing_output",min:35},{stat:"rare_minerals",min:20}]},HIGH:{requirements:[{stat:"manufacturing_output",min:60},{stat:"rare_minerals",min:40},{stat:"higher_education",min:45}]}},priceDrivers:["manufacturing_output","rare_minerals","inflation","fuel_prices"]},{key:"lumber",name:"Lumber",category:"RAW",tiers:{LOW:{requirements:[{stat:"arable_land",min:10}]},STD:{requirements:[{stat:"arable_land",min:30},{stat:"physical_infrastructure",min:20}]},HIGH:{requirements:[{stat:"arable_land",min:50},{stat:"manufacturing_output",min:30}]}},priceDrivers:["arable_land","physical_infrastructure","inflation"]},{key:"aggregate",name:"Aggregate",category:"RAW",tiers:{LOW:{requirements:[]},STD:{requirements:[{stat:"rare_minerals",min:15},{stat:"physical_infrastructure",min:20}]},HIGH:{requirements:[{stat:"rare_minerals",min:35},{stat:"manufacturing_output",min:25}]}},priceDrivers:["rare_minerals","physical_infrastructure","inflation"]},{key:"em",name:"E&M Systems",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:15}]},STD:{requirements:[{stat:"manufacturing_output",min:40},{stat:"digital_infrastructure",min:25}]},HIGH:{requirements:[{stat:"manufacturing_output",min:55},{stat:"digital_infrastructure",min:50},{stat:"energy_generation",min:40}]}},priceDrivers:["manufacturing_output","digital_infrastructure","inflation","energy_generation"]},{key:"glass",name:"Glass & Facades",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:20}]},STD:{requirements:[{stat:"manufacturing_output",min:40},{stat:"standard_of_living",min:35}]},HIGH:{requirements:[{stat:"manufacturing_output",min:60},{stat:"digital_infrastructure",min:40},{stat:"higher_education",min:50}]}},priceDrivers:["manufacturing_output","standard_of_living","inflation"]},{key:"asphalt",name:"Asphalt",category:"RAW",tiers:{LOW:{requirements:[{stat:"oil_and_gas",min:10}]},STD:{requirements:[{stat:"oil_and_gas",min:30},{stat:"manufacturing_output",min:25}]},HIGH:{requirements:[{stat:"oil_and_gas",min:45},{stat:"manufacturing_output",min:40},{stat:"physical_infrastructure",min:40}]}},priceDrivers:["oil_and_gas","manufacturing_output","inflation","fuel_prices"]},{key:"heavy",name:"Heavy Machinery Parts",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:40},{stat:"rare_minerals",min:30}]},STD:{requirements:[{stat:"manufacturing_output",min:60},{stat:"rare_minerals",min:45},{stat:"higher_education",min:40}]},HIGH:{requirements:[{stat:"manufacturing_output",min:75},{stat:"rare_minerals",min:60},{stat:"higher_education",min:55},{stat:"digital_infrastructure",min:45}]}},priceDrivers:["manufacturing_output","rare_minerals","higher_education","digital_infrastructure"]}];function he(i,e,t){const a=de.find(l=>l.key===i);if(!a)return{available:!1,failedStat:"unknown_material"};const n=a.tiers[e];if(!n)return{available:!1,failedStat:"unknown_tier"};for(const l of n.requirements){const s=Number(t?.[l.stat]??0);if(s<l.min)return{available:!1,failedStat:l.stat,failedMin:l.min,nationValue:s}}return{available:!0}}function Bt(i,e,t){const n={concrete:{LOW:200,STD:300,HIGH:500},steel:{LOW:400,STD:700,HIGH:1200},lumber:{LOW:80,STD:130,HIGH:200},aggregate:{LOW:40,STD:60,HIGH:100},em:{LOW:400,STD:700,HIGH:1200},glass:{LOW:300,STD:500,HIGH:900},asphalt:{LOW:120,STD:200,HIGH:350},heavy:{LOW:800,STD:1400,HIGH:2400}}[i]?.[e];if(!n)return 0;const l=de.find(d=>d.key===i);if(!l)return n;let s=1;for(const d of l.priceDrivers){const r=Number(t?.[d]??50);d==="inflation"||d==="fuel_prices"?s*=1+(r-50)/200:s*=1-(r-50)/250}return s=Math.max(.4,Math.min(2.5,s)),Math.round(n*s)}function ai(i,e,t){const n={concrete:{LOW:5e3,STD:3e3,HIGH:1e3},steel:{LOW:2e3,STD:1500,HIGH:500},lumber:{LOW:8e3,STD:4e3,HIGH:1500},aggregate:{LOW:15e3,STD:6e3,HIGH:2e3},em:{LOW:1e3,STD:700,HIGH:300},glass:{LOW:1500,STD:800,HIGH:300},asphalt:{LOW:4e3,STD:2e3,HIGH:800},heavy:{LOW:400,STD:200,HIGH:80}}[i]?.[e]||0,s=de.find(o=>o.key===i)?.priceDrivers?.[0],r=.3+(s?Number(t?.[s]??50):50)/50*.7;return Math.round(n*r)}const qt=["LOW","STD","HIGH"],Et={LOW:"Low",STD:"Standard",HIGH:"High"};let Le=[],f=null,k=null,P=null,Ce=[],Ne={},ee=[],j={},Tt=-1,D="concrete",H="STD",we=500,te=[],Ct=0,Y="trucks",ae=0,oe=1,ye=[],ke=null,Ve=[],It=null,We=null,Mt="ALL",St="TIMELINE";function B(i){return Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(1)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i}function Di(i){if(i>=12){const e=Math.floor(i/12),t=i%12;return t>0?e+"y "+t+"mo":e+"y"}return i+" ticks"}function K(i){return Math.abs(i)>=1e9?"$"+(i/1e9).toFixed(1)+"B":Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(0)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i}function Ke(i){return i==="civil_engineering"?"CIVIL":i==="industrial"?"INDUSTRIAL":i==="mega_project"?"MEGA":i?.toUpperCase()||"—"}function oi(i){return i==="civil_engineering"?"light":i==="industrial"?"heavy":i==="mega_project"?"mega":"light"}function Hi(){We&&clearInterval(We),We=setInterval(()=>{if(!It)return;const i=It-Date.now();if(i<=0){document.getElementById("tick-countdown").textContent="Tick due...",clearInterval(We);return}const e=Math.floor(i/36e5),t=Math.floor(i%36e5/6e4),a=Math.floor(i%6e4/1e3);document.getElementById("tick-countdown").textContent=e+"h "+t+"m "+a+"s"},1e3)}function ji(){document.body.classList.toggle("light-mode");const i=document.getElementById("theme-toggle");i.textContent=document.body.classList.contains("light-mode")?"Dark":"Light"}function Ui(i,e){i==="type"&&(Mt=e),i==="sort"&&(St=e),document.querySelectorAll(`.filter-pill[data-filter="${i}"]`).forEach(t=>{t.classList.toggle("active",t.dataset.value===e)}),ri()}function si(i){return!(!f||i.sector==="mega_project"&&f.corp_subsector!=="Megaprojects"||i.sector==="industrial"&&!["Heavy Infrastructure","Megaprojects"].includes(f.corp_subsector))}function ri(){const i=document.getElementById("oc-list");let e=[...Ce];if(Mt==="GOVERNMENT"?e=e.filter(n=>n.issuer_type==="GOVERNMENT"):Mt==="PRIVATE"&&(e=e.filter(n=>n.issuer_type==="PRIVATE")),St==="TIMELINE"&&e.sort((n,l)=>(n.timeline_ticks||0)-(l.timeline_ticks||0)),St==="BUDGET"&&e.sort((n,l)=>(l.budget_ceiling||0)-(n.budget_ceiling||0)),document.getElementById("oc-count").textContent=e.length+" AVAILABLE",e.length===0){i.innerHTML=`
            <div class="oc-empty">
            ...
            </div>`;return}const t=P?.current_tick||0;let a="";for(const n of e){const l=n.issuer_type==="GOVERNMENT",s=l?"gov":"private",d=si(n),r=d?"":" locked",o=oi(n.sector),c=Ke(n.sector),m=(n.timeline_ticks||0)>18?" warn":"",p=n.bidding_ends_tick?Math.max(0,n.bidding_ends_tick-t):"?";a+=`
            <div class="oc-item${r}" data-contract-id="${n.id}">
                ...
                <div class="oc-item__stats">
                    ...
                    <div class="oc-stat">
                        ...
                        <div class="oc-stat__value${m}">${Di(n.timeline_ticks||0)}</div>
                    </div>
                    <div class="oc-stat">
            ...
            </div>`}i.innerHTML=a,i.querySelectorAll(".oc-item:not(.locked)").forEach(n=>{n.addEventListener("click",()=>{const l=n.dataset.contractId,s=Ce.find(d=>d.id===l);s&&li(s)})})}let ce=null;function li(i){ce=i;const e=document.getElementById("cd-overlay"),t=i.issuer_type==="GOVERNMENT",a=t?"gov":"private",n=(k?.name||f.nation||"—").toUpperCase(),l=si(i);document.getElementById("cd-header-left").innerHTML=`
    ...
    `;const s=document.getElementById("cd-blueprint");i.blueprint_svg?(s.innerHTML=i.blueprint_svg,s.style.display=""):(s.innerHTML=ln(i),s.style.display="");const d=i.permits_required||[],r=i.required_equipment||i.equipment_required||[],o=i.required_materials||i.materials_estimated||{},m={civil_engineering:"Civil Engineering",industrial:"Industrial Construction",mega_project:"Megaprojects"}[i.sector]||i.spec_category||i.sector||"—";let p="var(--teal)";i.sector==="industrial"&&(p="var(--orange)"),i.sector==="mega_project"&&(p="var(--red)");let u=B(i.budget_ceiling||i.budget||0),y=(i.timeline_ticks||i.timeline_months||0)+" Months",v="";v+=`
        <div class="cd-issue">
            ...
            </div>`),v+='<div class="cd-details">',i.project_type&&(v+=ue("Type",i.project_type)),i.project_subtype&&(v+=ue("Sub-Type",i.project_subtype)),v+=ue("Specialization",m,p),v+=ue("Total Budget",u,"var(--green)"),v+=ue("Timeline",y),v+=ue("Nation",k?.name||f.nation||"—"),i.region&&(v+=ue("Region",i.region)),v+="</div>",d.length>0&&(v+=`
            <div class="cd-items">
                <div class="cd-section-label">Permits Required</div>
                <div class="cd-items__list">
                    ${d.map(E=>{const M=E.status==="approved"?"approved":"required",R=E.status==="approved"?"&#10003;":"&#9675;";return`<div class="cd-chip ${M}">
    ...
    `,e.classList.add("open"),document.body.style.overflow="hidden"}function Je(i){i&&i.target&&i.target!==document.getElementById("cd-overlay")||(document.getElementById("cd-overlay").classList.remove("open"),document.body.style.overflow="",ce=null)}const Gi=[{key:"concrete",name:"Concrete",unit:"units"},{key:"steel",name:"Steel",unit:"units"},{key:"lumber",name:"Lumber",unit:"units"},{key:"aggregate",name:"Aggregate",unit:"units"},{key:"em_systems",name:"E&M Systems",unit:"units"},{key:"glass_facades",name:"Glass & Facades",unit:"units"},{key:"asphalt",name:"Asphalt",unit:"units"},{key:"heavy_parts",name:"Heavy Machinery Parts",unit:"units"}],Wi=[{key:"work_trucks",name:"Work Trucks",tier:1},{key:"excavators",name:"Excavators",tier:1},{key:"bulldozers",name:"Bulldozers",tier:1},{key:"concrete_mixers",name:"Concrete Mixers",tier:1},{key:"tower_cranes",name:"Tower Cranes",tier:2},{key:"heavy_haulers",name:"Heavy Haulers",tier:2},{key:"pile_drivers",name:"Pile Drivers",tier:2},{key:"asphalt_plants",name:"Asphalt Plants",tier:2}],di={LOW:.7,STANDARD:1,HIGH:1.4},ci={LOW:35,STANDARD:65,HIGH:90},ct=15;let U=null;function Fi(i){if(!i)return;const e=i.required_materials||{},t=i.required_equipment||[],a=i.required_workforce||{},n={concrete:18e4,steel:25e4,lumber:12e4,aggregate:8e4,em_systems:32e4,glass_facades:28e4,asphalt:14e4,heavy_parts:4e5},l=Gi.filter(c=>e[c.key]>0).map(c=>({...c,qty:e[c.key],basePrice:n[c.key]||2e5,grade:c.key==="aggregate"?"LOW":"STANDARD",highDisabled:!1})),s=Wi.filter(c=>t.includes(c.key)).map(c=>({...c,owned:(te||[]).some(m=>m.equipment_key===c.key&&m.quantity>0)})),d=(a.general||100)+(a.skilled||20),r=i.budget_ceiling||1e8,o=Math.round(r*.03);U={contract:i,budgetCeiling:r,materials:l,laborCount:d,laborRate:15200,estimatedTicks:i.timeline_ticks||8,equipment:s,permits:[],overhead:o,markupPct:15,competitors:[],playerRep:f?.standing||50,requiredWorkforce:a},document.getElementById("bid-title").textContent="BID ASSEMBLY",document.getElementById("bid-subtitle").textContent=(i.name||"Contract")+" — "+Ke(i.sector)+" — "+(i.issuer_name||"Government"),document.getElementById("bid-overlay").classList.add("open"),document.body.style.overflow="hidden",Xe()}function pi(i){i&&i.target!==document.getElementById("bid-overlay")||(document.getElementById("bid-overlay").classList.remove("open"),document.body.style.overflow="",U=null)}function N(i){return Math.abs(i)>=1e9?"$"+(i/1e9).toFixed(2)+"B":Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(2)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i.toLocaleString()}function Vi(i,e){if(!U)return;const t=U.materials[i];e==="HIGH"&&t.highDisabled||(t.grade=e,Xe())}function Yi(i){U&&(U.laborCount=i,Xe())}function Qi(i){U&&(U.markupPct=Number(i),Xe())}function Xe(){if(!U)return;const i=U;let e=0;for(const M of i.materials)M.lineCost=Math.round(M.qty*M.basePrice*di[M.grade]),e+=M.lineCost;const t=Math.round(i.laborCount*i.laborRate*i.estimatedTicks),a=Math.round(i.equipment.filter(M=>M.owned).length*12e3*i.estimatedTicks);let n=0;const l=i.overhead,s=e+t+a+n+l,d=Math.round(s*i.markupPct/100),r=s+d,o=r>i.budgetCeiling,c=d,m=Math.round(i.materials.reduce((M,R)=>M+ci[R.grade],0)/i.materials.length),p=m>=80?"STRONG":m>=60?"PROMISING":m>=40?"UNCERTAIN":"POOR",u=m>=80?"var(--green)":m>=60?"var(--teal)":m>=40?"var(--orange)":"var(--red)",y=i.budgetCeiling>0?r/i.budgetCeiling:1,v=Math.max(0,Math.min(100,Math.round((1-y)*150))),h=v>=70?"STRONG":v>=40?"COMPETITIVE":v>=15?"WEAK":"UNLIKELY",$=v>=70?"var(--green)":v>=40?"var(--teal)":v>=15?"var(--orange)":"var(--red)",w=Math.round(s*(1-ct/100)),A=Math.round(s*(1+ct/100));let x="";x+='<div class="bid-section"><div class="bid-section__title">Materials</div>',i.materials.forEach((M,R)=>{const q=V=>{const X=M.grade===V,Ue=V==="HIGH"&&M.highDisabled;return`<button class="bid-grade-btn ${X?V==="LOW"?"active-low":V==="HIGH"?"active-high":"active":""} ${Ue?"disabled":""}" onclick="setBidGrade(${R},'${V}')">${V[0]}</button>`};x+=`<div class="bid-mat-row">
            <span class="bid-mat-row__name">${g(M.name)}</span>
            <span class="bid-mat-row__qty">×${M.qty}</span>
            <div class="bid-grade-btns">${q("LOW")}${q("STANDARD")}${q("HIGH")}</div>
            <span class="bid-mat-row__cost">${N(M.lineCost)}</span>
        </div>`}),x+=`<div class="bid-line-total"><span class="bid-line-total__label">MATERIALS TOTAL</span><span class="bid-line-total__value">${N(e)}</span></div></div>`;const T=(i.requiredWorkforce?.general||80)+(i.requiredWorkforce?.skilled||20),C=[Math.round(T*.8),T,Math.round(T*1.2),Math.round(T*1.4),Math.round(T*1.6)];x+='<div class="bid-section"><div class="bid-section__title">Labor</div>',x+='<div class="bid-labor-presets">',C.forEach(M=>{x+=`<button class="bid-labor-btn ${i.laborCount===M?"active":""}" onclick="setBidLabor(${M})">${M}</button>`}),x+="</div>";const S=i.requiredWorkforce||{};x+=`<div class="bid-labor-formula">Required: ${S.general||"?"} general + ${S.skilled||"?"} skilled<br>`,x+=`${i.laborCount} workers × ${N(i.laborRate)}/tick × ${i.estimatedTicks} ticks = <strong>${N(t)}</strong></div>`,x+=`<div class="bid-line-total"><span class="bid-line-total__label">LABOR TOTAL</span><span class="bid-line-total__value">${N(t)}</span></div></div>`,x+='<div class="bid-section"><div class="bid-section__title">Equipment</div>',i.equipment.forEach(M=>{const R=M.owned?"bid-equip-row__status--owned":"bid-equip-row__status--missing",q=M.owned?"✓ OWNED":"✗ NOT OWNED";x+=`<div class="bid-equip-row"><span class="bid-equip-row__name">${g(M.name)}</span><span class="bid-equip-row__status ${R}">${q}</span></div>`}),x+=`<div class="bid-line-total"><span class="bid-line-total__label">MAINTENANCE (${i.estimatedTicks}t)</span><span class="bid-line-total__value">${N(a)}</span></div></div>`,x+='<div class="bid-section"><div class="bid-section__title">Permits</div>',x+='<div style="font-family:var(--font-mono);font-size:9px;color:var(--text-dim);padding:8px 0;">No permits required yet.</div>',x+='<div class="bid-line-total"><span class="bid-line-total__label">PERMITS TOTAL</span><span class="bid-line-total__value">$0</span></div></div>',x+='<div class="bid-section"><div class="bid-section__title">Overhead &amp; Contingency</div>',x+='<div style="font-family:var(--font-mono);font-size:9px;color:var(--text-dim);">Site management, insurance, admin</div>',x+=`<div class="bid-line-total"><span class="bid-line-total__label">OVERHEAD</span><span class="bid-line-total__value">${N(l)}</span></div></div>`,document.getElementById("bid-left").innerHTML=x;let E="";E+='<div class="bid-section"><div class="bid-section__title">Cost Summary</div>',E+=`<div class="bid-summary-row"><span class="bid-summary-row__label">Materials</span><span class="bid-summary-row__value">${N(e)}</span></div>`,E+=`<div class="bid-summary-row"><span class="bid-summary-row__label">Labor</span><span class="bid-summary-row__value">${N(t)}</span></div>`,E+=`<div class="bid-summary-row"><span class="bid-summary-row__label">Equipment Maint.</span><span class="bid-summary-row__value">${N(a)}</span></div>`,E+=`<div class="bid-summary-row"><span class="bid-summary-row__label">Permits</span><span class="bid-summary-row__value">${N(n)}</span></div>`,E+=`<div class="bid-summary-row"><span class="bid-summary-row__label">Overhead</span><span class="bid-summary-row__value">${N(l)}</span></div>`,E+=`<div class="bid-cost-total"><span class="bid-cost-total__label">ESTIMATED COST</span><span class="bid-cost-total__value">${N(s)}</span></div>`,E+=`<div class="bid-accuracy">⚠ Estimate accuracy: ±${ct}%<br>Actual cost range: ${N(w)} — ${N(A)}</div>`,E+="</div>",E+='<div class="bid-section"><div class="bid-section__title">Markup</div>',E+=`<div class="bid-slider-wrap">
    ...
    </div>`,E+=`<div class="bid-profit"><span class="bid-profit__label">PROJECTED PROFIT</span><span class="bid-profit__value">+${N(c)}</span></div>`,E+=`<div class="bid-compete">
    ...
    </div>`,E+='<div class="bid-section" style="margin-top:8px;"><div class="bid-section__title">Competing Bids</div>',i.competitors.forEach(M=>{E+=`<div class="bid-competitor"><span class="bid-competitor__name">${g(M.name)}</span><span class="bid-competitor__rep">Rep ${M.rep}</span></div>`}),E+=`<div class="bid-competitor" style="color:var(--gold);"><span class="bid-competitor__name">You</span><span class="bid-competitor__rep">Rep ${i.playerRep}</span></div>`,E+="</div>",document.getElementById("bid-right").innerHTML=E,document.getElementById("bid-footer-price").textContent=N(r),document.getElementById("bid-footer-price").style.color=o?"var(--red)":"var(--gold)",document.getElementById("bid-footer-profit").textContent="+"+N(c),document.getElementById("bid-footer-quality").textContent=m+"/100",document.getElementById("bid-footer-quality").style.color=u,document.getElementById("bid-submit-btn").disabled=o}window.openBidModal=Fi;window.closeBidModal=pi;window.setBidGrade=Vi;window.setBidLabor=Yi;window.setBidMarkup=Qi;async function Ki(){if(!U||!f||Ae)return;const i=U,e=i.contract;let t=0;const a={};for(const m of i.materials)t+=Math.round(m.qty*m.basePrice*di[m.grade]),a[m.key]=m.grade;const n=Math.round(i.laborCount*i.laborRate*i.estimatedTicks),l=Math.round(i.equipment.filter(m=>m.owned).length*12e3*i.estimatedTicks),s=t+n+l+i.overhead,d=Math.round(s*i.markupPct/100),r=s+d,o=Math.round(i.materials.reduce((m,p)=>m+ci[p.grade],0)/(i.materials.length||1));if(r>i.budgetCeiling){alert("Bid exceeds budget ceiling. Lower your costs or markup.");return}const c=document.getElementById("bid-submit-btn");c.disabled=!0,c.textContent="SUBMITTING...",Ae=!0;try{const{data:m}=await _.from("shard").select("current_tick").eq("name","Alpha Shard").single(),p=m?.current_tick||0,{data:u}=await _.from("contract_bids").select("id").eq("contract_id",e.id).eq("faction_id",f.id).maybeSingle();if(u){const{error:v}=await _.from("contract_bids").update({bid_price:r,material_grades:a,labor_count:i.laborCount,markup_pct:i.markupPct,estimated_cost:s,estimated_quality:o,submitted_at_tick:p}).eq("id",u.id);if(v)throw v}else{const{error:v}=await _.from("contract_bids").insert({contract_id:e.id,faction_id:f.id,bid_price:r,material_grades:a,labor_count:i.laborCount,markup_pct:i.markupPct,estimated_cost:s,estimated_quality:o,status:"pending",submitted_at_tick:p});if(v)throw v}pi();const y=document.getElementById("oc-count");if(y){const v=y.textContent;y.textContent="✓ BID SUBMITTED",y.style.color="var(--green)",setTimeout(()=>{y.textContent=v,y.style.color=""},2e3)}await pe()}catch(m){console.error("Bid submission failed:",m),alert("Failed to submit bid: "+(m.message||"Unknown error")),c.disabled=!1,c.textContent="SUBMIT BID"}finally{Ae=!1}}window.submitBid=Ki;const ge=["Permits","Planning","Foundation","Structural","Systems","Finishing","Delivery"],Xt={WEATHER:{color:"var(--blue)",bg:"var(--blue-faint)",border:"var(--blue-border)"},SUPPLY:{color:"var(--gold)",bg:"var(--gold-faint)",border:"var(--gold-border)"},LABOR:{color:"var(--orange)",bg:"var(--orange-faint)",border:"var(--orange-border)"},REGULATORY:{color:"var(--red)",bg:"var(--red-faint)",border:"var(--red-border)"},EQUIPMENT:{color:"var(--amber)",bg:"var(--amber-faint)",border:"var(--amber-border)"},POLITICAL:{color:"var(--teal)",bg:"var(--teal-faint)",border:"var(--teal-border)"}},Zt={LOW:"var(--green)",MODERATE:"var(--amber)",HIGH:"var(--orange)",CRITICAL:"var(--red)"};let L=null;function Ji(i){const e=ee.find(S=>S.id===i);if(!e)return;const t=Array.isArray(e.contract_bids)?e.contract_bids[0]:e.contract_bids,a=P?.current_tick||0,n=e.awarded_at_tick||a,l=e.timeline_ticks||8,s=Math.max(0,a-n),d=Math.min(100,s/l*100);let r=Math.min(ge.length-1,Math.floor(d/(100/ge.length)));const o=Math.round(d%(100/ge.length)/(100/ge.length)*100),c=e.required_materials||{},m=t?.material_grades||{},p=Object.entries(c).map(([S,E])=>{const M=m[S]||"STANDARD",R=Math.round(E*(d/100)*(.6+Math.random()*.4));return{key:S,name:S.replace(/_/g," ").replace(/\b\w/g,q=>q.toUpperCase()),grade:M,allocated:E,used:Math.min(R,E)}}),y=(e.required_equipment||[]).map(S=>({key:S,name:S.replace(/_/g," ").replace(/\b\w/g,E=>E.toUpperCase()),qty:1+Math.floor(Math.random()*3),condition:55+Math.floor(Math.random()*40)})),v=e.budget_ceiling||0,h=t?.estimated_cost||0,$=Math.round(h*Math.min(1,s/l)),w=t?.estimated_quality||65,A=w>=80?"STRONG":w>=60?"PROMISING":w>=40?"FAIR":"UNCERTAIN",x=e.required_workforce||{},T=(x.general||0)+(x.skilled||0),C=t?.labor_count||T;L={project:e,bid:t,tab:"overview",expandedEvent:-1,selectedResponse:null,currentTick:a,awardedTick:n,totalTicks:l,ticksElapsed:s,phaseIdx:r,phaseProgress:o,materials:p,equipment:y,budget:v,estCost:h,spent:$,quality:w,qualityLabel:A,laborCount:C,wfNeeded:T,events:[]},document.getElementById("pm-overlay").classList.add("open"),document.body.style.overflow="hidden",mi(e.id).then(()=>Ee()),Ee()}function Xi(i){i&&i.target!==document.getElementById("pm-overlay")||(document.getElementById("pm-overlay").classList.remove("open"),document.body.style.overflow="",L=null)}function Zi(i){L&&(L.tab=i,L.expandedEvent=-1,L.selectedResponse=null,Ee())}function en(i){L&&(L.expandedEvent=L.expandedEvent===i?-1:i,L.selectedResponse=null,Ee())}function tn(i){L&&(L.selectedResponse=L.selectedResponse===i?null:i,Ee())}function Ee(){if(!L)return;const i=L,e=i.project,t=e.issuer_type==="GOVERNMENT",a=Ke(e.sector),n=f?.nation||"Nation",l=i.awardedTick+i.totalTicks,s=Math.max(0,l-i.currentTick),d=i.currentTick>l,r=i.budget>0?Math.round(i.spent/i.budget*100):0,o=r>85?"var(--red)":r>60?"var(--amber)":"var(--teal)",c=i.budget-i.spent,m=i.events.filter(h=>h.status==="ACTIVE").length;document.getElementById("pm-header").innerHTML=`
    ...
    `;let p='<div class="pm-phase__bar">';for(let h=0;h<ge.length;h++){const $=h<i.phaseIdx,w=h===i.phaseIdx;p+=`<div class="pm-phase__seg">
            <div class="pm-phase__seg-fill pm-phase__seg-fill--${$?"done":w?"active":"future"}"></div>
            <span class="pm-phase__seg-label pm-phase__seg-label--${$?"done":w?"active":""}">${ge[h]}</span>
        </div>`}p+="</div>",p+=`<div class="pm-phase__info">
        <span class="pm-phase__status">${ge[i.phaseIdx]} — ${i.phaseProgress}% complete</span>
    ...
    `}function nn(i,e,t,a,n,l,s){const d=pt(i.awardedTick+i.totalTicks);pt(i.awardedTick+i.totalTicks);const r=pt(i.awardedTick),o=[{label:"Budget",value:K(i.budget),sub:`${a}% spent`,color:t},{label:"Spent",value:K(i.spent),color:"var(--red)"},{label:"Remaining",value:K(n),color:"var(--green)"},{label:"Quality",value:`${i.quality}/100`,sub:i.qualityLabel,color:i.quality>=70?"var(--green)":i.quality>=50?"var(--amber)":"var(--red)"},{label:"Workforce",value:`${i.laborCount}/${i.wfNeeded}`,sub:`Bid: ${i.laborCount}`,color:i.laborCount<i.wfNeeded?"var(--orange)":"var(--text-bright)"},{label:"Remaining",value:`${l} ticks`,sub:s?"OVERDUE":`Deadline: ${d}`,color:s?"var(--red)":"var(--text-bright)"}];let c="";c+=`<div style="padding:0 16px"><div class="pm-section">
        ...
        <div class="pm-manager">
            ...
            </div>`;c+="</div></div>"}return c}function an(i){if(i.events.length===0)return'<div class="pm-evt-empty">No events have occurred on this project yet.<br>Events will appear here as construction progresses.</div>';let e="";for(let t=0;t<i.events.length;t++){const a=i.events[t],n=i.expandedEvent===t,l=a.status==="ACTIVE",s=Xt[a.type]||Xt.WEATHER,d=Zt[a.severity]||Zt.LOW;if(e+=`<div class="pm-evt ${l?"pm-evt--active":"pm-evt--resolved"}" style="${l?`border-left-color:${s.color}`:""}">`,e+=`<div class="pm-evt__header" onclick="pmToggleEvent(${t})" style="${n?`background:${s.bg}`:""}">`,e+=`<div class="pm-evt__row1">
        ...
        </div>`}return e}function sn(i){if(i.equipment.length===0)return'<div class="pm-evt-empty">No equipment deployed to this project.</div>';let e='<div class="pm-tab-header">Deployed Equipment</div>';for(const t of i.equipment){const a=t.condition>=75?"var(--green)":t.condition>=50?"var(--amber)":t.condition>=25?"var(--orange)":"var(--red)",n=t.condition<60;e+=`<div class="pm-eq">
        ...
        </div>`}return e}function pt(i){return`${["January","February","March","April","May","June","July","August","September","October","November","December"][i%12]}, ${2e3+Math.floor(i/12)}`}window.openProjectModal=Ji;window.closeProjectModal=Xi;window.pmSetTab=Zi;window.pmToggleEvent=en;window.pmSelectResponse=tn;async function mi(i){if(!L)return;const{data:e,error:t}=await _.from("construction_events").select("*").eq("contract_id",i).order("fired_at_tick",{ascending:!1});t?(console.warn("Failed to load project events:",t.message),L.events=[]):L.events=(e||[]).map(a=>({id:a.id,type:a.type,severity:a.severity,tick:a.fired_at_tick,title:a.title,desc:a.description,impact:a.impact,status:a.status==="ACTIVE"?"ACTIVE":"RESOLVED",resolution:a.resolution,responses:a.responses||[]})),Ee()}let mt=!1;async function rn(i,e){if(!(mt||!L)){mt=!0;try{const{data:t,error:a}=await _.rpc("resolve_construction_event",{p_event_id:i,p_response_key:e});if(a){console.error("Failed to resolve event:",a.message),alert("Failed to submit response: "+a.message);return}const n=typeof t=="string"?JSON.parse(t):t;if(n?.error){alert("Error: "+n.error);return}await mi(L.project.id),await fi(),n?.quality_applied&&n.quality_applied!==0&&(L.quality=Math.max(0,Math.min(100,L.quality+n.quality_applied)),L.qualityLabel=L.quality>=80?"STRONG":L.quality>=60?"PROMISING":L.quality>=40?"FAIR":"UNCERTAIN"),Ee()}finally{mt=!1}}}window.confirmEventResponse=rn;function ue(i,e,t){const a=t?` style="color:${t}"`:"";return`<div class="cd-detail-row">
        ...
        <g transform="translate(470,172)">
    ...
    </svg>`}let ft=!1;async function dn(){if(ft||!ce||!f)return;if((f.action_points??0)<2){alert("You need at least 2 AP to place a bid.");return}ft=!0;const i=document.querySelector(".cd-btn.primary");i&&(i.disabled=!0,i.textContent="...");try{const{data:e,error:t}=await _.rpc("deduct_ap",{p_faction_id:f.id,p_cost:2});if(t)throw t;if(e<0){const n=-e-1;f.action_points=n,document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+n+" AP</span>",i&&(i.disabled=!1,i.textContent="BID"),alert("Insufficient AP. You have "+n+" AP, need 2.");return}const{error:a}=await _.from("corp_contract_bids").insert({contract_id:ce.id,faction_id:f.id,nation_id:f.nation_id,ap_spent:2,created_at_tick:P?.current_tick||null});if(a)throw await _.rpc("deduct_ap",{p_faction_id:f.id,p_cost:-2}),f.action_points=e+2,document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+(e+2)+" AP</span>",a;f.action_points=e,document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+e+" AP</span>",i&&(i.textContent="BID PLACED")}catch(e){i&&(i.disabled=!1,i.textContent="BID"),e.code==="23505"?alert("You have already placed a bid on this contract."):alert("Failed to place bid: "+(e.message||"Unknown error"))}finally{ft=!1}}async function pe(){if(!f||!f.nation_id)return;const{data:i,error:e}=await _.from("construction_contracts").select("*").in("status",["open","bidding"]).order("generated_at_tick",{ascending:!1});if(e?(console.warn("Failed to load contracts:",e.message),Ce=[]):Ce=i||[],Ne={},f&&Ce.length>0){const t=Ce.map(n=>n.id),{data:a}=await _.from("contract_bids").select("contract_id, bid_price, estimated_quality, status").eq("faction_id",f.id).in("contract_id",t);for(const n of a||[])Ne[n.contract_id]=n}ri()}function cn(){const i=document.getElementById("ap-list"),e=document.getElementById("ap-footer");if(document.getElementById("ap-count").textContent=ee.length+" ACTIVE",ee.length===0){i.innerHTML=`<div class="ap-empty">
            <div class="ap-empty__text">No active projects.<br>Win a contract bid to start<br>your first construction project.</div>
        </div>`,e.style.display="none";return}const t=P?.current_tick||0;let a=0,n=0,l="";for(const s of ee){const d=s.issuer_type==="GOVERNMENT",r=d?"gov":"private",o=Array.isArray(s.contract_bids)?s.contract_bids[0]:s.contract_bids,c=o?.bid_price||0,m=o?.estimated_cost||0,p=o?.estimated_quality||0,u=s.budget_ceiling||0,y=s.awarded_at_tick||t,v=y+(s.timeline_ticks||8),h=Math.max(0,v-t),$=Math.max(0,t-y),w=s.timeline_ticks||8,A=Math.min(100,Math.round($/w*100)),x=t>v;oi(s.sector);const T=Ke(s.sector);a+=u,n+=c,l+=`<div class="ap-item" onclick="openProjectModal('${s.id}')">
            ...
            <div class="ap-budget">
                <div class="ap-budget__header">
                    ...
                    <span class="ap-budget__values" style="color:${x?"var(--red)":"var(--teal)"}">
                        ${$}/${w} ticks ${x?'<span style="color:var(--red);font-weight:700;"> OVERDUE</span>':""}
                    </span>
                </div>
        ...
        </div>`}i.innerHTML=l,e.style.display=ee.length>0?"":"none",ee.length>0&&(document.getElementById("ap-total-crew").textContent=ee.length,document.getElementById("ap-total-budget").textContent=K(a),document.getElementById("ap-total-spent").textContent=K(n))}async function fi(){if(!f)return;const{data:i,error:e}=await _.from("construction_contracts").select("*, contract_bids!inner(bid_price, material_grades, labor_count, estimated_cost, estimated_quality)").eq("awarded_to_faction",f.id).in("status",["awarded","in_progress"]).eq("contract_bids.faction_id",f.id).order("awarded_at_tick",{ascending:!0});e?(console.warn("Failed to load active projects:",e.message),ee=[]):ee=i||[],cn()}const ui=3e4;function yi(){let i=0,e=0;for(const t of de)for(const a of qt){const n=j[t.key]?.[a];n&&(i+=n.qty,e+=n.value)}return{totalUnits:i,totalValue:e}}function Nt(){const i=document.getElementById("wh-list"),{totalUnits:e,totalValue:t}=yi();document.getElementById("wh-count").textContent=e.toLocaleString()+" UNITS",document.getElementById("wh-total-value").textContent=B(t);const a=Math.round(e/ui*100),n=document.getElementById("wh-capacity");n.textContent=a+"%",n.style.color=a>80?"var(--red)":a>50?"var(--orange)":"var(--green)";let l="";for(let s=0;s<de.length;s++){const d=de[s],r=Tt===s,o=j[d.key]?.LOW||{qty:0,value:0},c=j[d.key]?.STD||{qty:0,value:0},m=j[d.key]?.HIGH||{qty:0,value:0},p=o.qty+c.qty+m.qty,u=o.value+c.value+m.value,y=p===0,v=he(d.key,"LOW",k),h=he(d.key,"STD",k),$=he(d.key,"HIGH",k),w=o.qty>0?"wh-dot wh-dot--low":"wh-dot wh-dot--empty",A=c.qty>0?"wh-dot wh-dot--std":"wh-dot wh-dot--empty",x=$.available?m.qty>0?"wh-dot wh-dot--high":"wh-dot wh-dot--empty":"wh-dot wh-dot--locked";if(l+='<div class="wh-row">',l+=`<div class="wh-row__collapsed${r?" expanded":""}" onclick="toggleWhRow(${s})">
            ...
            <span class="wh-row__val${y?" empty":""}">${u>0?B(u):"—"}</span>
                ...
                <span class="wh-expand__hdr-label" style="width:55px;text-align:right">$/UNIT</span>
                    ...
                    </div>`);l+="</div>"}l+="</div>"}i.innerHTML=l}function pn(i){Tt=Tt===i?-1:i,Nt()}async function mn(){if(!f)return;const{data:i,error:e}=await _.from("corp_warehouse").select("material_key, quality_tier, quantity, total_value").eq("faction_id",f.id);if(j={},e)console.warn("Failed to load warehouse:",e.message);else if(i)for(const t of i)j[t.material_key]||(j[t.material_key]={}),j[t.material_key][t.quality_tier]={qty:t.quantity||0,value:Number(t.total_value)||0};Nt()}const fn={manufacturing_output:"base production",rare_minerals:"raw input",inflation:"price modifier",fuel_prices:"transport cost",urbanization:"demand pressure",arable_land:"base production",physical_infrastructure:"transport",digital_infrastructure:"component quality",energy_generation:"electrical supply",standard_of_living:"demand tier",oil_and_gas:"base input",higher_education:"engineering"};function vi(){const i=(k?.name||f?.nation||"—").toUpperCase();document.getElementById("pr-nation-badge").textContent="LOCAL — "+i;const e=Number(f?.corp_cash_reserves)||0;document.getElementById("pr-cash").textContent=B(e);const{totalUnits:t}=yi(),a=Math.round(t/ui*100),n=document.getElementById("pr-wh-capacity");n.textContent=a+"%",n.style.color=a>80?"var(--red)":a>50?"var(--orange)":"var(--green)",gi(),Rt(),Ze()}function gi(){const i=document.getElementById("pr-mat-grid");let e="";for(const t of de){const a=D===t.key,n=qt.every(s=>!he(t.key,s,k).available),l="pr-mat-btn"+(a?" active":"")+(n?" all-locked":"");e+=`<span class="${l}" onclick="setPrMat('${t.key}')">${g(t.name)}</span>`}i.innerHTML=e}function Rt(){const i=document.getElementById("pr-tier-bar");let e='<span class="pr-tier-label">GRADE</span>';for(const t of qt){const a=he(D,t,k),n=H===t,l=a.available?Bt(D,t,k):null,s=ni[t],d=!a.available,r="pr-tier-btn"+(n?" active":"")+(d?" locked":"");e+=`<div class="${r}" onclick="${d?"":`setPrTier('${t}')`}">
            <div class="pr-tier-btn__label">
                <div class="wh-dot" style="width:5px;height:5px;background:${s};border-radius:1px;"></div>
                <span class="pr-tier-btn__name" style="color:${n?"var(--text-bright)":"var(--text-dim)"}">${Et[t]}</span>
            </div>
        ...
        </div>`}i.innerHTML=e}function Ze(){const i=document.getElementById("pr-content"),e=he(D,H,k),t=de.find($=>$.key===D);if(!t)return;if(!e.available){i.innerHTML=`<div class="pr-locked-panel">
        ...
        </div>`;return}const a=Bt(D,H,k),n=ai(D,H,k),l=a*we,s=n>3e3?"LOW":n>1e3?"MODERATE":"HIGH",d=s==="LOW"?"var(--green)":s==="MODERATE"?"var(--amber)":"var(--red)",r=Number(k?.inflation??50),o=r>55?"up":r<45?"down":"flat",c=o==="up"?"&#9650;":o==="down"?"&#9660;":"&#8212;",m=o==="up"?"var(--red)":o==="down"?"var(--green)":"var(--text-dim)";let p="";p+=`<div style="padding:8px 14px;border-bottom:1px solid var(--border-0);">
        ...
        <div class="pr-drivers__title">Price Drivers (${g(k?.name||"—")})</div>`;for(const $ of t.priceDrivers){const w=Number(k?.[$]??50),A=w>=50?"var(--green)":w>=30?"var(--amber)":w>=15?"var(--orange)":"var(--red)",x=fn[$]||$;p+=`<div class="pr-driver-row">
        ...
        </div>`}p+="</div>";const y=(Number(f?.corp_cash_reserves)||0)>=l,v=we>n,h=ni[H];p+=`<div class="pr-order">
        ...
        <div class="pr-order__box">
            <div class="pr-order__header">
                <div style="display:flex;align-items:center;gap:6px;">
                    ...
                    <span style="font-family:var(--font-mono);font-size:9px;color:${h}">${Et[H]}</span>
                </div>
            ...
            <div class="pr-qty-row">
                ...
                <div class="pr-qty-btns">
                    ${[100,250,500,1e3].map($=>`<span class="pr-qty-btn${we===$?" active":""}" onclick="setPrQty(${$})">${$>=1e3?$/1e3+"k":$}</span>`).join("")}
                </div>
            </div>
    ...
    </div>`,i.innerHTML=p}function un(i){D=i,H="STD";for(const e of["STD","HIGH","LOW"])if(he(i,e,k).available){H=e;break}gi(),Rt(),Ze()}function yn(i){H=i,Rt(),Ze()}function vn(i){we=i,Ze()}let ut=!1;async function gn(){if(ut||!f||!k)return;const i=Bt(D,H,k),e=ai(D,H,k),t=i*we,a=Number(f.corp_cash_reserves)||0;if(t>a){alert("Insufficient cash reserves.");return}if(we>e){alert("Exceeds available supply this tick.");return}ut=!0;const n=document.querySelector(".pr-purchase-btn");n&&(n.disabled=!0,n.textContent="...");try{const l=a-t,{error:s}=await _.from("factions").update({corp_cash_reserves:l}).eq("id",f.id);if(s)throw s;const d=j[D]?.[H],r=(d?.qty||0)+we,o=(d?.value||0)+t,{error:c}=await _.from("corp_warehouse").upsert({faction_id:f.id,nation_id:f.nation_id,material_key:D,quality_tier:H,quantity:r,total_value:o,last_purchased_tick:P?.current_tick||null,updated_at:new Date().toISOString()},{onConflict:"faction_id,material_key,quality_tier"});if(c){const{error:m}=await _.from("factions").update({corp_cash_reserves:a}).eq("id",f.id);throw m&&console.error("Cash refund failed after warehouse error:",m.message),c}f.corp_cash_reserves=l,j[D]||(j[D]={}),j[D][H]={qty:r,value:o},Nt(),vi(),n&&(n.textContent="PURCHASED",setTimeout(()=>{n.isConnected&&(n.disabled=!1,n.textContent="PURCHASE")},1500))}catch(l){n&&(n.disabled=!1,n.textContent="PURCHASE"),alert("Purchase failed: "+(l.message||"Unknown error"))}finally{ut=!1}}function xi(i){const e=ke||k;if(!e)return[];const t=Qe(i);if(!t)return[];const a=Pi(i,e),n=[],l=Number(e?.inflation??50),s=Number(e?.fuel_prices??50);Number(e?.manufacturing_output??50);const d=ke&&k&&ke.id!==k.id;let r=null;if(d&&(r=Oi(e,k)),a.newAvailable>0){const o=Jt(i,e),c=t.basePrice,m=Math.round(c*((l-50)/200)),p=Math.round(c*((s-50)/300));let u=o;const y=[{label:"Base price",value:B(c)},m!==0?{label:`Inflation (${l})`,mod:(m>=0?"+":"")+B(Math.abs(m))}:null,p!==0?{label:`Fuel transport (${s})`,mod:(p>=0?"+":"")+B(Math.abs(p))}:null].filter(Boolean),v=o-c-m-p;if(v!==0&&!d&&y.push({label:"Demand/scarcity",mod:(v>=0?"+":"")+B(Math.abs(v))}),d&&r){const h=Math.round(o*r.tariff),$=Math.round(o*r.transport);u=o+h+$,y.push({label:`Import tariff (${Math.round(r.tariff*100)}%)`,mod:"+"+B(h)}),y.push({label:`Transport (${r.deliveryTicks} tick${r.deliveryTicks>1?"s":""})`,mod:"+"+B($)})}n.push({seller:d?`${e.name} Buyers`:"Local Buyers",sellerType:"LOCAL",nation:e.name||"—",distance:d?r?.deliveryTicks||1:0,condition:100,price:Math.round(u),available:a.newAvailable,delivery:d?r.deliveryTicks+" tick"+(r.deliveryTicks>1?"s":""):"Immediate",deliveryTicks:d?r.deliveryTicks:0,used:!1,priceFactors:y,sourceNationId:e.id})}if(a.usedAvailable>0){const o=a.usedCondition,c=Jt(i,e,{used:!0,condition:o});let m=c;const p=[{label:"Base price",value:B(t.basePrice)},{label:`Condition (${o}%)`,mod:"-"+B(Math.max(0,t.basePrice-c))}];if(d&&r){const u=Math.round(c*r.tariff),y=Math.round(c*r.transport);m=c+u+y,p.push({label:`Import tariff (${Math.round(r.tariff*100)}%)`,mod:"+"+B(u)}),p.push({label:`Transport (${r.deliveryTicks} tick${r.deliveryTicks>1?"s":""})`,mod:"+"+B(y)})}n.push({seller:d?`${e.name} Buyers`:"Local Buyers",sellerType:"LOCAL",nation:e.name||"—",distance:d?r?.deliveryTicks||1:0,condition:o,price:Math.round(m),available:a.usedAvailable,delivery:d?r.deliveryTicks+" tick"+(r.deliveryTicks>1?"s":""):"Immediate",deliveryTicks:d?r.deliveryTicks:0,used:!0,priceFactors:p,sourceNationId:e.id})}return n}function et(){const i=Number(f?.corp_cash_reserves)||0;document.getElementById("em-cash").textContent=B(i);const e=Qe(Y),t=Re[e?.tier||1],a=document.getElementById("em-tier-badge");a&&(a.textContent=t.tag,a.style.color=t.color),a.style.background=t.color+"0a",a.style.border="1px solid "+t.color+"33";const n=document.getElementById("em-nation-select");if(n&&n.options.length===0){const d=k?.name||f?.nation||"—";let r=`<option value="">${g(d)} (HQ)</option>`;for(const o of Ve)o.id!==k?.id&&(r+=`<option value="${o.id}">${g(o.name)}</option>`);n.innerHTML=r}const l=document.getElementById("em-import-tag"),s=ke&&k&&ke.id!==k.id;l&&(l.style.display=s?"":"none"),xn(),Pt()}function xn(){let i="";for(let e=1;e<=3;e++){const t=Re[e],a=kt(e),n=e===3?"em-selector__grid em-selector__grid--t3":"em-selector__grid em-selector__grid--t12";i+=`<div class="em-selector__row">
            <div class="em-selector__tier-label" style="color:${t.color}">${t.tag}</div>
            <div class="${n}">`;for(const l of a){const s=Y===l.key,d=xi(l.key).length>0;i+=`<span class="em-selector__btn${s?" active":""}${d?"":" no-listings"}"
                style="${s?"background:"+t.color+";border-color:"+t.color:""}"
                onclick="setEmType('${l.key}')">${g(l.name)}</span>`}i+="</div></div>"}document.getElementById("em-selector").innerHTML=`<div class="em-selector">${i}</div>`}function Pt(){const i=document.getElementById("em-content");if(ye=xi(Y),ye.length===0){i.innerHTML=`<div class="em-no-listings"><div class="em-no-listings__box">
        ...
        </div></div>`;return}ae>=ye.length&&(ae=0);let e="";for(let a=0;a<ye.length;a++){const n=ye[a],l=ae===a,s=n.sellerType==="LOCAL"?"var(--teal)":"#5a8aaa",d=ii(n.condition);e+=`<div class="em-listing${l?" selected":""}" style="${l?"border-left-color:"+s:""}" onclick="setEmListing(${a})">`,e+=`<div class="em-listing__row1">
            <div style="display:flex;align-items:center;gap:6px">
            ...
            </div>`),e+="</div>"}const t=ye[ae];if(t){const a=Qe(Y),n=Re[a?.tier||1],l=Math.min(t.available,4),s=t.price*oe,d=(Number(f?.corp_cash_reserves)||0)>=s;e+=`<div class="em-purchase"><div class="em-purchase__box">
            ...
            <div class="em-purchase__qty">
                ...
                <div class="em-purchase__qty-btns">
                    ${Array.from({length:l},(r,o)=>o+1).map(r=>`<span class="em-qty-btn${oe===r?" active":""}" style="${oe===r?"background:"+n.color+";border-color:"+n.color:""}" onclick="setEmQty(${r})">${r}</span>`).join("")}
                </div>
            ...
            <div class="em-purchase__total">
        ...
        </div></div>`}i.innerHTML=e}async function bn(i){if(!i)ke=null;else{let t=Ve.find(a=>a.id===i);if(!t)try{const{data:a}=await _.from("nations").select("*").eq("id",i).single();t=a}catch{}ke=t||null}ae=0,oe=1;const e=document.getElementById("em-nation-select");e&&(e.value=i||""),et()}function $n(i){Y=i,ae=0,oe=1,et()}function _n(i){ae=i,oe=1,Pt()}function hn(i){oe=i,Pt()}let yt=!1;async function wn(){if(yt)return;const i=ye[ae];if(!i||!f)return;const e=Qe(Y);if(!e)return;const t=oe,a=i.price*t,n=Number(f.corp_cash_reserves)||0;if(a>n){alert("Insufficient cash reserves.");return}if(t>i.available){alert("Not enough units available.");return}const l=document.querySelector(".em-purchase-btn");l&&(l.disabled=!0,l.textContent="..."),yt=!0;try{const s=n-a,{error:d}=await _.from("factions").update({corp_cash_reserves:s}).eq("id",f.id);if(d)throw d;const r=!i.deliveryTicks||i.deliveryTicks===0;if(r){const c=te.find(A=>A.equipment_key===Y),m=(c?.owned||0)+t,p=c?.purchase_price_avg||0,u=c?.owned||0,y=u>0?Math.round((p*u+i.price*t)/m):i.price,v=e.maintenancePerUnit*m,h=c?.condition||100,$=Math.round((h*u+i.condition*t)/m),{error:w}=await _.from("corp_equipment").upsert({faction_id:f.id,nation_id:f.nation_id,equipment_key:Y,tier:e.tier,owned:m,deployed:c?.deployed||0,condition:$,maintenance_per_tick:v,purchase_price_avg:y,last_purchased_tick:P?.current_tick||null,updated_at:new Date().toISOString()},{onConflict:"faction_id,equipment_key"});if(w){const{error:A}=await _.from("factions").update({corp_cash_reserves:n}).eq("id",f.id);throw A&&console.error("Cash refund failed:",A.message),w}c?(c.owned=m,c.condition=$,c.maintenance_per_tick=v):te.push({equipment_key:Y,tier:e.tier,owned:m,deployed:0,condition:$,maintenance_per_tick:v,assigned_projects:[]})}else{const c=(P?.current_tick||0)+i.deliveryTicks,{error:m}=await _.from("corp_equipment_deliveries").insert({faction_id:f.id,equipment_key:Y,quantity:t,condition:i.condition,delivery_tick:c,source_nation_id:i.sourceNationId||null,seller_name:i.seller,price_paid:a});if(m){const{error:p}=await _.from("factions").update({corp_cash_reserves:n}).eq("id",f.id);throw p&&console.error("Cash refund failed:",p.message),m}}f.corp_cash_reserves=s,Ot(),et();const o=document.getElementById("pr-cash");o&&(o.textContent=B(s)),l&&(l.textContent=r?"PURCHASED":"ORDERED",setTimeout(()=>{l.isConnected&&(l.disabled=!1,l.textContent="PURCHASE")},1500))}catch(s){l&&(l.disabled=!1,l.textContent="PURCHASE"),alert("Purchase failed: "+(s.message||"Unknown error"))}finally{yt=!1}}let kn=-1,Be=[],At=[],bi=[];function vt(i){return Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(1)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i.toLocaleString()}function En(i,e,t){if(t)return"var(--orange)";const a=i/(e||1)*100;return a>50?"var(--green)":a>25?"var(--amber)":"var(--red)"}function Tn(){const i=document.getElementById("pm-list"),e=Be.length,t=At.length,a=bi.length,n=Be.filter(r=>r.expiring_soon).length;document.getElementById("pm-active-count").textContent=`(${e})`,document.getElementById("pm-pending-count").textContent=`(${t})`,document.getElementById("pm-apply-count").textContent=`(${a})`;const l=document.getElementById("pm-badges");let s="";n>0&&(s+=`<span class="pm-badge pm-badge--expiring">${n} EXPIRING</span>`),t>0&&(s+=`<span class="pm-badge pm-badge--pending">${t} PENDING</span>`),l.innerHTML=s;const d=Be.reduce((r,o)=>r+(o.cost||0),0)+At.reduce((r,o)=>r+(o.cost||0),0);document.getElementById("pm-total-cost").textContent=vt(d),document.getElementById("pm-footer-active").textContent=e,document.getElementById("pm-footer-pending").textContent=t;{if(e===0){i.innerHTML=`<div class="pm-empty">
            ...
            </div>`;return}let r="";Be.forEach((o,c)=>{const m=kn===c,p=En(o.ticks_left,o.total_ticks,o.expiring_soon),u=Math.min(o.ticks_left/(o.total_ticks||1)*100,100);r+=`<div class="pm-item ${o.expiring_soon?"pm-item--expiring":""} ${m?"expanded":""}" onclick="togglePmExpand(${c})">
                <div class="pm-item__inner">
                    ...
                    <div class="pm-detail__row">
                        ...
                        <span class="pm-detail__val">${vt(o.cost||0)}</span>
                    </div>
                    <div class="pm-detail__row">
                    ...
                    </div>`,o.note&&(r+=`<div class="pm-note"><span class="pm-note__text">${g(o.note)}</span></div>`),o.expiring_soon&&o.renewable&&(r+=`<div class="pm-btn-row"><button class="pm-btn pm-btn--renew">RENEW — ${vt(o.cost||0)}</button></div>`),r+="</div>"),r+="</div></div>"}),i.innerHTML=r;return}}function Cn(){Be=[],At=[],bi=[],Tn()}let xe=[],In=-1;function ne(i){return Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(2)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i.toLocaleString()}function ei(i){return i>=85?"var(--gold)":i>=60?"var(--green)":i>=40?"var(--orange)":"var(--red)"}function Mn(i){return"dl-result--"+i.toLowerCase()}function ti(){const i=document.getElementById("dl-list"),e=xe.length;document.getElementById("dl-count").textContent=`${e} COMPLETED`;const t=xe.reduce((d,r)=>{const o=r.financials||{};return d+((o.payment||0)+(o.bonus||0)-(o.penalty||0)-(o.total_cost||0))},0),a=document.getElementById("dl-lifetime-profit");a.textContent=(t>=0?"+":"")+ne(t),a.style.color=t>=0?"var(--green)":"var(--red)";const n={};xe.forEach(d=>{n[d.result]=(n[d.result]||0)+1});const l=document.getElementById("dl-footer-results");if(l.innerHTML=Object.entries(n).map(([d,r])=>`<div class="dl-footer__result-box">
        ...
        </div>`;return}let s="";xe.forEach((d,r)=>{const o=In===r,c=d.financials||{},m=(c.payment||0)+(c.bonus||0)-(c.penalty||0)-(c.total_cost||0),p=m>=0,u=Mn(d.result),v={DISTINCTION:"var(--gold)",PASS:"var(--green)",CONDITIONAL:"var(--orange)",FAIL:"var(--red)"}[d.result]||"var(--text-dim)",h=d.type==="GOVERNMENT";if(s+=`<div class="dl-item ${o?"expanded":""}" onclick="toggleDlExpand(${r})">
            <div class="dl-item__inner" style="border-left:2px solid ${v}">
                ...
                <div class="dl-summary-bar">
                    <div class="dl-summary-cell" style="flex:1;">
                        ...
                        <div style="display:flex;align-items:center;gap:4px;margin-top:2px;">
                            <span class="dl-summary-value" style="color:${ei(d.quality_score)}">${d.quality_score}</span>
                    ...
                    <div class="dl-summary-cell" style="flex:1.3;">
                        ...
                        <div class="dl-summary-value" style="color:${p?"var(--green)":"var(--red)"};margin-top:2px;">${p?"+":""}${ne(m)}</div>
                    </div>
                </div>`,o){const $=d.inspection||{};s+='<div style="margin-top:8px;">',s+='<div class="dl-section-label">Inspection Report</div>',["materials","structural","systems"].forEach(T=>{const C=$[T]||{score:0,issues:[]},S=ei(C.score),E=Math.min(C.score/100*100,100);s+=`<div class="dl-inspect-row">
                ...
                </div>`});const w=$.permits||{passed:!0,issues:[]};s+=`<div class="dl-permits-row ${w.passed?"dl-permits-row--pass":"dl-permits-row--fail"}">
                ...
                ${(w.issues||[]).map(T=>`<div class="dl-inspect-issue dl-inspect-issue--red">${g(T)}</div>`).join("")}
                ...
                </div>`}),s+="</div>",s+='<div class="dl-section-label">Financial Summary</div>',s+='<div class="dl-fin-panel">',s+=`<div class="dl-fin-row"><span class="dl-fin-key">CONTRACT VALUE</span><span class="dl-fin-val">${ne(c.contract_value||0)}</span></div>`,(c.bonus||0)>0&&(s+=`<div class="dl-fin-row"><span class="dl-fin-key">QUALITY BONUS</span><span class="dl-fin-val" style="color:var(--green)">+${ne(c.bonus)}</span></div>`),(c.penalty||0)>0&&(s+=`<div class="dl-fin-row"><span class="dl-fin-key">PENALTIES</span><span class="dl-fin-val" style="color:var(--red)">-${ne(c.penalty)}</span></div>`);const A=(c.payment||0)+(c.bonus||0)-(c.penalty||0);s+=`<div class="dl-fin-row"><span class="dl-fin-key">PAYMENT RECEIVED</span><span class="dl-fin-val" style="color:var(--green)">${ne(A)}</span></div>`,s+=`<div class="dl-fin-row"><span class="dl-fin-key">TOTAL COST</span><span class="dl-fin-val" style="color:var(--red)">-${ne(c.total_cost||0)}</span></div>`,s+=`<div class="dl-fin-total">
                <span class="dl-fin-total__label">${p?"NET PROFIT":"NET LOSS"}</span>
                <span class="dl-fin-total__value" style="color:${p?"var(--green)":"var(--red)"}">${p?"+":""}${ne(m)}</span>
            </div>`,s+="</div>";const x=d.timeline||{};s+=`<div class="dl-timeline">
                <span class="dl-timeline__label">TIMELINE</span>
                <span class="dl-timeline__ticks">${x.actual||0}/${x.expected||0} ticks</span>`,x.early?s+=`<span class="dl-timeline__badge dl-timeline__badge--early">${(x.expected||0)-(x.actual||0)} TICK${x.expected-x.actual!==1?"S":""} EARLY</span>`:!x.on_time&&x.actual>x.expected&&(s+=`<span class="dl-timeline__badge dl-timeline__badge--late">${(x.actual||0)-(x.expected||0)} TICK${x.actual-x.expected!==1?"S":""} LATE</span>`),s+="</div>",s+="</div>"}s+="</div></div>"}),i.innerHTML=s}async function Sn(){if(!f){xe=[],ti();return}const{data:i,error:e}=await _.from("construction_deliveries").select("*, construction_contracts(name, sector, issuer_name, issuer_type, timeline_ticks)").eq("faction_id",f.id).order("delivered_at_tick",{ascending:!1}).limit(20);e?(console.warn("Failed to load deliveries:",e.message),xe=[]):xe=(i||[]).map(t=>{const a=t.construction_contracts||{};return{id:t.contract_id,name:a.name||"Project",type:a.issuer_type||"GOVERNMENT",issuer:a.issuer_name||"Government",delivered:"Tick "+(t.delivered_at_tick||0),result:t.result,quality_score:t.quality_score,rep_change:t.rep_change,financials:{contract_value:t.contract_value||0,bonus:t.quality_bonus||0,penalty:t.penalties||0,payment:t.payment_received||0,total_cost:t.total_cost||0},inspection:t.inspection||{},materials_used:t.materials_used||[],timeline:{expected:t.timeline_expected||0,actual:t.timeline_actual||0,on_time:t.on_time,early:t.timeline_actual<t.timeline_expected}}}),ti()}function Ot(){const i=te.reduce((d,r)=>d+(r.owned||0),0),e=te.reduce((d,r)=>d+(r.deployed||0),0),t=Ri(te),a=i-e;document.getElementById("eq-count").textContent=i+" UNITS",document.getElementById("eq-summary").innerHTML=`
        ...
        </div>`;const n={};for(const d of te)n[d.equipment_key]=d;let l="";for(let d=1;d<=3;d++){const r=Re[d],o=kt(d),c=Ct===d,m=o.reduce((u,y)=>u+(n[y.key]?.owned||0),0),p=o.reduce((u,y)=>u+(n[y.key]?.deployed||0),0);if(l+=`<div class="eq-tier-hdr" onclick="toggleEqTier(${d})">
            ...
            ${m>0?`<span class="eq-tier-hdr__count">${p}/${m}</span>`:'<span class="eq-tier-hdr__none">NONE OWNED</span>'}
        </div>`,c)for(const u of o){const y=n[u.key],v=y?.owned||0,h=y?.deployed||0,$=y?.condition||0,w=u.maintenancePerUnit*v,A=v-h,x=v>0&&A===0,T=v>0&&$<65,C=ii($),S=y?.assigned_projects||[],E=S.length>0?S.map(M=>M.contract_name||"Project").join(", ").slice(0,30):v>0&&h>0?h+" project"+(h>1?"s":""):"—";l+=`<div class="eq-row${v===0?" unowned":""}">`,l+=`<div class="eq-row__top">
                    ...
                    ${v>0?`<div class="eq-row__right">
                        ...
                        <div class="eq-detail__cell" style="flex:0.7">
                            ...
                            <div class="eq-detail__value" style="color:var(--red)">${B(w)}<span style="font-size:7px;color:var(--text-dim)">/t</span></div>
                        </div>
                    </div>`:l+='<div class="eq-row__hint">Purchase from Equipment Market →</div>',l+="</div>"}}document.getElementById("eq-list").innerHTML=l;const s=[1,2,3].map(d=>{const r=Re[d],o=kt(d).reduce((c,m)=>c+(n[m.key]?.owned||0),0);return`<div class="eq-footer__tier-box" style="border-color:${o>0?r.color+"33":"var(--border-0)"};background:${o>0?r.color+"0a":"transparent"}">
        ...
        <div class="eq-footer__tiers">${s}</div>`}function An(i){Ct=Ct===i?-1:i,Ot()}async function zn(){if(!f)return;const{data:i,error:e}=await _.from("corp_equipment").select("equipment_key, tier, owned, deployed, condition, maintenance_per_tick, assigned_projects").eq("faction_id",f.id);e?(console.warn("Failed to load equipment:",e.message),te=[]):te=i||[],Ot()}async function Ln(){const{data:{user:i}}=await _.auth.getUser();if(!i){window.location.href="login.html";return}const{data:e}=await _.from("factions").select("*").or(`id.eq.${i.id},linked_user_id.eq.${i.id}`);Le=(e||[]).filter(o=>o.nation_id);const t=sessionStorage.getItem("active_faction_id");if(f=Le.find(o=>o.id===t)||Le.find(o=>o.faction_type==="corporation")||Le[0],!f){await _.auth.signOut(),window.location.href="login.html";return}if(f.faction_type!=="corporation"){window.location.href="dashboard.html";return}const[a,n]=await Promise.all([f.nation_id?_.from("nations").select("*").eq("id",f.nation_id).single():Promise.resolve({data:null}),_.from("shard").select("current_tick, current_date, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single()]);a.error&&console.warn("Nation load failed:",a.error.message),a.data&&(k=a.data),n.error&&console.warn("Shard load failed:",n.error.message),P=n.data;const l=f.corp_ticker||f.abbreviation||"";if(document.getElementById("corp-logo").textContent=l.slice(0,2)||"—",document.getElementById("corp-name-bar").textContent=f.faction_name||"Unnamed Corp",P){if(document.getElementById("game-date").textContent=P.current_date||"—",document.getElementById("tick-number").textContent=P.current_tick||"—",P.next_tick_at){const c=(Number(P.tick_interval_hours)||8)*36e5,m=new Date(P.next_tick_at).getTime(),u=m-c+c/2;It=new Date(u>Date.now()?u:m+c/2),Hi()}const o=document.querySelector("#tick-countdown")?.closest(".tick-item")?.querySelector(".tick-label");o&&(o.textContent="Next Corp Tick")}document.getElementById("corp-name-badge").textContent=(l?"["+l+"]":f.faction_name||"Corp")+" ▾";const s=document.getElementById("topbar-cash");if(s){const o=Number(f.corp_cash_reserves??0),c=o>=1e9?"$"+(o/1e9).toFixed(1)+"B":o>=1e6?"$"+(o/1e6).toFixed(1)+"M":"$"+Math.round(o/1e3)+"k";s.textContent="CASH: "+c}const d=f.action_points??0;document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+d+" AP</span>",document.getElementById("nation-pill").textContent=(k?.name||f.nation||"—").toUpperCase();const r=document.getElementById("corp-faction-dropdown");if(r){let o="";for(const c of Le){const m=c.id===f.id,p=c.faction_type==="corporation"?"CORP":"PARTY",u=c.faction_type==="corporation"?"var(--teal)":"var(--amber)";o+=`<div class="corp-dd-item${m?" active":""}" onclick="switchToFaction('${c.id}', '${c.faction_type}')">
            ...
            </div>`}r.innerHTML=o}await Promise.all([pe(),fi(),mn(),zn(),Cn(),Sn()]);try{const{data:o}=await _.from("nations").select("*").order("name");Ve=o||[]}catch{Ve=[]}if(vi(),et(),Ni(f,k,P),document.getElementById("loading").style.display="none",document.getElementById("page-content").style.display="block",new URLSearchParams(window.location.search).get("tab")==="expansion"){const o=document.querySelector('[data-tab="expansion"]');o&&_i({preventDefault:()=>{},target:o})}}async function Bn(){await _.auth.signOut(),window.location.href="login.html"}function qn(){const i=document.getElementById("corp-faction-dropdown");i&&i.classList.toggle("open")}function Nn(i,e){const t=document.getElementById("corp-faction-dropdown");t&&t.classList.remove("open"),sessionStorage.setItem("active_faction_id",i),e==="corporation"?window.location.href="corp-operations.html":window.location.href="dashboard.html"}document.addEventListener("click",i=>{const e=document.getElementById("faction-switcher"),t=document.getElementById("corp-faction-dropdown");t&&e&&!e.contains(i.target)&&t.classList.remove("open")});document.addEventListener("keydown",i=>{i.key==="Escape"&&Je()});window.doLogout=Bn;window.toggleTheme=ji;window.toggleCorpDropdown=qn;window.switchToFaction=Nn;window.setFilter=Ui;window.openContractDetail=li;window.closeContractDetail=Je;window.placeBid=dn;window.toggleWhRow=pn;window.toggleEqTier=An;window.switchEmNation=bn;window.setEmType=$n;window.setEmListing=_n;window.setEmQty=hn;window.purchaseEquipment=wn;window.setPrMat=un;window.setPrTier=yn;window.setPrQty=vn;window.purchaseMaterial=gn;let W={general:0,skilled:0,innovative:0},gt=!1;const Pe=[{id:"general",label:"General Workforce",multiplier:2,color:"#8b9a6b",factionKey:"corp_general_workforce"},{id:"skilled",label:"Skilled Workforce",multiplier:3,color:"#c8a832",factionKey:"corp_skilled_workforce"},{id:"innovative",label:"Innovative Workforce",multiplier:6,color:"#c84",factionKey:"corp_innovative_workforce"}];function $i(i){const e=Number(k?.minimum_wage??50),t=Number(k?.inflation??50),a=Number(k?.standard_of_living??50),n=e/100*48e3,l=1+(t-50)/100*.5,s=1+(a-50)/100*.5;return Math.round(n*i*l*s)}function b(i){const e=Math.abs(i),t=i<0?"-":"";return e>=1e9?t+"$"+(e/1e9).toFixed(2)+"B":e>=1e6?t+"$"+(e/1e6).toFixed(2)+"M":e>=1e3?t+"$"+(e/1e3).toFixed(1)+"k":t+"$"+e.toLocaleString()}async function _i(i){i.preventDefault(),document.getElementById("operations-content").style.display="none";const e=document.getElementById("expansion-content");e.style.display="flex",e.style.justifyContent="center",e.style.gap="12px",e.style.alignItems="flex-start",e.style.flexWrap="wrap",document.querySelectorAll(".corp-nav__tab").forEach(t=>t.classList.remove("active")),i.target.classList.add("active"),tt(),Dn(),await He(),await Dt(),it(),await Kn(),nt(),Ti(),await Zn(),at()}function hi(i){i&&i.preventDefault(),document.getElementById("operations-content").style.display="flex",document.getElementById("expansion-content").style.display="none",document.querySelectorAll(".corp-nav__tab").forEach(e=>e.classList.remove("active")),document.querySelector('[data-tab="operations"]')?.classList.add("active")}function wi(){return F.reduce((e,t)=>{const a=Number(t.capacity||0),n=Number(t.condition||0)/100;return e+Math.floor(a*n)},0)+500}function Rn(i,e){const t=Pe.find(l=>l.id===i),a=Number(f?.[t.factionKey]??0),n=W[i]+e;if(!(a+n<0)){if(e>0){const l=Pe.reduce((d,r)=>{const o=Number(f?.[r.factionKey]??0),c=r.id===i?n:W[r.id];return d+o+c},0),s=wi();if(l>s)return}W[i]=n,tt()}}function Pn(i){i?W[i]=0:W={general:0,skilled:0,innovative:0},tt()}async function On(){if(gt||!Object.values(W).some(n=>n!==0))return;let e=0;for(const n of Pe){const l=W[n.id];l>0&&(e+=l*$i(n.multiplier)*.1)}const t=Number(f?.corp_cash_reserves??0);if(e>t){alert("Insufficient cash reserves. Hiring cost: "+b(e)+", available: "+b(t));return}const a=e>0?`Confirm workforce changes?

Hiring fee: `+b(e)+" (deducted from cash reserves)":`Confirm workforce changes?

Firing workers — no cost.`;if(confirm(a)){gt=!0;try{const n={};for(const d of Pe){const r=Number(f?.[d.factionKey]??0);n[d.factionKey]=Math.max(0,r+W[d.id])}e>0&&(n.corp_cash_reserves=Math.max(0,t-Math.round(e)));const{error:l}=await _.from("factions").update(n).eq("id",f.id);if(l)throw l;Object.assign(f,n),W={general:0,skilled:0,innovative:0};const s=document.getElementById("topbar-cash");if(s){const d=Number(f.corp_cash_reserves??0);s.textContent="CASH: "+(d>=1e6?"$"+(d/1e6).toFixed(1)+"M":"$"+Math.round(d/1e3)+"k")}tt()}catch(n){alert("Error: "+n.message)}finally{gt=!1}}}function tt(){const i=document.getElementById("hf-card-container");if(!i)return;const e="'JetBrains Mono', monospace",t={bg:"#121210",surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",greenBright:"#5c5",red:"#c55",redDim:"#a44",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},a=Number(k?.minimum_wage??50),n=Number(k?.inflation??50),l=Number(k?.standard_of_living??50),s=a/100*48e3,d=(1+(n-50)/100*.5).toFixed(2),r=(1+(l-50)/100*.5).toFixed(2),o=k?.name||f?.nation||"Nation",c=Object.values(W).some(w=>w!==0),m=wi();let p=0,u=0,y=0,v=0,h="";for(const w of Pe){const A=Number(f?.[w.factionKey]??0),x=W[w.id],T=A+x,C=$i(w.multiplier),S=x>0,E=A*C,M=T*C,R=M-E;p+=A,u+=T,y+=E,v+=M;const q=x!==0?S?"rgba(92,204,92,0.02)":"rgba(204,85,85,0.02)":"transparent";h+=`
        <div style="padding:10px 14px;border-bottom:1px solid ${t.border};background:${q};">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                <span style="font-family:${e};font-size:8px;color:${t.dim}">WAGE (MIN × ${w.multiplier}.0 × ${d} × ${r})</span>
                <span style="font-family:${e};font-size:10px;color:${w.color}">${b(C)}/yr</span>
            </div>
    ...
    </div>`;for(const y of F){const v=Ye[y.style]||Ye.Basic;s+=Number(y.purchase_price||0),d+=Number(y.monthly_maintenance||0),l+=`
        <div style="padding:8px 12px;border-bottom:1px solid ${t.border};">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px;">
    ...
    </div>`}let Ie=[],G=null;const Ye={Basic:{color:"#6a6660",label:"BASIC"},Modern:{color:"#8b9a6b",label:"MODERN"},Sustainable:{color:"#5c5",label:"SUSTAINABLE"},Innovative:{color:"#c8a832",label:"INNOVATIVE"},Heritage:{color:"#c84",label:"HERITAGE"},Premium:{color:"#ca5",label:"PREMIUM"}};async function Dt(){if(!f?.nation_id)return;const{data:i,error:e}=await _.from("available_properties").select("*").eq("nation_id",f.nation_id).eq("status","available").order("price",{ascending:!0});if(e){console.warn("[Property] Failed to load marketplace:",e.message);return}Ie=(i||[]).map(t=>({...t,adjusted_cost:t.price,adjusted_maintenance:t.monthly_maintenance}))}function it(){const i=document.getElementById("new-property-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",redDim:"#a44",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"};(k?.name||f?.nation||"Nation").toUpperCase();const a=Number(k?.standard_of_living??50),n=Number(k?.gdp_growth??50),l=Number(k?.inflation??50),s=k?.capital||"Capital",d={capital:s,port:s+" Port",industrial:s+" Industrial Zone",suburban:s+" Suburbs",coastal:s+" Coast"};let r="";if(Ie.length===0)r=`<div style="padding:20px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">No properties available in this market.<br>Improve GDP Growth and Standard of Living to unlock more.</div>`;else for(let o=0;o<Ie.length;o++){const c=Ie[o],m=G===o,p=Ye[c.style]||Ye.Basic,u=d[c.city_template]||s;r+=`
    ...
    <div style="width:380px;height:450px;background:${t.surface};border:1px solid ${t.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
        <div style="padding:8px 14px;border-bottom:1px solid ${t.border};display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">
            ...
            <span style="font-family:${e};font-size:9px;color:${t.dim}">${Ie.length} AVAILABLE</span>
        </div>
    ...
    </div>`}function Hn(i){G=G===i?null:i,it()}let xt=!1;async function jn(){if(G===null||xt)return;const i=Ie[G];if(!i)return;const e=Number(f?.corp_cash_reserves??0);if(i.adjusted_cost>e){alert(`Insufficient cash reserves.
...
This will be deducted from your cash reserves.`)){xt=!0;try{const{error:t}=await _.from("corp_properties").insert({faction_id:f.id,nation_id:f.nation_id,catalog_id:i.catalog_id||null,name:i.name,type:i.type,style:i.style,capacity:i.capacity,purchase_price:i.adjusted_cost,monthly_maintenance:i.adjusted_maintenance,condition:i.condition,city:i.city,purchased_at_tick:Number(document.getElementById("tick-number")?.textContent||0),is_active:!0});if(t)throw t;const a=Math.max(0,e-i.adjusted_cost),{error:n}=await _.from("factions").update({corp_cash_reserves:a}).eq("id",f.id);if(n)throw n;f.corp_cash_reserves=a,i.id&&await _.from("available_properties").update({status:"sold",purchased_by:f.id}).eq("id",i.id);const l=document.getElementById("topbar-cash");l&&(l.textContent="CASH: "+(a>=1e6?"$"+(a/1e6).toFixed(1)+"M":"$"+Math.round(a/1e3)+"k")),G=null,await Dt(),it(),je(),alert("Property purchased: "+i.name+`

Deducted: `+b(i.adjusted_cost))}catch(t){alert("Purchase failed: "+t.message)}finally{xt=!1}}}const be={Basic:{costMod:1,maintMod:1,repGain:1,color:"#6a6660",desc:"Functional. No frills."},Modern:{costMod:1.4,maintMod:1.2,repGain:2,color:"#8b9a6b",desc:"Contemporary design. Good retention."},Sustainable:{costMod:1.6,maintMod:.8,repGain:3,color:"#5c5",desc:"Green building. Lower upkeep."},Innovative:{costMod:2,maintMod:1.5,repGain:4,color:"#c8a832",desc:"Cutting edge. Attracts top talent."},Heritage:{costMod:1.3,maintMod:1.4,repGain:2,color:"#c84",desc:"Restored historic. High character."},Premium:{costMod:2.5,maintMod:1.8,repGain:5,color:"#ca5",desc:"Flagship prestige. Maximum reputation."}};let Ht=!1,I={name:"",type:"Office Building",size:2e3,style:"Modern",timeline:36,budgetMod:0},bt=!1;function ki(){const e=1+(Number(k?.inflation??50)-50)/100*.3,t=be[I.style]?.costMod||1,a=I.type==="Warehouse"?.75:1,n=Math.round(I.size*1e5*e*t*a),l=Math.round(n*(1+I.budgetMod/100)),s=Math.round(l*.007*(be[I.style]?.maintMod||1));return{baseBudget:n,adjusted:l,maint:s,inflMod:e,styleMod:t}}function Un(){Ht=!0,I={name:"",type:"Office Building",size:2e3,style:"Modern",timeline:36,budgetMod:0},Ei()}function jt(){Ht=!1,document.getElementById("cp-modal-overlay")?.remove()}function Gn(i,e){I[i]=e,Ei()}async function Wn(){if(!(bt||!I.name.trim())){bt=!0;try{const i=ki(),e=k?.name||f?.nation||"Unknown",t=be[I.style]?.repGain||1,a=await _.from("shard").select("current_tick, current_date").eq("name","Alpha Shard").single(),n=a.data?.current_tick||0,l=(a.data?.current_date||"").match(/\d{4}/)?.[0]||"2015",{count:s}=await _.from("construction_contracts").select("id",{count:"exact",head:!0}).eq("nation_id",f.nation_id).eq("issuer_type","PRIVATE"),r=`PVT-C${(s||0)+1}-${l}`,{error:o}=await _.from("construction_contracts").insert({nation_id:f.nation_id,template_key:"custom_building",sector:"civil_engineering",name:I.name.trim(),description:`${I.type} (${I.style}) — ${I.size.toLocaleString()} employees, commissioned by ${f.faction_name}`,project_code:r,budget_ceiling:i.adjusted,timeline_ticks:I.timeline,required_materials:(()=>{const c=I.size/1e3,m=I.style,p={Basic:{concrete:1,steel:1,glass:.5,em:1,lumber:1.5,heavy:1,agg:1},Modern:{concrete:1.1,steel:1.1,glass:1.4,em:1.4,lumber:.5,heavy:1.1,agg:1.1},Sustainable:{concrete:.9,steel:.9,glass:.9,em:.9,lumber:1.5,heavy:.9,agg:.9},Innovative:{concrete:1.2,steel:1.2,glass:2,em:2,lumber:.5,heavy:2,agg:1.2},Heritage:{concrete:1.3,steel:1.3,glass:1.3,em:1.3,lumber:2,heavy:1.3,agg:1.3},Premium:{concrete:1.5,steel:1.5,glass:2.5,em:1.5,lumber:1,heavy:1.5,agg:1.5}}[m]||{concrete:1,steel:1,glass:1,em:1,lumber:1,heavy:1,agg:1},u=(y,v)=>Math.max(1,Math.ceil(c*y*v));return{concrete:u(8,p.concrete),steel:u(6,p.steel),glass_facades:u(3,p.glass),em_systems:u(4,p.em),lumber:u(1,p.lumber),heavy_parts:u(2,p.heavy),aggregate:u(3,p.agg)}})(),required_equipment:(()=>{const c=["work_trucks","concrete_mixers"];return I.size>1e3&&c.push("excavators","tower_cranes"),I.size>3e3&&c.push("bulldozers","heavy_haulers"),I.size>8e3&&c.push("pile_drivers"),c})(),required_workforce:{general:Math.ceil(I.size*.08),skilled:Math.ceil(I.size*.03)},status:"open",generated_at_tick:n,bidding_ends_tick:n+3,issuer_type:"PRIVATE",issuer_name:f.faction_name,issuer_faction_id:f.id});if(o)throw o;jt(),alert(`Construction project submitted!
...
All construction corporations in `+e+" can now bid on this project.")}catch(i){alert("Failed to submit project: "+i.message)}finally{bt=!1}}}function Ei(){if(document.getElementById("cp-modal-overlay")?.remove(),!Ht)return;const i="'JetBrains Mono', monospace",e={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",redDim:"#a44",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},t=ki(),a=k?.name||f?.nation||"Nation",n=be[I.style]?.repGain||1,l=n>=4?e.gold:n>=3?e.greenBright:n>=2?e.accent:e.dim,s=Object.entries(be).map(([o,c])=>{const m=I.style===o;return`<div onclick="cpSetField('style','${o}')" style="padding:5px 6px;cursor:pointer;text-align:center;background:${m?c.color+"18":"transparent"};border:1px solid ${m?c.color+"44":e.border};">
    ...
    <div style="width:440px;background:${e.surface};border:1px solid ${e.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;max-height:90vh;">
        ...
        <div style="padding:12px 16px;overflow:auto;flex:1;">
            ...
            <div style="margin-bottom:12px;">
                ...
                <div style="margin-top:4px;font-family:${i};font-size:8px;color:${be[I.style].color}">${be[I.style].desc}</div>
            </div>
    ...
    </div>`,document.body.appendChild(d);const r=document.getElementById("cp-name-input");r&&r.addEventListener("input",o=>{I.name=o.target.value}),d.addEventListener("click",o=>{o.target===d&&jt()})}function Fn(){const i=document.getElementById("cp-name-input");if(i&&(I.name=i.value),!I.name.trim()){alert("Please enter a building name.");return}Wn()}window.cpClose=jt;window.cpSetField=Gn;window.cpSubmitFromModal=Fn;window.npSelect=Hn;window.npBuyProperty=jn;window.npOpenConstructionModal=Un;let Se=!1;async function Vn(i){if(Se)return;const e=F.find(d=>d.id===i);if(!e)return;const t=1+(Number(k?.inflation??50)-50)/100*.3,a=Math.round((e.purchase_price||0)*.1*t),n=Number(f?.corp_cash_reserves??0);if(a>n){alert("Insufficient cash. Refurbishment costs "+b(a)+" (inflation-adjusted), you have "+b(n));return}if(e.condition>=95){alert("Property is already in excellent condition ("+e.condition+"%).");return}const l=5+Math.floor(Math.random()*21),s=Math.min(100,e.condition+l);if(confirm('Refurbish "'+e.name+`"?

Cost: `+b(a)+`
Expected improvement: +`+l+"% condition ("+e.condition+"% → "+s+"%)")){Se=!0;try{await _.from("corp_properties").update({condition:s}).eq("id",i);const d=Math.max(0,n-a);await _.from("factions").update({corp_cash_reserves:d}).eq("id",f.id),f.corp_cash_reserves=d;const r=document.getElementById("topbar-cash");r&&(r.textContent="CASH: "+(d>=1e6?"$"+(d/1e6).toFixed(1)+"M":"$"+Math.round(d/1e3)+"k")),await He(),je(),alert("Refurbished! Condition: "+e.condition+"% → "+s+"%")}catch(d){alert("Refurbishment failed: "+d.message)}finally{Se=!1}}}async function Yn(i){if(Se)return;const e=F.find(l=>l.id===i);if(!e)return;const t=1+(Number(k?.inflation??50)-50)/100*.3,a=(e.condition||50)/100,n=Math.round((e.purchase_price||0)*.6*a*t);if(confirm('Sell "'+e.name+`"?

Sale value: `+b(n)+" (60% × "+e.condition+`% condition × inflation)

The property will go back on the market for 6 ticks.
This cannot be undone.`)){Se=!0;try{await _.from("corp_properties").update({is_active:!1}).eq("id",i);const s=Number(f?.corp_cash_reserves??0)+n;await _.from("factions").update({corp_cash_reserves:s}).eq("id",f.id),f.corp_cash_reserves=s;const r=(await _.from("shard").select("current_tick").eq("name","Alpha Shard").single()).data?.current_tick||0;await _.from("available_properties").insert({nation_id:f.nation_id,catalog_id:e.catalog_id||null,name:e.name,type:e.type,style:e.style,capacity:e.capacity,price:Math.round(n*1.1),monthly_maintenance:e.monthly_maintenance,condition:e.condition,city:e.city,generated_at_tick:r,expires_at_tick:r+6,status:"available"});const o=document.getElementById("topbar-cash");o&&(o.textContent="CASH: "+(s>=1e6?"$"+(s/1e6).toFixed(1)+"M":"$"+Math.round(s/1e3)+"k")),await He(),je(),await Dt(),it(),alert('Sold "'+e.name+'" for '+b(n))}catch(l){alert("Sale failed: "+l.message)}finally{Se=!1}}}window.propRefurbish=Vn;window.propSell=Yn;let qe=0;function Ti(){const i=document.getElementById("manage-subsidiaries-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",blue:"#5a9abf",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",greenDark:"#2a5a3a",red:"#c55",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},a=F.filter(r=>r.type==="regional_hq");qe>=a.length&&(qe=0);const n=a[qe]||null;let l="";a.length===0&&(l=`<div style="padding:30px 14px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">No subsidiaries established.<br>Use Create Subsidiary to expand.</div>`);let s=0;for(let r=0;r<a.length;r++){const o=a[r],c=r===qe,m=Number(o.purchase_price||0);s+=m;const p=Oe.find(u=>u.id===o.nation_id)?.name||o.city||"—";l+=`
    ...
    </div>`}async function Qn(i){const e=F.find(a=>a.id===i);if(!e)return;const t=Math.round((e.purchase_price||0)*.6*(e.condition||50)/100);if(confirm('Dissolve subsidiary "'+e.name+`"?

Assets liquidated at 60% × condition: `+b(t)+`
All operations in this nation cease.

This cannot be undone.`))try{await _.from("corp_properties").update({is_active:!1}).eq("id",i);const a=Number(f?.corp_cash_reserves??0)+t;await _.from("factions").update({corp_cash_reserves:a}).eq("id",f.id),f.corp_cash_reserves=a;const n=document.getElementById("topbar-cash");n&&(n.textContent="CASH: "+(a>=1e6?"$"+(a/1e6).toFixed(1)+"M":"$"+Math.round(a/1e3)+"k")),qe=0,await He(),je(),Ti(),nt(),alert("Subsidiary dissolved. Received: "+b(t))}catch(a){alert("Failed: "+a.message)}}window.subDissolve=Qn;let Oe=[],se=null,$t=!1;async function Kn(){const{data:i}=await _.from("nations").select("*").order("name");Oe=(i||[]).filter(e=>e.id!==f?.nation_id)}function zt(i){const t=u=>Number(i[u]??50),a=t("standard_of_living"),n=t("cost_of_living"),l=t("corporate_tax"),s=t("minimum_wage"),d=t("urbanization"),r=t("union_strength"),o=t("corruption"),c=t("unemployment"),m=t("stability"),p=5e7*(1+(a-50)/100*.4)*(1+(n-50)/100*.3)*(1+(l-50)/100*.2)*(1+(s-50)/100*.15)*(1+(d-50)/100*.1)*(1+(r-50)/100*.1)*(1-(o-50)/100*.15)*(1-(c-50)/100*.1)*(1+(50-m)/100*.3);return Math.round(Math.max(1e7,p))}function Jn(i){se=se===i?null:i,nt()}async function Xn(){if($t||!se)return;const i=Oe.find(n=>n.id===se);if(!i)return;if(F.find(n=>n.nation_id===i.id&&n.type==="regional_hq")){alert("You already have a subsidiary in "+i.name);return}const t=zt(i),a=Number(f?.corp_cash_reserves??0);if(t>a){alert("Insufficient cash. Entry cost: "+b(t)+", available: "+b(a));return}if(confirm("Establish subsidiary in "+i.name+`?

Entry cost: `+b(t)+`
Creates a Regional HQ (500 capacity)
Unlocks `+i.name+` for operations

Deducted from cash reserves.`)){$t=!0;try{const l=(await _.from("shard").select("current_tick").eq("name","Alpha Shard").single()).data?.current_tick||0,s=85+Math.floor(Math.random()*16),d=Math.round(t*.005),{error:r}=await _.from("corp_properties").insert({faction_id:f.id,nation_id:i.id,name:"Regional Headquarters — "+i.name,type:"regional_hq",style:"Modern",capacity:500,purchase_price:t,monthly_maintenance:d,condition:s,city:i.capital||i.name,purchased_at_tick:l,is_active:!0});if(r)throw r;const o=Math.max(0,a-t);await _.from("factions").update({corp_cash_reserves:o}).eq("id",f.id),f.corp_cash_reserves=o;const c=document.getElementById("topbar-cash");c&&(c.textContent="CASH: "+(o>=1e6?"$"+(o/1e6).toFixed(1)+"M":"$"+Math.round(o/1e3)+"k")),se=null,await He(),je(),renderSubsidiariesCard(),nt(),alert("Subsidiary established in "+i.name+`!

Cost: `+b(t)+`
Regional HQ created with `+s+"% condition.")}catch(n){alert("Failed: "+n.message)}finally{$t=!1}}}function nt(){const i=document.getElementById("create-subsidiary-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",blue:"#5a9abf",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},a=new Set(F.filter(r=>r.type==="regional_hq").map(r=>r.nation_id)),n=Oe.filter(r=>!a.has(r.id)),l=se?n.find(r=>r.id===se):null;let s="";for(const r of n){const o=r.id===se,c=zt(r),m=Number(r.standard_of_living??50),p=Number(r.stability??50),u=c>6e7?t.red:c>4e7?t.orange:t.greenBright;s+=`
    ...
    </div>`}window.subSelectNation=Jn;window.subCreate=Xn;let Fe=[],$e=0,Z="ALL",ve="REPUTATION";async function Zn(){const{data:i}=await _.from("factions").select("id, faction_name, abbreviation, corp_sector, corp_subsector, corp_company_type, corp_ticker, corp_cash_reserves, nation_id, nation, linked_user_id").eq("faction_type","corporation").is("abandoned_at",null).order("faction_name");Fe=(i||[]).map(e=>({...e,abbr:e.corp_ticker||e.abbreviation||e.faction_name?.slice(0,4).toUpperCase()||"???",status:(e.corp_company_type||"Private").toUpperCase(),isPlayer:!!e.linked_user_id,reputation:50,revenue:e.status==="PUBLIC"?Number(e.corp_cash_reserves||0)*.1:null,valuation:e.status==="PUBLIC"?Number(e.corp_cash_reserves||0)*3:null}))}function ea(i){$e=i,at()}function ta(i){Z=i,$e=0,at()}function ia(i){ve=i,$e=0,at()}function at(){const i=document.getElementById("corporations-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",blue:"#5a8aaa",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},a={PUBLIC:{color:t.greenBright,bg:"rgba(92,204,92,0.06)",border:"rgba(92,204,92,0.15)"},PRIVATE:{color:t.gold,bg:"rgba(200,168,50,0.08)",border:"rgba(200,168,50,0.2)"},STATE:{color:t.orange,bg:"rgba(204,136,68,0.06)",border:"rgba(204,136,68,0.15)"}},n=[...new Set(Fe.map(p=>p.nation).filter(Boolean))];let l=[...Fe];Z!=="ALL"&&(l=l.filter(p=>p.nation===Z)),ve==="REPUTATION"?l.sort((p,u)=>(u.reputation||0)-(p.reputation||0)):ve==="REVENUE"?l.sort((p,u)=>(u.revenue||0)-(p.revenue||0)):ve==="VALUATION"&&l.sort((p,u)=>(u.valuation||0)-(p.valuation||0)),$e>=l.length&&($e=0);const s=l[$e]||null,d=s&&s.status==="PRIVATE",r=s&&s.status==="STATE";let o="";l.length===0&&(o=`<div style="padding:30px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">No corporations found.</div>`);for(let p=0;p<l.length;p++){const u=l[p],y=p===$e,v=a[u.status]||a.PRIVATE,h=u.status==="PRIVATE";o+=`
    ...
    <div style="padding:5px 14px;border-bottom:1px solid ${t.border};background:${t.card};display:flex;gap:12px;align-items:center;flex-shrink:0;">
        <div style="display:flex;gap:3px;align-items:center;">
            ...
            ${n.map(p=>`<span onclick="corpFilterNation('${p}')" style="padding:2px 7px;font-family:${e};font-size:7px;font-weight:700;cursor:pointer;color:${Z===p?"#000":t.dim};background:${Z===p?t.accent:"transparent"};border:1px solid ${Z===p?t.accent:t.border}">${p}</span>`).join("")}
        </div>
        <div style="flex:1"></div>
        <div style="display:flex;gap:3px;align-items:center;">
            <span style="font-family:${e};font-size:7px;color:${t.dim};letter-spacing:0.8px">SORT</span>
            ${["REPUTATION","REVENUE","VALUATION"].map(p=>`<span onclick="corpSort('${p}')" style="padding:2px 7px;font-family:${e};font-size:7px;font-weight:700;cursor:pointer;color:${ve===p?"#000":t.dim};background:${ve===p?t.accent:"transparent"};border:1px solid ${ve===p?t.accent:t.border}">${p}</span>`).join("")}
        </div>
    </div>`;i.innerHTML=`
    <div style="width:760px;height:450px;background:${t.surface};border:1px solid ${t.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
        <div style="padding:8px 14px;border-bottom:1px solid ${t.border};display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">
            ...
            <span style="font-family:${e};font-size:9px;color:${t.dim}">${Fe.length} IN DATABASE</span>
        </div>
        ${m}
        <div style="flex:1;display:flex;overflow:hidden;">
            <div style="flex:1;border-right:1px solid ${t.border};display:flex;flex-direction:column;">
    ...
    </div>`}window.corpSelect=ea;window.corpFilterNation=ta;window.corpSort=ia;let ie=null,re={},Q=120,le=15,Lt={},Me=[];async function na(){if(!ce)return;ie=ce,Lt={};try{const{data:t}=await _.from("corp_material_inventory").select("material_key, quantity").eq("faction_id",f.id);for(const a of t||[])Lt[a.material_key]=Number(a.quantity||0)}catch{}Me=[];try{const{data:t}=await _.from("contract_bids").select("faction_id, bid_price, estimated_quality, status, factions(faction_name, corp_ticker)").eq("contract_id",ie.id).in("status",["pending","won"]);Me=(t||[]).filter(a=>a.faction_id!==f?.id).map(a=>({name:a.factions?.faction_name||"Unknown",ticker:a.factions?.corp_ticker||"???",price:Number(a.bid_price||0),quality:Number(a.estimated_quality||0),status:a.status}))}catch{}re={};const i=ie.required_materials||{};for(const t of Object.keys(i))re[t]="STD";const e=ie.required_workforce||{};Q=Number(e.general||0)+Number(e.skilled||0)||120,le=15,Je(),ot()}function Ut(){document.getElementById("bid-assembly-overlay")?.remove(),ie=null}function aa(i,e){re[i]=e,ot()}function oa(i){Q=i,ot()}function sa(i){le=i,ot()}function ot(){if(document.getElementById("bid-assembly-overlay")?.remove(),!ie)return;const i="'JetBrains Mono', monospace",e={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",accentBright:"#a3b07e",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",redDim:"#a44",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},t=ie,a=t.issuer_type==="GOVERNMENT",n=k?.name||f?.nation||"—",l=Number(t.budget_ceiling||0),s=Number(t.timeline_ticks||8),d=t.required_materials||{},r=Object.keys(d),o={LOW:.5,STD:1,HIGH:2},c={LOW:e.orange,STD:e.yellow,HIGH:e.greenBright},m={LOW:"Low",STD:"Standard",HIGH:"High"},p={concrete:36e4,steel:5e5,glass_facades:56e4,em_systems:64e4,lumber:24e4,heavy_parts:8e5,aggregate:16e4,asphalt:28e4},u=Lt||{};let y=0,v="";for(const z of r){const O=Number(d[z]||0),Ft=re[z]||"STD",Vt=p[z]||3e5,Si=o[Ft],Ai=Math.round(Vt*Si),Yt=O*Ai;y+=Yt;const zi=z.replace(/_/g," ").replace(/\b\w/g,fe=>fe.toUpperCase()),Qt=Number(u[z]||0),lt=Math.max(0,O-Qt),Li=lt===0?e.greenBright:lt<O?e.yellow:e.red,Bi=lt===0?"✓ IN STOCK":`${Qt}/${O}`;v+=`
        <div style="display:flex;align-items:center;padding:5px 14px;border-bottom:1px solid ${e.border};">
            <div style="flex:1.2">
                ...
                <div style="font-family:${i};font-size:7px;color:${Li};margin-top:1px">${Bi}</div>
            </div>
            ...
            <div style="flex:1.2;display:flex;gap:2px;justify-content:center;">
                ${["LOW","STD","HIGH"].map(fe=>{const dt=Ft===fe,Kt=c[fe],qi=b(Math.round(Vt*o[fe]));return`<span onclick="bidSetGrade('${z}','${fe}')" style="padding:2px 6px;font-family:${i};font-size:7px;font-weight:700;cursor:pointer;color:${dt?"#000":e.dim};background:${dt?Kt:"transparent"};border:1px solid ${dt?Kt:e.border}" title="${qi}/unit">${m[fe]}</span>`}).join("")}
            </div>
            <div style="flex:0.8;text-align:right"><span style="font-family:${i};font-size:10px;color:${e.text}">${b(Yt)}</span></div>
        </div>`}const h=15200,$=Q*h*s,w=l,x=[{name:"Municipal Zoning Approval",cost:18e4,ticks:2,required:!0},{name:"Structural Engineering Cert.",cost:24e4,ticks:3,required:!0},{name:"Environmental Impact Assessment",cost:34e4,ticks:8,required:w>2e7},{name:"Seismic Resilience Compliance",cost:21e4,ticks:4,required:w>5e7},{name:"Heritage Conservation Review",cost:16e4,ticks:6,required:!1},{name:"Fire Safety Certification",cost:12e4,ticks:2,required:w>1e7}].filter(z=>z.required),T=new Set(["Municipal Zoning Approval","Structural Engineering Cert."]),C=x.filter(z=>!T.has(z.name)).reduce((z,O)=>z+O.cost,0),S=4e5,E=y+$+C+S,M=Math.round(E*(le/100)),R=E+M,q=R>l,V=M,X=q?0:Math.max(0,Math.min(100,Math.round(100-R/l*100+30))),Ue=X>70?e.greenBright:X>40?e.yellow:X>0?e.orange:e.red,Gt=q?"OVER CEILING":X>70?"STRONG":X>40?"COMPETITIVE":X>20?"WEAK":"UNLIKELY",rt=Object.values(re),me=rt.length>0?Math.round(rt.reduce((z,O)=>z+(O==="HIGH"?85:O==="STD"?65:45),0)/rt.length):50,Ge=me>=75?e.greenBright:me>=55?e.yellow:e.orange,Ii=me>=75?"STRONG":me>=55?"PROMISING":"UNCERTAIN",Wt=t.required_workforce||{},ze=Number(Wt.general||0)+Number(Wt.skilled||0)||100,Mi=[Math.max(40,Math.round(ze*.7)),Math.max(60,Math.round(ze*.85)),ze,Math.round(ze*1.15),Math.round(ze*1.3)],Te=document.createElement("div");Te.id="bid-assembly-overlay",Te.style.cssText="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.7);display:flex;align-items:center;justify-content:center;z-index:9999;",Te.addEventListener("click",z=>{z.target===Te&&Ut()}),Te.innerHTML=`
    <div style="width:740px;max-height:94vh;background:${e.surface};border:1px solid ${e.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
        ...
        <div style="flex:1;display:flex;overflow:hidden;">
            ...
            <div style="flex:1;border-right:1px solid ${e.border};overflow:auto;">
                ...
                <div style="padding:8px 14px;border-bottom:1px solid ${e.border};">
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
                        ...
                        <div style="display:flex;gap:3px;">
                            ${Mi.map(z=>`<span onclick="bidSetWorkers(${z})" style="padding:2px 8px;font-family:${i};font-size:8px;font-weight:700;cursor:pointer;color:${Q===z?"#000":e.dim};background:${Q===z?e.accent:"transparent"};border:1px solid ${Q===z?e.accent:e.border}">${z}</span>`).join("")}
                        </div>
                    </div>
                    <div style="display:flex;justify-content:space-between;">
                        <span style="font-family:${i};font-size:8px;color:${e.dim}">${Q} × $${h.toLocaleString()}/tick × ${s} ticks</span>
                ...
                ${x.map(z=>{const O=T.has(z.name);return`<div style="display:flex;align-items:center;padding:4px 14px;border-bottom:1px solid ${e.border};">
                ...
                <div style="display:flex;justify-content:space-between;padding:5px 14px;border-bottom:1px solid ${e.border};background:rgba(139,154,107,0.02);">
                    ...
                    <span style="font-family:${i};font-size:11px;font-weight:700;color:${e.text}">${b(C)}</span>
                </div>
            ...
            <div style="width:280px;display:flex;flex-direction:column;overflow:auto;">
                ...
                ${[{l:"Materials",v:y},{l:"Labor",v:$},{l:"Permits",v:C},{l:"Overhead",v:S}].map(z=>`<div style="display:flex;justify-content:space-between;padding:4px 14px;border-bottom:1px solid ${e.border};">
                ...
                <div style="display:flex;justify-content:space-between;padding:6px 14px;border-bottom:1px solid ${e.border};background:rgba(204,85,85,0.03);">
                    ...
                    <span style="font-family:${i};font-size:13px;font-weight:700;color:${e.red}">${b(E)}</span>
                </div>
                ...
                <div style="padding:10px 14px;border-bottom:1px solid ${e.border};">
                    <div style="display:flex;justify-content:space-between;margin-bottom:6px;">
                        ...
                        <span style="font-family:${i};font-size:16px;font-weight:700;color:${e.gold}">${le}%</span>
                    </div>
                    <input type="range" min="0" max="40" step="1" value="${le}" oninput="bidSetMarkup(+this.value)" style="width:100%;accent-color:${e.gold};height:6px;" />
                    <div style="display:flex;justify-content:space-between;font-family:${i};font-size:7px;color:${e.dim};margin-top:2px;">
                ...
                <div style="padding:10px 14px;border-bottom:1px solid ${e.border};background:${q?"rgba(204,85,85,0.04)":"rgba(200,168,50,0.03)"};">
                    ...
                    <div style="font-family:${i};font-size:22px;font-weight:700;color:${q?e.red:e.gold}">${b(R)}</div>
                    ${q?`<div style="font-family:${i};font-size:8px;font-weight:700;color:${e.red};margin-top:4px;padding:2px 6px;background:rgba(204,85,85,0.08);border:1px solid rgba(204,85,85,0.15);display:inline-block">EXCEEDS BUDGET CEILING (${b(l)})</div>`:""}
                </div>
                ...
                <div style="padding:8px 14px;border-bottom:1px solid ${e.border};">
                    <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
                        ...
                        <span style="font-family:${i};font-size:11px;font-weight:700;color:${Ue}">${Gt}</span>
                    </div>
                    <div style="width:100%;height:4px;background:${e.border}"><div style="width:${X}%;height:100%;background:${Ue}"></div></div>
                </div>
                <div style="padding:8px 14px;border-bottom:1px solid ${e.border};">
                    <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
                        <span style="font-family:${i};font-size:8px;color:${e.dim}">QUALITY ESTIMATE</span>
                        <div style="display:flex;align-items:baseline;gap:4px;">
                            <span style="font-family:${i};font-size:11px;font-weight:700;color:${Ge}">${me}</span>
                            <span style="font-family:${i};font-size:8px;color:${e.dim}">/100</span>
                            <span style="font-family:${i};font-size:8px;font-weight:700;color:${Ge}">${Ii}</span>
                        </div>
                    </div>
                    <div style="width:100%;height:4px;background:${e.border}"><div style="width:${me}%;height:100%;background:${Ge}"></div></div>
                ...
                <div style="padding:6px 14px;border-bottom:1px solid ${e.border};">
                    ...
                    ${Me.length===0?`<div style="font-family:${i};font-size:8px;color:${e.dim};padding:4px 0;">No competing bids yet. First mover advantage.</div>`:`<div style="display:flex;gap:4px;flex-wrap:wrap;">
                            ${Me.map(z=>`<span style="padding:2px 6px;font-family:${i};font-size:7px;color:${e.muted};background:${e.card};border:1px solid ${e.border};">${z.name} <span style="color:${e.dim}">Q:${z.quality}</span></span>`).join("")}
                        </div>
                        <div style="font-family:${i};font-size:7px;color:${e.dim};margin-top:3px">${Me.length} competing bid${Me.length!==1?"s":""}. Bid prices are sealed until resolution.</div>`}
        ...
        <div style="padding:10px 16px;border-top:1px solid ${e.border};background:${e.card};display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">
            <div style="display:flex;gap:12px;">
                <div><div style="font-family:${i};font-size:7px;color:${e.dim}">YOUR BID</div><div style="font-family:${i};font-size:14px;font-weight:700;color:${q?e.red:e.gold}">${b(R)}</div></div>
                ...
                <div><div style="font-family:${i};font-size:7px;color:${e.dim}">QUALITY</div><div style="font-family:${i};font-size:14px;font-weight:700;color:${Ge}">${me}</div></div>
            </div>
    ...
    </div>`,document.body.appendChild(Te)}let Ae=!1;async function ra(){if(Ae||!ie)return;const i=ie,e=i.required_materials||{},t=Object.keys(e),a=Number(i.budget_ceiling||0),n=Number(i.timeline_ticks||8),l={concrete:36e4,steel:5e5,glass_facades:56e4,em_systems:64e4,lumber:24e4,heavy_parts:8e5,aggregate:16e4,asphalt:28e4},s={LOW:.5,STD:1,HIGH:2};let d=0;for(const x of t){const T=Number(e[x]||0),C=re[x]||"STD",S=l[x]||3e5;d+=T*Math.round(S*s[C])}const o=Q*15200*n,c=a,m=[{name:"Municipal Zoning Approval",cost:18e4,required:!0},{name:"Structural Engineering Cert.",cost:24e4,required:!0},{name:"Environmental Impact Assessment",cost:34e4,required:c>2e7},{name:"Seismic Resilience Compliance",cost:21e4,required:c>5e7},{name:"Fire Safety Certification",cost:12e4,required:c>1e7}],p=new Set(["Municipal Zoning Approval","Structural Engineering Cert."]),u=m.filter(x=>x.required&&!p.has(x.name)).reduce((x,T)=>x+T.cost,0),v=d+o+u+4e5,h=Math.round(v*(le/100)),$=v+h;if($>a){alert("Bid exceeds budget ceiling. Reduce costs or markup.");return}const w=Object.values(re),A=w.length>0?Math.round(w.reduce((x,T)=>x+(T==="HIGH"?85:T==="STD"?65:45),0)/w.length):50;if(confirm('Submit bid for "'+i.name+`"?
...
Once submitted, your bid cannot be changed.`)){Ae=!0;try{const{data:x}=await _.from("shard").select("current_tick").eq("name","Alpha Shard").single(),T=x?.current_tick||0,C={};for(const E of t)C[E]=re[E]||"STD";const{error:S}=await _.from("contract_bids").insert({contract_id:i.id,faction_id:f.id,bid_price:$,material_grades:C,labor_count:Q,markup_pct:le,estimated_cost:v,estimated_quality:A,status:"pending",submitted_at_tick:T});if(S)throw S;i.status==="open"&&await _.from("construction_contracts").update({status:"bidding"}).eq("id",i.id).eq("status","open"),Ut(),alert(`Bid submitted successfully!
...
Lowest qualified bid wins.`),typeof pe=="function"&&await pe()}catch(x){alert("Bid submission failed: "+x.message)}finally{Ae=!1}}}window.openBidAssembly=na;window.closeBidAssembly=Ut;window.bidSetGrade=aa;window.bidSetWorkers=oa;window.bidSetMarkup=sa;window.submitBidAssembly=ra;let De=[],_e=0,J=null,_t=!1,ht=!1,wt=!1;async function la(){if(!ce||ht)return;ht=!0,J=ce,_e=0;const{data:i,error:e}=await _.from("contract_bids").select("*, factions(faction_name, corp_ticker, corp_subsector)").eq("contract_id",J.id).in("status",["pending","won"]).order("bid_price",{ascending:!0});if(ht=!1,e){alert("Failed to load bids: "+e.message);return}De=(i||[]).map(t=>({...t,corp:t.factions?.faction_name||"Unknown",abbr:t.factions?.corp_ticker||"???",subsector:t.factions?.corp_subsector||"—"})),Je(),Ci()}function st(){document.getElementById("bid-review-overlay")?.remove(),J=null}function da(i){_e=i,Ci()}async function ca(){if(_t||De.length===0)return;const i=De[_e];if(!(!i?.id||!i.faction_id)&&confirm("Accept bid from "+i.corp+`?

Bid Price: `+b(i.bid_price)+`
Quality: `+i.estimated_quality+`/100
Workers: `+i.labor_count+`

This will award the contract. The project begins immediately.`)){_t=!0;try{const{data:e}=await _.from("shard").select("current_tick").eq("name","Alpha Shard").single(),t=e?.current_tick||0,{error:a}=await _.from("contract_bids").update({status:"won"}).eq("id",i.id);if(a)throw a;const{error:n}=await _.from("contract_bids").update({status:"lost"}).eq("contract_id",J.id).neq("id",i.id);if(n)throw n;const{error:l}=await _.from("construction_contracts").update({status:"awarded",awarded_to_faction:i.faction_id,awarded_at_tick:t}).eq("id",J.id);if(l)throw l;st(),alert("Contract awarded to "+i.corp+`!

Bid: `+b(i.bid_price)+`
Project begins immediately.`),typeof pe=="function"&&await pe()}catch(e){alert("Failed to accept bid: "+(e.message||e))}finally{_t=!1}}}async function pa(){if(!(!J||wt)&&confirm(`Decline all bids and cancel this project?

No bids will be accepted. The contract will be removed.`)){wt=!0;try{const{error:i}=await _.from("contract_bids").update({status:"lost"}).eq("contract_id",J.id);if(i)throw i;const{error:e}=await _.from("construction_contracts").update({status:"expired"}).eq("id",J.id);if(e)throw e;st(),alert("All bids declined. Contract cancelled."),typeof pe=="function"&&await pe()}catch(i){alert("Failed: "+(i.message||i))}finally{wt=!1}}}function Ci(){if(document.getElementById("bid-review-overlay")?.remove(),!J||De.length===0)return;const i="'JetBrains Mono', monospace",e={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",accentBright:"#a3b07e",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},t=J,a=De;_e>=a.length&&(_e=0);const n=a[_e],l=Number(t.budget_ceiling||0),s=Number(t.timeline_ticks||36),d=Math.min(...a.map(u=>u.bid_price)),r=Math.max(...a.map(u=>u.estimated_quality||0));let o="";for(let u=0;u<a.length;u++){const y=a[u],v=u===_e,h=y.bid_price===d,$=(y.estimated_quality||0)===r,w=y.bid_price>l;o+=`
        <div onclick="reviewSelectBid(${u})" style="padding:10px 16px;border-bottom:1px solid ${e.border};cursor:pointer;border-left:2px solid ${v?e.accent:"transparent"};background:${v?"rgba(139,154,107,0.03)":"transparent"};">
            ...
            <div style="display:flex;gap:0;background:${e.card};border:1px solid ${e.border};">
                <div style="flex:1.2;padding:5px 10px;border-right:1px solid ${e.border}">
                    ...
                    <div style="font-family:${i};font-size:14px;font-weight:700;color:${w?e.red:e.text}">${b(y.bid_price)}</div>
                    ${w?`<div style="font-family:${i};font-size:7px;color:${e.red}">OVER BUDGET</div>`:""}
                </div>
    ...
    </div>`,document.body.appendChild(p)}window.openBidReview=la;window.closeBidReview=st;window.reviewSelectBid=da;window.acceptBid=ca;window.declineAllBids=pa;window.switchToExpansion=_i;window.switchToOperations=hi;window.hfSetChange=Rn;window.hfReset=Pn;window.hfConfirm=On;document.querySelector('[data-tab="operations"]')?.addEventListener("click",function(i){this.classList.contains("active")||(i.preventDefault(),hi(i))});Ln();
