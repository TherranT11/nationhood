import{_}from"./supabase-client-BXEzLDpS.js";import{e as $}from"./utils-C2W-HleY.js";import{initMessaging as dn}from"./messaging-B5Fng3EZ.js";import{c as cn,a as Wt,E as et,b as mt,d as Ii,e as pn,f as fn,h as hi}from"./equipment-DsuDdEne.js";const Mi={LOW:"#c84",STD:"#ca5",HIGH:"#5c5"},ve=[{key:"concrete",name:"Concrete",category:"RAW",tiers:{LOW:{requirements:[]},STD:{requirements:[{stat:"manufacturing_output",min:20}]},HIGH:{requirements:[{stat:"manufacturing_output",min:50},{stat:"higher_education",min:40}]}},priceDrivers:["manufacturing_output","inflation","fuel_prices","urbanization"]},{key:"steel",name:"Steel",category:"RAW",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:10}]},STD:{requirements:[{stat:"manufacturing_output",min:35},{stat:"rare_minerals",min:20}]},HIGH:{requirements:[{stat:"manufacturing_output",min:60},{stat:"rare_minerals",min:40},{stat:"higher_education",min:45}]}},priceDrivers:["manufacturing_output","rare_minerals","inflation","fuel_prices"]},{key:"lumber",name:"Lumber",category:"RAW",tiers:{LOW:{requirements:[{stat:"arable_land",min:10}]},STD:{requirements:[{stat:"arable_land",min:30},{stat:"physical_infrastructure",min:20}]},HIGH:{requirements:[{stat:"arable_land",min:50},{stat:"manufacturing_output",min:30}]}},priceDrivers:["arable_land","physical_infrastructure","inflation"]},{key:"aggregate",name:"Aggregate",category:"RAW",tiers:{LOW:{requirements:[]},STD:{requirements:[{stat:"rare_minerals",min:15},{stat:"physical_infrastructure",min:20}]},HIGH:{requirements:[{stat:"rare_minerals",min:35},{stat:"manufacturing_output",min:25}]}},priceDrivers:["rare_minerals","physical_infrastructure","inflation"]},{key:"em",name:"E&M Systems",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:15}]},STD:{requirements:[{stat:"manufacturing_output",min:40},{stat:"digital_infrastructure",min:25}]},HIGH:{requirements:[{stat:"manufacturing_output",min:55},{stat:"digital_infrastructure",min:50},{stat:"energy_generation",min:40}]}},priceDrivers:["manufacturing_output","digital_infrastructure","inflation","energy_generation"]},{key:"glass",name:"Glass & Facades",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:20}]},STD:{requirements:[{stat:"manufacturing_output",min:40},{stat:"standard_of_living",min:35}]},HIGH:{requirements:[{stat:"manufacturing_output",min:60},{stat:"digital_infrastructure",min:40},{stat:"higher_education",min:50}]}},priceDrivers:["manufacturing_output","standard_of_living","inflation"]},{key:"asphalt",name:"Asphalt",category:"RAW",tiers:{LOW:{requirements:[{stat:"oil_and_gas",min:10}]},STD:{requirements:[{stat:"oil_and_gas",min:30},{stat:"manufacturing_output",min:25}]},HIGH:{requirements:[{stat:"oil_and_gas",min:45},{stat:"manufacturing_output",min:40},{stat:"physical_infrastructure",min:40}]}},priceDrivers:["oil_and_gas","manufacturing_output","inflation","fuel_prices"]},{key:"heavy",name:"Heavy Machinery Parts",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:40},{stat:"rare_minerals",min:30}]},STD:{requirements:[{stat:"manufacturing_output",min:60},{stat:"rare_minerals",min:45},{stat:"higher_education",min:40}]},HIGH:{requirements:[{stat:"manufacturing_output",min:75},{stat:"rare_minerals",min:60},{stat:"higher_education",min:55},{stat:"digital_infrastructure",min:45}]}},priceDrivers:["manufacturing_output","rare_minerals","higher_education","digital_infrastructure"]}];function qe(i,e,t){const n=ve.find(s=>s.key===i);if(!n)return{available:!1,failedStat:"unknown_material"};const o=n.tiers[e];if(!o)return{available:!1,failedStat:"unknown_tier"};for(const s of o.requirements){const a=Number(t?.[s.stat]??0);if(a<s.min)return{available:!1,failedStat:s.stat,failedMin:s.min,nationValue:a}}return{available:!0}}function ti(i,e,t){const o={concrete:{LOW:200,STD:300,HIGH:500},steel:{LOW:400,STD:700,HIGH:1200},lumber:{LOW:80,STD:130,HIGH:200},aggregate:{LOW:40,STD:60,HIGH:100},em:{LOW:400,STD:700,HIGH:1200},glass:{LOW:300,STD:500,HIGH:900},asphalt:{LOW:120,STD:200,HIGH:350},heavy:{LOW:800,STD:1400,HIGH:2400}}[i]?.[e];if(!o)return 0;const s=ve.find(l=>l.key===i);if(!s)return o;let a=1;for(const l of s.priceDrivers){const d=Number(t?.[l]??50);l==="inflation"||l==="fuel_prices"?a*=1+(d-50)/200:a*=1-(d-50)/250}return a=Math.max(.4,Math.min(2.5,a)),Math.round(o*a)}function Ai(i,e,t){const o={concrete:{LOW:5e3,STD:3e3,HIGH:1e3},steel:{LOW:2e3,STD:1500,HIGH:500},lumber:{LOW:8e3,STD:4e3,HIGH:1500},aggregate:{LOW:15e3,STD:6e3,HIGH:2e3},em:{LOW:1e3,STD:700,HIGH:300},glass:{LOW:1500,STD:800,HIGH:300},asphalt:{LOW:4e3,STD:2e3,HIGH:800},heavy:{LOW:400,STD:200,HIGH:80}}[i]?.[e]||0,a=ve.find(r=>r.key===i)?.priceDrivers?.[0],d=.3+(a?Number(t?.[a]??50):50)/50*.7;return Math.round(o*d)}const ii=["LOW","STD","HIGH"],Ft={LOW:"Low",STD:"Standard",HIGH:"High"};let Xe=[],u=null,w=null,G=null,Ue=[],Qe={},ee=[],K={},Vt=-1,W="concrete",F="STD",le=500,X=[],dt={},Yt=0,ne="trucks",me=0,ue=1,ke=[],Ne=null,ct=[],Qt=null,st=null,Kt="ALL",Jt="TIMELINE";function P(i){return Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(1)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i}function mn(i){if(i>=12){const e=Math.floor(i/12),t=i%12;return t>0?e+"y "+t+"mo":e+"y"}return i+" ticks"}function zi(i){return!i||i.length===0?"":i.map(e=>{const t=dt[e];if(!t)return"";const n=t.reputation_bonus>0?"var(--green)":t.reputation_bonus<0?"var(--red)":"var(--text-dim)",o=t.reputation_bonus>0?"+"+t.reputation_bonus:t.reputation_bonus<0?String(t.reputation_bonus):"";return`<span style="display:inline-flex;align-items:center;gap:3px;padding:2px 6px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:3px;font-family:var(--font-mono);font-size:7px;color:var(--text-secondary);">${t.icon||"📍"} ${$(t.name)}${o?` <span style="color:${n};font-weight:700;">${o} REP</span>`:""}</span>`}).filter(Boolean).join(" ")}function oe(i){return Math.abs(i)>=1e9?"$"+(i/1e9).toFixed(1)+"B":Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(0)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i}function ni(i){return i==="civil_engineering"?"CIVIL":i==="industrial"?"INDUSTRIAL":i==="mega_project"?"MEGA":i?.toUpperCase()||"—"}function qi(i){return i==="civil_engineering"?"light":i==="industrial"?"heavy":i==="mega_project"?"mega":"light"}function un(){st&&clearInterval(st),st=setInterval(()=>{if(!Qt)return;const i=Qt-Date.now();if(i<=0){document.getElementById("tick-countdown").textContent="Tick due...",clearInterval(st);return}const e=Math.floor(i/36e5),t=Math.floor(i%36e5/6e4),n=Math.floor(i%6e4/1e3);document.getElementById("tick-countdown").textContent=e+"h "+t+"m "+n+"s"},1e3)}function yn(){document.body.classList.toggle("light-mode");const i=document.getElementById("theme-toggle");i.textContent=document.body.classList.contains("light-mode")?"Dark":"Light"}function gn(i,e){i==="type"&&(Kt=e),i==="sort"&&(Jt=e),document.querySelectorAll(`.filter-pill[data-filter="${i}"]`).forEach(t=>{t.classList.toggle("active",t.dataset.value===e)}),Li()}const wi={"Civil Engineering":"civil_engineering","Industrial Construction":"industrial",Megaprojects:"mega_project"};function Ni(i){if(!u)return!1;if(wi[u.corp_subsector]===i.sector)return!0;const t=(V||[]).filter(n=>n.type==="regional_hq"&&n.is_active&&n.nation_id===i.nation_id);for(const n of t)if(wi[n.subsector]===i.sector)return!0;return!1}function Li(){const i=document.getElementById("oc-list");let e=[...Ue];if(Kt==="GOVERNMENT"?e=e.filter(o=>o.issuer_type==="GOVERNMENT"):Kt==="PRIVATE"&&(e=e.filter(o=>o.issuer_type==="PRIVATE")),Jt==="TIMELINE"&&e.sort((o,s)=>(o.timeline_ticks||0)-(s.timeline_ticks||0)),Jt==="BUDGET"&&e.sort((o,s)=>(s.budget_ceiling||0)-(o.budget_ceiling||0)),document.getElementById("oc-count").textContent=e.length+" AVAILABLE",e.length===0){i.innerHTML=`
            ...
            </div>`;return}const t=G?.current_tick||0;let n="";for(const o of e){const s=o.issuer_type==="GOVERNMENT",a=s?"gov":"private",l=Ni(o),d=l?"":" locked",r=qi(o.sector),c=ni(o.sector),p=(o.timeline_ticks||0)>18?" warn":"",f=o.bidding_ends_tick?Math.max(0,o.bidding_ends_tick-t):"?";n+=`
    ...
    `;const a=document.getElementById("cd-blueprint");i.blueprint_svg?(a.innerHTML=i.blueprint_svg,a.style.display=""):(a.innerHTML=zn(i),a.style.display="");const l=i.permits_required||[],d=i.required_equipment||i.equipment_required||{},r=Array.isArray(d)?d.map(L=>({key:L,qty:1})):Object.entries(d).map(([L,B])=>({key:L,qty:B})),c=i.required_materials||i.materials_estimated||{},f={civil_engineering:"Civil Engineering",industrial:"Industrial Construction",mega_project:"Megaprojects"}[i.sector]||i.spec_category||i.sector||"—";let m="var(--teal)";i.sector==="industrial"&&(m="var(--orange)"),i.sector==="mega_project"&&(m="var(--red)");let y=P(i.budget_ceiling||i.budget||0),v=(i.timeline_ticks||i.timeline_months||0)+" Months",x="";x+=`
        <div class="cd-issue">
            ...
            </div>`}x+="</div></div>"}x+='<div class="cd-details">',i.project_type&&(x+=we("Type",i.project_type)),i.project_subtype&&(x+=we("Sub-Type",i.project_subtype)),x+=we("Specialization",f,m),x+=we("Total Budget",y,"var(--green)"),x+=we("Timeline",v),x+=we("Nation",w?.name||u.nation||"—"),i.region&&(x+=we("Region",i.region)),x+="</div>",l.length>0&&(x+=`
            <div class="cd-items">
            ...
            </div>`),document.getElementById("cd-content").innerHTML=x;const h=l.filter(L=>L.status==="approved").length,A=l.length-h,C=r.length,T=r.filter(L=>{const B=X.find(Q=>Q.equipment_key===L.key);return B&&B.owned>=L.qty}).length,b=C-T;let E="";C>0&&(b===0?E+='<span class="cd-footer__badge ok">ALL EQUIPMENT MET</span>':E+=`<span class="cd-footer__badge bad">${b} EQUIPMENT SHORT</span>`),l.length>0&&(A===0?E+='<span class="cd-footer__badge ok">ALL PERMITS APPROVED</span>':E+=`<span class="cd-footer__badge warn">${A} PERMITS PENDING</span>`);const M=s,z=i.issuer_faction_id===u?.id,D=i.status==="bidding",R=Qe[i.id];document.getElementById("cd-footer").innerHTML=`
    ...
    </div>`,document.getElementById("pm-phase").innerHTML=f;const m=[{id:"overview",label:"Overview"},{id:"events",label:"Events",badge:p},{id:"materials",label:"Materials"},{id:"equipment",label:"Equipment"}];document.getElementById("pm-tabs").innerHTML=m.map(x=>`<button class="pm-tab${i.tab===x.id?" active":""}" onclick="pmSetTab('${x.id}')">
    ...
    `}function Cn(i,e,t,n,o,s,a){const l=Se(i.awardedTick+i.totalTicks);Se(i.awardedTick+i.totalTicks);const d=Se(i.awardedTick),r=[{label:"Budget",value:oe(i.budget),sub:`${n}% spent`,color:t},{label:"Spent",value:oe(i.spent),color:"var(--red)"},{label:"Remaining",value:oe(o),color:"var(--green)"},{label:"Quality",value:`${i.quality}/100`,sub:i.qualityLabel,color:i.quality>=70?"var(--green)":i.quality>=50?"var(--amber)":"var(--red)"},{label:"Workforce",value:`${i.laborCount}/${i.wfNeeded}`,sub:`Bid: ${i.laborCount}`,color:i.laborCount<i.wfNeeded?"var(--orange)":"var(--text-bright)"},{label:"Remaining",value:`${s} ticks`,sub:a?"OVERDUE":`Deadline: ${l}`,color:a?"var(--red)":"var(--text-bright)"}];let c="";c+=`<div style="padding:0 16px"><div class="pm-section">
        ...
        <div class="pm-manager">
            ...
            <div class="pm-section__title">Permits</div>`;for(const x of f)c+=`<div class="pm-permit">
            ...
            <span class="pm-evt__status" style="color:${s?"var(--red)":"var(--text-dim)"};font-weight:${s?"700":"400"}">${s?"REQUIRES RESPONSE":"RESOLVED"}</span>
                    ...
                    <span class="pm-evt__impact-text">${$(n.impact)}</span>
                        ...
                        <div class="pm-resp__left">
                            <span class="pm-resp__label">${$(r.label)}</span>
                    ...
                    </div>`,e+=`<div class="pm-resp__detail">${$(r.detail)}</div>`,e+='<div class="pm-resp__costs">',r.cost&&(e+=`<span class="pm-resp__cost" style="color:var(--red)">Cost: ${oe(r.cost)}</span>`),r.qualityImpact&&r.qualityImpact!==0&&(e+=`<span class="pm-resp__cost" style="color:${r.qualityImpact>0?"var(--green)":"var(--red)"}">Quality: ${r.qualityImpact>0?"+":""}${r.qualityImpact}</span>`),!r.cost&&(!r.qualityImpact||r.qualityImpact===0)&&(e+='<span class="pm-resp__cost" style="color:var(--green)">No additional cost</span>'),e+="</div>",c&&(e+=`<div class="pm-resp__confirm">
        ...
        </div>`;const l=["STD","LOW","HIGH"],d=t.required-t.allocated;for(const r of l){const c=t.warehouseStock[r]||{qty:0},p=t.tiers[r]||{qty:0,consumed:0},f=p.qty-p.consumed;if(c.qty===0&&p.qty===0)continue;const m=r==="HIGH"?"var(--green)":r==="LOW"?"var(--orange)":"var(--text-muted)",y=r==="HIGH"?"HIGH":r==="LOW"?"LOW":"STD";if(e+='<div style="display:flex;align-items:center;justify-content:space-between;padding:4px 0;border-bottom:1px solid rgba(255,255,255,0.03);font-size:0.7rem;">',e+='<div style="display:flex;align-items:center;gap:6px;">',e+=`<span style="font-family:var(--font-mono);font-size:0.6rem;font-weight:700;color:${m};width:32px;">${y}</span>`,e+=`<span style="color:var(--text-dim);">Warehouse: <strong style="color:var(--text-primary);">${c.qty}</strong></span>`,p.qty>0&&(e+=`<span style="color:var(--text-dim);margin-left:8px;">Allocated: <strong style="color:var(--text-primary);">${p.qty}</strong></span>`),e+="</div>",e+='<div style="display:flex;gap:4px;">',c.qty>0&&d>0){const v=Math.min(c.qty,d);e+=`<button class="pm-alloc-btn" onclick="pmAllocateMaterial('${t.key}','${r}',${v})" style="font-size:0.6rem;padding:2px 8px;border:1px solid var(--green);color:var(--green);background:rgba(92,184,92,0.08);border-radius:3px;cursor:pointer;">Allocate ${v}</button>`}f>0&&(e+=`<button class="pm-alloc-btn" onclick="pmDeallocateMaterial('${t.key}','${r}',${f})" style="font-size:0.6rem;padding:2px 8px;border:1px solid var(--orange);color:var(--orange);background:rgba(232,114,74,0.08);border-radius:3px;cursor:pointer;">Return ${f}</button>`),e+="</div></div>"}e+="</div>"}return e}function Mn(i){if(i.equipment.length===0)return'<div class="pm-evt-empty">No equipment required for this project.</div>';let e='<div class="pm-tab-header">Project Equipment</div>';for(const t of i.equipment){const n=t.condition>=75?"var(--green)":t.condition>=50?"var(--amber)":t.condition>=25?"var(--orange)":"var(--red)",o=t.assignedToProject>=t.required,s=t.assignedToProject>0&&t.assignedToProject<t.required,a=o?"var(--green)":s||t.ownedTotal>0?"var(--amber)":"var(--red)",l=o?`${t.assignedToProject}/${t.required} DEPLOYED`:s?`${t.assignedToProject}/${t.required} PARTIAL`:t.ownedTotal>0?"NOT DEPLOYED":"NOT OWNED";e+=`<div class="pm-eq" style="margin-bottom:12px;">
        ...
        <g transform="translate(470,172)">
    ...
    </svg>`}async function xe(){if(!u||!u.nation_id)return;const{data:i,error:e}=await _.from("construction_contracts").select("*").in("status",["open","bidding"]).order("generated_at_tick",{ascending:!1});if(e?(console.warn("Failed to load contracts:",e.message),Ue=[]):Ue=i||[],Qe={},u&&Ue.length>0){const t=Ue.map(o=>o.id),{data:n}=await _.from("contract_bids").select("contract_id, bid_price, estimated_quality, status").eq("faction_id",u.id).in("contract_id",t);for(const o of n||[])Qe[o.contract_id]=o}Li()}function qn(){const i=document.getElementById("ap-list"),e=document.getElementById("ap-footer");if(document.getElementById("ap-count").textContent=ee.length+" ACTIVE",ee.length===0){i.innerHTML=`<div class="ap-empty">
            <div class="ap-empty__text">No active projects.<br>Win a contract bid to start<br>your first construction project.</div>
        </div>`,e.style.display="none";return}const t=G?.current_tick||0;let n=0,o=0,s="";for(const a of ee){const l=a.issuer_type==="GOVERNMENT",d=l?"gov":"private",r=Array.isArray(a.contract_bids)?a.contract_bids[0]:a.contract_bids,c=r?.bid_price||0,p=r?.estimated_cost||0,f=r?.estimated_quality||0,m=a.budget_ceiling||0,y=a.awarded_at_tick||t,v=a.stalled_ticks||0,x=Math.max(0,t-y),k=Math.max(0,x-v),h=a.timeline_ticks||8,A=Math.max(0,h-k),C=Math.min(100,Math.round(k/h*100)),T=k>h,b=v>0;qi(a.sector);const E=ni(a.sector);n+=m,o+=c,s+=`<div class="ap-item" onclick="openProjectModal('${a.id}')">
            ...
            <div class="ap-details">
                <div class="ap-detail-cell" style="flex:1">
                    ...
                    <div class="ap-detail-cell__value" style="font-size:10px;font-weight:700;color:var(--gold)">${oe(c)}</div>
                </div>
                <div class="ap-detail-cell" style="flex:1">
        ...
        </div>`}i.innerHTML=s,e.style.display=ee.length>0?"":"none",ee.length>0&&(document.getElementById("ap-total-crew").textContent=ee.length,document.getElementById("ap-total-budget").textContent=oe(n),document.getElementById("ap-total-spent").textContent=oe(o))}async function Pi(){if(!u)return;const{data:i,error:e}=await _.from("construction_contracts").select("*, contract_bids!inner(bid_price, material_grades, labor_count, estimated_cost, estimated_quality)").eq("awarded_to_faction",u.id).in("status",["awarded","in_progress"]).eq("contract_bids.faction_id",u.id).order("awarded_at_tick",{ascending:!0});e?(console.warn("Failed to load active projects:",e.message),ee=[]):ee=i||[],qn()}const ut=3e4;function yt(){let i=0,e=0;for(const t of ve)for(const n of ii){const o=K[t.key]?.[n];o&&(i+=o.qty,e+=o.value)}return{totalUnits:i,totalValue:e}}function oi(){const i=document.getElementById("wh-list"),{totalUnits:e,totalValue:t}=yt();document.getElementById("wh-count").textContent=e.toLocaleString()+" UNITS",document.getElementById("wh-total-value").textContent=P(t);const n=Math.round(e/ut*100),o=document.getElementById("wh-capacity");o.textContent=n+"%",o.style.color=n>80?"var(--red)":n>50?"var(--orange)":"var(--green)";let s="";for(let a=0;a<ve.length;a++){const l=ve[a],d=Vt===a,r=K[l.key]?.LOW||{qty:0,value:0},c=K[l.key]?.STD||{qty:0,value:0},p=K[l.key]?.HIGH||{qty:0,value:0},f=r.qty+c.qty+p.qty,m=r.value+c.value+p.value,y=f===0,v=qe(l.key,"LOW",w),x=qe(l.key,"STD",w),k=qe(l.key,"HIGH",w),h=r.qty>0?"wh-dot wh-dot--low":"wh-dot wh-dot--empty",A=c.qty>0?"wh-dot wh-dot--std":"wh-dot wh-dot--empty",C=k.available?p.qty>0?"wh-dot wh-dot--high":"wh-dot wh-dot--empty":"wh-dot wh-dot--locked";if(s+='<div class="wh-row">',s+=`<div class="wh-row__collapsed${d?" expanded":""}" onclick="toggleWhRow(${a})">
            ...
            <span class="wh-row__val${y?" empty":""}">${m>0?P(m):"—"}</span>
            ...
            </div>`;const T=[{key:"LOW",label:"Low",data:r,avail:v,color:"var(--orange)",dotClass:"wh-dot--low"},{key:"STD",label:"Standard",data:c,avail:x,color:"var(--amber)",dotClass:"wh-dot--std"},{key:"HIGH",label:"High",data:p,avail:k,color:"var(--green)",dotClass:"wh-dot--high"}];for(const b of T){const E=!b.avail.available,M=b.data.qty>0,z=M?"$"+Math.round(b.data.value/b.data.qty):"—";s+=`<div class="wh-grade${E?" unavailable":""}">
        ...
        </div>`}f+="</div>";const y=(Number(u?.corp_cash_reserves)||0)>=s,v=le>o,{totalUnits:x}=yt(),k=ut-x,h=le>k,A=k<=0,C=Mi[F];f+=`<div class="pr-order">
        ...
        <div class="pr-order__box">
    ...
    </div>`,i.innerHTML=f}function Bn(i){W=i,F="STD";for(const e of["STD","HIGH","LOW"])if(qe(i,e,w).available){F=e;break}Di(),si(),gt()}function Rn(i){F=i,si(),gt()}function Pn(i){le=i,gt()}let zt=!1;async function On(){if(zt||!u||!w)return;const i=ti(W,F,w),e=Ai(W,F,w),t=i*le,n=Number(u.corp_cash_reserves)||0;if(t>n){alert("Insufficient cash reserves.");return}if(le>e){alert("Exceeds available supply this tick.");return}const{totalUnits:o}=yt(),s=ut-o;if(s<=0){alert("Warehouse is full. Cannot purchase more materials.");return}if(le>s){alert(`Warehouse can only hold ${s.toLocaleString()} more units. Reduce quantity.`);return}zt=!0;const a=document.querySelector(".pr-purchase-btn");a&&(a.disabled=!0,a.textContent="...");try{const l=n-t,{error:d}=await _.from("factions").update({corp_cash_reserves:l}).eq("id",u.id);if(d)throw d;const r=K[W]?.[F],c=(r?.qty||0)+le,p=(r?.value||0)+t,{error:f}=await _.from("corp_warehouse").upsert({faction_id:u.id,nation_id:u.nation_id,material_key:W,quality_tier:F,quantity:c,total_value:p,last_purchased_tick:G?.current_tick||null,updated_at:new Date().toISOString()},{onConflict:"faction_id,material_key,quality_tier"});if(f){const{error:m}=await _.from("factions").update({corp_cash_reserves:n}).eq("id",u.id);throw m&&console.error("Cash refund failed after warehouse error:",m.message),f}u.corp_cash_reserves=l,K[W]||(K[W]={}),K[W][F]={qty:c,value:p},oi(),Oi(),a&&(a.textContent="PURCHASED",setTimeout(()=>{a.isConnected&&(a.disabled=!1,a.textContent="PURCHASE")},1500))}catch(l){a&&(a.disabled=!1,a.textContent="PURCHASE"),alert("Purchase failed: "+(l.message||"Unknown error"))}finally{zt=!1}}function ji(i){const e=Ne||w;if(!e)return[];const t=mt(i);if(!t)return[];const n=pn(i,e),o=[],s=Number(e?.inflation??50),a=Number(e?.fuel_prices??50);Number(e?.manufacturing_output??50);const l=Ne&&w&&Ne.id!==w.id;let d=null;if(l&&(d=fn(e,w)),n.newAvailable>0){const r=hi(i,e),c=t.basePrice,p=Math.round(c*((s-50)/200)),f=Math.round(c*((a-50)/300));let m=r;const y=[{label:"Base price",value:P(c)},p!==0?{label:`Inflation (${s})`,mod:(p>=0?"+":"")+P(Math.abs(p))}:null,f!==0?{label:`Fuel transport (${a})`,mod:(f>=0?"+":"")+P(Math.abs(f))}:null].filter(Boolean),v=r-c-p-f;if(v!==0&&!l&&y.push({label:"Demand/scarcity",mod:(v>=0?"+":"")+P(Math.abs(v))}),l&&d){const x=Math.round(r*d.tariff),k=Math.round(r*d.transport);m=r+x+k,y.push({label:`Import tariff (${Math.round(d.tariff*100)}%)`,mod:"+"+P(x)}),y.push({label:`Transport (${d.deliveryTicks} tick${d.deliveryTicks>1?"s":""})`,mod:"+"+P(k)})}o.push({seller:l?`${e.name} Buyers`:"Local Buyers",sellerType:"LOCAL",nation:e.name||"—",distance:l?d?.deliveryTicks||1:0,condition:100,price:Math.round(m),available:n.newAvailable,delivery:l?d.deliveryTicks+" tick"+(d.deliveryTicks>1?"s":""):"Immediate",deliveryTicks:l?d.deliveryTicks:0,used:!1,priceFactors:y,sourceNationId:e.id})}if(n.usedAvailable>0){const r=n.usedCondition,c=hi(i,e,{used:!0,condition:r});let p=c;const f=[{label:"Base price",value:P(t.basePrice)},{label:`Condition (${r}%)`,mod:"-"+P(Math.max(0,t.basePrice-c))}];if(l&&d){const m=Math.round(c*d.tariff),y=Math.round(c*d.transport);p=c+m+y,f.push({label:`Import tariff (${Math.round(d.tariff*100)}%)`,mod:"+"+P(m)}),f.push({label:`Transport (${d.deliveryTicks} tick${d.deliveryTicks>1?"s":""})`,mod:"+"+P(y)})}o.push({seller:l?`${e.name} Buyers`:"Local Buyers",sellerType:"LOCAL",nation:e.name||"—",distance:l?d?.deliveryTicks||1:0,condition:r,price:Math.round(p),available:n.usedAvailable,delivery:l?d.deliveryTicks+" tick"+(d.deliveryTicks>1?"s":""):"Immediate",deliveryTicks:l?d.deliveryTicks:0,used:!0,priceFactors:f,sourceNationId:e.id})}return o}function vt(){const i=Number(u?.corp_cash_reserves)||0;document.getElementById("em-cash").textContent=P(i);const e=mt(ne),t=et[e?.tier||1],n=document.getElementById("em-tier-badge");n&&(n.textContent=t.tag,n.style.color=t.color),n.style.background=t.color+"0a",n.style.border="1px solid "+t.color+"33";const o=document.getElementById("em-nation-select");if(o&&o.options.length===0){const l=w?.name||u?.nation||"—";let d=`<option value="">${$(l)} (HQ)</option>`;for(const r of ct)r.id!==w?.id&&(d+=`<option value="${r.id}">${$(r.name)}</option>`);o.innerHTML=d}const s=document.getElementById("em-import-tag"),a=Ne&&w&&Ne.id!==w.id;s&&(s.style.display=a?"":"none"),Dn(),ri()}function Dn(){let i="";for(let e=1;e<=3;e++){const t=et[e],n=Wt(e),o=e===3?"em-selector__grid em-selector__grid--t3":"em-selector__grid em-selector__grid--t12";i+=`<div class="em-selector__row">
            ...
            <div class="em-stat-cell" style="flex:1.2">
                ...
                ${o.priceFactors.map(d=>`<div class="em-breakdown__row">
                    <span class="em-breakdown__label">${$(d.label)}</span>
            ...
            </div>`),e+="</div>"}const t=ke[me];if(t){const n=mt(ne),o=et[n?.tier||1],s=Math.min(t.available,4),a=t.price*ue,l=(Number(u?.corp_cash_reserves)||0)>=a;e+=`<div class="em-purchase"><div class="em-purchase__box">
            ...
            <div class="em-purchase__qty">
                ...
                <div class="em-purchase__qty-btns">
                    ${Array.from({length:s},(d,r)=>r+1).map(d=>`<span class="em-qty-btn${ue===d?" active":""}" style="${ue===d?"background:"+o.color+";border-color:"+o.color:""}" onclick="setEmQty(${d})">${d}</span>`).join("")}
                </div>
        ...
        </div></div>`}i.innerHTML=e}async function jn(i){if(!i)Ne=null;else{let t=ct.find(n=>n.id===i);if(!t)try{const{data:n}=await _.from("nations").select("*").eq("id",i).single();t=n}catch{}Ne=t||null}me=0,ue=1;const e=document.getElementById("em-nation-select");e&&(e.value=i||""),vt()}function Hn(i){ne=i,me=0,ue=1,vt()}function Un(i){me=i,ue=1,ri()}function Gn(i){ue=i,ri()}let qt=!1;async function Wn(){if(qt)return;const i=ke[me];if(!i||!u)return;const e=mt(ne);if(!e)return;const t=ue,n=i.price*t,o=Number(u.corp_cash_reserves)||0;if(n>o){alert("Insufficient cash reserves.");return}if(t>i.available){alert("Not enough units available.");return}const s=document.querySelector(".em-purchase-btn");s&&(s.disabled=!0,s.textContent="..."),qt=!0;try{const a=o-n,{error:l}=await _.from("factions").update({corp_cash_reserves:a}).eq("id",u.id);if(l)throw l;const d=!i.deliveryTicks||i.deliveryTicks===0;if(d){const c=X.find(A=>A.equipment_key===ne),p=(c?.owned||0)+t,f=c?.purchase_price_avg||0,m=c?.owned||0,y=m>0?Math.round((f*m+i.price*t)/p):i.price,v=e.maintenancePerUnit*p,x=c?.condition||100,k=Math.round((x*m+i.condition*t)/p),{error:h}=await _.from("corp_equipment").upsert({faction_id:u.id,nation_id:u.nation_id,equipment_key:ne,tier:e.tier,owned:p,deployed:c?.deployed||0,condition:k,maintenance_per_tick:v,purchase_price_avg:y,last_purchased_tick:G?.current_tick||null,updated_at:new Date().toISOString()},{onConflict:"faction_id,equipment_key"});if(h){const{error:A}=await _.from("factions").update({corp_cash_reserves:o}).eq("id",u.id);throw A&&console.error("Cash refund failed:",A.message),h}c?(c.owned=p,c.condition=k,c.maintenance_per_tick=v):X.push({equipment_key:ne,tier:e.tier,owned:p,deployed:0,condition:k,maintenance_per_tick:v,assigned_projects:[]})}else{const c=(G?.current_tick||0)+i.deliveryTicks,{error:p}=await _.from("corp_equipment_deliveries").insert({faction_id:u.id,equipment_key:ne,quantity:t,condition:i.condition,delivery_tick:c,source_nation_id:i.sourceNationId||null,seller_name:i.seller,price_paid:n});if(p){const{error:f}=await _.from("factions").update({corp_cash_reserves:o}).eq("id",u.id);throw f&&console.error("Cash refund failed:",f.message),p}}u.corp_cash_reserves=a,li(),vt();const r=document.getElementById("pr-cash");r&&(r.textContent=P(a)),s&&(s.textContent=d?"PURCHASED":"ORDERED",setTimeout(()=>{s.isConnected&&(s.disabled=!1,s.textContent="PURCHASE")},1500))}catch(a){s&&(s.disabled=!1,s.textContent="PURCHASE"),alert("Purchase failed: "+(a.message||"Unknown error"))}finally{qt=!1}}let Fn=-1,Ge=[],pt=[],Xt=[];function Nt(i){return Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(1)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i.toLocaleString()}function Vn(i,e,t){if(t)return"var(--orange)";const n=i/(e||1)*100;return n>50?"var(--green)":n>25?"var(--amber)":"var(--red)"}function Ti(){const i=document.getElementById("pm-list"),e=Ge.length,t=pt.length,n=Xt.length,o=Ge.filter(d=>d.expiring_soon).length;document.getElementById("pm-active-count").textContent=`(${e})`,document.getElementById("pm-pending-count").textContent=`(${t})`,document.getElementById("pm-apply-count").textContent=`(${n})`;const s=document.getElementById("pm-badges");let a="";o>0&&(a+=`<span class="pm-badge pm-badge--expiring">${o} EXPIRING</span>`),t>0&&(a+=`<span class="pm-badge pm-badge--pending">${t} PENDING</span>`),s.innerHTML=a;const l=Ge.reduce((d,r)=>d+(r.cost||0),0)+pt.reduce((d,r)=>d+(r.cost||0),0);document.getElementById("pm-total-cost").textContent=Nt(l),document.getElementById("pm-footer-active").textContent=e,document.getElementById("pm-footer-pending").textContent=t;{if(e===0){i.innerHTML=`<div class="pm-empty">
            ...
            </div>`;return}let d="";Ge.forEach((r,c)=>{const p=Fn===c,f=Vn(r.ticks_left,r.total_ticks,r.expiring_soon),m=Math.min(r.ticks_left/(r.total_ticks||1)*100,100);d+=`<div class="pm-item ${r.expiring_soon?"pm-item--expiring":""} ${p?"expanded":""}" onclick="togglePmExpand(${c})">
                <div class="pm-item__inner">
                    <div class="pm-item__row1">
                        <span class="pm-item__name">${$(r.name)}</span>
                        <span class="pm-item__status pm-item__status--active">ACTIVE</span>
                    </div>
                    <div class="pm-item__row2">
                        <span class="pm-nation-tag">${$((r.nation||"").toUpperCase())}</span>
                        <span class="pm-item__expiry" style="color:${f}">Expires: ${$(r.expires||"")}</span>
                        <span class="pm-item__ticks">(${r.ticks_left} ticks)</span>
                    </div>
                    <div class="pm-bar"><div class="pm-bar__fill" style="width:${m}%;background:${f}"></div></div>`,p&&(d+=`<div class="pm-detail">
                    <div class="pm-detail__row">
                        <span class="pm-detail__key">SOURCE POLICY</span>
                        <span class="pm-detail__val">${$(r.policy||"")}</span>
                    </div>
                    <div class="pm-detail__row">
                        <span class="pm-detail__key">ISSUED</span>
                        <span class="pm-detail__val">${$(r.issued||"")}</span>
                    </div>
                    <div class="pm-detail__row">
                    ...
                    <div class="pm-projects">
                        ...
                        <div class="pm-projects__list">${(r.projects||[]).map(y=>`<span class="pm-project-chip">${$(y)}</span>`).join("")}</div>
                    </div>`,r.note&&(d+=`<div class="pm-note"><span class="pm-note__text">${$(r.note)}</span></div>`),r.expiring_soon&&r.renewable&&(d+=`<div class="pm-btn-row"><button class="pm-btn pm-btn--renew" onclick="event.stopPropagation(); pmApplyForPermit('${r.permit_key}');">RENEW — ${Nt(r.cost||0)}</button></div>`),d+="</div>"),d+="</div></div>"}),i.innerHTML=d;return}}let Lt=!1;async function Yn(i){if(!(Lt||!u||!w)){Lt=!0;try{const{data:e}=await _.from("shard").select("current_tick").eq("name","Alpha Shard").single(),t=e?.current_tick||0,{data:n,error:o}=await _.rpc("apply_for_permit",{p_faction_id:u.id,p_nation_id:w.id,p_permit_key:i,p_current_tick:t});if(o){alert("Application failed: "+o.message);return}if(n&&!n.success){alert(n.error||"Application failed");return}alert("Permit application submitted! Processing: "+(n.processing_ticks||0)+" ticks."),await Hi()}catch(e){alert("Error: "+e.message)}finally{Lt=!1}}}window.pmApplyForPermit=Yn;async function Hi(){if(!u||!w){Ge=[],pt=[],Xt=[],Ti();return}const{data:i}=await _.from("construction_permits").select("*"),e=i||[],t={};for(const p of e)t[p.permit_key]=p;const{data:n}=await _.from("corp_permits").select("*").eq("faction_id",u.id).eq("nation_id",w.id),o=n||[],{data:s}=await _.from("active_laws").select("policy_id, policies(permit_key, policy_name)").eq("nation_id",w.id).not("policies.permit_key","is",null),a=new Set,l={};for(const p of s||[])p.policies?.permit_key&&(a.add(p.policies.permit_key),l[p.policies.permit_key]=p.policies.policy_name);const{data:d}=await _.from("shard").select("current_tick").eq("name","Alpha Shard").single(),r=d?.current_tick||0;Ge=o.filter(p=>p.status==="active").map(p=>{const f=t[p.permit_key]||{},m=p.expires_at_tick?Math.max(0,p.expires_at_tick-r):999,y=f.duration_ticks||24;return{name:f.name||p.permit_key,permit_key:p.permit_key,nation:w.name,policy:l[p.permit_key]||"—",issued:p.granted_at_tick!=null?Se(p.granted_at_tick):"—",expires:p.expires_at_tick?Se(p.expires_at_tick):"Single-use",cost:p.cost_paid||0,ticks_left:m,total_ticks:y,expiring_soon:m<=3&&m>0,renewable:f.duration_ticks!=null,projects:[]}}),pt=o.filter(p=>p.status==="pending").map(p=>{const f=t[p.permit_key]||{},m=f.processing_ticks||2,y=r-p.applied_at_tick,v=Math.max(0,m-y);return{name:f.name||p.permit_key,permit_key:p.permit_key,nation:w.name,applied:Se(p.applied_at_tick),status:"PROCESSING",processing_total:m,ticks_remaining:v,est_approval:Se(p.applied_at_tick+m),cost:p.cost_paid||0,required_by:l[p.permit_key]||"—"}});const c=new Set(o.filter(p=>p.status==="active"||p.status==="pending").map(p=>p.permit_key));Xt=[...a].filter(p=>!c.has(p)).map(p=>{const f=t[p]||{};return{name:f.name||p,permit_key:p,nation:w.name,description:f.description||"",policy:l[p]||"—",cost:f.cost_is_percentage?15e4:f.cost||0,processing_time:f.processing_ticks||2,duration:f.duration_ticks?f.duration_ticks+" ticks":"Single-use",category:f.category||"",difficulty:f.difficulty||"EASY"}}),Ti()}let Ie=[],Qn=-1;function fe(i){return Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(2)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i.toLocaleString()}function Ci(i){return i>=85?"var(--gold)":i>=60?"var(--green)":i>=40?"var(--orange)":"var(--red)"}function Kn(i){return"dl-result--"+i.toLowerCase()}function Si(){const i=document.getElementById("dl-list"),e=Ie.length;document.getElementById("dl-count").textContent=`${e} COMPLETED`;const t=Ie.reduce((l,d)=>{const r=d.financials||{};return l+((r.payment||0)+(r.bonus||0)-(r.penalty||0)-(r.total_cost||0))},0),n=document.getElementById("dl-lifetime-profit");n.textContent=(t>=0?"+":"")+fe(t),n.style.color=t>=0?"var(--green)":"var(--red)";const o={};Ie.forEach(l=>{o[l.result]=(o[l.result]||0)+1});const s=document.getElementById("dl-footer-results");if(s.innerHTML=Object.entries(o).map(([l,d])=>`<div class="dl-footer__result-box">
        ...
        </div>`;return}let a="";Ie.forEach((l,d)=>{const r=Qn===d,c=l.financials||{},p=(c.payment||0)+(c.bonus||0)-(c.penalty||0)-(c.total_cost||0),f=p>=0,m=Kn(l.result),v={DISTINCTION:"var(--gold)",PASS:"var(--green)",CONDITIONAL:"var(--orange)",FAIL:"var(--red)"}[l.result]||"var(--text-dim)",x=l.type==="GOVERNMENT";if(a+=`<div class="dl-item ${r?"expanded":""}" onclick="toggleDlExpand(${d})">
            <div class="dl-item__inner" style="border-left:2px solid ${v}">
                <div class="dl-item__row1">
                    <span class="dl-item__name">${$(l.name)}</span>
                    <span class="dl-result-badge ${m}">${$(l.result)}</span>
                </div>
                ...
                </div>`,r){const k=l.inspection||{};a+='<div style="margin-top:8px;">',a+='<div class="dl-section-label">Inspection Report</div>',["materials","structural","systems"].forEach(T=>{const b=k[T]||{score:0,issues:[]},E=Ci(b.score),M=Math.min(b.score/100*100,100);a+=`<div class="dl-inspect-row">
                ...
                </div>`}),a+="</div>",a+='<div class="dl-section-label">Financial Summary</div>',a+='<div class="dl-fin-panel">',a+=`<div class="dl-fin-row"><span class="dl-fin-key">CONTRACT VALUE</span><span class="dl-fin-val">${fe(c.contract_value||0)}</span></div>`,(c.bonus||0)>0&&(a+=`<div class="dl-fin-row"><span class="dl-fin-key">QUALITY BONUS</span><span class="dl-fin-val" style="color:var(--green)">+${fe(c.bonus)}</span></div>`),(c.penalty||0)>0&&(a+=`<div class="dl-fin-row"><span class="dl-fin-key">PENALTIES</span><span class="dl-fin-val" style="color:var(--red)">-${fe(c.penalty)}</span></div>`);const A=(c.payment||0)+(c.bonus||0)-(c.penalty||0);a+=`<div class="dl-fin-row"><span class="dl-fin-key">PAYMENT RECEIVED</span><span class="dl-fin-val" style="color:var(--green)">${fe(A)}</span></div>`,a+=`<div class="dl-fin-row"><span class="dl-fin-key">TOTAL COST</span><span class="dl-fin-val" style="color:var(--red)">-${fe(c.total_cost||0)}</span></div>`,a+=`<div class="dl-fin-total">
        ...
        </div>`;const o={};for(const l of X)o[l.equipment_key]=l;let s="";for(let l=1;l<=3;l++){const d=et[l],r=Wt(l),c=Yt===l,p=r.reduce((m,y)=>m+(o[y.key]?.owned||0),0),f=r.reduce((m,y)=>m+(o[y.key]?.deployed||0),0);if(s+=`<div class="eq-tier-hdr" onclick="toggleEqTier(${l})">
            <div class="eq-tier-hdr__left">
                <span class="eq-tier-hdr__arrow">${c?"▾":"▸"}</span>
                <span class="eq-tier-hdr__name" style="color:${d.color}">${$(d.name)}</span>
        ...
        </div>`,c)for(const m of r){const y=o[m.key],v=y?.owned||0,x=y?.deployed||0,k=y?.condition||0,h=m.maintenancePerUnit*v,A=v-x,C=v>0&&A===0,T=v>0&&k<65,b=Ii(k),E=y?.assigned_projects||[],M=E.length>0?E.map(z=>z.contract_name||"Project").join(", ").slice(0,30):v>0&&x>0?x+" project"+(x>1?"s":""):"—";s+=`<div class="eq-row${v===0?" unowned":""}">`,s+=`<div class="eq-row__top">
                    <div class="eq-row__name-area">
                        <span class="eq-row__name${v===0?" dim":""}">${$(m.name)}</span>
        ...
        <div class="eq-footer__tiers">${a}</div>`}function Xn(i){Yt=Yt===i?-1:i,li()}async function di(){if(!u)return;const{data:i,error:e}=await _.from("corp_equipment").select("equipment_key, tier, owned, deployed, condition, maintenance_per_tick, assigned_projects").eq("faction_id",u.id);e?(console.warn("Failed to load equipment:",e.message),X=[]):X=i||[],li()}async function Zn(){const{data:{user:i}}=await _.auth.getUser();if(!i){window.location.href="login.html";return}const{data:e}=await _.from("factions").select("*").or(`id.eq.${i.id},linked_user_id.eq.${i.id}`);Xe=(e||[]).filter(r=>r.nation_id);const t=sessionStorage.getItem("active_faction_id");if(u=Xe.find(r=>r.id===t)||Xe.find(r=>r.faction_type==="corporation")||Xe[0],!u){await _.auth.signOut(),window.location.href="login.html";return}if(u.faction_type!=="corporation"){window.location.href="dashboard.html";return}const[n,o]=await Promise.all([u.nation_id?_.from("nations").select("*").eq("id",u.nation_id).single():Promise.resolve({data:null}),_.from("shard").select("current_tick, current_date, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single()]);n.error&&console.warn("Nation load failed:",n.error.message),n.data&&(w=n.data),o.error&&console.warn("Shard load failed:",o.error.message),G=o.data;const s=u.corp_ticker||u.abbreviation||"";if(document.getElementById("corp-logo").textContent=s.slice(0,2)||"—",document.getElementById("corp-name-bar").textContent=u.faction_name||"Unnamed Corp",G){if(document.getElementById("game-date").textContent=G.current_date||"—",document.getElementById("tick-number").textContent=G.current_tick||"—",G.next_tick_at){const c=(Number(G.tick_interval_hours)||8)*36e5,p=new Date(G.next_tick_at).getTime(),m=p-c+c/2;Qt=new Date(m>Date.now()?m:p+c/2),un()}const r=document.querySelector("#tick-countdown")?.closest(".tick-item")?.querySelector(".tick-label");r&&(r.textContent="Next Corp Tick")}document.getElementById("corp-name-badge").textContent=(s?"["+s+"]":u.faction_name||"Corp")+" ▾";const a=document.getElementById("topbar-cash");if(a){const r=Number(u.corp_cash_reserves??0),c=r>=1e9?"$"+(r/1e9).toFixed(1)+"B":r>=1e6?"$"+(r/1e6).toFixed(1)+"M":"$"+Math.round(r/1e3)+"k";a.textContent="CASH: "+c}const l=document.getElementById("topbar-ap");l&&(l.style.display="none"),document.getElementById("nation-pill").textContent=(w?.name||u.nation||"—").toUpperCase();const d=document.getElementById("corp-faction-dropdown");if(d){let r="";for(const c of Xe){const p=c.id===u.id,f=c.faction_type==="corporation"?"CORP":"PARTY",m=c.faction_type==="corporation"?"var(--teal)":"var(--amber)";r+=`<div class="corp-dd-item${p?" active":""}" onclick="switchToFaction('${c.id}', '${c.faction_type}')">
                <span class="corp-dd-type" style="color:${m}">${f}</span>
                <span class="corp-dd-name">${$(c.faction_name||"Unnamed")}</span>
                <span class="corp-dd-abbr">[${$(c.abbreviation||"—")}]</span>
            </div>`}d.innerHTML=r}try{const{data:r}=await _.from("building_modifiers").select("*");dt={};for(const c of r||[])dt[c.modifier_key]=c}catch{}await Promise.all([xe(),Pi(),ai(),di(),Hi(),Jn(),ot()]);try{const{data:r}=await _.from("nations").select("*").order("name");ct=r||[]}catch{ct=[]}if(Oi(),vt(),dn(u,w,G),document.getElementById("loading").style.display="none",document.getElementById("page-content").style.display="block",new URLSearchParams(window.location.search).get("tab")==="expansion"){const r=document.querySelector('[data-tab="expansion"]');r&&Gi({preventDefault:()=>{},target:r})}}async function eo(){await _.auth.signOut(),window.location.href="login.html"}function to(){const i=document.getElementById("corp-faction-dropdown");i&&i.classList.toggle("open")}function io(i,e){const t=document.getElementById("corp-faction-dropdown");t&&t.classList.remove("open"),sessionStorage.setItem("active_faction_id",i),e==="corporation"?window.location.href="corp-operations.html":window.location.href="dashboard.html"}document.addEventListener("click",i=>{const e=document.getElementById("faction-switcher"),t=document.getElementById("corp-faction-dropdown");t&&e&&!e.contains(i.target)&&t.classList.remove("open")});document.addEventListener("keydown",i=>{i.key==="Escape"&&nt()});window.doLogout=eo;window.toggleTheme=yn;window.toggleCorpDropdown=to;window.switchToFaction=io;window.setFilter=gn;window.openContractDetail=Bi;window.closeContractDetail=nt;window.toggleWhRow=Nn;window.toggleEqTier=Xn;window.switchEmNation=jn;window.setEmType=Hn;window.setEmListing=Un;window.setEmQty=Gn;window.purchaseEquipment=Wn;window.setPrMat=Bn;window.setPrTier=Rn;window.setPrQty=Pn;window.purchaseMaterial=On;let J={general:0,skilled:0,innovative:0},Bt=!1;const Be=[{id:"general",label:"General Workforce",multiplier:2,color:"#8b9a6b",factionKey:"corp_general_workforce"},{id:"skilled",label:"Skilled Workforce",multiplier:3,color:"#c8a832",factionKey:"corp_skilled_workforce"},{id:"innovative",label:"Innovative Workforce",multiplier:6,color:"#c84",factionKey:"corp_innovative_workforce"}];function Ui(i){const e=Number(w?.minimum_wage??50),t=Number(w?.inflation??50),n=Number(w?.standard_of_living??50),o=e/100*48e3,s=1+(t-50)/100*.5,a=1+(n-50)/100*.5;return Math.round(o*i*s*a)}function g(i){const e=Math.abs(i),t=i<0?"-":"";return e>=1e9?t+"$"+(e/1e9).toFixed(2)+"B":e>=1e6?t+"$"+(e/1e6).toFixed(2)+"M":e>=1e3?t+"$"+(e/1e3).toFixed(1)+"k":t+"$"+e.toLocaleString()}async function Gi(i){i.preventDefault(),document.getElementById("operations-content").style.display="none";const e=document.getElementById("expansion-content");e.style.display="flex",e.style.justifyContent="center",e.style.gap="12px",e.style.alignItems="flex-start",e.style.flexWrap="wrap",document.querySelectorAll(".corp-nav__tab").forEach(t=>t.classList.remove("active")),i.target.classList.add("active"),await ot(),bt(),so(),await ci(),_t(),await Eo(),await xo(),wt(),ht(),await qo(),kt()}function Wi(i){i&&i.preventDefault(),document.getElementById("operations-content").style.display="flex",document.getElementById("expansion-content").style.display="none",document.querySelectorAll(".corp-nav__tab").forEach(e=>e.classList.remove("active")),document.querySelector('[data-tab="operations"]')?.classList.add("active")}function xt(){return V.reduce((e,t)=>{const n=Number(t.capacity||0),o=Number(t.condition||0)/100;return e+Math.floor(n*o)},0)+500}function no(i,e){const t=Be.find(s=>s.id===i),n=Number(u?.[t.factionKey]??0),o=J[i]+e;if(!(n+o<0)){if(e>0){const s=Be.reduce((l,d)=>{const r=Number(u?.[d.factionKey]??0),c=d.id===i?o:J[d.id];return l+r+c},0),a=xt();if(s>a)return}J[i]=o,bt()}}function oo(i){i?J[i]=0:J={general:0,skilled:0,innovative:0},bt()}async function ao(){if(Bt||!Object.values(J).some(a=>a!==0))return;let e=0;for(const a of Be){const l=J[a.id];l>0&&(e+=l*Ui(a.multiplier)*.1)}const t=Number(u?.corp_cash_reserves??0);if(e>t){alert("Insufficient cash reserves. Hiring cost: "+g(e)+", available: "+g(t));return}const n=Be.reduce((a,l)=>a+Number(u?.[l.factionKey]??0)+J[l.id],0),o=xt();if(n>o){alert("Cannot hire beyond property capacity ("+o.toLocaleString()+"). You need more workplaces.");return}const s=e>0?`Confirm workforce changes?

Hiring fee: `+g(e)+" (deducted from cash reserves)":`Confirm workforce changes?

Firing workers — no cost.`;if(confirm(s)){Bt=!0;try{const a={};for(const r of Be){const c=Number(u?.[r.factionKey]??0);a[r.factionKey]=Math.max(0,c+J[r.id])}e>0&&(a.corp_cash_reserves=Math.max(0,t-Math.round(e)));const{error:l}=await _.from("factions").update(a).eq("id",u.id);if(l)throw l;Object.assign(u,a),J={general:0,skilled:0,innovative:0};const d=document.getElementById("topbar-cash");if(d){const r=Number(u.corp_cash_reserves??0);d.textContent="CASH: "+(r>=1e6?"$"+(r/1e6).toFixed(1)+"M":"$"+Math.round(r/1e3)+"k")}bt()}catch(a){alert("Error: "+a.message)}finally{Bt=!1}}}function bt(){const i=document.getElementById("hf-card-container");if(!i)return;const e="'JetBrains Mono', monospace",t={bg:"#121210",surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",greenBright:"#5c5",red:"#c55",redDim:"#a44",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},n=Number(w?.minimum_wage??50),o=Number(w?.inflation??50),s=Number(w?.standard_of_living??50),a=n/100*48e3,l=(1+(o-50)/100*.5).toFixed(2),d=(1+(s-50)/100*.5).toFixed(2),r=w?.name||u?.nation||"Nation",c=Object.values(J).some(h=>h!==0),p=xt();let f=0,m=0,y=0,v=0,x="";for(const h of Be){const A=Number(u?.[h.factionKey]??0),C=J[h.id],T=A+C,b=Ui(h.multiplier),E=C>0,M=A*b,z=T*b,D=z-M;f+=A,m+=T,y+=M,v+=z;const R=C!==0?E?"rgba(92,204,92,0.02)":"rgba(204,85,85,0.02)":"transparent";x+=`
        ...
        <div style="padding:8px 14px;border-top:1px solid ${t.border};background:${t.card};flex-shrink:0;">
            <div style="display:flex;justify-content:space-between;align-items:center;${c?"margin-bottom:6px;":""}">
                <div>
                    ...
                    <div style="display:flex;align-items:baseline;gap:4px;">
                        <span style="font-family:${e};font-size:13px;font-weight:700;color:${f>=p?t.red:t.text}">${c?m.toLocaleString():f.toLocaleString()}</span>
    ...
    </div>`}function so(){const i=document.getElementById("wf-summary-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",blue:"#5a9abf",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660",red:"#c55"},n=(w?.name||u?.nation||"Nation").toUpperCase(),o=Number(w?.minimum_wage??50),s=Number(w?.inflation??50),a=Number(w?.standard_of_living??50),l=o/100*48e3,d=1+(s-50)/100*.5,r=1+(a-50)/100*.5,c=[{label:"General Workforce",mult:2,color:t.accent,key:"corp_general_workforce",countColor:t.text},{label:"Skilled Workforce",mult:3,color:t.gold,key:"corp_skilled_workforce",countColor:t.blue},{label:"Innovative Workforce",mult:6,color:t.orange,key:"corp_innovative_workforce",countColor:t.gold}];let p=0,f=0,m="";for(const y of c){const v=Number(u?.[y.key]??0),x=Math.round(l*y.mult*d*r),k=v*x;p+=v,f+=k,m+=`
    ...
    </div>`}let V=[];async function ot(){if(!u?.id)return;const{data:i}=await _.from("corp_properties").select("*").eq("faction_id",u.id).eq("is_active",!0).order("type",{ascending:!1}).order("purchased_at_tick",{ascending:!1});V=i||[]}function $t(){const i=document.getElementById("property-card-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",green:"#4a8",red:"#c55",gold:"#c8a832",orange:"#c84",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},n=(w?.name||u?.nation||"Nation").toUpperCase(),o=1+(Number(w?.inflation??50)-50)/100*.3;let s="",a=0,l=0;const d=w?.name||u?.nation||"Home Nation",r=5e7,c=1+(Number(w?.inflation??50)-50)/100*.3,p=.8+Number(w?.stability??50)/100*.4,f=Math.round(r*c*p),m=Math.round(f*.005);a+=f,l+=m,s+=`
    <div style="padding:8px 12px;border-bottom:1px solid ${t.border};">
        ...
        <div style="display:flex;gap:0;background:${t.card};border:1px solid ${t.border}">
            ...
            <div style="flex:1;padding:3px 6px">
                ...
                <div style="font-family:${e};font-size:10px;font-weight:700;color:${t.red}">${g(m)}</div>
            </div>
        </div>
    ...
    <div style="width:380px;height:450px;background:${t.surface};border:1px solid ${t.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
        ...
        <div style="padding:8px 14px;border-top:1px solid ${t.border};background:${t.card};flex-shrink:0;">
    ...
    </div>`}let We=[],te=null;const ft={Basic:{color:"#6a6660",label:"BASIC"},Modern:{color:"#8b9a6b",label:"MODERN"},Sustainable:{color:"#5c5",label:"SUSTAINABLE"},Innovative:{color:"#c8a832",label:"INNOVATIVE"},Heritage:{color:"#c84",label:"HERITAGE"},Premium:{color:"#ca5",label:"PREMIUM"}};async function ci(){if(!u?.nation_id)return;const{data:i,error:e}=await _.from("available_properties").select("*").eq("nation_id",u.nation_id).eq("status","available").order("price",{ascending:!0});if(e){console.warn("[Property] Failed to load marketplace:",e.message);return}We=(i||[]).map(t=>({...t,adjusted_cost:t.price,adjusted_maintenance:t.monthly_maintenance}))}function _t(){const i=document.getElementById("new-property-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",redDim:"#a44",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"};(w?.name||u?.nation||"Nation").toUpperCase();const n=Number(w?.standard_of_living??50),o=Number(w?.gdp_growth??50),s=Number(w?.inflation??50),a=w?.capital||"Capital",l={capital:a,port:a+" Port",industrial:a+" Industrial Zone",suburban:a+" Suburbs",coastal:a+" Coast"};let d="";if(We.length===0)d=`<div style="padding:20px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">No properties available in this market.<br>Improve GDP Growth and Standard of Living to unlock more.</div>`;else for(let r=0;r<We.length;r++){const c=We[r],p=te===r,f=ft[c.style]||ft.Basic,m=l[c.city_template]||a;d+=`
            <div onclick="npSelect(${r})" style="padding:8px 14px;border-bottom:1px solid ${t.border};cursor:pointer;border-left:2px solid ${p?t.accent:"transparent"};background:${p?"rgba(139,154,107,0.03)":"transparent"};">
                ...
                <div style="font-family:${e};font-size:8px;color:${t.dim};margin-bottom:5px;">${m} · ${c.type.replace(/_/g," ")}</div>
                <div style="display:flex;gap:0;background:${t.card};border:1px solid ${t.border}">
    ...
    </div>`}function ro(i){te=te===i?null:i,_t()}let Rt=!1;async function lo(){if(te===null||Rt)return;const i=We[te];if(!i)return;const e=Number(u?.corp_cash_reserves??0);if(i.adjusted_cost>e){alert(`Insufficient cash reserves.
...
This will be deducted from your cash reserves.`)){Rt=!0;try{const{error:t}=await _.from("corp_properties").insert({faction_id:u.id,nation_id:u.nation_id,catalog_id:i.catalog_id||null,name:i.name,type:i.type,style:i.style,capacity:i.capacity,purchase_price:i.adjusted_cost,monthly_maintenance:i.adjusted_maintenance,condition:i.condition,city:i.city,purchased_at_tick:Number(document.getElementById("tick-number")?.textContent||0),is_active:!0});if(t)throw t;const n=Math.max(0,e-i.adjusted_cost),{error:o}=await _.from("factions").update({corp_cash_reserves:n}).eq("id",u.id);if(o)throw o;u.corp_cash_reserves=n,i.id&&await _.from("available_properties").update({status:"sold",purchased_by:u.id}).eq("id",i.id);const s=document.getElementById("topbar-cash");s&&(s.textContent="CASH: "+(n>=1e6?"$"+(n/1e6).toFixed(1)+"M":"$"+Math.round(n/1e3)+"k")),te=null,await ci(),_t(),$t(),alert("Property purchased: "+i.name+`

Deducted: `+g(i.adjusted_cost))}catch(t){alert("Purchase failed: "+t.message)}finally{Rt=!1}}}const Me={Basic:{costMod:1,maintMod:1,repGain:1,color:"#6a6660",desc:"Functional. No frills."},Modern:{costMod:1.4,maintMod:1.2,repGain:2,color:"#8b9a6b",desc:"Contemporary design. Good retention."},Sustainable:{costMod:1.6,maintMod:.8,repGain:3,color:"#5c5",desc:"Green building. Lower upkeep."},Innovative:{costMod:2,maintMod:1.5,repGain:4,color:"#c8a832",desc:"Cutting edge. Attracts top talent."},Heritage:{costMod:1.3,maintMod:1.4,repGain:2,color:"#c84",desc:"Restored historic. High character."},Premium:{costMod:2.5,maintMod:1.8,repGain:5,color:"#ca5",desc:"Flagship prestige. Maximum reputation."}};let pi=!1,S={name:"",type:"Office Building",size:2e3,style:"Modern",timeline:36,budgetMod:0},Pt=!1;function Fi(){const e=1+(Number(w?.inflation??50)-50)/100*.3,t=Me[S.style]?.costMod||1,n=S.type==="Warehouse"?.75:1,o=Math.round(S.size*1e5*e*t*n),s=Math.round(o*(1+S.budgetMod/100)),a=Math.round(s*.007*(Me[S.style]?.maintMod||1));return{baseBudget:o,adjusted:s,maint:a,inflMod:e,styleMod:t}}function co(){pi=!0,S={name:"",type:"Office Building",size:2e3,style:"Modern",timeline:36,budgetMod:0},Vi()}function fi(){pi=!1,document.getElementById("cp-modal-overlay")?.remove()}function po(i,e){S[i]=e,Vi()}async function fo(){if(!(Pt||!S.name.trim())){Pt=!0;try{const i=Fi(),e=w?.name||u?.nation||"Unknown",t=Me[S.style]?.repGain||1,n=await _.from("shard").select("current_tick, current_date").eq("name","Alpha Shard").single(),o=n.data?.current_tick||0,s=(n.data?.current_date||"").match(/\d{4}/)?.[0]||"2015",{count:a}=await _.from("construction_contracts").select("id",{count:"exact",head:!0}).eq("nation_id",u.nation_id).eq("issuer_type","PRIVATE"),d=`PVT-C${(a||0)+1}-${s}`,{error:r}=await _.from("construction_contracts").insert({nation_id:u.nation_id,template_key:"custom_building",sector:"civil_engineering",name:S.name.trim(),description:`${S.type} (${S.style}) — ${S.size.toLocaleString()} employees, commissioned by ${u.faction_name}`,project_code:d,budget_ceiling:i.adjusted,timeline_ticks:S.timeline,required_materials:(()=>{const c=S.size/1e3,p=S.style,f={Basic:{concrete:1,steel:1,glass:.5,em:1,lumber:1.5,heavy:1,agg:1},Modern:{concrete:1.1,steel:1.1,glass:1.4,em:1.4,lumber:.5,heavy:1.1,agg:1.1},Sustainable:{concrete:.9,steel:.9,glass:.9,em:.9,lumber:1.5,heavy:.9,agg:.9},Innovative:{concrete:1.2,steel:1.2,glass:2,em:2,lumber:.5,heavy:2,agg:1.2},Heritage:{concrete:1.3,steel:1.3,glass:1.3,em:1.3,lumber:2,heavy:1.3,agg:1.3},Premium:{concrete:1.5,steel:1.5,glass:2.5,em:1.5,lumber:1,heavy:1.5,agg:1.5}}[p]||{concrete:1,steel:1,glass:1,em:1,lumber:1,heavy:1,agg:1},m=(y,v)=>Math.max(1,Math.ceil(c*y*v));return{concrete:m(8,f.concrete),steel:m(6,f.steel),glass_facades:m(3,f.glass),em_systems:m(4,f.em),lumber:m(1,f.lumber),heavy_parts:m(2,f.heavy),aggregate:m(3,f.agg)}})(),required_equipment:(()=>{const c=S.size,p={work_trucks:Math.ceil(c/2e3)+1,concrete_mixers:Math.ceil(c/3e3)+1};return c>1e3&&(p.excavators=Math.ceil(c/3e3)+1,p.tower_cranes=Math.ceil(c/4e3)+1),c>3e3&&(p.bulldozers=Math.ceil(c/4e3)+1,p.heavy_haulers=Math.ceil(c/5e3)+1),c>8e3&&(p.pile_drivers=Math.ceil(c/6e3)+1),p})(),required_workforce:{general:Math.ceil(S.size*.08),skilled:Math.ceil(S.size*.03)},status:"open",generated_at_tick:o,bidding_ends_tick:o+3,issuer_type:"PRIVATE",issuer_name:u.faction_name,issuer_faction_id:u.id});if(r)throw r;fi(),alert(`Construction project submitted!
...
All construction corporations in `+e+" can now bid on this project.")}catch(i){alert("Failed to submit project: "+i.message)}finally{Pt=!1}}}function Vi(){if(document.getElementById("cp-modal-overlay")?.remove(),!pi)return;const i="'JetBrains Mono', monospace",e={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",redDim:"#a44",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},t=Fi(),n=w?.name||u?.nation||"Nation",o=Me[S.style]?.repGain||1,s=o>=4?e.gold:o>=3?e.greenBright:o>=2?e.accent:e.dim,a=Object.entries(Me).map(([r,c])=>{const p=S.style===r;return`<div onclick="cpSetField('style','${r}')" style="padding:5px 6px;cursor:pointer;text-align:center;background:${p?c.color+"18":"transparent"};border:1px solid ${p?c.color+"44":e.border};">
        ...
        <div style="padding:12px 16px;overflow:auto;flex:1;">
            ...
            <div style="margin-bottom:12px;">
                ...
                <div style="display:flex;gap:4px;">
                    ${["Regional HQ","Office Building",...u?.corp_sector==="Construction"?["Warehouse"]:[]].map(r=>`<span onclick="cpSetField('type','${r}')" style="flex:1;text-align:center;padding:5px 0;font-family:${i};font-size:9px;font-weight:700;cursor:pointer;color:${S.type===r?"#000":e.dim};background:${S.type===r?r==="Warehouse"?e.orange:e.accent:"transparent"};border:1px solid ${S.type===r?r==="Warehouse"?e.orange:e.accent:e.border}">${r}</span>`).join("")}
                </div>
    ...
    </div>`,document.body.appendChild(l);const d=document.getElementById("cp-name-input");d&&d.addEventListener("input",r=>{S.name=r.target.value}),l.addEventListener("click",r=>{r.target===l&&fi()})}function mo(){const i=document.getElementById("cp-name-input");if(i&&(S.name=i.value),!S.name.trim()){alert("Please enter a building name.");return}fo()}window.cpClose=fi;window.cpSetField=po;window.cpSubmitFromModal=mo;window.npSelect=ro;window.npBuyProperty=lo;window.npOpenConstructionModal=co;let Ye=!1;async function uo(i){if(Ye)return;const e=V.find(l=>l.id===i);if(!e)return;const t=1+(Number(w?.inflation??50)-50)/100*.3,n=Math.round((e.purchase_price||0)*.1*t),o=Number(u?.corp_cash_reserves??0);if(n>o){alert("Insufficient cash. Refurbishment costs "+g(n)+" (inflation-adjusted), you have "+g(o));return}if(e.condition>=95){alert("Property is already in excellent condition ("+e.condition+"%).");return}const s=5+Math.floor(Math.random()*21),a=Math.min(100,e.condition+s);if(confirm('Refurbish "'+e.name+`"?

Cost: `+g(n)+`
Expected improvement: +`+s+"% condition ("+e.condition+"% → "+a+"%)")){Ye=!0;try{await _.from("corp_properties").update({condition:a}).eq("id",i);const l=Math.max(0,o-n);await _.from("factions").update({corp_cash_reserves:l}).eq("id",u.id),u.corp_cash_reserves=l;const d=document.getElementById("topbar-cash");d&&(d.textContent="CASH: "+(l>=1e6?"$"+(l/1e6).toFixed(1)+"M":"$"+Math.round(l/1e3)+"k")),await ot(),$t(),alert("Refurbished! Condition: "+e.condition+"% → "+a+"%")}catch(l){alert("Refurbishment failed: "+l.message)}finally{Ye=!1}}}async function yo(i){if(Ye)return;const e=V.find(s=>s.id===i);if(!e)return;const t=1+(Number(w?.inflation??50)-50)/100*.3,n=(e.condition||50)/100,o=Math.round((e.purchase_price||0)*.6*n*t);if(confirm('Sell "'+e.name+`"?

Sale value: `+g(o)+" (60% × "+e.condition+`% condition × inflation)

The property will go back on the market for 6 ticks.
This cannot be undone.`)){Ye=!0;try{await _.from("corp_properties").update({is_active:!1}).eq("id",i);const a=Number(u?.corp_cash_reserves??0)+o;await _.from("factions").update({corp_cash_reserves:a}).eq("id",u.id),u.corp_cash_reserves=a;const d=(await _.from("shard").select("current_tick").eq("name","Alpha Shard").single()).data?.current_tick||0;await _.from("available_properties").insert({nation_id:u.nation_id,catalog_id:e.catalog_id||null,name:e.name,type:e.type,style:e.style,capacity:e.capacity,price:Math.round(o*1.1),monthly_maintenance:e.monthly_maintenance,condition:e.condition,city:e.city,generated_at_tick:d,expires_at_tick:d+6,status:"available"});const r=document.getElementById("topbar-cash");r&&(r.textContent="CASH: "+(a>=1e6?"$"+(a/1e6).toFixed(1)+"M":"$"+Math.round(a/1e3)+"k")),await ot(),$t(),await ci(),_t(),alert('Sold "'+e.name+'" for '+g(o))}catch(s){alert("Sale failed: "+s.message)}finally{Ye=!1}}}window.propRefurbish=uo;window.propSell=yo;const Te={SALE:.8,DISSOLVE:.6,REVENUE_BASE:.02,GDP_NEUTRAL:30,DEFAULT_REPUTATION:25};function go(i){if(!i)return 0;const e=i.trim().replace(/[$,]/g,""),t=e.match(/^([\d.]+)\s*[Mm]$/),n=e.match(/^([\d.]+)\s*[Kk]$/);return Math.round(t?parseFloat(t[1])*1e6:n?parseFloat(n[1])*1e3:parseFloat(e))}function Ke(i){const e=document.getElementById("topbar-cash");e&&(e.textContent="CASH: "+(i>=1e6?"$"+(i/1e6).toFixed(1)+"M":"$"+Math.round(i/1e3)+"k"))}function vo(i){return Je.find(e=>e.id===i)?.name||"—"}function mi(i){return V.filter(e=>e.nation_id===i)}async function ui(){Fe=0,await ot(),$t(),ht(),wt()}let ce=!1,Fe=0,rt={};async function xo(){if(u?.id)try{const{data:i}=await _.from("construction_contracts").select("nation_id").eq("awarded_to_faction",u.id).in("status",["in_progress","awarded"]);rt={};for(const e of i||[])e.nation_id&&(rt[e.nation_id]=(rt[e.nation_id]||0)+1)}catch{}}function Yi(i){const e=mi(i.nation_id),t=e.reduce((y,v)=>y+Number(v.purchase_price||0),0),n=e.reduce((y,v)=>y+Number(v.capacity||0),0),o=rt[i.nation_id]||0,s=Je.find(y=>y.id===i.nation_id),a=(i.name||"").trim().split(/\s+/),l=a.length>=2?a.map(y=>y[0]).join("").toUpperCase().slice(0,4):(i.name||"SUB").slice(0,4).toUpperCase(),d=Number(i.sub_cash||0),r=Number(s?.gdp_growth??50),c=d*Te.REVENUE_BASE,p=(r-Te.GDP_NEUTRAL)/100,f=Te.DEFAULT_REPUTATION/100,m=d>0?Math.round(c*(1+p)*f):0;return{id:i.id,name:i.name,abbr:l,nation:s?.name||i.city||"—",nationId:i.nation_id,sector:u?.corp_sector||"General",subsector:i.subsector||u?.corp_subsector||"—",revenue:m,debt:0,cash:d,reputation:Te.DEFAULT_REPUTATION,valuation:t,workforce:n,projects:o,established:i.created_at?new Date(i.created_at).getFullYear().toString():"—",trend:r>=40&&d>0?"up":r>=Te.GDP_NEUTRAL&&d>0?"flat":"down",profitable:m>0}}function ht(){const i=document.getElementById("manage-subsidiaries-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",greenDark:"#2a5a3a",red:"#c55",redDim:"#a44",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},o=V.filter(c=>c.type==="regional_hq").map(Yi);Fe>=o.length&&(Fe=0);const s=o[Fe]||null;let a="";o.length===0&&(a=`<div style="padding:30px 14px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">No subsidiaries established.<br>Use Create Subsidiary to expand.</div>`);let l=0,d=0;for(let c=0;c<o.length;c++){const p=o[c],f=c===Fe;l+=p.revenue,d+=p.valuation;const m=p.trend==="up"?t.greenBright:p.trend==="down"?t.red:t.dim,y=p.trend==="up"?"▲":p.trend==="down"?"▼":"–";a+=`
        <div onclick="selectSubsidiary(${c})" style="display:flex;align-items:center;padding:7px 14px;border-bottom:1px solid ${t.border};cursor:pointer;border-left:2px solid ${f?t.accent:"transparent"};background:${f?"rgba(139,154,107,0.03)":"transparent"};">
            ...
            <span style="width:12px;font-family:${e};font-size:8px;color:${m};text-align:right">${y}</span>
        </div>`}let r="";if(s){const c=s.trend==="up"?t.greenBright:s.trend==="down"?t.red:t.dim,p=s.trend==="up"?"▲":s.trend==="down"?"▼":"–",f=s.trend==="up"?"Growing":s.trend==="down"?"Declining":"Stable",m=s.reputation>=40?t.accent:s.reputation>=25?t.yellow:t.orange,y=[{label:"Revenue",value:g(s.revenue),color:s.profitable?t.greenBright:t.redDim},{label:"Cash",value:g(s.cash),color:t.text},{label:"Debt",value:s.debt>0?g(s.debt):"$0",color:s.debt>0?t.orange:t.dim},{label:"Reputation",value:s.reputation+"/100",color:m},{label:"Market Valuation",value:g(s.valuation),color:t.gold},{label:"Workforce",value:s.workforce.toLocaleString(),color:t.text},{label:"Active Projects",value:s.projects.toString(),color:s.projects>0?t.text:t.dim}],v=s.projects===0;r=`
    ...
    </div>`}async function Qi(i,e){if(ce)return;const t=V.find(m=>m.id===i);if(!t)return;const n=e==="sell",o=n?Te.SALE:Te.DISSOLVE,s=n?"SELL":"DISSOLVE",a=n?"sold":"dissolved",l=n?"80%":"60%",d=vo(t.nation_id),r=mi(t.nation_id),c=r.reduce((m,y)=>m+Math.round((y.purchase_price||0)*o*(y.condition||50)/100),0),p=Number(t.sub_cash||0),f=c+p;if(confirm(s+' subsidiary "'+t.name+`"?
...
This cannot be undone.`)){ce=!0;try{const m=r.map(v=>v.id);if(m.length===1){const{error:v}=await _.from("corp_properties").update({is_active:!1}).eq("id",m[0]);if(v)throw v}else if(m.length>1){const{error:v}=await _.from("corp_properties").update({is_active:!1}).in("id",m);if(v)throw v}await _.from("corp_properties").update({sub_cash:0}).eq("id",i).then(()=>{}).catch(()=>{});const y=Number(u?.corp_cash_reserves??0)+f;await _.from("factions").update({corp_cash_reserves:y}).eq("id",u.id),u.corp_cash_reserves=y,Ke(y),await ui(),alert("Subsidiary "+a+". "+r.length+` properties liquidated.
Total received: `+g(f))}catch(m){alert("Failed: "+m.message)}finally{ce=!1}}}function bo(i){Qi(i,"sell")}function $o(i){Qi(i,"dissolve")}async function Ki(i,e){if(ce)return;const t=V.find(p=>p.id===i);if(!t)return;const n=Number(u?.corp_cash_reserves??0),o=Number(t.sub_cash||0),s=e?"WITHDRAW":"INJECT CAPITAL";if(e&&o<=0){alert("This subsidiary has no cash to withdraw.");return}const a=e?o:n,l=prompt(s+(e?" from ":" into ")+t.name+`

Parent cash: `+g(n)+`
Subsidiary cash: `+g(o)+`

Enter amount (e.g., 5000000 or 5M):`);if(!l)return;const d=go(l);if(!d||d<=0||isNaN(d)){alert("Invalid amount.");return}if(d>a){alert("Insufficient "+(e?"subsidiary":"parent")+" cash. Available: "+g(a));return}const r=e?n+d:n-d,c=e?o-d:o+d;if(confirm(s+" "+g(d)+(e?" from ":" into ")+t.name+`?

Parent: `+g(n)+" → "+g(r)+`
Subsidiary: `+g(o)+" → "+g(c))){ce=!0;try{await Promise.all([_.from("factions").update({corp_cash_reserves:r}).eq("id",u.id),_.from("corp_properties").update({sub_cash:c}).eq("id",i)]),u.corp_cash_reserves=r,t.sub_cash=c,Ke(r),ht(),alert((e?"Withdrew ":"Injected ")+g(d)+(e?" from ":" into ")+t.name+".")}catch(p){alert("Failed: "+p.message)}finally{ce=!1}}}function _o(i){Ki(i,!1)}function ho(i){Ki(i,!0)}async function wo(i){if(ce)return;const e=V.find(v=>v.id===i);if(!e)return;const t=Yi(e);t.nation;const n=mi(e.nation_id),o=t.valuation,s=t.cash,a=t.reputation,l=t.subsector,d=Math.round(o*2.25),r=Math.round(a*.1),c=Math.round(a*.2),p=xt(),f=Be.reduce((v,x)=>v+Number(u?.[x.factionKey]??0),0),m=Math.max(0,p-f),y=Number(u?.corp_cash_reserves??0);if(d>y){alert(`Insufficient cash to acquire subsidiary.
...
This cannot be undone.`)){ce=!0;try{const v=u.nation_id;if(n.length>0){const E=n.filter(z=>z.id!==e.id).map(z=>z.id);if(E.length===1){const{error:z}=await _.from("corp_properties").update({nation_id:v,type:"office"}).eq("id",E[0]);if(z)throw z}else if(E.length>1){const{error:z}=await _.from("corp_properties").update({nation_id:v,type:"office"}).in("id",E);if(z)throw z}const{error:M}=await _.from("corp_properties").update({nation_id:v,type:"office",sub_cash:0,subsector:null}).eq("id",e.id);if(M)throw M}const x=y-d+s,h=Number(u?.corp_general_workforce??0)+m,A=Math.random()>=.5?r:-c,C=Number(u?.standing??50),T=Math.max(0,Math.min(100,C+A)),{error:b}=await _.from("factions").update({corp_cash_reserves:x,corp_general_workforce:h,standing:T}).eq("id",u.id);if(b)throw b;u.corp_cash_reserves=x,u.corp_general_workforce=h,u.standing=T,Ke(x),await ui(),alert(`Merger complete!
...
Deducted from cash reserves.`)){Ot=!0;try{const l=(await _.from("shard").select("current_tick").eq("name","Alpha Shard").single()).data?.current_tick||0,d=85+Math.floor(Math.random()*16),r=Math.round(o*.005),{error:c}=await _.from("corp_properties").insert({faction_id:u.id,nation_id:i.id,name:e,type:"regional_hq",style:"Modern",capacity:500,purchase_price:o,monthly_maintenance:r,condition:d,city:i.capital||i.name,purchased_at_tick:l,is_active:!0,subsector:Oe||u?.corp_subsector||null});if(c)throw c;const p=Math.max(0,s-o);await _.from("factions").update({corp_cash_reserves:p}).eq("id",u.id),u.corp_cash_reserves=p,Ke(p),ae=null,Pe="",tt="",await ui(),alert('Subsidiary "'+e+'" established in '+i.name+`!

Cost: `+g(o)+`
Regional HQ created with `+d+"% condition.")}catch(a){alert("Failed: "+a.message)}finally{Ot=!1}}}function wt(){const i=document.getElementById("create-subsidiary-container");if(!i)return;const e="'JetBrains Mono', monospace",t={bg:"#121210",surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",accentBright:"#a3b07e",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},n=u?.corp_sector||"General",o=u?.corp_subsector||"",s=Ji(),a=s.find(b=>b.name===Oe)||s[0],l=new Set(V.filter(b=>b.type==="regional_hq").map(b=>b.nation_id)),d=Je.filter(b=>!l.has(b.id)),r=ae?d.find(b=>b.id===ae):null,c=Pe.trim().length>0&&tt.trim().length>=2&&r!==null;let p=`
    ...
    </div>`,m="";if(d.length===0)m=`<div style="padding:20px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">Subsidiaries in all available nations.</div>`;else for(const b of d){const E=b.id===ae,M=Co(b.id),z=Ze[b.id]||0,D=Math.round(Number(b.standard_of_living??50)),R=Zt(b);m+=`
    ...
    <div style="padding:6px 14px;border-bottom:1px solid ${t.border};">
        <div style="margin-bottom:6px;">
            ...
            <input type="text" value="${(Pe||"").replace(/"/g,"&quot;")}" oninput="subSetName(this.value)" placeholder="e.g., ${(u?.faction_name||"Corp")+" "+(r?.name||"International")}" style="width:100%;padding:5px 8px;font-family:${e};font-size:10px;color:${t.text};background:${t.card};border:1px solid ${t.border};outline:none;box-sizing:border-box;" />
        </div>
        <div>
            ...
            <input type="text" value="${(tt||"").replace(/"/g,"&quot;")}" oninput="subSetAbbr(this.value)" placeholder="${(u?.faction_name||"CORP").slice(0,2).toUpperCase()+(r?.name||"XX").slice(0,2).toUpperCase()}" maxlength="4" style="width:80px;padding:5px 8px;font-family:${e};font-size:12px;font-weight:700;color:${t.gold};background:${t.card};border:1px solid ${t.border};outline:none;text-align:center;letter-spacing:2px;" />
        </div>
    ...
    </div>`;const k=5e7,h=a.mod,A=r?Zt(r):null,C=r?Xi(r):null;let T=`
    ...
    <div style="width:380px;height:450px;background:${t.surface};border:1px solid ${t.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
        ...
        <div onclick="corpSelect(${f})" style="display:flex;align-items:center;padding:6px 14px;border-bottom:1px solid ${t.border};cursor:pointer;border-left:2px solid ${y?t.accent:"transparent"};background:${y?"rgba(139,154,107,0.03)":"transparent"};">
            <span style="width:36px;font-family:${e};font-size:9px;font-weight:700;color:${t.gold}">${m.abbr}</span>
            <div style="flex:1.3;">
                <div style="font-size:10px;font-weight:600;color:${t.text};line-height:1.2">${m.faction_name}</div>
                <div style="font-family:${e};font-size:7px;color:${t.dim};margin-top:1px">${m.corp_subsector||m.corp_sector||"—"}</div>
            </div>
            <span style="width:55px"><span style="font-family:${e};font-size:7px;padding:1px 4px;color:${t.accent};background:rgba(139,154,107,0.08);border:1px solid rgba(139,154,107,0.15)">${(m.nation||"—").toUpperCase().slice(0,6)}</span></span>
            <span style="width:50px;font-family:${e};font-size:8px;font-weight:700;color:${x?t.dim:t.muted};text-align:right">${x?"—":g(m.revenue)}</span>
            <span style="width:30px;font-family:${e};font-size:9px;font-weight:700;color:${m.reputation>=70?t.greenBright:m.reputation>=40?t.accent:t.yellow};text-align:right">${m.reputation}</span>
            <span style="width:50px;font-family:${e};font-size:8px;color:${x?t.dim:t.muted};text-align:right">${x?"—":g(m.valuation)}</span>
            <span style="width:42px;text-align:center"><span style="font-family:${e};font-size:6px;font-weight:700;padding:1px 4px;color:${v.color};background:${v.bg};border:1px solid ${v.border}">${m.status}</span></span>
        </div>`}let c="";if(a){const f=n[a.status]||n.PRIVATE,m=[{label:"Sector",value:a.corp_sector||"—",color:t.text},{label:"Subsector",value:a.corp_subsector||"—",color:t.accent},{label:"Reputation",value:a.reputation+"/100",color:a.reputation>=70?t.greenBright:a.reputation>=40?t.accent:t.yellow},{label:"Revenue",value:l?"UNDISCLOSED":g(a.revenue),color:l?t.dim:t.greenBright},{label:"Cash Reserves",value:l?"UNDISCLOSED":g(a.corp_cash_reserves||0),color:l?t.dim:t.text},{label:"Market Valuation",value:l?"UNDISCLOSED":g(a.valuation),color:l?t.dim:t.gold}];c=`
        <div style="padding:8px 14px;border-bottom:1px solid ${t.border};background:${t.card};flex-shrink:0;">
            <div style="display:flex;align-items:center;gap:6px;margin-bottom:2px;">
        ...
        ${m.map(y=>`<div style="display:flex;justify-content:space-between;padding:4px 14px;border-bottom:1px solid ${t.border};">
    ...
    <div style="width:760px;height:450px;background:${t.surface};border:1px solid ${t.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
    ...
    </div>`}window.corpSelect=No;window.corpFilterNation=Lo;window.corpSort=Bo;let de=null,ye={},U=120,ge=15,ei={},Ve=[];async function Ro(){if(!Le)return;if(Qe[Le.id]){alert("You already have a bid on this contract. Retract it first if you want to re-bid.");return}de=Le,ei={};try{const{data:t}=await _.from("corp_material_inventory").select("material_key, quantity").eq("faction_id",u.id);for(const n of t||[])ei[n.material_key]=Number(n.quantity||0)}catch{}Ve=[];try{const{data:t}=await _.from("contract_bids").select("faction_id, bid_price, estimated_quality, status, factions(faction_name, corp_ticker)").eq("contract_id",de.id).in("status",["pending","won"]);Ve=(t||[]).filter(n=>n.faction_id!==u?.id).map(n=>({name:n.factions?.faction_name||"Unknown",ticker:n.factions?.corp_ticker||"???",price:Number(n.bid_price||0),quality:Number(n.estimated_quality||0),status:n.status}))}catch{}ye={};const i=de.required_materials||{};for(const t of Object.keys(i))ye[t]="STD";const e=de.required_workforce||{};U=Number(e.general||0)+Number(e.skilled||0)||120,ge=15,nt(),Et()}function yi(){document.getElementById("bid-assembly-overlay")?.remove(),de=null}function Po(i,e){ye[i]=e,Et()}function Oo(i){U=i,Et()}function Do(i){ge=i,Et()}function Et(){if(document.getElementById("bid-assembly-overlay")?.remove(),!de)return;const i="'JetBrains Mono', monospace",e={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",accentBright:"#a3b07e",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",redDim:"#a44",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},t=de,n=t.issuer_type==="GOVERNMENT",o=w?.name||u?.nation||"—",s=Number(t.budget_ceiling||0),a=Number(t.timeline_ticks||8),l=t.required_materials||{},d=Object.keys(l),r={LOW:.5,STD:1,HIGH:2},c={LOW:e.orange,STD:e.yellow,HIGH:e.greenBright},p={LOW:"Low",STD:"Standard",HIGH:"High"},f={concrete:36e4,steel:5e5,glass_facades:56e4,em_systems:64e4,lumber:24e4,heavy_parts:8e5,aggregate:16e4,asphalt:28e4},m=ei||{};let y=0,v="";for(const N of d){const H=Number(l[N]||0),vi=ye[N]||"STD",xi=f[N]||3e5,nn=r[vi],on=Math.round(xi*nn),bi=H*on;y+=bi;const an=N.replace(/_/g," ").replace(/\b\w/g,he=>he.toUpperCase()),$i=Number(m[N]||0),It=Math.max(0,H-$i),sn=It===0?e.greenBright:It<H?e.yellow:e.red,rn=It===0?"✓ IN STOCK":`${$i}/${H}`;v+=`
        <div style="display:flex;align-items:center;padding:5px 14px;border-bottom:1px solid ${e.border};">
            ...
            <div style="flex:1.2;display:flex;gap:2px;justify-content:center;">
                ${["LOW","STD","HIGH"].map(he=>{const Mt=vi===he,_i=c[he],ln=g(Math.round(xi*r[he]));return`<span onclick="bidSetGrade('${N}','${he}')" style="padding:2px 6px;font-family:${i};font-size:7px;font-weight:700;cursor:pointer;color:${Mt?"#000":e.dim};background:${Mt?_i:"transparent"};border:1px solid ${Mt?_i:e.border}" title="${ln}/unit">${p[he]}</span>`}).join("")}
            </div>
...
No bids will be accepted. The contract will be removed.`)){Gt=!0;try{const{error:i}=await _.from("contract_bids").update({status:"lost"}).eq("contract_id",se.id);if(i)throw i;const{error:e}=await _.from("construction_contracts").update({status:"expired"}).eq("id",se.id);if(e)throw e;Tt(),alert("All bids declined. Contract cancelled."),typeof xe=="function"&&await xe()}catch(i){alert("Failed: "+(i.message||i))}finally{Gt=!1}}}function Zi(){if(document.getElementById("bid-review-overlay")?.remove(),!se||it.length===0)return;const i="'JetBrains Mono', monospace",e={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",accentBright:"#a3b07e",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},t=se,n=it;ze>=n.length&&(ze=0);const o=n[ze],s=Number(t.budget_ceiling||0),a=Number(t.timeline_ticks||36),l=Math.min(...n.map(m=>m.bid_price)),d=Math.max(...n.map(m=>m.estimated_quality||0));let r="";for(let m=0;m<n.length;m++){const y=n[m],v=m===ze,x=y.bid_price===l,k=(y.estimated_quality||0)===d,h=y.bid_price>s;r+=`
        <div onclick="reviewSelectBid(${m})" style="padding:10px 16px;border-bottom:1px solid ${e.border};cursor:pointer;border-left:2px solid ${v?e.accent:"transparent"};background:${v?"rgba(139,154,107,0.03)":"transparent"};">
    ...
    <div style="width:640px;max-height:92vh;background:${e.surface};border:1px solid ${e.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
        ...
        <div style="flex:1;display:flex;overflow:hidden;">
            ...
            <div style="width:250px;display:flex;flex-direction:column;overflow:auto;">
                ...
                ${[{l:"Materials",v:Number(o.estimated_cost||0)*.45},{l:"Labor",v:Number(o.estimated_cost||0)*.45},{l:"Overhead",v:Number(o.estimated_cost||0)*.1}].map(m=>`<div style="display:flex;justify-content:space-between;padding:4px 14px;border-bottom:1px solid ${e.border};">
                    <span style="font-family:${i};font-size:9px;color:${e.dim};text-transform:uppercase">${m.l}</span>
                    <span style="font-family:${i};font-size:10px;color:${e.muted}">${g(Math.round(m.v))}</span>
                </div>`).join("")}
                ...
                ${[{l:"Quality",v:o.estimated_quality+"/100",c:(o.estimated_quality||0)>=75?e.greenBright:(o.estimated_quality||0)>=55?e.yellow:e.orange},{l:"Markup",v:o.markup_pct+"%",c:e.muted},{l:"Workers",v:o.labor_count+" workers",c:e.text}].map(m=>`<div style="display:flex;justify-content:space-between;padding:4px 14px;border-bottom:1px solid ${e.border};">
                    <span style="font-family:${i};font-size:9px;color:${e.dim};text-transform:uppercase">${m.l}</span>
                    <span style="font-family:${i};font-size:10px;font-weight:700;color:${m.c}">${m.v}</span>
                </div>`).join("")}
                <div style="flex:1"></div>
            </div>
        </div>
