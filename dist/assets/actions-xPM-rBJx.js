import{_supabase as y}from"./supabase-client-CiYoFhIh.js";/* empty css                    *//* empty css                         */import{r as lt}from"./role-actions-GCBcK_AR.js";import{e as p,h}from"./utils-oN1e812_.js";import{renderCorpTopBar as Ve}from"./corp-topbar-ClQJCe3f.js";import{c as ct}from"./corp-valuation-DGlSNvB8.js";import{E as be,a as he,g as Ke}from"./corp-executives-BG8FScP4.js";import{a as We,G as Me,R as dt}from"./lawsuit-types-mDq47olK.js";import"./preload-helper-BXl3LOEh.js";import"./factions-1eoRseVF.js";import"./political-actions-CLyoQfsu.js";import"./config-BdOpHGNJ.js";import"./government-types-CNjNcIHN.js";import"./stats-Nd7eW9dF.js";function me(e){if(e==null)return"";const t=document.createElement("div");return t.textContent=String(e),t.innerHTML}function pt(e,t){return(e||"?")[0]+(t||"?")[0]}function ft(e){return e>=1e6?"$"+(e/1e6).toFixed(1)+"M":e>=1e3?"$"+(e/1e3).toFixed(0)+"k":"$"+e}function He(e){const t=Number(e)||0,o=Math.abs(t),n=o>=1e9?(o/1e9).toFixed(1)+"B":o>=1e6?(o/1e6).toFixed(1)+"M":o>=1e3?Math.round(o/1e3)+"k":String(Math.round(o));return(t<0?"-$":"$")+n}function mt(e,t){if(!e)return;const{faction:o,shard:n,ownedProperties:r=[],vessels:a=[],executives:l=[],selectedExecIdx:i=0}=t||{},c=o?.faction_name||"Corporation",d=(o?.abbreviation||o?.corp_ticker||"??").toUpperCase(),m=o?.corp_sector||"",f=o?.corp_subsector||"",u=Number(o?.corp_cash_reserves||0),_=Number(o?.corp_loans||0),x=ct({cash:u,loans:_,properties:r,vessels:a,financeReceivables:0,currentTick:n?.current_tick||0}),$=Number(o?.corp_reputation??50),w=Math.max(0,Math.min(100,Math.round(Number.isFinite($)?$:50))),E=w>=60?"var(--green)":w>=40?"var(--text-bright)":"var(--red)",b=r.length,C=x<0?"var(--red)":"var(--green)";lt(e,{title:"Corporate Actions",entityName:`${c} · ${d}`,entityColor:"#8b9a6b",stats:[{label:"Cash",value:He(u),color:"var(--accent)"},{label:"Reputation",value:String(w),color:E},{label:"Valuation",value:He(x),color:C}],statusBarItems:[{type:"count",label:"Sector",big:m||"—",bigColor:"#8b9a6b",dim1:f||""},{type:"count",label:"Properties",big:String(b),bigColor:"#8b9a6b",dim1:b===1?"building":"buildings"}],rolesContainerId:"corp-exec-list",panelContainerId:"corp-actions-panel",rolesColumnWidth:262});const R=document.getElementById("corp-exec-list");if(R){const g=new Map(l.map(A=>[A.role,A]));let L="";for(let A=0;A<be.length;A++){const ee=be[A],nt=he[ee],N=g.get(ee)||null,fe=i===A,U=nt.color,at=!N;if(L+=`<div onclick="actSelectExec(${A})" style="
                padding:10px 12px;
                background:${fe?U+"0a":"var(--bg-2,#1a1a17)"};
                border:1px solid ${fe?U+"44":"var(--border-0,rgba(255,255,255,0.06))"};
                border-left:3px solid ${fe?U:"var(--border-0,rgba(255,255,255,0.06))"};
                cursor:pointer;
            ">`,at&&ee!=="CEO")L+=`<div style="display:flex;align-items:center;gap:10px;">
                    ...
                    <div style="flex:1;min-width:0;">
                        <div style="display:flex;align-items:center;gap:4px;">
                            <span style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:${U};">${me(ee)}</span>
                        </div>
                ...
                </div>`;else{const it=N?`${N.first_name} ${N.last_name}`:"—",je=N?N.age:0,rt=N?N.salary_per_year:0,st=N?pt(N.first_name,N.last_name):"—";L+=`<div style="display:flex;align-items:center;gap:10px;">
                    <div style="width:45px;height:45px;background:${U}15;border:1px solid ${U}33;display:flex;align-items:center;justify-content:center;font-family:var(--font-mono);font-size:14px;font-weight:700;color:${U};flex-shrink:0;">${me(st)}</div>
                    <div style="flex:1;min-width:0;">
                        <div style="display:flex;align-items:center;gap:4px;">
                            <span style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:${U};">${me(ee)}</span>
                        </div>
                        <div style="font-size:13px;font-weight:600;color:${fe?"var(--text-bright,#f0efe6)":"var(--text-muted,#666)"};overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${me(it)}${je?` <span style="font-family:var(--font-mono);font-size:10px;color:var(--text-dim);">(${je})</span>`:""}</div>
...
Range: $1M – $10B.`);if(n===null)return;const r=parseFloat(n);if(isNaN(r)||r<=0){alert("Amount must be a positive number.");return}const a=Math.round(r*1e6);if(a<vt){alert("Minimum raise is $1M.");return}if(a>yt){alert("Maximum raise is $10B.");return}const l=prompt(`APPLY FOR EQUITY — STEP 2 / 3
...
This action CANNOT be undone.`))return;if(prompt('Type "BANKRUPT" to confirm bankruptcy of '+r+":")!=="BANKRUPT"){alert("Bankruptcy cancelled.");return}$e=!0;try{const{data:l,error:i}=await y.rpc("declare_corp_bankruptcy",{p_faction_id:t});if(i)throw i;const c=Number(l?.total_payback||0),d=c>0?`
$`+c.toLocaleString()+" repaid to creditors.":"";sessionStorage.removeItem("active_faction_id"),sessionStorage.removeItem("nationhood_state");const{data:m,error:f}=await y.from("factions").select("id, faction_type").or(`id.eq.${e.id},linked_user_id.eq.${e.id}`);f&&console.warn("[Bankruptcy] remaining-factions lookup failed:",f.message);const u=(m||[]).find(x=>x.faction_type==="party"),_=(m||[]).find(x=>x.faction_type==="corporation");u?(sessionStorage.setItem("active_faction_id",u.id),alert(r+" has declared bankruptcy."+d+`

Redirecting to your political party.`),window.location.href="dashboard.html"):_?(sessionStorage.setItem("active_faction_id",_.id),alert(r+" has declared bankruptcy."+d+`

Redirecting to your other corporation.`),window.location.href="corp-dashboard.html"):(alert(r+" has declared bankruptcy."+d+`

You have no remaining factions.`),window.location.href="faction-select.html")}catch(l){alert("Bankruptcy failed: "+(l?.message||l)+`

Please try again or contact support.`)}finally{$e=!1}}const _e=2e6;let v=null;async function ht(e){if(!e?.id)return;v={plaintiff:e,relationships:[],loading:!0,submitting:!1,selectedDefendantId:null,selectedGrievance:null,selectedRelief:null,error:null};const t=document.createElement("div");t.id="sue-overlay",t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:600;display:flex;align-items:flex-start;justify-content:center;padding:40px 24px;overflow-y:auto;",t.onclick=o=>{o.target===t&&Te()},document.body.appendChild(t),B();try{v.relationships=await _t(e)}catch(o){console.warn("[sue-corp] relationship load failed:",o),v.relationships=[]}v.loading=!1,B()}function Te(){const e=document.getElementById("sue-overlay");e&&e.remove(),v=null}async function _t(e){const{data:t,error:o}=await y.from("bank_loans").select("id, lender_faction_id, borrower_faction_id, principal, outstanding, status").or(`borrower_faction_id.eq.${e.id},lender_faction_id.eq.${e.id}`).in("status",["active","called","late","delinquent"]);if(o)return console.warn("[sue-corp] loan fetch failed:",o.message),[];const n=t||[];if(n.length===0)return[];const r=new Set;for(const c of n){const d=c.borrower_faction_id===e.id?c.lender_faction_id:c.borrower_faction_id;d&&r.add(d)}if(r.size===0)return[];const{data:a,error:l}=await y.from("factions").select("id, faction_name, abbreviation, corp_ticker, corp_sector, nation_id, nations:nation_id(name)").in("id",Array.from(r));if(l)return console.warn("[sue-corp] counterparty fetch failed:",l.message),[];const i=new Map((a||[]).map(c=>[c.id,c]));return n.flatMap(c=>{const d=c.borrower_faction_id===e.id?c.lender_faction_id:c.borrower_faction_id,m=i.get(d);return m?[{id:c.id,kind:"loan",defendantId:m.id,defendantName:m.faction_name,defendantTicker:m.corp_ticker||m.abbreviation||"",defendantNation:m.nations?.name||"",defendantSector:m.corp_sector||"",label:`Active Loan ${h(c.outstanding??c.principal)}`,relationshipKind:"loan",snapshot:{principal:c.principal,outstanding:c.outstanding,status:c.status}}]:[]})}function wt(e){const t=v.relationships[e];if(!t)return;v.selectedDefendantId=t.defendantId,v.selectedRelationshipIdx=e;const o=We(t.defendantSector);if(v.selectedGrievance){const n=Me.find(r=>r.key===v.selectedGrievance);(!n||!o.has(n.sector))&&(v.selectedGrievance=null)}B()}function kt(e){v.selectedGrievance=e,B()}function $t(e){v.selectedRelief=e,B()}function Xe(){return!!v&&!v.submitting&&v.selectedDefendantId&&v.selectedGrievance&&v.selectedRelief&&Number(v.plaintiff?.corp_cash_reserves??0)>=_e}function B(){const e=document.getElementById("sue-overlay");if(!e||!v)return;const t=Number(v.plaintiff?.corp_cash_reserves??0),o=t>=_e,n=v.selectedRelationshipIdx!=null?v.relationships[v.selectedRelationshipIdx]:null,r=n?We(n.defendantSector):new Set(["universal"]);let a='<div onclick="event.stopPropagation()" style="width:760px;max-width:94vw;background:var(--panel-main);border:1px solid var(--panel-border);display:flex;flex-direction:column;overflow:hidden;">';if(a+=`<div style="padding:18px 24px;border-bottom:1px solid var(--panel-border);background:var(--bg-panel);display:flex;justify-content:space-between;align-items:flex-start;">
        <div>
        ...
        <span onclick="window.sueCorpClose()" style="font-family:var(--font-mono);font-size:18px;color:#6a6660;cursor:pointer;padding:0 6px;">&times;</span>
    </div>`,a+='<div style="padding:20px 24px;max-height:70vh;overflow-y:auto;">',a+=Ce("I.","Select Defendant","Active Relationships Only"),v.loading)a+='<div style="padding:24px;text-align:center;font-family:var(--font-mono);font-size:11px;color:#6a6660;">Loading relationships…</div>';else if(v.relationships.length===0)a+='<div style="padding:24px;text-align:center;font-family:var(--font-mono);font-size:11px;color:#6a6660;">No active business relationships. You can only sue corporations you have an open loan, trade agreement, or contract with.</div>';else{a+='<div style="display:flex;flex-direction:column;gap:6px;">';for(let c=0;c<v.relationships.length;c++){const d=v.relationships[c],m=c===v.selectedRelationshipIdx,f=d.kind==="loan"?"#5a8aaa":d.kind==="trade"?"#c8a832":"#a0633a";a+=`<div onclick="window.sueCorpSelectRel(${c})" style="
                padding:12px 14px;
            ...
            </div>`}a+="</div>"}const l=n?`Defendant: ${n.defendantSector||"Unknown"} Sector`:"Pick a defendant first";a+=Ce("II.","Grievance Type",l),a+='<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:6px;">';for(const c of Me){const d=v.selectedGrievance===c.key,m=!n||!r.has(c.sector),f=c.sector==="universal"?"#8a722f":c.sector==="banking"?"#5a8aaa":c.sector==="construction"?"#a0633a":"#4a8a87";a+=`<button ${m?"disabled":`onclick="window.sueCorpSelectGrievance('${c.key}')"`} style="
            padding:12px 14px;
        ...
        </button>`}a+="</div>",a+=Ce("III.","Relief Sought","What You Want the Court to Order"),a+='<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:6px;">';for(const c of dt){const d=v.selectedRelief===c.key;a+=`<button onclick="window.sueCorpSelectRelief('${c.key}')" style="
            padding:12px 14px;
            background:${d?"rgba(200,90,58,0.06)":"var(--bg-2,#1a1a17)"};
            border:1px solid ${d?"#c55":"var(--panel-border)"};
            cursor:pointer;
        ...
        </button>`}a+="</div>",a+="</div>";const i=Xe();a+=`<div style="padding:14px 24px;border-top:1px solid var(--panel-border);background:var(--bg-panel);display:flex;justify-content:space-between;align-items:center;gap:18px;">
    ...
    </div>`,v.error&&(a+=`<div style="padding:8px 24px;font-family:var(--font-mono);font-size:10px;color:#c55;background:var(--bg-panel);border-top:1px solid var(--panel-border);">${p(v.error)}</div>`),a+="</div>",e.innerHTML=a}function Ce(e,t,o){return`<div style="display:flex;justify-content:space-between;align-items:baseline;margin:18px 0 10px;padding-bottom:8px;border-bottom:1px dashed var(--panel-border);">
...
New salary: ${K(i.new_salary)}/yr  (+${i.markup_pct}%)`),await Ae(),D()}catch(i){alert("Renewal failed: "+(i?.message||i))}finally{Ee.delete(e)}}}window.confirmRenewExec=It;function D(){const e=document.getElementById("actions-container");e&&(mt(e,{faction:s,shard:k,ownedProperties:Qe,vessels:[],executives:ce,selectedExecIdx:ae}),Nt())}const Pe={CEO:[{id:"statement",name:"Issue Statement",desc:"Issue a press release to the public events feed. Other players and media corps see it.",cost:"~$20k",costColor:"#5cb85c",tags:["REPUTATION"],cooldown:"once/tick"},{id:"ipo",name:"IPO",desc:"Take the corporation public. Sell ~30% of shares for a massive cash injection. Permanent loss of full control.",cost:"COMING SOON",costColor:"var(--text-dim)",tags:["STRUCTURAL"],locked:!0,lockReason:"Coming soon"},{id:"bankruptcy",name:"Declare Bankruptcy",desc:"The CEO officially files for bankruptcy, ceasing all operations. Outstanding loans will be repaid up to 50% of the corporation's market valuation.",descRed:"This will dissolve your corporation. Loans will be paid back, and you will need to found a new corporation. There is a 24 tick cooldown on declaring bankruptcy.",cost:"IRREVERSIBLE",costColor:"#c55",tags:["IRREVERSIBLE"]}],CFO:[{id:"loan",name:"Request Loan",desc:"Submit a loan application to all finance corporations. Set amount, purpose, term, and collateral. Receive competing offers.",cost:"FREE",costColor:"#5cb85c",tags:["FINANCIAL"]},{id:"equity",name:"Apply for Equity",desc:"Raise capital by offering Investment corps a stake in your monthly profits. Series (A/B/C…) is auto-assigned by your prior funded raise count. No repayment — investors get a percentage of profit each tick going forward.",cost:"FREE",costColor:"#c89a4a",tags:["FINANCIAL","STRUCTURAL"],hideForSector:"Finance"},{id:"paydown",name:"Pay Down Debt",desc:"Use cash to retire outstanding loan principal early. Reduces debt and improves your credit standing. Paying off a loan in full grants +1 Reputation. Costs $100k admin fee.",cost:"$100k",costColor:"#5a8aaa",tags:["FINANCIAL"]}],COO:[{id:"restructure",name:"Restructure Operations",desc:"Lay off 10-20% of workforce, cut ~7% of debt. Takes a small reputation hit.",cost:"FREE",costColor:"#5cb85c",tags:["OPERATIONAL"],cooldown:"once/tick"}],CTO:[{id:"research",name:"Begin Research",desc:"Start researching a tech tree node. Opens the tech tree interface.",cost:"COMING SOON",costColor:"var(--text-dim)",tags:["INNOVATION"],locked:!0,lockReason:"Coming soon"}],CMO:[{id:"rebrand",name:"Rebrand Corporation",desc:"Change name and abbreviation. Costs ~$2M and takes a small reputation hit.",cost:"~$2M",costColor:"#ca5",tags:["STRUCTURAL"],cooldown:"once/tick"},{id:"branding",name:"Branding",desc:"Upload a custom corporate logo. Replaces the default monogram on the dashboard, nation roster, and shipping vessels.",cost:"$1M",costColor:"#ca5",tags:["IDENTITY"],cooldownTicks:12}],CLO:[{id:"sue_corp",name:"Sue Corporation",desc:"File a lawsuit against another corporation you have a business relationship with. The Ministry of Justice in your nation reviews and rules on the case.",cost:"$2M",costColor:"#c55",tags:["LEGAL"]}],Lobbyist:[{id:"donate",name:"Political Donation",desc:"Donate $1M to a political party in the nation where your National HQ is located. The target party receives $100k in party funds. You cannot donate to your own party.",cost:"$1M",costColor:"#ca5",tags:["POLITICAL"],cooldown:"once/tick"}]};function pe(e){return 1}let Je={};function Rt(e){const t=k?.current_tick||0;return Je[e]===t}function ie(e){const t=k?.current_tick||0;Je[e]=t}function Nt(){const e=document.getElementById("actions-right-panel");if(!e)return;const t=be[ae],o=he[t],n=Z(t),r=(Pe[t]||[]).filter(l=>!l.hideForSector||(s?.corp_sector||"")!==l.hideForSector);if(!n){e.innerHTML=`<div style="padding:48px;text-align:center;background:var(--bg-2,#1a1a17);border:1px solid var(--border-0,rgba(255,255,255,0.06));">
        ...
        <div style="display:flex;align-items:center;gap:12px;">
            <div style="width:56px;height:56px;background:${o.color}15;border:1px solid ${o.color}33;display:flex;align-items:center;justify-content:center;font-family:var(--font-mono);font-size:18px;font-weight:700;color:${o.color};">${p(Ie(n.first_name,n.last_name))}</div>
            <div>
                <div style="display:flex;align-items:baseline;gap:8px;">
                    <span style="font-family:var(--font-mono);font-size:22px;font-weight:700;color:${o.color};">${p(t)}</span>
                    <span style="font-size:19px;font-weight:700;color:var(--text-bright,#f0efe6);">${p(n.first_name)} ${p(n.last_name)}</span>
                </div>
        ...
        <div style="display:flex;gap:16px;align-items:center;">
            <div style="text-align:right;">
                ...
                <div style="font-family:var(--font-mono);font-size:16px;font-weight:700;color:var(--text-muted);margin-top:2px;">${K(n.salary_per_year)}/yr</div>
            </div>
            <div style="text-align:right;">
                <div style="font-family:var(--font-mono);font-size:10px;color:var(--text-dim);letter-spacing:0.8px;">CONTRACT</div>
                <div style="font-family:var(--font-mono);font-size:16px;font-weight:700;color:var(--text-muted);margin-top:2px;">${zt(n)}</div>
            </div>
            <div style="text-align:right;">
                <span onclick="event.stopPropagation();confirmRenewExec('${n.id}','${p(t)}','${p(n.first_name+" "+n.last_name)}',${n.salary_per_year||0})" style="font-family:var(--font-mono);font-size:9px;font-weight:700;letter-spacing:0.5px;padding:5px 12px;color:#5cb85c;border:1px solid rgba(92,184,92,0.3);background:rgba(92,184,92,0.07);cursor:pointer;">RENEW</span>
            </div>
            ${t!=="CEO"?`<div style="text-align:right;">
                <span onclick="event.stopPropagation();confirmFireExec('${n.id}','${p(t)}','${p(n.first_name+" "+n.last_name)}',${n.salary_per_year},${n.contract_end_tick||0})" style="font-family:var(--font-mono);font-size:9px;font-weight:700;letter-spacing:0.5px;padding:5px 12px;color:#d9534f;border:1px solid rgba(217,83,79,0.25);background:rgba(217,83,79,0.06);cursor:pointer;">FIRE</span>
            </div>`:""}
        </div>
    </div>`,a+='<div style="background:var(--bg-2,#1a1a17);border:1px solid var(--border-0,rgba(255,255,255,0.06));border-top:1px solid var(--border-0,rgba(255,255,255,0.06));flex:1;">';for(let l=0;l<r.length;l++){const i=r[l],c=!!i.locked;a+=`<div onmouseenter="this.dataset.hover='1';this.style.background='${c?"transparent":o.color+"06"}'" onmouseleave="this.dataset.hover='';this.style.background='transparent';var eb=this.querySelector('.act-exec-btn');if(eb)eb.style.display='none'" style="
            padding:16px 20px;
        ...
        ">`,a+=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
            <div style="display:flex;align-items:center;gap:10px;">
                <span style="font-size:18px;font-weight:700;color:${c?"var(--text-dim)":"var(--text-bright,#f0efe6)"};">${p(i.name)}</span>`;for(const d of i.tags)a+=`<span style="font-family:var(--font-mono);font-size:9px;font-weight:700;letter-spacing:0.3px;padding:2px 6px;line-height:14px;color:${d==="IRREVERSIBLE"?"#c55":d==="OFFENSIVE"?"#c84":d==="STRUCTURAL"?"#ca5":d==="POLITICAL"?"#8a6aaa":"var(--text-dim)"};background:var(--bg-3,#252525);border:1px solid var(--border-0,rgba(255,255,255,0.06));">${p(d)}</span>`;a+=`</div>
            <div style="display:flex;align-items:center;gap:8px;">
                <span style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:${i.costColor};">${p(i.cost)}</span>
            </div>
        </div>`,a+=`<div style="font-size:14px;color:${c?"var(--text-dim)":"var(--text-muted,#666)"};line-height:1.6;">${p(i.desc)}</div>`,i.descRed&&(a+=`<div style="font-size:13px;color:#c55;line-height:1.6;margin-top:4px;">${p(i.descRed)}</div>`),c&&i.lockReason&&(a+=`<div style="margin-top:6px;font-family:var(--font-mono);font-size:11px;color:#c84;display:flex;align-items:center;gap:4px;">
            ...
            </div>`),a+="</div>"}a+="</div>",e.innerHTML=a,e.querySelectorAll("[onmouseenter]").forEach(l=>{l.addEventListener("mouseenter",function(){const i=this.querySelector(".act-exec-btn");i&&(i.style.display="block")}),l.addEventListener("mouseleave",function(){const i=this.querySelector(".act-exec-btn");i&&(i.style.display="none")})})}function Mt(e,t,o,n,r){const a=k?.current_tick||0,l=Math.max(0,r-a),i=Math.round(n*(l/12)),c=`FIRE ${t}: ${o}
...
Are you sure?`;confirm(c)&&Tt(e,t,i)}async function Tt(e,t,o){try{const n=Number(s?.corp_cash_reserves??0);if(n<o){alert(`Insufficient funds. You need $${(o/1e6).toFixed(2)}M but only have $${(n/1e6).toFixed(2)}M.`);return}const r=n-o,{error:a}=await y.from("factions").update({corp_cash_reserves:r}).eq("id",s.id);if(a){alert("Failed to process payout: "+a.message);return}const{error:l}=await y.from("corp_executives").update({status:"fired",updated_at:new Date().toISOString()}).eq("id",e);if(l){await y.from("factions").update({corp_cash_reserves:n}).eq("id",s.id),alert("Failed to fire executive: "+l.message);return}s.corp_cash_reserves=r,ce=ce.filter(i=>i.id!==e),D()}catch(n){console.error("[CorpOps] Fire executive error:",n),alert("An error occurred.")}}function At(e,t){const o=(Pe[t]||[]).find(n=>n.id===e);if(o?.cooldown==="once/tick"&&Rt(e)){alert("This action can only be used once per tick. Wait for the next tick.");return}if(o?.cooldownTicks){const n=Pt(e);if(n>0){alert(`On cooldown — ${n} tick${n===1?"":"s"} remaining.`);return}}switch(e){case"statement":return Lt();case"loan":return Ft();case"equity":return xt(s,k,y);case"paydown":return ao();case"restructure":return Kt();case"rebrand":return Xt();case"branding":return Jt();case"donate":return to();case"sue_corp":return ht(s);case"bankruptcy":return bt()}}function Pt(e){if(e!=="branding")return 0;const o=(Pe.CMO||[]).find(l=>l.id===e)?.cooldownTicks||0,n=Number(s?.last_branding_tick);if(!Number.isFinite(n)||n<=0)return 0;const a=(Number(k?.current_tick)||0)-n;return a>=o?0:o-a}let Re=!1;function Lt(){if(Re)return;Re=!0;const e=document.createElement("div");e.id="stmt-overlay",e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:600;display:flex;align-items:center;justify-content:center;",e.onclick=function(c){c.target===e&&Le()};const t=s?.faction_name||"Corporation",o=(s?.abbreviation||s?.corp_ticker||"??").toUpperCase(),n=Number(s?.corp_cash_reserves??0),r=Z("CEO"),a=r?`${r.first_name} ${r.last_name}`:"CEO";e.innerHTML=`<div onclick="event.stopPropagation()" style="width:480px;background:var(--panel-main);border:1px solid var(--panel-border);display:flex;flex-direction:column;overflow:hidden;">
        <div style="padding:10px 16px;border-bottom:1px solid var(--panel-border);background:var(--bg-panel);">
            ...
            <div style="display:flex;align-items:center;gap:8px;margin-top:4px;">
                ...
                <span style="font-family:var(--font-mono);font-size:8px;color:#6a6660;">&middot; ${p(a)}</span>
            </div>
        </div>
    ...
    </div>`,document.body.appendChild(e);const l=document.getElementById("stmt-text"),i=document.getElementById("stmt-chars");l&&i&&(l.addEventListener("input",function(){i.textContent=this.value.length+"/500"}),l.focus())}function Le(){const e=document.getElementById("stmt-overlay");e&&e.remove(),Re=!1}let se=!1;async function Bt(){if(!s||!k||se)return;const e=document.getElementById("stmt-text"),t=document.getElementById("stmt-error"),o=(e?.value||"").trim();if(!o){t&&(t.textContent="Statement cannot be empty.",t.style.display="block");return}if(o.length>500){t&&(t.textContent="Statement too long (max 500 chars).",t.style.display="block");return}const n=Z("CEO"),r=n?n.skill:50,a=Math.round(2e4*pe()),l=Number(s.corp_cash_reserves??0);if(l<a){t&&(t.textContent="Insufficient cash. Need "+h(a)+".",t.style.display="block");return}se=!0;const i=document.getElementById("stmt-submit-btn");i&&(i.style.opacity="0.4",i.style.pointerEvents="none");const c=s.faction_name||"Corporation",d=n?`${n.first_name} ${n.last_name}`:"CEO",m=k.current_tick||0,{error:f}=await y.from("factions").update({corp_cash_reserves:l-a}).eq("id",s.id);if(f){se=!1,t&&(t.textContent="Failed to deduct cost: "+f.message,t.style.display="block"),i&&(i.style.opacity="1",i.style.pointerEvents="auto");return}const{error:u}=await y.from("event_log").insert({nation_id:s.nation_id,faction_id:s.id,event_name:c+" — Press Release",description_used:d+", CEO of "+c+': "'+o.replace(/[<>"]/g,"")+'"',category:"business",trigger_key:"ceo_statement",effects_applied:{cost:a,ceo:d,skill:r},fired_at_tick:m});if(u){await y.from("factions").update({corp_cash_reserves:l}).eq("id",s.id),se=!1,t&&(t.textContent="Failed to publish: "+u.message,t.style.display="block"),i&&(i.style.opacity="1",i.style.pointerEvents="auto");return}s.corp_cash_reserves=l-a,se=!1,ie("statement"),Le()}const we=5e6,ke=5e8,Ne=5e6;let F=25e7,I=new Set,X=new Set,z=[],G=[];function Ft(){F=25e7,I=new Set,X=new Set,z=[],G=[],document.getElementById("lr-overlay").style.display="flex",J(),Promise.all([Ht(),Gt()]).then(()=>J()).catch(e=>console.error("[lrOpen] load failed:",e))}function Ze(){document.getElementById("lr-overlay").style.display="none"}function Ot(e){const t=Number(e)||0,o=Math.round(t/Ne)*Ne;F=Math.max(we,Math.min(ke,o)),J()}function qt(e){e&&(I.has(e)?I.delete(e):I.add(e),J())}function Dt(){I.size===z.length&&z.length>0?I=new Set:I=new Set(z.map(e=>e.id)),J()}function Ut(){const e=s?.nation_id;if(e){for(const t of z)t.nation_id===e&&I.add(t.id);J()}}function jt(e){e&&(X.has(e)?X.delete(e):X.add(e),J())}async function Ht(){if(!s){z=[];return}const{data:e,error:t}=await y.from("factions").select("id, faction_name, corp_company_type, nation_id, corp_lending_capital, corp_interest_rates, corp_overleverage, nations:nation_id(name)").eq("faction_type","corporation").eq("corp_sector","Finance").is("abandoned_at",null).neq("id",s.id);if(t){console.warn("[lrLoadFinanceCorps] error:",t.message),z=[];return}z=e||[]}async function Gt(){if(!s){G=[];return}const{data:e,error:t}=await y.from("corp_properties").select("id, name, purchase_price, nation_id, role, nations:nation_id(name)").eq("faction_id",s.id).eq("is_active",!0).order("purchase_price",{ascending:!1});if(t){console.warn("[lrLoadCollateral] error:",t.message),G=[];return}G=e||[]}function ze(e,t){const o=Math.max(0,Math.min(10,Math.round(Number(e)||0))),n=[];for(let r=0;r<10;r++){const a=r<o;let l="";a&&(t==="leverage"?l=r<=2?" green":r<=5?" rust":" red":t&&t!=="gold"&&(l=" "+t)),n.push(`<span class="lr-pip${a?" filled"+l:""}"></span>`)}return n.join("")}function Yt(e){const t=Number(e)||0;return t>=7?"var(--red, #d9534f)":t>=4?"var(--orange, #e8724a)":"var(--green, #5cb85c)"}function J(){const e=document.getElementById("lr-modal-content");if(!e)return;const t=s?.faction_name||"Corporation",o=(s?.abbreviation||s?.corp_ticker||"??").toUpperCase(),n=Math.round(F/1e6),r=n/5*.1;let a=0;for(const b of G)X.has(b.id)&&(a+=Number(b.purchase_price||0));const l=F>0?Math.round(a/F*100):0,i=a===0?"unsecured":l<100?"partial":l<150?"full":"over",c=i==="unsecured"?"var(--text-dim, #6a6660)":i==="partial"?"var(--orange, #e8724a)":"var(--green, #5cb85c)",d=I.size,m=d===0;let f="";if(f+=`<div style="padding:18px 24px 16px;border-bottom:1px solid var(--panel-border);background:var(--bg-panel);flex-shrink:0;display:flex;justify-content:space-between;align-items:flex-start;">
        <div>
        ...
        <div style="background:var(--bg-panel);border:1px solid var(--panel-border);padding:18px 20px;">
            <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:14px;">
                <div style="font-family:var(--font-serif,'IBM Plex Serif',serif);font-weight:400;font-size:44px;line-height:1;color:var(--text-bright,#f0efe6);font-variant-numeric:tabular-nums;letter-spacing:-0.025em;">
                    <span style="font-family:var(--font-sans,'IBM Plex Sans',sans-serif);font-size:18px;color:#6a6660;margin-right:2px;vertical-align:top;line-height:44px;">$</span>${n}M
                </div>
                <div style="text-align:right;">
            ...
            <input type="range" min="${we}" max="${ke}" step="${Ne}" value="${F}"
        ...
        <div style="background:var(--bg-panel);border:1px solid var(--panel-border);padding:12px 16px;margin-bottom:10px;display:grid;grid-template-columns:1fr auto 1fr;gap:18px;align-items:center;">
            ...
            <div style="text-align:right;">
        ...
        </div>`,G.length===0)f+='<div style="background:var(--bg-panel);border:1px solid var(--panel-border);padding:18px;text-align:center;font-family:var(--font-mono);font-size:10px;letter-spacing:0.08em;color:#6a6660;font-style:italic;">No corporate properties available to pledge.</div>';else{f+='<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:6px;">';for(const b of G){const C=X.has(b.id),R=Math.round(Number(b.purchase_price||0)/1e6),T=b.name||(b.role||"Property").replace(/_/g," ").toUpperCase(),g=b.nations?.name||"—",L=b.synthetic?'<span style="font-family:var(--font-mono);font-size:7px;letter-spacing:0.12em;color:#8a722f;border:1px solid rgba(138,114,47,0.4);background:rgba(138,114,47,0.06);padding:1px 5px;margin-left:6px;text-transform:uppercase;">Structural</span>':"";f+=`<div onclick="lrToggleCollateral('${p(b.id)}')" style="background:${C?"rgba(200,168,50,0.04)":"var(--bg-panel)"};border:1px solid ${C?"var(--gold,#c8a832)":"var(--panel-border)"};padding:10px 12px;cursor:pointer;display:grid;grid-template-columns:14px 1fr auto;gap:10px;align-items:center;">
                ...
                <div style="min-width:0;">
                    ...
                    <div style="font-family:var(--font-mono);font-size:8px;letter-spacing:0.1em;color:#6a6660;text-transform:uppercase;margin-top:2px;">${p(g)} ◊ ${p((b.role||"").replace(/_/g," "))}${b.synthetic?" · Cannot be seized on default":""}</div>
                </div>
            ...
            </div>`}f+="</div>"}f+="</div>";const _=z.length>0&&d===z.length?"Deselect All":"Select All",x=s?.nation||"",$=!!s?.nation_id&&z.some(b=>b.nation_id===s.nation_id),w=z.length>0,E=b=>`font-family:var(--font-mono);font-size:9px;letter-spacing:0.12em;text-transform:uppercase;color:var(--gold,#c8a832);padding:4px 9px;border:1px solid rgba(200,168,50,0.4);background:rgba(200,168,50,0.06);user-select:none;${b?"cursor:pointer;":"opacity:0.4;pointer-events:none;"}`;if(f+=`<div>
        <div style="display:flex;align-items:baseline;justify-content:space-between;margin-bottom:12px;padding-bottom:8px;border-bottom:1px dashed var(--panel-border);gap:12px;flex-wrap:wrap;">
            ...
            <div style="display:flex;align-items:center;gap:8px;">
                ...
                ${x?`<span ${$?'onclick="lrSelectAllInMyNation()"':""} style="${E($)}">Select All in ${p(x)}</span>`:""}
        ...
        </div>`,z.length===0)f+='<div style="background:var(--bg-panel);border:1px solid var(--panel-border);padding:18px;text-align:center;font-family:var(--font-mono);font-size:10px;letter-spacing:0.08em;color:#6a6660;font-style:italic;">No Finance corporations available to lend.</div>';else{f+='<div style="display:flex;flex-direction:column;gap:5px;">';for(const b of z){const C=I.has(b.id),R=Number(b.corp_lending_capital??0),T=Number(b.corp_interest_rates??0),g=Number(b.corp_overleverage??0),L=b.nations?.name||"—",A=(b.corp_company_type||"PRIVATE").toUpperCase();f+=`<div onclick="lrToggleBank('${p(b.id)}')" style="background:${C?"rgba(200,168,50,0.04)":"var(--bg-panel)"};border:1px solid ${C?"var(--gold,#c8a832)":"var(--panel-border)"};padding:12px 14px;cursor:pointer;display:grid;grid-template-columns:16px 1fr 88px 88px 88px;gap:14px;align-items:center;">
                <div style="width:12px;height:12px;border:1px solid ${C?"var(--gold,#c8a832)":"var(--panel-border)"};background:${C?"var(--gold,#c8a832)":"var(--bg-panel)"};display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;color:#0e0e0c;">${C?"✓":""}</div>
                <div style="min-width:0;">
                    <div style="font-family:var(--font-serif,'IBM Plex Serif',serif);font-weight:500;font-size:14px;color:var(--text-bright,#f0efe6);line-height:1.2;">${p(b.faction_name||"Bank")}</div>
        ...
        <div style="display:flex;gap:24px;">
            <div>
                ...
                <div style="font-family:var(--font-serif,'IBM Plex Serif',serif);font-weight:500;font-size:16px;color:var(--gold,#c8a832);font-variant-numeric:tabular-nums;line-height:1;">$${n}M</div>
            </div>
            <div>
    ...
    </div>`,f+='<div id="lr-error" style="padding:6px 24px;font-family:var(--font-mono);font-size:9px;color:var(--red,#d9534f);display:none;background:var(--bg-panel);border-top:1px solid var(--panel-border);"></div>',e.innerHTML=f}let ve=!1;async function Vt(){if(!s||!k||ve)return;const e=document.getElementById("lr-error"),t=i=>{e&&(e.textContent=i,e.style.display="block")};if(F<we)return t(`Minimum loan amount is $${we/1e6}M.`);if(F>ke)return t(`Maximum loan amount is $${ke/1e6}M.`);if(I.size===0)return t("Select at least one lender.");const o=document.getElementById("lr-submit-btn");ve=!0,o&&(o.style.opacity="0.5",o.style.pointerEvents="none");const n=Array.from(I),r=(G||[]).filter(i=>X.has(i.id)&&!i.synthetic).map(i=>({kind:"property",id:i.id,name:i.name||(i.role||"Property").replace(/_/g," "),value:Math.round(Number(i.purchase_price||0))}));let a,l;try{const i=await y.rpc("submit_loan_request",{p_requesting_faction_id:s.id,p_target_bank_ids:n,p_principal:F,p_term_ticks:60,p_requested_apr:0,p_risk_grade:"B",p_purpose:null,p_expiry_ticks:6,p_collateral:r});a=i.data,l=i.error}catch(i){return ve=!1,o&&(o.style.opacity="1",o.style.pointerEvents="auto"),t("Network error: "+(i?.message||i))}if(ve=!1,o&&(o.style.opacity="1",o.style.pointerEvents="auto"),l)return t("Failed to submit: "+l.message);if(a&&a.success===!1)return t(a.error||"Loan request rejected.");Ze()}function Kt(){if(!s)return;const e=Number(s.corp_loans??0),t=Number(s.corp_reputation??50),o=Number(s.corp_general_workforce??0),n=Number(s.corp_skilled_workforce??0),r=Number(s.corp_innovative_workforce??0),a=o+n+r;if(a===0){alert("Cannot restructure — no employees to lay off.");return}const l=Z("COO");l&&l.skill;const i=pe(),c=10+Math.floor(Math.random()*11),d=Math.round(a*c/100),m=Math.round(e*.07),f=Math.round(m*(2-i)),u=3+Math.floor(Math.random()*10),_=Math.max(1,Math.round(u*i)),x=Math.round(o/a*d),$=Math.round(n/a*d),w=Math.max(0,Math.min(r,d-x-$)),E=document.createElement("div");E.id="restr-overlay",E.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:600;display:flex;align-items:center;justify-content:center;",E.onclick=function(b){b.target===E&&Be()},E.innerHTML=`<div onclick="event.stopPropagation()" style="width:440px;background:var(--panel-main);border:1px solid var(--panel-border);display:flex;flex-direction:column;overflow:hidden;">
        <div style="padding:10px 16px;border-bottom:1px solid var(--panel-border);background:var(--bg-panel);">
            <div style="display:flex;justify-content:space-between;align-items:center;">
                <div style="display:flex;align-items:center;gap:8px;">
        ...
        <div style="padding:12px 16px;">
            ...
            <div style="background:var(--bg-panel);border:1px solid var(--panel-border);padding:8px 12px;">
                ...
                <div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--panel-border);">
                    <span style="font-family:var(--font-mono);font-size:8px;color:#6a6660;">General: ${o} &rarr; ${o-x}</span>
                    <span style="font-family:var(--font-mono);font-size:8px;color:#c84;">-${x}</span>
                </div>
                <div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--panel-border);">
                    <span style="font-family:var(--font-mono);font-size:8px;color:#6a6660;">Skilled: ${n} &rarr; ${n-$}</span>
        ...
        <div style="padding:8px 16px;border-top:1px solid var(--panel-border);display:flex;justify-content:flex-end;gap:6px;">
            ...
            <div id="restr-btn" onclick="actSubmitRestructure(${c},${f},${_},${x},${$},${w})" style="padding:6px 20px;font-family:var(--font-mono);font-size:9px;font-weight:700;letter-spacing:1px;color:#000;background:#8b9a6b;cursor:pointer;">RESTRUCTURE</div>
        </div>
        <div id="restr-error" style="padding:4px 16px;font-family:var(--font-mono);font-size:8px;color:#c55;display:none;"></div>
    </div>`,document.body.appendChild(E)}function Be(){const e=document.getElementById("restr-overlay");e&&e.remove()}let ye=!1;async function Wt(e,t,o,n,r,a){if(!s||!k||ye)return;ye=!0;const l=document.getElementById("restr-btn");l&&(l.style.opacity="0.4",l.style.pointerEvents="none");const i=Number(s.corp_general_workforce??0),c=Number(s.corp_skilled_workforce??0),d=Number(s.corp_innovative_workforce??0),m=Number(s.corp_loans??0),f=Number(s.corp_reputation??50),u={corp_general_workforce:Math.max(0,i-n),corp_skilled_workforce:Math.max(0,c-r),corp_innovative_workforce:Math.max(0,d-a),corp_loans:Math.max(0,m-t),corp_reputation:Math.max(0,f-o)},{error:_}=await y.from("factions").update(u).eq("id",s.id);if(_){ye=!1;const w=document.getElementById("restr-error");w&&(w.textContent="Failed: "+_.message,w.style.display="block"),l&&(l.style.opacity="1",l.style.pointerEvents="auto");return}Object.assign(s,u);const x=k.current_tick||0,{error:$}=await y.from("event_log").insert({nation_id:s.nation_id,faction_id:s.id,event_name:(s.faction_name||"Corporation")+" — Restructuring",description_used:(s.faction_name||"A corporation")+" has announced a restructuring, laying off "+e+"% of its workforce.",category:"business",trigger_key:"corp_restructure",effects_applied:{layoff_pct:e,debt_cut:t,rep_loss:o},fired_at_tick:x});$&&console.warn("Failed to log restructure event:",$.message),ye=!1,ie("restructure"),Be(),D()}function Xt(){const e=Z("CMO");e&&e.skill;const t=pe(),o=Math.round(2e6*t),n=Math.max(1,Math.round(5*t)),r=Number(s?.corp_cash_reserves??0),a=Number(s?.corp_reputation??50),l=s?.faction_name||"",i=s?.abbreviation||s?.corp_ticker||"",c=document.createElement("div");c.id="rebrand-overlay",c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:600;display:flex;align-items:center;justify-content:center;",c.onclick=function(d){d.target===c&&Fe()},c.innerHTML=`<div onclick="event.stopPropagation()" style="width:440px;background:var(--panel-main);border:1px solid var(--panel-border);display:flex;flex-direction:column;overflow:hidden;">
        <div style="padding:10px 16px;border-bottom:1px solid var(--panel-border);background:var(--bg-panel);">
            <div style="display:flex;justify-content:space-between;align-items:center;">
                <div style="display:flex;align-items:center;gap:8px;">
        ...
        <div style="padding:8px 16px;border-top:1px solid var(--panel-border);display:flex;justify-content:flex-end;gap:6px;">
            ...
            <div id="rebrand-btn" onclick="actSubmitRebrand(${o},${n})" style="padding:6px 20px;font-family:var(--font-mono);font-size:9px;font-weight:700;letter-spacing:1px;color:#000;background:#c84;cursor:${r>=o?"pointer":"not-allowed"};${r<o?"opacity:0.4;pointer-events:none;":""}">REBRAND</div>
        </div>
        <div id="rebrand-error" style="padding:4px 16px;font-family:var(--font-mono);font-size:8px;color:#c55;display:none;"></div>
    </div>`,document.body.appendChild(c)}function Fe(){const e=document.getElementById("rebrand-overlay");e&&e.remove()}let ue=!1;async function Qt(e,t){if(!s||!k||ue)return;const o=e||2e6,n=t||5,r=document.getElementById("rebrand-error"),a=(document.getElementById("rebrand-name")?.value||"").trim().replace(/[<>"]/g,""),l=(document.getElementById("rebrand-abbr")?.value||"").trim().toUpperCase().replace(/[<>"]/g,"");if(!a||a.length<2){r&&(r.textContent="Name must be at least 2 characters.",r.style.display="block");return}if(!l||l.length<2||l.length>5){r&&(r.textContent="Abbreviation must be 2-5 characters.",r.style.display="block");return}const i=Number(s.corp_cash_reserves??0);if(i<o){r&&(r.textContent="Insufficient cash. Need "+h(o)+".",r.style.display="block");return}ue=!0;const c=document.getElementById("rebrand-btn");c&&(c.style.opacity="0.4",c.style.pointerEvents="none");const d=Number(s.corp_reputation??50),m=s.faction_name||"Corporation",{error:f}=await y.from("factions").update({faction_name:a,abbreviation:l,corp_ticker:l,corp_cash_reserves:i-o,corp_reputation:Math.max(0,d-n)}).eq("id",s.id);if(f){ue=!1,r&&(r.textContent="Failed: "+f.message,r.style.display="block"),c&&(c.style.opacity="1",c.style.pointerEvents="auto");return}s.faction_name=a,s.abbreviation=l,s.corp_ticker=l,s.corp_cash_reserves=i-o,s.corp_reputation=Math.max(0,d-n);const u=k.current_tick||0,{error:_}=await y.from("event_log").insert({nation_id:s.nation_id,faction_id:s.id,event_name:"Corporation Rebranded",description_used:m+" has rebranded to "+a+" ("+l+"). The rebrand costs $20M and reputation takes a temporary hit.",category:"corporate",trigger_key:"corp_rebrand",effects_applied:{old_name:m,new_name:a,new_abbr:l,rep_loss:n,cost:o},fired_at_tick:u});_&&console.warn("Failed to log rebrand event:",_.message),ue=!1,ie("rebrand"),Fe(),D(),document.getElementById("corp-name-bar").textContent=a;const x=document.getElementById("corp-logo");x&&(x.textContent=l.slice(0,2))}const P=1e6,xe=12;let O=null,te=!1;function Jt(){O=null;const e=Number(s?.corp_cash_reserves??0),t=s?.custom_logo_url||"",o=document.createElement("div");o.id="branding-overlay",o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:600;display:flex;align-items:center;justify-content:center;",o.onclick=function(r){r.target===o&&Oe()};const n=t?`<img src="${p(t)}" alt="" style="width:100%;height:100%;object-fit:cover;">`:`<span style="font-family:var(--font-mono);font-size:18px;font-weight:700;color:#6a6660;">${p((s?.abbreviation||s?.corp_ticker||"??").slice(0,2))}</span>`;o.innerHTML=`<div onclick="event.stopPropagation()" style="width:460px;background:var(--panel-main);border:1px solid var(--panel-border);display:flex;flex-direction:column;overflow:hidden;">
        <div style="padding:10px 16px;border-bottom:1px solid var(--panel-border);background:var(--bg-panel);">
            <div style="display:flex;justify-content:space-between;align-items:center;">
                <div style="display:flex;align-items:center;gap:8px;">
                    <span style="font-size:8px;color:#c84;">&#9679;</span>
    ...
    </div>`,document.body.appendChild(o)}function Oe(){const e=document.getElementById("branding-overlay");e&&e.remove(),O=null}function Zt(e){const t=e.target.files?.[0],o=document.getElementById("branding-error"),n=document.getElementById("branding-filename");if(!t){O=null,n&&(n.textContent="");return}if(t.size>128*1024){O=null,n&&(n.textContent=""),o&&(o.textContent="File too large — must be under 128 KB.",o.style.display="block");return}O=t,o&&(o.style.display="none"),n&&(n.textContent=t.name);const r=document.getElementById("branding-preview");if(r){const a=new FileReader;a.onload=l=>{r.innerHTML=`<img src="${l.target.result}" alt="" style="width:100%;height:100%;object-fit:cover;">`},a.readAsDataURL(t)}}async function eo(){if(!s||!k||te)return;const e=document.getElementById("branding-error"),t=document.getElementById("branding-btn");if(!O){e&&(e.textContent="Choose a logo file first.",e.style.display="block");return}const o=Number(k.current_tick)||0,{data:n,error:r}=await y.from("factions").select("corp_cash_reserves, last_branding_tick").eq("id",s.id).single();if(r||!n){e&&(e.textContent="Failed to verify cooldown: "+(r?.message||"unknown"),e.style.display="block");return}const a=n.last_branding_tick==null?null:Number(n.last_branding_tick);if(a!=null&&o-a<xe){const x=xe-(o-a);e&&(e.textContent=`On cooldown — ${x} tick${x===1?"":"s"} remaining.`,e.style.display="block");return}const l=Number(n.corp_cash_reserves??0);if(l<P){e&&(e.textContent="Insufficient cash. Need "+h(P)+".",e.style.display="block");return}te=!0,t&&(t.style.opacity="0.4",t.style.pointerEvents="none",t.textContent="UPLOADING...");let i;try{const x=(O.name.split(".").pop()||"png").toLowerCase().replace(/[^a-z0-9]/g,"")||"png",$=`party-logos/${s.id}/${Date.now()}.${x}`,{error:w}=await y.storage.from("public-assets").upload($,O,{contentType:O.type,upsert:!0});if(w)throw w;const{data:E}=y.storage.from("public-assets").getPublicUrl($);if(i=E?.publicUrl||null,!i)throw new Error("Could not resolve public URL.")}catch(x){te=!1,e&&(e.textContent="Upload failed: "+(x.message||"Unknown error"),e.style.display="block"),t&&(t.style.opacity="1",t.style.pointerEvents="auto",t.textContent="UPLOAD & CONFIRM");return}const c={custom_logo_url:i,corp_cash_reserves:l-P,last_branding_tick:o};let d=y.from("factions").update(c).eq("id",s.id);d=a==null?d.is("last_branding_tick",null):d.eq("last_branding_tick",a);const{data:m,error:f}=await d.select("id");if(f){te=!1,e&&(e.textContent="Failed: "+f.message,e.style.display="block"),t&&(t.style.opacity="1",t.style.pointerEvents="auto",t.textContent="UPLOAD & CONFIRM");return}if(!m||m.length===0){te=!1,e&&(e.textContent="Branding is on cooldown. Refresh to see the latest state.",e.style.display="block"),t&&(t.style.opacity="1",t.style.pointerEvents="auto",t.textContent="UPLOAD & CONFIRM");return}s.custom_logo_url=i,s.corp_cash_reserves=l-P,s.last_branding_tick=o;const{error:u}=await y.from("event_log").insert({nation_id:s.nation_id,faction_id:s.id,event_name:"Corporation Rebranded (Logo)",description_used:`${s.faction_name||"Corporation"} unveiled a new corporate logo.`,category:"corporate",trigger_key:"corp_branding",effects_applied:{logo_url:i,cost:P,cooldown_ticks:xe},fired_at_tick:o});u&&console.warn("Failed to log branding event:",u.message),te=!1,Oe(),D();const _=document.getElementById("corp-logo");_&&(_.innerHTML=`<img src="${i}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:5px;">`)}window.actBrandingPickFile=Zt;window.actSubmitBranding=eo;window.actCloseBranding=Oe;let W=[],Y=-1;async function to(){Number(s?.corp_cash_reserves??0);const e=[s.nation_id],t=new Set(j.map(r=>r.id)),{data:o}=await y.from("factions").select("id, faction_name, abbreviation, party_color, party_funds, seats, momentum, nation, nation_id, leader_ideology, linked_user_id").eq("faction_type","party").in("nation_id",e).is("abandoned_at",null).order("seats",{ascending:!1});W=(o||[]).filter(r=>!t.has(r.id)).map(r=>({...r})),Y=-1;const n=document.createElement("div");n.id="donate-overlay",n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:600;display:flex;align-items:center;justify-content:center;",n.onclick=function(r){r.target===n&&qe()},document.body.appendChild(n),et()}function qe(){const e=document.getElementById("donate-overlay");e&&e.remove(),W=[],Y=-1}function oo(e){Y=e,et()}function et(){const e=document.getElementById("donate-overlay");if(!e)return;const t=Z("Lobbyist");t&&t.skill;const o=Math.round(1e6*pe()),n=1e5,r=Number(s?.corp_cash_reserves??0),a=Y>=0?W[Y]:null,l=r>=o;let i='<div onclick="event.stopPropagation()" style="width:540px;max-height:80vh;background:var(--panel-main);border:1px solid var(--panel-border);display:flex;flex-direction:column;overflow:hidden;">';i+=`<div style="padding:14px 20px;border-bottom:1px solid var(--panel-border);background:var(--bg-panel);flex-shrink:0;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
            <div style="display:flex;align-items:center;gap:10px;">
        ...
        <div style="display:flex;align-items:center;gap:8px;margin-top:6px;">
            ...
            <span style="font-family:var(--font-mono);font-size:13px;font-weight:700;color:#5cb85c;">+${h(n)}</span>
        </div>
        ...
        <div style="display:flex;gap:16px;">
            ...
            ${a?`<div><div style="font-family:var(--font-mono);font-size:9px;color:#6a6660;letter-spacing:0.8px;">RECIPIENT</div><div style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:var(--panel-text);">${p(a.abbreviation||a.faction_name)}</div></div>`:""}
        </div>
        <div style="display:flex;gap:8px;">
            ...
            <div id="donate-btn" onclick="actSubmitDonation()" style="padding:8px 24px;font-family:var(--font-mono);font-size:11px;font-weight:700;letter-spacing:1px;color:${a&&l?"#000":"#6a6660"};background:${a&&l?"#8a6aaa":"var(--panel-border)"};cursor:${a&&l?"pointer":"not-allowed"};${!a||!l?"opacity:0.4;pointer-events:none;":""}">DONATE</div>
        </div>
    </div>`,i+='<div id="donate-error" style="padding:6px 20px;font-family:var(--font-mono);font-size:10px;color:#c55;display:none;"></div>',i+="</div>",e.innerHTML=i}let oe=!1;async function no(){if(!s||!k||Y<0||oe)return;const e=W[Y];if(!e)return;const t=Number(k?.current_tick||0);if(new Set(j.map(g=>g.id)).has(e.id)){const g=document.getElementById("donate-error");g&&(g.textContent="You cannot donate to your own party.",g.style.display="block");return}const n=Z("Lobbyist"),r=n?n.skill:50,a=Math.round(1e6*pe()),l=1e5,i=2,{data:c,error:d}=await y.from("factions").select("corp_cash_reserves, last_donation_tick").eq("id",s.id).single();if(d||!c){const g=document.getElementById("donate-error");g&&(g.textContent="Failed to verify cooldown: "+(d?.message||"unknown"),g.style.display="block");return}const m=Number(c.last_donation_tick??0);if(m===t){const g=document.getElementById("donate-error");g&&(g.textContent="Political Donation is on cooldown until next tick.",g.style.display="block"),ie("donate");return}const f=Number(c.corp_cash_reserves??0);if(f<a){const g=document.getElementById("donate-error");g&&(g.textContent="Insufficient cash. Need "+h(a)+", have "+h(f)+".",g.style.display="block");return}oe=!0;const u=document.getElementById("donate-btn");u&&(u.style.opacity="0.4",u.style.pointerEvents="none");const _=Number(s.corp_reputation??50),x=Math.max(0,_-i),{data:$,error:w}=await y.from("factions").update({corp_cash_reserves:f-a,corp_reputation:x,last_donation_tick:t}).eq("id",s.id).eq("last_donation_tick",m).select("id");if(w){const g=document.getElementById("donate-error");oe=!1,g&&(g.textContent="Failed: "+w.message,g.style.display="block"),u&&(u.style.opacity="1",u.style.pointerEvents="auto");return}if(!$||$.length===0){const g=document.getElementById("donate-error");oe=!1,g&&(g.textContent="Political Donation is on cooldown until next tick.",g.style.display="block"),u&&(u.style.opacity="1",u.style.pointerEvents="auto"),ie("donate");return}const{data:E}=await y.from("factions").select("party_funds").eq("id",e.id).single(),b=Number(E?.party_funds??0),{error:C}=await y.from("factions").update({party_funds:b+l}).eq("id",e.id);if(C){await y.from("factions").update({corp_cash_reserves:f}).eq("id",s.id);const g=document.getElementById("donate-error");oe=!1,g&&(g.textContent="Failed to transfer funds: "+C.message,g.style.display="block"),u&&(u.style.opacity="1",u.style.pointerEvents="auto");return}s.corp_cash_reserves=f-a,s.corp_reputation=x;const R=s.faction_name||"Corporation",{error:T}=await y.from("event_log").insert({nation_id:e.nation_id||s.nation_id,faction_id:s.id,event_name:R+" — Political Donation",description_chosen:R+" has donated "+h(a)+" to "+(e.faction_name||"a political party")+". The party receives "+h(l)+" in campaign funds. Corporate reputation decreases by "+i+".",category:"business",trigger_key:"corp_donation",effects_applied:{cost:a,recipient_faction_id:e.id,recipient_name:e.faction_name,funds_granted:l,reputation_loss:i,skill:r},fired_at_tick:t});T&&console.warn("Failed to log donation event:",T.message),oe=!1,ie("donate"),qe()}const re=1e5;let M=[],V=null,S=0,ne=!1,le=!1;async function ao(){M=[],V=null,S=0,ne=!1,le=!0;const e=document.createElement("div");if(e.id="paydown-overlay",e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:600;display:flex;align-items:center;justify-content:center;",e.onclick=function(n){n.target===e&&De()},document.body.appendChild(e),Q(),!s?.id){le=!1,Q();return}const{data:t,error:o}=await y.from("bank_loans").select("id, principal, outstanding, apr, issued_at_tick, matures_at_tick, status, lender:lender_faction_id(faction_name, abbreviation)").eq("borrower_faction_id",s.id).in("status",["active","called"]).gt("outstanding",0).order("issued_at_tick",{ascending:!0});o?(console.warn("[PayDown] failed to load loans:",o.message),M=[]):M=t||[],le=!1,Q()}function De(){const e=document.getElementById("paydown-overlay");e&&e.remove(),M=[],V=null,S=0,ne=!1,le=!1}function io(e){V=e;const t=M.find(r=>r.id===e),o=Number(s?.corp_cash_reserves??0),n=Math.max(0,o-re);t?S=Math.min(Number(t.outstanding)||0,n):S=0,Q()}function ro(e){const t=M.find(l=>l.id===V);if(!t){S=0,Q();return}const o=Number(s?.corp_cash_reserves??0),n=Math.max(0,o-re),r=Math.min(Number(t.outstanding)||0,n);let a=Number(String(e).replace(/[^0-9.]/g,""))||0;a=Math.max(0,Math.min(r,Math.floor(a))),S=a,Q()}function so(){const e=M.find(n=>n.id===V);if(!e)return;const t=Number(s?.corp_cash_reserves??0),o=Math.max(0,t-re);S=Math.min(Number(e.outstanding)||0,o),Q()}function Q(){const e=document.getElementById("paydown-overlay");if(!e)return;const t=Number(s?.corp_cash_reserves??0),o=M.find(i=>i.id===V)||null,n=o?S+re:0,r=o&&n>0&&t>=n&&S>0,a=o&&S>0&&S>=Number(o.outstanding||0);let l='<div onclick="event.stopPropagation()" style="width:620px;max-height:84vh;background:var(--panel-main);border:1px solid var(--panel-border);display:flex;flex-direction:column;overflow:hidden;">';if(l+=`<div style="padding:14px 20px;border-bottom:1px solid var(--panel-border);background:var(--bg-panel);flex-shrink:0;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
            <div style="display:flex;align-items:center;gap:10px;">
        ...
        <div style="font-family:var(--font-mono);font-size:10px;color:#6a6660;margin-top:6px;">Retire outstanding loan principal early. Pays down a single loan; the lender is credited the same amount. <span style="color:#ca5;">$100k admin fee</span> is charged on top of the payment. Paying a loan off in full grants <span style="color:#5cb85c;">+1 Reputation</span>.</div>
            ...
            <div style="display:flex;gap:16px;font-family:var(--font-mono);font-size:10px;color:#6a6660;">
                ...
                <span>= Total: <span style="color:${r?"var(--panel-text)":"#c55"};font-weight:700;">${h(n)}</span></span>
                ${a?'<span style="color:#5cb85c;font-weight:700;">PAYS OFF IN FULL · +1 REP</span>':""}
            </div>
        ...
        <div style="display:flex;gap:16px;">
            <div><div style="font-family:var(--font-mono);font-size:9px;color:#6a6660;letter-spacing:0.8px;">CASH</div><div style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:${t>=n?"var(--panel-text)":"#c55"};">${h(t)}</div></div>
            ${o?`<div><div style="font-family:var(--font-mono);font-size:9px;color:#6a6660;letter-spacing:0.8px;">CHARGE</div><div style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:#ca5;">${h(n)}</div></div>`:""}
        </div>
        <div style="display:flex;gap:8px;">
    ...
    <div id="paydown-error" style="padding:6px 20px;font-family:var(--font-mono);font-size:10px;color:#c55;display:none;"></div>`,l+="</div>",e.innerHTML=l}async function lo(){if(ne)return;const e=M.find(d=>d.id===V);if(!e||!s||S<=0)return;const t=()=>document.getElementById("paydown-error"),o=d=>{const m=t();m&&(m.textContent=d,m.style.display="block")};ne=!0;const n=document.getElementById("paydown-btn");n&&(n.style.opacity="0.4",n.style.pointerEvents="none");const{data:r,error:a}=await y.rpc("pay_down_debt",{p_borrower_id:s.id,p_loan_id:e.id,p_amount:S});if(a){ne=!1,n&&(n.style.opacity="1",n.style.pointerEvents="auto"),o("RPC failed: "+a.message);return}if(!r?.success){ne=!1,n&&(n.style.opacity="1",n.style.pointerEvents="auto"),o(r?.error||"Payment failed.");return}const{data:l,error:i}=await y.from("factions").select("corp_cash_reserves, corp_reputation, corp_debt").eq("id",s.id).single();i?console.warn("[PayDown] post-RPC faction refresh failed:",i.message):l&&(s.corp_cash_reserves=l.corp_cash_reserves,s.corp_reputation=l.corp_reputation,s.corp_debt=l.corp_debt),De(),D();const c=document.getElementById("corp-topbar-container");c&&Ve(c,{faction:s,shard:k,activeTab:"actions",allUserFactions:j})}function co(e){ae=e,D()}async function po(e){if(H=e,q=-1,document.getElementById("exec-search-overlay").style.display="flex",de.length===0&&s?.nation_id){const t=s.nation||"",o=Ke(s.nation_id,t),{error:n}=await y.from("executive_pool").insert(o);n&&console.warn("Failed to generate executive pool:",n.message);const{data:r,error:a}=await y.from("executive_pool").select("*").eq("nation_id",s.nation_id).eq("status","available").order("skill",{ascending:!1});a&&console.warn("Failed to reload executive pool:",a.message),de=r||[]}Ue()}function tt(){document.getElementById("exec-search-overlay").style.display="none",H=null,q=-1}let Se=!1;async function fo(){if(Se||!s?.nation_id)return;const e=document.getElementById("exec-search-refresh");Se=!0,e&&(e.style.opacity="0.5",e.style.cursor="wait",e.textContent="Refreshing…");try{const t=s.nation||"",o=Ke(s.nation_id,t),{error:n}=await y.from("executive_pool").insert(o);if(n){console.warn("Failed to refresh executive pool:",n.message);return}const{data:r,error:a}=await y.from("executive_pool").select("*").eq("nation_id",s.nation_id).eq("status","available").order("skill",{ascending:!1});if(a){console.warn("Failed to reload executive pool after refresh:",a.message);return}de=r||[],q=-1,Ue()}finally{Se=!1}}function ot(e){return de.filter(t=>t.status==="available"&&Array.isArray(t.specializations)&&t.specializations.includes(e)).sort((t,o)=>o.skill-t.skill)}function mo(e){q=e,Ue()}let ge=!1;async function vo(){if(!s||!k||!H||q<0||ge)return;const t=ot(H)[q];if(!t)return;ge=!0;const o=k.current_tick||0,n=document.getElementById("es-hire-btn");n&&(n.style.opacity="0.4",n.style.pointerEvents="none");const{error:r}=await y.from("corp_executives").insert({faction_id:s.id,role:H,first_name:t.first_name,last_name:t.last_name,age:t.age,origin_nation:t.origin_nation,skill:t.skill,salary_per_year:t.required_salary,contract_years:t.required_years,contract_start_tick:o,contract_end_tick:o+t.required_years*12,status:"active"});if(r){ge=!1;const l=document.getElementById("es-error");l&&(l.textContent="Failed: "+r.message,l.style.display="block"),n&&(n.style.opacity="1",n.style.pointerEvents="auto");return}const{error:a}=await y.from("executive_pool").update({status:"hired",hired_by_faction_id:s.id}).eq("id",t.id);a&&console.warn("Failed to mark pool candidate as hired:",a.message),ge=!1,tt(),await Ae(),ae=be.indexOf(H),ae<0&&(ae=0),D()}function Ue(){const e=document.getElementById("exec-search-content");if(!e||!H)return;const t=H,o=he[t],n=ot(t),r=q>=0?n[q]:null;let a="";a+=`<div style="padding:12px 20px;border-bottom:1px solid var(--panel-border);background:var(--bg-panel);flex-shrink:0;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
            <div style="display:flex;align-items:center;gap:10px;">
        ...
        </div>`);for(let l=0;l<n.length;l++){const i=n[l],c=q===l;a+=`<div onclick="esSelectCandidate(${l})" style="
            padding:10px 14px;
            border-bottom:1px solid var(--panel-border);
        ...
        ">
        ...
        </div>`}if(a+="</div>",a+='<div style="flex:1;overflow-y:auto;">',!r)a+=`<div style="padding:50px 24px;text-align:center;">
            <div style="font-family:var(--font-mono);font-size:14px;color:var(--text-dim);margin-bottom:10px;">Select a candidate</div>
            <div style="font-size:12px;color:#6a6660;">${n.length} candidate${n.length!==1?"s":""} available for ${p(t)}</div>
        </div>`;else{const l=r.required_salary*r.required_years;a+=`<div style="padding:20px;border-bottom:1px solid var(--panel-border);">
            <div style="display:flex;align-items:center;gap:14px;">
            ...
            <div style="display:flex;gap:5px;flex-wrap:wrap;">`;for(const i of r.specializations||[]){const c=he[i],d=i===t;a+=`<span style="font-family:var(--font-mono);font-size:10px;font-weight:700;padding:3px 10px;color:${d?"#000":c?.color||"#9e9a92"};background:${d?c?.color||"#5a8aaa":(c?.color||"#5a8aaa")+"10"};border:1px solid ${d?"transparent":(c?.color||"#5a8aaa")+"30"};">${p(i)}</span>`}a+="</div></div>",a+=`<div style="padding:12px 20px;border-bottom:1px solid var(--panel-border);">
        ...
        <div style="display:flex;gap:14px;">`,r?a+=`<div><div style="font-family:var(--font-mono);font-size:9px;color:#6a6660;letter-spacing:0.8px;">CANDIDATE</div><div style="font-family:var(--font-mono);font-size:15px;font-weight:700;color:var(--text-bright,#f0efe6);">${p(r.first_name)} ${p(r.last_name)}</div></div>
            <div><div style="font-family:var(--font-mono);font-size:9px;color:#6a6660;letter-spacing:0.8px;">SALARY</div><div style="font-family:var(--font-mono);font-size:15px;font-weight:700;color:#c84;">${K(r.required_salary)}/yr</div></div>`:a+='<div style="font-family:var(--font-mono);font-size:11px;color:#6a6660;">Select a candidate to hire</div>',a+=`</div>
        <div style="display:flex;gap:8px;">
    ...
    </div>`,a+='<div id="es-error" style="padding:5px 20px;font-family:var(--font-mono);font-size:10px;color:#c55;display:none;"></div>',e.innerHTML=a}async function yo(){const{data:{user:e}}=await y.auth.getUser();if(!e){window.location.href="login.html";return}const t=new URLSearchParams(location.search).get("faction_id");if(t){const{data:a,error:l}=await y.from("factions").select("*").eq("id",t).single();l?console.warn("[Inspector] faction fetch failed:",l.message):a?.faction_type==="corporation"&&(s=a)}if(!s){const{data:a}=await y.from("factions").select("*").or(`id.eq.${e.id},linked_user_id.eq.${e.id}`);j=(a||[]).filter(i=>i.nation_id);const l=sessionStorage.getItem("active_faction_id");if(s=j.find(i=>i.id===l)||j.find(i=>i.faction_type==="corporation")||j[0],!s){await y.auth.signOut(),window.location.href="login.html";return}if(s.faction_type!=="corporation"){window.location.href="dashboard.html";return}}const[o,n]=await Promise.all([s.nation_id?y.from("nations").select("*").eq("id",s.nation_id).single():Promise.resolve({data:null}),y.from("shard").select("current_tick, current_date, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single()]);o.data&&o.data,k=n.data;const r=document.getElementById("corp-topbar-container");r&&Ve(r,{faction:s,shard:k,activeTab:"actions",allUserFactions:j}),document.getElementById("loading").style.display="none",document.getElementById("page-content").style.display="block",await Promise.all([Et(),Ae()]),D()}window.actExecute=At;window.actSelectExec=co;window.confirmFireExec=Mt;window.actCloseStatement=Le;window.actSubmitStatement=Bt;window.actCloseRestructure=Be;window.actSubmitRestructure=Wt;window.actCloseRebrand=Fe;window.actSubmitRebrand=Qt;window.actCloseDonation=qe;window.actSubmitDonation=no;window.donateSelectParty=oo;window.actClosePayDown=De;window.actSubmitPayDown=lo;window.paydownSelectLoan=io;window.paydownSetAmount=ro;window.paydownMaxAmount=so;window.lrClose=Ze;window.lrSetAmount=Ot;window.lrToggleBank=qt;window.lrToggleCollateral=jt;window.lrToggleSelectAll=Dt;window.lrSelectAllInMyNation=Ut;window.lrSubmit=Vt;window.openExecSearch=po;window.closeExecSearch=tt;window.refreshExecPool=fo;window.esSelectCandidate=mo;window.esHireCandidate=vo;yo();
