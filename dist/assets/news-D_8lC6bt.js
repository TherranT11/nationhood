import{_ as le}from"./supabase-client-BXEzLDpS.js";/* empty css                  */import{_ as re,i as oe}from"./common-DC6iDOio.js";import{t as ce}from"./utils-C2W-HleY.js";let b=null,h=null,M=[],C="all",T=null,N=null,O=null,A=null,P=!1,_="cruceran",U=!1;const W={cruceran:{key:"cruceran",name:"The Cruceran",tagline:"Truth in the service of the people",nations:["Avelia","Palvera","San Estrella","Montequilla","Melizea","Sangreza"],style:"cruceran"},continental:{key:"continental",name:"The Continental",tagline:"Where Ideas Converge",nations:["Calveth","Flandis"],style:"continental"}};function de(e){for(const[s,n]of Object.entries(W))if(n.nations.some(a=>a.toLowerCase()===(e||"").toLowerCase()))return s;return"cruceran"}function Z(e,s){const n=W[e];return n?n.nations.some(a=>a.toLowerCase()===(s||"").toLowerCase()):!1}const pe=["Winter","Spring","Spring","Spring","Summer","Summer","Summer","Fall","Fall","Fall","Winter","Winter"];function S(e){const s=e%12,n=2e3+Math.floor(e/12),a=pe[s],l=s===0?n-1:n;return`${a} ${l}`}async function q(e,s){b=e,h=s,T=null,C="all",O=null;const n=document.getElementById("newspaper-root");if(!n)return;U||(_=de(s.nation?.name));const a=s.shard?.current_date||"[Month], [Year]",t=Z(_,s.nation?.name)?"Write Article":"Write Article (1 AP)",v=Object.entries(W).map(([d,u])=>`<option value="${d}" ${d===_?"selected":""}>${u.name}</option>`).join("");n.innerHTML=`<div class="newspaper-container nws-pub-${_}">
        ...
        <div class="nws-main-content">
            <div class="nws-reader">
                <span class="nws-section-tag">${r(E(s.category))} &mdash; ${r(a)}</span>
        ...
        <div class="nws-lead-main" data-article-id="${e.id}">
            ${L(e)}${x(e)}
            <span class="nws-section-tag">${r(E(e.category))}</span>
    ...
    `).join(""),l=5-e.length;let i="";for(let t=0;t<l;t++)i+=`
        ...
        </div>`,o+='<div class="ct-story-grid">';for(const c of s){const g=u[c.category]||"",f=c.image_url?`<img src="${t(c.image_url)}" alt="" style="width:100%;height:100%;object-fit:cover;">`:"";o+=`<div class="ct-card" data-article-id="${c.id}">
    ...
    </div>`,i.innerHTML=o,i.querySelectorAll("[data-article-id]").forEach(c=>{c.style.cursor="pointer",c.addEventListener("click",g=>{if(g.target.closest(".nws-edit-btn, .nws-delete-btn"))return;const f=c.dataset.articleId,y=M.find($=>String($.id)===String(f));y&&X(document.getElementById("newspaper-root"),y)})})}function $e(e){const s=e.querySelectorAll(".nws-nav-item[data-category]");s.forEach(n=>{n.addEventListener("click",()=>{s.forEach(a=>a.classList.remove("active")),n.classList.add("active"),C=n.dataset.category,D()})})}function Ee(e){const s=document.getElementById("nws-nav-archives");s&&s.addEventListener("click",()=>ae(e))}async function ae(e){if(!(!b||!h))try{const s=h.shard?.current_tick??0,n=h.shard?.current_date||"[Month], [Year]",a=await te(),{data:l,error:i}=await b.from("player_articles").select("*").in("nation_id",a).eq("status","published").order("published_tick",{ascending:!1});if(i){console.error("[News] Failed to load archive articles:",i);return}const t={};for(const d of l||[]){const u=d.published_tick??0,o=S(u);t[o]||(t[o]={label:o,maxTick:u,articles:[]}),t[o].articles.push(d),u>t[o].maxTick&&(t[o].maxTick=u)}const v=Object.values(t).sort((d,u)=>u.maxTick-d.maxTick),p=S(s),m=v.length>0?v.map(d=>{const u=d.label===p?' <span class="nws-archive-current">Current</span>':"";return`<div class="nws-archive-month" data-archive-season="${r(d.label)}">
                    <div class="nws-archive-month-name">${r(d.label)}${u}</div>
        ...
        </div>`,document.getElementById("nws-back-btn")?.addEventListener("click",()=>q(b,h)),e.querySelectorAll("[data-archive-season]").forEach(d=>{d.addEventListener("click",()=>{const u=d.dataset.archiveSeason,o=t[u];o&&ie(e,o.articles,o.label)})}),e.scrollTop=0}catch(s){console.error("[News] Error loading archives:",s)}}function ie(e,s,n){M=s;const a=s.filter(f=>f.category==="opinion"),i=[...s.filter(f=>f.category!=="opinion")].sort((f,y)=>(y.body||"").length-(f.body||"").length),t=i[0],v=i.slice(1,4),p=i.slice(4,7),m=i.slice(7,12),d=a.slice(0,4),u=t?ke(t,v,n):'<p class="nws-placeholder" style="padding:40px;text-align:center;">[No articles in this edition]</p>',o=Ie(p),c=Ce(d),g=Be(m);e.innerHTML=`<div class="newspaper-container">
        <!-- TOP RIBBON -->
        <div class="nws-top-ribbon">
            <div class="nws-top-ribbon-inner">
                <span>${r(n)}</span>
        ...
        <div class="nws-footer">
            ...
            <div class="nws-sidebar-story" data-article-id="${p.id}">
                ...
                <h3 class="nws-sidebar-headline">${r(p.headline)}</h3>
                <p class="nws-sidebar-deck">${r((p.body||"").replace(/\n+/g," ").substring(0,120))}${(p.body||"").length>120?"...":""}</p>
                <div class="nws-byline"><span class="nws-author">${r(p.author_name)}</span><span class="nws-dot">&middot;</span><span>${r(n)}</span></div>
            </div>
    ...
    `}function Ie(e){return e.length===0?"":e.map(s=>{const n=s.image_url?`<img src="${r(s.image_url)}" alt="${r(s.headline)}" style="width:100%;height:100%;object-fit:cover;">`:`<div class="nws-img-ph" style="background:${ne(s.category)};">${r(E(s.category))}</div>`;return`<div class="nws-sec-story" data-article-id="${s.id}">
            <div class="nws-sec-image">${n}</div>
            <span class="nws-section-tag">${r(E(s.category))}</span>
    ...
    `).join("")}function j(e){return e.w*3+e.d}function Te(e){return Object.values(e).sort((s,n)=>{const a=j(n)-j(s);return a!==0?a:n.w-s.w})}function Ae(e){return`<span class="vln-form-dot" style="background:${{W:"#1a4a1a",D:"#8a6a20",L:"#8b1a1a"}[e]||"#999"}"></span>`}function Se(e,s){return e<=3?"vln-pos-top":e>=s-2?"vln-pos-bottom":""}function J(e,s={}){const n=Te(e),a=s.muted||!1,l=n.length;let i=`<div class="vln-table${a?" vln-table-muted":""}">
        ...
        </div>`;return n.forEach((t,v)=>{const p=v+1,m=Se(p,l),d=p===3||p===7?" vln-zone-break":"",u=j(t),o=a?"":t.form.map(c=>Ae(c)).join("");i+=`<div class="vln-row${d}">
