import{_supabase as f}from"./supabase-client-qEAQbBjE.js";/* empty css                    *//* empty css                         */import{escapeHtml as v,tickToDate as kt}from"./utils-A98FEun4.js";import{initMessaging as oe}from"./messaging-Btjj7Mcp.js";import{a as ie}from"./corp-valuation-DvpMrhZz.js";import"./government-structure-C17uG6rl.js";const re=[{key:"trucks",name:"Work Trucks",tier:1,basePrice:28e4,maintenancePerUnit:1500,conditionDecayRate:2,startingOwned:4},{key:"excavators",name:"Excavators",tier:1,basePrice:15e5,maintenancePerUnit:5500,conditionDecayRate:2,startingOwned:2},{key:"bulldozers",name:"Bulldozers",tier:1,basePrice:12e5,maintenancePerUnit:7e3,conditionDecayRate:2,startingOwned:1},{key:"mixers",name:"Concrete Mixers",tier:1,basePrice:8e5,maintenancePerUnit:4500,conditionDecayRate:2,startingOwned:2},{key:"cranes",name:"Tower Cranes",tier:2,basePrice:72e5,maintenancePerUnit:32500,conditionDecayRate:2,startingOwned:0},{key:"haulers",name:"Heavy Haulers",tier:2,basePrice:35e5,maintenancePerUnit:15e3,conditionDecayRate:2,startingOwned:0},{key:"piledrivers",name:"Pile Drivers",tier:2,basePrice:48e5,maintenancePerUnit:18e3,conditionDecayRate:2,startingOwned:0},{key:"asphalt",name:"Asphalt Plants",tier:2,basePrice:55e5,maintenancePerUnit:22e3,conditionDecayRate:2,startingOwned:0},{key:"industrial",name:"Industrial Cranes",tier:3,basePrice:18e6,maintenancePerUnit:85e3,conditionDecayRate:2,startingOwned:0},{key:"tbm",name:"Tunnel Boring Machines",tier:3,basePrice:45e6,maintenancePerUnit:2e5,conditionDecayRate:2,startingOwned:0},{key:"dredge",name:"Dredging Equipment",tier:3,basePrice:22e6,maintenancePerUnit:95e3,conditionDecayRate:2,startingOwned:0}];function se(t){return re.find(e=>e.key===t)}function le(t){let e=0;for(const n of t||[]){const a=se(n.equipment_key);a&&n.owned>0&&(e+=a.maintenancePerUnit*n.owned)}return e}function At(t,e){var n=Number(t?.purchase_price)||0,a=Number(t?.refurbish_count)||0,i=n*.1*(Number(e)||1);return Math.round(i*Math.pow(1.25,a))}async function ce(t,e,n,a,i){if(!e||!n?.id)return{ok:!1,error:"Missing faction or property."};if(Number(n.condition)>=95)return{ok:!1,error:"Property already at excellent condition."};var s=At(n,i),{data:p,error:l}=await t.from("factions").select("corp_cash_reserves").eq("id",e).single();if(l)return{ok:!1,error:"Failed to read cash: "+l.message};var o=Number(p?.corp_cash_reserves??0);if(o<s)return{ok:!1,error:"Insufficient cash.",cost:s};var r=4+Math.floor(Math.random()*6),_=95+Math.floor(Math.random()*4),u=Number(a||0)+r,x=Math.max(0,o-s),{error:w}=await t.from("factions").update({corp_cash_reserves:x}).eq("id",e);if(w)return{ok:!1,error:"Cash deduct failed: "+w.message};var{data:T,error:L}=await t.from("corp_properties").update({refurbish_until_tick:u,refurbish_condition:_}).eq("id",n.id).is("refurbish_until_tick",null).select("id");return L||!T||T.length===0?(await t.from("factions").update({corp_cash_reserves:o}).eq("id",e),{ok:!1,error:L?.message||"Refurbishment already in progress on this property."}):{ok:!0,cost:s,duration:r,targetCondition:_,newCash:x}}function Ot(t,e){const n=Math.max(0,Number(t)||0),a=Math.max(0,Number(e)||0);return Math.round(n*(a/100)/12)}function de(t){if(!t)return 0;const e=t.original_principal??t.principal??0;return Math.max(0,Number(e)||0)}let ht=[],d=null,Q=null;function m(t){return Math.abs(t)>=1e6?"$"+(t/1e6).toFixed(2)+"M":Math.abs(t)>=1e3?"$"+(t/1e3).toFixed(1)+"k":"$"+Math.round(t).toLocaleString()}function vt(t){const e=Number(t)||0,n=Math.abs(e);return n>=1e9?{main:"$"+(e/1e9).toFixed(2),unit:"B"}:n>=1e6?{main:"$"+(e/1e6).toFixed(2),unit:"M"}:n>=1e3?{main:"$"+(e/1e3).toFixed(1),unit:"k"}:{main:"$"+Math.round(e).toLocaleString(),unit:""}}function pe(t){const e=Math.max(0,Math.min(100,Number(t)||0));return e>=90?{tag:"Aaa",tone:"good"}:e>=80?{tag:"Aa1",tone:"good"}:e>=70?{tag:"A1",tone:"good"}:e>=60?{tag:"Baa1",tone:"gold"}:e>=50?{tag:"Baa3",tone:"gold"}:e>=40?{tag:"Ba1",tone:"gold"}:e>=30?{tag:"Ba3",tone:"red"}:e>=20?{tag:"B2",tone:"red"}:e>=10?{tag:"Caa1",tone:"red"}:{tag:"Ca",tone:"red"}}function mt(t,e){return Number(t?.[e]??50)}async function fe(){const{data:{user:t}}=await f.auth.getUser();if(!t){window.location.href="login.html";return}const{data:e}=await f.from("factions").select("*").or(`id.eq.${t.id},linked_user_id.eq.${t.id}`);ht=(e||[]).filter(b=>b.nation_id&&!b.abandoned_at);const n=sessionStorage.getItem("active_faction_id");if(d=ht.find(b=>b.id===n)||ht.find(b=>b.faction_type==="corporation")||ht[0],!d){sessionStorage.removeItem("active_faction_id"),window.location.href="faction-select.html";return}if(sessionStorage.setItem("active_faction_id",d.id),d.faction_type!=="corporation"){window.location.href="dashboard.html";return}let a=d.nation||"",i=null;const[s,p]=await Promise.all([d.nation_id?f.from("nations").select("*").eq("id",d.nation_id).single():Promise.resolve({data:null}),f.from("shard").select("current_tick, current_date, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single()]);s.error&&console.warn("Nation load failed:",s.error.message),s.data&&(a=s.data.name,i=s.data),p.error&&console.warn("Shard load failed:",p.error.message),Q=p.data;let l=0;if(d?.id){const{data:b}=await f.from("construction_contracts").select("id, contract_bids!inner(id)").eq("issuer_faction_id",d.id).in("status",["open","bidding"]);if(b)for(const g of b)l+=(g.contract_bids||[]).length}(function(){const g=(d.corp_ticker||d.abbreviation||"").toUpperCase(),c=Q?.current_date||"",K=c?c.replace(/.*,\s*/,""):"",B=Q?.current_tick,Y=document.getElementById("h2-plate-logo");Y&&(d.custom_logo_url?Y.innerHTML=`<img src="${v(d.custom_logo_url)}" alt="logo">`:Y.textContent=g.slice(0,3)||"—");const k=document.getElementById("h2-eyebrow-loc");k&&(k.textContent=a?`The Boardroom · ${a}`:"The Boardroom");const X=document.getElementById("h2-tick-date");if(X){const C=[];B!=null&&C.push("Tick "+B),c&&C.push(c),X.textContent=C.length?C.join(" · "):"Tick — · —"}const J=document.getElementById("h2-next-close");J&&(J.textContent="Next close —");const G=document.getElementById("h2-ceo");if(G){const C=[d.leader_first_name,d.leader_last_name].filter(Boolean);if(C.length){const P=d.leader_age?" ("+d.leader_age+")":"",V=d.leader_role||"Chairman & Chief Executive";G.textContent=`${C.join(" ")} · ${V}${P}`}else G.textContent="—"}const at=document.getElementById("h2-brand");if(at){const C=d.faction_name||"Unnamed Corporation",P=C.split(" ");if(P.length>1){const V=P.slice(0,-1).join(" "),Z=P[P.length-1];at.innerHTML=`${v(V)} <em>${v(Z)}</em>`}else at.textContent=C}const lt=document.getElementById("h2-brand-sub");if(lt){const C=[];d.corp_company_type&&C.push(d.corp_company_type),K&&C.push("Est. "+K);const P=d.corp_subsector||d.corp_sector;P&&C.push(P),lt.textContent=C.length?C.join(" · "):"—"}const ct=document.getElementById("h2-tail-code");if(ct){const C=(a||"").split(" ").map(Z=>Z[0]||"").join("").toUpperCase().slice(0,4),P=d.party_description?'"'+d.party_description+'"':"",V=[];g&&V.push(g),C&&V.push(C+" EXCH"),P&&V.push(P),ct.textContent=V.length?V.join(" · "):"—"}const M=document.getElementById("h2-wire-corp");if(M){const C=(d.faction_name||"your corp").split(" ").slice(0,2).join(" ");M.textContent=C}const h=document.getElementById("h2-edit-toggle");h&&h.addEventListener("click",()=>{const C=document.body.classList.toggle("h2-edit-open");h.classList.toggle("on",C),h.textContent=C?"Close ✕":"Logo"})})(),(function(){const g=document.getElementById("h2-logout-btn");if(g&&g.addEventListener("click",async()=>{try{sessionStorage.clear(),await f.auth.signOut()}catch{}window.location.href="login.html"}),l>0){const c=document.getElementById("h2-nav-actions-badge");c&&(c.textContent=l,c.style.display="",c.classList.add("ok"))}})(),document.getElementById("id-type-badge").textContent=d.corp_company_type||"—";const o=document.getElementById("id-logo"),r=(d.corp_ticker||d.abbreviation||"").toUpperCase();d.custom_logo_url?o.innerHTML=`<img src="${v(d.custom_logo_url)}" alt="logo" style="width:100%;height:100%;object-fit:contain;border-radius:4px;">`:o.textContent=r.slice(0,3)||"—",document.getElementById("id-corp-name").textContent=d.faction_name||"Unnamed Corp";const _=d.party_description||"";document.getElementById("id-slogan").textContent=_?'"'+_+'"':'"--"';const u=Q?.current_date?Q.current_date.replace(/.*,\s*/,""):"—",x=d.leader_first_name&&d.leader_last_name?d.leader_first_name+" "+d.leader_last_name+(d.leader_age?" ("+d.leader_age+")":""):"—";document.getElementById("id-rows").innerHTML=`
    ...
    `;const w=d.last_rename_tick||0,T=Q?.current_tick||0,N=Math.max(0,w+120-T),A=!_||_==="-"||_==='"-"'||N<=0,F=document.getElementById("slogan-editor");F.innerHTML=`
    ...
    `,document.getElementById("corp-logo-file")?.addEventListener("change",Ce),window._corpFactionId=d.id,window._currentTick=T,window._nationStats=i,window._factionData=d;const $=_e(i,a,d);ge(a,d);const I=await St(i,a,d,Q);let D=0;if(d?.id){const{data:b,error:g}=await f.from("corp_equipment").select("equipment_key, owned").eq("faction_id",d.id);g||(D=le(b||[]))}let E=0;if(d?.id){const{data:b}=await f.from("corp_executives").select("salary_per_year").eq("faction_id",d.id).eq("status","active");E=(b||[]).reduce((g,c)=>g+(Number(c.salary_per_year)||0),0)}let R=0,U=0;if(d?.id&&d.corp_sector==="Shipping"){const{data:b}=await f.from("corp_vessels").select("base_maintenance, purchase_price, condition, built_at_tick, status").eq("faction_id",d.id).neq("status","for_sale");R=(b||[]).reduce((g,c)=>g+(Number(c.base_maintenance)||0),0),U=ie(b,T)}await me(i,Q,$,d,I.propertyMaintenance||0,D,E,I,R),await ye(i,a,d,$,I,U),oe(d,i,Q),st={nationId:d.nation_id},Qt(),document.getElementById("loading").style.display="none",document.getElementById("page-content").style.display="block"}function _e(t,e,n){const a=g=>mt(t,g),i=(e||"UNKNOWN").toUpperCase(),s=Number(n?.corp_general_workforce??2250),p=Number(n?.corp_skilled_workforce??600),l=Number(n?.corp_innovative_workforce??150),o=s+p+l,r=2,_=3,u=6,x=a("minimum_wage"),w=x/100*48e3,T=a("inflation"),L=a("standard_of_living"),N=1+(T-50)/100*.5,W=1+(L-50)/100*.5,A=g=>Math.round(w*g*N*W),F=A(r),O=A(_),$=A(u),I=s*F,D=p*O,E=l*$,R=I+D+E;function U(g){return"$"+Math.round(g).toLocaleString()+"/yr"}const b=`${N.toFixed(2)} &times; ${W.toFixed(2)}`;return document.getElementById("wf-total-header").textContent=o.toLocaleString(),document.getElementById("wf-body").innerHTML=`
        <div class="wf-tier">
            ...
            <div class="wf-tier__detail">
                ...
                <span class="wf-tier__value" style="color:var(--text-primary)">${m(I)}</span>
            </div>
        </div>
        ...
        <div class="wf-total">
            ...
            <div class="wf-total__row">
                ...
                <span class="wf-total__value" style="color:var(--red);">${m(R)}</span>
            </div>
            <div class="wf-total__row">
                <span class="wf-total__label">Per Tick (÷12)</span>
                <span class="wf-total__value" style="color:var(--red);">${m(R/12)}</span>
            </div>
        </div>
    `,{totalWages:R,generalTotal:I,skilledTotal:D,innovativeTotal:E,monthlyWages:Math.round(R/12)}}async function me(t,e,n,a,i,s,p,l,o){const r=e?.current_tick||0;document.getElementById("fin-tick").textContent="TICK "+r;const _=87e6,u=y=>mt(t,y),x=1+(u("gdp_growth")-50)/100*.4,w=1+(u("urbanization")-50)/100*.3,T=1+(u("population_growth")-50)/100*.2,L=1+(u("standard_of_living")-50)/100*.15,N=1+(50-u("physical_infrastructure"))/100*.1,W=1-Math.max(0,u("inflation")-50)/100*.1,A=1-Math.max(0,u("interest_rates")-50)/100*.1,F=x*w*T*L*N*W*A,O=Math.round(_*F),$=(a.corp_general_workforce||0)+(a.corp_skilled_workforce||0)+(a.corp_innovative_workforce||0),I=Math.max(500,l?.totalCapacity||500),D=Math.min(1,$/I),E=l?.propertyRevBonus||0,R=Math.round(Math.round(O/12)*D)+E;let U=0,b=0,g=0;if(a?.id){const y=a.corp_sector||"";if(y==="Finance"){const{data:S,error:q}=await f.from("finance_active_loans").select("monthly_payment, interest_rate, principal, original_principal, finance_loan_requests(request_type)").eq("lender_faction_id",a.id).in("status",["current","late","delinquent"]);q&&console.warn("[Finances] finance_active_loans query failed:",q.message);for(const z of S||[]){const H=z.finance_loan_requests?.request_type||"loan";H==="insurance"?b+=Number(z.monthly_payment||0):H==="loan"?b+=Ot(de(z),z.interest_rate):H==="bond"&&(b+=Number(z.monthly_payment||0))}}else if(y==="Construction"){const{data:S}=await f.from("construction_contracts").select("id, timeline_ticks").eq("awarded_to_faction",a.id).eq("status","in_progress"),q=(S||[]).map(H=>H.id).filter(Boolean);if(q.length>0){const{data:H}=await f.from("contract_bids").select("contract_id, estimated_cost").in("contract_id",q).eq("status","won"),ft={};for(const it of H||[])ft[it.contract_id]=Number(it.estimated_cost||0);for(const it of S||[]){const j=ft[it.id]||0;g+=Math.round(j/Math.max(1,it.timeline_ticks||1))}}const{data:z}=await f.from("construction_deliveries").select("payment_received").eq("faction_id",a.id).eq("delivered_at_tick",r);for(const H of z||[])b+=Number(H.payment_received||0)}else if(y==="Shipping"){const{data:S}=await f.from("shipping_claims").select("revenue_per_transit").eq("faction_id",a.id).eq("status","active");for(const q of S||[])b+=Number(q.revenue_per_transit||0)}}let c=[],K=0;try{const{data:y}=await f.from("corp_properties").select("id, nation_id, nations!nation_id(name)").eq("faction_id",a.id).eq("type","fuel_depot").eq("is_active",!0);if(y&&y.length>0){const S=y.map(q=>q.nation_id).filter(Boolean);if(S.length>0){const{data:q}=await f.from("shipping_claims").select("faction_id, shipping_routes!inner(destination_nation_id, status)").eq("status","active").in("shipping_routes.destination_nation_id",S),z=[...new Set((q||[]).map(j=>j.faction_id).filter(j=>j&&j!==a.id))],H=new Set;if(z.length>0){const{data:j}=await f.from("corp_properties").select("faction_id, nation_id").in("faction_id",z).in("nation_id",S).eq("type","fuel_depot").eq("is_active",!0);for(const et of j||[])H.add(et.faction_id+"|"+et.nation_id)}const ft={};for(const j of q||[]){const et=j.shipping_routes?.destination_nation_id;et&&j.faction_id!==a.id&&(H.has(j.faction_id+"|"+et)||(ft[et]=(ft[et]||0)+1))}const it=7500;for(const j of y){const et=ft[j.nation_id]||0,Pt=et*it;c.push({nation:j.nations?.name||"Unknown",revenue:Pt,visitors:et}),K+=Pt}c.sort((j,et)=>et.revenue-j.revenue)}}}catch(y){console.warn("Fuel depot revenue estimate failed (non-fatal):",y?.message||y)}const B=U+b+R+K,Y=n?.totalWages||0,k=Math.round(Y/12),X=0,J=i||0,G=s||0,at=Number(a?.corp_loans)||0,lt=.05,ct=at>0?Math.round(at*(lt/12)/(1-Math.pow(1+lt/12,-120))):0;let M=0,h=0;if(a?.id)try{const{data:y}=await f.from("finance_active_loans").select("monthly_payment, finance_loan_requests(request_type)").eq("borrower_faction_id",a.id).in("status",["current","late","delinquent"]);for(const S of y||[]){const q=S.finance_loan_requests?.request_type||"loan",z=Number(S.monthly_payment||0);if(!(z<=0))if(q==="insurance")h+=z;else{if(q==="bond")continue;M+=z}}}catch(y){console.warn("[Finances] borrower finance_active_loans lookup failed:",y)}const C=Math.round((p||0)/12),P=o||0,V=75e3,Z=Math.max(0,Math.min(1,Number(t?.corporate_tax??0)/100||0)),ot=k+C+X+J+G+P+ct+M+h+g+V,_t=Math.max(0,B-ot),dt=Math.round(_t*Z);let ut="";try{const y=new Set([a.nation_id]),{data:S}=await f.from("corp_properties").select("nation_id").eq("faction_id",a.id).eq("is_active",!0);if((S||[]).forEach(q=>{q.nation_id&&y.add(q.nation_id)}),y.size>0){const{data:q}=await f.from("nations").select("id, name, corporate_tax").in("id",[...y]);q&&q.length>0&&(ut=q.sort((z,H)=>(z.name||"").localeCompare(H.name||"")).map(z=>{const H=Math.round(Number(z.corporate_tax??0)),ft=Math.round(_t*(H/100)/q.length),it=H>25?"#c55":H>15?"#ca5":"#5c5";return`<div style="display:flex;justify-content:space-between;padding:1px 0;font-family:var(--font-mono);font-size:8px;">
                        <span style="color:var(--text-dim);">${z.name} (<span style="color:${it}">${H}%</span>)</span>
                        <span style="color:#a44;">${m(ft)}</span>
                    </div>`}).join(""))}}catch{}const Et=ot+dt,nt=Number(a?.monthly_profit||0),pt=Number(a?.corp_cash_reserves??0),Jt=at;let gt=null,tt=null,Mt=null,Tt="Does not include all capital/financing cash transfers.";if(a?.id)try{const{data:y}=await f.from("corp_cash_history").select("tick, cash_start, cash_end, cash_delta, non_pnl_cash_movements").eq("faction_id",a.id).lte("tick",r).order("tick",{ascending:!1}).limit(2),S=(y||[]).find(z=>Number(z.tick)===Number(r))||(y||[])[0]||null,q=(y||[]).find(z=>Number(z.tick)<Number(S?.tick??r))||null;S?(gt=S.cash_start!=null?Number(S.cash_start):q?.cash_end!=null?Number(q.cash_end):null,tt=S.cash_delta!=null?Number(S.cash_delta):gt!=null?pt-gt:null,Mt=S.non_pnl_cash_movements!=null?Number(S.non_pnl_cash_movements):tt!=null?tt-nt:null):Tt="Does not include all capital/financing cash transfers. Cash history snapshot not yet available."}catch(y){console.warn("[Finances] corp_cash_history lookup failed:",y),Tt="Does not include all capital/financing cash transfers. Cash history snapshot unavailable."}be(B,nt);const Zt=[{stat:"gdp_growth",value:u("gdp_growth"),weight:"0.4"},{stat:"urbanization",value:u("urbanization"),weight:"0.3"},{stat:"population_growth",value:u("population_growth"),weight:"0.2"},{stat:"standard_of_living",value:u("standard_of_living"),weight:"0.15"},{stat:"physical_infra",value:u("physical_infrastructure"),weight:"0.1",note:"INV"},{stat:"inflation",value:u("inflation"),weight:"-0.1",neg:!0},{stat:"interest_rates",value:u("interest_rates"),weight:"-0.1",neg:!0}];function te(y){return y.neg?y.value>50?"var(--red)":"var(--green)":y.note?y.value<50?"var(--green)":"var(--red)":y.value>=50?"var(--green)":y.value>=35?"var(--amber)":"var(--red)"}const qt=B||1,ee=(U/qt*100).toFixed(1),ne=((b+K)/qt*100).toFixed(1),ae=(R/qt*100).toFixed(1);document.getElementById("fin-body").innerHTML=`
        <!-- Revenue -->
        <div class="fin-section">
            ...
            <div class="fin-row"><span class="fin-row__label">Market Revenue<span class="fin-row__badge">DERIVED</span></span><span class="fin-row__value" style="color:var(--green)">${m(R-E)}</span></div>
            ...
            <div class="fin-total">
                ...
                <span class="fin-total__value" style="color:var(--green)">${m(B)}</span>
            </div>
        </div>
        <!-- Expenses -->
        <div class="fin-section">
            ...
            ${P>0?`<div class="fin-row"><span class="fin-row__label">Fleet Maintenance</span><span class="fin-row__value" style="color:#a44">${m(P)}</span></div>`:""}
        ...
        <div class="fin-net" style="background:${tt==null?"transparent":tt>=0?"rgba(92,204,92,0.02)":"rgba(204,85,85,0.02)"};">
            ...
            <span class="fin-net__value" style="color:${tt==null?"var(--text-dim)":tt>=0?"var(--green)":"var(--red)"};">${tt==null?"—":m(tt)}</span>
        </div>
        ...
        <div class="fin-section">
            ...
            <div class="fin-row"><span class="fin-row__label">Last Tick Net Profit</span><span class="fin-row__value" style="color:${nt>=0?"var(--green)":"var(--red)"}">${m(nt)}</span></div>
            ...
            <div class="fin-total">
                ...
                <span class="fin-total__value" style="color:${(tt||0)>=0?"var(--green)":"var(--red)"}">${tt==null?"—":m(tt)}</span>
            </div>
    ...
    `,Rt()}let It=!1;async function ve(t,e){if(!(!d||It)){It=!0;try{const{data:n,error:a}=await f.from("finance_loan_offers").select("*").eq("id",t).single();if(a||!n)return;const{data:i,error:s}=await f.from("finance_loan_requests").select("*").eq("id",e).single();if(s||!i||i.status!=="open")return;const p=i.term_months,l=Ot(i.amount,n.interest_rate),o=Math.round(i.amount/p),r=l+o,_=Q?.current_tick||0,{error:u}=await f.from("finance_loan_requests").update({status:"funded",accepted_offer_id:t,funded_tick:_}).eq("id",e);if(u)return;await f.from("finance_loan_offers").update({status:"accepted"}).eq("id",t),await f.from("finance_loan_offers").update({status:"declined"}).eq("request_id",e).neq("id",t).eq("status","pending"),await f.from("finance_active_loans").insert({request_id:e,offer_id:t,borrower_faction_id:i.requesting_faction_id,lender_faction_id:n.offering_faction_id,nation_id:i.nation_id,principal:i.amount,interest_rate:n.interest_rate,term_months:i.term_months,collateral_type:n.collateral_type,purpose:i.purpose,monthly_payment:r,started_tick:_});const{data:x}=await f.from("factions").select("corp_cash_reserves").eq("id",n.offering_faction_id).single();x&&await f.from("factions").update({corp_cash_reserves:Math.max(0,(Number(x.corp_cash_reserves)||0)-i.amount)}).eq("id",n.offering_faction_id);const{data:w}=await f.from("factions").select("corp_cash_reserves, corp_debt").eq("id",i.requesting_faction_id).single();if(w){const{error:T}=await f.from("factions").update({corp_cash_reserves:(Number(w.corp_cash_reserves)||0)+i.amount,corp_debt:(Number(w.corp_debt)||0)+i.amount}).eq("id",i.requesting_faction_id);T&&console.error("[Loans] Failed to credit borrower + track debt:",T.message)}}finally{It=!1}Rt()}}async function ue(t){await f.from("finance_loan_requests").update({status:"cancelled"}).eq("id",t),Rt()}async function Rt(){if(!d)return;const t=document.getElementById("fin-loans-list");if(t)try{const{data:e,error:n}=await f.from("finance_loan_requests").select("*, finance_loan_offers!request_id(*)").eq("requesting_faction_id",d.id).in("status",["open","funded"]).neq("request_type","equity").order("created_tick",{ascending:!1});n&&console.error("[Loans] Request query error:",n.message);const{data:a,error:i}=await f.from("finance_active_loans").select("*, finance_loan_requests!inner(request_type, insured_contract_id), lender:factions!lender_faction_id(faction_name, abbreviation)").eq("borrower_faction_id",d.id).in("status",["current","late","delinquent"]).is("equity_pct",null).order("started_tick",{ascending:!1});i&&console.error("[Loans] Active loans query error:",i.message);const s=[];for(const r of a||[]){const _=r.finance_loan_requests?.insured_contract_id;_&&s.push(_)}const p=[...new Set(s)];let l={};if(p.length>0){const{data:r,error:_}=await f.from("construction_contracts").select("id, status").in("id",p);_?console.error("[Loans] Contract status query error:",_.message):l=Object.fromEntries((r||[]).map(u=>[u.id,u.status]))}let o="";if(e&&e.length>0){for(const r of e)if(r.status==="open"){const _=(r.finance_loan_offers||[]).filter(u=>u.status==="pending");if(o+=`<div style="padding:6px 0;border-bottom:1px solid var(--border-hair);">
                ...
                </div>`;continue}const w=(r.status==="late"||r.status==="delinquent")&&Number(r.payments_missed||0)===0,T=w?"#d9a441":r.status==="current"?"var(--green)":r.status==="late"?"#c84":"#c55",L=r.term_months>0?Math.round(r.payments_made/r.term_months*100):0,N=r.lender?.faction_name||"Unknown bank",W=kt(r.started_tick);o+=`<div style="padding:6px 0;border-bottom:1px solid var(--border-hair);">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <div>
    ...
    </div>`,O=w;for(let g=0;g<s.length;g++){const c=s[g],K=p[c.style]||p.Basic;l+=Number(c.purchase_price||0),o+=Number(c.monthly_maintenance||0);const B=c.condition>=75?"var(--green)":c.condition>=50?"var(--amber)":"var(--orange)",Y=Number(c.capacity||0),k=u[g]||0,X=x>0?Math.min(r-O,Math.round(r*(k/x))):0;O+=X,F+=`
        <div class="prop-asset" style="margin-bottom:6px;">
            <div class="prop-asset__top">
                <div>
            ...
            ${c.refurbish_until_tick&&c.refurbish_until_tick>(a?.current_tick||0)?`<div style="margin-top:4px;padding:3px 6px;background:var(--amber-faint, rgba(176,154,91,0.08));border:1px solid var(--amber-border, rgba(176,154,91,0.2));font-size:8px;font-family:var(--font-mono);color:var(--amber, #b09a5b);text-align:center;">REFURBISHING — ${c.refurbish_until_tick-(a?.current_tick||0)} tick${c.refurbish_until_tick-(a?.current_tick||0)!==1?"s":""} remaining</div>`:`<div style="display:flex;gap:4px;margin-top:4px;">
                ...
                <div class="cp-section__header">
                    ...
                    <div style="display:flex;gap:0;background:var(--bg-3);border:1px solid var(--border-hair);margin:2px 0;">
                        ...
                        </div>`,k.status==="in_progress"&&k.awarded_at_tick!=null){const P=at-k.awarded_at_tick,V=k.timeline_ticks||1,Z=k.stalled_ticks||0,ot=Math.min(100,Math.round(P/(V+Z)*100)),_t=ot>=75?"#5c5":ot>=40?"#ca5":"#5aaa8b",dt=Math.max(0,V+Z-P);$+=`<div style="display:flex;justify-content:space-between;font-size:8px;font-family:var(--font-mono);margin-top:6px;">
                    ...
                    </div>`}$+="</div>"}$+="</div>"}}const D=document.getElementById("prop-count"),E=s.length+1,R=I.length,U=E+" ASSET"+(E!==1?"S":"")+(R>0?" · "+R+" PROJECT"+(R!==1?"S":""):"");D&&(D.textContent=U),document.getElementById("prop-body").innerHTML=`
        ${F}
        ${$}
    ...
    `;let b=0;b+=Math.round(_*50);for(const g of s){if(g.refurbish_until_tick&&(a?.current_tick||0)<g.refurbish_until_tick)continue;const c=Number(g.condition||0)/100;c>=.6&&(b+=Math.round(Number(g.capacity||0)*c*50))}return{propertyValue:l,propertyMaintenance:o,totalCapacity:x,propertyRevBonus:b}}async function ye(t,e,n,a,i,s=0){(e||"UNKNOWN").toUpperCase();const p=n.corp_company_type||"Private",l=Number(n.corp_cash_reserves)||0,o=i?.propertyValue||0;let r=0;if(n?.id&&n.corp_sector==="Finance")try{const{data:I}=await f.from("finance_active_loans").select("remaining_principal, finance_loan_requests(request_type)").eq("lender_faction_id",n.id).in("status",["current","late","delinquent"]);for(const D of I||[]){const E=D.finance_loan_requests?.request_type||"loan";if(E==="loan"||E==="bond"){const R=Math.max(0,Number(D.remaining_principal||0));r+=R}}}catch(I){console.warn("[Valuation] finance_active_loans lookup failed:",I)}const _=l+o+s+r,u=Number(n.corp_loans)||0,w=a?.monthlyWages||0,T=0,L=u+w+T,N=_-L,A=Math.round(N*(1+.3)),F=A-N,O=F>0;document.getElementById("val-type-badge").textContent=p.toUpperCase();function $(I,D,E={}){const R=E.indent?"val-line val-line--indent":"val-line",U=E.bold?"val-line__label val-line__label--bold":"val-line__label",b=E.bold?"val-line__value val-line__value--bold":"val-line__value",g=E.color||(E.bold?"var(--text-bright)":"var(--text-muted)");return`<div class="${R}"><span class="${U}">${I}</span><span class="${b}" style="color:${g}">${m(D)}</span></div>`}document.getElementById("val-body").innerHTML=`
        ...
        ${$("Total Liabilities",L,{bold:!0,color:"var(--red)"})}

        <div class="val-net">
            <span class="val-net__label">Net Worth</span>
            <span class="val-net__value" style="color:${N>=0?"var(--green)":"var(--red)"};">${m(N)}</span>
        </div>
    ...
    `,r.style.display="flex"}async function Me(t,e){if(xt)return;xt=!0;const n=document.getElementById("prop-sell-confirm");n&&(n.disabled=!0,n.textContent="Selling...");try{const a=window._corpFactionId;if(!a)throw new Error("No faction");const{error:i}=await f.from("corp_properties").update({is_active:!1}).eq("id",t).eq("faction_id",a);if(i)throw new Error("Failed to sell property: "+i.message);const{data:s}=await f.from("factions").select("corp_cash_reserves").eq("id",a).single(),p=Number(s?.corp_cash_reserves??0),{error:l}=await f.from("factions").update({corp_cash_reserves:p+e}).eq("id",a);l&&console.error("[Property] Failed to credit cash:",l.message),Ct(),alert("Property sold for "+m(e)+". Cash credited."),location.reload()}catch(a){alert("Sale failed: "+a.message)}finally{xt=!1,n&&(n.disabled=!1,n.textContent="Confirm Sale")}}let wt=!1;function Te(t,e,n,a,i){if(wt)return;const s=window._nationStats,p=window._factionData,o=1+(mt(s,"inflation")-50)/100*.3,r=At({purchase_price:a,refurbish_count:i},o),u=Number(p?.corp_cash_reserves??0)>=r,x=document.getElementById("prop-modal-overlay"),w=document.getElementById("prop-modal-content"),T=u&&n<95;let L="";n>=95?L='<div style="color:var(--green);font-size:9px;margin-bottom:8px;">Property already at excellent condition ('+n+"%).</div>":u||(L='<div style="color:var(--red);font-size:9px;margin-bottom:8px;">Insufficient cash reserves.</div>'),w.innerHTML=`
        ...
        <div style="display:flex;gap:6px;">
            <button class="prop-modal-btn prop-modal-btn--cancel" onclick="closePropModal()">Cancel</button>
            <button class="prop-modal-btn prop-modal-btn--confirm" id="prop-refurb-confirm" onclick="confirmRefurbish('${t}', ${a}, ${i}, ${n})" ${T?"":"disabled"}>Begin Refurbishment</button>
        </div>
