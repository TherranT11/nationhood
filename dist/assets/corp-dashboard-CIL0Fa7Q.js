...
import{_supabase as f}from"./supabase-client-qEAQbBjE.js";/* empty css                    *//* empty css                         */import{_ as _e}from"./preload-helper-BXl3LOEh.js";import{escapeHtml as _,tickToDate as Ct}from"./utils-A98FEun4.js";import{initMessaging as me}from"./messaging-1y3PTVCT.js";import{c as ue}from"./equipment-DsuDdEne.js";import{a as ge,b as ye,d as be}from"./corp-valuation-C0hsb2EQ.js";import{m as zt,p as Wt}from"./loan-math-Q4nHfU_i.js";import{c as xe,s as he}from"./corp-refurbish-eZ1qOCh2.js";let ut=[],v=null,V=null;function c(t){return Math.abs(t)>=1e6?"$"+(t/1e6).toFixed(2)+"M":Math.abs(t)>=1e3?"$"+(t/1e3).toFixed(1)+"k":"$"+Math.round(t).toLocaleString()}function pt(t,a){return Number(t?.[a]??50)}async function we(){const{data:{user:t}}=await f.auth.getUser();if(!t){window.location.href="login.html";return}const a=new URLSearchParams(location.search).get("faction_id");if(a){const{data:x,error:y}=await f.from("factions").select("*").eq("id",a).single();y?console.warn("[Inspector] faction fetch failed:",y.message):x?.faction_type==="corporation"&&(v=x)}if(!v){const{data:x}=await f.from("factions").select("*").or(`id.eq.${t.id},linked_user_id.eq.${t.id}`);ut=(x||[]).filter(d=>d.nation_id&&!d.abandoned_at);const y=sessionStorage.getItem("active_faction_id");if(v=ut.find(d=>d.id===y)||ut.find(d=>d.faction_type==="corporation")||ut[0],!v){sessionStorage.removeItem("active_faction_id"),window.location.href="faction-select.html";return}if(sessionStorage.setItem("active_faction_id",v.id),v.faction_type!=="corporation"){window.location.href="dashboard.html";return}}let e=v.nation||"",n=null;const[i,r]=await Promise.all([v.nation_id?f.from("nations").select("*").eq("id",v.nation_id).single():Promise.resolve({data:null}),f.from("shard").select("current_tick, current_date, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single()]);i.error&&console.warn("Nation load failed:",i.error.message),i.data&&(e=i.data.name,n=i.data),r.error&&console.warn("Shard load failed:",r.error.message),V=r.data;let s=0;if(v?.id){const{data:x}=await f.from("construction_contracts").select("id, contract_bids!inner(id)").eq("issuer_faction_id",v.id).in("status",["open","bidding"]);if(x)for(const y of x)s+=(y.contract_bids||[]).length}const l=document.getElementById("corp-topbar-container");if(l){const{renderCorpTopBar:x}=await _e(async()=>{const{renderCorpTopBar:d}=await import("./corp-topbar-rMK78I65.js");return{renderCorpTopBar:d}},__vite__mapDeps([0,1])),y={};s>0&&(y.home={color:"#c8a832",title:s+" pending bid"+(s!==1?"s":"")+" on your projects"}),x(l,{faction:v,shard:V,activeTab:"home",allUserFactions:ut,badges:y})}document.getElementById("id-type-badge").textContent=v.corp_company_type||"—";const o=document.getElementById("id-logo"),p=(v.corp_ticker||v.abbreviation||"").toUpperCase();v.custom_logo_url?o.innerHTML=`<img src="${_(v.custom_logo_url)}" alt="logo" style="width:100%;height:100%;object-fit:contain;border-radius:4px;">`:o.textContent=p.slice(0,3)||"—",document.getElementById("id-corp-name").textContent=v.faction_name||"Unnamed Corp";const g=v.party_description||"";document.getElementById("id-slogan").textContent=g?'"'+g+'"':'"--"';const b=V?.current_date?V.current_date.replace(/.*,\s*/,""):"—",k=v.leader_first_name&&v.leader_last_name?v.leader_first_name+" "+v.leader_last_name+(v.leader_age?" ("+v.leader_age+")":""):"—";document.getElementById("id-rows").innerHTML=`
        <div class="id-row">
    ...
    `;const R=v.last_rename_tick||0,I=V?.current_tick||0,j=Math.max(0,R+120-I),O=!g||g==="-"||g==='"-"'||j<=0,W=document.getElementById("slogan-editor");W.innerHTML=`
        <div class="slogan-bar">
            <input type="text" id="slogan-input" placeholder="Enter a slogan..." maxlength="60" value="${_(g)}">
    ...
    `,document.getElementById("corp-logo-file")?.addEventListener("change",qe),window._corpFactionId=v.id,window._currentTick=I,window._nationStats=n,window._factionData=v;const M=$e(n,e,v);Te(e,v);const E=await Bt(n,e,v,V);let P=0;if(v?.id){const{data:x,error:y}=await f.from("corp_equipment").select("equipment_key, owned").eq("faction_id",v.id);y||(P=ue(x||[]))}let z=0;if(v?.id){const{data:x}=await f.from("corp_executives").select("salary_per_year").eq("faction_id",v.id).eq("status","active");z=(x||[]).reduce((y,d)=>y+(Number(d.salary_per_year)||0),0)}let N=0,B=0;if(v?.id&&v.corp_sector==="Shipping"){const{data:x}=await f.from("corp_vessels").select("base_maintenance, purchase_price, condition, built_at_tick, status").eq("faction_id",v.id).neq("status","for_sale");N=(x||[]).reduce((y,d)=>y+(Number(d.base_maintenance)||0),0),B=ge(x,I)}await ke(n,V,M,v,E.propertyMaintenance||0,P,z,E,N),await Me(n,e,v,M,E,B),me(v,n,V),st={nationId:v.nation_id},ee(),document.getElementById("loading").style.display="none",document.getElementById("page-content").style.display="block"}function $e(t,a,e){const n=y=>pt(t,y),i=(a||"UNKNOWN").toUpperCase(),r=Number(e?.corp_general_workforce??2250),s=Number(e?.corp_skilled_workforce??600),l=Number(e?.corp_innovative_workforce??150),o=r+s+l,p=2,g=3,b=6,k=n("minimum_wage"),R=k/100*48e3,I=n("inflation"),H=n("standard_of_living"),j=1+(I-50)/100*.5,A=1+(H-50)/100*.5,O=y=>Math.round(R*y*j*A),W=O(p),G=O(g),M=O(b),E=r*W,P=s*G,z=l*M,N=E+P+z;function B(y){return"$"+Math.round(y).toLocaleString()+"/yr"}const x=`${j.toFixed(2)} &times; ${A.toFixed(2)}`;return document.getElementById("wf-total-header").textContent=o.toLocaleString(),document.getElementById("wf-body").innerHTML=`
        <div class="wf-tier">
            ...
            <div class="wf-tier__detail">
                <span class="wf-tier__label">Wage (min &times; ${p}.0 &times; ${x})</span>
        ...
        <div class="wf-tier">
            ...
            <div class="wf-tier__detail">
                <span class="wf-tier__label">Wage (min &times; ${g}.0 &times; ${x})</span>
    ...
    `,{totalWages:N,generalTotal:E,skilledTotal:P,innovativeTotal:z,monthlyWages:Math.round(N/12)}}async function ke(t,a,e,n,i,r,s,l,o){const p=a?.current_tick||0;document.getElementById("fin-tick").textContent="TICK "+p;const g=87e6,b=u=>pt(t,u),k=1+(b("gdp_growth")-50)/100*.4,R=1+(b("urbanization")-50)/100*.3,I=1+(b("population_growth")-50)/100*.2,H=1+(b("standard_of_living")-50)/100*.15,j=1+(50-b("physical_infrastructure"))/100*.1,A=1-Math.max(0,b("inflation")-50)/100*.1,O=1-Math.max(0,b("interest_rates")-50)/100*.1,W=k*R*I*H*j*A*O,G=Math.round(g*W),M=(n.corp_general_workforce||0)+(n.corp_skilled_workforce||0)+(n.corp_innovative_workforce||0),E=Math.max(500,l?.totalCapacity||500),P=Math.min(1,M/E),z=l?.propertyRevBonus||0,N=Math.round(Math.round(G/12)*P)+z;let B=0,x=0,y=0,d=0,tt=0,F=0,X=0,$=0,K=0;if(n?.id){const u=n.corp_sector||"";if(u==="Finance"){const{data:T}=await f.from("finance_active_loans").select("monthly_payment, payments_made, total_paid, total_interest_paid, interest_rate, remaining_principal, finance_loan_requests(request_type)").eq("lender_faction_id",n.id).in("status",["current","late","delinquent"]);for(const m of T||[]){const h=m.finance_loan_requests?.request_type||"loan";if(h==="insurance"){const C=Number(m.monthly_payment||0);x+=C,$+=C}else if(h==="loan"){const C=Number(m.monthly_payment||0);tt+=C;const q=Math.max(0,Number(m.remaining_principal||0)),D=Math.min(C,zt(q,m.interest_rate)),S=Wt(C,D);x+=D,y+=D,d+=S;const U=Number(m.payments_made||0),ht=Number(m.total_paid||0),fe=C*U,Ht=Math.max(0,ht||fe),ve=Math.max(0,Math.min(Ht,Number(m.total_interest_paid||0)));F+=Ht,X+=ve}else h==="bond"&&(x+=Number(m.monthly_payment||0))}}else if(u==="Construction"){const{data:T}=await f.from("construction_contracts").select("id, budget_ceiling, timeline_ticks").eq("awarded_to_faction",n.id).eq("status","in_progress"),m=[];for(const h of T||[])x+=Math.round((h.budget_ceiling||0)/(h.timeline_ticks||1)),h.id&&m.push(h.id);if(m.length>0){const{data:h}=await f.from("contract_bids").select("contract_id, estimated_cost").in("contract_id",m).eq("status","won"),C={};for(const q of h||[])C[q.contract_id]=Number(q.estimated_cost||0);for(const q of T||[]){const D=C[q.id]||0;K+=Math.round(D/Math.max(1,q.timeline_ticks||1))}}}else if(u==="Shipping"){const{data:T}=await f.from("shipping_claims").select("revenue_per_transit").eq("faction_id",n.id).eq("status","active");for(const m of T||[])x+=Number(m.revenue_per_transit||0)}}let J=[],Y=0;try{const{data:u}=await f.from("corp_properties").select("id, nation_id, nations!nation_id(name)").eq("faction_id",n.id).eq("type","fuel_depot").eq("is_active",!0);if(u&&u.length>0){const T=u.map(m=>m.nation_id).filter(Boolean);if(T.length>0){const{data:m}=await f.from("shipping_claims").select("faction_id, shipping_routes!inner(destination_nation_id, status)").eq("status","active").in("shipping_routes.destination_nation_id",T),h=[...new Set((m||[]).map(S=>S.faction_id).filter(S=>S&&S!==n.id))],C=new Set;if(h.length>0){const{data:S}=await f.from("corp_properties").select("faction_id, nation_id").in("faction_id",h).in("nation_id",T).eq("type","fuel_depot").eq("is_active",!0);for(const U of S||[])C.add(U.faction_id+"|"+U.nation_id)}const q={};for(const S of m||[]){const U=S.shipping_routes?.destination_nation_id;U&&S.faction_id!==n.id&&(C.has(S.faction_id+"|"+U)||(q[U]=(q[U]||0)+1))}const D=7500;for(const S of u){const U=q[S.nation_id]||0,ht=U*D;J.push({nation:S.nations?.name||"Unknown",revenue:ht,visitors:U}),Y+=ht}J.sort((S,U)=>U.revenue-S.revenue)}}}catch(u){console.warn("Fuel depot revenue estimate failed (non-fatal):",u?.message||u)}const vt=B+x+N+Y,bt=e?.totalWages||0,_t=Math.round(bt/12),L=0,w=i||0,ct=r||0,et=Number(n?.corp_loans)||0,dt=.05,nt=et>0?Math.round(et*(dt/12)/(1-Math.pow(1+dt/12,-120))):0;let Z=0,it=0,at=0,rt=[];if(n?.id)try{const{data:u}=await f.from("finance_active_loans").select("monthly_payment, equity_pct, series, total_paid, finance_loan_requests(request_type), lender:factions!lender_faction_id(faction_name)").eq("borrower_faction_id",n.id).in("status",["current","late","delinquent"]),T=Math.max(0,Number(n?.monthly_profit||0));for(const m of u||[]){const h=m.finance_loan_requests?.request_type||"loan";if(h==="equity"){const q=Number(m.equity_pct||0),D=Math.floor(T*q/100);at+=D,rt.push({investor:m.lender?.faction_name||"Unknown",stakePct:q,series:m.series||"?",paidToDate:Number(m.total_paid||0),nextDividend:D});continue}const C=Number(m.monthly_payment||0);if(!(C<=0))if(h==="insurance")it+=C;else{if(h==="bond")continue;Z+=C}}rt.sort((m,h)=>h.stakePct-m.stakePct)}catch(u){console.warn("[Finances] borrower finance_active_loans lookup failed:",u)}const xt=Math.round((s||0)/12),ot=o||0,ft=75e3,ae=Math.max(0,Math.min(1,Number(t?.corporate_tax??0)/100||0)),Ft=_t+xt+L+w+ct+ot+nt+Z+it+at+K+ft,Dt=Math.max(0,vt-Ft),Ut=Math.round(Dt*ae);let Tt="";try{const u=new Set([n.nation_id]),{data:T}=await f.from("corp_properties").select("nation_id").eq("faction_id",n.id).eq("is_active",!0);if((T||[]).forEach(m=>{m.nation_id&&u.add(m.nation_id)}),u.size>0){const{data:m}=await f.from("nations").select("id, name, corporate_tax").in("id",[...u]);m&&m.length>0&&(Tt=m.sort((h,C)=>(h.name||"").localeCompare(C.name||"")).map(h=>{const C=Math.round(Number(h.corporate_tax??0)),q=Math.round(Dt*(C/100)/m.length),D=C>25?"#c55":C>15?"#ca5":"#5c5";return`<div style="display:flex;justify-content:space-between;padding:1px 0;font-family:var(--font-mono);font-size:8px;">
                        <span style="color:var(--text-dim);">${h.name} (<span style="color:${D}">${C}%</span>)</span>
                        <span style="color:#a44;">${c(q)}</span>
                    </div>`}).join(""))}}catch{}const oe=Ft+Ut,Mt=Number(n?.monthly_profit||0),lt=Number(n?.corp_cash_reserves??0),ie=et;let mt=null,Q=null,Lt=null,Nt="Does not include all capital/financing cash transfers.";if(n?.id)try{const{data:u}=await f.from("corp_cash_history").select("tick, cash_start, cash_end, cash_delta, non_pnl_cash_movements").eq("faction_id",n.id).lte("tick",p).order("tick",{ascending:!1}).limit(2),T=(u||[]).find(h=>Number(h.tick)===Number(p))||(u||[])[0]||null,m=(u||[]).find(h=>Number(h.tick)<Number(T?.tick??p))||null;T?(mt=T.cash_start!=null?Number(T.cash_start):m?.cash_end!=null?Number(m.cash_end):null,Q=T.cash_delta!=null?Number(T.cash_delta):mt!=null?lt-mt:null,Lt=T.non_pnl_cash_movements!=null?Number(T.non_pnl_cash_movements):Q!=null?Q-Mt:null):Nt="Does not include all capital/financing cash transfers. Cash history snapshot not yet available."}catch(u){console.warn("[Finances] corp_cash_history lookup failed:",u),Nt="Does not include all capital/financing cash transfers. Cash history snapshot unavailable."}const It=tt-(y+d),se=Math.abs(It)>.01,re=[{stat:"gdp_growth",value:b("gdp_growth"),weight:"0.4"},{stat:"urbanization",value:b("urbanization"),weight:"0.3"},{stat:"population_growth",value:b("population_growth"),weight:"0.2"},{stat:"standard_of_living",value:b("standard_of_living"),weight:"0.15"},{stat:"physical_infra",value:b("physical_infrastructure"),weight:"0.1",note:"INV"},{stat:"inflation",value:b("inflation"),weight:"-0.1",neg:!0},{stat:"interest_rates",value:b("interest_rates"),weight:"-0.1",neg:!0}];function le(u){return u.neg?u.value>50?"var(--red)":"var(--green)":u.note?u.value<50?"var(--green)":"var(--red)":u.value>=50?"var(--green)":u.value>=35?"var(--amber)":"var(--red)"}const Rt=vt||1,ce=(B/Rt*100).toFixed(1),de=((x+Y)/Rt*100).toFixed(1),pe=(N/Rt*100).toFixed(1);document.getElementById("fin-body").innerHTML=`
        <!-- Revenue -->
        <div class="fin-section">
            ...
            <div class="seg-bar">
                <div class="seg-bar__seg" style="width:${ce}%;background:var(--teal);"></div>
                <div class="seg-bar__seg" style="width:${de}%;background:var(--amber);"></div>
                <div class="seg-bar__seg" style="width:${pe}%;background:var(--text-dim);"></div>
            </div>
            ...
            ${n?.corp_sector==="Finance"?`<div class="fin-row"><span class="fin-row__label" title="Interest portion of this tick's loan payments recognized as operating revenue in P&L.">Loan Interest Revenue (this tick) <span class="fin-row__badge">REVENUE</span></span><span class="fin-row__value" style="color:var(--green)">${c(y)}</span></div>`:""}
            ${n?.corp_sector==="Finance"&&$>0?`<div class="fin-row"><span class="fin-row__label" title="Insurance premiums collected this tick are recognized as operating revenue in P&L.">Insurance Premium Revenue (this tick) <span class="fin-row__badge">REVENUE</span></span><span class="fin-row__value" style="color:var(--green)">${c($)}</span></div>`:""}
            ...
            ${n?.corp_sector==="Finance"?`<div class="fin-row"><span class="fin-row__label" title="Loan cash collected this tick equals interest plus principal.">Loan Cash Collected (this tick) <span class="fin-row__badge">CASH</span></span><span class="fin-row__value" style="color:var(--text-bright)">${c(y+d)}</span></div>`:""}
            ${n?.corp_sector==="Finance"?`<div class="fin-row"><span class="fin-row__label" title="Checks this tick's scheduled loan payments against computed split. Portfolio Scheduled Loan Payments = Interest Revenue + Principal Repaid.">Loan Payment Reconciliation (this tick) <span class="fin-row__badge">CHECK</span></span><span class="fin-row__value" style="color:${se?"var(--red)":"var(--text-bright)"}">${c(tt)} = ${c(y)} + ${c(d)} (Δ ${It>=0?"+":"-"}${c(Math.abs(It))})</span></div>`:""}
            ...
            ${n?.corp_sector==="Finance"?`<div class="fin-row"><span class="fin-row__label" title="Portfolio cumulative cash from loans. Loan cash collected = interest + principal.">Loan Cash Collected (to date) <span class="fin-row__badge">PORTFOLIO</span></span><span class="fin-row__value" style="color:var(--text-bright)">${c(F)}</span></div>`:""}
            ${n?.corp_sector==="Finance"?`<div class="fin-row"><span class="fin-row__label" title="Portfolio cumulative interest portion collected from loans.">Loan Interest (to date) <span class="fin-row__badge">PORTFOLIO</span></span><span class="fin-row__value" style="color:var(--green)">${c(X)}</span></div>`:""}
        ...
        <div class="fin-section">
            ...
            <div class="fin-row"><span class="fin-row__label">Debt Service</span><span class="fin-row__value" style="color:#a44">${c(nt)}</span></div>
            ...
            ${at>0?`<div class="fin-row"><span class="fin-row__label" title="Dividends to equity holders. Each tick: equity_pct × last tick's monthly profit. $0 on loss ticks.">Equity Dividends</span><span class="fin-row__value" style="color:#a44">${c(at)}</span></div>`:""}
            ...
            <div class="fin-total">
                ...
                <span class="fin-total__value" style="color:var(--red)">${c(oe)}</span>
            </div>
        </div>
        ...
        <div class="fin-cash-debt">
            ...
            <div class="fin-cash-debt__cell">
                ...
                <div class="fin-cash-debt__value" style="color:var(--amber)">${c(ie)}</div>
            </div>
        </div>
        <!-- Purchasing Power -->
        ...
        ${rt.length>0?(()=>{const u=rt.reduce((m,h)=>m+(h.stakePct||0),0),T=rt.reduce((m,h)=>m+(h.paidToDate||0),0);return`<div style="padding:8px 14px;border-top:1px solid var(--border-0);">
                ...
                ${rt.map(m=>`
                    <div style="display:grid;grid-template-columns:1fr 50px 40px 1fr 1fr;gap:4px;font-family:var(--font-mono);font-size:8px;padding:2px;border-bottom:1px solid var(--border-hair);">
                        <span style="color:var(--text-bright);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${_(m.investor)}">${_(m.investor)}</span>
                        <span style="color:#c89a4a;text-align:right;">${m.stakePct.toFixed(2)}%</span>
                        <span style="color:#c89a4a;text-align:center;">${_(m.series)}</span>
        ...
        <div style="padding:8px 14px;">
            ...
            ${re.map(u=>`
                <div class="drv-row">
                    ...
                    <div class="drv-row__bar"><div class="drv-row__bar-fill" style="width:${u.value}%;background:${le(u)}"></div></div>
    ...
    `,At()}let qt=!1;async function Ce(t,a){if(!(!v||qt)){qt=!0;try{const{data:e,error:n}=await f.from("finance_loan_offers").select("*").eq("id",t).single();if(n||!e)return;const{data:i,error:r}=await f.from("finance_loan_requests").select("*").eq("id",a).single();if(r||!i||i.status!=="open")return;const s=i.term_months,l=zt(i.amount,e.interest_rate),o=Math.round(i.amount/s),p=l+o,g=V?.current_tick||0,{error:b}=await f.from("finance_loan_requests").update({status:"funded",accepted_offer_id:t,funded_tick:g}).eq("id",a);if(b)return;await f.from("finance_loan_offers").update({status:"accepted"}).eq("id",t),await f.from("finance_loan_offers").update({status:"declined"}).eq("request_id",a).neq("id",t).eq("status","pending"),await f.from("finance_active_loans").insert({request_id:a,offer_id:t,borrower_faction_id:i.requesting_faction_id,lender_faction_id:e.offering_faction_id,nation_id:i.nation_id,principal:i.amount,interest_rate:e.interest_rate,term_months:i.term_months,collateral_type:e.collateral_type,purpose:i.purpose,monthly_payment:p,started_tick:g});const{data:k}=await f.from("factions").select("corp_cash_reserves").eq("id",e.offering_faction_id).single();k&&await f.from("factions").update({corp_cash_reserves:Math.max(0,(Number(k.corp_cash_reserves)||0)-i.amount)}).eq("id",e.offering_faction_id);const{data:R}=await f.from("factions").select("corp_cash_reserves, corp_debt").eq("id",i.requesting_faction_id).single();if(R){const{error:I}=await f.from("factions").update({corp_cash_reserves:(Number(R.corp_cash_reserves)||0)+i.amount,corp_debt:(Number(R.corp_debt)||0)+i.amount}).eq("id",i.requesting_faction_id);I&&console.error("[Loans] Failed to credit borrower + track debt:",I.message)}}finally{qt=!1}At()}}async function Ee(t){await f.from("finance_loan_requests").update({status:"cancelled"}).eq("id",t),At()}async function At(){if(!v)return;const t=document.getElementById("fin-loans-list");if(t)try{const{data:a,error:e}=await f.from("finance_loan_requests").select("*, finance_loan_offers!request_id(*)").eq("requesting_faction_id",v.id).in("status",["open","funded"]).neq("request_type","equity").order("created_tick",{ascending:!1});e&&console.error("[Loans] Request query error:",e.message);const{data:n,error:i}=await f.from("finance_active_loans").select("*, lender:factions!lender_faction_id(faction_name, abbreviation)").eq("borrower_faction_id",v.id).in("status",["current","late","delinquent"]).is("equity_pct",null).order("started_tick",{ascending:!1});i&&console.error("[Loans] Active loans query error:",i.message);let r="";if(a&&a.length>0){for(const s of a)if(s.status==="open"){const l=(s.finance_loan_offers||[]).filter(o=>o.status==="pending");if(r+=`<div style="padding:6px 0;border-bottom:1px solid var(--border-hair);">
                    ...
                    <div style="font-size:7px;color:var(--text-dim);margin-top:2px;">${_(s.purpose||"")}</div>`,l.length>0){r+=`<div style="margin-top:4px;font-size:7px;color:#5a8aaa;font-weight:700;">${l.length} OFFER${l.length>1?"S":""}</div>`;for(const o of l.sort((p,g)=>p.interest_rate-g.interest_rate))r+=`<div style="display:flex;align-items:center;gap:6px;padding:3px 0;border-top:1px solid var(--border-hair);">
                        ...
                        </div>`}else r+='<div style="margin-top:2px;font-size:7px;color:var(--text-dim);">Awaiting offers from finance corporations...</div>';r+="</div>"}}if(n&&n.length>0)for(const s of n){const l=s.status==="current"?"var(--green)":s.status==="late"?"#c84":"#c55",o=s.term_months>0?Math.round(s.payments_made/s.term_months*100):0,p=Number(s.monthly_payment||0),g=Math.max(0,Number(s.remaining_principal||0)),b=Math.max(0,Math.min(p,zt(g,s.interest_rate))),k=Wt(p,b),R=s.lender?.faction_name||"Unknown bank",I=Ct(s.started_tick);r+=`<div style="padding:6px 0;border-bottom:1px solid var(--border-hair);">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <div>
                ...
                <div style="display:flex;justify-content:space-between;margin-top:2px;font-size:7px;color:var(--text-dim);">
                    <span>Bank: <span style="color:var(--text-primary);">${_(R)}</span></span>
                    <span>Issued: <span style="color:var(--text-primary);">${_(I)}</span></span>
                </div>
            ...
            </div>`}document.getElementById("stats-body").innerHTML=l}async function Bt(t,a,e,n){const i=(a||"UNKNOWN").toUpperCase();let r=[];if(e?.id){const{data:y}=await f.from("corp_properties").select("*").eq("faction_id",e.id).eq("is_active",!0).order("type",{ascending:!1}).order("purchased_at_tick",{ascending:!1});r=y||[]}const s={Basic:{color:"#6a6660"},Modern:{color:"#8b9a6b"},Sustainable:{color:"#5c5"},Innovative:{color:"#c8a832"},Heritage:{color:"#c84"},Premium:{color:"#ca5"}};let l=0,o=0;const p=Number(e?.corp_general_workforce??0)+Number(e?.corp_skilled_workforce??0)+Number(e?.corp_innovative_workforce??0),g=500,b=r.map(y=>{const d=Number(y.capacity||0),tt=Number(y.condition||0)/100;return Math.floor(d*tt)}),k=g+b.reduce((y,d)=>y+d,0),R=k>0?Math.min(p,Math.round(p*(g/k))):p,I=5e7,H=1+(pt(t,"inflation")-50)/100*.3,j=.8+pt(t,"stability")/100*.4,A=Math.round(I*H*j),O=Math.round(A*.005);l+=A,o+=O;let W=`
    <div class="prop-asset" style="margin-bottom:6px;">
    ...
    </div>`,G=R;for(let y=0;y<r.length;y++){const d=r[y],tt=s[d.style]||s.Basic;l+=Number(d.purchase_price||0),o+=Number(d.monthly_maintenance||0);const F=d.condition>=75?"var(--green)":d.condition>=50?"var(--amber)":"var(--orange)",X=Number(d.capacity||0),$=b[y]||0,K=k>0?Math.min(p-G,Math.round(p*($/k))):0;G+=K,W+=`
        <div class="prop-asset" style="margin-bottom:6px;">
            <div class="prop-asset__top">
                <div>
        ...
        </div>`}let M="",E=[];if(e?.id){const{data:y}=await f.from("construction_contracts").select("*, contract_bids(id, faction_id, bid_price, estimated_quality, estimated_cost, labor_count, markup_pct, material_grades, submitted_at_tick, status, factions!faction_id(faction_name, abbreviation, corp_reputation))").eq("issuer_faction_id",e.id).in("status",["open","bidding","awarded","in_progress"]).order("generated_at_tick",{ascending:!1});E=y||[];let d={};const tt=E.filter(F=>F.status==="in_progress").map(F=>F.id);if(tt.length>0){const{data:F}=await f.from("construction_events").select("contract_id, status, severity, title").in("contract_id",tt).eq("status","ACTIVE");for(const X of F||[])d[X.contract_id]||(d[X.contract_id]=[]),d[X.contract_id].push(X)}if(E.length>0){const F={open:{label:"OPEN",color:"#5a8aaa"},bidding:{label:"BIDDING",color:"#c8a832"},awarded:{label:"AWARDED",color:"#8b9a6b"},in_progress:{label:"IN PROGRESS",color:"#5aaa8b"}},X={LOW:"#ca5",MODERATE:"#c84",HIGH:"#c55",CRITICAL:"#f44"};M=`<div class="cp-section">
                <div class="cp-section__header">
                    ...
                    <div style="display:flex;gap:0;background:var(--bg-3);border:1px solid var(--border-hair);margin:2px 0;">
                        ...
                        <div class="cp-progress"><div class="cp-progress__bar" style="width:${Z}%;background:${it};"></div></div>`}else M+='<div style="font-size:8px;font-family:var(--font-mono);color:var(--amber);margin-top:6px;text-align:center;">Awarded — construction begins next tick</div>';M+="</div>"}if(bt.length>0)for(const L of bt){const w=X[L.severity]||"#ca5";M+=`<div style="display:flex;align-items:center;gap:5px;margin-top:4px;padding:3px 6px;background:${w}08;border:1px solid ${w}20;">
                            ...
                            <span style="font-size:8px;font-family:var(--font-mono);color:${w};">${_(L.title)}</span>
    ...
    `;let x=0;x+=Math.round(g*50);for(const y of r){if(y.refurbish_until_tick&&(n?.current_tick||0)<y.refurbish_until_tick)continue;const d=Number(y.condition||0)/100;d>=.6&&(x+=Math.round(Number(y.capacity||0)*d*50))}return{propertyValue:l,propertyMaintenance:o,totalCapacity:k,propertyRevBonus:x}}async function Me(t,a,e,n,i,r=0){(a||"UNKNOWN").toUpperCase();const s=e.corp_company_type||"Private",l=Number(e.corp_cash_reserves)||0,o=i?.propertyValue||0;let p={loans:0,bonds:0,insurance:0,total:0};if(e?.id&&e.corp_sector==="Finance")try{const{data:P}=await f.from("finance_active_loans").select("principal, remaining_principal, finance_loan_requests!inner(request_type)").eq("lender_faction_id",e.id).in("status",["current","late","delinquent"]);p=ye(P||[])}catch(P){console.warn("[Valuation] finance_active_loans lookup failed:",P)}const g=p.total,b=l+o+r+g,k=Number(e.corp_loans)||0,I=n?.monthlyWages||0,H=0,j=k+I+H,A=be({cash:l,propertyValue:o,equipmentValue:r,loans:j,financeReceivables:g}),O=A.valuationBasis,W=A.valuation,G=W-A.valuationBasis,M=G>0;document.getElementById("val-type-badge").textContent=s.toUpperCase();function E(P,z,N={}){const B=N.indent?"val-line val-line--indent":"val-line",x=N.bold?"val-line__label val-line__label--bold":"val-line__label",y=N.bold?"val-line__value val-line__value--bold":"val-line__value",d=N.color||(N.bold?"var(--text-bright)":"var(--text-muted)");return`<div class="${B}"><span class="${x}">${P}</span><span class="${y}" style="color:${d}">${c(z)}</span></div>`}document.getElementById("val-body").innerHTML=`
        ...
        ${E("Finance Receivables (Loans + Bonds)",g,{indent:!0})}
        ${E("Insurance Coverage (excluded)",p.insurance,{indent:!0,color:"var(--text-dim)"})}
    ...
    `}async function Le(){const t=document.getElementById("slogan-input"),a=document.getElementById("slogan-hint"),e=document.getElementById("slogan-save-btn"),n=(t.value||"").trim().slice(0,60);if(n.length===0){a.textContent="Slogan cannot be empty.",a.className="slogan-hint slogan-hint--error";return}e.disabled=!0,e.textContent="...",a.textContent="";try{const{error:i}=await f.from("factions").update({party_description:n,last_rename_tick:window._currentTick}).eq("id",window._corpFactionId);if(i)throw i;document.getElementById("id-slogan").textContent='"'+n+'"',a.textContent="Slogan saved! Next change in 120 ticks.",a.className="slogan-hint slogan-hint--ok",e.textContent="Save"}catch(i){console.error("Slogan save failed:",i),a.textContent="Failed to save slogan.",a.className="slogan-hint slogan-hint--error",e.disabled=!1,e.textContent="Save"}}async function Ne(){await f.auth.signOut(),window.location.href="login.html"}function Ie(){const t=document.getElementById("corp-faction-dropdown");t&&t.classList.toggle("open")}function Re(t,a){const e=document.getElementById("corp-faction-dropdown");e&&e.classList.remove("open"),sessionStorage.setItem("active_faction_id",t),a==="corporation"?window.location.href="corp-dashboard.html":window.location.href="dashboard.html"}document.addEventListener("click",t=>{const a=document.getElementById("faction-switcher"),e=document.getElementById("corp-faction-dropdown");e&&a&&!a.contains(t.target)&&e.classList.remove("open")});window.doLogout=Ne;async function qe(t){const a=t.target.files?.[0];if(!a)return;if(a.size>128*1024){alert("Logo must be under 128KB.");return}const e=window._corpFactionId;if(!e)return;const n=document.getElementById("corp-logo-label");n&&(n.textContent="Uploading...");try{const i=a.name.split(".").pop()||"png",r=`party-logos/${e}/${Date.now()}.${i}`,{error:s}=await f.storage.from("public-assets").upload(r,a,{contentType:a.type,upsert:!0});if(s)throw s;const{data:l}=f.storage.from("public-assets").getPublicUrl(r),o=l?.publicUrl||null;await f.from("factions").update({custom_logo_url:o}).eq("id",e);const p=document.getElementById("id-logo");p&&(p.innerHTML=`<img src="${o}" alt="logo" style="width:100%;height:100%;object-fit:contain;border-radius:4px;">`);const g=document.getElementById("corp-logo");g&&(g.innerHTML=`<img src="${o}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:5px;">`),n&&(n.textContent="Change Logo")}catch(i){console.error("Logo upload failed:",i),alert("Upload failed: "+(i.message||"Unknown error")),n&&(n.textContent="Upload Logo")}}window.saveSlogan=Le;window.toggleCorpDropdown=Ie;window.switchToFaction=Re;let wt=!1;function Pe(t,a,e,n){if(wt)return;const i=window._nationStats,s=1+(pt(i,"inflation")-50)/100*.3,l=Math.max(.1,n/100),o=Math.round(e*s*l),p=document.getElementById("prop-modal-overlay"),g=document.getElementById("prop-modal-content");g.innerHTML=`
        <div style="font-size:14px;font-weight:700;margin-bottom:8px;color:var(--text-primary);">Sell Property</div>
        <div style="font-size:10px;color:var(--text-dim);margin-bottom:12px;">${_(a)}</div>
    ...
    `,p.style.display="flex"}async function Se(t,a){if(wt)return;wt=!0;const e=document.getElementById("prop-sell-confirm");e&&(e.disabled=!0,e.textContent="Selling...");try{const n=window._corpFactionId;if(!n)throw new Error("No faction");const{error:i}=await f.from("corp_properties").update({is_active:!1}).eq("id",t).eq("faction_id",n);if(i)throw new Error("Failed to sell property: "+i.message);const{data:r}=await f.from("factions").select("corp_cash_reserves").eq("id",n).single(),s=Number(r?.corp_cash_reserves??0),{error:l}=await f.from("factions").update({corp_cash_reserves:s+a}).eq("id",n);l&&console.error("[Property] Failed to credit cash:",l.message),Et(),alert("Property sold for "+c(a)+". Cash credited."),location.reload()}catch(n){alert("Sale failed: "+n.message)}finally{wt=!1,e&&(e.disabled=!1,e.textContent="Confirm Sale")}}let $t=!1;function ze(t,a,e,n,i){if($t)return;const r=window._nationStats,s=window._factionData,o=1+(pt(r,"inflation")-50)/100*.3,p=xe({purchase_price:n,refurbish_count:i},o),b=Number(s?.corp_cash_reserves??0)>=p,k=document.getElementById("prop-modal-overlay"),R=document.getElementById("prop-modal-content"),I=b&&e<95;let H="";e>=95?H='<div style="color:var(--green);font-size:9px;margin-bottom:8px;">Property already at excellent condition ('+e+"%).</div>":b||(H='<div style="color:var(--red);font-size:9px;margin-bottom:8px;">Insufficient cash reserves.</div>'),R.innerHTML=`
        <div style="font-size:14px;font-weight:700;margin-bottom:8px;color:var(--text-primary);">Refurbish Property</div>
        <div style="font-size:10px;color:var(--text-dim);margin-bottom:12px;">${_(a)} — Refurbishment #${i+1} — Current Condition: ${e}%</div>
        ${H}
        <div style="background:var(--bg-3);border:1px solid var(--border-hair);padding:8px;margin-bottom:10px;">
            <div style="display:flex;justify-content:space-between;font-size:9px;font-family:var(--font-mono);margin-bottom:4px;">
    ...
    `,k.style.display="flex"}async function Ae(t,a,e,n){if($t)return;$t=!0;const i=document.getElementById("prop-refurb-confirm");i&&(i.disabled=!0,i.textContent="Starting...");try{const r=window._corpFactionId,s=window._currentTick,l=window._factionData,o=window._nationStats;if(!r)throw new Error("No faction");const g=1+(pt(o,"inflation")-50)/100*.3,k=await he(f,r,{id:t,purchase_price:a,refurbish_count:e,condition:n},s,g);if(!k.ok)throw new Error(k.error||"Refurbishment failed.");l&&(l.corp_cash_reserves=k.newCash),Et(),alert(`Refurbishment started! Duration: ${k.duration} ticks. Target condition: ${k.targetCondition}%. Cost: ${c(k.cost)}.`),location.reload()}catch(r){alert("Refurbishment failed: "+r.message)}finally{$t=!1,i&&(i.disabled=!1,i.textContent="Begin Refurbishment")}}function Et(){const t=document.getElementById("prop-modal-overlay");t&&(t.style.display="none")}window.showSellModal=Pe;window.confirmSellProperty=Se;window.showRefurbishModal=ze;window.confirmRefurbish=Ae;window.closePropModal=Et;window.showConvertModal=Fe;window.confirmConvertProperty=De;let Pt=!1;async function Be(t,a,e,n,i,r,s){if(!Pt&&confirm("Accept bid from "+e+`?

Bid Price: `+c(n)+`
Quality: `+i+`/100
Workers: `+r+`

This will award the contract. The project begins immediately.`)){Pt=!0;try{const{data:l}=await f.from("shard").select("current_tick").eq("name","Alpha Shard").single(),o=l?.current_tick||0,{error:p}=await f.from("contract_bids").update({status:"won"}).eq("id",a);if(p)throw p;const{error:g}=await f.from("contract_bids").update({status:"lost"}).eq("contract_id",t).neq("id",a);if(g)throw g;const{error:b}=await f.from("construction_contracts").update({status:"awarded",awarded_to_faction:s,awarded_at_tick:o}).eq("id",t);if(b)throw b;alert("Contract awarded to "+e+`!

Bid: `+c(n)+`
Project begins immediately.`),window._nationStats&&window._factionData&&V&&await Bt(window._nationStats,window._nationStats?.name||"",window._factionData,V)}catch(l){alert("Failed to accept bid: "+(l.message||l))}finally{Pt=!1}}}window.cpAcceptBid=Be;function Oe(t){const a=document.getElementById("cp-bid-"+t);a&&(a.style.display=a.style.display==="none"?"":"none")}window.cpToggleBid=Oe;let kt="branch_office",St=!1;function Fe(t,a,e){const n=(v?.corp_subsector||"").toLowerCase(),i=n==="banking"?[["branch_office","Branch Office"]]:n==="investment"?[["trading_floor","Trading Floor"]]:n==="insurance"?[["claims_office","Claims Office"],["insurance_office","Insurance Office"]]:[];if(i.length===0)return;kt=i[0][0];const r=Math.round(e*.15),s=Math.floor(Math.random()*6)+4,l=document.getElementById("prop-modal-overlay"),o=document.getElementById("prop-modal-content");o.innerHTML=`
        <div style="font-size:14px;font-weight:700;margin-bottom:8px;color:var(--text-primary);">Convert Property</div>
        <div style="font-size:10px;color:var(--text-muted);margin-bottom:10px;">${_(a)}</div>
        <div style="display:flex;gap:4px;margin-bottom:10px;">
            ${i.map(([p,g])=>`<span onclick="_convertTargetType='${p}';document.querySelectorAll('.conv-opt').forEach(e=>e.style.background='transparent');this.style.background='rgba(138,106,170,0.15)'" class="conv-opt" style="flex:1;text-align:center;padding:6px;font-family:var(--font-mono);font-size:9px;font-weight:700;cursor:pointer;border:1px solid rgba(138,106,170,0.3);color:#8a6aaa;${p===kt?"background:rgba(138,106,170,0.15)":""}">${g}</span>`).join("")}
        </div>
        <div style="display:flex;justify-content:space-between;font-size:9px;font-family:var(--font-mono);margin-bottom:4px;">
    ...
    `,l.style.display="flex"}async function De(t,a,e){if(!St){St=!0;try{const n=Number(v?.corp_cash_reserves??0);if(n<a){alert("Insufficient cash. Need "+c(a)+".");return}const{data:i,error:r}=await f.from("corp_properties").select("role").eq("id",t).single();if(r||!i){alert("Conversion failed: "+(r?.message||"property not found"));return}if(i.role==="subsidiary"){alert("Subsidiary HQs cannot be converted.");return}const s=V?.current_tick||0,l=Math.max(0,n-a),{error:o}=await f.from("factions").update({corp_cash_reserves:l}).eq("id",v.id);if(o){alert("Conversion failed: "+o.message);return}v.corp_cash_reserves=l;const{error:p}=await f.from("corp_properties").update({type:kt,role:kt,refurbish_until_tick:s+e,condition:100}).eq("id",t);if(p){const{error:b}=await f.from("factions").update({corp_cash_reserves:n}).eq("id",v.id);b||(v.corp_cash_reserves=n),alert("Conversion failed: "+p.message+(b?" (refund also failed — contact admin)":""));return}Et();const g=window._nationStats;await Bt(g,g?.name||v?.nation,v,V)}catch(n){alert("Conversion failed: "+n.message)}finally{St=!1}}}const Vt={crisis:{icon:"⚠",color:"#e74c3c",bg:"rgba(231,76,60,0.12)",label:"CRISIS"},government:{icon:"⚖",color:"#f1c40f",bg:"rgba(241,196,15,0.10)",label:"GOV"},executive_order:{icon:"✍",color:"#e67e22",bg:"rgba(230,126,34,0.12)",label:"EXEC ORDER"},political:{icon:"★",color:"#3498db",bg:"rgba(52,152,219,0.12)",label:"POLITICAL"},trade:{icon:"⚓",color:"#2ecc71",bg:"rgba(46,204,113,0.12)",label:"TRADE"},diplomatic:{icon:"🌐",color:"#9b59b6",bg:"rgba(155,89,182,0.12)",label:"DIPLOMATIC"},bill:{icon:"📜",color:"#3498db",bg:"rgba(52,152,219,0.10)",label:"BILL"},new_party:{icon:"🏳",color:"#9b59b6",bg:"rgba(155,89,182,0.10)",label:"NEW PARTY"},protest:{icon:"✊",color:"#e67e22",bg:"rgba(230,126,34,0.10)",label:"PROTEST"},military:{icon:"⚔",color:"#b07a4a",bg:"rgba(176,122,74,0.12)",label:"MILITARY"},economy:{icon:"💰",color:"#7a9a5b",bg:"rgba(122,154,91,0.12)",label:"ECONOMY"},corporate:{icon:"🏢",color:"#c8a64e",bg:"rgba(200,166,78,0.10)",label:"CORP"}},Gt={icon:"•",color:"#888",bg:"rgba(136,136,136,0.08)",label:""},Ue={1:"Embarrassing Backfire",2:"Protests Don't Materialise",3:"Modest Turnout",4:"Respectable Protest",5:"Strong Demonstration",6:"Nationwide Protests",7:"The Big One"};let gt="nation",yt="local",st=null;function He(t){return t?t.replace(/_/g," ").replace(/\b\w/g,a=>a.toUpperCase()):""}function Ot(t,a){if(!t)return"<em>Unknown</em>";const e=_(t);return a?`<span style="color:${a.replace(/[^a-zA-Z0-9#(),.\s%]/g,"")};font-weight:600">${e}</span>`:`<strong>${e}</strong>`}function Kt(t,a,e){const n=t.factions?.nation_id===(t.nation_id||a),i=t.proposer_name||(n?t.factions?.faction_name:null)||"A former party",r=t.proposer_color||(n?t.factions?.party_color:null);return{fired_at_tick:t.proposed_tick,event_name:t.bill_name,_desc_html:`${Ot(i,r)} proposed "${_(t.bill_name)}"`,category:"bill",_synthetic:!0,...e}}function Yt(t,a){const e=t.leader_first_name&&t.leader_last_name?`${t.leader_first_name} ${t.leader_last_name}`:null,n=e?` led by <strong>${_(e)}</strong>`:"";return{fired_at_tick:0,event_name:t.faction_name,_desc_html:`${Ot(t.faction_name,t.party_color)} founded${n}`,category:"new_party",_synthetic:!0,_created_at:t.created_at,...a}}function Qt(t,a){const e=Ue[t.tier]||`Tier ${t.tier}`,n=t.demand_label?` demanding "${_(t.demand_label)}"`:"",i=t.status==="crisis_active",r=t.tier>=6?"#e74c3c":t.tier>=4?"#f39c12":"",s=r?`<span style="color:${r};font-weight:600">${_(e)}</span>`:`<strong>${_(e)}</strong>`;return{fired_at_tick:t.tick_resolved||t.tick_called,event_name:e,_desc_html:`${Ot(t.factions?.faction_name,t.factions?.party_color)} organised a protest${n} — ${s}${i?' <span style="color:#e74c3c;font-weight:700;">(CRISIS)</span>':""}`,category:"protest",_synthetic:!0,...a}}function Xt(t,a,e,n,i){return[...t.map(r=>({...r,_synthetic:!1})),...a,...e,...n].sort((r,s)=>{const l=(s.fired_at_tick||0)-(r.fired_at_tick||0);if(l!==0)return l;const o=r._created_at||r.created_at||"",p=s._created_at||s.created_at||"";return p>o?1:p<o?-1:0}).slice(0,i)}function Jt(t){if(t._synthetic&&t._desc_html)return t._desc_html;const a=t.description_chosen||t.description_used||"",e=He(t.event_name),n=e?`<strong>${_(e)}</strong>`:"",i=a?_(a):"";return n&&i?`${n} — ${i}`:i||n||"Event"}function Zt(t){return t.map(a=>{const e=Ct(a.fired_at_tick),n=Vt[(a.category||"").toLowerCase()]||Gt;return`<div class="corp-ev-row">
            <span class="corp-ev-date">${_(e)}</span>
            <span class="corp-ev-icon" style="color:${n.color}">${n.icon}</span>
            <span class="corp-ev-text">${Jt(a)}</span>
            ${n.label?`<span class="corp-ev-cat" style="color:${n.color};background:${n.bg}">${n.label}</span>`:""}
        </div>`}).join("")}const jt=[{color:"#6b8cae",bg:"rgba(107,140,174,0.10)",border:"rgba(107,140,174,0.3)"},{color:"#7a9a5b",bg:"rgba(122,154,91,0.10)",border:"rgba(122,154,91,0.3)"},{color:"#b07a4a",bg:"rgba(176,122,74,0.10)",border:"rgba(176,122,74,0.3)"},{color:"#8b7ec8",bg:"rgba(139,126,200,0.10)",border:"rgba(139,126,200,0.3)"},{color:"#5b9a8b",bg:"rgba(91,154,139,0.10)",border:"rgba(91,154,139,0.3)"},{color:"#a65d5d",bg:"rgba(166,93,93,0.10)",border:"rgba(166,93,93,0.3)"}];function je(t){let a=0;for(let e=0;e<t.length;e++)a=(a<<5)-a+t.charCodeAt(e)|0;return jt[Math.abs(a)%jt.length]}function te(t){return t.map(a=>{const e=Ct(a.fired_at_tick),n=Vt[(a.category||"").toLowerCase()]||Gt,i=a.nations?.name||"Unknown",r=a.nations?.nation_profiles,s=Array.isArray(r)?r[0]?.flag_url:r?.flag_url,l=je(i),o=s?`<img src="${_(s)}" alt="">`:"";return`<div class="corp-ev-row">
            ...
            <span class="corp-ev-text">${Jt(a)}</span>
            ${n.label?`<span class="corp-ev-cat" style="color:${n.color};background:${n.bg}">${n.label}</span>`:""}
        </div>`}).join("")}async function We(){const t=document.getElementById("corp-events-list");if(!t||!st)return;const{nationId:a}=st;if(!a){t.innerHTML='<div class="subs-card__empty">No nation linked.</div>';return}t.innerHTML='<div style="color:var(--text-secondary);padding:16px;font-size:0.7rem;text-align:center;">Loading…</div>';try{const[e,n]=await Promise.all([f.from("factions").select("faction_name, abbreviation, corp_sector, corp_subsector, corp_ticker, leader_first_name, leader_last_name, nation_id, nation, created_at, founded_tick").eq("faction_type","corporation").eq("nation_id",a).is("abandoned_at",null).order("created_at",{ascending:!1}).limit(30),f.from("event_log").select("*").eq("nation_id",a).eq("category","corporate").order("fired_at_tick",{ascending:!1}).limit(30)]),i=e.data||[],r=n.data||[],s=i.map(o=>({event_name:"Corporation Founded",category:"corporate",description_chosen:`${o.faction_name} [${o.corp_ticker||o.abbreviation||"??"}] was founded with a specialty in ${o.corp_subsector||o.corp_sector||"General"}. Led by CEO ${[o.leader_first_name,o.leader_last_name].filter(Boolean).join(" ")||"Unknown"}.`,fired_at_tick:o.founded_tick||0})),l=[...r,...s].sort((o,p)=>(p.fired_at_tick||0)-(o.fired_at_tick||0)).slice(0,40);if(l.length===0){t.innerHTML='<div class="subs-card__empty" style="padding:40px 0;">No corporate events in this nation yet.</div>';return}t.innerHTML=Zt(l)}catch(e){console.error("Corp local events error:",e),t.innerHTML='<div class="subs-card__empty">Failed to load events.</div>'}}async function Ve(){const t=document.getElementById("corp-events-list");if(!t||!st)return;const{nationId:a}=st;if(!a){t.innerHTML='<div class="subs-card__empty">No nation linked.</div>';return}t.innerHTML='<div style="color:var(--text-secondary);padding:16px;font-size:0.7rem;text-align:center;">Loading…</div>';try{const[e,n]=await Promise.all([f.from("factions").select("faction_name, abbreviation, corp_sector, corp_subsector, corp_ticker, leader_first_name, leader_last_name, nation_id, nation, created_at, founded_tick").eq("faction_type","corporation").neq("nation_id",a).is("abandoned_at",null).order("created_at",{ascending:!1}).limit(30),f.from("event_log").select("*, nations(name, nation_profiles(flag_url))").neq("nation_id",a).eq("category","corporate").order("fired_at_tick",{ascending:!1}).limit(30)]),i=e.data||[],r=n.data||[],s=i.map(o=>({event_name:"Corporation Founded",category:"corporate",description_chosen:`${o.faction_name} [${o.corp_ticker||o.abbreviation||"??"}] was founded in ${o.nation||"Unknown"} with a specialty in ${o.corp_subsector||o.corp_sector||"General"}. Led by CEO ${[o.leader_first_name,o.leader_last_name].filter(Boolean).join(" ")||"Unknown"}.`,fired_at_tick:o.founded_tick||0,nations:{name:o.nation||"Unknown"}})),l=[...r,...s].sort((o,p)=>(p.fired_at_tick||0)-(o.fired_at_tick||0)).slice(0,40);if(l.length===0){t.innerHTML='<div class="subs-card__empty" style="padding:40px 0;">No corporate events from other nations yet.</div>';return}t.innerHTML=te(l);return}catch(e){console.error("Corp world events error:",e),t.innerHTML='<div class="subs-card__empty">Failed to load events.</div>';return}try{const{data:e,error:n}=await f.from("factions").select("faction_name, abbreviation, corp_sector, corp_subsector, corp_ticker, leader_first_name, leader_last_name, nation_id, nation, created_at, founded_tick").eq("faction_type","corporation").neq("nation_id",a).is("abandoned_at",null).order("created_at",{ascending:!1}).limit(30);if(n)throw n;if(!e||e.length===0){t.innerHTML='<div class="subs-card__empty" style="padding:40px 0;">No corporate events from other nations yet.</div>';return}t.innerHTML=Ge(e,!0)}catch(e){console.error("Corp world events error:",e),t.innerHTML='<div class="subs-card__empty">Failed to load events.</div>'}}function Ge(t,a){return t.map(e=>{const n=[e.leader_first_name,e.leader_last_name].filter(Boolean).join(" ")||"Unknown",i=e.nation||"Unknown",r=e.corp_subsector||e.corp_sector||"General",s=e.corp_ticker||e.abbreviation||"",l=e.founded_tick?Ct(e.founded_tick):"";let o='<div class="corp-event-row">';return o+='<div style="font-family:var(--font-mono);font-size:7px;font-weight:700;color:var(--teal);background:rgba(90,175,165,0.08);border:1px solid rgba(90,175,165,0.15);padding:1px 5px;display:inline-block;margin-bottom:2px;">'+_(i.toUpperCase())+"</div>",o+='<div style="font-size:11px;color:var(--text-primary);line-height:1.5;">',o+='<span style="font-weight:600;">'+_(e.faction_name)+"</span>",s&&(o+=' <span style="font-family:var(--font-mono);font-size:0.65rem;color:var(--text-dim);">['+_(s)+"]</span>"),o+=' was founded in <span style="font-weight:500;">'+_(i)+"</span>",o+=' with a specialty in <span style="color:var(--teal);font-weight:500;">'+_(r)+"</span>.",o+=' Led by CEO <span style="font-weight:500;">'+_(n)+"</span>.",o+="</div>",l&&(o+='<div style="font-family:var(--font-mono);font-size:8px;color:var(--text-dim);margin-top:1px;">'+_(l)+"</div>"),o+="</div>",o}).join("")}async function ee(){const t=document.getElementById("corp-events-list");if(!t||!st)return;const{nationId:a}=st;if(!a){t.innerHTML='<div class="subs-card__empty">No nation linked.</div>';return}t.innerHTML='<div style="color:var(--text-secondary);padding:16px;font-size:0.7rem;text-align:center;">Loading…</div>';try{const[e,n,i,r]=await Promise.all([f.from("event_log").select("*").eq("nation_id",a).order("fired_at_tick",{ascending:!1}).limit(50),f.from("bills").select("bill_name, proposed_tick, proposed_by, bill_type, proposer_name, proposer_color, factions!proposed_by(faction_name, party_color, nation_id)").eq("nation_id",a).not("bill_type","in",'("no_confidence","confirmation","minister_confirmation","veto_override","impeachment_conviction")').order("proposed_tick",{ascending:!1}).limit(30),f.from("factions").select("faction_name, party_color, leader_first_name, leader_last_name, created_at").eq("nation_id",a).order("created_at",{ascending:!1}).limit(20),f.from("protest_log").select("id, tick_called, tick_resolved, tier, status, demand_label, faction_id, factions(faction_name, party_color)").eq("nation_id",a).in("status",["resolved","crisis_active"]).order("tick_called",{ascending:!1}).limit(20)]);if(e.error)throw e.error;const s=e.data||[],l=Xt(s,(n.data||[]).map(o=>Kt(o,a)),(i.data||[]).map(o=>Yt(o)),(r.data||[]).map(o=>Qt(o)),60);if(l.length===0){t.innerHTML='<div class="subs-card__empty" style="padding:40px 0;">No nation events recorded yet.</div>';return}t.innerHTML=Zt(l)}catch(e){console.error("Nation events error:",e),t.innerHTML='<div class="subs-card__empty">Failed to load events.</div>'}}async function Ke(){const t=document.getElementById("corp-events-list");if(!t||!st)return;const{nationId:a}=st;if(!a){t.innerHTML='<div class="subs-card__empty">No nation linked.</div>';return}t.innerHTML='<div style="color:var(--text-secondary);padding:16px;font-size:0.7rem;text-align:center;">Loading world events…</div>';try{const[e,n,i,r]=await Promise.all([f.from("event_log").select("*, nations!inner(name, nation_profiles(flag_url))").neq("nation_id",a).order("fired_at_tick",{ascending:!1}).limit(60),f.from("bills").select("bill_name, proposed_tick, nation_id, proposed_by, bill_type, proposer_name, proposer_color, factions!proposed_by(faction_name, party_color, nation_id), nations!inner(name, nation_profiles(flag_url))").neq("nation_id",a).not("bill_type","in",'("no_confidence","confirmation","minister_confirmation","veto_override","impeachment_conviction")').order("proposed_tick",{ascending:!1}).limit(20),f.from("factions").select("faction_name, party_color, leader_first_name, leader_last_name, nation_id, created_at, nations!inner(name, nation_profiles(flag_url))").neq("nation_id",a).order("created_at",{ascending:!1}).limit(15),f.from("protest_log").select("id, tick_called, tick_resolved, tier, status, demand_label, faction_id, nation_id, factions(faction_name, party_color), nations!inner(name, nation_profiles(flag_url))").neq("nation_id",a).in("status",["resolved","crisis_active"]).order("tick_called",{ascending:!1}).limit(15)]);if(e.error)throw e.error;const s=e.data||[],l=Xt(s,(n.data||[]).map(o=>Kt(o,null,{nations:o.nations})),(i.data||[]).map(o=>Yt(o,{nations:o.nations})),(r.data||[]).map(o=>Qt(o,{nations:o.nations})),60);if(l.length===0){t.innerHTML='<div class="subs-card__empty" style="padding:40px 0;">No world events recorded yet.</div>';return}t.innerHTML=te(l)}catch(e){console.error("World events error:",e),t.innerHTML='<div class="subs-card__empty">Failed to load events.</div>'}}window.switchCorpEventsCat=function(t){t!==gt&&(gt=t,document.querySelectorAll("#corp-events-cat-bar .corp-events-tab").forEach(a=>a.classList.toggle("active",a.dataset.cat===t)),ne())};window.switchCorpEventsScope=function(t){t!==yt&&(yt=t,document.querySelectorAll("#corp-events-scope-bar .corp-events-tab").forEach(a=>a.classList.toggle("active",a.dataset.scope===t)),ne())};function ne(){gt==="nation"&&yt==="local"?ee():gt==="nation"&&yt==="world"?Ke():gt==="corporate"&&yt==="local"?We():Ve()}we();
