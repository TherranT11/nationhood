import{_supabase as p}from"./supabase-client-qEAQbBjE.js";/* empty css                    *//* empty css                         */import{escapeHtml as v,tickToDate as $t}from"./utils-A98FEun4.js";import{initMessaging as ne}from"./messaging-1y3PTVCT.js";import{c as oe}from"./equipment-DsuDdEne.js";import{a as ae}from"./corp-valuation-C0hsb2EQ.js";import{s as ie,c as se}from"./corp-refurbish-eZ1qOCh2.js";import{m as zt}from"./loan-math-Q4nHfU_i.js";let ht=[],d=null,Q=null;function _(t){return Math.abs(t)>=1e6?"$"+(t/1e6).toFixed(2)+"M":Math.abs(t)>=1e3?"$"+(t/1e3).toFixed(1)+"k":"$"+Math.round(t).toLocaleString()}function mt(t){const e=Number(t)||0,n=Math.abs(e);return n>=1e9?{main:"$"+(e/1e9).toFixed(2),unit:"B"}:n>=1e6?{main:"$"+(e/1e6).toFixed(2),unit:"M"}:n>=1e3?{main:"$"+(e/1e3).toFixed(1),unit:"k"}:{main:"$"+Math.round(e).toLocaleString(),unit:""}}function re(t){const e=Math.max(0,Math.min(100,Number(t)||0));return e>=90?{tag:"Aaa",tone:"good"}:e>=80?{tag:"Aa1",tone:"good"}:e>=70?{tag:"A1",tone:"good"}:e>=60?{tag:"Baa1",tone:"gold"}:e>=50?{tag:"Baa3",tone:"gold"}:e>=40?{tag:"Ba1",tone:"gold"}:e>=30?{tag:"Ba3",tone:"red"}:e>=20?{tag:"B2",tone:"red"}:e>=10?{tag:"Caa1",tone:"red"}:{tag:"Ca",tone:"red"}}function vt(t,e){return Number(t?.[e]??50)}async function le(){const{data:{user:t}}=await p.auth.getUser();if(!t){window.location.href="login.html";return}const{data:e}=await p.from("factions").select("*").or(`id.eq.${t.id},linked_user_id.eq.${t.id}`);ht=(e||[]).filter(b=>b.nation_id&&!b.abandoned_at);const n=sessionStorage.getItem("active_faction_id");if(d=ht.find(b=>b.id===n)||ht.find(b=>b.faction_type==="corporation")||ht[0],!d){sessionStorage.removeItem("active_faction_id"),window.location.href="faction-select.html";return}if(sessionStorage.setItem("active_faction_id",d.id),d.faction_type!=="corporation"){window.location.href="dashboard.html";return}let o=d.nation||"",i=null;const[r,f]=await Promise.all([d.nation_id?p.from("nations").select("*").eq("id",d.nation_id).single():Promise.resolve({data:null}),p.from("shard").select("current_tick, current_date, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single()]);r.error&&console.warn("Nation load failed:",r.error.message),r.data&&(o=r.data.name,i=r.data),f.error&&console.warn("Shard load failed:",f.error.message),Q=f.data;let l=0;if(d?.id){const{data:b}=await p.from("construction_contracts").select("id, contract_bids!inner(id)").eq("issuer_faction_id",d.id).in("status",["open","bidding"]);if(b)for(const g of b)l+=(g.contract_bids||[]).length}(function(){const g=(d.corp_ticker||d.abbreviation||"").toUpperCase(),c=Q?.current_date||"",K=c?c.replace(/.*,\s*/,""):"",B=Q?.current_tick,Y=document.getElementById("h2-plate-logo");Y&&(d.custom_logo_url?Y.innerHTML=`<img src="${v(d.custom_logo_url)}" alt="logo">`:Y.textContent=g.slice(0,3)||"—");const $=document.getElementById("h2-eyebrow-loc");$&&($.textContent=o?`The Boardroom · ${o}`:"The Boardroom");const X=document.getElementById("h2-tick-date");if(X){const k=[];B!=null&&k.push("Tick "+B),c&&k.push(c),X.textContent=k.length?k.join(" · "):"Tick — · —"}const J=document.getElementById("h2-next-close");J&&(J.textContent="Next close —");const G=document.getElementById("h2-ceo");if(G){const k=[d.leader_first_name,d.leader_last_name].filter(Boolean);if(k.length){const R=d.leader_age?" ("+d.leader_age+")":"",V=d.leader_role||"Chairman & Chief Executive";G.textContent=`${k.join(" ")} · ${V}${R}`}else G.textContent="—"}const st=document.getElementById("h2-brand");if(st){const k=d.faction_name||"Unnamed Corporation",R=k.split(" ");if(R.length>1){const V=R.slice(0,-1).join(" "),Z=R[R.length-1];st.innerHTML=`${v(V)} <em>${v(Z)}</em>`}else st.textContent=k}const dt=document.getElementById("h2-brand-sub");if(dt){const k=[];d.corp_company_type&&k.push(d.corp_company_type),K&&k.push("Est. "+K);const R=d.corp_subsector||d.corp_sector;R&&k.push(R),dt.textContent=k.length?k.join(" · "):"—"}const pt=document.getElementById("h2-tail-code");if(pt){const k=(o||"").split(" ").map(Z=>Z[0]||"").join("").toUpperCase().slice(0,4),R=d.party_description?'"'+d.party_description+'"':"",V=[];g&&V.push(g),k&&V.push(k+" EXCH"),R&&V.push(R),pt.textContent=V.length?V.join(" · "):"—"}const T=document.getElementById("h2-wire-corp");if(T){const k=(d.faction_name||"your corp").split(" ").slice(0,2).join(" ");T.textContent=k}const h=document.getElementById("h2-edit-toggle");h&&h.addEventListener("click",()=>{const k=document.body.classList.toggle("h2-edit-open");h.classList.toggle("on",k),h.textContent=k?"Close ✕":"Logo"})})(),(function(){const g=document.getElementById("h2-logout-btn");if(g&&g.addEventListener("click",async()=>{try{sessionStorage.clear(),await p.auth.signOut()}catch{}window.location.href="login.html"}),l>0){const c=document.getElementById("h2-nav-actions-badge");c&&(c.textContent=l,c.style.display="",c.classList.add("ok"))}})(),document.getElementById("id-type-badge").textContent=d.corp_company_type||"—";const a=document.getElementById("id-logo"),s=(d.corp_ticker||d.abbreviation||"").toUpperCase();d.custom_logo_url?a.innerHTML=`<img src="${v(d.custom_logo_url)}" alt="logo" style="width:100%;height:100%;object-fit:contain;border-radius:4px;">`:a.textContent=s.slice(0,3)||"—",document.getElementById("id-corp-name").textContent=d.faction_name||"Unnamed Corp";const m=d.party_description||"";document.getElementById("id-slogan").textContent=m?'"'+m+'"':'"--"';const u=Q?.current_date?Q.current_date.replace(/.*,\s*/,""):"—",x=d.leader_first_name&&d.leader_last_name?d.leader_first_name+" "+d.leader_last_name+(d.leader_age?" ("+d.leader_age+")":""):"—";document.getElementById("id-rows").innerHTML=`
    ...
    `,document.getElementById("corp-logo-file")?.addEventListener("change",xe),window._corpFactionId=d.id,window._currentTick=S,window._nationStats=i,window._factionData=d;const w=ce(i,o,d);_e(o,d);const L=await St(i,o,d,Q);let H=0;if(d?.id){const{data:b,error:g}=await p.from("corp_equipment").select("equipment_key, owned").eq("faction_id",d.id);g||(H=oe(b||[]))}let M=0;if(d?.id){const{data:b}=await p.from("corp_executives").select("salary_per_year").eq("faction_id",d.id).eq("status","active");M=(b||[]).reduce((g,c)=>g+(Number(c.salary_per_year)||0),0)}let N=0,F=0;if(d?.id&&d.corp_sector==="Shipping"){const{data:b}=await p.from("corp_vessels").select("base_maintenance, purchase_price, condition, built_at_tick, status").eq("faction_id",d.id).neq("status","for_sale");N=(b||[]).reduce((g,c)=>g+(Number(c.base_maintenance)||0),0),F=ae(b,S)}await de(i,Q,w,d,L.propertyMaintenance||0,H,M,L,N),await ve(i,o,d,w,L,F),ne(d,i,Q),ct={nationId:d.nation_id},Kt(),document.getElementById("loading").style.display="none",document.getElementById("page-content").style.display="block"}function ce(t,e,n){const o=g=>vt(t,g),i=(e||"UNKNOWN").toUpperCase(),r=Number(n?.corp_general_workforce??2250),f=Number(n?.corp_skilled_workforce??600),l=Number(n?.corp_innovative_workforce??150),a=r+f+l,s=2,m=3,u=6,x=o("minimum_wage"),E=x/100*48e3,S=o("inflation"),A=o("standard_of_living"),q=1+(S-50)/100*.5,W=1+(A-50)/100*.5,O=g=>Math.round(E*g*q*W),U=O(s),D=O(m),w=O(u),L=r*U,H=f*D,M=l*w,N=L+H+M;function F(g){return"$"+Math.round(g).toLocaleString()+"/yr"}const b=`${q.toFixed(2)} &times; ${W.toFixed(2)}`;return document.getElementById("wf-total-header").textContent=a.toLocaleString(),document.getElementById("wf-body").innerHTML=`
        <div class="wf-tier">
    ...
    `,{totalWages:N,generalTotal:L,skilledTotal:H,innovativeTotal:M,monthlyWages:Math.round(N/12)}}async function de(t,e,n,o,i,r,f,l,a){const s=e?.current_tick||0;document.getElementById("fin-tick").textContent="TICK "+s;const m=87e6,u=y=>vt(t,y),x=1+(u("gdp_growth")-50)/100*.4,E=1+(u("urbanization")-50)/100*.3,S=1+(u("population_growth")-50)/100*.2,A=1+(u("standard_of_living")-50)/100*.15,q=1+(50-u("physical_infrastructure"))/100*.1,W=1-Math.max(0,u("inflation")-50)/100*.1,O=1-Math.max(0,u("interest_rates")-50)/100*.1,U=x*E*S*A*q*W*O,D=Math.round(m*U),w=(o.corp_general_workforce||0)+(o.corp_skilled_workforce||0)+(o.corp_innovative_workforce||0),L=Math.max(500,l?.totalCapacity||500),H=Math.min(1,w/L),M=l?.propertyRevBonus||0,N=Math.round(Math.round(D/12)*H)+M;let F=0,b=0,g=0;if(o?.id){const y=o.corp_sector||"";if(y==="Finance"){const{data:I}=await p.from("finance_active_loans").select("monthly_payment, interest_rate, remaining_principal, finance_loan_requests(request_type)").eq("lender_faction_id",o.id).in("status",["current","late","delinquent"]);for(const C of I||[]){const P=C.finance_loan_requests?.request_type||"loan";if(P==="insurance")b+=Number(C.monthly_payment||0);else if(P==="loan"){const z=Math.max(0,Number(C.remaining_principal||0));b+=zt(z,C.interest_rate)}else P==="bond"&&(b+=Number(C.monthly_payment||0))}}else if(y==="Construction"){const{data:I}=await p.from("construction_contracts").select("id, timeline_ticks").eq("awarded_to_faction",o.id).eq("status","in_progress"),C=(I||[]).map(z=>z.id).filter(Boolean);if(C.length>0){const{data:z}=await p.from("contract_bids").select("contract_id, estimated_cost").in("contract_id",C).eq("status","won"),it={};for(const at of z||[])it[at.contract_id]=Number(at.estimated_cost||0);for(const at of I||[]){const j=it[at.id]||0;g+=Math.round(j/Math.max(1,at.timeline_ticks||1))}}const{data:P}=await p.from("construction_deliveries").select("payment_received").eq("faction_id",o.id).eq("delivered_at_tick",s);for(const z of P||[])b+=Number(z.payment_received||0)}else if(y==="Shipping"){const{data:I}=await p.from("shipping_claims").select("revenue_per_transit").eq("faction_id",o.id).eq("status","active");for(const C of I||[])b+=Number(C.revenue_per_transit||0)}}let c=[],K=0;try{const{data:y}=await p.from("corp_properties").select("id, nation_id, nations!nation_id(name)").eq("faction_id",o.id).eq("type","fuel_depot").eq("is_active",!0);if(y&&y.length>0){const I=y.map(C=>C.nation_id).filter(Boolean);if(I.length>0){const{data:C}=await p.from("shipping_claims").select("faction_id, shipping_routes!inner(destination_nation_id, status)").eq("status","active").in("shipping_routes.destination_nation_id",I),P=[...new Set((C||[]).map(j=>j.faction_id).filter(j=>j&&j!==o.id))],z=new Set;if(P.length>0){const{data:j}=await p.from("corp_properties").select("faction_id, nation_id").in("faction_id",P).in("nation_id",I).eq("type","fuel_depot").eq("is_active",!0);for(const nt of j||[])z.add(nt.faction_id+"|"+nt.nation_id)}const it={};for(const j of C||[]){const nt=j.shipping_routes?.destination_nation_id;nt&&j.faction_id!==o.id&&(z.has(j.faction_id+"|"+nt)||(it[nt]=(it[nt]||0)+1))}const at=7500;for(const j of y){const nt=it[j.nation_id]||0,Rt=nt*at;c.push({nation:j.nations?.name||"Unknown",revenue:Rt,visitors:nt}),K+=Rt}c.sort((j,nt)=>nt.revenue-j.revenue)}}}catch(y){console.warn("Fuel depot revenue estimate failed (non-fatal):",y?.message||y)}const B=F+b+N+K,Y=n?.totalWages||0,$=Math.round(Y/12),X=0,J=i||0,G=r||0,st=Number(o?.corp_loans)||0,dt=.05,pt=st>0?Math.round(st*(dt/12)/(1-Math.pow(1+dt/12,-120))):0;let T=0,h=0;if(o?.id)try{const{data:y}=await p.from("finance_active_loans").select("monthly_payment, finance_loan_requests(request_type)").eq("borrower_faction_id",o.id).in("status",["current","late","delinquent"]);for(const I of y||[]){const C=I.finance_loan_requests?.request_type||"loan",P=Number(I.monthly_payment||0);if(!(P<=0))if(C==="insurance")h+=P;else{if(C==="bond")continue;T+=P}}}catch(y){console.warn("[Finances] borrower finance_active_loans lookup failed:",y)}const k=Math.round((f||0)/12),R=a||0,V=75e3,Z=Math.max(0,Math.min(1,Number(t?.corporate_tax??0)/100||0)),rt=$+k+X+J+G+R+pt+T+h+g+V,_t=Math.max(0,B-rt),ft=Math.round(_t*Z);let ut="";try{const y=new Set([o.nation_id]),{data:I}=await p.from("corp_properties").select("nation_id").eq("faction_id",o.id).eq("is_active",!0);if((I||[]).forEach(C=>{C.nation_id&&y.add(C.nation_id)}),y.size>0){const{data:C}=await p.from("nations").select("id, name, corporate_tax").in("id",[...y]);C&&C.length>0&&(ut=C.sort((P,z)=>(P.name||"").localeCompare(z.name||"")).map(P=>{const z=Math.round(Number(P.corporate_tax??0)),it=Math.round(_t*(z/100)/C.length),at=z>25?"#c55":z>15?"#ca5":"#5c5";return`<div style="display:flex;justify-content:space-between;padding:1px 0;font-family:var(--font-mono);font-size:8px;">
                        <span style="color:var(--text-dim);">${P.name} (<span style="color:${at}">${z}%</span>)</span>
                        <span style="color:#a44;">${_(it)}</span>
                    </div>`}).join(""))}}catch{}const Ct=rt+ft,ot=Number(o?.monthly_profit||0),tt=Number(o?.corp_cash_reserves??0),Qt=st;let gt=null,et=null,Et=null,Mt="Does not include all capital/financing cash transfers.";if(o?.id)try{const{data:y}=await p.from("corp_cash_history").select("tick, cash_start, cash_end, cash_delta, non_pnl_cash_movements").eq("faction_id",o.id).lte("tick",s).order("tick",{ascending:!1}).limit(2),I=(y||[]).find(P=>Number(P.tick)===Number(s))||(y||[])[0]||null,C=(y||[]).find(P=>Number(P.tick)<Number(I?.tick??s))||null;I?(gt=I.cash_start!=null?Number(I.cash_start):C?.cash_end!=null?Number(C.cash_end):null,et=I.cash_delta!=null?Number(I.cash_delta):gt!=null?tt-gt:null,Et=I.non_pnl_cash_movements!=null?Number(I.non_pnl_cash_movements):et!=null?et-ot:null):Mt="Does not include all capital/financing cash transfers. Cash history snapshot not yet available."}catch(y){console.warn("[Finances] corp_cash_history lookup failed:",y),Mt="Does not include all capital/financing cash transfers. Cash history snapshot unavailable."}me(B,ot);const Xt=[{stat:"gdp_growth",value:u("gdp_growth"),weight:"0.4"},{stat:"urbanization",value:u("urbanization"),weight:"0.3"},{stat:"population_growth",value:u("population_growth"),weight:"0.2"},{stat:"standard_of_living",value:u("standard_of_living"),weight:"0.15"},{stat:"physical_infra",value:u("physical_infrastructure"),weight:"0.1",note:"INV"},{stat:"inflation",value:u("inflation"),weight:"-0.1",neg:!0},{stat:"interest_rates",value:u("interest_rates"),weight:"-0.1",neg:!0}];function Jt(y){return y.neg?y.value>50?"var(--red)":"var(--green)":y.note?y.value<50?"var(--green)":"var(--red)":y.value>=50?"var(--green)":y.value>=35?"var(--amber)":"var(--red)"}const Tt=B||1,Zt=(F/Tt*100).toFixed(1),te=((b+K)/Tt*100).toFixed(1),ee=(N/Tt*100).toFixed(1);document.getElementById("fin-body").innerHTML=`
        <!-- Revenue -->
        ...
        <div class="fin-section">
            ...
            <div class="fin-total">
                ...
                <span class="fin-total__value" style="color:var(--red)">${_(Ct)}</span>
            </div>
        </div>
        ...
        <div style="padding:2px 14px 8px 14px;font-family:var(--font-mono);font-size:7px;color:var(--text-dim);letter-spacing:0.2px;">${Mt}</div>
        <!-- Cash Reconciliation -->
        <div class="fin-section">
            ...
            <div class="fin-row"><span class="fin-row__label">+/- Non-P&amp;L cash movements</span><span class="fin-row__value" style="color:var(--text-bright)">${Et==null?"—":_(Et)}</span></div>
            <div class="fin-total">
    ...
    `,Nt()}let It=!1;async function pe(t,e){if(!(!d||It)){It=!0;try{const{data:n,error:o}=await p.from("finance_loan_offers").select("*").eq("id",t).single();if(o||!n)return;const{data:i,error:r}=await p.from("finance_loan_requests").select("*").eq("id",e).single();if(r||!i||i.status!=="open")return;const f=i.term_months,l=zt(i.amount,n.interest_rate),a=Math.round(i.amount/f),s=l+a,m=Q?.current_tick||0,{error:u}=await p.from("finance_loan_requests").update({status:"funded",accepted_offer_id:t,funded_tick:m}).eq("id",e);if(u)return;await p.from("finance_loan_offers").update({status:"accepted"}).eq("id",t),await p.from("finance_loan_offers").update({status:"declined"}).eq("request_id",e).neq("id",t).eq("status","pending"),await p.from("finance_active_loans").insert({request_id:e,offer_id:t,borrower_faction_id:i.requesting_faction_id,lender_faction_id:n.offering_faction_id,nation_id:i.nation_id,principal:i.amount,interest_rate:n.interest_rate,term_months:i.term_months,collateral_type:n.collateral_type,purpose:i.purpose,monthly_payment:s,started_tick:m});const{data:x}=await p.from("factions").select("corp_cash_reserves").eq("id",n.offering_faction_id).single();x&&await p.from("factions").update({corp_cash_reserves:Math.max(0,(Number(x.corp_cash_reserves)||0)-i.amount)}).eq("id",n.offering_faction_id);const{data:E}=await p.from("factions").select("corp_cash_reserves, corp_debt").eq("id",i.requesting_faction_id).single();if(E){const{error:S}=await p.from("factions").update({corp_cash_reserves:(Number(E.corp_cash_reserves)||0)+i.amount,corp_debt:(Number(E.corp_debt)||0)+i.amount}).eq("id",i.requesting_faction_id);S&&console.error("[Loans] Failed to credit borrower + track debt:",S.message)}}finally{It=!1}Nt()}}async function fe(t){await p.from("finance_loan_requests").update({status:"cancelled"}).eq("id",t),Nt()}async function Nt(){if(!d)return;const t=document.getElementById("fin-loans-list");if(t)try{const{data:e,error:n}=await p.from("finance_loan_requests").select("*, finance_loan_offers!request_id(*)").eq("requesting_faction_id",d.id).in("status",["open","funded"]).neq("request_type","equity").order("created_tick",{ascending:!1});n&&console.error("[Loans] Request query error:",n.message);const{data:o,error:i}=await p.from("finance_active_loans").select("*, finance_loan_requests!inner(request_type, insured_contract_id), lender:factions!lender_faction_id(faction_name, abbreviation)").eq("borrower_faction_id",d.id).in("status",["current","late","delinquent"]).is("equity_pct",null).order("started_tick",{ascending:!1});i&&console.error("[Loans] Active loans query error:",i.message);const r=[];for(const s of o||[]){const m=s.finance_loan_requests?.insured_contract_id;m&&r.push(m)}const f=[...new Set(r)];let l={};if(f.length>0){const{data:s,error:m}=await p.from("construction_contracts").select("id, status").in("id",f);m?console.error("[Loans] Contract status query error:",m.message):l=Object.fromEntries((s||[]).map(u=>[u.id,u.status]))}let a="";if(e&&e.length>0){for(const s of e)if(s.status==="open"){const m=(s.finance_loan_offers||[]).filter(u=>u.status==="pending");if(a+=`<div style="padding:6px 0;border-bottom:1px solid var(--border-hair);">
    ...
    </div>`,D=E;for(let g=0;g<r.length;g++){const c=r[g],K=f[c.style]||f.Basic;l+=Number(c.purchase_price||0),a+=Number(c.monthly_maintenance||0);const B=c.condition>=75?"var(--green)":c.condition>=50?"var(--amber)":"var(--orange)",Y=Number(c.capacity||0),$=u[g]||0,X=x>0?Math.min(s-D,Math.round(s*($/x))):0;D+=X,U+=`
        <div class="prop-asset" style="margin-bottom:6px;">
            <div class="prop-asset__top">
                <div>
                            ...
                            <div id="cp-bid-${k}" style="display:none;padding:4px 8px 6px;border-top:1px solid var(--border-hair);background: var(--border-hair);">
                                ...
                                ${ft.length>0?`<div style="padding:2px 4px;">
                                    ...
                                    <div style="display:flex;gap:6px;flex-wrap:wrap;">
                                        ${ft.map(([ot,tt])=>`<span style="font-size:7px;font-family:var(--font-mono);color:${Ct(tt)};">${ut(ot)}: ${tt}</span>`).join("")}
                                    </div>
                                </div>`:""}
                            </div>
    ...
    `;let b=0;b+=Math.round(m*50);for(const g of r){if(g.refurbish_until_tick&&(o?.current_tick||0)<g.refurbish_until_tick)continue;const c=Number(g.condition||0)/100;c>=.6&&(b+=Math.round(Number(g.capacity||0)*c*50))}return{propertyValue:l,propertyMaintenance:a,totalCapacity:x,propertyRevBonus:b}}async function ve(t,e,n,o,i,r=0){(e||"UNKNOWN").toUpperCase();const f=n.corp_company_type||"Private",l=Number(n.corp_cash_reserves)||0,a=i?.propertyValue||0;let s=0;if(n?.id&&n.corp_sector==="Finance")try{const{data:L}=await p.from("finance_active_loans").select("remaining_principal, finance_loan_requests(request_type)").eq("lender_faction_id",n.id).in("status",["current","late","delinquent"]);for(const H of L||[]){const M=H.finance_loan_requests?.request_type||"loan";if(M==="loan"||M==="bond"){const N=Math.max(0,Number(H.remaining_principal||0));s+=N}}}catch(L){console.warn("[Valuation] finance_active_loans lookup failed:",L)}const m=l+a+r+s,u=Number(n.corp_loans)||0,E=o?.monthlyWages||0,S=0,A=u+E+S,q=m-A,O=Math.round(q*(1+.3)),U=O-q,D=U>0;document.getElementById("val-type-badge").textContent=f.toUpperCase();function w(L,H,M={}){const N=M.indent?"val-line val-line--indent":"val-line",F=M.bold?"val-line__label val-line__label--bold":"val-line__label",b=M.bold?"val-line__value val-line__value--bold":"val-line__value",g=M.color||(M.bold?"var(--text-bright)":"var(--text-muted)");return`<div class="${N}"><span class="${F}">${L}</span><span class="${b}" style="color:${g}">${_(H)}</span></div>`}document.getElementById("val-body").innerHTML=`
    ...
    `,s.style.display="flex"}async function $e(t,e){if(xt)return;xt=!0;const n=document.getElementById("prop-sell-confirm");n&&(n.disabled=!0,n.textContent="Selling...");try{const o=window._corpFactionId;if(!o)throw new Error("No faction");const{error:i}=await p.from("corp_properties").update({is_active:!1}).eq("id",t).eq("faction_id",o);if(i)throw new Error("Failed to sell property: "+i.message);const{data:r}=await p.from("factions").select("corp_cash_reserves").eq("id",o).single(),f=Number(r?.corp_cash_reserves??0),{error:l}=await p.from("factions").update({corp_cash_reserves:f+e}).eq("id",o);l&&console.error("[Property] Failed to credit cash:",l.message),kt(),alert("Property sold for "+_(e)+". Cash credited."),location.reload()}catch(o){alert("Sale failed: "+o.message)}finally{xt=!1,n&&(n.disabled=!1,n.textContent="Confirm Sale")}}let wt=!1;function ke(t,e,n,o,i){if(wt)return;const r=window._nationStats,f=window._factionData,a=1+(vt(r,"inflation")-50)/100*.3,s=se({purchase_price:o,refurbish_count:i},a),u=Number(f?.corp_cash_reserves??0)>=s,x=document.getElementById("prop-modal-overlay"),E=document.getElementById("prop-modal-content"),S=u&&n<95;let A="";n>=95?A='<div style="color:var(--green);font-size:9px;margin-bottom:8px;">Property already at excellent condition ('+n+"%).</div>":u||(A='<div style="color:var(--red);font-size:9px;margin-bottom:8px;">Insufficient cash reserves.</div>'),E.innerHTML=`
    ...
    `,x.style.display="flex"}async function Ce(t,e,n,o){if(wt)return;wt=!0;const i=document.getElementById("prop-refurb-confirm");i&&(i.disabled=!0,i.textContent="Starting...");try{const r=window._corpFactionId,f=window._currentTick,l=window._factionData,a=window._nationStats;if(!r)throw new Error("No faction");const m=1+(vt(a,"inflation")-50)/100*.3,x=await ie(p,r,{id:t,purchase_price:e,refurbish_count:n,condition:o},f,m);if(!x.ok)throw new Error(x.error||"Refurbishment failed.");l&&(l.corp_cash_reserves=x.newCash),kt(),alert(`Refurbishment started! Duration: ${x.duration} ticks. Target condition: ${x.targetCondition}%. Cost: ${_(x.cost)}.`),location.reload()}catch(r){alert("Refurbishment failed: "+r.message)}finally{wt=!1,i&&(i.disabled=!1,i.textContent="Begin Refurbishment")}}function kt(){const t=document.getElementById("prop-modal-overlay");t&&(t.style.display="none")}window.showSellModal=we;window.confirmSellProperty=$e;window.showRefurbishModal=ke;window.confirmRefurbish=Ce;window.closePropModal=kt;window.showConvertModal=Te;window.confirmConvertProperty=Ie;let Lt=!1;async function Ee(t,e,n,o,i,r,f){if(!Lt&&confirm("Accept bid from "+n+`?

Bid Price: `+_(o)+`
Quality: `+i+`/100
Workers: `+r+`

This will award the contract. The project begins immediately.`)){Lt=!0;try{const{data:l}=await p.from("shard").select("current_tick").eq("name","Alpha Shard").single(),a=l?.current_tick||0,{error:s}=await p.from("contract_bids").update({status:"won"}).eq("id",e);if(s)throw s;const{error:m}=await p.from("contract_bids").update({status:"lost"}).eq("contract_id",t).neq("id",e);if(m)throw m;const{error:u}=await p.from("construction_contracts").update({status:"awarded",awarded_to_faction:f,awarded_at_tick:a}).eq("id",t);if(u)throw u;alert("Contract awarded to "+n+`!

Bid: `+_(o)+`
Project begins immediately.`),window._nationStats&&window._factionData&&Q&&await St(window._nationStats,window._nationStats?.name||"",window._factionData,Q)}catch(l){alert("Failed to accept bid: "+(l.message||l))}finally{Lt=!1}}}window.cpAcceptBid=Ee;function Me(t){const e=document.getElementById("cp-bid-"+t);e&&(e.style.display=e.style.display==="none"?"":"none")}window.cpToggleBid=Me;let qt="branch_office";function Te(t,e,n){const o=(d?.corp_subsector||"").toLowerCase(),i=o==="banking"?[["branch_office","Branch Office"]]:o==="investment"?[["trading_floor","Trading Floor"]]:o==="insurance"?[["claims_office","Claims Office"],["insurance_office","Insurance Office"]]:[];if(i.length===0)return;qt=i[0][0];const r=Math.round(n*.15),f=Math.floor(Math.random()*6)+4,l=document.getElementById("prop-modal-overlay"),a=document.getElementById("prop-modal-content");a.innerHTML=`
        ...
        <div style="display:flex;gap:4px;margin-bottom:10px;">
            ${i.map(([s,m])=>`<span onclick="_convertTargetType='${s}';document.querySelectorAll('.conv-opt').forEach(e=>e.style.background='transparent');this.style.background='rgba(138,106,170,0.15)'" class="conv-opt" style="flex:1;text-align:center;padding:6px;font-family:var(--font-mono);font-size:9px;font-weight:700;cursor:pointer;border:1px solid rgba(138,106,170,0.3);color:#8a6aaa;${s===qt?"background:rgba(138,106,170,0.15)":""}">${m}</span>`).join("")}
        </div>
        <div style="display:flex;justify-content:space-between;font-size:9px;font-family:var(--font-mono);margin-bottom:4px;">
