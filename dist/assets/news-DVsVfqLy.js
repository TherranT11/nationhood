import{_ as le}from"./supabase-client-BXEzLDpS.js";/* empty css                  */import{_ as re,i as oe}from"./common-Dsb3PwBk.js";import{t as ce}from"./utils-C2W-HleY.js";let b=null,w=null,M=[],B="all",T=null,O=null,P=null,A=null,q=!1,y="cruceran",U=!1;const G={cruceran:{key:"cruceran",name:"The Cruceran",tagline:"Truth in the service of the people",nations:["Avelia","Palvera","San Estrella","Montequilla","Melizea","Sangreza"],style:"cruceran"},continental:{key:"continental",name:"The Continental",tagline:"Where Ideas Converge",nations:["Calveth","Flandis","Vostia"],style:"continental"}};function de(e){for(const[s,n]of Object.entries(G))if(n.nations.some(a=>a.toLowerCase()===(e||"").toLowerCase()))return s;return"cruceran"}function Z(e,s){const n=G[e];return n?n.nations.some(a=>a.toLowerCase()===(s||"").toLowerCase()):!1}const pe=["Winter","Spring","Spring","Spring","Summer","Summer","Summer","Fall","Fall","Fall","Winter","Winter"];function S(e){const s=e%12,n=2e3+Math.floor(e/12),a=pe[s],l=s===0?n-1:n;return`${a} ${l}`}async function D(e,s){b=e,w=s,T=null,B="all",P=null;const n=document.getElementById("newspaper-root");if(!n)return;U||(y=de(s.nation?.name));const a=s.shard?.current_date||"[Month], [Year]",t=Z(y,s.nation?.name)?"Write Article":"Write Article (1 AP)",v=Object.entries(G).map(([p,h])=>`<option value="${p}" ${p===y?"selected":""}>${h.name}</option>`).join("");n.innerHTML=`<div class="newspaper-container nws-pub-${y}">
    ...
    </div>`,ve(),me(),ue(n),we(n),Q(n),Ee(n),$e(n);const d=document.getElementById("nws-pub-switcher");d&&d.addEventListener("change",async()=>{const p=d.value;if(p!==y){y=p,U=!0;try{await D(b,w)}catch(h){console.error("[News] Publication switch failed:",h)}}});const m=document.getElementById("nws-reward-badge");m&&(m.textContent="+3 Momentum (1st) / +1 subsequent"),await F(),Le()}function ve(){const e=document.getElementById("nws-modal-overlay"),s=document.getElementById("nws-write-article-btn"),n=document.getElementById("nws-modal-close"),a=document.getElementById("nws-article-body"),l=document.getElementById("nws-char-count"),i=document.getElementById("nws-article-image"),t=document.getElementById("nws-file-label-text"),v=document.getElementById("nws-image-preview"),d=document.getElementById("nws-image-preview-img");s&&s.addEventListener("click",()=>{R(),e.classList.add("active")}),n&&n.addEventListener("click",()=>{e.classList.remove("active"),R()});const m=document.getElementById("nws-remove-image-btn");m&&m.addEventListener("click",()=>{q=!0;const p=document.getElementById("nws-image-preview");p&&(p.style.display="none");const h=document.getElementById("nws-file-label-text");h&&(h.textContent="Click to select an image..."),m.style.display="none"}),a&&l&&a.addEventListener("input",()=>{const p=a.value.length;l.textContent=`${p} / 12000`,l.classList.toggle("nws-near-limit",p>=11500)}),i&&i.addEventListener("change",()=>{const p=i.files[0];if(!p){t.textContent="Click to select an image...",v.style.display="none";return}const h=2*1024*1024;if(p.size>h){I("Image must be under 2MB."),i.value="",t.textContent="Click to select an image...",v.style.display="none";return}t.textContent=p.name;const o=new FileReader;o.onload=c=>{d.src=c.target.result,v.style.display="block"},o.readAsDataURL(p)})}function I(e){const s=document.getElementById("nws-form-error");s&&(s.textContent=e,s.style.display="block",setTimeout(()=>{s.style.display="none"},5e3))}function V(e){const s=document.getElementById("nws-form-success");s&&(s.textContent=e,s.style.display="block",setTimeout(()=>{s.style.display="none"},5e3))}function me(){const e=document.getElementById("nws-submit-btn");e&&e.addEventListener("click",async()=>{if(e.disabled)return;const s=document.getElementById("nws-article-title").value.trim(),n=document.getElementById("nws-article-author").value.trim(),a=document.getElementById("nws-article-category").value,l=document.getElementById("nws-article-body").value.trim(),t=document.getElementById("nws-article-image").files[0]||null,v=!Z(y,w?.nation?.name);if(!s)return I("Please enter a headline.");if(!n)return I("Please enter a writer name.");if(!a)return I("Please select a category.");if(!l)return I("Please write an article body.");if(l.length>12e3)return I("Article body must be 12000 characters or fewer.");const d=!!A;e.disabled=!0,e.textContent=d?"Updating...":"Publishing...";try{const{nation:m,faction:p,shard:h}=w;if(d){let o;t?o=await K(m.id,t):q&&(o=null);const c={headline:s,author_name:n,body:l,category:a};o!==void 0&&(c.image_url=o);const{error:g}=await b.from("player_articles").update(c).eq("id",A).eq("author_faction_id",p.id);if(g)throw g;V("Article updated!")}else{if(v){const{deductAP:u}=await re(async()=>{const{deductAP:N}=await import("./config-fKhFNVuq.js");return{deductAP:N}},[]),E=await u(b,p.id,1);if(!E.success){I("Not enough AP to post on another publication (need 1 AP).");return}p.action_points=E.newAp}let o=null;t&&(o=await K(m.id,t));const{error:c}=await b.from("player_articles").insert({nation_id:m.id,author_faction_id:p.id,author_name:n,headline:s,body:l,category:a,image_url:o,status:"published",published_tick:h?.current_tick||0,publication:y});if(c)throw c;const g=h?.current_tick||0;let f=1;const{data:_,error:k}=await b.from("player_articles").select("id").eq("author_faction_id",p.id).eq("published_tick",g);k&&console.error("[News] Failed to count articles this tick:",k),!k&&(!_||_.length<=1)&&(f=3);const H=`News article published (+${f})`;let C="Article published!";try{const{error:u}=await b.rpc("adjust_momentum",{p_faction_id:p.id,p_delta:f,p_label:H,p_tick:g});u?(console.error("[News] Momentum reward failed:",u),C="Article published! (Momentum reward failed)"):C=`Article published! +${f} Momentum.`}catch(u){console.error("[News] Momentum reward error:",u),C="Article published! (Momentum reward failed)"}sessionStorage.removeItem("nationhood_state"),V(C)}R(),setTimeout(()=>{document.getElementById("nws-modal-overlay").classList.remove("active")},1500),await F()}catch(m){console.error(`[News] Article ${d?"update":"submission"} failed:`,m),I(`Failed to ${d?"update":"publish"} article. Please try again.`)}finally{e.disabled=!1,e.textContent=A?"Update Article":"Publish Article"}})}async function K(e,s){const n=s.name.split(".").pop()||"png",a=`player-articles/${e}/${Date.now()}.${n}`,{error:l}=await b.storage.from("public-assets").upload(a,s,{contentType:s.type,upsert:!0});if(l)throw l;const{data:i}=b.storage.from("public-assets").getPublicUrl(a);return i?.publicUrl||null}function x(e){return!w?.faction||e.author_faction_id!==w.faction.id?"":`<button class="nws-delete-btn" data-article-id="${e.id}" title="Delete article">&times;</button>`}function L(e){return!w?.faction||e.author_faction_id!==w.faction.id?"":`<button class="nws-edit-btn" data-edit-id="${e.id}" title="Edit article">&#9998;</button>`}function ue(e){e.addEventListener("click",async s=>{const n=s.target.closest(".nws-delete-btn");if(!n)return;const a=n.dataset.articleId;if(!(!a||!confirm("Delete this article?"))){n.disabled=!0,n.textContent="...";try{const{error:l}=await b.from("player_articles").delete().eq("id",a).eq("author_faction_id",w.faction.id);if(l)throw l;await F()}catch(l){console.error("[News] Failed to delete article:",l),n.disabled=!1,n.textContent="×"}}})}function R(){A=null,q=!1,document.getElementById("nws-article-title").value="",document.getElementById("nws-article-author").value="",document.getElementById("nws-article-category").value="",document.getElementById("nws-article-body").value="";const e=document.getElementById("nws-article-image");e&&(e.value="");const s=document.getElementById("nws-file-label-text");s&&(s.textContent="Click to select an image...");const n=document.getElementById("nws-image-preview");n&&(n.style.display="none");const a=document.getElementById("nws-char-count");a&&(a.textContent="0 / 12000",a.classList.remove("nws-near-limit","nws-ap-qualified"));const l=document.getElementById("nws-remove-image-btn");l&&(l.style.display="none");const i=document.querySelector(".nws-modal-header h3");i&&(i.textContent="Write Article");const t=document.getElementById("nws-submit-btn");t&&(t.textContent="Publish Article")}function he(e){A=e.id,q=!1,document.getElementById("nws-article-title").value=e.headline||"",document.getElementById("nws-article-author").value=e.author_name||"",document.getElementById("nws-article-category").value=e.category||"",document.getElementById("nws-article-body").value=e.body||"";const s=(e.body||"").length,n=document.getElementById("nws-char-count");n&&(n.textContent=`${s} / 12000`,n.classList.toggle("nws-near-limit",s>=11500));const a=document.getElementById("nws-image-preview"),l=document.getElementById("nws-image-preview-img"),i=document.getElementById("nws-file-label-text"),t=document.getElementById("nws-remove-image-btn"),v=document.getElementById("nws-article-image");v&&(v.value=""),e.image_url?(l&&(l.src=e.image_url),a&&(a.style.display="block"),i&&(i.textContent="Current image (select new to replace)"),t&&(t.style.display="inline")):(a&&(a.style.display="none"),i&&(i.textContent="Click to select an image..."),t&&(t.style.display="none"));const d=document.querySelector(".nws-modal-header h3");d&&(d.textContent="Edit Article");const m=document.getElementById("nws-submit-btn");m&&(m.textContent="Update Article"),document.getElementById("nws-modal-overlay").classList.add("active")}function we(e){e.addEventListener("click",s=>{const n=s.target.closest(".nws-edit-btn");if(!n)return;s.stopPropagation();const a=n.dataset.editId,l=M.find(i=>String(i.id)===String(a));l&&he(l)})}function Q(e){O&&e.removeEventListener("click",O),O=s=>{if(s.target.closest(".nws-delete-btn, .nws-edit-btn, .nws-write-btn, .nws-modal-overlay, button, a, input, select, textarea"))return;const n=s.target.closest("[data-article-id]");if(!n)return;const a=n.dataset.articleId,l=M.find(i=>String(i.id)===String(a));if(l)try{X(e,l)}catch(i){console.error("[News] Failed to open article:",i)}},e.addEventListener("click",O)}function X(e,s){const n=s.body||"",a=s.published_tick!=null?ce(s.published_tick):w?.shard?.current_date||"[Month], [Year]",i=!!P?"&larr; Back to Edition":"&larr; Back to Front Page",t=se(n),v=s.image_url?`<div class="nws-reader-image">
        ...
        <div class="nws-footer">
    ...
    </div>`;const d=P,m=d?()=>ie(d.root,d.articles,d.dateLabel):()=>D(b,w);document.getElementById("nws-back-btn")?.addEventListener("click",m),document.getElementById("nws-back-btn-bottom")?.addEventListener("click",m),e.scrollTop=0}function r(e){const s=document.createElement("div");return s.textContent=e,s.innerHTML}function ee(e,s=1200){if(e.length<=s)return se(e);let n=e.lastIndexOf(" ",s);n<s*.5&&(n=s);const l=e.substring(0,n).split(/\n\n+/).filter(t=>t.trim());return l.map((t,v)=>`<p class="${v===0?"nws-drop-cap":""}">${j(r(t.trim()))}${v===l.length-1?"...":""}</p>`).join("")+'<p class="nws-read-more">Read More &rarr;</p>'}function j(e){return e.replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>").replace(/\*(.+?)\*/g,"<em>$1</em>").replace(/__(.+?)__/g,"<u>$1</u>")}function se(e){const s=e.split(/\n\n+/).filter(n=>n.trim());return s.length<=1?`<p class="nws-drop-cap">${j(r(e))}</p>`:s.map((n,a)=>`<p class="${a===0?"nws-drop-cap":""}">${j(r(n.trim()))}</p>`).join("")}function $(e){return e==="elections"&&(e="politics"),{politics:"Politics",economy:"Business",international:"International",social:"Social",entertainment:"Entertainment",sports:"Sports",opinion:"Opinion"}[e]||e}function ne(e){return e==="elections"&&(e="politics"),{politics:"linear-gradient(135deg,#2a1a2a,#1a0d1a)",economy:"linear-gradient(135deg,#2a1a0a,#1a0d00)",international:"linear-gradient(135deg,#0a1a2a,#001a2a)",social:"linear-gradient(135deg,#1a2a1a,#0d1a0d)",entertainment:"linear-gradient(135deg,#2a2a0a,#1a1a00)",sports:"linear-gradient(135deg,#2a0a0a,#1a0000)",opinion:"linear-gradient(135deg,#2a2a2a,#1a1a1a)"}[e]||"linear-gradient(135deg,#1a1a1a,#0d0d0d)"}async function te(){if(T)return T;const e=w?.shard,s=w?.nation;if(!e||!s)return[s?.id].filter(Boolean);const{data:n,error:a}=await b.from("nations").select("id").eq("shard_id",e.id);return a&&console.error("[News] Failed to fetch shard nations:",a),T=n&&n.length>0?n.map(l=>l.id):[s.id],T}async function F(){if(!(!b||!w))try{const e=await te();let s=b.from("player_articles").select("*").in("nation_id",e).eq("status","published").order("created_at",{ascending:!1});y!=="cruceran"?s=s.or(`publication.eq.${y},publication.eq.international`):s=s.or("publication.eq.cruceran,publication.is.null,publication.eq.international");const{data:n,error:a}=await s;if(a){console.error("[News] Failed to load articles:",a);return}if(!n||n.length===0)return;const l=w.shard?.current_tick??0,i=S(l),t=n.filter(u=>S(u.published_tick??0)===i),v=n.filter(u=>S(u.published_tick??0)!==i),d=new Set(t.map(u=>u.category)),m=[],p=new Set;for(const u of v)u.category!=="opinion"&&(d.has(u.category)||(p.has(u.category)||p.add(u.category),m.push(u)));const h=n.filter(u=>u.category==="opinion").slice(0,4),o=[...t.filter(u=>u.category!=="opinion"),...m,...h];if(o.length===0)return;M=o;const c=B&&B!=="all"?o.filter(u=>(u.category==="elections"?"politics":u.category)===B):o;if(c.length===0&&B!=="all"){const u=`<p class="nws-placeholder" style="text-align:center;padding:40px;grid-column:1/-1;">No ${$(B)} articles in this edition.</p>`,E=document.getElementById("nws-lead-section");E&&(E.innerHTML=u);const N=document.getElementById("nws-secondary-grid");N&&(N.innerHTML="");const Y=document.querySelector(".nws-opinion-grid");Y&&(Y.innerHTML="");const z=document.querySelector(".nws-bottom-left");z&&(z.innerHTML="");return}const f=[...c.filter(u=>u.category!=="opinion")].sort((u,E)=>(E.published_tick??0)-(u.published_tick??0)),_=f[0],k=f.slice(1,4),H=f.slice(4,7),C=[...o].sort((u,E)=>new Date(E.created_at)-new Date(u.created_at)).slice(0,5);y==="continental"?_e(_,k,H,h.slice(0,4),C):(_&&ge(_,k),be(H),ye(h.slice(0,4)),fe(C))}catch(e){console.error("[News] Error loading articles:",e)}}function ge(e,s){const n=document.getElementById("nws-lead-section");if(!n||!e)return;const a=s.length>0?s.map(d=>`
            <div class="nws-sidebar-story" data-article-id="${d.id}">
                ...
                <div class="nws-byline"><span class="nws-author">${r(d.author_name)}</span><span class="nws-dot">&middot;</span><span>${w?.shard?.current_date||"—"}</span></div>
            </div>
    ...
    `}function be(e){const s=document.getElementById("nws-secondary-grid");if(!s||e.length===0)return;const n=[...e],a=["Crisis","Election","Business"],l=[0,1,2].map(i=>{const t=n[i];if(t){const v=t.image_url?`<img src="${r(t.image_url)}" alt="${r(t.headline)}" style="width:100%;height:100%;object-fit:cover;">`:`<div class="nws-img-ph" style="background:${ne(t.category)};">${r($(t.category))}</div>`;return`<div class="nws-sec-story" data-article-id="${t.id}">
                ...
                <div class="nws-byline"><span class="nws-author">${r(t.author_name)}</span><span class="nws-dot">&middot;</span><span>${w?.shard?.current_date||"—"}</span></div>
        ...
        <div class="nws-brief-row" data-article-id="${t.id}">
            ...
            <div class="nws-brief-text">
                <strong>${r(t.headline)}${L(t)}${x(t)}</strong>
    ...
    `).join(""),l=5-e.length;let i="";for(let t=0;t<l;t++)i+=`
        ...
        `;s.innerHTML=n+a+i}function ye(e){const s=document.querySelector(".nws-opinion-grid");if(!s||e.length===0)return;const n=[0,1,2,3].map(a=>{const l=e[a];if(l){const i=(l.body||"").length>80?(l.body||"").substring(0,80)+"...":l.body||"";return`<div class="nws-op-card" data-article-id="${l.id}">
                ...
                <div class="nws-op-author nws-placeholder">[Columnist] &mdash; [Topic]</div>
                <div class="nws-op-headline nws-placeholder">&ldquo;[Opinion headline will appear here.]&rdquo;</div>
            </div>`}).join("");s.innerHTML=n}function _e(e,s,n,a,l){const i=document.getElementById("nws-main-content")||document.querySelector(".nws-main-content");if(!i)return;const t=c=>(c||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"),v=c=>t((c.body||"").replace(/\n+/g," ").substring(0,200))+((c.body||"").length>200?"...":""),d=c=>t((c.body||"").replace(/\n+/g," ").substring(0,120))+((c.body||"").length>120?"...":""),m=c=>t($(c.category)),p=w?.shard?.current_date||"—",h={politics:"--politics",economy:"--economy",social:"--social",international:"--intl",entertainment:"--culture",science:"--science"};let o="";if(!e&&s.length===0&&n.length===0&&a.length===0&&l.length===0){o+='<div style="text-align:center;padding:60px 20px;color:#9e9b95;font-family:Outfit,sans-serif;">',o+='<div style="font-size:2rem;margin-bottom:12px;opacity:0.3;">📰</div>',o+='<div style="font-size:1rem;font-weight:600;">No articles yet</div>',o+='<div style="font-size:0.85rem;margin-top:6px;">Be the first to write for The Continental.</div>',o+="</div>",i.innerHTML=o;return}if(e){const c=e.image_url?`<img src="${t(e.image_url)}" alt="" style="width:100%;height:100%;object-fit:cover;">`:"";o+=`<div class="ct-hero" data-article-id="${e.id}">
            ...
            <div class="ct-section-divider__line"></div>
        </div>`,o+='<div class="ct-story-grid">';for(const c of s){const g=h[c.category]||"",f=c.image_url?`<img src="${t(c.image_url)}" alt="" style="width:100%;height:100%;object-fit:cover;">`:"";o+=`<div class="ct-card" data-article-id="${c.id}">
                ...
                <div class="ct-card__body">
                    ...
                    <p class="ct-card__summary">${d(c)}</p>
            ...
            <div class="ct-analysis-band__grid">`;for(const c of n)o+=`<div class="ct-analysis-story" data-article-id="${c.id}">
                <div class="ct-analysis-story__section">${m(c)}</div>
                <h2 class="ct-analysis-story__headline">${t(c.headline)}</h2>
                <p class="ct-analysis-story__summary">${d(c)}</p>
                <div class="ct-analysis-story__meta"><strong>${t(c.author_name)}</strong> &middot; ${p}</div>
            </div>`;o+='</div></div></div><div class="nws-main-content">'}if(a.length>0||l.length>0){o+=`<div class="ct-section-divider">
            <span class="ct-section-divider__label">Analysis &amp; Opinion</span>
            <div class="ct-section-divider__line"></div>
        </div>`,o+='<div class="ct-two-col">',o+='<div class="ct-analysis-list">';for(let c=0;c<a.length;c++){const g=a[c];o+=`<div class="ct-analysis-item" data-article-id="${g.id}">
            ...
            </div>`}o+="</div>",o+='<div class="ct-sidebar">',o+='<div class="ct-sidebar__section"><div class="ct-sidebar__section-title">Also in This Edition</div>';for(const c of l)o+=`<div class="ct-sidebar-brief" data-article-id="${c.id}">
    ...
    </div>`,i.innerHTML=o,i.querySelectorAll("[data-article-id]").forEach(c=>{c.style.cursor="pointer",c.addEventListener("click",g=>{if(g.target.closest(".nws-edit-btn, .nws-delete-btn"))return;const f=c.dataset.articleId,_=M.find(k=>String(k.id)===String(f));_&&X(document.getElementById("newspaper-root"),_)})})}function $e(e){const s=e.querySelectorAll(".nws-nav-item[data-category]");s.forEach(n=>{n.addEventListener("click",()=>{s.forEach(a=>a.classList.remove("active")),n.classList.add("active"),B=n.dataset.category,F()})})}function Ee(e){const s=document.getElementById("nws-nav-archives");s&&s.addEventListener("click",()=>ae(e))}async function ae(e){if(!(!b||!w))try{const s=w.shard?.current_tick??0,n=w.shard?.current_date||"[Month], [Year]",a=await te(),{data:l,error:i}=await b.from("player_articles").select("*").in("nation_id",a).eq("status","published").order("published_tick",{ascending:!1});if(i){console.error("[News] Failed to load archive articles:",i);return}const t={};for(const p of l||[]){const h=p.published_tick??0,o=S(h);t[o]||(t[o]={label:o,maxTick:h,articles:[]}),t[o].articles.push(p),h>t[o].maxTick&&(t[o].maxTick=h)}const v=Object.values(t).sort((p,h)=>h.maxTick-p.maxTick),d=S(s),m=v.length>0?v.map(p=>{const h=p.label===d?' <span class="nws-archive-current">Current</span>':"";return`<div class="nws-archive-month" data-archive-season="${r(p.label)}">
        ...
        </div>`,document.getElementById("nws-back-btn")?.addEventListener("click",()=>D(b,w)),e.querySelectorAll("[data-archive-season]").forEach(p=>{p.addEventListener("click",()=>{const h=p.dataset.archiveSeason,o=t[h];o&&ie(e,o.articles,o.label)})}),e.scrollTop=0}catch(s){console.error("[News] Error loading archives:",s)}}function ie(e,s,n){M=s;const a=s.filter(f=>f.category==="opinion"),i=[...s.filter(f=>f.category!=="opinion")].sort((f,_)=>(_.body||"").length-(f.body||"").length),t=i[0],v=i.slice(1,4),d=i.slice(4,7),m=i.slice(7,12),p=a.slice(0,4),h=t?ke(t,v,n):'<p class="nws-placeholder" style="padding:40px;text-align:center;">[No articles in this edition]</p>',o=Ie(d),c=Ce(p),g=Be(m);e.innerHTML=`<div class="newspaper-container">
        <!-- TOP RIBBON -->
        <div class="nws-top-ribbon">
            <div class="nws-top-ribbon-inner">
                <span>${r(n)}</span>
    ...
    </div>`,document.getElementById("nws-back-to-archives")?.addEventListener("click",()=>ae(e)),P={root:e,articles:s,dateLabel:n},Q(e),e.scrollTop=0}function ke(e,s,n){const a=s.length>0?s.map(d=>`
            <div class="nws-sidebar-story" data-article-id="${d.id}">
                <span class="nws-section-tag">${r($(d.category))}</span>
                <h3 class="nws-sidebar-headline">${r(d.headline)}</h3>
                <p class="nws-sidebar-deck">${r((d.body||"").replace(/\n+/g," ").substring(0,120))}${(d.body||"").length>120?"...":""}</p>
    ...
    `}function Ie(e){return e.length===0?"":e.map(s=>{const n=s.image_url?`<img src="${r(s.image_url)}" alt="${r(s.headline)}" style="width:100%;height:100%;object-fit:cover;">`:`<div class="nws-img-ph" style="background:${ne(s.category)};">${r($(s.category))}</div>`;return`<div class="nws-sec-story" data-article-id="${s.id}">
    ...
    `).join("")}function W(e){return e.w*3+e.d}function Te(e){return Object.values(e).sort((s,n)=>{const a=W(n)-W(s);return a!==0?a:n.w-s.w})}function Ae(e){return`<span class="vln-form-dot" style="background:${{W:"#1a4a1a",D:"#8a6a20",L:"#8b1a1a"}[e]||"#999"}"></span>`}function Se(e,s){return e<=3?"vln-pos-top":e>=s-2?"vln-pos-bottom":""}function J(e,s={}){const n=Te(e),a=s.muted||!1,l=n.length;let i=`<div class="vln-table${a?" vln-table-muted":""}">
        <div class="vln-thead">
        ...
        </div>`;return n.forEach((t,v)=>{const d=v+1,m=Se(d,l),p=d===3||d===7?" vln-zone-break":"",h=W(t),o=a?"":t.form.map(c=>Ae(c)).join("");i+=`<div class="vln-row${p}">
            ...
            ${a?"":`<span class="vln-col-form">${o}</span>`}
