import{_ as T}from"./supabase-client-BXEzLDpS.js";import{e as f}from"./utils-C2W-HleY.js";import{i as Bt}from"./messaging-5qyQ6ziq.js";import{c as Pt,a as Oe,E as fe,b as $e,d as st,e as Ot,f as Dt,h as et}from"./equipment-DsuDdEne.js";const ot={LOW:"#c84",STD:"#ca5",HIGH:"#5c5"},J=[{key:"concrete",name:"Concrete",category:"RAW",tiers:{LOW:{requirements:[]},STD:{requirements:[{stat:"manufacturing_output",min:20}]},HIGH:{requirements:[{stat:"manufacturing_output",min:50},{stat:"higher_education",min:40}]}},priceDrivers:["manufacturing_output","inflation","fuel_prices","urbanization"]},{key:"steel",name:"Steel",category:"RAW",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:10}]},STD:{requirements:[{stat:"manufacturing_output",min:35},{stat:"rare_minerals",min:20}]},HIGH:{requirements:[{stat:"manufacturing_output",min:60},{stat:"rare_minerals",min:40},{stat:"higher_education",min:45}]}},priceDrivers:["manufacturing_output","rare_minerals","inflation","fuel_prices"]},{key:"lumber",name:"Lumber",category:"RAW",tiers:{LOW:{requirements:[{stat:"arable_land",min:10}]},STD:{requirements:[{stat:"arable_land",min:30},{stat:"physical_infrastructure",min:20}]},HIGH:{requirements:[{stat:"arable_land",min:50},{stat:"manufacturing_output",min:30}]}},priceDrivers:["arable_land","physical_infrastructure","inflation"]},{key:"aggregate",name:"Aggregate",category:"RAW",tiers:{LOW:{requirements:[]},STD:{requirements:[{stat:"rare_minerals",min:15},{stat:"physical_infrastructure",min:20}]},HIGH:{requirements:[{stat:"rare_minerals",min:35},{stat:"manufacturing_output",min:25}]}},priceDrivers:["rare_minerals","physical_infrastructure","inflation"]},{key:"em",name:"E&M Systems",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:15}]},STD:{requirements:[{stat:"manufacturing_output",min:40},{stat:"digital_infrastructure",min:25}]},HIGH:{requirements:[{stat:"manufacturing_output",min:55},{stat:"digital_infrastructure",min:50},{stat:"energy_generation",min:40}]}},priceDrivers:["manufacturing_output","digital_infrastructure","inflation","energy_generation"]},{key:"glass",name:"Glass & Facades",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:20}]},STD:{requirements:[{stat:"manufacturing_output",min:40},{stat:"standard_of_living",min:35}]},HIGH:{requirements:[{stat:"manufacturing_output",min:60},{stat:"digital_infrastructure",min:40},{stat:"higher_education",min:50}]}},priceDrivers:["manufacturing_output","standard_of_living","inflation"]},{key:"asphalt",name:"Asphalt",category:"RAW",tiers:{LOW:{requirements:[{stat:"oil_and_gas",min:10}]},STD:{requirements:[{stat:"oil_and_gas",min:30},{stat:"manufacturing_output",min:25}]},HIGH:{requirements:[{stat:"oil_and_gas",min:45},{stat:"manufacturing_output",min:40},{stat:"physical_infrastructure",min:40}]}},priceDrivers:["oil_and_gas","manufacturing_output","inflation","fuel_prices"]},{key:"heavy",name:"Heavy Machinery Parts",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:40},{stat:"rare_minerals",min:30}]},STD:{requirements:[{stat:"manufacturing_output",min:60},{stat:"rare_minerals",min:45},{stat:"higher_education",min:40}]},HIGH:{requirements:[{stat:"manufacturing_output",min:75},{stat:"rare_minerals",min:60},{stat:"higher_education",min:55},{stat:"digital_infrastructure",min:45}]}},priceDrivers:["manufacturing_output","rare_minerals","higher_education","digital_infrastructure"]}];function ae(e,t,i){const n=J.find(c=>c.key===e);if(!n)return{available:!1,failedStat:"unknown_material"};const a=n.tiers[t];if(!a)return{available:!1,failedStat:"unknown_tier"};for(const c of a.requirements){const r=Number(i?.[c.stat]??0);if(r<c.min)return{available:!1,failedStat:c.stat,failedMin:c.min,nationValue:r}}return{available:!0}}function We(e,t,i){const a={concrete:{LOW:200,STD:300,HIGH:500},steel:{LOW:400,STD:700,HIGH:1200},lumber:{LOW:80,STD:130,HIGH:200},aggregate:{LOW:40,STD:60,HIGH:100},em:{LOW:400,STD:700,HIGH:1200},glass:{LOW:300,STD:500,HIGH:900},asphalt:{LOW:120,STD:200,HIGH:350},heavy:{LOW:800,STD:1400,HIGH:2400}}[e]?.[t];if(!a)return 0;const c=J.find(o=>o.key===e);if(!c)return a;let r=1;for(const o of c.priceDrivers){const l=Number(i?.[o]??50);o==="inflation"||o==="fuel_prices"?r*=1+(l-50)/200:r*=1-(l-50)/250}return r=Math.max(.4,Math.min(2.5,r)),Math.round(a*r)}function rt(e,t,i){const a={concrete:{LOW:5e3,STD:3e3,HIGH:1e3},steel:{LOW:2e3,STD:1500,HIGH:500},lumber:{LOW:8e3,STD:4e3,HIGH:1500},aggregate:{LOW:15e3,STD:6e3,HIGH:2e3},em:{LOW:1e3,STD:700,HIGH:300},glass:{LOW:1500,STD:800,HIGH:300},asphalt:{LOW:4e3,STD:2e3,HIGH:800},heavy:{LOW:400,STD:200,HIGH:80}}[e]?.[t]||0,r=J.find(s=>s.key===e)?.priceDrivers?.[0],l=.3+(r?Number(i?.[r]??50):50)/50*.7;return Math.round(a*l)}const Fe=["LOW","STD","HIGH"],De={LOW:"Low",STD:"Standard",HIGH:"High"};let pe=[],m=null,x=null,B=null,le=[],ue={},F=[],D={},ze=-1,P="concrete",O="STD",ne=500,V=[],Re=0,G="trucks",Q=0,K=1,Z=[],se=null,ge=[],He=null,ye=null,je="ALL",Ge="TIMELINE";function L(e){return Math.abs(e)>=1e6?"$"+(e/1e6).toFixed(1)+"M":Math.abs(e)>=1e3?"$"+(e/1e3).toFixed(0)+"k":"$"+e}function zt(e){if(e>=12){const t=Math.floor(e/12),i=e%12;return i>0?t+"y "+i+"mo":t+"y"}return e+" ticks"}function U(e){return Math.abs(e)>=1e9?"$"+(e/1e9).toFixed(1)+"B":Math.abs(e)>=1e6?"$"+(e/1e6).toFixed(0)+"M":Math.abs(e)>=1e3?"$"+(e/1e3).toFixed(0)+"k":"$"+e}function xe(e){return e==="civil_engineering"?"CIVIL":e==="industrial"?"INDUSTRIAL":e==="mega_project"?"MEGA":e?.toUpperCase()||"—"}function lt(e){return e==="civil_engineering"?"light":e==="industrial"?"heavy":e==="mega_project"?"mega":"light"}function Rt(){ye&&clearInterval(ye),ye=setInterval(()=>{if(!He)return;const e=He-Date.now();if(e<=0){document.getElementById("tick-countdown").textContent="Tick due...",clearInterval(ye);return}const t=Math.floor(e/36e5),i=Math.floor(e%36e5/6e4),n=Math.floor(e%6e4/1e3);document.getElementById("tick-countdown").textContent=t+"h "+i+"m "+n+"s"},1e3)}function Ht(){document.body.classList.toggle("light-mode");const e=document.getElementById("theme-toggle");e.textContent=document.body.classList.contains("light-mode")?"Dark":"Light"}function jt(e,t){e==="type"&&(je=t),e==="sort"&&(Ge=t),document.querySelectorAll(`.filter-pill[data-filter="${e}"]`).forEach(i=>{i.classList.toggle("active",i.dataset.value===t)}),dt()}function ct(e){return!(!m||e.sector==="mega_project"&&m.corp_subsector!=="Megaprojects"||e.sector==="industrial"&&!["Heavy Infrastructure","Megaprojects"].includes(m.corp_subsector))}function dt(){const e=document.getElementById("oc-list");let t=[...le];if(je==="GOVERNMENT"?t=t.filter(a=>a.issuer_type==="GOVERNMENT"):je==="PRIVATE"&&(t=t.filter(a=>a.issuer_type==="PRIVATE")),Ge==="TIMELINE"&&t.sort((a,c)=>(a.timeline_ticks||0)-(c.timeline_ticks||0)),Ge==="BUDGET"&&t.sort((a,c)=>(c.budget_ceiling||0)-(a.budget_ceiling||0)),document.getElementById("oc-count").textContent=t.length+" AVAILABLE",t.length===0){e.innerHTML=`
            ...
            </div>`}e.innerHTML=n,e.querySelectorAll(".oc-item:not(.locked)").forEach(a=>{a.addEventListener("click",()=>{const c=a.dataset.contractId,r=le.find(o=>o.id===c);r&&pt(r)})})}let _e=null;function pt(e){_e=e;const t=document.getElementById("cd-overlay"),i=e.contract_type==="GOVERNMENT",n=i?"gov":"private",a=(x?.name||m.nation||"—").toUpperCase(),c=ct(e);document.getElementById("cd-header-left").innerHTML=`
    ...
    `;const r=document.getElementById("cd-blueprint");e.blueprint_svg?(r.innerHTML=e.blueprint_svg,r.style.display=""):(r.innerHTML=oi(e),r.style.display="");const o=e.permits_required||[],l=e.equipment_required||[],s=e.materials_estimated||[];let d="var(--teal)";e.spec_category==="Heavy Infrastructure"&&(d="var(--orange)"),e.spec_category==="Megaproject"&&(d="var(--red)");let p=L(e.budget),u=e.timeline_months+" Months",v="";v+=`
        <div class="cd-issue">
            ...
            </div>`),v+='<div class="cd-details">',e.project_type&&(v+=X("Type",e.project_type)),e.project_subtype&&(v+=X("Sub-Type",e.project_subtype)),v+=X("Specialization",e.spec_category,d),v+=X("Total Budget",p,"var(--green)"),v+=X("Timeline",u),v+=X("Nation",x?.name||m.nation||"—"),e.region&&(v+=X("Region",e.region)),v+="</div>",o.length>0&&(v+=`
            <div class="cd-items">
            ...
            </div>`),document.getElementById("cd-content").innerHTML=v;const _=o.filter(h=>h.status==="approved").length,g=o.length-_,w=l.filter(h=>h.owned).length,b=l.length-w;let $="";l.length>0&&(b===0?$+='<span class="cd-footer__badge ok">ALL EQUIPMENT MET</span>':$+=`<span class="cd-footer__badge bad">${b} EQUIPMENT MISSING</span>`),o.length>0&&(g===0?$+='<span class="cd-footer__badge ok">ALL PERMITS APPROVED</span>':$+=`<span class="cd-footer__badge warn">${g} PERMITS PENDING</span>`);const A=c,y=(m.action_points??0)>=2;document.getElementById("cd-footer").innerHTML=`
    ...
    `,t.classList.add("open"),document.body.style.overflow="hidden"}function mt(e){e&&e.target&&e.target!==document.getElementById("cd-overlay")||(document.getElementById("cd-overlay").classList.remove("open"),document.body.style.overflow="",_e=null)}const Gt=[{key:"concrete",name:"Concrete",unit:"units"},{key:"steel",name:"Steel",unit:"units"},{key:"lumber",name:"Lumber",unit:"units"},{key:"aggregate",name:"Aggregate",unit:"units"},{key:"em_systems",name:"E&M Systems",unit:"units"},{key:"glass_facades",name:"Glass & Facades",unit:"units"},{key:"asphalt",name:"Asphalt",unit:"units"},{key:"heavy_parts",name:"Heavy Machinery Parts",unit:"units"}],Ut=[{key:"work_trucks",name:"Work Trucks",tier:1},{key:"excavators",name:"Excavators",tier:1},{key:"bulldozers",name:"Bulldozers",tier:1},{key:"concrete_mixers",name:"Concrete Mixers",tier:1},{key:"tower_cranes",name:"Tower Cranes",tier:2},{key:"heavy_haulers",name:"Heavy Haulers",tier:2},{key:"pile_drivers",name:"Pile Drivers",tier:2},{key:"asphalt_plants",name:"Asphalt Plants",tier:2}],ut={LOW:.7,STANDARD:1,HIGH:1.4},ft={LOW:35,STANDARD:65,HIGH:90},Te=15;let z=null;function Wt(e){if(!e)return;const t=e.required_materials||{},i=e.required_equipment||[],n=e.required_workforce||{},a={concrete:18e4,steel:25e4,lumber:12e4,aggregate:8e4,em_systems:32e4,glass_facades:28e4,asphalt:14e4,heavy_parts:4e5},c=Gt.filter(d=>t[d.key]>0).map(d=>({...d,qty:t[d.key],basePrice:a[d.key]||2e5,grade:d.key==="aggregate"?"LOW":"STANDARD",highDisabled:!1})),r=Ut.filter(d=>i.includes(d.key)).map(d=>({...d,owned:(V||[]).some(p=>p.equipment_key===d.key&&p.quantity>0)})),o=(n.general||100)+(n.skilled||20),l=e.budget_ceiling||1e8,s=Math.round(l*.03);z={contract:e,budgetCeiling:l,materials:c,laborCount:o,laborRate:15200,estimatedTicks:e.timeline_ticks||8,equipment:r,permits:[],overhead:s,markupPct:15,competitors:[],playerRep:m?.standing||50,requiredWorkforce:n},document.getElementById("bid-title").textContent="BID ASSEMBLY",document.getElementById("bid-subtitle").textContent=(e.name||"Contract")+" — "+xe(e.sector)+" — "+(e.issuer_name||"Government"),document.getElementById("bid-overlay").classList.add("open"),document.body.style.overflow="hidden",he()}function vt(e){e&&e.target!==document.getElementById("bid-overlay")||(document.getElementById("bid-overlay").classList.remove("open"),document.body.style.overflow="",z=null)}function N(e){return Math.abs(e)>=1e9?"$"+(e/1e9).toFixed(2)+"B":Math.abs(e)>=1e6?"$"+(e/1e6).toFixed(2)+"M":Math.abs(e)>=1e3?"$"+(e/1e3).toFixed(0)+"k":"$"+e.toLocaleString()}function Ft(e,t){if(!z)return;const i=z.materials[e];t==="HIGH"&&i.highDisabled||(i.grade=t,he())}function Vt(e){z&&(z.laborCount=e,he())}function Yt(e){z&&(z.markupPct=Number(e),he())}function he(){if(!z)return;const e=z;let t=0;for(const C of e.materials)C.lineCost=Math.round(C.qty*C.basePrice*ut[C.grade]),t+=C.lineCost;const i=Math.round(e.laborCount*e.laborRate*e.estimatedTicks),n=Math.round(e.equipment.filter(C=>C.owned).length*12e3*e.estimatedTicks);let a=0;const c=e.overhead,r=t+i+n+a+c,o=Math.round(r*e.markupPct/100),l=r+o,s=l>e.budgetCeiling,d=o,p=Math.round(e.materials.reduce((C,R)=>C+ft[R.grade],0)/e.materials.length),u=p>=80?"STRONG":p>=60?"PROMISING":p>=40?"UNCERTAIN":"POOR",v=p>=80?"var(--green)":p>=60?"var(--teal)":p>=40?"var(--orange)":"var(--red)",_=e.budgetCeiling>0?l/e.budgetCeiling:1,g=Math.max(0,Math.min(100,Math.round((1-_)*150))),w=g>=70?"STRONG":g>=40?"COMPETITIVE":g>=15?"WEAK":"UNLIKELY",b=g>=70?"var(--green)":g>=40?"var(--teal)":g>=15?"var(--orange)":"var(--red)",$=Math.round(r*(1-Te/100)),A=Math.round(r*(1+Te/100));let y="";y+='<div class="bid-section"><div class="bid-section__title">Materials</div>',e.materials.forEach((C,R)=>{const W=re=>{const Lt=C.grade===re,Nt=re==="HIGH"&&C.highDisabled;return`<button class="bid-grade-btn ${Lt?re==="LOW"?"active-low":re==="HIGH"?"active-high":"active":""} ${Nt?"disabled":""}" onclick="setBidGrade(${R},'${re}')">${re[0]}</button>`};y+=`<div class="bid-mat-row">
        ...
        </div>`}),y+=`<div class="bid-line-total"><span class="bid-line-total__label">MATERIALS TOTAL</span><span class="bid-line-total__value">${N(t)}</span></div></div>`;const h=(e.requiredWorkforce?.general||80)+(e.requiredWorkforce?.skilled||20),E=[Math.round(h*.8),h,Math.round(h*1.2),Math.round(h*1.4),Math.round(h*1.6)];y+='<div class="bid-section"><div class="bid-section__title">Labor</div>',y+='<div class="bid-labor-presets">',E.forEach(C=>{y+=`<button class="bid-labor-btn ${e.laborCount===C?"active":""}" onclick="setBidLabor(${C})">${C}</button>`}),y+="</div>";const M=e.requiredWorkforce||{};y+=`<div class="bid-labor-formula">Required: ${M.general||"?"} general + ${M.skilled||"?"} skilled<br>`,y+=`${e.laborCount} workers × ${N(e.laborRate)}/tick × ${e.estimatedTicks} ticks = <strong>${N(i)}</strong></div>`,y+=`<div class="bid-line-total"><span class="bid-line-total__label">LABOR TOTAL</span><span class="bid-line-total__value">${N(i)}</span></div></div>`,y+='<div class="bid-section"><div class="bid-section__title">Equipment</div>',e.equipment.forEach(C=>{const R=C.owned?"bid-equip-row__status--owned":"bid-equip-row__status--missing",W=C.owned?"✓ OWNED":"✗ NOT OWNED";y+=`<div class="bid-equip-row"><span class="bid-equip-row__name">${f(C.name)}</span><span class="bid-equip-row__status ${R}">${W}</span></div>`}),y+=`<div class="bid-line-total"><span class="bid-line-total__label">MAINTENANCE (${e.estimatedTicks}t)</span><span class="bid-line-total__value">${N(n)}</span></div></div>`,y+='<div class="bid-section"><div class="bid-section__title">Permits</div>',y+='<div style="font-family:var(--font-mono);font-size:9px;color:var(--text-dim);padding:8px 0;">No permits required yet.</div>',y+='<div class="bid-line-total"><span class="bid-line-total__label">PERMITS TOTAL</span><span class="bid-line-total__value">$0</span></div></div>',y+='<div class="bid-section"><div class="bid-section__title">Overhead &amp; Contingency</div>',y+='<div style="font-family:var(--font-mono);font-size:9px;color:var(--text-dim);">Site management, insurance, admin</div>',y+=`<div class="bid-line-total"><span class="bid-line-total__label">OVERHEAD</span><span class="bid-line-total__value">${N(c)}</span></div></div>`,document.getElementById("bid-left").innerHTML=y;let I="";I+='<div class="bid-section"><div class="bid-section__title">Cost Summary</div>',I+=`<div class="bid-summary-row"><span class="bid-summary-row__label">Materials</span><span class="bid-summary-row__value">${N(t)}</span></div>`,I+=`<div class="bid-summary-row"><span class="bid-summary-row__label">Labor</span><span class="bid-summary-row__value">${N(i)}</span></div>`,I+=`<div class="bid-summary-row"><span class="bid-summary-row__label">Equipment Maint.</span><span class="bid-summary-row__value">${N(n)}</span></div>`,I+=`<div class="bid-summary-row"><span class="bid-summary-row__label">Permits</span><span class="bid-summary-row__value">${N(a)}</span></div>`,I+=`<div class="bid-summary-row"><span class="bid-summary-row__label">Overhead</span><span class="bid-summary-row__value">${N(c)}</span></div>`,I+=`<div class="bid-cost-total"><span class="bid-cost-total__label">ESTIMATED COST</span><span class="bid-cost-total__value">${N(r)}</span></div>`,I+=`<div class="bid-accuracy">⚠ Estimate accuracy: ±${Te}%<br>Actual cost range: ${N($)} — ${N(A)}</div>`,I+="</div>",I+='<div class="bid-section"><div class="bid-section__title">Markup</div>',I+=`<div class="bid-slider-wrap">
    ...
    </div>`,I+=`<div class="bid-profit"><span class="bid-profit__label">PROJECTED PROFIT</span><span class="bid-profit__value">+${N(d)}</span></div>`,I+=`<div class="bid-compete">
        <div style="display:flex;justify-content:space-between;"><span class="bid-compete__label" style="color:${b}">${w}</span><span style="font-family:var(--font-mono);font-size:8px;color:var(--text-dim);">Competitiveness</span></div>
        <div class="bid-compete__bar-wrap"><div class="bid-compete__bar" style="width:${g}%;background:${b}"></div></div>
    ...
    `;let u='<div class="pm-phase__bar">';for(let w=0;w<ee.length;w++){const b=w<e.phaseIdx,$=w===e.phaseIdx;u+=`<div class="pm-phase__seg">
            <div class="pm-phase__seg-fill pm-phase__seg-fill--${b?"done":$?"active":"future"}"></div>
            <span class="pm-phase__seg-label pm-phase__seg-label--${b?"done":$?"active":""}">${ee[w]}</span>
    ...
    </div>`,document.getElementById("pm-phase").innerHTML=u;const v=[{id:"overview",label:"Overview"},{id:"events",label:"Events",badge:p},{id:"materials",label:"Materials"},{id:"equipment",label:"Equipment"}];document.getElementById("pm-tabs").innerHTML=v.map(w=>`<button class="pm-tab${e.tab===w.id?" active":""}" onclick="pmSetTab('${w.id}')">
            ${w.label}${w.badge>0?`<span class="pm-tab__badge">${w.badge}</span>`:""}
        </button>`).join("");let _="";e.tab==="overview"?_=ti(e,t,s,l,d,r,o):e.tab==="events"?_=ii(e):e.tab==="materials"?_=ai(e):e.tab==="equipment"&&(_=ni(e)),document.getElementById("pm-content").innerHTML=`<div style="padding:0">${_}</div>`;let g="";p>0&&(g+=`<span class="pm-ftr__badge" style="color:var(--red);background:var(--red-faint);border:1px solid var(--red-border)">${p} EVENT${p>1?"S":""} REQUIRES RESPONSE</span>`),g+=`<span class="pm-ftr__badge" style="color:${e.quality>=70?"var(--green)":e.quality>=50?"var(--amber)":"var(--orange)"};background:var(--bg-0);border:1px solid var(--border-0)">QUALITY: ${e.quality}/100 — ${e.qualityLabel}</span>`,document.getElementById("pm-footer").innerHTML=`
    ...
    `}function ti(e,t,i,n,a,c,r){const o=Ce(e.awardedTick+e.totalTicks);Ce(e.awardedTick+e.totalTicks);const l=Ce(e.awardedTick),s=[{label:"Budget",value:U(e.budget),sub:`${n}% spent`,color:i},{label:"Spent",value:U(e.spent),color:"var(--red)"},{label:"Remaining",value:U(a),color:"var(--green)"},{label:"Quality",value:`${e.quality}/100`,sub:e.qualityLabel,color:e.quality>=70?"var(--green)":e.quality>=50?"var(--amber)":"var(--red)"},{label:"Workforce",value:`${e.laborCount}/${e.wfNeeded}`,sub:`Bid: ${e.laborCount}`,color:e.laborCount<e.wfNeeded?"var(--orange)":"var(--text-bright)"},{label:"Remaining",value:`${c} ticks`,sub:r?"OVERDUE":`Deadline: ${o}`,color:r?"var(--red)":"var(--text-bright)"}];let d="";d+=`<div style="padding:0 16px"><div class="pm-section">
        ...
        </div>`}return t}function Ce(e){return`${["January","February","March","April","May","June","July","August","September","October","November","December"][e%12]}, ${2e3+Math.floor(e/12)}`}window.openProjectModal=Kt;window.closeProjectModal=Jt;window.pmSetTab=Xt;window.pmToggleEvent=Zt;window.pmSelectResponse=ei;async function yt(e){if(!q)return;const{data:t,error:i}=await T.from("construction_events").select("*").eq("contract_id",e).order("fired_at_tick",{ascending:!1});i?(console.warn("Failed to load project events:",i.message),q.events=[]):q.events=(t||[]).map(n=>({id:n.id,type:n.type,severity:n.severity,tick:n.fired_at_tick,title:n.title,desc:n.description,impact:n.impact,status:n.status==="ACTIVE"?"ACTIVE":"RESOLVED",resolution:n.resolution,responses:n.responses||[]})),oe()}let Me=!1;async function si(e,t){if(!(Me||!q)){Me=!0;try{const{data:i,error:n}=await T.rpc("resolve_construction_event",{p_event_id:e,p_response_key:t});if(n){console.error("Failed to resolve event:",n.message),alert("Failed to submit response: "+n.message);return}const a=typeof i=="string"?JSON.parse(i):i;if(a?.error){alert("Error: "+a.error);return}await yt(q.project.id),await _t(),a?.quality_applied&&a.quality_applied!==0&&(q.quality=Math.max(0,Math.min(100,q.quality+a.quality_applied)),q.qualityLabel=q.quality>=80?"STRONG":q.quality>=60?"PROMISING":q.quality>=40?"FAIR":"UNCERTAIN"),oe()}finally{Me=!1}}}window.confirmEventResponse=si;function X(e,t,i){const n=i?` style="color:${i}"`:"";return`<div class="cd-detail-row">
        ...
        <g transform="translate(470,172)">
    ...
    </svg>`}let Se=!1;async function ri(){if(Se||!_e||!m)return;if((m.action_points??0)<2){alert("You need at least 2 AP to place a bid.");return}Se=!0;const e=document.querySelector(".cd-btn.primary");e&&(e.disabled=!0,e.textContent="...");try{const{data:t,error:i}=await T.rpc("deduct_ap",{p_faction_id:m.id,p_cost:2});if(i)throw i;if(t<0){const a=-t-1;m.action_points=a,document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+a+" AP</span>",e&&(e.disabled=!1,e.textContent="BID"),alert("Insufficient AP. You have "+a+" AP, need 2.");return}const{error:n}=await T.from("corp_contract_bids").insert({contract_id:_e.id,faction_id:m.id,nation_id:m.nation_id,ap_spent:2,created_at_tick:B?.current_tick||null});if(n)throw await T.rpc("deduct_ap",{p_faction_id:m.id,p_cost:-2}),m.action_points=t+2,document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+(t+2)+" AP</span>",n;m.action_points=t,document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+t+" AP</span>",e&&(e.textContent="BID PLACED")}catch(t){e&&(e.disabled=!1,e.textContent="BID"),t.code==="23505"?alert("You have already placed a bid on this contract."):alert("Failed to place bid: "+(t.message||"Unknown error"))}finally{Se=!1}}async function gt(){if(!m||!m.nation_id)return;const{data:e,error:t}=await T.from("construction_contracts").select("*").in("status",["open","bidding"]).order("generated_at_tick",{ascending:!1});if(t?(console.warn("Failed to load contracts:",t.message),le=[]):le=e||[],ue={},m&&le.length>0){const i=le.map(a=>a.id),{data:n}=await T.from("contract_bids").select("contract_id, bid_price, estimated_quality, status").eq("faction_id",m.id).in("contract_id",i);for(const a of n||[])ue[a.contract_id]=a}dt()}function li(){const e=document.getElementById("ap-list"),t=document.getElementById("ap-footer");if(document.getElementById("ap-count").textContent=F.length+" ACTIVE",F.length===0){e.innerHTML=`<div class="ap-empty">
            <div class="ap-empty__text">No active projects.<br>Win a contract bid to start<br>your first construction project.</div>
        </div>`,t.style.display="none";return}const i=B?.current_tick||0;let n=0,a=0,c="";for(const r of F){const o=r.issuer_type==="GOVERNMENT",l=o?"gov":"private",s=Array.isArray(r.contract_bids)?r.contract_bids[0]:r.contract_bids,d=s?.bid_price||0,p=s?.estimated_cost||0,u=s?.estimated_quality||0,v=r.budget_ceiling||0,_=r.awarded_at_tick||i,g=_+(r.timeline_ticks||8),w=Math.max(0,g-i),b=Math.max(0,i-_),$=r.timeline_ticks||8,A=Math.min(100,Math.round(b/$*100)),y=i>g;lt(r.sector);const h=xe(r.sector);n+=v,a+=d,c+=`<div class="ap-item" onclick="openProjectModal('${r.id}')">
            ...
            <div class="ap-details">
                ...
                <div class="ap-detail-cell" style="flex:1">
                    ...
                    <div class="ap-detail-cell__value" style="font-size:10px;font-weight:700;color:${y?"var(--red)":"var(--text-bright)"}">${w} ticks</div>
                </div>
            </div>
        </div>`}e.innerHTML=c,t.style.display=F.length>0?"":"none",F.length>0&&(document.getElementById("ap-total-crew").textContent=F.length,document.getElementById("ap-total-budget").textContent=U(n),document.getElementById("ap-total-spent").textContent=U(a))}async function _t(){if(!m)return;const{data:e,error:t}=await T.from("construction_contracts").select("*, contract_bids!inner(bid_price, material_grades, labor_count, estimated_cost, estimated_quality)").eq("awarded_to_faction",m.id).in("status",["awarded","in_progress"]).eq("contract_bids.faction_id",m.id).order("awarded_at_tick",{ascending:!0});t?(console.warn("Failed to load active projects:",t.message),F=[]):F=e||[],li()}const bt=3e4;function $t(){let e=0,t=0;for(const i of J)for(const n of Fe){const a=D[i.key]?.[n];a&&(e+=a.qty,t+=a.value)}return{totalUnits:e,totalValue:t}}function Ve(){const e=document.getElementById("wh-list"),{totalUnits:t,totalValue:i}=$t();document.getElementById("wh-count").textContent=t.toLocaleString()+" UNITS",document.getElementById("wh-total-value").textContent=L(i);const n=Math.round(t/bt*100),a=document.getElementById("wh-capacity");a.textContent=n+"%",a.style.color=n>80?"var(--red)":n>50?"var(--orange)":"var(--green)";let c="";for(let r=0;r<J.length;r++){const o=J[r],l=ze===r,s=D[o.key]?.LOW||{qty:0,value:0},d=D[o.key]?.STD||{qty:0,value:0},p=D[o.key]?.HIGH||{qty:0,value:0},u=s.qty+d.qty+p.qty,v=s.value+d.value+p.value,_=u===0,g=ae(o.key,"LOW",x),w=ae(o.key,"STD",x),b=ae(o.key,"HIGH",x),$=s.qty>0?"wh-dot wh-dot--low":"wh-dot wh-dot--empty",A=d.qty>0?"wh-dot wh-dot--std":"wh-dot wh-dot--empty",y=b.available?p.qty>0?"wh-dot wh-dot--high":"wh-dot wh-dot--empty":"wh-dot wh-dot--locked";if(c+='<div class="wh-row">',c+=`<div class="wh-row__collapsed${l?" expanded":""}" onclick="toggleWhRow(${r})">
            ...
            <span class="wh-row__val${_?" empty":""}">${v>0?L(v):"—"}</span>
                ...
                <span class="wh-expand__hdr-label" style="width:55px;text-align:right">$/UNIT</span>
            </div>`;const h=[{key:"LOW",label:"Low",data:s,avail:g,color:"var(--orange)",dotClass:"wh-dot--low"},{key:"STD",label:"Standard",data:d,avail:w,color:"var(--amber)",dotClass:"wh-dot--std"},{key:"HIGH",label:"High",data:p,avail:b,color:"var(--green)",dotClass:"wh-dot--high"}];for(const E of h){const M=!E.avail.available,I=E.data.qty>0,C=I?"$"+Math.round(E.data.value/E.data.qty):"—";c+=`<div class="wh-grade${M?" unavailable":""}">
                    ...
                    <span class="wh-grade__qty" style="color:${I?"var(--text-bright)":"var(--text-dim)"}">${I?E.data.qty.toLocaleString():"—"}</span>
                    ...
                    <span class="wh-grade__cpu">${C}</span>
                        ...
                        <span class="wh-lock__text">${E.label.toUpperCase()} GRADE LOCKED — ${f(E.avail.failedStat)} &lt; ${E.avail.failedMin}</span>
                    </div>`);c+="</div>"}c+="</div>"}e.innerHTML=c}function ci(e){ze=ze===e?-1:e,Ve()}async function di(){if(!m)return;const{data:e,error:t}=await T.from("corp_warehouse").select("material_key, quality_tier, quantity, total_value").eq("faction_id",m.id);if(D={},t)console.warn("Failed to load warehouse:",t.message);else if(e)for(const i of e)D[i.material_key]||(D[i.material_key]={}),D[i.material_key][i.quality_tier]={qty:i.quantity||0,value:Number(i.total_value)||0};Ve()}const pi={manufacturing_output:"base production",rare_minerals:"raw input",inflation:"price modifier",fuel_prices:"transport cost",urbanization:"demand pressure",arable_land:"base production",physical_infrastructure:"transport",digital_infrastructure:"component quality",energy_generation:"electrical supply",standard_of_living:"demand tier",oil_and_gas:"base input",higher_education:"engineering"};function xt(){const e=(x?.name||m?.nation||"—").toUpperCase();document.getElementById("pr-nation-badge").textContent="LOCAL — "+e;const t=Number(m?.corp_cash_reserves)||0;document.getElementById("pr-cash").textContent=L(t);const{totalUnits:i}=$t(),n=Math.round(i/bt*100),a=document.getElementById("pr-wh-capacity");a.textContent=n+"%",a.style.color=n>80?"var(--red)":n>50?"var(--orange)":"var(--green)",ht(),Ye(),we()}function ht(){const e=document.getElementById("pr-mat-grid");let t="";for(const i of J){const n=P===i.key,a=Fe.every(r=>!ae(i.key,r,x).available),c="pr-mat-btn"+(n?" active":"")+(a?" all-locked":"");t+=`<span class="${c}" onclick="setPrMat('${i.key}')">${f(i.name)}</span>`}e.innerHTML=t}function Ye(){const e=document.getElementById("pr-tier-bar");let t='<span class="pr-tier-label">GRADE</span>';for(const i of Fe){const n=ae(P,i,x),a=O===i,c=n.available?We(P,i,x):null,r=ot[i],o=!n.available,l="pr-tier-btn"+(a?" active":"")+(o?" locked":"");t+=`<div class="${l}" onclick="${o?"":`setPrTier('${i}')`}">
        ...
        </div>`}e.innerHTML=t}function we(){const e=document.getElementById("pr-content"),t=ae(P,O,x),i=J.find(b=>b.key===P);if(!i)return;if(!t.available){e.innerHTML=`<div class="pr-locked-panel">
        ...
        </div>`;return}const n=We(P,O,x),a=rt(P,O,x),c=n*ne,r=a>3e3?"LOW":a>1e3?"MODERATE":"HIGH",o=r==="LOW"?"var(--green)":r==="MODERATE"?"var(--amber)":"var(--red)",l=Number(x?.inflation??50),s=l>55?"up":l<45?"down":"flat",d=s==="up"?"&#9650;":s==="down"?"&#9660;":"&#8212;",p=s==="up"?"var(--red)":s==="down"?"var(--green)":"var(--text-dim)";let u="";u+=`<div style="padding:8px 14px;border-bottom:1px solid var(--border-0);">
        ...
        <div class="pr-drivers__title">Price Drivers (${f(x?.name||"—")})</div>`;for(const b of i.priceDrivers){const $=Number(x?.[b]??50),A=$>=50?"var(--green)":$>=30?"var(--amber)":$>=15?"var(--orange)":"var(--red)",y=pi[b]||b;u+=`<div class="pr-driver-row">
        ...
        </div>`}u+="</div>";const _=(Number(m?.corp_cash_reserves)||0)>=c,g=ne>a,w=ot[O];u+=`<div class="pr-order">
        ...
        <div class="pr-order__box">
            <div class="pr-order__header">
                <div style="display:flex;align-items:center;gap:6px;">
                    ...
                    <span style="font-family:var(--font-mono);font-size:9px;color:${w}">${De[O]}</span>
                </div>
    ...
    </div>`,e.innerHTML=u}function mi(e){P=e,O="STD";for(const t of["STD","HIGH","LOW"])if(ae(e,t,x).available){O=t;break}ht(),Ye(),we()}function ui(e){O=e,Ye(),we()}function fi(e){ne=e,we()}let Ae=!1;async function vi(){if(Ae||!m||!x)return;const e=We(P,O,x),t=rt(P,O,x),i=e*ne,n=Number(m.corp_cash_reserves)||0;if(i>n){alert("Insufficient cash reserves.");return}if(ne>t){alert("Exceeds available supply this tick.");return}Ae=!0;const a=document.querySelector(".pr-purchase-btn");a&&(a.disabled=!0,a.textContent="...");try{const c=n-i,{error:r}=await T.from("factions").update({corp_cash_reserves:c}).eq("id",m.id);if(r)throw r;const o=D[P]?.[O],l=(o?.qty||0)+ne,s=(o?.value||0)+i,{error:d}=await T.from("corp_warehouse").upsert({faction_id:m.id,nation_id:m.nation_id,material_key:P,quality_tier:O,quantity:l,total_value:s,last_purchased_tick:B?.current_tick||null,updated_at:new Date().toISOString()},{onConflict:"faction_id,material_key,quality_tier"});if(d){const{error:p}=await T.from("factions").update({corp_cash_reserves:n}).eq("id",m.id);throw p&&console.error("Cash refund failed after warehouse error:",p.message),d}m.corp_cash_reserves=c,D[P]||(D[P]={}),D[P][O]={qty:l,value:s},Ve(),xt(),a&&(a.textContent="PURCHASED",setTimeout(()=>{a.isConnected&&(a.disabled=!1,a.textContent="PURCHASE")},1500))}catch(c){a&&(a.disabled=!1,a.textContent="PURCHASE"),alert("Purchase failed: "+(c.message||"Unknown error"))}finally{Ae=!1}}function wt(e){const t=se||x;if(!t)return[];const i=$e(e);if(!i)return[];const n=Ot(e,t),a=[],c=Number(t?.inflation??50),r=Number(t?.fuel_prices??50);Number(t?.manufacturing_output??50);const o=se&&x&&se.id!==x.id;let l=null;if(o&&(l=Dt(t,x)),n.newAvailable>0){const s=et(e,t),d=i.basePrice,p=Math.round(d*((c-50)/200)),u=Math.round(d*((r-50)/300));let v=s;const _=[{label:"Base price",value:L(d)},p!==0?{label:`Inflation (${c})`,mod:(p>=0?"+":"")+L(Math.abs(p))}:null,u!==0?{label:`Fuel transport (${r})`,mod:(u>=0?"+":"")+L(Math.abs(u))}:null].filter(Boolean),g=s-d-p-u;if(g!==0&&!o&&_.push({label:"Demand/scarcity",mod:(g>=0?"+":"")+L(Math.abs(g))}),o&&l){const w=Math.round(s*l.tariff),b=Math.round(s*l.transport);v=s+w+b,_.push({label:`Import tariff (${Math.round(l.tariff*100)}%)`,mod:"+"+L(w)}),_.push({label:`Transport (${l.deliveryTicks} tick${l.deliveryTicks>1?"s":""})`,mod:"+"+L(b)})}a.push({seller:o?`${t.name} Buyers`:"Local Buyers",sellerType:"LOCAL",nation:t.name||"—",distance:o?l?.deliveryTicks||1:0,condition:100,price:Math.round(v),available:n.newAvailable,delivery:o?l.deliveryTicks+" tick"+(l.deliveryTicks>1?"s":""):"Immediate",deliveryTicks:o?l.deliveryTicks:0,used:!1,priceFactors:_,sourceNationId:t.id})}if(n.usedAvailable>0){const s=n.usedCondition,d=et(e,t,{used:!0,condition:s});let p=d;const u=[{label:"Base price",value:L(i.basePrice)},{label:`Condition (${s}%)`,mod:"-"+L(Math.max(0,i.basePrice-d))}];if(o&&l){const v=Math.round(d*l.tariff),_=Math.round(d*l.transport);p=d+v+_,u.push({label:`Import tariff (${Math.round(l.tariff*100)}%)`,mod:"+"+L(v)}),u.push({label:`Transport (${l.deliveryTicks} tick${l.deliveryTicks>1?"s":""})`,mod:"+"+L(_)})}a.push({seller:o?`${t.name} Buyers`:"Local Buyers",sellerType:"LOCAL",nation:t.name||"—",distance:o?l?.deliveryTicks||1:0,condition:s,price:Math.round(p),available:n.usedAvailable,delivery:o?l.deliveryTicks+" tick"+(l.deliveryTicks>1?"s":""):"Immediate",deliveryTicks:o?l.deliveryTicks:0,used:!0,priceFactors:u,sourceNationId:t.id})}return a}function ke(){const e=Number(m?.corp_cash_reserves)||0;document.getElementById("em-cash").textContent=L(e);const t=$e(G),i=fe[t?.tier||1],n=document.getElementById("em-tier-badge");n&&(n.textContent=i.tag,n.style.color=i.color),n.style.background=i.color+"0a",n.style.border="1px solid "+i.color+"33";const a=document.getElementById("em-nation-select");if(a&&a.options.length===0){const o=x?.name||m?.nation||"—";let l=`<option value="">${f(o)} (HQ)</option>`;for(const s of ge)s.id!==x?.id&&(l+=`<option value="${s.id}">${f(s.name)}</option>`);a.innerHTML=l}const c=document.getElementById("em-import-tag"),r=se&&x&&se.id!==x.id;c&&(c.style.display=r?"":"none"),yi(),Qe()}function yi(){let e="";for(let t=1;t<=3;t++){const i=fe[t],n=Oe(t),a=t===3?"em-selector__grid em-selector__grid--t3":"em-selector__grid em-selector__grid--t12";e+=`<div class="em-selector__row">
        ...
        </div></div>`}e.innerHTML=t}async function gi(e){if(!e)se=null;else{let i=ge.find(n=>n.id===e);if(!i)try{const{data:n}=await T.from("nations").select("*").eq("id",e).single();i=n}catch{}se=i||null}Q=0,K=1;const t=document.getElementById("em-nation-select");t&&(t.value=e||""),ke()}function _i(e){G=e,Q=0,K=1,ke()}function bi(e){Q=e,K=1,Qe()}function $i(e){K=e,Qe()}let qe=!1;async function xi(){if(qe)return;const e=Z[Q];if(!e||!m)return;const t=$e(G);if(!t)return;const i=K,n=e.price*i,a=Number(m.corp_cash_reserves)||0;if(n>a){alert("Insufficient cash reserves.");return}if(i>e.available){alert("Not enough units available.");return}const c=document.querySelector(".em-purchase-btn");c&&(c.disabled=!0,c.textContent="..."),qe=!0;try{const r=a-n,{error:o}=await T.from("factions").update({corp_cash_reserves:r}).eq("id",m.id);if(o)throw o;const l=!e.deliveryTicks||e.deliveryTicks===0;if(l){const d=V.find(A=>A.equipment_key===G),p=(d?.owned||0)+i,u=d?.purchase_price_avg||0,v=d?.owned||0,_=v>0?Math.round((u*v+e.price*i)/p):e.price,g=t.maintenancePerUnit*p,w=d?.condition||100,b=Math.round((w*v+e.condition*i)/p),{error:$}=await T.from("corp_equipment").upsert({faction_id:m.id,nation_id:m.nation_id,equipment_key:G,tier:t.tier,owned:p,deployed:d?.deployed||0,condition:b,maintenance_per_tick:g,purchase_price_avg:_,last_purchased_tick:B?.current_tick||null,updated_at:new Date().toISOString()},{onConflict:"faction_id,equipment_key"});if($){const{error:A}=await T.from("factions").update({corp_cash_reserves:a}).eq("id",m.id);throw A&&console.error("Cash refund failed:",A.message),$}d?(d.owned=p,d.condition=b,d.maintenance_per_tick=g):V.push({equipment_key:G,tier:t.tier,owned:p,deployed:0,condition:b,maintenance_per_tick:g,assigned_projects:[]})}else{const d=(B?.current_tick||0)+e.deliveryTicks,{error:p}=await T.from("corp_equipment_deliveries").insert({faction_id:m.id,equipment_key:G,quantity:i,condition:e.condition,delivery_tick:d,source_nation_id:e.sourceNationId||null,seller_name:e.seller,price_paid:n});if(p){const{error:u}=await T.from("factions").update({corp_cash_reserves:a}).eq("id",m.id);throw u&&console.error("Cash refund failed:",u.message),p}}m.corp_cash_reserves=r,Ke(),ke();const s=document.getElementById("pr-cash");s&&(s.textContent=L(r)),c&&(c.textContent=l?"PURCHASED":"ORDERED",setTimeout(()=>{c.isConnected&&(c.disabled=!1,c.textContent="PURCHASE")},1500))}catch(r){c&&(c.disabled=!1,c.textContent="PURCHASE"),alert("Purchase failed: "+(r.message||"Unknown error"))}finally{qe=!1}}let hi=-1,me=[],Ue=[],kt=[];function Le(e){return Math.abs(e)>=1e6?"$"+(e/1e6).toFixed(1)+"M":Math.abs(e)>=1e3?"$"+(e/1e3).toFixed(0)+"k":"$"+e.toLocaleString()}function wi(e,t,i){if(i)return"var(--orange)";const n=e/(t||1)*100;return n>50?"var(--green)":n>25?"var(--amber)":"var(--red)"}function ki(){const e=document.getElementById("pm-list"),t=me.length,i=Ue.length,n=kt.length,a=me.filter(l=>l.expiring_soon).length;document.getElementById("pm-active-count").textContent=`(${t})`,document.getElementById("pm-pending-count").textContent=`(${i})`,document.getElementById("pm-apply-count").textContent=`(${n})`;const c=document.getElementById("pm-badges");let r="";a>0&&(r+=`<span class="pm-badge pm-badge--expiring">${a} EXPIRING</span>`),i>0&&(r+=`<span class="pm-badge pm-badge--pending">${i} PENDING</span>`),c.innerHTML=r;const o=me.reduce((l,s)=>l+(s.cost||0),0)+Ue.reduce((l,s)=>l+(s.cost||0),0);document.getElementById("pm-total-cost").textContent=Le(o),document.getElementById("pm-footer-active").textContent=t,document.getElementById("pm-footer-pending").textContent=i;{if(t===0){e.innerHTML=`<div class="pm-empty">
            ...
            </div>`;return}let l="";me.forEach((s,d)=>{const p=hi===d,u=wi(s.ticks_left,s.total_ticks,s.expiring_soon),v=Math.min(s.ticks_left/(s.total_ticks||1)*100,100);l+=`<div class="pm-item ${s.expiring_soon?"pm-item--expiring":""} ${p?"expanded":""}" onclick="togglePmExpand(${d})">
        ...
        </div>`;return}let r="";te.forEach((o,l)=>{const s=Ti===l,d=o.financials||{},p=(d.payment||0)+(d.bonus||0)-(d.penalty||0)-(d.total_cost||0),u=p>=0,v=Ii(o.result),g={DISTINCTION:"var(--gold)",PASS:"var(--green)",CONDITIONAL:"var(--orange)",FAIL:"var(--red)"}[o.result]||"var(--text-dim)",w=o.type==="GOVERNMENT";if(r+=`<div class="dl-item ${s?"expanded":""}" onclick="toggleDlExpand(${l})">
            <div class="dl-item__inner" style="border-left:2px solid ${g}">
                ...
                <div class="dl-item__row2">
                    ...
                    <span class="dl-item__issuer" style="color:${w?"var(--green)":"var(--gold)"}">${f(o.issuer)}</span>
                ...
                <div class="dl-summary-bar">
                ...
                </div>`,s){const b=o.inspection||{};r+='<div style="margin-top:8px;">',r+='<div class="dl-section-label">Inspection Report</div>',["materials","structural","systems"].forEach(h=>{const E=b[h]||{score:0,issues:[]},M=at(E.score),I=Math.min(E.score/100*100,100);r+=`<div class="dl-inspect-row">
                    <div class="dl-inspect-row__header">
                        ...
                        <div class="dl-inspect-row__score-area">
                            <div class="dl-inspect-bar"><div class="dl-inspect-bar__fill" style="width:${I}%;background:${M}"></div></div>
                            <span class="dl-inspect-row__score" style="color:${M}">${E.score}</span>
                        </div>
                    </div>
                    ${(E.issues||[]).map(C=>`<div class="dl-inspect-issue">${f(C)}</div>`).join("")}
                ...
                ${($.issues||[]).map(h=>`<div class="dl-inspect-issue dl-inspect-issue--red">${f(h)}</div>`).join("")}
                    ...
                    <span class="dl-mat-tag__grade" style="color:${E}">${f(h.grade)}</span>
                    <span class="dl-mat-tag__impact" style="color:${I}">${M}</span>
                </div>`}),r+="</div>",r+='<div class="dl-section-label">Financial Summary</div>',r+='<div class="dl-fin-panel">',r+=`<div class="dl-fin-row"><span class="dl-fin-key">CONTRACT VALUE</span><span class="dl-fin-val">${Y(d.contract_value||0)}</span></div>`,(d.bonus||0)>0&&(r+=`<div class="dl-fin-row"><span class="dl-fin-key">QUALITY BONUS</span><span class="dl-fin-val" style="color:var(--green)">+${Y(d.bonus)}</span></div>`),(d.penalty||0)>0&&(r+=`<div class="dl-fin-row"><span class="dl-fin-key">PENALTIES</span><span class="dl-fin-val" style="color:var(--red)">-${Y(d.penalty)}</span></div>`);const A=(d.payment||0)+(d.bonus||0)-(d.penalty||0);r+=`<div class="dl-fin-row"><span class="dl-fin-key">PAYMENT RECEIVED</span><span class="dl-fin-val" style="color:var(--green)">${Y(A)}</span></div>`,r+=`<div class="dl-fin-row"><span class="dl-fin-key">TOTAL COST</span><span class="dl-fin-val" style="color:var(--red)">-${Y(d.total_cost||0)}</span></div>`,r+=`<div class="dl-fin-total">
        ...
        </div>`;const a={};for(const o of V)a[o.equipment_key]=o;let c="";for(let o=1;o<=3;o++){const l=fe[o],s=Oe(o),d=Re===o,p=s.reduce((v,_)=>v+(a[_.key]?.owned||0),0),u=s.reduce((v,_)=>v+(a[_.key]?.deployed||0),0);if(c+=`<div class="eq-tier-hdr" onclick="toggleEqTier(${o})">
        ...
        </div>`,d)for(const v of s){const _=a[v.key],g=_?.owned||0,w=_?.deployed||0,b=_?.condition||0,$=v.maintenancePerUnit*g,A=g-w,y=g>0&&A===0,h=g>0&&b<65,E=st(b),M=_?.assigned_projects||[],I=M.length>0?M.map(C=>C.contract_name||"Project").join(", ").slice(0,30):g>0&&w>0?w+" project"+(w>1?"s":""):"—";c+=`<div class="eq-row${g===0?" unowned":""}">`,c+=`<div class="eq-row__top">
                        ...
                        <div class="eq-detail__cell" style="flex:1.2">
                            ...
                            <div class="eq-detail__bar">
                                <div class="eq-detail__bar-track">
                                    <div class="eq-detail__bar-fill" style="width:${b}%;background:${E}"></div>
                                </div>
                                <span class="eq-detail__bar-pct" style="color:${E}">${b}%</span>
                            </div>
                        </div>
        ...
        <div class="eq-footer__tiers">${r}</div>`}function Mi(e){Re=Re===e?-1:e,Ke()}async function Si(){if(!m)return;const{data:e,error:t}=await T.from("corp_equipment").select("equipment_key, tier, owned, deployed, condition, maintenance_per_tick, assigned_projects").eq("faction_id",m.id);t?(console.warn("Failed to load equipment:",t.message),V=[]):V=e||[],Ke()}async function Ai(){const{data:{user:e}}=await T.auth.getUser();if(!e){window.location.href="login.html";return}const{data:t}=await T.from("factions").select("*").or(`id.eq.${e.id},linked_user_id.eq.${e.id}`);pe=(t||[]).filter(s=>s.nation_id);const i=sessionStorage.getItem("active_faction_id");if(m=pe.find(s=>s.id===i)||pe.find(s=>s.faction_type==="corporation")||pe[0],!m){await T.auth.signOut(),window.location.href="login.html";return}if(m.faction_type!=="corporation"){window.location.href="dashboard.html";return}const[n,a]=await Promise.all([m.nation_id?T.from("nations").select("*").eq("id",m.nation_id).single():Promise.resolve({data:null}),T.from("shard").select("current_tick, current_date, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single()]);n.error&&console.warn("Nation load failed:",n.error.message),n.data&&(x=n.data),a.error&&console.warn("Shard load failed:",a.error.message),B=a.data;const c=m.corp_ticker||m.abbreviation||"";if(document.getElementById("corp-logo").textContent=c.slice(0,2)||"—",document.getElementById("corp-name-bar").textContent=m.faction_name||"Unnamed Corp",B){if(document.getElementById("game-date").textContent=B.current_date||"—",document.getElementById("tick-number").textContent=B.current_tick||"—",B.next_tick_at){const d=(Number(B.tick_interval_hours)||8)*36e5,p=new Date(B.next_tick_at).getTime(),v=p-d+d/2;He=new Date(v>Date.now()?v:p+d/2),Rt()}const s=document.querySelector("#tick-countdown")?.closest(".tick-item")?.querySelector(".tick-label");s&&(s.textContent="Next Corp Tick")}document.getElementById("corp-name-badge").textContent=(c?"["+c+"]":m.faction_name||"Corp")+" ▾";const r=document.getElementById("topbar-cash");if(r){const s=Number(m.corp_cash_reserves??0),d=s>=1e9?"$"+(s/1e9).toFixed(1)+"B":s>=1e6?"$"+(s/1e6).toFixed(1)+"M":"$"+Math.round(s/1e3)+"k";r.textContent="CASH: "+d}const o=m.action_points??0;document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+o+" AP</span>",document.getElementById("nation-pill").textContent=(x?.name||m.nation||"—").toUpperCase();const l=document.getElementById("corp-faction-dropdown");if(l){let s="";for(const d of pe){const p=d.id===m.id,u=d.faction_type==="corporation"?"CORP":"PARTY",v=d.faction_type==="corporation"?"var(--teal)":"var(--amber)";s+=`<div class="corp-dd-item${p?" active":""}" onclick="switchToFaction('${d.id}', '${d.faction_type}')">
...
Firing workers — no cost.`;if(confirm(n)){Ne=!0;try{const a={};for(const o of ve){const l=Number(m?.[o.factionKey]??0);a[o.factionKey]=Math.max(0,l+j[o.id])}t>0&&(a.corp_cash_reserves=Math.max(0,i-Math.round(t)));const{error:c}=await T.from("factions").update(a).eq("id",m.id);if(c)throw c;Object.assign(m,a),j={general:0,skilled:0,innovative:0};const r=document.getElementById("topbar-cash");if(r){const o=Number(m.corp_cash_reserves??0);r.textContent="CASH: "+(o>=1e6?"$"+(o/1e6).toFixed(1)+"M":"$"+Math.round(o/1e3)+"k")}Ee()}catch(a){alert("Error: "+a.message)}finally{Ne=!1}}}function Ee(){const e=document.getElementById("hf-card-container");if(!e)return;const t="'JetBrains Mono', monospace",i={bg:"#121210",surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",greenBright:"#5c5",red:"#c55",redDim:"#a44",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},n=Number(x?.minimum_wage??50),a=Number(x?.inflation??50),c=Number(x?.standard_of_living??50),r=n/100*48e3,o=(1+(a-50)/100*.5).toFixed(2),l=(1+(c-50)/100*.5).toFixed(2),s=x?.name||m?.nation||"Nation",d=Object.values(j).some($=>$!==0),p=Ct();let u=0,v=0,_=0,g=0,w="";for(const $ of ve){const A=Number(m?.[$.factionKey]??0),y=j[$.id],h=A+y,E=Et($.multiplier),M=y>0,I=A*E,C=h*E,R=C-I;u+=A,v+=h,_+=I,g+=C;const W=y!==0?M?"rgba(92,204,92,0.02)":"rgba(204,85,85,0.02)":"transparent";w+=`
        <div style="padding:10px 14px;border-bottom:1px solid ${i.border};background:${W};">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                ...
                <div style="display:flex;align-items:baseline;gap:4px;">
                    ...
                    <span style="font-family:${t};font-size:16px;font-weight:700;color:${M?i.greenBright:i.red}">${h.toLocaleString()}</span>`:""}
                </div>
            </div>
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                <span style="font-family:${t};font-size:8px;color:${i.dim}">WAGE (MIN × ${$.multiplier}.0 × ${o} × ${l})</span>
                <span style="font-family:${t};font-size:10px;color:${$.color}">${S(E)}/yr</span>
            </div>
            <div style="display:flex;align-items:center;gap:4px;">
                ...
                <div style="flex:1;text-align:center;padding:2px 0;background:${y!==0?i.card:"transparent"};border:1px solid ${y!==0?i.border:"transparent"}">
                    ${y!==0?`<div style="display:flex;align-items:center;justify-content:center;gap:6px;">
                        <span style="font-family:${t};font-size:12px;font-weight:700;color:${M?i.greenBright:i.red}">${M?"+":""}${y}</span>
            ...
            ${y!==0?`<div style="margin-top:6px;padding:4px 8px;background:${i.bg};border:1px solid ${i.border};display:flex;justify-content:space-between;align-items:center;">
                ...
                <span style="font-family:${t};font-size:10px;font-weight:700;color:${R>0?i.red:i.greenBright}">${R>0?"+":""}${S(R)}/yr</span>
            </div>`:""}
        </div>`}const b=g-_;e.innerHTML=`
    <div style="width:380px;height:450px;background:${i.surface};border:1px solid ${i.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
        ...
        <div style="padding:8px 14px;border-top:1px solid ${i.border};background:${i.card};flex-shrink:0;">
            <div style="display:flex;justify-content:space-between;align-items:center;${d?"margin-bottom:6px;":""}">
                <div>
                    ...
                    <div style="display:flex;align-items:baseline;gap:4px;">
                        <span style="font-family:${t};font-size:13px;font-weight:700;color:${v>p?i.red:i.text}">${d?v.toLocaleString():u.toLocaleString()}</span>
                ...
                <div style="text-align:right;">
                    ...
                    <div style="display:flex;align-items:baseline;gap:4px;justify-content:flex-end;">
                        <span style="font-family:${t};font-size:11px;font-weight:700;color:${i.text}">${S(_)}</span>
                        ${d?`<span style="font-family:${t};font-size:9px;color:${i.dim}">→</span>
                        <span style="font-family:${t};font-size:11px;font-weight:700;color:${b>0?i.red:i.greenBright}">${S(g)}</span>`:""}
                    </div>
                </div>
            </div>
    ...
    </div>`}function Di(){const e=document.getElementById("wf-summary-container");if(!e)return;const t="'JetBrains Mono', monospace",i={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",blue:"#5a9abf",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660",red:"#c55"},n=(x?.name||m?.nation||"Nation").toUpperCase(),a=Number(x?.minimum_wage??50),c=Number(x?.inflation??50),r=Number(x?.standard_of_living??50),o=a/100*48e3,l=1+(c-50)/100*.5,s=1+(r-50)/100*.5,d=[{label:"General Workforce",mult:2,color:i.accent,key:"corp_general_workforce",countColor:i.text},{label:"Skilled Workforce",mult:3,color:i.gold,key:"corp_skilled_workforce",countColor:i.blue},{label:"Innovative Workforce",mult:6,color:i.orange,key:"corp_innovative_workforce",countColor:i.gold}];let p=0,u=0,v="";for(const _ of d){const g=Number(m?.[_.key]??0),w=Math.round(o*_.mult*l*s),b=g*w;p+=g,u+=b,v+=`
        <div style="padding:8px 12px;border-bottom:1px solid ${i.border};">
            ...
            <div style="display:flex;justify-content:space-between;margin-top:2px;">
                ...
                <span style="font-family:${t};font-size:10px;color:${i.muted}">${S(w)}/yr</span>
            </div>
    ...
    <div style="width:380px;height:450px;background:${i.surface};border:1px solid ${i.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
        ...
        <div style="padding:8px 14px;border-top:1px solid ${i.border};background:${i.card};flex-shrink:0;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:2px;">
                ...
                <span style="font-family:${t};font-size:13px;font-weight:700;color:${i.green}">${S(c)}</span>
            </div>
            <div style="display:flex;justify-content:space-between;align-items:center;">
    ...
    </div>`}let de=[],H=null;const be={Basic:{color:"#6a6660",label:"BASIC"},Modern:{color:"#8b9a6b",label:"MODERN"},Sustainable:{color:"#5c5",label:"SUSTAINABLE"},Innovative:{color:"#c8a832",label:"INNOVATIVE"},Heritage:{color:"#c84",label:"HERITAGE"},Premium:{color:"#ca5",label:"PREMIUM"}};async function St(){if(!m?.nation_id)return;const{data:e,error:t}=await T.from("available_properties").select("*").eq("nation_id",m.nation_id).eq("status","available").order("price",{ascending:!0});if(t){console.warn("[Property] Failed to load marketplace:",t.message);return}de=(e||[]).map(i=>({...i,adjusted_cost:i.price,adjusted_maintenance:i.monthly_maintenance}))}function Je(){const e=document.getElementById("new-property-container");if(!e)return;const t="'JetBrains Mono', monospace",i={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",redDim:"#a44",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"};(x?.name||m?.nation||"Nation").toUpperCase();const n=Number(x?.standard_of_living??50),a=Number(x?.gdp_growth??50),c=Number(x?.inflation??50),r=x?.capital||"Capital",o={capital:r,port:r+" Port",industrial:r+" Industrial Zone",suburban:r+" Suburbs",coastal:r+" Coast"};let l="";if(de.length===0)l=`<div style="padding:20px;text-align:center;font-family:${t};font-size:10px;color:${i.dim}">No properties available in this market.<br>Improve GDP Growth and Standard of Living to unlock more.</div>`;else for(let s=0;s<de.length;s++){const d=de[s],p=H===s,u=be[d.style]||be.Basic,v=o[d.city_template]||r;l+=`
            <div onclick="npSelect(${s})" style="padding:8px 14px;border-bottom:1px solid ${i.border};cursor:pointer;border-left:2px solid ${p?i.accent:"transparent"};background:${p?"rgba(139,154,107,0.03)":"transparent"};">
                ...
                <div style="font-family:${t};font-size:8px;color:${i.dim};margin-bottom:5px;">${v} · ${d.type.replace(/_/g," ")}</div>
                <div style="display:flex;gap:0;background:${i.card};border:1px solid ${i.border}">
...
All construction corporations in `+t+" can now bid on this project.")}catch(e){alert("Failed to submit project: "+e.message)}finally{Pe=!1}}}function qt(){if(document.getElementById("cp-modal-overlay")?.remove(),!Xe)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",gold:"#c8a832",greenBright:"#5c5",red:"#c55",redDim:"#a44",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},i=At(),n=x?.name||m?.nation||"Nation",a=ie[k.style]?.repGain||1,c=a>=4?t.gold:a>=3?t.greenBright:a>=2?t.accent:t.dim,r=Object.entries(ie).map(([s,d])=>{const p=k.style===s;return`<div onclick="cpSetField('style','${s}')" style="padding:5px 6px;cursor:pointer;text-align:center;background:${p?d.color+"18":"transparent"};border:1px solid ${p?d.color+"44":t.border};">
    ...
    <div style="width:440px;background:${t.surface};border:1px solid ${t.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;max-height:90vh;">
        ...
        <div style="padding:12px 16px;overflow:auto;flex:1;">
            ...
            <div style="margin-bottom:12px;">
                ...
                <div style="margin-top:4px;font-family:${e};font-size:8px;color:${ie[k.style].color}">${ie[k.style].desc}</div>
            </div>
    ...
    </div>`,document.body.appendChild(o);const l=document.getElementById("cp-name-input");l&&l.addEventListener("input",s=>{k.name=s.target.value}),o.addEventListener("click",s=>{s.target===o&&Ze()})}function Wi(){const e=document.getElementById("cp-name-input");if(e&&(k.name=e.value),!k.name.trim()){alert("Please enter a building name.");return}Ui()}window.cpClose=Ze;window.cpSetField=Gi;window.cpSubmitFromModal=Wi;window.npSelect=Ri;window.npBuyProperty=Hi;window.npOpenConstructionModal=ji;window.switchToExpansion=Tt;window.switchToOperations=It;window.hfSetChange=Bi;window.hfReset=Pi;window.hfConfirm=Oi;document.querySelector('[data-tab="operations"]')?.addEventListener("click",function(e){this.classList.contains("active")||(e.preventDefault(),It(e))});Ai();
