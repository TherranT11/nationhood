import{_ as C}from"./supabase-client-BXEzLDpS.js";import{e as m}from"./utils-C2W-HleY.js";import{i as gt}from"./messaging-5qyQ6ziq.js";import{c as bt,a as qe,E as ce,b as ue,d as Qe,e as ht,f as $t,h as ze}from"./equipment-DsuDdEne.js";const Ye={LOW:"#c84",STD:"#ca5",HIGH:"#5c5"},Q=[{key:"concrete",name:"Concrete",category:"RAW",tiers:{LOW:{requirements:[]},STD:{requirements:[{stat:"manufacturing_output",min:20}]},HIGH:{requirements:[{stat:"manufacturing_output",min:50},{stat:"higher_education",min:40}]}},priceDrivers:["manufacturing_output","inflation","fuel_prices","urbanization"]},{key:"steel",name:"Steel",category:"RAW",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:10}]},STD:{requirements:[{stat:"manufacturing_output",min:35},{stat:"rare_minerals",min:20}]},HIGH:{requirements:[{stat:"manufacturing_output",min:60},{stat:"rare_minerals",min:40},{stat:"higher_education",min:45}]}},priceDrivers:["manufacturing_output","rare_minerals","inflation","fuel_prices"]},{key:"lumber",name:"Lumber",category:"RAW",tiers:{LOW:{requirements:[{stat:"arable_land",min:10}]},STD:{requirements:[{stat:"arable_land",min:30},{stat:"physical_infrastructure",min:20}]},HIGH:{requirements:[{stat:"arable_land",min:50},{stat:"manufacturing_output",min:30}]}},priceDrivers:["arable_land","physical_infrastructure","inflation"]},{key:"aggregate",name:"Aggregate",category:"RAW",tiers:{LOW:{requirements:[]},STD:{requirements:[{stat:"rare_minerals",min:15},{stat:"physical_infrastructure",min:20}]},HIGH:{requirements:[{stat:"rare_minerals",min:35},{stat:"manufacturing_output",min:25}]}},priceDrivers:["rare_minerals","physical_infrastructure","inflation"]},{key:"em",name:"E&M Systems",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:15}]},STD:{requirements:[{stat:"manufacturing_output",min:40},{stat:"digital_infrastructure",min:25}]},HIGH:{requirements:[{stat:"manufacturing_output",min:55},{stat:"digital_infrastructure",min:50},{stat:"energy_generation",min:40}]}},priceDrivers:["manufacturing_output","digital_infrastructure","inflation","energy_generation"]},{key:"glass",name:"Glass & Facades",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:20}]},STD:{requirements:[{stat:"manufacturing_output",min:40},{stat:"standard_of_living",min:35}]},HIGH:{requirements:[{stat:"manufacturing_output",min:60},{stat:"digital_infrastructure",min:40},{stat:"higher_education",min:50}]}},priceDrivers:["manufacturing_output","standard_of_living","inflation"]},{key:"asphalt",name:"Asphalt",category:"RAW",tiers:{LOW:{requirements:[{stat:"oil_and_gas",min:10}]},STD:{requirements:[{stat:"oil_and_gas",min:30},{stat:"manufacturing_output",min:25}]},HIGH:{requirements:[{stat:"oil_and_gas",min:45},{stat:"manufacturing_output",min:40},{stat:"physical_infrastructure",min:40}]}},priceDrivers:["oil_and_gas","manufacturing_output","inflation","fuel_prices"]},{key:"heavy",name:"Heavy Machinery Parts",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:40},{stat:"rare_minerals",min:30}]},STD:{requirements:[{stat:"manufacturing_output",min:60},{stat:"rare_minerals",min:45},{stat:"higher_education",min:40}]},HIGH:{requirements:[{stat:"manufacturing_output",min:75},{stat:"rare_minerals",min:60},{stat:"higher_education",min:55},{stat:"digital_infrastructure",min:45}]}},priceDrivers:["manufacturing_output","rare_minerals","higher_education","digital_infrastructure"]}];function ee(e,t,a){const i=Q.find(c=>c.key===e);if(!i)return{available:!1,failedStat:"unknown_material"};const s=i.tiers[t];if(!s)return{available:!1,failedStat:"unknown_tier"};for(const c of s.requirements){const o=Number(a?.[c.stat]??0);if(o<c.min)return{available:!1,failedStat:c.stat,failedMin:c.min,nationValue:o}}return{available:!0}}function Ne(e,t,a){const s={concrete:{LOW:200,STD:300,HIGH:500},steel:{LOW:400,STD:700,HIGH:1200},lumber:{LOW:80,STD:130,HIGH:200},aggregate:{LOW:40,STD:60,HIGH:100},em:{LOW:400,STD:700,HIGH:1200},glass:{LOW:300,STD:500,HIGH:900},asphalt:{LOW:120,STD:200,HIGH:350},heavy:{LOW:800,STD:1400,HIGH:2400}}[e]?.[t];if(!s)return 0;const c=Q.find(r=>r.key===e);if(!c)return s;let o=1;for(const r of c.priceDrivers){const l=Number(a?.[r]??50);r==="inflation"||r==="fuel_prices"?o*=1+(l-50)/200:o*=1-(l-50)/250}return o=Math.max(.4,Math.min(2.5,o)),Math.round(s*o)}function Ke(e,t,a){const s={concrete:{LOW:5e3,STD:3e3,HIGH:1e3},steel:{LOW:2e3,STD:1500,HIGH:500},lumber:{LOW:8e3,STD:4e3,HIGH:1500},aggregate:{LOW:15e3,STD:6e3,HIGH:2e3},em:{LOW:1e3,STD:700,HIGH:300},glass:{LOW:1500,STD:800,HIGH:300},asphalt:{LOW:4e3,STD:2e3,HIGH:800},heavy:{LOW:400,STD:200,HIGH:80}}[e]?.[t]||0,o=Q.find(n=>n.key===e)?.priceDrivers?.[0],l=.3+(o?Number(a?.[o]??50):50)/50*.7;return Math.round(s*l)}const Oe=["LOW","STD","HIGH"],Me={LOW:"Low",STD:"Standard",HIGH:"High"};let oe=[],u=null,T=null,S=null,ne=[],le={},z=[],D={},Le=-1,B="concrete",P="STD",te=500,W=[],Ae=0,R="trucks",F=0,V=1,J=[],ae=null,pe=[],Se=null,de=null,Be="ALL",Pe="TIMELINE";function L(e){return Math.abs(e)>=1e6?"$"+(e/1e6).toFixed(1)+"M":Math.abs(e)>=1e3?"$"+(e/1e3).toFixed(0)+"k":"$"+e}function xt(e){if(e>=12){const t=Math.floor(e/12),a=e%12;return a>0?t+"y "+a+"mo":t+"y"}return e+" ticks"}function H(e){return Math.abs(e)>=1e9?"$"+(e/1e9).toFixed(1)+"B":Math.abs(e)>=1e6?"$"+(e/1e6).toFixed(0)+"M":Math.abs(e)>=1e3?"$"+(e/1e3).toFixed(0)+"k":"$"+e}function fe(e){return e==="civil_engineering"?"CIVIL":e==="industrial"?"INDUSTRIAL":e==="mega_project"?"MEGA":e?.toUpperCase()||"—"}function Je(e){return e==="civil_engineering"?"light":e==="industrial"?"heavy":e==="mega_project"?"mega":"light"}function wt(){de&&clearInterval(de),de=setInterval(()=>{if(!Se)return;const e=Se-Date.now();if(e<=0){document.getElementById("tick-countdown").textContent="Tick due...",clearInterval(de);return}const t=Math.floor(e/36e5),a=Math.floor(e%36e5/6e4),i=Math.floor(e%6e4/1e3);document.getElementById("tick-countdown").textContent=t+"h "+a+"m "+i+"s"},1e3)}function Et(){document.body.classList.toggle("light-mode");const e=document.getElementById("theme-toggle");e.textContent=document.body.classList.contains("light-mode")?"Dark":"Light"}function kt(e,t){e==="type"&&(Be=t),e==="sort"&&(Pe=t),document.querySelectorAll(`.filter-pill[data-filter="${e}"]`).forEach(a=>{a.classList.toggle("active",a.dataset.value===t)}),Ze()}function Xe(e){return!(!u||e.sector==="mega_project"&&u.corp_subsector!=="Megaprojects"||e.sector==="industrial"&&!["Heavy Infrastructure","Megaprojects"].includes(u.corp_subsector))}function Ze(){const e=document.getElementById("oc-list");let t=[...ne];if(Be==="GOVERNMENT"?t=t.filter(s=>s.issuer_type==="GOVERNMENT"):Be==="PRIVATE"&&(t=t.filter(s=>s.issuer_type==="PRIVATE")),Pe==="TIMELINE"&&t.sort((s,c)=>(s.timeline_ticks||0)-(c.timeline_ticks||0)),Pe==="BUDGET"&&t.sort((s,c)=>(c.budget_ceiling||0)-(s.budget_ceiling||0)),document.getElementById("oc-count").textContent=t.length+" AVAILABLE",t.length===0){e.innerHTML=`
            ...
            </div>`;return}const a=S?.current_tick||0;let i="";for(const s of t){const c=s.issuer_type==="GOVERNMENT",o=c?"gov":"private",r=Xe(s),l=r?"":" locked",n=Je(s.sector),d=fe(s.sector),p=(s.timeline_ticks||0)>18?" warn":"",v=s.bidding_ends_tick?Math.max(0,s.bidding_ends_tick-a):"?";i+=`
            <div class="oc-item${l}" data-contract-id="${s.id}">
                ...
                <div class="oc-item__stats">
                    ...
                    <div class="oc-stat">
                        ...
                        <div class="oc-stat__value${p}">${xt(s.timeline_ticks||0)}</div>
                    </div>
                    <div class="oc-stat">
            ...
            </div>`}e.innerHTML=i,e.querySelectorAll(".oc-item:not(.locked)").forEach(s=>{s.addEventListener("click",()=>{const c=s.dataset.contractId,o=ne.find(r=>r.id===c);o&&et(o)})})}let me=null;function et(e){me=e;const t=document.getElementById("cd-overlay"),a=e.contract_type==="GOVERNMENT",i=a?"gov":"private",s=(T?.name||u.nation||"—").toUpperCase(),c=Xe(e);document.getElementById("cd-header-left").innerHTML=`
    ...
    `;const o=document.getElementById("cd-blueprint");e.blueprint_svg?(o.innerHTML=e.blueprint_svg,o.style.display=""):(o.innerHTML=zt(e),o.style.display="");const r=e.permits_required||[],l=e.equipment_required||[],n=e.materials_estimated||[];let d="var(--teal)";e.spec_category==="Heavy Infrastructure"&&(d="var(--orange)"),e.spec_category==="Megaproject"&&(d="var(--red)");let p=L(e.budget),v=e.timeline_months+" Months",f="";f+=`
        <div class="cd-issue">
            ...
            </div>`),f+='<div class="cd-details">',e.project_type&&(f+=K("Type",e.project_type)),e.project_subtype&&(f+=K("Sub-Type",e.project_subtype)),f+=K("Specialization",e.spec_category,d),f+=K("Total Budget",p,"var(--green)"),f+=K("Timeline",v),f+=K("Nation",T?.name||u.nation||"—"),e.region&&(f+=K("Region",e.region)),f+="</div>",r.length>0&&(f+=`
            <div class="cd-items">
    ...
    `;let v='<div class="pm-phase__bar">';for(let $=0;$<X.length;$++){const _=$<e.phaseIdx,x=$===e.phaseIdx;v+=`<div class="pm-phase__seg">
    ...
    `}function Ot(e,t,a,i,s,c,o){const r=xe(e.awardedTick+e.totalTicks);xe(e.awardedTick+e.totalTicks);const l=xe(e.awardedTick),n=[{label:"Budget",value:H(e.budget),sub:`${i}% spent`,color:a},{label:"Spent",value:H(e.spent),color:"var(--red)"},{label:"Remaining",value:H(s),color:"var(--green)"},{label:"Quality",value:`${e.quality}/100`,sub:e.qualityLabel,color:e.quality>=70?"var(--green)":e.quality>=50?"var(--amber)":"var(--red)"},{label:"Workforce",value:`${e.laborCount}/${e.wfNeeded}`,sub:`Bid: ${e.laborCount}`,color:e.laborCount<e.wfNeeded?"var(--orange)":"var(--text-bright)"},{label:"Remaining",value:`${c} ticks`,sub:o?"OVERDUE":`Deadline: ${r}`,color:o?"var(--red)":"var(--text-bright)"}];let d="";d+=`<div style="padding:0 16px"><div class="pm-section">
        ...
        <div class="pm-manager">
            ...
            <span class="pm-evt__status" style="color:${c?"var(--red)":"var(--text-dim)"};font-weight:${c?"700":"400"}">${c?"REQUIRES RESPONSE":"RESOLVED"}</span>
                    ...
                    <span class="pm-evt__impact-text">${m(i.impact)}</span>
                </div>`),c&&i.responses&&i.responses.length>0){t+='<div class="pm-evt__resp-title">Response Options</div>';for(let l=0;l<i.responses.length;l++){const n=i.responses[l],d=e.selectedResponse===l,v={SAFE:"var(--green)",RISKY:"var(--orange)",DANGEROUS:"var(--red)"}[n.tag]||"var(--text-secondary)";t+=`<div class="pm-resp${d?" selected":""}" style="${d?`border-color:${v}`:""}" onclick="event.stopPropagation();pmSelectResponse(${l})">`,t+=`<div class="pm-resp__row1">
                        <div class="pm-resp__left">
                            <span class="pm-resp__label">${m(n.label)}</span>
                            <span class="pm-resp__tag" style="color:${v};background:${v}12;border:1px solid ${v}25">${n.tag}</span>
                        </div>
                        <span class="pm-resp__delay" style="color:${n.delay>0?"var(--orange)":"var(--green)"}">
                            ...
                            <button class="pm-resp__confirm-btn" style="background:${v}" onclick="event.stopPropagation();confirmEventResponse('${i.id}','${n.key}')">CONFIRM</button>
        ...
        <g transform="translate(470,172)">
    ...
    </svg>`}let Ee=!1;async function Wt(){if(Ee||!me||!u)return;if((u.action_points??0)<2){alert("You need at least 2 AP to place a bid.");return}Ee=!0;const e=document.querySelector(".cd-btn.primary");e&&(e.disabled=!0,e.textContent="...");try{const{data:t,error:a}=await C.rpc("deduct_ap",{p_faction_id:u.id,p_cost:2});if(a)throw a;if(t<0){const s=-t-1;u.action_points=s,document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+s+" AP</span>",e&&(e.disabled=!1,e.textContent="BID"),alert("Insufficient AP. You have "+s+" AP, need 2.");return}const{error:i}=await C.from("corp_contract_bids").insert({contract_id:me.id,faction_id:u.id,nation_id:u.nation_id,ap_spent:2,created_at_tick:S?.current_tick||null});if(i)throw await C.rpc("deduct_ap",{p_faction_id:u.id,p_cost:-2}),u.action_points=t+2,document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+(t+2)+" AP</span>",i;u.action_points=t,document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+t+" AP</span>",e&&(e.textContent="BID PLACED")}catch(t){e&&(e.disabled=!1,e.textContent="BID"),t.code==="23505"?alert("You have already placed a bid on this contract."):alert("Failed to place bid: "+(t.message||"Unknown error"))}finally{Ee=!1}}async function ot(){if(!u||!u.nation_id)return;const{data:e,error:t}=await C.from("construction_contracts").select("*").in("status",["open","bidding"]).order("generated_at_tick",{ascending:!1});if(t?(console.warn("Failed to load contracts:",t.message),ne=[]):ne=e||[],le={},u&&ne.length>0){const a=ne.map(s=>s.id),{data:i}=await C.from("contract_bids").select("contract_id, bid_price, estimated_quality, status").eq("faction_id",u.id).in("contract_id",a);for(const s of i||[])le[s.contract_id]=s}Ze()}function jt(){const e=document.getElementById("ap-list"),t=document.getElementById("ap-footer");if(document.getElementById("ap-count").textContent=z.length+" ACTIVE",z.length===0){e.innerHTML=`<div class="ap-empty">
            <div class="ap-empty__text">No active projects.<br>Win a contract bid to start<br>your first construction project.</div>
        </div>`,t.style.display="none";return}const a=S?.current_tick||0;let i=0,s=0,c="";for(const o of z){const r=o.issuer_type==="GOVERNMENT",l=r?"gov":"private",n=Array.isArray(o.contract_bids)?o.contract_bids[0]:o.contract_bids,d=n?.bid_price||0,p=n?.estimated_cost||0,v=n?.estimated_quality||0,f=o.budget_ceiling||0,b=o.awarded_at_tick||a,y=b+(o.timeline_ticks||8),$=Math.max(0,y-a),_=Math.max(0,a-b),x=o.timeline_ticks||8,I=Math.min(100,Math.round(_/x*100)),g=a>y;Je(o.sector);const h=fe(o.sector);i+=f,s+=d,c+=`<div class="ap-item" onclick="openProjectModal('${o.id}')">
        ...
        </div>`}e.innerHTML=c,t.style.display=z.length>0?"":"none",z.length>0&&(document.getElementById("ap-total-crew").textContent=z.length,document.getElementById("ap-total-budget").textContent=H(i),document.getElementById("ap-total-spent").textContent=H(s))}async function rt(){if(!u)return;const{data:e,error:t}=await C.from("construction_contracts").select("*, contract_bids!inner(bid_price, material_grades, labor_count, estimated_cost, estimated_quality)").eq("awarded_to_faction",u.id).in("status",["awarded","in_progress"]).eq("contract_bids.faction_id",u.id).order("awarded_at_tick",{ascending:!0});t?(console.warn("Failed to load active projects:",t.message),z=[]):z=e||[],jt()}const lt=3e4;function ct(){let e=0,t=0;for(const a of Q)for(const i of Oe){const s=D[a.key]?.[i];s&&(e+=s.qty,t+=s.value)}return{totalUnits:e,totalValue:t}}function Re(){const e=document.getElementById("wh-list"),{totalUnits:t,totalValue:a}=ct();document.getElementById("wh-count").textContent=t.toLocaleString()+" UNITS",document.getElementById("wh-total-value").textContent=L(a);const i=Math.round(t/lt*100),s=document.getElementById("wh-capacity");s.textContent=i+"%",s.style.color=i>80?"var(--red)":i>50?"var(--orange)":"var(--green)";let c="";for(let o=0;o<Q.length;o++){const r=Q[o],l=Le===o,n=D[r.key]?.LOW||{qty:0,value:0},d=D[r.key]?.STD||{qty:0,value:0},p=D[r.key]?.HIGH||{qty:0,value:0},v=n.qty+d.qty+p.qty,f=n.value+d.value+p.value,b=v===0,y=ee(r.key,"LOW",T),$=ee(r.key,"STD",T),_=ee(r.key,"HIGH",T),x=n.qty>0?"wh-dot wh-dot--low":"wh-dot wh-dot--empty",I=d.qty>0?"wh-dot wh-dot--std":"wh-dot wh-dot--empty",g=_.available?p.qty>0?"wh-dot wh-dot--high":"wh-dot wh-dot--empty":"wh-dot wh-dot--locked";if(c+='<div class="wh-row">',c+=`<div class="wh-row__collapsed${l?" expanded":""}" onclick="toggleWhRow(${o})">
            ...
            <span class="wh-row__qty${b?" empty":""}">${v>0?v.toLocaleString():"—"}</span>
        ...
        <div class="pr-drivers__title">Price Drivers (${m(T?.name||"—")})</div>`;for(const _ of a.priceDrivers){const x=Number(T?.[_]??50),I=x>=50?"var(--green)":x>=30?"var(--amber)":x>=15?"var(--orange)":"var(--red)",g=Qt[_]||_;v+=`<div class="pr-driver-row">
        ...
        </div>`}v+="</div>";const b=(Number(u?.corp_cash_reserves)||0)>=c,y=te>s,$=Ye[P];v+=`<div class="pr-order">
        ...
        <div class="pr-order__box">
    ...
    </div>`,e.innerHTML=v}function Yt(e){B=e,P="STD";for(const t of["STD","HIGH","LOW"])if(ee(e,t,T).available){P=t;break}pt(),He(),ye()}function Kt(e){P=e,He(),ye()}function Jt(e){te=e,ye()}let ke=!1;async function Xt(){if(ke||!u||!T)return;const e=Ne(B,P,T),t=Ke(B,P,T),a=e*te,i=Number(u.corp_cash_reserves)||0;if(a>i){alert("Insufficient cash reserves.");return}if(te>t){alert("Exceeds available supply this tick.");return}ke=!0;const s=document.querySelector(".pr-purchase-btn");s&&(s.disabled=!0,s.textContent="...");try{const c=i-a,{error:o}=await C.from("factions").update({corp_cash_reserves:c}).eq("id",u.id);if(o)throw o;const r=D[B]?.[P],l=(r?.qty||0)+te,n=(r?.value||0)+a,{error:d}=await C.from("corp_warehouse").upsert({faction_id:u.id,nation_id:u.nation_id,material_key:B,quality_tier:P,quantity:l,total_value:n,last_purchased_tick:S?.current_tick||null,updated_at:new Date().toISOString()},{onConflict:"faction_id,material_key,quality_tier"});if(d){const{error:p}=await C.from("factions").update({corp_cash_reserves:i}).eq("id",u.id);throw p&&console.error("Cash refund failed after warehouse error:",p.message),d}u.corp_cash_reserves=c,D[B]||(D[B]={}),D[B][P]={qty:l,value:n},Re(),dt(),s&&(s.textContent="PURCHASED",setTimeout(()=>{s.isConnected&&(s.disabled=!1,s.textContent="PURCHASE")},1500))}catch(c){s&&(s.disabled=!1,s.textContent="PURCHASE"),alert("Purchase failed: "+(c.message||"Unknown error"))}finally{ke=!1}}function mt(e){const t=ae||T;if(!t)return[];const a=ue(e);if(!a)return[];const i=ht(e,t),s=[],c=Number(t?.inflation??50),o=Number(t?.fuel_prices??50);Number(t?.manufacturing_output??50);const r=ae&&T&&ae.id!==T.id;let l=null;if(r&&(l=$t(t,T)),i.newAvailable>0){const n=ze(e,t),d=a.basePrice,p=Math.round(d*((c-50)/200)),v=Math.round(d*((o-50)/300));let f=n;const b=[{label:"Base price",value:L(d)},p!==0?{label:`Inflation (${c})`,mod:(p>=0?"+":"")+L(Math.abs(p))}:null,v!==0?{label:`Fuel transport (${o})`,mod:(v>=0?"+":"")+L(Math.abs(v))}:null].filter(Boolean),y=n-d-p-v;if(y!==0&&!r&&b.push({label:"Demand/scarcity",mod:(y>=0?"+":"")+L(Math.abs(y))}),r&&l){const $=Math.round(n*l.tariff),_=Math.round(n*l.transport);f=n+$+_,b.push({label:`Import tariff (${Math.round(l.tariff*100)}%)`,mod:"+"+L($)}),b.push({label:`Transport (${l.deliveryTicks} tick${l.deliveryTicks>1?"s":""})`,mod:"+"+L(_)})}s.push({seller:r?`${t.name} Buyers`:"Local Buyers",sellerType:"LOCAL",nation:t.name||"—",distance:r?l?.deliveryTicks||1:0,condition:100,price:Math.round(f),available:i.newAvailable,delivery:r?l.deliveryTicks+" tick"+(l.deliveryTicks>1?"s":""):"Immediate",deliveryTicks:r?l.deliveryTicks:0,used:!1,priceFactors:b,sourceNationId:t.id})}if(i.usedAvailable>0){const n=i.usedCondition,d=ze(e,t,{used:!0,condition:n});let p=d;const v=[{label:"Base price",value:L(a.basePrice)},{label:`Condition (${n}%)`,mod:"-"+L(Math.max(0,a.basePrice-d))}];if(r&&l){const f=Math.round(d*l.tariff),b=Math.round(d*l.transport);p=d+f+b,v.push({label:`Import tariff (${Math.round(l.tariff*100)}%)`,mod:"+"+L(f)}),v.push({label:`Transport (${l.deliveryTicks} tick${l.deliveryTicks>1?"s":""})`,mod:"+"+L(b)})}s.push({seller:r?`${t.name} Buyers`:"Local Buyers",sellerType:"LOCAL",nation:t.name||"—",distance:r?l?.deliveryTicks||1:0,condition:n,price:Math.round(p),available:i.usedAvailable,delivery:r?l.deliveryTicks+" tick"+(l.deliveryTicks>1?"s":""):"Immediate",deliveryTicks:r?l.deliveryTicks:0,used:!0,priceFactors:v,sourceNationId:t.id})}return s}function ge(){const e=Number(u?.corp_cash_reserves)||0;document.getElementById("em-cash").textContent=L(e);const t=ue(R),a=ce[t?.tier||1],i=document.getElementById("em-tier-badge");i&&(i.textContent=a.tag,i.style.color=a.color),i.style.background=a.color+"0a",i.style.border="1px solid "+a.color+"33";const s=document.getElementById("em-nation-select");if(s&&s.options.length===0){const r=T?.name||u?.nation||"—";let l=`<option value="">${m(r)} (HQ)</option>`;for(const n of pe)n.id!==T?.id&&(l+=`<option value="${n.id}">${m(n.name)}</option>`);s.innerHTML=l}const c=document.getElementById("em-import-tag"),o=ae&&T&&ae.id!==T.id;c&&(c.style.display=o?"":"none"),Zt(),Ge()}function Zt(){let e="";for(let t=1;t<=3;t++){const a=ce[t],i=qe(t),s=t===3?"em-selector__grid em-selector__grid--t3":"em-selector__grid em-selector__grid--t12";e+=`<div class="em-selector__row">
            ...
            </div>`),t+="</div>"}const a=J[F];if(a){const i=ue(R),s=ce[i?.tier||1],c=Math.min(a.available,4),o=a.price*V,r=(Number(u?.corp_cash_reserves)||0)>=o;t+=`<div class="em-purchase"><div class="em-purchase__box">
            ...
            <div class="em-purchase__total">
        ...
        </div></div>`}e.innerHTML=t}async function ea(e){if(!e)ae=null;else{let a=pe.find(i=>i.id===e);if(!a)try{const{data:i}=await C.from("nations").select("*").eq("id",e).single();a=i}catch{}ae=a||null}F=0,V=1;const t=document.getElementById("em-nation-select");t&&(t.value=e||""),ge()}function ta(e){R=e,F=0,V=1,ge()}function aa(e){F=e,V=1,Ge()}function sa(e){V=e,Ge()}let Te=!1;async function ia(){if(Te)return;const e=J[F];if(!e||!u)return;const t=ue(R);if(!t)return;const a=V,i=e.price*a,s=Number(u.corp_cash_reserves)||0;if(i>s){alert("Insufficient cash reserves.");return}if(a>e.available){alert("Not enough units available.");return}const c=document.querySelector(".em-purchase-btn");c&&(c.disabled=!0,c.textContent="..."),Te=!0;try{const o=s-i,{error:r}=await C.from("factions").update({corp_cash_reserves:o}).eq("id",u.id);if(r)throw r;const l=!e.deliveryTicks||e.deliveryTicks===0;if(l){const d=W.find(I=>I.equipment_key===R),p=(d?.owned||0)+a,v=d?.purchase_price_avg||0,f=d?.owned||0,b=f>0?Math.round((v*f+e.price*a)/p):e.price,y=t.maintenancePerUnit*p,$=d?.condition||100,_=Math.round(($*f+e.condition*a)/p),{error:x}=await C.from("corp_equipment").upsert({faction_id:u.id,nation_id:u.nation_id,equipment_key:R,tier:t.tier,owned:p,deployed:d?.deployed||0,condition:_,maintenance_per_tick:y,purchase_price_avg:b,last_purchased_tick:S?.current_tick||null,updated_at:new Date().toISOString()},{onConflict:"faction_id,equipment_key"});if(x){const{error:I}=await C.from("factions").update({corp_cash_reserves:s}).eq("id",u.id);throw I&&console.error("Cash refund failed:",I.message),x}d?(d.owned=p,d.condition=_,d.maintenance_per_tick=y):W.push({equipment_key:R,tier:t.tier,owned:p,deployed:0,condition:_,maintenance_per_tick:y,assigned_projects:[]})}else{const d=(S?.current_tick||0)+e.deliveryTicks,{error:p}=await C.from("corp_equipment_deliveries").insert({faction_id:u.id,equipment_key:R,quantity:a,condition:e.condition,delivery_tick:d,source_nation_id:e.sourceNationId||null,seller_name:e.seller,price_paid:i});if(p){const{error:v}=await C.from("factions").update({corp_cash_reserves:s}).eq("id",u.id);throw v&&console.error("Cash refund failed:",v.message),p}}u.corp_cash_reserves=o,Ue(),ge();const n=document.getElementById("pr-cash");n&&(n.textContent=L(o)),c&&(c.textContent=l?"PURCHASED":"ORDERED",setTimeout(()=>{c.isConnected&&(c.disabled=!1,c.textContent="PURCHASE")},1500))}catch(o){c&&(c.disabled=!1,c.textContent="PURCHASE"),alert("Purchase failed: "+(o.message||"Unknown error"))}finally{Te=!1}}let na=-1,re=[],De=[],vt=[];function Ie(e){return Math.abs(e)>=1e6?"$"+(e/1e6).toFixed(1)+"M":Math.abs(e)>=1e3?"$"+(e/1e3).toFixed(0)+"k":"$"+e.toLocaleString()}function oa(e,t,a){if(a)return"var(--orange)";const i=e/(t||1)*100;return i>50?"var(--green)":i>25?"var(--amber)":"var(--red)"}function ra(){const e=document.getElementById("pm-list"),t=re.length,a=De.length,i=vt.length,s=re.filter(l=>l.expiring_soon).length;document.getElementById("pm-active-count").textContent=`(${t})`,document.getElementById("pm-pending-count").textContent=`(${a})`,document.getElementById("pm-apply-count").textContent=`(${i})`;const c=document.getElementById("pm-badges");let o="";s>0&&(o+=`<span class="pm-badge pm-badge--expiring">${s} EXPIRING</span>`),a>0&&(o+=`<span class="pm-badge pm-badge--pending">${a} PENDING</span>`),c.innerHTML=o;const r=re.reduce((l,n)=>l+(n.cost||0),0)+De.reduce((l,n)=>l+(n.cost||0),0);document.getElementById("pm-total-cost").textContent=Ie(r),document.getElementById("pm-footer-active").textContent=t,document.getElementById("pm-footer-pending").textContent=a;{if(t===0){e.innerHTML=`<div class="pm-empty">
            ...
            </div>`;return}let l="";re.forEach((n,d)=>{const p=na===d,v=oa(n.ticks_left,n.total_ticks,n.expiring_soon),f=Math.min(n.ticks_left/(n.total_ticks||1)*100,100);l+=`<div class="pm-item ${n.expiring_soon?"pm-item--expiring":""} ${p?"expanded":""}" onclick="togglePmExpand(${d})">
                <div class="pm-item__inner">
                    ...
                    <div class="pm-item__row2">
                        ...
                        <span class="pm-item__expiry" style="color:${v}">Expires: ${m(n.expires||"")}</span>
                        <span class="pm-item__ticks">(${n.ticks_left} ticks)</span>
                    </div>
                    <div class="pm-bar"><div class="pm-bar__fill" style="width:${f}%;background:${v}"></div></div>`,p&&(l+=`<div class="pm-detail">
        ...
        </div>`;return}let o="";Z.forEach((r,l)=>{const n=ca===l,d=r.financials||{},p=(d.payment||0)+(d.bonus||0)-(d.penalty||0)-(d.total_cost||0),v=p>=0,f=da(r.result),y={DISTINCTION:"var(--gold)",PASS:"var(--green)",CONDITIONAL:"var(--orange)",FAIL:"var(--red)"}[r.result]||"var(--text-dim)",$=r.type==="GOVERNMENT";if(o+=`<div class="dl-item ${n?"expanded":""}" onclick="toggleDlExpand(${l})">
            <div class="dl-item__inner" style="border-left:2px solid ${y}">
                ...
                <div class="dl-summary-bar">
                    ...
                    <div class="dl-summary-cell" style="flex:1.3;">
                        ...
                        <div class="dl-summary-value" style="color:${v?"var(--green)":"var(--red)"};margin-top:2px;">${v?"+":""}${j(p)}</div>
                    </div>
        ...
        </div>`;const s={};for(const r of W)s[r.equipment_key]=r;let c="";for(let r=1;r<=3;r++){const l=ce[r],n=qe(r),d=Ae===r,p=n.reduce((f,b)=>f+(s[b.key]?.owned||0),0),v=n.reduce((f,b)=>f+(s[b.key]?.deployed||0),0);if(c+=`<div class="eq-tier-hdr" onclick="toggleEqTier(${r})">
        ...
        <div class="eq-footer__tiers">${o}</div>`}function ma(e){Ae=Ae===e?-1:e,Ue()}async function va(){if(!u)return;const{data:e,error:t}=await C.from("corp_equipment").select("equipment_key, tier, owned, deployed, condition, maintenance_per_tick, assigned_projects").eq("faction_id",u.id);t?(console.warn("Failed to load equipment:",t.message),W=[]):W=e||[],Ue()}async function ua(){const{data:{user:e}}=await C.auth.getUser();if(!e){window.location.href="login.html";return}const{data:t}=await C.from("factions").select("*").or(`id.eq.${e.id},linked_user_id.eq.${e.id}`);oe=(t||[]).filter(n=>n.nation_id);const a=sessionStorage.getItem("active_faction_id");if(u=oe.find(n=>n.id===a)||oe.find(n=>n.faction_type==="corporation")||oe[0],!u){await C.auth.signOut(),window.location.href="login.html";return}if(u.faction_type!=="corporation"){window.location.href="dashboard.html";return}const[i,s]=await Promise.all([u.nation_id?C.from("nations").select("*").eq("id",u.nation_id).single():Promise.resolve({data:null}),C.from("shard").select("current_tick, current_date, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single()]);i.error&&console.warn("Nation load failed:",i.error.message),i.data&&(T=i.data),s.error&&console.warn("Shard load failed:",s.error.message),S=s.data;const c=u.corp_ticker||u.abbreviation||"";if(document.getElementById("corp-logo").textContent=c.slice(0,2)||"—",document.getElementById("corp-name-bar").textContent=u.faction_name||"Unnamed Corp",S){if(document.getElementById("game-date").textContent=S.current_date||"—",document.getElementById("tick-number").textContent=S.current_tick||"—",S.next_tick_at){const d=(Number(S.tick_interval_hours)||8)*36e5,p=new Date(S.next_tick_at).getTime(),f=p-d+d/2;Se=new Date(f>Date.now()?f:p+d/2),wt()}const n=document.querySelector("#tick-countdown")?.closest(".tick-item")?.querySelector(".tick-label");n&&(n.textContent="Next Corp Tick")}document.getElementById("corp-name-badge").textContent=(c?"["+c+"]":u.faction_name||"Corp")+" ▾";const o=document.getElementById("topbar-cash");if(o){const n=Number(u.corp_cash_reserves??0),d=n>=1e9?"$"+(n/1e9).toFixed(1)+"B":n>=1e6?"$"+(n/1e6).toFixed(1)+"M":"$"+Math.round(n/1e3)+"k";o.textContent="CASH: "+d}const r=u.action_points??0;document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+r+" AP</span>",document.getElementById("nation-pill").textContent=(T?.name||u.nation||"—").toUpperCase();const l=document.getElementById("corp-faction-dropdown");if(l){let n="";for(const d of oe){const p=d.id===u.id,v=d.faction_type==="corporation"?"CORP":"PARTY",f=d.faction_type==="corporation"?"var(--teal)":"var(--amber)";n+=`<div class="corp-dd-item${p?" active":""}" onclick="switchToFaction('${d.id}', '${d.faction_type}')">
                <span class="corp-dd-type" style="color:${f}">${v}</span>
                <span class="corp-dd-name">${m(d.faction_name||"Unnamed")}</span>
                <span class="corp-dd-abbr">[${m(d.abbreviation||"—")}]</span>
            </div>`}l.innerHTML=n}await Promise.all([ot(),rt(),Vt(),va(),la(),pa()]);try{const{data:n}=await C.from("nations").select("*").order("name");pe=n||[]}catch{pe=[]}dt(),ge(),gt(u,T,S),document.getElementById("loading").style.display="none",document.getElementById("page-content").style.display="block"}async function fa(){await C.auth.signOut(),window.location.href="login.html"}function _a(){const e=document.getElementById("corp-faction-dropdown");e&&e.classList.toggle("open")}function ya(e,t){const a=document.getElementById("corp-faction-dropdown");a&&a.classList.remove("open"),sessionStorage.setItem("active_faction_id",e),t==="corporation"?window.location.href="corp-operations.html":window.location.href="dashboard.html"}document.addEventListener("click",e=>{const t=document.getElementById("faction-switcher"),a=document.getElementById("corp-faction-dropdown");a&&t&&!t.contains(e.target)&&a.classList.remove("open")});document.addEventListener("keydown",e=>{e.key==="Escape"&&tt()});window.doLogout=fa;window.toggleTheme=Et;window.toggleCorpDropdown=_a;window.switchToFaction=ya;window.setFilter=kt;window.openContractDetail=et;window.closeContractDetail=tt;window.placeBid=Wt;window.toggleWhRow=Ft;window.toggleEqTier=ma;window.switchEmNation=ea;window.setEmType=ta;window.setEmListing=aa;window.setEmQty=sa;window.purchaseEquipment=ia;window.setPrMat=Yt;window.setPrTier=Kt;window.setPrQty=Jt;window.purchaseMaterial=Xt;let G={general:0,skilled:0,innovative:0},Ce=!1;const ve=[{id:"general",label:"General Workforce",multiplier:2,color:"#8b9a6b",factionKey:"corp_general_workforce"},{id:"skilled",label:"Skilled Workforce",multiplier:3,color:"#c8a832",factionKey:"corp_skilled_workforce"},{id:"innovative",label:"Innovative Workforce",multiplier:6,color:"#c84",factionKey:"corp_innovative_workforce"}];function ut(e){const t=Number(T?.minimum_wage??50),a=Number(T?.inflation??50),i=Number(T?.standard_of_living??50),s=t/100*48e3,c=1+(a-50)/100*.5,o=1+(i-50)/100*.5;return Math.round(s*e*c*o)}function O(e){const t=Math.abs(e),a=e<0?"-":"";return t>=1e9?a+"$"+(t/1e9).toFixed(2)+"B":t>=1e6?a+"$"+(t/1e6).toFixed(2)+"M":t>=1e3?a+"$"+(t/1e3).toFixed(1)+"k":a+"$"+t.toLocaleString()}function ga(e){e.preventDefault(),document.getElementById("operations-content").style.display="none",document.getElementById("expansion-content").style.display="",document.querySelectorAll(".corp-nav__tab").forEach(t=>t.classList.remove("active")),e.target.classList.add("active"),be()}function ft(e){e&&e.preventDefault(),document.getElementById("operations-content").style.display="",document.getElementById("expansion-content").style.display="none",document.querySelectorAll(".corp-nav__tab").forEach(t=>t.classList.remove("active")),document.querySelector('[data-tab="operations"]')?.classList.add("active")}function ba(e,t){const a=ve.find(c=>c.id===e),i=Number(u?.[a.factionKey]??0),s=G[e]+t;i+s<0||(G[e]=s,be())}function ha(e){e?G[e]=0:G={general:0,skilled:0,innovative:0},be()}async function $a(){if(Ce||!Object.values(G).some(s=>s!==0))return;let t=0;for(const s of ve){const c=G[s.id];c>0&&(t+=c*ut(s.multiplier)*.1)}const a=Number(u?.corp_cash_reserves??0);if(t>a){alert("Insufficient cash reserves. Hiring cost: "+O(t)+", available: "+O(a));return}const i=t>0?`Confirm workforce changes?

Hiring fee: `+O(t)+" (deducted from cash reserves)":`Confirm workforce changes?

Firing workers — no cost.`;if(confirm(i)){Ce=!0;try{const s={};for(const r of ve){const l=Number(u?.[r.factionKey]??0);s[r.factionKey]=Math.max(0,l+G[r.id])}t>0&&(s.corp_cash_reserves=Math.max(0,a-Math.round(t)));const{error:c}=await C.from("factions").update(s).eq("id",u.id);if(c)throw c;Object.assign(u,s),G={general:0,skilled:0,innovative:0};const o=document.getElementById("topbar-cash");if(o){const r=Number(u.corp_cash_reserves??0);o.textContent="CASH: "+(r>=1e6?"$"+(r/1e6).toFixed(1)+"M":"$"+Math.round(r/1e3)+"k")}be()}catch(s){alert("Error: "+s.message)}finally{Ce=!1}}}function be(){const e=document.getElementById("hf-card-container");if(!e)return;const t="'JetBrains Mono', monospace",a={bg:"#121210",surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",greenBright:"#5c5",red:"#c55",redDim:"#a44",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},i=Number(T?.minimum_wage??50),s=Number(T?.inflation??50),c=Number(T?.standard_of_living??50),o=i/100*48e3,r=(1+(s-50)/100*.5).toFixed(2),l=(1+(c-50)/100*.5).toFixed(2),n=T?.name||u?.nation||"Nation",d=Object.values(G).some(_=>_!==0);let p=0,v=0,f=0,b=0,y="";for(const _ of ve){const x=Number(u?.[_.factionKey]??0),I=G[_.id],g=x+I,h=ut(_.multiplier),w=I>0,q=x*h,E=g*h,k=E-q;p+=x,v+=g,f+=q,b+=E;const U=I!==0?w?"rgba(92,204,92,0.02)":"rgba(204,85,85,0.02)":"transparent";y+=`
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;${d?"margin-bottom:6px;":""}">
                <div>
                    ...
                    <div style="display:flex;align-items:baseline;gap:4px;">
                        ...
                        <span style="font-family:${t};font-size:13px;font-weight:700;color:${v>p?a.greenBright:v<p?a.red:a.text}">${v.toLocaleString()}</span>`:""}
                    </div>
                </div>
    ...
    </div>`}window.switchToExpansion=ga;window.switchToOperations=ft;window.hfSetChange=ba;window.hfReset=ha;window.hfConfirm=$a;document.querySelector('[data-tab="operations"]')?.addEventListener("click",function(e){this.classList.contains("active")||(e.preventDefault(),ft(e))});ua();
