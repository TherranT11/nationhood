import{_ as m}from"./supabase-client-BXEzLDpS.js";import{e as y}from"./utils-C2W-HleY.js";import{initMessaging as J}from"./messaging-B5Fng3EZ.js";import{c as X}from"./equipment-DsuDdEne.js";let B=[];function p(a){return Math.abs(a)>=1e6?"$"+(a/1e6).toFixed(2)+"M":Math.abs(a)>=1e3?"$"+(a/1e3).toFixed(1)+"k":"$"+Math.round(a).toLocaleString()}function P(a,n){return Number(a?.[n]??50)}async function Z(){const{data:{user:a}}=await m.auth.getUser();if(!a){window.location.href="login.html";return}const{data:n}=await m.from("factions").select("*").or(`id.eq.${a.id},linked_user_id.eq.${a.id}`);B=(n||[]).filter(l=>l.nation_id&&!l.abandoned_at);const i=sessionStorage.getItem("active_faction_id");let t=B.find(l=>l.id===i)||B.find(l=>l.faction_type==="corporation")||B[0];if(!t){sessionStorage.removeItem("active_faction_id"),window.location.href="select-nation.html";return}if(sessionStorage.setItem("active_faction_id",t.id),t.faction_type!=="corporation"){window.location.href="dashboard.html";return}let d=t.nation||"",r=null;const[s,o]=await Promise.all([t.nation_id?m.from("nations").select("*").eq("id",t.nation_id).single():Promise.resolve({data:null}),m.from("shard").select("current_tick, current_date, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single()]);s.error&&console.warn("Nation load failed:",s.error.message),s.data&&(d=s.data.name,r=s.data),o.error&&console.warn("Shard load failed:",o.error.message);const e=o.data,_=t.corp_ticker||t.abbreviation||"";if(document.getElementById("corp-logo").textContent=_.slice(0,2)||"—",document.getElementById("corp-name-bar").textContent=t.faction_name||"Unnamed Corp",e){if(document.getElementById("game-date").textContent=e.current_date||"—",document.getElementById("tick-number").textContent=e.current_tick||"—",e.next_tick_at){const g=(Number(e.tick_interval_hours)||8)*36e5,S=new Date(e.next_tick_at).getTime(),A=S-g+g/2;U=new Date(A>Date.now()?A:S+g/2),ot()}const l=document.querySelector("#tick-countdown")?.closest(".tick-item")?.querySelector(".tick-label");l&&(l.textContent="Next Corp Tick")}document.getElementById("corp-name-badge").textContent=(_?"["+_+"]":t.faction_name||"Corp")+" ▾";const h=document.getElementById("topbar-cash");if(h){const l=Number(t.corp_cash_reserves??0),g=l>=1e9?"$"+(l/1e9).toFixed(1)+"B":l>=1e6?"$"+(l/1e6).toFixed(1)+"M":"$"+Math.round(l/1e3)+"k";h.textContent="CASH: "+g}const $=t.action_points??0;document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+$+" AP</span>";const b=document.getElementById("corp-faction-dropdown");if(b){let l="";for(const v of B){const A=v.id===t.id,q=v.faction_type==="corporation"?"CORP":"PARTY",F=v.faction_type==="corporation"?"var(--teal)":"var(--amber)";l+=`<div class="corp-dd-item${A?" active":""}" onclick="switchToFaction('${v.id}', '${v.faction_type}')">
            ...
            </div>`),b.innerHTML=l}document.getElementById("id-type-badge").textContent=t.corp_company_type||"—",document.getElementById("id-logo").textContent=_.slice(0,3)||"—",document.getElementById("id-corp-name").textContent=t.faction_name||"Unnamed Corp";const f=t.party_description||"";document.getElementById("id-slogan").textContent=f?'"'+f+'"':'"--"';const k=e?.current_date?e.current_date.replace(/.*,\s*/,""):"—",M=t.leader_first_name&&t.leader_last_name?t.leader_first_name+" "+t.leader_last_name+(t.leader_age?" ("+t.leader_age+")":""):"—";document.getElementById("id-rows").innerHTML=`
        <div class="id-row">
        ...
        <div class="id-row">
            ...
            <span class="id-row__value">${y(t.corp_sector||"—")}</span>
        </div>
        <div class="id-row">
            <span class="id-row__label">Specialization</span>
            <span class="id-row__value">${y(t.corp_subsector||"—")}</span>
        </div>
        <div class="id-row">
        ...
        <div class="id-row">
            ...
            <span class="id-row__value">${y(t.corp_company_type||"—")}</span>
        </div>
        <div class="id-row">
            <span class="id-row__label">Stock Ticker</span>
            <span class="id-row__value" style="color:var(--teal);letter-spacing:0.1em;">${y(_)}</span>
        </div>
    `;const c=t.last_rename_tick||0,T=e?.current_tick||0,E=Math.max(0,c+120-T),I=E<=0,u=document.getElementById("slogan-editor");u.innerHTML=`
    ...
    `,window._corpFactionId=t.id,window._currentTick=T;const N=tt(r,d,t);at(d);const L=await nt(r,d,t);let x=0;if(t?.id){const{data:l,error:g}=await m.from("corp_equipment").select("equipment_key, owned").eq("faction_id",t.id);g||(x=X(l||[]))}et(r,e,N,t,L.propertyMaintenance||0,x),st(r,d,t,N,L),J(t,r,e),document.getElementById("loading").style.display="none",document.getElementById("page-content").style.display="block"}function tt(a,n,i){const t=v=>P(a,v),d=(n||"UNKNOWN").toUpperCase(),r=Number(i?.corp_general_workforce??2250),s=Number(i?.corp_skilled_workforce??600),o=Number(i?.corp_innovative_workforce??150),e=r+s+o,_=2,h=3,$=6,b=t("minimum_wage"),f=b/100*48e3,k=t("inflation"),M=t("standard_of_living"),c=1+(k-50)/100*.5,T=1+(M-50)/100*.5,C=v=>Math.round(f*v*c*T),E=C(_),I=C(h),u=C($),N=r*E,L=s*I,x=o*u,l=N+L+x;function g(v){return"$"+Math.round(v).toLocaleString()+"/yr"}const S=`${c.toFixed(2)} &times; ${T.toFixed(2)}`;return document.getElementById("wf-total-header").textContent=e.toLocaleString(),document.getElementById("wf-body").innerHTML=`
        <div class="wf-tier">
    ...
    `,{totalWages:l,generalTotal:N,skilledTotal:L,innovativeTotal:x,monthlyWages:Math.round(l/12)}}function et(a,n,i,t,d,r){const s=n?.current_tick||0;document.getElementById("fin-tick").textContent="TICK "+s;const o=5e7,e=w=>P(a,w),_=1+(e("gdp_growth")-50)/100*.4,h=1+(e("urbanization")-50)/100*.3,$=1+(e("population_growth")-50)/100*.2,b=1+(e("standard_of_living")-50)/100*.15,f=1+(50-e("physical_infrastructure"))/100*.1,k=1-Math.max(0,e("inflation")-50)/100*.1,M=1-Math.max(0,e("interest_rates")-50)/100*.1,c=_*h*$*b*f*k*M,T=Math.round(o*c),C=Math.round(T/12),E=0,I=0,u=E+I+C,N=i?.totalWages||0,L=Math.round(N/12),x=0,l=0,g=d||0,S=r||0,v=Number(t?.corp_loans)||0,A=.05,q=v>0?Math.round(v*(A/12)/(1-Math.pow(1+A/12,-120))):0,F=L+x+g+S+q+l,D=u-F,H=Number(t?.corp_cash_reserves??0),V=v,j=[{stat:"gdp_growth",value:e("gdp_growth"),weight:"0.4"},{stat:"urbanization",value:e("urbanization"),weight:"0.3"},{stat:"population_growth",value:e("population_growth"),weight:"0.2"},{stat:"standard_of_living",value:e("standard_of_living"),weight:"0.15"},{stat:"physical_infra",value:e("physical_infrastructure"),weight:"0.1",note:"INV"},{stat:"inflation",value:e("inflation"),weight:"-0.1",neg:!0},{stat:"interest_rates",value:e("interest_rates"),weight:"-0.1",neg:!0}];function Y(w){return w.neg?w.value>50?"var(--red)":"var(--green)":w.note?w.value<50?"var(--green)":"var(--red)":w.value>=50?"var(--green)":w.value>=35?"var(--amber)":"var(--red)"}const O=u||1,G=(E/O*100).toFixed(1),K=(I/O*100).toFixed(1),Q=(C/O*100).toFixed(1);document.getElementById("fin-body").innerHTML=`
        <!-- Revenue -->
        ...
        <div class="fin-net" style="background:${D>=0?"rgba(92,204,92,0.02)":"rgba(204,85,85,0.02)"}">
            ...
            <span class="fin-net__value" style="color:${D>=0?"var(--green)":"var(--red)"}">${p(D)}</span>
        </div>
        <!-- Cash & Debt -->
    ...
    `}function at(a){const n=a.toUpperCase(),i=[{label:"Reputation",value:65,change:0,decay:!0,max:100,isHero:!0},{label:"Workforce Size",value:3e3,change:0,section:"Core"},{label:"Workforce Skill",value:50,change:0,decay:!0,max:100},{label:"Operational Efficiency",value:50,change:0,decay:!0,max:100},{label:"Market Share",value:5,change:0,nation:n,max:100,section:"Market & Financials"},{label:"Credit Rating",value:50,change:0,max:100},{label:"Regulatory Standing",value:50,change:0,nation:n,max:100,section:"Political Standing"},{label:"Political Influence",value:10,change:0,decay:!0,nation:n,max:100},{label:"Innovation",value:20,change:0,decay:!0,max:100,section:"Innovation"}];function t(s,o){if(!o||o>100)return"var(--text-primary)";const e=s/o*100;return e>=70?"var(--green)":e>=40?"var(--amber)":e>=20?"var(--orange, #d48a3c)":"var(--red)"}function d(s){const o=parseFloat(s),e=o>0?"var(--green)":o<0?"var(--red)":"var(--text-dim)",_=o>0?"▲":o<0?"▼":"–";return`<div class="stat-item__change">
            ...
            <div class="stat-item">
                ...
                <div class="stat-item__right">
                    <span class="stat-item__val" style="color:${o?t(s.value,s.max):"var(--text-primary)"}">${typeof s.value=="number"?s.value.toLocaleString():s.value}</span>
                    ${o?'<span class="stat-item__max">/100</span>':""}
                    ${d(s.change)}
                </div>
            </div>`}document.getElementById("stats-body").innerHTML=r}async function nt(a,n,i,t){const d=(n||"UNKNOWN").toUpperCase();let r=[];if(i?.id){const{data:c}=await m.from("corp_properties").select("*").eq("faction_id",i.id).eq("is_active",!0).order("type",{ascending:!1}).order("purchased_at_tick",{ascending:!1});r=c||[]}const s={Basic:{color:"#6a6660"},Modern:{color:"#8b9a6b"},Sustainable:{color:"#5c5"},Innovative:{color:"#c8a832"},Heritage:{color:"#c84"},Premium:{color:"#ca5"}};let o=0,e=0;const _=5e7,h=1+(P(a,"inflation")-50)/100*.3,$=.8+P(a,"stability")/100*.4,b=Math.round(_*h*$),f=Math.round(b*.005);o+=b,e+=f;let k=`
    <div class="prop-asset" style="margin-bottom:6px;">
    ...
    </div>`;for(const c of r){const T=s[c.style]||s.Basic;o+=Number(c.purchase_price||0),e+=Number(c.monthly_maintenance||0);const C=c.condition>=75?"var(--green)":c.condition>=50?"var(--amber)":"var(--orange)";k+=`
        <div class="prop-asset" style="margin-bottom:6px;">
            <div class="prop-asset__top">
                <div>
        ...
        <div class="prop-total">
            ...
            <div style="display:flex;justify-content:space-between;">
                ...
                <span style="font-size:12px;font-weight:700;color:var(--red);font-family:var(--font-mono);">${p(e)}/mo</span>
            </div>
        </div>
    `,{propertyValue:o,propertyMaintenance:e}}function st(a,n,i,t,d){(n||"UNKNOWN").toUpperCase();const r=i.corp_company_type||"Private",s=Number(i.corp_cash_reserves)||0,o=d?.propertyValue||0,e=0,_=0,h=s+o+e+_,$=Number(i.corp_loans)||0,f=t?.monthlyWages||0,k=0,M=$+f+k,c=h-M,C=Math.round(c*(1+.3)),E=C-c,I=E>0;document.getElementById("val-type-badge").textContent=r.toUpperCase();function u(N,L,x={}){const l=x.indent?"val-line val-line--indent":"val-line",g=x.bold?"val-line__label val-line__label--bold":"val-line__label",S=x.bold?"val-line__value val-line__value--bold":"val-line__value",v=x.color||(x.bold?"var(--text-bright)":"var(--text-muted)");return`<div class="${l}"><span class="${g}">${N}</span><span class="${S}" style="color:${v}">${p(L)}</span></div>`}document.getElementById("val-body").innerHTML=`
