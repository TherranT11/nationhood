...
${R.IMPEACHMENT_COMMITTEE_TICKS}-tick committee debate begins now. The motion will then proceed to a floor vote.`),window.location.href=`bill.html?id=${M.id}`,{ok:!0,billId:M.id}}catch(m){return console.error("[impeachment] file failed:",m?.message||m),alert("Error: "+(m?.message||m)),{ok:!1}}}let C=null,g=null,V="leader",Z=[],fe=[],j=null,F=null,rt=!1,B=null,ft=[],ne=[],mt=!1,ot=null,W=null,_t=!1;const jt=new Set;let st=null,D=null,tt=!1,bt=[],Mt=!1,Ut=!1,At=new Set;function $(a){if(!a)return"";const t=document.createElement("div");return t.textContent=a,t.innerHTML}function X(a,t){return((a||"?")[0]+(t||"?")[0]).toUpperCase()}const Ge=[{id:"leader",title:"LEADER",fullTitle:"Party Leader",color:"#c8a832"},{id:"deputy",title:"DEPUTY",fullTitle:"Deputy Party Leader",color:"#8b9a6b"},{id:"chief",title:"CHIEF OF STAFF",fullTitle:"Chief of Staff",color:"#5cc55c"},{id:"campaign",title:"CAMPAIGN MGR",fullTitle:"Campaign Manager",color:"#c84"},{id:"comms",title:"COMMS DIR",fullTitle:"Communications Director",color:"#5a8aaa"},{id:"agitator",title:"AGITATOR",fullTitle:"Opposition Coordinator",color:"#d44a4a",oppositionOnly:!0}];let $t=0,zt=0,re=!1,G={eligible:!1,lockReason:"Loading...",metaLine:""};async function Ra(){if(!C||!g?.faction?.id||!g?.shard?.current_tick)return;const{count:a,error:t}=await C.from("campaign_actions").select("id",{count:"exact",head:!0}).eq("party_id",g.faction.id).eq("action_type","fundraise").eq("tick_performed",g.shard.current_tick);$t=!t&&a!=null?a:0}async function Oa(){if(zt=0,re=!1,!C||!g?.nation?.id||!g?.shard?.current_tick)return;const a=g.shard.current_tick,t=B?.pm_party_id;try{const{data:e}=await C.from("bills").select("id").eq("nation_id",g.nation.id).eq("bill_type","no_confidence").in("status",["committee","floor"]).limit(1);if(re=!!(e&&e.length),t){const{data:o}=await C.from("campaign_actions").select("tick_performed").eq("nation_id",g.nation.id).eq("action_type","no_confidence_filed").eq("target_id",t).order("tick_performed",{ascending:!1}).limit(1).maybeSingle();if(o){const r=a-Number(o.tick_performed||0),i=typeof R<"u"&&R.NO_CONFIDENCE_COOLDOWN_TICKS||12;zt=Math.max(0,i-r)}}}catch(e){console.warn("[PartyActions] loadNoConfidenceState failed:",e?.message||e)}}async function Fa(){if(G={eligible:!1,lockReason:"Loading...",metaLine:""},!C||!g?.nation?.id||!g?.faction?.id)return;const a=g.nation,t=g.faction,e=Number(g?.shard?.current_tick)||0,o=(a.government_type||"").toLowerCase();if(o.includes("absolute monarchy")||o.includes("absolute_monarchy")){G={eligible:!1,lockReason:"Only available in parliamentary systems.",metaLine:""};return}if((t.seats||0)<=0){G={eligible:!1,lockReason:"Your party has no parliamentary seats.",metaLine:""};return}if(W&&(W.status==="formed"||W.status==="caretaker")){G={eligible:!1,lockReason:W.formation_type==="emergency_minority"?"A minority government is already in place.":"A government is already in place.",metaLine:"Active Coalition"};return}const{data:r}=await C.from("elections").select("id, election_tick").eq("nation_id",a.id).eq("status","completed").not("results","is",null).order("election_tick",{ascending:!1}).limit(1).maybeSingle();if(!r){G={eligible:!1,lockReason:"No completed election to form a government from.",metaLine:"No election yet"};return}const i=Number(r.election_tick||0),n=He,l=i+n,c=e-i,v=`Last election: ${ge(i)} · Becomes available ${ge(l)}`;if(c<n){const b=n-c;G={eligible:!1,lockReason:`Coalition window still open: ${b} tick${b!==1?"s":""} remaining.`,metaLine:v};return}const p=Number(a.total_seats)||100,s=Math.floor(p/2)+1,{data:d,error:u}=await C.from("factions").select("id, faction_name, seats, last_seen_tick").eq("nation_id",a.id).eq("faction_type","party");if(u){G={eligible:!1,lockReason:"Could not load party state.",metaLine:v};return}const y=d||[];if(y.some(b=>(b.seats||0)>=s)){G={eligible:!1,lockReason:"A party already holds an outright majority — form a normal government instead.",metaLine:v};return}const h=4,x=y.filter(b=>(b.seats||0)>0&&(Number(b.last_seen_tick)||0)>=e-h).sort((b,_)=>(_.seats||0)-(b.seats||0)||(b.id<_.id?-1:1))[0];if(!x){G={eligible:!1,lockReason:"No active parties qualify to form a government.",metaLine:v};return}if(x.id!==t.id&&!da()){G={eligible:!1,lockReason:`Only the largest active party (${x.faction_name||"unknown"}) may form a minority government.`,metaLine:v};return}G={eligible:!0,lockReason:"",metaLine:v}}const Ve=[{id:"fundraise",name:"Fundraise",desc:"Host a themed event for one voter bloc. Once per tick. Costs −0.3 popularity with the host bloc (donor fatigue) and −0.5 with a paired opposition bloc (optics). Yields cash to party funds based on your rapport with the host bloc and its national weight. Corporate Gala is positioning-only.",cost:"ACTION",costColor:"#c8a832",moneyCost:0,tags:["CAMPAIGN","POSITIONING"],locked:!1},{id:"statement",name:"Issue Statement",desc:"Public declaration on an issue. Shifts party positioning and voter bloc reactions. Media covers it. Other parties may respond.",cost:"$20k",costColor:"#c8a832",moneyCost:2e4,tags:["PUBLIC","NARRATIVE"],locked:!1},{id:"platform",name:"Set Party Platform",desc:"Choose a political focus. Defines which stats you promise to change. Awards momentum based on how many rivals share the same platform.",cost:"$120k",costColor:"#c8a832",moneyCost:12e4,tags:["STRATEGIC"],locked:!1},{id:"no_confidence",name:"Vote of No Confidence",desc:"File a motion of no confidence against the Prime Minister. If a simple majority votes YES, the government falls and snap elections are triggered. PASS: +15 Momentum to you, -10 Momentum to the PM’s party. FAIL: -10 Momentum to you. 12-tick cooldown on the targeted PM party.",cost:"$0",costColor:"var(--text-dim)",moneyCost:0,tags:["LEGISLATIVE","OPPOSITION"],locked:!1},{id:"leadership_challenge",name:"Leadership Challenge",desc:"Claim the vacant Premiership for your party leader. Available only when there is no sitting Prime Minister. If multiple coalition parties claim on the same tick, the largest-by-seats wins (earliest claim breaks ties). Winning parties get a one-time +0.3 popularity boost across all voter sectors.",cost:"COALITION ONLY",costColor:"#c8a832",moneyCost:0,tags:["GOVERNMENT","COALITION"],locked:!1},{id:"form_coalition",name:"Form Coalition",desc:"Open the coalition formation flow when no coalition government exists — invite parties, assemble at least the majority threshold of seats, then assign ministries and install a new Prime Minister. If a coalition exists but the PM is vacant, coalition members should use Leadership Challenge instead.",cost:"GOVERNMENT",costColor:"#c8a832",moneyCost:0,tags:["GOVERNMENT","COALITION"],locked:!1},{id:"form_minority_government",name:"Form Minority Government",desc:"Deadlock breaker. After the coalition window closes (3 ticks post-election) with no government formed, the leader of the largest active party can govern alone. Bills pass with -20% effective YES votes; a snap election fires automatically in 36 ticks if a stable coalition isn't formed before then.",cost:"GOVERNMENT",costColor:"#c84",moneyCost:0,tags:["GOVERNMENT","DEADLOCK"],locked:!1},{id:"leave_coalition",name:"Leave Coalition",desc:"Walk out of the current governing coalition. Any ministries your party holds are vacated. You drop from governing to opposition. Coalition flips to minority if your exit drops it below the majority threshold. Cost: −3 Momentum to you, −5 Momentum to the PM’s party. 12-tick cooldown. PM’s party cannot use this — resign first.",cost:"−3 MOM",costColor:"#c84",moneyCost:0,tags:["GOVERNMENT","RISKY"],locked:!1},{id:"disband_party",name:"Disband Party",desc:"Voluntarily dissolve your party. Your seats are vacated and sit empty until the next election (no backfill or redistribution). All party funds and momentum are lost. You are removed from every nation chat. Cannot be undone. 24-tick cooldown per user. Cannot be used while Prime Minister, sitting President, or reigning Monarch — step down first.",cost:"IRREVERSIBLE",costColor:"#c55",moneyCost:0,tags:["IRREVERSIBLE"],locked:!1}],Ba=[{id:"fundraise",name:"Fundraise",desc:"Raise royal treasury funds proportional to your seat count. Each use yields less money and costs more momentum.",cost:"MOMENTUM",costColor:"#c84",moneyCost:0,tags:["FINANCIAL","CAMPAIGN"],locked:!1},{id:"grant_seats",name:"Grant Seats",desc:"Grant parliamentary seats to a noble house. Sharing power increases crown authority (+0.5 per seat). Hoarding >70% of seats causes tyranny decay.",cost:"FREE",costColor:"#5cc55c",moneyCost:0,tags:["ROYAL","STRUCTURAL"],locked:!1},{id:"revoke_seats",name:"Revoke Seats",desc:"Revoke seats from a noble house. Costs $100k and -1 Crown Authority per seat revoked. Use sparingly — the nobles do not forget.",cost:"$100k/seat",costColor:"#d44a4a",moneyCost:1e5,tags:["ROYAL","OFFENSIVE"],locked:!1},{id:"statement",name:"Royal Decree",desc:"Issue a public declaration on an issue. Shifts positioning and voter bloc reactions. Media covers it.",cost:"$20k",costColor:"#c8a832",moneyCost:2e4,tags:["PUBLIC","NARRATIVE"],locked:!1},{id:"appoint_pm",name:"Appoint Prime Minister",desc:"Choose a party to lead the government as Prime Minister. The PM can then assign cabinet ministries. You may appoint your own party.",cost:"FREE",costColor:"#5cc55c",moneyCost:0,tags:["ROYAL","STRUCTURAL"],locked:!1}],Et={PUBLIC:"#8b9a6b",NARRATIVE:"#5a8aaa",STRATEGIC:"#c8a832",INTERNAL:"#c84",COALITION:"#5aaa8a",RISKY:"#c55",PARLIAMENTARY:"#8b9a6b",FINANCIAL:"#5a8aaa",INTELLIGENCE:"#5a8aaa",DEFENSIVE:"#5cc55c",CAMPAIGN:"#c84",VOTER:"#c8a832",OFFENSIVE:"#c84",REACTIVE:"#ca5",STRUCTURAL:"#9e9a92",ROYAL:"#c8a832",LEGAL:"#5a8aaa"},we=[{id:"economy",label:"Economy & Jobs",icon:"💰"},{id:"healthcare",label:"Healthcare",icon:"🏥"},{id:"education",label:"Education",icon:"🎓"},{id:"security",label:"National Security",icon:"🛡️"},{id:"environment",label:"Environment",icon:"🌱"},{id:"corruption",label:"Anti-Corruption",icon:"🔍"},{id:"infrastructure",label:"Infrastructure",icon:"🏗️"},{id:"immigration",label:"Immigration",icon:"🌐"},{id:"housing",label:"Housing & Cost of Living",icon:"🏠"},{id:"crime",label:"Crime & Justice",icon:"⚖️"},{id:"labor",label:"Labor & Workers",icon:"🔨"},{id:"foreign_policy",label:"Foreign Policy",icon:"🕊️"}],ke=["{party_name} Calls for Action on {topic}","{leader_name}: '{topic}' Must Be National Priority","{leader_name} Pledges Bold Agenda on {topic}","{party_name} Leader Addresses Nation on {topic}"];async function Ye(a,t){C=a,g=t;const e=document.getElementById("pa-actions-root");if(!e)return;const o=t.faction;if(!o){e.innerHTML='<div style="display:flex;align-items:center;justify-content:center;min-height:300px;color:var(--text-dim);">No faction data.</div>';return}try{const{data:s}=await C.from("factions").select("momentum, party_funds, seats, action_points, bloc_id, last_petition_for_reform_tick").eq("id",o.id).single();s&&(o.momentum=s.momentum??o.momentum,o.party_funds=s.party_funds??o.party_funds,o.seats=s.seats??o.seats,o.action_points=s.action_points??o.action_points,o.bloc_id=s.bloc_id??null,o.last_petition_for_reform_tick=s.last_petition_for_reform_tick??null)}catch(s){console.warn("[PartyActions] faction refresh failed, using cached state:",s)}try{const s=t.nation?.id;if(s){const{data:d}=await C.from("petitions").select("id").eq("nation_id",s).eq("status","pending").maybeSingle();o._petitionPending=!!d}}catch(s){console.warn("[PartyActions] petition-pending check failed:",s?.message||s),o._petitionPending=!1}const[r,i,n,l,c,v]=await Promise.all([C.from("faction_platforms").select("*").eq("faction_id",o.id).order("slot"),C.from("faction_platforms").select("*").eq("nation_id",t.nation?.id),je(C,o.id),qe(C,t.nation?.id,o.id),C.from("faction_electoral_standing").select("visibility, raw_appeal").eq("faction_id",o.id).eq("nation_id",t.nation?.id).maybeSingle(),De(C,t.nation?.id)]);W=v||null,t.nation&&(t.nation.__coalition_status=v?.status||null),r.error&&console.error("[PartyActions] Failed to load faction platforms:",r.error.message),i.error&&console.error("[PartyActions] Failed to load nation platforms:",i.error.message),Z=r.data||[],fe=i.data||[],j=n,rt=l.isOpposition,B=l.administration,c.data,await Ra(),await Oa(),await Fa();try{const[{data:s},{data:d}]=await Promise.all([C.from("head_of_government").select("id, faction_id, active").eq("nation_id",t.nation?.id).eq("active",!0).maybeSingle(),C.from("shard").select("current_tick").eq("name","Alpha Shard").single()]);ot=s||null;const u=Number(d?.current_tick)||0,{data:y}=await C.from("leadership_challenges").select("id").eq("nation_id",t.nation?.id).eq("faction_id",o.id).is("resolved_at_tick",null).gte("claimed_at_tick",u-1).limit(1).maybeSingle();_t=!!y}catch(s){console.warn("[PartyActions] HOG / leadership claim state load failed:",s?.message||s),ot=null,_t=!1}const{data:p}=await C.from("faction_deputies").select("*").eq("faction_id",o.id).eq("status","active").maybeSingle();if(F=p||null,st=null,t.nation?.id&&(t.nation.government_type||"").toLowerCase().includes("presidential")){const{data:d}=await C.from("presidents").select("id, faction_id, first_name, last_name, elected_tick").eq("nation_id",t.nation.id).eq("is_active",!0).maybeSingle();st=d||null}if(ft=[],o?.id&&t.nation?.id){const{data:s,error:d}=await C.from("ministries").select("id, ministry_key, party_id, is_active, minister_first_name, minister_last_name, minister_age, discretionary_balance").eq("nation_id",t.nation.id).eq("party_id",o.id).eq("is_active",!0);d?console.warn("[PartyActions] ministries fetch failed:",d.message):ft=(s||[]).filter(u=>u.minister_first_name)}j&&(ne=await Ta(C,o.id)),await Ct(o.id,t.nation?.id),O(e)}function ve(a){return a?{isPM:!!B&&B.pm_party_id===a.id,isPresident:g?.nation?.hos_election_method==="elected"&&B?.president_party_id===a.id,isMonarchActing:K(g?.nation)&&g?.nation?.monarch_faction_id===a.id}:{isPM:!1,isPresident:!1,isMonarchActing:!1}}async function Ct(a,t){if(!a||!t){D=null,tt=!1,bt=[];return}try{const{data:e,error:o}=await C.from("bloc_invitations").select("id, bloc_id, invited_by_faction_id, created_at_tick, status, bloc:bloc_id(id,name,leader_faction_id), inviter:invited_by_faction_id(id,faction_name,party_color)").eq("invited_faction_id",a).eq("status","pending").order("created_at_tick",{ascending:!1});if(o)throw o;bt=e||[];const r=g?.faction?.bloc_id||null;if(r){const{data:i,error:n}=await C.from("blocs").select("*").eq("id",r).is("dissolved_at_tick",null).maybeSingle();if(n)throw n;if(i){const{data:l}=await C.from("factions").select("id, faction_name, seats, party_color, leader_first_name, leader_last_name").eq("bloc_id",i.id).order("seats",{ascending:!1});D={...i,members:l||[]},tt=i.leader_faction_id===a}else D=null,tt=!1}else D=null,tt=!1}catch(e){console.warn("[PartyActions] loadBlocState failed:",e?.message||e)}}function We(a){if(!D)return"";const t=tt?`<span style="margin-left:6px;font-family:var(--font-mono);font-size:7px;color:${a};letter-spacing:0.08em;">LEADER</span>`:"";return`<span class="pa-bloc-tag" style="display:inline-flex;align-items:center;padding:2px 8px;background:${a}18;border:1px solid ${a}55;color:${a};font-family:var(--font-mono);font-size:9px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;">
        ...
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
            <span style="font-family:var(--font-mono);font-size:9px;font-weight:700;color:${a};letter-spacing:0.08em;">BLOC &middot; ${$(D.name)}</span>
    ...
    </div>`}function Je(a){if(!bt||bt.length===0)return"";const t=o=>(Array.isArray(o)?o[0]:o)||null;return`<div style="margin:10px 0 4px;">${bt.map(o=>{const r=t(o.bloc),i=t(o.inviter),n=r?.name||"a bloc",l=i?.faction_name||"A party leader",c=i?.party_color||a,v=At.has(o.id);return`<div style="margin:6px 0;padding:8px 12px;border:1px solid ${c}55;border-left:3px solid ${c};background:${c}08;display:flex;align-items:center;justify-content:space-between;gap:10px;">
            <div style="flex:1;">
        ...
        </div>`}).join("")}</div>`}async function ue(a){const{data:t}=await C.from("factions").select("bloc_id, momentum").eq("id",a).single();t&&(g.faction.bloc_id=t.bloc_id||null,t.momentum!=null&&(g.faction.momentum=t.momentum))}async function Da(a,t,e){try{const o=g?.faction?.id;if(!o)throw new Error("No active faction");const r=t==="accept"?"accept_bloc_invite":"decline_bloc_invite",i=t==="accept"?"p_accepting_faction_id":"p_declining_faction_id",{data:n,error:l}=await C.rpc(r,{p_invitation_id:a,[i]:o});if(l)throw l;if(n&&n.success===!1)throw new Error(n.error||"Unknown error");await ue(o),await Ct(o,g.nation?.id),O(e)}catch(o){console.error("[PartyActions] respondToBlocInvite failed:",o),alert(t==="accept"?`Could not accept invitation: ${o.message||o}`:`Could not decline invitation: ${o.message||o}`)}}async function Ha(a){if(!D||Ut)return;const t=D,e=tt?`Leaving ${t.name} will DISSOLVE the entire bloc. All ${t.members?.length||0} members will be removed and pending invitations rescinded.

Proceed?`:`Leave the ${t.name} bloc?`;if(confirm(e)){Ut=!0;try{const{data:o,error:r}=await C.rpc("leave_bloc",{p_faction_id:g.faction.id});if(r)throw r;if(o&&o.success===!1)throw new Error(o.error||"Unknown error");await ue(g.faction.id),await Ct(g.faction.id,g.nation?.id),O(a)}catch(o){console.error("[PartyActions] leave_bloc failed:",o),alert(`Could not leave bloc: ${o.message||o}`)}finally{Ut=!1}}}async function qa(a){const t=document.getElementById("pa-bloc-modal");if(!t||D)return;const e=g.faction,o=e?.color||"#c8a832";t.innerHTML=`
    ...
    `,t.classList.add("active");const r=new Set;let i=[];const n=()=>t.classList.remove("active");document.getElementById("pa-bloc-close")?.addEventListener("click",n),document.getElementById("pa-bloc-cancel")?.addEventListener("click",n),t.addEventListener("click",s=>{s.target===t&&n()});try{const s=g.nation?.id,{data:d}=await C.from("factions").select("id, faction_name, seats, party_color, leader_first_name, leader_last_name, leader_age, bloc_id").eq("nation_id",s).eq("faction_type","party").is("abandoned_at",null),u=(d||[]).filter(f=>f.id!==e.id);i=u;const y=document.getElementById("pa-bloc-party-list");if(!y)return;if(u.length===0){y.innerHTML='<div style="padding:10px;font-family:var(--font-mono);font-size:9px;color:var(--text-dim);">No other parties in this nation.</div>';return}y.innerHTML=u.map(f=>{const h=f.party_color||"#7a7a7a",m=f.leader_first_name&&f.leader_last_name?`${f.leader_first_name} ${f.leader_last_name}`:"Party Leader",x=f.bloc_id?"Already in a bloc":null;return`<label class="pa-bloc-party-row" data-party-id="${$(f.id)}" data-ineligible="${x?"1":"0"}"
            ...
            </label>`}).join(""),y.addEventListener("change",f=>{const h=f.target.closest(".pa-bloc-party-row");if(!h)return;if(h.dataset.ineligible==="1"){f.target.checked=!1;return}const m=h.dataset.partyId;f.target.checked?r.add(m):r.delete(m),p()})}catch(s){console.error("[PartyActions] Create Bloc modal fetch failed:",s);const d=document.getElementById("pa-bloc-party-list");d&&(d.innerHTML=`<div style="padding:10px;font-family:var(--font-mono);font-size:9px;color:var(--red);">Failed to load parties: ${$(s.message||String(s))}</div>`)}const l=document.getElementById("pa-bloc-name"),c=document.getElementById("pa-bloc-submit"),v=document.getElementById("pa-bloc-name-count"),p=()=>{const s=(l?.value||"").trim();v&&(v.textContent=`${s.length} / 40`),c&&(c.disabled=!(s.length>0&&r.size>0)||Mt)};l?.addEventListener("input",p),c?.addEventListener("click",async()=>{if(Mt)return;const s=(l?.value||"").trim();if(!(s.length===0||r.size===0)){Mt=!0,c.disabled=!0,c.textContent="Creating...";try{const{data:d,error:u}=await C.rpc("create_bloc",{p_leader_faction_id:e.id,p_name:s,p_invitee_faction_ids:Array.from(r)});if(u)throw u;if(d&&d.success===!1)throw new Error(d.error||"Unknown error");g.faction.party_funds=Math.max(0,(g.faction.party_funds||0)-1e5),await ue(e.id),n(),await Ct(e.id,g.nation?.id),O(a)}catch(d){console.error("[PartyActions] create_bloc failed:",d),alert(`Could not create bloc: ${d.message||d}`),c.disabled=!1,c.textContent="Create Bloc & Send Invites"}finally{Mt=!1}}})}async function ja(a){if(!D||!tt)return;const t=document.getElementById("pa-bloc-modal");if(!t)return;const e=g.faction?.color||"#c8a832";t.innerHTML=`
        <div class="pa-modal" style="width:520px;max-height:75vh;overflow:hidden;display:flex;flex-direction:column;">
            <div class="pa-modal-header">
                <div class="pa-modal-header-left">
                    ...
                    <span class="pa-modal-title">Invite to ${$(D.name)}</span>
                </div>
        ...
        `}),document.getElementById("pa-leaders").innerHTML=Ua(n,i,t),document.getElementById("pa-actions-panel").innerHTML=ye(n,i,t);for(const m of Object.keys(Rt))ea(m);document.getElementById("pa-leaders")?.addEventListener("click",m=>{const x=m.target.closest(".pa-leader-card");if(!x||x.classList.contains("vacant"))return;const b=x.dataset.role;b&&b!==V&&(V=b,O(a))}),document.getElementById("pa-actions-panel")?.addEventListener("click",m=>{const x=m.target.closest(".pa-action-item");if(!x||x.classList.contains("locked"))return;const b=x.dataset.actionId;if(b==="fundraise")zo(a);else if(b==="grant_seats")wo(a);else if(b==="revoke_seats")ko(a);else if(b==="rally")oo(a);else if(b==="statement")Bo(a);else if(b==="platform")Do(a);else if(b==="file_lawsuit")_o(a);else if(b==="petition_for_reform")bo();else if(b==="appoint_pm")$o(a);else if(b==="modernize")fo(a);else if(b==="rebrand")vo(a);else if(b==="no_confidence")Ao();else if(b==="call_early_elections")Eo();else if(b==="resign_as_pm")So();else if(b==="leave_coalition")Io();else if(b==="disband_party")Lo();else if(b==="create_bloc")qa(a);else if(b==="leave_bloc")Ha(a);else if(b==="invite_to_bloc")ja(a);else if(b==="impeach_president")No();else if(b==="debt_payment")no(a);else if(b==="allocate_funds")ro(a);else if(b==="invest_in_sports_culture")io(a,t);else if(b==="expand_stadium_infrastructure")so(a,t);else if(b==="expand_infrastructure")lo(a,t);else if(Rt[b])ho(b,t);else if(b==="bid_to_host_vwc")po(a,t);else if(b==="leadership_challenge")Mo(a,t);else if(b==="form_coalition"){const _=document.querySelector('.pa-subtab[data-panel="election"]');_&&_.click()}else b==="form_minority_government"&&Co()}),document.getElementById("pa-actions-panel")?.addEventListener("click",async m=>{const x=m.target.closest(".pa-bloc-invite-btn");if(!x)return;const b=x.dataset.inviteId,_=x.dataset.decision;if(!(!b||!_)&&!At.has(b)){At.add(b);try{await Da(b,_,a)}finally{At.delete(b)}}}),document.getElementById("pa-hire-agitator-btn")?.addEventListener("click",()=>Le(a)),document.getElementById("pa-hire-agitator-panel")?.addEventListener("click",m=>{m.target.closest("#pa-hire-agitator-btn")||Le(a)}),document.getElementById("pa-hire-deputy-btn")?.addEventListener("click",()=>Ce(a)),document.getElementById("pa-hire-deputy-panel")?.addEventListener("click",m=>{m.target.closest("#pa-hire-deputy-btn")||Ce(a)})}function Ua(a,t,e){const o=K(g.nation)&&g.nation?.monarch_faction_id===e?.id;return Ge.map(r=>{const i=r.id==="leader",n=r.id==="agitator",l=V===r.id;let c,v,p,s,d;if(i){c=!1,v=a,p=X(e.leader_first_name,e.leader_last_name),s=Ve.length;const f=K(g.nation);if(f&&g.nation?.monarch_faction_id===e.id)d={text:(g.nation?.monarch_title||"KING").toUpperCase(),color:"#c8a832"};else if(f)d={text:"NOBLE HOUSE",color:"#8b9a6b"};else{const m=B?.pm_party_id===e.id,x=g.nation?.hos_election_method==="elected"&&B?.president_party_id===e.id;m?d={text:"PRIME MINISTER",color:"#5cc55c"}:x?d={text:"PRESIDENT",color:"#5cc55c"}:rt?d={text:"OPPOSITION",color:"#c84"}:d={text:"GOVERNING",color:"#8b9a6b"}}}else n&&j?(c=!1,v=`${j.first_name} ${j.last_name}`,p=X(j.first_name,j.last_name),s=1):n&&!j?(c=!1,v="Not Hired",p="+",s=0):r.id==="deputy"&&F?(c=!1,v=`${F.first_name} ${F.last_name}`,p=X(F.first_name,F.last_name),s=1):r.id==="deputy"&&!F?(c=!1,v="Not Hired",p="+",s=0):r.id==="campaign"?(c=!1,v="Campaign Mgr",p="CM",s=Qe.length):(c=!0,v="Vacant",p="—",s=0);const u=r.oppositionOnly&&!rt;return`
            <div class="pa-leader-card ${l?"active":""} ${c?"vacant":""} ${u?"vacant":""}"
                 data-role="${r.id}"
                 style="${l?`border-left-color:${r.color};`:""}${u?"opacity:0.35;":""}">
                ${r.oppositionOnly?`<div style="position:absolute;top:0;right:0;font-family:var(--font-mono);font-size:5px;font-weight:700;letter-spacing:0.04em;padding:1px 4px;color:${u?"var(--text-dim)":"#d44a4a"};background:${u?"rgba(100,100,100,0.1)":"rgba(212,74,74,0.1)"};border:1px solid ${u?"rgba(100,100,100,0.2)":"rgba(212,74,74,0.2)"};border-top:none;border-right:none;">${u?"IN GOVERNMENT":"OPPOSITION ONLY"}</div>`:""}
    ...
    `}function ye(a,t,e){if(typeof V=="string"&&V.startsWith("ministry:"))return Qa(e);const o=K(g.nation),r=o&&g.nation?.monarch_faction_id===e?.id,i=Ge.find(m=>m.id===V);if(!i)return"";const n=V==="leader",l=V==="agitator",c=V==="campaign",v=V==="deputy";if(!n&&!l&&!c&&!v)return`
        ...
        `;if(l&&j)return yo(i);if(v&&!F)return`
    ...
    `}function ao(a){const t=Fe(a),e=t.firstNames||[],o=t.lastNames||[];if(e.length===0||o.length===0)return[];const r=5+Math.floor(Math.random()*3),i=new Set,n=[];for(let l=0;l<r;l++){let c,v,p,s=0;do c=e[Math.floor(Math.random()*e.length)],v=o[Math.floor(Math.random()*o.length)],p=c+" "+v,s++;while(i.has(p)&&s<20);i.add(p);const d=20+Math.floor(Math.random()*66),u=28+Math.floor(Math.random()*30),y=Math.max(0,d-20)/65,f=Math.round((125e3+y*525e3)/25e3)*25e3;n.push({first_name:c,last_name:v,age:u,skill:d,hire_cost:f})}return n.sort((l,c)=>c.skill-l.skill)}async function Ce(a){const t=document.getElementById("pa-deputy-modal");if(!t)return;const e=g.nation?.name,o=ao(e);let r=null;function i(){const n=r!=null?o[r]:null,l=n?dt(n.skill):null,c=o.map((s,d)=>{const u=r===d,y=dt(s.skill);return`<div class="pa-hire-row ${u?"selected":""}" data-idx="${d}">
                ...
                <div style="flex:1;min-width:0;">
                    <div style="font-size:11px;font-weight:600;color:${u?"var(--text-bright)":"var(--text-secondary)"};">${$(s.first_name)} ${$(s.last_name)}</div>
                    <div style="display:flex;align-items:center;gap:3px;margin-top:2px;">
                        <div style="flex:1;height:2px;background:var(--border-mid);max-width:60px;"><div style="height:100%;width:${s.skill}%;background:${y.color};"></div></div>
                        <span style="font-family:var(--font-mono);font-size:8px;color:${y.color};">${s.skill}</span>
                    </div>
                </div>
        ...
        `;const p=()=>t.classList.remove("active");document.getElementById("pa-dep-close")?.addEventListener("click",p),t.onclick=s=>{s.target===t&&p()},document.getElementById("pa-dep-list")?.addEventListener("click",s=>{const d=s.target.closest(".pa-hire-row");d&&(r=parseInt(d.dataset.idx,10),i())}),document.getElementById("pa-dep-hire-confirm")?.addEventListener("click",async()=>{if(r==null)return;const s=o[r],d=g.faction?.party_funds||0;if(d<s.hire_cost){alert("Not enough funds.");return}const u=document.getElementById("pa-dep-hire-confirm");u&&(u.disabled=!0,u.textContent="Hiring...");try{const y=d-s.hire_cost,f=g.shard?.current_tick||0,{data:h,error:m}=await C.from("faction_deputies").insert({faction_id:g.faction.id,first_name:s.first_name,last_name:s.last_name,age:s.age,skill:s.skill,status:"active",hired_at_tick:f}).select("*").single();if(m){alert("Failed: "+m.message);return}await C.from("factions").update({party_funds:y}).eq("id",g.faction.id),g.faction.party_funds=y,F=h,V="deputy",p(),O(a)}catch(y){console.error("[Deputy] Hire error:",y)}finally{u&&(u.disabled=!1)}})}t.classList.add("active"),i()}function oo(a){const t=document.getElementById("pa-rally-modal");if(!t||!F)return;const o=g.faction.party_funds||0;let r=null,i=null;function n(){const l=Ee.map((p,s)=>{const d=o>=p.cost,u=r===s;return`<div class="pa-action-item ${u?"selected":""} ${d?"":"locked"}" data-tier="${s}" style="cursor:${d?"pointer":"not-allowed"};${u?"border-color:#8b9a6b;background:rgba(139,154,107,0.06);":""}">
                <div class="pa-action-top">
                    <span style="font-size:13px;font-weight:700;color:${u?"#8b9a6b":"var(--text-bright)"};">$${Math.round(p.cost/1e3)}k Investment</span>
        ...
        `;const v=()=>{t.classList.remove("active"),i&&O(a)};document.getElementById("rally-close")?.addEventListener("click",v),document.getElementById("rally-cancel")?.addEventListener("click",v),t.onclick=p=>{p.target===t&&v()},document.getElementById("rally-tiers")?.addEventListener("click",p=>{const s=p.target.closest("[data-tier]");!s||s.classList.contains("locked")||(r=parseInt(s.dataset.tier,10),n())}),document.getElementById("rally-submit")?.addEventListener("click",async()=>{if(r==null||i)return;const p=Ee[r],{data:s}=await C.from("factions").select("party_funds, momentum").eq("id",g.faction.id).single(),d=s?.party_funds||0;if(d<p.cost){alert("Not enough funds.");return}g.faction.party_funds=d,g.faction.momentum=s?.momentum??g.faction.momentum;const u=document.getElementById("rally-submit");u&&(u.disabled=!0,u.textContent="Rolling...");try{const y=1+Math.floor(Math.random()*6),f=to(y,p.bonus),h=d-p.cost,m=Math.max(1,(g.faction.momentum||0)+f.momentum);await C.from("factions").update({party_funds:h,momentum:m}).eq("id",g.faction.id);const x=g.shard?.current_tick||0;await C.from("campaign_actions").insert({party_id:g.faction.id,nation_id:g.nation?.id,action_type:"rally",ap_cost:0,money_cost:p.cost,tick_performed:x,result:{dieRoll:y,bonus:p.bonus,total:y+p.bonus,momentum:f.momentum,momentumDelta:f.momentum,label:f.label,outcomeName:f.label}}),g.faction.party_funds=h,g.faction.momentum=m,sessionStorage.removeItem("nationhood_state"),i={...f,dieRoll:y,bonus:p.bonus,total:y+p.bonus},n()}catch(y){console.error("[Rally] Error:",y),alert("Rally failed.")}})}t.classList.add("active"),n()}async function io(a,t){const e=document.getElementById("pa-vola-invest-modal");if(!e)return;const{data:o}=await C.from("ministries").select("id, party_id, discretionary_balance").eq("nation_id",g.nation.id).eq("ministry_key","sports").eq("is_active",!0).maybeSingle(),r=Number(o?.discretionary_balance)||0;let i=!1,n=null;function l(){const v=["low","moderate","high"].map(d=>({key:d,cfg:ga[d]})).map(d=>{const u=r>=d.cfg.cost,y="$"+d.cfg.cost/1e6;return`<div class="pa-action-item ${!u||i?"locked":""}" data-tier="${d.key}" style="cursor:${u&&!i?"pointer":"not-allowed"};">
                <div class="pa-action-top">
        ...
        `;const s=()=>{e.classList.remove("active"),n&&O(a)};document.getElementById("vola-close")?.addEventListener("click",s),document.getElementById("vola-cancel")?.addEventListener("click",s),e.onclick=d=>{d.target===e&&s()},document.getElementById("vola-tiers")?.addEventListener("click",async d=>{const u=d.target.closest("[data-tier]");if(!u||u.classList.contains("locked")||i||n)return;const y=u.dataset.tier;i=!0,l();try{const{data:f}=await C.from("shard").select("current_tick").eq("name","Alpha Shard").single(),h=Number(f?.current_tick)||0,m=await ba(C,g.nation,t.id,y,h);m?.success?n=m:alert("Could not invest: "+(m?.reason||"unknown error"))}catch(f){alert("Investment failed: "+(f?.message||f))}finally{i=!1,l()}})}e.classList.add("active"),l()}async function no(a,t){const e=document.getElementById("pa-debt-payment-modal");if(!e)return;const o=_=>Math.floor((Number(_)||0)/1e6),r=2;let i="";const[n,l]=await Promise.all([C.from("ministries").select("id, party_id, discretionary_balance").eq("nation_id",g.nation.id).eq("ministry_key","finance").eq("is_active",!0).maybeSingle(),C.from("nations").select("debt").eq("id",g.nation.id).maybeSingle()]);(n.error||l.error)&&(i=(n.error||l.error).message||"Could not load ministry / nation data.");const c=o(n.data?.discretionary_balance),v=o(l.data?.debt),p=Math.max(0,c-r);let s=!1,d=null,u="",y=i;function f(){const _=parseInt(u,10);return Number.isFinite(_)?_:null}function h(){const _=f();return _!=null&&_>=1&&_<=p}function m(){const _=f(),M=_!=null&&_>=1?_+r:null,w=M!=null?c-M:c,k=_!=null&&_>=1?Math.max(0,v-_):v,E=h(),S=d?`
            <div style="padding:12px;background:rgba(200,168,50,0.08);border:1px solid rgba(200,168,50,0.22);margin-top:12px;">
        ...
        `:"",L=y?`<div style="font-family:var(--font-mono);font-size:10px;color:var(--red);margin-top:6px;">${$(y)}</div>`:"";e.innerHTML=`
            <div class="pa-modal" style="width:480px;">
                <div class="pa-modal-header">
                    <div class="pa-modal-header-left">
                        <div class="pa-modal-dot" style="background:#c8a832;"></div>
                ...
                <div class="pa-modal-body" style="gap:10px;">
                    ...
                    <input id="pa-dp-input" class="pa-modal-input" type="number" min="1" max="${p}" step="1" placeholder="0" value="${$(u)}" ${d||s?"disabled":""} style="font-family:var(--font-mono);font-size:14px;">
                ...
                <div class="pa-modal-footer">
                    ...
                    <button class="pa-modal-btn pa-modal-btn--submit" id="pa-dp-pay" ${!E||s||d||i?"disabled":""}>${s?"Paying…":"Pay"}</button>
                </div>
            </div>
        `;const T=()=>{e.classList.remove("active"),d&&O(a)};document.getElementById("pa-dp-close")?.addEventListener("click",T),document.getElementById("pa-dp-cancel")?.addEventListener("click",T),e.onclick=I=>{I.target===e&&T()};const P=document.getElementById("pa-dp-input");P&&(P.addEventListener("input",I=>{u=(I.target.value||"").replace(/[^0-9]/g,""),y=i,m();const N=document.getElementById("pa-dp-input");N&&(N.focus(),N.setSelectionRange(N.value.length,N.value.length))}),P.addEventListener("keydown",I=>{I.key==="Enter"&&h()&&!s&&!d&&!i&&(I.preventDefault(),x())})),document.getElementById("pa-dp-pay")?.addEventListener("click",x)}async function x(){if(s||d||i)return;const _=f();if(!h()){y=`Enter an integer between 1 and ${p}.`,m();return}s=!0,y="",m();try{const{data:M,error:w}=await C.rpc("pay_down_national_debt",{p_payment:_});w?y=w.message||"Payment failed.":M?.success?d=M:y=b(M?.reason)||"Payment failed."}catch(M){y=M?.message||"Network error."}finally{s=!1,m()}}function b(_){switch(_){case"invalid_payment":return"Enter an integer of at least $1.";case"not_minister":return"Only the Finance Minister can fire this action.";case"no_shard":return"Shard not initialized.";case"cooldown":return"Already used this tick. Try again next tick.";case"insufficient_balance":return"Not enough discretionary budget for fee + payment.";case"no_debt":return"There is no national debt to pay down.";default:return _||""}}e.classList.add("active"),m()}async function ro(a){const t=document.getElementById("pa-allocate-funds-modal");if(!t)return;const e=w=>Math.floor((Number(w)||0)/1e6),o=g?.nation?.name||"the Nation";let r="";const[i,n]=await Promise.all([C.from("ministries").select("discretionary_balance").eq("nation_id",g.nation.id).eq("ministry_key","defense").eq("is_active",!0).maybeSingle(),C.from("factions").select("party_funds").eq("nation_id",g.nation.id).eq("faction_type","military").eq("branch","army").is("abandoned_at",null).or("is_banned.is.null,is_banned.eq.false").order("created_at",{ascending:!0}).limit(1).maybeSingle()]);i.error&&(r=i.error.message||"Could not load ministry data."),n.error&&n.error.code!=="PGRST116"&&(r=r||n.error.message||"Could not load army faction.");const l=e(i.data?.discretionary_balance),c=n.data||null,v=!!c,p=e(c?.party_funds),s=Math.max(0,l);let d=!1,u=null,y="",f=r;function h(){const w=parseInt(y,10);return Number.isFinite(w)?w:null}function m(){const w=h();return v&&w!=null&&w>=1&&w<=s}function x(w,k){return`
            ...
            </div>`}function b(){const w=h(),k=m(),E=w!=null&&w>=1?l-w:l,S=w!=null&&w>=1?p+w:p,L=u?`
            <div style="padding:12px;background:rgba(200,168,50,0.08);border:1px solid rgba(200,168,50,0.22);margin-top:12px;">
            ...
            <div class="pa-modal" style="width:500px;">
                ...
                <div class="pa-modal-body" style="gap:10px;">
                    ...
                    ${v?`
                        ...
                        <input id="pa-af-input" class="pa-modal-input" type="number" min="1" max="${s}" step="1" placeholder="0" value="${$(y)}" ${u||d?"disabled":""} style="font-family:var(--font-mono);font-size:14px;">
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;padding-top:4px;">
                ...
                <div class="pa-modal-footer">
                    ...
                    <button class="pa-modal-btn pa-modal-btn--submit" id="pa-af-submit" ${!k||d||u||r?"disabled":""}>${d?"Allocating…":"Allocate"}</button>
                </div>
            </div>
        `;const P=()=>{t.classList.remove("active"),u&&O(a)};document.getElementById("pa-af-close")?.addEventListener("click",P),document.getElementById("pa-af-cancel")?.addEventListener("click",P),t.onclick=N=>{N.target===t&&P()};const I=document.getElementById("pa-af-input");I&&(I.addEventListener("input",N=>{y=(N.target.value||"").replace(/[^0-9]/g,""),f=r,b();const A=document.getElementById("pa-af-input");A&&(A.focus(),A.setSelectionRange(A.value.length,A.value.length))}),I.addEventListener("keydown",N=>{N.key==="Enter"&&m()&&!d&&!u&&!r&&(N.preventDefault(),_())})),document.getElementById("pa-af-submit")?.addEventListener("click",_)}async function _(){if(d||u||r)return;if(!m()){f=`Enter an integer between 1 and ${s}.`,b();return}const w=h();d=!0,f="",b();try{const{data:k,error:E}=await C.rpc("allocate_defense_funds",{p_branch:"army",p_amount:w});E?f=E.message||"Allocation failed.":k?.success?u=k:f=M(k?.reason)||"Allocation failed."}catch(k){f=k?.message||"Network error."}finally{d=!1,b()}}function M(w){switch(w){case"not_authenticated":return"You are not signed in.";case"branch_unavailable":return"Only the Army can receive funds right now.";case"invalid_amount":return"Enter an integer of at least $1.";case"not_minister":return"Only the Minister of Defense can allocate funds.";case"insufficient_funds":return"Not enough discretionary budget for that allocation.";case"no_army_faction":return"No army has been established for this nation yet.";default:return w||""}}t.classList.add("active"),b()}async function so(a,t){const e=document.getElementById("pa-vola-stadium-modal");if(!e)return;let o="",r="",i=null,n=!1,l=null,c=[],v=null;async function p(){const{data:u}=await C.from("ministries").select("id, party_id, discretionary_balance").eq("nation_id",g.nation.id).eq("ministry_key","sports").eq("is_active",!0).maybeSingle(),y=Number(u?.discretionary_balance)||0,{data:f}=await C.from("corp_contracts").select("id, name, description, spec_category, expires_at_tick, created_at_tick").eq("issuer_nation_id",g.nation.id).eq("project_subtype","Vola Stadium").eq("status","open").order("created_at_tick",{ascending:!1}).limit(1).maybeSingle();if(l=f||null,c=[],l){const{data:h}=await C.from("corp_contract_bids").select("id, faction_id, bid_amount, quoted_timeline_months, status, created_at_tick, factions:faction_id(id, faction_name, nation_id, nations:nation_id(name))").eq("contract_id",l.id).eq("status","pending").order("created_at_tick",{ascending:!0});c=h||[]}return{balance:y,hasMinister:!!u,isMinister:u?.party_id===t.id}}async function s(){const{balance:u,hasMinister:y,isMinister:f}=await p(),h=["small","modest","extravagant"].map(k=>({key:k,cfg:xa[k]})),m=u>0?"var(--green)":"var(--red)";let x="";if(l){const k=l.spec_category==="Light Infrastructure"?2:l.spec_category==="Heavy Infrastructure"?4:l.spec_category==="Megaproject"?9:0,E=(l.description||"").replace(/^Home of:\s*/i,"").trim();if(x+=`
            ...
            `,x+=`<div class="pa-modal-step-label" style="margin-top:14px;">Submitted Stadium Bids ${c.length?"· "+c.length:""}</div>`,c.length===0)x+='<div style="padding:14px;font-family:var(--font-mono);font-size:10px;color:var(--text-dim);font-style:italic;text-align:center;">Awaiting corporate bids…</div>';else{const S=L=>{if(L==null)return"—";const T=["January","February","March","April","May","June","July","August","September","October","November","December"],P=2e3+Math.floor(L/12);return`${T[L%12]}, ${P}`};x+=c.map(L=>{const T=L.factions?.faction_name||"Unknown Corp",P=L.factions?.nations?.name||"—",I=Number(L.bid_amount||0),N=I>=1e9?"$"+(I/1e9).toFixed(2)+"B":I>=1e6?"$"+(I/1e6).toFixed(1)+"M":"$"+Math.round(I).toLocaleString(),A=Number(L.quoted_timeline_months||0),z=Number(L.created_at_tick||0)+A;return`<div class="pa-action-item" style="cursor:default;" data-bid-id="${$(L.id)}">
                    ...
                    </div>`}).join("")}}else{const k=h.map(E=>{const S=u>=E.cfg.postingCost,L=i===E.key,T="$"+E.cfg.postingCost/1e6,P="$"+E.cfg.budgetTarget/1e6+"M";return`<div class="pa-action-item ${!S||n?"locked":""} ${L?"selected":""}" data-size="${E.key}" style="cursor:${S&&!n?"pointer":"not-allowed"};${L?"border-color:#c8a832;background:rgba(200,168,50,0.06);":""}">
                    <div class="pa-action-top">
            ...
            <div class="pa-modal" style="width:560px;max-height:85vh;overflow-y:auto;">
                ...
                <div style="padding:10px 16px;border-bottom:1px solid var(--border-main);font-size:11px;color:var(--text-secondary);line-height:1.5;">
                    ${y&&f?`Posting cost pulls from the Sports Ministry's discretionary budget — <strong style="color:${m};">${vt(u)}</strong> available. Top it up via a funding article on a passed bill.`:'<span style="color:var(--red);">You are no longer the active Sports Minister.</span>'}
                </div>
                <div class="pa-modal-body" style="gap:6px;">
                    ${x}
                </div>
...
Discretionary cost will be refunded.`)){n=!0,s();try{const{data:k,error:E}=await C.rpc("cancel_stadium_contract",{p_contract_id:l.id});if(E)throw E;if(!k?.success)throw new Error(k?.error||"Cancel failed");l=null,c=[]}catch(k){alert("Cancel failed: "+(k?.message||k))}finally{n=!1,s()}}})}function d(u){return{no_minister:"No active Sports Minister.",not_minister:"Only the Sports Minister can post stadium contracts.",insufficient_balance:"Sports discretionary budget is below the tier cost — pass a funding bill first.",already_open:"A stadium contract is already open. Wait for it to resolve, or cancel it first.",no_stadium_name:"Stadium name is required.",invalid_size:"Pick a stadium size first.",insert_failed:"Could not post the contract. Try again in a moment."}[u]}e.classList.add("active"),await s()}async function lo(a,t){const e=document.getElementById("pa-expand-infra-modal");if(!e)return;let o=null,r=null,i=!1,n=null,l=[],c=null;async function v(){const{data:s}=await C.from("ministries").select("id, party_id, discretionary_balance").eq("nation_id",g.nation.id).eq("ministry_key","interior").eq("is_active",!0).maybeSingle(),d=Number(s?.discretionary_balance)||0,{data:u}=await C.from("corp_contracts").select("id, name, description, spec_category, budget, timeline_months, status, expires_at_tick, created_at_tick").eq("issuer_nation_id",g.nation.id).eq("project_subtype","Interior Infrastructure").in("status",["open","active"]).order("created_at_tick",{ascending:!1}).limit(1).maybeSingle();if(n=u||null,l=[],n&&n.status!=="active"){const{data:y}=await C.from("corp_contract_bids").select("id, faction_id, bid_amount, quoted_timeline_months, status, created_at_tick, factions:faction_id(id, faction_name, nation_id, nations:nation_id(name))").eq("contract_id",n.id).eq("status","pending").order("bid_amount",{ascending:!0});l=y||[]}if(!o){const{data:y,error:f}=await C.rpc("interior_infrastructure_tiers");f?(c="Could not load tier specs: "+f.message,o={}):o=y||{}}return{balance:d,isMinister:!!s&&s.party_id===t.id}}async function p(){const{balance:s,isMinister:d}=await v(),u=s>0?"var(--green)":"var(--red)",y=b=>"$"+(Number(b)/1e6).toFixed(Number(b)%1e6===0?0:1)+"M",f=n?.status==="active";let h="";if(n)h+=`
            ...
            `,f?h+='<div style="margin-top:10px;padding:10px 12px;font-family:var(--font-mono);font-size:10px;color:var(--text-dim);background:rgba(90,175,165,0.06);border:1px solid rgba(90,175,165,0.2);">Construction is underway. Stat boosts apply on completion.</div>':(h+=`<div class="pa-modal-step-label" style="margin-top:14px;">Pending Bids ${l.length?"· "+l.length:""}</div>`,l.length===0?h+='<div style="padding:14px;font-family:var(--font-mono);font-size:10px;color:var(--text-dim);font-style:italic;text-align:center;">Awaiting construction corporation bids…</div>':h+=l.map(b=>{const _=b.factions?.faction_name||"Unknown Corp",M=b.factions?.nations?.name||"—",w=Number(b.bid_amount||0),k=w>=1e9?"$"+(w/1e9).toFixed(2)+"B":w>=1e6?"$"+(w/1e6).toFixed(1)+"M":"$"+Math.round(w).toLocaleString(),E=Number(b.quoted_timeline_months||0);return`<div class="pa-action-item" style="cursor:default;">
                ...
                <div id="expand-infra-tiers">${b.map(w=>{const k=o?.[w];if(!k)return"";const E=Number(k.post_cost||0),S=Number(k.budget||0),L=s>=E,T=r===w,P=(k.stat_effects||[]).map(I=>{const N=Number(I.delta)>=0?"+":"",A=String(I.stat).replace(/_/g," ").replace(/\b\w/g,z=>z.toUpperCase());return`<span class="pa-action-tag" style="color:var(--green);">${N}${I.delta} ${A}</span>`}).join(" ");return`<div class="pa-action-item ${!L||i?"locked":""} ${T?"selected":""}" data-size="${w}" style="cursor:${L&&!i?"pointer":"not-allowed"};${T?"border-color:#5aafa5;background:rgba(90,175,165,0.06);":""}">
                    ...
                    <div style="font-family:var(--font-mono);font-size:9px;color:var(--text-dim);margin-top:2px;">
                        Post fee ${y(E)} · Contract budget ${y(S)} · Timeline ${k.timeline} ticks
                    </div>
                    ${P?`<div class="pa-action-tags" style="margin-top:4px;">${P}</div>`:""}
                    ${L?"":`<div style="font-family:var(--font-mono);font-size:8px;color:var(--red);margin-top:4px;">Insufficient discretionary — need ${y(E)}</div>`}
                </div>`}).join("")}</div>
            ...
            <div class="pa-modal" style="width:560px;max-height:85vh;overflow-y:auto;">
                ...
                <div style="padding:10px 16px;border-bottom:1px solid var(--border-main);font-size:11px;color:var(--text-secondary);line-height:1.5;">
                    ${d?`Post fee pulls from the Interior Ministry's discretionary budget — <strong style="color:${u};">${vt(s)}</strong> available. Construction corps bid; the best-scoring bid (cost, timeline, reputation) auto-wins when the bid window closes. Stat boosts apply on completion.`:'<span style="color:var(--red);">You are no longer the active Interior Minister.</span>'}
                </div>
                <div class="pa-modal-body" style="gap:6px;">
                    ${h}
                </div>
...
Resolves at the qualifier tick (12 ticks before the cup begins).`)){o=!0,r=null,c();try{const E=await _a(C,k,g.nation.id);if(E?.success){i=E;const{data:S}=await C.from("shard").select("current_tick").eq("name","Alpha Shard").single(),L=Number(S?.current_tick)||Number(E.cupStartTick)-999;n.set(k,L)}else E?.reason==="already_bid"&&!n.has(k)&&n.set(k,null),r=v(E?.reason)||"Could not submit: "+(E?.reason||"unknown error")}catch(E){r="Bid failed: "+(E?.message||E)}finally{o=!1,c()}}})})}function v(p){return{invalid_cup:"Invalid cup selection.",invalid_nation:"Nation context unavailable. Reload and try again.",not_minister:"Only the Sports Minister can submit host bids.",insufficient_balance:"Sports discretionary budget is below $10M — pass a funding bill first.",no_shard:"Game state unavailable. Try again.",bidding_closed:"Bidding window has closed for this cup.",already_hosted:"This cup has already been awarded.",already_bid:"You have already bid for this cup.",cup_not_open_yet:"This cup’s bid window opens 1 tick after the previous cup begins."}[p]}e.classList.add("active"),await c()}const Qe=[{id:"modernize",name:"Modernize Image",desc:"Upload a custom logo to refresh your party's brand. Grants +1 Momentum/tick while a custom logo is active. Quick and affordable.",cost:"$50k",costColor:"#5a8aaa",moneyCost:5e4,tags:["CAMPAIGN","BRANDING"],locked:!1},{id:"rebrand",name:"Rebrand Party",desc:'Change your party name, abbreviation, color, logo, and description. Costly but grants a "Fresh Start" modifier. Nuclear option after scandal or major defeat.',cost:"$150k",costColor:"#c84",moneyCost:15e4,tags:["CAMPAIGN","STRUCTURAL"],locked:!1}],Se=[{id:"crimson",hex:"#c43a3a",name:"Crimson"},{id:"scarlet",hex:"#d45a2a",name:"Scarlet"},{id:"amber",hex:"#c8a832",name:"Amber"},{id:"gold",hex:"#d4a017",name:"Gold"},{id:"olive",hex:"#8a9a4a",name:"Olive"},{id:"emerald",hex:"#2a8a4a",name:"Emerald"},{id:"forest",hex:"#3a6a3a",name:"Forest"},{id:"teal_c",hex:"#2a8a7a",name:"Teal"},{id:"sky",hex:"#4a8aba",name:"Sky"},{id:"cobalt",hex:"#3a5a9a",name:"Cobalt"},{id:"navy",hex:"#2a3a6a",name:"Navy"},{id:"violet",hex:"#7a4a9a",name:"Violet"},{id:"plum",hex:"#8a3a7a",name:"Plum"},{id:"rose",hex:"#ba4a6a",name:"Rose"},{id:"slate",hex:"#5a6a7a",name:"Slate"},{id:"iron",hex:"#4a4a4a",name:"Iron"}],se=[{emoji:"🏛️",name:"Parliament"},{emoji:"⚖️",name:"Scales"},{emoji:"🗽",name:"Liberty"},{emoji:"🕊️",name:"Dove"},{emoji:"🦅",name:"Eagle"},{emoji:"🦁",name:"Lion"},{emoji:"🐻",name:"Bear"},{emoji:"🐉",name:"Dragon"},{emoji:"🐘",name:"Elephant"},{emoji:"🏔️",name:"Mountain"},{emoji:"🌊",name:"Wave"},{emoji:"🔥",name:"Flame"},{emoji:"⭐",name:"Star"},{emoji:"🌟",name:"Glow Star"},{emoji:"💎",name:"Diamond"},{emoji:"🛡️",name:"Shield"},{emoji:"⚔️",name:"Swords"},{emoji:"🏗️",name:"Builder"},{emoji:"🌿",name:"Leaf"},{emoji:"🌾",name:"Wheat"},{emoji:"🔨",name:"Hammer"},{emoji:"⚡",name:"Lightning"},{emoji:"🎯",name:"Target"},{emoji:"🏴",name:"Flag"},{emoji:"🚩",name:"Red Flag"},{emoji:"✊",name:"Fist"},{emoji:"🤝",name:"Handshake"},{emoji:"📜",name:"Scroll"},{emoji:"🗳️",name:"Ballot"},{emoji:"👑",name:"Crown"}];function mo(a,t){const e=Qe.map(o=>{const r=o.tags.map(i=>`<span class="pa-action-tag" style="color:${Et[i]||"var(--text-dim)"};">${i}</span>`).join("");return`
    ...
    `}function fo(a){const t=document.getElementById("pa-modernize-modal");if(!t)return;const e=g.faction;let o=null,r=e.custom_logo_url||null,i=!1;function n(){const l=!!r,v=Number(e.party_funds??0)>=5e4,p=!!o&&v&&!i;t.innerHTML=`
        ...
        `,document.getElementById("mod-close")?.addEventListener("click",()=>t.classList.remove("active")),document.getElementById("mod-cancel")?.addEventListener("click",()=>t.classList.remove("active")),t.onclick=s=>{s.target===t&&t.classList.remove("active")},document.getElementById("mod-file-input")?.addEventListener("change",s=>{const d=s.target.files?.[0];if(d){if(d.size>2*1024*1024){alert("Logo must be under 2MB.");return}o=d,r=URL.createObjectURL(d),n()}}),document.getElementById("mod-submit")?.addEventListener("click",async()=>{if(i||!o)return;i=!0;const s=document.getElementById("mod-submit");s&&(s.disabled=!0,s.textContent="Uploading...");try{const d=o.name.split(".").pop()?.toLowerCase()||"png",u=`${e.id}/logo_${Date.now()}.${d}`,{error:y}=await C.storage.from("party-logos").upload(u,o,{cacheControl:"3600",upsert:!0,contentType:o.type});if(y)throw new Error("Upload failed: "+y.message);const{data:f}=C.storage.from("party-logos").getPublicUrl(u),h=f?.publicUrl;if(!h)throw new Error("Failed to get logo URL");const m=Math.max(0,Number(e.party_funds??0)-5e4),{error:x}=await C.from("factions").update({custom_logo_url:h,party_funds:m}).eq("id",e.id);if(x)throw x;e.custom_logo_url=h,e.party_funds=m,t.classList.remove("active"),alert("Logo updated! Your party now earns +1 Momentum/tick from the modernized image."),O(a)}catch(d){alert("Modernize failed: "+(d.message||"Error")),i=!1,s&&(s.disabled=!1,s.textContent="Modernize — $50k")}})}t.classList.add("active"),n()}function vo(a){const t=document.getElementById("pa-rebrand-modal");if(!t)return;const e=g.faction;g.nation;const o=e.momentum??50;(g._allParties||[]).filter(d=>d.id!==e.id);const r={current:e.party_color||"#4a8aba"},i={current:0},n={current:e.custom_logo_url||null},l={current:null},c={current:!!e.custom_logo_url},v={current:!1};function p(){return r.current}function s(){const d=p(),u=Se.find(w=>w.hex===d)?.name||"Custom",y=se[i.current]?.emoji||"🏛️",f=c.current&&(n.current||l.current),h=n.current||(l.current?URL.createObjectURL(l.current):null),m=document.getElementById("rb-name")?.value??e.faction_name??"",x=document.getElementById("rb-abbr")?.value??e.abbreviation??"",b=document.getElementById("rb-desc")?.value??"",_=Se.map(w=>{const k=d===w.hex;return`<div class="rb-color-swatch ${k?"selected":""}" data-hex="${w.hex}" style="background:${w.hex};${k?`box-shadow:0 0 8px ${w.hex}44;border:2px solid var(--text-bright);`:""}">
                ${k?'<span style="font-size:10px;color:#fff;text-shadow:0 1px 2px rgba(0,0,0,0.5);">✓</span>':""}
            </div>`}).join(""),M=se.map((w,k)=>{const E=i.current===k;return`<div class="rb-logo-item ${E?"selected":""}" data-idx="${k}" style="${E?`background:${d}15;border:2px solid ${d};box-shadow:0 0 6px ${d}33;`:""}">
                ${w.emoji}
                        ...
                        <div style="background:var(--bg-card);border:1px solid var(--border-main);border-left:3px solid ${d};padding:10px;">
                            <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
                                <div style="width:40px;height:40px;background:${d}15;border:1.5px solid ${d};display:flex;align-items:center;justify-content:center;font-size:22px;overflow:hidden;">
                                    ${f&&h?`<img src="${h}" style="width:100%;height:100%;object-fit:contain;" alt="">`:y}
                                </div>
                                <div>
        ...
        `}t._rbCustomLogoFile=null,t._rbCustomLogoUrl=n.current,t._rbUseCustomLogo=c.current,s(),t.classList.add("active"),t.addEventListener("change",function(u){if(u.target.id==="rb-logo-file"){const y=u.target.files?.[0];if(!y)return;if(y.size>2*1024*1024){alert("Logo must be under 2MB. Selected file: "+(y.size/(1024*1024)).toFixed(1)+"MB"),u.target.value="";return}if(!["image/png","image/jpeg","image/svg+xml","image/webp"].includes(y.type)){alert("Unsupported file type. Use PNG, JPG, SVG, or WebP."),u.target.value="";return}l.current=y,n.current=null,c.current=!0,t._rbCustomLogoFile=y,t._rbCustomLogoUrl=null,t._rbUseCustomLogo=!0,s()}}),t.addEventListener("click",function d(u){if(u.target===t||u.target.closest("#rb-close")||u.target.closest("#rb-cancel")){t.classList.remove("active"),t.removeEventListener("click",d);return}const y=u.target.closest(".rb-color-swatch");if(y){r.current=y.dataset.hex,s();return}const f=u.target.closest(".rb-logo-item");if(f){i.current=parseInt(f.dataset.idx)||0,c.current=!1,t._rbUseCustomLogo=!1,s();return}if(u.target.closest("#rb-remove-logo")){n.current=null,l.current=null,c.current=!1,t._rbCustomLogoFile=null,t._rbCustomLogoUrl=null,t._rbUseCustomLogo=!1,s();return}if(u.target.closest("#rb-submit")){const h=document.getElementById("rb-name")?.value?.trim()||"",m=document.getElementById("rb-abbr")?.value?.trim()||"";if(h.length<3||m.length<2){alert("Name must be 3+ chars, abbreviation 2-4 chars.");return}v.current=!0,s();return}if(u.target.closest("#rb-back")){v.current=!1,s();return}if(u.target.closest("#rb-confirm")){uo(t,a,d);return}})}async function uo(a,t,e){const o=g.faction,r=document.getElementById("rb-name")?.value?.trim()||"",i=document.getElementById("rb-abbr")?.value?.trim()||"";document.getElementById("rb-desc")?.value?.trim();const n=document.querySelector(".rb-color-swatch.selected")?.dataset?.hex||o.party_color,l=document.querySelector(".rb-logo-item.selected")?.dataset?.idx,c=l!=null?se[parseInt(l)]?.emoji:null,v=a._rbCustomLogoFile,p=a._rbUseCustomLogo,s=a._rbCustomLogoUrl,d=document.getElementById("rb-confirm");d&&(d.disabled=!0,d.textContent="Rebranding...");try{const u=g.shard?.current_tick||0;let y=s;if(p&&v){const b=v.name.split(".").pop()?.toLowerCase()||"png",_=`${o.id}/logo_${Date.now()}.${b}`,{data:M,error:w}=await C.storage.from("party-logos").upload(_,v,{cacheControl:"3600",upsert:!0,contentType:v.type});if(w){console.error("[Rebrand] Logo upload failed:",w.message),alert("Logo upload failed: "+w.message);return}const{data:k}=C.storage.from("party-logos").getPublicUrl(_);y=k?.publicUrl||null}else p||(y=null);const f=15e4,h=o.party_funds||0;if(h<f){alert(`Not enough funds. You have $${Math.round(h/1e3)}k, need $150k.`);return}const m=h-f,x=Math.max(1,(o.momentum||0)-10);await C.from("factions").update({party_funds:m,momentum:x,faction_name:r,abbreviation:i.toUpperCase(),party_color:n,party_logo:p?null:c,custom_logo_url:y,rebrand_cooldown_until_tick:u+120}).eq("id",o.id),await C.from("campaign_actions").insert({party_id:o.id,nation_id:g.nation?.id,action_type:"rebrand",ap_cost:3,money_cost:0,tick_performed:u,result:{oldName:o.faction_name,newName:r,oldAbbr:o.abbreviation,newAbbr:i,oldColor:o.party_color,newColor:n}}),o.party_funds=m,o.momentum=x,o.faction_name=r,o.abbreviation=i.toUpperCase(),o.party_color=n,o.party_logo=p?null:c,o.custom_logo_url=y,a.classList.remove("active"),a.removeEventListener("click",e),O(t)}catch(u){console.error("[PartyActions] Rebrand error:",u),alert("Failed to rebrand: "+(u.message||u))}finally{d&&(d.disabled=!1,d.textContent="⚠ Confirm Rebrand")}}const Ze=[{id:"file_lawsuit",name:"File Lawsuit",desc:"Sue a government ministry alleging corruption or negligence. 8-tick timeline with milestone events. Outcome depends on actual corruption growth since government took office.",cost:"$250k",costColor:"#c8a832",moneyCost:25e4,tags:["LEGAL","OFFENSIVE"],locked:!1},{id:"petition_for_reform",name:"Petition for Reform",desc:"Organize a popular petition for political reform. Roll 1d100 + petition strength (education, professional/cultural/religious rapport, inequality, low SoL, crown authority). 0-40 ignored; 41-69 grants minor reform; 70+ forces major reform.",cost:"$100k",costColor:"#c8a832",moneyCost:1e5,tags:["POLITICAL","MONARCHY"],locked:!1,monarchyOnly:!0,cooldownTicks:6}];function yo(a){const t=j,e=X(t.first_name,t.last_name),o=dt(t.skill),r=rt?'<span style="color:#5cc55c;margin-left:6px;">✓ IN OPPOSITION</span>':'<span style="color:#c84;margin-left:6px;">⚠ IN GOVERNMENT (actions limited)</span>',i=K(g?.nation),n=Number(g?.shard?.current_tick)||0,l=g?.faction,v=Ze.filter(p=>!p.monarchyOnly||i).map(p=>{let s=null;if(p.id==="petition_for_reform"&&p.cooldownTicks){if(l?._petitionPending&&(s="A petition is already pending in this nation."),!s){const f=Number(l?.last_petition_for_reform_tick);if(Number.isFinite(f)&&f>0){const h=f+p.cooldownTicks;n<h&&(s=`Cooldown — ready at tick ${h}.`)}}const y=Number(l?.party_funds)||0;!s&&y<p.moneyCost&&(s="Insufficient party funds.")}const d=p.locked||!!s,u=p.tags.map(y=>`<span class="pa-action-tag" style="color:${Et[y]||"var(--text-dim)"};">${y}</span>`).join("");return`
    ...
    `}function go(){if(ne.length===0)return"";const a=g.shard?.current_tick||0;return`
        <div class="pa-ls-section">
            ...
            ${ne.map(e=>{const o=Pt.find(m=>m.key===e.target_ministry),r=o?o.label:e.target_ministry,i=o?o.icon:"⚖️",n=me(e.corruption_growth||0),l=it[e.tier]||it[1],c=e.status==="active",v=Math.max(0,a-e.filed_at_tick),p=8,s=Math.min(1,v/p),d=Math.max(0,e.resolves_at_tick-a),u=[{tick:0,label:"Filed",type:"filing"},{tick:2,label:"Discovery",type:"discovery"},{tick:5,label:"Evidence",type:"evidence"},{tick:7,label:"Pre-trial",type:"pre_trial"},{tick:8,label:"Verdict",type:"resolution"}],y=u.map(m=>{const x=e.filed_at_tick+m.tick,b=a>=x,_=a>=x&&(m.tick===8||a<e.filed_at_tick+u[u.indexOf(m)+1]?.tick),M=m.tick/p*100;return`<div class="pa-ls-milestone ${b?"passed":""} ${_?"current":""}" style="left:${M}%;" title="${m.label} (Tick ${x})">
                <div class="pa-ls-milestone-dot"></div>
                <div class="pa-ls-milestone-label">${m.label}</div>
            </div>`}).join("");let f="";if(!c){const m=l===it[1]?"FRIVOLOUS":l===it[2]?"PARTIAL WIN":l===it[3]?"MAJOR WIN":"DEVASTATING",x=e.tier===1?"var(--red)":e.tier===2?"#ca5":e.tier===3?"#c84":"var(--green)";f=`<span class="pa-ls-tier-badge" style="color:${x};border-color:${x}44;background:${x}0a;">${m}</span>`}const h=c?"":`
...
Next ${e.actionNoun} ready at tick ${i.ready_at_tick}.`),alert(`Could not run ${e.actionNoun}: ${m}${x}`),ea(a);return}const l=e.bucketLabels[i.bucket]||i.bucket,c=(Number(i.total)-Number(i.d100)).toFixed(1),v=i[`${e.primaryStat}_before`],p=i[`${e.primaryStat}_after`],s=Number(p)-Number(v),d=`${e.primaryStatLabel}: ${v} → ${p}`+(s>0?" (+"+s+")":"");let u="";if(e.secondaryStat){const m=Number(i[`${e.secondaryStat}_delta`]||0);m>0&&(u=`
...
`+(i.description||"")),g?.nation&&(g.nation[e.primaryStat]=Number(p),e.secondaryStat&&(g.nation[e.secondaryStat]=Number(i[`${e.secondaryStat}_after`])));const y=(ft||[]).find(m=>m.ministry_key===e.ministryKey);if(y){const m=Number(i.cost_paid)||0;y.discretionary_balance=Math.max(0,Number(y.discretionary_balance||0)-m)}const f=document.getElementById("pa-actions-panel");f&&(f.innerHTML=ye(null,null,t));const h=document.querySelector(`.pa-action-item[data-action-id="${a}"]`);h&&ta(h,e,Number(i.next_cost),i.cooldown_until!=null?Number(i.cooldown_until):null)}catch(i){alert(`${e.actionLabel} failed: ${i?.message||i}`)}finally{jt.delete(a)}}}function ta(a,t,e,o){const r=a.querySelector(".pa-action-cost");r&&Number.isFinite(e)&&e>0&&(r.textContent="$"+Math.round(e/1e6));const i=Number(g?.shard?.current_tick)||0,n=(ft||[]).find(p=>p.ministry_key===t.ministryKey),l=Number(n?.discretionary_balance??0);let c="";if(Number.isFinite(o)&&o>i){const p=o-i;c=`Cooldown — next ${t.actionNoun} ready at tick ${o} (${p} tick${p===1?"":"s"} away).`}else if(Number.isFinite(e)&&l<e){const p=Math.round(e/1e6);c=`${t.ministryName} discretionary budget is below $${p} — next ${t.actionNoun} cost has outgrown the budget.`}const v=a.querySelector("."+t.lockLineClass);if(c)if(a.classList.add("locked"),v){const p=v.querySelector("span:last-child");p&&(p.textContent=c)}else{const p=document.createElement("div");p.className=t.lockLineClass,p.style.cssText="margin-top:4px;font-family:var(--font-mono);font-size:7px;color:var(--orange);display:flex;align-items:center;gap:4px;";const s=document.createElement("span");s.textContent="⊘";const d=document.createElement("span");d.textContent=c,p.appendChild(s),p.appendChild(d),a.appendChild(p)}else a.classList.remove("locked"),v&&v.remove()}async function ea(a){const t=Rt[a];if(!t)return;const e=document.querySelector(`.pa-action-item[data-action-id="${a}"]`);if(!e)return;const o=g?.nation?.id;if(!o)return;let r,i;try{[r,i]=await Promise.all([C.rpc(t.nextCostRpc,{p_nation_id:o}),C.rpc(t.cooldownRpc,{p_nation_id:o})])}catch(c){console.warn(`[${t.actionLabel}] RPC fetch threw:`,c?.message||c);return}r.error&&console.warn(`[${t.actionLabel}] next_cost RPC failed:`,r.error.message),i.error&&console.warn(`[${t.actionLabel}] cooldown_until RPC failed:`,i.error.message);const n=Number(r.data),l=i.data!=null?Number(i.data):null;ta(e,t,n,l)}let Wt=!1;async function Le(a){const t=document.getElementById("pa-hire-modal");if(!t)return;const e=g.nation?.id,o=g.nation?.name;if(!e||!o)return;t.innerHTML='<div class="pa-modal"><div style="padding:40px;text-align:center;color:var(--text-dim);font-family:var(--font-mono);font-size:10px;">Searching for candidates...</div></div>',t.classList.add("active");const r=await La(C,e,o);let i=null;function n(){const l=i!=null?r[i]:null,c=l?dt(l.skill):null,v=r.map((d,u)=>{const y=i===u,f=dt(d.skill);return`<div class="pa-hire-row ${y?"selected":""}" data-idx="${u}">
                ...
                <div style="flex:1;min-width:0;">
                    ...
                    <div style="display:flex;align-items:center;gap:3px;margin-top:2px;">
                        <div style="flex:1;height:2px;background:var(--border-mid);max-width:60px;"><div style="height:100%;width:${d.skill}%;background:${f.color};"></div></div>
                        <span style="font-family:var(--font-mono);font-size:8px;color:${f.color};">${d.skill}</span>
                    </div>
                </div>
                ...
                <div style="padding:10px 20px;border-top:1px solid var(--border-main);background:var(--bg-card);display:flex;justify-content:flex-end;">
                    ...
                    <button class="pa-modal-btn pa-modal-btn--submit" id="pa-hire-confirm" style="background:#d44a4a;"${(g.faction?.party_funds||0)<l.hire_cost?' disabled title="Not enough funds"':""}>Hire ${$(l.first_name)}</button>
                </div>
        ...
        `;const s=()=>t.classList.remove("active");document.getElementById("pa-hire-close")?.addEventListener("click",s),t.onclick=d=>{d.target===t&&s()},document.getElementById("pa-hire-list")?.addEventListener("click",d=>{const u=d.target.closest(".pa-hire-row");u&&(i=parseInt(u.dataset.idx,10),n())}),document.getElementById("pa-hire-confirm")?.addEventListener("click",async()=>{if(Wt||i==null)return;Wt=!0;const d=document.getElementById("pa-hire-confirm");d&&(d.disabled=!0,d.textContent="Hiring...");try{const u=g.shard?.current_tick||0,y=r[i],f=y.hire_cost||0,h=g.faction?.party_funds||0;if(f>0&&h<f){alert(`Not enough funds. You have $${Math.round(h/1e3)}k, need $${Math.round(f/1e3)}k.`);return}if(f>0){const x=h-f,{error:b}=await C.from("factions").update({party_funds:x}).eq("id",g.faction.id);if(b){alert("Failed to deduct funds.");return}g.faction.party_funds=x}const m=await Na(C,g.faction?.id,y,u);if(!m.success){alert(m.error||"Failed to hire agitator.");return}j=m.agitator,V="agitator",s(),O(a)}catch(u){console.error("[PartyActions] Hire agitator error:",u)}finally{Wt=!1,d&&(d.disabled=!1)}})}n()}let Lt=!1;function _o(a){const t=document.getElementById("pa-lawsuit-modal");if(!t)return;if(!B){alert("No active government to file against.");return}const e=g.faction,o=j;let r=null,i=null;function n(){const l=r&&i,c=Pt.map(s=>{const d=r===s.key;return`<div class="pa-lawsuit-target ${d?"selected":""}" data-target="${s.key}">
                <span style="font-size:18px;">${s.icon}</span>
                <span style="font-size:12px;font-weight:600;color:${d?"var(--text-bright)":"var(--text-secondary)"};">${$(s.label)}</span>
            </div>`}).join(""),v=Ue.map(s=>{const d=i===s.key;return`<div class="pa-lawsuit-basis ${d?"selected":""}" data-basis="${s.key}">
                <div style="display:flex;align-items:center;gap:10px;">
                    ...
                    <div>
                        <div style="font-size:14px;font-weight:600;color:${d?"var(--text-bright)":"var(--text-secondary)"};">${$(s.label)}</div>
        ...
        `;const p=()=>t.classList.remove("active");document.getElementById("pa-lawsuit-close")?.addEventListener("click",p),document.getElementById("pa-lawsuit-cancel")?.addEventListener("click",p),t.onclick=s=>{s.target===t&&p()},document.getElementById("pa-lawsuit-targets")?.addEventListener("click",s=>{const d=s.target.closest(".pa-lawsuit-target");d&&(r=d.dataset.target,n())}),document.getElementById("pa-lawsuit-bases")?.addEventListener("click",s=>{const d=s.target.closest(".pa-lawsuit-basis");d&&(i=d.dataset.basis,n())}),document.getElementById("pa-lawsuit-submit")?.addEventListener("click",async()=>{if(Lt||!r||!i)return;Lt=!0;const s=document.getElementById("pa-lawsuit-submit");s&&(s.disabled=!0,s.textContent="Filing...");try{const{data:u}=await C.from("factions").select("party_funds").eq("id",e.id).single(),y=u?.party_funds||0;if(y<25e4){alert(`Not enough funds. You have $${Math.round(y/1e3)}k, need $250k.`),Lt=!1,s&&(s.disabled=!1,s.textContent="File Lawsuit");return}const f=y-25e4;await C.from("factions").update({party_funds:f}).eq("id",e.id),e.party_funds=f,sessionStorage.removeItem("nationhood_state");const h=g.shard?.current_tick||0,m=await Aa(C,{factionId:e?.id,nationId:g.nation?.id,agitatorId:o?.id,targetMinistry:r,basis:i,currentTick:h,partyName:e?.faction_name||"Opposition",administration:B});if(!m.success){alert(m.error||"Failed to file lawsuit.");return}const x=me(m.lawsuit?.corruption_growth||0),b=it[m.tier]||it[1];p(),alert(`Lawsuit filed against ${Pt.find(_=>_.key===r)?.label||r}.
The case is now under investigation. Results will be determined when it resolves in 8 ticks.`),O(a)}catch(d){console.error("[PartyActions] File lawsuit error:",d),alert("An error occurred. Please try again.")}finally{Lt=!1,s&&(s.disabled=!1,s.textContent="File Lawsuit")}})}t.classList.add("active"),n()}async function $o(a){const t=document.getElementById("pa-appoint-pm-modal");if(!t)return;const e=g.nation,o=g.faction,{data:r}=await C.from("factions").select("id, faction_name, abbreviation, party_color, seats, leader_first_name, leader_last_name, leader_age").eq("nation_id",e.id).eq("faction_type","party").is("abandoned_at",null).order("seats",{ascending:!1}),i=r||[];let n=null,l=!1;const{data:c}=await C.from("head_of_government").select("faction_id, first_name, last_name, factions(faction_name)").eq("nation_id",e.id).eq("active",!0).maybeSingle();function v(){const p=i.find(f=>f.id===n),s=c?`${c.first_name} ${c.last_name}`:null,d=c?.factions?.faction_name||null,u=c&&n===c.faction_id;t.innerHTML=`
            <div class="pa-modal" style="width:500px;">
                <div class="pa-modal-header">
                    <div class="pa-modal-header-left">
                        <div class="pa-modal-dot" style="background:#c8a832;"></div>
                ...
                <div style="padding:8px 20px;border-bottom:1px solid var(--border-main);font-size:12px;color:var(--text-secondary);line-height:1.5;">
                    ...
                    ${s?`<div style="margin-top:4px;font-family:var(--font-mono);font-size:9px;color:var(--text-dim);">Current PM: <strong style="color:var(--text-bright);">${$(s)}</strong> (${$(d||"?")})</div>`:'<div style="margin-top:4px;font-family:var(--font-mono);font-size:9px;color:var(--amber);">No Prime Minister appointed.</div>'}
                </div>
                ...
                <div class="pa-modal-footer">
                    ...
                    <button class="pa-modal-btn pa-modal-btn--submit" id="apm-confirm" ${!p||l||u?"disabled":""} style="background:#c8a832;">${p?u?"Already PM":`Appoint ${$(p.faction_name)}`:"Select a party"}</button>
                </div>
            </div>
        `;const y=()=>t.classList.remove("active");document.getElementById("apm-close")?.addEventListener("click",y),document.getElementById("apm-cancel")?.addEventListener("click",y),t.onclick=f=>{f.target===t&&y()},t.querySelector(".pa-modal-body")?.addEventListener("click",f=>{const h=f.target.closest("[data-party-id]");h&&(n=h.dataset.partyId,v())}),document.getElementById("apm-confirm")?.addEventListener("click",async()=>{if(!n||l)return;const f=i.find(m=>m.id===n);if(!f||!confirm(`Appoint ${f.leader_first_name} ${f.leader_last_name} of ${f.faction_name} as Prime Minister?`))return;l=!0;const h=document.getElementById("apm-confirm");h&&(h.disabled=!0,h.textContent="Appointing...");try{const m=g.shard?.current_tick||0;await ya(C,{nationId:e.id,factionId:n,firstName:f.leader_first_name||"Unknown",lastName:f.leader_last_name||"Unknown",age:f.leader_age||50,currentTick:m});try{await C.from("government_formations").update({status:"dissolved"}).eq("nation_id",e.id).in("status",["formed","caretaker","active"]);const{data:E}=await C.from("shard").select("current_date").eq("name","Alpha Shard").single();await C.from("government_formations").insert({nation_id:e.id,election_id:null,proposed_by:o.id,party_ids:[n],status:"formed",formation_type:"monarchy",formed_at:new Date().toISOString(),ministry_assignments:{prime_minister:n},game_year:E?.current_date||""})}catch(E){console.warn("[AppointPM] government_formations write failed (non-blocking — synthetic fallback still works):",E?.message||E)}let x=0;const b=e.monarch_faction_id,_=c?.faction_id||null,M=_&&_!==b&&_!==n,w=n!==b&&n!==_;if(M&&(x-=4),w&&(x+=3),x!==0){const E=Number(e.crown_authority??50),S=Math.max(0,Math.min(100,E+x));try{await C.from("nations").update({crown_authority:S}).eq("id",e.id),e.crown_authority=S}catch{}}try{await C.from("event_log").insert({nation_id:e.id,event_name:`${e.monarch_title||"King"} appoints Prime Minister`,category:"government",description_chosen:`${e.monarch_title||"The King"} has appointed ${f.leader_first_name} ${f.leader_last_name} of ${f.faction_name} as Prime Minister.`,fired_at_tick:m})}catch{}y();const k=x>0?`

Crown Authority +${x}.`:x<0?`

Crown Authority ${x}.`:"";alert(`${f.leader_first_name} ${f.leader_last_name} of ${f.faction_name} has been appointed Prime Minister.${k}`),O(a)}catch(m){alert("Failed to appoint PM: "+(m.message||"Error")),l=!1,h&&(h.disabled=!1,h.textContent=`Appoint ${$(f.faction_name)}`)}})}t.classList.add("active"),v()}async function wo(a){const t=document.getElementById("pa-royal-modal");if(!t)return;const e=g.nation,o=g.faction,r=o.seats||0,i=e?.total_seats||100,{data:n}=await C.from("factions").select("id, faction_name, abbreviation, party_color, seats").eq("nation_id",e.id).eq("faction_type","party").is("abandoned_at",null).order("faction_name"),l=(n||[]).filter(d=>d.id!==o.id);let c=null;const v=Math.max(0,r-1);let p=Math.min(5,v||1);function s(){const d=l.find(y=>y.id===c);t.innerHTML=`
            <div class="pa-modal" style="width:560px;">
                ...
                <div class="pa-modal-body">
                    ...
                    <div style="display:flex;flex-direction:column;gap:4px;">
                        ${l.length>0?l.map(y=>{const f=y.id===c;return`<div class="pa-action-item ${f?"selected":""}" data-faction-id="${y.id}" style="cursor:pointer;${f?`border-color:${y.party_color||"#888"};background:${y.party_color||"#888"}08;`:""}">
                                <div style="display:flex;justify-content:space-between;align-items:center;">
                                    <div style="display:flex;align-items:center;gap:8px;">
                    ...
                    ${d?`
                        <div style="margin-top:14px;">
                            ...
                            <div style="font-family:var(--font-mono);font-size:10px;color:var(--text-dim);margin-top:4px;">
                                ...
                                &middot; Your seats after: ${r-p} &middot; Their seats after: ${(d.seats||0)+p}
                            </div>
                        </div>
                    `:""}
                </div>
        ...
        `;const u=()=>t.classList.remove("active");document.getElementById("royal-close")?.addEventListener("click",u),document.getElementById("royal-cancel")?.addEventListener("click",u),t.onclick=y=>{y.target===t&&u()},t.querySelector(".pa-modal-body")?.addEventListener("click",y=>{const f=y.target.closest("[data-faction-id]");f&&(c=f.dataset.factionId,s())}),document.getElementById("grant-slider")?.addEventListener("input",y=>{p=parseInt(y.target.value)||1,document.getElementById("grant-count").textContent=p;const f=document.getElementById("royal-grant");f&&(f.textContent=`Grant ${p} Seats`)}),document.getElementById("royal-grant")?.addEventListener("click",async()=>{if(!c||mt)return;mt=!0;const y=document.getElementById("royal-grant");y&&(y.disabled=!0,y.textContent="Granting...");try{const{data:f}=await C.from("factions").select("id, faction_name, seats").eq("nation_id",e.id).eq("faction_type","party").is("abandoned_at",null),h=(f||[]).find(I=>I.id===o.id),m=(f||[]).find(I=>I.id===c);if(!h||!m){alert("Faction not found.");return}const x=(f||[]).reduce((I,N)=>I+Math.max(0,N.seats||0),0),b=new Map;for(const I of f||[])b.set(I.id,Math.max(0,I.seats||0));let _=p;const M=Math.max(0,(b.get(o.id)||0)-1),w=Math.min(_,M);if(w>0&&(b.set(o.id,(b.get(o.id)||0)-w),_-=w),_>0){const I=(f||[]).filter(A=>A.id!==o.id&&A.id!==c&&(b.get(A.id)||0)>0);let N=I.reduce((A,z)=>A+(b.get(z.id)||0),0);for(const A of I){if(_<=0||N<=0)break;const z=Math.round(_*(b.get(A.id)||0)/N),q=Math.min(z,b.get(A.id)||0,_);q>0&&(b.set(A.id,(b.get(A.id)||0)-q),N-=q,_-=q)}if(_>0)for(const A of I){if(_<=0)break;const z=b.get(A.id)||0,q=Math.min(_,z);q>0&&(b.set(A.id,z-q),_-=q)}}const k=p-_;if(k<=0){alert("No seats available to grant.");return}b.set(c,(b.get(c)||0)+k);let E=0;for(const I of b.values())E+=I;if(E!==x){console.error("[GrantSeats] Conservation violated",{sumBefore:x,sumAfter:E,grantAmount:p,actualGrant:k}),alert("Internal error: seat totals would not balance. Aborting.");return}const S=[];for(const I of f||[]){const N=Math.max(0,I.seats||0),A=b.get(I.id)||0;N!==A&&S.push({id:I.id,seats:A})}for(const I of S){const{error:N}=await C.from("factions").update({seats:I.seats}).eq("id",I.id);if(N){alert("Failed to grant seats: "+N.message);return}}const L=k*.5,T=Math.min(100,(Number(e.crown_authority)||50)+L),{error:P}=await C.from("nations").update({crown_authority:T}).eq("id",e.id);if(P){alert("Failed to update crown authority.");return}o.seats=b.get(o.id)||0,e.crown_authority=T;try{const I=l.find(N=>N.id===c);await C.from("event_log").insert({nation_id:e.id,event_name:`${e.monarch_title||"King"} grants ${k} seats to ${I?.faction_name||"unknown"}`,category:"government",description_chosen:`The ${e.monarch_title||"King"} has granted ${k} parliamentary seat${k!==1?"s":""} to ${I?.faction_name}. Crown Authority +${L.toFixed(1)}.`,fired_at_tick:g.shard?.current_tick||0})}catch{}u(),O(a)}catch(f){console.error("[GrantSeats] Error:",f),alert("Failed to grant seats.")}finally{mt=!1}})}t.classList.add("active"),s()}async function ko(a){const t=document.getElementById("pa-royal-modal");if(!t)return;const e=g.nation,o=g.faction,{data:r}=await C.from("factions").select("id, faction_name, abbreviation, party_color, seats").eq("nation_id",e.id).eq("faction_type","party").is("abandoned_at",null).order("faction_name"),i=(r||[]).filter(v=>v.id!==o.id&&(v.seats||0)>0);let n=null,l=1;function c(){const v=i.find(f=>f.id===n),p=v&&v.seats||0,d=l*1e5,u=o.party_funds||0;t.innerHTML=`
            <div class="pa-modal" style="width:560px;">
                <div class="pa-modal-header">
                    <div class="pa-modal-header-left">
                        <div class="pa-modal-dot" style="background:#d44a4a;"></div>
                ...
                <div class="pa-modal-footer">
                    <button class="pa-modal-btn pa-modal-btn--cancel" id="royal-cancel">Cancel</button>
                    <button class="pa-modal-btn pa-modal-btn--submit" id="royal-revoke" ${!v||u<d?"disabled":""} style="background:#d44a4a;">Revoke ${l} Seats</button>
                </div>
            </div>
        `;const y=()=>t.classList.remove("active");document.getElementById("royal-close")?.addEventListener("click",y),document.getElementById("royal-cancel")?.addEventListener("click",y),t.onclick=f=>{f.target===t&&y()},t.querySelector(".pa-modal-body")?.addEventListener("click",f=>{const h=f.target.closest("[data-faction-id]");h&&(n=h.dataset.factionId,l=1,c())}),document.getElementById("revoke-slider")?.addEventListener("input",f=>{l=parseInt(f.target.value)||1,document.getElementById("revoke-count").textContent=l;const h=document.getElementById("royal-revoke");h&&(h.textContent=`Revoke ${l} Seats`)}),document.getElementById("royal-revoke")?.addEventListener("click",async()=>{if(!n||mt)return;mt=!0;const f=document.getElementById("royal-revoke");f&&(f.disabled=!0,f.textContent="Revoking...");try{const h=l*1e5,{data:m}=await C.from("factions").select("id, faction_name, seats, party_funds").eq("nation_id",e.id).eq("faction_type","party").is("abandoned_at",null),x=(m||[]).find(z=>z.id===o.id),b=(m||[]).find(z=>z.id===n);if(!x||!b){alert("Faction not found.");return}const _=x.party_funds||0;if(_<h){alert("Not enough funds.");return}const M=(m||[]).reduce((z,q)=>z+Math.max(0,q.seats||0),0),w=Math.min(l,b.seats||0);if(w<=0){alert("Target has no seats to revoke.");return}const k=_-h,E=(x.seats||0)+w,S=(b.seats||0)-w,L=w,T=Math.max(0,(Number(e.crown_authority)||50)-L),P=M-(x.seats||0)-(b.seats||0)+E+S;if(P!==M){console.error("[RevokeSeats] Conservation violated",{sumBefore:M,sumAfter:P,take:w}),alert("Internal error: seat totals would not balance. Aborting.");return}const{error:I}=await C.from("factions").update({seats:E,party_funds:k}).eq("id",o.id);if(I){alert("Failed to revoke seats: "+I.message);return}const{error:N}=await C.from("factions").update({seats:S}).eq("id",n);if(N){alert("Failed to revoke seats: "+N.message);return}const{error:A}=await C.from("nations").update({crown_authority:T}).eq("id",e.id);if(A){alert("Failed to update crown authority.");return}o.seats=E,o.party_funds=k,e.crown_authority=T,sessionStorage.removeItem("nationhood_state");try{await C.from("event_log").insert({nation_id:e.id,event_name:`${e.monarch_title||"King"} revokes ${w} seats from ${b.faction_name||"unknown"}`,category:"political",description_chosen:`The ${e.monarch_title||"King"} has revoked ${w} seat${w!==1?"s":""} from ${b.faction_name}. Crown Authority -${L}.`,fired_at_tick:g.shard?.current_tick||0})}catch{}y(),O(a)}catch(h){console.error("[RevokeSeats] Error:",h),alert("Failed to revoke seats.")}finally{mt=!1}})}t.classList.add("active"),c()}let Kt=!1;async function Eo(){if(Kt||!g?.faction?.id||!g?.nation?.id)return;if(!Dt(g.nation)){alert("Early elections are only available in parliamentary and semi-presidential systems.");return}if(K(g.nation)){alert("Elections are not held under absolute monarchy.");return}const a=B?.pm_party_id;if(!a||a!==g.faction.id){alert("Prime Minister’s party only.");return}if(confirm(`⚡ CALL EARLY ELECTIONS?
...
This action CANNOT be undone.`))return;if(prompt('Type "DISBAND" to confirm dissolution of '+t+":")!=="DISBAND"){alert("Disband cancelled.");return}te=!0;try{const{data:l,error:c}=await C.rpc("disband_party",{p_faction_id:a.id});if(c)throw c;if(l&&l.success===!1)throw new Error(l.error||"Unknown error");sessionStorage.removeItem("active_faction_id"),sessionStorage.removeItem("nationhood_state");const{data:{user:v}}=await C.auth.getUser();if(v){const{data:p}=await C.from("factions").select("id, faction_type").or(`id.eq.${v.id},linked_user_id.eq.${v.id}`),s=(p||[]).find(u=>u.faction_type==="party"),d=(p||[]).find(u=>u.faction_type==="corporation");if(s){sessionStorage.setItem("active_faction_id",s.id),alert(t+` has been disbanded.
...
Voting is now open for ${R.NO_CONFIDENCE_VOTING_TICKS} ticks.`),window.location.href=`bill.html?id=${u.billId}`}catch(u){console.error("[PartyActions] No confidence file failed:",u),alert("Failed to file motion: "+(u?.message||"unknown error"))}finally{ae=!1}}}let Tt=!1,lt=[],wt=null;async function To(){if(lt.length>0)return;const{data:a,error:t}=await C.from("fundraiser_events").select("event_key, name, icon, host_sector_key, opposition_sector_key, display_order").order("display_order");if(t){console.warn("[PartyActions] fundraiser_events load failed:",t.message),lt=[];return}lt=a||[]}async function Po(a){if(!a||!g?.nation?.id||!g?.faction?.id)return null;const{data:t}=await C.from("sectors").select("id, name, weight").eq("nation_id",g.nation.id).eq("sector_key",a).eq("is_active",!0).maybeSingle();if(!t?.id)return null;const{data:e}=await C.from("faction_sector_popularity").select("popularity").eq("faction_id",g.faction.id).eq("sector_id",t.id).maybeSingle();return{id:t.id,name:t.name,weight:Number(t.weight)||1,popularity_tenths:Number(e?.popularity)||0}}async function zo(a){if(!Tt){if($t>=1){alert("You have already hosted a fundraiser this tick. Try again next tick.");return}if(await To(),lt.length===0){alert("No fundraiser events configured. Run migration 20260728.");return}wt=null,await Ro(a)}}async function Ro(a){let t=document.getElementById("pa-fundraise-modal");t||(t=document.createElement("div"),t.id="pa-fundraise-modal",t.className="pa-modal-overlay",t.innerHTML=`
            ...
            <div class="pa-fr-card" data-event-key="${$(n.event_key)}" style="padding:10px 14px;border-bottom:1px dashed var(--border-main);cursor:pointer;">
                ...
                <div style="font-family:var(--font-mono);font-size:9px;letter-spacing:0.06em;color:var(--text-dim);text-transform:uppercase;margin-top:2px;">
                    ${$(l?.name||n.host_sector_key)} · w${v} · pop ${c}
                </div>
        ...
        </div>`,e.querySelectorAll(".pa-fr-card").forEach(n=>{n.addEventListener("click",()=>{wt=n.dataset.eventKey,e.querySelectorAll(".pa-fr-card").forEach(l=>l.style.background=""),n.style.background="rgba(200,168,50,0.08)",Oo(e,o,a)})})}function Oo(a,t,e){const o=a.querySelector("#pa-fr-detail"),r=lt.find(f=>f.event_key===wt);if(!r)return;const i=t[r.host_sector_key],n=t[r.opposition_sector_key],l=i?(i.popularity_tenths/10).toFixed(1):"—",c=i?.weight||1,v=n?(n.popularity_tenths/10).toFixed(1):"—",p=!i,s=!n,d=r.event_key!=="corporate_gala",u=d&&i?1250*(i.popularity_tenths||0)*Math.max(1,i.weight||1):0;o.innerHTML=`
        <div style="display:flex;align-items:baseline;gap:8px;">
        ...
        <div style="margin-top:10px;padding:10px 12px;background:var(--bg-card);border:1px solid var(--border-main);">
            ...
            <div style="display:flex;justify-content:space-between;font-family:var(--font-mono);font-size:11px;color:var(--text-bright);padding:2px 0;">
                ...
                ${d?`<span style="color:#5cb85c;font-weight:700;">+${ze(u)}</span>`:'<span style="color:var(--text-dim);font-style:italic;">positioning only — no yield</span>'}
            </div>
        </div>
    ...
    `,t.classList.add("active");let n=null,l=!1;const c=()=>t.classList.remove("active");document.getElementById("pa-stmt-close")?.addEventListener("click",c),document.getElementById("pa-stmt-cancel")?.addEventListener("click",c),t.addEventListener("click",p=>{p.target===t&&c()}),document.getElementById("pa-stmt-topics")?.addEventListener("click",p=>{const s=p.target.closest(".pa-topic-card");s&&(n=s.dataset.topic,document.querySelectorAll(".pa-topic-card").forEach(d=>{const u=d.dataset.topic===n;d.style.borderColor=u?o:"var(--border-mid)",d.style.background=u?o+"0a":"";const y=d.querySelector("span:last-child");y&&(y.style.color=u?"var(--text-bright)":"var(--text-secondary)")}),v())});const v=()=>{const p=document.getElementById("pa-stmt-body")?.value?.trim()||"",s=document.getElementById("pa-stmt-submit"),d=document.getElementById("pa-stmt-charcount");d&&(d.textContent=`${p.length} characters`),s&&(s.disabled=!(n&&p.length>=10))};document.getElementById("pa-stmt-body")?.addEventListener("input",v),document.getElementById("pa-stmt-submit")?.addEventListener("click",async()=>{if(l)return;const p=document.getElementById("pa-stmt-body")?.value?.trim();if(!n||!p||p.length<10)return;l=!0;const s=document.getElementById("pa-stmt-submit");s&&(s.disabled=!0,s.textContent="Issuing...");try{const d=g.shard?.current_tick||0,y=we.find(L=>L.id===n)?.label||n,f=2e4,{data:h}=await C.from("factions").select("party_funds").eq("id",e.id).single(),m=h?.party_funds||0;if(m<f){alert(`Not enough funds. You have $${Math.round(m/1e3)}k, need $20k.`);return}const x=m-f,{error:b}=await C.from("factions").update({party_funds:x}).eq("id",e.id);if(b){alert("Failed to deduct funds: "+b.message);return}e.party_funds=x;const M=ke[Math.floor(Math.random()*ke.length)].replace("{party_name}",e.faction_name||"Unknown Party").replace("{leader_name}",r).replace("{topic}",y),{error:w}=await C.from("campaign_actions").insert({party_id:e.id,nation_id:g.nation?.id,action_type:"issue_statement",ap_cost:1,money_cost:0,tick_performed:d,result:{topic:n,topicLabel:y,headline:M,body:p,leaderName:r}});w&&console.error("[PartyActions] Statement log failed:",w.message);const{error:k}=await C.from("valdorian_articles").insert({nation_id:g.nation?.id,event_type:"issue_statement",tier:3,section:"politics",headline:M,subheadline:y,lede:p.substring(0,200)+(p.length>200?"...":""),body_paragraphs:JSON.stringify(p.split(/\n\n+/).filter(L=>L.trim())),quotes:JSON.stringify([{posture:"assertive",text:p.substring(0,150)}]),byline_reporter:"Political Desk",topic_tags:JSON.stringify([n]),source_event_id:"statement_"+Date.now(),tick:d});k&&console.error("[PartyActions] Article creation failed:",k.message);const{error:E}=await C.from("event_log").insert({nation_id:g.nation?.id,event_name:M,category:"political",description_chosen:`${e.faction_name} issues the following statement regarding ${y}: "${p}"`,fired_at_tick:d});E&&console.warn("[Statement] event_log insert failed:",E.message);const{error:S}=await C.from("admin_timeline_events").insert({nation_id:g.nation?.id,tick:d,type:"communications",title:"Statement Issued",description:`${r} issued a public statement on ${y}: "${p.substring(0,120)}${p.length>120?"...":""}"`});S&&console.warn("[Statement] timeline insert failed:",S.message),c(),O(a)}catch(d){console.error("[PartyActions] Statement error:",d),alert("Failed to issue statement. Please try again.")}finally{l=!1,s&&(s.disabled=!1,s.textContent="Issue Statement")}})}const Ot=10;function Do(a){const t=document.getElementById("pa-platform-modal");if(!t)return;const e=g.faction;g.nation;const o=e?.color||"#c8a832";let r=null,i=!1;const n={};for(const v of fe)v.faction_id!==e?.id&&(n[v.platform_key]=(n[v.platform_key]||0)+1);const l=new Set(Z.map(v=>v.platform_key));function c(){const v=ht.find(f=>f.id===r),p=`+${(be.adoptTenths/10).toFixed(1)}`,s=(be.failTenths/10).toFixed(1),d="#5cc55c",u=ht.map(f=>{const h=r===f.id,m=l.has(f.id),x=n[f.id]||0;return`<div class="pa-plat-card ${h?"selected":""} ${m?"adopted":""}" data-plat="${f.id}">
            ...
            </div>`}).join("");let y;if(!v)y=`<div style="flex:1;display:flex;align-items:center;justify-content:center;padding:40px;">
                <div style="text-align:center;">
            ...
            </div>`;else{const f=v.improve.map(_=>{const M=xe(_,"improve");return`<span style="display:inline-flex;align-items:center;gap:3px;font-family:var(--font-mono);font-size:10px;padding:2px 6px;background:rgba(92,204,92,0.05);border:1px solid rgba(92,204,92,0.15);color:${M.color};white-space:nowrap;">${M.arrow} ${he[_]||_}</span>`}).join(""),h=v.worsen.map(_=>{const M=xe(_,"worsen");return`<span style="display:inline-flex;align-items:center;gap:3px;font-family:var(--font-mono);font-size:10px;padding:2px 6px;background:rgba(204,85,85,0.05);border:1px solid rgba(204,85,85,0.15);color:${M.color};white-space:nowrap;">${M.arrow} ${he[_]||_}</span>`}).join(""),m=l.has(v.id),x=Z.length;let b;m?b=`<div style="font-family:var(--font-mono);font-size:12px;font-weight:700;color:${o};display:flex;align-items:center;gap:6px;">✓ CURRENT PLATFORM</div>`:x>=3?b='<div style="font-family:var(--font-mono);font-size:11px;color:var(--red);">All 3 platform slots are full.</div>':i?b=`<div style="display:flex;align-items:center;gap:8px;">
        ...
        `,document.getElementById("pa-plat-close")?.addEventListener("click",()=>t.classList.remove("active")),t.onclick=f=>{f.target===t&&t.classList.remove("active")},document.getElementById("pa-plat-grid")?.addEventListener("click",f=>{const h=f.target.closest(".pa-plat-card");h&&(r=h.dataset.plat,i=!1,c())}),document.getElementById("pa-plat-adopt")?.addEventListener("click",()=>{i=!0,c()}),document.getElementById("pa-plat-conf-cancel")?.addEventListener("click",()=>{i=!1,c()}),document.getElementById("pa-plat-conf-yes")?.addEventListener("click",()=>Ho(a,r))}t.classList.add("active"),c()}let Nt=!1;async function Ho(a,t){if(Nt)return;Nt=!0;const e=document.getElementById("pa-platform-modal"),o=g.faction,r=g.nation;if(!o||!r||!t){Nt=!1;return}const i=ht.find(v=>v.id===t);if(!i)return;const n={},l={},c=v=>Re.has(v);for(const v of i.improve){const p=Number(r[v]??50);n[v]=p,c(v)?l[v]=Math.max(0,p-Ot):l[v]=Math.min(100,p+Ot)}try{const v=g.shard?.current_tick||0,{data:p,error:s}=await C.rpc("adopt_platform",{p_faction_id:o.id,p_nation_id:r.id,p_platform_key:t,p_tick:v,p_baseline_stats:n,p_target_stats:l});if(s){console.error("[PartyActions] Platform adoption failed:",s.message),alert("Failed to adopt platform: "+s.message);return}if(p&&!p.success){alert(p.error||"Failed to adopt platform.");return}const d=p?.slot||Z.length+1;Z.push({faction_id:o.id,nation_id:r.id,platform_key:t,slot:d,adopted_at_tick:v,baseline_stats:n,target_stats:l,status:"active"}),fe.push(Z[Z.length-1]),e?.classList.remove("active"),O(a)}catch(v){console.error("[PartyActions] Platform adoption error:",v),alert("An error occurred. Please try again.")}finally{Nt=!1}}let ut=null,aa={isGoverning:!1,statusLabel:"OPPOSITION",administration:null,ticksInPower:0,myFaction:null,allParties:[],rivalParties:[],strongholdsByParty:{},passedBills:[],sectors:[],caucuses:[],nextElection:null,nextElectionTicks:null};function U(a){if(!a)return"";const t=document.createElement("div");return t.textContent=a,t.innerHTML}function qo(a,t,e){const o={};for(const r of a)o[r.id]=wa(r.id,t,e,3);return o}function jo(a,t,e){const o=new Map(a.map(i=>[i.id,i])),r=new Map;for(const i of e){const n=r.get(i.sector_id)||[];n.push({party_id:i.faction_id,popularity:Number(i.popularity)||0}),r.set(i.sector_id,n)}return t.map(i=>{const n=(r.get(i.id)||[]).filter(l=>l.popularity>0&&o.has(l.party_id)).map(l=>{const c=o.get(l.party_id);return{party_id:c.id,abbreviation:c.abbreviation||(c.faction_name||"?").slice(0,3).toUpperCase(),color:c.party_color||"#888",seats:Number(c.seats)||0,popularity:l.popularity}});return n.sort((l,c)=>c.popularity!==l.popularity?c.popularity-l.popularity:c.seats-l.seats),{sector_key:i.sector_key,name:i.name,description:i.description||"",weight:Number(i.weight)||0,candidates:n}}).sort((i,n)=>n.weight!==i.weight?n.weight-i.weight:(i.name||"").localeCompare(n.name||""))}async function Uo(a,t,e){ut=t;const o=document.getElementById(e);if(!o)return;const r=t.faction,i=t.nation,n=i?.id,l=r?.id;if(!r||!n){o.innerHTML='<div style="padding:40px;text-align:center;color:var(--text-dim);font-family:var(--font-mono);font-size:10px;">No faction data.</div>';return}o.innerHTML='<div style="padding:40px;text-align:center;color:var(--text-dim);font-family:var(--font-mono);font-size:10px;">Loading party overview...</div>';try{const c=t.shard?.current_tick||0,[v,p,s,d,u,y,f,h]=await Promise.all([qe(a,n,l),a.from("factions").select("*").eq("nation_id",n).eq("faction_type","party"),a.from("sectors").select("id, sector_key, name, description, weight, base_turnout, is_active").eq("nation_id",n).eq("is_active",!0).order("display_order"),a.from("bills").select("id, bill_name, bill_type, proposed_by, passed_tick, bill_articles(selected_option:policy_options!selected_option_id(sector_effects)), bill_support(faction_id, stance)").eq("nation_id",n).eq("status","passed").not("passed_tick","is",null).order("passed_tick",{ascending:!1}).limit(15),Promise.resolve({data:[],error:null}),a.from("elections").select("*").eq("nation_id",n).eq("status","scheduled").order("election_tick",{ascending:!0}).limit(5),a.from("ministries").select("party_id").eq("nation_id",n).eq("is_active",!0),ca(n)]);p.error&&console.error("[PartyOverview] Parties fetch error:",p.error.message),s.error&&console.error("[PartyOverview] Sectors fetch error:",s.error.message),u.error&&console.error("[PartyOverview] Caucus fetch error:",u.error.message),y.error&&console.error("[PartyOverview] Election fetch error:",y.error.message),d.error&&console.error("[PartyOverview] Passed-bills fetch error:",d.error.message);const m=p.data||[],x=s.data||[],b=v.administration,_=new Set((f.data||[]).map(I=>I.party_id).filter(Boolean));let M=[];if(m.length>0&&x.length>0){const I=m.map(z=>z.id),{data:N,error:A}=await a.from("faction_sector_popularity").select("faction_id, sector_id, popularity").in("faction_id",I);A&&console.error("[PartyOverview] Popularity fetch error:",A.message),M=N||[]}const w=qo(m,x,M),k=jo(m,x,M),E=b?.started_at_tick!=null?Math.max(0,c-b.started_at_tick):0,S=y.data||[],L=S[0]||null,T=L?Math.max(0,L.election_tick-c):null;let P=null;L&&i&&kt(i)&&(P=S.some(N=>N.election_type==="presidential"&&N.election_tick===L.election_tick)?"General":"Midterm"),aa={isGoverning:v.isGoverning,statusLabel:v.label,administration:b,ministryPartyIds:_,ticksInPower:E,myFaction:r,allParties:m,rivalParties:m.filter(I=>I.id!==l),blocMap:h,strongholdsByParty:w,sectorRanking:k,passedBills:d.data||[],sectors:x,caucuses:u.data||[],nextElection:L,nextElectionTicks:T,nextElectionLabel:P},Go(o)}catch(c){console.error("[PartyOverview] Init error:",c),o.innerHTML='<div style="padding:40px;text-align:center;color:var(--red);font-family:var(--font-mono);font-size:10px;">Failed to load party overview.</div>'}}function Go(a){const t=aa,e=t.myFaction,o=ut.nation,r=e?.party_color||e?.color||"#c8a832";ut.shard?.current_tick,t.administration?.admin_name||t.isGoverning;const i=t.statusLabel,n=t.isGoverning?"var(--green)":"var(--orange)",l=e?.seats||0,c=o?.total_seats||100,v=e?.momentum??50;a.innerHTML=`<div class="po-page">
        ${Vo(t,r,l,c,v)}
    ...
    </div>`}function Vo(a,t,e,o,r){const i=a.isGoverning?a.administration?.admin_name||"Government":"Opposition",n=(ut.nation?.government_type||"").toLowerCase().includes("monarchy"),l=n?"No elections":a.nextElectionTicks!=null?a.nextElectionTicks:"—",c=n?"var(--text-dim)":typeof l=="number"&&l<=3?"var(--red)":"var(--text-bright)",v=n?"NEXT ELECTION":a.nextElectionLabel?"NEXT "+a.nextElectionLabel.toUpperCase():"NEXT ELECTION";return`<div class="po-summary">
    ...
    </div>`}function Yo(a,t,e,o,r){const i=t?.leader_first_name&&t?.leader_last_name?`${t.leader_first_name} ${t.leader_last_name}`:"Unknown",n=((t?.leader_first_name||"?")[0]+(t?.leader_last_name||"?")[0]).toUpperCase();t?.leader_age&&`${t.leader_age}`;const l=t?.approval_rating??0;return`<div class="po-card po-identity" style="border-left-color:${e};">
    ...
    </div>`}function Wo(a,t,e){const o=t?.id,r=a.sectorRanking||[],i=(l,c)=>{const v=l.party_id===o,p=v?e:l.color||"#888",s=(Math.round(l.popularity)/10).toFixed(1),d=v?'<span class="po-stronghold-chip-label" style="font-weight:700;">You</span>':`<span class="po-stronghold-chip-label">${U(l.abbreviation)}</span>`;return`<div class="po-stronghold-chip" style="border-color:${p}66;background:${p}14;">
            ${d}
        ...
        <div style="padding:8px 12px;">
            ${r.map(l=>{const c=l.candidates||[],v=c.filter(x=>x.party_id!==o).slice(0,3).map(x=>i(x)).join(""),p=c.find(x=>x.party_id===o)||null,s=i(p||{party_id:o,popularity:0,color:e}),d=v?`<div class="po-stronghold-chips">${v}</div>`:'<div style="font-size:9px;color:var(--text-dim);font-family:var(--font-mono);padding:4px 0;">No other party popularity yet</div>',u=Number(l.weight)||0,y=u>=3?"var(--gold, #c9a449)":u===2?"var(--amber, #c8a64e)":"var(--text-secondary)",f=`<span style="display:inline-block;min-width:18px;padding:1px 5px;font-family:var(--font-mono);font-size:9px;font-weight:700;color:${y};border:1px solid ${y}66;background:${y}14;text-align:center;letter-spacing:0;">w${u}</span>`,h=(l.description||"").trim(),m=h?`<div style="font-family:var(--font-mono);font-size:9.5px;color:var(--text-dim);line-height:1.4;margin-top:2px;">${U(h)}</div>`:"";return`<div class="po-stronghold-row" style="align-items:flex-start;">
        ...
        </div>`;const e=a.faction?.seats||0,o=t.map(r=>{const i=r.relationship_score??50,n=i>60?"var(--green)":i>40?"var(--amber)":"var(--red)",l=Math.round((r.seat_share||0)*e),c=(r.dominant_axis||"").replace(/_/g,"/"),v=r.wing_end==="left"?c.split("/")[0]:c.split("/")[1]||"";return`<div class="po-caucus-row">
            <div>
        ...
        </div>`;const r=new Map((a.allParties||[]).map(l=>[l.id,l])),i=new Map((a.sectors||[]).map(l=>[l.sector_key,l.name]));return`<div class="po-card" style="flex:1;">
        <div class="po-card-header">
            <span class="po-card-title">RECENT BILLS</span>
            <span class="po-card-subtitle">passed bills · sector outcomes</span>
        </div>
        <div style="max-height:520px;overflow-y:auto;">${o.map(l=>{const c=Xo(l),v=e-(l.passed_tick||0),p=v===0?"just now":v+"t ago",s=new Map;for(const x of l.bill_support||[]){const b=x.stance==="accept"?"yes":x.stance==="reject"?"no":x.stance;(b==="yes"||b==="no")&&s.set(x.faction_id,b)}l.proposed_by&&s.set(l.proposed_by,"yes");const d=[],u=[];for(const[x,b]of s){const _=r.get(x);if(!_)continue;const M=Qo(_,x===l.proposed_by);b==="yes"?d.push(M):b==="no"&&u.push(M)}const y=r.get(l.proposed_by),f=y?`<span style="color:${y.party_color||y.color||"#888"};font-weight:700;">${U(y.abbreviation||y.faction_name||"?")}</span>`:'<span style="color:var(--text-dim);">unknown</span>',h=d.length?`<div style="margin-top:5px;display:flex;flex-wrap:wrap;gap:4px;align-items:center;">
               ...
               <div style="margin-left:40px;font-family:var(--font-mono);font-size:8px;line-height:1.6;margin-top:2px;">
                    ${Ne(c,i,!1)}
               </div>`:"",m=u.length?`<div style="margin-top:4px;display:flex;flex-wrap:wrap;gap:4px;align-items:center;">
    ...
    </div>`}function ti(a,t){const e=a.rivalParties,o=a.administration,r=ut.nation,i=o?.pm_party_id,n=r?.total_seats||100,l=e.map(c=>{const v=c.party_color||"#666",p=c.abbreviation||c.faction_name?.slice(0,3)?.toUpperCase()||"?",s=c.leader_first_name&&c.leader_last_name?`${c.leader_first_name} ${c.leader_last_name}`:"Unknown",d=c.seats||0,u=Sa(c,o,a.ministryPartyIds,r);let y=u.label;const f=u.isGoverning?"var(--green)":"var(--orange)";u.isGoverning&&u.label==="GOVERNING"&&(c.id===i?y="GOVERNING — LEAD":y="GOVERNING — JUNIOR");const h=d-(t?.seats||0),m=h>0?"var(--green)":h<0?"var(--red)":"var(--text-dim)",x=a.strongholdsByParty?.[c.id]||[],b=x.length>0?`<div style="display:flex;flex-wrap:wrap;gap:4px;">${x.map(_=>`<span style="font-family:var(--font-mono);font-size:9px;padding:2px 6px;border:1px solid ${v}44;background:${v}10;color:var(--text-bright);white-space:nowrap;">${U(_.name)}</span>`).join("")}</div>`:'<div style="font-family:var(--font-mono);font-size:9px;color:var(--text-dim);">Unaligned</div>';return`<div style="padding:12px 16px;border-bottom:1px solid var(--border-main);">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                ...
                <div style="display:flex;flex-direction:column;align-items:flex-end;gap:3px;">
                    <span style="font-family:var(--font-mono);font-size:9px;font-weight:700;padding:2px 7px;color:${f};background:${f}0a;border:1px solid ${f}44;white-space:nowrap;">${y}</span>
    ...
    ">
        <div class="pe-row-head" style="padding:12px 20px;cursor:pointer;display:flex;align-items:center;justify-content:space-between;gap:10px;">
            <div class="pe-row-head-left" style="display:flex;align-items:center;gap:12px;min-width:0;flex-wrap:wrap;">
                ...
                <div class="pe-top-chips" style="display:flex;gap:8px;margin-left:10px;flex-wrap:wrap;">
                    ${s.map(f=>`<div style="display:flex;align-items:center;gap:4px;">
                        ...
                        <span style="font-family:var(--font-mono);font-size:10px;color:var(--text-secondary);">${H(f.abbreviation)}</span>
            ...
            </div>`}).join(""),x=o?v?"Won Runoff":"Elected Outright":null,b=v?"#d4a83c":"#5c5",_=o?`<div style="margin:0 20px 16px;background:var(--bg-card);border:1px solid var(--border-main);border-left:3px solid ${u};">
            <div style="padding:12px 16px;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                    ...
                    ${x?`<span style="font-family:var(--font-mono);font-size:9px;font-weight:700;padding:2px 8px;color:${b};background:${b}0a;border:1px solid ${b}30;">${H(x).toUpperCase()}</span>`:""}
                </div>
                <div style="display:flex;align-items:center;gap:10px;">
                    ...
                    <div>
                        ...
                        <div style="font-family:var(--font-mono);font-size:9px;color:var(--text-dim);">President &middot; ${H(o.party_name||"")} &middot; ${(o.vote_percentage||0).toFixed(1)}% of vote${v?" (runoff)":""}</div>
                    </div>
                </div>
            </div>
            ...
            ${_}
        </div>`}return y}function ia(a){if(nt.length===0){a.innerHTML=`<div style="padding:12px 16px;">
        ...
        </div>`;return}const t=xt.faction?.id,e=xt.nation?.total_seats||100,o=Math.ceil(e/2)+1,r=nt.map((i,n)=>{if(i.election_type==="presidential")return ni(i);const l=Ft===i.id,c=(i.results?.votes||[]).sort((E,S)=>(S.seats||0)-(E.seats||0)),v=c.slice(0,3),p=i.results?.turnout_pct??0,s=i.results?.total_votes_cast??0,d=i.results?.sector_breakdown?.independent_seats??0,u=oa(i.election_tick),y=le.find(E=>E.started_at_tick>=i.election_tick&&E.started_at_tick<=i.election_tick+5),f=le.find(E=>E.ended_at_tick!=null&&E.ended_at_tick>=i.election_tick-2&&E.ended_at_tick<=i.election_tick+2),h=ai(i,f),m=kt(xt.nation),x=m?"President":"PM",b=y?.prime_minister||"Unknown",_=y?.pm_party_id&&c.find(E=>E.party_id===y.pm_party_id)?.color||"#888",w=(n<nt.length-1?nt[n+1]:null)?.results?.votes||[];let k=`<div data-election-id="${i.id}" style="
            background:var(--bg-panel);border:1px solid var(--border-main);
        ...
        ">
            <div class="pe-row-head" style="padding:12px 20px;cursor:pointer;display:flex;align-items:center;justify-content:space-between;gap:10px;">
                <div class="pe-row-head-left" style="display:flex;align-items:center;gap:12px;min-width:0;flex-wrap:wrap;">
                    ...
                    <div class="pe-top-chips" style="display:flex;gap:8px;margin-left:10px;flex-wrap:wrap;">
                        ${v.map(E=>`<div style="display:flex;align-items:center;gap:4px;">
                            ...
                            <span style="font-family:var(--font-mono);font-size:10px;color:var(--text-secondary);">${H(E.abbreviation)}</span>
                ...
                <div class="pe-row-head-right" style="display:flex;align-items:center;gap:10px;flex-shrink:0;">
                    ...
                    <div class="pe-col-party" style="flex:1;min-width:0;">
                        <div style="display:flex;align-items:center;gap:5px;">
                            <span style="font-size:13px;font-weight:700;color:var(--text-bright);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${H(I.party_name)}</span>
                        ...
                        <div style="font-family:var(--font-mono);font-size:9px;color:${I.color};">${H(I.abbreviation)}</div>
                    </div>
                ...
                </div>`:"");let P="";if(y){const I=y.coalition_parties||[],N=y.total_seats||I.reduce((J,It)=>J+(It.seats||0),0),A=N>=o,z=A?"Majority Coalition":"Minority Coalition",q=y.ended_at_tick?y.end_reason||"Ended":"Current Government",Y=y.ended_at_tick?"var(--text-dim)":"#5c5",et=y.ended_at_tick?Math.abs(y.ended_at_tick-y.started_at_tick)+" ticks":"Ongoing",ra=I.map(J=>{const It=c.find(Ht=>Ht.party_id===J.party_id)?.color||"#666";return`<div style="width:${N>0?(J.seats||0)/N*100:0}%;background:${It};height:100%;"></div>`}).join(""),sa=I.map(J=>`<div style="display:flex;align-items:center;gap:4px;">
                        <div style="width:8px;height:8px;background:${c.find(Ht=>Ht.party_id===J.party_id)?.color||"#666"};"></div>
                        <span style="font-family:var(--font-mono);font-size:10px;color:var(--text-secondary);">${H(J.party_name?.slice(0,3)?.toUpperCase()||"?")}</span>
                    ...
                    <div style="padding:12px 16px;">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                            <div style="display:flex;align-items:center;gap:8px;">
                                ...
                                <span style="font-family:var(--font-mono);font-size:9px;font-weight:700;padding:2px 8px;color:${Y};background:${Y}0a;border:1px solid ${Y}25;">${H(q.toUpperCase())}</span>
                            </div>
                            <span style="font-family:var(--font-mono);font-size:9px;color:var(--text-dim);">Lasted: ${et}</span>
                        </div>
                        <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
                            <div style="width:36px;height:36px;background:${_}15;border:1.5px solid ${_};display:flex;align-items:center;justify-content:center;font-family:var(--font-mono);font-size:11px;font-weight:700;color:${_};">${H(b.split(" ").map(J=>J[0]).join(""))}</div>
                            <div>
                                <div style="font-size:14px;font-weight:700;color:var(--text-bright);">${H(b)}</div>
                                <div style="font-family:var(--font-mono);font-size:9px;color:var(--text-dim);">${m?"President":"Prime Minister"} &middot; ${H(y.pm_party_name||"")} &middot; ${z}</div>
                            </div>
                        </div>
                ...
                <div style="display:flex;border-bottom:1px solid var(--border-main);">
                    <div style="flex:1;padding:12px 20px;border-right:1px solid var(--border-main);">
                        <div style="font-family:var(--font-mono);font-size:9px;font-weight:700;letter-spacing:1px;color:var(--text-dim);margin-bottom:4px;">CONTEXT</div>
                        <div style="font-size:13px;color:var(--text-secondary);line-height:1.5;">${H(h.label)}</div>
                    </div>
                    <div style="width:260px;padding:12px 20px;display:flex;gap:16px;flex-shrink:0;">
                        <div>
                            <div style="font-family:var(--font-mono);font-size:8px;color:var(--text-dim);letter-spacing:0.5px;">TURNOUT</div>
