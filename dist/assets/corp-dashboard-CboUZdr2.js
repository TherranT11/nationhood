import{_ as N}from"./supabase-client-BXEzLDpS.js";import{e as m}from"./utils-C2W-HleY.js";import{i as j}from"./messaging-5qyQ6ziq.js";let B=[];function c(e){return Math.abs(e)>=1e6?"$"+(e/1e6).toFixed(2)+"M":Math.abs(e)>=1e3?"$"+(e/1e3).toFixed(1)+"k":"$"+Math.round(e).toLocaleString()}function D(e,n){return Number(e?.[n]??50)}async function Y(){const{data:{user:e}}=await N.auth.getUser();if(!e){window.location.href="login.html";return}const{data:n}=await N.from("factions").select("*").or(`id.eq.${e.id},linked_user_id.eq.${e.id}`);B=(n||[]).filter(s=>s.nation_id);const o=sessionStorage.getItem("active_faction_id");let a=B.find(s=>s.id===o)||B.find(s=>s.faction_type==="corporation")||B[0];if(!a){console.error("Corp dashboard: no factions found"),await N.auth.signOut(),window.location.href="login.html";return}if(a.faction_type!=="corporation"){window.location.href="dashboard.html";return}let r=a.nation||"",p=null;const[t,i]=await Promise.all([a.nation_id?N.from("nations").select("*").eq("id",a.nation_id).single():Promise.resolve({data:null}),N.from("shard").select("current_tick, current_date, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single()]);t.error&&console.warn("Nation load failed:",t.error.message),t.data&&(r=t.data.name,p=t.data),i.error&&console.warn("Shard load failed:",i.error.message);const l=i.data,_=a.corp_ticker||a.abbreviation||"";if(document.getElementById("corp-logo").textContent=_.slice(0,2)||"—",document.getElementById("corp-name-bar").textContent=a.faction_name||"Unnamed Corp",l){if(document.getElementById("game-date").textContent=l.current_date||"—",document.getElementById("tick-number").textContent=l.current_tick||"—",l.next_tick_at){const u=(Number(l.tick_interval_hours)||8)*36e5,x=new Date(l.next_tick_at).getTime(),g=x-u+u/2;O=new Date(g>Date.now()?g:x+u/2),Z()}const s=document.querySelector("#tick-countdown")?.closest(".tick-item")?.querySelector(".tick-label");s&&(s.textContent="Next Corp Tick")}document.getElementById("corp-name-badge").textContent=(_?"["+_+"]":a.faction_name||"Corp")+" ▾";const h=document.getElementById("topbar-cash");if(h){const s=Number(a.corp_cash_reserves??0),u=s>=1e9?"$"+(s/1e9).toFixed(1)+"B":s>=1e6?"$"+(s/1e6).toFixed(1)+"M":"$"+Math.round(s/1e3)+"k";h.textContent="CASH: "+u}const d=a.action_points??0;document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+d+" AP</span>";const T=document.getElementById("corp-faction-dropdown");if(T){let s="";for(const v of B){const g=v.id===a.id,F=v.faction_type==="corporation"?"CORP":"PARTY",W=v.faction_type==="corporation"?"var(--teal)":"var(--amber)";s+=`<div class="corp-dd-item${g?" active":""}" onclick="switchToFaction('${v.id}', '${v.faction_type}')">
    ...
    `;const b=a.last_rename_tick||0,C=l?.current_tick||0,$=Math.max(0,b+120-C),k=$<=0,f=document.getElementById("slogan-editor");f.innerHTML=`
    ...
    `,window._corpFactionId=a.id,window._currentTick=C;const I=G(p,r,a);K(p,l,I,a),Q(r);const M=await J(p,r,a);X(p,r,a,I,M),j(a,p,l),document.getElementById("loading").style.display="none",document.getElementById("page-content").style.display="block"}function G(e,n,o){const a=g=>D(e,g),r=(n||"UNKNOWN").toUpperCase(),p=Number(o?.corp_general_workforce??2250),t=Number(o?.corp_skilled_workforce??600),i=Number(o?.corp_innovative_workforce??150),l=p+t+i,_=2,h=3,d=6,T=a("minimum_wage"),w=T/100*48e3,S=a("inflation"),L=a("standard_of_living"),b=1+(S-50)/100*.5,C=1+(L-50)/100*.5,E=g=>Math.round(w*g*b*C),$=E(_),k=E(h),f=E(d),I=p*$,M=t*k,s=i*f,u=I+M+s;function x(g){return"$"+Math.round(g).toLocaleString()+"/yr"}const v=`${b.toFixed(2)} &times; ${C.toFixed(2)}`;return document.getElementById("wf-total-header").textContent=l.toLocaleString(),document.getElementById("wf-body").innerHTML=`
        <div class="wf-tier">
    ...
    `,{totalWages:u,generalTotal:I,skilledTotal:M,innovativeTotal:s,monthlyWages:Math.round(u/12)}}function K(e,n,o,a){const r=n?.current_tick||0;document.getElementById("fin-tick").textContent="TICK "+r;const p=5e7,t=y=>D(e,y),i=1+(t("gdp_growth")-50)/100*.4,l=1+(t("urbanization")-50)/100*.3,_=1+(t("population_growth")-50)/100*.2,h=1+(t("standard_of_living")-50)/100*.15,d=1+(50-t("physical_infrastructure"))/100*.1,T=1-Math.max(0,t("inflation")-50)/100*.1,w=1-Math.max(0,t("interest_rates")-50)/100*.1,S=i*l*_*h*d*T*w,L=Math.round(p*S),b=Math.round(L/12),C=0,E=0,$=C+E+b,k=o?.totalWages||0,f=Math.round(k/12),I=0,M=0,s=0,u=0,x=0,v=f+I+M+s+u+x,g=$-v,F=Number(a?.corp_cash_reserves??0),W=0,z=[{stat:"gdp_growth",value:t("gdp_growth"),weight:"0.4"},{stat:"urbanization",value:t("urbanization"),weight:"0.3"},{stat:"population_growth",value:t("population_growth"),weight:"0.2"},{stat:"standard_of_living",value:t("standard_of_living"),weight:"0.15"},{stat:"physical_infra",value:t("physical_infrastructure"),weight:"0.1",note:"INV"},{stat:"inflation",value:t("inflation"),weight:"-0.1",neg:!0},{stat:"interest_rates",value:t("interest_rates"),weight:"-0.1",neg:!0}];function H(y){return y.neg?y.value>50?"var(--red)":"var(--green)":y.note?y.value<50?"var(--green)":"var(--red)":y.value>=50?"var(--green)":y.value>=35?"var(--amber)":"var(--red)"}const P=$||1,U=(C/P*100).toFixed(1),q=(E/P*100).toFixed(1),V=(b/P*100).toFixed(1);document.getElementById("fin-body").innerHTML=`
        <!-- Revenue -->
        <div class="fin-section">
            <div class="fin-section__title" style="color:var(--green);">Revenue</div>
            <div class="seg-bar">
                <div class="seg-bar__seg" style="width:${U}%;background:var(--teal);"></div>
    ...
    `}function Q(e){const n=e.toUpperCase(),o=[{label:"Reputation",value:65,change:0,decay:!0,max:100,isHero:!0},{label:"Workforce Size",value:3e3,change:0,section:"Core"},{label:"Workforce Skill",value:50,change:0,decay:!0,max:100},{label:"Operational Efficiency",value:50,change:0,decay:!0,max:100},{label:"Market Share",value:5,change:0,nation:n,max:100,section:"Market & Financials"},{label:"Credit Rating",value:50,change:0,max:100},{label:"Regulatory Standing",value:50,change:0,nation:n,max:100,section:"Political Standing"},{label:"Political Influence",value:10,change:0,decay:!0,nation:n,max:100},{label:"Innovation",value:20,change:0,decay:!0,max:100,section:"Innovation"}];function a(t,i){if(!i||i>100)return"var(--text-primary)";const l=t/i*100;return l>=70?"var(--green)":l>=40?"var(--amber)":l>=20?"var(--orange, #d48a3c)":"var(--red)"}function r(t){const i=parseFloat(t),l=i>0?"var(--green)":i<0?"var(--red)":"var(--text-dim)",_=i>0?"▲":i<0?"▼":"–";return`<div class="stat-item__change">
            ...
            </div>`}document.getElementById("stats-body").innerHTML=p}async function J(e,n,o,a){const r=(n||"UNKNOWN").toUpperCase();let p=[];if(o?.id){const{data:d}=await N.from("corp_properties").select("*").eq("faction_id",o.id).eq("is_active",!0).order("type",{ascending:!1}).order("purchased_at_tick",{ascending:!1});p=d||[]}const t={Basic:{color:"#6a6660"},Modern:{color:"#8b9a6b"},Sustainable:{color:"#5c5"},Innovative:{color:"#c8a832"},Heritage:{color:"#c84"},Premium:{color:"#ca5"}};let i=0,l=0,_=`
    <div class="prop-asset" style="margin-bottom:6px;">
        ...
        <div style="display:flex;gap:0;background:var(--bg-3);border:1px solid var(--border-hair);margin:4px 0;">
            ...
            <div style="flex:1;padding:3px 6px;border-right:1px solid var(--border-hair);">
                ...
                <div style="font-size:10px;font-weight:700;color:var(--gold);font-family:var(--font-mono);">Included</div>
            </div>
            <div style="flex:1;padding:3px 6px;">
                <div style="font-size:7px;color:var(--text-dim);font-family:var(--font-mono);letter-spacing:0.5px;">MAINT/MO</div>
                <div style="font-size:10px;font-weight:700;color:var(--text-dim);font-family:var(--font-mono);">$0</div>
            </div>
        </div>
    </div>`;for(const d of p){const T=t[d.style]||t.Basic;i+=Number(d.purchase_price||0),l+=Number(d.monthly_maintenance||0);const w=d.condition>=75?"var(--green)":d.condition>=50?"var(--amber)":"var(--orange)";_+=`
        <div class="prop-asset" style="margin-bottom:6px;">
            <div class="prop-asset__top">
                <div>
        ...
        </div>`}const h=document.getElementById("prop-count");return h&&(h.textContent=p.length+1+" ASSET"+(p.length+1!==1?"S":"")),document.getElementById("prop-body").innerHTML=`
        ${_}
    ...
    `,{propertyValue:i}}function X(e,n,o,a,r){(n||"UNKNOWN").toUpperCase();const p=o.corp_company_type||"Private",t=Number(o.corp_cash_reserves)||0,i=r?.propertyValue||0,l=0,_=0,h=t+i+l+_,d=Number(o.corp_loans)||0,w=a?.monthlyWages||0,S=0,L=d+w+S,b=h-L,E=Math.round(b*(1+.3)),$=E-b,k=$>0;document.getElementById("val-type-badge").textContent=p.toUpperCase();function f(I,M,s={}){const u=s.indent?"val-line val-line--indent":"val-line",x=s.bold?"val-line__label val-line__label--bold":"val-line__label",v=s.bold?"val-line__value val-line__value--bold":"val-line__value",g=s.color||(s.bold?"var(--text-bright)":"var(--text-muted)");return`<div class="${u}"><span class="${x}">${I}</span><span class="${v}" style="color:${g}">${c(M)}</span></div>`}document.getElementById("val-body").innerHTML=`
        ...
        ${f("Property",i,{indent:!0})}
        ${f("Equipment",l,{indent:!0})}
        ${f("Active Contracts",_,{indent:!0})}
        ${f("Total Assets",h,{bold:!0,color:"var(--green)"})}

        <div class="val-section-hdr"><span class="val-section-hdr__label" style="color:var(--red);">Liabilities</span></div>
        ${f("Outstanding Loans",d,{indent:!0})}
        ${f("Accounts Payable",w,{indent:!0})}
        ${f("Pending Project Costs",S,{indent:!0})}
        ${f("Total Liabilities",L,{bold:!0,color:"var(--red)"})}
