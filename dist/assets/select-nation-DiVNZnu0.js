import{_supabase as g}from"./supabase-client-qEAQbBjE.js";import{countEligibleParties as N,escapeHtml as m}from"./utils-A98FEun4.js";const V={Melizea:"assets/flags/Melizea.png",Avelia:"assets/flags/Avelia.png",Sangreza:"assets/flags/sangreza.png",Montequilla:"assets/flags/Montequilla.png","San Estrella":"assets/flags/sanestrella.png",Palvera:"assets/flags/Palvera.png",Calveth:"assets/flags/Calveth.png",Flandis:"assets/flags/Flandis.png",Vostia:"assets/flags/Vostia.png",Sierramar:"assets/flags/Sierramar.png",Hajjara:"assets/flags/Hajjara.png",Dravka:"assets/flags/Dravka.png"},B={Melizea:"A parliamentary republic with a diverse economy. Analogous to Colombia.",Sangreza:"A parliamentary democracy with strong traditions. Analogous to Spain.","San Estrella":"A presidential republic with a growing economy. Analogous to Mexico.",Palvera:"A presidential nation rich in natural resources. Analogous to Venezuela.",Montequilla:"A parliamentary republic with a service-driven economy. Analogous to Argentina.",Avelia:"A semi-presidential republic blending Spanish and Italian culture. Analogous to Italy.",Calveth:"A parliamentary democracy with strong social systems. Analogous to Denmark.",Flandis:"A parliamentary democracy with a trade-focused economy. Analogous to the Netherlands.",Vostia:"A parliamentary nation with a complex political landscape. Analogous to Serbia.",Sierramar:"A small Caribbean island democracy with a young, religious population. Analogous to Puerto Rico.",Hajjara:"A vast desert monarchy ruled by an absolute king, rich in oil and gas with deep Islamic traditions. Analogous to Iran.",Dravka:"Dravka is analogous culturally to Albania."},v=["#5b9bd5","#d48a3c","#5aafa5","#8b7ec8","#5cb85c","#d9534f","#c8a64e"],Y={Liberty:"#3b82f6",Equality:"#ef4444",Tradition:"#a855f7",Progress:"#22c55e",Security:"#f59e0b",Freedom:"#06b6d4",Globalism:"#14b8a6",Nationalism:"#f97316",Individualism:"#eab308",Collectivism:"#ec4899"};let A=null,$=[],f=null,T="Crucera",h={},b={},C={};const L=new Set;async function J(){try{const{data:{user:e}}=await g.auth.getUser();if(!e){window.location.href="login.html";return}if(A=e,sessionStorage.getItem("pending_faction_type")==="corp"){window.location.href="corp-setup.html";return}const a=sessionStorage.getItem("pending_faction_type")==="party",{data:n}=await g.from("factions").select("id, nation_id, faction_type").eq("id",e.id).maybeSingle();if(n&&n.nation_id&&!(a&&n.faction_type==="corporation")){window.location.href="dashboard.html";return}await Q(),document.getElementById("loading").style.display="none";const o=document.getElementById("page-content");o.style.display="flex"}catch(e){document.getElementById("loading").textContent="FAILED TO LOAD — PLEASE REFRESH";const a=document.getElementById("error-message");a&&(a.textContent=e.message||"Unknown error",a.style.display="block")}}function K(e){T=e,document.querySelectorAll(".continent-tab").forEach(a=>{a.classList.toggle("active",a.dataset.continent===e)}),R($,h,b,C)}async function Q(){const[e,a,n,o]=await Promise.all([g.from("nations").select("*, nation_profiles(flag_url)"),g.from("factions").select("id, faction_type, faction_name, abbreviation, seats, nation_id, party_color, ideology_value_1, ideology_value_2").eq("faction_type","party"),g.from("active_crises").select("nation_id, crisis_id, crisis_templates(name)"),g.from("faction_ideology").select("faction_id, liberty_equality, tradition_progress, security_freedom, globalism_nationalism, individualism_collectivism")]),r=e.data||[],d=a.data||[],t=n.data||[],l=o.data||[];h={},b={},C={};for(const i of d)h[i.nation_id]||(h[i.nation_id]=[]),h[i.nation_id].push(i);for(const i of t)b[i.nation_id]||(b[i.nation_id]=[]),b[i.nation_id].push(i);for(const i of l)C[i.faction_id]=i;$=r,R(r,h,b,C)}function F(e){return e==="Presidential"||e==="Semi-Presidential"?"gov-presidential":"gov-democracy"}function O(e){const a=typeof e=="string"?e:e?.government_type,n=typeof e=="object"?e?.hos_election_method:null;return a==="Presidential"?"PRESIDENTIAL":a==="Semi-Presidential"?"SEMI-PRESIDENTIAL":n==="hereditary"?"CONSTITUTIONAL MONARCHY":"PARLIAMENTARY"}function W(e){if(!e)return"";const a=[{key:"liberty_equality",neg:"Liberty",pos:"Equality"},{key:"tradition_progress",neg:"Tradition",pos:"Progress"},{key:"security_freedom",neg:"Security",pos:"Freedom"},{key:"globalism_nationalism",neg:"Globalism",pos:"Nationalism"},{key:"individualism_collectivism",neg:"Individualism",pos:"Collectivism"}],n=[];for(const o of a){const r=e[o.key]||0;Math.abs(r)>=20&&n.push(r<0?o.neg:o.pos)}return n.slice(0,2).join(", ")}function X(e){return e?e.split(", ").map(a=>{const n=Y[a];return n?`<span style="color:${n}">${m(a)}</span>`:m(a)}).join('<span style="color:var(--text-dim)">, </span>'):""}function R(e,a,n,o){const r=document.getElementById("nations-grid"),d=e.filter(t=>(t.continent||"Crucera")===T);if(d.sort((t,l)=>{const i=N(a[t.id]||[],t.id),c=N(a[l.id]||[],l.id),p=i>=(t.max_parties||8)?1:0,_=c>=(l.max_parties||8)?1:0;return p!==_?p-_:(t.name||"").localeCompare(l.name||"")}),d.length===0){r.innerHTML='<div style="font-family:var(--font-mono);font-size:10px;color:var(--text-dim);text-align:center;padding:40px 0;grid-column:1/-1;">No nations in this continent yet.</div>';return}r.innerHTML=d.map(t=>{const l=a[t.id]||[],i=n[t.id]||[],c=t.total_seats||100,p=t.max_parties||8,_=N(l,t.id),w=_>=p,S=t.nation_profiles,P=(Array.isArray(S)?S[0]?.flag_url:S?.flag_url)||t.flag_url||V[t.name]||`assets/flags/${t.name}.png`,q=F(t.government_type),D=O(t),j=L.has(t.id),E=[...l].sort((s,u)=>(u.seats||0)-(s.seats||0)),M=l.reduce((s,u)=>s+(u.seats||0),0),U=E.filter(s=>s.seats>0).map((s,u)=>{const I=(s.seats/c*100).toFixed(1),x=s.party_color||v[u%v.length];return`<div class="seat-bar__segment" style="width:${I}%;background:${x};" title="${m(s.faction_name)}: ${s.seats} seats"></div>`}).join(""),z=E.filter(s=>s.seats>0).map((s,u)=>`<span class="seat-legend-item"><span class="seat-legend-dot" style="background:${s.party_color||v[u%v.length]};"></span>${m(s.abbreviation||"?")} ${s.seats}</span>`).join(""),H=E.map((s,u)=>{const I=s.party_color||v[u%v.length],x=o[s.id],k=s.ideology_value_1&&s.ideology_value_2?`${s.ideology_value_1}, ${s.ideology_value_2}`:W(x);return`
        <div class="party-row">
      ...
      <div class="nation-card ${w?"disabled":""}" data-nation-id="${t.id}" onclick="${w?"":"selectNation('"+t.id+"')"}">
        <div class="nation-card__header">
          ...
          <span class="nation-card__name">${m(t.name)}${j?'<span class="alpha-badge">Alpha</span>':""}</span>
          <span class="nation-card__gov-type ${q}">${D}</span>
        </div>
        <div class="nation-card__body">
          ...
          <div class="seat-bar">
            ...
            <div class="seat-bar__legend">${z}</div>
          </div>
          `:""}
