import{_supabase as y}from"./supabase-client-qEAQbBjE.js";/* empty css                    *//* empty css                         */import{r as Ge}from"./role-actions-ClNxOfgz.js";import{escapeHtml as f,hfFmtBig as $}from"./utils-A98FEun4.js";import{renderCorpTopBar as Ve}from"./corp-topbar-BOR6jYkY.js";import{c as Ye}from"./corp-valuation-DvpMrhZz.js";import{E as fe,a as me,c as Ke,g as Xe}from"./corp-executives-Dy9E4H6_.js";import"./preload-helper-BXl3LOEh.js";import"./political-actions-CoM-LDWz.js";import"./config-CHsHqv7d.js";import"./government-structure-C17uG6rl.js";import"./stats-4gK98flh.js";function ae(e){if(e==null)return"";const t=document.createElement("div");return t.textContent=String(e),t.innerHTML}function We(e,t){return(e||"?")[0]+(t||"?")[0]}function Qe(e){return e>=70?"#5cb85c":e>=50?"#ca5":"#c84"}function Je(e){return e>=1e6?"$"+(e/1e6).toFixed(1)+"M":e>=1e3?"$"+(e/1e3).toFixed(0)+"k":"$"+e}function Ie(e){const t=Number(e)||0,o=Math.abs(t),i=o>=1e9?(o/1e9).toFixed(1)+"B":o>=1e6?(o/1e6).toFixed(1)+"M":o>=1e3?Math.round(o/1e3)+"k":String(Math.round(o));return(t<0?"-$":"$")+i}function Ze(e,t){if(!e)return;const{faction:o,shard:i,ownedProperties:n=[],vessels:a=[],executives:s=[],selectedExecIdx:l=0}=t||{},c=o?.faction_name||"Corporation",d=(o?.abbreviation||o?.corp_ticker||"??").toUpperCase(),v=o?.corp_sector||"",p=o?.corp_subsector||"",m=Number(o?.corp_cash_reserves||0),h=Number(o?.corp_loans||0),u=Ye({cash:m,loans:h,properties:n,vessels:a,financeReceivables:0,currentTick:i?.current_tick||0}),_=Number(o?.corp_reputation??50),b=Math.max(0,Math.min(100,Math.round(Number.isFinite(_)?_:50))),z=b>=60?"var(--green)":b>=40?"var(--text-bright)":"var(--red)",x=n.length,k=u<0?"var(--red)":"var(--green)";Ge(e,{title:"Corporate Actions",entityName:`${c} · ${d}`,entityColor:"#8b9a6b",stats:[{label:"Cash",value:Ie(m),color:"var(--accent)"},{label:"Reputation",value:String(b),color:z},{label:"Valuation",value:Ie(u),color:k}],statusBarItems:[{type:"count",label:"Sector",big:v||"—",bigColor:"#8b9a6b",dim1:p||""},{type:"count",label:"Properties",big:String(x),bigColor:"#8b9a6b",dim1:x===1?"building":"buildings"}],rolesContainerId:"corp-exec-list",panelContainerId:"corp-actions-panel",rolesColumnWidth:262});const I=document.getElementById("corp-exec-list");if(I){const g=new Map(s.map(T=>[T.role,T]));let M="";for(let T=0;T<fe.length;T++){const V=fe[T],qe=me[V],E=g.get(V)||null,ie=l===T,O=qe.color,Ue=!E;if(M+=`<div onclick="actSelectExec(${T})" style="
                padding:10px 12px;
                ...
                cursor:pointer;
            ">`,Ue&&V!=="CEO")M+=`<div style="display:flex;align-items:center;gap:10px;">
                    ...
                    <div style="flex:1;min-width:0;">
                        <div style="display:flex;align-items:center;gap:4px;">
                            <span style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:${O};">${ae(V)}</span>
                        </div>
                        <div style="font-size:13px;font-weight:600;color:var(--text-dim);margin-top:2px;">VACANT</div>
                        <div style="margin-top:4px;">
                            <span onclick="event.stopPropagation();openExecSearch('${V}')" style="font-family:var(--font-mono);font-size:9px;font-weight:700;letter-spacing:0.5px;padding:3px 8px;color:#5a8aaa;border:1px solid rgba(90,138,170,0.25);background:rgba(90,138,170,0.06);cursor:pointer;">EXECUTIVE SEARCH</span>
                        </div>
                    </div>
                </div>`;else{const je=E?`${E.first_name} ${E.last_name}`:"—",Se=E?E.age:0,ge=E?E.skill:0,De=E?E.salary_per_year:0,He=E?We(E.first_name,E.last_name):"—";M+=`<div style="display:flex;align-items:center;gap:10px;">
                    <div style="width:45px;height:45px;background:${O}15;border:1px solid ${O}33;display:flex;align-items:center;justify-content:center;font-family:var(--font-mono);font-size:14px;font-weight:700;color:${O};flex-shrink:0;">${ae(He)}</div>
                    <div style="flex:1;min-width:0;">
                        <div style="display:flex;align-items:center;gap:4px;">
                            <span style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:${O};">${ae(V)}</span>
                        </div>
                        <div style="font-size:13px;font-weight:600;color:${ie?"var(--text-bright,#f0efe6)":"var(--text-muted,#666)"};overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${ae(je)}${Se?` <span style="font-family:var(--font-mono);font-size:10px;color:var(--text-dim);">(${Se})</span>`:""}</div>
                        <div style="display:flex;align-items:center;gap:6px;margin-top:3px;">
                            <div style="display:flex;align-items:center;gap:3px;flex:1;">
                                <div style="flex:1;height:3px;background:var(--border-0,rgba(255,255,255,0.06));">
                                    <div style="width:${ge}%;height:100%;background:${Qe(ge)};"></div>
                                </div>
                                <span style="font-family:var(--font-mono);font-size:10px;color:var(--text-muted);width:18px;text-align:right;">${ge}</span>
                            </div>
                            <span style="font-family:var(--font-mono);font-size:9px;color:var(--text-dim);">${Je(De)}/yr</span>
                        </div>
                    </div>
                </div>`}M+="</div>"}I.innerHTML=M}const N=document.getElementById("corp-actions-panel");N&&(N.innerHTML='<div id="actions-right-panel"></div>')}const et=1e6,tt=1e10,Ne=1,Te=50,ot=12;function nt(e){if(e<0)return"A";const t=Math.min(e,25);return String.fromCharCode(65+t)}async function it(e,t,o){if(!e||!t||!o)return;if((e.corp_sector||"").toLowerCase()==="finance"){alert("Finance-sector corps fund equity; they do not raise it.");return}const i=prompt(`APPLY FOR EQUITY — STEP 1 / 3

How much capital do you want to raise? (in millions USD)

Example: 50 for a $50M raise.
Range: $1M – $10B.`);if(i===null)return;const n=parseFloat(i);if(isNaN(n)||n<=0){alert("Amount must be a positive number.");return}const a=Math.round(n*1e6);if(a<et){alert("Minimum raise is $1M.");return}if(a>tt){alert("Maximum raise is $10B.");return}const s=prompt(`APPLY FOR EQUITY — STEP 2 / 3

What stake are you offering in exchange? (percent)

Example: 12.5 for a 12.5% share of monthly profits.
Range: 1% – 50%.`);if(s===null)return;const l=parseFloat(s);if(isNaN(l)||l<Ne||l>Te){alert(`Stake must be between ${Ne}% and ${Te}%.`);return}const c=prompt(`APPLY FOR EQUITY — STEP 3 / 3

Describe the purpose of this raise.

Example: "Series B to fund fleet expansion across Mira ports."
Investment corps see this in Deal Flow when deciding whether to fund you.`);if(c===null)return;const d=(c||"").trim()||"Equity capital raise",{data:v,error:p}=await o.from("finance_loan_requests").select("id").eq("requesting_faction_id",e.id).eq("request_type","equity").eq("status","funded");if(p){alert("Could not look up prior raises: "+p.message);return}const m=nt((v||[]).length),h=`Post Series ${m} equity raise?
...
This action CANNOT be undone.`))return;if(prompt('Type "BANKRUPT" to confirm bankruptcy of '+n+":")!=="BANKRUPT"){alert("Bankruptcy cancelled.");return}xe=!0;try{const{data:s,error:l}=await y.rpc("declare_corp_bankruptcy",{p_faction_id:t});if(l)throw l;const c=Number(s?.total_payback||0),d=c>0?`
...
Please try again or contact support.`)}finally{xe=!1}}let q=[],r=null,de=null,w=null,J=[],oe=[],X=0,B=null,P=-1,Re=[];async function rt(){if(!r?.id)return;const{data:e}=await y.from("corp_properties").select("*").eq("faction_id",r.id).eq("is_active",!0);Re=e||[]}async function Me(){if(!r)return;const[e,t]=await Promise.all([y.from("corp_executives").select("*").eq("faction_id",r.id).eq("status","active"),y.from("executive_pool").select("*").eq("nation_id",r.nation_id).eq("status","available").order("skill",{ascending:!1})]);e.error&&console.warn("Failed to load executives:",e.error.message),t.error&&console.warn("Failed to load executive pool:",t.error.message),J=e.data||[],oe=t.data||[];const o=await Ke({supabase:y,faction:r,currentTick:w?.current_tick||0,poolCandidates:oe});o?.error&&console.warn("Failed to seed initial executive roster:",o.error.message||o.error),o?.executives&&(J=o.executives)}function ee(e){return e>=1e6?"$"+(e/1e6).toFixed(1)+"M":e>=1e3?"$"+(e/1e3).toFixed(0)+"k":"$"+e}function H(e){return J.find(t=>t.role===e)||null}function be(e,t){return(e||"?")[0]+(t||"?")[0]}function te(e){return e>=70?"#5cb85c":e>=50?"#ca5":"#c84"}function G(){const e=document.getElementById("actions-container");e&&(Ze(e,{faction:r,shard:w,ownedProperties:Re,vessels:[],executives:J,selectedExecIdx:X}),lt())}const we={CEO:[{id:"statement",name:"Issue Statement",desc:"Issue a press release to the public events feed. Other players and media corps see it. Cost scales with CEO skill.",cost:"~$20k",costColor:"#5cb85c",tags:["REPUTATION"],cooldown:"once/tick"},{id:"ipo",name:"IPO",desc:"Take the corporation public. Sell ~30% of shares for a massive cash injection. Permanent loss of full control.",cost:"COMING SOON",costColor:"var(--text-dim)",tags:["STRUCTURAL"],locked:!0,lockReason:"Coming soon"},{id:"bankruptcy",name:"Declare Bankruptcy",desc:"The CEO officially files for bankruptcy, ceasing all operations. Outstanding loans will be repaid up to 50% of the corporation's market valuation.",descRed:"This will dissolve your corporation. Loans will be paid back, and you will need to found a new corporation. There is a 24 tick cooldown on declaring bankruptcy.",cost:"IRREVERSIBLE",costColor:"#c55",tags:["IRREVERSIBLE"]}],CFO:[{id:"loan",name:"Request Loan",desc:"Submit a loan application to all finance corporations. Set amount, purpose, term, and collateral. Receive competing offers.",cost:"FREE",costColor:"#5cb85c",tags:["FINANCIAL"]},{id:"equity",name:"Apply for Equity",desc:"Raise capital by offering Investment corps a stake in your monthly profits. Series (A/B/C…) is auto-assigned by your prior funded raise count. No repayment — investors get a percentage of profit each tick going forward.",cost:"FREE",costColor:"#c89a4a",tags:["FINANCIAL","STRUCTURAL"],hideForSector:"Finance"}],COO:[{id:"restructure",name:"Restructure Operations",desc:"Lay off 10-20% of workforce, cut ~7% of debt. Reputation hit scales with COO skill — high skill minimizes damage.",cost:"FREE",costColor:"#5cb85c",tags:["OPERATIONAL"],cooldown:"once/tick"}],CTO:[{id:"research",name:"Begin Research",desc:"Start researching a tech tree node. Opens the tech tree interface.",cost:"COMING SOON",costColor:"var(--text-dim)",tags:["INNOVATION"],locked:!0,lockReason:"Coming soon"}],CMO:[{id:"rebrand",name:"Rebrand Corporation",desc:"Change name and abbreviation. Cost and reputation hit scale with CMO skill — high skill reduces both.",cost:"~$2M",costColor:"#ca5",tags:["STRUCTURAL"],cooldown:"once/tick"},{id:"branding",name:"Branding",desc:"Upload a custom corporate logo. Replaces the default monogram on the dashboard, nation roster, and shipping vessels.",cost:"$1M",costColor:"#ca5",tags:["IDENTITY"],cooldownTicks:12}],CLO:[{id:"sue_corp",name:"Sue Corporation",desc:"File a lawsuit against another corporation for patent infringement, contract breach, or predatory practices.",cost:"COMING SOON",costColor:"var(--text-dim)",tags:["LEGAL"],locked:!0,lockReason:"Coming soon"}],Lobbyist:[{id:"donate",name:"Political Donation",desc:"Donate $1M to a political party in the nation where your National HQ is located. The target party receives $100k in party funds. You cannot donate to your own party.",cost:"$1M",costColor:"#ca5",tags:["POLITICAL"],cooldown:"once/tick"}]};function ne(e){return 1.5-e/100}let Ae={};function st(e){const t=w?.current_tick||0;return Ae[e]===t}function W(e){const t=w?.current_tick||0;Ae[e]=t}function lt(){const e=document.getElementById("actions-right-panel");if(!e)return;const t=fe[X],o=me[t],i=H(t),n=(we[t]||[]).filter(s=>!s.hideForSector||(r?.corp_sector||"")!==s.hideForSector);if(!i){e.innerHTML=`<div style="padding:48px;text-align:center;background:var(--bg-2,#1a1a17);border:1px solid var(--border-0,rgba(255,255,255,0.06));">
        ...
        <div style="display:flex;align-items:center;gap:12px;">
            <div style="width:56px;height:56px;background:${o.color}15;border:1px solid ${o.color}33;display:flex;align-items:center;justify-content:center;font-family:var(--font-mono);font-size:18px;font-weight:700;color:${o.color};">${f(be(i.first_name,i.last_name))}</div>
            <div>
    ...
    </div>`,e.innerHTML=a,e.querySelectorAll("[onmouseenter]").forEach(s=>{s.addEventListener("mouseenter",function(){const l=this.querySelector(".act-exec-btn");l&&(l.style.display="block")}),s.addEventListener("mouseleave",function(){const l=this.querySelector(".act-exec-btn");l&&(l.style.display="none")})})}function ct(e,t,o,i,n){const a=w?.current_tick||0,s=Math.max(0,n-a),l=Math.round(i*(s/12)),c=`FIRE ${t}: ${o}
...
Are you sure?`;confirm(c)&&dt(e,t,l)}async function dt(e,t,o){try{const i=Number(r?.corp_cash_reserves??0);if(i<o){alert(`Insufficient funds. You need $${(o/1e6).toFixed(2)}M but only have $${(i/1e6).toFixed(2)}M.`);return}const n=i-o,{error:a}=await y.from("factions").update({corp_cash_reserves:n}).eq("id",r.id);if(a){alert("Failed to process payout: "+a.message);return}const{error:s}=await y.from("corp_executives").update({status:"fired",updated_at:new Date().toISOString()}).eq("id",e);if(s){await y.from("factions").update({corp_cash_reserves:i}).eq("id",r.id),alert("Failed to fire executive: "+s.message);return}r.corp_cash_reserves=n,J=J.filter(l=>l.id!==e),G()}catch(i){console.error("[CorpOps] Fire executive error:",i),alert("An error occurred.")}}function pt(e,t){const o=(we[t]||[]).find(i=>i.id===e);if(o?.cooldown==="once/tick"&&st(e)){alert("This action can only be used once per tick. Wait for the next tick.");return}if(o?.cooldownTicks){const i=ft(e);if(i>0){alert(`On cooldown — ${i} tick${i===1?"":"s"} remaining.`);return}}switch(e){case"statement":return mt();case"loan":return yt();case"equity":return it(r,w,y);case"restructure":return Ct();case"rebrand":return zt();case"branding":return It();case"donate":return Rt();case"bankruptcy":return at()}}function ft(e){if(e!=="branding")return 0;const o=(we.CMO||[]).find(s=>s.id===e)?.cooldownTicks||0,i=Number(r?.last_branding_tick);if(!Number.isFinite(i)||i<=0)return 0;const a=(Number(w?.current_tick)||0)-i;return a>=o?0:o-a}let he=!1;function mt(){if(he)return;he=!0;const e=document.createElement("div");e.id="stmt-overlay",e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:600;display:flex;align-items:center;justify-content:center;",e.onclick=function(c){c.target===e&&ke()};const t=r?.faction_name||"Corporation",o=(r?.abbreviation||r?.corp_ticker||"??").toUpperCase(),i=Number(r?.corp_cash_reserves??0),n=H("CEO"),a=n?`${n.first_name} ${n.last_name}`:"CEO";e.innerHTML=`<div onclick="event.stopPropagation()" style="width:480px;background:var(--panel-main);border:1px solid var(--panel-border);display:flex;flex-direction:column;overflow:hidden;">
    ...
    </div>`,document.body.appendChild(e);const s=document.getElementById("stmt-text"),l=document.getElementById("stmt-chars");s&&l&&(s.addEventListener("input",function(){l.textContent=this.value.length+"/500"}),s.focus())}function ke(){const e=document.getElementById("stmt-overlay");e&&e.remove(),he=!1}let Z=!1;async function vt(){if(!r||!w||Z)return;const e=document.getElementById("stmt-text"),t=document.getElementById("stmt-error"),o=(e?.value||"").trim();if(!o){t&&(t.textContent="Statement cannot be empty.",t.style.display="block");return}if(o.length>500){t&&(t.textContent="Statement too long (max 500 chars).",t.style.display="block");return}const i=H("CEO"),n=i?i.skill:50,a=Math.round(2e4*ne(n)),s=Number(r.corp_cash_reserves??0);if(s<a){t&&(t.textContent="Insufficient cash. Need "+$(a)+".",t.style.display="block");return}Z=!0;const l=document.getElementById("stmt-submit-btn");l&&(l.style.opacity="0.4",l.style.pointerEvents="none");const c=r.faction_name||"Corporation",d=i?`${i.first_name} ${i.last_name}`:"CEO",v=w.current_tick||0,{error:p}=await y.from("factions").update({corp_cash_reserves:s-a}).eq("id",r.id);if(p){Z=!1,t&&(t.textContent="Failed to deduct cost: "+p.message,t.style.display="block"),l&&(l.style.opacity="1",l.style.pointerEvents="auto");return}const{error:m}=await y.from("event_log").insert({nation_id:r.nation_id,faction_id:r.id,event_name:c+" — Press Release",description_used:d+", CEO of "+c+': "'+o.replace(/[<>"]/g,"")+'"',category:"business",trigger_key:"ceo_statement",effects_applied:{cost:a,ceo:d,skill:n},fired_at_tick:v});if(m){await y.from("factions").update({corp_cash_reserves:s}).eq("id",r.id),Z=!1,t&&(t.textContent="Failed to publish: "+m.message,t.style.display="block"),l&&(l.style.opacity="1",l.style.pointerEvents="auto");return}r.corp_cash_reserves=s-a,Z=!1,W("statement"),ke()}const ve=5e6,ye=5e8,_e=5e6;let A=25e7,S=new Set,Q=new Set,C=[],j=[];function yt(){A=25e7,S=new Set,Q=new Set,C=[],j=[],document.getElementById("lr-overlay").style.display="flex",D(),Promise.all([_t(),wt()]).then(()=>D()).catch(e=>console.error("[lrOpen] load failed:",e))}function Le(){document.getElementById("lr-overlay").style.display="none"}function gt(e){const t=Number(e)||0,o=Math.round(t/_e)*_e;A=Math.max(ve,Math.min(ye,o)),D()}function xt(e){e&&(S.has(e)?S.delete(e):S.add(e),D())}function ut(){S.size===C.length&&C.length>0?S=new Set:S=new Set(C.map(e=>e.id)),D()}function bt(){const e=r?.nation_id;if(e){for(const t of C)t.nation_id===e&&S.add(t.id);D()}}function ht(e){e&&(Q.has(e)?Q.delete(e):Q.add(e),D())}async function _t(){if(!r){C=[];return}const{data:e,error:t}=await y.from("factions").select("id, faction_name, corp_company_type, nation_id, corp_lending_capital, corp_interest_rates, corp_overleverage, nations:nation_id(name)").eq("faction_type","corporation").eq("corp_sector","Finance").is("abandoned_at",null).neq("id",r.id);if(t){console.warn("[lrLoadFinanceCorps] error:",t.message),C=[];return}C=e||[]}async function wt(){if(!r){j=[];return}const{data:e,error:t}=await y.from("corp_properties").select("id, name, purchase_price, nation_id, role, nations:nation_id(name)").eq("faction_id",r.id).eq("is_active",!0).order("purchase_price",{ascending:!1});if(t){console.warn("[lrLoadCollateral] error:",t.message),j=[];return}const o=e||[],i=[];if(r&&de){const n=Math.max(0,Math.min(100,Number(de.standard_of_living)||50)),a=Math.round(5e7+25e6*(n/100));i.push({id:"__national_hq__",synthetic:!0,seizable:!1,name:(r.faction_name||"Corporation")+" Headquarters",purchase_price:a,nation_id:r.nation_id,role:"national_hq",nations:{name:de.name||""}})}j=[...i,...o]}function ue(e,t){const o=Math.max(0,Math.min(10,Math.round(Number(e)||0))),i=[];for(let n=0;n<10;n++){const a=n<o;let s="";a&&(t==="leverage"?s=n<=2?" green":n<=5?" rust":" red":t&&t!=="gold"&&(s=" "+t)),i.push(`<span class="lr-pip${a?" filled"+s:""}"></span>`)}return i.join("")}function kt(e){const t=Number(e)||0;return t>=7?"var(--red, #d9534f)":t>=4?"var(--orange, #e8724a)":"var(--green, #5cb85c)"}function D(){const e=document.getElementById("lr-modal-content");if(!e)return;const t=r?.faction_name||"Corporation",o=(r?.abbreviation||r?.corp_ticker||"??").toUpperCase(),i=Math.round(A/1e6),n=i/5*.1;let a=0;for(const x of j)Q.has(x.id)&&(a+=Number(x.purchase_price||0));const s=A>0?Math.round(a/A*100):0,l=a===0?"unsecured":s<100?"partial":s<150?"full":"over",c=l==="unsecured"?"var(--text-dim, #6a6660)":l==="partial"?"var(--orange, #e8724a)":"var(--green, #5cb85c)",d=S.size,v=d===0;let p="";if(p+=`<div style="padding:18px 24px 16px;border-bottom:1px solid var(--panel-border);background:var(--bg-panel);flex-shrink:0;display:flex;justify-content:space-between;align-items:flex-start;">
        <div>
        ...
        <div style="background:var(--bg-panel);border:1px solid var(--panel-border);padding:18px 20px;">
            ...
            <input type="range" min="${ve}" max="${ye}" step="${_e}" value="${A}"
        ...
        <div style="display:flex;align-items:baseline;justify-content:space-between;margin-bottom:12px;padding-bottom:8px;border-bottom:1px dashed var(--panel-border);gap:12px;flex-wrap:wrap;">
            ...
            <div style="display:flex;align-items:center;gap:8px;">
                ...
                <div>
                    ...
                    <div style="display:flex;gap:2px;margin-top:2px;">${ue(I,"green")}</div>
                </div>
                <div>
                    ...
                    <div style="display:flex;gap:2px;margin-top:2px;">${ue(N,"rust")}</div>
                </div>
                <div>
                    ...
                    <div style="font-family:var(--font-mono);font-size:12px;font-weight:500;font-variant-numeric:tabular-nums;color:${kt(g)};">${g.toFixed(g%1===0?0:1)}</div>
                    <div style="display:flex;gap:2px;margin-top:2px;">${ue(g,"leverage")}</div>
                </div>
        ...
        <div style="padding:8px 16px;border-top:1px solid var(--panel-border);">
            ...
            <div style="background:var(--bg-panel);border:1px solid var(--panel-border);padding:6px 10px;">
                ...
                <div style="display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px solid var(--panel-border);">
                    ...
                    <span style="font-family:var(--font-mono);font-size:10px;font-weight:700;color:var(--panel-text);">${pe} TICKS</span>
                </div>
                <div style="display:flex;justify-content:space-between;padding:3px 0;">
                    <span style="font-family:var(--font-mono);font-size:8px;color:#6a6660;">CASH AFTER</span>
    ...
    </div>`,document.body.appendChild(o)}function Ee(){const e=document.getElementById("branding-overlay");e&&e.remove(),L=null}function Nt(e){const t=e.target.files?.[0],o=document.getElementById("branding-error"),i=document.getElementById("branding-filename");if(!t){L=null,i&&(i.textContent="");return}if(t.size>128*1024){L=null,i&&(i.textContent=""),o&&(o.textContent="File too large — must be under 128 KB.",o.style.display="block");return}L=t,o&&(o.style.display="none"),i&&(i.textContent=t.name);const n=document.getElementById("branding-preview");if(n){const a=new FileReader;a.onload=s=>{n.innerHTML=`<img src="${s.target.result}" alt="" style="width:100%;height:100%;object-fit:cover;">`},a.readAsDataURL(t)}}async function Tt(){if(!r||!w||Y)return;const e=document.getElementById("branding-error"),t=document.getElementById("branding-btn");if(!L){e&&(e.textContent="Choose a logo file first.",e.style.display="block");return}const o=Number(w.current_tick)||0,{data:i,error:n}=await y.from("factions").select("corp_cash_reserves, last_branding_tick").eq("id",r.id).single();if(n||!i){e&&(e.textContent="Failed to verify cooldown: "+(n?.message||"unknown"),e.style.display="block");return}const a=i.last_branding_tick==null?null:Number(i.last_branding_tick);if(a!=null&&o-a<pe){const u=pe-(o-a);e&&(e.textContent=`On cooldown — ${u} tick${u===1?"":"s"} remaining.`,e.style.display="block");return}const s=Number(i.corp_cash_reserves??0);if(s<R){e&&(e.textContent="Insufficient cash. Need "+$(R)+".",e.style.display="block");return}Y=!0,t&&(t.style.opacity="0.4",t.style.pointerEvents="none",t.textContent="UPLOADING...");let l;try{const u=(L.name.split(".").pop()||"png").toLowerCase().replace(/[^a-z0-9]/g,"")||"png",_=`party-logos/${r.id}/${Date.now()}.${u}`,{error:b}=await y.storage.from("public-assets").upload(_,L,{contentType:L.type,upsert:!0});if(b)throw b;const{data:z}=y.storage.from("public-assets").getPublicUrl(_);if(l=z?.publicUrl||null,!l)throw new Error("Could not resolve public URL.")}catch(u){Y=!1,e&&(e.textContent="Upload failed: "+(u.message||"Unknown error"),e.style.display="block"),t&&(t.style.opacity="1",t.style.pointerEvents="auto",t.textContent="UPLOAD & CONFIRM");return}const c={custom_logo_url:l,corp_cash_reserves:s-R,last_branding_tick:o};let d=y.from("factions").update(c).eq("id",r.id);d=a==null?d.is("last_branding_tick",null):d.eq("last_branding_tick",a);const{data:v,error:p}=await d.select("id");if(p){Y=!1,e&&(e.textContent="Failed: "+p.message,e.style.display="block"),t&&(t.style.opacity="1",t.style.pointerEvents="auto",t.textContent="UPLOAD & CONFIRM");return}if(!v||v.length===0){Y=!1,e&&(e.textContent="Branding is on cooldown. Refresh to see the latest state.",e.style.display="block"),t&&(t.style.opacity="1",t.style.pointerEvents="auto",t.textContent="UPLOAD & CONFIRM");return}r.custom_logo_url=l,r.corp_cash_reserves=s-R,r.last_branding_tick=o;const{error:m}=await y.from("event_log").insert({nation_id:r.nation_id,faction_id:r.id,event_name:"Corporation Rebranded (Logo)",description_used:`${r.faction_name||"Corporation"} unveiled a new corporate logo.`,category:"corporate",trigger_key:"corp_branding",effects_applied:{logo_url:l,cost:R,cooldown_ticks:pe},fired_at_tick:o});m&&console.warn("Failed to log branding event:",m.message),Y=!1,Ee(),G();const h=document.getElementById("corp-logo");h&&(h.innerHTML=`<img src="${l}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:5px;">`)}window.actBrandingPickFile=Nt;window.actSubmitBranding=Tt;window.actCloseBranding=Ee;let U=[],F=-1;async function Rt(){Number(r?.corp_cash_reserves??0);const e=[r.nation_id],t=new Set(q.map(n=>n.id)),{data:o}=await y.from("factions").select("id, faction_name, abbreviation, party_color, party_funds, seats, momentum, nation, nation_id, leader_ideology, linked_user_id").eq("faction_type","party").in("nation_id",e).is("abandoned_at",null).order("seats",{ascending:!1});U=(o||[]).filter(n=>!t.has(n.id)).map(n=>({...n})),F=-1;const i=document.createElement("div");i.id="donate-overlay",i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:600;display:flex;align-items:center;justify-content:center;",i.onclick=function(n){n.target===i&&ze()},document.body.appendChild(i),Oe()}function ze(){const e=document.getElementById("donate-overlay");e&&e.remove(),U=[],F=-1}function Mt(e){F=e,Oe()}function Oe(){const e=document.getElementById("donate-overlay");if(!e)return;const t=H("Lobbyist"),o=t?t.skill:50,i=Math.round(1e6*ne(o)),n=1e5,a=Number(r?.corp_cash_reserves??0),s=F>=0?U[F]:null,l=a>=i;let c='<div onclick="event.stopPropagation()" style="width:540px;max-height:80vh;background:var(--panel-main);border:1px solid var(--panel-border);display:flex;flex-direction:column;overflow:hidden;">';c+=`<div style="padding:14px 20px;border-bottom:1px solid var(--panel-border);background:var(--bg-panel);flex-shrink:0;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
            <div style="display:flex;align-items:center;gap:10px;">
    ...
    </div>`,c+='<div id="donate-error" style="padding:6px 20px;font-family:var(--font-mono);font-size:10px;color:#c55;display:none;"></div>',c+="</div>",e.innerHTML=c}let K=!1;async function At(){if(!r||!w||F<0||K)return;const e=U[F];if(!e)return;const t=Number(w?.current_tick||0);if(new Set(q.map(g=>g.id)).has(e.id)){const g=document.getElementById("donate-error");g&&(g.textContent="You cannot donate to your own party.",g.style.display="block");return}const i=H("Lobbyist"),n=i?i.skill:50,a=Math.round(1e6*ne(n)),s=1e5,l=2,{data:c,error:d}=await y.from("factions").select("corp_cash_reserves, last_donation_tick").eq("id",r.id).single();if(d||!c){const g=document.getElementById("donate-error");g&&(g.textContent="Failed to verify cooldown: "+(d?.message||"unknown"),g.style.display="block");return}const v=Number(c.last_donation_tick??0);if(v===t){const g=document.getElementById("donate-error");g&&(g.textContent="Political Donation is on cooldown until next tick.",g.style.display="block"),W("donate");return}const p=Number(c.corp_cash_reserves??0);if(p<a){const g=document.getElementById("donate-error");g&&(g.textContent="Insufficient cash. Need "+$(a)+", have "+$(p)+".",g.style.display="block");return}K=!0;const m=document.getElementById("donate-btn");m&&(m.style.opacity="0.4",m.style.pointerEvents="none");const h=Number(r.corp_reputation??50),u=Math.max(0,h-l),{data:_,error:b}=await y.from("factions").update({corp_cash_reserves:p-a,corp_reputation:u,last_donation_tick:t}).eq("id",r.id).eq("last_donation_tick",v).select("id");if(b){const g=document.getElementById("donate-error");K=!1,g&&(g.textContent="Failed: "+b.message,g.style.display="block"),m&&(m.style.opacity="1",m.style.pointerEvents="auto");return}if(!_||_.length===0){const g=document.getElementById("donate-error");K=!1,g&&(g.textContent="Political Donation is on cooldown until next tick.",g.style.display="block"),m&&(m.style.opacity="1",m.style.pointerEvents="auto"),W("donate");return}const{data:z}=await y.from("factions").select("party_funds").eq("id",e.id).single(),x=Number(z?.party_funds??0),{error:k}=await y.from("factions").update({party_funds:x+s}).eq("id",e.id);if(k){await y.from("factions").update({corp_cash_reserves:p}).eq("id",r.id);const g=document.getElementById("donate-error");K=!1,g&&(g.textContent="Failed to transfer funds: "+k.message,g.style.display="block"),m&&(m.style.opacity="1",m.style.pointerEvents="auto");return}r.corp_cash_reserves=p-a,r.corp_reputation=u;const I=r.faction_name||"Corporation",{error:N}=await y.from("event_log").insert({nation_id:e.nation_id||r.nation_id,faction_id:r.id,event_name:I+" — Political Donation",description_chosen:I+" has donated "+$(a)+" to "+(e.faction_name||"a political party")+". The party receives "+$(s)+" in campaign funds. Corporate reputation decreases by "+l+".",category:"business",trigger_key:"corp_donation",effects_applied:{cost:a,recipient_faction_id:e.id,recipient_name:e.faction_name,funds_granted:s,reputation_loss:l,skill:n},fired_at_tick:t});N&&console.warn("Failed to log donation event:",N.message),K=!1,W("donate"),ze()}function Lt(e){X=e,G()}async function Ot(e){if(B=e,P=-1,document.getElementById("exec-search-overlay").style.display="flex",oe.length===0&&r?.nation_id){const t=r.nation||"",o=Xe(r.nation_id,t),{error:i}=await y.from("executive_pool").insert(o);i&&console.warn("Failed to generate executive pool:",i.message);const{data:n,error:a}=await y.from("executive_pool").select("*").eq("nation_id",r.nation_id).eq("status","available").order("skill",{ascending:!1});a&&console.warn("Failed to reload executive pool:",a.message),oe=n||[]}Fe()}function Be(){document.getElementById("exec-search-overlay").style.display="none",B=null,P=-1}function Pe(e){return oe.filter(t=>t.status==="available"&&Array.isArray(t.specializations)&&t.specializations.includes(e)).sort((t,o)=>o.skill-t.skill)}function Bt(e){P=e,Fe()}let ce=!1;async function Pt(){if(!r||!w||!B||P<0||ce)return;const t=Pe(B)[P];if(!t)return;ce=!0;const o=w.current_tick||0,i=document.getElementById("es-hire-btn");i&&(i.style.opacity="0.4",i.style.pointerEvents="none");const{error:n}=await y.from("corp_executives").insert({faction_id:r.id,role:B,first_name:t.first_name,last_name:t.last_name,age:t.age,origin_nation:t.origin_nation,skill:t.skill,salary_per_year:t.required_salary,contract_years:t.required_years,contract_start_tick:o,contract_end_tick:o+t.required_years*12,status:"active"});if(n){ce=!1;const s=document.getElementById("es-error");s&&(s.textContent="Failed: "+n.message,s.style.display="block"),i&&(i.style.opacity="1",i.style.pointerEvents="auto");return}const{error:a}=await y.from("executive_pool").update({status:"hired",hired_by_faction_id:r.id}).eq("id",t.id);a&&console.warn("Failed to mark pool candidate as hired:",a.message),ce=!1,Be(),await Me(),X=fe.indexOf(B),X<0&&(X=0),G()}function Fe(){const e=document.getElementById("exec-search-content");if(!e||!B)return;const t=B,o=me[t],i=Pe(t),n=P>=0?i[P]:null;let a="";a+=`<div style="padding:12px 20px;border-bottom:1px solid var(--panel-border);background:var(--bg-panel);flex-shrink:0;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
            <div style="display:flex;align-items:center;gap:10px;">
        ...
        ">
            <div style="display:flex;align-items:center;gap:10px;">
                <div style="width:40px;height:40px;background:${o.color}10;border:1px solid ${o.color}22;display:flex;align-items:center;justify-content:center;font-family:var(--font-mono);font-size:12px;font-weight:700;color:${o.color};flex-shrink:0;">${f(be(l.first_name,l.last_name))}</div>
                <div style="flex:1;min-width:0;">
            ...
            <div style="display:flex;align-items:center;gap:14px;">
                <div style="width:64px;height:64px;background:${o.color}12;border:1px solid ${o.color}28;display:flex;align-items:center;justify-content:center;font-family:var(--font-mono);font-size:20px;font-weight:700;color:${o.color};">${f(be(n.first_name,n.last_name))}</div>
                <div>
    ...
    </div>`,a+='<div id="es-error" style="padding:5px 20px;font-family:var(--font-mono);font-size:10px;color:#c55;display:none;"></div>',e.innerHTML=a}async function Ft(){const{data:{user:e}}=await y.auth.getUser();if(!e){window.location.href="login.html";return}const t=new URLSearchParams(location.search).get("faction_id");if(t){const{data:a,error:s}=await y.from("factions").select("*").eq("id",t).single();s?console.warn("[Inspector] faction fetch failed:",s.message):a?.faction_type==="corporation"&&(r=a)}if(!r){const{data:a}=await y.from("factions").select("*").or(`id.eq.${e.id},linked_user_id.eq.${e.id}`);q=(a||[]).filter(l=>l.nation_id);const s=sessionStorage.getItem("active_faction_id");if(r=q.find(l=>l.id===s)||q.find(l=>l.faction_type==="corporation")||q[0],!r){await y.auth.signOut(),window.location.href="login.html";return}if(r.faction_type!=="corporation"){window.location.href="dashboard.html";return}}const[o,i]=await Promise.all([r.nation_id?y.from("nations").select("*").eq("id",r.nation_id).single():Promise.resolve({data:null}),y.from("shard").select("current_tick, current_date, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single()]);o.data&&(de=o.data),w=i.data;const n=document.getElementById("corp-topbar-container");n&&Ve(n,{faction:r,shard:w,activeTab:"actions",allUserFactions:q}),document.getElementById("loading").style.display="none",document.getElementById("page-content").style.display="block",await Promise.all([rt(),Me()]),G()}window.actExecute=pt;window.actSelectExec=Lt;window.confirmFireExec=ct;window.actCloseStatement=ke;window.actSubmitStatement=vt;window.actCloseRestructure=$e;window.actSubmitRestructure=Et;window.actCloseRebrand=Ce;window.actSubmitRebrand=St;window.actCloseDonation=ze;window.actSubmitDonation=At;window.donateSelectParty=Mt;window.lrClose=Le;window.lrSetAmount=gt;window.lrToggleBank=xt;window.lrToggleCollateral=ht;window.lrToggleSelectAll=ut;window.lrSelectAllInMyNation=bt;window.lrSubmit=$t;window.openExecSearch=Ot;window.closeExecSearch=Be;window.esSelectCandidate=Bt;window.esHireCandidate=Pt;Ft();
