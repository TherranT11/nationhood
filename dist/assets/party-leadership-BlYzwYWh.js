import{_ as m}from"./supabase-client-BXEzLDpS.js";/* empty css                  */import{i as j}from"./common-BNB7HOYn.js";import{e as v}from"./utils-C2W-HleY.js";import{deductAP as U}from"./config-fKhFNVuq.js";import{A as x,B as A,C as R,D as I,I as G,E as z,N as V,F as T,d as B}from"./elections-CyO2onts.js";import{k as H}from"./government-types-BwzErBjP.js";import"./ideology-5B3UuuGK.js";import"./stats-DqgGwtpW.js";import"./government-structure-3HZRNYFO.js";let p=null,i=null,L=null,_={leader:[]},E={leader:null},w={leader:null},b=!1,$=null;j("politics",async t=>{const{nation:e,faction:a,shard:s}=t;if(p=e,i=a,L=s,!e||!a){document.getElementById("pl-container").innerHTML='<div style="padding:40px;text-align:center;color:var(--text-dim);font-family:var(--font-mono);font-size:10px;">No nation or party found.</div>';return}W(),await Y(),P()});function W(){i.leader_first_name&&i.leader_last_name&&(E.leader={firstName:i.leader_first_name,lastName:i.leader_last_name,age:i.leader_age||null,electability:i.electability??50,ideology:i.leader_ideology||null,positiveTraits:i.leader_positive_traits||[],negativeTraits:i.leader_negative_traits||[]})}function O(){const t=E.leader;return t?{firstName:t.firstName,lastName:t.lastName,age:t.age||45,ideology:t.ideology||i.ideology_value_1||"PROGRESS",positiveTraits:t.positiveTraits||[],negativeTraits:t.negativeTraits||[],apCost:0,costBreakdown:{positiveTotal:0,negativeTotal:0,rawCost:0,apCost:0},isCurrent:!0,electability:t.electability??50}:null}async function J(t,e,a){const{error:s}=await m.from("leadership_candidates").upsert({faction_id:t,role:e,candidates:a,generated_at:new Date().toISOString()},{onConflict:"faction_id,role"});s&&console.warn(`Failed to persist ${e} candidates (non-blocking):`,s.message)}async function Y(){const t=p.name||"",e=i.id;let a=null;const s=`leadership_candidates_${e}_leader`,{data:d,error:n}=await m.from("leadership_candidates").select("candidates, last_refresh_tick").eq("faction_id",e).eq("role","leader").maybeSingle();if(!n&&d&&Array.isArray(d.candidates)&&d.candidates.length>0){a=d.candidates,$=d.last_refresh_tick;try{sessionStorage.setItem(s,JSON.stringify(a))}catch{}}else{try{const c=sessionStorage.getItem(s);if(c){const f=JSON.parse(c);Array.isArray(f)&&f.length>0&&(a=f)}}catch{}if(!a){a=x(t,B,3,"leader"),await J(e,"leader",a);try{sessionStorage.setItem(s,JSON.stringify(a))}catch{}}}_.leader=a;const o=O();o&&_.leader.unshift(o),w.leader=null}function P(){const t=document.getElementById("pl-container"),e=_.leader,a=w.leader,s=a!==null?e[a]:null;let d="",n="";if(s&&!s.isCurrent){const o=A(s.apCost),c=R(s.apCost);d=`<span class="pl-header-cost" style="color:${o}">${c}</span>`,n='<button class="pl-appoint-btn" onclick="window._appointLeader()">Appoint Party Leader</button>'}t.innerHTML=`
        <div class="pl-header">
    ...
    `}function Q(t,e){return!t||t.length===0?'<div style="padding:20px;text-align:center;color:var(--text-dim);font-family:var(--font-mono);font-size:9px;">No candidates available.</div>':t.map((a,s)=>{const d=s===e,n=A(a.apCost),o=R(a.apCost),c=a.ideology||"",f=G[c]||"#888",y=c?c.charAt(0)+c.slice(1).toLowerCase():"Unknown",u=a.electability!=null?I(a.electability):null,g=a.isCurrent?'<span class="pl-current-pill">CURRENT</span>':"";return`
        ...
        `}
    `}function Z(){const t=L?.current_tick||0;return $!=null&&$>=t?`<div class="pl-refresh-wrap">
    ...
    </div>`}window._refreshLeaders=async function(){if(b)return;const t=L?.current_tick||0;if($!=null&&$>=t){alert("Can only be used once per tick.");return}if(!confirm(`Refresh all leadership candidates?
