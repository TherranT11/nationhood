import{_ as p}from"./supabase-client-BXEzLDpS.js";import{e as v,t as te}from"./utils-C2W-HleY.js";import{initMessaging as fe}from"./messaging-B5Fng3EZ.js";import{c as me}from"./equipment-DsuDdEne.js";let B=[];function f(e){return Math.abs(e)>=1e6?"$"+(e/1e6).toFixed(2)+"M":Math.abs(e)>=1e3?"$"+(e/1e3).toFixed(1)+"k":"$"+Math.round(e).toLocaleString()}function q(e,t){return Number(e?.[t]??50)}async function ue(){const{data:{user:e}}=await p.auth.getUser();if(!e){window.location.href="login.html";return}const{data:t}=await p.from("factions").select("*").or(`id.eq.${e.id},linked_user_id.eq.${e.id}`);B=(t||[]).filter(m=>m.nation_id&&!m.abandoned_at);const n=sessionStorage.getItem("active_faction_id");let a=B.find(m=>m.id===n)||B.find(m=>m.faction_type==="corporation")||B[0];if(!a){sessionStorage.removeItem("active_faction_id"),window.location.href="faction-select.html";return}if(sessionStorage.setItem("active_faction_id",a.id),a.faction_type!=="corporation"){window.location.href="dashboard.html";return}let s=a.nation||"",r=null;const[c,i]=await Promise.all([a.nation_id?p.from("nations").select("*").eq("id",a.nation_id).single():Promise.resolve({data:null}),p.from("shard").select("current_tick, current_date, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single()]);c.error&&console.warn("Nation load failed:",c.error.message),c.data&&(s=c.data.name,r=c.data),i.error&&console.warn("Shard load failed:",i.error.message);const o=i.data,d=a.corp_ticker||a.abbreviation||"";if(document.getElementById("corp-logo").textContent=d.slice(0,2)||"—",document.getElementById("corp-name-bar").textContent=a.faction_name||"Unnamed Corp",o){if(document.getElementById("game-date").textContent=o.current_date||"—",document.getElementById("tick-number").textContent=o.current_tick||"—",o.next_tick_at){const $=(Number(o.tick_interval_hours)||8)*36e5,M=new Date(o.next_tick_at).getTime(),P=M-$+$/2;V=new Date(P>Date.now()?P:M+$/2),xe()}const m=document.querySelector("#tick-countdown")?.closest(".tick-item")?.querySelector(".tick-label");m&&(m.textContent="Next Corp Tick")}document.getElementById("corp-name-badge").textContent=(d?"["+d+"]":a.faction_name||"Corp")+" ▾";const _=document.getElementById("topbar-cash");if(_){const m=Number(a.corp_cash_reserves??0),$=m>=1e9?"$"+(m/1e9).toFixed(1)+"B":m>=1e6?"$"+(m/1e6).toFixed(1)+"M":"$"+Math.round(m/1e3)+"k";_.textContent="CASH: "+$}const u=document.getElementById("topbar-ap");u&&(u.style.display="none");const b=document.getElementById("corp-faction-dropdown");if(b){let m="";for(const S of B){const P=S.id===a.id,F=S.faction_type==="corporation"?"CORP":"PARTY",D=S.faction_type==="corporation"?"var(--teal)":"var(--amber)";m+=`<div class="corp-dd-item${P?" active":""}" onclick="switchToFaction('${S.id}', '${S.faction_type}')">
            ...
            </div>`}B.some(S=>S.faction_type==="corporation")||(m+=`<div class="corp-dd-item corp-dd-item--create" onclick="window.location.href='corp-setup.html'">
                <span class="corp-dd-type" style="color:var(--teal)">+</span>
                <span class="corp-dd-name">Found a Corporation</span>
            </div>`),B.some(S=>S.faction_type==="party")||(m+=`<div class="corp-dd-item corp-dd-item--create" onclick="sessionStorage.setItem('pending_faction_type','party'); window.location.href='select-nation.html'">
                <span class="corp-dd-type" style="color:var(--amber)">+</span>
                <span class="corp-dd-name">Found a Political Party</span>
            </div>`),b.innerHTML=m}document.getElementById("id-type-badge").textContent=a.corp_company_type||"—",document.getElementById("id-logo").textContent=d.slice(0,3)||"—",document.getElementById("id-corp-name").textContent=a.faction_name||"Unnamed Corp";const g=a.party_description||"";document.getElementById("id-slogan").textContent=g?'"'+g+'"':'"--"';const x=o?.current_date?o.current_date.replace(/.*,\s*/,""):"—",R=a.leader_first_name&&a.leader_last_name?a.leader_first_name+" "+a.leader_last_name+(a.leader_age?" ("+a.leader_age+")":""):"—";document.getElementById("id-rows").innerHTML=`
        <div class="id-row">
    ...
    `;const y=a.last_rename_tick||0,T=o?.current_tick||0,C=Math.max(0,y+120-T),l=!g||g==="-"||g==='"-"'||C<=0,L=document.getElementById("slogan-editor");L.innerHTML=`
    ...
    `,window._corpFactionId=a.id,window._currentTick=T,window._nationStats=r,window._factionData=a;const N=ge(r,s,a);ye(s,a);const w=await he(r,s,a,o);let I=0;if(a?.id){const{data:m,error:$}=await p.from("corp_equipment").select("equipment_key, owned").eq("faction_id",a.id);$||(I=me(m||[]))}be(r,o,N,a,w.propertyMaintenance||0,I),we(r,s,a,N,w),fe(a,r,o),A={nationId:a.nation_id},re(),document.getElementById("loading").style.display="none",document.getElementById("page-content").style.display="block"}function ge(e,t,n){const a=M=>q(e,M),s=(t||"UNKNOWN").toUpperCase(),r=Number(n?.corp_general_workforce??2250),c=Number(n?.corp_skilled_workforce??600),i=Number(n?.corp_innovative_workforce??150),o=r+c+i,d=2,_=3,u=6,b=a("minimum_wage"),g=b/100*48e3,x=a("inflation"),R=a("standard_of_living"),y=1+(x-50)/100*.5,T=1+(R-50)/100*.5,h=M=>Math.round(g*M*y*T),C=h(d),E=h(_),l=h(u),L=r*C,N=c*E,w=i*l,I=L+N+w;function m(M){return"$"+Math.round(M).toLocaleString()+"/yr"}const $=`${y.toFixed(2)} &times; ${T.toFixed(2)}`;return document.getElementById("wf-total-header").textContent=o.toLocaleString(),document.getElementById("wf-body").innerHTML=`
        <div class="wf-tier">
    ...
    `,{totalWages:I,generalTotal:L,skilledTotal:N,innovativeTotal:w,monthlyWages:Math.round(I/12)}}function be(e,t,n,a,s,r){const c=t?.current_tick||0;document.getElementById("fin-tick").textContent="TICK "+c;const i=5e7,o=k=>q(e,k),d=1+(o("gdp_growth")-50)/100*.4,_=1+(o("urbanization")-50)/100*.3,u=1+(o("population_growth")-50)/100*.2,b=1+(o("standard_of_living")-50)/100*.15,g=1+(50-o("physical_infrastructure"))/100*.1,x=1-Math.max(0,o("inflation")-50)/100*.1,R=1-Math.max(0,o("interest_rates")-50)/100*.1,y=d*_*u*b*g*x*R,T=Math.round(i*y),h=Math.round(T/12),C=0,E=0,l=C+E+h,L=n?.totalWages||0,N=Math.round(L/12),w=0,I=0,m=s||0,$=r||0,M=Number(a?.corp_loans)||0,S=.05,P=M>0?Math.round(M*(S/12)/(1-Math.pow(1+S/12,-120))):0,F=N+w+m+$+P+I,D=l-F,ie=Number(a?.corp_cash_reserves??0),le=M,ce=[{stat:"gdp_growth",value:o("gdp_growth"),weight:"0.4"},{stat:"urbanization",value:o("urbanization"),weight:"0.3"},{stat:"population_growth",value:o("population_growth"),weight:"0.2"},{stat:"standard_of_living",value:o("standard_of_living"),weight:"0.15"},{stat:"physical_infra",value:o("physical_infrastructure"),weight:"0.1",note:"INV"},{stat:"inflation",value:o("inflation"),weight:"-0.1",neg:!0},{stat:"interest_rates",value:o("interest_rates"),weight:"-0.1",neg:!0}];function de(k){return k.neg?k.value>50?"var(--red)":"var(--green)":k.note?k.value<50?"var(--green)":"var(--red)":k.value>=50?"var(--green)":k.value>=35?"var(--amber)":"var(--red)"}const H=l||1,pe=(C/H*100).toFixed(1),ve=(E/H*100).toFixed(1),_e=(h/H*100).toFixed(1);document.getElementById("fin-body").innerHTML=`
        <!-- Revenue -->
    ...
    `}function ye(e,t){const n=(e||"").toUpperCase(),a=Number(t.corp_general_workforce??0)+Number(t.corp_skilled_workforce??0)+Number(t.corp_innovative_workforce??0),s=[{label:"Reputation",value:Number(t.corp_reputation??65),change:0,decay:!0,max:100,isHero:!0},{label:"Workforce Size",value:a||3e3,change:0,section:"Core"},{label:"Workforce Skill",value:Number(t.corp_workforce_skill??50),change:0,decay:!0,max:100},{label:"Operational Efficiency",value:Number(t.corp_operational_efficiency??50),change:0,decay:!0,max:100},{label:"Market Share",value:Number(t.corp_market_share??5),change:0,nation:n,max:100,section:"Market & Financials"},{label:"Credit Rating",value:Number(t.corp_credit_rating??50),change:0,max:100},{label:"Regulatory Standing",value:Number(t.corp_regulatory_standing??50),change:0,nation:n,max:100,section:"Political Standing"},{label:"Political Influence",value:Number(t.corp_political_influence??10),change:0,decay:!0,nation:n,max:100},{label:"Innovation",value:Number(t.corp_innovation??20),change:0,decay:!0,max:100,section:"Innovation"}];function r(o,d){if(!d||d>100)return"var(--text-primary)";const _=o/d*100;return _>=70?"var(--green)":_>=40?"var(--amber)":_>=20?"var(--orange, #d48a3c)":"var(--red)"}function c(o){const d=parseFloat(o),_=d>0?"var(--green)":d<0?"var(--red)":"var(--text-dim)",u=d>0?"▲":d<0?"▼":"–";return`<div class="stat-item__change">
            ...
            </div>`}document.getElementById("stats-body").innerHTML=i}async function he(e,t,n,a){const s=(t||"UNKNOWN").toUpperCase();let r=[];if(n?.id){const{data:l}=await p.from("corp_properties").select("*").eq("faction_id",n.id).eq("is_active",!0).order("type",{ascending:!1}).order("purchased_at_tick",{ascending:!1});r=l||[]}const c={Basic:{color:"#6a6660"},Modern:{color:"#8b9a6b"},Sustainable:{color:"#5c5"},Innovative:{color:"#c8a832"},Heritage:{color:"#c84"},Premium:{color:"#ca5"}};let i=0,o=0;const d=Number(n?.corp_general_workforce??0)+Number(n?.corp_skilled_workforce??0)+Number(n?.corp_innovative_workforce??0),_=500,u=_+r.reduce((l,L)=>l+Number(L.capacity||0),0),b=u>0?Math.round(d*(_/u)):d,g=5e7,x=1+(q(e,"inflation")-50)/100*.3,R=.8+q(e,"stability")/100*.4,y=Math.round(g*x*R),T=Math.round(y*.005);i+=y,o+=T;let h=`
    ...
    </div>`,C=b;for(const l of r){const L=c[l.style]||c.Basic;i+=Number(l.purchase_price||0),o+=Number(l.monthly_maintenance||0);const N=l.condition>=75?"var(--green)":l.condition>=50?"var(--amber)":"var(--orange)",w=Number(l.capacity||0),I=u>0?Math.min(d-C,Math.round(d*(w/u))):0;C+=I,h+=`
        <div class="prop-asset" style="margin-bottom:6px;">
            <div class="prop-asset__top">
                <div>
    ...
    `,{propertyValue:i,propertyMaintenance:o}}function we(e,t,n,a,s){(t||"UNKNOWN").toUpperCase();const r=n.corp_company_type||"Private",c=Number(n.corp_cash_reserves)||0,i=s?.propertyValue||0,o=0,d=0,_=c+i+o+d,u=Number(n.corp_loans)||0,g=a?.monthlyWages||0,x=0,R=u+g+x,y=_-R,h=Math.round(y*(1+.3)),C=h-y,E=C>0;document.getElementById("val-type-badge").textContent=r.toUpperCase();function l(L,N,w={}){const I=w.indent?"val-line val-line--indent":"val-line",m=w.bold?"val-line__label val-line__label--bold":"val-line__label",$=w.bold?"val-line__value val-line__value--bold":"val-line__value",M=w.color||(w.bold?"var(--text-bright)":"var(--text-muted)");return`<div class="${I}"><span class="${m}">${L}</span><span class="${$}" style="color:${M}">${f(N)}</span></div>`}document.getElementById("val-body").innerHTML=`
...
Please try again or contact support.`),i&&(i.disabled=!1,i.textContent="Dissolve Corporation",i.style.opacity="1")}finally{U=!1}}window.dissolveCorporation=Me;let W=!1;function Se(e,t,n,a){if(W)return;const s=window._nationStats,c=1+(q(s,"inflation")-50)/100*.3,i=Math.max(.1,a/100),o=Math.round(n*c*i),d=document.getElementById("prop-modal-overlay"),_=document.getElementById("prop-modal-content");_.innerHTML=`
    ...
    `,d.style.display="flex"}async function Te(e,t){if(W)return;W=!0;const n=document.getElementById("prop-sell-confirm");n&&(n.disabled=!0,n.textContent="Selling...");try{const a=window._corpFactionId;if(!a)throw new Error("No faction");const{error:s}=await p.from("corp_properties").update({is_active:!1}).eq("id",e).eq("faction_id",a);if(s)throw new Error("Failed to sell property: "+s.message);const{data:r}=await p.from("factions").select("corp_cash_reserves").eq("id",a).single(),c=Number(r?.corp_cash_reserves??0),{error:i}=await p.from("factions").update({corp_cash_reserves:c+t}).eq("id",a);i&&console.error("[Property] Failed to credit cash:",i.message),j(),alert("Property sold for "+f(t)+". Cash credited."),location.reload()}catch(a){alert("Sale failed: "+a.message)}finally{W=!1,n&&(n.disabled=!1,n.textContent="Confirm Sale")}}let z=!1;function Ne(e,t,n,a){if(z)return;const s=window._nationStats,r=window._factionData,i=1+(q(s,"inflation")-50)/100*.3,o=Math.round(2e6*(a/1e3)),d=Math.round(o*i),_=Math.max(50,Math.round(a*.1)),u=Number(r?.corp_general_workforce??0),b=u>=_,x=Number(r?.corp_cash_reserves??0)>=d,R=document.getElementById("prop-modal-overlay"),y=document.getElementById("prop-modal-content"),T=b&&x&&n<100;let h="";n>=100?h='<div style="color:var(--green);font-size:9px;margin-bottom:8px;">Property is already at 100% condition.</div>':x?b||(h='<div style="color:var(--red);font-size:9px;margin-bottom:8px;">Insufficient general workforce (need '+_.toLocaleString()+", have "+u.toLocaleString()+").</div>"):h='<div style="color:var(--red);font-size:9px;margin-bottom:8px;">Insufficient cash reserves.</div>',y.innerHTML=`
    ...
    `,R.style.display="flex"}async function Le(e,t,n){if(z)return;z=!0;const a=document.getElementById("prop-refurb-confirm");a&&(a.disabled=!0,a.textContent="Starting...");try{const s=window._corpFactionId,r=window._currentTick;if(!s)throw new Error("No faction");const c=Math.floor(Math.random()*6)+1,o=94+(Math.floor(Math.random()*6)+1),d=r+c,{data:_}=await p.from("factions").select("corp_cash_reserves").eq("id",s).single(),u=Number(_?.corp_cash_reserves??0);if(u<t)throw new Error("Insufficient cash");const{error:b}=await p.from("factions").update({corp_cash_reserves:u-t}).eq("id",s);if(b)throw new Error("Failed to deduct cost: "+b.message);const{error:g}=await p.from("corp_properties").update({refurbish_until_tick:d,refurbish_condition:o}).eq("id",e).eq("faction_id",s);if(g)throw new Error("Failed to start refurbishment: "+g.message);j(),alert("Refurbishment started! Duration: "+c+" tick"+(c!==1?"s":"")+". Condition will be restored to "+Math.min(100,o)+"% when complete."),location.reload()}catch(s){alert("Refurbishment failed: "+s.message)}finally{z=!1,a&&(a.disabled=!1,a.textContent="Begin Refurbishment")}}function j(){const e=document.getElementById("prop-modal-overlay");e&&(e.style.display="none")}window.showSellModal=Se;window.confirmSellProperty=Te;window.showRefurbishModal=Ne;window.confirmRefurbish=Le;window.closePropModal=j;const ae={crisis:{icon:"⚠",color:"#e74c3c",bg:"rgba(231,76,60,0.12)",label:"CRISIS"},government:{icon:"⚖",color:"#f1c40f",bg:"rgba(241,196,15,0.10)",label:"GOV"},executive_order:{icon:"✍",color:"#e67e22",bg:"rgba(230,126,34,0.12)",label:"EXEC ORDER"},political:{icon:"★",color:"#3498db",bg:"rgba(52,152,219,0.12)",label:"POLITICAL"},trade:{icon:"⚓",color:"#2ecc71",bg:"rgba(46,204,113,0.12)",label:"TRADE"},diplomatic:{icon:"🌐",color:"#9b59b6",bg:"rgba(155,89,182,0.12)",label:"DIPLOMATIC"},bill:{icon:"📜",color:"#3498db",bg:"rgba(52,152,219,0.10)",label:"BILL"},new_party:{icon:"🏳",color:"#9b59b6",bg:"rgba(155,89,182,0.10)",label:"NEW PARTY"},protest:{icon:"✊",color:"#e67e22",bg:"rgba(230,126,34,0.10)",label:"PROTEST"},military:{icon:"⚔",color:"#b07a4a",bg:"rgba(176,122,74,0.12)",label:"MILITARY"},economy:{icon:"💰",color:"#7a9a5b",bg:"rgba(122,154,91,0.12)",label:"ECONOMY"},corporate:{icon:"🏢",color:"#c8a64e",bg:"rgba(200,166,78,0.10)",label:"CORP"}},oe={icon:"•",color:"#888",bg:"rgba(136,136,136,0.08)",label:""},Re={1:"Embarrassing Backfire",2:"Protests Don't Materialise",3:"Modest Turnout",4:"Respectable Protest",5:"Strong Demonstration",6:"Nationwide Protests",7:"The Big One"};let J="corporate",A=null;function Pe(e){return e?e.replace(/_/g," ").replace(/\b\w/g,t=>t.toUpperCase()):""}function G(e,t){if(!e)return"<em>Unknown</em>";const n=v(e);return t?`<span style="color:${t.replace(/[^a-zA-Z0-9#(),.\s%]/g,"")};font-weight:600">${n}</span>`:`<strong>${n}</strong>`}function Y(e,t,n){const a=e.factions?.nation_id===(e.nation_id||t),s=e.proposer_name||(a?e.factions?.faction_name:null)||"A former party",r=e.proposer_color||(a?e.factions?.party_color:null);return{fired_at_tick:e.proposed_tick,event_name:e.bill_name,_desc_html:`${G(s,r)} proposed "${v(e.bill_name)}"`,category:"bill",_synthetic:!0,...n}}function K(e,t){const n=e.leader_first_name&&e.leader_last_name?`${e.leader_first_name} ${e.leader_last_name}`:null,a=n?` led by <strong>${v(n)}</strong>`:"";return{fired_at_tick:0,event_name:e.faction_name,_desc_html:`${G(e.faction_name,e.party_color)} founded${a}`,category:"new_party",_synthetic:!0,_created_at:e.created_at,...t}}function Q(e,t){const n=Re[e.tier]||`Tier ${e.tier}`,a=e.demand_label?` demanding "${v(e.demand_label)}"`:"",s=e.status==="crisis_active",r=e.tier>=6?"#e74c3c":e.tier>=4?"#f39c12":"",c=r?`<span style="color:${r};font-weight:600">${v(n)}</span>`:`<strong>${v(n)}</strong>`;return{fired_at_tick:e.tick_resolved||e.tick_called,event_name:n,_desc_html:`${G(e.factions?.faction_name,e.factions?.party_color)} organised a protest${a} — ${c}${s?' <span style="color:#e74c3c;font-weight:700;">(CRISIS)</span>':""}`,category:"protest",_synthetic:!0,...t}}function X(e,t,n,a,s){return[...e.map(r=>({...r,_synthetic:!1})),...t,...n,...a].sort((r,c)=>{const i=(c.fired_at_tick||0)-(r.fired_at_tick||0);if(i!==0)return i;const o=r._created_at||r.created_at||"",d=c._created_at||c.created_at||"";return d>o?1:d<o?-1:0}).slice(0,s)}function ne(e){if(e._synthetic&&e._desc_html)return e._desc_html;const t=e.description_chosen||e.description_used||"",n=Pe(e.event_name),a=n?`<strong>${v(n)}</strong>`:"",s=t?v(t):"";return a&&s?`${a} — ${s}`:s||a||"Event"}function se(e){return e.map(t=>{const n=te(t.fired_at_tick),a=ae[(t.category||"").toLowerCase()]||oe;return`<div class="corp-ev-row">
            <span class="corp-ev-date">${v(n)}</span>
            <span class="corp-ev-icon" style="color:${a.color}">${a.icon}</span>
            <span class="corp-ev-text">${ne(t)}</span>
            ${a.label?`<span class="corp-ev-cat" style="color:${a.color};background:${a.bg}">${a.label}</span>`:""}
        </div>`}).join("")}const ee=[{color:"#6b8cae",bg:"rgba(107,140,174,0.10)",border:"rgba(107,140,174,0.3)"},{color:"#7a9a5b",bg:"rgba(122,154,91,0.10)",border:"rgba(122,154,91,0.3)"},{color:"#b07a4a",bg:"rgba(176,122,74,0.10)",border:"rgba(176,122,74,0.3)"},{color:"#8b7ec8",bg:"rgba(139,126,200,0.10)",border:"rgba(139,126,200,0.3)"},{color:"#5b9a8b",bg:"rgba(91,154,139,0.10)",border:"rgba(91,154,139,0.3)"},{color:"#a65d5d",bg:"rgba(166,93,93,0.10)",border:"rgba(166,93,93,0.3)"}];function Be(e){let t=0;for(let n=0;n<e.length;n++)t=(t<<5)-t+e.charCodeAt(n)|0;return ee[Math.abs(t)%ee.length]}function Ae(e){return e.map(t=>{const n=te(t.fired_at_tick),a=ae[(t.category||"").toLowerCase()]||oe,s=t.nations?.name||"Unknown",r=t.nations?.nation_profiles,c=Array.isArray(r)?r[0]?.flag_url:r?.flag_url,i=Be(s),o=c?`<img src="${v(c)}" alt="">`:"";return`<div class="corp-ev-row">
