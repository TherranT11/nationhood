import{_ as de}from"./preload-helper-BXl3LOEh.js";import{t as M}from"./utils-CY90Gazr.js";let g=null,h=null,O=[],C="all",A=null,D=null,R=null,S=null,F=!1,f="cruceran",Z=!1;const V={cruceran:{key:"cruceran",name:"The Cruceran",tagline:"Truth in the service of the people",nations:["Avelia","Palvera","San Estrella","Montequilla","Melizea","Sangreza","Sierramar"],style:"cruceran"},continental:{key:"continental",name:"The Continental",tagline:"Where Ideas Converge",nations:["Calveth","Flandis","Vostia"],style:"continental"},alsahwa:{key:"alsahwa",name:"Al-Sahwa",tagline:"Independent Voice of Al-Makir",nations:["Hajjara"],continent:"Al-Makir",style:"alsahwa"}};function pe(e){for(const[s,n]of Object.entries(V))if(n.nations.some(t=>t.toLowerCase()===(e||"").toLowerCase()))return s;return"cruceran"}function ne(e,s){const n=V[e];return n?n.nations.some(t=>t.toLowerCase()===(s||"").toLowerCase())?!0:L&&L.length>0?n.nations.some(t=>L.includes(t.toLowerCase())):!1:!1}let L=[];const ve=["Winter","Spring","Spring","Spring","Summer","Summer","Summer","Fall","Fall","Fall","Winter","Winter"];function x(e){const s=e%12,n=2e3+Math.floor(e/12),t=ve[s],i=s===0?n-1:n;return`${t} ${i}`}async function Y(e,s){g=e,h=s,A=null,C="all",R=null;const n=document.getElementById("newspaper-root");if(!n)return;if(L=[],s.faction?.faction_type==="corporation")try{const{data:w}=await e.from("corp_properties").select("nation_id, nations:nation_id(name)").eq("faction_id",s.faction.id).eq("is_active",!0);w&&(L=w.map(p=>(p.nations?.name||"").toLowerCase()).filter(Boolean))}catch{}Z||(f=pe(s.nation?.name));const t=s.shard?.current_date||"[Month], [Year]",i=ne(f,s.nation?.name),l="Write Article",a=Object.entries(V).map(([w,p])=>`<option value="${w}" ${w===f?"selected":""}>${p.name}</option>`).join("");n.innerHTML=`<div class="newspaper-container nws-pub-${f}">
        ...
        <div class="nws-main-content">
            <div class="nws-reader">
                <span class="nws-section-tag">${c(k(s.category))} &mdash; ${c(t)}</span>
    ...
    </div>`;const d=R,w=d?()=>oe(d.root,d.articles,d.dateLabel):()=>Y(g,h);document.getElementById("nws-back-btn")?.addEventListener("click",w),document.getElementById("nws-back-btn-bottom")?.addEventListener("click",w),be(s),e.scrollTop=0}async function be(e){const s=document.getElementById("nws-like-btn"),n=document.getElementById("nws-like-icon"),t=document.getElementById("nws-like-count");if(!s||!g||!h?.faction?.id)return;const i=h.faction.id;let l=!1;const{data:a}=await g.from("article_likes").select("id").eq("article_id",e.id).eq("faction_id",i).maybeSingle();a&&(s.classList.add("nws-like-btn--liked"),n.innerHTML="&#9829;"),s.addEventListener("click",async()=>{if(!l){l=!0,s.disabled=!0;try{const v=h.shard?.current_tick||0,{data:d,error:w}=await g.rpc("toggle_article_like",{p_article_id:e.id,p_faction_id:i,p_tick:v});if(w){console.error("[News] Like failed:",w.message);return}d.liked?(s.classList.add("nws-like-btn--liked"),n.innerHTML="&#9829;"):(s.classList.remove("nws-like-btn--liked"),n.innerHTML="&#9825;"),t.textContent=d.like_count,e.like_count=d.like_count}catch(v){console.error("[News] Like error:",v)}finally{l=!1,s.disabled=!1}}})}function c(e){const s=document.createElement("div");return s.textContent=e,s.innerHTML}function ie(e,s=1200){if(e.length<=s)return le(e);let n=e.lastIndexOf(" ",s);n<s*.5&&(n=s);const i=e.substring(0,n).split(/\n\n+/).filter(a=>a.trim());return i.map((a,v)=>`<p class="${v===0?"nws-drop-cap":""}">${W(c(a.trim()))}${v===i.length-1?"...":""}</p>`).join("")+'<p class="nws-read-more">Read More &rarr;</p>'}function W(e){return e.replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>").replace(/\*(.+?)\*/g,"<em>$1</em>").replace(/__(.+?)__/g,"<u>$1</u>")}function le(e){const s=e.split(/\n\n+/).filter(n=>n.trim());return s.length<=1?`<p class="nws-drop-cap">${W(c(e))}</p>`:s.map((n,t)=>`<p class="${t===0?"nws-drop-cap":""}">${W(c(n.trim()))}</p>`).join("")}function k(e){return e==="elections"&&(e="politics"),{politics:"Politics",economy:"Business",international:"International",social:"Social",entertainment:"Entertainment",sports:"Sports",opinion:"Opinion"}[e]||e}function ce(e){return e==="elections"&&(e="politics"),{politics:"linear-gradient(135deg,#2a1a2a,#1a0d1a)",economy:"linear-gradient(135deg,#2a1a0a,#1a0d00)",international:"linear-gradient(135deg,#0a1a2a,#001a2a)",social:"linear-gradient(135deg,#1a2a1a,#0d1a0d)",entertainment:"linear-gradient(135deg,#2a2a0a,#1a1a00)",sports:"linear-gradient(135deg,#2a0a0a,#1a0000)",opinion:"linear-gradient(135deg,#2a2a2a,#1a1a1a)"}[e]||"linear-gradient(135deg,#1a1a1a,#0d0d0d)"}async function z(){if(A)return A;const e=h?.shard,s=h?.nation;if(!e||!s)return[s?.id].filter(Boolean);const{data:n,error:t}=await g.from("nations").select("id").eq("shard_id",e.id);return t&&console.error("[News] Failed to fetch shard nations:",t),A=n&&n.length>0?n.map(i=>i.id):[s.id],A}async function q(){if(!(!g||!h))try{let e=await z();if(f==="alsahwa"){const{data:u}=await g.from("nations").select("id").eq("continent","Al-Makir");u&&u.length>0&&(e=u.map($=>$.id))}let s=g.from("player_articles").select("*").in("nation_id",e).eq("status","published").order("created_at",{ascending:!1});f==="alsahwa"?s=s.or("publication.eq.alsahwa,publication.eq.international"):f!=="cruceran"?s=s.or(`publication.eq.${f},publication.eq.international`):s=s.or("publication.eq.cruceran,publication.is.null,publication.eq.international");const{data:n,error:t}=await s;if(t){console.error("[News] Failed to load articles:",t);return}if(!n||n.length===0)return;const i=h.shard?.current_tick??0,l=x(i),a=n.filter(u=>x(u.published_tick??0)===l),v=n.filter(u=>x(u.published_tick??0)!==l),d=new Set(a.map(u=>u.category)),w=[],p=new Set;for(const u of v)u.category!=="opinion"&&(d.has(u.category)||(p.has(u.category)||p.add(u.category),w.push(u)));const m=n.filter(u=>u.category==="opinion").slice(0,4),o=[...a.filter(u=>u.category!=="opinion"),...w,...m];if(o.length===0)return;O=o;const r=C&&C!=="all"?o.filter(u=>(u.category==="elections"?"politics":u.category)===C):o;if(r.length===0&&C!=="all"){const u=`<p class="nws-placeholder" style="text-align:center;padding:40px;grid-column:1/-1;">No ${k(C)} articles in this edition.</p>`,$=document.getElementById("nws-lead-section");$&&($.innerHTML=u);const U=document.getElementById("nws-secondary-grid");U&&(U.innerHTML="");const K=document.querySelector(".nws-opinion-grid");K&&(K.innerHTML="");const J=document.querySelector(".nws-bottom-left");J&&(J.innerHTML="");return}const b=C==="opinion",_=[...b?r:r.filter(u=>u.category!=="opinion")].sort((u,$)=>($.published_tick??0)-(u.published_tick??0)),E=_[0],P=_.slice(1,4),T=_.slice(4,7),I=[...o].sort((u,$)=>new Date($.created_at)-new Date(u.created_at)).slice(0,5);f==="continental"?$e(E,P,T,m.slice(0,4),I):(E&&fe(E,P),ye(T),ee(b?[]:m.slice(0,4)),_e(I))}catch(e){console.error("[News] Error loading articles:",e)}}function fe(e,s){const n=document.getElementById("nws-lead-section");if(!n||!e)return;const t=s.length>0?s.map(d=>`
            <div class="nws-sidebar-story" data-article-id="${d.id}">
                ${N(d)}${H(d)}
                <span class="nws-section-tag">${c(k(d.category))}</span>
                ...
                <div class="nws-byline"><span class="nws-author">${c(d.author_name)}</span><span class="nws-dot">&middot;</span><span>${d.published_tick!=null?M(d.published_tick):h?.shard?.current_date||"—"}</span></div>
            </div>
        ...
        <div class="nws-lead-main" data-article-id="${e.id}">
            ${N(e)}${H(e)}
            <span class="nws-section-tag">${c(k(e.category))}</span>
            <h2 class="nws-lead-headline">${c(e.headline)}</h2>
            <p class="nws-lead-deck">${c(v)}</p>
            <div class="nws-byline">
    ...
    `}function ye(e){const s=document.getElementById("nws-secondary-grid");if(!s||e.length===0)return;const n=[...e],t=["Crisis","Election","Business"],i=[0,1,2].map(l=>{const a=n[l];if(a){const v=a.image_url?`<img src="${c(a.image_url)}" alt="${c(a.headline)}" style="width:100%;height:100%;object-fit:cover;">`:`<div class="nws-img-ph" style="background:${ce(a.category)};">${c(k(a.category))}</div>`;return`<div class="nws-sec-story" data-article-id="${a.id}">
                ${N(a)}${H(a)}
                <div class="nws-sec-image">${v}</div>
                <span class="nws-section-tag">${c(k(a.category))}</span>
                <h3 class="nws-sec-headline">${c(a.headline)}</h3>
                <p class="nws-sec-deck">${c((a.body||"").replace(/\n+/g," ").substring(0,150))}${(a.body||"").length>150?"...":""}</p>
                <div class="nws-byline"><span class="nws-author">${c(a.author_name)}</span><span class="nws-dot">&middot;</span><span>${a.published_tick!=null?M(a.published_tick):h?.shard?.current_date||"—"}</span></div>
        ...
        <div class="nws-brief-row" data-article-id="${a.id}">
            ...
            <div class="nws-brief-text">
                <strong>${c(a.headline)}${N(a)}${H(a)}</strong>
    ...
    `).join(""),i=5-e.length;let l="";for(let a=0;a<i;a++)l+=`
        ...
        `;s.innerHTML=n+t+l}function ee(e){const s=document.querySelector(".nws-opinion-grid");if(!s||e.length===0)return;const n=[0,1,2,3].map(t=>{const i=e[t];if(i){const l=(i.body||"").length>80?(i.body||"").substring(0,80)+"...":i.body||"";return`<div class="nws-op-card" data-article-id="${i.id}">
        ...
        </div>`,o+='<div class="ct-story-grid">';for(const r of s){const b=m[r.category]||"",y=r.image_url?`<img src="${a(r.image_url)}" alt="" style="width:100%;height:100%;object-fit:cover;">`:"";o+=`<div class="ct-card" data-article-id="${r.id}">
            ...
            <div class="ct-section-divider__line"></div>
        </div>`,o+='<div class="ct-two-col">',o+='<div class="ct-analysis-list">';for(let r=0;r<t.length;r++){const b=t[r];o+=`<div class="ct-analysis-item" data-article-id="${b.id}">
    ...
    </div>`,l.innerHTML=o,l.querySelectorAll("[data-article-id]").forEach(r=>{r.style.cursor="pointer",r.addEventListener("click",b=>{if(b.target.closest(".nws-edit-btn, .nws-delete-btn"))return;const y=r.dataset.articleId,_=O.find(E=>String(E.id)===String(y));_&&te(document.getElementById("newspaper-root"),_)})})}function ke(e){const s=e.querySelectorAll(".nws-nav-item[data-category]");s.forEach(t=>{t.addEventListener("click",()=>{s.forEach(i=>i.classList.remove("active")),t.classList.add("active"),C=t.dataset.category,q()})});const n=e.querySelectorAll(".nws-alsahwa-nav-item[data-cat]");n.forEach(t=>{t.addEventListener("click",()=>{n.forEach(i=>i.classList.remove("nws-alsahwa-nav-item--active")),t.classList.add("nws-alsahwa-nav-item--active"),C=t.dataset.cat,q()})})}function Ee(e){const s=document.getElementById("nws-nav-archives");s&&s.addEventListener("click",()=>re(e))}async function re(e){if(!(!g||!h))try{const s=h.shard?.current_tick??0,n=h.shard?.current_date||"[Month], [Year]",t=await z(),{data:i,error:l}=await g.from("player_articles").select("*").in("nation_id",t).eq("status","published").order("published_tick",{ascending:!1});if(l){console.error("[News] Failed to load archive articles:",l);return}const a={};for(const p of i||[]){const m=p.published_tick??0,o=x(m);a[o]||(a[o]={label:o,maxTick:m,articles:[]}),a[o].articles.push(p),m>a[o].maxTick&&(a[o].maxTick=m)}const v=Object.values(a).sort((p,m)=>m.maxTick-p.maxTick),d=x(s),w=v.length>0?v.map(p=>{const m=p.label===d?' <span class="nws-archive-current">Current</span>':"";return`<div class="nws-archive-month" data-archive-season="${c(p.label)}">
                    <div class="nws-archive-month-name">${c(p.label)}${m}</div>
        ...
        </div>`,document.getElementById("nws-back-btn")?.addEventListener("click",()=>Y(g,h)),e.querySelectorAll("[data-archive-season]").forEach(p=>{p.addEventListener("click",()=>{const m=p.dataset.archiveSeason,o=a[m];o&&oe(e,o.articles,o.label)})}),e.scrollTop=0}catch(s){console.error("[News] Error loading archives:",s)}}function oe(e,s,n){O=s;const t=s.filter(y=>y.category==="opinion"),l=[...s.filter(y=>y.category!=="opinion")].sort((y,_)=>(_.body||"").length-(y.body||"").length),a=l[0],v=l.slice(1,4),d=l.slice(4,7),w=l.slice(7,12),p=t.slice(0,4),m=a?Ie(a,v,n):'<p class="nws-placeholder" style="padding:40px;text-align:center;">[No articles in this edition]</p>',o=Be(d),r=Ce(p),b=Te(w);e.innerHTML=`<div class="newspaper-container">
        <!-- TOP RIBBON -->
        <div class="nws-top-ribbon">
            <div class="nws-top-ribbon-inner">
                <span>${c(n)}</span>
    ...
    </div>`,document.getElementById("nws-back-to-archives")?.addEventListener("click",()=>re(e)),R={root:e,articles:s,dateLabel:n},ae(e),e.scrollTop=0}function Ie(e,s,n){const t=s.length>0?s.map(d=>`
            <div class="nws-sidebar-story" data-article-id="${d.id}">
                <span class="nws-section-tag">${c(k(d.category))}</span>
                <h3 class="nws-sidebar-headline">${c(d.headline)}</h3>
                <p class="nws-sidebar-deck">${c((d.body||"").replace(/\n+/g," ").substring(0,120))}${(d.body||"").length>120?"...":""}</p>
    ...
    `}function Be(e){return e.length===0?"":e.map(s=>{const n=s.image_url?`<img src="${c(s.image_url)}" alt="${c(s.headline)}" style="width:100%;height:100%;object-fit:cover;">`:`<div class="nws-img-ph" style="background:${ce(s.category)};">${c(k(s.category))}</div>`;return`<div class="nws-sec-story" data-article-id="${s.id}">
            <div class="nws-sec-image">${n}</div>
            <span class="nws-section-tag">${c(k(s.category))}</span>
    ...
    `).join("")}function G(e){return e.w*3+e.d}function Ae(e){return Object.values(e).sort((s,n)=>{const t=G(n)-G(s);return t!==0?t:n.w-s.w})}function Se(e){return`<span class="vln-form-dot" style="background:${{W:"#1a4a1a",D:"#8a6a20",L:"#8b1a1a"}[e]||"#999"}"></span>`}function Le(e,s){return e<=3?"vln-pos-top":e>=s-2?"vln-pos-bottom":""}function se(e,s={}){const n=Ae(e),t=s.muted||!1,i=n.length;let l=`<div class="vln-table${t?" vln-table-muted":""}">
        <div class="vln-thead">
        ...
        </div>`;return n.forEach((a,v)=>{const d=v+1,w=Le(d,i),p=d===3||d===7?" vln-zone-break":"",m=G(a),o=t?"":a.form.map(r=>Se(r)).join("");l+=`<div class="vln-row${p}">
