import{_supabase as f}from"./supabase-client-CiYoFhIh.js";/* empty css                  */import{i as K}from"./common-DtfiRwOl.js";import{e as y}from"./utils-CY90Gazr.js";import{deductAP as H}from"./config-fKhFNVuq.js";import{M as P,N as L,O,Q as I,R as j,S as z,T as V,U as A,j as E}from"./elections-B2jRdA_W.js";import{j as D}from"./government-types-CONVKpUN.js";import"./preload-helper-BXl3LOEh.js";import"./ideology-BIAflN4K.js";import"./stats-tIiBSaQA.js";let p=null,i=null,S=null,h={leader:[]},v={leader:null},C={leader:null},w=!1,$=null;K("politics",async t=>{const{nation:e,faction:a,shard:s}=t;if(p=e,i=a,S=s,!e||!a){document.getElementById("pl-container").innerHTML='<div style="padding:40px;text-align:center;color:var(--text-dim);font-family:var(--font-mono);font-size:10px;">No nation or party found.</div>';return}await W(),await Y(),x()});async function W(){if(i.leader_first_name&&i.leader_last_name&&(v.leader={firstName:i.leader_first_name,lastName:i.leader_last_name,age:i.leader_age||null,electability:i.electability??50,ideology:i.leader_ideology||null,positiveTraits:i.leader_positive_traits||[],negativeTraits:i.leader_negative_traits||[]},(!i.leader_positive_traits||i.leader_positive_traits.length===0)&&(!i.leader_negative_traits||i.leader_negative_traits.length===0))){const t=P(p?.name||"",E,1,"leader");if(t.length>0){const e=t[0];v.leader.positiveTraits=e.positiveTraits||[],v.leader.negativeTraits=e.negativeTraits||[],await f.from("factions").update({leader_positive_traits:v.leader.positiveTraits,leader_negative_traits:v.leader.negativeTraits}).eq("id",i.id),i.leader_positive_traits=v.leader.positiveTraits,i.leader_negative_traits=v.leader.negativeTraits,console.log("[PartyLeadership] Auto-backfilled traits for current leader:",v.leader.positiveTraits,v.leader.negativeTraits)}}}function B(){const t=v.leader;return t?{firstName:t.firstName,lastName:t.lastName,age:t.age||45,ideology:t.ideology||i.ideology_value_1||"PROGRESS",positiveTraits:t.positiveTraits||[],negativeTraits:t.negativeTraits||[],apCost:0,costBreakdown:{positiveTotal:0,negativeTotal:0,rawCost:0,apCost:0},isCurrent:!0,electability:t.electability??50}:null}async function G(t,e,a){const{error:s}=await f.from("leadership_candidates").upsert({faction_id:t,role:e,candidates:a,generated_at:new Date().toISOString()},{onConflict:"faction_id,role"});s&&console.warn(`Failed to persist ${e} candidates (non-blocking):`,s.message)}async function Y(){const t=p.name||"",e=i.id;let a=null;const s=`leadership_candidates_${e}_leader`,{data:d,error:n}=await f.from("leadership_candidates").select("candidates, last_refresh_tick").eq("faction_id",e).eq("role","leader").maybeSingle();if(!n&&d&&Array.isArray(d.candidates)&&d.candidates.length>0){a=d.candidates,$=d.last_refresh_tick;try{sessionStorage.setItem(s,JSON.stringify(a))}catch{}}else{try{const c=sessionStorage.getItem(s);if(c){const g=JSON.parse(c);Array.isArray(g)&&g.length>0&&(a=g)}}catch{}if(!a){a=P(t,E,3,"leader"),await G(e,"leader",a);try{sessionStorage.setItem(s,JSON.stringify(a))}catch{}}}h.leader=a;const o=B();o&&h.leader.unshift(o),C.leader=null}function x(){const t=document.getElementById("pl-container"),e=h.leader,a=C.leader,s=a!==null?e[a]:null;let d="",n="";if(s&&!s.isCurrent){const o=L(s.apCost),c=O(s.apCost);d=`<span class="pl-header-cost" style="color:${o}">${c}</span>`,n='<button class="pl-appoint-btn" onclick="window._appointLeader()">Appoint Party Leader</button>'}t.innerHTML=`
    ...
    `}function Q(t,e){return!t||t.length===0?'<div style="padding:20px;text-align:center;color:var(--text-dim);font-family:var(--font-mono);font-size:9px;">No candidates available.</div>':t.map((a,s)=>{const d=s===e,n=L(a.apCost),o=O(a.apCost),c=a.ideology||"",g=j[c]||"#888",_=c?c.charAt(0)+c.slice(1).toLowerCase():"Unknown",m=a.electability!=null?I(a.electability):null,u=a.isCurrent?'<span class="pl-current-pill">CURRENT</span>':"";return`
...
Cost: 5 AP`))return;w=!0;const e=document.querySelector(".pl-refresh-btn");e&&(e.disabled=!0,e.textContent="Refreshing...");try{const a=await H(f,i.id,5);if(!a.success){alert("Not enough AP. Need 5 AP, have "+(a.currentAp??"?")+".");return}i.action_points=a.newAp;const s=p.name||"",d=P(s,E,3,"leader"),{error:n}=await f.from("leadership_candidates").upsert({faction_id:i.id,role:"leader",candidates:d,generated_at:new Date().toISOString(),last_refresh_tick:t},{onConflict:"faction_id,role"});if(n){console.error("Failed to persist refreshed candidates:",n.message),alert("Error saving new candidates: "+n.message);return}$=t,h.leader=d;try{sessionStorage.setItem(`leadership_candidates_${i.id}_leader`,JSON.stringify(d))}catch{}const o=B();o&&h.leader.unshift(o),C.leader=null,x()}catch(a){alert("Error: "+a.message)}finally{w=!1,e&&e.isConnected&&(e.disabled=!1,e.textContent="Refresh Leaders")}};window._selectCandidate=function(t){C.leader=t,x()};window._appointLeader=async function(){if(w)return;const t=C.leader;if(t===null)return;const e=h.leader[t];if(!e)return;if(e.isCurrent){!(i.leader_positive_traits&&i.leader_positive_traits.length>0)&&e.positiveTraits&&e.positiveTraits.length>0&&(await f.from("factions").update({leader_positive_traits:e.positiveTraits||[],leader_negative_traits:e.negativeTraits||[]}).eq("id",i.id),i.leader_positive_traits=e.positiveTraits,i.leader_negative_traits=e.negativeTraits,alert("Leader traits synced. They will now apply to campaign actions."));return}const a=[`Appoint ${e.firstName} ${e.lastName} as Party Leader?
`,`Cost: ${O(e.apCost)}`,`Ideology: ${e.ideology}`];e.electability!=null&&a.push(`Electability: ${I(e.electability).label}`),a.push("","This will replace the current party leader.");const s=p?.ruling_faction_id===i?.id,d=D(p);if(s&&d&&a.push("As the ruling party leader, they will also become Prime Minister."),!confirm(a.join(`
`)))return;w=!0;const n=document.querySelector(".pl-appoint-btn");n&&(n.disabled=!0,n.textContent="Appointing...");try{const o=S?.current_tick||0;if(e.apCost>0){const l=await H(f,i.id,e.apCost);if(!l.success){alert("Not enough AP. Need "+e.apCost+" AP.");return}i.action_points=l.newAp}const c={leader_first_name:e.firstName,leader_last_name:e.lastName,leader_age:e.age,electability:e.electability,leader_ideology:e.ideology||null,leader_positive_traits:e.positiveTraits||[],leader_negative_traits:e.negativeTraits||[]},{error:g}=await f.from("factions").update(c).eq("id",i.id);if(g)throw new Error("Failed to update faction: "+g.message);const _=p.ruling_faction_id===i.id,m=D(p);if(_&&m){const l=e.positiveTraits&&e.positiveTraits.length>0?e.positiveTraits[0]:null,F=e.age||35+Math.floor(Math.random()*16),R=`${e.firstName} ${e.lastName}`,{error:M}=await f.from("head_of_government").update({first_name:e.firstName,last_name:e.lastName,age:F,trait_key:l,appointed_tick:o}).eq("nation_id",p.id).eq("active",!0);M&&console.warn("Failed to update head_of_government (non-blocking):",M.message);const{error:q}=await f.from("ministries").update({minister_first_name:e.firstName,minister_last_name:e.lastName,minister_age:F}).eq("nation_id",p.id).eq("ministry_key","prime_minister").eq("is_active",!0);q&&console.warn("Failed to update PM ministry (non-blocking):",q.message);const{error:U}=await f.from("administrations").update({prime_minister:R,admin_name:`${e.lastName} Administration`,updated_at:new Date().toISOString()}).eq("nation_id",p.id).is("ended_at_tick",null);U&&console.warn("Failed to update administration (non-blocking):",U.message);try{await f.rpc("fire_system_event",{p_trigger_key:"pm_appointed",p_nation_id:p.id,p_tick:o,p_placeholders:{nation:p.name||"",pm_name:R,party:i.faction_name,trait:l}})}catch(J){console.warn("PM appointed event fire failed (non-blocking):",J)}console.log(`Ruling party leader change → PM updated: ${R} (trait: ${l})`)}const u=v.leader,b=u?`${e.firstName} ${e.lastName} replaces ${u.firstName} ${u.lastName} as Party Leader of ${i.faction_name}.`:`${e.firstName} ${e.lastName} appointed as Party Leader of ${i.faction_name}.`;await f.from("event_log").insert({nation_id:p.id,event_name:"New Party Leader",description_chosen:b,category:"POLITICAL",fired_at_tick:o}).then(({error:l})=>{l&&console.warn("Event log insert failed (non-blocking):",l.message)});const N={firstName:e.firstName,lastName:e.lastName,age:e.age,ideology:e.ideology,positiveTraits:e.positiveTraits,negativeTraits:e.negativeTraits};e.electability!=null&&(N.electability=e.electability),v.leader=N;const k=p.name||"",T=P(k,E,3,"leader");await G(i.id,"leader",T);try{sessionStorage.setItem(`leadership_candidates_${i.id}_leader`,JSON.stringify(T))}catch{}h.leader=T;const r=B();r&&h.leader.unshift(r),C.leader=0,x(),alert(`${e.firstName} ${e.lastName} has been appointed as Party Leader!`)}catch(o){alert("Error: "+o.message),n&&(n.disabled=!1,n.textContent="Appoint Party Leader")}finally{w=!1}};
