...
`;function Be(){if(document.getElementById(te))return;const e=document.createElement("style");e.id=te,e.textContent=Ee,document.head.appendChild(e)}function B(e){const t=Number(e)||0;return t>=1e9?"$"+(t/1e9).toFixed(2)+"B":t>=1e6?"$"+(t/1e6).toFixed(1)+"M":t>=1e3?"$"+(t/1e3).toFixed(1)+"k":"$"+Math.round(t).toLocaleString()}function A(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function ae(e){return Math.max(0,Math.min(100,Number(e)||0))}function Se(e){const t=ae(e);return Math.max(0,Math.min(100,25+t))}async function Ne(e,t){const{data:o,error:n}=await e.from("corp_tax_bills").select("id, nation_id, year, revenue_taxed, rate_pct, amount_due, status, due_at_tick, nations:nation_id(name, corruption)").eq("corp_id",t).in("status",["due","partial","delinquent"]).is("ignored_at_tick",null).order("due_at_tick",{ascending:!0});return n?(console.warn("[corp-tax-pressing] fetch failed:",n.message),[]):o||[]}function Ie(e,t){const o=e.nations?.name||"Unknown nation",n=e.status,a=n==="delinquent"?"kind-delinquent":n==="partial"?"kind-partial":"kind-due",s=n==="delinquent"?"Delinquent":n==="partial"?"Partial":"Tax Due",i=t>Number(e.due_at_tick)&&n==="due",g=`Due by ${Y(e.due_at_tick)}`,m=n==="due"?`<button class="cti-btn" data-action="cook" data-id="${A(e.id)}">Cook the Books</button>`:"";return`<div class="cti-card ${a}">
        <div class="cti-meta-row">
            <span class="cti-tag ${a}">${s}</span>
            <span class="cti-deadline${i?" warn":""}">${A(g)}</span>
        </div>
        <div class="cti-name">${A(o)} Corporate Tax — ${e.year}</div>
        <div class="cti-stats">
...
-1 corporate reputation. Bill stays in your records and is hidden from this list until next year's assessment.`))return;const{data:c,error:f}=await e.rpc("ignore_corporate_tax_bill",{p_bill_id:u});if(f){alert(`Ignore failed: ${f.message}`);return}if(!c?.success){alert(`Ignore failed: ${c?.reason||"unknown"}`);return}window.dispatchEvent(new CustomEvent("corp-tax:changed",{detail:{action:l,billId:u,result:c}})),await m();return}}}return o.addEventListener("click",async u=>{const l=u.target.closest("button[data-action][data-id]");if(l&&!l.disabled){l.disabled=!0;try{await v(l.dataset.id,l.dataset.action)}catch(p){console.error("[corp-tax-pressing] action threw:",p?.message||p),alert("Action failed — check console.")}finally{l.disabled=!1}}}),m(),{refresh:m,getCount:()=>i.length}}function Ae(e){const t=Number(e)||0;return t>=7?"good":t>=4?"mid":"warn"}const Me=[{key:"crews",column:"corp_work_crews",eyebrow:"CAPACITY",name:"Work",emName:"Crews",tooltip:"Your pool of skilled labor — engineers, foremen, equipment operators. Caps how many construction projects you can bid on at once and drives bid-prep speed. Strong crews mean faster turnarounds and more bids in flight; weak crews stall the pipeline.",blurb:{good:"<strong>Strong skilled labor pool.</strong> Crews fully staffed; apprenticeship pipeline producing qualified hires.",mid:"<strong>Adequate crew strength.</strong> Operating at workable capacity, room to scale before next bid window.",warn:"<strong>Crew shortages.</strong> Active bids limited; recruitment overdue and turnover rising."},impacts:{good:[{label:"Max Active Projects",value:"5+"},{label:"Avg. Bid Speed",value:"+18%",tone:"good"}],mid:[{label:"Max Active Projects",value:"3"},{label:"Avg. Bid Speed",value:"Average"}],warn:[{label:"Max Active Projects",value:"1–2"},{label:"Avg. Bid Speed",value:"−12%",tone:"warn"}]}},{key:"regulatory",column:"corp_regulatory_standing",eyebrow:"RISK",name:"Regulatory",emName:"Standing",tooltip:"Your relationship with the host nation's ministries and inspectors. High standing = permits clear quickly, audits stay light, government contracts flow your way. Low standing = approvals stall, audit risk climbs, sanctioned projects slip away to rivals.",blurb:{good:"<strong>Strong ministry rapport.</strong> Permits clear quickly; audit risk minimal; the regime takes your calls.",mid:"<strong>Average standing.</strong> No major scandals on the books; permits move at the standard pace.",warn:"<strong>Recent incidents have soured ministry relations.</strong> Permits stall; audit risk elevated; favors are no longer free."},impacts:{good:[{label:"Permit Speed",value:"+22%",tone:"good"},{label:"Audit Risk",value:"Low",tone:"good"}],mid:[{label:"Permit Speed",value:"Average"},{label:"Audit Risk",value:"Standard"}],warn:[{label:"Permit Speed",value:"−15%",tone:"warn"},{label:"Audit Risk",value:"Elevated",tone:"warn"}]}},{key:"supply",column:"corp_supply_chain",eyebrow:"REACH",name:"Material",emName:"Supply Chain",tooltip:"How diversified your sourcing is — concrete, steel, fixtures, fuel. Strong chains keep reserves deep and shrug off single-supplier disruptions or sanctions. Weak chains run hand-to-mouth: one shock and active projects slip into delay penalties.",blurb:{good:"<strong>Diversified sourcing across domestic and overseas suppliers.</strong> Reserves healthy; sanctions exposure manageable.",mid:"<strong>Mixed sourcing.</strong> Reserves stable but vulnerable to single-source disruption; suppliers cordial, not loyal.",warn:"<strong>Single-source dependence.</strong> Stock running low; one disruption away from project delays."},impacts:{good:[{label:"Material Reserves",value:"60+ days",tone:"good"},{label:"Sanctions Exposure",value:"Low",tone:"good"}],mid:[{label:"Material Reserves",value:"30–45 days"},{label:"Sanctions Exposure",value:"Moderate",tone:"rust"}],warn:[{label:"Material Reserves",value:"<14 days",tone:"warn"},{label:"Sanctions Exposure",value:"High",tone:"warn"}]}}];function se(){const e=document.getElementById("co-hero-stats");e&&(e.innerHTML=Me.map(qe).join(""))}function qe(e){const t=Number(d[e.column])||0,o=Ae(t),n=Number.isInteger(t)?String(t):t.toFixed(1),a='<div class="co-hero-stat-trend">— Latest</div>',s=Le(t,o),i=e.blurb[o]||"",m=(e.impacts[o]||[]).map(u=>`<div>
        ...
        <div class="co-hero-stat-value-row">
            <div class="co-hero-stat-value">${r(n)}<span class="co-max">/10</span></div>
            ${a}
        </div>
        ${s}
        <div class="co-hero-stat-desc">${i}</div>
        <div class="co-hero-stat-impact">${m}</div>
    </div>`}function Le(e,t){const o=Math.max(0,Math.min(10,Math.round(Number(e)||0))),n=t==="good"?"filled good":t==="warn"?"filled warn":"filled",a=[];for(let s=0;s<10;s++)a.push(`<div class="co-hero-meter-cell${s<o?" "+n:""}"></div>`);return`<div class="co-hero-meter">${a.join("")}</div>`}let q=[],ie=null,re=null,ce=null;async function le(){if(!d?.id)return;const{data:e,error:t}=await b.from("corp_contract_events").select("id, contract_id, type, severity, title, description, impact, responses, expires_at_tick, corp_contracts:contract_id(name)").eq("faction_id",d.id).eq("status","ACTIVE").order("expires_at_tick",{ascending:!0});t?(console.warn("[corp-operations] Pressing Issues fetch failed:",t.message),q=[]):q=e||[],R()}function Re(e){const t=String(e||"").toUpperCase();return t==="CRITICAL"?"critical":t==="HIGH"?"high":t==="MODERATE"?"moderate":"low"}function Pe(e,t){const o=Math.max(0,Number(e||0)-(Number(t)||0));return o<=0?"Resolving now":`Expires in ${o} tick${o===1?"":"s"}`}function R(){const e=document.getElementById("co-issues-list"),t=document.getElementById("co-issues-meta");if(!e||!t)return;const o=ie?.getCount?.()??0,n=re?.getCount?.()??0,a=ce?.getCount?.()??0,s=d?.displaced_from_nation_id?1:0,i=q.length+a+o+n+s;t.textContent=i===0?"Time-Sensitive ◊ Decide Before Tick Resolves":`${i} OPEN ◊ DECIDE BEFORE TICK RESOLVES`;const g=!!d?.displaced_from_nation_id;if(q.length===0&&!g){e.innerHTML=o===0&&n===0&&a===0?'<div class="co-contract-empty">No pressing issues right now. Time-sensitive decisions will appear here when triggered.</div>':"";return}const m=Number(x?.current_tick)||0,v=d?.displaced_from_nation_id?`
        ...
        </div>`,document.body.appendChild(a),a.querySelector("#co-relocate-x")?.addEventListener("click",V),a.querySelectorAll(".co-relocate-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.borderColor="var(--co-accent-gold,#c8a832)",i.style.background="#1f1f1a"}),i.addEventListener("mouseleave",()=>{i.style.borderColor="rgba(255,255,255,0.1)",i.style.background="#1a1a17"}),i.addEventListener("click",()=>He(i.dataset.nationId))})}function V(){document.getElementById("co-relocate-overlay")?.remove()}async function He(e){if(P||!e||!confirm("Relocate HQ to this nation? The $5M fee is deducted now (or deferred as debt if cash is short). All properties in the old nation are dissolved and open bids withdrawn. This cannot be undone."))return;P=!0;const t=document.getElementById("co-relocate-error");t&&(t.style.display="none",t.textContent="");try{const{data:o,error:n}=await b.rpc("relocate_corp_hq",{p_new_nation_id:e});if(n){t&&(t.textContent=n.message,t.style.display="block");return}if(!o?.success){t&&(t.textContent=`Relocate failed: ${o?.reason||"unknown"}`,t.style.display="block");return}V(),location.reload()}catch(o){t&&(t.textContent=o?.message||String(o),t.style.display="block")}finally{P=!1}}async function je(e){if(!O.has(e)){O.add(e);try{const{data:t,error:o}=await b.rpc("acknowledge_corp_contract_event",{p_event_id:e,p_response_key:null});if(o)return _("Failed: "+o.message,"error");if(t&&t.success===!1)return _(t.error||"Failed to acknowledge","error");const n=t?.cost_applied?` − ${C(t.cost_applied)}`:"",a=t?.delay_applied?` ◊ +${t.delay_applied}t delay`:"";_(`Acknowledged${n}${a}`,"success"),await J(),await le()}catch(t){console.error("[corp-operations] acknowledge failed:",t),_("Acknowledge failed: "+(t?.message||"unknown"),"error")}finally{O.delete(e)}}}let I=[],S=[],T=new Map,D=!1,W=!1;function C(e){const t=Number(e)||0,o=t<0?"-":"",n=Math.abs(t);return n>=1e9?o+"$"+(n/1e9).toFixed(2)+"B":n>=1e6?o+"$"+(n/1e6).toFixed(1)+"M":n>=1e3?o+"$"+(n/1e3).toFixed(1)+"k":o+"$"+Math.round(n).toLocaleString()}const oe=["id","contract_number","name","description","contract_type","issuer_name","issuer_nation_id","budget","timeline_months","status","requirements","expires_at_tick","deadline_tick","expected_finish_tick","started_at_tick","progress_pct","amount_spent","crews_working","winner_faction_id"].join(", ")+", nations:issuer_nation_id(name)";async function F(){const{data:e}=await b.from("corp_properties").select("nation_id").eq("faction_id",d.id).eq("role","regional_hq").eq("is_active",!0),t=Array.from(new Set([d.nation_id,...(e||[]).map(s=>s.nation_id).filter(Boolean)])),[o,n]=await Promise.all([b.from("corp_contracts").select(oe).eq("required_sector","Construction").eq("status","open").in("issuer_nation_id",t).order("budget",{ascending:!1}).limit(40),b.from("corp_contracts").select(oe).eq("winner_faction_id",d.id).in("status",["awarded","active"]).order("expected_finish_tick",{ascending:!0})]);o.error?(console.error("[corp-operations] available contracts error:",o.error.message),I=[]):I=o.data||[],n.error?(console.error("[corp-operations] active contracts error:",n.error.message),S=[]):S=n.data||[],T=new Map;const a=[...I.map(s=>s.id),...S.map(s=>s.id)];if(a.length>0){const{data:s,error:i}=await b.from("corp_contract_bids").select("contract_id, bid_amount, crews_committed, markup_pct, quoted_timeline_months, status").eq("faction_id",d.id).in("contract_id",a);if(i)console.warn("[corp-operations] my bids fetch failed:",i.message);else for(const g of s||[])(!T.get(g.contract_id)||g.status==="accepted")&&T.set(g.contract_id,g)}de(),Oe()}function de(){const e=document.getElementById("co-available-list"),t=document.getElementById("co-available-meta");if(!e||!t)return;const o=fe(),n=I.filter(a=>!o.has(a.id));if(n.length===0){t.textContent="None open",e.innerHTML='<div class="co-contract-empty">No open contracts at the moment. Check back next tick.</div>';return}t.textContent=`${n.length} Open Bid${n.length===1?"":"s"}`,e.innerHTML=n.map(We).join("")}function Oe(){const e=document.getElementById("co-active-list"),t=document.getElementById("co-active-meta");if(!e||!t)return;if(S.length===0){t.textContent="None active",e.innerHTML='<div class="co-contract-empty">No active contracts. Win a bid to get going.</div>';return}const o=S.reduce((n,a)=>n+(Number(a.budget)||0),0);t.textContent=`${S.length} In Progress ◊ Total Value ${C(o)}`,e.innerHTML=S.map(Ue).join("")}function ue(e){const t=String(e||"").toLowerCase();return t==="private"?"private":t==="foreign"?"foreign":"gov"}function pe(e){switch(e){case"work_crews":return"Work Crews";case"regulatory_standing":return"Reg. Standing";case"supply_chain":return"Supply Chain";default:return e}}function De(e){if(!d)return null;switch(e){case"work_crews":return Number(d.corp_work_crews??0);case"regulatory_standing":return Number(d.corp_regulatory_standing??0);case"supply_chain":return Number(d.corp_supply_chain??0);default:return null}}function We(e){const t=ue(e.contract_type),o=String(e.contract_type||"GOVERNMENT"),n=Number(x?.current_tick)||0,a=e.expires_at_tick!=null?Math.max(0,e.expires_at_tick-n):null,s=a==null?"No deadline":a<=0?"Closing now":`Closes in ${a} tick${a===1?"":"s"}`,i=e.requirements&&typeof e.requirements=="object"?e.requirements:{},g=Number(i.work_crews)||0;let m=!0;const v=Object.entries(i).map(([k,w])=>{const y=De(k),$=Number(w),E=y!=null&&Number.isFinite($)&&y>=$;return E||(m=!1),`<span class="co-req-chip ${E?"met":"unmet"}">${E?"✓":"✗"} ${r(pe(k))} ${r(String(w))}+</span>`}).join(""),u=e.issuer_name?`— ${r(e.issuer_name)}${e.nations?.name?`, ${r(e.nations.name)}`:""}`:"— Unknown issuer",l=T.get(e.id),p=m?"Place Bid ▸":"Requirements Unmet",c=m?`data-action="bid" data-id="${r(e.id)}"`:"disabled",f=l?`<div class="co-contract-bid-placed">
        ...
        <div class="co-contract-meta-row">
            <span class="co-contract-tag ${t}">${r(o)}</span>
        ...
        ${f}
    </div>`}function Ue(e){const t=ue(e.contract_type),o=String(e.contract_type||"GOVERNMENT"),n=Number(x?.current_tick)||0,a=Math.max(0,Math.min(100,Number(e.progress_pct)||0)),s=e.started_at_tick!=null?Number(e.started_at_tick):null,i=Number(e.timeline_months)||0,g=s!=null&&i>0?Math.max(0,Math.min(100,(n-s)/i*100)):0;let m="On Track",v=!1;a>=100?m="Complete":g-a>5&&(m="Late",v=!0);const u=e.expected_finish_tick!=null?`Slated to Complete on: ${r(Y(e.expected_finish_tick))}`:"No deadline set",l=T.get(e.id),p=Number(l?.crews_committed||0),c=Number(e.crews_working||0),f=p?`${c.toFixed(0)} / ${p.toFixed(0)}`:"—",k=p>0&&c<p?" warn":"",w=Number(e.budget)||0,y=Number(e.amount_spent)||0,$=w>0?(w-y)/w*100:0,E=w>0?`${$.toFixed(1)}%`:"—",N=$<5?" warn":"";return`<div class="co-contract-card">
        <div class="co-contract-meta-row">
            <span class="co-contract-tag ${t}">${r(o)}</span>
        ...
        <div class="co-contract-actions">
            ...
            <button class="co-contract-btn" data-action="view-details" data-id="${r(e.id)}">Details</button>
    ...
    </div>`}function me(){return`co-declined-${d?.id||"anon"}`}function fe(){try{const e=localStorage.getItem(me());return new Set(e?JSON.parse(e):[])}catch{return new Set}}function ze(e){const t=fe();t.add(e);try{localStorage.setItem(me(),JSON.stringify(Array.from(t)))}catch{}}function Ve(){document.getElementById("co-available-list")?.addEventListener("click",ne),document.getElementById("co-active-list")?.addEventListener("click",ne),document.getElementById("co-issues-list")?.addEventListener("click",async e=>{const t=e.target.closest("[data-action]");if(!t||t.disabled)return;const o=t.getAttribute("data-action");if(o==="open-relocate")return Fe();const n=t.getAttribute("data-id");if(n&&o==="ack-issue")return je(n)}),document.getElementById("co-modal-overlay")?.addEventListener("click",e=>{(e.target===e.currentTarget||e.target.matches("[data-modal-close]"))&&L()}),document.addEventListener("keydown",e=>{e.key==="Escape"&&L()})}async function ne(e){const t=e.target.closest(".co-contract-btn");if(!t||t.disabled)return;const o=t.getAttribute("data-action"),n=t.getAttribute("data-id");if(!(!o||!n)){if(o==="decline"){ze(n),de();return}if(o==="bid"){const a=I.find(s=>s.id===n);a&&Qe(a);return}if(o==="view-details"){const a=S.find(s=>s.id===n)||I.find(s=>s.id===n);a&&Xe(a);return}if(o==="manage-crews"){const a=S.find(s=>s.id===n);a&&Ze(a);return}if(o==="renegotiate"){_("Renegotiation isn't available yet.","error");return}}}const Ge={1:1,2:1.05,3:1.1},Ye={1:1,2:.8,3:.7};function Je(e,t,o){const n=1.3-(Number(d.corp_supply_chain)||0)*.06,a=Ge[t]||1,s=Ye[t]||1,i=(Number(e.budget)||0)*.7,g=Math.round(i*n*a*(1+o/100)),m=Math.round((Number(e.timeline_months)||0)*s);return{bidAmount:g,months:m,supplyMult:n,crewCost:a,crewTime:s}}function Qe(e){const t=document.getElementById("co-modal");if(!t)return;t.classList.add("co-bid-modal");const o=Math.floor(Number(d.corp_work_crews)||0),n=Number(d.corp_regulatory_standing)||0,a=Number(d.corp_supply_chain)||0,s=e.requirements&&typeof e.requirements=="object"?e.requirements:{},i=Number(s.regulatory_standing)||0,g=!e.issuer_nation_id||e.issuer_nation_id!==d.nation_id,m=g?1.5:1,v={1:o>=1,2:o>=2,3:o>=3};let u=v[3]?3:v[2]?2:(v[1],1),l=30;const p=()=>{const{bidAmount:c,months:f,supplyMult:k}=Je(e,u,l),w=Number(e.budget)||0,y=w>0?c/w*100:0,$=c>w,E=[1,2,3].map(h=>{const Z=v[h];return`<button type="button" class="co-bid-pill${h===u&&Z?" active":""}"
                ...
                <span class="sub">${h===1?"baseline":h===2?"−20% time · ×1.05 cost":"−30% time · ×1.10 cost"}</span>
                    ...
                    data-bid-markup="${h}">${h}%</button>`).join(""),H=n>=i?"good":"warn",be=`${n.toFixed(n%1===0?0:1)}<span class="muted"> / 10${i?` · req ${i}+`:""}</span>`,ye=$?"warn":y<85?"good":"",_e=`${C(c)}<span class="muted"> · ${y.toFixed(0)}% of budget</span>`,Q=5e4,K=!v[u],j=(Number(d.corp_cash_reserves)||0)<Q,X=n<i,we=K||j||X,he=j?"Need $50k":X?`Need Reg. ${i}+`:K?"No crews available":"Submit Bid ▸";t.innerHTML=`
            ...
            <div class="co-bid-section">
                ...
                <div class="co-bid-section-help">More crews shorten the timeline at a slightly higher cost. Your Work Crews stat caps the maximum (you have <strong>${o}</strong>).</div>
            </div>
        ...
        `,t.querySelectorAll("[data-bid-crews]").forEach(h=>{h.addEventListener("click",()=>{h.disabled||(u=Number(h.getAttribute("data-bid-crews")),p())})}),t.querySelectorAll("[data-bid-markup]").forEach(h=>{h.addEventListener("click",()=>{l=Number(h.getAttribute("data-bid-markup")),p()})}),t.querySelector("#co-bid-submit")?.addEventListener("click",()=>{Ke(e.id,u,l)})};p(),document.getElementById("co-modal-overlay").classList.add("open")}async function Ke(e,t,o){if(D)return;D=!0;const n=document.getElementById("co-bid-submit");n&&(n.disabled=!0);try{const{data:a,error:s}=await b.rpc("place_construction_bid",{p_contract_id:e,p_bidder_faction_id:d.id,p_crews_committed:t,p_markup_pct:o,p_bid_message:null});if(s){_(s.message,"error");return}if(!a?.success){_(a?.error||"Failed to place bid","error");return}_(`Bid placed: ${C(a.bid_amount)} · ${a.quoted_timeline_months} ticks`,"success"),L(),await J(),await F()}catch(a){console.error("[corp-operations] place bid failed:",a),_("Failed to place bid: "+(a?.message||"unknown"),"error")}finally{D=!1,n&&(n.disabled=!1)}}function Xe(e){const t=document.getElementById("co-modal");if(!t)return;const o=e.requirements&&typeof e.requirements=="object"?e.requirements:{},n=Object.entries(o).map(([a,s])=>`<div class="co-modal-detail-row">
    ...
    `,document.getElementById("co-modal-overlay").classList.add("open")}async function Ze(e){const t=document.getElementById("co-modal");if(!t)return;const o=T.get(e.id),n=Number(o?.crews_committed||0),a=Number(e.crews_working)||0,{data:s,error:i}=await b.rpc("crew_capacity_summary",{p_corp_id:d.id,p_exclude_contract_id:e.id});if(i){_("Failed to load crew capacity: "+i.message,"error");return}const g=Number(s?.owned)||0,m=Number(s?.deployed_other)||0,v=Number(s?.free_for_this)||0,u=Math.min(n,v);t.innerHTML=`
        ...
        <div class="co-modal-body">
            ...
            This contract committed <strong>${n}</strong> crews at bid time.<br><br>
    ...
    </div>`}function ve(e){const t=[`<span class="co-effect cost">−${C(e.cost).replace("-","")}</span>`];for(const[o,n]of Object.entries(e.effects||{})){const a=n>0?"+":"",s=n>0?"positive":"negative";t.push(`<span class="co-effect ${s}">${a}${n} ${r(et[o]||o)}</span>`)}return t.join("")}function at(){document.getElementById("co-actions-grid")?.addEventListener("click",e=>{const t=e.target.closest(".co-action-option");if(!t||t.hasAttribute("data-locked"))return;const o=Number(t.getAttribute("data-card-idx")),n=Number(t.getAttribute("data-opt-idx")),a=G[o],s=a?.options[n];!a||!s||st(a,s)})}function st(e,t){const o=document.getElementById("co-modal");if(!o)return;const n=ve(t);o.innerHTML=`
    ...
    `,document.getElementById("co-modal-overlay").classList.add("open"),document.getElementById("co-action-confirm")?.addEventListener("click",async()=>{if(U)return;U=!0;const a=document.getElementById("co-action-confirm");a&&(a.disabled=!0);try{const{data:s,error:i}=await b.rpc("fire_construction_action",{p_corp_id:d.id,p_action_key:e.key,p_choice:t.letter});if(i){_(i.message,"error");return}if(!s?.success){_(s?.error||"Action failed","error");return}_("Action taken — locked for 12 ticks.","success"),L(),await J(),ge(),se()}catch(s){console.error("[corp-operations] fire_construction_action failed:",s),_("Failed: "+(s?.message||"unknown"),"error")}finally{U=!1,a&&(a.disabled=!1)}},{once:!0})}async function J(){const{data:e,error:t}=await b.from("factions").select("corp_cash_reserves, corp_work_crews, corp_regulatory_standing, corp_supply_chain, corp_construction_action_locked_until_tick, action_points").eq("id",d.id).single();if(t||!e){console.error("[corp-operations] refetch faction state failed:",t?.message);return}Object.assign(d,e)}function _(e,t){document.querySelectorAll(".co-toast").forEach(n=>n.remove());const o=document.createElement("div");o.className="co-toast"+(t?" "+t:""),o.textContent=e,document.body.appendChild(o),setTimeout(()=>{o.classList.add("fade"),setTimeout(()=>o.remove(),280)},3e3)}let d=null,x=null,z=[];async function it(){const{data:{user:e}}=await b.auth.getUser();if(!e){window.location.href="login.html";return}const o=new URL(location.href).searchParams.get("faction_id"),{data:n,error:a}=await b.from("factions").select("*").or(`id.eq.${e.id},linked_user_id.eq.${e.id}`);if(a){M("Failed to load factions: "+a.message);return}if(z=(n||[]).filter(c=>c.nation_id),o){const{data:c,error:f}=await b.from("factions").select("*").eq("id",o).single();if(f||!c){M("Inspector faction not found.");return}d=c}else d=z.find(c=>c.faction_type==="corporation");if(!d||d.faction_type!=="corporation"){M("No corporation linked to this account.");return}const s=d.corp_sector||"";if(s!=="Construction"){const c=Ce[s]||"corp-dashboard.html",f=o?`?faction_id=${encodeURIComponent(o)}`:"";window.location.replace(c+f);return}const[i,g,m]=await Promise.all([b.from("shard").select("current_tick, current_date, name, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single(),d.nation_id?b.from("nations").select("id, name, capital").eq("id",d.nation_id).single():Promise.resolve({data:null,error:null}),b.from("corp_contracts").select("id",{count:"exact",head:!0}).eq("winner_faction_id",d.id).in("status",["awarded","active"])]);if(i.error){M("Failed to load shard: "+i.error.message);return}x=i.data,g?.data,g?.error&&console.error("[corp-operations] nation fetch error:",g.error.message),m?.error&&console.error("[corp-operations] active contracts count error:",m.error.message);const v=m?.count??0,u=document.getElementById("corp-topbar-container");if(u)try{const{renderCorpTopBar:c}=await ke(async()=>{const{renderCorpTopBar:f}=await import("./corp-topbar-6QBAGqD5.js");return{renderCorpTopBar:f}},__vite__mapDeps([0,1,2]));c(u,{faction:d,shard:x,activeTab:"operations",allUserFactions:z})}catch(c){console.error("[corp-operations] topbar render failed:",c)}document.getElementById("co-active-count").textContent=String(v),document.getElementById("co-footer-date").textContent=x?.current_date||Y(x?.current_tick)||"—",se(),ge(),Ve(),at(),document.getElementById("loading").style.display="none",document.getElementById("page-content").style.display="",F();let l=Number(x?.current_tick)||0;const p=setInterval(async()=>{if(!document.hidden)try{const{data:c,error:f}=await b.from("shard").select("current_tick, current_date").eq("name","Alpha Shard").single();if(f||!c)return;const k=Number(c.current_tick)||0;if(k===l)return;l=k,x={...x,...c},await F()}catch(c){console.error("[corp-operations] tick poll failed:",c)}},3e4);window.addEventListener("beforeunload",()=>clearInterval(p)),ce=xe({supabase:b,faction:d,host:document.getElementById("co-loan-pressing"),showEmpty:!1,onChange:()=>R()}),ie=$e({supabase:b,faction:d,host:document.getElementById("co-lawsuit-pressing"),currentTick:()=>Number(x?.current_tick)||0,showEmpty:!1,onChange:()=>R()}),re=Te({supabase:b,faction:d,host:document.getElementById("co-tax-pressing"),currentTick:()=>Number(x?.current_tick)||0,showEmpty:!1,onChange:()=>R()}),le()}function M(e){const t=document.getElementById("loading");t.textContent=e,t.style.color="var(--co-accent-red)"}it().catch(e=>{console.error("[corp-operations] init failed:",e),M("Failed to load: "+(e?.message||"unknown error"))});
