import{_}from"./supabase-client-BXEzLDpS.js";import{e as g}from"./utils-C2W-HleY.js";import{initMessaging as Ui}from"./messaging-B5Fng3EZ.js";import{c as Gi,a as St,E as Re,b as Xe,d as li,e as Wi,f as Fi,h as ni}from"./equipment-DsuDdEne.js";const di={LOW:"#c84",STD:"#ca5",HIGH:"#5c5"},le=[{key:"concrete",name:"Concrete",category:"RAW",tiers:{LOW:{requirements:[]},STD:{requirements:[{stat:"manufacturing_output",min:20}]},HIGH:{requirements:[{stat:"manufacturing_output",min:50},{stat:"higher_education",min:40}]}},priceDrivers:["manufacturing_output","inflation","fuel_prices","urbanization"]},{key:"steel",name:"Steel",category:"RAW",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:10}]},STD:{requirements:[{stat:"manufacturing_output",min:35},{stat:"rare_minerals",min:20}]},HIGH:{requirements:[{stat:"manufacturing_output",min:60},{stat:"rare_minerals",min:40},{stat:"higher_education",min:45}]}},priceDrivers:["manufacturing_output","rare_minerals","inflation","fuel_prices"]},{key:"lumber",name:"Lumber",category:"RAW",tiers:{LOW:{requirements:[{stat:"arable_land",min:10}]},STD:{requirements:[{stat:"arable_land",min:30},{stat:"physical_infrastructure",min:20}]},HIGH:{requirements:[{stat:"arable_land",min:50},{stat:"manufacturing_output",min:30}]}},priceDrivers:["arable_land","physical_infrastructure","inflation"]},{key:"aggregate",name:"Aggregate",category:"RAW",tiers:{LOW:{requirements:[]},STD:{requirements:[{stat:"rare_minerals",min:15},{stat:"physical_infrastructure",min:20}]},HIGH:{requirements:[{stat:"rare_minerals",min:35},{stat:"manufacturing_output",min:25}]}},priceDrivers:["rare_minerals","physical_infrastructure","inflation"]},{key:"em",name:"E&M Systems",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:15}]},STD:{requirements:[{stat:"manufacturing_output",min:40},{stat:"digital_infrastructure",min:25}]},HIGH:{requirements:[{stat:"manufacturing_output",min:55},{stat:"digital_infrastructure",min:50},{stat:"energy_generation",min:40}]}},priceDrivers:["manufacturing_output","digital_infrastructure","inflation","energy_generation"]},{key:"glass",name:"Glass & Facades",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:20}]},STD:{requirements:[{stat:"manufacturing_output",min:40},{stat:"standard_of_living",min:35}]},HIGH:{requirements:[{stat:"manufacturing_output",min:60},{stat:"digital_infrastructure",min:40},{stat:"higher_education",min:50}]}},priceDrivers:["manufacturing_output","standard_of_living","inflation"]},{key:"asphalt",name:"Asphalt",category:"RAW",tiers:{LOW:{requirements:[{stat:"oil_and_gas",min:10}]},STD:{requirements:[{stat:"oil_and_gas",min:30},{stat:"manufacturing_output",min:25}]},HIGH:{requirements:[{stat:"oil_and_gas",min:45},{stat:"manufacturing_output",min:40},{stat:"physical_infrastructure",min:40}]}},priceDrivers:["oil_and_gas","manufacturing_output","inflation","fuel_prices"]},{key:"heavy",name:"Heavy Machinery Parts",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:40},{stat:"rare_minerals",min:30}]},STD:{requirements:[{stat:"manufacturing_output",min:60},{stat:"rare_minerals",min:45},{stat:"higher_education",min:40}]},HIGH:{requirements:[{stat:"manufacturing_output",min:75},{stat:"rare_minerals",min:60},{stat:"higher_education",min:55},{stat:"digital_infrastructure",min:45}]}},priceDrivers:["manufacturing_output","rare_minerals","higher_education","digital_infrastructure"]}];function we(i,e,t){const a=le.find(l=>l.key===i);if(!a)return{available:!1,failedStat:"unknown_material"};const n=a.tiers[e];if(!n)return{available:!1,failedStat:"unknown_tier"};for(const l of n.requirements){const s=Number(t?.[l.stat]??0);if(s<l.min)return{available:!1,failedStat:l.stat,failedMin:l.min,nationValue:s}}return{available:!0}}function Dt(i,e,t){const n={concrete:{LOW:200,STD:300,HIGH:500},steel:{LOW:400,STD:700,HIGH:1200},lumber:{LOW:80,STD:130,HIGH:200},aggregate:{LOW:40,STD:60,HIGH:100},em:{LOW:400,STD:700,HIGH:1200},glass:{LOW:300,STD:500,HIGH:900},asphalt:{LOW:120,STD:200,HIGH:350},heavy:{LOW:800,STD:1400,HIGH:2400}}[i]?.[e];if(!n)return 0;const l=le.find(d=>d.key===i);if(!l)return n;let s=1;for(const d of l.priceDrivers){const r=Number(t?.[d]??50);d==="inflation"||d==="fuel_prices"?s*=1+(r-50)/200:s*=1-(r-50)/250}return s=Math.max(.4,Math.min(2.5,s)),Math.round(n*s)}function ci(i,e,t){const n={concrete:{LOW:5e3,STD:3e3,HIGH:1e3},steel:{LOW:2e3,STD:1500,HIGH:500},lumber:{LOW:8e3,STD:4e3,HIGH:1500},aggregate:{LOW:15e3,STD:6e3,HIGH:2e3},em:{LOW:1e3,STD:700,HIGH:300},glass:{LOW:1500,STD:800,HIGH:300},asphalt:{LOW:4e3,STD:2e3,HIGH:800},heavy:{LOW:400,STD:200,HIGH:80}}[i]?.[e]||0,s=le.find(o=>o.key===i)?.priceDrivers?.[0],r=.3+(s?Number(t?.[s]??50):50)/50*.7;return Math.round(n*r)}const Ht=["LOW","STD","HIGH"],At={LOW:"Low",STD:"Standard",HIGH:"High"};let Le=[],f=null,w=null,R=null,Ie=[],Ne={},Z=[],U={},zt=-1,O="concrete",D="STD",ke=500,ee=[],Lt=0,Q="trucks",ne=0,ae=1,ve=[],Ee=null,Ke=[],Bt=null,Ye=null,qt="ALL",Nt="TIMELINE";function B(i){return Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(1)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i}function Vi(i){if(i>=12){const e=Math.floor(i/12),t=i%12;return t>0?e+"y "+t+"mo":e+"y"}return i+" ticks"}function K(i){return Math.abs(i)>=1e9?"$"+(i/1e9).toFixed(1)+"B":Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(0)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i}function Ze(i){return i==="civil_engineering"?"CIVIL":i==="industrial"?"INDUSTRIAL":i==="mega_project"?"MEGA":i?.toUpperCase()||"—"}function pi(i){return i==="civil_engineering"?"light":i==="industrial"?"heavy":i==="mega_project"?"mega":"light"}function Yi(){Ye&&clearInterval(Ye),Ye=setInterval(()=>{if(!Bt)return;const i=Bt-Date.now();if(i<=0){document.getElementById("tick-countdown").textContent="Tick due...",clearInterval(Ye);return}const e=Math.floor(i/36e5),t=Math.floor(i%36e5/6e4),a=Math.floor(i%6e4/1e3);document.getElementById("tick-countdown").textContent=e+"h "+t+"m "+a+"s"},1e3)}function Qi(){document.body.classList.toggle("light-mode");const i=document.getElementById("theme-toggle");i.textContent=document.body.classList.contains("light-mode")?"Dark":"Light"}function Ki(i,e){i==="type"&&(qt=e),i==="sort"&&(Nt=e),document.querySelectorAll(`.filter-pill[data-filter="${i}"]`).forEach(t=>{t.classList.toggle("active",t.dataset.value===e)}),fi()}function mi(i){return!(!f||i.sector==="mega_project"&&f.corp_subsector!=="Megaprojects"||i.sector==="industrial"&&!["Heavy Infrastructure","Megaprojects"].includes(f.corp_subsector))}function fi(){const i=document.getElementById("oc-list");let e=[...Ie];if(qt==="GOVERNMENT"?e=e.filter(n=>n.issuer_type==="GOVERNMENT"):qt==="PRIVATE"&&(e=e.filter(n=>n.issuer_type==="PRIVATE")),Nt==="TIMELINE"&&e.sort((n,l)=>(n.timeline_ticks||0)-(l.timeline_ticks||0)),Nt==="BUDGET"&&e.sort((n,l)=>(l.budget_ceiling||0)-(n.budget_ceiling||0)),document.getElementById("oc-count").textContent=e.length+" AVAILABLE",e.length===0){i.innerHTML=`
            <div class="oc-empty">
            ...
            </div>`;return}const t=R?.current_tick||0;let a="";for(const n of e){const l=n.issuer_type==="GOVERNMENT",s=l?"gov":"private",d=mi(n),r=d?"":" locked",o=pi(n.sector),c=Ze(n.sector),m=(n.timeline_ticks||0)>18?" warn":"",p=n.bidding_ends_tick?Math.max(0,n.bidding_ends_tick-t):"?";a+=`
            <div class="oc-item${r}" data-contract-id="${n.id}">
                ...
                <div class="oc-item__stats">
                    ...
                    <div class="oc-stat">
                        ...
                        <div class="oc-stat__value${m}">${Vi(n.timeline_ticks||0)}</div>
                    </div>
                    <div class="oc-stat">
                    ...
                    <div class="oc-eligibility">
                        ${Ne[n.id]?`<span class="oc-eligible-badge yes" style="background:var(--green-faint);border-color:var(--green-border);color:var(--green);">BID: ${K(Ne[n.id].bid_price)}</span>`:`<span class="oc-eligible-badge ${d?"yes":"no"}">${d?"ELIGIBLE":"LOCKED"}</span>`}
                    </div>
                    ${d?`<button class="bid-btn bid-btn--submit" style="padding:3px 10px;font-size:8px;margin-left:auto;" onclick="event.stopPropagation();openBidModal(contracts.find(x=>x.id==='${n.id}'))">${Ne[n.id]?"EDIT":"VIEW"}</button>`:""}
                </div>
                ${n.description?`<div style="font-size:9px;color:var(--text-dim);padding:4px 0 0;font-style:italic;">${g(n.description)}</div>`:""}
            </div>`}i.innerHTML=a,i.querySelectorAll(".oc-item:not(.locked)").forEach(n=>{n.addEventListener("click",()=>{const l=n.dataset.contractId,s=Ie.find(d=>d.id===l);s&&ui(s)})})}let de=null;function ui(i){de=i;const e=document.getElementById("cd-overlay"),t=i.issuer_type==="GOVERNMENT",a=t?"gov":"private",n=(w?.name||f.nation||"—").toUpperCase(),l=mi(i);document.getElementById("cd-header-left").innerHTML=`
    ...
    `;const s=document.getElementById("cd-blueprint");i.blueprint_svg?(s.innerHTML=i.blueprint_svg,s.style.display=""):(s.innerHTML=yn(i),s.style.display="");const d=i.permits_required||[],r=i.required_equipment||i.equipment_required||[],o=i.required_materials||i.materials_estimated||{},m={civil_engineering:"Civil Engineering",industrial:"Industrial Construction",mega_project:"Megaprojects"}[i.sector]||i.spec_category||i.sector||"—";let p="var(--teal)";i.sector==="industrial"&&(p="var(--orange)"),i.sector==="mega_project"&&(p="var(--red)");let u=B(i.budget_ceiling||i.budget||0),y=(i.timeline_ticks||i.timeline_months||0)+" Months",v="";v+=`
        <div class="cd-issue">
            ...
            </div>`),v+='<div class="cd-details">',i.project_type&&(v+=ye("Type",i.project_type)),i.project_subtype&&(v+=ye("Sub-Type",i.project_subtype)),v+=ye("Specialization",m,p),v+=ye("Total Budget",u,"var(--green)"),v+=ye("Timeline",y),v+=ye("Nation",w?.name||f.nation||"—"),i.region&&(v+=ye("Region",i.region)),v+="</div>",d.length>0&&(v+=`
            <div class="cd-items">
                <div class="cd-section-label">Permits Required</div>
                <div class="cd-items__list">
                    ${d.map(E=>{const M=E.status==="approved"?"approved":"required",N=E.status==="approved"?"&#10003;":"&#9675;";return`<div class="cd-chip ${M}">
            ...
            </div>`),document.getElementById("cd-content").innerHTML=v;const h=d.filter(E=>E.status==="approved").length,$=d.length-h,k=r.filter(E=>E.owned).length,A=r.length-k;let x="";r.length>0&&(A===0?x+='<span class="cd-footer__badge ok">ALL EQUIPMENT MET</span>':x+=`<span class="cd-footer__badge bad">${A} EQUIPMENT MISSING</span>`),d.length>0&&($===0?x+='<span class="cd-footer__badge ok">ALL PERMITS APPROVED</span>':x+=`<span class="cd-footer__badge warn">${$} PERMITS PENDING</span>`);const T=l;(f.action_points??0)>=2;const C=i.issuer_faction_id===f?.id,S=i.status==="bidding";document.getElementById("cd-footer").innerHTML=`
        ...
        <div class="cd-footer__actions">
            ...
            ${C?`<button class="cd-btn primary" onclick="openBidReview()" style="background:#c8a832;border-color:#c8a832;"
                    ${S?"":"disabled"} title="${S?"Review submitted bids":"No bids received yet"}">REVIEW BIDS</button>`:`<button class="cd-btn primary" onclick="openBidAssembly()" ${T?"":"disabled"}
                    title="${T?"Assemble and submit a bid":"Not qualified for this contract"}">BID</button>`}
        </div>
    `,e.classList.add("open"),document.body.style.overflow="hidden"}function et(i){i&&i.target&&i.target!==document.getElementById("cd-overlay")||(document.getElementById("cd-overlay").classList.remove("open"),document.body.style.overflow="",de=null)}const Ji=[{key:"concrete",name:"Concrete",unit:"units"},{key:"steel",name:"Steel",unit:"units"},{key:"lumber",name:"Lumber",unit:"units"},{key:"aggregate",name:"Aggregate",unit:"units"},{key:"em_systems",name:"E&M Systems",unit:"units"},{key:"glass_facades",name:"Glass & Facades",unit:"units"},{key:"asphalt",name:"Asphalt",unit:"units"},{key:"heavy_parts",name:"Heavy Machinery Parts",unit:"units"}],Xi=[{key:"work_trucks",name:"Work Trucks",tier:1},{key:"excavators",name:"Excavators",tier:1},{key:"bulldozers",name:"Bulldozers",tier:1},{key:"concrete_mixers",name:"Concrete Mixers",tier:1},{key:"tower_cranes",name:"Tower Cranes",tier:2},{key:"heavy_haulers",name:"Heavy Haulers",tier:2},{key:"pile_drivers",name:"Pile Drivers",tier:2},{key:"asphalt_plants",name:"Asphalt Plants",tier:2}],yi={LOW:.7,STANDARD:1,HIGH:1.4},vi={LOW:35,STANDARD:65,HIGH:90},vt=15;let G=null;function Zi(i){if(!i)return;const e=i.required_materials||{},t=i.required_equipment||[],a=i.required_workforce||{},n={concrete:18e4,steel:25e4,lumber:12e4,aggregate:8e4,em_systems:32e4,glass_facades:28e4,asphalt:14e4,heavy_parts:4e5},l=Ji.filter(c=>e[c.key]>0).map(c=>({...c,qty:e[c.key],basePrice:n[c.key]||2e5,grade:c.key==="aggregate"?"LOW":"STANDARD",highDisabled:!1})),s=Xi.filter(c=>t.includes(c.key)).map(c=>({...c,owned:(ee||[]).some(m=>m.equipment_key===c.key&&m.quantity>0)})),d=(a.general||100)+(a.skilled||20),r=i.budget_ceiling||1e8,o=Math.round(r*.03);G={contract:i,budgetCeiling:r,materials:l,laborCount:d,laborRate:15200,estimatedTicks:i.timeline_ticks||8,equipment:s,permits:[],overhead:o,markupPct:15,competitors:[],playerRep:f?.standing||50,requiredWorkforce:a},document.getElementById("bid-title").textContent="BID ASSEMBLY",document.getElementById("bid-subtitle").textContent=(i.name||"Contract")+" — "+Ze(i.sector)+" — "+(i.issuer_name||"Government"),document.getElementById("bid-overlay").classList.add("open"),document.body.style.overflow="hidden",tt()}function gi(i){i&&i.target!==document.getElementById("bid-overlay")||(document.getElementById("bid-overlay").classList.remove("open"),document.body.style.overflow="",G=null)}function q(i){return Math.abs(i)>=1e9?"$"+(i/1e9).toFixed(2)+"B":Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(2)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i.toLocaleString()}function en(i,e){if(!G)return;const t=G.materials[i];e==="HIGH"&&t.highDisabled||(t.grade=e,tt())}function tn(i){G&&(G.laborCount=i,tt())}function nn(i){G&&(G.markupPct=Number(i),tt())}function tt(){if(!G)return;const i=G;let e=0;for(const M of i.materials)M.lineCost=Math.round(M.qty*M.basePrice*yi[M.grade]),e+=M.lineCost;const t=Math.round(i.laborCount*i.laborRate*i.estimatedTicks),a=Math.round(i.equipment.filter(M=>M.owned).length*12e3*i.estimatedTicks);let n=0;const l=i.overhead,s=e+t+a+n+l,d=Math.round(s*i.markupPct/100),r=s+d,o=r>i.budgetCeiling,c=d,m=Math.round(i.materials.reduce((M,N)=>M+vi[N.grade],0)/i.materials.length),p=m>=80?"STRONG":m>=60?"PROMISING":m>=40?"UNCERTAIN":"POOR",u=m>=80?"var(--green)":m>=60?"var(--teal)":m>=40?"var(--orange)":"var(--red)",y=i.budgetCeiling>0?r/i.budgetCeiling:1,v=Math.max(0,Math.min(100,Math.round((1-y)*150))),h=v>=70?"STRONG":v>=40?"COMPETITIVE":v>=15?"WEAK":"UNLIKELY",$=v>=70?"var(--green)":v>=40?"var(--teal)":v>=15?"var(--orange)":"var(--red)",k=Math.round(s*(1-vt/100)),A=Math.round(s*(1+vt/100));let x="";x+='<div class="bid-section"><div class="bid-section__title">Materials</div>',i.materials.forEach((M,N)=>{const H=pe=>{const Ue=M.grade===pe,Ge=pe==="HIGH"&&M.highDisabled;return`<button class="bid-grade-btn ${Ue?pe==="LOW"?"active-low":pe==="HIGH"?"active-high":"active":""} ${Ge?"disabled":""}" onclick="setBidGrade(${N},'${pe}')">${pe[0]}</button>`};x+=`<div class="bid-mat-row">
            <span class="bid-mat-row__name">${g(M.name)}</span>
            <span class="bid-mat-row__qty">×${M.qty}</span>
            <div class="bid-grade-btns">${H("LOW")}${H("STANDARD")}${H("HIGH")}</div>
            <span class="bid-mat-row__cost">${q(M.lineCost)}</span>
        </div>`}),x+=`<div class="bid-line-total"><span class="bid-line-total__label">MATERIALS TOTAL</span><span class="bid-line-total__value">${q(e)}</span></div></div>`;const T=(i.requiredWorkforce?.general||80)+(i.requiredWorkforce?.skilled||20),C=[Math.round(T*.8),T,Math.round(T*1.2),Math.round(T*1.4),Math.round(T*1.6)];x+='<div class="bid-section"><div class="bid-section__title">Labor</div>',x+='<div class="bid-labor-presets">',C.forEach(M=>{x+=`<button class="bid-labor-btn ${i.laborCount===M?"active":""}" onclick="setBidLabor(${M})">${M}</button>`}),x+="</div>";const S=i.requiredWorkforce||{};x+=`<div class="bid-labor-formula">Required: ${S.general||"?"} general + ${S.skilled||"?"} skilled<br>`,x+=`${i.laborCount} workers × ${q(i.laborRate)}/tick × ${i.estimatedTicks} ticks = <strong>${q(t)}</strong></div>`,x+=`<div class="bid-line-total"><span class="bid-line-total__label">LABOR TOTAL</span><span class="bid-line-total__value">${q(t)}</span></div></div>`,x+='<div class="bid-section"><div class="bid-section__title">Equipment</div>',i.equipment.forEach(M=>{const N=M.owned?"bid-equip-row__status--owned":"bid-equip-row__status--missing",H=M.owned?"✓ OWNED":"✗ NOT OWNED";x+=`<div class="bid-equip-row"><span class="bid-equip-row__name">${g(M.name)}</span><span class="bid-equip-row__status ${N}">${H}</span></div>`}),x+=`<div class="bid-line-total"><span class="bid-line-total__label">MAINTENANCE (${i.estimatedTicks}t)</span><span class="bid-line-total__value">${q(a)}</span></div></div>`,x+='<div class="bid-section"><div class="bid-section__title">Permits</div>',x+='<div style="font-family:var(--font-mono);font-size:9px;color:var(--text-dim);padding:8px 0;">No permits required yet.</div>',x+='<div class="bid-line-total"><span class="bid-line-total__label">PERMITS TOTAL</span><span class="bid-line-total__value">$0</span></div></div>',x+='<div class="bid-section"><div class="bid-section__title">Overhead &amp; Contingency</div>',x+='<div style="font-family:var(--font-mono);font-size:9px;color:var(--text-dim);">Site management, insurance, admin</div>',x+=`<div class="bid-line-total"><span class="bid-line-total__label">OVERHEAD</span><span class="bid-line-total__value">${q(l)}</span></div></div>`,document.getElementById("bid-left").innerHTML=x;let E="";E+='<div class="bid-section"><div class="bid-section__title">Cost Summary</div>',E+=`<div class="bid-summary-row"><span class="bid-summary-row__label">Materials</span><span class="bid-summary-row__value">${q(e)}</span></div>`,E+=`<div class="bid-summary-row"><span class="bid-summary-row__label">Labor</span><span class="bid-summary-row__value">${q(t)}</span></div>`,E+=`<div class="bid-summary-row"><span class="bid-summary-row__label">Equipment Maint.</span><span class="bid-summary-row__value">${q(a)}</span></div>`,E+=`<div class="bid-summary-row"><span class="bid-summary-row__label">Permits</span><span class="bid-summary-row__value">${q(n)}</span></div>`,E+=`<div class="bid-summary-row"><span class="bid-summary-row__label">Overhead</span><span class="bid-summary-row__value">${q(l)}</span></div>`,E+=`<div class="bid-cost-total"><span class="bid-cost-total__label">ESTIMATED COST</span><span class="bid-cost-total__value">${q(s)}</span></div>`,E+=`<div class="bid-accuracy">⚠ Estimate accuracy: ±${vt}%<br>Actual cost range: ${q(k)} — ${q(A)}</div>`,E+="</div>",E+='<div class="bid-section"><div class="bid-section__title">Markup</div>',E+=`<div class="bid-slider-wrap">
        <div class="bid-slider-label"><span class="bid-slider-label__pct">${i.markupPct}%</span><span style="color:var(--text-dim)">${q(d)}</span></div>
        <input type="range" class="bid-slider" min="0" max="40" value="${i.markupPct}" oninput="setBidMarkup(this.value)">
    </div></div>`,E+=`<div class="bid-price-hero ${o?"bid-price-hero--over":""}">
        <div class="bid-price-hero__label">YOUR BID PRICE</div>
        <div class="bid-price-hero__value">${q(r)}</div>
        ${o?'<div class="bid-price-hero__warning">EXCEEDS BUDGET CEILING ('+q(i.budgetCeiling)+")</div>":""}
    </div>`,E+=`<div class="bid-profit"><span class="bid-profit__label">PROJECTED PROFIT</span><span class="bid-profit__value">+${q(c)}</span></div>`,E+=`<div class="bid-compete">
    ...
    </div>`,E+='<div class="bid-section" style="margin-top:8px;"><div class="bid-section__title">Competing Bids</div>',i.competitors.forEach(M=>{E+=`<div class="bid-competitor"><span class="bid-competitor__name">${g(M.name)}</span><span class="bid-competitor__rep">Rep ${M.rep}</span></div>`}),E+=`<div class="bid-competitor" style="color:var(--gold);"><span class="bid-competitor__name">You</span><span class="bid-competitor__rep">Rep ${i.playerRep}</span></div>`,E+="</div>",document.getElementById("bid-right").innerHTML=E,document.getElementById("bid-footer-price").textContent=q(r),document.getElementById("bid-footer-price").style.color=o?"var(--red)":"var(--gold)",document.getElementById("bid-footer-profit").textContent="+"+q(c),document.getElementById("bid-footer-quality").textContent=m+"/100",document.getElementById("bid-footer-quality").style.color=u,document.getElementById("bid-submit-btn").disabled=o}window.openBidModal=Zi;window.closeBidModal=gi;window.setBidGrade=en;window.setBidLabor=tn;window.setBidMarkup=nn;async function an(){if(!G||!f||ze)return;const i=G,e=i.contract;let t=0;const a={};for(const m of i.materials)t+=Math.round(m.qty*m.basePrice*yi[m.grade]),a[m.key]=m.grade;const n=Math.round(i.laborCount*i.laborRate*i.estimatedTicks),l=Math.round(i.equipment.filter(m=>m.owned).length*12e3*i.estimatedTicks),s=t+n+l+i.overhead,d=Math.round(s*i.markupPct/100),r=s+d,o=Math.round(i.materials.reduce((m,p)=>m+vi[p.grade],0)/(i.materials.length||1));if(r>i.budgetCeiling){alert("Bid exceeds budget ceiling. Lower your costs or markup.");return}const c=document.getElementById("bid-submit-btn");c.disabled=!0,c.textContent="SUBMITTING...",ze=!0;try{const{data:m}=await _.from("shard").select("current_tick").eq("name","Alpha Shard").single(),p=m?.current_tick||0,{data:u}=await _.from("contract_bids").select("id").eq("contract_id",e.id).eq("faction_id",f.id).maybeSingle();if(u){const{error:v}=await _.from("contract_bids").update({bid_price:r,material_grades:a,labor_count:i.laborCount,markup_pct:i.markupPct,estimated_cost:s,estimated_quality:o,submitted_at_tick:p}).eq("id",u.id);if(v)throw v}else{const{error:v}=await _.from("contract_bids").insert({contract_id:e.id,faction_id:f.id,bid_price:r,material_grades:a,labor_count:i.laborCount,markup_pct:i.markupPct,estimated_cost:s,estimated_quality:o,status:"pending",submitted_at_tick:p});if(v)throw v}gi();const y=document.getElementById("oc-count");if(y){const v=y.textContent;y.textContent="✓ BID SUBMITTED",y.style.color="var(--green)",setTimeout(()=>{y.textContent=v,y.style.color=""},2e3)}await ce()}catch(m){console.error("Bid submission failed:",m),alert("Failed to submit bid: "+(m.message||"Unknown error")),c.disabled=!1,c.textContent="SUBMIT BID"}finally{ze=!1}}window.submitBid=an;const xe=["Permits","Planning","Foundation","Structural","Systems","Finishing","Delivery"],ai={WEATHER:{color:"var(--blue)",bg:"var(--blue-faint)",border:"var(--blue-border)"},SUPPLY:{color:"var(--gold)",bg:"var(--gold-faint)",border:"var(--gold-border)"},LABOR:{color:"var(--orange)",bg:"var(--orange-faint)",border:"var(--orange-border)"},REGULATORY:{color:"var(--red)",bg:"var(--red-faint)",border:"var(--red-border)"},EQUIPMENT:{color:"var(--amber)",bg:"var(--amber-faint)",border:"var(--amber-border)"},POLITICAL:{color:"var(--teal)",bg:"var(--teal-faint)",border:"var(--teal-border)"}},oi={LOW:"var(--green)",MODERATE:"var(--amber)",HIGH:"var(--orange)",CRITICAL:"var(--red)"};let L=null;function on(i){const e=Z.find(S=>S.id===i);if(!e)return;const t=Array.isArray(e.contract_bids)?e.contract_bids[0]:e.contract_bids,a=R?.current_tick||0,n=e.awarded_at_tick||a,l=e.timeline_ticks||8,s=Math.max(0,a-n),d=Math.min(100,s/l*100);let r=Math.min(xe.length-1,Math.floor(d/(100/xe.length)));const o=Math.round(d%(100/xe.length)/(100/xe.length)*100),c=e.required_materials||{},m=t?.material_grades||{},p=Object.entries(c).map(([S,E])=>{const M=m[S]||"STANDARD",N=Math.round(E*(d/100)*(.6+Math.random()*.4));return{key:S,name:S.replace(/_/g," ").replace(/\b\w/g,H=>H.toUpperCase()),grade:M,allocated:E,used:Math.min(N,E)}}),y=(e.required_equipment||[]).map(S=>({key:S,name:S.replace(/_/g," ").replace(/\b\w/g,E=>E.toUpperCase()),qty:1+Math.floor(Math.random()*3),condition:55+Math.floor(Math.random()*40)})),v=e.budget_ceiling||0,h=t?.estimated_cost||0,$=Math.round(h*Math.min(1,s/l)),k=t?.estimated_quality||65,A=k>=80?"STRONG":k>=60?"PROMISING":k>=40?"FAIR":"UNCERTAIN",x=e.required_workforce||{},T=(x.general||0)+(x.skilled||0),C=t?.labor_count||T;L={project:e,bid:t,tab:"overview",expandedEvent:-1,selectedResponse:null,currentTick:a,awardedTick:n,totalTicks:l,ticksElapsed:s,phaseIdx:r,phaseProgress:o,materials:p,equipment:y,budget:v,estCost:h,spent:$,quality:k,qualityLabel:A,laborCount:C,wfNeeded:T,events:[]},document.getElementById("pm-overlay").classList.add("open"),document.body.style.overflow="hidden",xi(e.id).then(()=>Te()),Te()}function sn(i){i&&i.target!==document.getElementById("pm-overlay")||(document.getElementById("pm-overlay").classList.remove("open"),document.body.style.overflow="",L=null)}function rn(i){L&&(L.tab=i,L.expandedEvent=-1,L.selectedResponse=null,Te())}function ln(i){L&&(L.expandedEvent=L.expandedEvent===i?-1:i,L.selectedResponse=null,Te())}function dn(i){L&&(L.selectedResponse=L.selectedResponse===i?null:i,Te())}function Te(){if(!L)return;const i=L,e=i.project,t=e.issuer_type==="GOVERNMENT",a=Ze(e.sector),n=f?.nation||"Nation",l=i.awardedTick+i.totalTicks,s=Math.max(0,l-i.currentTick),d=i.currentTick>l,r=i.budget>0?Math.round(i.spent/i.budget*100):0,o=r>85?"var(--red)":r>60?"var(--amber)":"var(--teal)",c=i.budget-i.spent,m=i.events.filter(h=>h.status==="ACTIVE").length;document.getElementById("pm-header").innerHTML=`
    ...
    `;let p='<div class="pm-phase__bar">';for(let h=0;h<xe.length;h++){const $=h<i.phaseIdx,k=h===i.phaseIdx;p+=`<div class="pm-phase__seg">
            <div class="pm-phase__seg-fill pm-phase__seg-fill--${$?"done":k?"active":"future"}"></div>
            <span class="pm-phase__seg-label pm-phase__seg-label--${$?"done":k?"active":""}">${xe[h]}</span>
        </div>`}p+="</div>",p+=`<div class="pm-phase__info">
        <span class="pm-phase__status">${xe[i.phaseIdx]} — ${i.phaseProgress}% complete</span>
    ...
    `}function cn(i,e,t,a,n,l,s){const d=gt(i.awardedTick+i.totalTicks);gt(i.awardedTick+i.totalTicks);const r=gt(i.awardedTick),o=[{label:"Budget",value:K(i.budget),sub:`${a}% spent`,color:t},{label:"Spent",value:K(i.spent),color:"var(--red)"},{label:"Remaining",value:K(n),color:"var(--green)"},{label:"Quality",value:`${i.quality}/100`,sub:i.qualityLabel,color:i.quality>=70?"var(--green)":i.quality>=50?"var(--amber)":"var(--red)"},{label:"Workforce",value:`${i.laborCount}/${i.wfNeeded}`,sub:`Bid: ${i.laborCount}`,color:i.laborCount<i.wfNeeded?"var(--orange)":"var(--text-bright)"},{label:"Remaining",value:`${l} ticks`,sub:s?"OVERDUE":`Deadline: ${d}`,color:s?"var(--red)":"var(--text-bright)"}];let c="";c+=`<div style="padding:0 16px"><div class="pm-section">
        ...
        <div class="pm-manager">
            ...
            </div>`;c+="</div></div>"}return c}function pn(i){if(i.events.length===0)return'<div class="pm-evt-empty">No events have occurred on this project yet.<br>Events will appear here as construction progresses.</div>';let e="";for(let t=0;t<i.events.length;t++){const a=i.events[t],n=i.expandedEvent===t,l=a.status==="ACTIVE",s=ai[a.type]||ai.WEATHER,d=oi[a.severity]||oi.LOW;if(e+=`<div class="pm-evt ${l?"pm-evt--active":"pm-evt--resolved"}" style="${l?`border-left-color:${s.color}`:""}">`,e+=`<div class="pm-evt__header" onclick="pmToggleEvent(${t})" style="${n?`background:${s.bg}`:""}">`,e+=`<div class="pm-evt__row1">
        ...
        </div>`}return e}function fn(i){if(i.equipment.length===0)return'<div class="pm-evt-empty">No equipment deployed to this project.</div>';let e='<div class="pm-tab-header">Deployed Equipment</div>';for(const t of i.equipment){const a=t.condition>=75?"var(--green)":t.condition>=50?"var(--amber)":t.condition>=25?"var(--orange)":"var(--red)",n=t.condition<60;e+=`<div class="pm-eq">
        ...
        </div>`}return e}function gt(i){return`${["January","February","March","April","May","June","July","August","September","October","November","December"][i%12]}, ${2e3+Math.floor(i/12)}`}window.openProjectModal=on;window.closeProjectModal=sn;window.pmSetTab=rn;window.pmToggleEvent=ln;window.pmSelectResponse=dn;async function xi(i){if(!L)return;const{data:e,error:t}=await _.from("construction_events").select("*").eq("contract_id",i).order("fired_at_tick",{ascending:!1});t?(console.warn("Failed to load project events:",t.message),L.events=[]):L.events=(e||[]).map(a=>({id:a.id,type:a.type,severity:a.severity,tick:a.fired_at_tick,title:a.title,desc:a.description,impact:a.impact,status:a.status==="ACTIVE"?"ACTIVE":"RESOLVED",resolution:a.resolution,responses:a.responses||[]})),Te()}let xt=!1;async function un(i,e){if(!(xt||!L)){xt=!0;try{const{data:t,error:a}=await _.rpc("resolve_construction_event",{p_event_id:i,p_response_key:e});if(a){console.error("Failed to resolve event:",a.message),alert("Failed to submit response: "+a.message);return}const n=typeof t=="string"?JSON.parse(t):t;if(n?.error){alert("Error: "+n.error);return}await xi(L.project.id),await bi(),n?.quality_applied&&n.quality_applied!==0&&(L.quality=Math.max(0,Math.min(100,L.quality+n.quality_applied)),L.qualityLabel=L.quality>=80?"STRONG":L.quality>=60?"PROMISING":L.quality>=40?"FAIR":"UNCERTAIN"),Te()}finally{xt=!1}}}window.confirmEventResponse=un;function ye(i,e,t){const a=t?` style="color:${t}"`:"";return`<div class="cd-detail-row">
        <span class="cd-detail-row__label">${g(i)}</span>
        <span class="cd-detail-row__value"${a}>${g(e)}</span>
    </div>`}function yn(i){const e={bg:"#1a2a3a",line:"#3a6a8a",dim:"#2a4a5a",accent:"#5a9aba",text:"#4a8aaa"},t=i.drawing_number||i.contract_number+"-A1",a=R?.current_date||"",n=a?a.replace(/,\s*/," "):"",l=i.spec_category==="Heavy Infrastructure",s=i.spec_category==="Megaproject";let d=g(i.project_subtype||i.project_type||"STRUCTURE"),r=l?"80.0m":s?"200.0m":"60.0m",o=l?"40.0m":s?"100.0m":"20.0m";return`<svg viewBox="0 0 680 200" style="width:100%;display:block;background:${e.bg}">
        <!-- Grid -->
        ...
        <g transform="translate(470,172)">
    ...
    </svg>`}let bt=!1;async function vn(){if(bt||!de||!f)return;if((f.action_points??0)<2){alert("You need at least 2 AP to place a bid.");return}bt=!0;const i=document.querySelector(".cd-btn.primary");i&&(i.disabled=!0,i.textContent="...");try{const{data:e,error:t}=await _.rpc("deduct_ap",{p_faction_id:f.id,p_cost:2});if(t)throw t;if(e<0){const n=-e-1;f.action_points=n,document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+n+" AP</span>",i&&(i.disabled=!1,i.textContent="BID"),alert("Insufficient AP. You have "+n+" AP, need 2.");return}const{error:a}=await _.from("corp_contract_bids").insert({contract_id:de.id,faction_id:f.id,nation_id:f.nation_id,ap_spent:2,created_at_tick:R?.current_tick||null});if(a)throw await _.rpc("deduct_ap",{p_faction_id:f.id,p_cost:-2}),f.action_points=e+2,document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+(e+2)+" AP</span>",a;f.action_points=e,document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+e+" AP</span>",i&&(i.textContent="BID PLACED")}catch(e){i&&(i.disabled=!1,i.textContent="BID"),e.code==="23505"?alert("You have already placed a bid on this contract."):alert("Failed to place bid: "+(e.message||"Unknown error"))}finally{bt=!1}}async function ce(){if(!f||!f.nation_id)return;const{data:i,error:e}=await _.from("construction_contracts").select("*").in("status",["open","bidding"]).order("generated_at_tick",{ascending:!1});if(e?(console.warn("Failed to load contracts:",e.message),Ie=[]):Ie=i||[],Ne={},f&&Ie.length>0){const t=Ie.map(n=>n.id),{data:a}=await _.from("contract_bids").select("contract_id, bid_price, estimated_quality, status").eq("faction_id",f.id).in("contract_id",t);for(const n of a||[])Ne[n.contract_id]=n}fi()}function gn(){const i=document.getElementById("ap-list"),e=document.getElementById("ap-footer");if(document.getElementById("ap-count").textContent=Z.length+" ACTIVE",Z.length===0){i.innerHTML=`<div class="ap-empty">
            <div class="ap-empty__text">No active projects.<br>Win a contract bid to start<br>your first construction project.</div>
        </div>`,e.style.display="none";return}const t=R?.current_tick||0;let a=0,n=0,l="";for(const s of Z){const d=s.issuer_type==="GOVERNMENT",r=d?"gov":"private",o=Array.isArray(s.contract_bids)?s.contract_bids[0]:s.contract_bids,c=o?.bid_price||0,m=o?.estimated_cost||0,p=o?.estimated_quality||0,u=s.budget_ceiling||0,y=s.awarded_at_tick||t,v=y+(s.timeline_ticks||8),h=Math.max(0,v-t),$=Math.max(0,t-y),k=s.timeline_ticks||8,A=Math.min(100,Math.round($/k*100)),x=t>v;pi(s.sector);const T=Ze(s.sector);a+=u,n+=c,l+=`<div class="ap-item" onclick="openProjectModal('${s.id}')">
        ...
        </div>`}i.innerHTML=l,e.style.display=Z.length>0?"":"none",Z.length>0&&(document.getElementById("ap-total-crew").textContent=Z.length,document.getElementById("ap-total-budget").textContent=K(a),document.getElementById("ap-total-spent").textContent=K(n))}async function bi(){if(!f)return;const{data:i,error:e}=await _.from("construction_contracts").select("*, contract_bids!inner(bid_price, material_grades, labor_count, estimated_cost, estimated_quality)").eq("awarded_to_faction",f.id).in("status",["awarded","in_progress"]).eq("contract_bids.faction_id",f.id).order("awarded_at_tick",{ascending:!0});e?(console.warn("Failed to load active projects:",e.message),Z=[]):Z=i||[],gn()}const $i=3e4;function _i(){let i=0,e=0;for(const t of le)for(const a of Ht){const n=U[t.key]?.[a];n&&(i+=n.qty,e+=n.value)}return{totalUnits:i,totalValue:e}}function jt(){const i=document.getElementById("wh-list"),{totalUnits:e,totalValue:t}=_i();document.getElementById("wh-count").textContent=e.toLocaleString()+" UNITS",document.getElementById("wh-total-value").textContent=B(t);const a=Math.round(e/$i*100),n=document.getElementById("wh-capacity");n.textContent=a+"%",n.style.color=a>80?"var(--red)":a>50?"var(--orange)":"var(--green)";let l="";for(let s=0;s<le.length;s++){const d=le[s],r=zt===s,o=U[d.key]?.LOW||{qty:0,value:0},c=U[d.key]?.STD||{qty:0,value:0},m=U[d.key]?.HIGH||{qty:0,value:0},p=o.qty+c.qty+m.qty,u=o.value+c.value+m.value,y=p===0,v=we(d.key,"LOW",w),h=we(d.key,"STD",w),$=we(d.key,"HIGH",w),k=o.qty>0?"wh-dot wh-dot--low":"wh-dot wh-dot--empty",A=c.qty>0?"wh-dot wh-dot--std":"wh-dot wh-dot--empty",x=$.available?m.qty>0?"wh-dot wh-dot--high":"wh-dot wh-dot--empty":"wh-dot wh-dot--locked";if(l+='<div class="wh-row">',l+=`<div class="wh-row__collapsed${r?" expanded":""}" onclick="toggleWhRow(${s})">
            ...
            <span class="wh-row__val${y?" empty":""}">${u>0?B(u):"—"}</span>
                ...
                <span class="wh-expand__hdr-label" style="width:55px;text-align:right">$/UNIT</span>
            </div>`;const T=[{key:"LOW",label:"Low",data:o,avail:v,color:"var(--orange)",dotClass:"wh-dot--low"},{key:"STD",label:"Standard",data:c,avail:h,color:"var(--amber)",dotClass:"wh-dot--std"},{key:"HIGH",label:"High",data:m,avail:$,color:"var(--green)",dotClass:"wh-dot--high"}];for(const C of T){const S=!C.avail.available,E=C.data.qty>0,M=E?"$"+Math.round(C.data.value/C.data.qty):"—";l+=`<div class="wh-grade${S?" unavailable":""}">
                    <div class="wh-grade__label">
                        <div class="wh-dot ${C.dotClass}" style="width:5px;height:5px"></div>
                        <span class="wh-grade__name" style="color:${S?"var(--red)":C.color}">${C.label}</span>
                    ...
                    <span class="wh-grade__val" style="color:${C.data.value>0?"var(--text-muted)":"var(--text-dim)"}">${C.data.value>0?B(C.data.value):"—"}</span>
                    <span class="wh-grade__cpu">${M}</span>
                </div>`}for(const C of T)!C.avail.available&&C.avail.failedStat&&(l+=`<div class="wh-lock">
                        <span class="wh-lock__text">${C.label.toUpperCase()} GRADE LOCKED — ${g(C.avail.failedStat)} &lt; ${C.avail.failedMin}</span>
                    </div>`);l+="</div>"}l+="</div>"}i.innerHTML=l}function xn(i){zt=zt===i?-1:i,jt()}async function bn(){if(!f)return;const{data:i,error:e}=await _.from("corp_warehouse").select("material_key, quality_tier, quantity, total_value").eq("faction_id",f.id);if(U={},e)console.warn("Failed to load warehouse:",e.message);else if(i)for(const t of i)U[t.material_key]||(U[t.material_key]={}),U[t.material_key][t.quality_tier]={qty:t.quantity||0,value:Number(t.total_value)||0};jt()}const $n={manufacturing_output:"base production",rare_minerals:"raw input",inflation:"price modifier",fuel_prices:"transport cost",urbanization:"demand pressure",arable_land:"base production",physical_infrastructure:"transport",digital_infrastructure:"component quality",energy_generation:"electrical supply",standard_of_living:"demand tier",oil_and_gas:"base input",higher_education:"engineering"};function hi(){const i=(w?.name||f?.nation||"—").toUpperCase();document.getElementById("pr-nation-badge").textContent="LOCAL — "+i;const e=Number(f?.corp_cash_reserves)||0;document.getElementById("pr-cash").textContent=B(e);const{totalUnits:t}=_i(),a=Math.round(t/$i*100),n=document.getElementById("pr-wh-capacity");n.textContent=a+"%",n.style.color=a>80?"var(--red)":a>50?"var(--orange)":"var(--green)",wi(),Ut(),it()}function wi(){const i=document.getElementById("pr-mat-grid");let e="";for(const t of le){const a=O===t.key,n=Ht.every(s=>!we(t.key,s,w).available),l="pr-mat-btn"+(a?" active":"")+(n?" all-locked":"");e+=`<span class="${l}" onclick="setPrMat('${t.key}')">${g(t.name)}</span>`}i.innerHTML=e}function Ut(){const i=document.getElementById("pr-tier-bar");let e='<span class="pr-tier-label">GRADE</span>';for(const t of Ht){const a=we(O,t,w),n=D===t,l=a.available?Dt(O,t,w):null,s=di[t],d=!a.available,r="pr-tier-btn"+(n?" active":"")+(d?" locked":"");e+=`<div class="${r}" onclick="${d?"":`setPrTier('${t}')`}">
            <div class="pr-tier-btn__label">
                <div class="wh-dot" style="width:5px;height:5px;background:${s};border-radius:1px;"></div>
                <span class="pr-tier-btn__name" style="color:${n?"var(--text-bright)":"var(--text-dim)"}">${At[t]}</span>
            </div>
        ...
        </div>`}i.innerHTML=e}function it(){const i=document.getElementById("pr-content"),e=we(O,D,w),t=le.find($=>$.key===O);if(!t)return;if(!e.available){i.innerHTML=`<div class="pr-locked-panel">
            <div class="pr-locked-box">
                <div class="pr-locked-box__title">QUALITY TIER UNAVAILABLE</div>
                <div class="pr-locked-box__desc">
                    ${g(t.name)} — ${At[D]} grade
                    is not produced domestically in ${g(w?.name||"—")}.
                </div>
        ...
        </div>`;return}const a=Dt(O,D,w),n=ci(O,D,w),l=a*ke,s=n>3e3?"LOW":n>1e3?"MODERATE":"HIGH",d=s==="LOW"?"var(--green)":s==="MODERATE"?"var(--amber)":"var(--red)",r=Number(w?.inflation??50),o=r>55?"up":r<45?"down":"flat",c=o==="up"?"&#9650;":o==="down"?"&#9660;":"&#8212;",m=o==="up"?"var(--red)":o==="down"?"var(--green)":"var(--text-dim)";let p="";p+=`<div style="padding:8px 14px;border-bottom:1px solid var(--border-0);">
        ...
        <div class="pr-drivers__title">Price Drivers (${g(w?.name||"—")})</div>`;for(const $ of t.priceDrivers){const k=Number(w?.[$]??50),A=k>=50?"var(--green)":k>=30?"var(--amber)":k>=15?"var(--orange)":"var(--red)",x=$n[$]||$;p+=`<div class="pr-driver-row">
        ...
        </div>`}p+="</div>";const y=(Number(f?.corp_cash_reserves)||0)>=l,v=ke>n,h=di[D];p+=`<div class="pr-order">
        ...
        <div class="pr-order__box">
            <div class="pr-order__header">
                <div style="display:flex;align-items:center;gap:6px;">
                    ...
                    <span style="font-family:var(--font-mono);font-size:9px;color:${h}">${At[D]}</span>
                </div>
            ...
            <div class="pr-qty-row">
                ...
                <div class="pr-qty-btns">
                    ${[100,250,500,1e3].map($=>`<span class="pr-qty-btn${ke===$?" active":""}" onclick="setPrQty(${$})">${$>=1e3?$/1e3+"k":$}</span>`).join("")}
                </div>
            </div>
            ...
            <div class="pr-order__total">
                <div>
                    ...
                    <div class="pr-order__total-value">${B(l)}</div>
                </div>
    ...
    </div>`,i.innerHTML=p}function _n(i){O=i,D="STD";for(const e of["STD","HIGH","LOW"])if(we(i,e,w).available){D=e;break}wi(),Ut(),it()}function hn(i){D=i,Ut(),it()}function wn(i){ke=i,it()}let $t=!1;async function kn(){if($t||!f||!w)return;const i=Dt(O,D,w),e=ci(O,D,w),t=i*ke,a=Number(f.corp_cash_reserves)||0;if(t>a){alert("Insufficient cash reserves.");return}if(ke>e){alert("Exceeds available supply this tick.");return}$t=!0;const n=document.querySelector(".pr-purchase-btn");n&&(n.disabled=!0,n.textContent="...");try{const l=a-t,{error:s}=await _.from("factions").update({corp_cash_reserves:l}).eq("id",f.id);if(s)throw s;const d=U[O]?.[D],r=(d?.qty||0)+ke,o=(d?.value||0)+t,{error:c}=await _.from("corp_warehouse").upsert({faction_id:f.id,nation_id:f.nation_id,material_key:O,quality_tier:D,quantity:r,total_value:o,last_purchased_tick:R?.current_tick||null,updated_at:new Date().toISOString()},{onConflict:"faction_id,material_key,quality_tier"});if(c){const{error:m}=await _.from("factions").update({corp_cash_reserves:a}).eq("id",f.id);throw m&&console.error("Cash refund failed after warehouse error:",m.message),c}f.corp_cash_reserves=l,U[O]||(U[O]={}),U[O][D]={qty:r,value:o},jt(),hi(),n&&(n.textContent="PURCHASED",setTimeout(()=>{n.isConnected&&(n.disabled=!1,n.textContent="PURCHASE")},1500))}catch(l){n&&(n.disabled=!1,n.textContent="PURCHASE"),alert("Purchase failed: "+(l.message||"Unknown error"))}finally{$t=!1}}function ki(i){const e=Ee||w;if(!e)return[];const t=Xe(i);if(!t)return[];const a=Wi(i,e),n=[],l=Number(e?.inflation??50),s=Number(e?.fuel_prices??50);Number(e?.manufacturing_output??50);const d=Ee&&w&&Ee.id!==w.id;let r=null;if(d&&(r=Fi(e,w)),a.newAvailable>0){const o=ni(i,e),c=t.basePrice,m=Math.round(c*((l-50)/200)),p=Math.round(c*((s-50)/300));let u=o;const y=[{label:"Base price",value:B(c)},m!==0?{label:`Inflation (${l})`,mod:(m>=0?"+":"")+B(Math.abs(m))}:null,p!==0?{label:`Fuel transport (${s})`,mod:(p>=0?"+":"")+B(Math.abs(p))}:null].filter(Boolean),v=o-c-m-p;if(v!==0&&!d&&y.push({label:"Demand/scarcity",mod:(v>=0?"+":"")+B(Math.abs(v))}),d&&r){const h=Math.round(o*r.tariff),$=Math.round(o*r.transport);u=o+h+$,y.push({label:`Import tariff (${Math.round(r.tariff*100)}%)`,mod:"+"+B(h)}),y.push({label:`Transport (${r.deliveryTicks} tick${r.deliveryTicks>1?"s":""})`,mod:"+"+B($)})}n.push({seller:d?`${e.name} Buyers`:"Local Buyers",sellerType:"LOCAL",nation:e.name||"—",distance:d?r?.deliveryTicks||1:0,condition:100,price:Math.round(u),available:a.newAvailable,delivery:d?r.deliveryTicks+" tick"+(r.deliveryTicks>1?"s":""):"Immediate",deliveryTicks:d?r.deliveryTicks:0,used:!1,priceFactors:y,sourceNationId:e.id})}if(a.usedAvailable>0){const o=a.usedCondition,c=ni(i,e,{used:!0,condition:o});let m=c;const p=[{label:"Base price",value:B(t.basePrice)},{label:`Condition (${o}%)`,mod:"-"+B(Math.max(0,t.basePrice-c))}];if(d&&r){const u=Math.round(c*r.tariff),y=Math.round(c*r.transport);m=c+u+y,p.push({label:`Import tariff (${Math.round(r.tariff*100)}%)`,mod:"+"+B(u)}),p.push({label:`Transport (${r.deliveryTicks} tick${r.deliveryTicks>1?"s":""})`,mod:"+"+B(y)})}n.push({seller:d?`${e.name} Buyers`:"Local Buyers",sellerType:"LOCAL",nation:e.name||"—",distance:d?r?.deliveryTicks||1:0,condition:o,price:Math.round(m),available:a.usedAvailable,delivery:d?r.deliveryTicks+" tick"+(r.deliveryTicks>1?"s":""):"Immediate",deliveryTicks:d?r.deliveryTicks:0,used:!0,priceFactors:p,sourceNationId:e.id})}return n}function nt(){const i=Number(f?.corp_cash_reserves)||0;document.getElementById("em-cash").textContent=B(i);const e=Xe(Q),t=Re[e?.tier||1],a=document.getElementById("em-tier-badge");a&&(a.textContent=t.tag,a.style.color=t.color),a.style.background=t.color+"0a",a.style.border="1px solid "+t.color+"33";const n=document.getElementById("em-nation-select");if(n&&n.options.length===0){const d=w?.name||f?.nation||"—";let r=`<option value="">${g(d)} (HQ)</option>`;for(const o of Ke)o.id!==w?.id&&(r+=`<option value="${o.id}">${g(o.name)}</option>`);n.innerHTML=r}const l=document.getElementById("em-import-tag"),s=Ee&&w&&Ee.id!==w.id;l&&(l.style.display=s?"":"none"),En(),Gt()}function En(){let i="";for(let e=1;e<=3;e++){const t=Re[e],a=St(e),n=e===3?"em-selector__grid em-selector__grid--t3":"em-selector__grid em-selector__grid--t12";i+=`<div class="em-selector__row">
            <div class="em-selector__tier-label" style="color:${t.color}">${t.tag}</div>
            <div class="${n}">`;for(const l of a){const s=Q===l.key,d=ki(l.key).length>0;i+=`<span class="em-selector__btn${s?" active":""}${d?"":" no-listings"}"
                style="${s?"background:"+t.color+";border-color:"+t.color:""}"
                onclick="setEmType('${l.key}')">${g(l.name)}</span>`}i+="</div></div>"}document.getElementById("em-selector").innerHTML=`<div class="em-selector">${i}</div>`}function Gt(){const i=document.getElementById("em-content");if(ve=ki(Q),ve.length===0){i.innerHTML=`<div class="em-no-listings"><div class="em-no-listings__box">
        ...
        </div></div>`;return}ne>=ve.length&&(ne=0);let e="";for(let a=0;a<ve.length;a++){const n=ve[a],l=ne===a,s=n.sellerType==="LOCAL"?"var(--teal)":"#5a8aaa",d=li(n.condition);e+=`<div class="em-listing${l?" selected":""}" style="${l?"border-left-color:"+s:""}" onclick="setEmListing(${a})">`,e+=`<div class="em-listing__row1">
            <div style="display:flex;align-items:center;gap:6px">
            ...
            </div>`),e+="</div>"}const t=ve[ne];if(t){const a=Xe(Q),n=Re[a?.tier||1],l=Math.min(t.available,4),s=t.price*ae,d=(Number(f?.corp_cash_reserves)||0)>=s;e+=`<div class="em-purchase"><div class="em-purchase__box">
            ...
            <div class="em-purchase__qty">
                ...
                <div class="em-purchase__qty-btns">
                    ${Array.from({length:l},(r,o)=>o+1).map(r=>`<span class="em-qty-btn${ae===r?" active":""}" style="${ae===r?"background:"+n.color+";border-color:"+n.color:""}" onclick="setEmQty(${r})">${r}</span>`).join("")}
                </div>
            ...
            <div class="em-purchase__total">
        ...
        </div></div>`}i.innerHTML=e}async function Tn(i){if(!i)Ee=null;else{let t=Ke.find(a=>a.id===i);if(!t)try{const{data:a}=await _.from("nations").select("*").eq("id",i).single();t=a}catch{}Ee=t||null}ne=0,ae=1;const e=document.getElementById("em-nation-select");e&&(e.value=i||""),nt()}function Cn(i){Q=i,ne=0,ae=1,nt()}function In(i){ne=i,ae=1,Gt()}function Mn(i){ae=i,Gt()}let _t=!1;async function Sn(){if(_t)return;const i=ve[ne];if(!i||!f)return;const e=Xe(Q);if(!e)return;const t=ae,a=i.price*t,n=Number(f.corp_cash_reserves)||0;if(a>n){alert("Insufficient cash reserves.");return}if(t>i.available){alert("Not enough units available.");return}const l=document.querySelector(".em-purchase-btn");l&&(l.disabled=!0,l.textContent="..."),_t=!0;try{const s=n-a,{error:d}=await _.from("factions").update({corp_cash_reserves:s}).eq("id",f.id);if(d)throw d;const r=!i.deliveryTicks||i.deliveryTicks===0;if(r){const c=ee.find(A=>A.equipment_key===Q),m=(c?.owned||0)+t,p=c?.purchase_price_avg||0,u=c?.owned||0,y=u>0?Math.round((p*u+i.price*t)/m):i.price,v=e.maintenancePerUnit*m,h=c?.condition||100,$=Math.round((h*u+i.condition*t)/m),{error:k}=await _.from("corp_equipment").upsert({faction_id:f.id,nation_id:f.nation_id,equipment_key:Q,tier:e.tier,owned:m,deployed:c?.deployed||0,condition:$,maintenance_per_tick:v,purchase_price_avg:y,last_purchased_tick:R?.current_tick||null,updated_at:new Date().toISOString()},{onConflict:"faction_id,equipment_key"});if(k){const{error:A}=await _.from("factions").update({corp_cash_reserves:n}).eq("id",f.id);throw A&&console.error("Cash refund failed:",A.message),k}c?(c.owned=m,c.condition=$,c.maintenance_per_tick=v):ee.push({equipment_key:Q,tier:e.tier,owned:m,deployed:0,condition:$,maintenance_per_tick:v,assigned_projects:[]})}else{const c=(R?.current_tick||0)+i.deliveryTicks,{error:m}=await _.from("corp_equipment_deliveries").insert({faction_id:f.id,equipment_key:Q,quantity:t,condition:i.condition,delivery_tick:c,source_nation_id:i.sourceNationId||null,seller_name:i.seller,price_paid:a});if(m){const{error:p}=await _.from("factions").update({corp_cash_reserves:n}).eq("id",f.id);throw p&&console.error("Cash refund failed:",p.message),m}}f.corp_cash_reserves=s,Wt(),nt();const o=document.getElementById("pr-cash");o&&(o.textContent=B(s)),l&&(l.textContent=r?"PURCHASED":"ORDERED",setTimeout(()=>{l.isConnected&&(l.disabled=!1,l.textContent="PURCHASE")},1500))}catch(s){l&&(l.disabled=!1,l.textContent="PURCHASE"),alert("Purchase failed: "+(s.message||"Unknown error"))}finally{_t=!1}}let An=-1,Be=[],Rt=[],Ei=[];function ht(i){return Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(1)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i.toLocaleString()}function zn(i,e,t){if(t)return"var(--orange)";const a=i/(e||1)*100;return a>50?"var(--green)":a>25?"var(--amber)":"var(--red)"}function Ln(){const i=document.getElementById("pm-list"),e=Be.length,t=Rt.length,a=Ei.length,n=Be.filter(r=>r.expiring_soon).length;document.getElementById("pm-active-count").textContent=`(${e})`,document.getElementById("pm-pending-count").textContent=`(${t})`,document.getElementById("pm-apply-count").textContent=`(${a})`;const l=document.getElementById("pm-badges");let s="";n>0&&(s+=`<span class="pm-badge pm-badge--expiring">${n} EXPIRING</span>`),t>0&&(s+=`<span class="pm-badge pm-badge--pending">${t} PENDING</span>`),l.innerHTML=s;const d=Be.reduce((r,o)=>r+(o.cost||0),0)+Rt.reduce((r,o)=>r+(o.cost||0),0);document.getElementById("pm-total-cost").textContent=ht(d),document.getElementById("pm-footer-active").textContent=e,document.getElementById("pm-footer-pending").textContent=t;{if(e===0){i.innerHTML=`<div class="pm-empty">
            ...
            </div>`;return}let r="";Be.forEach((o,c)=>{const m=An===c,p=zn(o.ticks_left,o.total_ticks,o.expiring_soon),u=Math.min(o.ticks_left/(o.total_ticks||1)*100,100);r+=`<div class="pm-item ${o.expiring_soon?"pm-item--expiring":""} ${m?"expanded":""}" onclick="togglePmExpand(${c})">
                <div class="pm-item__inner">
                    ...
                    <div class="pm-detail__row">
                        ...
                        <span class="pm-detail__val">${ht(o.cost||0)}</span>
                    </div>
                    <div class="pm-detail__row">
                    ...
                    </div>`,o.note&&(r+=`<div class="pm-note"><span class="pm-note__text">${g(o.note)}</span></div>`),o.expiring_soon&&o.renewable&&(r+=`<div class="pm-btn-row"><button class="pm-btn pm-btn--renew">RENEW — ${ht(o.cost||0)}</button></div>`),r+="</div>"),r+="</div></div>"}),i.innerHTML=r;return}}function Bn(){Be=[],Rt=[],Ei=[],Ln()}let be=[],qn=-1;function ie(i){return Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(2)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i.toLocaleString()}function si(i){return i>=85?"var(--gold)":i>=60?"var(--green)":i>=40?"var(--orange)":"var(--red)"}function Nn(i){return"dl-result--"+i.toLowerCase()}function ri(){const i=document.getElementById("dl-list"),e=be.length;document.getElementById("dl-count").textContent=`${e} COMPLETED`;const t=be.reduce((d,r)=>{const o=r.financials||{};return d+((o.payment||0)+(o.bonus||0)-(o.penalty||0)-(o.total_cost||0))},0),a=document.getElementById("dl-lifetime-profit");a.textContent=(t>=0?"+":"")+ie(t),a.style.color=t>=0?"var(--green)":"var(--red)";const n={};be.forEach(d=>{n[d.result]=(n[d.result]||0)+1});const l=document.getElementById("dl-footer-results");if(l.innerHTML=Object.entries(n).map(([d,r])=>`<div class="dl-footer__result-box">
        ...
        </div>`;return}let s="";be.forEach((d,r)=>{const o=qn===r,c=d.financials||{},m=(c.payment||0)+(c.bonus||0)-(c.penalty||0)-(c.total_cost||0),p=m>=0,u=Nn(d.result),v={DISTINCTION:"var(--gold)",PASS:"var(--green)",CONDITIONAL:"var(--orange)",FAIL:"var(--red)"}[d.result]||"var(--text-dim)",h=d.type==="GOVERNMENT";if(s+=`<div class="dl-item ${o?"expanded":""}" onclick="toggleDlExpand(${r})">
            <div class="dl-item__inner" style="border-left:2px solid ${v}">
                ...
                <div class="dl-item__row2">
                    ...
                    <span class="dl-item__issuer" style="color:${h?"var(--green)":"var(--gold)"}">${g(d.issuer)}</span>
                ...
                <div class="dl-summary-bar">
                    <div class="dl-summary-cell" style="flex:1;">
                        ...
                        <div style="display:flex;align-items:center;gap:4px;margin-top:2px;">
                            <span class="dl-summary-value" style="color:${si(d.quality_score)}">${d.quality_score}</span>
                ...
                </div>`,o){const $=d.inspection||{};s+='<div style="margin-top:8px;">',s+='<div class="dl-section-label">Inspection Report</div>',["materials","structural","systems"].forEach(T=>{const C=$[T]||{score:0,issues:[]},S=si(C.score),E=Math.min(C.score/100*100,100);s+=`<div class="dl-inspect-row">
                    <div class="dl-inspect-row__header">
                        ...
                        <div class="dl-inspect-row__score-area">
                            <div class="dl-inspect-bar"><div class="dl-inspect-bar__fill" style="width:${E}%;background:${S}"></div></div>
                            <span class="dl-inspect-row__score" style="color:${S}">${C.score}</span>
                        </div>
                    </div>
                    ${(C.issues||[]).map(M=>`<div class="dl-inspect-issue">${g(M)}</div>`).join("")}
            ...
            </div>`,s+="</div>";const x=d.timeline||{};s+=`<div class="dl-timeline">
                <span class="dl-timeline__label">TIMELINE</span>
                <span class="dl-timeline__ticks">${x.actual||0}/${x.expected||0} ticks</span>`,x.early?s+=`<span class="dl-timeline__badge dl-timeline__badge--early">${(x.expected||0)-(x.actual||0)} TICK${x.expected-x.actual!==1?"S":""} EARLY</span>`:!x.on_time&&x.actual>x.expected&&(s+=`<span class="dl-timeline__badge dl-timeline__badge--late">${(x.actual||0)-(x.expected||0)} TICK${x.actual-x.expected!==1?"S":""} LATE</span>`),s+="</div>",s+="</div>"}s+="</div></div>"}),i.innerHTML=s}async function Rn(){if(!f){be=[],ri();return}const{data:i,error:e}=await _.from("construction_deliveries").select("*, construction_contracts(name, sector, issuer_name, issuer_type, timeline_ticks)").eq("faction_id",f.id).order("delivered_at_tick",{ascending:!1}).limit(20);e?(console.warn("Failed to load deliveries:",e.message),be=[]):be=(i||[]).map(t=>{const a=t.construction_contracts||{};return{id:t.contract_id,name:a.name||"Project",type:a.issuer_type||"GOVERNMENT",issuer:a.issuer_name||"Government",delivered:"Tick "+(t.delivered_at_tick||0),result:t.result,quality_score:t.quality_score,rep_change:t.rep_change,financials:{contract_value:t.contract_value||0,bonus:t.quality_bonus||0,penalty:t.penalties||0,payment:t.payment_received||0,total_cost:t.total_cost||0},inspection:t.inspection||{},materials_used:t.materials_used||[],timeline:{expected:t.timeline_expected||0,actual:t.timeline_actual||0,on_time:t.on_time,early:t.timeline_actual<t.timeline_expected}}}),ri()}function Wt(){const i=ee.reduce((d,r)=>d+(r.owned||0),0),e=ee.reduce((d,r)=>d+(r.deployed||0),0),t=Gi(ee),a=i-e;document.getElementById("eq-count").textContent=i+" UNITS",document.getElementById("eq-summary").innerHTML=`
        ...
        <div class="eq-summary__cell">
        ...
        </div>`;const n={};for(const d of ee)n[d.equipment_key]=d;let l="";for(let d=1;d<=3;d++){const r=Re[d],o=St(d),c=Lt===d,m=o.reduce((u,y)=>u+(n[y.key]?.owned||0),0),p=o.reduce((u,y)=>u+(n[y.key]?.deployed||0),0);if(l+=`<div class="eq-tier-hdr" onclick="toggleEqTier(${d})">
            ...
            ${m>0?`<span class="eq-tier-hdr__count">${p}/${m}</span>`:'<span class="eq-tier-hdr__none">NONE OWNED</span>'}
        </div>`,c)for(const u of o){const y=n[u.key],v=y?.owned||0,h=y?.deployed||0,$=y?.condition||0,k=u.maintenancePerUnit*v,A=v-h,x=v>0&&A===0,T=v>0&&$<65,C=li($),S=y?.assigned_projects||[],E=S.length>0?S.map(M=>M.contract_name||"Project").join(", ").slice(0,30):v>0&&h>0?h+" project"+(h>1?"s":""):"—";l+=`<div class="eq-row${v===0?" unowned":""}">`,l+=`<div class="eq-row__top">
                        ...
                        <div class="eq-detail__cell" style="flex:1.2">
                            ...
                            <div class="eq-detail__bar">
                                <div class="eq-detail__bar-track">
                                    <div class="eq-detail__bar-fill" style="width:${$}%;background:${C}"></div>
                                </div>
                                <span class="eq-detail__bar-pct" style="color:${C}">${$}%</span>
                            </div>
                        </div>
                        <div class="eq-detail__cell" style="flex:0.8">
                            <div class="eq-detail__label">ASSIGNED</div>
                            <div class="eq-detail__value" style="color:var(--text-muted)">${g(E)}</div>
                        </div>
                    ...
                    </div>`:l+='<div class="eq-row__hint">Purchase from Equipment Market →</div>',l+="</div>"}}document.getElementById("eq-list").innerHTML=l;const s=[1,2,3].map(d=>{const r=Re[d],o=St(d).reduce((c,m)=>c+(n[m.key]?.owned||0),0);return`<div class="eq-footer__tier-box" style="border-color:${o>0?r.color+"33":"var(--border-0)"};background:${o>0?r.color+"0a":"transparent"}">
        ...
        <div class="eq-footer__tiers">${s}</div>`}function Pn(i){Lt=Lt===i?-1:i,Wt()}async function On(){if(!f)return;const{data:i,error:e}=await _.from("corp_equipment").select("equipment_key, tier, owned, deployed, condition, maintenance_per_tick, assigned_projects").eq("faction_id",f.id);e?(console.warn("Failed to load equipment:",e.message),ee=[]):ee=i||[],Wt()}async function Dn(){const{data:{user:i}}=await _.auth.getUser();if(!i){window.location.href="login.html";return}const{data:e}=await _.from("factions").select("*").or(`id.eq.${i.id},linked_user_id.eq.${i.id}`);Le=(e||[]).filter(o=>o.nation_id);const t=sessionStorage.getItem("active_faction_id");if(f=Le.find(o=>o.id===t)||Le.find(o=>o.faction_type==="corporation")||Le[0],!f){await _.auth.signOut(),window.location.href="login.html";return}if(f.faction_type!=="corporation"){window.location.href="dashboard.html";return}const[a,n]=await Promise.all([f.nation_id?_.from("nations").select("*").eq("id",f.nation_id).single():Promise.resolve({data:null}),_.from("shard").select("current_tick, current_date, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single()]);a.error&&console.warn("Nation load failed:",a.error.message),a.data&&(w=a.data),n.error&&console.warn("Shard load failed:",n.error.message),R=n.data;const l=f.corp_ticker||f.abbreviation||"";if(document.getElementById("corp-logo").textContent=l.slice(0,2)||"—",document.getElementById("corp-name-bar").textContent=f.faction_name||"Unnamed Corp",R){if(document.getElementById("game-date").textContent=R.current_date||"—",document.getElementById("tick-number").textContent=R.current_tick||"—",R.next_tick_at){const c=(Number(R.tick_interval_hours)||8)*36e5,m=new Date(R.next_tick_at).getTime(),u=m-c+c/2;Bt=new Date(u>Date.now()?u:m+c/2),Yi()}const o=document.querySelector("#tick-countdown")?.closest(".tick-item")?.querySelector(".tick-label");o&&(o.textContent="Next Corp Tick")}document.getElementById("corp-name-badge").textContent=(l?"["+l+"]":f.faction_name||"Corp")+" ▾";const s=document.getElementById("topbar-cash");if(s){const o=Number(f.corp_cash_reserves??0),c=o>=1e9?"$"+(o/1e9).toFixed(1)+"B":o>=1e6?"$"+(o/1e6).toFixed(1)+"M":"$"+Math.round(o/1e3)+"k";s.textContent="CASH: "+c}const d=f.action_points??0;document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+d+" AP</span>",document.getElementById("nation-pill").textContent=(w?.name||f.nation||"—").toUpperCase();const r=document.getElementById("corp-faction-dropdown");if(r){let o="";for(const c of Le){const m=c.id===f.id,p=c.faction_type==="corporation"?"CORP":"PARTY",u=c.faction_type==="corporation"?"var(--teal)":"var(--amber)";o+=`<div class="corp-dd-item${m?" active":""}" onclick="switchToFaction('${c.id}', '${c.faction_type}')">
            ...
            </div>`}r.innerHTML=o}await Promise.all([ce(),bi(),bn(),On(),Bn(),Rn()]);try{const{data:o}=await _.from("nations").select("*").order("name");Ke=o||[]}catch{Ke=[]}if(hi(),nt(),Ui(f,w,R),document.getElementById("loading").style.display="none",document.getElementById("page-content").style.display="block",new URLSearchParams(window.location.search).get("tab")==="expansion"){const o=document.querySelector('[data-tab="expansion"]');o&&Ci({preventDefault:()=>{},target:o})}}async function Hn(){await _.auth.signOut(),window.location.href="login.html"}function jn(){const i=document.getElementById("corp-faction-dropdown");i&&i.classList.toggle("open")}function Un(i,e){const t=document.getElementById("corp-faction-dropdown");t&&t.classList.remove("open"),sessionStorage.setItem("active_faction_id",i),e==="corporation"?window.location.href="corp-operations.html":window.location.href="dashboard.html"}document.addEventListener("click",i=>{const e=document.getElementById("faction-switcher"),t=document.getElementById("corp-faction-dropdown");t&&e&&!e.contains(i.target)&&t.classList.remove("open")});document.addEventListener("keydown",i=>{i.key==="Escape"&&et()});window.doLogout=Hn;window.toggleTheme=Qi;window.toggleCorpDropdown=jn;window.switchToFaction=Un;window.setFilter=Ki;window.openContractDetail=ui;window.closeContractDetail=et;window.placeBid=vn;window.toggleWhRow=xn;window.toggleEqTier=Pn;window.switchEmNation=Tn;window.setEmType=Cn;window.setEmListing=In;window.setEmQty=Mn;window.purchaseEquipment=Sn;window.setPrMat=_n;window.setPrTier=hn;window.setPrQty=wn;window.purchaseMaterial=kn;let V={general:0,skilled:0,innovative:0},wt=!1;const Pe=[{id:"general",label:"General Workforce",multiplier:2,color:"#8b9a6b",factionKey:"corp_general_workforce"},{id:"skilled",label:"Skilled Workforce",multiplier:3,color:"#c8a832",factionKey:"corp_skilled_workforce"},{id:"innovative",label:"Innovative Workforce",multiplier:6,color:"#c84",factionKey:"corp_innovative_workforce"}];function Ti(i){const e=Number(w?.minimum_wage??50),t=Number(w?.inflation??50),a=Number(w?.standard_of_living??50),n=e/100*48e3,l=1+(t-50)/100*.5,s=1+(a-50)/100*.5;return Math.round(n*i*l*s)}function b(i){const e=Math.abs(i),t=i<0?"-":"";return e>=1e9?t+"$"+(e/1e9).toFixed(2)+"B":e>=1e6?t+"$"+(e/1e6).toFixed(2)+"M":e>=1e3?t+"$"+(e/1e3).toFixed(1)+"k":t+"$"+e.toLocaleString()}async function Ci(i){i.preventDefault(),document.getElementById("operations-content").style.display="none";const e=document.getElementById("expansion-content");e.style.display="flex",e.style.justifyContent="center",e.style.gap="12px",e.style.alignItems="flex-start",e.style.flexWrap="wrap",document.querySelectorAll(".corp-nav__tab").forEach(t=>t.classList.remove("active")),i.target.classList.add("active"),at(),Vn(),await He(),await Ft(),ot(),await na(),st(),zi(),await sa(),rt()}function Ii(i){i&&i.preventDefault(),document.getElementById("operations-content").style.display="flex",document.getElementById("expansion-content").style.display="none",document.querySelectorAll(".corp-nav__tab").forEach(e=>e.classList.remove("active")),document.querySelector('[data-tab="operations"]')?.classList.add("active")}function Mi(){return Y.reduce((e,t)=>{const a=Number(t.capacity||0),n=Number(t.condition||0)/100;return e+Math.floor(a*n)},0)+500}function Gn(i,e){const t=Pe.find(l=>l.id===i),a=Number(f?.[t.factionKey]??0),n=V[i]+e;if(!(a+n<0)){if(e>0){const l=Pe.reduce((d,r)=>{const o=Number(f?.[r.factionKey]??0),c=r.id===i?n:V[r.id];return d+o+c},0),s=Mi();if(l>s)return}V[i]=n,at()}}function Wn(i){i?V[i]=0:V={general:0,skilled:0,innovative:0},at()}async function Fn(){if(wt||!Object.values(V).some(n=>n!==0))return;let e=0;for(const n of Pe){const l=V[n.id];l>0&&(e+=l*Ti(n.multiplier)*.1)}const t=Number(f?.corp_cash_reserves??0);if(e>t){alert("Insufficient cash reserves. Hiring cost: "+b(e)+", available: "+b(t));return}const a=e>0?`Confirm workforce changes?

Hiring fee: `+b(e)+" (deducted from cash reserves)":`Confirm workforce changes?

Firing workers — no cost.`;if(confirm(a)){wt=!0;try{const n={};for(const d of Pe){const r=Number(f?.[d.factionKey]??0);n[d.factionKey]=Math.max(0,r+V[d.id])}e>0&&(n.corp_cash_reserves=Math.max(0,t-Math.round(e)));const{error:l}=await _.from("factions").update(n).eq("id",f.id);if(l)throw l;Object.assign(f,n),V={general:0,skilled:0,innovative:0};const s=document.getElementById("topbar-cash");if(s){const d=Number(f.corp_cash_reserves??0);s.textContent="CASH: "+(d>=1e6?"$"+(d/1e6).toFixed(1)+"M":"$"+Math.round(d/1e3)+"k")}at()}catch(n){alert("Error: "+n.message)}finally{wt=!1}}}function at(){const i=document.getElementById("hf-card-container");if(!i)return;const e="'JetBrains Mono', monospace",t={bg:"#121210",surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",greenBright:"#5c5",red:"#c55",redDim:"#a44",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},a=Number(w?.minimum_wage??50),n=Number(w?.inflation??50),l=Number(w?.standard_of_living??50),s=a/100*48e3,d=(1+(n-50)/100*.5).toFixed(2),r=(1+(l-50)/100*.5).toFixed(2),o=w?.name||f?.nation||"Nation",c=Object.values(V).some(k=>k!==0),m=Mi();let p=0,u=0,y=0,v=0,h="";for(const k of Pe){const A=Number(f?.[k.factionKey]??0),x=V[k.id],T=A+x,C=Ti(k.multiplier),S=x>0,E=A*C,M=T*C,N=M-E;p+=A,u+=T,y+=E,v+=M;const H=x!==0?S?"rgba(92,204,92,0.02)":"rgba(204,85,85,0.02)":"transparent";h+=`
        <div style="padding:10px 14px;border-bottom:1px solid ${t.border};background:${H};">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                <span style="font-family:${e};font-size:8px;color:${t.dim}">WAGE (MIN × ${k.multiplier}.0 × ${d} × ${r})</span>
                <span style="font-family:${e};font-size:10px;color:${k.color}">${b(C)}/yr</span>
            </div>
            ...
            ${x!==0?`<div style="margin-top:6px;padding:4px 8px;background:${t.bg};border:1px solid ${t.border};display:flex;justify-content:space-between;align-items:center;">
                ...
                <span style="font-family:${e};font-size:10px;font-weight:700;color:${N>0?t.red:t.greenBright}">${N>0?"+":""}${b(N)}/yr</span>
            </div>`:""}
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;${c?"margin-bottom:6px;":""}">
                ...
                <div style="text-align:right;">
                    ...
                    <div style="display:flex;align-items:baseline;gap:4px;justify-content:flex-end;">
                        <span style="font-family:${e};font-size:11px;font-weight:700;color:${t.text}">${b(y)}</span>
                        ${c?`<span style="font-family:${e};font-size:9px;color:${t.dim}">→</span>
                        <span style="font-family:${e};font-size:11px;font-weight:700;color:${$>0?t.red:t.greenBright}">${b(v)}</span>`:""}
                    </div>
                </div>
            </div>
    ...
    </div>`}function Vn(){const i=document.getElementById("wf-summary-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",blue:"#5a9abf",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660",red:"#c55"},a=(w?.name||f?.nation||"Nation").toUpperCase(),n=Number(w?.minimum_wage??50),l=Number(w?.inflation??50),s=Number(w?.standard_of_living??50),d=n/100*48e3,r=1+(l-50)/100*.5,o=1+(s-50)/100*.5,c=[{label:"General Workforce",mult:2,color:t.accent,key:"corp_general_workforce",countColor:t.text},{label:"Skilled Workforce",mult:3,color:t.gold,key:"corp_skilled_workforce",countColor:t.blue},{label:"Innovative Workforce",mult:6,color:t.orange,key:"corp_innovative_workforce",countColor:t.gold}];let m=0,p=0,u="";for(const y of c){const v=Number(f?.[y.key]??0),h=Math.round(d*y.mult*r*o),$=v*h;m+=v,p+=$,u+=`
        ...
        <div style="flex:1;overflow:auto;">
            ...
            <div style="padding:8px 12px;background:${t.card};border-bottom:1px solid ${t.border};">
                ...
                <div style="display:flex;justify-content:space-between;">
                    ...
                    <span style="font-family:${e};font-size:9px;color:${t.text}">${n}/100 → ${b(d)}/yr</span>
                </div>
                <div style="display:flex;justify-content:space-between;margin-top:2px;">
    ...
    </div>`}let Y=[];async function He(){if(!f?.id)return;const{data:i}=await _.from("corp_properties").select("*").eq("faction_id",f.id).eq("is_active",!0).order("type",{ascending:!1}).order("purchased_at_tick",{ascending:!1});Y=i||[]}function je(){const i=document.getElementById("property-card-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",green:"#4a8",red:"#c55",gold:"#c8a832",orange:"#c84",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},a=(w?.name||f?.nation||"Nation").toUpperCase(),n=1+(Number(w?.inflation??50)-50)/100*.3;let l="",s=0,d=0;const r=w?.name||f?.nation||"Home Nation",o=5e7,c=1+(Number(w?.inflation??50)-50)/100*.3,m=.8+Number(w?.stability??50)/100*.4,p=Math.round(o*c*m),u=Math.round(p*.005);s+=p,d+=u,l+=`
    <div style="padding:8px 12px;border-bottom:1px solid ${t.border};">
    ...
    </div>`;for(const y of Y){const v=Je[y.style]||Je.Basic;s+=Number(y.purchase_price||0),d+=Number(y.monthly_maintenance||0),l+=`
        <div style="padding:8px 12px;border-bottom:1px solid ${t.border};">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px;">
            ...
            <div style="display:flex;gap:0;background:${t.card};border:1px solid ${t.border}">
                ...
                <div style="flex:1;padding:3px 6px;border-right:1px solid ${t.border}">
                    ...
                    <div style="font-family:${e};font-size:10px;font-weight:700;color:${t.gold}">${b(y.purchase_price||0)}</div>
                </div>
                <div style="flex:1;padding:3px 6px">
                    <div style="font-family:${e};font-size:7px;color:${t.dim}">MAINT/MO</div>
                    <div style="font-family:${e};font-size:10px;font-weight:700;color:${t.red}">${b(y.monthly_maintenance||0)}</div>
                </div>
            </div>
            ...
            <div style="display:flex;gap:4px;margin-top:6px;">
                <div onclick="propRefurbish('${y.id}')" style="flex:1;padding:3px 0;text-align:center;font-family:${e};font-size:7px;font-weight:700;letter-spacing:0.8px;color:${t.accent};border:1px solid ${t.accent}33;cursor:pointer;">REFURBISH (${b(Math.round((y.purchase_price||0)*.1*n))})</div>
    ...
    <div style="width:380px;height:450px;background:${t.surface};border:1px solid ${t.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
        ...
        <div style="padding:8px 14px;border-top:1px solid ${t.border};background:${t.card};flex-shrink:0;">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;">
                ...
                <span style="font-family:${e};font-size:13px;font-weight:700;color:${t.red}">${b(d)}/mo</span>
            </div>
        </div>
    </div>`}let Me=[],F=null;const Je={Basic:{color:"#6a6660",label:"BASIC"},Modern:{color:"#8b9a6b",label:"MODERN"},Sustainable:{color:"#5c5",label:"SUSTAINABLE"},Innovative:{color:"#c8a832",label:"INNOVATIVE"},Heritage:{color:"#c84",label:"HERITAGE"},Premium:{color:"#ca5",label:"PREMIUM"}};async function Ft(){if(!f?.nation_id)return;const{data:i,error:e}=await _.from("available_properties").select("*").eq("nation_id",f.nation_id).eq("status","available").order("price",{ascending:!0});if(e){console.warn("[Property] Failed to load marketplace:",e.message);return}Me=(i||[]).map(t=>({...t,adjusted_cost:t.price,adjusted_maintenance:t.monthly_maintenance}))}function ot(){const i=document.getElementById("new-property-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",redDim:"#a44",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"};(w?.name||f?.nation||"Nation").toUpperCase();const a=Number(w?.standard_of_living??50),n=Number(w?.gdp_growth??50),l=Number(w?.inflation??50),s=w?.capital||"Capital",d={capital:s,port:s+" Port",industrial:s+" Industrial Zone",suburban:s+" Suburbs",coastal:s+" Coast"};let r="";if(Me.length===0)r=`<div style="padding:20px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">No properties available in this market.<br>Improve GDP Growth and Standard of Living to unlock more.</div>`;else for(let o=0;o<Me.length;o++){const c=Me[o],m=F===o,p=Je[c.style]||Je.Basic,u=d[c.city_template]||s;r+=`
    ...
    <div style="width:380px;height:450px;background:${t.surface};border:1px solid ${t.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
        <div style="padding:8px 14px;border-bottom:1px solid ${t.border};display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">
            ...
            <span style="font-family:${e};font-size:9px;color:${t.dim}">${Me.length} AVAILABLE</span>
        </div>
        ...
        <div style="padding:8px 14px;border-top:1px solid ${t.border};background:${t.card};flex-shrink:0;">
            <div style="display:flex;gap:6px;justify-content:space-between;">
                ...
                <div onclick="npBuyProperty()" style="flex:1;padding:6px 0;text-align:center;font-family:${e};font-size:9px;font-weight:700;letter-spacing:1px;color:${F!==null?"#000":t.dim};background:${F!==null?t.accent:"transparent"};border:1px solid ${F!==null?t.accent:t.border};cursor:${F!==null?"pointer":"default"};opacity:${F!==null?1:.4}">BUY PROPERTY</div>
            </div>
        </div>
    </div>`}function Yn(i){F=F===i?null:i,ot()}let kt=!1;async function Qn(){if(F===null||kt)return;const i=Me[F];if(!i)return;const e=Number(f?.corp_cash_reserves??0);if(i.adjusted_cost>e){alert(`Insufficient cash reserves.
...
This will be deducted from your cash reserves.`)){kt=!0;try{const{error:t}=await _.from("corp_properties").insert({faction_id:f.id,nation_id:f.nation_id,catalog_id:i.catalog_id||null,name:i.name,type:i.type,style:i.style,capacity:i.capacity,purchase_price:i.adjusted_cost,monthly_maintenance:i.adjusted_maintenance,condition:i.condition,city:i.city,purchased_at_tick:Number(document.getElementById("tick-number")?.textContent||0),is_active:!0});if(t)throw t;const a=Math.max(0,e-i.adjusted_cost),{error:n}=await _.from("factions").update({corp_cash_reserves:a}).eq("id",f.id);if(n)throw n;f.corp_cash_reserves=a,i.id&&await _.from("available_properties").update({status:"sold",purchased_by:f.id}).eq("id",i.id);const l=document.getElementById("topbar-cash");l&&(l.textContent="CASH: "+(a>=1e6?"$"+(a/1e6).toFixed(1)+"M":"$"+Math.round(a/1e3)+"k")),F=null,await Ft(),ot(),je(),alert("Property purchased: "+i.name+`

Deducted: `+b(i.adjusted_cost))}catch(t){alert("Purchase failed: "+t.message)}finally{kt=!1}}}const $e={Basic:{costMod:1,maintMod:1,repGain:1,color:"#6a6660",desc:"Functional. No frills."},Modern:{costMod:1.4,maintMod:1.2,repGain:2,color:"#8b9a6b",desc:"Contemporary design. Good retention."},Sustainable:{costMod:1.6,maintMod:.8,repGain:3,color:"#5c5",desc:"Green building. Lower upkeep."},Innovative:{costMod:2,maintMod:1.5,repGain:4,color:"#c8a832",desc:"Cutting edge. Attracts top talent."},Heritage:{costMod:1.3,maintMod:1.4,repGain:2,color:"#c84",desc:"Restored historic. High character."},Premium:{costMod:2.5,maintMod:1.8,repGain:5,color:"#ca5",desc:"Flagship prestige. Maximum reputation."}};let Vt=!1,I={name:"",type:"Office Building",size:2e3,style:"Modern",timeline:36,budgetMod:0},Et=!1;function Si(){const e=1+(Number(w?.inflation??50)-50)/100*.3,t=$e[I.style]?.costMod||1,a=I.type==="Warehouse"?.75:1,n=Math.round(I.size*1e5*e*t*a),l=Math.round(n*(1+I.budgetMod/100)),s=Math.round(l*.007*($e[I.style]?.maintMod||1));return{baseBudget:n,adjusted:l,maint:s,inflMod:e,styleMod:t}}function Kn(){Vt=!0,I={name:"",type:"Office Building",size:2e3,style:"Modern",timeline:36,budgetMod:0},Ai()}function Yt(){Vt=!1,document.getElementById("cp-modal-overlay")?.remove()}function Jn(i,e){I[i]=e,Ai()}async function Xn(){if(!(Et||!I.name.trim())){Et=!0;try{const i=Si(),e=w?.name||f?.nation||"Unknown",t=$e[I.style]?.repGain||1,a=await _.from("shard").select("current_tick, current_date").eq("name","Alpha Shard").single(),n=a.data?.current_tick||0,l=(a.data?.current_date||"").match(/\d{4}/)?.[0]||"2015",{count:s}=await _.from("construction_contracts").select("id",{count:"exact",head:!0}).eq("nation_id",f.nation_id).eq("issuer_type","PRIVATE"),r=`PVT-C${(s||0)+1}-${l}`,{error:o}=await _.from("construction_contracts").insert({nation_id:f.nation_id,template_key:"custom_building",sector:"civil_engineering",name:I.name.trim(),description:`${I.type} (${I.style}) — ${I.size.toLocaleString()} employees, commissioned by ${f.faction_name}`,project_code:r,budget_ceiling:i.adjusted,timeline_ticks:I.timeline,required_materials:(()=>{const c=I.size/1e3,m=I.style,p={Basic:{concrete:1,steel:1,glass:.5,em:1,lumber:1.5,heavy:1,agg:1},Modern:{concrete:1.1,steel:1.1,glass:1.4,em:1.4,lumber:.5,heavy:1.1,agg:1.1},Sustainable:{concrete:.9,steel:.9,glass:.9,em:.9,lumber:1.5,heavy:.9,agg:.9},Innovative:{concrete:1.2,steel:1.2,glass:2,em:2,lumber:.5,heavy:2,agg:1.2},Heritage:{concrete:1.3,steel:1.3,glass:1.3,em:1.3,lumber:2,heavy:1.3,agg:1.3},Premium:{concrete:1.5,steel:1.5,glass:2.5,em:1.5,lumber:1,heavy:1.5,agg:1.5}}[m]||{concrete:1,steel:1,glass:1,em:1,lumber:1,heavy:1,agg:1},u=(y,v)=>Math.max(1,Math.ceil(c*y*v));return{concrete:u(8,p.concrete),steel:u(6,p.steel),glass_facades:u(3,p.glass),em_systems:u(4,p.em),lumber:u(1,p.lumber),heavy_parts:u(2,p.heavy),aggregate:u(3,p.agg)}})(),required_equipment:(()=>{const c=["work_trucks","concrete_mixers"];return I.size>1e3&&c.push("excavators","tower_cranes"),I.size>3e3&&c.push("bulldozers","heavy_haulers"),I.size>8e3&&c.push("pile_drivers"),c})(),required_workforce:{general:Math.ceil(I.size*.08),skilled:Math.ceil(I.size*.03)},status:"open",generated_at_tick:n,bidding_ends_tick:n+3,issuer_type:"PRIVATE",issuer_name:f.faction_name,issuer_faction_id:f.id});if(o)throw o;Yt(),alert(`Construction project submitted!
...
All construction corporations in `+e+" can now bid on this project.")}catch(i){alert("Failed to submit project: "+i.message)}finally{Et=!1}}}function Ai(){if(document.getElementById("cp-modal-overlay")?.remove(),!Vt)return;const i="'JetBrains Mono', monospace",e={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",redDim:"#a44",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},t=Si(),a=w?.name||f?.nation||"Nation",n=$e[I.style]?.repGain||1,l=n>=4?e.gold:n>=3?e.greenBright:n>=2?e.accent:e.dim,s=Object.entries($e).map(([o,c])=>{const m=I.style===o;return`<div onclick="cpSetField('style','${o}')" style="padding:5px 6px;cursor:pointer;text-align:center;background:${m?c.color+"18":"transparent"};border:1px solid ${m?c.color+"44":e.border};">
    ...
    <div style="width:440px;background:${e.surface};border:1px solid ${e.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;max-height:90vh;">
        ...
        <div style="padding:12px 16px;overflow:auto;flex:1;">
            ...
            <div style="margin-bottom:12px;">
                ...
                <div style="margin-top:4px;font-family:${i};font-size:8px;color:${$e[I.style].color}">${$e[I.style].desc}</div>
            </div>
    ...
    </div>`,document.body.appendChild(d);const r=document.getElementById("cp-name-input");r&&r.addEventListener("input",o=>{I.name=o.target.value}),d.addEventListener("click",o=>{o.target===d&&Yt()})}function Zn(){const i=document.getElementById("cp-name-input");if(i&&(I.name=i.value),!I.name.trim()){alert("Please enter a building name.");return}Xn()}window.cpClose=Yt;window.cpSetField=Jn;window.cpSubmitFromModal=Zn;window.npSelect=Yn;window.npBuyProperty=Qn;window.npOpenConstructionModal=Kn;let Ae=!1;async function ea(i){if(Ae)return;const e=Y.find(d=>d.id===i);if(!e)return;const t=1+(Number(w?.inflation??50)-50)/100*.3,a=Math.round((e.purchase_price||0)*.1*t),n=Number(f?.corp_cash_reserves??0);if(a>n){alert("Insufficient cash. Refurbishment costs "+b(a)+" (inflation-adjusted), you have "+b(n));return}if(e.condition>=95){alert("Property is already in excellent condition ("+e.condition+"%).");return}const l=5+Math.floor(Math.random()*21),s=Math.min(100,e.condition+l);if(confirm('Refurbish "'+e.name+`"?

Cost: `+b(a)+`
Expected improvement: +`+l+"% condition ("+e.condition+"% → "+s+"%)")){Ae=!0;try{await _.from("corp_properties").update({condition:s}).eq("id",i);const d=Math.max(0,n-a);await _.from("factions").update({corp_cash_reserves:d}).eq("id",f.id),f.corp_cash_reserves=d;const r=document.getElementById("topbar-cash");r&&(r.textContent="CASH: "+(d>=1e6?"$"+(d/1e6).toFixed(1)+"M":"$"+Math.round(d/1e3)+"k")),await He(),je(),alert("Refurbished! Condition: "+e.condition+"% → "+s+"%")}catch(d){alert("Refurbishment failed: "+d.message)}finally{Ae=!1}}}async function ta(i){if(Ae)return;const e=Y.find(l=>l.id===i);if(!e)return;const t=1+(Number(w?.inflation??50)-50)/100*.3,a=(e.condition||50)/100,n=Math.round((e.purchase_price||0)*.6*a*t);if(confirm('Sell "'+e.name+`"?

Sale value: `+b(n)+" (60% × "+e.condition+`% condition × inflation)

The property will go back on the market for 6 ticks.
This cannot be undone.`)){Ae=!0;try{await _.from("corp_properties").update({is_active:!1}).eq("id",i);const s=Number(f?.corp_cash_reserves??0)+n;await _.from("factions").update({corp_cash_reserves:s}).eq("id",f.id),f.corp_cash_reserves=s;const r=(await _.from("shard").select("current_tick").eq("name","Alpha Shard").single()).data?.current_tick||0;await _.from("available_properties").insert({nation_id:f.nation_id,catalog_id:e.catalog_id||null,name:e.name,type:e.type,style:e.style,capacity:e.capacity,price:Math.round(n*1.1),monthly_maintenance:e.monthly_maintenance,condition:e.condition,city:e.city,generated_at_tick:r,expires_at_tick:r+6,status:"available"});const o=document.getElementById("topbar-cash");o&&(o.textContent="CASH: "+(s>=1e6?"$"+(s/1e6).toFixed(1)+"M":"$"+Math.round(s/1e3)+"k")),await He(),je(),await Ft(),ot(),alert('Sold "'+e.name+'" for '+b(n))}catch(l){alert("Sale failed: "+l.message)}finally{Ae=!1}}}window.propRefurbish=ea;window.propSell=ta;let qe=0;function zi(){const i=document.getElementById("manage-subsidiaries-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",blue:"#5a9abf",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",greenDark:"#2a5a3a",red:"#c55",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},a=Y.filter(r=>r.type==="regional_hq");qe>=a.length&&(qe=0);const n=a[qe]||null;let l="";a.length===0&&(l=`<div style="padding:30px 14px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">No subsidiaries established.<br>Use Create Subsidiary to expand.</div>`);let s=0;for(let r=0;r<a.length;r++){const o=a[r],c=r===qe,m=Number(o.purchase_price||0);s+=m;const p=Oe.find(u=>u.id===o.nation_id)?.name||o.city||"—";l+=`
    ...
    </div>`}async function ia(i){const e=Y.find(a=>a.id===i);if(!e)return;const t=Math.round((e.purchase_price||0)*.6*(e.condition||50)/100);if(confirm('Dissolve subsidiary "'+e.name+`"?

Assets liquidated at 60% × condition: `+b(t)+`
All operations in this nation cease.

This cannot be undone.`))try{await _.from("corp_properties").update({is_active:!1}).eq("id",i);const a=Number(f?.corp_cash_reserves??0)+t;await _.from("factions").update({corp_cash_reserves:a}).eq("id",f.id),f.corp_cash_reserves=a;const n=document.getElementById("topbar-cash");n&&(n.textContent="CASH: "+(a>=1e6?"$"+(a/1e6).toFixed(1)+"M":"$"+Math.round(a/1e3)+"k")),qe=0,await He(),je(),zi(),st(),alert("Subsidiary dissolved. Received: "+b(t))}catch(a){alert("Failed: "+a.message)}}window.subDissolve=ia;let Oe=[],oe=null,Tt=!1;async function na(){const{data:i}=await _.from("nations").select("*").order("name");Oe=(i||[]).filter(e=>e.id!==f?.nation_id)}function Pt(i){const t=u=>Number(i[u]??50),a=t("standard_of_living"),n=t("cost_of_living"),l=t("corporate_tax"),s=t("minimum_wage"),d=t("urbanization"),r=t("union_strength"),o=t("corruption"),c=t("unemployment"),m=t("stability"),p=5e7*(1+(a-50)/100*.4)*(1+(n-50)/100*.3)*(1+(l-50)/100*.2)*(1+(s-50)/100*.15)*(1+(d-50)/100*.1)*(1+(r-50)/100*.1)*(1-(o-50)/100*.15)*(1-(c-50)/100*.1)*(1+(50-m)/100*.3);return Math.round(Math.max(1e7,p))}function aa(i){oe=oe===i?null:i,st()}async function oa(){if(Tt||!oe)return;const i=Oe.find(n=>n.id===oe);if(!i)return;if(Y.find(n=>n.nation_id===i.id&&n.type==="regional_hq")){alert("You already have a subsidiary in "+i.name);return}const t=Pt(i),a=Number(f?.corp_cash_reserves??0);if(t>a){alert("Insufficient cash. Entry cost: "+b(t)+", available: "+b(a));return}if(confirm("Establish subsidiary in "+i.name+`?

Entry cost: `+b(t)+`
Creates a Regional HQ (500 capacity)
Unlocks `+i.name+` for operations

Deducted from cash reserves.`)){Tt=!0;try{const l=(await _.from("shard").select("current_tick").eq("name","Alpha Shard").single()).data?.current_tick||0,s=85+Math.floor(Math.random()*16),d=Math.round(t*.005),{error:r}=await _.from("corp_properties").insert({faction_id:f.id,nation_id:i.id,name:"Regional Headquarters — "+i.name,type:"regional_hq",style:"Modern",capacity:500,purchase_price:t,monthly_maintenance:d,condition:s,city:i.capital||i.name,purchased_at_tick:l,is_active:!0});if(r)throw r;const o=Math.max(0,a-t);await _.from("factions").update({corp_cash_reserves:o}).eq("id",f.id),f.corp_cash_reserves=o;const c=document.getElementById("topbar-cash");c&&(c.textContent="CASH: "+(o>=1e6?"$"+(o/1e6).toFixed(1)+"M":"$"+Math.round(o/1e3)+"k")),oe=null,await He(),je(),renderSubsidiariesCard(),st(),alert("Subsidiary established in "+i.name+`!

Cost: `+b(t)+`
Regional HQ created with `+s+"% condition.")}catch(n){alert("Failed: "+n.message)}finally{Tt=!1}}}function st(){const i=document.getElementById("create-subsidiary-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",blue:"#5a9abf",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},a=new Set(Y.filter(r=>r.type==="regional_hq").map(r=>r.nation_id)),n=Oe.filter(r=>!a.has(r.id)),l=oe?n.find(r=>r.id===oe):null;let s="";for(const r of n){const o=r.id===oe,c=Pt(r),m=Number(r.standard_of_living??50),p=Number(r.stability??50),u=c>6e7?t.red:c>4e7?t.orange:t.greenBright;s+=`
    ...
    <div style="width:380px;height:450px;background:${t.surface};border:1px solid ${t.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
    ...
    </div>`}window.subSelectNation=aa;window.subCreate=oa;let Qe=[],_e=0,X="ALL",ge="REPUTATION";async function sa(){const{data:i}=await _.from("factions").select("id, faction_name, abbreviation, corp_sector, corp_subsector, corp_company_type, corp_ticker, corp_cash_reserves, nation_id, nation, linked_user_id").eq("faction_type","corporation").is("abandoned_at",null).order("faction_name");Qe=(i||[]).map(e=>({...e,abbr:e.corp_ticker||e.abbreviation||e.faction_name?.slice(0,4).toUpperCase()||"???",status:(e.corp_company_type||"Private").toUpperCase(),isPlayer:!!e.linked_user_id,reputation:50,revenue:e.status==="PUBLIC"?Number(e.corp_cash_reserves||0)*.1:null,valuation:e.status==="PUBLIC"?Number(e.corp_cash_reserves||0)*3:null}))}function ra(i){_e=i,rt()}function la(i){X=i,_e=0,rt()}function da(i){ge=i,_e=0,rt()}function rt(){const i=document.getElementById("corporations-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",blue:"#5a8aaa",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},a={PUBLIC:{color:t.greenBright,bg:"rgba(92,204,92,0.06)",border:"rgba(92,204,92,0.15)"},PRIVATE:{color:t.gold,bg:"rgba(200,168,50,0.08)",border:"rgba(200,168,50,0.2)"},STATE:{color:t.orange,bg:"rgba(204,136,68,0.06)",border:"rgba(204,136,68,0.15)"}},n=[...new Set(Qe.map(p=>p.nation).filter(Boolean))];let l=[...Qe];X!=="ALL"&&(l=l.filter(p=>p.nation===X)),ge==="REPUTATION"?l.sort((p,u)=>(u.reputation||0)-(p.reputation||0)):ge==="REVENUE"?l.sort((p,u)=>(u.revenue||0)-(p.revenue||0)):ge==="VALUATION"&&l.sort((p,u)=>(u.valuation||0)-(p.valuation||0)),_e>=l.length&&(_e=0);const s=l[_e]||null,d=s&&s.status==="PRIVATE",r=s&&s.status==="STATE";let o="";l.length===0&&(o=`<div style="padding:30px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">No corporations found.</div>`);for(let p=0;p<l.length;p++){const u=l[p],y=p===_e,v=a[u.status]||a.PRIVATE,h=u.status==="PRIVATE";o+=`
        ...
        </div>`}let c="";if(s){const p=a[s.status]||a.PRIVATE,u=[{label:"Sector",value:s.corp_sector||"—",color:t.text},{label:"Subsector",value:s.corp_subsector||"—",color:t.accent},{label:"Reputation",value:s.reputation+"/100",color:s.reputation>=70?t.greenBright:s.reputation>=40?t.accent:t.yellow},{label:"Revenue",value:d?"UNDISCLOSED":b(s.revenue),color:d?t.dim:t.greenBright},{label:"Cash Reserves",value:d?"UNDISCLOSED":b(s.corp_cash_reserves||0),color:d?t.dim:t.text},{label:"Market Valuation",value:d?"UNDISCLOSED":b(s.valuation),color:d?t.dim:t.gold}];c=`
        <div style="padding:8px 14px;border-bottom:1px solid ${t.border};background:${t.card};flex-shrink:0;">
    ...
    </div>`}window.corpSelect=ra;window.corpFilterNation=la;window.corpSort=da;let te=null,se={},j=120,re=15;function Li(i,e){const t=e.required_workforce||{},a=Number(t.general||0)+Number(t.skilled||0)||100,n=Math.max(40,Math.round(a*.5)),l=a*2,s=Math.max(0,Math.min(1,(i-n)/(l-n||1))),d=(a-n)/(l-n||1),r=Number(e.timeline_ticks||36);let o;s<=d?o=Math.round(5*(1-s/d)):o=Math.round(-4*((s-d)/(1-d)));const c=Math.max(Math.round(r*.6),r+o);return{delta:o,estTicks:c}}let Ot={},Se=[];async function ca(){if(!de)return;te=de,Ot={};try{const{data:t}=await _.from("corp_material_inventory").select("material_key, quantity").eq("faction_id",f.id);for(const a of t||[])Ot[a.material_key]=Number(a.quantity||0)}catch{}Se=[];try{const{data:t}=await _.from("contract_bids").select("faction_id, bid_price, estimated_quality, status, factions(faction_name, corp_ticker)").eq("contract_id",te.id).in("status",["pending","won"]);Se=(t||[]).filter(a=>a.faction_id!==f?.id).map(a=>({name:a.factions?.faction_name||"Unknown",ticker:a.factions?.corp_ticker||"???",price:Number(a.bid_price||0),quality:Number(a.estimated_quality||0),status:a.status}))}catch{}se={};const i=te.required_materials||{};for(const t of Object.keys(i))se[t]="STD";const e=te.required_workforce||{};j=Number(e.general||0)+Number(e.skilled||0)||120,re=15,et(),lt()}function Qt(){document.getElementById("bid-assembly-overlay")?.remove(),te=null}function pa(i,e){se[i]=e,lt()}function ma(i){j=i,lt()}function fa(i){re=i,lt()}function lt(){if(document.getElementById("bid-assembly-overlay")?.remove(),!te)return;const i="'JetBrains Mono', monospace",e={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",accentBright:"#a3b07e",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",redDim:"#a44",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},t=te,a=t.issuer_type==="GOVERNMENT",n=w?.name||f?.nation||"—",l=Number(t.budget_ceiling||0),s=Number(t.timeline_ticks||8),d=t.required_materials||{},r=Object.keys(d),o={LOW:.5,STD:1,HIGH:2},c={LOW:e.orange,STD:e.yellow,HIGH:e.greenBright},m={LOW:"Low",STD:"Standard",HIGH:"High"},p={concrete:36e4,steel:5e5,glass_facades:56e4,em_systems:64e4,lumber:24e4,heavy_parts:8e5,aggregate:16e4,asphalt:28e4},u=Ot||{};let y=0,v="";for(const z of r){const P=Number(d[z]||0),Xt=se[z]||"STD",Zt=p[z]||3e5,Ri=o[Xt],Pi=Math.round(Zt*Ri),ei=P*Pi;y+=ei;const Oi=z.replace(/_/g," ").replace(/\b\w/g,ue=>ue.toUpperCase()),ti=Number(u[z]||0),ut=Math.max(0,P-ti),Di=ut===0?e.greenBright:ut<P?e.yellow:e.red,Hi=ut===0?"✓ IN STOCK":`${ti}/${P}`;v+=`
        <div style="display:flex;align-items:center;padding:5px 14px;border-bottom:1px solid ${e.border};">
            <div style="flex:1.2">
                <span style="font-size:11px;color:${e.text}">${Oi}</span>
                <div style="font-family:${i};font-size:7px;color:${Di};margin-top:1px">${Hi}</div>
            </div>
            ...
            <div style="flex:1.2;display:flex;gap:2px;justify-content:center;">
                ${["LOW","STD","HIGH"].map(ue=>{const yt=Xt===ue,ii=c[ue],ji=b(Math.round(Zt*o[ue]));return`<span onclick="bidSetGrade('${z}','${ue}')" style="padding:2px 6px;font-family:${i};font-size:7px;font-weight:700;cursor:pointer;color:${yt?"#000":e.dim};background:${yt?ii:"transparent"};border:1px solid ${yt?ii:e.border}" title="${ji}/unit">${m[ue]}</span>`}).join("")}
            </div>
            <div style="flex:0.8;text-align:right"><span style="font-family:${i};font-size:10px;color:${e.text}">${b(ei)}</span></div>
        </div>`}const h=t.required_workforce||{},$=Number(h.general||0)+Number(h.skilled||0)||100,k=Math.max(40,Math.round($*.5)),A=$*2,x=[k,Math.round($*.75),$,Math.round($*1.5),A],{delta:T,estTicks:C}=Li(j,t),S=T>0?`+${T}mo`:T<0?`${T}mo`:"On schedule",E=T>0?e.red:T<0?e.greenBright:e.yellow,M=15200,N=j*M*C,H=l,Ue=[{name:"Municipal Zoning Approval",cost:18e4,ticks:2,required:!0},{name:"Structural Engineering Cert.",cost:24e4,ticks:3,required:!0},{name:"Environmental Impact Assessment",cost:34e4,ticks:8,required:H>2e7},{name:"Seismic Resilience Compliance",cost:21e4,ticks:4,required:H>5e7},{name:"Heritage Conservation Review",cost:16e4,ticks:6,required:!1},{name:"Fire Safety Certification",cost:12e4,ticks:2,required:H>1e7}].filter(z=>z.required),Ge=new Set(["Municipal Zoning Approval","Structural Engineering Cert."]),We=Ue.filter(z=>!Ge.has(z.name)).reduce((z,P)=>z+P.cost,0),ct=4e5,pt=y+N+We+ct,Kt=Math.round(pt*(re/100)),Fe=pt+Kt,W=Fe>l,mt=Kt,me=W?0:Math.max(0,Math.min(100,Math.round(100-Fe/l*100+30))),Jt=me>70?e.greenBright:me>40?e.yellow:me>0?e.orange:e.red,qi=W?"OVER CEILING":me>70?"STRONG":me>40?"COMPETITIVE":me>20?"WEAK":"UNLIKELY",ft=Object.values(se),fe=ft.length>0?Math.round(ft.reduce((z,P)=>z+(P==="HIGH"?85:P==="STD"?65:45),0)/ft.length):50,Ve=fe>=75?e.greenBright:fe>=55?e.yellow:e.orange,Ni=fe>=75?"STRONG":fe>=55?"PROMISING":"UNCERTAIN",Ce=document.createElement("div");Ce.id="bid-assembly-overlay",Ce.style.cssText="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.7);display:flex;align-items:center;justify-content:center;z-index:9999;",Ce.addEventListener("click",z=>{z.target===Ce&&Qt()}),Ce.innerHTML=`
    <div style="width:740px;max-height:94vh;background:${e.surface};border:1px solid ${e.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
        ...
        <div style="padding:10px 16px;border-bottom:1px solid ${e.border};background:${e.card};flex-shrink:0;">
            ...
            <div style="display:flex;align-items:center;gap:8px;margin-top:4px;">
                ...
                <span style="font-family:${i};font-size:9px;color:${e.dim}">·</span>
                <span style="font-family:${i};font-size:9px;color:${e.muted}">Ceiling: <span style="color:${e.text};font-weight:700">${b(l)}</span></span>
                <span style="font-family:${i};font-size:9px;color:${e.dim}">·</span>
        ...
        <div style="flex:1;display:flex;overflow:hidden;">
            ...
            <div style="flex:1;border-right:1px solid ${e.border};overflow:auto;">
                ...
                <div style="display:flex;justify-content:space-between;padding:5px 14px;border-bottom:1px solid ${e.border};background:rgba(139,154,107,0.02);">
                    ...
                    <span style="font-family:${i};font-size:11px;font-weight:700;color:${e.text}">${b(y)}</span>
                </div>
                ...
                <div style="padding:8px 14px;border-bottom:1px solid ${e.border};">
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
                        ...
                        <div style="display:flex;gap:3px;">
                            ${x.map(z=>`<span onclick="bidSetWorkers(${z})" style="padding:2px 8px;font-family:${i};font-size:8px;font-weight:700;cursor:pointer;color:${j===z?"#000":e.dim};background:${j===z?e.accent:"transparent"};border:1px solid ${j===z?e.accent:e.border}">${z}</span>`).join("")}
                        </div>
                    </div>
                    <div style="display:flex;justify-content:space-between;">
                        <span style="font-family:${i};font-size:8px;color:${e.dim}">${j} × $${M.toLocaleString()}/tick × ${C} ticks</span>
                        <span style="font-family:${i};font-size:11px;font-weight:700;color:${e.text}">${b(N)}</span>
                    </div>
                    <div style="display:flex;justify-content:space-between;margin-top:6px;padding-top:6px;border-top:1px solid ${e.border};">
                        ...
                        <span style="font-family:${i};font-size:10px;font-weight:700;color:${E}">${C}mo <span style="font-size:8px;opacity:0.7">(${S})</span></span>
                    </div>
                </div>
                ...
                ${Ue.map(z=>{const P=Ge.has(z.name);return`<div style="display:flex;align-items:center;padding:4px 14px;border-bottom:1px solid ${e.border};">
                        <div style="flex:1;display:flex;align-items:center;gap:6px;">
                            <span style="font-family:${i};font-size:8px;font-weight:700;color:${P?e.greenBright:e.orange}">${P?"✓":"○"}</span>
                            <span style="font-size:10px;color:${P?e.muted:e.text}">${z.name}</span>
                        </div>
                        ${P?`<span style="font-family:${i};font-size:8px;color:${e.greenBright}">HELD</span>`:`<div style="text-align:right">
                                <span style="font-family:${i};font-size:9px;color:${e.redDim}">${b(z.cost)}</span>
                ...
                <div style="display:flex;justify-content:space-between;padding:5px 14px;border-bottom:1px solid ${e.border};background:rgba(139,154,107,0.02);">
                    ...
                    <span style="font-family:${i};font-size:11px;font-weight:700;color:${e.text}">${b(We)}</span>
                </div>
                ...
                <div style="display:flex;justify-content:space-between;padding:5px 14px;border-bottom:1px solid ${e.border};">
                    ...
                    <span style="font-family:${i};font-size:11px;font-weight:700;color:${e.text}">${b(ct)}</span>
                </div>
            </div>
            ...
            <div style="width:280px;display:flex;flex-direction:column;overflow:auto;">
                ...
                ${[{l:"Materials",v:y},{l:"Labor",v:N},{l:"Permits",v:We},{l:"Overhead",v:ct}].map(z=>`<div style="display:flex;justify-content:space-between;padding:4px 14px;border-bottom:1px solid ${e.border};">
                    <span style="font-size:10px;color:${e.muted}">${z.l}</span>
                    <span style="font-family:${i};font-size:10px;color:${e.redDim}">${b(z.v)}</span>
                </div>`).join("")}
                <div style="display:flex;justify-content:space-between;padding:6px 14px;border-bottom:1px solid ${e.border};background:rgba(204,85,85,0.03);">
                    ...
                    <span style="font-family:${i};font-size:13px;font-weight:700;color:${e.red}">${b(pt)}</span>
                </div>
                ...
                <div style="padding:10px 14px;border-bottom:1px solid ${e.border};background:${W?"rgba(204,85,85,0.04)":"rgba(200,168,50,0.03)"};">
                    ...
                    <div style="font-family:${i};font-size:22px;font-weight:700;color:${W?e.red:e.gold}">${b(Fe)}</div>
                    ${W?`<div style="font-family:${i};font-size:8px;font-weight:700;color:${e.red};margin-top:4px;padding:2px 6px;background:rgba(204,85,85,0.08);border:1px solid rgba(204,85,85,0.15);display:inline-block">EXCEEDS BUDGET CEILING (${b(l)})</div>`:""}
                </div>

                <div style="padding:8px 14px;border-bottom:1px solid ${e.border};">
                    <div style="display:flex;justify-content:space-between;">
                        <span style="font-family:${i};font-size:8px;color:${e.dim}">PROJECTED PROFIT</span>
                        <span style="font-family:${i};font-size:14px;font-weight:700;color:${mt>0?e.greenBright:e.dim}">+${b(mt)}</span>
                    </div>
                </div>
                ...
                <div style="padding:8px 14px;border-bottom:1px solid ${e.border};">
                    <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
                        ...
                        <span style="font-family:${i};font-size:11px;font-weight:700;color:${Jt}">${qi}</span>
                    </div>
                    <div style="width:100%;height:4px;background:${e.border}"><div style="width:${me}%;height:100%;background:${Jt}"></div></div>
                </div>
                <div style="padding:8px 14px;border-bottom:1px solid ${e.border};">
                    <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
                        <span style="font-family:${i};font-size:8px;color:${e.dim}">QUALITY ESTIMATE</span>
                        <div style="display:flex;align-items:baseline;gap:4px;">
                            <span style="font-family:${i};font-size:11px;font-weight:700;color:${Ve}">${fe}</span>
                            <span style="font-family:${i};font-size:8px;color:${e.dim}">/100</span>
                            <span style="font-family:${i};font-size:8px;font-weight:700;color:${Ve}">${Ni}</span>
                        </div>
                    </div>
                    <div style="width:100%;height:4px;background:${e.border}"><div style="width:${fe}%;height:100%;background:${Ve}"></div></div>
                ...
                <div style="padding:6px 14px;border-bottom:1px solid ${e.border};">
                    ...
                    ${Se.length===0?`<div style="font-family:${i};font-size:8px;color:${e.dim};padding:4px 0;">No competing bids yet. First mover advantage.</div>`:`<div style="display:flex;gap:4px;flex-wrap:wrap;">
                            ${Se.map(z=>`<span style="padding:2px 6px;font-family:${i};font-size:7px;color:${e.muted};background:${e.card};border:1px solid ${e.border};">${z.name} <span style="color:${e.dim}">Q:${z.quality}</span></span>`).join("")}
                        </div>
                        <div style="font-family:${i};font-size:7px;color:${e.dim};margin-top:3px">${Se.length} competing bid${Se.length!==1?"s":""}. Bid prices are sealed until resolution.</div>`}
        ...
        <div style="padding:10px 16px;border-top:1px solid ${e.border};background:${e.card};display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">
            <div style="display:flex;gap:12px;">
                <div><div style="font-family:${i};font-size:7px;color:${e.dim}">YOUR BID</div><div style="font-family:${i};font-size:14px;font-weight:700;color:${W?e.red:e.gold}">${b(Fe)}</div></div>
                <div><div style="font-family:${i};font-size:7px;color:${e.dim}">EST. PROFIT</div><div style="font-family:${i};font-size:14px;font-weight:700;color:${e.greenBright}">+${b(mt)}</div></div>
                <div><div style="font-family:${i};font-size:7px;color:${e.dim}">QUALITY</div><div style="font-family:${i};font-size:14px;font-weight:700;color:${Ve}">${fe}</div></div>
            </div>
    ...
    </div>`,document.body.appendChild(Ce)}let ze=!1;async function ua(){if(ze||!te)return;const i=te,e=i.required_materials||{},t=Object.keys(e),a=Number(i.budget_ceiling||0);Number(i.timeline_ticks||8);const n={concrete:36e4,steel:5e5,glass_facades:56e4,em_systems:64e4,lumber:24e4,heavy_parts:8e5,aggregate:16e4,asphalt:28e4},l={LOW:.5,STD:1,HIGH:2};let s=0;for(const x of t){const T=Number(e[x]||0),C=se[x]||"STD",S=n[x]||3e5;s+=T*Math.round(S*l[C])}const d=15200,{estTicks:r}=Li(j,i),o=j*d*r,c=a,m=[{name:"Municipal Zoning Approval",cost:18e4,required:!0},{name:"Structural Engineering Cert.",cost:24e4,required:!0},{name:"Environmental Impact Assessment",cost:34e4,required:c>2e7},{name:"Seismic Resilience Compliance",cost:21e4,required:c>5e7},{name:"Fire Safety Certification",cost:12e4,required:c>1e7}],p=new Set(["Municipal Zoning Approval","Structural Engineering Cert."]),u=m.filter(x=>x.required&&!p.has(x.name)).reduce((x,T)=>x+T.cost,0),v=s+o+u+4e5,h=Math.round(v*(re/100)),$=v+h;if($>a){alert("Bid exceeds budget ceiling. Reduce costs or markup.");return}const k=Object.values(se),A=k.length>0?Math.round(k.reduce((x,T)=>x+(T==="HIGH"?85:T==="STD"?65:45),0)/k.length):50;if(confirm('Submit bid for "'+i.name+`"?
...
Once submitted, your bid cannot be changed.`)){ze=!0;try{const{data:x}=await _.from("shard").select("current_tick").eq("name","Alpha Shard").single(),T=x?.current_tick||0,C={};for(const E of t)C[E]=se[E]||"STD";const{error:S}=await _.from("contract_bids").insert({contract_id:i.id,faction_id:f.id,bid_price:$,material_grades:C,labor_count:j,markup_pct:re,estimated_cost:v,estimated_quality:A,status:"pending",submitted_at_tick:T});if(S)throw S;i.status==="open"&&await _.from("construction_contracts").update({status:"bidding"}).eq("id",i.id).eq("status","open"),Qt(),alert(`Bid submitted successfully!
...
Lowest qualified bid wins.`),typeof ce=="function"&&await ce()}catch(x){alert("Bid submission failed: "+x.message)}finally{ze=!1}}}window.openBidAssembly=ca;window.closeBidAssembly=Qt;window.bidSetGrade=pa;window.bidSetWorkers=ma;window.bidSetMarkup=fa;window.submitBidAssembly=ua;let De=[],he=0,J=null,Ct=!1,It=!1,Mt=!1;async function ya(){if(!de||It)return;It=!0,J=de,he=0;const{data:i,error:e}=await _.from("contract_bids").select("*, factions(faction_name, corp_ticker, corp_subsector)").eq("contract_id",J.id).in("status",["pending","won"]).order("bid_price",{ascending:!0});if(It=!1,e){alert("Failed to load bids: "+e.message);return}De=(i||[]).map(t=>({...t,corp:t.factions?.faction_name||"Unknown",abbr:t.factions?.corp_ticker||"???",subsector:t.factions?.corp_subsector||"—"})),et(),Bi()}function dt(){document.getElementById("bid-review-overlay")?.remove(),J=null}function va(i){he=i,Bi()}async function ga(){if(Ct||De.length===0)return;const i=De[he];if(!(!i?.id||!i.faction_id)&&confirm("Accept bid from "+i.corp+`?

Bid Price: `+b(i.bid_price)+`
Quality: `+i.estimated_quality+`/100
Workers: `+i.labor_count+`

This will award the contract. The project begins immediately.`)){Ct=!0;try{const{data:e}=await _.from("shard").select("current_tick").eq("name","Alpha Shard").single(),t=e?.current_tick||0,{error:a}=await _.from("contract_bids").update({status:"won"}).eq("id",i.id);if(a)throw a;const{error:n}=await _.from("contract_bids").update({status:"lost"}).eq("contract_id",J.id).neq("id",i.id);if(n)throw n;const{error:l}=await _.from("construction_contracts").update({status:"awarded",awarded_to_faction:i.faction_id,awarded_at_tick:t}).eq("id",J.id);if(l)throw l;dt(),alert("Contract awarded to "+i.corp+`!

Bid: `+b(i.bid_price)+`
Project begins immediately.`),typeof ce=="function"&&await ce()}catch(e){alert("Failed to accept bid: "+(e.message||e))}finally{Ct=!1}}}async function xa(){if(!(!J||Mt)&&confirm(`Decline all bids and cancel this project?

No bids will be accepted. The contract will be removed.`)){Mt=!0;try{const{error:i}=await _.from("contract_bids").update({status:"lost"}).eq("contract_id",J.id);if(i)throw i;const{error:e}=await _.from("construction_contracts").update({status:"expired"}).eq("id",J.id);if(e)throw e;dt(),alert("All bids declined. Contract cancelled."),typeof ce=="function"&&await ce()}catch(i){alert("Failed: "+(i.message||i))}finally{Mt=!1}}}function Bi(){if(document.getElementById("bid-review-overlay")?.remove(),!J||De.length===0)return;const i="'JetBrains Mono', monospace",e={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",accentBright:"#a3b07e",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},t=J,a=De;he>=a.length&&(he=0);const n=a[he],l=Number(t.budget_ceiling||0),s=Number(t.timeline_ticks||36),d=Math.min(...a.map(u=>u.bid_price)),r=Math.max(...a.map(u=>u.estimated_quality||0));let o="";for(let u=0;u<a.length;u++){const y=a[u],v=u===he,h=y.bid_price===d,$=(y.estimated_quality||0)===r,k=y.bid_price>l;o+=`
        <div onclick="reviewSelectBid(${u})" style="padding:10px 16px;border-bottom:1px solid ${e.border};cursor:pointer;border-left:2px solid ${v?e.accent:"transparent"};background:${v?"rgba(139,154,107,0.03)":"transparent"};">
            ...
            <div style="display:flex;gap:0;background:${e.card};border:1px solid ${e.border};">
                <div style="flex:1.2;padding:5px 10px;border-right:1px solid ${e.border}">
                    ...
                    <div style="font-family:${i};font-size:14px;font-weight:700;color:${k?e.red:e.text}">${b(y.bid_price)}</div>
    ...
    <div style="width:640px;max-height:92vh;background:${e.surface};border:1px solid ${e.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
        <div style="padding:10px 16px;border-bottom:1px solid ${e.border};background:${e.card};flex-shrink:0;">
            ...
            <div style="display:flex;gap:8px;margin-top:4px;font-family:${i};font-size:9px;color:${e.dim};">
                ...
                <span>·</span>
                <span>Budget: <span style="color:${e.text};font-weight:700">${b(l)}</span></span>
                <span>·</span>
        ...
        <div style="padding:6px 16px;border-bottom:1px solid ${e.border};display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">
            ...
            <div style="display:flex;gap:8px;font-family:${i};font-size:8px;color:${e.dim};">
                <span>Cheapest: <span style="color:${e.greenBright}">${b(d)}</span></span>
        ...
        <div style="flex:1;display:flex;overflow:hidden;">
            ...
            <div style="width:250px;display:flex;flex-direction:column;overflow:auto;">
                ...
                ${[{l:"Materials",v:Number(n.estimated_cost||0)*.45},{l:"Labor",v:Number(n.estimated_cost||0)*.45},{l:"Overhead",v:Number(n.estimated_cost||0)*.1}].map(u=>`<div style="display:flex;justify-content:space-between;padding:4px 14px;border-bottom:1px solid ${e.border};">
                    ...
                    <span style="font-family:${i};font-size:10px;color:${e.muted}">${b(Math.round(u.v))}</span>
                </div>`).join("")}
                <div style="display:flex;justify-content:space-between;padding:6px 14px;border-bottom:1px solid ${e.border};background:${c?"rgba(204,85,85,0.03)":"rgba(200,168,50,0.03)"};">
                    ...
                    <span style="font-family:${i};font-size:14px;font-weight:700;color:${c?e.red:e.gold}">${b(n.bid_price)}</span>
                </div>
                <div style="padding:6px 14px;border-bottom:1px solid ${e.border};">
                    <div style="display:flex;justify-content:space-between;margin-bottom:3px;">
        ...
        <div style="padding:10px 16px;border-top:1px solid ${e.border};background:${e.card};display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">
            <div style="display:flex;gap:12px;">
                <div><div style="font-family:${i};font-size:7px;color:${e.dim}">SELECTED BID</div><div style="font-family:${i};font-size:12px;font-weight:700;color:${e.gold}">${b(n.bid_price)}</div></div>
    ...
    </div>`,document.body.appendChild(p)}window.openBidReview=ya;window.closeBidReview=dt;window.reviewSelectBid=va;window.acceptBid=ga;window.declineAllBids=xa;window.switchToExpansion=Ci;window.switchToOperations=Ii;window.hfSetChange=Gn;window.hfReset=Wn;window.hfConfirm=Fn;document.querySelector('[data-tab="operations"]')?.addEventListener("click",function(i){this.classList.contains("active")||(i.preventDefault(),Ii(i))});Dn();
