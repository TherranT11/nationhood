import{_ as F}from"./supabase-client-BXEzLDpS.js";import{t as La,e as u}from"./utils-C2W-HleY.js";let B=[],c=null,P=null;function n(s){return Math.abs(s)>=1e6?"$"+(s/1e6).toFixed(1)+"M":Math.abs(s)>=1e3?"$"+(s/1e3).toFixed(0)+"k":"$"+Math.round(s).toLocaleString()}const Ca={LOAN:{class:"loan",label:"LOAN"},INSURE:{class:"insure",label:"INSURE"},BOND:{class:"bond",label:"BOND"}},Na={LOW:{class:"df-risk-low",label:"LOW RISK"},MODERATE:{class:"df-risk-moderate",label:"MODERATE"},ELEVATED:{class:"df-risk-elevated",label:"ELEVATED"},HIGH:{class:"df-risk-high",label:"HIGH RISK"}},h=[{type:"LOAN",applicant:"Salazar Construction",abbr:"SZC",entity:"CORP",nation:"Melizea",amount:18e6,purpose:"Equipment acquisition — Heavy Infrastructure expansion. 2 Tower Cranes, 3 Heavy Haulers.",term:24,reputation:65,revenue:474e5,debt:22e6,risk:"MODERATE",isNew:!0},{type:"BOND",applicant:"Republic of Melizea",abbr:"MEL",entity:"GOV",nation:"Melizea",amount:1e8,purpose:"National Infrastructure Bond — Series 2013-B. Funding highway and rail expansion over 5 years.",term:60,couponRate:6.2,stability:23,debtToGdp:42,creditRating:38,risk:"ELEVATED",isNew:!0},{type:"INSURE",applicant:"Constructora del Sur",abbr:"CDS",entity:"CORP",nation:"Melizea",amount:12e6,purpose:"Project liability insurance — San Maria Water Treatment Facility. Coverage for construction defects, delays, and third-party claims.",term:18,reputation:52,projectValue:285e5,risk:"MODERATE",isNew:!1},{type:"LOAN",applicant:"Torres & Vega Group",abbr:"TVG",entity:"CORP",nation:"Melizea",amount:5e6,purpose:"Working capital — bridge financing to cover material procurement pending contract payment.",term:6,reputation:34,revenue:148e5,debt:22e5,risk:"ELEVATED",isNew:!1},{type:"INSURE",applicant:"McKenna Construction",abbr:"MKAV",entity:"CORP",nation:"Avelia",amount:8e6,purpose:"Fleet insurance — 32 units of construction equipment across all active project sites.",term:12,reputation:38,fleetValue:164e5,risk:"LOW",isNew:!1},{type:"BOND",applicant:"Republic of Sangreza",abbr:"SNG",entity:"GOV",nation:"Sangreza",amount:45e6,purpose:"Emergency Fiscal Bond — Series 2013-A. Covering budget shortfall from commodity price collapse.",term:36,couponRate:9.8,stability:44,debtToGdp:68,creditRating:24,risk:"HIGH",isNew:!0}];let Z="ALL",z=-1;function ba(s){if(!c)return!1;const a=(c.corp_subsector||"").toLowerCase(),r=Ia[a];return s.type===r}function Ta(s){Z=s,z=-1,da()}function Sa(s){z=z===s?-1:s,da()}function da(){const s=document.getElementById("df-container");if(!s)return;const a=Z==="ALL"?h:h.filter(l=>l.type===Z),r=h.filter(l=>l.isNew).length,p=h.length;let o=`<div class="df-header">
        ...
        <div style="display:flex;gap:4px;align-items:center;">
            ...
            <div style="flex:0.8;text-align:center;">
                ...
                </div>`}if(O){if(e.type==="LOAN"){const g=Math.round(e.debt/e.revenue*100),E=g>50?"#c84":"#5c5",D=e.debt>e.revenue*.5?"#c84":"#9e9a92";o+=`<div class="df-detail-row"><span class="df-detail-label">ANNUAL REVENUE</span><span class="df-detail-value" style="color:#9e9a92;">${n(e.revenue)}</span></div>`,o+=`<div class="df-detail-row"><span class="df-detail-label">EXISTING DEBT</span><span class="df-detail-value" style="color:${D};">${n(e.debt)}</span></div>`,o+=`<div class="df-detail-row"><span class="df-detail-label">DEBT/REVENUE</span><span class="df-detail-value" style="color:${E};font-weight:700;">${g}%</span></div>`}else if(e.type==="BOND"){const g=e.stability>=50?"#5c5":e.stability>=30?"#ca5":"#c84",E=e.debtToGdp>60?"#c55":e.debtToGdp>40?"#c84":"#5c5",D=e.creditRating>=60?"#5c5":e.creditRating>=35?"#ca5":"#c55";o+=`<div class="df-detail-row"><span class="df-detail-label">STABILITY</span><span class="df-detail-value" style="color:${g};">${e.stability}/100</span></div>`,o+=`<div class="df-detail-row"><span class="df-detail-label">DEBT/GDP</span><span class="df-detail-value" style="color:${E};">${e.debtToGdp}%</span></div>`,o+=`<div class="df-detail-row"><span class="df-detail-label">CREDIT RATING</span><span class="df-detail-value" style="color:${D};font-weight:700;">${e.creditRating}/100</span></div>`}else if(e.type==="INSURE"){const g=e.reputation>=60?"#5c5":e.reputation>=35?"#ca5":"#c84",E=e.projectValue?"PROJECT VALUE":"FLEET VALUE",D=e.projectValue||e.fleetValue;o+=`<div class="df-detail-row"><span class="df-detail-label">REPUTATION</span><span class="df-detail-value" style="color:${g};">${e.reputation}/100</span></div>`,o+=`<div class="df-detail-row"><span class="df-detail-label">${E}</span><span class="df-detail-value" style="color:#9e9a92;">${n(D)}</span></div>`}o+="</div>"}}o+="</div>"}o+="</div>";const I=h.filter(l=>l.type==="LOAN").length,ta=h.filter(l=>l.type==="INSURE").length,ea=h.filter(l=>l.type==="BOND").length;o+=`<div class="df-footer">
        ...
        <span class="rd-tab ${i==="LOAN"?"active-loan":i==="INSURE"?"active-insure":"active-bond"}" style="cursor:default;">${i==="LOAN"?"Loan":i==="INSURE"?"Insure":"Bond"} — ${u(a.applicant)}</span>
        ...
        </div>`;const $=Math.round((a.amount||18e6)*1.4),T=(_-5e6)/Math.max(1,$-5e6)*100;t+=`<div class="rd-control">
            <div class="rd-control__header">
                <span class="rd-control__label">LOAN AMOUNT</span>
                <span class="rd-control__value" style="font-size:14px;color:#e8e4dc;">${n(_)}</span>
            </div>
            <input type="range" class="rd-control__range" min="5000000" max="${$}" step="1000000" value="${_}"
                oninput="rdSetLoanAmount(this.value)"
                style="background:linear-gradient(90deg,#5a8aaa ${T}%,#2a2a24 ${T}%);">
            <div class="rd-control__hints"><span>$5M (partial)</span><span>${n($)} (over-fund)</span></div>
        ...
        </div>`}if(i==="INSURE"){const y=(a.reputation||50)>=60?"#5c5":(a.reputation||50)>=35?"#ca5":"#c84",b=a.projectValue?"PROJECT":"FLEET",$=a.projectValue||a.fleetValue||0;t+=`<div class="rd-applicant">
            <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;">
                <span style="font-family:var(--font-mono);font-size:10px;font-weight:700;color:#c8a832;">${u(a.abbr)}</span>
                <span style="font-size:13px;font-weight:700;color:#e8e4dc;">${u(a.applicant)}</span>
                <span class="df-badge df-badge-${a.entity.toLowerCase()}">${a.entity}</span>
            ...
            <div class="rd-applicant__stats">
                <div class="rd-applicant__stat"><div class="rd-applicant__stat-label">REP</div><div class="rd-applicant__stat-value" style="color:${y};">${a.reputation||"—"}</div></div>
                <div class="rd-applicant__stat"><div class="rd-applicant__stat-label">${b}</div><div class="rd-applicant__stat-value" style="color:#e8e4dc;">${n($)}</div></div>
                <div class="rd-applicant__stat"><div class="rd-applicant__stat-label">REQUESTED</div><div class="rd-applicant__stat-value" style="color:#e8e4dc;">${n(a.amount)}</div></div>
        ...
        </div>`,t+='<div class="rd-section-bar" style="color:#8a6aaa;">Purchase Amount</div>';const b=a.amount,$=Math.max(5e6,Math.ceil(b*.05/5e6)*5e6),T=(R-$)/(b-$)*100;t+=`<div class="rd-control">
            <div class="rd-control__header">
                <span class="rd-control__label">BUY AMOUNT</span>
                <span class="rd-control__value" style="font-size:16px;color:#8a6aaa;">${n(R)}</span>
            </div>
            <input type="range" class="rd-control__range" min="${$}" max="${b}" step="5000000" value="${R}"
                oninput="rdSetBondAmount(this.value)"
                style="background:linear-gradient(90deg,#8a6aaa ${T}%,#2a2a24 ${T}%);">
            <div class="rd-control__hints"><span>${n($)} (small position)</span><span>${n(b)} (full issuance)</span></div>
        ...
        </div>`,t+='<div class="rd-section-bar" style="color:#5c5;">Projected Returns</div>',t+=`<div class="rd-risk-row"><span class="rd-risk-label">MAX EXPOSURE</span><span class="rd-risk-value" style="color:#c55;">${n(oa)}</span></div>`,t+=`<div class="rd-risk-row"><span class="rd-risk-label">TOTAL PREMIUMS</span><span class="rd-risk-value" style="color:#5c5;">${n(E)}</span></div>`,t+=`<div class="rd-risk-row"><span class="rd-risk-label">EXPECTED PAYOUT</span><span class="rd-risk-value" style="color:#c84;">${n(ca)}</span></div>`;const y=Q>0?"":" negative",b=Q>0?"#5c5":"#c55";t+=`<div class="rd-expected${y}"><span style="font-family:var(--font-mono);font-size:9px;font-weight:700;color:#e8e4dc;">EXPECTED PROFIT</span><span style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:${b};">${n(Q)}</span></div>`,t+=`<div class="rd-formula">Premiums (${n(E)}) − expected payout (${C}% × ${n(oa)})</div>`}i==="BOND"&&(t+='<div class="rd-section-bar" style="color:#a3b07e;">Risk Assessment</div>',t+=`<div class="rd-control">
            <div class="rd-control__header">
