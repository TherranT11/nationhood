...
import{_supabase as _}from"./supabase-client-qEAQbBjE.js";/* empty css                    *//* empty css                         */import{c as fe,i as xa,M as at,Q as di,a as pi,b as Kt,d as Fi,e as Hi}from"./corp-auto-services-BIIzQFak.js";import{_ as ha}from"./preload-helper-BXl3LOEh.js";import{escapeHtml as y,hfFmtBig as T}from"./utils-A98FEun4.js";import{initMessaging as $a}from"./messaging-hdfDukBE.js";import{c as wa,a as Jt,E as ot,b as Et,d as Gi,e as ka,f as Ea,h as zi}from"./equipment-DsuDdEne.js";import{l as Ta,a as Ca}from"./corp-shipping-data-DA_tOdLs.js";import{V as yt}from"./vessels-CjafVZ4G.js";import{SECTOR_OPS_PAGE as Vi}from"./corp-topbar-CPI0igZM.js";import"./loan-math-Q4nHfU_i.js";let Ce=[],p=null,L=null,M=null,Ae=[];const Xt={};let Ye={},W=[],K={},Zt=-1;const Ia={em:"em_systems",glass:"glass_facades",heavy:"heavy_parts"},_t=e=>Ia[e]||e;let Z="concrete",G="STD",me=500,Ge=null,te=[],gt={},ei=0,mi=[];async function qa(){if(!p?.id)return;const{data:e}=await _.from("corp_properties").select("*").eq("faction_id",p.id).eq("is_active",!0).order("type",{ascending:!1}).order("purchased_at_tick",{ascending:!1});mi=e||[]}let de=[],nt=null,Xe={},bt={},fi=[],xt=null,se="trucks",ue=0,ge=1,Ie=[],Le=null,Qe=[],ti=null,ft=null;function Be(){return Ge||L}let ii="ALL",ai="TIMELINE";function P(e){return Math.abs(e)>=1e6?"$"+(e/1e6).toFixed(1)+"M":Math.abs(e)>=1e3?"$"+(e/1e3).toFixed(0)+"k":"$"+e}function Aa(e){if(e>=12){const t=Math.floor(e/12),i=e%12;return i>0?t+"y "+i+"mo":t+"y"}return e+" ticks"}function Wi(e){return!e||e.length===0?"":e.map(t=>{const i=gt[t];if(!i)return"";const a=i.reputation_bonus>0?"var(--green)":i.reputation_bonus<0?"var(--red)":"var(--text-dim)",o=i.reputation_bonus>0?"+"+i.reputation_bonus:i.reputation_bonus<0?String(i.reputation_bonus):"";return`<span style="display:inline-flex;align-items:center;gap:3px;padding:2px 6px;background: var(--border-hair);border:1px solid var(--border-0);border-radius:3px;font-family:var(--font-mono);font-size:7px;color:var(--text-secondary);">${i.icon||"📍"} ${y(i.name)}${o?` <span style="color:${a};font-weight:700;">${o} REP</span>`:""}</span>`}).filter(Boolean).join(" ")}function re(e){return Math.abs(e)>=1e9?"$"+(e/1e9).toFixed(1)+"B":Math.abs(e)>=1e6?"$"+(e/1e6).toFixed(0)+"M":Math.abs(e)>=1e3?"$"+(e/1e3).toFixed(0)+"k":"$"+e}function vi(e){return e==="civil_engineering"?"CIVIL":e==="industrial"?"INDUSTRIAL":e==="mega_project"?"MEGA":e?.toUpperCase()||"—"}function Yi(e){return e==="civil_engineering"?"light":e==="industrial"?"heavy":e==="mega_project"?"mega":"light"}function Na(){ft&&clearInterval(ft),ft=setInterval(()=>{if(!ti)return;const e=ti-Date.now();if(e<=0){document.getElementById("tick-countdown").textContent="Tick due...",clearInterval(ft);return}const t=Math.floor(e/36e5),i=Math.floor(e%36e5/6e4),a=Math.floor(e%6e4/1e3);document.getElementById("tick-countdown").textContent=t+"h "+i+"m "+a+"s"},1e3)}function Sa(e,t){e==="type"&&(ii=t),e==="sort"&&(ai=t),document.querySelectorAll(`.filter-pill[data-filter="${e}"]`).forEach(i=>{i.classList.toggle("active",i.dataset.value===t)}),Qi()}const Oi={"Civil Engineering":"civil_engineering","Industrial Construction":"industrial",Megaprojects:"mega_project"};function oi(e){if(!p)return!1;if(Oi[p.corp_subsector]===e.sector)return!0;const i=(mi||[]).filter(a=>a.type==="regional_hq"&&a.is_active&&a.nation_id===e.nation_id);for(const a of i)if(Oi[a.subsector]===e.sector)return!0;return!1}function Qi(){const e=document.getElementById("oc-list");let t=[...Ae];ii==="GOVERNMENT"?t=t.filter(r=>r.issuer_type==="GOVERNMENT"):ii==="PRIVATE"&&(t=t.filter(r=>r.issuer_type==="PRIVATE"));const i=new Set;p?.nation_id&&i.add(p.nation_id);for(const r of mi||[])r.type==="regional_hq"&&r.is_active&&r.nation_id&&i.add(r.nation_id);const a=r=>i.has(r.nation_id)&&oi(r),o=(r,l)=>ai==="TIMELINE"?(r.timeline_ticks||0)-(l.timeline_ticks||0):ai==="BUDGET"?(l.budget_ceiling||0)-(r.budget_ceiling||0):0;if(t.sort((r,l)=>{const d=a(r)?1:0,m=a(l)?1:0;return d!==m?m-d:o(r,l)}),document.getElementById("oc-count").textContent=t.length+" AVAILABLE",t.length===0){e.innerHTML=`
            ...
            </div>`;return}const n=M?.current_tick||0;let c="";for(const r of t){const l=r.issuer_type==="GOVERNMENT",d=l?"gov":"private",m=oi(r),s=m?"":" locked",f=Yi(r.sector),u=vi(r.sector),g=(r.timeline_ticks||0)>18?" warn":"",b=r.bidding_ends_tick?Math.max(0,r.bidding_ends_tick-n):"?",v=Xt[r.nation_id]||"—",x=i.has(r.nation_id);c+=`
            ...
            </div>`}e.innerHTML=c,e.querySelectorAll(".oc-item:not(.locked)").forEach(r=>{r.addEventListener("click",()=>{const l=r.dataset.contractId,d=Ae.find(m=>m.id===l);d&&Ki(d)})})}let ze=null;function Ki(e){ze=e;const t=document.getElementById("cd-overlay"),i=e.issuer_type==="GOVERNMENT",a=i?"gov":"private",o=(L?.name||p.nation||"—").toUpperCase(),n=oi(e);document.getElementById("cd-header-left").innerHTML=`
    ...
    `;const c=document.getElementById("cd-blueprint");e.blueprint_svg?(c.innerHTML=e.blueprint_svg,c.style.display=""):(c.innerHTML=Xa(e),c.style.display="");const r=e.permits_required||[],l=e.required_equipment||e.equipment_required||{},d=Array.isArray(l)?l.map(z=>({key:z,qty:1})):Object.entries(l).map(([z,E])=>({key:z,qty:E})),m=e.required_materials||e.materials_estimated||{},f={civil_engineering:"Civil Engineering",industrial:"Industrial Construction",mega_project:"Megaprojects"}[e.sector]||e.spec_category||e.sector||"—";let u="var(--teal)";e.sector==="industrial"&&(u="var(--orange)"),e.sector==="mega_project"&&(u="var(--red)");let g=P(e.budget_ceiling||e.budget||0),b=(e.timeline_ticks||e.timeline_months||0)+" Months",v="";v+=`
        <div class="cd-issue">
            ...
            </div>`),document.getElementById("cd-content").innerHTML=v;const h=r.filter(z=>z.status==="approved").length,$=r.length-h,I=d.length,C=[];for(const z of d){const E=Tt[z.key]||z.key,j=te.find(B=>B.equipment_key===E||B.equipment_key===z.key);j&&j.owned>=z.qty||C.push(z)}const w=C.length,k=e.required_materials||{},S=typeof k=="object"&&!Array.isArray(k)?Object.entries(k):[],A=[];for(const[z,E]of S){const j=K[z]||{},B=(j.LOW?.qty||0)+(j.STD?.qty||0)+(j.HIGH?.qty||0);B<E&&A.push({key:z,need:E,have:B})}const q=z=>z.replace(/_/g," ").replace(/\b\w/g,E=>E.toUpperCase());let O="";if(I>0)if(w===0)O+='<span class="cd-footer__badge ok">ALL EQUIPMENT MET</span>';else{const z=C.map(E=>q(E.key)).join(", ");O+=`<span class="cd-footer__badge bad" title="${y(z)}">${w} SHORT: ${y(z)}</span>`}if(S.length>0)if(A.length===0)O+='<span class="cd-footer__badge ok">ALL MATERIALS MET</span>';else{const z=A.map(E=>q(E.key)+" ("+E.have+"/"+E.need+")").join(", ");O+=`<span class="cd-footer__badge bad" title="${y(z)}">${A.length} MAT SHORT: ${y(z)}</span>`}r.length>0&&($===0?O+='<span class="cd-footer__badge ok">ALL PERMITS APPROVED</span>':O+=`<span class="cd-footer__badge warn">${$} PERMITS PENDING</span>`);const D=n,Y=e.issuer_faction_id===p?.id,U=e.status==="bidding",ie=Ye[e.id];document.getElementById("cd-footer").innerHTML=`
...
Net Profit: `+(u>=0?"+":"")+T(u)),Ot.add(e),await st(),await sa()}catch(n){alert("Delivery failed: "+(n.message||n)),Di(e,!1)}finally{zt=!1}}}window.deliverProject=Ka;window.openProjectModal=De;window.closeProjectModal=Da;window.pmSetTab=ja;window.pmToggleEvent=Ua;window.pmSelectResponse=Fa;window.pmAllocateMaterial=Ra;window.pmDeallocateMaterial=La;window.pmDeployEquipment=Pa;window.pmUndeployEquipment=Ba;window.pmAssignWorkers=za;window.pmUnassignWorkers=Oa;async function Ji(e){if(!N)return;const{data:t,error:i}=await _.from("construction_events").select("*").eq("contract_id",e).order("fired_at_tick",{ascending:!1});i?(console.warn("Failed to load project events:",i.message),N.events=[]):N.events=(t||[]).map(a=>({id:a.id,type:a.type,severity:a.severity,tick:a.fired_at_tick,title:a.title,desc:a.description,impact:a.impact,status:a.status==="ACTIVE"?"ACTIVE":"RESOLVED",resolution:a.resolution,responses:a.responses||[]})),Pe()}let Pt=!1;async function Ja(e,t){if(!(Pt||!N)){Pt=!0;try{const{data:i,error:a}=await _.rpc("resolve_construction_event",{p_event_id:e,p_response_key:t});if(a){console.error("Failed to resolve event:",a.message),alert("Failed to submit response: "+a.message);return}const o=typeof i=="string"?JSON.parse(i):i;if(o?.error){alert("Error: "+o.error);return}await Ji(N.project.id),await st(),o?.quality_applied&&o.quality_applied!==0&&(N.quality=Math.max(0,Math.min(100,N.quality+o.quality_applied)),N.qualityLabel=N.quality>=75?"EXCELLENT":N.quality>=50?"FAIR":N.quality>=25?"POOR":"BAD"),Pe()}finally{Pt=!1}}}window.confirmEventResponse=Ja;function Ee(e,t,i){const a=i?` style="color:${i}"`:"";return`<div class="cd-detail-row">
        ...
        <g transform="translate(470,172)">
            ...
            <span class="wh-row__val${g?" empty":""}">${u>0?P(u):"—"}</span>
        ...
        </div>`}u+="</div>";const b=(Number(p?.corp_cash_reserves)||0)>=c,v=me>n,{totalUnits:x}=It(),h=Ct-x,$=me>h,I=h<=0,C=Hi[G];u+=`<div class="pr-order">
        ...
        <div class="pr-order__box">
            ...
            <div class="pr-order__total">
                <div>
                    ...
                    <div class="pr-order__total-value">${P(c)}</div>
                </div>
        ...
        </div></div>`;return}ue>=Ie.length&&(ue=0);let t="";for(let a=0;a<Ie.length;a++){const o=Ie[a],n=ue===a,c=o.sellerType==="LOCAL"?"var(--teal)":"#5a8aaa",r=Gi(o.condition);t+=`<div class="em-listing${n?" selected":""}" style="${n?"border-left-color:"+c:""}" onclick="setEmListing(${a})">`,t+=`<div class="em-listing__row1">
            <div style="display:flex;align-items:center;gap:6px">
            ...
            </div>`),t+="</div>"}const i=Ie[ue];if(i){const a=Et(se),o=ot[a?.tier||1],n=Math.min(i.available,4),c=i.price*ge,r=(Number(p?.corp_cash_reserves)||0)>=c;t+=`<div class="em-purchase"><div class="em-purchase__box">
            <div class="em-purchase__header">
                ...
                <span class="em-purchase__price">${P(i.price)}/unit</span>
            </div>
            ...
            <div class="em-purchase__total">
                <div>
                    ...
                    <div class="em-purchase__total-value">${P(c)}</div>
            ...
            </div>`;return}let l="";Fe.forEach((d,m)=>{const s=rt===m,f=yo(d.ticks_left,d.total_ticks,d.expiring_soon),u=Math.min(d.ticks_left/(d.total_ticks||1)*100,100);l+=`<div class="pm-item ${d.expiring_soon?"pm-item--expiring":""} ${s?"expanded":""}" onclick="togglePmExpand(${m})">
            ...
            </div>`;return}let l="";Ze.forEach((d,m)=>{const s=d.processing_total-d.ticks_remaining,f=Math.min(s/(d.processing_total||1)*100,100),u=d.status==="PROCESSING"?"pm-item__status--processing":"pm-item__status--review";l+=`<div style="padding:8px 14px;border-bottom:1px solid var(--border-0);">
                <div class="pm-item__row1">
                    <span class="pm-item__name">${y(d.name)}</span>
        ...
        </div>`;return}let c="";Se.forEach((r,l)=>{const d=ni===l,m=r.financials||{},s=(m.payment||0)+(m.bonus||0)-(m.penalty||0)-(m.total_cost||0),f=s>=0,u=bo(r.result),b={DISTINCTION:"var(--gold)",PASS:"var(--green)",CONDITIONAL:"var(--orange)",FAIL:"var(--red)"}[r.result]||"var(--text-dim)",v=r.type==="GOVERNMENT";if(c+=`<div class="dl-item ${d?"expanded":""}" onclick="toggleDlExpand(${l})">
            <div class="dl-item__inner" style="border-left:2px solid ${b}">
                ...
                </div>`,d){const x=r.inspection||{};c+='<div style="margin-top:8px;">',c+='<div class="dl-section-label">Inspection Report</div>',["materials","structural","systems"].forEach(C=>{const w=x[C]||{score:0,issues:[]},k=ji(w.score),S=Math.min(w.score/100*100,100);c+=`<div class="dl-inspect-row">
                ...
                </div>`});const h=x.permits||{passed:!0,issues:[]};c+=`<div class="dl-permits-row ${h.passed?"dl-permits-row--pass":"dl-permits-row--fail"}">
                ...
                </div>`}),c+="</div>",c+='<div class="dl-section-label">Financial Summary</div>',c+='<div class="dl-fin-panel">',c+=`<div class="dl-fin-row"><span class="dl-fin-key">CONTRACT VALUE</span><span class="dl-fin-val">${ve(m.contract_value||0)}</span></div>`,(m.bonus||0)>0&&(c+=`<div class="dl-fin-row"><span class="dl-fin-key">QUALITY BONUS</span><span class="dl-fin-val" style="color:var(--green)">+${ve(m.bonus)}</span></div>`),(m.penalty||0)>0&&(c+=`<div class="dl-fin-row"><span class="dl-fin-key">PENALTIES</span><span class="dl-fin-val" style="color:var(--red)">-${ve(m.penalty)}</span></div>`);const $=(m.payment||0)+(m.bonus||0)-(m.penalty||0);c+=`<div class="dl-fin-row"><span class="dl-fin-key">PAYMENT RECEIVED</span><span class="dl-fin-val" style="color:var(--green)">${ve($)}</span></div>`,c+=`<div class="dl-fin-row"><span class="dl-fin-key">TOTAL COST</span><span class="dl-fin-val" style="color:var(--red)">-${ve(m.total_cost||0)}</span></div>`,c+=`<div class="dl-fin-total">
        ...
        <div class="eq-summary__cell">
        ...
        </div>`;const o={};for(const r of te)o[r.equipment_key]=r;let n="";for(let r=1;r<=3;r++){const l=ot[r],d=Jt(r),m=ei===r,s=d.reduce((u,g)=>u+(o[g.key]?.owned||0),0),f=d.reduce((u,g)=>u+(o[g.key]?.deployed||0),0);if(n+=`<div class="eq-tier-hdr" onclick="toggleEqTier(${r})">
            ...
            ${s>0?`<span class="eq-tier-hdr__count">${f}/${s}</span>`:'<span class="eq-tier-hdr__none">NONE OWNED</span>'}
        </div>`,m)for(const u of d){const g=o[u.key],b=g?.owned||0,v=g?.deployed||0,x=g?.condition||0,h=u.maintenancePerUnit*b,$=b-v,I=b>0&&$===0,C=b>0&&x<65,w=Gi(x),k=g?.assigned_projects||[],S=k.length>0?k.map(A=>A.contract_name||"Project").join(", ").slice(0,30):b>0&&v>0?v+" project"+(v>1?"s":""):"—";n+=`<div class="eq-row${b===0?" unowned":""}">`,n+=`<div class="eq-row__top">
                    ...
                    </div>`:n+='<div class="eq-row__hint">Purchase from Equipment Market →</div>',n+="</div>"}}document.getElementById("eq-list").innerHTML=n;const c=[1,2,3].map(r=>{const l=ot[r],d=Jt(r).reduce((m,s)=>m+(o[s.key]?.owned||0),0);return`<div class="eq-footer__tier-box" style="border-color:${d>0?l.color+"33":"var(--border-0)"};background:${d>0?l.color+"0a":"transparent"}">
            ...
            <div style="flex:1;border-right:1px solid ${t.border};overflow:auto;">
                ...
                <div style="padding:8px 14px;border-bottom:1px solid ${t.border};">
                    ...
                    <div style="display:flex;justify-content:space-between;">
                        <span style="font-family:${e};font-size:8px;color:${t.dim}">${Q} × $${O.toLocaleString()}/tick × ${S} ticks</span>
    ...
    </div>`,document.body.appendChild(je)}let Ft=!1;async function Do(){if(Ft||!ae)return;const e=ae,t=e.required_materials||{},i=Object.keys(t),a=Number(e.budget_ceiling||0),o=Number(e.timeline_ticks||8),n={concrete:36e4,steel:5e5,glass_facades:56e4,em_systems:64e4,lumber:24e4,heavy_parts:8e5,aggregate:16e4,asphalt:28e4},c={LOW:.5,STD:1,HIGH:2};let r=0;for(const S of i){const A=Number(t[S]||0),q=be[S]||"STD",O=n[S]||3e5;r+=A*Math.round(O*c[q])}const l=15200,d=e.required_workforce||{},m=Number(d.general||0)+Number(d.skilled||0)||100,s=Math.max(40,Math.round(m*.5)),f=m*2,u=Math.max(0,Math.min(1,(Q-s)/(f-s||1))),g=Math.round(4.5-u*8),b=Math.max(Math.round(o*.6),o+g),v=Q*l*b,x=(Me||[]).filter(S=>!S.has_permit).reduce((S,A)=>S+Number(We[A.permit_key]?.cost||0),0),$=r+v+x+4e5,I=Math.round($*(xe/100)),C=$+I;if(C>a){alert("Bid exceeds budget ceiling. Reduce costs or markup.");return}const w=Object.values(be),k=w.length>0?Math.round(w.reduce((S,A)=>S+(A==="HIGH"?85:A==="STD"?65:45),0)/w.length):50;if(confirm('Submit bid for "'+e.name+`"?
...
Once submitted, your bid cannot be changed.`)){Ft=!0;try{const{data:S}=await _.from("shard").select("current_tick").eq("name","Alpha Shard").single(),A=S?.current_tick||0,q={};for(const D of i)q[D]=be[D]||"STD";const{error:O}=await _.from("contract_bids").insert({contract_id:e.id,faction_id:p.id,bid_price:C,material_grades:q,labor_count:Q,markup_pct:xe,estimated_cost:$,estimated_quality:k,status:"pending",submitted_at_tick:A});if(O)throw O;e.status==="open"&&await _.from("construction_contracts").update({status:"bidding"}).eq("id",e.id).eq("status","open"),Ii(),alert(`Bid submitted successfully!
    ...
    <div style="width:640px;max-height:92vh;background:${t.surface};border:1px solid ${t.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
        ...
        <div style="font-family:var(--font-mono);font-size:12px;font-weight:700;color:${s.color};margin-top:1px;">${s.value}</div>
                ...
                <div style="flex:1;padding:3px 6px;text-align:right;">
                    ...
                    </div>`;k+="</div>";const q=$||I,O=Math.round((s.purchase_price||3e6)*.08*(1+(100-s.condition)/100)),D=Math.round((s.fuel_capacity||1e3)*50*(1-s.fuel/100)),Y=Math.round((s.purchase_price||3e6)*(s.condition/100)*.6);if(k+=`<div style="display:flex;gap:4px;">
                    <div onclick="${q?"":"flRefurbish('"+s.id+"',"+O+")"}" style="flex:1;padding:5px 0;text-align:center;cursor:${q?"not-allowed":"pointer"};font-family:var(--font-mono);font-size:7px;font-weight:700;letter-spacing:0.5px;color:${q?"var(--text-dim)":"#5c5"};border:1px solid ${q?"var(--border-0)":"#2a5a3a"};background:${q?"transparent":"rgba(74,170,136,0.06)"};opacity:${q?.35:1};">REFURBISH<br><span style="font-weight:400;font-size:6px;">${T(O)}</span></div>
...
The insurer (`+T(n)+" coverage) has been notified. Claim details are visible in the events feed.")}window.flFileClaim=Zo;const ci={fuel_depot:{label:"FUEL DEPOT",color:"#c86a4a",icon:"⛽",desc:"Bunkering facility — refuel at base cost, earn revenue from visiting fleets."},dry_dock:{label:"DRY DOCK",color:"#c84",icon:"🔧",desc:"Repair & maintenance dock — dock at base cost, earn revenue from visiting fleets."}},ut=[{type:"fuel_depot",name:"Fuel Depot — Standard",cost:105e6,maint:85e3,style:"Basic",desc:"Bulk fuel storage and bunkering facility."},{type:"fuel_depot",name:"Fuel Depot — Advanced",cost:14e7,maint:11e4,style:"Modern",desc:"High-capacity fuel terminal with pipeline infrastructure."},{type:"dry_dock",name:"Dry Dock — Standard",cost:85e6,maint:15e4,style:"Basic",desc:"Ship repair and maintenance facility."},{type:"dry_dock",name:"Dry Dock — Advanced",cost:115e6,maint:2e5,style:"Modern",desc:"Full-service shipyard with drydock and crane facilities."}];let kt=[];async function en(){if(!p||p.corp_sector!=="Shipping")return;const{data:e}=await _.from("corp_properties").select("*, nations!nation_id(name)").eq("faction_id",p.id).in("type",["fuel_depot","dry_dock"]).eq("is_active",!0).order("created_at",{ascending:!1});kt=e||[],tn()}function tn(){const e=document.getElementById("pf-count"),t=document.getElementById("pf-list"),i=document.getElementById("pf-footer");if(!e||!t||!i)return;const a=kt;if(e.textContent=a.length+" FACILIT"+(a.length===1?"Y":"IES"),a.length===0)t.innerHTML=`<div style="padding:20px;text-align:center;">
        ...
        </div>`;else{let c=0;t.innerHTML=a.map(r=>{const l=ci[r.type]||ci.fuel_depot,d=r.condition>=75?"#5c5":r.condition>=50?"#ca5":"#c84";return c+=Number(r.monthly_maintenance||0),`<div style="padding:8px 12px;border-bottom:1px solid var(--border-0);">
        ...
        </div>`}let Qt=!1;async function an(e){if(Qt||!p||!M)return;const t=ut.filter(v=>v.type===e);if(t.length===0)return;const i=ci[e],a=p.nation_id,o=L?.name||p?.nation||"Home Nation",n=L?.capital||"Port City",c=[{id:a,name:o,capital:n,label:"National HQ"}],{data:r}=await _.from("corp_properties").select("nation_id, name, city, nations!nation_id(name, capital)").eq("faction_id",p.id).eq("type","regional_hq").eq("is_active",!0);for(const v of r||[])v.nation_id!==a&&c.push({id:v.nation_id,name:v.nations?.name||v.city||"Unknown",capital:v.nations?.capital||v.city||"Port City",label:v.name||"Subsidiary"});let l=c[0];if(c.length>1){let v=i.label+` — SELECT LOCATION
`+"─".repeat(30)+`
`;v+=`Build in which nation?

`;for(let $=0;$<c.length;$++){const I=c[$],C=kt.filter(w=>w.type===e&&w.nation_id===I.id).length;v+=$+1+". "+I.name+"  ("+I.label+")",C>0&&(v+="  ["+C+" existing]"),v+=`
`}v+=`
Enter number (or cancel):`;const x=prompt(v);if(!x)return;const h=parseInt(x,10)-1;if(isNaN(h)||h<0||h>=c.length){alert("Invalid selection.");return}l=c[h]}const d=kt.filter(v=>v.type===e&&v.nation_id===l.id).length;let m=i.label+" CONSTRUCTION — "+l.name.toUpperCase()+`
`+"─".repeat(30)+`
...
Construction corporations in `+l.name+" can now bid on this project.")}catch(v){alert("Failed to post contract: "+(v.message||"Error"))}finally{Qt=!1}}window.pfOpenBuild=an;const qi={"Bulk Cargo":["Reefer","Bulk","Coastal"],"Container Freight":["Coastal","Container"],"Specialized Transport":["Tanker","LNG","Bulk"]};async function da(){if(!p||p.corp_sector!=="Shipping")return;const{data:e,error:t}=await _.from("ship_market_listings").select("*, nation:nation_id(id, name)").eq("status","available").order("asking_price",{ascending:!0});t&&console.warn("Failed to load ship market:",t.message),fi=e||[],xt=null,pa()}function on(e){xt=xt===e?null:e,pa()}function nn(e){return(qi[p?.corp_subsector]||[]).includes(e)}function pa(){const e=document.getElementById("sm-count"),t=document.getElementById("sm-list"),i=document.getElementById("sm-footer");if(!e||!t)return;const a=fi;e.textContent=a.length+" AVAILABLE",a.length===0?t.innerHTML='<div style="flex:1;display:flex;align-items:center;justify-content:center;font-family:var(--font-mono);font-size:9px;color:var(--text-dim);text-align:center;line-height:1.8;">No vessels on the market.<br>Check back next cycle.</div>':t.innerHTML=a.map((c,r)=>{const l=xt===r,d=Ke[c.vessel_class]||{color:"#666",label:"?"},m=c.seller_type==="CORP"?"#5a8aaa":"#8b9a6b",s=la(c.condition),f=c.nation?.name||"—",u=nn(c.vessel_class);M?.current_tick;const g=c.age_ticks||0,b=Math.max(1,Math.floor(g/12)),v=f!==p?.nation?Number(p?.tariffs||L?.tariffs||0):0,x=Math.round(c.asking_price*v/100),h=c.asking_price+x;let $=`<div onclick="smSelectListing(${r})" style="border-bottom:1px solid var(--border-0);cursor:pointer;border-left:2px solid ${l?d.color:"transparent"};background:${l?d.color+"06":"transparent"};">
                ...
                <div style="flex:1;padding:3px 6px;text-align:right;">
                    ...
                    <div style="display:flex;justify-content:space-between;padding:2px 0;border-bottom:1px solid var(--border-0);">
                        ...
                        <span style="font-family:var(--font-mono);font-size:8px;color:${d.color};">${(Ke[c.vessel_class]||{}).label||"?"} class cargo</span>
                    </div>
                    ...
                    </div>`),u?$+=`<div onclick="event.stopPropagation();smPurchase('${c.id}',${h})" style="padding:4px 14px;text-align:center;font-family:var(--font-mono);font-size:8px;font-weight:700;letter-spacing:1px;color:#000;background:${d.color};cursor:pointer;">${T(h)} — PURCHASE</div>`:$+=`<div style="padding:4px 14px;text-align:center;font-family:var(--font-mono);font-size:8px;font-weight:700;letter-spacing:0.5px;color:var(--text-dim);border:1px solid var(--border-0);opacity:0.4;">⊘ ${c.vessel_class} not available for ${p?.corp_subsector||"your subsector"}</div>`,$+="</div>"),$+="</div></div>",$}).join("");const o=a.filter(c=>c.seller_type==="CORP").length,n=a.filter(c=>c.seller_type==="LOCAL").length;i.innerHTML=`<div style="display:flex;gap:6px;">
