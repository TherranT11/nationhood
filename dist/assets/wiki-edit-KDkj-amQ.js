import{_supabase as s}from"./supabase-client-CiYoFhIh.js";/* empty css                  */import{i as J,f as X,c as Z,u as H,s as A,r as ee}from"./wiki-BLHkPfw9.js";import{e as d}from"./utils-oN1e812_.js";const te=[{value:"",label:"None"},{value:"nation",label:"Nation"},{value:"person",label:"Person"},{value:"corporation",label:"Corporation"},{value:"religion",label:"Religion"},{value:"culture",label:"Culture"},{value:"political_party",label:"Political Party"},{value:"law",label:"Law"},{value:"agreement",label:"Agreement"},{value:"event",label:"Event"},{value:"organization",label:"Organization"},{value:"military_unit",label:"Military Unit"},{value:"product",label:"Product"},{value:"location",label:"Location"},{value:"sports",label:"Sports"}];(async()=>{const L=await J({requireAuth:!0});if(!L)return;const{faction:p,nation:k}=L,w=document.getElementById("wiki-root"),E=new URLSearchParams(window.location.search),z=E.has("new"),$=E.get("slug");let t=null;if($&&!z){try{t=await X(s,$)}catch(e){w.innerHTML=`<div class="wiki-empty"><h2>Error</h2><p>${d(e.message)}</p></div>`;return}if(!t){w.innerHTML='<div class="wiki-empty"><h2>Page Not Found</h2><p><a href="wiki-list.html" class="wiki-btn">Back to Wiki</a></p></div>';return}}if(t&&t.locked_by&&t.locked_by!==p.id){w.innerHTML=`<div class="wiki-empty"><h2>Page Locked</h2><p>This page is locked by its creator. Only they can edit it.</p>
    ...
    `;const I=Z({initialTags:R,placeholder:"add tag..."});document.getElementById("wiki-tag-mount").appendChild(I.container);const a=new Quill("#quill-editor",{theme:"snow",placeholder:"Write your page content here... Use [[Page Name]] to link to other wiki pages.",modules:{toolbar:{container:[[{header:[1,2,3,!1]}],["bold","italic","underline"],[{list:"ordered"},{list:"bullet"}],["link","image","wiki-link"],["clean"]],handlers:{image:G,"wiki-link":F}}}});C&&(a.root.innerHTML=C);const f=document.querySelector(".ql-wiki-link");f&&(f.innerHTML="[[]]",f.title="Insert wiki link",f.style.cssText="font-size:11px;font-weight:700;width:auto;padding:0 4px;color:var(--text-secondary);");function F(){const e=a.getSelection(!0);if(e.length>0){const i=a.getText(e.index,e.length);a.deleteText(e.index,e.length),a.insertText(e.index,`[[${i}]]`),a.setSelection(e.index+i.length+4)}else{const i=prompt("Enter the wiki page name to link to:");i&&(a.insertText(e.index,`[[${i}]]`),a.setSelection(e.index+i.length+4))}}async function G(){const e=document.createElement("input");e.type="file",e.accept="image/*",e.onchange=async()=>{const i=e.files[0];if(!i)return;const l=document.getElementById("wiki-status");l.textContent="Uploading image...";try{const o=await H(s,k.id,i),u=a.getSelection(!0);a.insertEmbed(u.index,"image",o),a.setSelection(u.index+1),l.textContent=""}catch(o){l.textContent="Image upload failed: "+o.message,l.style.color="var(--red)"}},e.click()}document.getElementById("wiki-template").addEventListener("change",e=>{document.getElementById("infobox-section").style.display=e.target.value?"block":"none"}),document.getElementById("infobox-upload-btn").addEventListener("click",()=>{document.getElementById("infobox-file").click()}),document.getElementById("infobox-file").addEventListener("change",async e=>{const i=e.target.files[0];if(!i)return;const l=document.getElementById("wiki-status"),o=document.getElementById("infobox-upload-btn");o.textContent="Uploading...",o.disabled=!0;try{r=await H(s,k.id,i),document.getElementById("infobox-preview").src=r,o.textContent="Upload Image",o.disabled=!1,l.textContent=""}catch(u){o.textContent="Upload Image",o.disabled=!1,l.textContent="Image upload failed: "+u.message,l.style.color="var(--red)"}});const B=document.getElementById("infobox-remove-img");B&&B.addEventListener("click",()=>{r="",document.getElementById("infobox-preview").src="data:image/svg+xml;utf8,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%2280%22 height=%2280%22><rect fill=%22%23252525%22 width=%2280%22 height=%2280%22/><text x=%2240%22 y=%2244%22 text-anchor=%22middle%22 fill=%22%23666%22 font-size=%2211%22>No image</text></svg>",B.remove()});const _=document.getElementById("kv-rows");function S(e="",i=""){const l=document.createElement("div");l.className="wiki-kv-row",l.innerHTML=`
        ...
        `,l.querySelector(".wiki-kv-remove").addEventListener("click",()=>l.remove()),_.appendChild(l)}N.forEach(e=>S(e.label,e.value)),document.getElementById("add-kv-btn").addEventListener("click",()=>S());function K(){const e=[];return _.querySelectorAll(".wiki-kv-row").forEach(i=>{const l=i.querySelector(".kv-label").value.trim(),o=i.querySelector(".kv-value").value.trim();(l||o)&&e.push({label:l,value:o})}),e}let h=!1;const y=()=>{h=!0};document.getElementById("wiki-title").addEventListener("input",y),document.getElementById("wiki-template").addEventListener("change",y),a.on("text-change",y);const P=document.getElementById("wiki-slug");P&&P.addEventListener("input",y),I.container.addEventListener("click",()=>setTimeout(y,50)),window.addEventListener("beforeunload",e=>{h&&(e.preventDefault(),e.returnValue="")}),document.getElementById("save-btn").addEventListener("click",async()=>{const e=document.getElementById("wiki-title").value.trim();if(!e){document.getElementById("wiki-status").textContent="Title is required.",document.getElementById("wiki-status").style.color="var(--red)";return}const i=document.getElementById("wiki-slug");let l;if(t&&i?l=A(i.value.trim())||t.slug:l=A(e),!l){document.getElementById("wiki-status").textContent="Could not generate a valid slug from the title.",document.getElementById("wiki-status").style.color="var(--red)";return}const o=document.getElementById("wiki-template").value||null,u=a.root.innerHTML,Q=o?K():[],q=document.getElementById("wiki-lock"),Y=q&&q.checked?p.id:null,x=document.getElementById("save-btn");x.textContent="Saving...",x.disabled=!0;const D=document.getElementById("wiki-status"),m={nation_id:k.id,slug:l,title:e,body:u,template_type:o,template_data:Q,infobox_image:r||null,locked_by:Y,tags:I.getTags(),updated_by:p.id,updated_at:new Date().toISOString()};try{let v;if(t){const{error:n}=await s.from("wiki_pages").update(m).eq("id",t.id);v=n}else{m.created_by=p.id;const{error:n}=await s.from("wiki_pages").insert(m);v=n}if(v)if(delete m.tags,t){const{error:n}=await s.from("wiki_pages").update(m).eq("id",t.id);if(n)throw n}else{const{error:n}=await s.from("wiki_pages").insert(m);if(n)throw n}h=!1;try{s.functions.invoke("trigger-wiki-rebuild",{body:{}}).then(({error:n})=>{n&&console.warn("[wiki-rebuild] trigger failed:",n.message)}).catch(n=>console.warn("[wiki-rebuild] trigger threw:",n?.message||n))}catch(n){console.warn("[wiki-rebuild] invoke threw synchronously:",n?.message||n)}window.location.href=`wiki.html?slug=${encodeURIComponent(l)}`}catch(v){x.textContent="Save Page",x.disabled=!1,D.textContent="Save failed: "+v.message,D.style.color="var(--red)"}});const c=document.getElementById("delete-btn");c&&t&&c.addEventListener("click",async()=>{if(confirm("Delete this wiki page? This cannot be undone.")){c.textContent="Deleting...",c.disabled=!0;try{const{error:e}=await s.from("wiki_pages").delete().eq("id",t.id);if(e)throw e;h=!1,window.location.href="wiki-list.html"}catch(e){c.textContent="Delete Page",c.disabled=!1,document.getElementById("wiki-status").textContent="Delete failed: "+e.message,document.getElementById("wiki-status").style.color="var(--red)"}}});const U=document.getElementById("toggle-gallery"),g=document.getElementById("image-gallery");U&&g&&(U.addEventListener("click",()=>{g.style.display=g.style.display==="none"?"block":"none"}),document.getElementById("close-gallery")?.addEventListener("click",()=>{g.style.display="none"}),g.querySelectorAll(".wiki-gallery-thumb").forEach(e=>{e.addEventListener("click",()=>{const i=e.dataset.url,l=a.getSelection(!0);a.insertEmbed(l.index,"image",i),a.setSelection(l.index+1),g.style.display="none"})})),await ee(w)})();
