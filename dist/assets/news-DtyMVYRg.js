import{_ as de}from"./preload-helper-BXl3LOEh.js";import{tickToDate as M}from"./utils-A98FEun4.js";import{f as pe}from"./government-structure-DUNrPmll.js";let g=null,h=null,O=[],B="all",A=null,D=null,F=null,S=null,R=!1,y="cruceran",Z=!1;const Y={cruceran:{key:"cruceran",name:"The Cruceran",tagline:"Truth in the service of the people",nations:["Avelia","Palvera","San Estrella","Montequilla","Melizea","Sangreza","Sierramar"],style:"cruceran"},continental:{key:"continental",name:"The Continental",tagline:"Where Ideas Converge",nations:["Calveth","Flandis","Vostia","Dravka"],style:"continental"},alsahwa:{key:"alsahwa",name:"Al-Sahwa",tagline:"Independent Voice of Al-Makir",nations:["Hajjara"],continent:"Al-Makir",style:"alsahwa"}};function ve(e){for(const[s,n]of Object.entries(Y))if(n.nations.some(t=>t.toLowerCase()===(e||"").toLowerCase()))return s;return"cruceran"}function ne(e,s){const n=Y[e];return n?n.nations.some(t=>t.toLowerCase()===(s||"").toLowerCase())?!0:L&&L.length>0?n.nations.some(t=>L.includes(t.toLowerCase())):!1:!1}let L=[];const we=["Winter","Spring","Spring","Spring","Summer","Summer","Summer","Fall","Fall","Fall","Winter","Winter"];function x(e){const s=e%12,n=2e3+Math.floor(e/12),t=we[s],i=s===0?n-1:n;return`${t} ${i}`}async function z(e,s){g=e,h=s,A=null,B="all",F=null;const n=document.getElementById("newspaper-root");if(!n)return;if(L=[],s.faction?.faction_type==="corporation")try{const{data:w}=await e.from("corp_properties").select("nation_id, nations:nation_id(name)").eq("faction_id",s.faction.id).eq("is_active",!0);w&&(L=w.map(p=>(p.nations?.name||"").toLowerCase()).filter(Boolean))}catch{}Z||(y=ve(s.nation?.name));const t=s.shard?.current_date||"[Month], [Year]",i=ne(y,s.nation?.name),l="Write Article",a=Object.entries(Y).map(([w,p])=>`<option value="${w}" ${w===y?"selected":""}>${p.name}</option>`).join("");n.innerHTML=`<div class="newspaper-container nws-pub-${y}">

        <!-- TOP RIBBON -->
        <div class="nws-top-ribbon${y==="continental"?" nws-top-ribbon--continental":y==="alsahwa"?" nws-top-ribbon--alsahwa":""}">
    ...
    </div>`,ue(),me(),he(n),fe(n),ae(n),Ie(n),Ee(n);const v=document.getElementById("nws-pub-switcher");v&&v.addEventListener("change",async()=>{const w=v.value;if(w!==y){y=w,Z=!0;try{await z(g,h)}catch(p){console.error("[News] Publication switch failed:",p)}}});const d=document.getElementById("nws-reward-badge");if(d){const w=s.faction?.faction_type==="corporation";d.textContent=w?"+1 Reputation (1st Article)":"+2 Momentum (1st Article)"}await q(),He(),Ne()}function ue(){const e=document.getElementById("nws-modal-overlay"),s=document.getElementById("nws-write-article-btn"),n=document.getElementById("nws-modal-close"),t=document.getElementById("nws-article-body"),i=document.getElementById("nws-char-count"),l=document.getElementById("nws-article-image"),a=document.getElementById("nws-file-label-text"),v=document.getElementById("nws-image-preview"),d=document.getElementById("nws-image-preview-img");s&&s.addEventListener("click",()=>{W(),e.classList.add("active")}),n&&n.addEventListener("click",()=>{e.classList.remove("active"),W()});const w=document.getElementById("nws-remove-image-btn");w&&w.addEventListener("click",()=>{R=!0;const p=document.getElementById("nws-image-preview");p&&(p.style.display="none");const m=document.getElementById("nws-file-label-text");m&&(m.textContent="Click to select an image..."),w.style.display="none"}),t&&i&&t.addEventListener("input",()=>{const p=t.value.length,m=p<4e3;i.textContent=m?`${p.toLocaleString()} / 4,000 min`:`${p.toLocaleString()} / 12,000`,i.style.color=m?"var(--dred, #c55)":"",i.classList.toggle("nws-near-limit",p>=11500)}),l&&l.addEventListener("change",()=>{const p=l.files[0];if(!p){a.textContent="Click to select an image...",v.style.display="none";return}const m=2*1024*1024;if(p.size>m){I("Image must be under 2MB."),l.value="",a.textContent="Click to select an image...",v.style.display="none";return}a.textContent=p.name;const c=new FileReader;c.onload=r=>{d.src=r.target.result,v.style.display="block"},c.readAsDataURL(p)})}function I(e){const s=document.getElementById("nws-form-error");s&&(s.textContent=e,s.style.display="block",setTimeout(()=>{s.style.display="none"},5e3))}function Q(e){const s=document.getElementById("nws-form-success");s&&(s.textContent=e,s.style.display="block",setTimeout(()=>{s.style.display="none"},5e3))}function me(){const e=document.getElementById("nws-submit-btn");e&&e.addEventListener("click",async()=>{if(e.disabled)return;const s=document.getElementById("nws-article-title").value.trim(),n=document.getElementById("nws-article-author").value.trim(),t=document.getElementById("nws-article-category").value,i=document.getElementById("nws-article-body").value.trim(),a=document.getElementById("nws-article-image").files[0]||null,v=!ne(y,h?.nation?.name);if(!s)return I("Please enter a headline.");if(!n)return I("Please enter a writer name.");if(!t)return I("Please select a category.");if(!i)return I("Please write an article body.");if(i.length<4e3)return I("Article must be at least 4,000 characters. Currently: "+i.length.toLocaleString()+".");if(i.length>12e3)return I("Article body must be 12,000 characters or fewer.");const d=!!S;e.disabled=!0,e.textContent=d?"Updating...":"Publishing...";try{const{nation:w,faction:p,shard:m}=h;if(d){let c;a?c=await X(w.id,a):R&&(c=null);const r={headline:s,author_name:n,body:i,category:t};c!==void 0&&(r.image_url=c);const{error:f}=await g.from("player_articles").update(r).eq("id",S).eq("author_faction_id",p.id);if(f)throw f;Q("Article updated!")}else{if(v){const{deductAP:u}=await de(async()=>{const{deductAP:C}=await import("./config-CKNXR-qR.js");return{deductAP:C}},[]),_=await u(g,p.id,1);if(!_.success){I("Not enough AP to post on another publication (need 1 AP).");return}p.action_points=_.newAp}let c=null;a&&(c=await X(w.id,a));const{error:r}=await g.from("player_articles").insert({nation_id:w.id,author_faction_id:p.id,author_name:n,headline:s,body:i,category:t,image_url:c,status:"published",published_tick:m?.current_tick||0,publication:y});if(r)throw r;const f=m?.current_tick||0;let b=0;const{data:$,error:E}=await g.from("player_articles").select("id").eq("author_faction_id",p.id).eq("published_tick",f);if(E&&console.error("[News] Failed to count articles this tick:",E),!E&&(!$||$.length<=1))b=2;else try{const{data:u}=await g.from("active_laws").select("id, policies!inner(policy_key)").eq("nation_id",w.id).eq("is_reversal",!1).eq("policies.policy_key","internet_sovereignty").limit(1).maybeSingle();if(u){const _=await pe(g,w.id),C=new Set(_?.party_ids||[]);_?.lead_party_id&&C.add(_.lead_party_id),C.has(p.id)&&(b=1)}}catch(u){console.warn("[News] ISA coalition-bonus check failed:",u?.message||u)}const P=`News article published (+${b})`;let T="Article published!";if(b>0)try{const{error:u}=await g.rpc("adjust_momentum",{p_faction_id:p.id,p_delta:b,p_label:P,p_tick:f});u?console.error("[News] Momentum reward failed:",u):T=`Article published! +${b} Momentum.`}catch(u){console.error("[News] Momentum reward error:",u)}sessionStorage.removeItem("nationhood_state"),Q(T)}W(),setTimeout(()=>{document.getElementById("nws-modal-overlay").classList.remove("active")},1500),await q()}catch(w){console.error(`[News] Article ${d?"update":"submission"} failed:`,w),I(`Failed to ${d?"update":"publish"} article. Please try again.`)}finally{e.disabled=!1,e.textContent=S?"Update Article":"Publish Article"}})}async function X(e,s){const n=s.name.split(".").pop()||"png",t=`player-articles/${e}/${Date.now()}.${n}`,{error:i}=await g.storage.from("public-assets").upload(t,s,{contentType:s.type,upsert:!0});if(i)throw i;const{data:l}=g.storage.from("public-assets").getPublicUrl(t);return l?.publicUrl||null}function N(e){return!h?.faction||e.author_faction_id!==h.faction.id?"":`<button class="nws-delete-btn" data-article-id="${e.id}" title="Delete article">&times;</button>`}function H(e){return!h?.faction||e.author_faction_id!==h.faction.id?"":`<button class="nws-edit-btn" data-edit-id="${e.id}" title="Edit article">&#9998;</button>`}function he(e){e.addEventListener("click",async s=>{const n=s.target.closest(".nws-delete-btn");if(!n)return;const t=n.dataset.articleId;if(!(!t||!confirm("Delete this article?"))){n.disabled=!0,n.textContent="...";try{const{error:i}=await g.from("player_articles").delete().eq("id",t).eq("author_faction_id",h.faction.id);if(i)throw i;await q()}catch(i){console.error("[News] Failed to delete article:",i),n.disabled=!1,n.textContent="×"}}})}function W(){S=null,R=!1,document.getElementById("nws-article-title").value="",document.getElementById("nws-article-author").value="",document.getElementById("nws-article-category").value="",document.getElementById("nws-article-body").value="";const e=document.getElementById("nws-article-image");e&&(e.value="");const s=document.getElementById("nws-file-label-text");s&&(s.textContent="Click to select an image...");const n=document.getElementById("nws-image-preview");n&&(n.style.display="none");const t=document.getElementById("nws-char-count");t&&(t.textContent="0 / 12000",t.classList.remove("nws-near-limit","nws-ap-qualified"));const i=document.getElementById("nws-remove-image-btn");i&&(i.style.display="none");const l=document.querySelector(".nws-modal-header h3");l&&(l.textContent="Write Article");const a=document.getElementById("nws-submit-btn");a&&(a.textContent="Publish Article")}function ge(e){S=e.id,R=!1,document.getElementById("nws-article-title").value=e.headline||"",document.getElementById("nws-article-author").value=e.author_name||"",document.getElementById("nws-article-category").value=e.category||"",document.getElementById("nws-article-body").value=e.body||"";const s=(e.body||"").length,n=document.getElementById("nws-char-count");n&&(n.textContent=`${s} / 12000`,n.classList.toggle("nws-near-limit",s>=11500));const t=document.getElementById("nws-image-preview"),i=document.getElementById("nws-image-preview-img"),l=document.getElementById("nws-file-label-text"),a=document.getElementById("nws-remove-image-btn"),v=document.getElementById("nws-article-image");v&&(v.value=""),e.image_url?(i&&(i.src=e.image_url),t&&(t.style.display="block"),l&&(l.textContent="Current image (select new to replace)"),a&&(a.style.display="inline")):(t&&(t.style.display="none"),l&&(l.textContent="Click to select an image..."),a&&(a.style.display="none"));const d=document.querySelector(".nws-modal-header h3");d&&(d.textContent="Edit Article");const w=document.getElementById("nws-submit-btn");w&&(w.textContent="Update Article"),document.getElementById("nws-modal-overlay").classList.add("active")}function fe(e){e.addEventListener("click",s=>{const n=s.target.closest(".nws-edit-btn");if(!n)return;s.stopPropagation();const t=n.dataset.editId,i=O.find(l=>String(l.id)===String(t));i&&ge(i)})}function ae(e){D&&e.removeEventListener("click",D),D=s=>{if(s.target.closest(".nws-delete-btn, .nws-edit-btn, .nws-write-btn, .nws-modal-overlay, button, a, input, select, textarea"))return;const n=s.target.closest("[data-article-id]");if(!n)return;const t=n.dataset.articleId,i=O.find(l=>String(l.id)===String(t));if(i)try{te(e,i)}catch(l){console.error("[News] Failed to open article:",l)}},e.addEventListener("click",D)}function te(e,s){const n=s.body||"",t=s.published_tick!=null?M(s.published_tick):h?.shard?.current_date||"[Month], [Year]",l=!!F?"&larr; Back to Edition":"&larr; Back to Front Page",a=le(n),v=s.image_url?`<div class="nws-reader-image">
        ...
        <div class="nws-main-content">
            <div class="nws-reader">
                ...
                <span class="nws-section-tag">${o(k(s.category))} &mdash; ${o(t)}</span>
    ...
    </div>`;const d=F,w=d?()=>ce(d.root,d.articles,d.dateLabel):()=>z(g,h);document.getElementById("nws-back-btn")?.addEventListener("click",w),document.getElementById("nws-back-btn-bottom")?.addEventListener("click",w),be(s),e.scrollTop=0}async function be(e){const s=document.getElementById("nws-like-btn"),n=document.getElementById("nws-like-icon"),t=document.getElementById("nws-like-count");if(!s||!g||!h?.faction?.id)return;const i=h.faction.id;let l=!1;const{data:a}=await g.from("article_likes").select("id").eq("article_id",e.id).eq("faction_id",i).maybeSingle();a&&(s.classList.add("nws-like-btn--liked"),n.innerHTML="&#9829;"),s.addEventListener("click",async()=>{if(!l){l=!0,s.disabled=!0;try{const v=h.shard?.current_tick||0,{data:d,error:w}=await g.rpc("toggle_article_like",{p_article_id:e.id,p_faction_id:i,p_tick:v});if(w){console.error("[News] Like failed:",w.message);return}d.liked?(s.classList.add("nws-like-btn--liked"),n.innerHTML="&#9829;"):(s.classList.remove("nws-like-btn--liked"),n.innerHTML="&#9825;"),t.textContent=d.like_count,e.like_count=d.like_count}catch(v){console.error("[News] Like error:",v)}finally{l=!1,s.disabled=!1}}})}function o(e){const s=document.createElement("div");return s.textContent=e,s.innerHTML}function ie(e,s=1200){if(e.length<=s)return le(e);let n=e.lastIndexOf(" ",s);n<s*.5&&(n=s);const i=e.substring(0,n).split(/\n\n+/).filter(a=>a.trim());return i.map((a,v)=>`<p class="${v===0?"nws-drop-cap":""}">${G(o(a.trim()))}${v===i.length-1?"...":""}</p>`).join("")+'<p class="nws-read-more">Read More &rarr;</p>'}function G(e){return e.replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>").replace(/\*(.+?)\*/g,"<em>$1</em>").replace(/__(.+?)__/g,"<u>$1</u>")}function le(e){const s=e.split(/\n\n+/).filter(n=>n.trim());return s.length<=1?`<p class="nws-drop-cap">${G(o(e))}</p>`:s.map((n,t)=>`<p class="${t===0?"nws-drop-cap":""}">${G(o(n.trim()))}</p>`).join("")}function k(e){return e==="elections"&&(e="politics"),{politics:"Politics",economy:"Business",international:"International",social:"Social",entertainment:"Entertainment",sports:"Sports",opinion:"Opinion"}[e]||e}function oe(e){return e==="elections"&&(e="politics"),{politics:"linear-gradient(135deg,#2a1a2a,#1a0d1a)",economy:"linear-gradient(135deg,#2a1a0a,#1a0d00)",international:"linear-gradient(135deg,#0a1a2a,#001a2a)",social:"linear-gradient(135deg,#1a2a1a,#0d1a0d)",entertainment:"linear-gradient(135deg,#2a2a0a,#1a1a00)",sports:"linear-gradient(135deg,#2a0a0a,#1a0000)",opinion:"linear-gradient(135deg,#2a2a2a,#1a1a1a)"}[e]||"linear-gradient(135deg,#1a1a1a,#0d0d0d)"}async function U(){if(A)return A;const e=h?.shard,s=h?.nation;if(!e||!s)return[s?.id].filter(Boolean);const{data:n,error:t}=await g.from("nations").select("id").eq("shard_id",e.id);return t&&console.error("[News] Failed to fetch shard nations:",t),A=n&&n.length>0?n.map(i=>i.id):[s.id],A}async function q(){if(!(!g||!h))try{let e=await U();if(y==="alsahwa"){const{data:u}=await g.from("nations").select("id").eq("continent","Al-Makir");u&&u.length>0&&(e=u.map(_=>_.id))}let s=g.from("player_articles").select("*").in("nation_id",e).eq("status","published").order("created_at",{ascending:!1});y==="alsahwa"?s=s.or("publication.eq.alsahwa,publication.eq.international"):y!=="cruceran"?s=s.or(`publication.eq.${y},publication.eq.international`):s=s.or("publication.eq.cruceran,publication.is.null,publication.eq.international");const{data:n,error:t}=await s;if(t){console.error("[News] Failed to load articles:",t);return}if(!n||n.length===0)return;const i=h.shard?.current_tick??0,l=x(i),a=n.filter(u=>x(u.published_tick??0)===l),v=n.filter(u=>x(u.published_tick??0)!==l),d=new Set(a.map(u=>u.category)),w=[],p=new Set;for(const u of v)u.category!=="opinion"&&(d.has(u.category)||(p.has(u.category)||p.add(u.category),w.push(u)));const m=n.filter(u=>u.category==="opinion").slice(0,4),c=[...a.filter(u=>u.category!=="opinion"),...w,...m];if(c.length===0)return;O=c;const r=B&&B!=="all"?c.filter(u=>(u.category==="elections"?"politics":u.category)===B):c;if(r.length===0&&B!=="all"){const u=`<p class="nws-placeholder" style="text-align:center;padding:40px;grid-column:1/-1;">No ${k(B)} articles in this edition.</p>`,_=document.getElementById("nws-lead-section");_&&(_.innerHTML=u);const C=document.getElementById("nws-secondary-grid");C&&(C.innerHTML="");const K=document.querySelector(".nws-opinion-grid");K&&(K.innerHTML="");const J=document.querySelector(".nws-bottom-left");J&&(J.innerHTML="");return}const f=B==="opinion",$=[...f?r:r.filter(u=>u.category!=="opinion")].sort((u,_)=>(_.published_tick??0)-(u.published_tick??0)),E=$[0],j=$.slice(1,4),P=$.slice(4,7),T=[...c].sort((u,_)=>new Date(_.created_at)-new Date(u.created_at)).slice(0,5);y==="continental"?ke(E,j,P,m.slice(0,4),T):(E&&ye(E,j),_e(P),ee(f?[]:m.slice(0,4)),$e(T))}catch(e){console.error("[News] Error loading articles:",e)}}function ye(e,s){const n=document.getElementById("nws-lead-section");if(!n||!e)return;const t=s.length>0?s.map(d=>`
            <div class="nws-sidebar-story" data-article-id="${d.id}">
                ${H(d)}${N(d)}
                <span class="nws-section-tag">${o(k(d.category))}</span>
                ...
                <div class="nws-byline"><span class="nws-author">${o(d.author_name)}</span><span class="nws-dot">&middot;</span><span>${d.published_tick!=null?M(d.published_tick):h?.shard?.current_date||"—"}</span></div>
            </div>
        `).join(""):'<div class="nws-sidebar-story"><p class="nws-placeholder">[More stories will appear as articles are published.]</p></div>',i=e.image_url?`<img src="${o(e.image_url)}" alt="${o(e.headline)}">`:"",l=e.body||"",a=l.replace(/\n+/g," "),v=a.length>200?a.substring(0,200)+"...":a;n.innerHTML=`
        <div class="nws-lead-main" data-article-id="${e.id}">
            ${H(e)}${N(e)}
            <span class="nws-section-tag">${o(k(e.category))}</span>
    ...
    `}function _e(e){const s=document.getElementById("nws-secondary-grid");if(!s||e.length===0)return;const n=[...e],t=["Crisis","Election","Business"],i=[0,1,2].map(l=>{const a=n[l];if(a){const v=a.image_url?`<img src="${o(a.image_url)}" alt="${o(a.headline)}" style="width:100%;height:100%;object-fit:cover;">`:`<div class="nws-img-ph" style="background:${oe(a.category)};">${o(k(a.category))}</div>`;return`<div class="nws-sec-story" data-article-id="${a.id}">
                ${H(a)}${N(a)}
                <div class="nws-sec-image">${v}</div>
                <span class="nws-section-tag">${o(k(a.category))}</span>
                <h3 class="nws-sec-headline">${o(a.headline)}</h3>
                <p class="nws-sec-deck">${o((a.body||"").replace(/\n+/g," ").substring(0,150))}${(a.body||"").length>150?"...":""}</p>
                <div class="nws-byline"><span class="nws-author">${o(a.author_name)}</span><span class="nws-dot">&middot;</span><span>${a.published_tick!=null?M(a.published_tick):h?.shard?.current_date||"—"}</span></div>
                ...
                <div class="nws-byline"><span class="nws-author nws-placeholder">[Author]</span><span class="nws-dot">&middot;</span><span class="nws-placeholder">[Date]</span></div>
            </div>`}}).join("");s.innerHTML=i}function $e(e){if(e.length===0)return;const s=document.querySelector(".nws-bottom-left");if(!s)return;const n='<div class="nws-col-header">In Brief</div>',t=e.map((a,v)=>`
        <div class="nws-brief-row" data-article-id="${a.id}">
            <div class="nws-brief-num">${v+1}</div>
            <div class="nws-brief-text">
                <strong>${o(a.headline)}${H(a)}${N(a)}</strong>
    ...
    `).join(""),i=5-e.length;let l="";for(let a=0;a<i;a++)l+=`
        ...
        `;s.innerHTML=n+t+l}function ee(e){const s=document.querySelector(".nws-opinion-grid");if(!s||e.length===0)return;const n=[0,1,2,3].map(t=>{const i=e[t];if(i){const l=(i.body||"").length>80?(i.body||"").substring(0,80)+"...":i.body||"";return`<div class="nws-op-card" data-article-id="${i.id}">
                ${H(i)}${N(i)}
                <div class="nws-op-author">${o(i.author_name)} &mdash; Opinion</div>
                <div class="nws-op-headline">&ldquo;${o(l)}&rdquo;</div>
            </div>`}else return`<div class="nws-op-card">
                <div class="nws-op-author nws-placeholder">[Columnist] &mdash; [Topic]</div>
                <div class="nws-op-headline nws-placeholder">&ldquo;[Opinion headline will appear here.]&rdquo;</div>
            </div>`}).join("");s.innerHTML=n}function ke(e,s,n,t,i){const l=document.getElementById("nws-main-content")||document.querySelector(".nws-main-content");if(!l)return;const a=r=>(r||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"),v=r=>a((r.body||"").replace(/\n+/g," ").substring(0,200))+((r.body||"").length>200?"...":""),d=r=>a((r.body||"").replace(/\n+/g," ").substring(0,120))+((r.body||"").length>120?"...":""),w=r=>a(k(r.category)),p=r=>r?.published_tick!=null?M(r.published_tick):h?.shard?.current_date||"—",m={politics:"--politics",economy:"--economy",social:"--social",international:"--intl",entertainment:"--culture",science:"--science"};let c="";if(!e&&s.length===0&&n.length===0&&t.length===0&&i.length===0){c+='<div style="text-align:center;padding:60px 20px;color:#9e9b95;font-family:Outfit,sans-serif;">',c+='<div style="font-size:2rem;margin-bottom:12px;opacity:0.3;">📰</div>',c+='<div style="font-size:1rem;font-weight:600;">No articles yet</div>',c+='<div style="font-size:0.85rem;margin-top:6px;">Be the first to write for The Continental.</div>',c+="</div>",l.innerHTML=c;return}if(e){const r=e.image_url?`<img src="${a(e.image_url)}" alt="" style="width:100%;height:100%;object-fit:cover;">`:"";c+=`<div class="ct-hero" data-article-id="${e.id}">
        ...
        </div>`,c+='<div class="ct-story-grid">';for(const r of s){const f=m[r.category]||"",b=r.image_url?`<img src="${a(r.image_url)}" alt="" style="width:100%;height:100%;object-fit:cover;">`:"";c+=`<div class="ct-card" data-article-id="${r.id}">
                <div class="ct-card__image ct-card__image${f}">${b}
                    <span class="ct-card__image-label">${w(r)}</span>
                </div>
                <div class="ct-card__body">
                    <div class="ct-card__section">${w(r)}</div>
            ...
            <div class="ct-analysis-band__grid">`;for(const r of n)c+=`<div class="ct-analysis-story" data-article-id="${r.id}">
                <div class="ct-analysis-story__section">${w(r)}</div>
                <h2 class="ct-analysis-story__headline">${a(r.headline)}</h2>
                <p class="ct-analysis-story__summary">${d(r)}</p>
                <div class="ct-analysis-story__meta"><strong>${a(r.author_name)}</strong> &middot; ${p(r)}</div>
            ...
            <div class="ct-section-divider__line"></div>
        </div>`,c+='<div class="ct-two-col">',c+='<div class="ct-analysis-list">';for(let r=0;r<t.length;r++){const f=t[r];c+=`<div class="ct-analysis-item" data-article-id="${f.id}">
                <div class="ct-analysis-item__number">${String(r+1).padStart(2,"0")}</div>
                <div class="ct-analysis-item__content">
                    <div class="ct-analysis-item__section">${w(f)}</div>
                    <h3 class="ct-analysis-item__headline">${a(f.headline)}</h3>
                    <p class="ct-analysis-item__summary">${d(f)}</p>
                    <div class="ct-analysis-item__meta"><strong>${a(f.author_name)}</strong> &middot; ${p(f)}</div>
                </div>
            </div>`}c+="</div>",c+='<div class="ct-sidebar">',c+='<div class="ct-sidebar__section"><div class="ct-sidebar__section-title">Also in This Edition</div>';for(const r of i)c+=`<div class="ct-sidebar-brief" data-article-id="${r.id}">
                <div class="ct-sidebar-brief__section">${w(r)}</div>
    ...
    </div>`,l.innerHTML=c,l.querySelectorAll("[data-article-id]").forEach(r=>{r.style.cursor="pointer",r.addEventListener("click",f=>{if(f.target.closest(".nws-edit-btn, .nws-delete-btn"))return;const b=r.dataset.articleId,$=O.find(E=>String(E.id)===String(b));$&&te(document.getElementById("newspaper-root"),$)})})}function Ee(e){const s=e.querySelectorAll(".nws-nav-item[data-category]");s.forEach(t=>{t.addEventListener("click",()=>{s.forEach(i=>i.classList.remove("active")),t.classList.add("active"),B=t.dataset.category,q()})});const n=e.querySelectorAll(".nws-alsahwa-nav-item[data-cat]");n.forEach(t=>{t.addEventListener("click",()=>{n.forEach(i=>i.classList.remove("nws-alsahwa-nav-item--active")),t.classList.add("nws-alsahwa-nav-item--active"),B=t.dataset.cat,q()})})}function Ie(e){const s=document.getElementById("nws-nav-archives");s&&s.addEventListener("click",()=>re(e))}async function re(e){if(!(!g||!h))try{const s=h.shard?.current_tick??0,n=h.shard?.current_date||"[Month], [Year]",t=await U(),{data:i,error:l}=await g.from("player_articles").select("*").in("nation_id",t).eq("status","published").order("published_tick",{ascending:!1});if(l){console.error("[News] Failed to load archive articles:",l);return}const a={};for(const p of i||[]){const m=p.published_tick??0,c=x(m);a[c]||(a[c]={label:c,maxTick:m,articles:[]}),a[c].articles.push(p),m>a[c].maxTick&&(a[c].maxTick=m)}const v=Object.values(a).sort((p,m)=>m.maxTick-p.maxTick),d=x(s),w=v.length>0?v.map(p=>{const m=p.label===d?' <span class="nws-archive-current">Current</span>':"";return`<div class="nws-archive-month" data-archive-season="${o(p.label)}">
                    <div class="nws-archive-month-name">${o(p.label)}${m}</div>
        ...
        </div>`,document.getElementById("nws-back-btn")?.addEventListener("click",()=>z(g,h)),e.querySelectorAll("[data-archive-season]").forEach(p=>{p.addEventListener("click",()=>{const m=p.dataset.archiveSeason,c=a[m];c&&ce(e,c.articles,c.label)})}),e.scrollTop=0}catch(s){console.error("[News] Error loading archives:",s)}}function ce(e,s,n){O=s;const t=s.filter(b=>b.category==="opinion"),l=[...s.filter(b=>b.category!=="opinion")].sort((b,$)=>($.body||"").length-(b.body||"").length),a=l[0],v=l.slice(1,4),d=l.slice(4,7),w=l.slice(7,12),p=t.slice(0,4),m=a?Be(a,v,n):'<p class="nws-placeholder" style="padding:40px;text-align:center;">[No articles in this edition]</p>',c=Ce(d),r=Te(p),f=Ae(w);e.innerHTML=`<div class="newspaper-container">
        <!-- TOP RIBBON -->
        <div class="nws-top-ribbon">
            <div class="nws-top-ribbon-inner">
                <span>${o(n)}</span>
    ...
    </div>`,document.getElementById("nws-back-to-archives")?.addEventListener("click",()=>re(e)),F={root:e,articles:s,dateLabel:n},ae(e),e.scrollTop=0}function Be(e,s,n){const t=s.length>0?s.map(d=>`
            <div class="nws-sidebar-story" data-article-id="${d.id}">
                <span class="nws-section-tag">${o(k(d.category))}</span>
                <h3 class="nws-sidebar-headline">${o(d.headline)}</h3>
                <p class="nws-sidebar-deck">${o((d.body||"").replace(/\n+/g," ").substring(0,120))}${(d.body||"").length>120?"...":""}</p>
        ...
        `).join(""):"",i=e.image_url?`<img src="${o(e.image_url)}" alt="${o(e.headline)}">`:"",l=e.body||"",a=l.replace(/\n+/g," "),v=a.length>200?a.substring(0,200)+"...":a;return`
        <div class="nws-lead-main" data-article-id="${e.id}">
            <span class="nws-section-tag">${o(k(e.category))}</span>
            <h2 class="nws-lead-headline">${o(e.headline)}</h2>
            <p class="nws-lead-deck">${o(v)}</p>
    ...
    `}function Ce(e){return e.length===0?"":e.map(s=>{const n=s.image_url?`<img src="${o(s.image_url)}" alt="${o(s.headline)}" style="width:100%;height:100%;object-fit:cover;">`:`<div class="nws-img-ph" style="background:${oe(s.category)};">${o(k(s.category))}</div>`;return`<div class="nws-sec-story" data-article-id="${s.id}">
            <div class="nws-sec-image">${n}</div>
            <span class="nws-section-tag">${o(k(s.category))}</span>
        ...
        <div class="nws-opinion-inner">
            ...
            <div class="nws-op-headline">&ldquo;${o(t)}&rdquo;</div>
        </div>`}).join("")}</div>
        </div>
    ...
    `).join("")}function V(e){return e.w*3+e.d}function Se(e){return Object.values(e).sort((s,n)=>{const t=V(n)-V(s);return t!==0?t:n.w-s.w})}function Le(e){return`<span class="vln-form-dot" style="background:${{W:"#1a4a1a",D:"#8a6a20",L:"#8b1a1a"}[e]||"#999"}"></span>`}function xe(e,s){return e<=3?"vln-pos-top":e>=s-2?"vln-pos-bottom":""}function se(e,s={}){const n=Se(e),t=s.muted||!1,i=n.length;let l=`<div class="vln-table${t?" vln-table-muted":""}">
        ...
        </div>`;return n.forEach((a,v)=>{const d=v+1,w=xe(d,i),p=d===3||d===7?" vln-zone-break":"",m=V(a),c=t?"":a.form.map(r=>Le(r)).join("");l+=`<div class="vln-row${p}">
    ...
    </div>`:""}async function Ne(){const e=document.getElementById("vln-widget");if(!(!e||!g))try{const{data:s,error:n}=await g.from("vln_state").select("*").eq("shard_name","Alpha Shard").maybeSingle();if(n||!s){e.innerHTML=`
                <div class="nws-col-header">Volbal Ligue Nationale</div>
                <div class="vln-offseason">Season concludes in March.</div>`;return}const t=h?.shard?.current_tick??0,i=2e3+Math.floor(t/12);let l='<div class="nws-col-header">Volbal Ligue Nationale</div>';s.active?((s.matchweek>=18||(s.fixtures||[]).every(v=>v.played))&&(!s.last_results||s.last_results.length===0)?l+='<div class="vln-matchweek">Final Standings</div>':l+=`<div class="vln-matchweek">Matchweek ${s.matchweek} of 18</div>`,l+=se(s.standings),l+=Me(s.match_of_week,s.matchweek)):s.standings&&Object.keys(s.standings).length>0&&Object.values(s.standings).some(v=>v.played>0)?(l+='<div class="vln-offseason">Off-season. Final standings below.</div>',l+=`<div class="vln-final-label">Final Standings &mdash; Year ${s.season}</div>`,l+=se(s.standings,{muted:!0})):l+='<div class="vln-offseason">Season concludes in March.</div>',e.innerHTML=l}catch(s){console.error("[VLN] Widget render failed:",s)}}async function He(){if(!(!g||!h))try{const e=await U(),s=h.shard?.current_tick||0,{data:n}=await g.from("event_log").select("description_chosen, fired_at_tick").in("nation_id",e).eq("category","corporate").gte("fired_at_tick",Math.max(0,s-12)).order("fired_at_tick",{ascending:!1}).limit(20),t=document.querySelector(".nws-ticker-scroll");if(!t)return;const i='  <span class="nws-ticker-sep">◆</span>  ';if(n&&n.length>0){const l=n.map(a=>a.description_chosen);t.innerHTML=l.join(i)+i+l.join(i)}}catch(e){console.error("[Ticker] Failed to load corporate events:",e)}}export{z as i};
