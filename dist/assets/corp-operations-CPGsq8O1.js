import{_ as $}from"./supabase-client-BXEzLDpS.js";import{e as b}from"./utils-C2W-HleY.js";import{initMessaging as on}from"./messaging-B5Fng3EZ.js";import{c as an,a as Dt,E as We,b as at,d as Ei,e as sn,f as rn,h as bi}from"./equipment-DsuDdEne.js";const Ti={LOW:"#c84",STD:"#ca5",HIGH:"#5c5"},me=[{key:"concrete",name:"Concrete",category:"RAW",tiers:{LOW:{requirements:[]},STD:{requirements:[{stat:"manufacturing_output",min:20}]},HIGH:{requirements:[{stat:"manufacturing_output",min:50},{stat:"higher_education",min:40}]}},priceDrivers:["manufacturing_output","inflation","fuel_prices","urbanization"]},{key:"steel",name:"Steel",category:"RAW",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:10}]},STD:{requirements:[{stat:"manufacturing_output",min:35},{stat:"rare_minerals",min:20}]},HIGH:{requirements:[{stat:"manufacturing_output",min:60},{stat:"rare_minerals",min:40},{stat:"higher_education",min:45}]}},priceDrivers:["manufacturing_output","rare_minerals","inflation","fuel_prices"]},{key:"lumber",name:"Lumber",category:"RAW",tiers:{LOW:{requirements:[{stat:"arable_land",min:10}]},STD:{requirements:[{stat:"arable_land",min:30},{stat:"physical_infrastructure",min:20}]},HIGH:{requirements:[{stat:"arable_land",min:50},{stat:"manufacturing_output",min:30}]}},priceDrivers:["arable_land","physical_infrastructure","inflation"]},{key:"aggregate",name:"Aggregate",category:"RAW",tiers:{LOW:{requirements:[]},STD:{requirements:[{stat:"rare_minerals",min:15},{stat:"physical_infrastructure",min:20}]},HIGH:{requirements:[{stat:"rare_minerals",min:35},{stat:"manufacturing_output",min:25}]}},priceDrivers:["rare_minerals","physical_infrastructure","inflation"]},{key:"em",name:"E&M Systems",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:15}]},STD:{requirements:[{stat:"manufacturing_output",min:40},{stat:"digital_infrastructure",min:25}]},HIGH:{requirements:[{stat:"manufacturing_output",min:55},{stat:"digital_infrastructure",min:50},{stat:"energy_generation",min:40}]}},priceDrivers:["manufacturing_output","digital_infrastructure","inflation","energy_generation"]},{key:"glass",name:"Glass & Facades",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:20}]},STD:{requirements:[{stat:"manufacturing_output",min:40},{stat:"standard_of_living",min:35}]},HIGH:{requirements:[{stat:"manufacturing_output",min:60},{stat:"digital_infrastructure",min:40},{stat:"higher_education",min:50}]}},priceDrivers:["manufacturing_output","standard_of_living","inflation"]},{key:"asphalt",name:"Asphalt",category:"RAW",tiers:{LOW:{requirements:[{stat:"oil_and_gas",min:10}]},STD:{requirements:[{stat:"oil_and_gas",min:30},{stat:"manufacturing_output",min:25}]},HIGH:{requirements:[{stat:"oil_and_gas",min:45},{stat:"manufacturing_output",min:40},{stat:"physical_infrastructure",min:40}]}},priceDrivers:["oil_and_gas","manufacturing_output","inflation","fuel_prices"]},{key:"heavy",name:"Heavy Machinery Parts",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:40},{stat:"rare_minerals",min:30}]},STD:{requirements:[{stat:"manufacturing_output",min:60},{stat:"rare_minerals",min:45},{stat:"higher_education",min:40}]},HIGH:{requirements:[{stat:"manufacturing_output",min:75},{stat:"rare_minerals",min:60},{stat:"higher_education",min:55},{stat:"digital_infrastructure",min:45}]}},priceDrivers:["manufacturing_output","rare_minerals","higher_education","digital_infrastructure"]}];function Ce(i,e,t){const n=me.find(s=>s.key===i);if(!n)return{available:!1,failedStat:"unknown_material"};const o=n.tiers[e];if(!o)return{available:!1,failedStat:"unknown_tier"};for(const s of o.requirements){const a=Number(t?.[s.stat]??0);if(a<s.min)return{available:!1,failedStat:s.stat,failedMin:s.min,nationValue:a}}return{available:!0}}function Kt(i,e,t){const o={concrete:{LOW:200,STD:300,HIGH:500},steel:{LOW:400,STD:700,HIGH:1200},lumber:{LOW:80,STD:130,HIGH:200},aggregate:{LOW:40,STD:60,HIGH:100},em:{LOW:400,STD:700,HIGH:1200},glass:{LOW:300,STD:500,HIGH:900},asphalt:{LOW:120,STD:200,HIGH:350},heavy:{LOW:800,STD:1400,HIGH:2400}}[i]?.[e];if(!o)return 0;const s=me.find(l=>l.key===i);if(!s)return o;let a=1;for(const l of s.priceDrivers){const d=Number(t?.[l]??50);l==="inflation"||l==="fuel_prices"?a*=1+(d-50)/200:a*=1-(d-50)/250}return a=Math.max(.4,Math.min(2.5,a)),Math.round(o*a)}function Ci(i,e,t){const o={concrete:{LOW:5e3,STD:3e3,HIGH:1e3},steel:{LOW:2e3,STD:1500,HIGH:500},lumber:{LOW:8e3,STD:4e3,HIGH:1500},aggregate:{LOW:15e3,STD:6e3,HIGH:2e3},em:{LOW:1e3,STD:700,HIGH:300},glass:{LOW:1500,STD:800,HIGH:300},asphalt:{LOW:4e3,STD:2e3,HIGH:800},heavy:{LOW:400,STD:200,HIGH:80}}[i]?.[e]||0,a=me.find(r=>r.key===i)?.priceDrivers?.[0],d=.3+(a?Number(t?.[a]??50):50)/50*.7;return Math.round(o*d)}const Jt=["LOW","STD","HIGH"],jt={LOW:"Low",STD:"Standard",HIGH:"High"};let Ue=[],f=null,E=null,j=null,Ne=[],De={},oe=[],F={},Ht=-1,H="concrete",U="STD",ae=500,J=[],Ut=0,X="trucks",de=0,ce=1,be=[],Se=null,nt=[],Gt=null,et=null,Ft="ALL",Wt="TIMELINE";function B(i){return Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(1)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i}function ln(i){if(i>=12){const e=Math.floor(i/12),t=i%12;return t>0?e+"y "+t+"mo":e+"y"}return i+" ticks"}function Z(i){return Math.abs(i)>=1e9?"$"+(i/1e9).toFixed(1)+"B":Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(0)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i}function Xt(i){return i==="civil_engineering"?"CIVIL":i==="industrial"?"INDUSTRIAL":i==="mega_project"?"MEGA":i?.toUpperCase()||"—"}function Si(i){return i==="civil_engineering"?"light":i==="industrial"?"heavy":i==="mega_project"?"mega":"light"}function dn(){et&&clearInterval(et),et=setInterval(()=>{if(!Gt)return;const i=Gt-Date.now();if(i<=0){document.getElementById("tick-countdown").textContent="Tick due...",clearInterval(et);return}const e=Math.floor(i/36e5),t=Math.floor(i%36e5/6e4),n=Math.floor(i%6e4/1e3);document.getElementById("tick-countdown").textContent=e+"h "+t+"m "+n+"s"},1e3)}function cn(){document.body.classList.toggle("light-mode");const i=document.getElementById("theme-toggle");i.textContent=document.body.classList.contains("light-mode")?"Dark":"Light"}function pn(i,e){i==="type"&&(Ft=e),i==="sort"&&(Wt=e),document.querySelectorAll(`.filter-pill[data-filter="${i}"]`).forEach(t=>{t.classList.toggle("active",t.dataset.value===e)}),Mi()}const $i={"Civil Engineering":"civil_engineering","Industrial Construction":"industrial",Megaprojects:"mega_project"};function Ii(i){if(!f)return!1;if($i[f.corp_subsector]===i.sector)return!0;const t=(G||[]).filter(n=>n.type==="regional_hq"&&n.is_active&&n.nation_id===i.nation_id);for(const n of t)if($i[n.subsector]===i.sector)return!0;return!1}function Mi(){const i=document.getElementById("oc-list");let e=[...Ne];if(Ft==="GOVERNMENT"?e=e.filter(o=>o.issuer_type==="GOVERNMENT"):Ft==="PRIVATE"&&(e=e.filter(o=>o.issuer_type==="PRIVATE")),Wt==="TIMELINE"&&e.sort((o,s)=>(o.timeline_ticks||0)-(s.timeline_ticks||0)),Wt==="BUDGET"&&e.sort((o,s)=>(s.budget_ceiling||0)-(o.budget_ceiling||0)),document.getElementById("oc-count").textContent=e.length+" AVAILABLE",e.length===0){i.innerHTML=`
            ...
            </div>`;return}const t=j?.current_tick||0;let n="";for(const o of e){const s=o.issuer_type==="GOVERNMENT",a=s?"gov":"private",l=Ii(o),d=l?"":" locked",r=Si(o.sector),c=Xt(o.sector),m=(o.timeline_ticks||0)>18?" warn":"",p=o.bidding_ends_tick?Math.max(0,o.bidding_ends_tick-t):"?";n+=`
            <div class="oc-item${d}" data-contract-id="${o.id}">
                ...
                <div class="oc-item__stats">
                    <div class="oc-stat">
                        ...
                        <div class="oc-stat__value">${Z(o.budget_ceiling||0)}</div>
                    </div>
                    <div class="oc-stat">
                    ...
                    <div class="oc-eligibility">
                        ${De[o.id]?`<span class="oc-eligible-badge yes" style="background:var(--green-faint);border-color:var(--green-border);color:var(--green);">BID: ${Z(De[o.id].bid_price)}</span>`:`<span class="oc-eligible-badge ${l?"yes":"no"}">${l?"ELIGIBLE":"LOCKED"}</span>`}
                    </div>
            ...
            </div>`}i.innerHTML=n,i.querySelectorAll(".oc-item:not(.locked)").forEach(o=>{o.addEventListener("click",()=>{const s=o.dataset.contractId,a=Ne.find(l=>l.id===s);a&&Ai(a)})})}let Ie=null;function Ai(i){Ie=i;const e=document.getElementById("cd-overlay"),t=i.issuer_type==="GOVERNMENT",n=t?"gov":"private",o=(E?.name||f.nation||"—").toUpperCase(),s=Ii(i);document.getElementById("cd-header-left").innerHTML=`
    ...
    `;const a=document.getElementById("cd-blueprint");i.blueprint_svg?(a.innerHTML=i.blueprint_svg,a.style.display=""):(a.innerHTML=En(i),a.style.display="");const l=i.permits_required||[],d=i.required_equipment||i.equipment_required||[],r=i.required_materials||i.materials_estimated||{},m={civil_engineering:"Civil Engineering",industrial:"Industrial Construction",mega_project:"Megaprojects"}[i.sector]||i.spec_category||i.sector||"—";let p="var(--teal)";i.sector==="industrial"&&(p="var(--orange)"),i.sector==="mega_project"&&(p="var(--red)");let u=B(i.budget_ceiling||i.budget||0),y=(i.timeline_ticks||i.timeline_months||0)+" Months",g="";g+=`
        <div class="cd-issue">
            ...
            </div>`),g+='<div class="cd-details">',i.project_type&&(g+=xe("Type",i.project_type)),i.project_subtype&&(g+=xe("Sub-Type",i.project_subtype)),g+=xe("Specialization",m,p),g+=xe("Total Budget",u,"var(--green)"),g+=xe("Timeline",y),g+=xe("Nation",E?.name||f.nation||"—"),i.region&&(g+=xe("Region",i.region)),g+="</div>",l.length>0&&(g+=`
            <div class="cd-items">
                <div class="cd-section-label">Permits Required</div>
                <div class="cd-items__list">
                    ${l.map(M=>{const A=M.status==="approved"?"approved":"required",L=M.status==="approved"?"&#10003;":"&#9675;";return`<div class="cd-chip ${A}">
            ...
            </div>`),document.getElementById("cd-content").innerHTML=g;const _=l.filter(M=>M.status==="approved").length,h=l.length-_,w=d.filter(M=>M.owned).length,I=d.length-w;let C="";d.length>0&&(I===0?C+='<span class="cd-footer__badge ok">ALL EQUIPMENT MET</span>':C+=`<span class="cd-footer__badge bad">${I} EQUIPMENT MISSING</span>`),l.length>0&&(h===0?C+='<span class="cd-footer__badge ok">ALL PERMITS APPROVED</span>':C+=`<span class="cd-footer__badge warn">${h} PERMITS PENDING</span>`);const k=s;(f.action_points??0)>=2;const x=i.issuer_faction_id===f?.id,T=i.status==="bidding",q=De[i.id];document.getElementById("cd-footer").innerHTML=`
        ...
        <div class="cd-footer__actions">
            ...
            ${x?`<button class="cd-btn primary" onclick="openBidReview()" style="background:#c8a832;border-color:#c8a832;"
                    ${T?"":"disabled"} title="${T?"Review submitted bids":"No bids received yet"}">REVIEW BIDS</button>`:q?`<button class="cd-btn primary" onclick="retractBid('${i.id}')" style="background:#c55;border-color:#c55;"
                        title="Retract your bid ($1M penalty)">RETRACT BID</button>`:`<button class="cd-btn primary" onclick="openBidAssembly()" ${k?"":"disabled"}
                        title="${k?"Assemble and submit a bid":"Not qualified for this contract"}">BID</button>`}
        </div>
    `,e.classList.add("open"),document.body.style.overflow="hidden"}function Qe(i){i&&i.target&&i.target!==document.getElementById("cd-overlay")||(document.getElementById("cd-overlay").classList.remove("open"),document.body.style.overflow="",Ie=null)}const he=["Permits","Planning","Foundation","Structural","Systems","Finishing","Delivery"],_i={WEATHER:{color:"var(--blue)",bg:"var(--blue-faint)",border:"var(--blue-border)"},SUPPLY:{color:"var(--gold)",bg:"var(--gold-faint)",border:"var(--gold-border)"},LABOR:{color:"var(--orange)",bg:"var(--orange-faint)",border:"var(--orange-border)"},REGULATORY:{color:"var(--red)",bg:"var(--red-faint)",border:"var(--red-border)"},EQUIPMENT:{color:"var(--amber)",bg:"var(--amber-faint)",border:"var(--amber-border)"},POLITICAL:{color:"var(--teal)",bg:"var(--teal-faint)",border:"var(--teal-border)"}},hi={LOW:"var(--green)",MODERATE:"var(--amber)",HIGH:"var(--orange)",CRITICAL:"var(--red)"};let z=null;async function Ke(i){const e=oe.find(A=>A.id===i);if(!e)return;const t=Array.isArray(e.contract_bids)?e.contract_bids[0]:e.contract_bids,n=j?.current_tick||0,o=e.awarded_at_tick||n,s=e.timeline_ticks||8,a=Math.max(0,n-o),l=Math.min(100,a/s*100);let d=Math.min(he.length-1,Math.floor(l/(100/he.length)));const r=Math.round(l%(100/he.length)/(100/he.length)*100),c=e.required_materials||{},m=t?.material_grades||{};let p=[];try{const{data:A}=await $.from("project_material_allocations").select("material_key, quality_tier, quantity, consumed").eq("contract_id",e.id);p=A||[]}catch{}const u={};for(const A of p)u[A.material_key]||(u[A.material_key]={totalAllocated:0,totalConsumed:0,tiers:{}}),u[A.material_key].totalAllocated+=A.quantity,u[A.material_key].totalConsumed+=A.consumed,u[A.material_key].tiers[A.quality_tier]={qty:A.quantity,consumed:A.consumed};const y=Object.entries(c).map(([A,L])=>{const R=m[A]||"STD",O=u[A]||{totalAllocated:0,totalConsumed:0,tiers:{}};return{key:A,name:A.replace(/_/g," ").replace(/\b\w/g,ie=>ie.toUpperCase()),grade:R,required:Number(L),allocated:O.totalAllocated,consumed:O.totalConsumed,tiers:O.tiers,warehouseStock:F[A]||{}}}),g=e.required_equipment||[],_=e.equipment_condition||{},h=g.map(A=>{const L=J.find(Y=>Y.equipment_key===A),O=(L?.assigned_projects||[]).find(Y=>Y.contract_id===e.id),ie=O?O.units:0;return{key:A,name:A.replace(/_/g," ").replace(/\b\w/g,Y=>Y.toUpperCase()),ownedTotal:L?.owned||0,deployed:L?.deployed||0,available:Math.max(0,(L?.owned||0)-(L?.deployed||0)),assignedToProject:ie,condition:_[A]??(L?.condition||100)}}),w=e.budget_ceiling||0,I=t?.estimated_cost||0,C=Math.round(I*Math.min(1,a/s)),k=t?.estimated_quality||65,x=k>=80?"STRONG":k>=60?"PROMISING":k>=40?"FAIR":"UNCERTAIN",T=e.required_workforce||{},q=(T.general||0)+(T.skilled||0),M=t?.labor_count||q;z={project:e,bid:t,tab:"overview",expandedEvent:-1,selectedResponse:null,currentTick:n,awardedTick:o,totalTicks:s,ticksElapsed:a,phaseIdx:d,phaseProgress:r,materials:y,equipment:h,budget:w,estCost:I,spent:C,quality:k,qualityLabel:x,laborCount:M,wfNeeded:q,events:[]},document.getElementById("pm-overlay").classList.add("open"),document.body.style.overflow="hidden",zi(e.id).then(()=>Ae()),Ae()}let V=!1;async function fn(i,e,t){if(!(V||!z||!f)){V=!0;try{const{data:n,error:o}=await $.rpc("allocate_material_to_project",{p_contract_id:z.project.id,p_faction_id:f.id,p_material_key:i,p_quality_tier:e,p_quantity:t});if(o){alert("Allocation failed: "+o.message);return}if(n&&!n.success){alert(n.error||"Allocation failed");return}await ei(),await Ke(z.project.id)}catch(n){alert("Allocation error: "+n.message)}finally{V=!1}}}async function mn(i,e,t){if(!(V||!z||!f)){V=!0;try{const{data:n,error:o}=await $.rpc("deallocate_material_from_project",{p_contract_id:z.project.id,p_faction_id:f.id,p_material_key:i,p_quality_tier:e,p_quantity:t});if(o){alert("Return failed: "+o.message);return}if(n&&!n.success){alert(n.error||"Return failed");return}await ei(),await Ke(z.project.id)}catch(n){alert("Return error: "+n.message)}finally{V=!1}}}async function un(i,e){if(!(V||!z||!f)){V=!0;try{const t=J.find(d=>d.equipment_key===i);if(!t){alert("Equipment not found in inventory.");return}const n=Math.max(0,(t.owned||0)-(t.deployed||0));if(n<e){alert("Not enough available "+i+" ("+n+" available).");return}const o=(t.deployed||0)+e,s=[...t.assigned_projects||[]],a=s.find(d=>d.contract_id===z.project.id);a?a.units+=e:s.push({contract_id:z.project.id,contract_name:z.project.name,units:e});const{error:l}=await $.from("corp_equipment").update({deployed:o,assigned_projects:s}).eq("faction_id",f.id).eq("equipment_key",i);if(l){alert("Deploy failed: "+l.message);return}await oi(),await Ke(z.project.id)}catch(t){alert("Deploy error: "+t.message)}finally{V=!1}}}async function yn(i){if(!(V||!z||!f)){V=!0;try{const e=J.find(l=>l.equipment_key===i);if(!e){alert("Equipment not found.");return}const t=[...e.assigned_projects||[]],n=t.findIndex(l=>l.contract_id===z.project.id);if(n===-1){alert("Equipment not deployed to this project.");return}const o=t[n].units;t.splice(n,1);const s=Math.max(0,(e.deployed||0)-o),{error:a}=await $.from("corp_equipment").update({deployed:s,assigned_projects:t}).eq("faction_id",f.id).eq("equipment_key",i);if(a){alert("Undeploy failed: "+a.message);return}await oi(),await Ke(z.project.id)}catch(e){alert("Undeploy error: "+e.message)}finally{V=!1}}}function gn(i){i&&i.target!==document.getElementById("pm-overlay")||(document.getElementById("pm-overlay").classList.remove("open"),document.body.style.overflow="",z=null)}function vn(i){z&&(z.tab=i,z.expandedEvent=-1,z.selectedResponse=null,Ae())}function xn(i){z&&(z.expandedEvent=z.expandedEvent===i?-1:i,z.selectedResponse=null,Ae())}function bn(i){z&&(z.selectedResponse=z.selectedResponse===i?null:i,Ae())}function Ae(){if(!z)return;const i=z,e=i.project,t=e.issuer_type==="GOVERNMENT",n=Xt(e.sector),o=f?.nation||"Nation",s=i.awardedTick+i.totalTicks,a=Math.max(0,s-i.currentTick),l=i.currentTick>s,d=i.budget>0?Math.round(i.spent/i.budget*100):0,r=d>85?"var(--red)":d>60?"var(--amber)":"var(--teal)",c=i.budget-i.spent,m=i.events.filter(_=>_.status==="ACTIVE").length;document.getElementById("pm-header").innerHTML=`
    ...
    `;let p='<div class="pm-phase__bar">';for(let _=0;_<he.length;_++){const h=_<i.phaseIdx,w=_===i.phaseIdx;p+=`<div class="pm-phase__seg">
            <div class="pm-phase__seg-fill pm-phase__seg-fill--${h?"done":w?"active":"future"}"></div>
            <span class="pm-phase__seg-label pm-phase__seg-label--${h?"done":w?"active":""}">${he[_]}</span>
        </div>`}p+="</div>",p+=`<div class="pm-phase__info">
        <span class="pm-phase__status">${he[i.phaseIdx]} — ${i.phaseProgress}% complete</span>
    ...
    `}function $n(i,e,t,n,o,s,a){const l=Tt(i.awardedTick+i.totalTicks);Tt(i.awardedTick+i.totalTicks);const d=Tt(i.awardedTick),r=[{label:"Budget",value:Z(i.budget),sub:`${n}% spent`,color:t},{label:"Spent",value:Z(i.spent),color:"var(--red)"},{label:"Remaining",value:Z(o),color:"var(--green)"},{label:"Quality",value:`${i.quality}/100`,sub:i.qualityLabel,color:i.quality>=70?"var(--green)":i.quality>=50?"var(--amber)":"var(--red)"},{label:"Workforce",value:`${i.laborCount}/${i.wfNeeded}`,sub:`Bid: ${i.laborCount}`,color:i.laborCount<i.wfNeeded?"var(--orange)":"var(--text-bright)"},{label:"Remaining",value:`${s} ticks`,sub:a?"OVERDUE":`Deadline: ${l}`,color:a?"var(--red)":"var(--text-bright)"}];let c="";c+=`<div style="padding:0 16px"><div class="pm-section">
        ...
        <div class="pm-manager">
            ...
            <span class="pm-evt__status" style="color:${s?"var(--red)":"var(--text-dim)"};font-weight:${s?"700":"400"}">${s?"REQUIRES RESPONSE":"RESOLVED"}</span>
                    ...
                    </div>`,e+=`<div class="pm-resp__detail">${b(r.detail)}</div>`,e+='<div class="pm-resp__costs">',r.cost&&(e+=`<span class="pm-resp__cost" style="color:var(--red)">Cost: ${Z(r.cost)}</span>`),r.qualityImpact&&r.qualityImpact!==0&&(e+=`<span class="pm-resp__cost" style="color:${r.qualityImpact>0?"var(--green)":"var(--red)"}">Quality: ${r.qualityImpact>0?"+":""}${r.qualityImpact}</span>`),!r.cost&&(!r.qualityImpact||r.qualityImpact===0)&&(e+='<span class="pm-resp__cost" style="color:var(--green)">No additional cost</span>'),e+="</div>",c&&(e+=`<div class="pm-resp__confirm">
            ...
            </div>`),e+='<div style="display:flex;align-items:center;justify-content:space-between;padding:4px 0;font-size:0.7rem;">',e+=`<span style="color:var(--text-dim);">Owned: <strong style="color:var(--text-primary);">${t.ownedTotal}</strong>`,e+=` · Deployed: <strong style="color:var(--text-primary);">${t.deployed}</strong>`,e+=` · Available: <strong style="color:var(--text-primary);">${t.available}</strong></span>`,e+='<div style="display:flex;gap:4px;">',!o&&t.available>0&&(e+=`<button onclick="pmDeployEquipment('${t.key}',1)" style="font-size:0.6rem;padding:2px 8px;border:1px solid var(--green);color:var(--green);background:rgba(92,184,92,0.08);border-radius:3px;cursor:pointer;">Deploy</button>`),o&&(e+=`<button onclick="pmUndeployEquipment('${t.key}')" style="font-size:0.6rem;padding:2px 8px;border:1px solid var(--orange);color:var(--orange);background:rgba(232,114,74,0.08);border-radius:3px;cursor:pointer;">Undeploy</button>`),e+="</div></div>",e+="</div>"}return e}function Tt(i){return`${["January","February","March","April","May","June","July","August","September","October","November","December"][i%12]}, ${2e3+Math.floor(i/12)}`}window.openProjectModal=Ke;window.closeProjectModal=gn;window.pmSetTab=vn;window.pmToggleEvent=xn;window.pmSelectResponse=bn;window.pmAllocateMaterial=fn;window.pmDeallocateMaterial=mn;window.pmDeployEquipment=un;window.pmUndeployEquipment=yn;async function zi(i){if(!z)return;const{data:e,error:t}=await $.from("construction_events").select("*").eq("contract_id",i).order("fired_at_tick",{ascending:!1});t?(console.warn("Failed to load project events:",t.message),z.events=[]):z.events=(e||[]).map(n=>({id:n.id,type:n.type,severity:n.severity,tick:n.fired_at_tick,title:n.title,desc:n.description,impact:n.impact,status:n.status==="ACTIVE"?"ACTIVE":"RESOLVED",resolution:n.resolution,responses:n.responses||[]})),Ae()}let Ct=!1;async function kn(i,e){if(!(Ct||!z)){Ct=!0;try{const{data:t,error:n}=await $.rpc("resolve_construction_event",{p_event_id:i,p_response_key:e});if(n){console.error("Failed to resolve event:",n.message),alert("Failed to submit response: "+n.message);return}const o=typeof t=="string"?JSON.parse(t):t;if(o?.error){alert("Error: "+o.error);return}await zi(z.project.id),await qi(),o?.quality_applied&&o.quality_applied!==0&&(z.quality=Math.max(0,Math.min(100,z.quality+o.quality_applied)),z.qualityLabel=z.quality>=80?"STRONG":z.quality>=60?"PROMISING":z.quality>=40?"FAIR":"UNCERTAIN"),Ae()}finally{Ct=!1}}}window.confirmEventResponse=kn;function xe(i,e,t){const n=t?` style="color:${t}"`:"";return`<div class="cd-detail-row">
        <span class="cd-detail-row__label">${b(i)}</span>
        <span class="cd-detail-row__value"${n}>${b(e)}</span>
    </div>`}function En(i){const e={bg:"#1a2a3a",line:"#3a6a8a",dim:"#2a4a5a",accent:"#5a9aba",text:"#4a8aaa"},t=i.drawing_number||i.contract_number+"-A1",n=j?.current_date||"",o=n?n.replace(/,\s*/," "):"",s=i.spec_category==="Heavy Infrastructure",a=i.spec_category==="Megaproject";let l=b(i.project_subtype||i.project_type||"STRUCTURE"),d=s?"80.0m":a?"200.0m":"60.0m",r=s?"40.0m":a?"100.0m":"20.0m";return`<svg viewBox="0 0 680 200" style="width:100%;display:block;background:${e.bg}">
        <!-- Grid -->
        ...
        <g transform="translate(470,172)">
    ...
    </svg>`}async function ue(){if(!f||!f.nation_id)return;const{data:i,error:e}=await $.from("construction_contracts").select("*").in("status",["open","bidding"]).order("generated_at_tick",{ascending:!1});if(e?(console.warn("Failed to load contracts:",e.message),Ne=[]):Ne=i||[],De={},f&&Ne.length>0){const t=Ne.map(o=>o.id),{data:n}=await $.from("contract_bids").select("contract_id, bid_price, estimated_quality, status").eq("faction_id",f.id).in("contract_id",t);for(const o of n||[])De[o.contract_id]=o}Mi()}function Tn(){const i=document.getElementById("ap-list"),e=document.getElementById("ap-footer");if(document.getElementById("ap-count").textContent=oe.length+" ACTIVE",oe.length===0){i.innerHTML=`<div class="ap-empty">
            <div class="ap-empty__text">No active projects.<br>Win a contract bid to start<br>your first construction project.</div>
        </div>`,e.style.display="none";return}const t=j?.current_tick||0;let n=0,o=0,s="";for(const a of oe){const l=a.issuer_type==="GOVERNMENT",d=l?"gov":"private",r=Array.isArray(a.contract_bids)?a.contract_bids[0]:a.contract_bids,c=r?.bid_price||0,m=r?.estimated_cost||0,p=r?.estimated_quality||0,u=a.budget_ceiling||0,y=a.awarded_at_tick||t,g=a.stalled_ticks||0,_=Math.max(0,t-y),h=Math.max(0,_-g),w=a.timeline_ticks||8,I=Math.max(0,w-h),C=Math.min(100,Math.round(h/w*100)),k=h>w,x=g>0;Si(a.sector);const T=Xt(a.sector);n+=u,o+=c,s+=`<div class="ap-item" onclick="openProjectModal('${a.id}')">
            ...
            <div class="ap-budget">
                <div class="ap-budget__header">
                    ...
                    <span class="ap-budget__values" style="color:${k?"var(--red)":x?"var(--orange)":"var(--teal)"}">
                        ${h}/${w} ticks ${k?'<span style="color:var(--red);font-weight:700;"> OVERDUE</span>':""}
                    </span>
                </div>
                <div class="ap-budget__bar">
                    <div class="ap-budget__fill" style="width:${C}%;background:${k?"var(--red)":x?"var(--orange)":"var(--teal)"}"></div>
                </div>
            </div>
            <div class="ap-details">
                <div class="ap-detail-cell" style="flex:1">
                    <div class="ap-detail-cell__label">YOUR BID</div>
                    <div class="ap-detail-cell__value" style="font-size:10px;font-weight:700;color:var(--gold)">${Z(c)}</div>
                </div>
                <div class="ap-detail-cell" style="flex:1">
                    <div class="ap-detail-cell__label">EST. COST</div>
                    <div class="ap-detail-cell__value" style="font-size:10px;font-weight:700;color:var(--red)">${Z(m)}</div>
                </div>
                ...
                <div class="ap-detail-cell" style="flex:1">
                    ...
                    <div class="ap-detail-cell__value" style="font-size:10px;font-weight:700;color:${k?"var(--red)":"var(--text-bright)"}">${I} ticks</div>
                </div>
            </div>
        </div>`}i.innerHTML=s,e.style.display=oe.length>0?"":"none",oe.length>0&&(document.getElementById("ap-total-crew").textContent=oe.length,document.getElementById("ap-total-budget").textContent=Z(n),document.getElementById("ap-total-spent").textContent=Z(o))}async function qi(){if(!f)return;const{data:i,error:e}=await $.from("construction_contracts").select("*, contract_bids!inner(bid_price, material_grades, labor_count, estimated_cost, estimated_quality)").eq("awarded_to_faction",f.id).in("status",["awarded","in_progress"]).eq("contract_bids.faction_id",f.id).order("awarded_at_tick",{ascending:!0});e?(console.warn("Failed to load active projects:",e.message),oe=[]):oe=i||[],Tn()}const st=3e4;function rt(){let i=0,e=0;for(const t of me)for(const n of Jt){const o=F[t.key]?.[n];o&&(i+=o.qty,e+=o.value)}return{totalUnits:i,totalValue:e}}function Zt(){const i=document.getElementById("wh-list"),{totalUnits:e,totalValue:t}=rt();document.getElementById("wh-count").textContent=e.toLocaleString()+" UNITS",document.getElementById("wh-total-value").textContent=B(t);const n=Math.round(e/st*100),o=document.getElementById("wh-capacity");o.textContent=n+"%",o.style.color=n>80?"var(--red)":n>50?"var(--orange)":"var(--green)";let s="";for(let a=0;a<me.length;a++){const l=me[a],d=Ht===a,r=F[l.key]?.LOW||{qty:0,value:0},c=F[l.key]?.STD||{qty:0,value:0},m=F[l.key]?.HIGH||{qty:0,value:0},p=r.qty+c.qty+m.qty,u=r.value+c.value+m.value,y=p===0,g=Ce(l.key,"LOW",E),_=Ce(l.key,"STD",E),h=Ce(l.key,"HIGH",E),w=r.qty>0?"wh-dot wh-dot--low":"wh-dot wh-dot--empty",I=c.qty>0?"wh-dot wh-dot--std":"wh-dot wh-dot--empty",C=h.available?m.qty>0?"wh-dot wh-dot--high":"wh-dot wh-dot--empty":"wh-dot wh-dot--locked";if(s+='<div class="wh-row">',s+=`<div class="wh-row__collapsed${d?" expanded":""}" onclick="toggleWhRow(${a})">
            ...
            <span class="wh-row__val${y?" empty":""}">${u>0?B(u):"—"}</span>
                ...
                <span class="wh-expand__hdr-label" style="width:55px;text-align:right">$/UNIT</span>
            </div>`;const k=[{key:"LOW",label:"Low",data:r,avail:g,color:"var(--orange)",dotClass:"wh-dot--low"},{key:"STD",label:"Standard",data:c,avail:_,color:"var(--amber)",dotClass:"wh-dot--std"},{key:"HIGH",label:"High",data:m,avail:h,color:"var(--green)",dotClass:"wh-dot--high"}];for(const x of k){const T=!x.avail.available,q=x.data.qty>0,M=q?"$"+Math.round(x.data.value/x.data.qty):"—";s+=`<div class="wh-grade${T?" unavailable":""}">
                    ...
                    <span class="wh-grade__qty" style="color:${q?"var(--text-bright)":"var(--text-dim)"}">${q?x.data.qty.toLocaleString():"—"}</span>
                ...
                </div>`}for(const x of k)!x.avail.available&&x.avail.failedStat&&(s+=`<div class="wh-lock">
                        <span class="wh-lock__text">${x.label.toUpperCase()} GRADE LOCKED — ${b(x.avail.failedStat)} &lt; ${x.avail.failedMin}</span>
                    </div>`);s+="</div>"}s+="</div>"}i.innerHTML=s}function Cn(i){Ht=Ht===i?-1:i,Zt()}async function ei(){if(!f)return;const{data:i,error:e}=await $.from("corp_warehouse").select("material_key, quality_tier, quantity, total_value").eq("faction_id",f.id);if(F={},e)console.warn("Failed to load warehouse:",e.message);else if(i)for(const t of i)F[t.material_key]||(F[t.material_key]={}),F[t.material_key][t.quality_tier]={qty:t.quantity||0,value:Number(t.total_value)||0};Zt()}const Sn={manufacturing_output:"base production",rare_minerals:"raw input",inflation:"price modifier",fuel_prices:"transport cost",urbanization:"demand pressure",arable_land:"base production",physical_infrastructure:"transport",digital_infrastructure:"component quality",energy_generation:"electrical supply",standard_of_living:"demand tier",oil_and_gas:"base input",higher_education:"engineering"};function Li(){const i=(E?.name||f?.nation||"—").toUpperCase();document.getElementById("pr-nation-badge").textContent="LOCAL — "+i;const e=Number(f?.corp_cash_reserves)||0;document.getElementById("pr-cash").textContent=B(e);const{totalUnits:t}=rt(),n=Math.round(t/st*100),o=document.getElementById("pr-wh-capacity");o.textContent=n+"%",o.style.color=n>80?"var(--red)":n>50?"var(--orange)":"var(--green)",Ni(),ti(),lt()}function Ni(){const i=document.getElementById("pr-mat-grid");let e="";for(const t of me){const n=H===t.key,o=Jt.every(a=>!Ce(t.key,a,E).available),s="pr-mat-btn"+(n?" active":"")+(o?" all-locked":"");e+=`<span class="${s}" onclick="setPrMat('${t.key}')">${b(t.name)}</span>`}i.innerHTML=e}function ti(){const i=document.getElementById("pr-tier-bar");let e='<span class="pr-tier-label">GRADE</span>';for(const t of Jt){const n=Ce(H,t,E),o=U===t,s=n.available?Kt(H,t,E):null,a=Ti[t],l=!n.available,d="pr-tier-btn"+(o?" active":"")+(l?" locked":"");e+=`<div class="${d}" onclick="${l?"":`setPrTier('${t}')`}">
            <div class="pr-tier-btn__label">
                <div class="wh-dot" style="width:5px;height:5px;background:${a};border-radius:1px;"></div>
                <span class="pr-tier-btn__name" style="color:${o?"var(--text-bright)":"var(--text-dim)"}">${jt[t]}</span>
            </div>
        ...
        </div>`}i.innerHTML=e}function lt(){const i=document.getElementById("pr-content"),e=Ce(H,U,E),t=me.find(k=>k.key===H);if(!t)return;if(!e.available){i.innerHTML=`<div class="pr-locked-panel">
        ...
        <div class="pr-drivers__title">Price Drivers (${b(E?.name||"—")})</div>`;for(const k of t.priceDrivers){const x=Number(E?.[k]??50),T=x>=50?"var(--green)":x>=30?"var(--amber)":x>=15?"var(--orange)":"var(--red)",q=Sn[k]||k;p+=`<div class="pr-driver-row">
            <span class="pr-driver-row__stat">${b(k)}</span>
        ...
        </div>`}p+="</div>";const y=(Number(f?.corp_cash_reserves)||0)>=s,g=ae>o,{totalUnits:_}=rt(),h=st-_,w=ae>h,I=h<=0,C=Ti[U];p+=`<div class="pr-order">
        ...
        <div class="pr-order__box">
            <div class="pr-order__header">
                <div style="display:flex;align-items:center;gap:6px;">
                    ...
                    <span style="font-family:var(--font-mono);font-size:9px;color:${C}">${jt[U]}</span>
                </div>
            ...
            <div class="pr-qty-row">
                ...
                <div class="pr-qty-btns">
                    ${[100,250,500,1e3].map(k=>`<span class="pr-qty-btn${ae===k?" active":""}" onclick="setPrQty(${k})">${k>=1e3?k/1e3+"k":k}</span>`).join("")}
                </div>
            </div>
            ...
            </div>`:w?`<div class="pr-supply-warn">
            ...
            <div class="pr-order__total">
                ...
                <button class="pr-purchase-btn" onclick="purchaseMaterial()"
                    ${y&&!g&&!w&&!I?"":"disabled"}
                    title="${y?g?"Exceeds supply":I?"Warehouse full":w?"Exceeds warehouse capacity":"Purchase materials":"Insufficient cash"}"
                >PURCHASE</button>
            </div>
        </div>
    </div>`,i.innerHTML=p}function In(i){H=i,U="STD";for(const e of["STD","HIGH","LOW"])if(Ce(i,e,E).available){U=e;break}Ni(),ti(),lt()}function Mn(i){U=i,ti(),lt()}function An(i){ae=i,lt()}let St=!1;async function zn(){if(St||!f||!E)return;const i=Kt(H,U,E),e=Ci(H,U,E),t=i*ae,n=Number(f.corp_cash_reserves)||0;if(t>n){alert("Insufficient cash reserves.");return}if(ae>e){alert("Exceeds available supply this tick.");return}const{totalUnits:o}=rt(),s=st-o;if(s<=0){alert("Warehouse is full. Cannot purchase more materials.");return}if(ae>s){alert(`Warehouse can only hold ${s.toLocaleString()} more units. Reduce quantity.`);return}St=!0;const a=document.querySelector(".pr-purchase-btn");a&&(a.disabled=!0,a.textContent="...");try{const l=n-t,{error:d}=await $.from("factions").update({corp_cash_reserves:l}).eq("id",f.id);if(d)throw d;const r=F[H]?.[U],c=(r?.qty||0)+ae,m=(r?.value||0)+t,{error:p}=await $.from("corp_warehouse").upsert({faction_id:f.id,nation_id:f.nation_id,material_key:H,quality_tier:U,quantity:c,total_value:m,last_purchased_tick:j?.current_tick||null,updated_at:new Date().toISOString()},{onConflict:"faction_id,material_key,quality_tier"});if(p){const{error:u}=await $.from("factions").update({corp_cash_reserves:n}).eq("id",f.id);throw u&&console.error("Cash refund failed after warehouse error:",u.message),p}f.corp_cash_reserves=l,F[H]||(F[H]={}),F[H][U]={qty:c,value:m},Zt(),Li(),a&&(a.textContent="PURCHASED",setTimeout(()=>{a.isConnected&&(a.disabled=!1,a.textContent="PURCHASE")},1500))}catch(l){a&&(a.disabled=!1,a.textContent="PURCHASE"),alert("Purchase failed: "+(l.message||"Unknown error"))}finally{St=!1}}function Bi(i){const e=Se||E;if(!e)return[];const t=at(i);if(!t)return[];const n=sn(i,e),o=[],s=Number(e?.inflation??50),a=Number(e?.fuel_prices??50);Number(e?.manufacturing_output??50);const l=Se&&E&&Se.id!==E.id;let d=null;if(l&&(d=rn(e,E)),n.newAvailable>0){const r=bi(i,e),c=t.basePrice,m=Math.round(c*((s-50)/200)),p=Math.round(c*((a-50)/300));let u=r;const y=[{label:"Base price",value:B(c)},m!==0?{label:`Inflation (${s})`,mod:(m>=0?"+":"")+B(Math.abs(m))}:null,p!==0?{label:`Fuel transport (${a})`,mod:(p>=0?"+":"")+B(Math.abs(p))}:null].filter(Boolean),g=r-c-m-p;if(g!==0&&!l&&y.push({label:"Demand/scarcity",mod:(g>=0?"+":"")+B(Math.abs(g))}),l&&d){const _=Math.round(r*d.tariff),h=Math.round(r*d.transport);u=r+_+h,y.push({label:`Import tariff (${Math.round(d.tariff*100)}%)`,mod:"+"+B(_)}),y.push({label:`Transport (${d.deliveryTicks} tick${d.deliveryTicks>1?"s":""})`,mod:"+"+B(h)})}o.push({seller:l?`${e.name} Buyers`:"Local Buyers",sellerType:"LOCAL",nation:e.name||"—",distance:l?d?.deliveryTicks||1:0,condition:100,price:Math.round(u),available:n.newAvailable,delivery:l?d.deliveryTicks+" tick"+(d.deliveryTicks>1?"s":""):"Immediate",deliveryTicks:l?d.deliveryTicks:0,used:!1,priceFactors:y,sourceNationId:e.id})}if(n.usedAvailable>0){const r=n.usedCondition,c=bi(i,e,{used:!0,condition:r});let m=c;const p=[{label:"Base price",value:B(t.basePrice)},{label:`Condition (${r}%)`,mod:"-"+B(Math.max(0,t.basePrice-c))}];if(l&&d){const u=Math.round(c*d.tariff),y=Math.round(c*d.transport);m=c+u+y,p.push({label:`Import tariff (${Math.round(d.tariff*100)}%)`,mod:"+"+B(u)}),p.push({label:`Transport (${d.deliveryTicks} tick${d.deliveryTicks>1?"s":""})`,mod:"+"+B(y)})}o.push({seller:l?`${e.name} Buyers`:"Local Buyers",sellerType:"LOCAL",nation:e.name||"—",distance:l?d?.deliveryTicks||1:0,condition:r,price:Math.round(m),available:n.usedAvailable,delivery:l?d.deliveryTicks+" tick"+(d.deliveryTicks>1?"s":""):"Immediate",deliveryTicks:l?d.deliveryTicks:0,used:!0,priceFactors:p,sourceNationId:e.id})}return o}function dt(){const i=Number(f?.corp_cash_reserves)||0;document.getElementById("em-cash").textContent=B(i);const e=at(X),t=We[e?.tier||1],n=document.getElementById("em-tier-badge");n&&(n.textContent=t.tag,n.style.color=t.color),n.style.background=t.color+"0a",n.style.border="1px solid "+t.color+"33";const o=document.getElementById("em-nation-select");if(o&&o.options.length===0){const l=E?.name||f?.nation||"—";let d=`<option value="">${b(l)} (HQ)</option>`;for(const r of nt)r.id!==E?.id&&(d+=`<option value="${r.id}">${b(r.name)}</option>`);o.innerHTML=d}const s=document.getElementById("em-import-tag"),a=Se&&E&&Se.id!==E.id;s&&(s.style.display=a?"":"none"),qn(),ii()}function qn(){let i="";for(let e=1;e<=3;e++){const t=We[e],n=Dt(e),o=e===3?"em-selector__grid em-selector__grid--t3":"em-selector__grid em-selector__grid--t12";i+=`<div class="em-selector__row">
            <div class="em-selector__tier-label" style="color:${t.color}">${t.tag}</div>
            <div class="${o}">`;for(const s of n){const a=X===s.key,l=Bi(s.key).length>0;i+=`<span class="em-selector__btn${a?" active":""}${l?"":" no-listings"}"
                style="${a?"background:"+t.color+";border-color:"+t.color:""}"
                onclick="setEmType('${s.key}')">${b(s.name)}</span>`}i+="</div></div>"}document.getElementById("em-selector").innerHTML=`<div class="em-selector">${i}</div>`}function ii(){const i=document.getElementById("em-content");if(be=Bi(X),be.length===0){i.innerHTML=`<div class="em-no-listings"><div class="em-no-listings__box">
        ...
        </div></div>`;return}de>=be.length&&(de=0);let e="";for(let n=0;n<be.length;n++){const o=be[n],s=de===n,a=o.sellerType==="LOCAL"?"var(--teal)":"#5a8aaa",l=Ei(o.condition);e+=`<div class="em-listing${s?" selected":""}" style="${s?"border-left-color:"+a:""}" onclick="setEmListing(${n})">`,e+=`<div class="em-listing__row1">
            <div style="display:flex;align-items:center;gap:6px">
            ...
            </div>`),e+="</div>"}const t=be[de];if(t){const n=at(X),o=We[n?.tier||1],s=Math.min(t.available,4),a=t.price*ce,l=(Number(f?.corp_cash_reserves)||0)>=a;e+=`<div class="em-purchase"><div class="em-purchase__box">
            ...
            <div class="em-purchase__qty">
                ...
                <div class="em-purchase__qty-btns">
                    ${Array.from({length:s},(d,r)=>r+1).map(d=>`<span class="em-qty-btn${ce===d?" active":""}" style="${ce===d?"background:"+o.color+";border-color:"+o.color:""}" onclick="setEmQty(${d})">${d}</span>`).join("")}
                </div>
            ...
            <div class="em-purchase__total">
        ...
        </div></div>`}i.innerHTML=e}async function Ln(i){if(!i)Se=null;else{let t=nt.find(n=>n.id===i);if(!t)try{const{data:n}=await $.from("nations").select("*").eq("id",i).single();t=n}catch{}Se=t||null}de=0,ce=1;const e=document.getElementById("em-nation-select");e&&(e.value=i||""),dt()}function Nn(i){X=i,de=0,ce=1,dt()}function Bn(i){de=i,ce=1,ii()}function Rn(i){ce=i,ii()}let It=!1;async function On(){if(It)return;const i=be[de];if(!i||!f)return;const e=at(X);if(!e)return;const t=ce,n=i.price*t,o=Number(f.corp_cash_reserves)||0;if(n>o){alert("Insufficient cash reserves.");return}if(t>i.available){alert("Not enough units available.");return}const s=document.querySelector(".em-purchase-btn");s&&(s.disabled=!0,s.textContent="..."),It=!0;try{const a=o-n,{error:l}=await $.from("factions").update({corp_cash_reserves:a}).eq("id",f.id);if(l)throw l;const d=!i.deliveryTicks||i.deliveryTicks===0;if(d){const c=J.find(I=>I.equipment_key===X),m=(c?.owned||0)+t,p=c?.purchase_price_avg||0,u=c?.owned||0,y=u>0?Math.round((p*u+i.price*t)/m):i.price,g=e.maintenancePerUnit*m,_=c?.condition||100,h=Math.round((_*u+i.condition*t)/m),{error:w}=await $.from("corp_equipment").upsert({faction_id:f.id,nation_id:f.nation_id,equipment_key:X,tier:e.tier,owned:m,deployed:c?.deployed||0,condition:h,maintenance_per_tick:g,purchase_price_avg:y,last_purchased_tick:j?.current_tick||null,updated_at:new Date().toISOString()},{onConflict:"faction_id,equipment_key"});if(w){const{error:I}=await $.from("factions").update({corp_cash_reserves:o}).eq("id",f.id);throw I&&console.error("Cash refund failed:",I.message),w}c?(c.owned=m,c.condition=h,c.maintenance_per_tick=g):J.push({equipment_key:X,tier:e.tier,owned:m,deployed:0,condition:h,maintenance_per_tick:g,assigned_projects:[]})}else{const c=(j?.current_tick||0)+i.deliveryTicks,{error:m}=await $.from("corp_equipment_deliveries").insert({faction_id:f.id,equipment_key:X,quantity:t,condition:i.condition,delivery_tick:c,source_nation_id:i.sourceNationId||null,seller_name:i.seller,price_paid:n});if(m){const{error:p}=await $.from("factions").update({corp_cash_reserves:o}).eq("id",f.id);throw p&&console.error("Cash refund failed:",p.message),m}}f.corp_cash_reserves=a,ni(),dt();const r=document.getElementById("pr-cash");r&&(r.textContent=B(a)),s&&(s.textContent=d?"PURCHASED":"ORDERED",setTimeout(()=>{s.isConnected&&(s.disabled=!1,s.textContent="PURCHASE")},1500))}catch(a){s&&(s.disabled=!1,s.textContent="PURCHASE"),alert("Purchase failed: "+(a.message||"Unknown error"))}finally{It=!1}}let Pn=-1,Ge=[],Vt=[],Ri=[];function Mt(i){return Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(1)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i.toLocaleString()}function Dn(i,e,t){if(t)return"var(--orange)";const n=i/(e||1)*100;return n>50?"var(--green)":n>25?"var(--amber)":"var(--red)"}function jn(){const i=document.getElementById("pm-list"),e=Ge.length,t=Vt.length,n=Ri.length,o=Ge.filter(d=>d.expiring_soon).length;document.getElementById("pm-active-count").textContent=`(${e})`,document.getElementById("pm-pending-count").textContent=`(${t})`,document.getElementById("pm-apply-count").textContent=`(${n})`;const s=document.getElementById("pm-badges");let a="";o>0&&(a+=`<span class="pm-badge pm-badge--expiring">${o} EXPIRING</span>`),t>0&&(a+=`<span class="pm-badge pm-badge--pending">${t} PENDING</span>`),s.innerHTML=a;const l=Ge.reduce((d,r)=>d+(r.cost||0),0)+Vt.reduce((d,r)=>d+(r.cost||0),0);document.getElementById("pm-total-cost").textContent=Mt(l),document.getElementById("pm-footer-active").textContent=e,document.getElementById("pm-footer-pending").textContent=t;{if(e===0){i.innerHTML=`<div class="pm-empty">
            ...
            </div>`;return}let d="";Ge.forEach((r,c)=>{const m=Pn===c,p=Dn(r.ticks_left,r.total_ticks,r.expiring_soon),u=Math.min(r.ticks_left/(r.total_ticks||1)*100,100);d+=`<div class="pm-item ${r.expiring_soon?"pm-item--expiring":""} ${m?"expanded":""}" onclick="togglePmExpand(${c})">
                <div class="pm-item__inner">
                    ...
                    <div class="pm-detail__row">
                        ...
                        <span class="pm-detail__val">${Mt(r.cost||0)}</span>
                    </div>
                    <div class="pm-detail__row">
                    ...
                    </div>`,r.note&&(d+=`<div class="pm-note"><span class="pm-note__text">${b(r.note)}</span></div>`),r.expiring_soon&&r.renewable&&(d+=`<div class="pm-btn-row"><button class="pm-btn pm-btn--renew">RENEW — ${Mt(r.cost||0)}</button></div>`),d+="</div>"),d+="</div></div>"}),i.innerHTML=d;return}}function Hn(){Ge=[],Vt=[],Ri=[],jn()}let we=[],Un=-1;function le(i){return Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(2)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i.toLocaleString()}function wi(i){return i>=85?"var(--gold)":i>=60?"var(--green)":i>=40?"var(--orange)":"var(--red)"}function Gn(i){return"dl-result--"+i.toLowerCase()}function ki(){const i=document.getElementById("dl-list"),e=we.length;document.getElementById("dl-count").textContent=`${e} COMPLETED`;const t=we.reduce((l,d)=>{const r=d.financials||{};return l+((r.payment||0)+(r.bonus||0)-(r.penalty||0)-(r.total_cost||0))},0),n=document.getElementById("dl-lifetime-profit");n.textContent=(t>=0?"+":"")+le(t),n.style.color=t>=0?"var(--green)":"var(--red)";const o={};we.forEach(l=>{o[l.result]=(o[l.result]||0)+1});const s=document.getElementById("dl-footer-results");if(s.innerHTML=Object.entries(o).map(([l,d])=>`<div class="dl-footer__result-box">
        ...
        </div>`;return}let a="";we.forEach((l,d)=>{const r=Un===d,c=l.financials||{},m=(c.payment||0)+(c.bonus||0)-(c.penalty||0)-(c.total_cost||0),p=m>=0,u=Gn(l.result),g={DISTINCTION:"var(--gold)",PASS:"var(--green)",CONDITIONAL:"var(--orange)",FAIL:"var(--red)"}[l.result]||"var(--text-dim)",_=l.type==="GOVERNMENT";if(a+=`<div class="dl-item ${r?"expanded":""}" onclick="toggleDlExpand(${d})">
            <div class="dl-item__inner" style="border-left:2px solid ${g}">
                ...
                <div class="dl-summary-bar">
                    ...
                    <div class="dl-summary-cell" style="flex:1.3;">
                        ...
                        <div class="dl-summary-value" style="color:${p?"var(--green)":"var(--red)"};margin-top:2px;">${p?"+":""}${le(m)}</div>
                    </div>
                </div>`,r){const h=l.inspection||{};a+='<div style="margin-top:8px;">',a+='<div class="dl-section-label">Inspection Report</div>',["materials","structural","systems"].forEach(k=>{const x=h[k]||{score:0,issues:[]},T=wi(x.score),q=Math.min(x.score/100*100,100);a+=`<div class="dl-inspect-row">
                    <div class="dl-inspect-row__header">
                        <span class="dl-inspect-row__label">${b(k.charAt(0).toUpperCase()+k.slice(1))}</span>
                        <div class="dl-inspect-row__score-area">
                            <div class="dl-inspect-bar"><div class="dl-inspect-bar__fill" style="width:${q}%;background:${T}"></div></div>
                            <span class="dl-inspect-row__score" style="color:${T}">${x.score}</span>
                        </div>
                    </div>
                    ${(x.issues||[]).map(M=>`<div class="dl-inspect-issue">${b(M)}</div>`).join("")}
                </div>`});const w=h.permits||{passed:!0,issues:[]};a+=`<div class="dl-permits-row ${w.passed?"dl-permits-row--pass":"dl-permits-row--fail"}">
                ...
                ${(w.issues||[]).map(k=>`<div class="dl-inspect-issue dl-inspect-issue--red">${b(k)}</div>`).join("")}
            </div>`,a+='<div class="dl-section-label" style="margin-top:8px;">Material Quality Impact</div>',a+='<div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:8px;">',(l.materials_used||[]).forEach(k=>{const x=k.grade==="HIGH"?"var(--green)":k.grade==="STANDARD"?"var(--amber)":"var(--orange)",T=k.impact==="positive"?"▲":k.impact==="negative"?"▼":"–",q=k.impact==="positive"?"var(--green)":k.impact==="negative"?"var(--red)":"var(--text-dim)";a+=`<div class="dl-mat-tag">
                    <span class="dl-mat-tag__name">${b(k.name)}</span>
                    <div class="dl-mat-tag__dot" style="background:${x}"></div>
                    <span class="dl-mat-tag__grade" style="color:${x}">${b(k.grade)}</span>
                    <span class="dl-mat-tag__impact" style="color:${q}">${T}</span>
                </div>`}),a+="</div>",a+='<div class="dl-section-label">Financial Summary</div>',a+='<div class="dl-fin-panel">',a+=`<div class="dl-fin-row"><span class="dl-fin-key">CONTRACT VALUE</span><span class="dl-fin-val">${le(c.contract_value||0)}</span></div>`,(c.bonus||0)>0&&(a+=`<div class="dl-fin-row"><span class="dl-fin-key">QUALITY BONUS</span><span class="dl-fin-val" style="color:var(--green)">+${le(c.bonus)}</span></div>`),(c.penalty||0)>0&&(a+=`<div class="dl-fin-row"><span class="dl-fin-key">PENALTIES</span><span class="dl-fin-val" style="color:var(--red)">-${le(c.penalty)}</span></div>`);const I=(c.payment||0)+(c.bonus||0)-(c.penalty||0);a+=`<div class="dl-fin-row"><span class="dl-fin-key">PAYMENT RECEIVED</span><span class="dl-fin-val" style="color:var(--green)">${le(I)}</span></div>`,a+=`<div class="dl-fin-row"><span class="dl-fin-key">TOTAL COST</span><span class="dl-fin-val" style="color:var(--red)">-${le(c.total_cost||0)}</span></div>`,a+=`<div class="dl-fin-total">
                <span class="dl-fin-total__label">${p?"NET PROFIT":"NET LOSS"}</span>
                <span class="dl-fin-total__value" style="color:${p?"var(--green)":"var(--red)"}">${p?"+":""}${le(m)}</span>
            </div>`,a+="</div>";const C=l.timeline||{};a+=`<div class="dl-timeline">
                <span class="dl-timeline__label">TIMELINE</span>
                <span class="dl-timeline__ticks">${C.actual||0}/${C.expected||0} ticks</span>`,C.early?a+=`<span class="dl-timeline__badge dl-timeline__badge--early">${(C.expected||0)-(C.actual||0)} TICK${C.expected-C.actual!==1?"S":""} EARLY</span>`:!C.on_time&&C.actual>C.expected&&(a+=`<span class="dl-timeline__badge dl-timeline__badge--late">${(C.actual||0)-(C.expected||0)} TICK${C.actual-C.expected!==1?"S":""} LATE</span>`),a+="</div>",a+="</div>"}a+="</div></div>"}),i.innerHTML=a}async function Fn(){if(!f){we=[],ki();return}const{data:i,error:e}=await $.from("construction_deliveries").select("*, construction_contracts(name, sector, issuer_name, issuer_type, timeline_ticks)").eq("faction_id",f.id).order("delivered_at_tick",{ascending:!1}).limit(20);e?(console.warn("Failed to load deliveries:",e.message),we=[]):we=(i||[]).map(t=>{const n=t.construction_contracts||{};return{id:t.contract_id,name:n.name||"Project",type:n.issuer_type||"GOVERNMENT",issuer:n.issuer_name||"Government",delivered:"Tick "+(t.delivered_at_tick||0),result:t.result,quality_score:t.quality_score,rep_change:t.rep_change,financials:{contract_value:t.contract_value||0,bonus:t.quality_bonus||0,penalty:t.penalties||0,payment:t.payment_received||0,total_cost:t.total_cost||0},inspection:t.inspection||{},materials_used:t.materials_used||[],timeline:{expected:t.timeline_expected||0,actual:t.timeline_actual||0,on_time:t.on_time,early:t.timeline_actual<t.timeline_expected}}}),ki()}function ni(){const i=J.reduce((l,d)=>l+(d.owned||0),0),e=J.reduce((l,d)=>l+(d.deployed||0),0),t=an(J),n=i-e;document.getElementById("eq-count").textContent=i+" UNITS",document.getElementById("eq-summary").innerHTML=`
        ...
        </div>`;const o={};for(const l of J)o[l.equipment_key]=l;let s="";for(let l=1;l<=3;l++){const d=We[l],r=Dt(l),c=Ut===l,m=r.reduce((u,y)=>u+(o[y.key]?.owned||0),0),p=r.reduce((u,y)=>u+(o[y.key]?.deployed||0),0);if(s+=`<div class="eq-tier-hdr" onclick="toggleEqTier(${l})">
            ...
            ${m>0?`<span class="eq-tier-hdr__count">${p}/${m}</span>`:'<span class="eq-tier-hdr__none">NONE OWNED</span>'}
        </div>`,c)for(const u of r){const y=o[u.key],g=y?.owned||0,_=y?.deployed||0,h=y?.condition||0,w=u.maintenancePerUnit*g,I=g-_,C=g>0&&I===0,k=g>0&&h<65,x=Ei(h),T=y?.assigned_projects||[],q=T.length>0?T.map(M=>M.contract_name||"Project").join(", ").slice(0,30):g>0&&_>0?_+" project"+(_>1?"s":""):"—";s+=`<div class="eq-row${g===0?" unowned":""}">`,s+=`<div class="eq-row__top">
                    ...
                    ${g>0?`<div class="eq-row__right">
                            <div style="display:flex;gap:3px">
                                <span class="eq-row__free" style="color:${C?"var(--orange)":"var(--green)"}">${I}</span>
                    ...
                    </div>`:s+='<div class="eq-row__hint">Purchase from Equipment Market →</div>',s+="</div>"}}document.getElementById("eq-list").innerHTML=s;const a=[1,2,3].map(l=>{const d=We[l],r=Dt(l).reduce((c,m)=>c+(o[m.key]?.owned||0),0);return`<div class="eq-footer__tier-box" style="border-color:${r>0?d.color+"33":"var(--border-0)"};background:${r>0?d.color+"0a":"transparent"}">
        ...
        <div class="eq-footer__tiers">${a}</div>`}function Wn(i){Ut=Ut===i?-1:i,ni()}async function oi(){if(!f)return;const{data:i,error:e}=await $.from("corp_equipment").select("equipment_key, tier, owned, deployed, condition, maintenance_per_tick, assigned_projects").eq("faction_id",f.id);e?(console.warn("Failed to load equipment:",e.message),J=[]):J=i||[],ni()}async function Vn(){const{data:{user:i}}=await $.auth.getUser();if(!i){window.location.href="login.html";return}const{data:e}=await $.from("factions").select("*").or(`id.eq.${i.id},linked_user_id.eq.${i.id}`);Ue=(e||[]).filter(r=>r.nation_id);const t=sessionStorage.getItem("active_faction_id");if(f=Ue.find(r=>r.id===t)||Ue.find(r=>r.faction_type==="corporation")||Ue[0],!f){await $.auth.signOut(),window.location.href="login.html";return}if(f.faction_type!=="corporation"){window.location.href="dashboard.html";return}const[n,o]=await Promise.all([f.nation_id?$.from("nations").select("*").eq("id",f.nation_id).single():Promise.resolve({data:null}),$.from("shard").select("current_tick, current_date, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single()]);n.error&&console.warn("Nation load failed:",n.error.message),n.data&&(E=n.data),o.error&&console.warn("Shard load failed:",o.error.message),j=o.data;const s=f.corp_ticker||f.abbreviation||"";if(document.getElementById("corp-logo").textContent=s.slice(0,2)||"—",document.getElementById("corp-name-bar").textContent=f.faction_name||"Unnamed Corp",j){if(document.getElementById("game-date").textContent=j.current_date||"—",document.getElementById("tick-number").textContent=j.current_tick||"—",j.next_tick_at){const c=(Number(j.tick_interval_hours)||8)*36e5,m=new Date(j.next_tick_at).getTime(),u=m-c+c/2;Gt=new Date(u>Date.now()?u:m+c/2),dn()}const r=document.querySelector("#tick-countdown")?.closest(".tick-item")?.querySelector(".tick-label");r&&(r.textContent="Next Corp Tick")}document.getElementById("corp-name-badge").textContent=(s?"["+s+"]":f.faction_name||"Corp")+" ▾";const a=document.getElementById("topbar-cash");if(a){const r=Number(f.corp_cash_reserves??0),c=r>=1e9?"$"+(r/1e9).toFixed(1)+"B":r>=1e6?"$"+(r/1e6).toFixed(1)+"M":"$"+Math.round(r/1e3)+"k";a.textContent="CASH: "+c}const l=f.action_points??0;document.getElementById("topbar-ap").innerHTML='<span class="topbar-ap__count">'+l+" AP</span>",document.getElementById("nation-pill").textContent=(E?.name||f.nation||"—").toUpperCase();const d=document.getElementById("corp-faction-dropdown");if(d){let r="";for(const c of Ue){const m=c.id===f.id,p=c.faction_type==="corporation"?"CORP":"PARTY",u=c.faction_type==="corporation"?"var(--teal)":"var(--amber)";r+=`<div class="corp-dd-item${m?" active":""}" onclick="switchToFaction('${c.id}', '${c.faction_type}')">
            ...
            </div>`}d.innerHTML=r}await Promise.all([ue(),qi(),ei(),oi(),Hn(),Fn(),Je()]);try{const{data:r}=await $.from("nations").select("*").order("name");nt=r||[]}catch{nt=[]}if(Li(),dt(),on(f,E,j),document.getElementById("loading").style.display="none",document.getElementById("page-content").style.display="block",new URLSearchParams(window.location.search).get("tab")==="expansion"){const r=document.querySelector('[data-tab="expansion"]');r&&Pi({preventDefault:()=>{},target:r})}}async function Yn(){await $.auth.signOut(),window.location.href="login.html"}function Qn(){const i=document.getElementById("corp-faction-dropdown");i&&i.classList.toggle("open")}function Kn(i,e){const t=document.getElementById("corp-faction-dropdown");t&&t.classList.remove("open"),sessionStorage.setItem("active_faction_id",i),e==="corporation"?window.location.href="corp-operations.html":window.location.href="dashboard.html"}document.addEventListener("click",i=>{const e=document.getElementById("faction-switcher"),t=document.getElementById("corp-faction-dropdown");t&&e&&!e.contains(i.target)&&t.classList.remove("open")});document.addEventListener("keydown",i=>{i.key==="Escape"&&Qe()});window.doLogout=Yn;window.toggleTheme=cn;window.toggleCorpDropdown=Qn;window.switchToFaction=Kn;window.setFilter=pn;window.openContractDetail=Ai;window.closeContractDetail=Qe;window.toggleWhRow=Cn;window.toggleEqTier=Wn;window.switchEmNation=Ln;window.setEmType=Nn;window.setEmListing=Bn;window.setEmQty=Rn;window.purchaseEquipment=On;window.setPrMat=In;window.setPrTier=Mn;window.setPrQty=An;window.purchaseMaterial=zn;let W={general:0,skilled:0,innovative:0},At=!1;const Me=[{id:"general",label:"General Workforce",multiplier:2,color:"#8b9a6b",factionKey:"corp_general_workforce"},{id:"skilled",label:"Skilled Workforce",multiplier:3,color:"#c8a832",factionKey:"corp_skilled_workforce"},{id:"innovative",label:"Innovative Workforce",multiplier:6,color:"#c84",factionKey:"corp_innovative_workforce"}];function Oi(i){const e=Number(E?.minimum_wage??50),t=Number(E?.inflation??50),n=Number(E?.standard_of_living??50),o=e/100*48e3,s=1+(t-50)/100*.5,a=1+(n-50)/100*.5;return Math.round(o*i*s*a)}function v(i){const e=Math.abs(i),t=i<0?"-":"";return e>=1e9?t+"$"+(e/1e9).toFixed(2)+"B":e>=1e6?t+"$"+(e/1e6).toFixed(2)+"M":e>=1e3?t+"$"+(e/1e3).toFixed(1)+"k":t+"$"+e.toLocaleString()}async function Pi(i){i.preventDefault(),document.getElementById("operations-content").style.display="none";const e=document.getElementById("expansion-content");e.style.display="flex",e.style.justifyContent="center",e.style.gap="12px",e.style.alignItems="flex-start",e.style.flexWrap="wrap",document.querySelectorAll(".corp-nav__tab").forEach(t=>t.classList.remove("active")),i.target.classList.add("active"),await Je(),pt(),eo(),await ai(),mt(),await bo(),await fo(),yt(),ut(),await Co(),gt()}function Di(i){i&&i.preventDefault(),document.getElementById("operations-content").style.display="flex",document.getElementById("expansion-content").style.display="none",document.querySelectorAll(".corp-nav__tab").forEach(e=>e.classList.remove("active")),document.querySelector('[data-tab="operations"]')?.classList.add("active")}function ct(){return G.reduce((e,t)=>{const n=Number(t.capacity||0),o=Number(t.condition||0)/100;return e+Math.floor(n*o)},0)+500}function Jn(i,e){const t=Me.find(s=>s.id===i),n=Number(f?.[t.factionKey]??0),o=W[i]+e;if(!(n+o<0)){if(e>0){const s=Me.reduce((l,d)=>{const r=Number(f?.[d.factionKey]??0),c=d.id===i?o:W[d.id];return l+r+c},0),a=ct();if(s>a)return}W[i]=o,pt()}}function Xn(i){i?W[i]=0:W={general:0,skilled:0,innovative:0},pt()}async function Zn(){if(At||!Object.values(W).some(a=>a!==0))return;let e=0;for(const a of Me){const l=W[a.id];l>0&&(e+=l*Oi(a.multiplier)*.1)}const t=Number(f?.corp_cash_reserves??0);if(e>t){alert("Insufficient cash reserves. Hiring cost: "+v(e)+", available: "+v(t));return}const n=Me.reduce((a,l)=>a+Number(f?.[l.factionKey]??0)+W[l.id],0),o=ct();if(n>o){alert("Cannot hire beyond property capacity ("+o.toLocaleString()+"). You need more workplaces.");return}const s=e>0?`Confirm workforce changes?

Hiring fee: `+v(e)+" (deducted from cash reserves)":`Confirm workforce changes?

Firing workers — no cost.`;if(confirm(s)){At=!0;try{const a={};for(const r of Me){const c=Number(f?.[r.factionKey]??0);a[r.factionKey]=Math.max(0,c+W[r.id])}e>0&&(a.corp_cash_reserves=Math.max(0,t-Math.round(e)));const{error:l}=await $.from("factions").update(a).eq("id",f.id);if(l)throw l;Object.assign(f,a),W={general:0,skilled:0,innovative:0};const d=document.getElementById("topbar-cash");if(d){const r=Number(f.corp_cash_reserves??0);d.textContent="CASH: "+(r>=1e6?"$"+(r/1e6).toFixed(1)+"M":"$"+Math.round(r/1e3)+"k")}pt()}catch(a){alert("Error: "+a.message)}finally{At=!1}}}function pt(){const i=document.getElementById("hf-card-container");if(!i)return;const e="'JetBrains Mono', monospace",t={bg:"#121210",surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",greenBright:"#5c5",red:"#c55",redDim:"#a44",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},n=Number(E?.minimum_wage??50),o=Number(E?.inflation??50),s=Number(E?.standard_of_living??50),a=n/100*48e3,l=(1+(o-50)/100*.5).toFixed(2),d=(1+(s-50)/100*.5).toFixed(2),r=E?.name||f?.nation||"Nation",c=Object.values(W).some(w=>w!==0),m=ct();let p=0,u=0,y=0,g=0,_="";for(const w of Me){const I=Number(f?.[w.factionKey]??0),C=W[w.id],k=I+C,x=Oi(w.multiplier),T=C>0,q=I*x,M=k*x,A=M-q;p+=I,u+=k,y+=q,g+=M;const L=C!==0?T?"rgba(92,204,92,0.02)":"rgba(204,85,85,0.02)":"transparent";_+=`
        <div style="padding:10px 14px;border-bottom:1px solid ${t.border};background:${L};">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                ...
                <div style="display:flex;align-items:baseline;gap:4px;">
                    <span style="font-family:${e};font-size:16px;font-weight:700;color:${t.text}">${I.toLocaleString()}</span>
                    ${C!==0?`<span style="font-family:${e};font-size:10px;color:${t.dim}">→</span>
                    <span style="font-family:${e};font-size:16px;font-weight:700;color:${T?t.greenBright:t.red}">${k.toLocaleString()}</span>`:""}
                </div>
            </div>
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                <span style="font-family:${e};font-size:8px;color:${t.dim}">WAGE (MIN × ${w.multiplier}.0 × ${l} × ${d})</span>
                <span style="font-family:${e};font-size:10px;color:${w.color}">${v(x)}/yr</span>
            </div>
            ...
            ${C!==0?`<div style="margin-top:6px;padding:4px 8px;background:${t.bg};border:1px solid ${t.border};display:flex;justify-content:space-between;align-items:center;">
                ...
                <span style="font-family:${e};font-size:10px;font-weight:700;color:${A>0?t.red:t.greenBright}">${A>0?"+":""}${v(A)}/yr</span>
            </div>`:""}
    ...
    </div>`;for(const y of G){const g=ot[y.style]||ot.Basic;a+=Number(y.purchase_price||0),l+=Number(y.monthly_maintenance||0),s+=`
        <div style="padding:8px 12px;border-bottom:1px solid ${t.border};">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px;">
    ...
    </div>`}let Be=[],K=null;const ot={Basic:{color:"#6a6660",label:"BASIC"},Modern:{color:"#8b9a6b",label:"MODERN"},Sustainable:{color:"#5c5",label:"SUSTAINABLE"},Innovative:{color:"#c8a832",label:"INNOVATIVE"},Heritage:{color:"#c84",label:"HERITAGE"},Premium:{color:"#ca5",label:"PREMIUM"}};async function ai(){if(!f?.nation_id)return;const{data:i,error:e}=await $.from("available_properties").select("*").eq("nation_id",f.nation_id).eq("status","available").order("price",{ascending:!0});if(e){console.warn("[Property] Failed to load marketplace:",e.message);return}Be=(i||[]).map(t=>({...t,adjusted_cost:t.price,adjusted_maintenance:t.monthly_maintenance}))}function mt(){const i=document.getElementById("new-property-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",redDim:"#a44",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"};(E?.name||f?.nation||"Nation").toUpperCase();const n=Number(E?.standard_of_living??50),o=Number(E?.gdp_growth??50),s=Number(E?.inflation??50),a=E?.capital||"Capital",l={capital:a,port:a+" Port",industrial:a+" Industrial Zone",suburban:a+" Suburbs",coastal:a+" Coast"};let d="";if(Be.length===0)d=`<div style="padding:20px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">No properties available in this market.<br>Improve GDP Growth and Standard of Living to unlock more.</div>`;else for(let r=0;r<Be.length;r++){const c=Be[r],m=K===r,p=ot[c.style]||ot.Basic,u=l[c.city_template]||a;d+=`
    ...
    </div>`}function to(i){K=K===i?null:i,mt()}let zt=!1;async function io(){if(K===null||zt)return;const i=Be[K];if(!i)return;const e=Number(f?.corp_cash_reserves??0);if(i.adjusted_cost>e){alert(`Insufficient cash reserves.
...
All construction corporations in `+e+" can now bid on this project.")}catch(i){alert("Failed to submit project: "+i.message)}finally{qt=!1}}}function Hi(){if(document.getElementById("cp-modal-overlay")?.remove(),!si)return;const i="'JetBrains Mono', monospace",e={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",redDim:"#a44",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},t=ji(),n=E?.name||f?.nation||"Nation",o=ke[S.style]?.repGain||1,s=o>=4?e.gold:o>=3?e.greenBright:o>=2?e.accent:e.dim,a=Object.entries(ke).map(([r,c])=>{const m=S.style===r;return`<div onclick="cpSetField('style','${r}')" style="padding:5px 6px;cursor:pointer;text-align:center;background:${m?c.color+"18":"transparent"};border:1px solid ${m?c.color+"44":e.border};">
    ...
    <div style="width:440px;background:${e.surface};border:1px solid ${e.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;max-height:90vh;">
        ...
        <div style="padding:12px 16px;overflow:auto;flex:1;">
            ...
            <div style="margin-bottom:12px;">
                ...
                <div style="margin-top:4px;font-family:${i};font-size:8px;color:${ke[S.style].color}">${ke[S.style].desc}</div>
            </div>
    ...
    </div>`,document.body.appendChild(l);const d=document.getElementById("cp-name-input");d&&d.addEventListener("input",r=>{S.name=r.target.value}),l.addEventListener("click",r=>{r.target===l&&ri()})}function so(){const i=document.getElementById("cp-name-input");if(i&&(S.name=i.value),!S.name.trim()){alert("Please enter a building name.");return}ao()}window.cpClose=ri;window.cpSetField=oo;window.cpSubmitFromModal=so;window.npSelect=to;window.npBuyProperty=io;window.npOpenConstructionModal=no;let Pe=!1;async function ro(i){if(Pe)return;const e=G.find(l=>l.id===i);if(!e)return;const t=1+(Number(E?.inflation??50)-50)/100*.3,n=Math.round((e.purchase_price||0)*.1*t),o=Number(f?.corp_cash_reserves??0);if(n>o){alert("Insufficient cash. Refurbishment costs "+v(n)+" (inflation-adjusted), you have "+v(o));return}if(e.condition>=95){alert("Property is already in excellent condition ("+e.condition+"%).");return}const s=5+Math.floor(Math.random()*21),a=Math.min(100,e.condition+s);if(confirm('Refurbish "'+e.name+`"?

Cost: `+v(n)+`
Expected improvement: +`+s+"% condition ("+e.condition+"% → "+a+"%)")){Pe=!0;try{await $.from("corp_properties").update({condition:a}).eq("id",i);const l=Math.max(0,o-n);await $.from("factions").update({corp_cash_reserves:l}).eq("id",f.id),f.corp_cash_reserves=l;const d=document.getElementById("topbar-cash");d&&(d.textContent="CASH: "+(l>=1e6?"$"+(l/1e6).toFixed(1)+"M":"$"+Math.round(l/1e3)+"k")),await Je(),ft(),alert("Refurbished! Condition: "+e.condition+"% → "+a+"%")}catch(l){alert("Refurbishment failed: "+l.message)}finally{Pe=!1}}}async function lo(i){if(Pe)return;const e=G.find(s=>s.id===i);if(!e)return;const t=1+(Number(E?.inflation??50)-50)/100*.3,n=(e.condition||50)/100,o=Math.round((e.purchase_price||0)*.6*n*t);if(confirm('Sell "'+e.name+`"?

Sale value: `+v(o)+" (60% × "+e.condition+`% condition × inflation)

The property will go back on the market for 6 ticks.
This cannot be undone.`)){Pe=!0;try{await $.from("corp_properties").update({is_active:!1}).eq("id",i);const a=Number(f?.corp_cash_reserves??0)+o;await $.from("factions").update({corp_cash_reserves:a}).eq("id",f.id),f.corp_cash_reserves=a;const d=(await $.from("shard").select("current_tick").eq("name","Alpha Shard").single()).data?.current_tick||0;await $.from("available_properties").insert({nation_id:f.nation_id,catalog_id:e.catalog_id||null,name:e.name,type:e.type,style:e.style,capacity:e.capacity,price:Math.round(o*1.1),monthly_maintenance:e.monthly_maintenance,condition:e.condition,city:e.city,generated_at_tick:d,expires_at_tick:d+6,status:"available"});const r=document.getElementById("topbar-cash");r&&(r.textContent="CASH: "+(a>=1e6?"$"+(a/1e6).toFixed(1)+"M":"$"+Math.round(a/1e3)+"k")),await Je(),ft(),await ai(),mt(),alert('Sold "'+e.name+'" for '+v(o))}catch(s){alert("Sale failed: "+s.message)}finally{Pe=!1}}}window.propRefurbish=ro;window.propSell=lo;const _e={SALE:.8,DISSOLVE:.6,REVENUE_BASE:.02,GDP_NEUTRAL:30,DEFAULT_REPUTATION:25};function co(i){if(!i)return 0;const e=i.trim().replace(/[$,]/g,""),t=e.match(/^([\d.]+)\s*[Mm]$/),n=e.match(/^([\d.]+)\s*[Kk]$/);return Math.round(t?parseFloat(t[1])*1e6:n?parseFloat(n[1])*1e3:parseFloat(e))}function je(i){const e=document.getElementById("topbar-cash");e&&(e.textContent="CASH: "+(i>=1e6?"$"+(i/1e6).toFixed(1)+"M":"$"+Math.round(i/1e3)+"k"))}function po(i){return He.find(e=>e.id===i)?.name||"—"}function li(i){return G.filter(e=>e.nation_id===i)}async function di(){Re=0,await Je(),ft(),ut(),yt()}let re=!1,Re=0,tt={};async function fo(){if(f?.id)try{const{data:i}=await $.from("construction_contracts").select("nation_id").eq("awarded_to_faction",f.id).in("status",["in_progress","awarded"]);tt={};for(const e of i||[])e.nation_id&&(tt[e.nation_id]=(tt[e.nation_id]||0)+1)}catch{}}function Ui(i){const e=li(i.nation_id),t=e.reduce((y,g)=>y+Number(g.purchase_price||0),0),n=e.reduce((y,g)=>y+Number(g.capacity||0),0),o=tt[i.nation_id]||0,s=He.find(y=>y.id===i.nation_id),a=(i.name||"").trim().split(/\s+/),l=a.length>=2?a.map(y=>y[0]).join("").toUpperCase().slice(0,4):(i.name||"SUB").slice(0,4).toUpperCase(),d=Number(i.sub_cash||0),r=Number(s?.gdp_growth??50),c=d*_e.REVENUE_BASE,m=(r-_e.GDP_NEUTRAL)/100,p=_e.DEFAULT_REPUTATION/100,u=d>0?Math.round(c*(1+m)*p):0;return{id:i.id,name:i.name,abbr:l,nation:s?.name||i.city||"—",nationId:i.nation_id,sector:f?.corp_sector||"General",subsector:i.subsector||f?.corp_subsector||"—",revenue:u,debt:0,cash:d,reputation:_e.DEFAULT_REPUTATION,valuation:t,workforce:n,projects:o,established:i.created_at?new Date(i.created_at).getFullYear().toString():"—",trend:r>=40&&d>0?"up":r>=_e.GDP_NEUTRAL&&d>0?"flat":"down",profitable:u>0}}function ut(){const i=document.getElementById("manage-subsidiaries-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",greenDark:"#2a5a3a",red:"#c55",redDim:"#a44",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},o=G.filter(c=>c.type==="regional_hq").map(Ui);Re>=o.length&&(Re=0);const s=o[Re]||null;let a="";o.length===0&&(a=`<div style="padding:30px 14px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">No subsidiaries established.<br>Use Create Subsidiary to expand.</div>`);let l=0,d=0;for(let c=0;c<o.length;c++){const m=o[c],p=c===Re;l+=m.revenue,d+=m.valuation;const u=m.trend==="up"?t.greenBright:m.trend==="down"?t.red:t.dim,y=m.trend==="up"?"▲":m.trend==="down"?"▼":"–";a+=`
    ...
    </div>`}async function Gi(i,e){if(re)return;const t=G.find(u=>u.id===i);if(!t)return;const n=e==="sell",o=n?_e.SALE:_e.DISSOLVE,s=n?"SELL":"DISSOLVE",a=n?"sold":"dissolved",l=n?"80%":"60%",d=po(t.nation_id),r=li(t.nation_id),c=r.reduce((u,y)=>u+Math.round((y.purchase_price||0)*o*(y.condition||50)/100),0),m=Number(t.sub_cash||0),p=c+m;if(confirm(s+' subsidiary "'+t.name+`"?
...
This cannot be undone.`)){re=!0;try{const u=r.map(g=>g.id);if(u.length===1){const{error:g}=await $.from("corp_properties").update({is_active:!1}).eq("id",u[0]);if(g)throw g}else if(u.length>1){const{error:g}=await $.from("corp_properties").update({is_active:!1}).in("id",u);if(g)throw g}await $.from("corp_properties").update({sub_cash:0}).eq("id",i).then(()=>{}).catch(()=>{});const y=Number(f?.corp_cash_reserves??0)+p;await $.from("factions").update({corp_cash_reserves:y}).eq("id",f.id),f.corp_cash_reserves=y,je(y),await di(),alert("Subsidiary "+a+". "+r.length+` properties liquidated.
Total received: `+v(p))}catch(u){alert("Failed: "+u.message)}finally{re=!1}}}function mo(i){Gi(i,"sell")}function uo(i){Gi(i,"dissolve")}async function Fi(i,e){if(re)return;const t=G.find(m=>m.id===i);if(!t)return;const n=Number(f?.corp_cash_reserves??0),o=Number(t.sub_cash||0),s=e?"WITHDRAW":"INJECT CAPITAL";if(e&&o<=0){alert("This subsidiary has no cash to withdraw.");return}const a=e?o:n,l=prompt(s+(e?" from ":" into ")+t.name+`

Parent cash: `+v(n)+`
Subsidiary cash: `+v(o)+`

Enter amount (e.g., 5000000 or 5M):`);if(!l)return;const d=co(l);if(!d||d<=0||isNaN(d)){alert("Invalid amount.");return}if(d>a){alert("Insufficient "+(e?"subsidiary":"parent")+" cash. Available: "+v(a));return}const r=e?n+d:n-d,c=e?o-d:o+d;if(confirm(s+" "+v(d)+(e?" from ":" into ")+t.name+`?

Parent: `+v(n)+" → "+v(r)+`
Subsidiary: `+v(o)+" → "+v(c))){re=!0;try{await Promise.all([$.from("factions").update({corp_cash_reserves:r}).eq("id",f.id),$.from("corp_properties").update({sub_cash:c}).eq("id",i)]),f.corp_cash_reserves=r,t.sub_cash=c,je(r),ut(),alert((e?"Withdrew ":"Injected ")+v(d)+(e?" from ":" into ")+t.name+".")}catch(m){alert("Failed: "+m.message)}finally{re=!1}}}function yo(i){Fi(i,!1)}function go(i){Fi(i,!0)}async function vo(i){if(re)return;const e=G.find(g=>g.id===i);if(!e)return;const t=Ui(e);t.nation;const n=li(e.nation_id),o=t.valuation,s=t.cash,a=t.reputation,l=t.subsector,d=Math.round(o*2.25),r=Math.round(a*.1),c=Math.round(a*.2),m=ct(),p=Me.reduce((g,_)=>g+Number(f?.[_.factionKey]??0),0),u=Math.max(0,m-p),y=Number(f?.corp_cash_reserves??0);if(d>y){alert(`Insufficient cash to acquire subsidiary.
...
This cannot be undone.`)){re=!0;try{const g=f.nation_id;if(n.length>0){const T=n.filter(M=>M.id!==e.id).map(M=>M.id);if(T.length===1){const{error:M}=await $.from("corp_properties").update({nation_id:g,type:"office"}).eq("id",T[0]);if(M)throw M}else if(T.length>1){const{error:M}=await $.from("corp_properties").update({nation_id:g,type:"office"}).in("id",T);if(M)throw M}const{error:q}=await $.from("corp_properties").update({nation_id:g,type:"office",sub_cash:0,subsector:null}).eq("id",e.id);if(q)throw q}const _=y-d+s,w=Number(f?.corp_general_workforce??0)+u,I=Math.random()>=.5?r:-c,C=Number(f?.standing??50),k=Math.max(0,Math.min(100,C+I)),{error:x}=await $.from("factions").update({corp_cash_reserves:_,corp_general_workforce:w,standing:k}).eq("id",f.id);if(x)throw x;f.corp_cash_reserves=_,f.corp_general_workforce=w,f.standing=k,je(_),await di(),alert(`Merger complete!

"`+e.name+`" absorbed into your corporation.
Cost: `+v(d)+" | Cash absorbed: "+v(s)+`
Reputation `+(I>=0?"+":"")+I+" (now "+k+`)
Workers hired: +`+u.toLocaleString()+` general workforce
Properties: `+n.length+" transferred to parent")}catch(g){alert("Merge failed: "+g.message)}finally{re=!1}}}window.subDissolve=uo;window.subInjectCapital=yo;window.subWithdraw=go;window.subMerge=vo;window.subSell=mo;window.selectSubsidiary=function(i){Re=i,ut()};let He=[],Fe={},ee=null,Lt=!1,ze="",Ve="",qe="";const xo={Construction:[{id:"civil",name:"Civil Engineering",mod:0},{id:"industrial",name:"Industrial Construction",mod:.25},{id:"mega",name:"Megaprojects",mod:.4}],Finance:[{id:"banking",name:"Banking",mod:0},{id:"insurance",name:"Insurance",mod:.15},{id:"investment",name:"Investment Management",mod:.3}],Technology:[{id:"software",name:"Software Development",mod:0},{id:"hardware",name:"Hardware Manufacturing",mod:.2},{id:"telecom",name:"Telecommunications",mod:.35}],Energy:[{id:"oil_gas",name:"Oil & Gas",mod:0},{id:"renewables",name:"Renewables",mod:.2},{id:"mining",name:"Mining",mod:.3}],Healthcare:[{id:"pharma",name:"Pharmaceuticals",mod:0},{id:"hospitals",name:"Hospital Systems",mod:.2},{id:"biotech",name:"Biotechnology",mod:.35}]};async function bo(){const{data:i,error:e}=await $.from("nations").select("*").order("name");e&&console.warn("[Subsidiary] Failed to load nations:",e.message),He=(i||[]).filter(n=>n.id!==f?.nation_id);const{data:t}=await $.from("factions").select("nation_id").eq("faction_type","corporation").is("abandoned_at",null);Fe={};for(const n of t||[])n.nation_id&&(Fe[n.nation_id]=(Fe[n.nation_id]||0)+1);qe=f?.corp_subsector||""}function Wi(){const i=f?.corp_sector||"";return xo[i]||[{id:"general",name:i||"General",mod:0}]}function $o(){const e=Wi().find(t=>t.name===qe);return e?e.mod:0}function Yt(i){const e=Number(i.standard_of_living??50);return Math.max(.5,Math.round(e/50*100)/100)}function Vi(i){const t=1+$o(),n=Yt(i);return Math.round(Math.max(1e7,5e7*t*n))}function _o(i){const e=Fe[i]||0;return e<=1?{label:"HIGH",color:"#5c5"}:e<=3?{label:"MODERATE",color:"#ca5"}:{label:"LOW",color:"#c55"}}function ho(i){if(ee=ee===i?null:i,ee){const e=He.find(t=>t.id===ee);ze=(f?.faction_name||"Subsidiary")+" "+(e?.name||"")}else ze="";yt()}function wo(i){qe=i,yt()}function ko(i){ze=i}function Eo(i){Ve=i.toUpperCase().slice(0,4)}async function To(){if(Lt||!ee)return;const i=He.find(a=>a.id===ee);if(!i)return;const e=(ze||"").trim(),t=(Ve||"").trim();if(!e){alert("Please enter a corporation name for the subsidiary.");return}if(t.length<2){alert("Please enter an abbreviation (2-4 chars).");return}if(G.find(a=>a.nation_id===i.id&&a.type==="regional_hq")){alert("You already have a subsidiary in "+i.name);return}const o=Vi(i),s=Number(f?.corp_cash_reserves??0);if(o>s){alert("Insufficient cash. Entry cost: "+v(o)+", available: "+v(s));return}if(confirm("Establish subsidiary in "+i.name+`?
...
Deducted from cash reserves.`)){Lt=!0;try{const l=(await $.from("shard").select("current_tick").eq("name","Alpha Shard").single()).data?.current_tick||0,d=85+Math.floor(Math.random()*16),r=Math.round(o*.005),{error:c}=await $.from("corp_properties").insert({faction_id:f.id,nation_id:i.id,name:e,type:"regional_hq",style:"Modern",capacity:500,purchase_price:o,monthly_maintenance:r,condition:d,city:i.capital||i.name,purchased_at_tick:l,is_active:!0,subsector:qe||f?.corp_subsector||null});if(c)throw c;const m=Math.max(0,s-o);await $.from("factions").update({corp_cash_reserves:m}).eq("id",f.id),f.corp_cash_reserves=m,je(m),ee=null,ze="",Ve="",await di(),alert('Subsidiary "'+e+'" established in '+i.name+`!

Cost: `+v(o)+`
Regional HQ created with `+d+"% condition.")}catch(a){alert("Failed: "+a.message)}finally{Lt=!1}}}function yt(){const i=document.getElementById("create-subsidiary-container");if(!i)return;const e="'JetBrains Mono', monospace",t={bg:"#121210",surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",accentBright:"#a3b07e",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},n=f?.corp_sector||"General",o=f?.corp_subsector||"",s=Wi(),a=s.find(x=>x.name===qe)||s[0],l=new Set(G.filter(x=>x.type==="regional_hq").map(x=>x.nation_id)),d=He.filter(x=>!l.has(x.id)),r=ee?d.find(x=>x.id===ee):null,c=ze.trim().length>0&&Ve.trim().length>=2&&r!==null;let m=`
    ...
    <div style="padding:6px 14px;border-bottom:1px solid ${t.border};">
        ...
        <div style="display:flex;gap:3px;">
            ${s.map(x=>{const T=x.name===qe,q=x.name===o;return`<div onclick="subSetSubsector('${x.name.replace(/'/g,"\\'")}')" style="flex:1;padding:5px 4px;text-align:center;cursor:pointer;background:${T?t.accent+"18":"transparent"};border:1px solid ${T?t.accent+"44":t.border};">
                    <div style="font-family:${e};font-size:8px;font-weight:700;color:${T?t.accentBright:t.dim}">${x.name}</div>
                    <div style="font-family:${e};font-size:7px;margin-top:2px;color:${q?t.greenBright:x.mod>0?t.orange:t.dim}">${q?"SAME — ±0%":x.mod>0?"+"+Math.round(x.mod*100)+"%":"±0%"}</div>
                </div>`}).join("")}
        </div>
    </div>`,u="";if(d.length===0)u=`<div style="padding:20px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">Subsidiaries in all available nations.</div>`;else for(const x of d){const T=x.id===ee,q=_o(x.id),M=Fe[x.id]||0,A=Math.round(Number(x.standard_of_living??50)),L=Yt(x);u+=`
            <div onclick="subSelectNation('${x.id}')" style="display:flex;align-items:center;padding:4px 8px;margin-bottom:2px;cursor:pointer;background:${T?"rgba(139,154,107,0.04)":"transparent"};border:1px solid ${T?t.accent+"44":t.border};border-left:${T?"2px solid "+t.accent:"2px solid transparent"};">
                <div style="flex:1;">
                    ...
                    <div style="display:flex;gap:8px;margin-top:2px;">
                        <span style="font-family:${e};font-size:7px;color:${t.dim}">STD/LIVING: <span style="color:${t.muted}">${A}</span></span>
    ...
    <div style="padding:6px 14px;border-bottom:1px solid ${t.border};">
        <div style="margin-bottom:6px;">
            ...
            <input type="text" value="${(ze||"").replace(/"/g,"&quot;")}" oninput="subSetName(this.value)" placeholder="e.g., ${(f?.faction_name||"Corp")+" "+(r?.name||"International")}" style="width:100%;padding:5px 8px;font-family:${e};font-size:10px;color:${t.text};background:${t.card};border:1px solid ${t.border};outline:none;box-sizing:border-box;" />
        </div>
        <div>
            ...
            <input type="text" value="${(Ve||"").replace(/"/g,"&quot;")}" oninput="subSetAbbr(this.value)" placeholder="${(f?.faction_name||"CORP").slice(0,2).toUpperCase()+(r?.name||"XX").slice(0,2).toUpperCase()}" maxlength="4" style="width:80px;padding:5px 8px;font-family:${e};font-size:12px;font-weight:700;color:${t.gold};background:${t.card};border:1px solid ${t.border};outline:none;text-align:center;letter-spacing:2px;" />
        </div>
    ...
    </div>`;const h=5e7,w=a.mod,I=r?Yt(r):null,C=r?Vi(r):null;let k=`
    <div style="background:${t.bg};border:1px solid ${t.border};padding:6px 8px;margin-bottom:6px;">
        ...
        <div style="display:flex;justify-content:space-between;padding:2px 0;border-bottom:1px solid ${t.border};">
            ...
            <span style="font-family:${e};font-size:9px;color:${w===0?t.greenBright:t.orange}">${w===0?"±0%":"+"+Math.round(w*100)+"%"}</span>
        </div>
        <div style="display:flex;justify-content:space-between;padding:2px 0;border-bottom:1px solid ${t.border};">
            <span style="font-family:${e};font-size:8px;color:${t.dim}">NATION (${r?r.name:"—"})</span>
            <span style="font-family:${e};font-size:9px;color:${r?I>1?t.orange:t.greenBright:t.dim}">${r?"×"+I.toFixed(2):"—"}</span>
        </div>
    ...
    </div>`}window.subSelectNation=ho;window.subCreate=To;window.subSetName=ko;window.subSetAbbr=Eo;window.subSetSubsector=wo;let it=[],Ee=0,ne="ALL",$e="REPUTATION";async function Co(){const{data:i}=await $.from("factions").select("id, faction_name, abbreviation, corp_sector, corp_subsector, corp_company_type, corp_ticker, corp_cash_reserves, nation_id, nation, linked_user_id").eq("faction_type","corporation").is("abandoned_at",null).order("faction_name");it=(i||[]).map(e=>({...e,abbr:e.corp_ticker||e.abbreviation||e.faction_name?.slice(0,4).toUpperCase()||"???",status:(e.corp_company_type||"Private").toUpperCase(),isPlayer:!!e.linked_user_id,reputation:50,revenue:e.status==="PUBLIC"?Number(e.corp_cash_reserves||0)*.1:null,valuation:e.status==="PUBLIC"?Number(e.corp_cash_reserves||0)*3:null}))}function So(i){Ee=i,gt()}function Io(i){ne=i,Ee=0,gt()}function Mo(i){$e=i,Ee=0,gt()}function gt(){const i=document.getElementById("corporations-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",blue:"#5a8aaa",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},n={PUBLIC:{color:t.greenBright,bg:"rgba(92,204,92,0.06)",border:"rgba(92,204,92,0.15)"},PRIVATE:{color:t.gold,bg:"rgba(200,168,50,0.08)",border:"rgba(200,168,50,0.2)"},STATE:{color:t.orange,bg:"rgba(204,136,68,0.06)",border:"rgba(204,136,68,0.15)"}},o=[...new Set(it.map(p=>p.nation).filter(Boolean))];let s=[...it];ne!=="ALL"&&(s=s.filter(p=>p.nation===ne)),$e==="REPUTATION"?s.sort((p,u)=>(u.reputation||0)-(p.reputation||0)):$e==="REVENUE"?s.sort((p,u)=>(u.revenue||0)-(p.revenue||0)):$e==="VALUATION"&&s.sort((p,u)=>(u.valuation||0)-(p.valuation||0)),Ee>=s.length&&(Ee=0);const a=s[Ee]||null,l=a&&a.status==="PRIVATE",d=a&&a.status==="STATE";let r="";s.length===0&&(r=`<div style="padding:30px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">No corporations found.</div>`);for(let p=0;p<s.length;p++){const u=s[p],y=p===Ee,g=n[u.status]||n.PRIVATE,_=u.status==="PRIVATE";r+=`
    ...
    <div style="padding:5px 14px;border-bottom:1px solid ${t.border};background:${t.card};display:flex;gap:12px;align-items:center;flex-shrink:0;">
        <div style="display:flex;gap:3px;align-items:center;">
            ...
            <span onclick="corpFilterNation('ALL')" style="padding:2px 7px;font-family:${e};font-size:7px;font-weight:700;cursor:pointer;color:${ne==="ALL"?"#000":t.dim};background:${ne==="ALL"?t.accent:"transparent"};border:1px solid ${ne==="ALL"?t.accent:t.border}">ALL</span>
            ${o.map(p=>`<span onclick="corpFilterNation('${p}')" style="padding:2px 7px;font-family:${e};font-size:7px;font-weight:700;cursor:pointer;color:${ne===p?"#000":t.dim};background:${ne===p?t.accent:"transparent"};border:1px solid ${ne===p?t.accent:t.border}">${p}</span>`).join("")}
        </div>
        <div style="flex:1"></div>
        <div style="display:flex;gap:3px;align-items:center;">
            <span style="font-family:${e};font-size:7px;color:${t.dim};letter-spacing:0.8px">SORT</span>
            ${["REPUTATION","REVENUE","VALUATION"].map(p=>`<span onclick="corpSort('${p}')" style="padding:2px 7px;font-family:${e};font-size:7px;font-weight:700;cursor:pointer;color:${$e===p?"#000":t.dim};background:${$e===p?t.accent:"transparent"};border:1px solid ${$e===p?t.accent:t.border}">${p}</span>`).join("")}
        </div>
    </div>`;i.innerHTML=`
    <div style="width:760px;height:450px;background:${t.surface};border:1px solid ${t.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
        <div style="padding:8px 14px;border-bottom:1px solid ${t.border};display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">
            ...
            <span style="font-family:${e};font-size:9px;color:${t.dim}">${it.length} IN DATABASE</span>
        </div>
        ${m}
        <div style="flex:1;display:flex;overflow:hidden;">
            <div style="flex:1;border-right:1px solid ${t.border};display:flex;flex-direction:column;">
    ...
    </div>`}window.corpSelect=So;window.corpFilterNation=Io;window.corpSort=Mo;let se=null,pe={},D=120,fe=15,Qt={},Oe=[];async function Ao(){if(!Ie)return;if(De[Ie.id]){alert("You already have a bid on this contract. Retract it first if you want to re-bid.");return}se=Ie,Qt={};try{const{data:t}=await $.from("corp_material_inventory").select("material_key, quantity").eq("faction_id",f.id);for(const n of t||[])Qt[n.material_key]=Number(n.quantity||0)}catch{}Oe=[];try{const{data:t}=await $.from("contract_bids").select("faction_id, bid_price, estimated_quality, status, factions(faction_name, corp_ticker)").eq("contract_id",se.id).in("status",["pending","won"]);Oe=(t||[]).filter(n=>n.faction_id!==f?.id).map(n=>({name:n.factions?.faction_name||"Unknown",ticker:n.factions?.corp_ticker||"???",price:Number(n.bid_price||0),quality:Number(n.estimated_quality||0),status:n.status}))}catch{}pe={};const i=se.required_materials||{};for(const t of Object.keys(i))pe[t]="STD";const e=se.required_workforce||{};D=Number(e.general||0)+Number(e.skilled||0)||120,fe=15,Qe(),vt()}function ci(){document.getElementById("bid-assembly-overlay")?.remove(),se=null}function zo(i,e){pe[i]=e,vt()}function qo(i){D=i,vt()}function Lo(i){fe=i,vt()}function vt(){if(document.getElementById("bid-assembly-overlay")?.remove(),!se)return;const i="'JetBrains Mono', monospace",e={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",accentBright:"#a3b07e",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",redDim:"#a44",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},t=se,n=t.issuer_type==="GOVERNMENT",o=E?.name||f?.nation||"—",s=Number(t.budget_ceiling||0),a=Number(t.timeline_ticks||8),l=t.required_materials||{},d=Object.keys(l),r={LOW:.5,STD:1,HIGH:2},c={LOW:e.orange,STD:e.yellow,HIGH:e.greenBright},m={LOW:"Low",STD:"Standard",HIGH:"High"},p={concrete:36e4,steel:5e5,glass_facades:56e4,em_systems:64e4,lumber:24e4,heavy_parts:8e5,aggregate:16e4,asphalt:28e4},u=Qt||{};let y=0,g="";for(const N of d){const P=Number(l[N]||0),ui=pe[N]||"STD",yi=p[N]||3e5,Ji=r[ui],Xi=Math.round(yi*Ji),gi=P*Xi;y+=gi;const Zi=N.replace(/_/g," ").replace(/\b\w/g,ve=>ve.toUpperCase()),vi=Number(u[N]||0),kt=Math.max(0,P-vi),en=kt===0?e.greenBright:kt<P?e.yellow:e.red,tn=kt===0?"✓ IN STOCK":`${vi}/${P}`;g+=`
        <div style="display:flex;align-items:center;padding:5px 14px;border-bottom:1px solid ${e.border};">
            ...
            <div style="flex:1.2;display:flex;gap:2px;justify-content:center;">
                ${["LOW","STD","HIGH"].map(ve=>{const Et=ui===ve,xi=c[ve],nn=v(Math.round(yi*r[ve]));return`<span onclick="bidSetGrade('${N}','${ve}')" style="padding:2px 6px;font-family:${i};font-size:7px;font-weight:700;cursor:pointer;color:${Et?"#000":e.dim};background:${Et?xi:"transparent"};border:1px solid ${Et?xi:e.border}" title="${nn}/unit">${m[ve]}</span>`}).join("")}
            </div>
            <div style="flex:0.8;text-align:right"><span style="font-family:${i};font-size:10px;color:${e.text}">${v(gi)}</span></div>
        </div>`}const _=t.required_workforce||{},h=Number(_.general||0)+Number(_.skilled||0)||100,w=Math.max(40,Math.round(h*.5)),I=h*2,C=[w,Math.round(h*.75),h,Math.round(h*1.5),I],k=Math.max(0,Math.min(1,(D-w)/(I-w||1))),x=a,T=Math.round(4.5-k*8),q=Math.max(Math.round(x*.6),x+T),M=T>0?`+${T}mo`:T<0?`${T}mo`:"On schedule",A=T>0?e.red:T<0?e.greenBright:e.yellow,L=15200,R=D*L*q,O=s,Y=[{name:"Municipal Zoning Approval",cost:18e4,ticks:2,required:!0},{name:"Structural Engineering Cert.",cost:24e4,ticks:3,required:!0},{name:"Environmental Impact Assessment",cost:34e4,ticks:8,required:O>2e7},{name:"Seismic Resilience Compliance",cost:21e4,ticks:4,required:O>5e7},{name:"Heritage Conservation Review",cost:16e4,ticks:6,required:!1},{name:"Fire Safety Certification",cost:12e4,ticks:2,required:O>1e7}].filter(N=>N.required),pi=new Set(["Municipal Zoning Approval","Structural Engineering Cert."]),bt=Y.filter(N=>!pi.has(N.name)).reduce((N,P)=>N+P.cost,0),$t=4e5,_t=y+R+bt+$t,fi=Math.round(_t*(fe/100)),Xe=_t+fi,Q=Xe>s,ht=fi,ye=Q?0:Math.max(0,Math.min(100,Math.round(100-Xe/s*100+30))),mi=ye>70?e.greenBright:ye>40?e.yellow:ye>0?e.orange:e.red,Qi=Q?"OVER CEILING":ye>70?"STRONG":ye>40?"COMPETITIVE":ye>20?"WEAK":"UNLIKELY",wt=Object.values(pe),ge=wt.length>0?Math.round(wt.reduce((N,P)=>N+(P==="HIGH"?85:P==="STD"?65:45),0)/wt.length):50,Ze=ge>=75?e.greenBright:ge>=55?e.yellow:e.orange,Ki=ge>=75?"STRONG":ge>=55?"PROMISING":"UNCERTAIN",Le=document.createElement("div");Le.id="bid-assembly-overlay",Le.style.cssText="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.7);display:flex;align-items:center;justify-content:center;z-index:9999;",Le.addEventListener("click",N=>{N.target===Le&&ci()}),Le.innerHTML=`
    <div style="width:740px;max-height:94vh;background:${e.surface};border:1px solid ${e.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
        ...
        <div style="flex:1;display:flex;overflow:hidden;">
            ...
            <div style="flex:1;border-right:1px solid ${e.border};overflow:auto;">
                ...
                <div style="padding:8px 14px;border-bottom:1px solid ${e.border};">
                    ...
                    <div style="display:flex;justify-content:space-between;">
                        <span style="font-family:${i};font-size:8px;color:${e.dim}">${D} × $${L.toLocaleString()}/tick × ${q} ticks</span>
                        <span style="font-family:${i};font-size:11px;font-weight:700;color:${e.text}">${v(R)}</span>
                    </div>
                    ...
                    <div style="display:flex;justify-content:space-between;margin-top:6px;padding-top:6px;border-top:1px solid ${e.border};">
                        ...
                        <span style="font-family:${i};font-size:10px;font-weight:700;color:${A}">${q}mo <span style="font-size:8px;opacity:0.7">(${M})</span></span>
                    </div>
                </div>
                ...
                ${Y.map(N=>{const P=pi.has(N.name);return`<div style="display:flex;align-items:center;padding:4px 14px;border-bottom:1px solid ${e.border};">
                ...
                <div style="display:flex;justify-content:space-between;padding:5px 14px;border-bottom:1px solid ${e.border};background:rgba(139,154,107,0.02);">
                    ...
                    <span style="font-family:${i};font-size:11px;font-weight:700;color:${e.text}">${v(bt)}</span>
                </div>
                ...
                <div style="display:flex;justify-content:space-between;padding:5px 14px;border-bottom:1px solid ${e.border};">
                    ...
                    <span style="font-family:${i};font-size:11px;font-weight:700;color:${e.text}">${v($t)}</span>
                </div>
            </div>
            ...
            <div style="width:280px;display:flex;flex-direction:column;overflow:auto;">
                ...
                ${[{l:"Materials",v:y},{l:"Labor",v:R},{l:"Permits",v:bt},{l:"Overhead",v:$t}].map(N=>`<div style="display:flex;justify-content:space-between;padding:4px 14px;border-bottom:1px solid ${e.border};">
                ...
                <div style="display:flex;justify-content:space-between;padding:6px 14px;border-bottom:1px solid ${e.border};background:rgba(204,85,85,0.03);">
                    ...
                    <span style="font-family:${i};font-size:13px;font-weight:700;color:${e.red}">${v(_t)}</span>
                </div>
                ...
                <div style="padding:10px 14px;border-bottom:1px solid ${e.border};">
                    <div style="display:flex;justify-content:space-between;margin-bottom:6px;">
                        ...
                        <span style="font-family:${i};font-size:16px;font-weight:700;color:${e.gold}">${fe}%</span>
                    </div>
                    <input type="range" min="0" max="40" step="1" value="${fe}" oninput="bidSetMarkup(+this.value)" style="width:100%;accent-color:${e.gold};height:6px;" />
                    <div style="display:flex;justify-content:space-between;font-family:${i};font-size:7px;color:${e.dim};margin-top:2px;">
                ...
                <div style="padding:10px 14px;border-bottom:1px solid ${e.border};background:${Q?"rgba(204,85,85,0.04)":"rgba(200,168,50,0.03)"};">
                    ...
                    <div style="font-family:${i};font-size:22px;font-weight:700;color:${Q?e.red:e.gold}">${v(Xe)}</div>
                ...
                <div style="padding:8px 14px;border-bottom:1px solid ${e.border};">
                    <div style="display:flex;justify-content:space-between;">
                        ...
                        <span style="font-family:${i};font-size:14px;font-weight:700;color:${ht>0?e.greenBright:e.dim}">+${v(ht)}</span>
                    </div>
                </div>
                ...
                <div style="padding:8px 14px;border-bottom:1px solid ${e.border};">
                    ...
                    <div style="width:100%;height:4px;background:${e.border}"><div style="width:${ye}%;height:100%;background:${mi}"></div></div>
                </div>
                <div style="padding:8px 14px;border-bottom:1px solid ${e.border};">
                    <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
                        <span style="font-family:${i};font-size:8px;color:${e.dim}">QUALITY ESTIMATE</span>
                        <div style="display:flex;align-items:baseline;gap:4px;">
                            <span style="font-family:${i};font-size:11px;font-weight:700;color:${Ze}">${ge}</span>
                            <span style="font-family:${i};font-size:8px;color:${e.dim}">/100</span>
                            <span style="font-family:${i};font-size:8px;font-weight:700;color:${Ze}">${Ki}</span>
                        </div>
                    </div>
                    <div style="width:100%;height:4px;background:${e.border}"><div style="width:${ge}%;height:100%;background:${Ze}"></div></div>
                ...
                <div style="padding:6px 14px;border-bottom:1px solid ${e.border};">
                    ...
                    ${Oe.length===0?`<div style="font-family:${i};font-size:8px;color:${e.dim};padding:4px 0;">No competing bids yet. First mover advantage.</div>`:`<div style="display:flex;gap:4px;flex-wrap:wrap;">
                            ${Oe.map(N=>`<span style="padding:2px 6px;font-family:${i};font-size:7px;color:${e.muted};background:${e.card};border:1px solid ${e.border};">${N.name} <span style="color:${e.dim}">Q:${N.quality}</span></span>`).join("")}
                        </div>
        ...
        <div style="padding:10px 16px;border-top:1px solid ${e.border};background:${e.card};display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">
            <div style="display:flex;gap:12px;">
                <div><div style="font-family:${i};font-size:7px;color:${e.dim}">YOUR BID</div><div style="font-family:${i};font-size:14px;font-weight:700;color:${Q?e.red:e.gold}">${v(Xe)}</div></div>
                <div><div style="font-family:${i};font-size:7px;color:${e.dim}">EST. PROFIT</div><div style="font-family:${i};font-size:14px;font-weight:700;color:${e.greenBright}">+${v(ht)}</div></div>
                <div><div style="font-family:${i};font-size:7px;color:${e.dim}">QUALITY</div><div style="font-family:${i};font-size:14px;font-weight:700;color:${Ze}">${ge}</div></div>
            </div>
    ...
    </div>`,document.body.appendChild(Le)}let Nt=!1;async function No(){if(Nt||!se)return;const i=se,e=i.required_materials||{},t=Object.keys(e),n=Number(i.budget_ceiling||0),o=Number(i.timeline_ticks||8),s={concrete:36e4,steel:5e5,glass_facades:56e4,em_systems:64e4,lumber:24e4,heavy_parts:8e5,aggregate:16e4,asphalt:28e4},a={LOW:.5,STD:1,HIGH:2};let l=0;for(const L of t){const R=Number(e[L]||0),O=pe[L]||"STD",ie=s[L]||3e5;l+=R*Math.round(ie*a[O])}const d=15200,r=i.required_workforce||{},c=Number(r.general||0)+Number(r.skilled||0)||100,m=Math.max(40,Math.round(c*.5)),p=c*2,u=Math.max(0,Math.min(1,(D-m)/(p-m||1))),y=Math.round(4.5-u*8),g=Math.max(Math.round(o*.6),o+y),_=D*d*g,h=n,w=[{name:"Municipal Zoning Approval",cost:18e4,required:!0},{name:"Structural Engineering Cert.",cost:24e4,required:!0},{name:"Environmental Impact Assessment",cost:34e4,required:h>2e7},{name:"Seismic Resilience Compliance",cost:21e4,required:h>5e7},{name:"Fire Safety Certification",cost:12e4,required:h>1e7}],I=new Set(["Municipal Zoning Approval","Structural Engineering Cert."]),C=w.filter(L=>L.required&&!I.has(L.name)).reduce((L,R)=>L+R.cost,0),x=l+_+C+4e5,T=Math.round(x*(fe/100)),q=x+T;if(q>n){alert("Bid exceeds budget ceiling. Reduce costs or markup.");return}const M=Object.values(pe),A=M.length>0?Math.round(M.reduce((L,R)=>L+(R==="HIGH"?85:R==="STD"?65:45),0)/M.length):50;if(confirm('Submit bid for "'+i.name+`"?
...
Once submitted, your bid cannot be changed.`)){Nt=!0;try{const{data:L}=await $.from("shard").select("current_tick").eq("name","Alpha Shard").single(),R=L?.current_tick||0,O={};for(const Y of t)O[Y]=pe[Y]||"STD";const{error:ie}=await $.from("contract_bids").insert({contract_id:i.id,faction_id:f.id,bid_price:q,material_grades:O,labor_count:D,markup_pct:fe,estimated_cost:x,estimated_quality:A,status:"pending",submitted_at_tick:R});if(ie)throw ie;i.status==="open"&&await $.from("construction_contracts").update({status:"bidding"}).eq("id",i.id).eq("status","open"),ci(),alert(`Bid submitted successfully!
...
Lowest qualified bid wins.`),typeof ue=="function"&&await ue()}catch(L){alert("Bid submission failed: "+L.message)}finally{Nt=!1}}}window.openBidAssembly=Ao;window.closeBidAssembly=ci;window.bidSetGrade=zo;window.bidSetWorkers=qo;window.bidSetMarkup=Lo;window.submitBidAssembly=No;let Bt=!1;async function Bo(i){if(Bt)return;const e=1e6,t=Number(f?.corp_cash_reserves??0);if(t<e){alert("Cannot retract — you need at least $1M in cash reserves to cover the retraction penalty.");return}if(confirm(`Retract your bid?

This will cost $1M as a retraction penalty.
This action cannot be undone.`)){Bt=!0;try{const n=t-e,{error:o}=await $.from("factions").update({corp_cash_reserves:n}).eq("id",f.id);if(o)throw o;const{error:s}=await $.from("contract_bids").delete().eq("contract_id",i).eq("faction_id",f.id);if(s)throw s;f.corp_cash_reserves=n,typeof je=="function"&&je(n),alert("Bid retracted. $1M penalty applied."),Qe(),await ue()}catch(n){alert("Failed to retract bid: "+(n.message||"Unknown error"))}finally{Bt=!1}}}window.retractBid=Bo;let Ye=[],Te=0,te=null,Rt=!1,Ot=!1,Pt=!1;async function Ro(){if(!Ie||Ot)return;Ot=!0,te=Ie,Te=0;const{data:i,error:e}=await $.from("contract_bids").select("*, factions(faction_name, corp_ticker, corp_subsector)").eq("contract_id",te.id).in("status",["pending","won"]).order("bid_price",{ascending:!0});if(Ot=!1,e){alert("Failed to load bids: "+e.message);return}Ye=(i||[]).map(t=>({...t,corp:t.factions?.faction_name||"Unknown",abbr:t.factions?.corp_ticker||"???",subsector:t.factions?.corp_subsector||"—"})),Qe(),Yi()}function xt(){document.getElementById("bid-review-overlay")?.remove(),te=null}function Oo(i){Te=i,Yi()}async function Po(){if(Rt||Ye.length===0)return;const i=Ye[Te];if(!(!i?.id||!i.faction_id)&&confirm("Accept bid from "+i.corp+`?

Bid Price: `+v(i.bid_price)+`
Quality: `+i.estimated_quality+`/100
Workers: `+i.labor_count+`

This will award the contract. The project begins immediately.`)){Rt=!0;try{const{data:e}=await $.from("shard").select("current_tick").eq("name","Alpha Shard").single(),t=e?.current_tick||0,{error:n}=await $.from("contract_bids").update({status:"won"}).eq("id",i.id);if(n)throw n;const{error:o}=await $.from("contract_bids").update({status:"lost"}).eq("contract_id",te.id).neq("id",i.id);if(o)throw o;const{error:s}=await $.from("construction_contracts").update({status:"awarded",awarded_to_faction:i.faction_id,awarded_at_tick:t}).eq("id",te.id);if(s)throw s;xt(),alert("Contract awarded to "+i.corp+`!

Bid: `+v(i.bid_price)+`
Project begins immediately.`),typeof ue=="function"&&await ue()}catch(e){alert("Failed to accept bid: "+(e.message||e))}finally{Rt=!1}}}async function Do(){if(!(!te||Pt)&&confirm(`Decline all bids and cancel this project?

No bids will be accepted. The contract will be removed.`)){Pt=!0;try{const{error:i}=await $.from("contract_bids").update({status:"lost"}).eq("contract_id",te.id);if(i)throw i;const{error:e}=await $.from("construction_contracts").update({status:"expired"}).eq("id",te.id);if(e)throw e;xt(),alert("All bids declined. Contract cancelled."),typeof ue=="function"&&await ue()}catch(i){alert("Failed: "+(i.message||i))}finally{Pt=!1}}}function Yi(){if(document.getElementById("bid-review-overlay")?.remove(),!te||Ye.length===0)return;const i="'JetBrains Mono', monospace",e={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",accentBright:"#a3b07e",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},t=te,n=Ye;Te>=n.length&&(Te=0);const o=n[Te],s=Number(t.budget_ceiling||0),a=Number(t.timeline_ticks||36),l=Math.min(...n.map(u=>u.bid_price)),d=Math.max(...n.map(u=>u.estimated_quality||0));let r="";for(let u=0;u<n.length;u++){const y=n[u],g=u===Te,_=y.bid_price===l,h=(y.estimated_quality||0)===d,w=y.bid_price>s;r+=`
        <div onclick="reviewSelectBid(${u})" style="padding:10px 16px;border-bottom:1px solid ${e.border};cursor:pointer;border-left:2px solid ${g?e.accent:"transparent"};background:${g?"rgba(139,154,107,0.03)":"transparent"};">
            ...
            <div style="display:flex;gap:0;background:${e.card};border:1px solid ${e.border};">
                <div style="flex:1.2;padding:5px 10px;border-right:1px solid ${e.border}">
                    ...
                    <div style="font-family:${i};font-size:14px;font-weight:700;color:${w?e.red:e.text}">${v(y.bid_price)}</div>
    ...
    </div>`,document.body.appendChild(p)}window.openBidReview=Ro;window.closeBidReview=xt;window.reviewSelectBid=Oo;window.acceptBid=Po;window.declineAllBids=Do;window.switchToExpansion=Pi;window.switchToOperations=Di;window.hfSetChange=Jn;window.hfReset=Xn;window.hfConfirm=Zn;document.querySelector('[data-tab="operations"]')?.addEventListener("click",function(i){this.classList.contains("active")||(i.preventDefault(),Di(i))});Vn();
