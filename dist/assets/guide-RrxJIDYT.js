...
`},nation:{title:"Nation Overview Guide",html:`
...
`},government:{title:"Government Guide",html:`
...
`},events:{title:"Events Guide",html:`
...
`};let e=null;function l(){return e||(e=document.createElement("div"),e.className="guide-overlay",e.id="guide-overlay",e.innerHTML=`
    ...
    `,document.body.appendChild(e),e.querySelector("#guide-close").addEventListener("click",s),e.addEventListener("click",t=>{t.target===e&&s()}),document.addEventListener("keydown",t=>{t.key==="Escape"&&e.classList.contains("active")&&s()}),e)}function d(){const t=window.__currentTab||"dashboard",r=a[t]||n,i=l();document.getElementById("guide-panel-title").textContent=r.title,document.getElementById("guide-panel-body").innerHTML=r.html,i.classList.add("active")}function s(){e&&e.classList.remove("active")}const c={dashboard:"Home",nation:"Nation",government:"Government",politics:"Politics",laws:"Bills",diplomacy:"Diplomacy",economy:"Economy",events:"Events",elections:"Elections"},h=["dashboard","home"];function o(){const t=document.getElementById("guide-btn");if(!t)return;const r=window.__currentTab||"dashboard";if(h.includes(r)){t.style.display="none";return}const i=c[r]||r.charAt(0).toUpperCase()+r.slice(1);t.textContent=i+" Guide",t.style.display="",t.addEventListener("click",d)}if(document.getElementById("guide-btn"))o();else{const t=new MutationObserver(()=>{document.getElementById("guide-btn")&&(o(),t.disconnect())});t.observe(document.body,{childList:!0,subtree:!0})}
