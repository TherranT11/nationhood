import{_ as b}from"./supabase-client-BXEzLDpS.js";import{e as $}from"./utils-C2W-HleY.js";import{initMessaging as rn}from"./messaging-B5Fng3EZ.js";import{c as ln,a as Gt,E as Ze,b as ft,d as Si,e as dn,f as cn,h as _i}from"./equipment-DsuDdEne.js";const Ii={LOW:"#c84",STD:"#ca5",HIGH:"#5c5"},ye=[{key:"concrete",name:"Concrete",category:"RAW",tiers:{LOW:{requirements:[]},STD:{requirements:[{stat:"manufacturing_output",min:20}]},HIGH:{requirements:[{stat:"manufacturing_output",min:50},{stat:"higher_education",min:40}]}},priceDrivers:["manufacturing_output","inflation","fuel_prices","urbanization"]},{key:"steel",name:"Steel",category:"RAW",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:10}]},STD:{requirements:[{stat:"manufacturing_output",min:35},{stat:"rare_minerals",min:20}]},HIGH:{requirements:[{stat:"manufacturing_output",min:60},{stat:"rare_minerals",min:40},{stat:"higher_education",min:45}]}},priceDrivers:["manufacturing_output","rare_minerals","inflation","fuel_prices"]},{key:"lumber",name:"Lumber",category:"RAW",tiers:{LOW:{requirements:[{stat:"arable_land",min:10}]},STD:{requirements:[{stat:"arable_land",min:30},{stat:"physical_infrastructure",min:20}]},HIGH:{requirements:[{stat:"arable_land",min:50},{stat:"manufacturing_output",min:30}]}},priceDrivers:["arable_land","physical_infrastructure","inflation"]},{key:"aggregate",name:"Aggregate",category:"RAW",tiers:{LOW:{requirements:[]},STD:{requirements:[{stat:"rare_minerals",min:15},{stat:"physical_infrastructure",min:20}]},HIGH:{requirements:[{stat:"rare_minerals",min:35},{stat:"manufacturing_output",min:25}]}},priceDrivers:["rare_minerals","physical_infrastructure","inflation"]},{key:"em",name:"E&M Systems",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:15}]},STD:{requirements:[{stat:"manufacturing_output",min:40},{stat:"digital_infrastructure",min:25}]},HIGH:{requirements:[{stat:"manufacturing_output",min:55},{stat:"digital_infrastructure",min:50},{stat:"energy_generation",min:40}]}},priceDrivers:["manufacturing_output","digital_infrastructure","inflation","energy_generation"]},{key:"glass",name:"Glass & Facades",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:20}]},STD:{requirements:[{stat:"manufacturing_output",min:40},{stat:"standard_of_living",min:35}]},HIGH:{requirements:[{stat:"manufacturing_output",min:60},{stat:"digital_infrastructure",min:40},{stat:"higher_education",min:50}]}},priceDrivers:["manufacturing_output","standard_of_living","inflation"]},{key:"asphalt",name:"Asphalt",category:"RAW",tiers:{LOW:{requirements:[{stat:"oil_and_gas",min:10}]},STD:{requirements:[{stat:"oil_and_gas",min:30},{stat:"manufacturing_output",min:25}]},HIGH:{requirements:[{stat:"oil_and_gas",min:45},{stat:"manufacturing_output",min:40},{stat:"physical_infrastructure",min:40}]}},priceDrivers:["oil_and_gas","manufacturing_output","inflation","fuel_prices"]},{key:"heavy",name:"Heavy Machinery Parts",category:"MANUFACTURED",tiers:{LOW:{requirements:[{stat:"manufacturing_output",min:40},{stat:"rare_minerals",min:30}]},STD:{requirements:[{stat:"manufacturing_output",min:60},{stat:"rare_minerals",min:45},{stat:"higher_education",min:40}]},HIGH:{requirements:[{stat:"manufacturing_output",min:75},{stat:"rare_minerals",min:60},{stat:"higher_education",min:55},{stat:"digital_infrastructure",min:45}]}},priceDrivers:["manufacturing_output","rare_minerals","higher_education","digital_infrastructure"]}];function ze(i,e,t){const n=ye.find(s=>s.key===i);if(!n)return{available:!1,failedStat:"unknown_material"};const o=n.tiers[e];if(!o)return{available:!1,failedStat:"unknown_tier"};for(const s of o.requirements){const a=Number(t?.[s.stat]??0);if(a<s.min)return{available:!1,failedStat:s.stat,failedMin:s.min,nationValue:a}}return{available:!0}}function ei(i,e,t){const o={concrete:{LOW:200,STD:300,HIGH:500},steel:{LOW:400,STD:700,HIGH:1200},lumber:{LOW:80,STD:130,HIGH:200},aggregate:{LOW:40,STD:60,HIGH:100},em:{LOW:400,STD:700,HIGH:1200},glass:{LOW:300,STD:500,HIGH:900},asphalt:{LOW:120,STD:200,HIGH:350},heavy:{LOW:800,STD:1400,HIGH:2400}}[i]?.[e];if(!o)return 0;const s=ye.find(l=>l.key===i);if(!s)return o;let a=1;for(const l of s.priceDrivers){const d=Number(t?.[l]??50);l==="inflation"||l==="fuel_prices"?a*=1+(d-50)/200:a*=1-(d-50)/250}return a=Math.max(.4,Math.min(2.5,a)),Math.round(o*a)}function Mi(i,e,t){const o={concrete:{LOW:5e3,STD:3e3,HIGH:1e3},steel:{LOW:2e3,STD:1500,HIGH:500},lumber:{LOW:8e3,STD:4e3,HIGH:1500},aggregate:{LOW:15e3,STD:6e3,HIGH:2e3},em:{LOW:1e3,STD:700,HIGH:300},glass:{LOW:1500,STD:800,HIGH:300},asphalt:{LOW:4e3,STD:2e3,HIGH:800},heavy:{LOW:400,STD:200,HIGH:80}}[i]?.[e]||0,a=ye.find(r=>r.key===i)?.priceDrivers?.[0],d=.3+(a?Number(t?.[a]??50):50)/50*.7;return Math.round(o*d)}const ti=["LOW","STD","HIGH"],Wt={LOW:"Low",STD:"Standard",HIGH:"High"};let Je=[],m=null,E=null,j=null,He=[],Ye={},ae=[],F={},Ft=-1,H="concrete",U="STD",se=500,J=[],Vt=0,ee="trucks",pe=0,fe=1,we=[],qe=null,dt=[],Yt=null,st=null,Qt="ALL",Kt="TIMELINE";function B(i){return Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(1)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i}function pn(i){if(i>=12){const e=Math.floor(i/12),t=i%12;return t>0?e+"y "+t+"mo":e+"y"}return i+" ticks"}function te(i){return Math.abs(i)>=1e9?"$"+(i/1e9).toFixed(1)+"B":Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(0)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i}function ii(i){return i==="civil_engineering"?"CIVIL":i==="industrial"?"INDUSTRIAL":i==="mega_project"?"MEGA":i?.toUpperCase()||"—"}function Ai(i){return i==="civil_engineering"?"light":i==="industrial"?"heavy":i==="mega_project"?"mega":"light"}function fn(){st&&clearInterval(st),st=setInterval(()=>{if(!Yt)return;const i=Yt-Date.now();if(i<=0){document.getElementById("tick-countdown").textContent="Tick due...",clearInterval(st);return}const e=Math.floor(i/36e5),t=Math.floor(i%36e5/6e4),n=Math.floor(i%6e4/1e3);document.getElementById("tick-countdown").textContent=e+"h "+t+"m "+n+"s"},1e3)}function mn(){document.body.classList.toggle("light-mode");const i=document.getElementById("theme-toggle");i.textContent=document.body.classList.contains("light-mode")?"Dark":"Light"}function un(i,e){i==="type"&&(Qt=e),i==="sort"&&(Kt=e),document.querySelectorAll(`.filter-pill[data-filter="${i}"]`).forEach(t=>{t.classList.toggle("active",t.dataset.value===e)}),qi()}const hi={"Civil Engineering":"civil_engineering","Industrial Construction":"industrial",Megaprojects:"mega_project"};function zi(i){if(!m)return!1;if(hi[m.corp_subsector]===i.sector)return!0;const t=(G||[]).filter(n=>n.type==="regional_hq"&&n.is_active&&n.nation_id===i.nation_id);for(const n of t)if(hi[n.subsector]===i.sector)return!0;return!1}function qi(){const i=document.getElementById("oc-list");let e=[...He];if(Qt==="GOVERNMENT"?e=e.filter(o=>o.issuer_type==="GOVERNMENT"):Qt==="PRIVATE"&&(e=e.filter(o=>o.issuer_type==="PRIVATE")),Kt==="TIMELINE"&&e.sort((o,s)=>(o.timeline_ticks||0)-(s.timeline_ticks||0)),Kt==="BUDGET"&&e.sort((o,s)=>(s.budget_ceiling||0)-(o.budget_ceiling||0)),document.getElementById("oc-count").textContent=e.length+" AVAILABLE",e.length===0){i.innerHTML=`
            ...
            </div>`;return}const t=j?.current_tick||0;let n="";for(const o of e){const s=o.issuer_type==="GOVERNMENT",a=s?"gov":"private",l=zi(o),d=l?"":" locked",r=Ai(o.sector),c=ii(o.sector),p=(o.timeline_ticks||0)>18?" warn":"",f=o.bidding_ends_tick?Math.max(0,o.bidding_ends_tick-t):"?";n+=`
            <div class="oc-item${d}" data-contract-id="${o.id}">
                ...
                <div class="oc-item__stats">
                    ...
                    <div class="oc-stat">
                        ...
                        <div class="oc-stat__value${p}">${pn(o.timeline_ticks||0)}</div>
                    </div>
                    <div class="oc-stat">
                    ...
                    <div class="oc-eligibility">
                        ${Ye[o.id]?`<span class="oc-eligible-badge yes" style="background:var(--green-faint);border-color:var(--green-border);color:var(--green);">BID: ${te(Ye[o.id].bid_price)}</span>`:`<span class="oc-eligible-badge ${l?"yes":"no"}">${l?"ELIGIBLE":"LOCKED"}</span>`}
                    </div>
            ...
            </div>`}i.innerHTML=n,i.querySelectorAll(".oc-item:not(.locked)").forEach(o=>{o.addEventListener("click",()=>{const s=o.dataset.contractId,a=He.find(l=>l.id===s);a&&Ni(a)})})}let Ne=null;function Ni(i){Ne=i;const e=document.getElementById("cd-overlay"),t=i.issuer_type==="GOVERNMENT",n=t?"gov":"private",o=(E?.name||m.nation||"—").toUpperCase(),s=zi(i);document.getElementById("cd-header-left").innerHTML=`
    ...
    `;const a=document.getElementById("cd-blueprint");i.blueprint_svg?(a.innerHTML=i.blueprint_svg,a.style.display=""):(a.innerHTML=Mn(i),a.style.display="");const l=i.permits_required||[],d=i.required_equipment||i.equipment_required||[],r=i.required_materials||i.materials_estimated||{},p={civil_engineering:"Civil Engineering",industrial:"Industrial Construction",mega_project:"Megaprojects"}[i.sector]||i.spec_category||i.sector||"—";let f="var(--teal)";i.sector==="industrial"&&(f="var(--orange)"),i.sector==="mega_project"&&(f="var(--red)");let u=B(i.budget_ceiling||i.budget||0),y=(i.timeline_ticks||i.timeline_months||0)+" Months",g="";g+=`
        <div class="cd-issue">
            ...
            </div>`),g+='<div class="cd-details">',i.project_type&&(g+=he("Type",i.project_type)),i.project_subtype&&(g+=he("Sub-Type",i.project_subtype)),g+=he("Specialization",p,f),g+=he("Total Budget",u,"var(--green)"),g+=he("Timeline",y),g+=he("Nation",E?.name||m.nation||"—"),i.region&&(g+=he("Region",i.region)),g+="</div>",l.length>0&&(g+=`
            <div class="cd-items">
                <div class="cd-section-label">Permits Required</div>
                <div class="cd-items__list">
                    ${l.map(A=>{const P=A.status==="approved"?"approved":"required",L=A.status==="approved"?"&#10003;":"&#9675;";return`<div class="cd-chip ${P}">
                            <span class="cd-chip__icon">${L}</span>
                            <span class="cd-chip__label">${$(A.name)}</span>
                        </div>`}).join("")}
                </div>
            </div>`),r.length>0&&(g+=`
            <div class="cd-materials">
                ...
                ${r.map(A=>`
                    <div class="cd-mat-row">
                        <span class="cd-mat-row__name">${$(A.name)}</span>
                        <span class="cd-mat-row__qty">${$(String(A.quantity||"—"))}</span>
                    </div>`).join("")}
            </div>`),document.getElementById("cd-content").innerHTML=g;const _=l.filter(A=>A.status==="approved").length,h=l.length-_,k=d.filter(A=>A.owned).length,M=d.length-k;let C="";d.length>0&&(M===0?C+='<span class="cd-footer__badge ok">ALL EQUIPMENT MET</span>':C+=`<span class="cd-footer__badge bad">${M} EQUIPMENT MISSING</span>`),l.length>0&&(h===0?C+='<span class="cd-footer__badge ok">ALL PERMITS APPROVED</span>':C+=`<span class="cd-footer__badge warn">${h} PERMITS PENDING</span>`);const w=s,x=i.issuer_faction_id===m?.id,T=i.status==="bidding",z=Ye[i.id];document.getElementById("cd-footer").innerHTML=`
    ...
    `,e.classList.add("open"),document.body.style.overflow="hidden"}function it(i){i&&i.target&&i.target!==document.getElementById("cd-overlay")||(document.getElementById("cd-overlay").classList.remove("open"),document.body.style.overflow="",Ne=null)}const Te=["Permits","Planning","Foundation","Structural","Systems","Finishing","Delivery"],wi={WEATHER:{color:"var(--blue)",bg:"var(--blue-faint)",border:"var(--blue-border)"},SUPPLY:{color:"var(--gold)",bg:"var(--gold-faint)",border:"var(--gold-border)"},LABOR:{color:"var(--orange)",bg:"var(--orange-faint)",border:"var(--orange-border)"},REGULATORY:{color:"var(--red)",bg:"var(--red-faint)",border:"var(--red-border)"},EQUIPMENT:{color:"var(--amber)",bg:"var(--amber-faint)",border:"var(--amber-border)"},POLITICAL:{color:"var(--teal)",bg:"var(--teal-faint)",border:"var(--teal-border)"}},ki={LOW:"var(--green)",MODERATE:"var(--amber)",HIGH:"var(--orange)",CRITICAL:"var(--red)"};let I=null;async function Oe(i){const e=ae.find(N=>N.id===i);if(!e)return;const t=Array.isArray(e.contract_bids)?e.contract_bids[0]:e.contract_bids,n=j?.current_tick||0,o=e.awarded_at_tick||n,s=e.timeline_ticks||8,a=Math.max(0,n-o),l=Math.min(100,a/s*100);let d=Math.min(Te.length-1,Math.floor(l/(100/Te.length)));const r=Math.round(l%(100/Te.length)/(100/Te.length)*100),c=e.required_materials||{},p=t?.material_grades||{};let f=[];try{const{data:N}=await b.from("project_material_allocations").select("material_key, quality_tier, quantity, consumed").eq("contract_id",e.id);f=N||[]}catch{}const u={};for(const N of f)u[N.material_key]||(u[N.material_key]={totalAllocated:0,totalConsumed:0,tiers:{}}),u[N.material_key].totalAllocated+=N.quantity,u[N.material_key].totalConsumed+=N.consumed,u[N.material_key].tiers[N.quality_tier]={qty:N.quantity,consumed:N.consumed};const y=Object.entries(c).map(([N,Y])=>{const De=p[N]||"STD",Z=u[N]||{totalAllocated:0,totalConsumed:0,tiers:{}};return{key:N,name:N.replace(/_/g," ").replace(/\b\w/g,xe=>xe.toUpperCase()),grade:De,required:Number(Y),allocated:Z.totalAllocated,consumed:Z.totalConsumed,tiers:Z.tiers,warehouseStock:F[N]||{}}}),g=e.required_equipment||[],_=e.equipment_condition||{},h=g.map(N=>{const Y=J.find(de=>de.equipment_key===N),Z=(Y?.assigned_projects||[]).find(de=>de.contract_id===e.id),xe=Z?Z.units:0;return{key:N,name:N.replace(/_/g," ").replace(/\b\w/g,de=>de.toUpperCase()),ownedTotal:Y?.owned||0,deployed:Y?.deployed||0,available:Math.max(0,(Y?.owned||0)-(Y?.deployed||0)),assignedToProject:xe,condition:_[N]??(Y?.condition||100)}}),k=e.budget_ceiling||0,M=t?.estimated_cost||0,C=Math.round(M*Math.min(1,a/s)),w=t?.estimated_quality||65,x=w>=80?"STRONG":w>=60?"PROMISING":w>=40?"FAIR":"UNCERTAIN",T=e.required_workforce||{},z=e.workers_assigned||{},A=(T.general||0)+(T.skilled||0)+(T.innovative||0),P=(z.general||0)+(z.skilled||0)+(z.innovative||0),L=t?.labor_count||A,W=Number(m?.corp_general_workforce??0),X=Number(m?.corp_skilled_workforce??0),ve=Number(m?.corp_innovative_workforce??0);I={project:e,bid:t,tab:"overview",expandedEvent:-1,selectedResponse:null,currentTick:n,awardedTick:o,totalTicks:s,ticksElapsed:a,phaseIdx:d,phaseProgress:r,materials:y,equipment:h,budget:k,estCost:M,spent:C,quality:w,qualityLabel:x,laborCount:L,wfNeeded:A,wfAssigned:P,reqWf:T,assignedWf:z,corpGeneral:W,corpSkilled:X,corpInnovative:ve,events:[]},document.getElementById("pm-overlay").classList.add("open"),document.body.style.overflow="hidden",Li(e.id).then(()=>Be()),Be()}let R=!1;async function yn(i,e,t){if(!(R||!I||!m)){R=!0;try{const{data:n,error:o}=await b.rpc("allocate_material_to_project",{p_contract_id:I.project.id,p_faction_id:m.id,p_material_key:i,p_quality_tier:e,p_quantity:t});if(o){alert("Allocation failed: "+o.message);return}if(n&&!n.success){alert(n.error||"Allocation failed");return}await oi(),await Oe(I.project.id)}catch(n){alert("Allocation error: "+n.message)}finally{R=!1}}}async function gn(i,e,t){if(!(R||!I||!m)){R=!0;try{const{data:n,error:o}=await b.rpc("deallocate_material_from_project",{p_contract_id:I.project.id,p_faction_id:m.id,p_material_key:i,p_quality_tier:e,p_quantity:t});if(o){alert("Return failed: "+o.message);return}if(n&&!n.success){alert(n.error||"Return failed");return}await oi(),await Oe(I.project.id)}catch(n){alert("Return error: "+n.message)}finally{R=!1}}}async function vn(i,e){if(!(R||!I||!m)){R=!0;try{const t=I.project,n=t.workers_assigned||{},o=Number(n[i]||0),s=Number((t.required_workforce||{})[i]||0),a=Math.min(e,s-o);if(a<=0){alert("Already fully staffed for "+i);return}const l={...n,[i]:o+a},{error:d}=await b.from("construction_contracts").update({workers_assigned:l}).eq("id",t.id);if(d){alert("Assign failed: "+d.message);return}await Oe(t.id)}catch(t){alert("Assign error: "+t.message)}finally{R=!1}}}async function xn(i,e){if(!(R||!I||!m)){R=!0;try{const t=I.project,n=t.workers_assigned||{},o=Number(n[i]||0),s=Math.min(e,o);if(s<=0){alert("No "+i+" assigned");return}const a={...n,[i]:o-s},{error:l}=await b.from("construction_contracts").update({workers_assigned:a}).eq("id",t.id);if(l){alert("Unassign failed: "+l.message);return}await Oe(t.id)}catch(t){alert("Unassign error: "+t.message)}finally{R=!1}}}async function bn(i,e){if(!(R||!I||!m)){R=!0;try{const t=J.find(d=>d.equipment_key===i);if(!t){alert("Equipment not found in inventory.");return}const n=Math.max(0,(t.owned||0)-(t.deployed||0));if(n<e){alert("Not enough available "+i+" ("+n+" available).");return}const o=(t.deployed||0)+e,s=[...t.assigned_projects||[]],a=s.find(d=>d.contract_id===I.project.id);a?a.units+=e:s.push({contract_id:I.project.id,contract_name:I.project.name,units:e});const{error:l}=await b.from("corp_equipment").update({deployed:o,assigned_projects:s}).eq("faction_id",m.id).eq("equipment_key",i);if(l){alert("Deploy failed: "+l.message);return}await li(),await Oe(I.project.id)}catch(t){alert("Deploy error: "+t.message)}finally{R=!1}}}async function $n(i){if(!(R||!I||!m)){R=!0;try{const e=J.find(l=>l.equipment_key===i);if(!e){alert("Equipment not found.");return}const t=[...e.assigned_projects||[]],n=t.findIndex(l=>l.contract_id===I.project.id);if(n===-1){alert("Equipment not deployed to this project.");return}const o=t[n].units;t.splice(n,1);const s=Math.max(0,(e.deployed||0)-o),{error:a}=await b.from("corp_equipment").update({deployed:s,assigned_projects:t}).eq("faction_id",m.id).eq("equipment_key",i);if(a){alert("Undeploy failed: "+a.message);return}await li(),await Oe(I.project.id)}catch(e){alert("Undeploy error: "+e.message)}finally{R=!1}}}function _n(i){i&&i.target!==document.getElementById("pm-overlay")||(document.getElementById("pm-overlay").classList.remove("open"),document.body.style.overflow="",I=null)}function hn(i){I&&(I.tab=i,I.expandedEvent=-1,I.selectedResponse=null,Be())}function wn(i){I&&(I.expandedEvent=I.expandedEvent===i?-1:i,I.selectedResponse=null,Be())}function kn(i){I&&(I.selectedResponse=I.selectedResponse===i?null:i,Be())}function Be(){if(!I)return;const i=I,e=i.project,t=e.issuer_type==="GOVERNMENT",n=ii(e.sector),o=m?.nation||"Nation",s=i.awardedTick+i.totalTicks,a=Math.max(0,s-i.currentTick),l=i.currentTick>s,d=i.budget>0?Math.round(i.spent/i.budget*100):0,r=d>85?"var(--red)":d>60?"var(--amber)":"var(--teal)",c=i.budget-i.spent,p=i.events.filter(_=>_.status==="ACTIVE").length;document.getElementById("pm-header").innerHTML=`
    ...
    `;let f='<div class="pm-phase__bar">';for(let _=0;_<Te.length;_++){const h=_<i.phaseIdx,k=_===i.phaseIdx;f+=`<div class="pm-phase__seg">
    ...
    `}function En(i,e,t,n,o,s,a){const l=Ce(i.awardedTick+i.totalTicks);Ce(i.awardedTick+i.totalTicks);const d=Ce(i.awardedTick),r=[{label:"Budget",value:te(i.budget),sub:`${n}% spent`,color:t},{label:"Spent",value:te(i.spent),color:"var(--red)"},{label:"Remaining",value:te(o),color:"var(--green)"},{label:"Quality",value:`${i.quality}/100`,sub:i.qualityLabel,color:i.quality>=70?"var(--green)":i.quality>=50?"var(--amber)":"var(--red)"},{label:"Workforce",value:`${i.laborCount}/${i.wfNeeded}`,sub:`Bid: ${i.laborCount}`,color:i.laborCount<i.wfNeeded?"var(--orange)":"var(--text-bright)"},{label:"Remaining",value:`${s} ticks`,sub:a?"OVERDUE":`Deadline: ${l}`,color:a?"var(--red)":"var(--text-bright)"}];let c="";c+=`<div style="padding:0 16px"><div class="pm-section">
        ...
        <div class="pm-desc">${$(e.description||e.name)}</div>
            ...
            ${g.sub?`<div class="pm-metric__sub">${$(g.sub)}</div>`:""}
        ...
        <div class="pm-manager">
            ...
            <span class="pm-evt__status" style="color:${s?"var(--red)":"var(--text-dim)"};font-weight:${s?"700":"400"}">${s?"REQUIRES RESPONSE":"RESOLVED"}</span>
                    ...
                    </div>`,e+=`<div class="pm-resp__detail">${$(r.detail)}</div>`,e+='<div class="pm-resp__costs">',r.cost&&(e+=`<span class="pm-resp__cost" style="color:var(--red)">Cost: ${te(r.cost)}</span>`),r.qualityImpact&&r.qualityImpact!==0&&(e+=`<span class="pm-resp__cost" style="color:${r.qualityImpact>0?"var(--green)":"var(--red)"}">Quality: ${r.qualityImpact>0?"+":""}${r.qualityImpact}</span>`),!r.cost&&(!r.qualityImpact||r.qualityImpact===0)&&(e+='<span class="pm-resp__cost" style="color:var(--green)">No additional cost</span>'),e+="</div>",c&&(e+=`<div class="pm-resp__confirm">
        ...
        <g transform="translate(470,172)">
    ...
    </svg>`}async function ge(){if(!m||!m.nation_id)return;const{data:i,error:e}=await b.from("construction_contracts").select("*").in("status",["open","bidding"]).order("generated_at_tick",{ascending:!1});if(e?(console.warn("Failed to load contracts:",e.message),He=[]):He=i||[],Ye={},m&&He.length>0){const t=He.map(o=>o.id),{data:n}=await b.from("contract_bids").select("contract_id, bid_price, estimated_quality, status").eq("faction_id",m.id).in("contract_id",t);for(const o of n||[])Ye[o.contract_id]=o}qi()}function An(){const i=document.getElementById("ap-list"),e=document.getElementById("ap-footer");if(document.getElementById("ap-count").textContent=ae.length+" ACTIVE",ae.length===0){i.innerHTML=`<div class="ap-empty">
            <div class="ap-empty__text">No active projects.<br>Win a contract bid to start<br>your first construction project.</div>
        </div>`,e.style.display="none";return}const t=j?.current_tick||0;let n=0,o=0,s="";for(const a of ae){const l=a.issuer_type==="GOVERNMENT",d=l?"gov":"private",r=Array.isArray(a.contract_bids)?a.contract_bids[0]:a.contract_bids,c=r?.bid_price||0,p=r?.estimated_cost||0,f=r?.estimated_quality||0,u=a.budget_ceiling||0,y=a.awarded_at_tick||t,g=a.stalled_ticks||0,_=Math.max(0,t-y),h=Math.max(0,_-g),k=a.timeline_ticks||8,M=Math.max(0,k-h),C=Math.min(100,Math.round(h/k*100)),w=h>k,x=g>0;Ai(a.sector);const T=ii(a.sector);n+=u,o+=c,s+=`<div class="ap-item" onclick="openProjectModal('${a.id}')">
            <div class="ap-item__row1">
                <div class="ap-item__info">
                    <div class="ap-item__name">${$(a.name)}</div>
                    <div style="font-family:var(--font-mono);font-size:8px;color:var(--text-dim);">${$(a.issuer_name||"—")} · ${T}</div>
                </div>
            ...
            <div class="ap-details">
                ...
                <div class="ap-detail-cell" style="flex:1">
                    ...
                    <div class="ap-detail-cell__value" style="font-size:10px;font-weight:700;color:var(--red)">${te(p)}</div>
                </div>
        ...
        </div>`}i.innerHTML=s,e.style.display=ae.length>0?"":"none",ae.length>0&&(document.getElementById("ap-total-crew").textContent=ae.length,document.getElementById("ap-total-budget").textContent=te(n),document.getElementById("ap-total-spent").textContent=te(o))}async function Bi(){if(!m)return;const{data:i,error:e}=await b.from("construction_contracts").select("*, contract_bids!inner(bid_price, material_grades, labor_count, estimated_cost, estimated_quality)").eq("awarded_to_faction",m.id).in("status",["awarded","in_progress"]).eq("contract_bids.faction_id",m.id).order("awarded_at_tick",{ascending:!0});e?(console.warn("Failed to load active projects:",e.message),ae=[]):ae=i||[],An()}const mt=3e4;function ut(){let i=0,e=0;for(const t of ye)for(const n of ti){const o=F[t.key]?.[n];o&&(i+=o.qty,e+=o.value)}return{totalUnits:i,totalValue:e}}function ni(){const i=document.getElementById("wh-list"),{totalUnits:e,totalValue:t}=ut();document.getElementById("wh-count").textContent=e.toLocaleString()+" UNITS",document.getElementById("wh-total-value").textContent=B(t);const n=Math.round(e/mt*100),o=document.getElementById("wh-capacity");o.textContent=n+"%",o.style.color=n>80?"var(--red)":n>50?"var(--orange)":"var(--green)";let s="";for(let a=0;a<ye.length;a++){const l=ye[a],d=Ft===a,r=F[l.key]?.LOW||{qty:0,value:0},c=F[l.key]?.STD||{qty:0,value:0},p=F[l.key]?.HIGH||{qty:0,value:0},f=r.qty+c.qty+p.qty,u=r.value+c.value+p.value,y=f===0,g=ze(l.key,"LOW",E),_=ze(l.key,"STD",E),h=ze(l.key,"HIGH",E),k=r.qty>0?"wh-dot wh-dot--low":"wh-dot wh-dot--empty",M=c.qty>0?"wh-dot wh-dot--std":"wh-dot wh-dot--empty",C=h.available?p.qty>0?"wh-dot wh-dot--high":"wh-dot wh-dot--empty":"wh-dot wh-dot--locked";if(s+='<div class="wh-row">',s+=`<div class="wh-row__collapsed${d?" expanded":""}" onclick="toggleWhRow(${a})">
            ...
            <span class="wh-row__val${y?" empty":""}">${u>0?B(u):"—"}</span>
                ...
                <span class="wh-expand__hdr-label" style="width:55px;text-align:right">$/UNIT</span>
            </div>`;const w=[{key:"LOW",label:"Low",data:r,avail:g,color:"var(--orange)",dotClass:"wh-dot--low"},{key:"STD",label:"Standard",data:c,avail:_,color:"var(--amber)",dotClass:"wh-dot--std"},{key:"HIGH",label:"High",data:p,avail:h,color:"var(--green)",dotClass:"wh-dot--high"}];for(const x of w){const T=!x.avail.available,z=x.data.qty>0,A=z?"$"+Math.round(x.data.value/x.data.qty):"—";s+=`<div class="wh-grade${T?" unavailable":""}">
                    ...
                    <span class="wh-grade__cpu">${A}</span>
                        ...
                        <span class="wh-lock__text">${x.label.toUpperCase()} GRADE LOCKED — ${$(x.avail.failedStat)} &lt; ${x.avail.failedMin}</span>
                    </div>`);s+="</div>"}s+="</div>"}i.innerHTML=s}function zn(i){Ft=Ft===i?-1:i,ni()}async function oi(){if(!m)return;const{data:i,error:e}=await b.from("corp_warehouse").select("material_key, quality_tier, quantity, total_value").eq("faction_id",m.id);if(F={},e)console.warn("Failed to load warehouse:",e.message);else if(i)for(const t of i)F[t.material_key]||(F[t.material_key]={}),F[t.material_key][t.quality_tier]={qty:t.quantity||0,value:Number(t.total_value)||0};ni()}const qn={manufacturing_output:"base production",rare_minerals:"raw input",inflation:"price modifier",fuel_prices:"transport cost",urbanization:"demand pressure",arable_land:"base production",physical_infrastructure:"transport",digital_infrastructure:"component quality",energy_generation:"electrical supply",standard_of_living:"demand tier",oil_and_gas:"base input",higher_education:"engineering"};function Ri(){const i=(E?.name||m?.nation||"—").toUpperCase();document.getElementById("pr-nation-badge").textContent="LOCAL — "+i;const e=Number(m?.corp_cash_reserves)||0;document.getElementById("pr-cash").textContent=B(e);const{totalUnits:t}=ut(),n=Math.round(t/mt*100),o=document.getElementById("pr-wh-capacity");o.textContent=n+"%",o.style.color=n>80?"var(--red)":n>50?"var(--orange)":"var(--green)",Pi(),ai(),yt()}function Pi(){const i=document.getElementById("pr-mat-grid");let e="";for(const t of ye){const n=H===t.key,o=ti.every(a=>!ze(t.key,a,E).available),s="pr-mat-btn"+(n?" active":"")+(o?" all-locked":"");e+=`<span class="${s}" onclick="setPrMat('${t.key}')">${$(t.name)}</span>`}i.innerHTML=e}function ai(){const i=document.getElementById("pr-tier-bar");let e='<span class="pr-tier-label">GRADE</span>';for(const t of ti){const n=ze(H,t,E),o=U===t,s=n.available?ei(H,t,E):null,a=Ii[t],l=!n.available,d="pr-tier-btn"+(o?" active":"")+(l?" locked":"");e+=`<div class="${d}" onclick="${l?"":`setPrTier('${t}')`}">
        ...
        </div>`}i.innerHTML=e}function yt(){const i=document.getElementById("pr-content"),e=ze(H,U,E),t=ye.find(w=>w.key===H);if(!t)return;if(!e.available){i.innerHTML=`<div class="pr-locked-panel">
            <div class="pr-locked-box">
                <div class="pr-locked-box__title">QUALITY TIER UNAVAILABLE</div>
                <div class="pr-locked-box__desc">
                    ${$(t.name)} — ${Wt[U]} grade
                    is not produced domestically in ${$(E?.name||"—")}.
                </div>
        ...
        </div>`;return}const n=ei(H,U,E),o=Mi(H,U,E),s=n*se,a=o>3e3?"LOW":o>1e3?"MODERATE":"HIGH",l=a==="LOW"?"var(--green)":a==="MODERATE"?"var(--amber)":"var(--red)",d=Number(E?.inflation??50),r=d>55?"up":d<45?"down":"flat",c=r==="up"?"&#9650;":r==="down"?"&#9660;":"&#8212;",p=r==="up"?"var(--red)":r==="down"?"var(--green)":"var(--text-dim)";let f="";f+=`<div style="padding:8px 14px;border-bottom:1px solid var(--border-0);">
        ...
        <div class="pr-drivers__title">Price Drivers (${$(E?.name||"—")})</div>`;for(const w of t.priceDrivers){const x=Number(E?.[w]??50),T=x>=50?"var(--green)":x>=30?"var(--amber)":x>=15?"var(--orange)":"var(--red)",z=qn[w]||w;f+=`<div class="pr-driver-row">
            <span class="pr-driver-row__stat">${$(w)}</span>
            ...
            <span class="pr-driver-row__effect">${$(z)}</span>
        </div>`}f+="</div>";const y=(Number(m?.corp_cash_reserves)||0)>=s,g=se>o,{totalUnits:_}=ut(),h=mt-_,k=se>h,M=h<=0,C=Ii[U];f+=`<div class="pr-order">
        ...
        <div class="pr-order__box">
            <div class="pr-order__header">
                <div style="display:flex;align-items:center;gap:6px;">
                    ...
                    <span style="font-family:var(--font-mono);font-size:9px;color:${C}">${Wt[U]}</span>
                </div>
    ...
    </div>`,i.innerHTML=f}function Nn(i){H=i,U="STD";for(const e of["STD","HIGH","LOW"])if(ze(i,e,E).available){U=e;break}Pi(),ai(),yt()}function Ln(i){U=i,ai(),yt()}function Bn(i){se=i,yt()}let At=!1;async function Rn(){if(At||!m||!E)return;const i=ei(H,U,E),e=Mi(H,U,E),t=i*se,n=Number(m.corp_cash_reserves)||0;if(t>n){alert("Insufficient cash reserves.");return}if(se>e){alert("Exceeds available supply this tick.");return}const{totalUnits:o}=ut(),s=mt-o;if(s<=0){alert("Warehouse is full. Cannot purchase more materials.");return}if(se>s){alert(`Warehouse can only hold ${s.toLocaleString()} more units. Reduce quantity.`);return}At=!0;const a=document.querySelector(".pr-purchase-btn");a&&(a.disabled=!0,a.textContent="...");try{const l=n-t,{error:d}=await b.from("factions").update({corp_cash_reserves:l}).eq("id",m.id);if(d)throw d;const r=F[H]?.[U],c=(r?.qty||0)+se,p=(r?.value||0)+t,{error:f}=await b.from("corp_warehouse").upsert({faction_id:m.id,nation_id:m.nation_id,material_key:H,quality_tier:U,quantity:c,total_value:p,last_purchased_tick:j?.current_tick||null,updated_at:new Date().toISOString()},{onConflict:"faction_id,material_key,quality_tier"});if(f){const{error:u}=await b.from("factions").update({corp_cash_reserves:n}).eq("id",m.id);throw u&&console.error("Cash refund failed after warehouse error:",u.message),f}m.corp_cash_reserves=l,F[H]||(F[H]={}),F[H][U]={qty:c,value:p},ni(),Ri(),a&&(a.textContent="PURCHASED",setTimeout(()=>{a.isConnected&&(a.disabled=!1,a.textContent="PURCHASE")},1500))}catch(l){a&&(a.disabled=!1,a.textContent="PURCHASE"),alert("Purchase failed: "+(l.message||"Unknown error"))}finally{At=!1}}function Oi(i){const e=qe||E;if(!e)return[];const t=ft(i);if(!t)return[];const n=dn(i,e),o=[],s=Number(e?.inflation??50),a=Number(e?.fuel_prices??50);Number(e?.manufacturing_output??50);const l=qe&&E&&qe.id!==E.id;let d=null;if(l&&(d=cn(e,E)),n.newAvailable>0){const r=_i(i,e),c=t.basePrice,p=Math.round(c*((s-50)/200)),f=Math.round(c*((a-50)/300));let u=r;const y=[{label:"Base price",value:B(c)},p!==0?{label:`Inflation (${s})`,mod:(p>=0?"+":"")+B(Math.abs(p))}:null,f!==0?{label:`Fuel transport (${a})`,mod:(f>=0?"+":"")+B(Math.abs(f))}:null].filter(Boolean),g=r-c-p-f;if(g!==0&&!l&&y.push({label:"Demand/scarcity",mod:(g>=0?"+":"")+B(Math.abs(g))}),l&&d){const _=Math.round(r*d.tariff),h=Math.round(r*d.transport);u=r+_+h,y.push({label:`Import tariff (${Math.round(d.tariff*100)}%)`,mod:"+"+B(_)}),y.push({label:`Transport (${d.deliveryTicks} tick${d.deliveryTicks>1?"s":""})`,mod:"+"+B(h)})}o.push({seller:l?`${e.name} Buyers`:"Local Buyers",sellerType:"LOCAL",nation:e.name||"—",distance:l?d?.deliveryTicks||1:0,condition:100,price:Math.round(u),available:n.newAvailable,delivery:l?d.deliveryTicks+" tick"+(d.deliveryTicks>1?"s":""):"Immediate",deliveryTicks:l?d.deliveryTicks:0,used:!1,priceFactors:y,sourceNationId:e.id})}if(n.usedAvailable>0){const r=n.usedCondition,c=_i(i,e,{used:!0,condition:r});let p=c;const f=[{label:"Base price",value:B(t.basePrice)},{label:`Condition (${r}%)`,mod:"-"+B(Math.max(0,t.basePrice-c))}];if(l&&d){const u=Math.round(c*d.tariff),y=Math.round(c*d.transport);p=c+u+y,f.push({label:`Import tariff (${Math.round(d.tariff*100)}%)`,mod:"+"+B(u)}),f.push({label:`Transport (${d.deliveryTicks} tick${d.deliveryTicks>1?"s":""})`,mod:"+"+B(y)})}o.push({seller:l?`${e.name} Buyers`:"Local Buyers",sellerType:"LOCAL",nation:e.name||"—",distance:l?d?.deliveryTicks||1:0,condition:r,price:Math.round(p),available:n.usedAvailable,delivery:l?d.deliveryTicks+" tick"+(d.deliveryTicks>1?"s":""):"Immediate",deliveryTicks:l?d.deliveryTicks:0,used:!0,priceFactors:f,sourceNationId:e.id})}return o}function gt(){const i=Number(m?.corp_cash_reserves)||0;document.getElementById("em-cash").textContent=B(i);const e=ft(ee),t=Ze[e?.tier||1],n=document.getElementById("em-tier-badge");n&&(n.textContent=t.tag,n.style.color=t.color),n.style.background=t.color+"0a",n.style.border="1px solid "+t.color+"33";const o=document.getElementById("em-nation-select");if(o&&o.options.length===0){const l=E?.name||m?.nation||"—";let d=`<option value="">${$(l)} (HQ)</option>`;for(const r of dt)r.id!==E?.id&&(d+=`<option value="${r.id}">${$(r.name)}</option>`);o.innerHTML=d}const s=document.getElementById("em-import-tag"),a=qe&&E&&qe.id!==E.id;s&&(s.style.display=a?"":"none"),Pn(),si()}function Pn(){let i="";for(let e=1;e<=3;e++){const t=Ze[e],n=Gt(e),o=e===3?"em-selector__grid em-selector__grid--t3":"em-selector__grid em-selector__grid--t12";i+=`<div class="em-selector__row">
            <div class="em-selector__tier-label" style="color:${t.color}">${t.tag}</div>
            <div class="${o}">`;for(const s of n){const a=ee===s.key,l=Oi(s.key).length>0;i+=`<span class="em-selector__btn${a?" active":""}${l?"":" no-listings"}"
                style="${a?"background:"+t.color+";border-color:"+t.color:""}"
                onclick="setEmType('${s.key}')">${$(s.name)}</span>`}i+="</div></div>"}document.getElementById("em-selector").innerHTML=`<div class="em-selector">${i}</div>`}function si(){const i=document.getElementById("em-content");if(we=Oi(ee),we.length===0){i.innerHTML=`<div class="em-no-listings"><div class="em-no-listings__box">
        ...
        </div></div>`;return}pe>=we.length&&(pe=0);let e="";for(let n=0;n<we.length;n++){const o=we[n],s=pe===n,a=o.sellerType==="LOCAL"?"var(--teal)":"#5a8aaa",l=Si(o.condition);e+=`<div class="em-listing${s?" selected":""}" style="${s?"border-left-color:"+a:""}" onclick="setEmListing(${n})">`,e+=`<div class="em-listing__row1">
            <div style="display:flex;align-items:center;gap:6px">
            ...
            </div>`),e+="</div>"}const t=we[pe];if(t){const n=ft(ee),o=Ze[n?.tier||1],s=Math.min(t.available,4),a=t.price*fe,l=(Number(m?.corp_cash_reserves)||0)>=a;e+=`<div class="em-purchase"><div class="em-purchase__box">
            ...
            <div class="em-purchase__total">
        ...
        </div></div>`}i.innerHTML=e}async function On(i){if(!i)qe=null;else{let t=dt.find(n=>n.id===i);if(!t)try{const{data:n}=await b.from("nations").select("*").eq("id",i).single();t=n}catch{}qe=t||null}pe=0,fe=1;const e=document.getElementById("em-nation-select");e&&(e.value=i||""),gt()}function Dn(i){ee=i,pe=0,fe=1,gt()}function jn(i){pe=i,fe=1,si()}function Hn(i){fe=i,si()}let zt=!1;async function Un(){if(zt)return;const i=we[pe];if(!i||!m)return;const e=ft(ee);if(!e)return;const t=fe,n=i.price*t,o=Number(m.corp_cash_reserves)||0;if(n>o){alert("Insufficient cash reserves.");return}if(t>i.available){alert("Not enough units available.");return}const s=document.querySelector(".em-purchase-btn");s&&(s.disabled=!0,s.textContent="..."),zt=!0;try{const a=o-n,{error:l}=await b.from("factions").update({corp_cash_reserves:a}).eq("id",m.id);if(l)throw l;const d=!i.deliveryTicks||i.deliveryTicks===0;if(d){const c=J.find(M=>M.equipment_key===ee),p=(c?.owned||0)+t,f=c?.purchase_price_avg||0,u=c?.owned||0,y=u>0?Math.round((f*u+i.price*t)/p):i.price,g=e.maintenancePerUnit*p,_=c?.condition||100,h=Math.round((_*u+i.condition*t)/p),{error:k}=await b.from("corp_equipment").upsert({faction_id:m.id,nation_id:m.nation_id,equipment_key:ee,tier:e.tier,owned:p,deployed:c?.deployed||0,condition:h,maintenance_per_tick:g,purchase_price_avg:y,last_purchased_tick:j?.current_tick||null,updated_at:new Date().toISOString()},{onConflict:"faction_id,equipment_key"});if(k){const{error:M}=await b.from("factions").update({corp_cash_reserves:o}).eq("id",m.id);throw M&&console.error("Cash refund failed:",M.message),k}c?(c.owned=p,c.condition=h,c.maintenance_per_tick=g):J.push({equipment_key:ee,tier:e.tier,owned:p,deployed:0,condition:h,maintenance_per_tick:g,assigned_projects:[]})}else{const c=(j?.current_tick||0)+i.deliveryTicks,{error:p}=await b.from("corp_equipment_deliveries").insert({faction_id:m.id,equipment_key:ee,quantity:t,condition:i.condition,delivery_tick:c,source_nation_id:i.sourceNationId||null,seller_name:i.seller,price_paid:n});if(p){const{error:f}=await b.from("factions").update({corp_cash_reserves:o}).eq("id",m.id);throw f&&console.error("Cash refund failed:",f.message),p}}m.corp_cash_reserves=a,ri(),gt();const r=document.getElementById("pr-cash");r&&(r.textContent=B(a)),s&&(s.textContent=d?"PURCHASED":"ORDERED",setTimeout(()=>{s.isConnected&&(s.disabled=!1,s.textContent="PURCHASE")},1500))}catch(a){s&&(s.disabled=!1,s.textContent="PURCHASE"),alert("Purchase failed: "+(a.message||"Unknown error"))}finally{zt=!1}}let Gn=-1,Ue=[],ct=[],Jt=[];function qt(i){return Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(1)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i.toLocaleString()}function Wn(i,e,t){if(t)return"var(--orange)";const n=i/(e||1)*100;return n>50?"var(--green)":n>25?"var(--amber)":"var(--red)"}function Ei(){const i=document.getElementById("pm-list"),e=Ue.length,t=ct.length,n=Jt.length,o=Ue.filter(d=>d.expiring_soon).length;document.getElementById("pm-active-count").textContent=`(${e})`,document.getElementById("pm-pending-count").textContent=`(${t})`,document.getElementById("pm-apply-count").textContent=`(${n})`;const s=document.getElementById("pm-badges");let a="";o>0&&(a+=`<span class="pm-badge pm-badge--expiring">${o} EXPIRING</span>`),t>0&&(a+=`<span class="pm-badge pm-badge--pending">${t} PENDING</span>`),s.innerHTML=a;const l=Ue.reduce((d,r)=>d+(r.cost||0),0)+ct.reduce((d,r)=>d+(r.cost||0),0);document.getElementById("pm-total-cost").textContent=qt(l),document.getElementById("pm-footer-active").textContent=e,document.getElementById("pm-footer-pending").textContent=t;{if(e===0){i.innerHTML=`<div class="pm-empty">
            ...
            </div>`;return}let d="";Ue.forEach((r,c)=>{const p=Gn===c,f=Wn(r.ticks_left,r.total_ticks,r.expiring_soon),u=Math.min(r.ticks_left/(r.total_ticks||1)*100,100);d+=`<div class="pm-item ${r.expiring_soon?"pm-item--expiring":""} ${p?"expanded":""}" onclick="togglePmExpand(${c})">
                <div class="pm-item__inner">
                    ...
                    <div class="pm-detail__row">
                        ...
                        <span class="pm-detail__val">${qt(r.cost||0)}</span>
                    </div>
                    <div class="pm-detail__row">
                    ...
                    </div>`,r.note&&(d+=`<div class="pm-note"><span class="pm-note__text">${$(r.note)}</span></div>`),r.expiring_soon&&r.renewable&&(d+=`<div class="pm-btn-row"><button class="pm-btn pm-btn--renew" onclick="event.stopPropagation(); pmApplyForPermit('${r.permit_key}');">RENEW — ${qt(r.cost||0)}</button></div>`),d+="</div>"),d+="</div></div>"}),i.innerHTML=d;return}}let Nt=!1;async function Fn(i){if(!(Nt||!m||!nation)){Nt=!0;try{const{data:e}=await b.from("shard").select("current_tick").eq("name","Alpha Shard").single(),t=e?.current_tick||0,{data:n,error:o}=await b.rpc("apply_for_permit",{p_faction_id:m.id,p_nation_id:nation.id,p_permit_key:i,p_current_tick:t});if(o){alert("Application failed: "+o.message);return}if(n&&!n.success){alert(n.error||"Application failed");return}alert("Permit application submitted! Processing: "+(n.processing_ticks||0)+" ticks."),await Di()}catch(e){alert("Error: "+e.message)}finally{Nt=!1}}}window.pmApplyForPermit=Fn;async function Di(){if(!m||!nation){Ue=[],ct=[],Jt=[],Ei();return}const{data:i}=await b.from("construction_permits").select("*"),e=i||[],t={};for(const p of e)t[p.permit_key]=p;const{data:n}=await b.from("corp_permits").select("*").eq("faction_id",m.id).eq("nation_id",nation.id),o=n||[],{data:s}=await b.from("active_laws").select("policy_id, policies(permit_key, policy_name)").eq("nation_id",nation.id).not("policies.permit_key","is",null),a=new Set,l={};for(const p of s||[])p.policies?.permit_key&&(a.add(p.policies.permit_key),l[p.policies.permit_key]=p.policies.policy_name);const{data:d}=await b.from("shard").select("current_tick").eq("name","Alpha Shard").single(),r=d?.current_tick||0;Ue=o.filter(p=>p.status==="active").map(p=>{const f=t[p.permit_key]||{},u=p.expires_at_tick?Math.max(0,p.expires_at_tick-r):999,y=f.duration_ticks||24;return{name:f.name||p.permit_key,permit_key:p.permit_key,nation:nation.name,policy:l[p.permit_key]||"—",issued:p.granted_at_tick!=null?Ce(p.granted_at_tick):"—",expires:p.expires_at_tick?Ce(p.expires_at_tick):"Single-use",cost:p.cost_paid||0,ticks_left:u,total_ticks:y,expiring_soon:u<=3&&u>0,renewable:f.duration_ticks!=null,projects:[]}}),ct=o.filter(p=>p.status==="pending").map(p=>{const f=t[p.permit_key]||{},u=f.processing_ticks||2,y=r-p.applied_at_tick,g=Math.max(0,u-y);return{name:f.name||p.permit_key,permit_key:p.permit_key,nation:nation.name,applied:Ce(p.applied_at_tick),status:"PROCESSING",processing_total:u,ticks_remaining:g,est_approval:Ce(p.applied_at_tick+u),cost:p.cost_paid||0,required_by:l[p.permit_key]||"—"}});const c=new Set(o.filter(p=>p.status==="active"||p.status==="pending").map(p=>p.permit_key));Jt=[...a].filter(p=>!c.has(p)).map(p=>{const f=t[p]||{};return{name:f.name||p,permit_key:p,nation:nation.name,description:f.description||"",policy:l[p]||"—",cost:f.cost_is_percentage?15e4:f.cost||0,processing_time:f.processing_ticks||2,duration:f.duration_ticks?f.duration_ticks+" ticks":"Single-use",category:f.category||"",difficulty:f.difficulty||"EASY"}}),Ei()}let Se=[],Vn=-1;function ce(i){return Math.abs(i)>=1e6?"$"+(i/1e6).toFixed(2)+"M":Math.abs(i)>=1e3?"$"+(i/1e3).toFixed(0)+"k":"$"+i.toLocaleString()}function Ti(i){return i>=85?"var(--gold)":i>=60?"var(--green)":i>=40?"var(--orange)":"var(--red)"}function Yn(i){return"dl-result--"+i.toLowerCase()}function Ci(){const i=document.getElementById("dl-list"),e=Se.length;document.getElementById("dl-count").textContent=`${e} COMPLETED`;const t=Se.reduce((l,d)=>{const r=d.financials||{};return l+((r.payment||0)+(r.bonus||0)-(r.penalty||0)-(r.total_cost||0))},0),n=document.getElementById("dl-lifetime-profit");n.textContent=(t>=0?"+":"")+ce(t),n.style.color=t>=0?"var(--green)":"var(--red)";const o={};Se.forEach(l=>{o[l.result]=(o[l.result]||0)+1});const s=document.getElementById("dl-footer-results");if(s.innerHTML=Object.entries(o).map(([l,d])=>`<div class="dl-footer__result-box">
        ...
        </div>`;return}let a="";Se.forEach((l,d)=>{const r=Vn===d,c=l.financials||{},p=(c.payment||0)+(c.bonus||0)-(c.penalty||0)-(c.total_cost||0),f=p>=0,u=Yn(l.result),g={DISTINCTION:"var(--gold)",PASS:"var(--green)",CONDITIONAL:"var(--orange)",FAIL:"var(--red)"}[l.result]||"var(--text-dim)",_=l.type==="GOVERNMENT";if(a+=`<div class="dl-item ${r?"expanded":""}" onclick="toggleDlExpand(${d})">
            <div class="dl-item__inner" style="border-left:2px solid ${g}">
                ...
                <div class="dl-item__row2">
                    ...
                    <span class="dl-item__issuer" style="color:${_?"var(--green)":"var(--gold)"}">${$(l.issuer)}</span>
                ...
                <div class="dl-summary-bar">
                    <div class="dl-summary-cell" style="flex:1;">
                        ...
                        <div style="display:flex;align-items:center;gap:4px;margin-top:2px;">
                            <span class="dl-summary-value" style="color:${Ti(l.quality_score)}">${l.quality_score}</span>
                    ...
                    <div class="dl-summary-cell" style="flex:1.3;">
                        ...
                        <div class="dl-summary-value" style="color:${f?"var(--green)":"var(--red)"};margin-top:2px;">${f?"+":""}${ce(p)}</div>
                    </div>
                </div>`,r){const h=l.inspection||{};a+='<div style="margin-top:8px;">',a+='<div class="dl-section-label">Inspection Report</div>',["materials","structural","systems"].forEach(w=>{const x=h[w]||{score:0,issues:[]},T=Ti(x.score),z=Math.min(x.score/100*100,100);a+=`<div class="dl-inspect-row">
                    <div class="dl-inspect-row__header">
                        <span class="dl-inspect-row__label">${$(w.charAt(0).toUpperCase()+w.slice(1))}</span>
                    ...
                    ${(x.issues||[]).map(A=>`<div class="dl-inspect-issue">${$(A)}</div>`).join("")}
                ...
                ${(k.issues||[]).map(w=>`<div class="dl-inspect-issue dl-inspect-issue--red">${$(w)}</div>`).join("")}
                    ...
                    <span class="dl-mat-tag__name">${$(w.name)}</span>
                    <div class="dl-mat-tag__dot" style="background:${x}"></div>
                    <span class="dl-mat-tag__grade" style="color:${x}">${$(w.grade)}</span>
        ...
        </div>`;const o={};for(const l of J)o[l.equipment_key]=l;let s="";for(let l=1;l<=3;l++){const d=Ze[l],r=Gt(l),c=Vt===l,p=r.reduce((u,y)=>u+(o[y.key]?.owned||0),0),f=r.reduce((u,y)=>u+(o[y.key]?.deployed||0),0);if(s+=`<div class="eq-tier-hdr" onclick="toggleEqTier(${l})">
            ...
            ${p>0?`<span class="eq-tier-hdr__count">${f}/${p}</span>`:'<span class="eq-tier-hdr__none">NONE OWNED</span>'}
        </div>`,c)for(const u of r){const y=o[u.key],g=y?.owned||0,_=y?.deployed||0,h=y?.condition||0,k=u.maintenancePerUnit*g,M=g-_,C=g>0&&M===0,w=g>0&&h<65,x=Si(h),T=y?.assigned_projects||[],z=T.length>0?T.map(A=>A.contract_name||"Project").join(", ").slice(0,30):g>0&&_>0?_+" project"+(_>1?"s":""):"—";s+=`<div class="eq-row${g===0?" unowned":""}">`,s+=`<div class="eq-row__top">
                    <div class="eq-row__name-area">
                        <span class="eq-row__name${g===0?" dim":""}">${$(u.name)}</span>
                    ...
                    ${g>0?`<div class="eq-row__right">
                        ...
                        <div class="eq-detail__cell" style="flex:0.8">
                            ...
                            <div class="eq-detail__value" style="color:var(--text-muted)">${$(z)}</div>
                        </div>
        ...
        <div class="eq-footer__tiers">${a}</div>`}function Kn(i){Vt=Vt===i?-1:i,ri()}async function li(){if(!m)return;const{data:i,error:e}=await b.from("corp_equipment").select("equipment_key, tier, owned, deployed, condition, maintenance_per_tick, assigned_projects").eq("faction_id",m.id);e?(console.warn("Failed to load equipment:",e.message),J=[]):J=i||[],ri()}async function Jn(){const{data:{user:i}}=await b.auth.getUser();if(!i){window.location.href="login.html";return}const{data:e}=await b.from("factions").select("*").or(`id.eq.${i.id},linked_user_id.eq.${i.id}`);Je=(e||[]).filter(r=>r.nation_id);const t=sessionStorage.getItem("active_faction_id");if(m=Je.find(r=>r.id===t)||Je.find(r=>r.faction_type==="corporation")||Je[0],!m){await b.auth.signOut(),window.location.href="login.html";return}if(m.faction_type!=="corporation"){window.location.href="dashboard.html";return}const[n,o]=await Promise.all([m.nation_id?b.from("nations").select("*").eq("id",m.nation_id).single():Promise.resolve({data:null}),b.from("shard").select("current_tick, current_date, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single()]);n.error&&console.warn("Nation load failed:",n.error.message),n.data&&(E=n.data),o.error&&console.warn("Shard load failed:",o.error.message),j=o.data;const s=m.corp_ticker||m.abbreviation||"";if(document.getElementById("corp-logo").textContent=s.slice(0,2)||"—",document.getElementById("corp-name-bar").textContent=m.faction_name||"Unnamed Corp",j){if(document.getElementById("game-date").textContent=j.current_date||"—",document.getElementById("tick-number").textContent=j.current_tick||"—",j.next_tick_at){const c=(Number(j.tick_interval_hours)||8)*36e5,p=new Date(j.next_tick_at).getTime(),u=p-c+c/2;Yt=new Date(u>Date.now()?u:p+c/2),fn()}const r=document.querySelector("#tick-countdown")?.closest(".tick-item")?.querySelector(".tick-label");r&&(r.textContent="Next Corp Tick")}document.getElementById("corp-name-badge").textContent=(s?"["+s+"]":m.faction_name||"Corp")+" ▾";const a=document.getElementById("topbar-cash");if(a){const r=Number(m.corp_cash_reserves??0),c=r>=1e9?"$"+(r/1e9).toFixed(1)+"B":r>=1e6?"$"+(r/1e6).toFixed(1)+"M":"$"+Math.round(r/1e3)+"k";a.textContent="CASH: "+c}const l=document.getElementById("topbar-ap");l&&(l.style.display="none"),document.getElementById("nation-pill").textContent=(E?.name||m.nation||"—").toUpperCase();const d=document.getElementById("corp-faction-dropdown");if(d){let r="";for(const c of Je){const p=c.id===m.id,f=c.faction_type==="corporation"?"CORP":"PARTY",u=c.faction_type==="corporation"?"var(--teal)":"var(--amber)";r+=`<div class="corp-dd-item${p?" active":""}" onclick="switchToFaction('${c.id}', '${c.faction_type}')">
            ...
            </div>`}d.innerHTML=r}await Promise.all([ge(),Bi(),oi(),li(),Di(),Qn(),nt()]);try{const{data:r}=await b.from("nations").select("*").order("name");dt=r||[]}catch{dt=[]}if(Ri(),gt(),rn(m,E,j),document.getElementById("loading").style.display="none",document.getElementById("page-content").style.display="block",new URLSearchParams(window.location.search).get("tab")==="expansion"){const r=document.querySelector('[data-tab="expansion"]');r&&Hi({preventDefault:()=>{},target:r})}}async function Xn(){await b.auth.signOut(),window.location.href="login.html"}function Zn(){const i=document.getElementById("corp-faction-dropdown");i&&i.classList.toggle("open")}function eo(i,e){const t=document.getElementById("corp-faction-dropdown");t&&t.classList.remove("open"),sessionStorage.setItem("active_faction_id",i),e==="corporation"?window.location.href="corp-operations.html":window.location.href="dashboard.html"}document.addEventListener("click",i=>{const e=document.getElementById("faction-switcher"),t=document.getElementById("corp-faction-dropdown");t&&e&&!e.contains(i.target)&&t.classList.remove("open")});document.addEventListener("keydown",i=>{i.key==="Escape"&&it()});window.doLogout=Xn;window.toggleTheme=mn;window.toggleCorpDropdown=Zn;window.switchToFaction=eo;window.setFilter=un;window.openContractDetail=Ni;window.closeContractDetail=it;window.toggleWhRow=zn;window.toggleEqTier=Kn;window.switchEmNation=On;window.setEmType=Dn;window.setEmListing=jn;window.setEmQty=Hn;window.purchaseEquipment=Un;window.setPrMat=Nn;window.setPrTier=Ln;window.setPrQty=Bn;window.purchaseMaterial=Rn;let V={general:0,skilled:0,innovative:0},Lt=!1;const Le=[{id:"general",label:"General Workforce",multiplier:2,color:"#8b9a6b",factionKey:"corp_general_workforce"},{id:"skilled",label:"Skilled Workforce",multiplier:3,color:"#c8a832",factionKey:"corp_skilled_workforce"},{id:"innovative",label:"Innovative Workforce",multiplier:6,color:"#c84",factionKey:"corp_innovative_workforce"}];function ji(i){const e=Number(E?.minimum_wage??50),t=Number(E?.inflation??50),n=Number(E?.standard_of_living??50),o=e/100*48e3,s=1+(t-50)/100*.5,a=1+(n-50)/100*.5;return Math.round(o*i*s*a)}function v(i){const e=Math.abs(i),t=i<0?"-":"";return e>=1e9?t+"$"+(e/1e9).toFixed(2)+"B":e>=1e6?t+"$"+(e/1e6).toFixed(2)+"M":e>=1e3?t+"$"+(e/1e3).toFixed(1)+"k":t+"$"+e.toLocaleString()}async function Hi(i){i.preventDefault(),document.getElementById("operations-content").style.display="none";const e=document.getElementById("expansion-content");e.style.display="flex",e.style.justifyContent="center",e.style.gap="12px",e.style.alignItems="flex-start",e.style.flexWrap="wrap",document.querySelectorAll(".corp-nav__tab").forEach(t=>t.classList.remove("active")),i.target.classList.add("active"),await nt(),xt(),oo(),await di(),$t(),await wo(),await go(),ht(),_t(),await Ao(),wt()}function Ui(i){i&&i.preventDefault(),document.getElementById("operations-content").style.display="flex",document.getElementById("expansion-content").style.display="none",document.querySelectorAll(".corp-nav__tab").forEach(e=>e.classList.remove("active")),document.querySelector('[data-tab="operations"]')?.classList.add("active")}function vt(){return G.reduce((e,t)=>{const n=Number(t.capacity||0),o=Number(t.condition||0)/100;return e+Math.floor(n*o)},0)+500}function to(i,e){const t=Le.find(s=>s.id===i),n=Number(m?.[t.factionKey]??0),o=V[i]+e;if(!(n+o<0)){if(e>0){const s=Le.reduce((l,d)=>{const r=Number(m?.[d.factionKey]??0),c=d.id===i?o:V[d.id];return l+r+c},0),a=vt();if(s>a)return}V[i]=o,xt()}}function io(i){i?V[i]=0:V={general:0,skilled:0,innovative:0},xt()}async function no(){if(Lt||!Object.values(V).some(a=>a!==0))return;let e=0;for(const a of Le){const l=V[a.id];l>0&&(e+=l*ji(a.multiplier)*.1)}const t=Number(m?.corp_cash_reserves??0);if(e>t){alert("Insufficient cash reserves. Hiring cost: "+v(e)+", available: "+v(t));return}const n=Le.reduce((a,l)=>a+Number(m?.[l.factionKey]??0)+V[l.id],0),o=vt();if(n>o){alert("Cannot hire beyond property capacity ("+o.toLocaleString()+"). You need more workplaces.");return}const s=e>0?`Confirm workforce changes?

Hiring fee: `+v(e)+" (deducted from cash reserves)":`Confirm workforce changes?

Firing workers — no cost.`;if(confirm(s)){Lt=!0;try{const a={};for(const r of Le){const c=Number(m?.[r.factionKey]??0);a[r.factionKey]=Math.max(0,c+V[r.id])}e>0&&(a.corp_cash_reserves=Math.max(0,t-Math.round(e)));const{error:l}=await b.from("factions").update(a).eq("id",m.id);if(l)throw l;Object.assign(m,a),V={general:0,skilled:0,innovative:0};const d=document.getElementById("topbar-cash");if(d){const r=Number(m.corp_cash_reserves??0);d.textContent="CASH: "+(r>=1e6?"$"+(r/1e6).toFixed(1)+"M":"$"+Math.round(r/1e3)+"k")}xt()}catch(a){alert("Error: "+a.message)}finally{Lt=!1}}}function xt(){const i=document.getElementById("hf-card-container");if(!i)return;const e="'JetBrains Mono', monospace",t={bg:"#121210",surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",greenBright:"#5c5",red:"#c55",redDim:"#a44",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},n=Number(E?.minimum_wage??50),o=Number(E?.inflation??50),s=Number(E?.standard_of_living??50),a=n/100*48e3,l=(1+(o-50)/100*.5).toFixed(2),d=(1+(s-50)/100*.5).toFixed(2),r=E?.name||m?.nation||"Nation",c=Object.values(V).some(k=>k!==0),p=vt();let f=0,u=0,y=0,g=0,_="";for(const k of Le){const M=Number(m?.[k.factionKey]??0),C=V[k.id],w=M+C,x=ji(k.multiplier),T=C>0,z=M*x,A=w*x,P=A-z;f+=M,u+=w,y+=z,g+=A;const L=C!==0?T?"rgba(92,204,92,0.02)":"rgba(204,85,85,0.02)":"transparent";_+=`
        <div style="padding:10px 14px;border-bottom:1px solid ${t.border};background:${L};">
            ...
            ${C!==0?`<div style="margin-top:6px;padding:4px 8px;background:${t.bg};border:1px solid ${t.border};display:flex;justify-content:space-between;align-items:center;">
                ...
                <span style="font-family:${e};font-size:10px;font-weight:700;color:${P>0?t.red:t.greenBright}">${P>0?"+":""}${v(P)}/yr</span>
            </div>`:""}
    ...
    </div>`;for(const y of G){const g=pt[y.style]||pt.Basic;a+=Number(y.purchase_price||0),l+=Number(y.monthly_maintenance||0),s+=`
        <div style="padding:8px 12px;border-bottom:1px solid ${t.border};">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px;">
    ...
    </div>`}let Ge=[],K=null;const pt={Basic:{color:"#6a6660",label:"BASIC"},Modern:{color:"#8b9a6b",label:"MODERN"},Sustainable:{color:"#5c5",label:"SUSTAINABLE"},Innovative:{color:"#c8a832",label:"INNOVATIVE"},Heritage:{color:"#c84",label:"HERITAGE"},Premium:{color:"#ca5",label:"PREMIUM"}};async function di(){if(!m?.nation_id)return;const{data:i,error:e}=await b.from("available_properties").select("*").eq("nation_id",m.nation_id).eq("status","available").order("price",{ascending:!0});if(e){console.warn("[Property] Failed to load marketplace:",e.message);return}Ge=(i||[]).map(t=>({...t,adjusted_cost:t.price,adjusted_maintenance:t.monthly_maintenance}))}function $t(){const i=document.getElementById("new-property-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",redDim:"#a44",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"};(E?.name||m?.nation||"Nation").toUpperCase();const n=Number(E?.standard_of_living??50),o=Number(E?.gdp_growth??50),s=Number(E?.inflation??50),a=E?.capital||"Capital",l={capital:a,port:a+" Port",industrial:a+" Industrial Zone",suburban:a+" Suburbs",coastal:a+" Coast"};let d="";if(Ge.length===0)d=`<div style="padding:20px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">No properties available in this market.<br>Improve GDP Growth and Standard of Living to unlock more.</div>`;else for(let r=0;r<Ge.length;r++){const c=Ge[r],p=K===r,f=pt[c.style]||pt.Basic,u=l[c.city_template]||a;d+=`
    ...
    </div>`}function ao(i){K=K===i?null:i,$t()}let Bt=!1;async function so(){if(K===null||Bt)return;const i=Ge[K];if(!i)return;const e=Number(m?.corp_cash_reserves??0);if(i.adjusted_cost>e){alert(`Insufficient cash reserves.
...
This will be deducted from your cash reserves.`)){Bt=!0;try{const{error:t}=await b.from("corp_properties").insert({faction_id:m.id,nation_id:m.nation_id,catalog_id:i.catalog_id||null,name:i.name,type:i.type,style:i.style,capacity:i.capacity,purchase_price:i.adjusted_cost,monthly_maintenance:i.adjusted_maintenance,condition:i.condition,city:i.city,purchased_at_tick:Number(document.getElementById("tick-number")?.textContent||0),is_active:!0});if(t)throw t;const n=Math.max(0,e-i.adjusted_cost),{error:o}=await b.from("factions").update({corp_cash_reserves:n}).eq("id",m.id);if(o)throw o;m.corp_cash_reserves=n,i.id&&await b.from("available_properties").update({status:"sold",purchased_by:m.id}).eq("id",i.id);const s=document.getElementById("topbar-cash");s&&(s.textContent="CASH: "+(n>=1e6?"$"+(n/1e6).toFixed(1)+"M":"$"+Math.round(n/1e3)+"k")),K=null,await di(),$t(),bt(),alert("Property purchased: "+i.name+`

Deducted: `+v(i.adjusted_cost))}catch(t){alert("Purchase failed: "+t.message)}finally{Bt=!1}}}const Ie={Basic:{costMod:1,maintMod:1,repGain:1,color:"#6a6660",desc:"Functional. No frills."},Modern:{costMod:1.4,maintMod:1.2,repGain:2,color:"#8b9a6b",desc:"Contemporary design. Good retention."},Sustainable:{costMod:1.6,maintMod:.8,repGain:3,color:"#5c5",desc:"Green building. Lower upkeep."},Innovative:{costMod:2,maintMod:1.5,repGain:4,color:"#c8a832",desc:"Cutting edge. Attracts top talent."},Heritage:{costMod:1.3,maintMod:1.4,repGain:2,color:"#c84",desc:"Restored historic. High character."},Premium:{costMod:2.5,maintMod:1.8,repGain:5,color:"#ca5",desc:"Flagship prestige. Maximum reputation."}};let ci=!1,S={name:"",type:"Office Building",size:2e3,style:"Modern",timeline:36,budgetMod:0},Rt=!1;function Gi(){const e=1+(Number(E?.inflation??50)-50)/100*.3,t=Ie[S.style]?.costMod||1,n=S.type==="Warehouse"?.75:1,o=Math.round(S.size*1e5*e*t*n),s=Math.round(o*(1+S.budgetMod/100)),a=Math.round(s*.007*(Ie[S.style]?.maintMod||1));return{baseBudget:o,adjusted:s,maint:a,inflMod:e,styleMod:t}}function ro(){ci=!0,S={name:"",type:"Office Building",size:2e3,style:"Modern",timeline:36,budgetMod:0},Wi()}function pi(){ci=!1,document.getElementById("cp-modal-overlay")?.remove()}function lo(i,e){S[i]=e,Wi()}async function co(){if(!(Rt||!S.name.trim())){Rt=!0;try{const i=Gi(),e=E?.name||m?.nation||"Unknown",t=Ie[S.style]?.repGain||1,n=await b.from("shard").select("current_tick, current_date").eq("name","Alpha Shard").single(),o=n.data?.current_tick||0,s=(n.data?.current_date||"").match(/\d{4}/)?.[0]||"2015",{count:a}=await b.from("construction_contracts").select("id",{count:"exact",head:!0}).eq("nation_id",m.nation_id).eq("issuer_type","PRIVATE"),d=`PVT-C${(a||0)+1}-${s}`,{error:r}=await b.from("construction_contracts").insert({nation_id:m.nation_id,template_key:"custom_building",sector:"civil_engineering",name:S.name.trim(),description:`${S.type} (${S.style}) — ${S.size.toLocaleString()} employees, commissioned by ${m.faction_name}`,project_code:d,budget_ceiling:i.adjusted,timeline_ticks:S.timeline,required_materials:(()=>{const c=S.size/1e3,p=S.style,f={Basic:{concrete:1,steel:1,glass:.5,em:1,lumber:1.5,heavy:1,agg:1},Modern:{concrete:1.1,steel:1.1,glass:1.4,em:1.4,lumber:.5,heavy:1.1,agg:1.1},Sustainable:{concrete:.9,steel:.9,glass:.9,em:.9,lumber:1.5,heavy:.9,agg:.9},Innovative:{concrete:1.2,steel:1.2,glass:2,em:2,lumber:.5,heavy:2,agg:1.2},Heritage:{concrete:1.3,steel:1.3,glass:1.3,em:1.3,lumber:2,heavy:1.3,agg:1.3},Premium:{concrete:1.5,steel:1.5,glass:2.5,em:1.5,lumber:1,heavy:1.5,agg:1.5}}[p]||{concrete:1,steel:1,glass:1,em:1,lumber:1,heavy:1,agg:1},u=(y,g)=>Math.max(1,Math.ceil(c*y*g));return{concrete:u(8,f.concrete),steel:u(6,f.steel),glass_facades:u(3,f.glass),em_systems:u(4,f.em),lumber:u(1,f.lumber),heavy_parts:u(2,f.heavy),aggregate:u(3,f.agg)}})(),required_equipment:(()=>{const c=["work_trucks","concrete_mixers"];return S.size>1e3&&c.push("excavators","tower_cranes"),S.size>3e3&&c.push("bulldozers","heavy_haulers"),S.size>8e3&&c.push("pile_drivers"),c})(),required_workforce:{general:Math.ceil(S.size*.08),skilled:Math.ceil(S.size*.03)},status:"open",generated_at_tick:o,bidding_ends_tick:o+3,issuer_type:"PRIVATE",issuer_name:m.faction_name,issuer_faction_id:m.id});if(r)throw r;pi(),alert(`Construction project submitted!
...
All construction corporations in `+e+" can now bid on this project.")}catch(i){alert("Failed to submit project: "+i.message)}finally{Rt=!1}}}function Wi(){if(document.getElementById("cp-modal-overlay")?.remove(),!ci)return;const i="'JetBrains Mono', monospace",e={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",redDim:"#a44",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},t=Gi(),n=E?.name||m?.nation||"Nation",o=Ie[S.style]?.repGain||1,s=o>=4?e.gold:o>=3?e.greenBright:o>=2?e.accent:e.dim,a=Object.entries(Ie).map(([r,c])=>{const p=S.style===r;return`<div onclick="cpSetField('style','${r}')" style="padding:5px 6px;cursor:pointer;text-align:center;background:${p?c.color+"18":"transparent"};border:1px solid ${p?c.color+"44":e.border};">
    ...
    </div>`,document.body.appendChild(l);const d=document.getElementById("cp-name-input");d&&d.addEventListener("input",r=>{S.name=r.target.value}),l.addEventListener("click",r=>{r.target===l&&pi()})}function po(){const i=document.getElementById("cp-name-input");if(i&&(S.name=i.value),!S.name.trim()){alert("Please enter a building name.");return}co()}window.cpClose=pi;window.cpSetField=lo;window.cpSubmitFromModal=po;window.npSelect=ao;window.npBuyProperty=so;window.npOpenConstructionModal=ro;let Ve=!1;async function fo(i){if(Ve)return;const e=G.find(l=>l.id===i);if(!e)return;const t=1+(Number(E?.inflation??50)-50)/100*.3,n=Math.round((e.purchase_price||0)*.1*t),o=Number(m?.corp_cash_reserves??0);if(n>o){alert("Insufficient cash. Refurbishment costs "+v(n)+" (inflation-adjusted), you have "+v(o));return}if(e.condition>=95){alert("Property is already in excellent condition ("+e.condition+"%).");return}const s=5+Math.floor(Math.random()*21),a=Math.min(100,e.condition+s);if(confirm('Refurbish "'+e.name+`"?

Cost: `+v(n)+`
Expected improvement: +`+s+"% condition ("+e.condition+"% → "+a+"%)")){Ve=!0;try{await b.from("corp_properties").update({condition:a}).eq("id",i);const l=Math.max(0,o-n);await b.from("factions").update({corp_cash_reserves:l}).eq("id",m.id),m.corp_cash_reserves=l;const d=document.getElementById("topbar-cash");d&&(d.textContent="CASH: "+(l>=1e6?"$"+(l/1e6).toFixed(1)+"M":"$"+Math.round(l/1e3)+"k")),await nt(),bt(),alert("Refurbished! Condition: "+e.condition+"% → "+a+"%")}catch(l){alert("Refurbishment failed: "+l.message)}finally{Ve=!1}}}async function mo(i){if(Ve)return;const e=G.find(s=>s.id===i);if(!e)return;const t=1+(Number(E?.inflation??50)-50)/100*.3,n=(e.condition||50)/100,o=Math.round((e.purchase_price||0)*.6*n*t);if(confirm('Sell "'+e.name+`"?

Sale value: `+v(o)+" (60% × "+e.condition+`% condition × inflation)

The property will go back on the market for 6 ticks.
This cannot be undone.`)){Ve=!0;try{await b.from("corp_properties").update({is_active:!1}).eq("id",i);const a=Number(m?.corp_cash_reserves??0)+o;await b.from("factions").update({corp_cash_reserves:a}).eq("id",m.id),m.corp_cash_reserves=a;const d=(await b.from("shard").select("current_tick").eq("name","Alpha Shard").single()).data?.current_tick||0;await b.from("available_properties").insert({nation_id:m.nation_id,catalog_id:e.catalog_id||null,name:e.name,type:e.type,style:e.style,capacity:e.capacity,price:Math.round(o*1.1),monthly_maintenance:e.monthly_maintenance,condition:e.condition,city:e.city,generated_at_tick:d,expires_at_tick:d+6,status:"available"});const r=document.getElementById("topbar-cash");r&&(r.textContent="CASH: "+(a>=1e6?"$"+(a/1e6).toFixed(1)+"M":"$"+Math.round(a/1e3)+"k")),await nt(),bt(),await di(),$t(),alert('Sold "'+e.name+'" for '+v(o))}catch(s){alert("Sale failed: "+s.message)}finally{Ve=!1}}}window.propRefurbish=fo;window.propSell=mo;const Ee={SALE:.8,DISSOLVE:.6,REVENUE_BASE:.02,GDP_NEUTRAL:30,DEFAULT_REPUTATION:25};function uo(i){if(!i)return 0;const e=i.trim().replace(/[$,]/g,""),t=e.match(/^([\d.]+)\s*[Mm]$/),n=e.match(/^([\d.]+)\s*[Kk]$/);return Math.round(t?parseFloat(t[1])*1e6:n?parseFloat(n[1])*1e3:parseFloat(e))}function Qe(i){const e=document.getElementById("topbar-cash");e&&(e.textContent="CASH: "+(i>=1e6?"$"+(i/1e6).toFixed(1)+"M":"$"+Math.round(i/1e3)+"k"))}function yo(i){return Ke.find(e=>e.id===i)?.name||"—"}function fi(i){return G.filter(e=>e.nation_id===i)}async function mi(){We=0,await nt(),bt(),_t(),ht()}let le=!1,We=0,rt={};async function go(){if(m?.id)try{const{data:i}=await b.from("construction_contracts").select("nation_id").eq("awarded_to_faction",m.id).in("status",["in_progress","awarded"]);rt={};for(const e of i||[])e.nation_id&&(rt[e.nation_id]=(rt[e.nation_id]||0)+1)}catch{}}function Fi(i){const e=fi(i.nation_id),t=e.reduce((y,g)=>y+Number(g.purchase_price||0),0),n=e.reduce((y,g)=>y+Number(g.capacity||0),0),o=rt[i.nation_id]||0,s=Ke.find(y=>y.id===i.nation_id),a=(i.name||"").trim().split(/\s+/),l=a.length>=2?a.map(y=>y[0]).join("").toUpperCase().slice(0,4):(i.name||"SUB").slice(0,4).toUpperCase(),d=Number(i.sub_cash||0),r=Number(s?.gdp_growth??50),c=d*Ee.REVENUE_BASE,p=(r-Ee.GDP_NEUTRAL)/100,f=Ee.DEFAULT_REPUTATION/100,u=d>0?Math.round(c*(1+p)*f):0;return{id:i.id,name:i.name,abbr:l,nation:s?.name||i.city||"—",nationId:i.nation_id,sector:m?.corp_sector||"General",subsector:i.subsector||m?.corp_subsector||"—",revenue:u,debt:0,cash:d,reputation:Ee.DEFAULT_REPUTATION,valuation:t,workforce:n,projects:o,established:i.created_at?new Date(i.created_at).getFullYear().toString():"—",trend:r>=40&&d>0?"up":r>=Ee.GDP_NEUTRAL&&d>0?"flat":"down",profitable:u>0}}function _t(){const i=document.getElementById("manage-subsidiaries-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",greenDark:"#2a5a3a",red:"#c55",redDim:"#a44",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},o=G.filter(c=>c.type==="regional_hq").map(Fi);We>=o.length&&(We=0);const s=o[We]||null;let a="";o.length===0&&(a=`<div style="padding:30px 14px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">No subsidiaries established.<br>Use Create Subsidiary to expand.</div>`);let l=0,d=0;for(let c=0;c<o.length;c++){const p=o[c],f=c===We;l+=p.revenue,d+=p.valuation;const u=p.trend==="up"?t.greenBright:p.trend==="down"?t.red:t.dim,y=p.trend==="up"?"▲":p.trend==="down"?"▼":"–";a+=`
    ...
    </div>`}async function Vi(i,e){if(le)return;const t=G.find(u=>u.id===i);if(!t)return;const n=e==="sell",o=n?Ee.SALE:Ee.DISSOLVE,s=n?"SELL":"DISSOLVE",a=n?"sold":"dissolved",l=n?"80%":"60%",d=yo(t.nation_id),r=fi(t.nation_id),c=r.reduce((u,y)=>u+Math.round((y.purchase_price||0)*o*(y.condition||50)/100),0),p=Number(t.sub_cash||0),f=c+p;if(confirm(s+' subsidiary "'+t.name+`"?
...
This cannot be undone.`)){le=!0;try{const u=r.map(g=>g.id);if(u.length===1){const{error:g}=await b.from("corp_properties").update({is_active:!1}).eq("id",u[0]);if(g)throw g}else if(u.length>1){const{error:g}=await b.from("corp_properties").update({is_active:!1}).in("id",u);if(g)throw g}await b.from("corp_properties").update({sub_cash:0}).eq("id",i).then(()=>{}).catch(()=>{});const y=Number(m?.corp_cash_reserves??0)+f;await b.from("factions").update({corp_cash_reserves:y}).eq("id",m.id),m.corp_cash_reserves=y,Qe(y),await mi(),alert("Subsidiary "+a+". "+r.length+` properties liquidated.
Total received: `+v(f))}catch(u){alert("Failed: "+u.message)}finally{le=!1}}}function vo(i){Vi(i,"sell")}function xo(i){Vi(i,"dissolve")}async function Yi(i,e){if(le)return;const t=G.find(p=>p.id===i);if(!t)return;const n=Number(m?.corp_cash_reserves??0),o=Number(t.sub_cash||0),s=e?"WITHDRAW":"INJECT CAPITAL";if(e&&o<=0){alert("This subsidiary has no cash to withdraw.");return}const a=e?o:n,l=prompt(s+(e?" from ":" into ")+t.name+`

Parent cash: `+v(n)+`
Subsidiary cash: `+v(o)+`

Enter amount (e.g., 5000000 or 5M):`);if(!l)return;const d=uo(l);if(!d||d<=0||isNaN(d)){alert("Invalid amount.");return}if(d>a){alert("Insufficient "+(e?"subsidiary":"parent")+" cash. Available: "+v(a));return}const r=e?n+d:n-d,c=e?o-d:o+d;if(confirm(s+" "+v(d)+(e?" from ":" into ")+t.name+`?

Parent: `+v(n)+" → "+v(r)+`
Subsidiary: `+v(o)+" → "+v(c))){le=!0;try{await Promise.all([b.from("factions").update({corp_cash_reserves:r}).eq("id",m.id),b.from("corp_properties").update({sub_cash:c}).eq("id",i)]),m.corp_cash_reserves=r,t.sub_cash=c,Qe(r),_t(),alert((e?"Withdrew ":"Injected ")+v(d)+(e?" from ":" into ")+t.name+".")}catch(p){alert("Failed: "+p.message)}finally{le=!1}}}function bo(i){Yi(i,!1)}function $o(i){Yi(i,!0)}async function _o(i){if(le)return;const e=G.find(g=>g.id===i);if(!e)return;const t=Fi(e);t.nation;const n=fi(e.nation_id),o=t.valuation,s=t.cash,a=t.reputation,l=t.subsector,d=Math.round(o*2.25),r=Math.round(a*.1),c=Math.round(a*.2),p=vt(),f=Le.reduce((g,_)=>g+Number(m?.[_.factionKey]??0),0),u=Math.max(0,p-f),y=Number(m?.corp_cash_reserves??0);if(d>y){alert(`Insufficient cash to acquire subsidiary.
...
This cannot be undone.`)){le=!0;try{const g=m.nation_id;if(n.length>0){const T=n.filter(A=>A.id!==e.id).map(A=>A.id);if(T.length===1){const{error:A}=await b.from("corp_properties").update({nation_id:g,type:"office"}).eq("id",T[0]);if(A)throw A}else if(T.length>1){const{error:A}=await b.from("corp_properties").update({nation_id:g,type:"office"}).in("id",T);if(A)throw A}const{error:z}=await b.from("corp_properties").update({nation_id:g,type:"office",sub_cash:0,subsector:null}).eq("id",e.id);if(z)throw z}const _=y-d+s,k=Number(m?.corp_general_workforce??0)+u,M=Math.random()>=.5?r:-c,C=Number(m?.standing??50),w=Math.max(0,Math.min(100,C+M)),{error:x}=await b.from("factions").update({corp_cash_reserves:_,corp_general_workforce:k,standing:w}).eq("id",m.id);if(x)throw x;m.corp_cash_reserves=_,m.corp_general_workforce=k,m.standing=w,Qe(_),await mi(),alert(`Merger complete!
...
Properties: `+n.length+" transferred to parent")}catch(g){alert("Merge failed: "+g.message)}finally{le=!1}}}window.subDissolve=xo;window.subInjectCapital=bo;window.subWithdraw=$o;window.subMerge=_o;window.subSell=vo;window.selectSubsidiary=function(i){We=i,_t()};let Ke=[],Xe={},ie=null,Pt=!1,Re="",et="",Pe="";const ho={Construction:[{id:"civil",name:"Civil Engineering",mod:0},{id:"industrial",name:"Industrial Construction",mod:.25},{id:"mega",name:"Megaprojects",mod:.4}],Finance:[{id:"banking",name:"Banking",mod:0},{id:"insurance",name:"Insurance",mod:.15},{id:"investment",name:"Investment Management",mod:.3}],Technology:[{id:"software",name:"Software Development",mod:0},{id:"hardware",name:"Hardware Manufacturing",mod:.2},{id:"telecom",name:"Telecommunications",mod:.35}],Energy:[{id:"oil_gas",name:"Oil & Gas",mod:0},{id:"renewables",name:"Renewables",mod:.2},{id:"mining",name:"Mining",mod:.3}],Healthcare:[{id:"pharma",name:"Pharmaceuticals",mod:0},{id:"hospitals",name:"Hospital Systems",mod:.2},{id:"biotech",name:"Biotechnology",mod:.35}]};async function wo(){const{data:i,error:e}=await b.from("nations").select("*").order("name");e&&console.warn("[Subsidiary] Failed to load nations:",e.message),Ke=(i||[]).filter(n=>n.id!==m?.nation_id);const{data:t}=await b.from("factions").select("nation_id").eq("faction_type","corporation").is("abandoned_at",null);Xe={};for(const n of t||[])n.nation_id&&(Xe[n.nation_id]=(Xe[n.nation_id]||0)+1);Pe=m?.corp_subsector||""}function Qi(){const i=m?.corp_sector||"";return ho[i]||[{id:"general",name:i||"General",mod:0}]}function ko(){const e=Qi().find(t=>t.name===Pe);return e?e.mod:0}function Xt(i){const e=Number(i.standard_of_living??50);return Math.max(.5,Math.round(e/50*100)/100)}function Ki(i){const t=1+ko(),n=Xt(i);return Math.round(Math.max(1e7,5e7*t*n))}function Eo(i){const e=Xe[i]||0;return e<=1?{label:"HIGH",color:"#5c5"}:e<=3?{label:"MODERATE",color:"#ca5"}:{label:"LOW",color:"#c55"}}function To(i){if(ie=ie===i?null:i,ie){const e=Ke.find(t=>t.id===ie);Re=(m?.faction_name||"Subsidiary")+" "+(e?.name||"")}else Re="";ht()}function Co(i){Pe=i,ht()}function So(i){Re=i}function Io(i){et=i.toUpperCase().slice(0,4)}async function Mo(){if(Pt||!ie)return;const i=Ke.find(a=>a.id===ie);if(!i)return;const e=(Re||"").trim(),t=(et||"").trim();if(!e){alert("Please enter a corporation name for the subsidiary.");return}if(t.length<2){alert("Please enter an abbreviation (2-4 chars).");return}if(G.find(a=>a.nation_id===i.id&&a.type==="regional_hq")){alert("You already have a subsidiary in "+i.name);return}const o=Ki(i),s=Number(m?.corp_cash_reserves??0);if(o>s){alert("Insufficient cash. Entry cost: "+v(o)+", available: "+v(s));return}if(confirm("Establish subsidiary in "+i.name+`?
...
Deducted from cash reserves.`)){Pt=!0;try{const l=(await b.from("shard").select("current_tick").eq("name","Alpha Shard").single()).data?.current_tick||0,d=85+Math.floor(Math.random()*16),r=Math.round(o*.005),{error:c}=await b.from("corp_properties").insert({faction_id:m.id,nation_id:i.id,name:e,type:"regional_hq",style:"Modern",capacity:500,purchase_price:o,monthly_maintenance:r,condition:d,city:i.capital||i.name,purchased_at_tick:l,is_active:!0,subsector:Pe||m?.corp_subsector||null});if(c)throw c;const p=Math.max(0,s-o);await b.from("factions").update({corp_cash_reserves:p}).eq("id",m.id),m.corp_cash_reserves=p,Qe(p),ie=null,Re="",et="",await mi(),alert('Subsidiary "'+e+'" established in '+i.name+`!

Cost: `+v(o)+`
Regional HQ created with `+d+"% condition.")}catch(a){alert("Failed: "+a.message)}finally{Pt=!1}}}function ht(){const i=document.getElementById("create-subsidiary-container");if(!i)return;const e="'JetBrains Mono', monospace",t={bg:"#121210",surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",accentBright:"#a3b07e",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},n=m?.corp_sector||"General",o=m?.corp_subsector||"",s=Qi(),a=s.find(x=>x.name===Pe)||s[0],l=new Set(G.filter(x=>x.type==="regional_hq").map(x=>x.nation_id)),d=Ke.filter(x=>!l.has(x.id)),r=ie?d.find(x=>x.id===ie):null,c=Re.trim().length>0&&et.trim().length>=2&&r!==null;let p=`
    ...
    <div style="padding:6px 14px;border-bottom:1px solid ${t.border};">
        ...
        <div style="display:flex;gap:3px;">
            ${s.map(x=>{const T=x.name===Pe,z=x.name===o;return`<div onclick="subSetSubsector('${x.name.replace(/'/g,"\\'")}')" style="flex:1;padding:5px 4px;text-align:center;cursor:pointer;background:${T?t.accent+"18":"transparent"};border:1px solid ${T?t.accent+"44":t.border};">
    ...
    </div>`,u="";if(d.length===0)u=`<div style="padding:20px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">Subsidiaries in all available nations.</div>`;else for(const x of d){const T=x.id===ie,z=Eo(x.id),A=Xe[x.id]||0,P=Math.round(Number(x.standard_of_living??50)),L=Xt(x);u+=`
            <div onclick="subSelectNation('${x.id}')" style="display:flex;align-items:center;padding:4px 8px;margin-bottom:2px;cursor:pointer;background:${T?"rgba(139,154,107,0.04)":"transparent"};border:1px solid ${T?t.accent+"44":t.border};border-left:${T?"2px solid "+t.accent:"2px solid transparent"};">
                <div style="flex:1;">
                    ...
                    <div style="display:flex;gap:8px;margin-top:2px;">
                        <span style="font-family:${e};font-size:7px;color:${t.dim}">STD/LIVING: <span style="color:${t.muted}">${P}</span></span>
    ...
    </div>`;const h=5e7,k=a.mod,M=r?Xt(r):null,C=r?Ki(r):null;let w=`
    ...
    </div>`}window.subSelectNation=To;window.subCreate=Mo;window.subSetName=So;window.subSetAbbr=Io;window.subSetSubsector=Co;let lt=[],Me=0,oe="ALL",ke="REPUTATION";async function Ao(){const{data:i}=await b.from("factions").select("id, faction_name, abbreviation, corp_sector, corp_subsector, corp_company_type, corp_ticker, corp_cash_reserves, nation_id, nation, linked_user_id").eq("faction_type","corporation").is("abandoned_at",null).order("faction_name");lt=(i||[]).map(e=>({...e,abbr:e.corp_ticker||e.abbreviation||e.faction_name?.slice(0,4).toUpperCase()||"???",status:(e.corp_company_type||"Private").toUpperCase(),isPlayer:!!e.linked_user_id,reputation:50,revenue:e.status==="PUBLIC"?Number(e.corp_cash_reserves||0)*.1:null,valuation:e.status==="PUBLIC"?Number(e.corp_cash_reserves||0)*3:null}))}function zo(i){Me=i,wt()}function qo(i){oe=i,Me=0,wt()}function No(i){ke=i,Me=0,wt()}function wt(){const i=document.getElementById("corporations-container");if(!i)return;const e="'JetBrains Mono', monospace",t={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",blue:"#5a8aaa",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},n={PUBLIC:{color:t.greenBright,bg:"rgba(92,204,92,0.06)",border:"rgba(92,204,92,0.15)"},PRIVATE:{color:t.gold,bg:"rgba(200,168,50,0.08)",border:"rgba(200,168,50,0.2)"},STATE:{color:t.orange,bg:"rgba(204,136,68,0.06)",border:"rgba(204,136,68,0.15)"}},o=[...new Set(lt.map(f=>f.nation).filter(Boolean))];let s=[...lt];oe!=="ALL"&&(s=s.filter(f=>f.nation===oe)),ke==="REPUTATION"?s.sort((f,u)=>(u.reputation||0)-(f.reputation||0)):ke==="REVENUE"?s.sort((f,u)=>(u.revenue||0)-(f.revenue||0)):ke==="VALUATION"&&s.sort((f,u)=>(u.valuation||0)-(f.valuation||0)),Me>=s.length&&(Me=0);const a=s[Me]||null,l=a&&a.status==="PRIVATE",d=a&&a.status==="STATE";let r="";s.length===0&&(r=`<div style="padding:30px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">No corporations found.</div>`);for(let f=0;f<s.length;f++){const u=s[f],y=f===Me,g=n[u.status]||n.PRIVATE,_=u.status==="PRIVATE";r+=`
    ...
    <div style="padding:5px 14px;border-bottom:1px solid ${t.border};background:${t.card};display:flex;gap:12px;align-items:center;flex-shrink:0;">
        <div style="display:flex;gap:3px;align-items:center;">
            ...
            ${o.map(f=>`<span onclick="corpFilterNation('${f}')" style="padding:2px 7px;font-family:${e};font-size:7px;font-weight:700;cursor:pointer;color:${oe===f?"#000":t.dim};background:${oe===f?t.accent:"transparent"};border:1px solid ${oe===f?t.accent:t.border}">${f}</span>`).join("")}
        </div>
        <div style="flex:1"></div>
        <div style="display:flex;gap:3px;align-items:center;">
    ...
    </div>`}window.corpSelect=zo;window.corpFilterNation=qo;window.corpSort=No;let re=null,me={},D=120,ue=15,Zt={},Fe=[];async function Lo(){if(!Ne)return;if(Ye[Ne.id]){alert("You already have a bid on this contract. Retract it first if you want to re-bid.");return}re=Ne,Zt={};try{const{data:t}=await b.from("corp_material_inventory").select("material_key, quantity").eq("faction_id",m.id);for(const n of t||[])Zt[n.material_key]=Number(n.quantity||0)}catch{}Fe=[];try{const{data:t}=await b.from("contract_bids").select("faction_id, bid_price, estimated_quality, status, factions(faction_name, corp_ticker)").eq("contract_id",re.id).in("status",["pending","won"]);Fe=(t||[]).filter(n=>n.faction_id!==m?.id).map(n=>({name:n.factions?.faction_name||"Unknown",ticker:n.factions?.corp_ticker||"???",price:Number(n.bid_price||0),quality:Number(n.estimated_quality||0),status:n.status}))}catch{}me={};const i=re.required_materials||{};for(const t of Object.keys(i))me[t]="STD";const e=re.required_workforce||{};D=Number(e.general||0)+Number(e.skilled||0)||120,ue=15,it(),kt()}function ui(){document.getElementById("bid-assembly-overlay")?.remove(),re=null}function Bo(i,e){me[i]=e,kt()}function Ro(i){D=i,kt()}function Po(i){ue=i,kt()}function kt(){if(document.getElementById("bid-assembly-overlay")?.remove(),!re)return;const i="'JetBrains Mono', monospace",e={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",accentBright:"#a3b07e",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",redDim:"#a44",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},t=re,n=t.issuer_type==="GOVERNMENT",o=E?.name||m?.nation||"—",s=Number(t.budget_ceiling||0),a=Number(t.timeline_ticks||8),l=t.required_materials||{},d=Object.keys(l),r={LOW:.5,STD:1,HIGH:2},c={LOW:e.orange,STD:e.yellow,HIGH:e.greenBright},p={LOW:"Low",STD:"Standard",HIGH:"High"},f={concrete:36e4,steel:5e5,glass_facades:56e4,em_systems:64e4,lumber:24e4,heavy_parts:8e5,aggregate:16e4,asphalt:28e4},u=Zt||{};let y=0,g="";for(const q of d){const O=Number(l[q]||0),gi=me[q]||"STD",vi=f[q]||3e5,en=r[gi],tn=Math.round(vi*en),xi=O*tn;y+=xi;const nn=q.replace(/_/g," ").replace(/\b\w/g,_e=>_e.toUpperCase()),bi=Number(u[q]||0),St=Math.max(0,O-bi),on=St===0?e.greenBright:St<O?e.yellow:e.red,an=St===0?"✓ IN STOCK":`${bi}/${O}`;g+=`
        <div style="display:flex;align-items:center;padding:5px 14px;border-bottom:1px solid ${e.border};">
            <div style="flex:1.2">
                <span style="font-size:11px;color:${e.text}">${nn}</span>
                <div style="font-family:${i};font-size:7px;color:${on};margin-top:1px">${an}</div>
            </div>
            ...
            <div style="flex:1.2;display:flex;gap:2px;justify-content:center;">
                ${["LOW","STD","HIGH"].map(_e=>{const It=gi===_e,$i=c[_e],sn=v(Math.round(vi*r[_e]));return`<span onclick="bidSetGrade('${q}','${_e}')" style="padding:2px 6px;font-family:${i};font-size:7px;font-weight:700;cursor:pointer;color:${It?"#000":e.dim};background:${It?$i:"transparent"};border:1px solid ${It?$i:e.border}" title="${sn}/unit">${p[_e]}</span>`}).join("")}
            </div>
            <div style="flex:0.8;text-align:right"><span style="font-family:${i};font-size:10px;color:${e.text}">${v(xi)}</span></div>
        </div>`}const _=t.required_workforce||{},h=Number(_.general||0)+Number(_.skilled||0)||100,k=Math.max(40,Math.round(h*.5)),M=h*2,C=[k,Math.round(h*.75),h,Math.round(h*1.5),M],w=Math.max(0,Math.min(1,(D-k)/(M-k||1))),x=a,T=Math.round(4.5-w*8),z=Math.max(Math.round(x*.6),x+T),A=T>0?`+${T}mo`:T<0?`${T}mo`:"On schedule",P=T>0?e.red:T<0?e.greenBright:e.yellow,L=15200,W=D*L*z,X=s,N=[{name:"Municipal Zoning Approval",cost:18e4,ticks:2,required:!0},{name:"Structural Engineering Cert.",cost:24e4,ticks:3,required:!0},{name:"Environmental Impact Assessment",cost:34e4,ticks:8,required:X>2e7},{name:"Seismic Resilience Compliance",cost:21e4,ticks:4,required:X>5e7},{name:"Heritage Conservation Review",cost:16e4,ticks:6,required:!1},{name:"Fire Safety Certification",cost:12e4,ticks:2,required:X>1e7}].filter(q=>q.required),Y=new Set(["Municipal Zoning Approval","Structural Engineering Cert."]),De=N.filter(q=>!Y.has(q.name)).reduce((q,O)=>q+O.cost,0),Z=4e5,xe=y+W+De+Z,de=Math.round(xe*(ue/100)),ot=xe+de,Q=ot>s,Tt=de,be=Q?0:Math.max(0,Math.min(100,Math.round(100-ot/s*100+30))),yi=be>70?e.greenBright:be>40?e.yellow:be>0?e.orange:e.red,Xi=Q?"OVER CEILING":be>70?"STRONG":be>40?"COMPETITIVE":be>20?"WEAK":"UNLIKELY",Ct=Object.values(me),$e=Ct.length>0?Math.round(Ct.reduce((q,O)=>q+(O==="HIGH"?85:O==="STD"?65:45),0)/Ct.length):50,at=$e>=75?e.greenBright:$e>=55?e.yellow:e.orange,Zi=$e>=75?"STRONG":$e>=55?"PROMISING":"UNCERTAIN",je=document.createElement("div");je.id="bid-assembly-overlay",je.style.cssText="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.7);display:flex;align-items:center;justify-content:center;z-index:9999;",je.addEventListener("click",q=>{q.target===je&&ui()}),je.innerHTML=`
    <div style="width:740px;max-height:94vh;background:${e.surface};border:1px solid ${e.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
        ...
        <div style="flex:1;display:flex;overflow:hidden;">
            ...
            <div style="flex:1;border-right:1px solid ${e.border};overflow:auto;">
                ...
                <div style="padding:8px 14px;border-bottom:1px solid ${e.border};">
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
                        ...
                        <div style="display:flex;gap:3px;">
                            ${C.map(q=>`<span onclick="bidSetWorkers(${q})" style="padding:2px 8px;font-family:${i};font-size:8px;font-weight:700;cursor:pointer;color:${D===q?"#000":e.dim};background:${D===q?e.accent:"transparent"};border:1px solid ${D===q?e.accent:e.border}">${q}</span>`).join("")}
                        </div>
                    </div>
                    <div style="display:flex;justify-content:space-between;">
                    ...
                    <div style="display:flex;justify-content:space-between;margin-top:6px;padding-top:6px;border-top:1px solid ${e.border};">
                        ...
                        <span style="font-family:${i};font-size:10px;font-weight:700;color:${P}">${z}mo <span style="font-size:8px;opacity:0.7">(${A})</span></span>
                    </div>
                </div>
                ...
                ${N.map(q=>{const O=Y.has(q.name);return`<div style="display:flex;align-items:center;padding:4px 14px;border-bottom:1px solid ${e.border};">
                        <div style="flex:1;display:flex;align-items:center;gap:6px;">
                            <span style="font-family:${i};font-size:8px;font-weight:700;color:${O?e.greenBright:e.orange}">${O?"✓":"○"}</span>
                            <span style="font-size:10px;color:${O?e.muted:e.text}">${q.name}</span>
                        </div>
                        ${O?`<span style="font-family:${i};font-size:8px;color:${e.greenBright}">HELD</span>`:`<div style="text-align:right">
                                <span style="font-family:${i};font-size:9px;color:${e.redDim}">${v(q.cost)}</span>
                                <span style="font-family:${i};font-size:7px;color:${e.dim};margin-left:4px">${q.ticks}t</span>
                            </div>`}
                    </div>`}).join("")}
                <div style="display:flex;justify-content:space-between;padding:5px 14px;border-bottom:1px solid ${e.border};background:rgba(139,154,107,0.02);">
            ...
            <div style="width:280px;display:flex;flex-direction:column;overflow:auto;">
                ...
                ${[{l:"Materials",v:y},{l:"Labor",v:W},{l:"Permits",v:De},{l:"Overhead",v:Z}].map(q=>`<div style="display:flex;justify-content:space-between;padding:4px 14px;border-bottom:1px solid ${e.border};">
                    <span style="font-size:10px;color:${e.muted}">${q.l}</span>
                    <span style="font-family:${i};font-size:10px;color:${e.redDim}">${v(q.v)}</span>
                </div>`).join("")}
                ...
                <div style="padding:10px 14px;border-bottom:1px solid ${e.border};background:${Q?"rgba(204,85,85,0.04)":"rgba(200,168,50,0.03)"};">
                    ...
                    <div style="font-family:${i};font-size:22px;font-weight:700;color:${Q?e.red:e.gold}">${v(ot)}</div>
                ...
                <div style="padding:8px 14px;border-bottom:1px solid ${e.border};">
                    <div style="display:flex;justify-content:space-between;">
                        ...
                        <span style="font-family:${i};font-size:14px;font-weight:700;color:${Tt>0?e.greenBright:e.dim}">+${v(Tt)}</span>
                    </div>
                </div>
                ...
                <div style="padding:8px 14px;border-bottom:1px solid ${e.border};">
                    <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
                        ...
                        <span style="font-family:${i};font-size:11px;font-weight:700;color:${yi}">${Xi}</span>
                    </div>
                    <div style="width:100%;height:4px;background:${e.border}"><div style="width:${be}%;height:100%;background:${yi}"></div></div>
                </div>
                <div style="padding:8px 14px;border-bottom:1px solid ${e.border};">
                    <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
                        ...
                        <div style="display:flex;align-items:baseline;gap:4px;">
                            ...
                            <span style="font-family:${i};font-size:8px;font-weight:700;color:${at}">${Zi}</span>
                        </div>
                    </div>
                ...
                <div style="padding:6px 14px;border-bottom:1px solid ${e.border};">
                    ...
                    ${Fe.length===0?`<div style="font-family:${i};font-size:8px;color:${e.dim};padding:4px 0;">No competing bids yet. First mover advantage.</div>`:`<div style="display:flex;gap:4px;flex-wrap:wrap;">
                            ${Fe.map(q=>`<span style="padding:2px 6px;font-family:${i};font-size:7px;color:${e.muted};background:${e.card};border:1px solid ${e.border};">${q.name} <span style="color:${e.dim}">Q:${q.quality}</span></span>`).join("")}
                        </div>
        ...
        <div style="padding:10px 16px;border-top:1px solid ${e.border};background:${e.card};display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">
            <div style="display:flex;gap:12px;">
                <div><div style="font-family:${i};font-size:7px;color:${e.dim}">YOUR BID</div><div style="font-family:${i};font-size:14px;font-weight:700;color:${Q?e.red:e.gold}">${v(ot)}</div></div>
                <div><div style="font-family:${i};font-size:7px;color:${e.dim}">EST. PROFIT</div><div style="font-family:${i};font-size:14px;font-weight:700;color:${e.greenBright}">+${v(Tt)}</div></div>
    ...
    </div>`,document.body.appendChild(je)}let Ot=!1;async function Oo(){if(Ot||!re)return;const i=re,e=i.required_materials||{},t=Object.keys(e),n=Number(i.budget_ceiling||0),o=Number(i.timeline_ticks||8),s={concrete:36e4,steel:5e5,glass_facades:56e4,em_systems:64e4,lumber:24e4,heavy_parts:8e5,aggregate:16e4,asphalt:28e4},a={LOW:.5,STD:1,HIGH:2};let l=0;for(const L of t){const W=Number(e[L]||0),X=me[L]||"STD",ve=s[L]||3e5;l+=W*Math.round(ve*a[X])}const d=15200,r=i.required_workforce||{},c=Number(r.general||0)+Number(r.skilled||0)||100,p=Math.max(40,Math.round(c*.5)),f=c*2,u=Math.max(0,Math.min(1,(D-p)/(f-p||1))),y=Math.round(4.5-u*8),g=Math.max(Math.round(o*.6),o+y),_=D*d*g,h=n,k=[{name:"Municipal Zoning Approval",cost:18e4,required:!0},{name:"Structural Engineering Cert.",cost:24e4,required:!0},{name:"Environmental Impact Assessment",cost:34e4,required:h>2e7},{name:"Seismic Resilience Compliance",cost:21e4,required:h>5e7},{name:"Fire Safety Certification",cost:12e4,required:h>1e7}],M=new Set(["Municipal Zoning Approval","Structural Engineering Cert."]),C=k.filter(L=>L.required&&!M.has(L.name)).reduce((L,W)=>L+W.cost,0),x=l+_+C+4e5,T=Math.round(x*(ue/100)),z=x+T;if(z>n){alert("Bid exceeds budget ceiling. Reduce costs or markup.");return}const A=Object.values(me),P=A.length>0?Math.round(A.reduce((L,W)=>L+(W==="HIGH"?85:W==="STD"?65:45),0)/A.length):50;if(confirm('Submit bid for "'+i.name+`"?
...
Once submitted, your bid cannot be changed.`)){Ot=!0;try{const{data:L}=await b.from("shard").select("current_tick").eq("name","Alpha Shard").single(),W=L?.current_tick||0,X={};for(const N of t)X[N]=me[N]||"STD";const{error:ve}=await b.from("contract_bids").insert({contract_id:i.id,faction_id:m.id,bid_price:z,material_grades:X,labor_count:D,markup_pct:ue,estimated_cost:x,estimated_quality:P,status:"pending",submitted_at_tick:W});if(ve)throw ve;i.status==="open"&&await b.from("construction_contracts").update({status:"bidding"}).eq("id",i.id).eq("status","open"),ui(),alert(`Bid submitted successfully!
...
Lowest qualified bid wins.`),typeof ge=="function"&&await ge()}catch(L){alert("Bid submission failed: "+L.message)}finally{Ot=!1}}}window.openBidAssembly=Lo;window.closeBidAssembly=ui;window.bidSetGrade=Bo;window.bidSetWorkers=Ro;window.bidSetMarkup=Po;window.submitBidAssembly=Oo;let Dt=!1;async function Do(i){if(Dt)return;const e=1e6,t=Number(m?.corp_cash_reserves??0);if(t<e){alert("Cannot retract — you need at least $1M in cash reserves to cover the retraction penalty.");return}if(confirm(`Retract your bid?

This will cost $1M as a retraction penalty.
This action cannot be undone.`)){Dt=!0;try{const n=t-e,{error:o}=await b.from("factions").update({corp_cash_reserves:n}).eq("id",m.id);if(o)throw o;const{error:s}=await b.from("contract_bids").delete().eq("contract_id",i).eq("faction_id",m.id);if(s)throw s;m.corp_cash_reserves=n,typeof Qe=="function"&&Qe(n),alert("Bid retracted. $1M penalty applied."),it(),await ge()}catch(n){alert("Failed to retract bid: "+(n.message||"Unknown error"))}finally{Dt=!1}}}window.retractBid=Do;let tt=[],Ae=0,ne=null,jt=!1,Ht=!1,Ut=!1;async function jo(){if(!Ne||Ht)return;Ht=!0,ne=Ne,Ae=0;const{data:i,error:e}=await b.from("contract_bids").select("*, factions(faction_name, corp_ticker, corp_subsector)").eq("contract_id",ne.id).in("status",["pending","won"]).order("bid_price",{ascending:!0});if(Ht=!1,e){alert("Failed to load bids: "+e.message);return}tt=(i||[]).map(t=>({...t,corp:t.factions?.faction_name||"Unknown",abbr:t.factions?.corp_ticker||"???",subsector:t.factions?.corp_subsector||"—"})),it(),Ji()}function Et(){document.getElementById("bid-review-overlay")?.remove(),ne=null}function Ho(i){Ae=i,Ji()}async function Uo(){if(jt||tt.length===0)return;const i=tt[Ae];if(!(!i?.id||!i.faction_id)&&confirm("Accept bid from "+i.corp+`?

Bid Price: `+v(i.bid_price)+`
Quality: `+i.estimated_quality+`/100
Workers: `+i.labor_count+`

This will award the contract. The project begins immediately.`)){jt=!0;try{const{data:e}=await b.from("shard").select("current_tick").eq("name","Alpha Shard").single(),t=e?.current_tick||0,{error:n}=await b.from("contract_bids").update({status:"won"}).eq("id",i.id);if(n)throw n;const{error:o}=await b.from("contract_bids").update({status:"lost"}).eq("contract_id",ne.id).neq("id",i.id);if(o)throw o;const{error:s}=await b.from("construction_contracts").update({status:"awarded",awarded_to_faction:i.faction_id,awarded_at_tick:t}).eq("id",ne.id);if(s)throw s;Et(),alert("Contract awarded to "+i.corp+`!

Bid: `+v(i.bid_price)+`
Project begins immediately.`),typeof ge=="function"&&await ge()}catch(e){alert("Failed to accept bid: "+(e.message||e))}finally{jt=!1}}}async function Go(){if(!(!ne||Ut)&&confirm(`Decline all bids and cancel this project?

No bids will be accepted. The contract will be removed.`)){Ut=!0;try{const{error:i}=await b.from("contract_bids").update({status:"lost"}).eq("contract_id",ne.id);if(i)throw i;const{error:e}=await b.from("construction_contracts").update({status:"expired"}).eq("id",ne.id);if(e)throw e;Et(),alert("All bids declined. Contract cancelled."),typeof ge=="function"&&await ge()}catch(i){alert("Failed: "+(i.message||i))}finally{Ut=!1}}}function Ji(){if(document.getElementById("bid-review-overlay")?.remove(),!ne||tt.length===0)return;const i="'JetBrains Mono', monospace",e={surface:"#1a1a16",card:"#1c1c18",border:"#2a2a24",accent:"#8b9a6b",accentBright:"#a3b07e",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},t=ne,n=tt;Ae>=n.length&&(Ae=0);const o=n[Ae],s=Number(t.budget_ceiling||0),a=Number(t.timeline_ticks||36),l=Math.min(...n.map(u=>u.bid_price)),d=Math.max(...n.map(u=>u.estimated_quality||0));let r="";for(let u=0;u<n.length;u++){const y=n[u],g=u===Ae,_=y.bid_price===l,h=(y.estimated_quality||0)===d,k=y.bid_price>s;r+=`
    ...
    </div>`,document.body.appendChild(f)}window.openBidReview=jo;window.closeBidReview=Et;window.reviewSelectBid=Ho;window.acceptBid=Uo;window.declineAllBids=Go;window.switchToExpansion=Hi;window.switchToOperations=Ui;window.hfSetChange=to;window.hfReset=io;window.hfConfirm=no;document.querySelector('[data-tab="operations"]')?.addEventListener("click",function(i){this.classList.contains("active")||(i.preventDefault(),Ui(i))});Jn();
