...
import{_supabase as y}from"./supabase-client-CiYoFhIh.js";/* empty css                    *//* empty css                         */import{c as _e,i as Ca,a as Ta,l as Sa,M as jt,Q as Ni,b as Ai,d as vi,e as _n,f as hn,g as za,h as Ia}from"./corp-shipping-data-CcJ84lK3.js";import{_ as Na}from"./preload-helper-BXl3LOEh.js";import{e as x}from"./utils-CY90Gazr.js";import{initMessaging as Aa}from"./messaging-BUrQna7p.js";import{c as Ma,a as yi,E as Ft,b as zo,d as $n,e as Ra,f as qa,h as cn}from"./equipment-DsuDdEne.js";import{a as La,E as uo,b as vo,g as Oa}from"./corp-executives-BOrCkuAI.js";import"./political-actions-BF080n5r.js";import"./config-CRvw5bg0.js";import"./government-types-D9n0pQb0.js";import"./ideology-BqLjustE.js";import"./stats-tIiBSaQA.js";let ke=[],d=null,N=null,I=null,je=[];const gi={};let Et={},J=[],X={},xi=-1;const Ba={em:"em_systems",glass:"glass_facades",heavy:"heavy_parts"},yo=o=>Ba[o]||o;let te="concrete",Y="STD",be=500,vt=null,re=[],go={},bi=0,Ut=[],Ht=[],yt=0,Ee=null,Te=-1,he=[],Gt=null,Rt={},xo={},Mi=[],bo=null,me="trucks",Ce=0,Se=1,Be=[],Ke=null,Ct=[],_i=null,ro=null;function nt(){return vt||N}let hi="ALL",$i="TIMELINE";function D(o){return Math.abs(o)>=1e6?"$"+(o/1e6).toFixed(1)+"M":Math.abs(o)>=1e3?"$"+(o/1e3).toFixed(0)+"k":"$"+o}function Pa(o){if(o>=12){const e=Math.floor(o/12),t=o%12;return t>0?e+"y "+t+"mo":e+"y"}return o+" ticks"}function wn(o){return!o||o.length===0?"":o.map(e=>{const t=go[e];if(!t)return"";const i=t.reputation_bonus>0?"var(--green)":t.reputation_bonus<0?"var(--red)":"var(--text-dim)",n=t.reputation_bonus>0?"+"+t.reputation_bonus:t.reputation_bonus<0?String(t.reputation_bonus):"";return`<span style="display:inline-flex;align-items:center;gap:3px;padding:2px 6px;background: var(--border-hair);border:1px solid var(--border-0);border-radius:3px;font-family:var(--font-mono);font-size:7px;color:var(--text-secondary);">${t.icon||"📍"} ${x(t.name)}${n?` <span style="color:${i};font-weight:700;">${n} REP</span>`:""}</span>`}).filter(Boolean).join(" ")}function ue(o){return Math.abs(o)>=1e9?"$"+(o/1e9).toFixed(1)+"B":Math.abs(o)>=1e6?"$"+(o/1e6).toFixed(0)+"M":Math.abs(o)>=1e3?"$"+(o/1e3).toFixed(0)+"k":"$"+o}function Ri(o){return o==="civil_engineering"?"CIVIL":o==="industrial"?"INDUSTRIAL":o==="mega_project"?"MEGA":o?.toUpperCase()||"—"}function kn(o){return o==="civil_engineering"?"light":o==="industrial"?"heavy":o==="mega_project"?"mega":"light"}function Da(){ro&&clearInterval(ro),ro=setInterval(()=>{if(!_i)return;const o=_i-Date.now();if(o<=0){document.getElementById("tick-countdown").textContent="Tick due...",clearInterval(ro);return}const e=Math.floor(o/36e5),t=Math.floor(o%36e5/6e4),i=Math.floor(o%6e4/1e3);document.getElementById("tick-countdown").textContent=e+"h "+t+"m "+i+"s"},1e3)}function ja(o,e){o==="type"&&(hi=e),o==="sort"&&($i=e),document.querySelectorAll(`.filter-pill[data-filter="${o}"]`).forEach(t=>{t.classList.toggle("active",t.dataset.value===e)}),En()}const pn={"Civil Engineering":"civil_engineering","Industrial Construction":"industrial",Megaprojects:"mega_project"};function wi(o){if(!d)return!1;if(pn[d.corp_subsector]===o.sector)return!0;const t=(G||[]).filter(i=>i.type==="regional_hq"&&i.is_active&&i.nation_id===o.nation_id);for(const i of t)if(pn[i.subsector]===o.sector)return!0;return!1}function En(){const o=document.getElementById("oc-list");let e=[...je];hi==="GOVERNMENT"?e=e.filter(s=>s.issuer_type==="GOVERNMENT"):hi==="PRIVATE"&&(e=e.filter(s=>s.issuer_type==="PRIVATE"));const t=new Set;d?.nation_id&&t.add(d.nation_id);for(const s of G||[])s.type==="regional_hq"&&s.is_active&&s.nation_id&&t.add(s.nation_id);const i=s=>t.has(s.nation_id)&&wi(s),n=(s,l)=>$i==="TIMELINE"?(s.timeline_ticks||0)-(l.timeline_ticks||0):$i==="BUDGET"?(l.budget_ceiling||0)-(s.budget_ceiling||0):0;if(e.sort((s,l)=>{const p=i(s)?1:0,f=i(l)?1:0;return p!==f?f-p:n(s,l)}),document.getElementById("oc-count").textContent=e.length+" AVAILABLE",e.length===0){o.innerHTML=`
            <div class="oc-empty">
            ...
            </div>`;return}const r=I?.current_tick||0;let a="";for(const s of e){const l=s.issuer_type==="GOVERNMENT",p=l?"gov":"private",f=wi(s),c=f?"":" locked",v=kn(s.sector),m=Ri(s.sector),u=(s.timeline_ticks||0)>18?" warn":"",g=s.bidding_ends_tick?Math.max(0,s.bidding_ends_tick-r):"?",b=gi[s.nation_id]||"—",$=t.has(s.nation_id);a+=`
            ...
            </div>`}o.innerHTML=a,o.querySelectorAll(".oc-item:not(.locked)").forEach(s=>{s.addEventListener("click",()=>{const l=s.dataset.contractId,p=je.find(f=>f.id===l);p&&Cn(p)})})}let Je=null;function Cn(o){Je=o;const e=document.getElementById("cd-overlay"),t=o.issuer_type==="GOVERNMENT",i=t?"gov":"private",n=(N?.name||d.nation||"—").toUpperCase(),r=wi(o);document.getElementById("cd-header-left").innerHTML=`
    ...
    `;let v='<div class="pm-phase__bar">';for(let b=0;b<Ue.length;b++){const $=b<o.phaseIdx,h=b===o.phaseIdx;v+=`<div class="pm-phase__seg">
        ...
        </button>`).join("");let u="";o.tab==="overview"?u=er(o,e,p,l,f,a,s):o.tab==="events"?u=tr(o):o.tab==="materials"?u=or(o):o.tab==="equipment"&&(u=ir(o)),document.getElementById("pm-content").innerHTML=`<div style="padding:0">${u}</div>`;let g="";c>0&&(g+=`<span class="pm-ftr__badge" style="color:var(--red);background:var(--red-faint);border:1px solid var(--red-border)">${c} EVENT${c>1?"S":""} REQUIRES RESPONSE</span>`),g+=`<span class="pm-ftr__badge" style="color:${o.quality>=75?"var(--green)":o.quality>=50?"var(--amber)":o.quality>=25?"var(--orange)":"var(--red)"};background:var(--bg-0);border:1px solid var(--border-0)">QUALITY: ${o.quality}/100 — ${o.qualityLabel}</span>`,document.getElementById("pm-footer").innerHTML=`
        ...
        <div class="pm-manager">
    ...
    </div></div>`;const c=e.modifiers||[];c.length>0&&(f+='<div style="padding:0 16px"><div class="pm-section">',f+='<div class="pm-section__title">Building Modifiers</div>',f+='<div style="display:flex;flex-wrap:wrap;gap:4px;">',f+=wn(c),f+="</div></div></div>");const v=Array.isArray(o.permitRequirements)?o.permitRequirements:[];if(v.length>0){f+=`<div style="padding:0 16px"><div class="pm-section">
            ...
            <span class="pm-evt__status" style="color:${r?"var(--red)":"var(--text-dim)"};font-weight:${r?"700":"400"}">${r?"REQUIRES RESPONSE":"RESOLVED"}</span>
                    ...
                    <span class="pm-evt__impact-text">${x(i.impact)}</span>
                        ...
                        </div>`),e+="</div>"}}!r&&i.resolution&&(e+=`<div class="pm-evt__resolution">
                    <div class="pm-evt__resolution-label">RESOLUTION</div>
                    <div class="pm-evt__resolution-text">${x(i.resolution)}</div>
                </div>`),e+="</div>"}e+="</div></div>"}return e}function or(o){if(o.materials.length===0)return'<div class="pm-evt-empty">No materials required for this project.</div>';let e='<div class="pm-tab-header">Project Materials</div>';for(const t of o.materials){const i=t.required>0?Math.round(t.allocated/t.required*100):0;t.allocated>0&&Math.round(t.consumed/t.allocated*100);const n=t.allocated>=t.required,r=n?"var(--green)":t.allocated>0?"var(--amber)":"var(--red)",a=n?"FULLY ALLOCATED":t.allocated>0?"PARTIAL":"NONE ALLOCATED";e+='<div class="pm-mat" style="margin-bottom:14px;">',e+=`<div class="pm-mat__row1">
...
Penalties: -`+_(c):"")+`
Reputation: `+(u>0?"+":"")+u+`
Net Profit: `+(m>=0?"+":"")+_(m)),Yo.add(o),await Vt(),await qn()}catch(r){alert("Delivery failed: "+(r.message||r)),un(o,!1)}finally{Wo=!1}}}window.deliverProject=rr;window.openProjectModal=at;window.closeProjectModal=Ka;window.pmSetTab=Ja;window.pmToggleEvent=Xa;window.pmSelectResponse=Za;window.pmAllocateMaterial=Ha;window.pmDeallocateMaterial=Ga;window.pmDeployEquipment=Ya;window.pmUndeployEquipment=Qa;window.pmAssignWorkers=Va;window.pmUnassignWorkers=Wa;async function Tn(o){if(!R)return;const{data:e,error:t}=await y.from("construction_events").select("*").eq("contract_id",o).order("fired_at_tick",{ascending:!1});t?(console.warn("Failed to load project events:",t.message),R.events=[]):R.events=(e||[]).map(i=>({id:i.id,type:i.type,severity:i.severity,tick:i.fired_at_tick,title:i.title,desc:i.description,impact:i.impact,status:i.status==="ACTIVE"?"ACTIVE":"RESOLVED",resolution:i.resolution,responses:i.responses||[]})),et()}let Qo=!1;async function sr(o,e){if(!(Qo||!R)){Qo=!0;try{const{data:t,error:i}=await y.rpc("resolve_construction_event",{p_event_id:o,p_response_key:e});if(i){console.error("Failed to resolve event:",i.message),alert("Failed to submit response: "+i.message);return}const n=typeof t=="string"?JSON.parse(t):t;if(n?.error){alert("Error: "+n.error);return}await Tn(R.project.id),await Vt(),n?.quality_applied&&n.quality_applied!==0&&(R.quality=Math.max(0,Math.min(100,R.quality+n.quality_applied)),R.qualityLabel=R.quality>=75?"EXCELLENT":R.quality>=50?"FAIR":R.quality>=25?"POOR":"BAD"),et()}finally{Qo=!1}}}window.confirmEventResponse=sr;function Oe(o,e,t){const i=t?` style="color:${t}"`:"";return`<div class="cd-detail-row">
        ...
        <g transform="translate(470,172)">
    ...
    </svg>`}async function Ae(){if(!d||!d.nation_id)return;const{data:o,error:e}=await y.from("construction_contracts").select("*").in("status",["open","bidding"]).order("generated_at_tick",{ascending:!1});if(e)console.warn("Failed to load contracts:",e.message),je=[];else{const n=Number(d.corp_reputation??0);je=(o||[]).filter(r=>n>=(r.min_reputation||0))}const i=[...new Set(je.map(n=>n.nation_id).filter(Boolean))].filter(n=>!gi[n]);if(i.length>0){const{data:n}=await y.from("nations").select("id, name").in("id",i);for(const r of n||[])gi[r.id]=r.name}if(Et={},d&&je.length>0){const n=je.map(a=>a.id),{data:r}=await y.from("contract_bids").select("contract_id, bid_price, estimated_quality, status").eq("faction_id",d.id).in("contract_id",n);for(const a of r||[])Et[a.contract_id]=a}En()}function dr(){const o=document.getElementById("ap-list"),e=document.getElementById("ap-footer");if(document.getElementById("ap-count").textContent=J.length+" ACTIVE",J.length===0){o.innerHTML=`<div class="ap-empty">
            <div class="ap-empty__text">No active projects.<br>Win a contract bid to start<br>your first construction project.</div>
        </div>`,e.style.display="none";return}const t=I?.current_tick||0;let i=0,n=0,r="";for(const a of J){const s=a.issuer_type==="GOVERNMENT",l=s?"gov":"private",p=Array.isArray(a.contract_bids)?a.contract_bids[0]:a.contract_bids,f=p?.bid_price||0,c=p?.estimated_cost||0,v=p?.estimated_quality||0,m=a.budget_ceiling||0,u=a.awarded_at_tick||t,g=a.stalled_ticks||0,b=Math.max(0,t-u),$=Math.max(0,b-g),h=a.timeline_ticks||8,E=Math.max(0,h-$),T=Math.min(100,Math.round($/h*100)),S=$>h,w=g>0;let C="";if(w){const k=a.required_workforce||{},z=a.workers_assigned||{},q=[];(Number(z.general)||0)<(Number(k.general)||0)&&q.push("General: "+(Number(z.general)||0)+"/"+(Number(k.general)||0)),(Number(z.skilled)||0)<(Number(k.skilled)||0)&&q.push("Skilled: "+(Number(z.skilled)||0)+"/"+(Number(k.skilled)||0)),(Number(z.innovative)||0)<(Number(k.innovative)||0)&&q.push("Innovative: "+(Number(z.innovative)||0)+"/"+(Number(k.innovative)||0)),q.length>0?C="Workers needed — "+q.join(", "):C="Materials needed — allocate from warehouse"}kn(a.sector);const M=Ri(a.sector);i+=m,n+=f,r+=`<div class="ap-item" onclick="openProjectModal('${a.id}')">
        ...
        </div>`}o.innerHTML=r,e.style.display=J.length>0?"":"none",J.length>0&&(document.getElementById("ap-total-crew").textContent=J.length,document.getElementById("ap-total-budget").textContent=ue(i),document.getElementById("ap-total-spent").textContent=ue(n))}async function Vt(){if(!d)return;const{data:o,error:e}=await y.from("construction_contracts").select("*, contract_bids!inner(bid_price, material_grades, labor_count, estimated_cost, estimated_quality)").eq("awarded_to_faction",d.id).in("status",["awarded","in_progress"]).eq("contract_bids.faction_id",d.id).order("awarded_at_tick",{ascending:!0});if(e?(console.warn("Failed to load active projects:",e.message),J=[]):J=o||[],J.length>0){const t=J.map(s=>s.id),{data:i}=await y.from("finance_loan_requests").select("insured_contract_id, status").eq("request_type","insurance").in("insured_contract_id",t),{data:n}=await y.from("finance_active_loans").select("request_id, finance_loan_requests!inner(insured_contract_id)").in("status",["current"]).eq("finance_loan_requests.request_type","insurance"),r=new Set((n||[]).map(s=>s.finance_loan_requests?.insured_contract_id).filter(Boolean)),a=new Set((i||[]).filter(s=>s.status==="open").map(s=>s.insured_contract_id));for(const s of J)s._hasInsurance=r.has(s.id),s._insurancePending=a.has(s.id)}dr()}const No=3e4;function Ao(){let o=0,e=0;for(const t of jt)for(const i of Ni){const n=X[t.key]?.[i];n&&(o+=n.qty,e+=n.value)}return{totalUnits:o,totalValue:e}}function qi(){const o=document.getElementById("wh-list"),{totalUnits:e,totalValue:t}=Ao();document.getElementById("wh-count").textContent=e.toLocaleString()+" UNITS",document.getElementById("wh-total-value").textContent=D(t);const i=Math.round(e/No*100),n=document.getElementById("wh-capacity");n.textContent=i+"%",n.style.color=i>80?"var(--red)":i>50?"var(--orange)":"var(--green)";let r="";for(let a=0;a<jt.length;a++){const s=jt[a],l=xi===a,p=X[s.key]?.LOW||{qty:0,value:0},f=X[s.key]?.STD||{qty:0,value:0},c=X[s.key]?.HIGH||{qty:0,value:0},v=p.qty+f.qty+c.qty,m=p.value+f.value+c.value,u=v===0,g=_e(s.key,"LOW",N),b=_e(s.key,"STD",N),$=_e(s.key,"HIGH",N),h=p.qty>0?"wh-dot wh-dot--low":"wh-dot wh-dot--empty",E=f.qty>0?"wh-dot wh-dot--std":"wh-dot wh-dot--empty",T=$.available?c.qty>0?"wh-dot wh-dot--high":"wh-dot wh-dot--empty":"wh-dot wh-dot--locked";if(r+='<div class="wh-row">',r+=`<div class="wh-row__collapsed${l?" expanded":""}" onclick="toggleWhRow(${a})">
            ...
            <span class="wh-row__val${u?" empty":""}">${m>0?D(m):"—"}</span>
                ...
                <span class="wh-expand__hdr-label" style="width:55px;text-align:right">$/UNIT</span>
                    ...
                    </div>`);r+="</div>"}r+="</div>"}o.innerHTML=r}function cr(o){xi=xi===o?-1:o,qi()}async function Li(){if(!d)return;const{data:o,error:e}=await y.from("corp_warehouse").select("material_key, quality_tier, quantity, total_value").eq("faction_id",d.id);X={};const t=[];if(e)console.warn("Failed to load warehouse:",e.message);else if(o){for(const i of o){const n=yo(i.material_key);X[n]||(X[n]={}),X[n][i.quality_tier]={qty:i.quantity||0,value:Number(i.total_value)||0},n!==i.material_key&&t.push(i)}if(t.length>0){const i=t.map(n=>({faction_id:d.id,nation_id:d.nation_id,material_key:yo(n.material_key),quality_tier:n.quality_tier,quantity:n.quantity||0,total_value:Number(n.total_value)||0,updated_at:new Date().toISOString()}));await y.from("corp_warehouse").upsert(i,{onConflict:"faction_id,material_key,quality_tier"});for(const n of t)await y.from("corp_warehouse").delete().eq("faction_id",d.id).eq("material_key",n.material_key).eq("quality_tier",n.quality_tier)}}qi()}const pr={manufacturing_output:"base production",rare_minerals:"raw input",inflation:"price modifier",fuel_prices:"transport cost",urbanization:"demand pressure",arable_land:"base production",physical_infrastructure:"transport",digital_infrastructure:"component quality",energy_generation:"electrical supply",standard_of_living:"demand tier",oil_and_gas:"base input",higher_education:"engineering"};function Oi(){const e=(nt()?.name||N?.name||d?.nation||"—").toUpperCase(),t=!!(vt&&N&&vt.id!==N.id);document.getElementById("pr-nation-badge").textContent=(t?"IMPORT — ":"LOCAL — ")+e;const i=document.getElementById("pr-nation-select");if(i&&i.options.length===0){const l=N?.name||d?.nation||"—";let p=`<option value="">${x(l)} (HQ)</option>`;for(const f of Ct)f.id!==N?.id&&(p+=`<option value="${f.id}">${x(f.name)}</option>`);i.innerHTML=p}i&&(i.value=vt?.id||"");const n=Number(d?.corp_cash_reserves)||0;document.getElementById("pr-cash").textContent=D(n);const{totalUnits:r}=Ao(),a=Math.round(r/No*100),s=document.getElementById("pr-wh-capacity");s.textContent=a+"%",s.style.color=a>80?"var(--red)":a>50?"var(--orange)":"var(--green)",Sn(),Bi(),Mo()}function Sn(){const o=nt(),e=document.getElementById("pr-mat-grid");let t="";for(const i of jt){const n=te===i.key,r=Ni.every(s=>!_e(i.key,s,o).available),a="pr-mat-btn"+(n?" active":"")+(r?" all-locked":"");t+=`<span class="${a}" onclick="setPrMat('${i.key}')">${x(i.name)}</span>`}e.innerHTML=t}function Bi(){const o=nt(),e=document.getElementById("pr-tier-bar");let t='<span class="pr-tier-label">GRADE</span>';for(const i of Ni){const n=_e(te,i,o),r=Y===i,a=n.available?Ai(te,i,o):null,s=hn[i],l=!n.available,p="pr-tier-btn"+(r?" active":"")+(l?" locked":"");t+=`<div class="${p}" onclick="${l?"":`setPrTier('${i}')`}">
            <div class="pr-tier-btn__label">
                <div class="wh-dot" style="width:5px;height:5px;background:${s};border-radius:1px;"></div>
                <span class="pr-tier-btn__name" style="color:${r?"var(--text-bright)":"var(--text-dim)"}">${vi[i]}</span>
            </div>
        ...
        </div>`;return}const n=Ai(te,Y,o),r=_n(te,Y,o),a=n*be,s=r>3e3?"LOW":r>1e3?"MODERATE":"HIGH",l=s==="LOW"?"var(--green)":s==="MODERATE"?"var(--amber)":"var(--red)",p=Number(o?.inflation??50),f=p>55?"up":p<45?"down":"flat",c=f==="up"?"&#9650;":f==="down"?"&#9660;":"&#8212;",v=f==="up"?"var(--red)":f==="down"?"var(--green)":"var(--text-dim)";let m="";m+=`<div style="padding:8px 14px;border-bottom:1px solid var(--border-0);">
        ...
        <div class="pr-drivers__title">Price Drivers (${x(o?.name||"—")})</div>`;for(const w of i.priceDrivers){const C=Number(o?.[w]??50),M=C>=50?"var(--green)":C>=30?"var(--amber)":C>=15?"var(--orange)":"var(--red)",k=pr[w]||w;m+=`<div class="pr-driver-row">
        ...
        </div>`}m+="</div>";const g=(Number(d?.corp_cash_reserves)||0)>=a,b=be>r,{totalUnits:$}=Ao(),h=No-$,E=be>h,T=h<=0,S=hn[Y];m+=`<div class="pr-order">
        ...
        <div class="pr-order__box">
    ...
    </div>`,e.innerHTML=m}function fr(o){const e=nt();te=o,Y="STD";for(const t of["STD","HIGH","LOW"])if(_e(o,t,e).available){Y=t;break}Sn(),Bi(),Mo()}function mr(o){Y=o,Bi(),Mo()}function ur(o){be=o,Mo()}let Ko=!1;async function vr(o){if(!o)vt=null;else{let i=Ct.find(n=>n.id===o);if(!i)try{const{data:n}=await y.from("nations").select("*").eq("id",o).single();i=n}catch{}vt=i||null}const e=nt();if(!_e(te,Y,e).available){Y="STD";for(const i of["STD","HIGH","LOW"])if(_e(te,i,e).available){Y=i;break}}const t=document.getElementById("pr-nation-select");t&&(t.value=o||""),Oi()}async function yr(){if(Ko||!d||!N)return;const o=nt(),e=Ai(te,Y,o),t=_n(te,Y,o),i=e*be,n=Number(d.corp_cash_reserves)||0;if(i>n){alert("Insufficient cash reserves.");return}if(be>t){alert("Exceeds available supply this tick.");return}const{totalUnits:r}=Ao(),a=No-r;if(a<=0){alert("Warehouse is full. Cannot purchase more materials.");return}if(be>a){alert(`Warehouse can only hold ${a.toLocaleString()} more units. Reduce quantity.`);return}Ko=!0;const s=document.querySelector(".pr-purchase-btn");s&&(s.disabled=!0,s.textContent="...");try{const l=n-i,{error:p}=await y.from("factions").update({corp_cash_reserves:l}).eq("id",d.id);if(p)throw p;const f=yo(te),c=X[f]?.[Y],v=(c?.qty||0)+be,m=(c?.value||0)+i,{error:u}=await y.from("corp_warehouse").upsert({faction_id:d.id,nation_id:d.nation_id,material_key:f,quality_tier:Y,quantity:v,total_value:m,last_purchased_tick:I?.current_tick||null,updated_at:new Date().toISOString()},{onConflict:"faction_id,material_key,quality_tier"});if(u){const{error:b}=await y.from("factions").update({corp_cash_reserves:n}).eq("id",d.id);throw b&&console.error("Cash refund failed after warehouse error:",b.message),u}d.corp_cash_reserves=l,X[f]||(X[f]={}),X[f][Y]={qty:v,value:m};const g=Math.floor(i/1e6);if(g>=1&&o?.id){const b=g*.01,{data:$,error:h}=await y.from("nations").select("gdp_growth").eq("id",o.id).single();if(!h&&$){const E=Math.min(100,Math.round((Number($.gdp_growth??50)+b)*100)/100);await y.from("nations").update({gdp_growth:E}).eq("id",o.id),N?.id===o.id&&(N.gdp_growth=E)}}qi(),Oi(),s&&(s.textContent="PURCHASED",setTimeout(()=>{s.isConnected&&(s.disabled=!1,s.textContent="PURCHASE")},1500))}catch(l){s&&(s.disabled=!1,s.textContent="PURCHASE"),alert("Purchase failed: "+(l.message||"Unknown error"))}finally{Ko=!1}}function zn(o){const e=Ke||N;if(!e)return[];const t=zo(o);if(!t)return[];const i=Ra(o,e),n=[],r=Number(e?.inflation??50),a=Number(e?.fuel_prices??50);Number(e?.manufacturing_output??50);const s=Ke&&N&&Ke.id!==N.id;let l=null;if(s&&(l=qa(e,N)),i.newAvailable>0){const p=cn(o,e),f=t.basePrice,c=Math.round(f*((r-50)/200)),v=Math.round(f*((a-50)/300));let m=p;const u=[{label:"Base price",value:D(f)},c!==0?{label:`Inflation (${r})`,mod:(c>=0?"+":"")+D(Math.abs(c))}:null,v!==0?{label:`Fuel transport (${a})`,mod:(v>=0?"+":"")+D(Math.abs(v))}:null].filter(Boolean),g=p-f-c-v;if(g!==0&&!s&&u.push({label:"Demand/scarcity",mod:(g>=0?"+":"")+D(Math.abs(g))}),s&&l){const b=Math.round(p*l.tariff),$=Math.round(p*l.transport);m=p+b+$,u.push({label:`Import tariff (${Math.round(l.tariff*100)}%)`,mod:"+"+D(b)}),u.push({label:`Transport (${l.deliveryTicks} tick${l.deliveryTicks>1?"s":""})`,mod:"+"+D($)})}n.push({seller:s?`${e.name} Buyers`:"Local Buyers",sellerType:"LOCAL",nation:e.name||"—",distance:s?l?.deliveryTicks||1:0,condition:100,price:Math.round(m),available:i.newAvailable,delivery:s?l.deliveryTicks+" tick"+(l.deliveryTicks>1?"s":""):"Immediate",deliveryTicks:s?l.deliveryTicks:0,used:!1,priceFactors:u,sourceNationId:e.id})}if(i.usedAvailable>0){const p=i.usedCondition,f=cn(o,e,{used:!0,condition:p});let c=f;const v=[{label:"Base price",value:D(t.basePrice)},{label:`Condition (${p}%)`,mod:"-"+D(Math.max(0,t.basePrice-f))}];if(s&&l){const m=Math.round(f*l.tariff),u=Math.round(f*l.transport);c=f+m+u,v.push({label:`Import tariff (${Math.round(l.tariff*100)}%)`,mod:"+"+D(m)}),v.push({label:`Transport (${l.deliveryTicks} tick${l.deliveryTicks>1?"s":""})`,mod:"+"+D(u)})}n.push({seller:s?`${e.name} Buyers`:"Local Buyers",sellerType:"LOCAL",nation:e.name||"—",distance:s?l?.deliveryTicks||1:0,condition:p,price:Math.round(c),available:i.usedAvailable,delivery:s?l.deliveryTicks+" tick"+(l.deliveryTicks>1?"s":""):"Immediate",deliveryTicks:s?l.deliveryTicks:0,used:!0,priceFactors:v,sourceNationId:e.id})}return n}function Ro(){const o=Number(d?.corp_cash_reserves)||0;document.getElementById("em-cash").textContent=D(o);const e=zo(me),t=Ft[e?.tier||1],i=document.getElementById("em-tier-badge");i&&(i.textContent=t.tag,i.style.color=t.color),i.style.background=t.color+"0a",i.style.border="1px solid "+t.color+"33";const n=document.getElementById("em-nation-select");if(n&&n.options.length===0){const s=N?.name||d?.nation||"—";let l=`<option value="">${x(s)} (HQ)</option>`;for(const p of Ct)p.id!==N?.id&&(l+=`<option value="${p.id}">${x(p.name)}</option>`);n.innerHTML=l}const r=document.getElementById("em-import-tag"),a=Ke&&N&&Ke.id!==N.id;r&&(r.style.display=a?"":"none"),gr(),Pi()}function gr(){let o="";for(let e=1;e<=3;e++){const t=Ft[e],i=yi(e),n=e===3?"em-selector__grid em-selector__grid--t3":"em-selector__grid em-selector__grid--t12";o+=`<div class="em-selector__row">
            <div class="em-selector__tier-label" style="color:${t.color}">${t.tag}</div>
            <div class="${n}">`;for(const r of i){const a=me===r.key,s=zn(r.key).length>0;o+=`<span class="em-selector__btn${a?" active":""}${s?"":" no-listings"}"
                style="${a?"background:"+t.color+";border-color:"+t.color:""}"
                onclick="setEmType('${r.key}')">${x(r.name)}</span>`}o+="</div></div>"}document.getElementById("em-selector").innerHTML=`<div class="em-selector">${o}</div>`}function Pi(){const o=document.getElementById("em-content");if(Be=zn(me),Be.length===0){o.innerHTML=`<div class="em-no-listings"><div class="em-no-listings__box">
        ...
        </div></div>`;return}Ce>=Be.length&&(Ce=0);let e="";for(let i=0;i<Be.length;i++){const n=Be[i],r=Ce===i,a=n.sellerType==="LOCAL"?"var(--teal)":"#5a8aaa",s=$n(n.condition);e+=`<div class="em-listing${r?" selected":""}" style="${r?"border-left-color:"+a:""}" onclick="setEmListing(${i})">`,e+=`<div class="em-listing__row1">
            <div style="display:flex;align-items:center;gap:6px">
        ...
        </div>`,r&&n.priceFactors&&(e+=`<div class="em-breakdown">
            ...
            </div>`),e+="</div>"}const t=Be[Ce];if(t){const i=zo(me),n=Ft[i?.tier||1],r=Math.min(t.available,4),a=t.price*Se,s=(Number(d?.corp_cash_reserves)||0)>=a;e+=`<div class="em-purchase"><div class="em-purchase__box">
            ...
            <div class="em-purchase__qty">
                ...
                <div class="em-purchase__qty-btns">
                    ${Array.from({length:r},(l,p)=>p+1).map(l=>`<span class="em-qty-btn${Se===l?" active":""}" style="${Se===l?"background:"+n.color+";border-color:"+n.color:""}" onclick="setEmQty(${l})">${l}</span>`).join("")}
                </div>
            ...
            <div class="em-purchase__total">
        ...
        </div></div>`}o.innerHTML=e}async function xr(o){if(!o)Ke=null;else{let t=Ct.find(i=>i.id===o);if(!t)try{const{data:i}=await y.from("nations").select("*").eq("id",o).single();t=i}catch{}Ke=t||null}Ce=0,Se=1;const e=document.getElementById("em-nation-select");e&&(e.value=o||""),Ro()}function br(o){me=o,Ce=0,Se=1,Ro()}function _r(o){Ce=o,Se=1,Pi()}function hr(o){Se=o,Pi()}let Jo=!1;async function $r(){if(Jo)return;const o=Be[Ce];if(!o||!d)return;const e=zo(me);if(!e)return;const t=Se,i=o.price*t,n=Number(d.corp_cash_reserves)||0;if(i>n){alert("Insufficient cash reserves.");return}if(t>o.available){alert("Not enough units available.");return}const r=document.querySelector(".em-purchase-btn");r&&(r.disabled=!0,r.textContent="..."),Jo=!0;try{const a=n-i,{error:s}=await y.from("factions").update({corp_cash_reserves:a}).eq("id",d.id);if(s)throw s;const l=!o.deliveryTicks||o.deliveryTicks===0;if(l){const f=re.find(E=>E.equipment_key===me),c=(f?.owned||0)+t,v=f?.purchase_price_avg||0,m=f?.owned||0,u=m>0?Math.round((v*m+o.price*t)/c):o.price,g=e.maintenancePerUnit*c,b=f?.condition||100,$=Math.round((b*m+o.condition*t)/c),{error:h}=await y.from("corp_equipment").upsert({faction_id:d.id,nation_id:d.nation_id,equipment_key:me,tier:e.tier,owned:c,deployed:f?.deployed||0,condition:$,maintenance_per_tick:g,purchase_price_avg:u,last_purchased_tick:I?.current_tick||null,updated_at:new Date().toISOString()},{onConflict:"faction_id,equipment_key"});if(h){const{error:E}=await y.from("factions").update({corp_cash_reserves:n}).eq("id",d.id);throw E&&console.error("Cash refund failed:",E.message),h}f?(f.owned=c,f.condition=$,f.maintenance_per_tick=g):re.push({equipment_key:me,tier:e.tier,owned:c,deployed:0,condition:$,maintenance_per_tick:g,assigned_projects:[]})}else{const f=(I?.current_tick||0)+o.deliveryTicks,{error:c}=await y.from("corp_equipment_deliveries").insert({faction_id:d.id,equipment_key:me,quantity:t,condition:o.condition,delivery_tick:f,source_nation_id:o.sourceNationId||null,seller_name:o.seller,price_paid:i});if(c){const{error:v}=await y.from("factions").update({corp_cash_reserves:n}).eq("id",d.id);throw v&&console.error("Cash refund failed:",v.message),c}}d.corp_cash_reserves=a,Vi(),Ro();const p=document.getElementById("pr-cash");p&&(p.textContent=D(a)),r&&(r.textContent=l?"PURCHASED":"ORDERED",setTimeout(()=>{r.isConnected&&(r.disabled=!1,r.textContent="PURCHASE")},1500))}catch(a){r&&(r.disabled=!1,r.textContent="PURCHASE"),alert("Purchase failed: "+(a.message||"Unknown error"))}finally{Jo=!1}}let wr=-1,ct=[],_o=[],ki=[];function Xo(o){return Math.abs(o)>=1e6?"$"+(o/1e6).toFixed(1)+"M":Math.abs(o)>=1e3?"$"+(o/1e3).toFixed(0)+"k":"$"+o.toLocaleString()}function kr(o,e,t){if(t)return"var(--orange)";const i=o/(e||1)*100;return i>50?"var(--green)":i>25?"var(--amber)":"var(--red)"}function vn(){const o=document.getElementById("pm-list"),e=ct.length,t=_o.length,i=ki.length,n=ct.filter(l=>l.expiring_soon).length;document.getElementById("pm-active-count").textContent=`(${e})`,document.getElementById("pm-pending-count").textContent=`(${t})`,document.getElementById("pm-apply-count").textContent=`(${i})`;const r=document.getElementById("pm-badges");let a="";n>0&&(a+=`<span class="pm-badge pm-badge--expiring">${n} EXPIRING</span>`),t>0&&(a+=`<span class="pm-badge pm-badge--pending">${t} PENDING</span>`),r.innerHTML=a;const s=ct.reduce((l,p)=>l+(p.cost||0),0)+_o.reduce((l,p)=>l+(p.cost||0),0);document.getElementById("pm-total-cost").textContent=Xo(s),document.getElementById("pm-footer-active").textContent=e,document.getElementById("pm-footer-pending").textContent=t;{if(e===0){o.innerHTML=`<div class="pm-empty">
                ...
                <div class="pm-item__inner">
                    ...
                    </div>`,p.note&&(l+=`<div class="pm-note"><span class="pm-note__text">${x(p.note)}</span></div>`),p.expiring_soon&&p.renewable&&(l+=`<div class="pm-btn-row"><button class="pm-btn pm-btn--renew" onclick="event.stopPropagation(); pmApplyForPermit('${p.permit_key}');">RENEW — ${Xo(p.cost||0)}</button></div>`),l+="</div>"),l+="</div></div>"}),o.innerHTML=l;return}}let Zo=!1;async function Er(o){if(!(Zo||!d||!N)){Zo=!0;try{const{data:e}=await y.from("shard").select("current_tick").eq("name","Alpha Shard").single(),t=e?.current_tick||0,{data:i,error:n}=await y.rpc("apply_for_permit",{p_faction_id:d.id,p_nation_id:N.id,p_permit_key:o,p_current_tick:t});if(n){alert("Application failed: "+n.message);return}if(i&&!i.success){alert(i.error||"Application failed");return}alert("Permit application submitted! Processing: "+(i.processing_ticks||0)+" ticks."),await In()}catch(e){alert("Error: "+e.message)}finally{Zo=!1}}}window.pmApplyForPermit=Er;async function In(){if(!d||!N){ct=[],_o=[],ki=[],vn();return}const{data:o}=await y.from("construction_permits").select("*"),e=o||[],t={};for(const c of e)t[c.permit_key]=c;const{data:i}=await y.from("corp_permits").select("*").eq("faction_id",d.id).eq("nation_id",N.id),n=i||[],{data:r}=await y.from("active_laws").select("policy_id, policies(permit_key, policy_name)").eq("nation_id",N.id).not("policies.permit_key","is",null),a=new Set,s={};for(const c of r||[])c.policies?.permit_key&&(a.add(c.policies.permit_key),s[c.policies.permit_key]=c.policies.policy_name);const{data:l}=await y.from("shard").select("current_tick").eq("name","Alpha Shard").single(),p=l?.current_tick||0;ct=n.filter(c=>c.status==="active").map(c=>{const v=t[c.permit_key]||{},m=c.expires_at_tick?Math.max(0,c.expires_at_tick-p):999,u=v.duration_ticks||24;return{name:v.name||c.permit_key,permit_key:c.permit_key,nation:N.name,policy:s[c.permit_key]||"—",issued:c.granted_at_tick!=null?He(c.granted_at_tick):"—",expires:c.expires_at_tick?He(c.expires_at_tick):"Single-use",cost:c.cost_paid||0,ticks_left:m,total_ticks:u,expiring_soon:m<=3&&m>0,renewable:v.duration_ticks!=null,projects:[]}}),_o=n.filter(c=>c.status==="pending").map(c=>{const v=t[c.permit_key]||{},m=v.processing_ticks||2,u=p-c.applied_at_tick,g=Math.max(0,m-u);return{name:v.name||c.permit_key,permit_key:c.permit_key,nation:N.name,applied:He(c.applied_at_tick),status:"PROCESSING",processing_total:m,ticks_remaining:g,est_approval:He(c.applied_at_tick+m),cost:c.cost_paid||0,required_by:s[c.permit_key]||"—"}});const f=new Set(n.filter(c=>c.status==="active"||c.status==="pending").map(c=>c.permit_key));ki=[...a].filter(c=>!f.has(c)).map(c=>{const v=t[c]||{};return{name:v.name||c,permit_key:c,nation:N.name,description:v.description||"",policy:s[c]||"—",cost:v.cost_is_percentage?15e4:v.cost||0,processing_time:v.processing_ticks||2,duration:v.duration_ticks?v.duration_ticks+" ticks":"Single-use",category:v.category||"",difficulty:v.difficulty||"EASY"}}),vn()}let Ge=[],Cr=-1;function we(o){return Math.abs(o)>=1e6?"$"+(o/1e6).toFixed(2)+"M":Math.abs(o)>=1e3?"$"+(o/1e3).toFixed(0)+"k":"$"+o.toLocaleString()}function yn(o){return o>=85?"var(--gold)":o>=60?"var(--green)":o>=40?"var(--orange)":"var(--red)"}function Tr(o){return"dl-result--"+o.toLowerCase()}function gn(){const o=document.getElementById("dl-list"),e=Ge.length;document.getElementById("dl-count").textContent=`${e} COMPLETED`;const t=Ge.reduce((s,l)=>{const p=l.financials||{};return s+((p.payment||0)+(p.bonus||0)-(p.penalty||0)-(p.total_cost||0))},0),i=document.getElementById("dl-lifetime-profit");i.textContent=(t>=0?"+":"")+we(t),i.style.color=t>=0?"var(--green)":"var(--red)";const n={};Ge.forEach(s=>{n[s.result]=(n[s.result]||0)+1});const r=document.getElementById("dl-footer-results");if(r.innerHTML=Object.entries(n).map(([s,l])=>`<div class="dl-footer__result-box">
            ...
            <div class="dl-item__inner" style="border-left:2px solid ${g}">
                ...
                </div>`,p){const $=s.inspection||{};a+='<div style="margin-top:8px;">',a+='<div class="dl-section-label">Inspection Report</div>',["materials","structural","systems"].forEach(S=>{const w=$[S]||{score:0,issues:[]},C=yn(w.score),M=Math.min(w.score/100*100,100);a+=`<div class="dl-inspect-row">
        ...
        <div class="eq-summary__cell">
        ...
        </div>`;const n={};for(const s of re)n[s.equipment_key]=s;let r="";for(let s=1;s<=3;s++){const l=Ft[s],p=yi(s),f=bi===s,c=p.reduce((m,u)=>m+(n[u.key]?.owned||0),0),v=p.reduce((m,u)=>m+(n[u.key]?.deployed||0),0);if(r+=`<div class="eq-tier-hdr" onclick="toggleEqTier(${s})">
            ...
            ${c>0?`<span class="eq-tier-hdr__count">${v}/${c}</span>`:'<span class="eq-tier-hdr__none">NONE OWNED</span>'}
        </div>`,f)for(const m of p){const u=n[m.key],g=u?.owned||0,b=u?.deployed||0,$=u?.condition||0,h=m.maintenancePerUnit*g,E=g-b,T=g>0&&E===0,S=g>0&&$<65,w=$n($),C=u?.assigned_projects||[],M=C.length>0?C.map(k=>k.contract_name||"Project").join(", ").slice(0,30):g>0&&b>0?b+" project"+(b>1?"s":""):"—";r+=`<div class="eq-row${g===0?" unowned":""}">`,r+=`<div class="eq-row__top">
                    ...
                    </div>`:r+='<div class="eq-row__hint">Purchase from Equipment Market →</div>',r+="</div>"}}document.getElementById("eq-list").innerHTML=r;const a=[1,2,3].map(s=>{const l=Ft[s],p=yi(s).reduce((f,c)=>f+(n[c.key]?.owned||0),0);return`<div class="eq-footer__tier-box" style="border-color:${p>0?l.color+"33":"var(--border-0)"};background:${p>0?l.color+"0a":"transparent"}">
        ...
        <div class="eq-footer__tiers">${a}</div>`}function Pr(o){bi=bi===o?-1:o,Vi()}async function Wi(){if(!d)return;const{data:o,error:e}=await y.from("corp_equipment").select("equipment_key, tier, owned, deployed, condition, maintenance_per_tick, assigned_projects").eq("faction_id",d.id);e?(console.warn("Failed to load equipment:",e.message),re=[]):re=o||[],Vi()}async function Dr(){const{data:{user:o}}=await y.auth.getUser();if(!o){window.location.href="login.html";return}const{data:e}=await y.from("factions").select("*").or(`id.eq.${o.id},linked_user_id.eq.${o.id}`);ke=(e||[]).filter(m=>m.nation_id);const t=sessionStorage.getItem("active_faction_id");if(d=ke.find(m=>m.id===t)||ke.find(m=>m.faction_type==="corporation")||ke[0],!d){await y.auth.signOut(),window.location.href="login.html";return}if(d.faction_type!=="corporation"){window.location.href="dashboard.html";return}const i=new URLSearchParams(window.location.search).get("tab"),n=i==="expansion"||i==="actions";if(d.corp_sector!=="Construction"&&!n){const u={Finance:"corp-operations-finance.html",Shipping:"corp-operations-shipping.html"}[d.corp_sector];if(u){window.location.href=u;return}}const[r,a]=await Promise.all([d.nation_id?y.from("nations").select("*").eq("id",d.nation_id).single():Promise.resolve({data:null}),y.from("shard").select("current_tick, current_date, next_tick_at, tick_interval_hours").eq("name","Alpha Shard").single()]);r.error&&console.warn("Nation load failed:",r.error.message),r.data&&(N=r.data),a.error&&console.warn("Shard load failed:",a.error.message),I=a.data;let s=0;if(d?.id){const{data:m}=await y.from("construction_contracts").select("id, contract_bids!inner(id)").eq("issuer_faction_id",d.id).in("status",["open","bidding"]);if(m)for(const u of m)s+=(u.contract_bids||[]).length}const l=document.getElementById("corp-topbar-container");if(l){const{renderCorpTopBar:m}=await Na(async()=>{const{renderCorpTopBar:b}=await import("./corp-topbar-DC7OCaX3.js");return{renderCorpTopBar:b}},__vite__mapDeps([0,1])),u=new URLSearchParams(window.location.search).get("tab")||"operations",g={};s>0&&(g.home={color:"#c8a832",title:s+" pending bid"+(s!==1?"s":"")+" on your projects"}),m(l,{faction:d,shard:I,activeTab:u,allUserFactions:ke,badges:g})}if(I){if(document.getElementById("game-date").textContent=I.current_date||"—",document.getElementById("tick-number").textContent=I.current_tick||"—",I.next_tick_at){const u=(Number(I.tick_interval_hours)||8)*36e5,g=new Date(I.next_tick_at).getTime(),$=g-u+u/2;_i=new Date($>Date.now()?$:g+u/2),Da()}const m=document.querySelector("#tick-countdown")?.closest(".tick-item")?.querySelector(".tick-label");m&&(m.textContent="Next Corp Tick")}const p=document.getElementById("topbar-cash");p&&(p.textContent="CASH: "+Nn(Number(d.corp_cash_reserves??0)));const f=document.getElementById("topbar-ap");f&&(f.style.display="none");const c=document.getElementById("nation-pill");c&&(c.textContent=(N?.name||d.nation||"—").toUpperCase());const v=document.getElementById("corp-faction-dropdown");if(v){let m="";for(const u of ke){const g=u.id===d.id,b=u.faction_type==="corporation"?"CORP":"PARTY",$=u.faction_type==="corporation"?"var(--teal)":"var(--amber)";m+=`<div class="corp-dd-item${g?" active":""}" onclick="switchToFaction('${u.id}', '${u.faction_type}')">
        ...
        <div style="display:flex;align-items:center;gap:12px;">
    ...
    </div>`,i+='<div style="display:flex;gap:8px;">',i+='<div style="width:262px;display:flex;flex-direction:column;gap:5px;flex-shrink:0;">';for(let n=0;n<uo.length;n++){const r=uo[n],a=vo[r],s=Re(r),l=yt===n,p=a.color,f=!s;if(i+=`<div onclick="actSelectExec(${n})" style="
            padding:10px 12px;
            ...
            cursor:pointer;
        ">`,f&&r!=="CEO")i+=`<div style="display:flex;align-items:center;gap:10px;">
            ...
            </div>`;else{const c=s?`${s.first_name} ${s.last_name}`:"—",v=s?s.age:0,m=s?s.skill:0,u=s?s.salary_per_year:0,g=s?wo(s.first_name,s.last_name):"—";i+=`<div style="display:flex;align-items:center;gap:10px;">
    ...
    </div>`,r+='<div style="background:var(--bg-2,#1a1a17);border:1px solid var(--border-0,rgba(255,255,255,0.06));border-top:1px solid var(--border-0,rgba(255,255,255,0.06));flex:1;">';for(let a=0;a<n.length;a++){const s=n[a],l=!!s.locked;r+=`<div onmouseenter="this.dataset.hover='1';this.style.background='${l?"transparent":t.color+"06"}'" onmouseleave="this.dataset.hover='';this.style.background='transparent';var eb=this.querySelector('.act-exec-btn');if(eb)eb.style.display='none'" style="
            padding:16px 20px;
            ${a<n.length-1?"border-bottom:1px solid var(--border-0,rgba(255,255,255,0.06));":""}
            opacity:${l?"0.4":"1"};
    ...
    </div>`,o.innerHTML=r,o.querySelectorAll("[onmouseenter]").forEach(a=>{a.addEventListener("mouseenter",function(){const s=this.querySelector(".act-exec-btn");s&&(s.style.display="block")}),a.addEventListener("mouseleave",function(){const s=this.querySelector(".act-exec-btn");s&&(s.style.display="none")})})}function Wr(o,e,t,i,n){const r=I?.current_tick||0,a=Math.max(0,n-r),s=Math.round(i*(a/12)),l=`FIRE ${e}: ${t}
...
Are you sure?`;confirm(l)&&Yr(o,e,s)}async function Yr(o,e,t){try{const i=Number(d?.corp_cash_reserves??0);if(i<t){alert(`Insufficient funds. You need $${(t/1e6).toFixed(2)}M but only have $${(i/1e6).toFixed(2)}M.`);return}const n=i-t,{error:r}=await y.from("factions").update({corp_cash_reserves:n}).eq("id",d.id);if(r){alert("Failed to process payout: "+r.message);return}const{error:a}=await y.from("corp_executives").update({status:"fired",updated_at:new Date().toISOString()}).eq("id",o);if(a){await y.from("factions").update({corp_cash_reserves:i}).eq("id",d.id),alert("Failed to fire executive: "+a.message);return}d.corp_cash_reserves=n,Ut=Ut.filter(s=>s.id!==o),St()}catch(i){console.error("[CorpOps] Fire executive error:",i),alert("An error occurred.")}}function Qr(o,e){if((jn[e]||[]).find(i=>i.id===o)?.cooldown==="once/tick"&&Gr(o)){alert("This action can only be used once per tick. Wait for the next tick.");return}switch(o){case"statement":return Un();case"loan":return Vn();case"restructure":return Yn();case"rebrand":return Qn();case"donate":return Kn();case"bankruptcy":return Hn()}}let Ci=!1;function Un(){if(Ci)return;Ci=!0;const o=document.createElement("div");o.id="stmt-overlay",o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:600;display:flex;align-items:center;justify-content:center;",o.onclick=function(l){l.target===o&&Yi()};const e=d?.faction_name||"Corporation",t=(d?.abbreviation||d?.corp_ticker||"??").toUpperCase(),i=Number(d?.corp_cash_reserves??0),n=Re("CEO"),r=n?`${n.first_name} ${n.last_name}`:"CEO";o.innerHTML=`<div onclick="event.stopPropagation()" style="width:480px;background:#f8f7f2;border:1px solid #e8e7e0;display:flex;flex-direction:column;overflow:hidden;">
    ...
    </div>`,document.body.appendChild(o);const a=document.getElementById("stmt-text"),s=document.getElementById("stmt-chars");a&&s&&(a.addEventListener("input",function(){s.textContent=this.value.length+"/500"}),a.focus())}function Yi(){const o=document.getElementById("stmt-overlay");o&&o.remove(),Ci=!1}let Nt=!1;async function Kr(){if(!d||!I||Nt)return;const o=document.getElementById("stmt-text"),e=document.getElementById("stmt-error"),t=(o?.value||"").trim();if(!t){e&&(e.textContent="Statement cannot be empty.",e.style.display="block");return}if(t.length>500){e&&(e.textContent="Statement too long (max 500 chars).",e.style.display="block");return}const i=Re("CEO"),n=i?i.skill:50,r=Math.round(2e4*Zt(n)),a=Number(d.corp_cash_reserves??0);if(a<r){e&&(e.textContent="Insufficient cash. Need "+_(r)+".",e.style.display="block");return}Nt=!0;const s=document.getElementById("stmt-submit-btn");s&&(s.style.opacity="0.4",s.style.pointerEvents="none");const l=d.faction_name||"Corporation",p=i?`${i.first_name} ${i.last_name}`:"CEO",f=I.current_tick||0,{error:c}=await y.from("factions").update({corp_cash_reserves:a-r}).eq("id",d.id);if(c){Nt=!1,e&&(e.textContent="Failed to deduct cost: "+c.message,e.style.display="block"),s&&(s.style.opacity="1",s.style.pointerEvents="auto");return}const{error:v}=await y.from("event_log").insert({nation_id:d.nation_id,faction_id:d.id,event_name:l+" — Press Release",description_used:p+", CEO of "+l+': "'+t.replace(/[<>"]/g,"")+'"',category:"business",trigger_key:"ceo_statement",effects_applied:{cost:r,ceo:p,skill:n},fired_at_tick:f});if(v){await y.from("factions").update({corp_cash_reserves:a}).eq("id",d.id),Nt=!1,e&&(e.textContent="Failed to publish: "+v.message,e.style.display="block"),s&&(s.style.opacity="1",s.style.pointerEvents="auto");return}d.corp_cash_reserves=a-r,Nt=!1,_t("statement"),Yi()}const bn=24,Jr=.5;async function Xr(o,e){const t=e-bn,{data:i}=await y.from("event_log").select("fired_at_tick, effects_applied").eq("trigger_key","corp_bankruptcy").gte("fired_at_tick",t).order("fired_at_tick",{ascending:!1}).limit(20),n=(i||[]).find(a=>a.effects_applied?.user_id===o),r=n?Math.max(0,n.fired_at_tick+bn-e):0;return{onCooldown:r>0,ticksLeft:r}}let oi=!1;async function Hn(){if(oi)return;const{data:{user:o}}=await y.auth.getUser();if(!o){alert("Not logged in.");return}const e=d?.id||sessionStorage.getItem("active_faction_id");if(!e){alert("No active faction selected.");return}const{data:t,error:i}=await y.from("factions").select("*").eq("id",e).eq("faction_type","corporation").is("abandoned_at",null).single();if(i||!t){alert("No active corporation found. It may have already been dissolved.");return}const n=t,r=n.faction_name||"this corporation",{data:a,error:s}=await y.from("shard").select("current_tick").eq("name","Alpha Shard").single();if(s||!a){alert("Failed to read game tick. Please try again.");return}const l=a.current_tick||0,{onCooldown:p,ticksLeft:f}=await Xr(o.id,l);if(p){alert("Bankruptcy is on cooldown. You must wait "+f+" more tick"+(f!==1?"s":"")+" before declaring bankruptcy again.");return}if(!confirm("DECLARE BANKRUPTCY — "+r.toUpperCase()+`?
...
Please try again or contact support.`)}finally{oi=!1}}const Gn=[{id:"equipment",label:"Equipment Acquisition",desc:"Purchase vehicles, cranes, or heavy machinery",icon:"&#9881;"},{id:"working",label:"Working Capital",desc:"Bridge financing for active project costs",icon:"$"},{id:"property",label:"Property Purchase",desc:"Acquire office, warehouse, or HQ building",icon:"&#9632;"},{id:"subsidiary",label:"Subsidiary Expansion",desc:"Fund new subsidiary establishment",icon:"&#9672;"},{id:"materials",label:"Material Procurement",desc:"Bulk material purchase for upcoming projects",icon:"&#9638;"}],ii=[{id:"none",label:"None",desc:"Unsecured — lenders may charge higher rates",risk:"HIGH",riskColor:"#c84"},{id:"equipment",label:"Equipment",desc:"Financed equipment serves as collateral",risk:"MODERATE",riskColor:"#ca5"},{id:"property",label:"Property",desc:"Corporate property lien",risk:"LOW",riskColor:"#8b9a6b"},{id:"full",label:"Full Assets",desc:"All corporate assets — maximum lender security",risk:"MINIMAL",riskColor:"#5c5"}];let ie=25e7,Wt="equipment",ht=48,fe="equipment",ko="",Mt=[];function Vn(){ie=25e7,Wt="equipment",ht=48,fe="equipment",ko="",document.getElementById("lr-overlay").style.display="flex",is(),zt()}function Wn(){document.getElementById("lr-overlay").style.display="none"}function Zr(o){ie=Math.max(1e6,Math.min(5e9,Number(o)||0)),zt()}function es(o){Wt=o,zt()}function ts(o){ht=o,zt()}function os(o){fe=o,zt()}async function is(){if(!d)return;const{data:o}=await y.from("factions").select("id, faction_name, abbreviation, corp_ticker, corp_company_type").eq("faction_type","corporation").eq("corp_sector","Finance").is("abandoned_at",null).neq("id",d.id);Mt=o||[],zt()}function zt(){const o=document.getElementById("lr-modal-content");if(!o)return;const e=Number(d?.corp_cash_reserves??0),t=Number(d?.corp_loans??0),i=Number(d?.corp_reputation??50),n=d?.faction_name||"Corporation",r=(d?.abbreviation||d?.corp_ticker||"??").toUpperCase(),a=t+ie,s=a>e*3?"#c55":a>e*1.5?"#c84":a>e?"#ca5":"#5c5",l=a>e*3?"DANGEROUS":a>e*1.5?"HEAVY":a>e?"MODERATE":"HEALTHY",p=fe==="none"?"10-16%":fe==="equipment"?"7-12%":fe==="property"?"5-9%":"4-7%",c=Math.round(ie*(fe==="none"?.13:fe==="equipment"?.095:fe==="property"?.07:.055)/12+ie/ht),v=ii.find(u=>u.id===fe)||ii[0];let m="";m+=`<div style="padding:10px 16px;border-bottom:1px solid #e8e7e0;background:#faf9f5;flex-shrink:0;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
            <div style="display:flex;align-items:center;gap:8px;">
    ...
    <div style="display:flex;gap:0;border-bottom:1px solid #e8e7e0;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:3px;">`;for(const u of ii){const g=fe===u.id;m+=`<div onclick="lrSetCollateral('${u.id}')" style="padding:6px 8px;cursor:pointer;background:${g?"rgba(90,138,170,0.06)":"transparent"};border:1px solid ${g?"#5a8aaa44":"#e8e7e0"};">
            <div style="display:flex;justify-content:space-between;align-items:center;">
        ...
        <textarea id="lr-note" rows="2" maxlength="300" onchange="lrNote=this.value"
            ...
            style="width:100%;padding:6px 8px;font-family:var(--font-ui);font-size:10px;color:#e8e4dc;background:#faf9f5;border:1px solid #e8e7e0;outline:none;resize:none;box-sizing:border-box;line-height:1.5;">${x(ko)}</textarea>
    </div>`,m+=`<div style="padding:8px 16px;border-bottom:1px solid #e8e7e0;">
    ...
    </div>`,m+='<div id="lr-error" style="padding:4px 16px;font-family:var(--font-mono);font-size:8px;color:#c55;display:none;"></div>',o.innerHTML=m}let so=!1;async function ns(){if(!d||!I||so)return;const o=document.getElementById("lr-error");if(ie<1e6){o.textContent="Minimum loan amount is $1M.",o.style.display="block";return}if(ie>5e9){o.textContent="Maximum loan amount is $5B.",o.style.display="block";return}const t=((Gn.find(a=>a.id===Wt)||{}).label||Wt)+(ko?" — "+ko:""),i=document.getElementById("lr-submit-btn");so=!0,i.style.opacity="0.5",i.style.pointerEvents="none";const n=I.current_tick||0,{error:r}=await y.from("finance_loan_requests").insert({requesting_faction_id:d.id,nation_id:d.nation_id,amount:ie,term_months:ht,purpose:t,created_tick:n,expires_tick:n+5});if(i.style.opacity="1",i.style.pointerEvents="auto",r){so=!1,o.textContent="Failed to submit: "+r.message,o.style.display="block",i.style.opacity="1",i.style.pointerEvents="auto";return}so=!1,Wn()}function Yn(){if(!d)return;const o=Number(d.corp_loans??0),e=Number(d.corp_reputation??50),t=Number(d.corp_general_workforce??0),i=Number(d.corp_skilled_workforce??0),n=Number(d.corp_innovative_workforce??0),r=t+i+n;if(r===0){alert("Cannot restructure — no employees to lay off.");return}const a=Re("COO"),s=a?a.skill:50,l=Zt(s),p=10+Math.floor(Math.random()*11),f=Math.round(r*p/100),c=Math.round(o*.07),v=Math.round(c*(2-l)),m=3+Math.floor(Math.random()*10),u=Math.max(1,Math.round(m*l)),g=Math.round(t/r*f),b=Math.round(i/r*f),$=Math.max(0,Math.min(n,f-g-b)),h=document.createElement("div");h.id="restr-overlay",h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:600;display:flex;align-items:center;justify-content:center;",h.onclick=function(E){E.target===h&&Qi()},h.innerHTML=`<div onclick="event.stopPropagation()" style="width:440px;background:#f8f7f2;border:1px solid #e8e7e0;display:flex;flex-direction:column;overflow:hidden;">
        <div style="padding:10px 16px;border-bottom:1px solid #e8e7e0;background:#faf9f5;">
            <div style="display:flex;justify-content:space-between;align-items:center;">
                <div style="display:flex;align-items:center;gap:8px;">
    ...
    </div>`,document.body.appendChild(h)}function Qi(){const o=document.getElementById("restr-overlay");o&&o.remove()}let lo=!1;async function as(o,e,t,i,n,r){if(!d||!I||lo)return;lo=!0;const a=document.getElementById("restr-btn");a&&(a.style.opacity="0.4",a.style.pointerEvents="none");const s=Number(d.corp_general_workforce??0),l=Number(d.corp_skilled_workforce??0),p=Number(d.corp_innovative_workforce??0),f=Number(d.corp_loans??0),c=Number(d.corp_reputation??50),v={corp_general_workforce:Math.max(0,s-i),corp_skilled_workforce:Math.max(0,l-n),corp_innovative_workforce:Math.max(0,p-r),corp_loans:Math.max(0,f-e),corp_reputation:Math.max(0,c-t)},{error:m}=await y.from("factions").update(v).eq("id",d.id);if(m){lo=!1;const b=document.getElementById("restr-error");b&&(b.textContent="Failed: "+m.message,b.style.display="block"),a&&(a.style.opacity="1",a.style.pointerEvents="auto");return}Object.assign(d,v);const u=I.current_tick||0,{error:g}=await y.from("event_log").insert({nation_id:d.nation_id,faction_id:d.id,event_name:(d.faction_name||"Corporation")+" — Restructuring",description_used:(d.faction_name||"A corporation")+" has announced a restructuring, laying off "+o+"% of its workforce.",category:"business",trigger_key:"corp_restructure",effects_applied:{layoff_pct:o,debt_cut:e,rep_loss:t},fired_at_tick:u});g&&console.warn("Failed to log restructure event:",g.message),lo=!1,_t("restructure"),Qi(),St()}function Qn(){const o=Re("CMO"),e=o?o.skill:50,t=Zt(e),i=Math.round(2e7*t),n=Math.max(1,Math.round(5*t)),r=Number(d?.corp_cash_reserves??0),a=Number(d?.corp_reputation??50),s=d?.faction_name||"",l=d?.abbreviation||d?.corp_ticker||"",p=document.createElement("div");p.id="rebrand-overlay",p.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:600;display:flex;align-items:center;justify-content:center;",p.onclick=function(f){f.target===p&&Ki()},p.innerHTML=`<div onclick="event.stopPropagation()" style="width:440px;background:#f8f7f2;border:1px solid #e8e7e0;display:flex;flex-direction:column;overflow:hidden;">
        <div style="padding:10px 16px;border-bottom:1px solid #e8e7e0;background:#faf9f5;">
            <div style="display:flex;justify-content:space-between;align-items:center;">
                <div style="display:flex;align-items:center;gap:8px;">
        ...
        <div style="padding:8px 16px;border-top:1px solid #e8e7e0;">
            ...
            <div style="background:#faf9f5;border:1px solid #e8e7e0;padding:6px 10px;">
                ...
                <div style="display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px solid #e8e7e0;">
                    ...
                    <span style="font-family:var(--font-mono);font-size:10px;font-weight:700;color:#c55;">-${n} (${a} &rarr; ${Math.max(0,a-n)})</span>
                </div>
                <div style="display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px solid #e8e7e0;">
                ...
                <div style="display:flex;justify-content:space-between;padding:3px 0;">
                    ...
                    <span style="font-family:var(--font-mono);font-size:10px;font-weight:700;color:${r<i?"#c55":"#e8e4dc"};">${_(r-i)}</span>
                </div>
            </div>
        </div>
        <div style="padding:8px 16px;border-top:1px solid #e8e7e0;display:flex;justify-content:flex-end;gap:6px;">
            ...
            <div id="rebrand-btn" onclick="actSubmitRebrand(${i},${n})" style="padding:6px 20px;font-family:var(--font-mono);font-size:9px;font-weight:700;letter-spacing:1px;color:#000;background:#c84;cursor:${r>=i?"pointer":"not-allowed"};${r<i?"opacity:0.4;pointer-events:none;":""}">REBRAND</div>
        </div>
        <div id="rebrand-error" style="padding:4px 16px;font-family:var(--font-mono);font-size:8px;color:#c55;display:none;"></div>
    </div>`,document.body.appendChild(p)}function Ki(){const o=document.getElementById("rebrand-overlay");o&&o.remove()}let co=!1;async function rs(o,e){if(!d||!I||co)return;const t=o||2e7,i=e||5,n=document.getElementById("rebrand-error"),r=(document.getElementById("rebrand-name")?.value||"").trim().replace(/[<>"]/g,""),a=(document.getElementById("rebrand-abbr")?.value||"").trim().toUpperCase().replace(/[<>"]/g,"");if(!r||r.length<2){n&&(n.textContent="Name must be at least 2 characters.",n.style.display="block");return}if(!a||a.length<2||a.length>5){n&&(n.textContent="Abbreviation must be 2-5 characters.",n.style.display="block");return}const s=Number(d.corp_cash_reserves??0);if(s<t){n&&(n.textContent="Insufficient cash. Need "+_(t)+".",n.style.display="block");return}co=!0;const l=document.getElementById("rebrand-btn");l&&(l.style.opacity="0.4",l.style.pointerEvents="none");const p=Number(d.corp_reputation??50),f=d.faction_name||"Corporation",{error:c}=await y.from("factions").update({faction_name:r,abbreviation:a,corp_ticker:a,corp_cash_reserves:s-t,corp_reputation:Math.max(0,p-i)}).eq("id",d.id);if(c){co=!1,n&&(n.textContent="Failed: "+c.message,n.style.display="block"),l&&(l.style.opacity="1",l.style.pointerEvents="auto");return}d.faction_name=r,d.abbreviation=a,d.corp_ticker=a,d.corp_cash_reserves=s-t,d.corp_reputation=Math.max(0,p-i);const v=I.current_tick||0,{error:m}=await y.from("event_log").insert({nation_id:d.nation_id,faction_id:d.id,event_name:"Corporation Rebranded",description_used:f+" has rebranded to "+r+" ("+a+"). The rebrand costs $20M and reputation takes a temporary hit.",category:"corporate",trigger_key:"corp_rebrand",effects_applied:{old_name:f,new_name:r,new_abbr:a,rep_loss:i,cost:t},fired_at_tick:v});m&&console.warn("Failed to log rebrand event:",m.message),co=!1,_t("rebrand"),Ki(),St(),document.getElementById("corp-name-bar").textContent=r;const u=document.getElementById("corp-logo");u&&(u.textContent=a.slice(0,2))}const ss={liberty:"#9C27B0",equality:"#E91E63",freedom:"#5b9bd5",security:"#d48a3c",individualism:"#eab308",collectivism:"#ec4899",tradition:"#795548",progress:"#00BCD4",nationalism:"#FF5722",globalism:"#3F51B5"};function st(o){return ss[(o||"").toLowerCase()]||"#9C27B0"}let Ve=[],ze=-1;async function Kn(){Number(d?.corp_cash_reserves??0);const o=[d.nation_id],e=new Set(ke.map(n=>n.id)),{data:t}=await y.from("factions").select("id, faction_name, abbreviation, party_color, party_funds, seats, momentum, nation, nation_id, leader_ideology, linked_user_id, ideology_value_1, ideology_value_2").eq("faction_type","party").in("nation_id",o).is("abandoned_at",null).order("seats",{ascending:!1});Ve=(t||[]).filter(n=>!e.has(n.id)).map(n=>({...n})),ze=-1;const i=document.createElement("div");i.id="donate-overlay",i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:600;display:flex;align-items:center;justify-content:center;",i.onclick=function(n){n.target===i&&Ji()},document.body.appendChild(i),Jn()}function Ji(){const o=document.getElementById("donate-overlay");o&&o.remove(),Ve=[],ze=-1}function ls(o){ze=o,Jn()}function Jn(){const o=document.getElementById("donate-overlay");if(!o)return;const e=Re("Lobbyist"),t=e?e.skill:50,i=Math.round(1e6*Zt(t)),n=1e5,r=Number(d?.corp_cash_reserves??0),a=ze>=0?Ve[ze]:null,s=r>=i;let l='<div onclick="event.stopPropagation()" style="width:540px;max-height:80vh;background:#f8f7f2;border:1px solid #e8e7e0;display:flex;flex-direction:column;overflow:hidden;">';l+=`<div style="padding:14px 20px;border-bottom:1px solid #e8e7e0;background:#faf9f5;flex-shrink:0;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
            <div style="display:flex;align-items:center;gap:10px;">
    ...
    </div>`,l+='<div style="flex:1;overflow-y:auto;padding:10px 0;">',l+='<div style="padding:0 20px 8px;font-family:var(--font-mono);font-size:10px;letter-spacing:1.5px;color:#6a6660;text-transform:uppercase;">Select a Party</div>',Ve.length===0&&(l+='<div style="padding:20px;text-align:center;font-family:var(--font-mono);font-size:12px;color:#6a6660;">No eligible parties found.</div>');for(let p=0;p<Ve.length;p++){const f=Ve[p],c=ze===p,v=f.party_color||"#8a6aaa",m=(f.momentum||0)>0?"#e8e4dc":"#c55";l+=`<div onclick="donateSelectParty(${p})" style="
            padding:10px 20px;
        ...
        ">
            <div style="display:flex;justify-content:space-between;align-items:center;">
                <div style="display:flex;align-items:center;gap:10px;">
                    ...
                    <div>
                        <div style="display:flex;align-items:center;gap:8px;">
                            <span style="font-size:14px;font-weight:600;color:${c?"#e8e4dc":"#9e9a92"};">${x(f.faction_name)}</span>
                        </div>
        ...
        <div style="display:flex;gap:16px;">
            ...
            <div><div style="font-family:var(--font-mono);font-size:9px;color:#6a6660;letter-spacing:0.8px;">CASH</div><div style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:${s?"#e8e4dc":"#c55"};">${_(r)}</div></div>
            ${a?`<div><div style="font-family:var(--font-mono);font-size:9px;color:#6a6660;letter-spacing:0.8px;">RECIPIENT</div><div style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:#e8e4dc;">${x(a.abbreviation||a.faction_name)}</div></div>`:""}
        </div>
        <div style="display:flex;gap:8px;">
    ...
    </div>`,r+='<div style="display:flex;flex:1;min-height:0;overflow:hidden;">',r+='<div style="width:300px;border-right:1px solid #e8e7e0;overflow-y:auto;flex-shrink:0;">',i.length===0&&(r+=`<div style="padding:30px 20px;text-align:center;">
        ...
        </div>`);for(let a=0;a<i.length;a++){const s=i[a],l=Te===a,p=bt(s.skill);r+=`<div onclick="esSelectCandidate(${a})" style="
            padding:10px 14px;
            border-bottom:1px solid #e8e7e0;
        ...
        ">
        ...
        </div>`}if(r+="</div>",r+='<div style="flex:1;overflow-y:auto;">',!n)r+=`<div style="padding:50px 24px;text-align:center;">
        ...
        </div>`;else{const a=n.required_salary*n.required_years,s=bt(n.skill);r+=`<div style="padding:20px;border-bottom:1px solid #e8e7e0;">
        ...
        </div>`,r+=`<div style="display:flex;gap:0;border-bottom:1px solid #e8e7e0;">
        ...
        </div>`,r+=`<div style="padding:12px 20px;border-bottom:1px solid #e8e7e0;">
            ...
            <div style="display:flex;gap:5px;flex-wrap:wrap;">`;for(const f of n.specializations||[]){const c=vo[f],v=f===e;r+=`<span style="font-family:var(--font-mono);font-size:10px;font-weight:700;padding:3px 10px;color:${v?"#000":c?.color||"#9e9a92"};background:${v?c?.color||"#5a8aaa":(c?.color||"#5a8aaa")+"10"};border:1px solid ${v?"transparent":(c?.color||"#5a8aaa")+"30"};">${x(f)}</span>`}r+="</div></div>",r+=`<div style="padding:12px 20px;border-bottom:1px solid #e8e7e0;">
        ...
        </div>`;const l=n.skill>=80?"EXCEPTIONAL":n.skill>=65?"STRONG":n.skill>=50?"COMPETENT":n.skill>=35?"DEVELOPING":"WEAK",p=n.skill>=80?"Elite talent. Actions have high success rate and reduced costs.":n.skill>=65?"Strong performer. Reliable outcomes across most actions.":n.skill>=50?"Adequate for the role. Outcomes are average.":n.skill>=35?"Below average. Actions may fail or cost more. Consider alternatives.":"Poor fit. High failure rates. Replacement recommended.";r+=`<div style="padding:12px 20px;">
        ...
        </div>`}r+="</div>",r+="</div>",r+=`<div style="padding:12px 20px;border-top:1px solid #e8e7e0;background:#faf9f5;flex-shrink:0;display:flex;justify-content:space-between;align-items:center;">
        <div style="display:flex;gap:14px;">`,n?r+=`<div><div style="font-family:var(--font-mono);font-size:9px;color:#6a6660;letter-spacing:0.8px;">CANDIDATE</div><div style="font-family:var(--font-mono);font-size:15px;font-weight:700;color:var(--text-bright,#f0efe6);">${x(n.first_name)} ${x(n.last_name)}</div></div>
            ...
            <div><div style="font-family:var(--font-mono);font-size:9px;color:#6a6660;letter-spacing:0.8px;">SALARY</div><div style="font-family:var(--font-mono);font-size:15px;font-weight:700;color:#c84;">${pt(n.required_salary)}/yr</div></div>`:r+='<div style="font-family:var(--font-mono);font-size:11px;color:#6a6660;">Select a candidate to hire</div>',r+=`</div>
        <div style="display:flex;gap:8px;">
    ...
    </div>`,r+='<div id="es-error" style="padding:5px 20px;font-family:var(--font-mono);font-size:10px;color:#c55;display:none;"></div>',o.innerHTML=r}function qo(){return G.reduce((e,t)=>{const i=Number(t.capacity||0),n=Number(t.condition||0)/100;return e+Math.floor(i*n)},0)+500}function us(o,e){const t=Xe.find(r=>r.id===o),i=Number(d?.[t.factionKey]??0),n=ae[o]+e;if(!(i+n<0)){if(e>0){const r=Xe.reduce((s,l)=>{const p=Number(d?.[l.factionKey]??0),f=l.id===o?n:ae[l.id];return s+p+f},0),a=qo();if(r>a)return}ae[o]=n,Lo()}}function vs(o){o?ae[o]=0:ae={general:0,skilled:0,innovative:0},Lo()}async function ys(){if(ti||!Object.values(ae).some(a=>a!==0))return;let e=0;for(const a of Xe){const s=ae[a.id];s>0&&(e+=s*Ln(a.multiplier)*.1)}const t=Number(d?.corp_cash_reserves??0);if(e>t){alert("Insufficient cash reserves. Hiring cost: "+_(e)+", available: "+_(t));return}const i=Xe.reduce((a,s)=>a+Number(d?.[s.factionKey]??0)+ae[s.id],0),n=qo();if(i>n){alert("Cannot hire beyond property capacity ("+n.toLocaleString()+"). You need more workplaces.");return}const r=e>0?`Confirm workforce changes?

Hiring fee: `+_(e)+" (deducted from cash reserves)":`Confirm workforce changes?

Firing workers — no cost.`;if(confirm(r)){ti=!0;try{const a={};for(const p of Xe){const f=Number(d?.[p.factionKey]??0);a[p.factionKey]=Math.max(0,f+ae[p.id])}e>0&&(a.corp_cash_reserves=Math.max(0,t-Math.round(e)));const{error:s}=await y.from("factions").update(a).eq("id",d.id);if(s)throw s;Object.assign(d,a),ae={general:0,skilled:0,innovative:0};const l=document.getElementById("topbar-cash");if(l){const p=Number(d.corp_cash_reserves??0);l.textContent="CASH: "+(p>=1e6?"$"+(p/1e6).toFixed(1)+"M":"$"+Math.round(p/1e3)+"k")}Lo()}catch(a){alert("Error: "+a.message)}finally{ti=!1}}}function Lo(){const o=document.getElementById("hf-card-container");if(!o)return;const e="'JetBrains Mono', monospace",t={bg:"#ffffff",surface:"#f8f7f2",card:"#faf9f5",border:"#e8e7e0",accent:"#8b9a6b",greenBright:"#5c5",red:"#c55",redDim:"#a44",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},i=Number(N?.minimum_wage??50),n=Number(N?.inflation??50),r=Number(N?.standard_of_living??50),a=i/100*48e3,s=(1+(n-50)/100*.5).toFixed(2),l=(1+(r-50)/100*.5).toFixed(2),p=N?.name||d?.nation||"Nation",f=Object.values(ae).some(h=>h!==0),c=qo();let v=0,m=0,u=0,g=0,b="";for(const h of Xe){const E=Number(d?.[h.factionKey]??0),T=ae[h.id],S=E+T,w=Ln(h.multiplier),C=T>0,M=E*w,k=S*w,z=k-M;v+=E,m+=S,u+=M,g+=k;const q=T!==0?C?"rgba(92,204,92,0.02)":"rgba(204,85,85,0.02)":"transparent";b+=`
    ...
    </div>`}function gs(){const o=document.getElementById("wf-summary-container");if(!o)return;const e="'JetBrains Mono', monospace",t={surface:"#f8f7f2",card:"#faf9f5",border:"#e8e7e0",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",blue:"#5a9abf",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660",red:"#c55"},i=(N?.name||d?.nation||"Nation").toUpperCase(),n=Number(N?.minimum_wage??50),r=Number(N?.inflation??50),a=Number(N?.standard_of_living??50),s=n/100*48e3,l=1+(r-50)/100*.5,p=1+(a-50)/100*.5,f=[{label:"General Workforce",mult:2,color:t.accent,key:"corp_general_workforce",countColor:t.text},{label:"Skilled Workforce",mult:3,color:t.gold,key:"corp_skilled_workforce",countColor:t.blue},{label:"Innovative Workforce",mult:6,color:t.orange,key:"corp_innovative_workforce",countColor:t.gold}];let c=0,v=0,m="";for(const u of f){const g=Number(d?.[u.key]??0),b=Math.round(s*u.mult*l*p),$=g*b;c+=g,v+=$,m+=`
    ...
    </div>`}let ft=[],le=null;const Eo={Basic:{color:"#6a6660",label:"BASIC"},Modern:{color:"#8b9a6b",label:"MODERN"},Sustainable:{color:"#5c5",label:"SUSTAINABLE"},Innovative:{color:"#c8a832",label:"INNOVATIVE"},Heritage:{color:"#c84",label:"HERITAGE"},Premium:{color:"#ca5",label:"PREMIUM"}};async function Xi(){if(!d?.nation_id)return;const{data:o,error:e}=await y.from("available_properties").select("*").eq("nation_id",d.nation_id).eq("status","available").order("price",{ascending:!0});if(e){console.warn("[Property] Failed to load marketplace:",e.message);return}const t=d?.corp_sector==="Construction";ft=(o||[]).filter(i=>t||i.type!=="warehouse").map(i=>({...i,adjusted_cost:i.price,adjusted_maintenance:i.monthly_maintenance}))}function Bo(){const o=document.getElementById("new-property-container");if(!o)return;const e="'JetBrains Mono', monospace",t={surface:"#f8f7f2",card:"#faf9f5",border:"#e8e7e0",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",redDim:"#a44",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"};(N?.name||d?.nation||"Nation").toUpperCase();const i=Number(N?.standard_of_living??50),n=Number(N?.gdp_growth??50),r=Number(N?.inflation??50),a=N?.capital||"Capital",s={capital:a,port:a+" Port",industrial:a+" Industrial Zone",suburban:a+" Suburbs",coastal:a+" Coast"};let l="";if(ft.length===0)l=`<div style="padding:20px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">No properties available in this market.<br>Improve GDP Growth and Standard of Living to unlock more.</div>`;else for(let p=0;p<ft.length;p++){const f=ft[p],c=le===p,v=Eo[f.style]||Eo.Basic,m=s[f.city_template]||a;l+=`
    ...
    </div>`}function xs(o){le=le===o?null:o,Bo()}let ni=!1;async function bs(){if(le===null||ni)return;const o=ft[le];if(!o)return;const e=Number(d?.corp_cash_reserves??0);if(o.adjusted_cost>e){alert(`Insufficient cash reserves.
...
All construction corporations in `+t+" can now bid on this project.")}catch(o){alert("Failed to submit project: "+o.message)}finally{ai=!1}}}function oa(){if(document.getElementById("cp-modal-overlay")?.remove(),!Zi)return;const o="'JetBrains Mono', monospace",e={surface:"#f8f7f2",card:"#faf9f5",border:"#e8e7e0",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",redDim:"#a44",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},t=ta(),i=Math.ceil(t.total/1e8*3),n=i>=4?e.gold:i>=3?e.greenBright:i>=2?e.accent:e.dim,r=Object.entries($t).map(([l,p])=>{const f=L.style===l;return`<div onclick="cpSetField('style','${l}')" style="padding:5px 6px;cursor:pointer;text-align:center;background:${f?p.color+"18":"transparent"};border:1px solid ${f?p.color+"44":e.border};">
    ...
    </div>`,document.body.appendChild(a);const s=document.getElementById("cp-name-input");s&&s.addEventListener("input",l=>{L.name=l.target.value}),a.addEventListener("click",l=>{l.target===a&&en()})}function ws(){const o=document.getElementById("cp-name-input");if(o&&(L.name=o.value),!L.name.trim()){alert("Please enter a building name.");return}$s()}window.cpClose=en;window.cpSetField=hs;window.cpSubmitFromModal=ws;window.npSelect=xs;window.npBuyProperty=bs;window.npOpenConstructionModal=_s;let wt=!1;async function ks(o){if(wt)return;const e=G.find(s=>s.id===o);if(!e)return;const t=1+(Number(N?.inflation??50)-50)/100*.3,i=Math.round((e.purchase_price||0)*.1*t),n=Number(d?.corp_cash_reserves??0);if(i>n){alert("Insufficient cash. Refurbishment costs "+_(i)+" (inflation-adjusted), you have "+_(n));return}if(e.condition>=95){alert("Property is already in excellent condition ("+e.condition+"%).");return}const r=5+Math.floor(Math.random()*21),a=Math.min(100,e.condition+r);if(confirm('Refurbish "'+e.name+`"?

Cost: `+_(i)+`
Expected improvement: +`+r+"% condition ("+e.condition+"% → "+a+"%)")){wt=!0;try{await y.from("corp_properties").update({condition:a}).eq("id",o);const s=Math.max(0,n-i);await y.from("factions").update({corp_cash_reserves:s}).eq("id",d.id),d.corp_cash_reserves=s;const l=document.getElementById("topbar-cash");l&&(l.textContent="CASH: "+(s>=1e6?"$"+(s/1e6).toFixed(1)+"M":"$"+Math.round(s/1e3)+"k")),await eo(),Oo(),alert("Refurbished! Condition: "+e.condition+"% → "+a+"%")}catch(s){alert("Refurbishment failed: "+s.message)}finally{wt=!1}}}async function Es(o){if(wt)return;const e=G.find(r=>r.id===o);if(!e)return;const t=1+(Number(N?.inflation??50)-50)/100*.3,i=(e.condition||50)/100,n=Math.round((e.purchase_price||0)*.6*i*t);if(confirm('Sell "'+e.name+`"?

Sale value: `+_(n)+" (60% × "+e.condition+`% condition × inflation)

The property will go back on the market for 6 ticks.
This cannot be undone.`)){wt=!0;try{await y.from("corp_properties").update({is_active:!1}).eq("id",o);const a=Number(d?.corp_cash_reserves??0)+n;await y.from("factions").update({corp_cash_reserves:a}).eq("id",d.id),d.corp_cash_reserves=a;const l=(await y.from("shard").select("current_tick").eq("name","Alpha Shard").single()).data?.current_tick||0;await y.from("available_properties").insert({nation_id:d.nation_id,catalog_id:e.catalog_id||null,name:e.name,type:e.type,style:e.style,capacity:e.capacity,price:Math.round(n*1.1),monthly_maintenance:e.monthly_maintenance,condition:e.condition,city:e.city,generated_at_tick:l,expires_at_tick:l+6,status:"available"});const p=document.getElementById("topbar-cash");p&&(p.textContent="CASH: "+(a>=1e6?"$"+(a/1e6).toFixed(1)+"M":"$"+Math.round(a/1e3)+"k")),await eo(),Oo(),await Xi(),Bo(),alert('Sold "'+e.name+'" for '+_(n))}catch(r){alert("Sale failed: "+r.message)}finally{wt=!1}}}window.propRefurbish=ks;window.propSell=Es;const Fe={SALE:.8,DISSOLVE:.6,REVENUE_BASE:.02,GDP_NEUTRAL:30,DEFAULT_REPUTATION:25};function Cs(o){if(!o)return 0;const e=o.trim().replace(/[$,]/g,""),t=e.match(/^([\d.]+)\s*[Mm]$/),i=e.match(/^([\d.]+)\s*[Kk]$/);return Math.round(t?parseFloat(t[1])*1e6:i?parseFloat(i[1])*1e3:parseFloat(e))}function tt(o){const e=document.getElementById("topbar-cash");e&&(e.textContent="CASH: "+(o>=1e6?"$"+(o/1e6).toFixed(1)+"M":"$"+Math.round(o/1e3)+"k"))}function ia(o){return It.find(e=>e.id===o)?.name||"—"}function Po(o){return G.filter(e=>e.nation_id===o)}async function to(){mt=0,await eo(),Oo(),oo(),io()}let se=!1,mt=0,fo={};async function Ts(){if(d?.id)try{const{data:o}=await y.from("construction_contracts").select("nation_id").eq("awarded_to_faction",d.id).in("status",["in_progress","awarded"]);fo={};for(const e of o||[])e.nation_id&&(fo[e.nation_id]=(fo[e.nation_id]||0)+1)}catch{}}function na(o){const e=Po(o.nation_id),t=e.reduce((u,g)=>u+Number(g.purchase_price||0),0),i=e.reduce((u,g)=>u+Number(g.capacity||0),0),n=fo[o.nation_id]||0,r=It.find(u=>u.id===o.nation_id),a=(o.name||"").trim().split(/\s+/),s=a.length>=2?a.map(u=>u[0]).join("").toUpperCase().slice(0,4):(o.name||"SUB").slice(0,4).toUpperCase(),l=Number(o.sub_cash||0),p=Number(r?.gdp_growth??50),f=l*Fe.REVENUE_BASE,c=(p-Fe.GDP_NEUTRAL)/100,v=Fe.DEFAULT_REPUTATION/100,m=l>0?Math.round(f*(1+c)*v):0;return{id:o.id,name:o.name,abbr:s,nation:r?.name||o.city||"—",nationId:o.nation_id,sector:d?.corp_sector||"General",subsector:o.subsector||d?.corp_subsector||"—",revenue:m,debt:0,cash:l,reputation:Fe.DEFAULT_REPUTATION,valuation:t,workforce:i,projects:n,established:o.created_at?new Date(o.created_at).getFullYear().toString():"—",trend:p>=40&&l>0?"up":p>=Fe.GDP_NEUTRAL&&l>0?"flat":"down",profitable:m>0,hqProp:o}}function oo(){const o=document.getElementById("manage-subsidiaries-container");if(!o)return;const e="'JetBrains Mono', monospace",t={surface:"#f8f7f2",card:"#faf9f5",border:"#e8e7e0",accent:"#8b9a6b",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",greenDark:"#2a5a3a",red:"#c55",redDim:"#a44",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},n=G.filter(f=>f.type==="regional_hq").map(na);mt>=n.length&&(mt=0);const r=n[mt]||null;let a="";n.length===0&&(a=`<div style="padding:30px 14px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">No subsidiaries established.<br>Use Create Subsidiary to expand.</div>`);let s=0,l=0;for(let f=0;f<n.length;f++){const c=n[f],v=f===mt;s+=c.revenue,l+=c.valuation;const m=c.trend==="up"?t.greenBright:c.trend==="down"?t.red:t.dim,u=c.trend==="up"?"▲":c.trend==="down"?"▼":"–";a+=`
...
Total received: `+_(v))}catch(m){alert("Failed: "+m.message)}finally{se=!1}}}function Ss(o){aa(o,"sell")}async function zs(o){if(se)return;const e=G.find(s=>s.id===o);if(!e)return;const t=ia(e.nation_id),n=Po(e.nation_id).reduce((s,l)=>s+Math.round((l.purchase_price||0)*.8*(l.condition||50)/100),0),r=Number(e.sub_cash||0),a=Math.round(r*.05);if(confirm('PUT UP FOR SALE: "'+e.name+`"
...
Other corporations will see it on the Expansion tab and can place bids.`),await to()}catch(s){alert("Failed: "+s.message)}finally{se=!1}}}let Co=[],ra="ready",Ot=null;async function Do(){const o=await za(y);Co=o.listings,ra=o.state,Ot=o.error,Ot&&console.error("[SubMarket] Load failed:",Ot.message)}function jo(){let o=document.getElementById("sub-marketplace-card");o||(o=document.createElement("div"),o.id="sub-marketplace-card",document.getElementById("expansion-content")?.appendChild(o));const e=Co.filter(l=>l.seller_faction_id!==d?.id),t=Co.filter(l=>l.seller_faction_id===d?.id),i="'JetBrains Mono',monospace",n=getComputedStyle(document.body),r=(l,p)=>n.getPropertyValue(l).trim()||p,a={surface:r("--bg-2","#ffffff"),card:r("--bg-3","#f0efeb"),border:r("--border-0","rgba(0,0,0,0.08)"),dim:r("--text-dim","#aaa"),muted:r("--text-muted","#888"),text:r("--text-primary","#333"),bright:r("--text-bright","#1a1a17"),orange:r("--orange","#d35400"),green:r("--green","#2d8a2d"),blue:r("--blue","#2874a6"),red:r("--red","#c0392b"),gold:r("--gold","#a88520")};let s=`<div style="width:760px;background:${a.surface};border:1px solid ${a.border};font-family:'IBM Plex Sans',sans-serif;">
        <div style="padding:10px 14px;border-bottom:1px solid ${a.border};display:flex;align-items:center;gap:8px;">
            ...
            <span style="font-family:${i};font-size:11px;font-weight:700;letter-spacing:1.5px;color:${a.orange};text-transform:uppercase;">Subsidiary Marketplace</span>
            <span style="font-family:${i};font-size:9px;color:${a.dim};">${e.length} available</span>
        </div>`;if(t.length>0){s+=`<div style="padding:8px 14px;border-bottom:1px solid ${a.border};background:${a.card};">
            <div style="font-family:${i};font-size:8px;letter-spacing:1px;color:${a.gold};text-transform:uppercase;margin-bottom:6px;">YOUR LISTINGS</div>`;for(const l of t){const f=(l.subsidiary_bids||[]).filter(c=>c.status==="pending");s+=`<div style="padding:6px 0;border-bottom:1px solid var(--border-hair);display:flex;justify-content:space-between;align-items:center;">
                <div>
                    <span style="font-size:11px;font-weight:700;color:${a.bright};">${x(l.subsidiary_name)}</span>
                    <span style="font-family:${i};font-size:8px;color:${a.dim};margin-left:6px;">${x(l.subsector||"")}</span>
                </div>
                <div style="display:flex;align-items:center;gap:8px;">
                    <span style="font-family:${i};font-size:9px;color:${f.length>0?a.green:a.dim};">${f.length} bid${f.length!==1?"s":""}</span>
                    ${f.length>0?`<span onclick="subViewBids('${l.id}')" style="font-family:${i};font-size:8px;font-weight:700;padding:3px 8px;color:${a.green};border:1px solid ${a.green}44;cursor:pointer;">VIEW BIDS</span>`:""}
                    <span onclick="subCancelSale('${l.id}')" style="font-family:${i};font-size:8px;font-weight:700;padding:3px 8px;color:${a.red};border:1px solid ${a.red}44;cursor:pointer;">CANCEL</span>
                </div>
            </div>`}s+="</div>"}if(ra==="error")s+=`<div style="padding:24px 14px;text-align:center;font-family:${i};font-size:10px;color:${a.red};font-style:italic;">${x(Ot&&Ot.message||"Subsidiary marketplace is temporarily unavailable.")}</div>`;else if(e.length===0)s+=`<div style="padding:24px 14px;text-align:center;font-family:${i};font-size:10px;color:${a.dim};font-style:italic;">No subsidiaries for sale right now.</div>`;else for(const l of e){const p=(l.subsidiary_bids||[]).find(v=>v.bidder_faction_id===d?.id&&v.status==="pending"),c=(_allNations||[]).find(v=>v.id===l.nation_id)?.name||"Unknown";s+=`<div style="padding:10px 14px;border-bottom:1px solid ${a.border};">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                    <div style="display:flex;align-items:center;gap:8px;">
                        <span style="font-size:12px;font-weight:700;color:${a.bright};">${x(l.subsidiary_name)}</span>
                        <span style="font-family:${i};font-size:7px;font-weight:700;padding:1px 5px;color:${a.orange};border:1px solid ${a.orange}44;background:${a.orange}0a;">${x(l.subsector||"General")}</span>
                    </div>
                    <span style="font-family:${i};font-size:8px;color:${a.dim};">${x(c)}</span>
                </div>
                <div style="display:flex;gap:16px;font-family:${i};font-size:8px;color:${a.muted};margin-bottom:8px;">
                ...
                <div style="display:flex;justify-content:flex-end;">
                    ${p?`<span style="font-family:${i};font-size:8px;font-weight:700;color:${a.green};">✓ BID PLACED: ${_(p.bid_amount)}</span>`:`<span onclick="subPlaceBid('${l.id}','${x(l.subsidiary_name)}',${l.valuation})" style="font-family:${i};font-size:8px;font-weight:700;padding:4px 14px;color:#000;background:${a.orange};cursor:pointer;">PLACE BID</span>`}
                </div>
            </div>`}s+="</div>",o.innerHTML=s}async function Is(o,e,t){const i=prompt('Place bid for "'+e+`"

Valuation: `+_(t)+`
Minimum bid: $1M

Enter bid amount ($):`);if(!i)return;const n=Math.round(Number(i));if(isNaN(n)||n<1e6){alert("Minimum bid is $1,000,000.");return}const r=Number(d?.corp_cash_reserves??0);if(n>r){alert("Insufficient funds. You have "+_(r)+".");return}const{error:a}=await y.from("subsidiary_bids").insert({sale_id:o,bidder_faction_id:d.id,bid_amount:n,status:"pending",placed_at_tick:I?.current_tick||0});if(a){a.message.includes("duplicate")||a.message.includes("unique")?alert("You already have a bid on this subsidiary."):alert("Failed to place bid: "+a.message);return}alert("Bid of "+_(n)+' placed on "'+e+`".
The seller will review your bid.`),await Do(),jo()}async function Ns(o){const e=Co.find(v=>v.id===o);if(!e)return;const t=(e.subsidiary_bids||[]).filter(v=>v.status==="pending");if(t.length===0){alert("No pending bids.");return}const i=t.map(v=>v.bidder_faction_id),{data:n}=await y.from("factions").select("id, faction_name").in("id",i),r={};(n||[]).forEach(v=>{r[v.id]=v.faction_name});let a='Bids for "'+e.subsidiary_name+`":
...
This cannot be undone.`)&&await As(e,f)}let ri=!1;async function As(o,e){if(!ri){ri=!0;try{const n=I?.current_tick||0,{data:r}=await y.from("factions").select("corp_cash_reserves").eq("id",e.bidder_faction_id).single(),a=Number(r?.corp_cash_reserves??0);if(a<e.bid_amount){alert("Buyer has insufficient funds. Bid cannot be completed."),await y.from("subsidiary_bids").update({status:"rejected",resolved_at_tick:n}).eq("id",e.id);return}var{error:t}=await y.from("factions").update({corp_cash_reserves:a-e.bid_amount}).eq("id",e.bidder_faction_id);if(t){alert("Failed to deduct from buyer: "+t.message);return}const s=Number(d?.corp_cash_reserves??0);var{error:i}=await y.from("factions").update({corp_cash_reserves:s+e.bid_amount}).eq("id",d.id);if(i){await y.from("factions").update({corp_cash_reserves:a}).eq("id",e.bidder_faction_id),alert("Failed to credit seller: "+i.message);return}d.corp_cash_reserves=s+e.bid_amount,await y.from("corp_properties").update({faction_id:e.bidder_faction_id}).eq("id",o.subsidiary_id);const l=G.filter(p=>p.nation_id===o.nation_id&&p.faction_id===d.id);for(const p of l)await y.from("corp_properties").update({faction_id:e.bidder_faction_id}).eq("id",p.id);await y.from("subsidiary_sales").update({status:"completed",completed_at_tick:n,accepted_bid_id:e.id}).eq("id",o.id),await y.from("subsidiary_bids").update({status:"accepted",resolved_at_tick:n}).eq("id",e.id),await y.from("subsidiary_bids").update({status:"rejected",resolved_at_tick:n}).eq("sale_id",o.id).neq("id",e.id),tt(d.corp_cash_reserves),alert("Sale complete! Received "+_(e.bid_amount)+`.

"`+o.subsidiary_name+'" has been transferred to the buyer.'),await to(),await Do(),jo()}catch(n){console.error("[SubMarket] Accept bid error:",n),alert("Transfer failed: "+n.message)}finally{ri=!1}}}async function Ms(o){if(!confirm("Cancel this listing? The subsidiary will no longer be for sale."))return;const{error:e}=await y.from("subsidiary_sales").update({status:"cancelled"}).eq("id",o);if(e){alert("Failed: "+e.message);return}await Do(),jo()}function Rs(o){aa(o,"dissolve")}async function sa(o,e){if(se)return;const t=G.find(c=>c.id===o);if(!t)return;const i=Number(d?.corp_cash_reserves??0),n=Number(t.sub_cash||0),r=e?"WITHDRAW":"INJECT CAPITAL";if(e&&n<=0){alert("This subsidiary has no cash to withdraw.");return}const a=e?n:i,s=prompt(r+(e?" from ":" into ")+t.name+`

Parent cash: `+_(i)+`
Subsidiary cash: `+_(n)+`

Enter amount (e.g., 5000000 or 5M):`);if(!s)return;const l=Cs(s);if(!l||l<=0||isNaN(l)){alert("Invalid amount.");return}if(l>a){alert("Insufficient "+(e?"subsidiary":"parent")+" cash. Available: "+_(a));return}const p=e?i+l:i-l,f=e?n-l:n+l;if(confirm(r+" "+_(l)+(e?" from ":" into ")+t.name+`?

Parent: `+_(i)+" → "+_(p)+`
Subsidiary: `+_(n)+" → "+_(f))){se=!0;try{await Promise.all([y.from("factions").update({corp_cash_reserves:p}).eq("id",d.id),y.from("corp_properties").update({sub_cash:f}).eq("id",o)]),d.corp_cash_reserves=p,t.sub_cash=f,tt(p),oo(),alert((e?"Withdrew ":"Injected ")+_(l)+(e?" from ":" into ")+t.name+".")}catch(c){alert("Failed: "+c.message)}finally{se=!1}}}function qs(o){sa(o,!1)}function Ls(o){sa(o,!0)}async function Os(o){if(se)return;const e=G.find(g=>g.id===o);if(!e)return;const t=na(e);t.nation;const i=Po(e.nation_id),n=t.valuation,r=t.cash,a=t.reputation,s=t.subsector,l=Math.round(n*2.25),p=Math.round(a*.1),f=Math.round(a*.2),c=qo(),v=Xe.reduce((g,b)=>g+Number(d?.[b.factionKey]??0),0),m=Math.max(0,c-v),u=Number(d?.corp_cash_reserves??0);if(l>u){alert(`Insufficient cash to acquire subsidiary.
...
This cannot be undone.`)){se=!0;try{const g=d.nation_id;if(i.length>0){const C=i.filter(k=>k.id!==e.id).map(k=>k.id);if(C.length===1){const{error:k}=await y.from("corp_properties").update({nation_id:g,type:"office"}).eq("id",C[0]);if(k)throw k}else if(C.length>1){const{error:k}=await y.from("corp_properties").update({nation_id:g,type:"office"}).in("id",C);if(k)throw k}const{error:M}=await y.from("corp_properties").update({nation_id:g,type:"office",sub_cash:0,subsector:null}).eq("id",e.id);if(M)throw M}const b=u-l+r,h=Number(d?.corp_general_workforce??0)+m,E=Math.random()>=.5?p:-f,T=Number(d?.standing??50),S=Math.max(0,Math.min(100,T+E)),{error:w}=await y.from("factions").update({corp_cash_reserves:b,corp_general_workforce:h,standing:S}).eq("id",d.id);if(w)throw w;d.corp_cash_reserves=b,d.corp_general_workforce=h,d.standing=S,tt(b),await to(),alert(`Merger complete!
...
Properties: `+i.length+" transferred to parent")}catch(g){alert("Merge failed: "+g.message)}finally{se=!1}}}window.subDissolve=Rs;window.subInjectCapital=qs;window.subWithdraw=Ls;window.subMerge=Os;window.subSell=Ss;window.subPutForSale=zs;window.subPlaceBid=Is;window.subViewBids=Ns;window.subCancelSale=Ms;window.selectSubsidiary=function(o){mt=o,oo()};let It=[],Bt={},ve=null,si=!1,ot="",Yt="",it="",Me="";const la={Construction:4,Finance:5,Shipping:4},Bs=["Construction","Shipping","Finance"],da={Construction:[{id:"civil",name:"Civil Engineering",mod:0},{id:"industrial",name:"Industrial Construction",mod:.25},{id:"mega",name:"Megaprojects",mod:.4}],Shipping:[{id:"bulk_cargo",name:"Bulk Cargo",mod:0},{id:"container_freight",name:"Container Freight",mod:.2},{id:"specialized_transport",name:"Specialized Transport",mod:.35}],Finance:[{id:"banking",name:"Banking",mod:0},{id:"insurance",name:"Insurance",mod:.15},{id:"investment",name:"Investment Management",mod:.3}],Technology:[{id:"software",name:"Software Development",mod:0},{id:"hardware",name:"Hardware Manufacturing",mod:.2},{id:"telecom",name:"Telecommunications",mod:.35}],Energy:[{id:"oil_gas",name:"Oil & Gas",mod:0},{id:"renewables",name:"Renewables",mod:.2},{id:"mining",name:"Mining",mod:.3}],Healthcare:[{id:"pharma",name:"Pharmaceuticals",mod:0},{id:"hospitals",name:"Hospital Systems",mod:.2},{id:"biotech",name:"Biotechnology",mod:.35}]};async function Ps(){const{data:o,error:e}=await y.from("nations").select("*").order("name");e&&console.warn("[Subsidiary] Failed to load nations:",e.message),It=(o||[]).filter(i=>i.id!==d?.nation_id);const{data:t}=await y.from("factions").select("nation_id").eq("faction_type","corporation").is("abandoned_at",null);Bt={};for(const i of t||[])i.nation_id&&(Bt[i.nation_id]=(Bt[i.nation_id]||0)+1);it=d?.corp_sector||"",Me=d?.corp_subsector||""}function ca(){const o=it||d?.corp_sector||"";return da[o]||[{id:"general",name:o||"General",mod:0}]}function Ds(o){it=o;const e=da[o];Me=e?e[0].name:"",io()}function pa(){const o=d?.corp_sector||"";return it===o?1:la[it]||4}function js(){const e=ca().find(t=>t.name===Me);return e?e.mod:0}function Si(o){const e=Number(o.standard_of_living??50);return Math.max(.5,Math.round(e/50*100)/100)}function fa(o){const t=pa(),i=1+js(),n=Si(o);return Math.round(Math.max(1e7,5e7*t*i*n))}function Fs(o){const e=Bt[o]||0;return e<=1?{label:"HIGH",color:"#5c5"}:e<=3?{label:"MODERATE",color:"#ca5"}:{label:"LOW",color:"#c55"}}function Us(o){if(ve=ve===o?null:o,ve){const e=It.find(t=>t.id===ve);ot=(d?.faction_name||"Subsidiary")+" "+(e?.name||"")}else ot="";io()}function Hs(o){Me=o,io()}function Gs(o){ot=o}function Vs(o){Yt=o.toUpperCase().slice(0,4)}async function Ws(){if(si||!ve)return;const o=It.find(a=>a.id===ve);if(!o)return;const e=(ot||"").trim(),t=(Yt||"").trim();if(!e){alert("Please enter a corporation name for the subsidiary.");return}if(t.length<2){alert("Please enter an abbreviation (2-4 chars).");return}if(G.find(a=>a.nation_id===o.id&&a.type==="regional_hq")){alert("You already have a subsidiary in "+o.name);return}const n=fa(o),r=Number(d?.corp_cash_reserves??0);if(n>r){alert("Insufficient cash. Entry cost: "+_(n)+", available: "+_(r));return}if(confirm("Establish subsidiary in "+o.name+`?
...
Deducted from cash reserves.`)){si=!0;try{const s=(await y.from("shard").select("current_tick").eq("name","Alpha Shard").single()).data?.current_tick||0,l=85+Math.floor(Math.random()*16),p=Math.round(n*.005),{error:f}=await y.from("corp_properties").insert({faction_id:d.id,nation_id:o.id,name:e,type:"regional_hq",style:"Modern",capacity:500,purchase_price:n,monthly_maintenance:p,condition:l,city:o.capital||o.name,purchased_at_tick:s,is_active:!0,subsector:Me||d?.corp_subsector||null});if(f)throw f;const c=Math.max(0,r-n);await y.from("factions").update({corp_cash_reserves:c}).eq("id",d.id),d.corp_cash_reserves=c,tt(c);const v=it||d?.corp_sector||"Unknown";try{await y.from("event_log").insert({nation_id:o.id,event_name:"New Subsidiary Established",category:"corporate",description_chosen:`${d.faction_name} has invested ${_(n)} to establish ${e}, a new ${v} corporation in ${o.name}.`,fired_at_tick:I?.current_tick||0})}catch{}try{const{data:m}=await y.from("nations").select("gdp_growth").eq("id",o.id).single();m&&await y.from("nations").update({gdp_growth:Math.min(100,Number(m.gdp_growth||50)+.2)}).eq("id",o.id)}catch{}ve=null,ot="",Yt="",await to(),alert('Subsidiary "'+e+'" established in '+o.name+`!

Cost: `+_(n)+`
Regional HQ created with `+l+"% condition.")}catch(a){alert("Failed: "+a.message)}finally{si=!1}}}function io(){const o=document.getElementById("create-subsidiary-container");if(!o)return;const e="'JetBrains Mono', monospace",t={bg:"#ffffff",surface:"#f8f7f2",card:"#faf9f5",border:"#e8e7e0",accent:"#8b9a6b",accentBright:"#a3b07e",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},i=d?.corp_sector||"General",n=d?.corp_subsector||"",r=ca(),a=r.find(k=>k.name===Me)||r[0],s=new Set(G.filter(k=>k.type==="regional_hq").map(k=>k.nation_id)),l=It.filter(k=>!s.has(k.id)),p=ve?l.find(k=>k.id===ve):null,f=ot.trim().length>0&&Yt.trim().length>=2&&p!==null,c=it||i,v=pa();let m=`
    <div style="padding:6px 14px;border-bottom:1px solid ${t.border};">
        ...
        <div style="display:flex;gap:3px;">
            ${Bs.map(k=>{const z=k===c,q=k===i,P=q?1:la[k]||4,U=q?t.greenBright:t.orange;return`<div onclick="subSetSector('${k}')" style="flex:1;padding:5px 4px;text-align:center;cursor:pointer;background:${z?t.accent+"18":"transparent"};border:1px solid ${z?t.accent+"44":t.border};">
    ...
    <div style="padding:6px 14px;border-bottom:1px solid ${t.border};">
        ...
        <div style="display:flex;gap:3px;">
            ${r.map(k=>{const z=k.name===Me,q=k.name===n;return`<div onclick="subSetSubsector('${k.name.replace(/'/g,"\\'")}')" style="flex:1;padding:5px 4px;text-align:center;cursor:pointer;background:${z?t.accent+"18":"transparent"};border:1px solid ${z?t.accent+"44":t.border};">
    ...
    </div>`,g="";if(l.length===0)g=`<div style="padding:20px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">Subsidiaries in all available nations.</div>`;else for(const k of l){const z=k.id===ve,q=Fs(k.id),P=Bt[k.id]||0,U=Math.round(Number(k.standard_of_living??50)),F=Si(k);g+=`
    ...
    </div>`;const E=5e7,T=a.mod,S=p?Si(p):null,w=p?fa(p):null,C=Math.round(E*v*(1+T));let M=`
    ...
    </div>`}window.subSelectNation=Us;window.subCreate=Ws;window.subSetName=Gs;window.subSetAbbr=Vs;window.subSetSector=Ds;window.subSetSubsector=Hs;let Pt=[],We=0,To=JSON.parse(localStorage.getItem("nationhood_investigated_corps")||"{}"),xe="ALL",De="REPUTATION";async function Ys(){const[o,e]=await Promise.all([y.from("factions").select("id, faction_name, abbreviation, corp_sector, corp_subsector, corp_company_type, corp_ticker, corp_cash_reserves, corp_reputation, nation_id, nation, linked_user_id").eq("faction_type","corporation").is("abandoned_at",null).order("faction_name"),y.from("corp_properties").select("id, faction_id, name, nation_id, subsector, type, factions(faction_name, corp_sector, corp_ticker, abbreviation, corp_reputation, corp_company_type, linked_user_id)").eq("type","regional_hq").eq("is_active",!0)]),t={};for(const a of o.data||[])t[a.id]=a;const i=(o.data||[]).map(a=>{const s=(a.corp_company_type||"Private").toUpperCase(),l=Number(a.corp_cash_reserves||0);return{...a,abbr:a.corp_ticker||a.abbreviation||a.faction_name?.slice(0,4).toUpperCase()||"???",status:s,isPlayer:!!a.linked_user_id,reputation:Math.round(Number(a.corp_reputation??50)),revenue:Math.round(l*.1),valuation:Math.round(l*3),_isSub:!1}}),{data:n}=await y.from("nations").select("id, name"),r={};(n||[]).forEach(a=>{r[a.id]=a.name});for(const a of e.data||[]){const s=t[a.faction_id];if(!s)continue;const l=(s.corp_company_type||"Private").toUpperCase();i.push({id:a.id,faction_name:a.name||"Subsidiary",abbreviation:s.abbreviation,corp_sector:s.corp_sector,corp_subsector:a.subsector||s.corp_subsector,corp_ticker:s.corp_ticker,nation_id:a.nation_id,nation:r[a.nation_id]||"?",abbr:(s.corp_ticker||s.abbreviation||"??").slice(0,4),status:l,isPlayer:!!s.linked_user_id,reputation:Math.round(Number(s.corp_reputation??50)),revenue:0,valuation:0,_isSub:!0,_parentName:s.faction_name})}Pt=i}function Qs(o){We=o,no()}function Ks(o){xe=o,We=0,no()}function Js(o){De=o,We=0,no()}async function Xs(o){if(!d||!I)return;const e=Number(d.corp_cash_reserves??0);if(e<5e5){alert("Insufficient cash. Need $500k.");return}const{error:t}=await y.from("factions").update({corp_cash_reserves:e-5e5}).eq("id",d.id);if(t){alert("Failed: "+t.message);return}d.corp_cash_reserves=e-5e5,To[o]=!0,localStorage.setItem("nationhood_investigated_corps",JSON.stringify(To));const{data:i}=await y.from("factions").select("corp_cash_reserves, corp_loans, corp_reputation, corp_general_workforce, corp_skilled_workforce, corp_innovative_workforce").eq("id",o).single();if(i){const n=Pt.find(r=>r.id===o);if(n){Object.assign(n,i);const r=Number(i.corp_cash_reserves||0);n.reputation=Math.round(Number(i.corp_reputation??50)),n.revenue=Math.round(r*.1),n.valuation=Math.round(r*3)}}no()}function no(){const o=document.getElementById("corporations-container");if(!o)return;const e="'JetBrains Mono', monospace",t={surface:"#f8f7f2",card:"#faf9f5",border:"#e8e7e0",accent:"#8b9a6b",blue:"#5a8aaa",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},i={PUBLIC:{color:t.greenBright,bg:"rgba(92,204,92,0.06)",border:"rgba(92,204,92,0.15)"},PRIVATE:{color:t.gold,bg:"rgba(200,168,50,0.08)",border:"rgba(200,168,50,0.2)"},STATE:{color:t.orange,bg:"rgba(204,136,68,0.06)",border:"rgba(204,136,68,0.15)"}},n=[...new Set(Pt.map(m=>m.nation).filter(Boolean))];let r=[...Pt];xe!=="ALL"&&(r=r.filter(m=>m.nation===xe)),De==="REPUTATION"?r.sort((m,u)=>(u.reputation||0)-(m.reputation||0)):De==="REVENUE"?r.sort((m,u)=>(u.revenue||0)-(m.revenue||0)):De==="VALUATION"&&r.sort((m,u)=>(u.valuation||0)-(m.valuation||0)),We>=r.length&&(We=0);const a=r[We]||null;I?.current_tick;const s=a&&!!To[a.id],l=a&&a.status==="PRIVATE"&&!s,p=a&&a.status==="STATE";let f="";r.length===0&&(f=`<div style="padding:30px;text-align:center;font-family:${e};font-size:10px;color:${t.dim}">No corporations found.</div>`);for(let m=0;m<r.length;m++){const u=r[m],g=m===We,b=i[u.status]||i.PRIVATE,$=u.status==="PRIVATE"&&!To[u.id];f+=`
    ...
    <div style="width:760px;height:450px;background:${t.surface};border:1px solid ${t.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
    ...
    </div>`}window.corpSelect=Qs;window.corpInvestigate=Xs;window.corpFilterNation=Ks;window.corpSort=Js;let de=null,Ie={},K=120,Ne=15,zi={},ut=[],Ye=[],kt={};async function Zs(){if(!Je)return;if(Et[Je.id]){alert("You already have a bid on this contract. Retract it first if you want to re-bid.");return}de=Je,zi={};try{const{data:t}=await y.from("corp_material_inventory").select("material_key, quantity").eq("faction_id",d.id);for(const i of t||[])zi[yo(i.material_key)]=Number(i.quantity||0)}catch{}ut=[];try{const{data:t}=await y.from("contract_bids").select("faction_id, bid_price, estimated_quality, status, factions(faction_name, corp_ticker)").eq("contract_id",de.id).in("status",["pending","won"]);ut=(t||[]).filter(i=>i.faction_id!==d?.id).map(i=>({name:i.factions?.faction_name||"Unknown",ticker:i.factions?.corp_ticker||"???",price:Number(i.bid_price||0),quality:Number(i.estimated_quality||0),status:i.status}))}catch{}Ye=[],kt={};try{const{data:t,error:i}=await y.rpc("get_project_permit_requirements",{p_contract_id:de.id,p_faction_id:d.id,p_nation_id:de.nation_id});if(i)throw i;Ye=Array.isArray(t)?t:[];const n=Ye.map(r=>r.permit_key).filter(Boolean);if(n.length>0){const{data:r,error:a}=await y.from("construction_permits").select("permit_key, cost, processing_ticks").in("permit_key",n);if(a)throw a;for(const s of r||[])kt[s.permit_key]={cost:Number(s.cost||0),ticks:Number(s.processing_ticks||0)}}}catch(t){console.warn("Failed to load project permit requirements",t),Ye=[],kt={}}Ie={};const o=de.required_materials||{};for(const t of Object.keys(o))Ie[t]="STD";const e=de.required_workforce||{};K=Number(e.general||0)+Number(e.skilled||0)||120,Ne=15,Xt(),Fo()}function tn(){document.getElementById("bid-assembly-overlay")?.remove(),de=null,Ye=[],kt={}}function el(o,e){Ie[o]=e,Fo()}function tl(o){K=o,Fo()}function ol(o){Ne=o,Fo()}function Fo(){if(document.getElementById("bid-assembly-overlay")?.remove(),!de)return;const o="'JetBrains Mono', monospace",e={surface:"#f8f7f2",card:"#faf9f5",border:"#e8e7e0",accent:"#8b9a6b",accentBright:"#a3b07e",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",redDim:"#a44",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},t=de,i=t.issuer_type==="GOVERNMENT",n=N?.name||d?.nation||"—",r=Number(t.budget_ceiling||0),a=Number(t.timeline_ticks||8),s=t.required_materials||{},l=Object.keys(s),p={LOW:.5,STD:1,HIGH:2},f={LOW:e.orange,STD:e.yellow,HIGH:e.greenBright},c={LOW:"Low",STD:"Standard",HIGH:"High"},v={concrete:36e4,steel:5e5,glass_facades:56e4,em_systems:64e4,lumber:24e4,heavy_parts:8e5,aggregate:16e4,asphalt:28e4},m=zi||{};let u=0,g="";for(const O of l){const W=Number(s[O]||0),an=Ie[O]||"STD",rn=v[O]||3e5,_a=p[an],ha=Math.round(rn*_a),sn=W*ha;u+=sn;const $a=O.replace(/_/g," ").replace(/\b\w/g,Le=>Le.toUpperCase()),ln=Number(m[O]||0),Go=Math.max(0,W-ln),wa=Go===0?e.greenBright:Go<W?e.yellow:e.red,ka=Go===0?"✓ IN STOCK":`${ln}/${W}`;g+=`
        <div style="display:flex;align-items:center;padding:5px 14px;border-bottom:1px solid ${e.border};">
            ...
            <div style="flex:1.2;display:flex;gap:2px;justify-content:center;">
                ${["LOW","STD","HIGH"].map(Le=>{const Vo=an===Le,dn=f[Le],Ea=_(Math.round(rn*p[Le]));return`<span onclick="bidSetGrade('${O}','${Le}')" style="padding:2px 6px;font-family:${o};font-size:7px;font-weight:700;cursor:pointer;color:${Vo?"#000":e.dim};background:${Vo?dn:"transparent"};border:1px solid ${Vo?dn:e.border}" title="${Ea}/unit">${c[Le]}</span>`}).join("")}
            </div>
    ...
    </div>`,document.body.appendChild(rt)}let li=!1;async function il(){if(li||!de)return;const o=de,e=o.required_materials||{},t=Object.keys(e),i=Number(o.budget_ceiling||0),n=Number(o.timeline_ticks||8),r={concrete:36e4,steel:5e5,glass_facades:56e4,em_systems:64e4,lumber:24e4,heavy_parts:8e5,aggregate:16e4,asphalt:28e4},a={LOW:.5,STD:1,HIGH:2};let s=0;for(const M of t){const k=Number(e[M]||0),z=Ie[M]||"STD",q=r[M]||3e5;s+=k*Math.round(q*a[z])}const l=15200,p=o.required_workforce||{},f=Number(p.general||0)+Number(p.skilled||0)||100,c=Math.max(40,Math.round(f*.5)),v=f*2,m=Math.max(0,Math.min(1,(K-c)/(v-c||1))),u=Math.round(4.5-m*8),g=Math.max(Math.round(n*.6),n+u),b=K*l*g,$=(Ye||[]).filter(M=>!M.has_permit).reduce((M,k)=>M+Number(kt[k.permit_key]?.cost||0),0),E=s+b+$+4e5,T=Math.round(E*(Ne/100)),S=E+T;if(S>i){alert("Bid exceeds budget ceiling. Reduce costs or markup.");return}const w=Object.values(Ie),C=w.length>0?Math.round(w.reduce((M,k)=>M+(k==="HIGH"?85:k==="STD"?65:45),0)/w.length):50;if(confirm('Submit bid for "'+o.name+`"?
...
Once submitted, your bid cannot be changed.`)){li=!0;try{const{data:M}=await y.from("shard").select("current_tick").eq("name","Alpha Shard").single(),k=M?.current_tick||0,z={};for(const P of t)z[P]=Ie[P]||"STD";const{error:q}=await y.from("contract_bids").insert({contract_id:o.id,faction_id:d.id,bid_price:S,material_grades:z,labor_count:K,markup_pct:Ne,estimated_cost:E,estimated_quality:C,status:"pending",submitted_at_tick:k});if(q)throw q;o.status==="open"&&await y.from("construction_contracts").update({status:"bidding"}).eq("id",o.id).eq("status","open"),tn(),alert(`Bid submitted successfully!
...
Lowest qualified bid wins.`),typeof Ae=="function"&&await Ae()}catch(M){alert("Bid submission failed: "+M.message)}finally{li=!1}}}window.openBidAssembly=Zs;window.closeBidAssembly=tn;window.bidSetGrade=el;window.bidSetWorkers=tl;window.bidSetMarkup=ol;window.submitBidAssembly=il;let di=!1;async function nl(o){if(di)return;const e=1e6,t=Number(d?.corp_cash_reserves??0);if(t<e){alert("Cannot retract — you need at least $1M in cash reserves to cover the retraction penalty.");return}if(confirm(`Retract your bid?

This will cost $1M as a retraction penalty.
This action cannot be undone.`)){di=!0;try{const i=t-e,{error:n}=await y.from("factions").update({corp_cash_reserves:i}).eq("id",d.id);if(n)throw n;const{error:r}=await y.from("contract_bids").delete().eq("contract_id",o).eq("faction_id",d.id);if(r)throw r;d.corp_cash_reserves=i,typeof tt=="function"&&tt(i),alert("Bid retracted. $1M penalty applied."),Xt(),await Ae()}catch(i){alert("Failed to retract bid: "+(i.message||"Unknown error"))}finally{di=!1}}}window.retractBid=nl;let Qt=[],Qe=0,ye=null,ci=!1,pi=!1,fi=!1;async function al(){if(!Je||pi)return;pi=!0,ye=Je,Qe=0;const{data:o,error:e}=await y.from("contract_bids").select("*, factions(faction_name, corp_ticker, corp_subsector)").eq("contract_id",ye.id).in("status",["pending","won"]).order("bid_price",{ascending:!0});if(pi=!1,e){alert("Failed to load bids: "+e.message);return}Qt=(o||[]).map(t=>({...t,corp:t.factions?.faction_name||"Unknown",abbr:t.factions?.corp_ticker||"???",subsector:t.factions?.corp_subsector||"—"})),Xt(),ma()}function Uo(){document.getElementById("bid-review-overlay")?.remove(),ye=null}function rl(o){Qe=o,ma()}async function sl(){if(ci||Qt.length===0)return;const o=Qt[Qe];if(!(!o?.id||!o.faction_id)&&confirm("Accept bid from "+o.corp+`?

Bid Price: `+_(o.bid_price)+`
Quality: `+o.estimated_quality+`/100
Workers: `+o.labor_count+`

This will award the contract. The project begins immediately.`)){ci=!0;try{const{data:e}=await y.from("shard").select("current_tick").eq("name","Alpha Shard").single(),t=e?.current_tick||0,{error:i}=await y.from("contract_bids").update({status:"won"}).eq("id",o.id);if(i)throw i;const{error:n}=await y.from("contract_bids").update({status:"lost"}).eq("contract_id",ye.id).neq("id",o.id);if(n)throw n;const{error:r}=await y.from("construction_contracts").update({status:"awarded",awarded_to_faction:o.faction_id,awarded_at_tick:t}).eq("id",ye.id);if(r)throw r;Uo(),alert("Contract awarded to "+o.corp+`!

Bid: `+_(o.bid_price)+`
Project begins immediately.`),typeof Ae=="function"&&await Ae()}catch(e){alert("Failed to accept bid: "+(e.message||e))}finally{ci=!1}}}async function ll(){if(!(!ye||fi)&&confirm(`Decline all bids and cancel this project?

No bids will be accepted. The contract will be removed.`)){fi=!0;try{const{error:o}=await y.from("contract_bids").update({status:"lost"}).eq("contract_id",ye.id);if(o)throw o;const{error:e}=await y.from("construction_contracts").update({status:"expired"}).eq("id",ye.id);if(e)throw e;Uo(),alert("All bids declined. Contract cancelled."),typeof Ae=="function"&&await Ae()}catch(o){alert("Failed: "+(o.message||o))}finally{fi=!1}}}function ma(){if(document.getElementById("bid-review-overlay")?.remove(),!ye||Qt.length===0)return;const o="'JetBrains Mono', monospace",e={surface:"#f8f7f2",card:"#faf9f5",border:"#e8e7e0",accent:"#8b9a6b",accentBright:"#a3b07e",gold:"#c8a832",orange:"#c84",greenBright:"#5c5",red:"#c55",yellow:"#ca5",text:"#e8e4dc",muted:"#9e9a92",dim:"#6a6660"},t=ye,i=Qt;Qe>=i.length&&(Qe=0);const n=i[Qe],r=Number(t.budget_ceiling||0),a=Number(t.timeline_ticks||36),s=Math.min(...i.map(m=>m.bid_price)),l=Math.max(...i.map(m=>m.estimated_quality||0));let p="";for(let m=0;m<i.length;m++){const u=i[m],g=m===Qe,b=u.bid_price===s,$=(u.estimated_quality||0)===l,h=u.bid_price>r;p+=`
        ...
        </div>`}const f=n.bid_price>r,c=r>0?Math.round(n.bid_price/r*100):0,v=document.createElement("div");v.id="bid-review-overlay",v.style.cssText="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.7);display:flex;align-items:center;justify-content:center;z-index:9999;",v.addEventListener("click",m=>{m.target===v&&Uo()}),v.innerHTML=`
    <div style="width:640px;max-height:92vh;background:${e.surface};border:1px solid ${e.border};display:flex;flex-direction:column;overflow:hidden;font-family:'IBM Plex Sans',sans-serif;">
    ...
    </div>`,document.body.appendChild(v)}const Tt={Coastal:{color:"#8b9a6b",label:"COASTAL"},Container:{color:"#5a7aaa",label:"CONTAINER"},Bulk:{color:"#c8a832",label:"BULK"},Tanker:{color:"#c86a4a",label:"TANKER"},Reefer:{color:"#6a9a5a",label:"REEFER"},LNG:{color:"#c55",label:"LNG"}},dl={in_port:{color:"#8b9a6b",label:"IN PORT"},in_transit:{color:"#5a8aaa",label:"IN TRANSIT"},dry_dock:{color:"#c84",label:"DRY DOCK"},anchored:{color:"#ca5",label:"ANCHORED"},for_sale:{color:"#9e9a92",label:"FOR SALE"}};function ua(o){return o>=75?"#5c5":o>=50?"#ca5":o>=25?"#c84":"#c55"}function cl(o){return o>=60?"#5c5":o>=30?"#ca5":o>=15?"#c84":"#c55"}async function $e(){if(!d||d.corp_sector!=="Shipping")return;const{data:o,error:e}=await y.from("corp_vessels").select("*").eq("faction_id",d.id).order("vessel_class");e&&console.warn("Failed to load fleet:",e.message),he=o||[],Gt=null,Rt={},xo={};try{const t=he.map(i=>i.id);if(t.length>0){const{data:i}=await y.from("finance_active_loans").select("insured_vessel_id").in("insured_vessel_id",t).in("status",["current"]);for(const r of i||[])r.insured_vessel_id&&(Rt[r.insured_vessel_id]=!0);const{data:n}=await y.from("finance_loan_requests").select("insured_vessel_id").eq("requesting_faction_id",d.id).eq("request_type","insurance").eq("status","open").not("insured_vessel_id","is",null);for(const r of n||[])r.insured_vessel_id&&!Rt[r.insured_vessel_id]&&(xo[r.insured_vessel_id]=!0)}}catch(t){console.warn("Failed to load vessel insurance status:",t.message)}va()}function pl(o){Gt=Gt===o?null:o,va()}function va(){const o=document.getElementById("fl-count"),e=document.getElementById("fl-summary"),t=document.getElementById("fl-list"),i=document.getElementById("fl-footer");if(!o||!t)return;const n=he;o.textContent=n.length+" VESSEL"+(n.length!==1?"S":"");const r=n.filter(c=>c.status==="in_transit").length,a=n.filter(c=>c.status==="in_port"||c.status==="anchored").length,s=n.filter(c=>c.status==="dry_dock").length,l=n.reduce((c,v)=>c+(v.base_maintenance||0),0);e.innerHTML=[{label:"TRANSIT",value:r,color:"#5a8aaa"},{label:"IN PORT",value:a,color:"#8b9a6b"},{label:"DRY DOCK",value:s,color:"#c84"},{label:"MAINT/TICK",value:_(l),color:"#a44"}].map((c,v)=>`<div style="flex:1;padding:5px 8px;text-align:center;${v<3?"border-right:1px solid var(--border-0);":""}">
        ...
        </div>`);f+="</div>",f+=`<span style="font-family:var(--font-mono);font-size:8px;color:#a44;">${_(l)}/tick</span>`,i.innerHTML=f}let ne=!1;async function fl(o,e){if(ne||!d)return;const t=(he||[]).find(m=>m.id===o);if(!t)return;const i=t.current_port_nation_id||null;let n="state",r=3,a=3,s=null,l="State Dry Dock (3x cost, 3 ticks)";if(i){const{data:m}=await y.from("corp_properties").select("id").eq("faction_id",d.id).eq("nation_id",i).eq("type","dry_dock").eq("is_active",!0).limit(1).maybeSingle();if(m)n="own",r=1,a=2,l="Your Dry Dock (base cost, 2 ticks)";else{const{data:u}=await y.from("corp_properties").select("id, faction_id, factions(faction_name)").neq("faction_id",d.id).eq("nation_id",i).eq("type","dry_dock").eq("is_active",!0).limit(1).maybeSingle();u&&(n="other",r=1.2,a=2,s=u.faction_id,l=(u.factions?.faction_name||"Another corp")+"'s Dry Dock (+20%, 2 ticks)")}}else l="State Dry Dock (3x cost, 3 ticks) — no private dock in port";const p=Math.round(e*r),{data:f}=await y.from("factions").select("corp_cash_reserves").eq("id",d.id).single(),c=Number(f?.corp_cash_reserves??0);if(c<p){alert("Insufficient cash. Need "+_(p)+", have "+_(c)+".");return}if(!confirm("Send "+(t.vessel_name||"vessel")+` to dry dock?

Dock: `+l+`
Cost: `+_(p)+`
Duration: `+a+` ticks
Condition restored to 85-100%.`))return;ne=!0;const v=I?.current_tick||0;try{const{error:m}=await y.from("factions").update({corp_cash_reserves:c-p}).eq("id",d.id);if(m){alert("Failed: "+m.message);return}if(n==="other"&&s){const g=p-e,{data:b}=await y.from("factions").select("corp_cash_reserves").eq("id",s).single();b&&await y.from("factions").update({corp_cash_reserves:Number(b.corp_cash_reserves||0)+g}).eq("id",s)}const{error:u}=await y.from("corp_vessels").update({status:"dry_dock",drydock_until_tick:v+a,active_claim_id:null}).eq("id",o);if(u){await y.from("factions").update({corp_cash_reserves:c}).eq("id",d.id),alert("Failed: "+u.message);return}d.corp_cash_reserves=c-p,await $e()}catch(m){alert("Dry dock failed: "+(m.message||"Error"))}finally{ne=!1}}async function ml(o,e){if(ne||!d)return;if(e<=0){alert("Fuel tanks are already full.");return}const t=(he||[]).find(c=>c.id===o);if(!t)return;const i=t.current_port_nation_id||d.nation_id;let n="state",r=3,a=null,s="State Fuel (3x cost) — no private depot in port";if(i){const{data:c}=await y.from("corp_properties").select("id").eq("faction_id",d.id).eq("nation_id",i).eq("type","fuel_depot").eq("is_active",!0).limit(1).maybeSingle();if(c)n="own",r=1,s="Your Fuel Depot (base cost)";else{const{data:v}=await y.from("corp_properties").select("id, faction_id, factions(faction_name)").neq("faction_id",d.id).eq("nation_id",i).eq("type","fuel_depot").eq("is_active",!0).limit(1).maybeSingle();v&&(n="other",r=1.15,a=v.faction_id,s=(v.factions?.faction_name||"Another corp")+"'s Fuel Depot (+15%)")}}const l=Math.round(e*r),{data:p}=await y.from("factions").select("corp_cash_reserves").eq("id",d.id).single(),f=Number(p?.corp_cash_reserves??0);if(f<l){alert("Insufficient cash. Need "+_(l)+", have "+_(f)+".");return}if(confirm("Refuel "+(t.vessel_name||"vessel")+`?

Source: `+s+`
Cost: `+_(l)+`
Fuel restored to 100%.`)){ne=!0;try{const{error:c}=await y.from("factions").update({corp_cash_reserves:f-l}).eq("id",d.id);if(c){alert("Failed: "+c.message);return}if(n==="other"&&a){const m=l-e,{data:u}=await y.from("factions").select("corp_cash_reserves").eq("id",a).single();u&&await y.from("factions").update({corp_cash_reserves:Number(u.corp_cash_reserves||0)+m}).eq("id",a)}const{error:v}=await y.from("corp_vessels").update({fuel:100}).eq("id",o);if(v){await y.from("factions").update({corp_cash_reserves:f}).eq("id",d.id),alert("Failed: "+v.message);return}d.corp_cash_reserves=f-l,await $e()}catch(c){alert("Refuel failed: "+(c.message||"Error"))}finally{ne=!1}}}async function ul(o,e,t){if(ne||!d||!I||!confirm("List "+e+" on the Ship Market for "+_(t)+`?

The vessel will be removed from your fleet and listed for sale. You will receive payment when another corporation purchases it.`))return;ne=!0;const i=I.current_tick||0,n=he.find(l=>l.id===o);if(!n){ne=!1;return}const r=Math.max(0,i-(n.built_at_tick||0)),{error:a}=await y.from("ship_market_listings").insert({nation_id:d.nation_id,vessel_name:n.vessel_name,vessel_class:n.vessel_class,capacity_dwt:n.capacity_dwt,capacity_unit:n.capacity_unit,condition:n.condition,fuel:n.fuel,age_ticks:r,fuel_capacity:n.fuel_capacity,base_maintenance:n.base_maintenance,asking_price:t,purchase_price_new:n.purchase_price||t,seller_type:"CORP",seller_name:d.faction_name,seller_faction_id:d.id,sale_reason:"Listed for sale by "+(d.faction_name||"corporation"),status:"available",listed_at_tick:i});if(a){alert("Failed to create listing: "+a.message),ne=!1;return}const{error:s}=await y.from("corp_vessels").delete().eq("id",o);if(s){await y.from("ship_market_listings").delete().eq("seller_faction_id",d.id).eq("vessel_name",n.vessel_name).eq("listed_at_tick",i),alert("Failed to remove vessel: "+s.message),ne=!1;return}ne=!1,Gt=null,await Promise.all([$e(),ya()])}async function vl(o,e){const t=prompt("Rename vessel:",e);if(!t||t.trim()===e||t.trim().length<2)return;const{error:i}=await y.from("corp_vessels").update({vessel_name:t.trim().slice(0,40)}).eq("id",o);if(i){alert("Failed: "+i.message);return}await $e()}async function yl(o,e,t){if(!d||!I||!confirm("Request insurance for "+e+`?
...
Insurance corporations can now offer coverage for `+e+"."),await $e()}let mi=!1;async function gl(o,e){if(mi||!d||!I)return;const t=prompt(`Describe the claim reason:

e.g., "Storm damage during transit — hull breach repaired at sea" or "Engine failure requiring emergency dry dock"`);if(!t||t.trim().length<5)return;const i=I.current_tick||0,{data:n}=await y.from("finance_active_loans").select("id, lender_faction_id, principal, deductible_pct").eq("insured_vessel_id",o).eq("status","current").limit(1).maybeSingle();if(!n){alert("No active insurance policy found for this vessel.");return}const r=Number(n.principal||0),a=Number(n.deductible_pct||10),s=Math.round(r*a/100);if(!confirm("File insurance claim for "+e+`?
...
The insurer (`+_(r)+" coverage) has been notified. Claim details are visible in the events feed.")}window.flRequestInsurance=yl;window.flFileClaim=gl;const Ii={fuel_depot:{label:"FUEL DEPOT",color:"#c86a4a",icon:"⛽",desc:"Bunkering facility — refuel at base cost, earn revenue from visiting fleets."},dry_dock:{label:"DRY DOCK",color:"#c84",icon:"🔧",desc:"Repair & maintenance dock — dock at base cost, earn revenue from visiting fleets."}},mo=[{type:"fuel_depot",name:"Fuel Depot — Standard",cost:105e6,maint:85e3,style:"Basic",desc:"Bulk fuel storage and bunkering facility."},{type:"fuel_depot",name:"Fuel Depot — Advanced",cost:14e7,maint:11e4,style:"Modern",desc:"High-capacity fuel terminal with pipeline infrastructure."},{type:"dry_dock",name:"Dry Dock — Standard",cost:85e6,maint:15e4,style:"Basic",desc:"Ship repair and maintenance facility."},{type:"dry_dock",name:"Dry Dock — Advanced",cost:115e6,maint:2e5,style:"Modern",desc:"Full-service shipyard with drydock and crane facilities."}];let So=[];async function xl(){if(!d||d.corp_sector!=="Shipping")return;const{data:o}=await y.from("corp_properties").select("*, nations!nation_id(name)").eq("faction_id",d.id).in("type",["fuel_depot","dry_dock"]).eq("is_active",!0).order("created_at",{ascending:!1});So=o||[],bl()}function bl(){const o=document.getElementById("pf-count"),e=document.getElementById("pf-list"),t=document.getElementById("pf-footer");if(!o||!e||!t)return;const i=So;if(o.textContent=i.length+" FACILIT"+(i.length===1?"Y":"IES"),i.length===0)e.innerHTML=`<div style="padding:20px;text-align:center;">
        ...
        </div>`;else{let a=0;e.innerHTML=i.map(s=>{const l=Ii[s.type]||Ii.fuel_depot,p=s.condition>=75?"#5c5":s.condition>=50?"#ca5":"#c84";return a+=Number(s.monthly_maintenance||0),`<div style="padding:8px 12px;border-bottom:1px solid var(--border-0);">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px;">
                    <div style="display:flex;align-items:center;gap:6px;">
            ...
            </div>`}).join("")}Number(d?.corp_cash_reserves??0);const n=i.some(a=>a.type==="fuel_depot"),r=i.some(a=>a.type==="dry_dock");t.innerHTML=`
        ...
        </div>`}let ui=!1;async function _l(o){if(ui||!d||!I)return;const e=mo.filter(b=>b.type===o);if(e.length===0)return;const t=Ii[o],i=d.nation_id,n=N?.name||d?.nation||"Home Nation",r=N?.capital||"Port City",a=[{id:i,name:n,capital:r,label:"National HQ"}],{data:s}=await y.from("corp_properties").select("nation_id, name, city, nations!nation_id(name, capital)").eq("faction_id",d.id).eq("type","regional_hq").eq("is_active",!0);for(const b of s||[])b.nation_id!==i&&a.push({id:b.nation_id,name:b.nations?.name||b.city||"Unknown",capital:b.nations?.capital||b.city||"Port City",label:b.name||"Subsidiary"});let l=a[0];if(a.length>1){let b=t.label+` — SELECT LOCATION
`+"─".repeat(30)+`
...
Construction corporations in `+l.name+" can now bid on this project.")}catch(b){alert("Failed to post contract: "+(b.message||"Error"))}finally{ui=!1}}window.pfOpenBuild=_l;const on={"Bulk Cargo":["Reefer","Bulk","Coastal"],"Container Freight":["Coastal","Container"],"Specialized Transport":["Tanker","LNG","Bulk"]};async function ya(){if(!d||d.corp_sector!=="Shipping")return;const{data:o,error:e}=await y.from("ship_market_listings").select("*, nation:nation_id(id, name)").eq("status","available").order("asking_price",{ascending:!0});e&&console.warn("Failed to load ship market:",e.message),Mi=o||[],bo=null,ga()}function hl(o){bo=bo===o?null:o,ga()}function $l(o){return(on[d?.corp_subsector]||[]).includes(o)}function ga(){const o=document.getElementById("sm-count"),e=document.getElementById("sm-list"),t=document.getElementById("sm-footer");if(!o||!e)return;const i=Mi;o.textContent=i.length+" AVAILABLE",i.length===0?e.innerHTML='<div style="flex:1;display:flex;align-items:center;justify-content:center;font-family:var(--font-mono);font-size:9px;color:var(--text-dim);text-align:center;line-height:1.8;">No vessels on the market.<br>Check back next cycle.</div>':e.innerHTML=i.map((a,s)=>{const l=bo===s,p=Tt[a.vessel_class]||{color:"#666",label:"?"},f=a.seller_type==="CORP"?"#5a8aaa":"#8b9a6b",c=ua(a.condition),v=a.nation?.name||"—",m=$l(a.vessel_class);I?.current_tick;const u=a.age_ticks||0,g=Math.max(1,Math.floor(u/12)),b=v!==d?.nation?Number(d?.tariffs||N?.tariffs||0):0,$=Math.round(a.asking_price*b/100),h=a.asking_price+$;let E=`<div onclick="smSelectListing(${s})" style="border-bottom:1px solid var(--border-0);cursor:pointer;border-left:2px solid ${l?p.color:"transparent"};background:${l?p.color+"06":"transparent"};">
    ...
    <div onclick="smOpenCommission()" style="padding:4px 14px;font-family:var(--font-mono);font-size:9px;font-weight:700;letter-spacing:1px;color:var(--gold);border:1px solid rgba(200,168,50,0.3);cursor:pointer;">COMMISSION VESSEL</div>`}let dt=!1;async function wl(o,e){if(dt||!d||!I)return;const t=Number(d.corp_cash_reserves??0);if(t<e){alert("Insufficient cash. Need "+_(e)+".");return}if(!confirm("Purchase this vessel for "+_(e)+"?"))return;dt=!0;const i=Mi.find(c=>c.id===o);if(!i){dt=!1;return}const n=I.current_tick||0,r={Coastal:{capacity_dwt:14e3,capacity_unit:"DWT",base_maintenance:18e4,fuel_capacity:800,purchase_price:3e6},Container:{capacity_dwt:4800,capacity_unit:"TEU",base_maintenance:29e4,fuel_capacity:2100,purchase_price:65e6},Bulk:{capacity_dwt:28e3,capacity_unit:"DWT",base_maintenance:35e4,fuel_capacity:1800,purchase_price:3e6},Tanker:{capacity_dwt:42e3,capacity_unit:"DWT",base_maintenance:38e4,fuel_capacity:2400,purchase_price:53e6},Reefer:{capacity_dwt:12e3,capacity_unit:"DWT",base_maintenance:28e4,fuel_capacity:1600,purchase_price:6e6},LNG:{capacity_dwt:18e3,capacity_unit:"DWT",base_maintenance:58e4,fuel_capacity:1400,purchase_price:78e6}},a=r[i.vessel_class]||r.Coastal,{error:s}=await y.from("factions").update({corp_cash_reserves:t-e}).eq("id",d.id);if(s){alert("Failed: "+s.message),dt=!1;return}const{error:l}=await y.from("corp_vessels").insert({faction_id:d.id,nation_id:d.nation_id,vessel_name:i.vessel_name,vessel_class:i.vessel_class,condition:i.condition,fuel:i.fuel||50,status:"in_port",capacity_dwt:i.capacity_dwt||a.capacity_dwt,capacity_unit:i.capacity_unit||a.capacity_unit,base_maintenance:i.base_maintenance||a.base_maintenance,fuel_capacity:i.fuel_capacity||a.fuel_capacity,purchase_price:e,built_at_tick:n-(i.age_ticks||0),current_port_nation_id:d.nation_id});if(l){await y.from("factions").update({corp_cash_reserves:t}).eq("id",d.id),alert("Failed to create vessel: "+l.message),dt=!1;return}var{error:p}=await y.from("ship_market_listings").update({status:"sold",purchased_by:d.id,purchased_at_tick:n}).eq("id",o);if(p&&console.warn("Failed to mark listing as sold:",p.message),i.seller_faction_id){const{data:c}=await y.from("factions").select("corp_cash_reserves").eq("id",i.seller_faction_id).single();if(c){var{error:f}=await y.from("factions").update({corp_cash_reserves:Number(c.corp_cash_reserves||0)+i.asking_price}).eq("id",i.seller_faction_id);f&&console.warn("Failed to credit seller:",f.message)}}d.corp_cash_reserves=t-e,dt=!1,await Promise.all([$e(),ya()])}const Dt=[{cls:"Coastal",baseCost:12e6,baseBuild:3,cargo:"Bulk, Containers (coastal)"},{cls:"Container",baseCost:65e6,baseBuild:5,cargo:"Manufactured, Tech, General"},{cls:"Bulk",baseCost:38e6,baseBuild:4,cargo:"Minerals, Aggregate, Military"},{cls:"Tanker",baseCost:52e6,baseBuild:5,cargo:"Fuel, Petroleum, Chemicals"},{cls:"Reefer",baseCost:45e6,baseBuild:4,cargo:"Food, Perishables, Agriculture"},{cls:"LNG",baseCost:78e6,baseBuild:6,cargo:"Liquefied Natural Gas only"}];let ce="Coastal",Kt=0,Jt="",Ze=[];function kl(){ce=(on[d?.corp_subsector]||["Coastal"])[0],Kt=0,Jt="",Ze=[],document.getElementById("comm-overlay").style.display="flex",El()}async function El(){const{data:o}=await y.from("nations").select("id, name, manufacturing_output, physical_infrastructure, tariffs").order("name");Ze=(o||[]).map(e=>{const t=Number(e.manufacturing_output??50),i=Math.round((.75+t/100*.5)*100)/100,n=Math.round((1.5-t/100*.65)*100)/100,r=e.id===d?.nation_id;return{id:e.id,name:e.name,mfg:t,costMod:i,buildMod:n,isHome:r,tariffs:Number(e.tariffs??0)}}),Ze.sort((e,t)=>(t.isHome?1:0)-(e.isHome?1:0)),nn()}function xa(){document.getElementById("comm-overlay").style.display="none"}function Cl(o){ce=o,nn()}function Tl(o){Kt=o,nn()}function Sl(o){Jt=o}function nn(){const o=document.getElementById("comm-content");if(!o)return;const e=I?.current_tick||0,t=Dt.find(u=>u.cls===ce)||Dt[0],i=Ze[Kt]||{name:"—",costMod:1,buildMod:1},n=Tt[ce]||{color:"#666"},r=Math.round(t.baseCost*i.costMod),a=Math.max(2,Math.round(t.baseBuild*i.buildMod)),s=Math.round(r*.5),l=r-s,p=e+a,f=on[d?.corp_subsector]||[];let c="";c+=`<div style="padding:10px 16px;border-bottom:1px solid #e8e7e0;background:#faf9f5;flex-shrink:0;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
            <div style="display:flex;align-items:center;gap:6px;">
                <span style="font-size:8px;color:#c8a832;">&#9679;</span>
    ...
    </div>`,c+='<div style="flex:1;overflow-y:auto;">',c+=`<div style="padding:8px 16px;border-bottom:1px solid #e8e7e0;">
        ...
        </div>`}c+="</div>",c+=`<div style="padding:8px 16px;border-bottom:1px solid #e8e7e0;">
        ...
        <input id="comm-name-input" value="${x(Jt)}" oninput="commSetName(this.value)" placeholder="e.g., MV 'Sierra Nevada'" style="width:100%;padding:6px 10px;font-family:var(--font-mono);font-size:11px;color:#e8e4dc;background:#faf9f5;border:1px solid #e8e7e0;outline:none;box-sizing:border-box;" />
    </div>`,c+=`<div style="padding:8px 16px;border-bottom:1px solid #e8e7e0;">
        ...
        <div style="background:#faf9f5;border:1px solid #e8e7e0;padding:6px 10px;">`;const v=[{label:"VESSEL CLASS",value:ce,color:n.color},{label:"SHIPYARD",value:i.name,color:"#9e9a92"},{label:"BASE COST",value:_(t.baseCost)+" × "+i.costMod.toFixed(2),color:"#9e9a92"},{label:"BUILD TIME",value:a+" ticks",color:a>t.baseBuild?"#c84":a<t.baseBuild?"#5c5":"#9e9a92"},{label:"COMPLETION",value:"~Tick "+p,color:"#9e9a92"}];for(const u of v)c+=`<div style="display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px solid #e8e7e0;">
        ...
        </div>`;c+=`<div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid #e8e7e0;">
    ...
    </div>`,c+=`<div style="display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px solid #e8e7e0;">
    ...
    </div>`,c+="</div>";const m=Jt.trim().length>=2;c+=`<div style="padding:10px 16px;border-top:1px solid #e8e7e0;background:#faf9f5;flex-shrink:0;display:flex;justify-content:space-between;align-items:center;">
        <div>
    ...
    </div>`,o.innerHTML=c}let At=!1;async function zl(){if(At||!d||!I)return;const o=Jt.trim();if(o.length<2)return;const e=Dt.find(b=>b.cls===ce)||Dt[0],t=Ze[Kt];if(!t)return;const i=Math.round(e.baseCost*t.costMod),n=Math.max(2,Math.round(e.baseBuild*t.buildMod)),r=Math.round(i*.5),a=i-r,s=I.current_tick||0,l=Number(d.corp_cash_reserves??0);if(l<r){alert("Insufficient cash for deposit. Need "+_(r)+".");return}if(!confirm("Commission "+ce+" from "+t.name+`?

Deposit: `+_(r)+` (non-refundable)
Balance: `+_(a)+" on delivery at tick "+(s+n)))return;At=!0;const p=document.getElementById("comm-order-btn");p&&(p.style.opacity="0.4",p.style.pointerEvents="none");const{error:f}=await y.from("factions").update({corp_cash_reserves:l-r}).eq("id",d.id);if(f){alert("Failed: "+f.message),At=!1;return}const{data:c}=await y.from("nations").select("budget_reserves").eq("id",t.id).single();if(c){var{error:v}=await y.from("nations").update({budget_reserves:Number(c.budget_reserves||0)+r}).eq("id",t.id);v&&console.warn("Failed to credit shipyard nation budget:",v.message)}const m={Coastal:{dwt:14e3,unit:"DWT",maint:18e4,fuel:800},Container:{dwt:4800,unit:"TEU",maint:29e4,fuel:2100},Bulk:{dwt:28e3,unit:"DWT",maint:35e4,fuel:1800},Tanker:{dwt:42e3,unit:"DWT",maint:38e4,fuel:2400},Reefer:{dwt:12e3,unit:"DWT",maint:28e4,fuel:1600},LNG:{dwt:18e3,unit:"DWT",maint:58e4,fuel:1400}},u=m[ce]||m.Coastal,{error:g}=await y.from("vessel_orders").insert({faction_id:d.id,vessel_name:o,vessel_class:ce,capacity_dwt:u.dwt,capacity_unit:u.unit,base_maintenance:u.maint,fuel_capacity:u.fuel,purchase_price:e.baseCost,shipyard_nation_id:t.id,shipyard_nation:t.name,cost_modifier:t.costMod,build_modifier:t.buildMod,total_cost:i,deposit_paid:r,balance_due:a,ordered_at_tick:s,delivery_tick:s+n,build_ticks:n,status:"building"});if(g){await y.from("factions").update({corp_cash_reserves:l}).eq("id",d.id),alert("Failed to place order: "+g.message),At=!1;return}d.corp_cash_reserves=l-r,At=!1,xa(),alert(o+` commissioned!

Class: `+ce+`
Shipyard: `+t.name+`
Deposit: `+_(r)+`
Delivery: Tick `+(s+n))}window.smSelectListing=hl;window.smPurchase=wl;window.smOpenCommission=kl;window.smCloseCommission=xa;window.commSetClass=Cl;window.commSetNation=Tl;window.commSetName=Sl;window.smPlaceOrder=zl;window.flSelectVessel=pl;window.flRefurbish=fl;window.flRefuel=ml;window.flSell=ul;window.flRename=vl;window.openBidReview=al;window.closeBidReview=Uo;window.reviewSelectBid=rl;window.acceptBid=sl;window.declineAllBids=ll;window.switchToActions=Pn;window.actSelectExec=cs;window.actExecute=Qr;window.confirmFireExec=Wr;window.actOpenStatement=Un;window.actCloseStatement=Yi;window.actSubmitStatement=Kr;window.actDeclareBankruptcy=Hn;window.actOpenRestructure=Yn;window.actCloseRestructure=Qi;window.actSubmitRestructure=as;window.actOpenRebrand=Qn;window.actCloseRebrand=Ki;window.actSubmitRebrand=rs;window.actOpenDonation=Kn;window.actCloseDonation=Ji;window.actSubmitDonation=ds;window.donateSelectParty=ls;window.lrOpen=Vn;window.lrClose=Wn;window.lrSubmit=ns;window.lrSetAmount=Zr;window.lrSetPurpose=es;window.lrSetTerm=ts;window.lrSetCollateral=os;window.openExecSearch=ps;window.closeExecSearch=Xn;window.esSelectCandidate=fs;window.esHireCandidate=ms;window.switchToExpansion=On;window.switchToOperations=Bn;window.hfSetChange=us;window.hfReset=vs;window.hfConfirm=ys;document.addEventListener("click",function(o){const e=o.target.closest(".corp-nav-tab[href]:not([data-tab-action])");if(!e)return;const t=e.getAttribute("href");if(!t)return;const i=new URL(t,window.location.href);i.pathname!==window.location.pathname||i.searchParams.get("tab")||e.classList.contains("active")||(o.preventDefault(),Bn(o))});Dr();
