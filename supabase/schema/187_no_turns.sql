...
-- every tick (= every month), may take up to two INDEPENDENT actions:
--   • ONE terminal card choice — either PLAY A CARD (resolve it + bank its Action Points) OR DISCARD A
--     CARD (return it to the deck for +3 Influence). Play and discard are mutually exclusive with each
--     other — one or the other, not both, per tick.
--   • BID ON CARDS — place/raise sealed bids on the market. DECOUPLED from the play/discard choice: you
--     may always bid (repeatable this tick, up to the hand cap), whether or not you also played/discarded.
--
...
--
-- ── How the once-per-tick play/discard gate is enforced, with NO new column ──
--   • turn_acted_tick marks the tick a party PLAYED or DISCARDED (the terminal choice). Set by those two.
--   Therefore:
--     – play / discard are allowed only when turn_acted_tick ≠ tick (whichever fires first blocks the
--       other for that tick);
--     – bid carries no turn gate at all (it never sets turn_acted_tick, so bids stay repeatable and are
--       always available regardless of the play/discard choice).
--
...
begin
  ...
  -- can't be raced, and guards the escrow. (No turn gate — bidding is decoupled from play/discard.)
...
-- in_hand + the play-time effect pipeline. The per-tick gate is now just: you haven't already
-- played/discarded this tick (turn_acted_tick). Play and discard are mutually exclusive with each other,
-- but bidding is decoupled — outstanding bids no longer block a play. On play the card's `acts` are ADDED
-- to the persistent AP bank (no overwrite, no expiry). ──
...
begin
  ...
  -- Per-tick gate: not already played/discarded this tick. (Bidding is decoupled — it no longer blocks a
  -- play; play and discard remain mutually exclusive with each other via turn_acted_tick.)
  select current_tick into v_tick from public.game_state where id;
  if (select turn_acted_tick from public.parties where id = v_party.id) = v_tick then
    raise exception 'You have already played or discarded a card this month.';
  end if;
  ...
  -- surfaces in every OTHER nation's world feed (the acting nation sees it in its own home feed). A CHOICE
  -- card announces here too now (the play IS the description); the government's pick is a SEPARATE second
  -- event at DECIDE time (schema/178). Only a hex_pop card is excluded — it keeps its own thematic
...
-- ── DISCARD A CARD — supersedes schema/174. Drops the rotation turn gate; per-tick gate is now just
-- "not already played/discarded this tick" (play and discard are mutually exclusive with each other;
-- bidding is decoupled and does not block a discard). Returns the card to the nation's deck, banks +3
...
begin
  ...
  if v_acted = v_tick then raise exception 'You have already played or discarded a card this month.'; end if;
