-- ===========================================================================
-- 330 · Kingdoms — a Holding at 3+ Unrest pays no taxes (deck-wide Unrest rule).
--
-- Per the deck's Unrest rule: at 3 Unrest a Holding pays no taxes. Taxation now skips any held Holding with
-- Unrest >= 3 entirely — it yields no Gold AND gains no Unrest (an untaxed Holding is not disturbed). Holdings
-- under 3 tax normally (+1 Unrest unless a Courthouse). The result message notes any restless Holdings skipped.
--
-- NOT YET (flagged): the 5-Unrest revolt ("lost until retaken") needs a revolt/retake mechanic; passive Unrest
-- removal by Courthouse/Abbey/Shrine needs the yearly tick. Redefines kingdoms_play_card (taxation branch only)
-- — otherwise byte-identical to 328. Depends on: 314 (unrest), 322/328 (taxation + play RPC). Idempotent.
-- Apply after 329.
-- ===========================================================================
