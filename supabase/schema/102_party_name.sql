...
create or replace function public.edit_party(p_name text, p_abbr text, p_logo_url text)
returns jsonb language plpgsql security definer set search_path = public as $$
declare v_party public.parties%rowtype; v_name text; v_abbr text; v_logo text; v_cost constant int := 5;
begin
  ...
  update public.parties
     set name = v_name, abbreviation = v_abbr, logo_url = v_logo, influence = influence - v_cost
   where id = v_party.id;
  ...
  return jsonb_build_object('ok', true, 'actions', v_party.influence - v_cost, 'name', v_name, 'abbreviation', v_abbr, 'logo_url', v_logo);
end $$;
grant execute on function public.edit_party(text, text, text) to authenticated;
