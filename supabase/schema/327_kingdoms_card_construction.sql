-- ===========================================================================
-- 327 · Kingdoms — sixth card: Construction (top) / Plague (bottom).
--
-- Construction is the deck's engine of growth: a building may no longer be raised for free — you must hold a
-- Construction card and spend it. This migration card-gates building:
--   * kingdoms_build now requires a Construction card in the caller's hand, counts against the 2-plays-per-turn
--     limit, spends the building's Gold, consumes the Construction card, and returns a narrative message.
--   * A holding under Plague may not be built upon (kingdoms_counties.plague — future-proof, always false for
--     now since Plague targets another House and cannot be self-inflicted).
--   * kingdoms_play_card rejects Construction on the normal play path ('use_build') so the card can never be
--     silently spent without a build — it must go through kingdoms_build (the client's Construction picker).
-- Plague (bottom) targets another House, so it is stored for display only.
--
-- DEFERRED (flagged): the whole Plague side (needs other Houses, adjacency, yearly rolls, trade bans, spread);
-- the yearly Plague upkeep/removal cycle (no year/tick system yet). Razing stays free and direct (the card
-- only gates raising). Depends on: 312/313 (buildings + build RPC), 318/326 (cards + play RPC + turn limit).
-- Idempotent. Apply after 326.
-- ===========================================================================

-- Future-proof: a holding can carry Plague (set only by the deferred Plague card). Blocks construction there.
alter table public.kingdoms_counties
  add column if not exists plague boolean not null default false;

insert into public.kingdoms_cards (card_key, side, name, cost_text, cost_gold, cost_food, effects, notes, playable) values
  ('construction', 'top', 'Construction', 'The building''s Gold cost', 0, 0,
   $j$["Build one building in a Holding you own, paying its Gold cost and consuming its Plots.","A building may not be raised in a Holding that has Plague.","Buildings cannot be raised without this card — Construction is the deck's engine of growth."]$j$::jsonb,
   $j$["Playing Construction opens the build picker — choose a Holding and a building to raise."]$j$::jsonb, true),
  ('construction', 'bottom', 'Plague', 'None', 0, 0,
   $j$["Select a Holding in another House. Add Plague to it. It immediately loses 1D2 Population.","May not target a Kin-Bound or Guest-Bound House.","While Plague remains: at the start of each year the Holding loses 1 Population and gains 1 Unrest. No building may be raised there. Trade with that Holding is forbidden.","Removing Plague: the owner may pay 4 Gold, or roll 1D6 at year's end and remove it on a 5+. An Abbey in the Holding grants +2 to that roll.","Spread: on a natural 6 during the yearly roll, Plague spreads to one adjacent Holding of the same House."]$j$::jsonb,
   $j$["Plague targets a Holding in another House — stored for display until other Houses exist."]$j$::jsonb, false)
...
-- Redefine kingdoms_build: card-gated (needs a Construction card), turn-limited, Plague-blocked; returns text.
-- Return type changes (uuid → text), so drop first.
drop function if exists public.kingdoms_build(uuid, int);
...
as $$
declare
  v_house uuid; v_gold int; v_avail int; v_hills boolean; v_plague boolean; v_seat text; v_played int;
  v_cost int; v_land int; v_needs text; v_bname text; v_used int; v_card uuid;
begin
  ...
  select l.id, coalesce((l.resources->>'gold')::int, 0), c.available_land, coalesce(c.hills, false),
         coalesce(c.plague, false), c.name, coalesce(l.cards_played, 0)
    into v_house, v_gold, v_avail, v_hills, v_plague, v_seat, v_played
  ...
  -- Building requires a Construction card in hand and counts against the per-turn play limit.
  if v_played >= 2 then raise exception 'turn_limit'; end if;
  select id into v_card from public.kingdoms_hand
    where house_id = v_house and card_key = 'construction' order by created_at limit 1;
  if v_card is null then raise exception 'no_construction_card'; end if;

  select cost, land, needs, name into v_cost, v_land, v_needs, v_bname
  from public.kingdoms_building_specs where num = p_building;
  if v_cost is null then raise exception 'unknown_building'; end if;

  if v_plague then raise exception 'has_plague'; end if;
  ...
  -- Spend the gold, count the play, raise the building, consume the Construction card. (Card only after all checks.)
  update public.kingdoms_leaders
     set resources = jsonb_set(resources, '{gold}', to_jsonb(v_gold - v_cost)),
         cards_played = cards_played + 1
   where id = v_house;
  insert into public.kingdoms_holding_buildings (county_id, building_num) values (p_county, p_building);
  delete from public.kingdoms_hand where id = v_card;

  return 'The ' || v_bname || ' rises in ' || v_seat || '. −' || v_cost || ' Gold.';
end;
$$;
...
-- Redefine kingdoms_play_card to reject Construction on the normal play path (it must go through the build
-- picker → kingdoms_build). Otherwise byte-identical to 326.
