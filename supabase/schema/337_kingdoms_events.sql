-- ===========================================================================
-- 337 · Kingdoms — News of the Realm: an event log written by the card actions.
--
-- kingdoms_events records notable deeds so the Home page can show a chronicle ("Concerns Your House"). Events
-- are written server-side, inside the action's own transaction (no client-supplied text, no reload race), via
-- _kingdoms_log — an internal helper the card RPCs call. This migration redefines the three card-driven RPCs
-- (kingdoms_play_card, kingdoms_build, kingdoms_muster_levies) to log the message they already produce.
--
-- DEFERRED (flagged): the realm-wide + foreign ("Aldren" / "Abroad") tiers need other Houses / a sovereign AI /
-- a death system / foreign realms; and Claim the Throne + Decree are page-actions (not cards) whose nice text is
-- composed client-side, so they are not chronicled yet. Otherwise the three RPCs are byte-identical to their
-- prior definitions (336 / 327 / 328). Depends on: 304, 327, 328, 336. Idempotent. Apply after 336.
-- ===========================================================================

create table if not exists public.kingdoms_events (
...
create index if not exists kingdoms_events_house_idx on public.kingdoms_events(house_id, created_at desc);

alter table public.kingdoms_events enable row level security;
grant select on public.kingdoms_events to anon, authenticated;   -- the chronicle is public game data
drop policy if exists "kingdoms_events_select_all" on public.kingdoms_events;
create policy "kingdoms_events_select_all" on public.kingdoms_events for select using (true);

-- Internal: append one event. Called only from the security-definer action RPCs (they run as owner), so it
-- bypasses RLS to insert; revoked from clients so no one can forge chronicle entries directly.
create or replace function public._kingdoms_log(p_house uuid, p_tag text, p_place text, p_body text)
returns void
language sql
as $$
  insert into public.kingdoms_events (house_id, tag, place, body)
  values (p_house, left(coalesce(p_tag, 'Court'), 20), left(p_place, 80), left(coalesce(p_body, ''), 300));
$$;
revoke all on function public._kingdoms_log(uuid, text, text, text) from public, anon, authenticated;
...
declare
  ...
  v_cost int; v_land int; v_needs text; v_bname text; v_used int; v_card uuid; v_msg text;
begin
  if auth.uid() is null then raise exception 'unauthenticated'; end if;
  ...
  v_msg := 'The ' || v_bname || ' rises in ' || v_seat || '. −' || v_cost || ' Gold.';
  perform public._kingdoms_log(v_house, 'Built', v_seat, v_msg);   -- chronicle the deed
  return v_msg;
end;
$$;
...
begin
  ...
  perform public._kingdoms_log(v_house, 'Muster', v_seat, v_msg);   -- chronicle the deed
  return v_msg;
end;
$$;
