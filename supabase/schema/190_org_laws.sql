...
returns jsonb language sql immutable set search_path = public as $$
  select jsonb_build_object(
    ...
    'res', jsonb_build_array('survey', 'reserves', 'pricenote')
  );
$$;
...
declare v_members int; v_need int; v_aye int; v_nay int; v_tick int;
begin
  ...
  if v_aye >= v_need or v_nay >= v_need then
    ...
    update public.organization_laws
       set status = case when v_aye >= v_need then 'enacted' else 'rejected' end, resolved_tick = v_tick
...
-- propose_org_law: a member's Foreign Affairs minister moves a Level I law onto the Agenda. Free. The
-- mover is recorded Aye (proposing is supporting). Rejects a law already proposed or enacted.
create or replace function public.propose_org_law(p_org uuid, p_sector text, p_law text)
returns void language plpgsql security definer set search_path = public as $$
...
begin
  ...
  select current_tick into v_tick from public.game_state where id;
  insert into public.organization_laws (org_id, sector, law_id, status, proposed_by, proposed_tick)
...
grant execute on function public.propose_org_law(uuid, text, text) to authenticated;
