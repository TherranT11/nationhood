...
begin
  ...
  if p_ic then
    ...
    select btrim(coalesce(l.first_name,'') || ' ' || coalesce(l.last_name,'')) into v_leader
      from public._party_leader(v_party.id) l;   -- the party leader's name (shared source, schema/30)
