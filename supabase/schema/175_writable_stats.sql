...
-- The client display pages read these deltas through the nation_stat_values RPC (schema/177) — the one
-- source that returns each stat's base + policy contributions + delta — so card display-stat effects
-- show wherever the number renders (Government / Growth / Bureaucracy). Never recompute a stat's value
-- client-side; always read it from that RPC.
