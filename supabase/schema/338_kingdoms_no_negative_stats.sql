-- ===========================================================================
-- 338 · Kingdoms — treasury stats never go negative.
--
-- The top-bar stats (Gold, Food, Population, Available Plots, House Prestige) cannot drop below 0. Deterministic
-- costs already block the action when unaffordable (cannot_pay / not_enough_gold / no_room / not_enough_people),
-- and Population / Unrest were already floored with greatest(0, …). The one remaining gap was the Marry card's
-- Lowborn result, which subtracted 1 House Prestige with no floor (could show -1). This floors that deduction so
-- the marriage still stands but Prestige cannot go below 0, and backfills any house already sitting negative.
--
-- Redefines kingdoms_play_card (Lowborn-marriage branch only) — otherwise byte-identical to 337. Available Plots
-- is derived client-side and already floored. Future Prestige-losing cards (Assassination, Banditry — deferred)
-- must likewise floor with greatest(0, …). Depends on: 337. Idempotent. Apply after 337.
-- ===========================================================================
...
-- Backfill: clamp any stat already sitting below 0 up to 0 (one-time repair).
update public.kingdoms_leaders set resources =
  jsonb_set(jsonb_set(jsonb_set(jsonb_set(resources,
    '{prestige}',   to_jsonb(greatest(0, coalesce((resources->>'prestige')::int, 0)))),
    '{gold}',       to_jsonb(greatest(0, coalesce((resources->>'gold')::int, 0)))),
    '{food}',       to_jsonb(greatest(0, coalesce((resources->>'food')::int, 0)))),
    '{population}', to_jsonb(greatest(0, coalesce((resources->>'population')::int, 0))))
 where coalesce((resources->>'prestige')::int, 0) < 0
    or coalesce((resources->>'gold')::int, 0) < 0
    or coalesce((resources->>'food')::int, 0) < 0
    or coalesce((resources->>'population')::int, 0) < 0;
