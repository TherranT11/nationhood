-- ===========================================================================
-- 245 · FDI Phase 1 — board resolution, the four-door end path, nationalize, tick wiring.
--
-- The tick resolves PENDING deals (the board decides, activate or reject) and expires ACTIVE deals
-- past their term. ALL ends run through one function (_end_fdi_deal) so the clawback can never be
-- skipped; because FDI Growth is derived over ACTIVE deals (schema/243), "removing" it is just the
-- state flip to ENDED — nothing to unwind. Events publish into host + owner feeds, and the world
-- feed for the dramatic ends (NATIONALIZED / DEPARTED).
--
-- Board probability = base(tier) [t1 .85, t2 .60, t3 .35] + 0.05·temperature + 0.02·(relation−5)
--   + climate tilt (±0.15), clamped [0.02, 0.98]; the roll is a DETERMINISTIC hash of (deal, tick)
--   so a tick re-run reproduces the same outcome (no Math.random drift).
--
-- Phase 1 deferrals (noted where they bite): NATIONALIZED fully ends the deal — retaining a state-owned
-- plant at half Growth needs the buildings/asset system (Phase 2). DEPARTED (footloose) and CLOSED
-- (corp bankruptcy) reasons are supported by _end_fdi_deal but their automatic triggers come later.
--
-- Depends on: 243 (fdi_deals/_nation_fdi_growth), 244 (_fdi_climate/_corp_fdi_tier), 137
-- (_relation_adjust), 40 (_begin_action/events/current_game_date), 233 (_advance_tick body). Idempotent.
-- ===========================================================================

set check_function_bodies = off;

-- One emitter: publish a lifecycle transition into the host feed (always), the owner feed (where it
-- matters), and the world feed (kind 'declaration' → World Timeline) for the dramatic ends.
...
-- The ONE end path — flips the deal to ENDED (Growth stops the same tick, being derived) and emits.
-- No-op if already ended, so a double-fire (e.g. corp death + expiry same tick) is safe.
...
-- Tick step: resolve due PENDING board decisions, then expire ACTIVE deals past their term.
...
begin
  for v_d in
    select d.*, c.size as corp_size from public.fdi_deals d
  ...
  loop
    ...
    -- deterministic roll in [0,1): 16-bit hash of (deal, tick) → reproducible on a tick re-run
...
-- Nationalize — the host seizes a foreign operation. Minister of Finance, 1 AP. Relations with the
-- owner nation take −4 and the world sees it. (Phase 1 ends the deal outright; retaining the plant as
-- a state-owned asset at half Growth is a Phase-2 feature that needs the buildings system.)
create or replace function public.nationalize_fdi_deal(p_deal uuid)
returns jsonb language plpgsql security definer set search_path = public as $$
declare v_p public.parties%rowtype; v_d public.fdi_deals%rowtype;
begin
  v_p := public._begin_action(0);   -- 1 AP
  if not public._party_holds_ministry(v_p.id, 'Treasury') then
    raise exception 'Only the Minister of Finance can nationalise a foreign operation.'; end if;
  select * into v_d from public.fdi_deals where id = p_deal;
  if not found or v_d.state <> 'ACTIVE' then raise exception 'There is no active foreign operation to nationalise.'; end if;
  if v_d.host_nation_id <> v_p.nation_id then raise exception 'You can only nationalise operations inside your own nation.'; end if;
  perform public._relation_adjust(v_d.host_nation_id, v_d.owner_nation_id, -4);
  perform public._end_fdi_deal(p_deal, 'NATIONALIZED');
  return jsonb_build_object('ok', true);
end $$;
grant execute on function public.nationalize_fdi_deal(uuid) to authenticated;

-- ---------------------------------------------------------------------------
-- Redefine _advance_tick (body from schema/233) with the FDI resolution step added before the winds
-- + snapshots. Every other line is reproduced verbatim.
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Foreign Direct Investment (schema/243-245): resolve PENDING board decisions that are due
  -- (activate or reject) and expire ACTIVE deals past their term. FDI Growth is a derived term over
  -- ACTIVE deals, so a state change here moves national Growth the same tick. Isolated like every step.
  begin perform public._resolve_fdi_deals(v_tick);
  exception when others then raise warning 'tick %: FDI resolution failed — %', v_tick, sqlerrm; end;
