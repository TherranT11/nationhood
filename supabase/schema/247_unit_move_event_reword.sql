-- ===========================================================================
-- 247 · Reword the unit-movement order event.
--
-- Redefines move_unit (live body schema/220) to change ONLY the feed line to the desired wording:
--   "The {unit name} has been ordered to move into {province name}."
-- The destination hex IS the province; an unnamed hex reads "a neighbouring province". Still kind
-- 'party' (Home feed only — filtered from the World Timeline, since units move every tick). Drops the
-- now-unused nation-name lookup. Every other line of move_unit is reproduced verbatim from schema/220.
--
-- Depends on: 220/204 (move_unit), 40 (events), 101 (world_hexes), 199 (military_units). Idempotent.
-- ===========================================================================
...
begin
  ...
  insert into public.events (nation_id, party_id, kind, body, game_date)
    values (v_nation, v_p.id, 'party',
      'The ' || coalesce(v_u.name, 'unit') || ' has been ordered to move into ' ||
      coalesce(nullif(v_hexname, ''), 'a neighbouring province') || '.', public.current_game_date());
