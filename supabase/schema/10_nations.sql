...
-- every tradeable resource (Energy/Food/Minerals/Goods/Services/Military; Diplomacy isn't
-- traded) — Energy 1 → 1 on hand. adminsetup mirrors this rule when creating a nation. Stored
-- so the Market can move it later; seeded ONCE (guarded on empty) so a re-apply never clobbers
-- a stockpile that trading has since changed.
