-- Seed data: the Denzgraad Union name pool (a Russian-style one-party state).
--
-- Run in the Supabase SQL Editor AFTER schema/50_names.sql, and after the
-- Denzgraad nation exists (created via /adminsetup). The names are keyed to the
-- nation's actual id by matching its NAME below (any nation whose name contains
-- "Denzgraad" — e.g. "The Supreme Union of Denzgraadic Republics" or "Denzgraad
-- Union"). If no such nation exists yet, this inserts nothing (no error) — just
-- re-run it once the nation is there.
--
