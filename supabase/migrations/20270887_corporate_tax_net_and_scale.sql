-- ════════════════════════════════════════════════════════════════════
-- 20270887 — Corporate tax: legacy-scale rates normalized, base nets
-- out payroll
--
-- User report (Sierramar): 'corporate tax rate is 100%… also seems
-- to be a tax on gross income and not net profit.' Two defects:
--
--   1. nations.corporate_tax is meant to ride the 0-10 Active Laws
--      scale (Tax Haven 1 … Heavy 9; filings charge ×5 as a real
--      percent). Nations carrying legacy PERCENT-scale values
--      (Sierramar seeded on the old numeric scale and drifted to
--      ≥20) hit the ×5 mapping and clamp at 100%. Data fix: any
--      stored rate above 10 is unambiguously percent-scale and
--      converts back (÷5, clamped 0-10) — 20 becomes 4, i.e. a
--      20%% filing rate. Values ≤10 are left alone (they are
--      legitimate codebook positions).
--
--   2. file_corporate_tax taxed the corp_cash_events ledger with no
--      cost side — the ledger only receives revenue credits, so the
--      base was gross income. Re-emitted (20270873 body) deducting
--      the payroll_ytd accumulator (year-guarded; payroll debits
--      treasury directly and never writes the ledger, so no double
--      count). KNOWN FOLLOW-UP: production/materials spend has no
--      YTD accumulator and is not yet deductible.
--
-- Client mirror (business-corp.html) moves in lockstep: TAXES OWED
-- and the Pay Taxes modal compute on revenue − payroll YTD.
-- ════════════════════════════════════════════════════════════════════

BEGIN;

-- ── 1. Normalize percent-scale strays onto the 0-10 codebook ─────
UPDATE nations
   SET corporate_tax = LEAST(10, GREATEST(0, ROUND(corporate_tax / 5.0)))
 WHERE corporate_tax > 10;

-- ── 2. file_corporate_tax — net of payroll ────────────────────────
