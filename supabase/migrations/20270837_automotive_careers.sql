-- ════════════════════════════════════════════════════════════════════
-- 20270837 — The three AUTOMOTIVE careers: Manufacturing · Product ·
-- Commercial
--
--   MANUFACTURING — Production Engineer → Chief Manufacturing
--   Officer (CMO). PRODUCT — Product Engineer → Chief Product
--   Officer (CPO). COMMERCIAL — Field Sales Representative /
--   Marketing Analyst → Chief Commercial Officer (CCO).
--
-- All six rungs of each post from day one. Tracks pair with their
-- industry — construction hires Operations and Leadership, automotive
-- hires these three — and everything downstream (salary band, NPC
-- applicant rolls, ×12 headcount, hire → home-page employment record,
-- promote to the terminal rung) rides the existing machinery.
--
--   job_openings.track CHECK widened to the five tracks.
--   post_job_opening, _roll_job_applicants, and promote_employee
--   re-emitted byte-faithful to 20270830 except the new track CASEs
--   and the industry/track pairing gate.
-- ════════════════════════════════════════════════════════════════════
...
ALTER TABLE public.job_openings
    ...
    CHECK (track IN ('operations', 'leadership', 'manufacturing', 'product', 'commercial'));

-- ── _roll_job_applicants — automotive NPC flavor ──────────────────
...
-- ── post_job_opening — the three automotive ladders ───────────────
...
BEGIN
    ...
    END IF;
    IF p_track NOT IN ('operations', 'leadership', 'manufacturing', 'product', 'commercial') THEN
    ...
    END IF;
    IF v_corp.industry NOT IN ('construction', 'automotive') THEN
    ...
    -- and Leadership; automotive hires its three careers.
    IF (v_corp.industry = 'construction' AND p_track NOT IN ('operations', 'leadership'))
       OR (v_corp.industry = 'automotive' AND p_track NOT IN ('manufacturing', 'product', 'commercial')) THEN
        RETURN jsonb_build_object('success', false, 'reason', 'invalid_track');
    END IF;
...
-- ── promote_employee — automotive promotions ──────────────────────
