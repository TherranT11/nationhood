-- ════════════════════════════════════════════════════════════════════
-- 20270857 — Attaché Reputation: embassy decisions can move the
-- politician's REAL Reputation
--
-- Event authors gain a fifth stat, attache_reputation. Unlike the
-- four embassy-scoped stats, its deltas land on
-- factions.politician_reputation — the attaché's own name rises and
-- falls with the calls they make at the gate. Clamped 0-100 like the
-- rest; 'applied' reports the post-clamp movement as before.
-- embassy_event_decide re-emitted byte-faithful to 20270856 plus the
-- branch and the column in the write-back.
-- ════════════════════════════════════════════════════════════════════

BEGIN;

-- ── embassy_event_decide — the attaché's own name on the line ────
...
BEGIN
    ...
    v_att    := COALESCE(v_pol.politician_reputation, 0);
    ...
    FOR v_eff IN SELECT * FROM jsonb_array_elements(COALESCE(v_decision->'effects', '[]'::jsonb)) LOOP
        ...
        ELSIF v_stat = 'attache_reputation' THEN
            -- The attaché's OWN name (20270857): this one moves the
            -- politician's real Reputation stat, clamped 0-100.
            v_prev  := v_att;
            v_att   := LEAST(100, GREATEST(0, v_att + v_delta));
            v_delta := v_att - v_prev;
        ELSE
            CONTINUE;
        END IF;
        IF v_delta <> 0 THEN
    ...
    -- 20270856: a decided dispatch puts the embassy desk on a
    -- 12-tick cooldown — one foreign event per in-game year.
    UPDATE factions
       SET next_embassy_action_tick = v_tick + 12
     WHERE id = v_pol.id;
