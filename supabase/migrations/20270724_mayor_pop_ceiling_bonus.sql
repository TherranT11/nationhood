-- ════════════════════════════════════════════════════════════════════
-- 20270724 — Mayor wins raise the party's approval ceiling
--
-- Per user spec: every city your party holds a mayoralty in lifts
-- your party's popularity ceiling. +3% for a regular city, +5% for
-- the capital. Bonus is granted on the resolver's mayor-WIN branch
-- and reversed on incumbent LOSS so the ceiling tracks current
-- mayor holdings without drift.
--
-- Scope decisions:
--   • Player wins only. NPC mayors do not contribute to any party's
--     ceiling — the bonus is the player's earned mechanic. Existing
--     NPC-seeded cities therefore don't need a backfill pass.
--   • Mayor (non-capital) is symmetric — +3 on win, -3 on incumbent
--     eviction (existing 20270719 loss path). The cap subtraction
--     also clamps popularity_pct down so a previously high pop
--     doesn't sit above a freshly-lowered cap.
--   • Mayor of Capital pays +5 on win but has NO subtract path —
--     the office has no auto-term, no incumbent-loss branch, and
--     the cities row isn't yet stamped. Forward-only for now;
--     loss handling lands when the capital picker + cities stamp
--     do. Drift is bounded by the rarity of capital mayor turnover.
--
-- Body otherwise byte-faithful to 20270722 — only the mayor party
-- reward branch and the eviction block change. New mayor_of_capital
-- party reward branch added (was previously empty).
-- ════════════════════════════════════════════════════════════════════
