-- ════════════════════════════════════════════════════════════════════
-- 20270666 — Always-on governing coalition + Prime Minister
--
-- Symptom: nations with no majority party rendered "GOVERNING PARTY:
-- No majority" + "PRIME MINISTER: Vacant" on the Government & Politics
-- panel even after the general election had resolved. Montequilla
-- (252 seats, largest party Montequillan Dawn at 52) was stuck this
-- way despite multiple election cycles passing.
--
-- Root cause: three separate storage layers feed the display, and
-- only one was auto-populated by 20270594:
--
--   • head_of_government table       — auto-filled by 20270594 ✓
--   • administrations.pm_party_id    — only written by manual
--                                       coalition-formation RPC ✗
--   • ministries.prime_minister      — only written by manual
--                                       coalition-formation RPC ✗
--
-- politician-nation.html's Government strip reads from
-- administrations (coalition + PM party) and ministries (PM name),
-- not from head_of_government — so the HoG install was invisible.
--
-- Per user direction: every nation should ALWAYS have a governing
-- coalition and a Prime Minister. Coalition is built by walking
-- parties in seat-share order (desc, tie-break created_at ASC) and
-- accumulating until the coalition reaches ⌊total_seats / 2⌋ + 1.
-- The PM is the leader of the largest member (the first one added).
--
-- Two parts:
--
--   1. Re-emit resolve_due_general_elections to also write the
--      administrations row and the ministries.prime_minister row
--      whenever it installs HoG. End any prior active administrations
--      row + flip the prior PM ministry inactive before insert.
--
--   2. One-time backfill — for every nation that currently lacks an
--      active administrations row, run the same coalition + PM
--      install. Skips nations with no seated parties, no leader on
--      the largest party, or semi-presidential government types
--      (matching 20261120's existing v_is_semi_pres gate).
--
-- Re-emits resolve_due_general_elections — fifth time in this push
-- window (20270421 origin → 20270422 audit fix → 20270592 Deputy
-- Speaker clear → 20270593 Speaker clear → 20270594 HoG install →
-- 20270666 this). The orchestrator is officially overdue for a
-- per-nation-helper refactor; flagged again here, still deferred
-- for narrow scope.
-- ════════════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════════════
-- Part 1 — resolve_due_general_elections
-- ════════════════════════════════════════════════════════════════════
    ...
    LOOP
        ...
        IF v_largest_party.id IS NOT NULL
           ...
           AND v_largest_party.leader_age IS NOT NULL THEN
            ...
            -- Semi-presidential systems have a separately-elected PM
            -- (or no PM at all). 20261120's coalition-formation RPC
            -- skips administrations + ministries writes for them; mirror
            -- that gate here so we don't overwrite a separately-driven
            -- arrangement.
            ...
            IF NOT v_is_semi_pres THEN
                ...
                -- Walk parties by seat share desc; accumulate until the
                -- coalition reaches the majority threshold. PM is the
                -- leader of the largest member (the first one added).
                -- Tie-break on created_at ASC, same convention as the
                -- HoG pick above.
                FOR v_party IN
                ...
                -- Snapshot fields — mirror 20261120's coalition-formation
                -- INSERT shape so we don't trip an unknown NOT NULL on
                -- the administrations table (CREATE TABLE lives outside
                -- the migration tree; play defensive on schema unknowns).
                ...
                -- End any active administrations row before inserting
                -- the new one. end_reason='general_election' distinguishes
                -- this auto-flip from the manual 'new_government' reason
                -- used by 20261120's coalition-formation RPC.
                ...
                -- Update the prime_minister ministry row in place if it
                -- exists, otherwise insert. Matches 20261119's
                -- UPDATE-or-INSERT pattern that drops the is_active=true
                -- filter so old inactive rows get reactivated rather
                -- than 23505'ing on ministries_nation_ministry_unique.
...
-- ════════════════════════════════════════════════════════════════════
-- For every nation that currently lacks an active administrations row,
-- run the same coalition + PM install logic. Skips nations with no
-- seated parties, no leader on the largest party, or semi-presidential
-- government types. Idempotent: re-running this DO block on a nation
-- that already has an active admin row is a no-op (the WHERE clause
-- filters it out).
DO $$
DECLARE
    v_tick               int;
...
BEGIN
    ...
    LOOP
        ...
        -- Largest-party leader guard — matches 20270594 + Part 1.
        SELECT * INTO v_largest_party
          FROM factions
         WHERE nation_id    = v_nation.id
           AND faction_type = 'movement_party'
        ...
        -- Coalition walk (identical to Part 1).
        v_majority        := (v_seats / 2) + 1;
        v_coalition_array := '[]'::jsonb;
        v_coalition_seats := 0;
        v_pm_party_id     := NULL;
        ...
        -- Snapshot fields — mirror 20261120 / Part 1.
        v_hos_name := NULLIF(btrim(
            COALESCE(v_nation.head_of_state_first_name, '') || ' ' ||
            COALESCE(v_nation.head_of_state_last_name,  '')
        ), '');
        ...
        -- Ensure HoG is also seated (20270594's path may have skipped
        -- this nation if it landed before that migration ran an
        -- election cycle). Same active-row deactivate-then-insert
        -- pattern.
        UPDATE head_of_government
           SET active = false
         WHERE nation_id = v_nation.id
    ...
    RAISE NOTICE '20270666 backfill: installed coalition + PM in % nation(s)', v_installed;
END $$;
