-- ════════════════════════════════════════════════════════════════
-- finalize_government_formation: drop is_active=true filter on the
-- per-ministry UPDATE so an inactive row from a prior government
-- doesn't trigger a duplicate-key conflict on INSERT.
--
-- Bug shape:
--   The unique constraint ministries_nation_ministry_unique is on
--   (nation_id, ministry_key) — full uniqueness, NOT partial on
--   is_active. The 20260906 / 20260910 RPC body's per-ministry
--   UPSERT used "UPDATE ... WHERE nation_id = X AND ministry_key = Y
--   AND is_active = true" then "IF NOT FOUND, INSERT". When a prior
--   government left an INACTIVE row at (X, Y), the UPDATE missed it
--   (filtered out by is_active=true), the INSERT then 23505'd on the
--   unique constraint, and form-government failed with:
--     duplicate key value violates unique constraint
--     "ministries_nation_ministry_unique"
--
-- Fix: drop "AND is_active = true" from the per-ministry UPDATE
-- (lines 306-308 in this body). The UPDATE now matches the inactive
-- row, the SET clause includes "is_active = true" so it gets
-- reactivated in place. INSERT only fires when no row exists at all
-- — a true first-time creation.
--
-- The other three is_active=true filters in this RPC are unchanged:
--   - Line 124 (SELECT discretionary balances): only active rows
--     have meaningful balances; correct.
--   - Line 192 (vacate UPDATE): the inactive rows are already vacant
--     (no party/minister to clear); skipping them is correct.
--   - Line 266 (PM balance restore): restores to the active PM row;
--     correct since only one PM row should be active.
--
-- All other RPC logic preserved verbatim from
-- 20260910_add_sports_ministry_to_finalize_formation.sql.
-- ════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF NOT v_is_semi_pres THEN
    ...
    LOOP
      ...
      -- BUG FIX (20261119): dropped "AND is_active = true" so the
      -- UPDATE matches inactive rows from prior governments and
      -- reactivates them in place via the SET is_active = true above.
      -- Without this, the INSERT below 23505'd on the unique
      -- constraint ministries_nation_ministry_unique.
      UPDATE ministries SET
