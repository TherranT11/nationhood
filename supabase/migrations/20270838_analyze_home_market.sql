-- ════════════════════════════════════════════════════════════════════
-- 20270838 — Analyze Market reads your nation only
--
-- Design change to Executive Action #3 (20270836): when the action
-- starts there is no nation pick — the analysis always reads the
-- corp's HQ nation. Foreign market intel arrives with a later
-- unlock. Adding/removing a parameter means a new signature, so the
-- old 3-arg function is DROPPED (PostgREST overload ambiguity) and
-- the body re-emitted byte-faithful to 20270836 except the nation
-- source.
-- ════════════════════════════════════════════════════════════════════

BEGIN;

DROP FUNCTION IF EXISTS public.analyze_vehicle_market(uuid, uuid, text);

-- ── analyze_vehicle_market — HQ nation only ───────────────────────
