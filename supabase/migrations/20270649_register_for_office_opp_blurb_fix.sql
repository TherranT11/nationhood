-- ════════════════════════════════════════════════════════════════════
-- 20270649 — register_for_office: non-null opp_blurb for Tier-2 races
--
-- Bug: politician_register_for_office (latest body in 20270635) inserts
-- NULL for opp_blurb on the city_council and senior_mp branches, but
-- politician_active_election.opp_blurb has been text NOT NULL since
-- 20270398. The community-tier path sources its blurb from a stored
-- politician_get_election_offer row; the Tier-2 paths auto-generate
-- candidates and have nowhere to pull a blurb from, so 20270635 just
-- left it NULL — the INSERT crashes with SQLSTATE 23502 at the moment
-- the player clicks Run for Election.
--
-- Re-emit byte-identical to 20270635 with one change: the opp_blurb
-- column in the INSERT VALUES is a CASE on race tier instead of NULL.
-- Strings are short flavor-only fillers; the client renders them in
-- the active-race card next to the opponent name. opp_party_name
-- stays NULL — the client renders "Independent" for null parties.
-- ════════════════════════════════════════════════════════════════════
