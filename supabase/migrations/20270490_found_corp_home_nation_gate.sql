...
--
-- Production parity note: factions.ent_origin_nation was added in
-- sql/migrations/20270349_entrepreneur_travel.sql, which never made
-- it into supabase/migrations and so isn't guaranteed on the prod DB.
-- The ALTER + UPDATE below mirror the relevant slice of 20270349 so
-- the column + initial snapshot exist before the function compiles;
-- without them, COALESCE(v_fac.ent_origin_nation, …) errors out.

BEGIN;

-- Ensure ent_origin_nation exists and is seeded (snapshot from current
-- nation for any entrepreneur that pre-dates the travel migration).
ALTER TABLE factions ADD COLUMN IF NOT EXISTS ent_origin_nation TEXT;
UPDATE factions
   SET ent_origin_nation = nation
 WHERE faction_type = 'entrepreneur'
   AND ent_origin_nation IS NULL;
