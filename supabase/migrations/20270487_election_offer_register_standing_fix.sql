-- Second round of the politician_charisma → politician_standing
-- rename gap closure. 20270485 fixed _mp_action_check +
-- politician_mp_fundraising_dinner + politician_resolve_due_elections.
-- This migration fixes the two remaining live functions that read the
-- renamed column:
--
--   1. politician_get_election_offer (20270398 line 274)
--      Loads the parliament race opponent's stat at offer-generation
--      time. Symptom: "Could not load races. record 'v_pol' has no
--      field 'politician_charisma'" when the Run for Election modal
--      opens. (Reported.)
--
--   2. politician_register_for_office (20270432 line 187)
--      Reads the politician's standing as the Senior MP stat at
--      registration time. Would have hit the same error as soon as
--      a user tried to Register for Senior MP.
--
-- Audit done in this commit: comprehensive grep across every live
-- politician_* function (and _mp_action_check). Only these two
-- carried stale v_pol.politician_charisma references after 20270485;
-- everything else either already updated (helper, MP rally, fundraising
-- dinner) or was superseded by a later definition that doesn't touch
-- the column (door_knock, give_speech, stand_for_election,
-- floor_speech, etc.).
--
-- Fix shape — same as 20270485: surgical column swap only. JSON keys
-- + value semantics ('charisma', stat_reward.kind = 'charisma') stay
-- as-is so client formatters keep working without a paired change.

BEGIN;

-- ── 1. politician_get_election_offer ────────────────────────────────
...
BEGIN
    ...
    v_parl_your := COALESCE(v_pol.politician_standing, 1);
...
BEGIN
    ...
    ELSE
        ...
        v_your_stat := COALESCE(v_pol.politician_standing, 1);
        v_opp_stat  := 1 + floor(random() * 6)::int;
