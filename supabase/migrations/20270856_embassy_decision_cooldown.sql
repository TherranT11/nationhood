-- ════════════════════════════════════════════════════════════════════
-- 20270856 — Embassy dispatches cool down 12 ticks after a decision
--
-- Design ruling: once a foreign event is decided, the embassy desk
-- rests for 12 ticks (an in-game year) before the next one can be
-- drawn. Previously only the DRAW burned a single tick and deciding
-- was free, so a fresh dispatch was available every tick.
-- embassy_event_decide re-emitted byte-faithful to 20270779 except
-- the cooldown stamp and the next_action_tick in the payload (the
-- desk tile reads factions.next_embassy_action_tick as before — no
-- new client plumbing).
-- ════════════════════════════════════════════════════════════════════

BEGIN;

-- ── embassy_event_decide — the year of quiet after the call ──────
