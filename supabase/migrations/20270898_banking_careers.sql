-- ════════════════════════════════════════════════════════════════════
-- 20270898 — Banking career #1: the MANAGEMENT track
--
-- Five rungs, field to headquarters — run a branch, run a region,
-- run all the branches, run the bank's money, run the books:
--   1. Branch Manager
--   2. Regional Director
...
-- Hiring rides the existing job board end to end: post_job_opening
-- learns the management titles and the banking↔management industry
-- pairing; promote_employee learns the promotion map; the NPC
-- applicant roller gets banking flavor (titles + employers). The
-- hire path already copies the opening's title onto the player's
-- faction (biz_job_title, 20270799), and businessman-home's
-- EMPLOYMENT affiliation card resolves the track from the title via
-- the client JOB_TRACKS catalog — so accepting a banking job
-- surfaces the affiliation with zero new client machinery.
--
-- Re-emits the three 20270837 emissions byte-identical except the
-- management branches — plus ONE PRODUCTION FIX: the applicant
-- roller still read cities.jobs, which 20270881 renamed to
-- unemployment, so every job posting has thrown since that merge.
-- The weighted city draw now reads unemployment.
-- ════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    FOR i IN 1..v_n LOOP
        ...
        IF v_exp = 0 OR random() < 0.35 THEN
            ...
            v_title      := '—';
        ELSE
            v_employment := CASE WHEN p_track = 'management'
...
BEGIN
    ...
    END IF;
    IF p_track NOT IN ('operations', 'leadership', 'manufacturing', 'product', 'commercial', 'management') THEN
    ...
    -- hires Management (its first of three, 20270898).
    IF (v_corp.industry = 'construction' AND p_track NOT IN ('operations', 'leadership'))
       ...
       OR (v_corp.industry = 'banking' AND p_track <> 'management') THEN
        RETURN jsonb_build_object('success', false, 'reason', 'invalid_track');
    END IF;
