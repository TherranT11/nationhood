-- ════════════════════════════════════════════════════════════════════
-- 20270751 — Mayor race: 6-tick filing window for player-held cities
--
-- Today: anyone with enough Experience can file against any sitting
-- Mayor, NPC or player. Race resolves the next tick.
--
-- New rule per user spec: when the target city's Mayor is a PLAYER
-- (signalled by cities.mayor_term_end_tick IS NOT NULL — NPC
-- mayors leave it NULL per the 20270719 stamp convention), filing
-- is gated to a 6-tick window before the term ends. Opens at tick
-- mayor_term_end_tick - 6; before that the RPC returns
-- 'mayor_player_too_early' with the unlock tick so the client can
-- display "Eligible to run on Tick X" instead of a vague rejection.
--
-- NPC-held cities (mayor_term_end_tick IS NULL) keep the immediate
-- file-then-resolve-next-tick cadence.
--
-- Applies to:
--   • p_target_office = 'mayor'              — picked city
--   • p_target_office = 'mayor_of_capital'   — the capital city
--
-- CCP / CCM / Regional Leader / Senior MP / MP races are unchanged
-- here; the user's spec is specifically about the Mayor race.
-- Tail of the function (opponent name pool, polling math,
-- politician_active_election insert, career_events insert, RETURN
-- shape) is preserved byte-for-byte from 20270740 so this is a
-- pure additive gate.
-- ════════════════════════════════════════════════════════════════════
