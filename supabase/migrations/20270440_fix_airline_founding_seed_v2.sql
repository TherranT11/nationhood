-- ════════════════════════════════════════════════════════════════════
-- Fix found_entrepreneur_corp airline seed — corp_aircraft INSERT
--
-- 20270425 (entrepreneur_starter_building_cost) regressed the airline
-- starter seed: its CREATE OR REPLACE FUNCTION found_entrepreneur_corp
-- still has
--
--     UPDATE entrepreneur_corps SET aircraft_regional_owned = 2 WHERE …
--
-- but that column was DROPPED by 20270204
-- (entrepreneur_aircraft_rpcs.sql), which moved aircraft inventory
-- onto its own corp_aircraft rows. 20270226 and 20270254 BOTH had
-- the correct corp_aircraft INSERT in their copies of the RPC, but
-- 20270425 was forked from a stale source and lost the fix.
--
-- Player symptom: founding an Airline corp throws
--   "Could not found: column "aircraft_regional_owned" of relation
--    "entrepreneur_corps" does not exist"
-- after the modal hits FOUNDING — and the entire transaction rolls
-- back (no partial corp lands).
--
-- This migration restores the corp_aircraft INSERT (2 regional rows
-- via generate_series, same pattern as 20270204 / 20270226 / 20270254).
-- Everything else in 20270425's RPC body (nation-scaled building cost,
-- listing, terminal seed, aviation 1D6, freighters) is preserved
-- byte-for-byte.
-- ════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    -- Airline starter seed: 2 free regional aircraft (corp_aircraft rows
    -- are the source of truth since 20270204 — aircraft_regional_owned
    -- column was dropped there) + 2 same-nation starter terminals.
    ...
    -- Starter unique building. cost_paid is v_building_cost (the
    -- nation-scaled starter cost set by 20270425). Aviation
    -- Manufacturing rolls 1D6 for light vs engine.
