-- ════════════════════════════════════════════════════════════════════
-- 20270901 — Three-tier MP ladder: Junior MP → Member of Parliament →
--            Senior MP (BACKEND ONLY — client lands in a follow-up)
--
-- The elected-chamber climb gains a middle rung. Display names (client
-- commit): member_of_parliament = "Junior MP", the new full_mp =
-- "Member of Parliament", senior_mp = "Senior MP". This migration is
-- the engine half only — the politician-career.html rungs + labels and
-- the Senior-MP "Register"→"Run for Re-Election" swap come next, so the
-- old client keeps working against this backend in the meantime.
--
-- Promotion model (user spec): you advance one rung by WINNING a
-- re-election filed in the final 5 ticks before the general election.
--   • Junior MP (member_of_parliament): wins the initial seat via the
--     ordinary parliament race (unchanged). Re-electing files for the
--     next rung up.
--   • Member of Parliament (full_mp): a sitting Junior MP re-elects
--     (≤5 ticks to the general election). No term gate.
--   • Senior MP (senior_mp): a sitting Member of Parliament re-elects
--     (≤5 ticks) AND must have completed a full parliamentary term as
--     full_mp. A sitting Senior MP re-runs to hold the seat.
--
-- How it's wired without a new client:
--   politician_stand_for_election still takes p_tier='parliament' from
--   the existing [Run for Re-Election] button. Internally it now picks
--   the *race_tier* by the caller's current office — member_of_parliament
--   → 'full_mp', full_mp → 'senior_mp', senior_mp → 'senior_mp' — so the
--   resolver promotes the winner up one rung. The standalone Senior-MP
--   path (politician_register_for_office('senior_mp')) is left intact
--   here; the client commit retires that action.
--
-- Resolver parity: the new 'full_mp' race_tier behaves like the other
-- parliament tiers for the party seat (+1 on win / −1 on incumbent
-- loss), the +1 Experience reward, and the office stamp. Eviction now
-- fires for a sitting member_of_parliament or full_mp who loses their
-- promotion re-run, mirroring the senior_mp eviction added in 20270882.
--
-- NOTE: nothing here drops the two retired Speaker columns (20270900)
-- or the senior_mp register branch; those are untouched.
-- ════════════════════════════════════════════════════════════════════

BEGIN;

-- ── 1. Allow 'full_mp' as an office and a race tier ──────────────
...
ALTER TABLE factions
    ...
    CHECK (politician_office IS NULL
           OR politician_office IN (
               'community_organizer', 'member_of_parliament', 'full_mp',
...
ALTER TABLE politician_active_election
    ...
    CHECK (race_tier IN (
        'community', 'parliament', 'full_mp',
        'city_council', 'senior_mp',
        'city_council_president',
        'mayor', 'mayor_of_capital', 'regional_leader'
    ));

-- ── 2. politician_stand_for_election — re-election promotes a rung ─
-- 20270770 body, with v_race_tier chosen by the caller's current
-- office on a re-election filing (everything else byte-identical).
...
BEGIN
    ...
    -- Incumbent re-election (tier='parliament' only), in the final 5
    -- ticks of the term, resolving AT the general election. 20270901:
    -- the win promotes one rung, so the *race_tier* is chosen by the
    -- caller's current office rather than always 'parliament'.
    IF v_pol.politician_office IS NOT NULL THEN
        SELECT next_election_tick, COALESCE(parliamentary_term_ticks, 24)
          INTO v_election_tick, v_term_ticks
          FROM nations WHERE id = v_pol.nation_id;
        IF NOT (
            p_tier = 'parliament'
            AND v_pol.politician_office IN ('member_of_parliament','full_mp','senior_mp')
        ...
        IF v_pol.politician_office = 'member_of_parliament' THEN
            v_race_tier := 'full_mp';                      -- Junior MP → Member of Parliament
        ELSIF v_pol.politician_office = 'full_mp' THEN
            -- Senior MP requires a completed full term as full_mp.
            IF v_pol.politician_office_won_at_tick IS NULL
               OR v_tick < v_pol.politician_office_won_at_tick + v_term_ticks THEN
                RETURN jsonb_build_object('success', false, 'reason', 'term_too_short',
                    ...
                    'current_tick',     v_tick);
            END IF;
            v_race_tier := 'senior_mp';                    -- Member of Parliament → Senior MP
        ELSE
            v_race_tier := 'senior_mp';                    -- sitting Senior MP re-runs to hold
        END IF;
...
-- ── 3. politician_resolve_due_elections — handle the full_mp tier ──
-- 20270882 body. Edits: 'full_mp' joins the parliament-tier party-seat
-- reward, the +1 Experience reward, and the office-stamp CASE; the
-- incumbent-loss eviction now covers a member_of_parliament or full_mp
-- who loses a promotion re-run. (The one-time repair DO block from
-- 20270882 is not re-run.)
...
BEGIN
    ...
    IF v_won AND v_race.party_id IS NOT NULL THEN
        IF v_race.race_tier IN ('parliament', 'full_mp', 'senior_mp') THEN
            UPDATE factions
    ...
    -- 20270709. 20270901: full_mp joins the parliament tier here.
    IF v_won THEN
        IF v_race.race_tier IN ('parliament', 'full_mp', 'senior_mp') THEN
            UPDATE factions
    ...
    -- capital race tier).
    -- 20270882 / 20270901: MP-tier incumbent eviction — losing the
    -- re-election vacates the seat, mirroring the Mayor branch below.
    -- A sitting Junior MP (member_of_parliament) losing a promotion
    -- re-run (race_tier 'full_mp', or a legacy in-flight 'parliament')
    -- is out; a sitting Member of Parliament (full_mp) losing the
    -- 'senior_mp' promotion is out; a sitting Senior MP losing any
    -- parliament-tier race is out. The party's seat count rides down
    -- (wins ride it up above).
    IF NOT v_won
       AND ((v_race.race_tier IN ('parliament', 'full_mp') AND v_pol.politician_office = 'member_of_parliament')
            OR (v_race.race_tier = 'senior_mp' AND v_pol.politician_office = 'full_mp')
            OR (v_race.race_tier IN ('parliament', 'full_mp', 'senior_mp')
