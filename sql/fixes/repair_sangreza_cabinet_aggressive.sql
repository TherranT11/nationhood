-- Aggressive repair for Sangreza's empty cabinet.
--
-- Symptom: Government → Administrative shows "Pacheco Administration" with
-- PM Luca Pacheco (PNI), but CABINET = "0 ministries" and the legend says
-- "PNI holds 1, NLP holds 0".
--
...
BEGIN
    ...
    IF v_formation.id IS NULL THEN
        RAISE EXCEPTION 'No formed/active government_formation for Sangreza';
    END IF;
    ...
    IF array_length(v_coalition_ids, 1) IS NULL THEN
        RAISE EXCEPTION 'Unable to determine coalition parties for Sangreza';
    END IF;

    RAISE NOTICE 'Sangreza: formation=%, pm=%, coalition size=%',
