-- =========================================================================
-- Re-CREATE generate_organic_shipping_routes so newly-generated fuel_energy
-- lanes carry shipping_subsector='specialized_transport'.
--
-- Companion migration to 20260429_shipping_fuel_energy_specialized_transport
-- (which backfilled existing rows) and the matching js change in
-- js/game/shipping.js + supabase/functions/advance-corp-tick/index.ts.
-- The RPC was missed by the first round — without this, the engine would
-- keep generating fuel routes under the old wrong subsector each tick,
-- silently reverting the backfill. The CHECKED INTERIM bug:
--   tick N:    backfill flips fuel routes to specialized_transport.
--   tick N+1:  engine regenerates fuel lanes as bulk_cargo (RPC unfixed).
-- This migration closes the loop.
--
-- Body is a verbatim copy of the function defined in
-- 20260427_unify_transit_ticks_formula.sql with one row changed in the
-- sector_meta CTE: fuel_energy.subsector flipped from bulk_cargo to
-- specialized_transport. Every other formula (transit_ticks, revenue
-- rate, scope rules, expiry window) is unchanged.
-- =========================================================================
...
BEGIN
    ...
    WITH sector_meta(sector, subsector, category, goods, goods_sub, vessel, vessel_note, unit, revenue_rate) AS (
        VALUES
            ('fuel_energy',        'specialized_transport', 'FUEL',     'Fuel & Energy Products',    'Petroleum, LNG, coal, refined fuels',         'Tanker',         'Double-hull required.',    'tons', 0.06::NUMERIC),
    ...
    inserted AS (
        ...
        ON CONFLICT (origin_nation_id, destination_nation_id, trade_sector, status) DO UPDATE SET
            ...
            shipping_subsector  = EXCLUDED.shipping_subsector,
