-- Hold a Rally — server-side RPC (replaces the client-side mutation)
--
-- The rally moved off the retired momentum/AP system onto sector
-- popularity + party_funds. But faction_sector_popularity is
-- admin/service-role write-only (RLS "Admin writes" USING is_admin();
-- 20260426_sectors_phase0.sql), so the client-side upsert in
-- js/game/political-actions.js was RLS-rejected — every rally failed.
--
-- This is the same fix the player-initiated sector actions already use
-- (petition_for_reform, adopt_platform, the energy/agriculture surveys):
-- a SECURITY DEFINER RPC that gates auth + one-per-tick + escalating cost
-- ATOMICALLY (FOR UPDATE on the faction row), then rolls the weighted
-- outcome and writes popularity, funds, and the campaign_actions log in
-- one transaction. Server-authoritative tick + the row lock close the
-- non-atomic double-fire / stale-read gaps the client version had.
--
-- Rally logic now lives in exactly one place (this function). The JS
-- side becomes a thin supabase.rpc('hold_rally', ...) caller.
--
-- Cost:      $50,000 base + $25,000 per escalation level
-- Escalation: +1 per rally in the last 10 ticks, -1 per tick since the
--             last rally (same curve the AP cost used)
-- Cooldown:  one rally per tick (hard); rallied_recently within 5 ticks
--            feeds the "stale material" weight penalty
-- Effect:    weighted 1-of-6 outcome shifts the faction's popularity in
--            the chosen sector (integer tenths, clamp 0..100); divisive
--            / counter-protest also bleed -0.2 into every other sector
-- Traits:    crowd_pleaser / wooden_speaker shift the outcome weights;
--            telegenic / crowd_pleaser scale a positive result; ported
--            from party-leadership.js so behaviour is unchanged
--
-- Idempotent: CREATE OR REPLACE + GRANT. No schema changes.
...
BEGIN
    ...
    SELECT COUNT(*) INTO v_crisis FROM active_crises WHERE nation_id = v_nation.id;
    ...
    IF v_crisis > 0 THEN
        w_gaffe := w_gaffe + 6; w_divisive := w_divisive + 4; w_counter := w_counter + 10;
        w_rousing := w_rousing - 8; w_solid := w_solid - 6;
    END IF;
