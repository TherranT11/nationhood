-- Finance Minister "Debt Payment" action.
--
-- Lets the minister move discretionary cash to the national debt:
--   discretionary -= ($2M flat fee + p_payment * $1M)
--   nation.debt  -= p_payment * $1M  (clamped to 0)
--
-- Cooldown: 1 tick, tracked via ministry_action_log (same pattern as
-- invest_in_vola_culture so the per-ministry cooldown logic stays in
-- one place). p_payment is an INT in the unified $1M abstract scale —
-- player types "5" meaning $5 → $5M raw applied to both columns.
...
DECLARE
    ...
    v_fee_raw      BIGINT := 2000000;   -- $2M flat
    v_total_raw    BIGINT;
...
BEGIN
    ...
    -- Caller must hold the Finance ministry seat. Mirrors the
    -- invest_in_vola_culture caller check so the auth path is identical.
    ...
    SELECT debt INTO v_debt FROM nations WHERE id = v_nation_id FOR UPDATE;
