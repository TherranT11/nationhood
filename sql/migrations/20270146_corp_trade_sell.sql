-- ════════════════════════════════════════════════════════════════════
-- ENTREPRENEUR STOCK SYSTEM — Phase 4: corp_trade (sell)
-- ════════════════════════════════════════════════════════════════════
-- Fills the sell branch left disabled in Phase 3 (20270145). SAME
-- signature corp_trade(uuid, text, int) — body-only CREATE OR REPLACE,
-- so PostgREST routing is unchanged and NO schema reload is needed
-- (20270143: a signature change is "the one case" that needs the
-- reload — this is not one). GRANT re-stated for idempotency.
--
-- Bonding curve, ±5% per share. With F = rᴺ (r = 1.05) and the shared
-- unit U = (F − 1)/(r − 1):
--   BUY  N:  cost     = P · U          price → P · F
--   SELL N:  proceeds = P · U / F      price → P / F
-- The two are exact inverses: buy N then sell N restores the price to
-- P and the cash nets to zero (to whole dollars). F uses an integer
-- exponent → Postgres numeric_power's exact path (terminating
-- decimal); negative exponents are avoided entirely.
--
-- Rounding always favours the treasury so it can never be drained by
-- rounding and no round-trip can mint money: BUY pays ceil(cost),
-- SELL pays floor(proceeds). A defensive treasury-solvency check
-- guards the sell payout (the model already guarantees solvency —
-- selling everything down returns < what was seeded — but it is money,
-- so we assert).
--
-- One source of truth: share_price / shares_outstanding /
-- treasury_cash / corp_shareholdings are mutated ONLY here. RLS blocks
-- client writes (Phase 1: column REVOKE + no write policy); this
-- SECURITY DEFINER function is the sole writer. Lock order
-- (deadlock-safe, keep consistent): corp row, then faction row, then
-- the holding row.
--
