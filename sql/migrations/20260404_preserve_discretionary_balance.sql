...
BEGIN
  ...
  -- Stats snapshot for administration history — select only stat columns to avoid
  -- "cannot cast type uuid[] to jsonb" if nations table has array columns at runtime.
  SELECT to_jsonb(sub) INTO v_stats_snapshot FROM (
    SELECT gdp, gdp_growth, debt, debt_growth, inflation, interest_rates,
    ...
    FROM nations WHERE id = v_nation.id
  ) sub;
