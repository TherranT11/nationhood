-- Fix finalize_government_formation: populate HoS / PM-party / HoS-election-method
-- on the administration row at creation time.
--
-- PROBLEM
--   Prior versions of this RPC left `administrations.head_of_state`,
--   `pm_party_id`, `pm_party_name`, and `hos_election_method` NULL. The
--   PAST ADMINISTRATIONS card then showed "Unknown" for the Head of State
--   and produced blank fallbacks for PM party metadata, because the only
--   fields written at INSERT time were `head_of_state_title`, `total_seats`,
--   `coalition_parties`, and (later, in a follow-up UPDATE) `prime_minister`
--   and `admin_name`.
--
-- FIX
--   Load the PM faction BEFORE inserting the admin row so we have
--   `v_pm_faction.faction_name` available for the INSERT. Compute a
--   concatenated `v_hos_name` from `nations.head_of_state_first_name/last_name`,
--   NULLIF-trimmed so a row with no HoS still inserts NULL (not empty string).
--   Add `head_of_state`, `hos_election_method`, `pm_party_id`, `pm_party_name`
--   to the INSERT column list. The follow-up UPDATE is unchanged.
--
--   Full function body re-declared (matches 20260411_fix_formation_ideology_join.sql)
--   so the PM ideology fix is preserved; this migration is a superset.
--
...
BEGIN
  ...
  -- Load PM faction BEFORE inserting the admin row so pm_party_name is
  -- available for INSERT (nullable — if v_pm_party_id is NULL, v_pm_faction
  -- stays NULL and the INSERT falls back to NULL for the party columns).
  IF v_pm_party_id IS NOT NULL THEN
    SELECT f.* INTO v_pm_faction FROM factions f WHERE f.id = v_pm_party_id;
  END IF;

  -- Compose Head of State full name from the nation row. NULLIF(TRIM(...), '')
  -- ensures a nation with no HoS stored (both first_name / last_name NULL)
  -- results in NULL, not a blank string.
  v_hos_name := NULLIF(TRIM(
      COALESCE(v_nation.head_of_state_first_name, '') || ' ' ||
      COALESCE(v_nation.head_of_state_last_name, '')
  ), '');

  -- Create new administration — now writes HoS name, HoS election method,
  -- PM party ID, and PM party name so the UI doesn't need to fall back.
  ...
  IF v_pm_faction.id IS NOT NULL AND v_pm_faction.leader_first_name IS NOT NULL THEN
    ...
    -- Fill prime_minister / admin_name on the row we just inserted.
    -- pm_party_name was already written at INSERT; repeat it here defensively
    -- so any legacy UPDATE path that reaches this branch also converges.
    UPDATE administrations SET
