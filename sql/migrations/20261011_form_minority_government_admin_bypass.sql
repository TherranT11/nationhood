-- ════════════════════════════════════════════════════════════════
-- form_minority_government: admin-override bypass on auth gates.
--
-- Reported: Calveth softlocked in coalition formation; admin viewing
-- the action as a non-CPP party hit "Only the largest active party
-- (Calveth People's Party) may form a minority government."
--
-- Two gates reject admin override:
--   1. v_caller_faction lookup (id = auth.uid() OR linked_user_id =
--      auth.uid()) — fails because the admin's UUID doesn't map to a
--      party in the inspected nation.
--   2. v_largest_active.id <> v_caller_faction.id — would fail even
--      if (1) returned a stray faction; admin isn't acting as CPP.
--
-- Fix: under is_admin(), skip the v_caller_faction lookup and assign
-- v_largest_active into v_caller_faction once it's computed. All
-- downstream INSERTs (proposed_by, party_ids, ministry_assignments,
-- finalize_government_formation arg, event_log faction_name) then
-- attribute correctly to the largest active party — admins can't
-- arbitrarily nominate a different party as PM, mirroring the
-- non-admin invariant.
--
-- finalize_government_formation already has its own is_admin() bypass
-- (20260910:91), so the delegated call goes through without further
-- patching.
--
-- Idempotent (CREATE OR REPLACE). The formed_at_tick column added in
-- 20261003 stays — re-running this migration doesn't duplicate it.
-- ════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    -- failed_formation_attempts. finalize already has its own
    -- is_admin() bypass (20260910:91).
...
COMMENT ON FUNCTION form_minority_government(UUID) IS
    'Player-triggered minority government formation. Caller must be leader of the largest active party (last_seen_tick within 4 ticks); admin override bypasses both the party-ownership lookup and the largest-active comparison, acting as the largest active party for attribution. Gates: post-election formation deadline (3 ticks) elapsed, no existing formed/caretaker coalition, no party with outright majority, parliamentary government type. Inserts an emergency_minority formation and delegates to finalize_government_formation; existing bills.js code reads formation_type=''emergency_minority'' as a -20%% effective YES-vote penalty. Auto-snap election fires from the tick processor at formed_at_tick + 36.';
