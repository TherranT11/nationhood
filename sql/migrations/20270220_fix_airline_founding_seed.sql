-- ════════════════════════════════════════════════════════════════════
-- FIX: airline founding seeds corp_aircraft ROWS, not a dropped column
-- ════════════════════════════════════════════════════════════════════
-- Regression: 20270204 moved aircraft ownership from count columns on
-- entrepreneur_corps (aircraft_regional_owned, …) to per-aircraft rows in
-- corp_aircraft, and DROPPED those columns. 20270204's found_entrepreneur_corp
-- correctly seeds an airline with 2 corp_aircraft rows.
--
-- 20270213 (aviation_manufacturing) then re-defined found_entrepreneur_corp
-- from an older copy whose airline branch still ran
--   UPDATE entrepreneur_corps SET aircraft_regional_owned = 2
-- (its comment claimed "unchanged" — it wasn't). Against the post-20270204
-- schema that column no longer exists, so founding any airline corp fails
-- with: column "aircraft_regional_owned" of relation "entrepreneur_corps"
-- does not exist.
--
-- This re-defines found_entrepreneur_corp identical to 20270213 (keeping its
-- aviation_manufacturing building branch) with ONLY the airline starter seed
-- corrected back to inserting corp_aircraft rows. Apply after 20270213.
