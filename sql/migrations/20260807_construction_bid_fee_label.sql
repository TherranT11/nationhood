-- 20260807_construction_bid_fee_label.sql
--
-- Same fix pattern as 'Project per-tick cost' → 'Project: <name>'
-- (commit 31893c0): the bid fee cash event was logged with a generic
-- 'Construction bid fee' label, so a corp bidding on multiple
-- contracts saw N indistinguishable -$50k entries each tick. Now
-- includes the contract name.
--
-- Idempotent CREATE OR REPLACE. Function body otherwise unchanged
-- from 20260714_route_silent_cash_leaks.
