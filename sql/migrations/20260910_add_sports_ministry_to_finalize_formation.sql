...
--
-- Two changes in one migration, both supporting the new Minister of
-- Sports cabinet seat:
--
-- 1. Backfill: insert a vacant 'sports' ministry row for every existing
--    nation that doesn't already have one. The cabinet UI renders from
--    existing ministries rows, so without this the new seat wouldn't
--    appear at all in nations that have never re-formed since the
--    change. Idempotent (NOT EXISTS guard).
--
-- 2. RPC update: add 'sports' → 'Ministry of Sports' to the
--    v_office_names map inside finalize_government_formation so future
--    formations write the correct display name (instead of falling back
--    to the raw key 'sports'). Mirrors the new entry added to
--    MINISTRY_OFFICE_NAMES in js/game/government-types.js.
--
-- The RPC body below is a CREATE OR REPLACE — only v_office_names
-- changes versus 20260906_finalize_formation_atomic_ministries.sql.
-- All other function logic is preserved verbatim. Idempotent.

BEGIN;

-- 1. Backfill vacant Sports row for every existing nation
INSERT INTO ministries (nation_id, ministry_key, ministry_name, is_active)
SELECT n.id, 'sports', 'Ministry of Sports', true
FROM nations n
WHERE NOT EXISTS (
    SELECT 1 FROM ministries m
    WHERE m.nation_id = n.id AND m.ministry_key = 'sports'
);

-- 2. Updated finalize_government_formation with sports in v_office_names
