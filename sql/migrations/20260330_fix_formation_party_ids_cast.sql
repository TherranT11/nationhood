-- Fix "cannot cast type uuid[] to jsonb" error in finalize_government_formation.
-- party_ids is a UUID[] column; the old code did party_ids::JSONB which is
-- invalid in PostgreSQL. Use ANY(party_ids) directly instead of the
-- roundabout jsonb_array_elements_text conversion.
...
BEGIN
  ...
  LOOP
    ...
    IF v_existing_ministry_id IS NOT NULL THEN
      UPDATE ministries SET
        ...
        minister_approval = 50, -- all new ministers start at 50% on coalition formation
  ...
  -- party_ids is UUID[] — use ANY() directly instead of casting to jsonb
  SELECT jsonb_agg(
    jsonb_build_object(
      'party_id', f.id,
