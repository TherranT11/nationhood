-- ════════════════════════════════════════════════════════════════
-- create_unit — emit a dispatch event when a unit is created.
--
--   "The {faction_name} has created the {unit name}."
--   e.g. "The Army of Avelia has created the 17th Infantry Division."
--
-- faction_name already encodes "{branch} of {nation}" (generated at
-- join time — same convention the COS report / resign dispatch rely
-- on), so it is used alone, NOT "{faction} of {nation}" (that double-
-- printed "Avelia of Avelia" in an earlier bug). category 'government'
-- + trigger_key 'military_unit_created' so the Dispatches feed buckets
-- it into the Military tab (dispatchBucketForRow matches 'military_*').
--
-- Authoritative + atomic: emitted INSIDE the RPC, after the army_units
-- INSERT and in the same transaction (mirrors resign_military_faction
-- / 20270123) — no event on a failed/guarded call, never a duplicate.
--
-- CREATE OR REPLACE, same signature as 20270125 (callers/grants
-- unaffected). Body is byte-for-byte 20270125 with ONLY the event_log
-- INSERT added between the unit RETURNING and the success RETURN.
-- Ordered after 20270125 (party_funds) / 20270121 (table + RPC).
-- ════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    -- Dispatch: "The {faction_name} has created the {unit name}."
    -- faction_name alone (already "{branch} of {nation}"). Same shape
    -- as the resign dispatch (20270123) → Military tab via trigger_key.
    INSERT INTO event_log (
        ...
        p_faction_id,
        v_name,
        format('The %s has created the %s.',
               COALESCE(NULLIF(btrim(v_fac.faction_name), ''), 'army'), v_name),
        'government',
        'military_unit_created',
        COALESCE(v_tick, 0)
    );
