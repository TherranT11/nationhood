-- ════════════════════════════════════════════════════════════════════
-- ENTREPRENEUR STOCK SYSTEM — corp_trade: flat-fill pricing
-- ════════════════════════════════════════════════════════════════════
-- Supersedes the per-share ladder in 20270178. Player-facing rule change:
-- a trade fills the WHOLE quantity at the CURRENT price, then the price
-- moves by 1 percentage point per share AFTER the fill — the move is driven
-- by the trade, not walked share-by-share within it.
--
--   BUY  N : cost     = P · N            (all N at the current price)
--            new_price = P · (1 + 0.01·N)
--   SELL N : proceeds = P · N
--            new_price = P · (1 − 0.01·N)
--
-- Was (20270178, the ladder): cost = P·N·(1 + 0.01·(N−1)/2) — share #k
-- priced at P·(1+0.01·(k−1)). The new_price formula is UNCHANGED; only the
-- amount the trader pays/receives changes (flat instead of laddered).
--
-- Everything else — treasury-favouring rounding (ceil buy / floor sell),
-- the fixed-supply float, the lock order, the SELL price_floor guard, and
-- the source-attribution session vars — is byte-identical to 20270178.
-- Same signature → body-only CREATE OR REPLACE.
...
COMMENT ON FUNCTION corp_trade(uuid, text, int) IS
    'Flat-fill ±1%-per-share curve (20270199). The whole quantity fills at the current price (cost/proceeds = P·N); the price then moves 1pp per share (new_price = P·(1 ± 0.01·N)). Rounding favours treasury (ceil buy / floor sell). Fixed-supply float. SELL rejected (price_floor) if it would drop new_price ≤ 0 (N ≥ 100). Source-attribution session vars consumed by _trg_record_share_price_change.';
...
-- ── ROLLBACK ──
-- Re-apply 20270178's corp_trade body to restore the per-share ladder
-- pricing (cost = P·N·(1 ± 0.01·(N−1)/2)). new_price formula is identical
-- in both, so the swap is reversible at any time.
