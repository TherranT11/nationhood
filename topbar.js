...
<span class="tb-chip tb-bal" id="tbBal" title="National income (per year)" hidden>—</span>
...
// The nation's income — the per-year revenue figure (economy.budget), shown
// ±-coloured and suffixed "bn /yr" to read as a yearly flow, not a treasury stock.
// Internal: loadChrome sets it once per page; it only moves on a tick, not per action.
function setTopbarBudget(currency, budget){
  const el = document.getElementById('tbBal');
  if (!el) return;
  if (budget == null) { el.hidden = true; return; }
  const neg = budget < 0;
  el.textContent = (neg ? '−' : '') + (currency || '$') + Math.abs(budget) + 'bn /yr';
