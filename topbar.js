...
<div class="tb-gear">
  ...
  <div class="gearmenu" id="gearMenu" hidden>
    <div id="gearMain">
      <button class="gearmenu__item" id="handleBtn" type="button">Set handle</button>
...
export function mountTopbar(){
  ...
  // gear → Set handle: the player's out-of-character Forum identity (@handle).
  const handleBtn = document.getElementById('handleBtn');
  if (handleBtn) handleBtn.addEventListener('click', async function () {
    const h = prompt('Choose a handle — 3–20 letters, digits, or underscores. This is your out-of-character name in the Forum.');
    if (h == null) return;
    try {
      const { data, error } = await supabase.rpc('set_handle', { p_handle: h.trim() });
      if (error) throw error;
      alert('Handle set to @' + data);
    } catch (e) { alert(e.message || 'Couldn’t set that handle.'); }
  });
